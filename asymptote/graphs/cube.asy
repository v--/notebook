...
struct CubeGraph {
  ...
  void draw_vertices() {
    for (int x = 0; x <= segments; ++x) {
      for (int y = 0; y <= segments; ++y) {
        for (int z = 0; z <= segments; ++z) {
          draw_vertex(this.vertices[x][y][z]);
        }
      }
    }
  }
  ...
  void draw_edges() {
    for (int x = 0; x <= segments; ++x) {
      for (int y = 0; y <= segments; ++y) {
        for (int z = 1; z <= segments; ++z) {
          draw_edge(this.vertices[x][y][z - 1], this.vertices[x][y][z], dash=x < segments && y < segments);
        }
      }
    }
    ...
    for (int x = 0; x <= segments; ++x) {
      for (int y = 1; y <= segments; ++y) {
        for (int z = 0; z <= segments; ++z) {
          draw_edge(this.vertices[x][y - 1][z], this.vertices[x][y][z], dash=x < segments && z > 0);
        }
      }
    }
    ...
    for (int x = 1; x <= segments; ++x) {
      for (int y = 0; y <= segments; ++y) {
        for (int z = 0; z <= segments; ++z) {
          draw_edge(this.vertices[x - 1][y][z], this.vertices[x][y][z], dash=y < segments && z > 0);
        }
      }
    }
  }
