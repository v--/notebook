...
from ..signature import PROP_TYPED_SIGNATURE
...
def test_parsing_constants(term: str, expected: Variable) -> None:
    assert parse_term(PROP_TYPED_SIGNATURE, term) == expected
...
def test_rebuilding_term_with_constants(term: str) -> None:
    assert str(parse_term(PROP_TYPED_SIGNATURE, term)) == term
...
def test_rebuilding_schema(schema: str) -> None:
    assert str(parse_term_schema(PROP_TYPED_SIGNATURE, schema)) == schema
...
def test_rebuilding_type_schema(schema: str) -> None:
    assert str(parse_type_schema(PROP_TYPED_SIGNATURE, schema)) == schema


def test_parsing_type_schema_with_regular_parser() -> None:
    with pytest.raises(ParsingError) as excinfo:
        parse_type(PROP_TYPED_SIGNATURE, 'τ')
...
def test_parsing_type_assertion_missing_arrow() -> None:
    with pytest.raises(ParsingError) as excinfo:
        parse_type(PROP_TYPED_SIGNATURE, '(ι o)')
...
def test_parsing_type_assertion(assertion: str) -> None:
    ...
    expected = TypeAssertion(
        parse_term(PROP_TYPED_SIGNATURE, term),
        parse_type(PROP_TYPED_SIGNATURE, type_)
    )

    assert parse_type_assertion(PROP_TYPED_SIGNATURE, assertion) == expected


def test_parsing_type_assertion_missing_colon() -> None:
    with pytest.raises(ParsingError) as excinfo:
        parse_type_assertion(PROP_TYPED_SIGNATURE, 'x ι')
...
def test_rebuilding_type_rules(rule: str) -> None:
    assert str(parse_type_rule(PROP_TYPED_SIGNATURE, rule)) == rule


def test_parsing_discharge_schema_with_no_name() -> None:
    with pytest.raises(ParsingError) as excinfo:
        parse_type_rule(PROP_TYPED_SIGNATURE, '(R) [] x: ι ⫢ y: ι')
...
def test_parsing_discharge_schema_with_no_closing_bracket() -> None:
    with pytest.raises(ParsingError) as excinfo:
        parse_type_rule(PROP_TYPED_SIGNATURE, '(R) [x: ι y: ι ⫢ z: ι')
