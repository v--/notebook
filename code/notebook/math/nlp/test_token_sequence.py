...
def test_split_limit(quick_fox: TokenSequence) -> None:
    splits = [str(s) for s in quick_fox.split_by(lambda token: isinstance(token, Whitespace), limit=3)]
    assert splits == ['The', 'quick', 'brown fox jumps over the lazy dog']
