...
ARROW_INTRO_RULE_IMPLICIT = parse_typing_rule(EMPTY_SIGNATURE, '(→⁺) [x: α] M: β ⫢ (λx.M): (α → β)', TypingStyle.implicit)
ARROW_INTRO_RULE_EXPLICIT = parse_typing_rule(EMPTY_SIGNATURE, '(→⁺) [x: α] M: β ⫢ (λx:α.M): (α → β)', TypingStyle.explicit)
