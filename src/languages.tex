...
\begin{definition}\label{def:language}
  ...
  \Def{Suffixes} are defined analogously. We say that \( v \) is a \Def{subword} of \( w \) if there exists a prefix \( p \) and a suffix \( s \) such that \( w = pvs \). We define the partial order\Tinyref{def:poset} \( v \leq w \iff v \) is a subword of \( w \).
