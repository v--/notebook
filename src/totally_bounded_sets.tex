...
\begin{definition}\label{def:totally_bounded_set}
  The space $A \subseteq X$ is called \underLine{totally bounded} if any of the following equivalent conditions hold:
  ...
  \begin{defenum}
    ...
    \item\label{def:totally_bounded_set/fundamental_subsequences} Every sequence in $A$ admits a fundamental subsequence\Tinyref{def:fundamental_sequence}.
  \end{defenum}

  Totally bounded sets are sometimes called \underLine{precompact}\Tinyref{def:compact_set} because of~\cref{thm:metric_compact_iff_sequentially_compact}. This equivalence requires the metric space to be complete, however.
\end{definition}
\begin{proof}
