...
\begin{definition}\label{def:lipschitz_continuity}
  ...
  \begin{defenum}
    \DItem{def:lipschitz_continuity/holder} We say that \( f: X \to Y \) is \Def{H\"older continuous} at \( x \in X \) with constant \( L \geq 0 \) and exponent \( \alpha > 0 \) if
    \begin{equation*}
    ...
    \DItem{def:lipschitz_continuity/locally_holder} We say that \( f \) is \Def{locally H\"older continuous} if every point has a neighborhood where \( f \) is H\"older continuous with the same exponent, but possibly with with a different constant.

    \DItem{def:lipschitz_continuity/lipschitz} If \( \alpha = 1 \), we say that \( f \) is \Def{Lipschitz continuous}.

    \DItem{def:lipschitz_continuity/contraction} If \( X = Y \) and if \( f \) is Lipschitz with constant \( L < 1 \), we call \( f \) a \Def{contraction mapping}.

    \DItem{def:lipschitz_continuity/calm}\cite[53]{Dontchev2014} We say that \( f \) is \Def{calm} at \( x \) if it satisfies the Lipschitz condition with one of the points fixed:
    \begin{equation*}
...
\begin{theorem}[Banach's fixed point theorem]\label{thm:banach_fixed_point_theorem}\cite[exercise 4.3.J]{Engelking1989}
  A contraction mapping\Tinyref{def:lipschitz_continuity/contraction} in a complete metric space\Tinyref{def:complete_metric_space} has a unique fixed point\Tinyref{def:fixed_point}.
\end{theorem}
\begin{proof}
  ...
  Because of the continuity of \( f \)\Tinyref{thm:holder_map_is_uniformly_continuous},
  \begin{equation*}
