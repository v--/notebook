...
\begin{definition}\label{def:euclidean_transformation}
  The following bijections from \( \BbbR^n \) to \( \BbbR^n \) are collectively called \Def{Euclidean transformations} or \Def{rigid motions} in \( \BbbR^n \):
  ...
  \begin{DefEnum}
    ...
    \end{equation*}
    is called a \Def{translation} along the \Def{direction} \( v \).
    ...
    \end{equation*}
    is called a \Def{dilation} or \Def{scaling} by \( \lambda \).

    This easily generalizes to an arbitrary (left) \hyperref[def:magma]{module} \( (M, +, \cdot) \).

    \ILabel{def:euclidean_transformation/homothety} A composition of a \hyperref[def:euclidean_transformation/dilation]{dilation} with a \hyperref[def:euclidean_transformation/translation]{translation} is called a \Def{homothety}.
    ...
    \end{equation*}
    is called a \Def{rotation} by \( A \) around \( p \).

    \ILabel{def:euclidean_transformation/reflection} For any point \( p \in \BbbR^n \), the function
    \begin{equation*}
      \Op{ref}_p(x) \coloneqq 2p - x
    \end{equation*}
    is called a \Def{point reflection} or \Def{inversion} with respect to the point \( p \).
  \end{DefEnum}
\end{definition}
...
\begin{definition}\label{def:geometric_line}
  A particularly important \hyperref[def:hypersurface]{curve} is a \Def{line} in a vector space \( X \) over any field \( \BbbK \), which can be defined equivalently as
  ...
  \begin{DefEnum}
    ...
    \ILabel{def:geometric_line/parametric} If the field \( \BbbK \) is ordered (usually when \( \BbbK = \BbbR \)), we can define a line with \Def{directional vector} \( x \) and \Def{origin} \( a \) as the parametric curve
...
\begin{definition}\label{def:geometric_ray}
  If \( X \) is a vector space over \( \BbbK \in \{ \BbbR, \BbbC \} \), we define \Def{closed rays} with a vertex \( a \) as the parametric curves
  \begin{BreakableAlign*}
     & l^+: [0, \infty) \to X \\
     & l^+(t) = tx + a
  \end{BreakableAlign*}
...
\begin{definition}\label{def:geometric_cone}
  An open (resp. closed) \Def{cone} in a vector space over \( \BbbK \in \{ \BbbR, \BbbC \} \) is a union of open (resp. closed) \hyperref[def:geometric_ray]{rays} with a common vertex, called the \Def{vertex} of the cone.
\end{definition}
...
\begin{definition}\label{def:hyperplane}
  ...
  \begin{DefEnum}
    \ILabel{def:hyperplane/subspace} A \Def{linear hyperplane} is simply a subspace of \( X \) of \hyperref[def:vector_space_dimension]{codimension} one. As in \fullref{def:geometric_line/subspace}, we define a \Def{affine hyperplane} to be a \hyperref[def:euclidean_transformation/translation]{translation} of a linear hyperplane.
...
\begin{definition}\label{def:half_space}
  Vector spaces over \( \BbbR \) have the concept of \Def{half-spaces}. Given a \hyperref[def:hyperplane]{hyperplane} \( H \) of the real vector space \( X \), defined by the affine functional \( l(x) = \Prod {x^*} x + a \), its closed half-spaces are defined as
  \begin{equation*}
...
\begin{definition}\label{def:polyhedron}
  A \Def{polyhedron} in a real vector space is an intersection of \hyperref[def:half_space]{half-spaces}.
\end{definition}

\begin{definition}\label{def:hyperplane_separation}
  Although \hyperref[def:hyperplane]{hyperplanes} are defined for vector spaces over an arbitrary field \( \BbbK \), we define \Def{hyperplane separation} only for \( \BbbK \in \{ \BbbR, \BbbC \} \) (see \fullref{rem:real_field_extensions}).

  We say that the sets \( A, B \subseteq X \) are \Def{separated} by the linear functional \( l \in X^* \) if there exists a real number \( c \in \BbbR \) such that
...
\begin{definition}\label{def:convex_set}\mbox{}
  \begin{DefEnum}
    \ILabel{def:convex_set/line_segment} Given two points \( x, y \in X \) in a Banach space \( X \), we define the \Def{line segment} between \( x \) and \( y \) as the parametric curve \( t \mapsto tx + (1-t)y, t \in [0, 1] \). The image
    \begin{equation*}
    ...
    \ILabel{def:convex_set/set} We call a set \Def{convex} if it coincides with its convex hull, that is, if it contains the line segment between any two of its points.
  \end{DefEnum}
\end{definition}
...
\begin{definition}\label{def:simplex}
  ...
  \begin{DefEnum}
    ...
    \ILabel{def:simplex/tetrahedron} A \( 3 \)-simplex is called a \Def{tetrahedron}.
  \end{DefEnum}
\end{definition}
...
\begin{definition}\label{def:k_cell}
  ...
  \begin{DefEnum}
    ...
    \ILabel{def:k_cell/rectangle} A \( 2 \)-cell is called a \Def{rectangle}. If a rectangle \( R \) is a product of two copies of the same interval, i.e. if \( R = [a, b]^2 \), we say that \( R \) is a \Def{square} with side \( b - a \).
    \ILabel{def:k_cell/parallelepiped} A \( 3 \)-cell is called a \Def{parallelepiped}. If \( R = [a, b]^3 \), we say that \( R \) is a \Def{cube} with side \( b - a \).
  \end{DefEnum}
\end{definition}
...
\begin{definition}\label{def:neighborhood_set_types}
  ...
  \begin{DefEnum}
    \ILabel{def:neighborhood_set_types/absorbing} \( A \) is \Def{absorbing} if \( \bigcup_{k=0}^\infty kA = X \).
    \ILabel{def:neighborhood_set_types/symmetric} \( A \) is \Def{symmetric} if \( -A = A \).
    \ILabel{def:neighborhood_set_types/balanced} \( A \) is \Def{balanced} if \( tA \subseteq A \) for any \( t \in [0, 1] \).
  \end{DefEnum}
\end{definition}
