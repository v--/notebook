...
\begin{definition}\label{def:half_space}
  Vector spaces over \( \BbbR \) have the concept of \term{half-spaces}. Given a \hyperref[def:hyperplane]{hyperplane} \( H \) of the real vector space \( X \), defined by the affine functional \( l(x) = \inner {x^*} x + a \), its closed half-spaces are defined as
  \begin{equation*}
