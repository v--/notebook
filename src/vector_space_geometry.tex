...
\begin{proposition}\label{thm:convex_set_properties}
  ...
  \begin{thmenum}
    \thmitem{thm:convex_set_properties/closed_under_combinations} A convex set is closed under convex \hyperref[def:linear_combination/convex]{combinations}.
    \thmitem{thm:convex_set_properties/cone_closed_under_combinations} A closed \hyperref[def:convex_set]{convex} \hyperref[def:geometric_cone]{cone} is closed under conic \hyperref[def:linear_combination/conic]{combinations}.
    \thmitem{thm:convex_set_properties/closed_under_intersections} Any intersection of convex sets is convex.
  \end{thmenum}
\end{proposition}
\begin{proof}
...
\begin{definition}\label{def:k_cell}
  A \( k \)-cell is a \hyperref[def:tuple_and_cartesian_product]{Cartesian product} of \( k \) nonempty \hyperref[def:partially_ordered_set_interval/closed]{closed intervals} of real numbers.
...
\begin{proposition}\label{thm:moment_curve}
  Consider \hyperref[def:topological_curve]{curve}
  \begin{equation*}
    \begin{aligned}
      &\gamma: \BbbR \to \BbbR^n \\
