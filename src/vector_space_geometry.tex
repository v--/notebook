...
\begin{definition}\label{rem:geometric_shape}
  ...
  \begin{thmenum}
    \thmitem{rem:geometric_shape/parametric} Parametric shape, defined in \fullref{def:parametric_shape}, are a topological construction.
    \thmitem{rem:geometric_shape/algebraic} Affine varieties, defined in \fullref{def:affine_algebraic_set}, are an algebraic construction.
    \thmitem{rem:geometric_shape/geometric} Manifolds, defined in \fullref{def:topological_manifold}, are a geometric construction.
  \end{thmenum}
...
\begin{definition}\label{rem:point}
  A \term{point} is a simple \hyperref[rem:geometric_shape]{geometric shape} comprising a singleton subset of a topological space. We use the convention \fullref{rem:singleton_sets} and, unless the distinction is important, we do not distinguish between singleton sets and their only element --- see \fullref{ex:def:simplex/point} for an example.

  Within vector spaces points are also called vectors, which is justified by \fullref{def:euclidean_plane_coordinate_system}. Despite treating them as equals in vector spaces, we informally distinguish between points and (free) vectors.
...
\begin{remark}\label{rem:collinear_complanar}
  ...
  \begin{thmenum}
    \thmitem{rem:collinear_complanar/collinear} We say that the set \( A \) of points is \term{collinear} (\enquote{on the same line}) if the dimension of its linear span \( \linspan(A) \) is at most \( 1 \).

    \thmitem{rem:collinear_complanar/complanar} Similarly, we say that the set \( A \) is \term{complanar} (\enquote{on the same plane}) if the dimension of its linear span \( \linspan(A) \) is at most \( 2 \).
  \end{thmenum}
\end{remark}
...
\begin{example}\label{ex:def:k_cell}
  ...
  \begin{thmenum}
    \thmitem{ex:def:k_cell/point} A \( 0 \)-cell is simply a \hyperref[rem:point]{point}.
    \thmitem{ex:def:k_cell/interval} A \( 1 \)-cell is simply a closed interval.
  \end{thmenum}
\end{example}
...
\begin{definition}\label{def:rigid_motion}\mimprovised
  ...
  \begin{thmenum}
    ...
    A \term{homothety} is a translation preceded by a \hyperref[rem:point]{dilation}. It is no longer a rigid motion in general.
...
\begin{definition}\label{def:geometric_line}\mimprovised
  \term{Lines} are particularly important and simple \hyperref[rem:geometric_shape]{curves}.
...
\begin{definition}\label{def:geometric_ray}\mimprovised
  Let \( X \) be a \hi{real} vector space. We define the \term{positive closed ray} with \term{origin} \( o \) and \term{direction} \( d \) as the \hyperref[def:parametric_curve]{curve}
  \begin{equation*}
    \begin{aligned}
       &l^+: [0, \infty) \to X \\
       &l^+(t) \coloneqq o + td.
...
\begin{definition}\label{def:geometric_cone}\mcite[20]{Clarke2013}
  An open (resp. closed) \term{cone} in a \hi{real} vector space is a union of open (resp. closed) \hyperref[def:geometric_ray]{rays} with a common vertex.
...
\begin{definition}\label{def:hyperplane}\mcite[41]{Clarke2013}
  Dually to \hyperref[def:geometric_line]{lines}, another particularly important \hyperref[rem:geometric_shape]{shape} is a \term{hyperplane}.
...
\begin{definition}\label{def:affine_dependence}\mimprovised
  We say that the vectors in the set \( B \) are \term{affinely independent} if any of the following conditions holds:

  \begin{thmenum}
    \thmitem{def:affine_dependence/hull} The \hyperref[def:affine_hull]{affine hull} of \( B \) strictly contains the affine hulls of any subset of \( B \).

    \thmitem{def:affine_dependence/combinations} If the linear combination
    \begin{equation*}
    ...
    Relaxing the last condition, we obtain the condition for linear dependence from \fullref{def:linear_dependence/direct}.

    \thmitem{def:affine_dependence/linear} If for any vector \( b_0 \) from \( B \), the set
    \begin{equation*}
...
\begin{proof}
  ...
  \ImplicationSubProof{def:affine_dependence/hull}{def:affine_dependence/combinations} Suppose that the affine hull of \( B \) strictly contains the affine hull of any subset.
  ...
  \ImplicationSubProof{def:affine_dependence/combinations}{def:affine_dependence/linear} Suppose that if a linear combination of vectors from \( B \) sums to the zero vector and if its coefficients sum to zero, then all coefficients are zero.
  ...
  \ImplicationSubProof{def:affine_dependence/linear}{def:affine_dependence/hull} Let \( A \) be a strict subset of \( B \). Fix some vector \( b_0 \) from \( B \setminus A \) and suppose that the set
  \begin{equation*}
    \set{ b - b_0 \given b \in B \T{and} b \neq b_0 }
  \end{equation*}
  is linearly independent.
  ...
  This in turn implies that \( b_0 \) is the zero vector. Therefore, \( B \) itself is linearly independent. But \( B \) cannot be linearly independent since it contains the zero vector as a consequence of \fullref{thm:def:linear_dependence/zero}.
