...
\begin{definition}\label{def:zero_locus}\mimprovised
  The \term{zero locus} or \term{set of zeros} of a function \( f: S \to V \) from some set \( S \) into a vector space \( V \) (or, more generally, a monoid \( V \)) is the \hyperref[thm:def:function/preimage]{preimage}
  \begin{equation*}
    f^{-1}(0_V) = \set{ x \in X \given f(x) = 0_V }.
  \end{equation*}
\end{definition}
...
\begin{definition}\label{def:geometric_cone}\mimprovised
  ...
  Despite the name a cone is not necessarily a \hyperref[def:conic_hull]{conic set}, i.e. it may be strictly contained in its conic hull. For example, the union of two rays is a cone, but it may not contain the sum of two arbitrary vectors.
\end{definition}

\begin{definition}\label{def:hyperplane}\mimprovised
...
\begin{definition}\label{def:half_space}\mimprovised
