...
\begin{definition}\label{def:zero_locus}
  ...
  See also \fullref{def:pointed_set_kernel}, \fullref{def:semiring_kernel} and \fullref{thm:semimodule_kernels}.
\end{definition}
...
\begin{definition}\label{def:affine_operator}
  Let \( M \) and \( N \) be two left \( R \)-modules. We say that the function \( f: M \to N \) is \term{affine} if it is a translation of a \hyperref[def:semimodule/homomorphism]{linear function}, that is, if there exists a linear function \( l: M \to N \) and a constant \( a \in N \) such that \( f(x) = l(x) + a \).
\end{definition}
...
\begin{proposition}\label{thm:def:convex_set}
  ...
  \begin{thmenum}
    \thmitem{thm:def:convex_set/closed_under_combinations} A convex set is closed under convex \hyperref[rem:linear_combinations/convex]{combinations}.
    \thmitem{thm:def:convex_set/cone_closed_under_combinations} A closed \hyperref[def:convex_set]{convex} \hyperref[def:geometric_cone]{cone} is closed under conic \hyperref[rem:linear_combinations/conic]{combinations}.
...
\begin{definition}\label{def:collinear_complanar}
  The geometric version of \hyperref[def:semimodule_basis/independent]{linear independence} has two special names: we say that the set \( A \subseteq X \) of any vector space \( X \) is \term{collinear} (on the same line) if \( \dim(\linspan(A)) \leq 1 \) and \term{complanar} (on the same plane) if \( \dim(\linspan(A)) \leq 2 \).
\end{definition}
