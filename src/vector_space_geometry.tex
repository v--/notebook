...
\begin{remark}\label{remark:vector_space_set_operations}
  ...
  If we use the convention in \fullref{remark:singleton_sets}, this turns sets in vector spaces into an ill-defined algebraic structure, but it is useful when no metric is available, e.g. in topological vector \hyperref[def:topological_vector_space]{spaces} or set-valued analysis.
\end{remark}
...
\begin{definition}\label{def:geometric_line}
  ...
  \begin{defenum}
    ...
    \DItem{def:geometric_line/algebraic} An algebraic \hyperref[def:affine_variety/algebraic_curve]{curve} in \( X \) given by a polynomial of degree one.
