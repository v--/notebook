...
\begin{remark}\label{remark:real_field_extensions}
  When speaking about vector spaces, we usually restrict ourselves to vector spaces over \( \R \) or, at most, \( \C \). This restriction may seem arbitrary, however important concepts like rays\Tinyref{def:geometric_ray} or convexity\Tinyref{def:convex_set} requires the field to be an extension of \( \R \) and it just so happens that, by \fullref{thm:fundamental_theorem_of_algebra} and \cref{thm:no_finite_extensions_of_closed_fields}, the only finite extension fields\Tinyref{def:field_extension} of \( \R \) are \( \R \) and \( \C \). It is technically possible to work with infinite extension fields, however in practice vector spaces over \( \C \) are isoteric enough. A benefit of considering only \( \C \) is given in \cref{remark:linear_functionals_over_c}.
\end{remark}
...
\begin{definition}\label{def:geometric_line}
  A particularly important curve\Tinyref{def:hypersurface} is a \Def{line} in a vector space \( X \) over any field \( \K \), which can be defined equivalently as
  ...
  \begin{defenum}
    ...
    \DItem{def:geometric_line/parametric} If the field \( \K \) is ordered (usually when \( \K = \R \)), we can define a line with \Def{directional vector} \( x \) and \Def{origin} \( a \) as the parametric curve
    \begin{align*}
      &l: \K \to X \\
      &l(t) = tx + a.
    \end{align*}
...
\begin{definition}\label{def:geometric_ray}
  If \( X \) is a vector space over \( \K \in \{ \R, \C \} \), we define \Def{closed rays} with a vertex \( a \) as the parametric curves
  \begin{align*}
    &l^+: [0, \infty) \to X \\
    &l^+(t) = tx + a
  \end{align*}
...
\begin{definition}\label{def:geometric_cone}
  An open (resp. closed) \Def{cone} in a vector space over \( \K \in \{ \R, \C \} \) is a union of open (resp. closed) rays\Tinyref{def:geometric_ray} with a common vertex, called the \Def{vertex} of the cone.
\end{definition}
...
\begin{example}\label{ex:hyperplanes}
  Affine hyperplanes\Tinyref{def:hyperplane} in \( \R^2 \) are lines\Tinyref{def:geometric_line} and affine hyperplanes in \( \R^3 \) are planes.

  Linear hyperplanes\Tinyref{def:hyperplane} in \( \R^2 \) are the lines passing through the origin \( (0, 0) \) and linear hyperplanes in \( \R^3 \) are the planes incident to \( (0, 0, 0) \).
\end{example}

\begin{definition}\label{def:half_space}
  Vector spaces over \( \R \) have the concept of \Def{half-spaces}. Given a hyperplane\Tinyref{def:hyperplane} \( H \) of the real vector space \( X \), defined by the affine functional \( l(x) = \Prod {x^*} x + a \), its closed half-spaces are defined as
  \begin{equation*}
...
\begin{definition}\label{def:hyperplane_separation}
  Although hyperplanes\Tinyref{def:hyperplane} are defined for vector spaces over an arbitrary field \( \K \), we define \Def{hyperplane separation} only for \( \K \in \{ \R, \C \} \) (see \cref{remark:real_field_extensions}).

  We say that the sets \( A, B \subseteq X \) are \Def{separated} by the linear functional \( l \in X^* \) if there exists a real number \( c \in \R \) such that
  ...
  We can regard \( l \) as a hyperplane as in \cref{def:hyperplane/kernel}, which justifies the terminology \enquote{hyperplane separation}. It is more correct, however, especially if \( \K = \R \), to say that they are separated by the affine hyperplane \( l(x) + c \).

  If \( \K = \R \) \cref{def:hyperplane_separation/normal} is equivalent to requiring that \( A \) is contained in an open half-space\Tinyref{def:half_space} relative to \( l(x) + c \) and that \( B \) is contained in the complementing closed half-space (or vice-versa). \Cref{def:hyperplane_separation/strong} then states that both \( A \) and \( B \) are contained in opposite open half-spaces.
\end{definition}
