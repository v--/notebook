...
\begin{remark}\label{remark:vector_space_set_operations}
  ...
  If we use the convention in \fullref{remark:singleton_sets}, this turns sets in vector spaces into an ill-defined algebraic structure, but it is useful when no metric is available, e.g. in topological vector spaces\Tinyref{def:topological_vector_space} or set-valued analysis.
\end{remark}
...
\begin{definition}\label{def:hyperplane}
  ...
  \begin{defenum}
    \DItem{def:hyperplane/subspace} A \Def{linear hyperplane} is simply a subspace of \( X \) of codimension\Tinyref{def:vector_space_dimension} one. As in \fullref{def:geometric_line/subspace}, we define a \Def{affine hyperplane} to be a translation of a linear hyperplane.

    \DItem{def:hyperplane/kernel} Linear hyperplanes (as defined in \fullref{def:hyperplane/subspace}) are simply zero loci\Tinyref{def:zero_locus} (kernels\Tinyref{def:semiring_kernel}) of linear functionals\Tinyref{def:linear_operator}. and affine hyperplanes are zero loci of affine functionals\Tinyref{def:affine_operator}.
  \end{defenum}
\end{definition}
...
\begin{definition}\label{def:hyperplane_separation}
  Although hyperplanes\Tinyref{def:hyperplane} are defined for vector spaces over an arbitrary field \( \BK \), we define \Def{hyperplane separation} only for \( \BK \in \{ \BR, \BC \} \) (see \fullref{remark:real_field_extensions}).
