...
\begin{definition}\label{def:convex_functions}
  A function \( f: D \to \R \) is called \Def{convex} if any of the following equivalent conditions hold:
  ...
  If \( -g \) is convex for some function \( g: D \to \R \), we call \( g \) \Def{concave}.
...
\begin{definition}\label{def:affine_functions_concave_and_convex}
  Affine functions\Tinyref{def:affine_operator} \( f: X \to \R \) are simultaneously convex and concave.
\end{definition}
