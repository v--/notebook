...
Let \( X \) be a Hausdorff topological vector space\Tinyref{def:topological_vector_space} and \( D \) be a convex\Tinyref{def:convex_set} subset of \( X \).

\begin{definition}\label{def:convex_functions}
  A function \( f: D \to \BR \) is called \Def{convex} if any of the following equivalent conditions hold:

  \begin{defenum}
    \DItem{def:convex_functions/ineq} For any two points \( x, y \in D \) and any \( t \in [0, 1] \) we have
    \begin{equation*}
      f(tx + (1-t)y) \leq tf(x) + (1-t)f(y).
    \end{equation*}

    \DItem{def:convex_functions/epi} The epigraph\Tinyref{def:function_graph}
    \begin{equation*}
  ...
  If \( -g \) is convex for some function \( g: D \to \BR \), we call \( g \) \Def{concave}.
...
\begin{proof}
  ...
  \begin{description}
    \Implies[def:convex_functions/ineq][def:convex_functions/epi] Let \( \Epi f \) be a convex set. Obviously \( (x, f(x)) \in D \) and \( (y, f(y)) \in D \). By the convexity of \( \Epi f \), we have
    \begin{equation*}
      f(tx + (1-t)y) \leq tf(x) + (1-t)f(y).
    \end{equation*}
    ...
    \Implies[def:convex_functions/epi][def:convex_functions/ineq] Let \( f \) be convex. Let \( a \geq f(x) \) and \( b \geq f(y) \) so that \( (x, a) \in \Epi f \) and \( (y, b) \in \Epi f \). Hence
    \begin{equation*}
...
\begin{definition}\label{def:affine_functions_concave_and_convex}
  Affine functions\Tinyref{def:affine_operator} \( f: X \to \BR \) are simultaneously convex and concave.
\end{definition}
...
\begin{proposition}\label{thm:convex_one_sided_derivatives_sublinear}
  For every point \( x \in X \) and every direction \( h \in S_X \) the one-sided derivative \( f_+'(x)(h) \) is a sublinear functional\Tinyref{def:sublinear_functional}.
\end{proposition}
\begin{proof}\mbox{} 
  \begin{description}
    \RItem{def:sublinear_functional/subadditivity} It follows directly from
    \begin{align*}
    ...
    \RItem{def:sublinear_functional/positive_homogeneity} For \( \lambda > 0 \) the equality \( f_+'(x)(\lambda h) = \lambda f_+'(x)(h) \) follows from
    \begin{align*}
...
\end{theorem}
\begin{proof}
  Let \( D \subseteq \BR^n \) be an open and convex set and let \( f: D \to \BR \) be continuous and convex. Then \( f_+'(x) \) exists everywhere by \fullref{thm:convex_one_sided_derivatives_exist} and is a subdifferential functional by \fullref{thm:convex_one_sided_derivatives_sublinear}.
...
\begin{proof}
  ...
  Suppose\LEM that \( f \) is not Frechet differentiable at \( x \). Inverting the condition in \fullref{def:differentiability/frechet}, we obtain that there exist \( \varepsilon > 0 \) and a sequence \( \{ h_n \}_n \subseteq B(x, \delta) \setminus \{ 0 \} \) such that \( \Norm{h_n} \to 0 \) and yet for all \( n \in \BZ_{>0} \),
