...
\begin{proof}
  ...
  \begin{RefList}
    \IImplies{def:convex_functions/ineq}{def:convex_functions/epi} Let \( \Epi f \) be a convex set. Obviously \( (x, f(x)) \in D \) and \( (y, f(y)) \in D \). By the convexity of \( \Epi f \), we have
    \begin{equation*}
      f(tx + (1-t)y) \leq tf(x) + (1-t)f(y).
    \end{equation*}
    ...
    \IImplies{def:convex_functions/epi}{def:convex_functions/ineq} Let \( f \) be convex. Let \( a \geq f(x) \) and \( b \geq f(y) \) so that \( (x, a) \in \Epi f \) and \( (y, b) \in \Epi f \). Hence
    \begin{equation*}
...
\begin{proof}\mbox{} 
  \begin{RefList}
    \IRef{def:sublinear_functional/subadditivity} It follows directly from
    \begin{align*}
    ...
    \IRef{def:sublinear_functional/positive_homogeneity} For \( \lambda > 0 \) the equality \( f_+'(x)(\lambda h) = \lambda f_+'(x)(h) \) follows from
    \begin{align*}
