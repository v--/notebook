...
\begin{example}\label{ex:def:lexicographic_order}
  ...
  A slightly more relevant example for mathematics is the ordering \( AB < AD < BC < CD \) of the names of the edges of a rectangle. The reverse lexicographic ordering is \( AB < BC < AD < CD \). For the sides of a \hyperref[def:triangle]{triangle} we have \( AB < AC < BC \) for both orderings.

  \Fullref{thm:ordinal_addition_disjoin_union} and \fullref{thm:ordinal_multiplication_disjoin_union} contain more interesting applications of lexicographic orders.
\end{example}
