...
\begin{definition}\label{def:partially_ordered_set}
  ...
  \begin{thmenum}[resume=def:partially_ordered_set]
    ...
    \thmitem{def:partially_ordered_set/category} We denote the \hyperref[def:category_of_small_first_order_models]{category of models} by \( \cat{Pos} \). It is a full subcategory of the \hyperref[def:preordered_set/category]{category of preordered sets}.
...
\begin{proposition}\label{thm:partial_order_category_correspondence}
  To every \hyperref[def:partially_ordered_set]{partially ordered set} there corresponds exactly one \hyperref[def:category_size]{small} \hyperref[def:thin_category]{thin} \hyperref[def:skeletal_category]{skeletal} category.
...
\begin{example}\label{ex:def:lexicographic_order}
  ...
  The edges of the graph in \fullref{ex:def:graph/simple_directed} are numbered in lexicographic order, which also happens to be the reverse lexicographic order.
