...
\end{definition}
\begin{proof}
  \SubProofImplication{def:noetherian_module/ascending_chain}{def:noetherian_module/finite_basis} We can construct a basis as follows: choose\AOC any element \( x_1 \) of \( M \). Next, choose\AOC an element \( x_2 \in M \setminus \Bracket {x_1} \), then \( x_3 \in M \setminus \Bracket {x_1, x_2} \) and so on.

  This process must stop after finitely many steps because we obtain the strict chain
  \begin{equation*}
    \Bracket {x_1} \subsetneq \Bracket {x_1, x_2} \cdots
  \end{equation*}
  of submodules.
