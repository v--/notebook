...
\begin{example}\label{ex:topological_space_is_heyting_algebra}
  ...
  \begin{itemize}
    ...
    \item The \hyperref[def:partially_ordered_set_extremal_points/top_and_bottom]{top} is the entire domain \( L \).
...
\begin{theorem}[De Morgan's laws]\label{thm:de_morgans_laws}
  If \( L \) is a \hyperref[def:boolean_algebra]{Boolean algebra}, the following hold for any finite \hyperref[def:cartesian_product/indexed_family]{family} \( \{ x_k \}_{k \in \mscrK} \subseteq X \):
  \begin{align}
