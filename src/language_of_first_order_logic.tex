...
\begin{definition}\label{def:first_order_language}\mcite[187]{OpenLogic20201202}
  ...
  \begin{description}
    ...
    \begin{defenum}[series=def:first_order_language]
      ...
      \ilabel{def:first_order_language/var} A nonempty at most \hyperref[rem:cardinals/countable]{countable} alphabet of variables \( \bop{Var} \), usually denoted by \( \xi_1, \xi_2, \ldots \) or \( \xi, \eta, \zeta \).
...
\begin{definition}\label{def:first_order_syntax}
  ...
  \begin{defenum}
    ...
    \begin{bnf*}
      \bnfprod{variable}        {v \in \bop{Var}} \\
      \bnfprod{connective}      {\circ \in \Sigma} \\
      \bnfprod{quantifier}      {\bnfts{\( \forall \)} \bnfor \bnfts{\( \exists \)}} \\
      \bnfprod{unary function}  {f \in \bop{Fun}, \#f = 1} \\
      \bnfmore                  {\vdots} \\
      \bnfprod{n-ary function}  {f \in \bop{Fun}, \#f = n \text{ (separate rule for each \( n \))}} \\
      \bnfmore                  {\vdots} \\
      \bnfprod{unary predicate} {p \in \bop{Pred}, \#p = 1} \\
      \bnfmore                  {\vdots} \\
      \bnfprod{n-ary predicate} {p \in \bop{Pred}, \#p = n \text{ (separate rule for each \( n \))}} \\
      \bnfmore                  {\vdots} \\
    ...
    \ilabel{def:first_order_syntax/term}\mcite[189]{OpenLogic20201202}The set \( \mscrT \) of \term{terms} is the language \hyperref[def:grammar_derivation/grammar_language]{generated} by this grammar schema with \( \bnfpn{term} \) as a starting rule.
    ...
    \begin{equation}\label{eq:def:first_order_syntax/term_variables}
      \bop{Var}(\tau) \coloneqq \begin{cases}
        \xi,                                                    &\tau = \xi \in \bop{Var},        \\
        \bop{Var}(\tau_1) \cup \ldots \cup \bop{Var}(\tau_n), &\tau = f(\tau_1, \ldots, \tau_n).
      \end{cases}
    \end{equation}

    As in \fullref{def:propositional_syntax/variables}, \( \bop{Var} \) is ordered by the position of the first occurrence of a variable.

    \ilabel{def:first_order_syntax/ground_term} A term \( \tau \) is called a \term{closed term} if \( \bop{Var}(\tau) = \varnothing \).
    ...
    \begin{equation}\label{eq:def:first_order_syntax/formula_free_variables}
      \bop{Free}(\varphi) \coloneqq \begin{cases}
        \varnothing,                                            &\varphi \in \set{ \top, \bot } \\
        \bop{Var}(\tau_1) \cup \ldots \cup \bop{Var}(\tau_n), &\varphi = p(\tau_1, \ldots, \tau_n) \\
        \bop{Var}(\tau_1) \cup \bop{Var}(\tau_2),             &\varphi = \tau_1 \doteq \tau_2, \\
        \bop{Free}(\psi),                                      &\varphi = \neg \psi, \\
        \bop{Free}(\psi_1) \cup \bop{Free}(\psi_2),           &\varphi = \psi_1 \circ \psi_2, \circ \in \Sigma, \\
        \bop{Free}(\psi) \setminus \set{ \xi },                &\varphi = \quantifier{Q}{\xi} \psi, Q \in \set{ \forall, \exists }
      \end{cases}
    \end{equation}

    \ilabel{def:first_order_syntax/ground_formula} A formula \( \varphi \) is called a \term{closed formula} if \( \bop{Free}(\varphi) = \varnothing \).
    ...
    \begin{equation}\label{eq:def:first_order_syntax/formula_bound_variables}
      \bop{Bound}(\varphi) \coloneqq \begin{cases}
        \varnothing,                                    &\varphi \text{ is atomic}, \\
        \bop{Bound}(\psi),                             &\varphi = \neg \psi, \\
        \bop{Bound}(\psi_1) \cup \bop{Bound}(\psi_2), &\varphi = \psi_1 \circ \psi_2, \circ \in \Sigma, \\
        \bop{Bound}(\psi) \cup \set{ \xi },            &\varphi = \quantifier{Q}{\xi} \psi, Q \in \set{ \forall, \exists }.
      \end{cases}
    \end{equation}
    ...
    \begin{equation}\label{eq:def:first_order_syntax/formula_variables}
      \bop{Var}(\varphi) \coloneqq \bop{Free}(\varphi) \cup \bop{Bound}(\varphi).
    \end{equation}
  \end{defenum}
\end{definition}
...
\begin{definition}\label{def:first_order_valuation}
  ...
  \begin{defenum}
    \ilabel{def:first_order_valuation/variable_assignment} A \term{variable assignment} for the variables of \( \mscrL \) is any function \( v: \bop{Var} \to \mscrX \) (loosely similar to \hyperref[def:propositional_valuation/interpretation]{propositional interpretations}).
...
\begin{definition}\label{def:first_order_substitution}
  ...
  \begin{defenum}
    \ilabel{def:first_order_substitution/predicate} Let \( \varphi \) be a \hyperref[def:propositional_syntax/formula]{propositional formula} with variables \( \bop{Var}(\varphi) = \set{ P_1, \ldots, P_n } \). For brevity, denote \( V \coloneqq \bop{Var}(\varphi) \). Let \( \Theta = \set{ \theta_1, \ldots, \theta_n } \) be a set of \hyperref[def:first_order_syntax/formula]{first order formulas}.
    ...
    \begin{align}
      &\varphi                                                                    &&\T{if} \xi \in \bop{Var}(\kappa), \label{eq:def:first_order_substitution/term_in_formula/quantifiers/trivial} \\
      &\quantifier{Q}{\xi} \p[\Big]{\psi[\kappa \mapsto \mu]}                    &&\T{if} \xi \not\in \bop{Var}(\kappa) \cup \bop{Var}(\mu), \label{eq:def:first_order_substitution/term_in_formula/quantifiers/direct} \\
      &\quantifier{Q}{\eta} \p[\Big]{\psi[\xi \mapsto \eta][\kappa \mapsto \mu]} &&\T{if} \xi \not\in \bop{Var}(\kappa) \T{and} \xi \in \bop{Var}(\mu) \T{and} \label{eq:def:first_order_substitution/term_in_formula/quantifiers/renaming} \\
                                                                                  &&&\phantom{\T{if}} \eta \not\in \bop{Var}(\kappa) \cup \bop{Var}(\mu) \cup \bop{Var}(\psi). \nonumber
    \end{align}
    ...
    The rule \eqref{eq:def:first_order_substitution/term_in_formula/quantifiers/trivial} may seem redundant but when doing inductive proofs (e.g. the proof of \fullref{thm:swapping_assignment_compatibility}), we usually need to separately consider the cases where \( \xi \in \bop{Var}(\kappa) \) and \( \xi \not\in \bop{Var}(\kappa) \setminus \bop{Var}(\mu) \) and the rule \eqref{eq:def:first_order_substitution/term_in_formula/quantifiers/direct} being trivial simplifies the proofs.
...
\begin{remark}\label{rem:first_order_substitution_renaming_justification}
  ...
  \begin{remenum}
    ...
    \begin{equation*}
      \p[\Big]{ \qforall{\eta} p(\xi, \eta) }[\xi \mapsto \eta] = \qforall{\eta} p(\eta, \eta)
    \end{equation*}
    by instead giving
    \begin{equation*}
      \p[\Big]{ \qforall{\eta} p(\xi, \eta) }[\xi \mapsto \eta] = \qforall{\zeta} p(\eta, \zeta).
    \end{equation*}
    ...
    \begin{equation*}
      \p[\Big]{ \qforall{\xi} \qforall{\eta} p(\xi, \eta) }[\xi \mapsto \eta] = \qforall{\xi} \qforall{\eta} p(\eta, \eta)
    \end{equation*}
    by instead giving
    \begin{equation*}
      \p[\Big]{ \qforall{\xi} \qforall{\eta} p(\xi, \eta) }[\xi \mapsto \eta] = \qforall{\zeta} \qforall{\sigma} p(\zeta, \sigma).
    \end{equation*}
  \end{remenum}
\end{remark}
...
\begin{remark}\label{rem:first_order_substitution_parentheses}
  ...
  \begin{equation*}
    \p[\Big]{ \qforall{\xi} p(\xi, \eta) } [\eta \mapsto \zeta]
  \end{equation*}
  or
  \begin{equation*}
    \qforall{\xi} \p[\Big]{ p(\xi, \eta)[\eta \mapsto \zeta] }.
  \end{equation*}
...
\begin{example}\label{ex:first_order_substitution}
  ...
  \begin{exenum}
    ...
    \begin{align*}
      &{}\phantom{=}{}
      \p[\Big]{\qforall{\xi} p(\xi, \eta)}[\xi \mapsto \eta]
    ...
    \begin{align*}
      &{}\phantom{=}{}
      \p[\Big]{\qforall{\xi} p(\xi, \eta)}[\eta \mapsto \xi]
      \overset {\eqref{eq:def:first_order_substitution/term_in_formula/quantifiers/renaming}} = \\ &=
      \qforall{\zeta} \p[\Big]{p(\xi, \eta)[\xi \mapsto \zeta][\eta \mapsto \xi]}
      \overset {\eqref{eq:def:first_order_substitution/term_in_formula}} = \\ &=
    ...
    \begin{align*}
      &{}\phantom{=}{}
      \p[\Big]{\qforall{\xi} p(\xi, \eta)}[\eta \mapsto f(\xi)]
      \overset {\eqref{eq:def:first_order_substitution/term_in_formula/quantifiers/renaming}} = \\ &=
      \qforall{\zeta} \p[\Big]{p(\xi, \eta)[\xi \mapsto \zeta][\eta \mapsto f(\xi)]}
      \overset {\eqref{eq:def:first_order_substitution/term_in_formula}} = \\ &=
    ...
    \begin{align*}
      &{}\phantom{=}{}
      \p[\Big]{\qforall{\eta} \qforall{\xi} p(\xi, \eta)}[\xi \mapsto \eta]
      \overset {\eqref{eq:def:first_order_substitution/term_in_formula/quantifiers/renaming}} = \\ &=
      \qforall{\zeta} \p*{ \p[\Big]{ \qforall{\xi} p(\xi, \eta) }[\eta \mapsto \zeta][\xi \mapsto \eta]}
      \overset {\eqref{eq:def:first_order_substitution/term_in_formula}} = \\ &=
      \qforall{\zeta} \p*{ \p[\Big]{ \qforall{\xi} p(\xi, \zeta) }[\xi \mapsto \eta]}
      \overset {\eqref{eq:def:first_order_substitution/term_in_formula/quantifiers/trivial}} = \\ &=
    ...
    \begin{align*}
      &{}\phantom{=}{}
      \p[\Big]{ \qforall{\eta} (p(\xi, \eta) \vee \qforall{\xi} p(\xi, \eta)) }[\xi \mapsto \eta]
      \overset {\eqref{eq:def:first_order_substitution/term_in_formula/quantifiers/renaming}} = \\ &=
      \qforall{\zeta} \p*{ \p[\Big]{ p(\xi, \eta) \vee \qforall{\xi} p(\xi, \eta) }[\eta \mapsto \zeta][\xi \mapsto \eta] }
      \overset {\eqref{eq:def:first_order_substitution/term_in_formula}} = \\ &=
      \qforall{\zeta} \p*{ p(\eta, \zeta) \vee \p[\Big]{ \qforall{\xi} p(\xi, \eta) }[\eta \mapsto \zeta][\xi \mapsto \eta] }
      \overset {\eqref{eq:def:first_order_substitution/term_in_formula/quantifiers/direct}} = \\ &=
      \qforall{\zeta} \p*{ p(\eta, \zeta) \vee \p[\Big]{ \qforall{\xi} p(\xi, \zeta) }[\xi \mapsto \eta] }
      \overset {\eqref{eq:def:first_order_substitution/term_in_formula/quantifiers/trivial}} = \\ &=
    ...
    \begin{align*}
      &{}\phantom{=}{}
      \p*{\qforall{\xi} p(\xi, \eta, f(\eta))}[f(\eta) \mapsto g(\eta, \xi)]
      \overset {\eqref{eq:def:first_order_substitution/term_in_formula/quantifiers/renaming}} = \\ &=
      \qforall{\zeta} \p[\Big]{p(\xi, \eta, f(\eta))[\xi \mapsto \zeta][f(\eta) \mapsto g(\eta, \xi)]}
      \overset {\eqref{eq:def:first_order_substitution/term_in_formula}} = \\ &=
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \begin{balign*}
      \p[\Big]{ \tau[\xi \mapsto \eta] }\eval{v}
      &=
      \p[\Big]{ f(\tau_1[\xi \mapsto \eta], \ldots, \tau_n[\xi \mapsto \eta]) }\eval{v}
      = \\ &=
      I(f) \p[\Bigg]{ \p[\Big]{ \tau_1[\xi \mapsto \eta] }\eval{v}, \ldots, \p[\Big]{ \tau_1[\xi \mapsto \eta] }\eval{v} }
      \overset {\IndHyp} = \\ &=
      I(f) \p[\Big]{ \tau_1\eval{v_{\xi \mapsto \eta}}, \ldots, \tau_n\eval{v_{\xi \mapsto \eta}} }
      = \\ &=
      \p[\Big]{ f(\tau_1, \ldots, \tau_n) }\eval{v_{\xi \mapsto \eta}}
      = \\ &=
      \tau\eval{v_{\xi \mapsto \eta}}.
    \end{balign*}
  \end{itemize}
  ...
  \begin{itemize}
    ...
    \begin{equation*}
      \p[\Big]{ \tau_k[\xi \mapsto \eta] }\eval{v} = \tau_k\eval{v_{\xi \mapsto \eta}}
    \end{equation*}
    and thus
    \begin{equation*}
      I(p)\p*{ \p[\Big]{ \tau_1[\xi \mapsto \eta] }\eval{v}, \ldots, \p[\Big]{ \tau_1[\xi \mapsto \eta] }\eval{v} }
      \overset {\IndHyp} =
      I(p)\p[\Big]{ \tau_1\eval{v_{\xi \mapsto \eta}}, \ldots, \tau_n\eval{v_{\xi \mapsto \eta}} }.
    \end{equation*}
    ...
    \begin{balign*}
      \p[\Big]{ \varphi[\xi \mapsto \eta] }\eval{v}
      &=
      \p[\Big]{ p(\tau_1[\xi \mapsto \eta], \ldots, \tau_n[\xi \mapsto \eta]) }\eval{v}
      = \\ &=
      \p[\Big]{ p(\tau_1, \ldots, \tau_n) }\eval{v_{\xi \mapsto \eta}}
      = \\ &=
      \tau\eval{v_{\xi \mapsto \eta}}.
    \end{balign*}
    ...
    \begin{itemize}
      ...
      \begin{equation*}
        \varphi[\xi \mapsto \eta]
        =
        \qforall{\zeta} \p[\Big]{ \psi[\xi \mapsto \eta] }.
      \end{equation*}

      \begin{itemize}
        \item If \( \p[\Big]{\varphi[\xi \mapsto \eta]}\eval{v} = T \), by definition of \hyperref[def:first_order_valuation/formula_valuation]{quantifier formula valuation}, for any \( x \in \mscrX \) we have
        \begin{equation}\label{eq:thm:swapping_assignment_compatibility/formulas/true_modified_assignment}
          \p[\Bigg]{\qforall{\zeta} \p[\Big]{ \psi[\xi \mapsto \eta] }}\eval{v}
          =
          \p[\Big]{\psi[\xi \mapsto \eta]}\eval{v_{\zeta \mapsto x}}
          =
          T.
        \end{equation}
        ...
        \begin{equation*}
          \p[\Big]{ \psi[\xi \mapsto \eta] }\eval{v} = \psi\eval{v_{\xi \mapsto \eta}}
        \end{equation*}
        ...
        \begin{equation}\label{eq:thm:swapping_assignment_compatibility/formulas/ind_hyp_modified_assignment}
          \p[\Big]{ \psi[\xi \mapsto \eta] }\eval{v_{\zeta \mapsto x}} = \psi\eval{v_{\xi \mapsto \eta, \zeta \mapsto x}}.
        \end{equation}
        ...
        \item If \( \p[\Big]{\varphi[\xi \mapsto \eta]}\eval{v} = F \), then there exists \( x \in \mscrX \) such that
        \begin{equation*}
          \p[\Big]{\psi[\xi \mapsto \eta]}\eval{v_{\zeta \mapsto x}} = F.
        \end{equation*}
        ...
        \begin{equation*}
          \psi\eval{v_{\xi \mapsto \eta, \zeta \mapsto x}} = F
        \end{equation*}
        for the same \( x \).

        It follows that \( \varphi\eval{v_{\xi \mapsto \eta}} = F \), which proves the case.
      \end{itemize}

      \item This case holds vacuously because it is impossible to reach it - we have \( \eta \neq \zeta \) by the assumption \( \eta \not\in \bop{Var}(\varphi) \cup \bop{Var}(\mu) \).
    \end{itemize}
...
\begin{proposition}\label{thm:first_order_substitution_equivalence}
  ...
  \begin{propenum}
    ...
    \begin{equation}\label{eq:thm:first_order_substitution_equivalence/propositional}
      \p[\Big]{ \varphi[V \mapsto \Theta] }\eval{v} = \varphi \eval{J}.
    \end{equation}
    ...
    \begin{equation}\label{eq:thm:first_order_substitution_equivalence/term_in_term}
      \p[\Big]{ \tau[\kappa \mapsto \mu] }\eval{v} = \tau\eval{v}.
    \end{equation}
    ...
    \begin{equation}\label{eq:thm:first_order_substitution_equivalence/term_in_formula}
      \p[\Big]{ \varphi[\kappa \mapsto \mu] }\eval{v} = \varphi\eval{v}.
    \end{equation}
    ...
    \begin{equation}\label{eq:thm:first_order_substitution_equivalence/formula_in_formula}
      \p[\Big]{ \varphi[\rho \mapsto \theta] }\eval{v} = \varphi\eval{v}.
    \end{equation}
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \begin{equation*}
      \p[\Big]{ \psi[V \mapsto \Theta] }\eval{v}
      =
      \overline{\p[\Big]{ \psi[V \mapsto \Theta] }\eval{v}}
      \overset {\IndHyp} =
      \overline{\psi \eval{J}}
      =
      \varphi \eval{J}.
    ...
    \begin{equation*}
      \p[\Big]{ \psi[V \mapsto \Theta] }\eval{v}
      =
      \p[\Big]{ \psi_1[\rho \mapsto \theta] }\eval{v} \circ \p[\Big]{ \psi_2[\rho \mapsto \theta] }\eval{v}
      \overset {\IndHyp} =
  ...
  \begin{equation}\label{eq:thm:first_order_substitution_equivalence/term_in_formula/nested_induction}
    \p[\Bigg]{ \p*{ \qforall{\eta} \p[\Big]{ \psi[\xi \mapsto \eta] } }[\kappa \mapsto \mu] }\eval{v}
    =
    \p[\Bigg]{\qforall{\eta} \p[\Big]{ \psi[\xi \mapsto \eta] } }\eval{v},
  \end{equation}
  ...
  \begin{equation*}
    \p[\Big]{ \psi[\xi \mapsto \eta] }\eval{v_{\eta \mapsto x}} = \psi\eval{v_{\xi \mapsto \eta,\eta \mapsto x}}
  \end{equation*}
  ...
  \begin{equation}\label{eq:thm:first_order_substitution_equivalence/term_in_formula/modified_assignment}
    \p[\Bigg]{\qforall{\eta} \p[\Big]{ \psi[\xi \mapsto \eta] } }\eval{v}
    =
    \p[\Big]{\qforall{\xi} \psi }\eval{v_{\xi \mapsto \eta}}.
  \end{equation}
