...
\begin{definition}\label{def:preordered_set}\mcite[def. 2.14]{OpenLogicFull}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:preordered_set/opposite} We define the \term{opposite preordered set} of \( (P, \leq) \) as \( (P, \geq) \).
...
\begin{definition}\label{def:directed_set}\mcite[8]{Engelking1989}
  A \hyperref[def:preordered_set]{preordered set} \( P \) is called a \term{directed set} if every finite subset of \( P \) has an \hyperref[def:partially_ordered_set_extremal_points/upper_and_lower_bounds]{upper bound}, i.e. for all \( x, y \in P \) there must exist \( z \in P \) such that \( x \leq z \) and \( y \leq z \). We do not care how many upper bounds exist and how they are related, we simply need one upper bound to exist for every pair of elements of \( P \).
