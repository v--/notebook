...
\begin{definition}\label{def:preordered_set}\mcite[def. 2.14]{OpenLogicFull}
  ...
  \begin{thmenum}[series=def:preordered_set]
    ...
    \thmitem{def:partially_ordered_set/homomorphism} There are two types of \hyperref[def:first_order_homomorphism]{homomorphisms} between the preordered sets.

    A \term{nonstrict homomorphism} from \( \mscrP \) to \( \mscrQ \) is, explicitly, a function \( f: \mscrP \to \mscrQ \) such that
    \begin{equation}\label{eq:def:partially_ordered_set/homomorphism}
      x \leq_\mscrP y \T{implies} f(x) \leq_\mscrQ f(y)
    \end{equation}

    Nonstrict homomorphisms are used in the theory of \hyperref[subsec:partially_ordered_sets]{partially ordered sets}, in particular in \hyperref[subsec:lattices]{lattice theory}.

    A nonstrict homomorphism is \term{strict} if
    \begin{equation*}
      x \neq y \T{implies} f(x) \neq f(y),
    \end{equation*}
    ...
    Within the \hyperref[def:preordered_set/theory]{first-order theory of preordered sets}, the first-order homomorphisms are precisely the nonstrict homomorphisms. For partially ordered sets, however, a distinction is made depending on whether we use the language of strict or nonstrict orders.

    In general, we must show that an order isomorphism is a bijective order homomorphism whose inverse is also monotone. For strict homomorphisms on totally ordered sets, however, the second part follows automatically --- see \fullref{thm:totally_ordered_strict_isomorphisms}.

    Order homomorphisms are also called \term{monotone} or \term{order-preserving} maps.
