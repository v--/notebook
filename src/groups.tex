...
\begin{proposition}\label{def:group_properties}
  ...
  \begin{defenum}
    \item\label{def:group_properties/unique_identity} The identity $e$ is unique (also true for monoids).
...
\begin{proposition}\label{thm:group_homomorphism_properties}
  ...
  \begin{defenum}
    \item\label{def:group_homomorphism_properties/preserves_identity} $f$ preserves the identity, that is, $f(e_G) = e_K$.
    \item\label{def:group_homomorphism_properties/preserves_inverses} $f$ preserves inverses, that is, $f(a^{-1}) = f(a)^{-1}$ for any $a \in G$.
    \item\label{def:group_homomorphism_properties/preserves_subgroup} If $H$ is a subgroup of $G$, its image $f(H)$ is a subgroup of $K$.
