...
\begin{definition}\label{def:group_generators}\cite[176]{Knapp2016BAlg}
  A set \( S \subseteq G \) is said to \Def{generate} the group \( G \) if
  \begin{equation*}
    G = \Gen{ S } = \{ s_1 \cdots s_n \colon s_1, \ldots, s_n \in S \cup S^{-1} \},
  \end{equation*}
  where \( S^{-1} = \{ s^{-1} \colon s \in S \} \).

  If \( S \) is finite, then \( G \) is called \Def{finitely generated}.
\end{definition}

\begin{definition}\label{def:cyclic_group}
  A group \( G \) is \Def{cyclic} if it is generated\Tinyref{def:group_generators} by a single element.
\end{definition}
...
\begin{definition}\label{def:unital_magma_kernel}
  ...
  The \Def{kernel} \( \ker(f) \) of a function \( f: X \to M \) is the preimage\Tinyref{def:function_preimage} \( f^{-1}(e_N) \).
\end{definition}

\begin{proposition}\label{thm:unital_magma_kernel_is_submonoid}
  The kernel\Tinyref{def:unital_magma_kernel} of a unital magma homomorphism \( f: M \to N \) is a submonoid\Tinyref{def:first_order_structure/substructure} \( M \).
\end{proposition}
\begin{proof}
...
\begin{definition}\label{def:free_monoid}\cite[306]{Knapp2016BAlg}
  Let \( A \) be an arbitrary set. We associate with \( A \) its \Def{free monoid} \( FM(A) \coloneqq (\Cal{A}^{*}, \cdot) \)\Tinyref{def:language}. It is a monoid due to \cref{thm:set_of_all_words_is_monoid}. In category-theoretic terms, we define a functor\Tinyref{def:functor} \( FM: \Cat{Set} \to \Cat{Mon} \) that is left-adjoint\Tinyref{def:adjoint_functor} to the corresponding forgetful functor\Tinyref{def:first_order_model_category}.
\end{definition}
...
\begin{definition}\label{def:group_commutator}
  ...
  The commutator subgroup of \( G \) is the subgroup generated\Tinyref{def:group_generators} by all the commutators in \( G \).
\end{definition}
