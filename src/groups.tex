...
\begin{definition}\label{def:symmetric_group}
  ...
  This generalizes to other contexts where we consider special types of functions. Given a locally small category\Tinyref{def:category} \( \Bold{C} \), we call \( \Bold{C}(A) \) the \Def{automorphism group} over \( A \) and denote it by \( \Aut(A) \).
\end{definition}
...
\begin{proposition}\label{thm:group_homomorphism_properties}
  ...
  \begin{defenum}
    ...
    \DItem{thm:group_homomorphism_properties/preserves_subgroups} If \( H \) is a subgroup of \( G \), its image \( f(H) \) is a subgroup of \( K \) (also holds for submagmas, subsemigroups and submonoids).
...
\begin{proof}\mbox{}
  \begin{itemize}
    ...
    \RItem{thm:group_homomorphism_properties/preserves_subgroups} If \( a, b \in G \), then
    \begin{equation*}
      f(a) f(b) = f(ab) \in f(G),
    \end{equation*}
...
\end{definition}
\begin{proof}
  \Cref{thm:group_homomorphism_properties/composition} shows that the composition of two group homomorphisms is indeed a homomorphism.
\end{proof}
...
\begin{proposition}\label{thm:monoid_completion_to_abelian_group}\cite{nLab:grothendieck_group_of_a_commutative_monoid}
  Every commutative monoid\Tinyref{def:magma} can be completed\Tinyref{def:monoid_completion} using the \Def{Grothendieck} completion to form an abelian group.
\end{proposition}
\begin{proof}
  ...
  \begin{description}
    ...
    \RItem{def:order/equivalence/symmetry} By \cref{def:magma/commutativity},
    \begin{align*}
  ...
  \end{equation*}
  which, by \cref{def:magma/commutativity}, belongs to \( [(0, 0)] \).
