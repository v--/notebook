...
\begin{definition}\label{def:group}
  ...
  \begin{thmenum}
    ...
    \begin{itemize}
      \item It is compatible with the group operation: \( x_1 \cong x_2 \) and \( y_1 \cong y_2 \) imply \( x_1 y_1 \cong x_2 y_2 \).
      ...
      \item It is compatible with inverses: \( x_1 \cong x_2 \) implies \( x_1^{-1} \cong x_2^{-1} \). This also follows from the first condition: \( x_1^{-1} \cong x_1^{-1} \) implies \( e \cong x_1^{-1} x_2 \), and thus \( x_2^{-1} \cong x_1^{-1} \).
    \end{itemize}
...
\begin{proof}
  ...
  It is well-defined because, if \( x_1 H = x_2 H \), then there exists \( h \) such that \( x_1 = x_2 h \), and thus
  \begin{equation*}
    H x_2 = H h^{-1} x_1 = H x_1.
  \end{equation*}
...
\begin{proof}
  ...
  The homomorphism \( \widetilde{\varphi} \) is well-defined because, if \( x_1 N = x_2 N \), since \( N \subseteq \ker \varphi \), we have
  \begin{equation*}
    \varphi(x_1)
    =
    \varphi(x_1) e_N
...
\begin{theorem}[Quotient subgroup lattice theorem]\label{thm:quotient_subgroup_lattice_theorem}\mcite[prop. II.8.9]{Aluffi2009}
  Given a \hyperref[thm:normal_subgroup_equivalences]{normal subgroup} \( N \) of \( G \), the function \( H \mapsto H / N \) is a \hyperref[def:semilattice/homomorphism]{lattice isomorphism} between the lattice of \hyperref[def:group/submodel]{subgroups} of \( G \) containing \( N \) and the lattice of subgroups of the \hyperref[def:group/quotient]{quotient} \( G / N \).
  ...
  \begin{figure}[h]
    ...
    \label{fig:thm:lattice_theorem_for_groups}
  \end{figure}
...
\begin{proof}
  ...
  \SubProofOf[def:partially_ordered_set/homomorphism]{monotonicity} Let \( K \subseteq H \) be supergroups of \( N \). Then
  \begin{equation*}
    K / N = \set{ xN \given x \in K } \subseteq \set{ xN \given x \in H } = H / N.
  \end{equation*}

  Therefore, the map \( H \mapsto H / N \) is nonstrictly monotone.
\end{proof}
...
\begin{example}\label{ex:subgroups_of_integers}
  ...
  Since \( 2\BbbZ \) is a subgroup of \( \BbbZ \), the quotient \( 2\BbbZ / 4\BbbZ \) must a subgroup of \( \BbbZ / 4\BbbZ \) as a consequence of \fullref{thm:lattice_theorem_for_groups}. We may not know the structure of the quotient groups (although we do, see \fullref{thm:group_of_integers_modulo}), but we know how \( 4\BbbZ \), \( 2\BbbZ \) and \( \BbbZ \) relate to each other, and we are able to determine how the quotient groups relate to each other.
\end{example}
