...
\begin{definition}\label{def:magma}
  ...
  \begin{itemize}
    \DItem{def:magma/magma} \Def{Magmas} have a single binary operation and no axioms. The operation is denoted by juxtaposition, by \( \cdot \) or by \( + \) (see \fullref{remark:additive_group}). The \Def{order} of a magma is the number of elements in its set. The trivial \hyperref[def:first_order_substructure]{magma} is the empty set.

    \DItem{def:magma/semigroup} \Def{Semigroups} are magmas where the operation is \hyperref[def:algebraic_theory/associativity]{associative}.
...
\begin{proposition}\label{thm:group_properties}
  ...
  \begin{propenum}
    \DItem{thm:group_properties/cancellative} The operation is \hyperref[def:algebraic_theory/cancellative]{cancellative}.
...
\begin{proof}\mbox{}
  \begin{itemize}
    ...
    \RItem{thm:group_properties/negative_power} Using \fullref{thm:group_properties/double_inverse},
    \begin{align*}
      x^{-n}
      =
      (x^n)^{-1}
...
\begin{proof}\mbox{}
  \begin{description}
    ...
    \ImpliedBy Let the function \( f \) satisfy \fullref{thm:group_homomorphism_single_condition/condition}. Then it preserves identities since
    \begin{equation*}
...
\begin{definition}\label{def:normal_subgroup}
  ...
  \begin{defenum}
    ...
    \DItem{def:normal_subgroup/kernel} \( N \) is the \hyperref[def:unital_magma_kernel]{kernel} of some group homomorphism (in particular, kernels are always normal subgroups).
  \end{defenum}
\end{definition}
\begin{proof}
  ...
  \begin{description}
    ...
    \Implies[def:normal_subgroup/cosets][def:normal_subgroup/kernel] The proof of correctness in \fullref{def:quotient_group} only uses \fullref{def:normal_subgroup/cosets}. Therefore we can take the canonical projection \( \pi(x) \coloneqq xN \) as the homomorphism without causing circular references.
...
\begin{definition}\label{def:group_direct_product}
  ...
  We define their \Def{direct product} as the group \( \prod_{\al \in \CA} X_\al \), the group operation defined componentwise as
  \begin{align*}
  ...
  We define their \Def{direct sum} as the subgroup of \( \prod_{\al \in \CA} X_\al \) (see \fullref{def:group_direct_product}) where only finitely many components of any group element are different from zero.
\end{definition}

\begin{remark}\label{def:group_direct_sum_external_internal}\cite[126]{Knapp2016BAlg}
  If we are given a family of groups as in \fullref{def:group_direct_product}, their sum \( \oplus_{\al \in \CA} X_\al \) is sometimes called an \Def{external direct sum}.

  If instead we are given a group \( X \) and a family of subgroups \( \{ X_\al \}_{\al \in \CA} \), we say that \( X \) is their \Def{internal direct sum} if the homomorphism
  \begin{align*}
...
\begin{proposition}\label{thm:group_categorical_limits}
  We are interested in \hyperref[def:categorical_limit]{categorical limits} and \hyperref[def:categorical_colimit]{colimits} in \( \Cat{Grp} \). Fix an indexed family  \( \{ X_\al \}_{\al \in \CA} \) of groups.
  \begin{defenum}
    \DItem{thm:group_categorical_limits/product} Their \hyperref[def:categorical_product]{categorical product} is their \hyperref[def:group_direct_product]{direct product} \( \prod_{\al \in \CA} X_\al \), the projection morphisms being inherited from \fullref{thm:set_categorical_limits/product}.

    \DItem{thm:group_categorical_limits/coproduct} Their \hyperref[def:categorical_coproduct]{categorical coproduct} is their \hyperref[def:group_free_product]{free product} \( \ast_{\al \in \CA} X_\al \), the injection morphisms being
    \begin{align*}
