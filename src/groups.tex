...
\begin{definition}\label{def:group_cosets}
  ...
  \end{align*}
  are called the left and right \Def{cosets} of \( x \) with respect to \( \CH \). The name is justified by \fullref{thm:coset_partition}.
\end{definition}
...
\begin{definition}\label{def:normal_subgroup}
  ...
  \begin{DefEnum}
    \ILabel{def:normal_subgroup/direct} For any \( x \in \CG \), we have the set equality \( x \CN x^{-1} = \CN \).
