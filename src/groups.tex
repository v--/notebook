...
\begin{definition}\label{def:group}
  ...
  \begin{thmenum}
    ...
    If \( [x] = [y] \), we say that \( x \) is \term{congruent modulo} \( y \) and write \( x \cong y \pmod N \).
...
\begin{proposition}\label{thm:def:group/properties}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:def:group/properties/zero_kernel} The \hyperref[def:group/kernel]{kernel} of a group homomorphism \( \varphi: G \to H \) is trivial if and only if \( \varphi \) is an \hyperref[def:first_order_homomorphism_invertibility/embedding]{embedding} (injective homomorphism).
...
\begin{proof}
  ...
  \SubProofOf{thm:def:group/properties/zero_kernel}
