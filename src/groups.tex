...
\begin{definition}\label{def:symmetric_group}
  ...
  This generalizes to other contexts where we consider special types of functions. Given a locally small category\Tinyref{def:category} \( \Bold{C} \), we call \( \Bold{C}(A) \) the \textbf{automorphism group} and denote it by \( \Aut(A) \).
\end{definition}
...
\begin{definition}\label{def:magma_homomorphism}
  Let \( M \) and \( N \) be magmas. We say that the function \( f: M \to N \) is a \textbf{magma homomorphism} (resp. \textbf{semigroup}, or \textbf{group homomorphism}) if it is \textbf{compatible} with the magma structure on \( M \) and \( N \), that is,
  \begin{description}
...
\begin{definition}\label{def:free_abelian_group}
  As a special case of \cref{def:free_module}, since abelian groups are \( \Z \)-modules by \cref{def:abelian_group_z_module}, we define \textbf{free abelian groups} to be free \( \Z \)-modules.
