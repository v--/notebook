...
\begin{definition}\label{def:group}(See \cref{ex:algebraic_theory_language})
  ...
  We say that
  \begin{defenum}
    \DItem{def:group/trivial_group} the group \( \{ e \} \) is called the \textbf{trivial group}.
    \DItem{def:group/subgroup} the subset \( H \subseteq G \) is a \textbf{submagma} (resp. \textbf{subsemigroup}, \textbf{submonoid}, \textbf{subgroup}, etc) of \( G \)} if \( H \) is closed under the magma operation, that is, \( ab \in H \) whenever \( a, b \in H \).
    \DItem{def:group/trivial_subgroup} the subgroup \( \{ e_G \} \) of \( G \) is called the \textbf{trivial subgroup}
    \DItem{def:group/proper_subgroup} all subgroups except for \( G \) itself are \textbf{proper subgroups}.
    \DItem{def:group/finite_order} \( a \) is of \textbf{finite order} \( \Ord(a) = n \) if \( n \) is the smallest positive integer such that \( a^n = e \).
    \DItem{def:group/infinite_order} \( a \) is of \textbf{infinite order} \( \Ord(a) = \infty \) if \( a^n \neq e \) for any positive integer \( n \).
    \DItem{def:group/absorbing_element} \( a \) is an \textbf{absorbing element} if \( ba = ab = a \) for any \( b \in G \).
  \end{defenum}
\end{definition}
...
\begin{definition}\label{def:group_homomorphism}
  ...
  \begin{description}
    \DItem{def:group_homomorphism/compatibility}[compatibility] \( f(ab) = f(a) f(b) \) for all \( a, b \in M \).
  \end{description}
...
\begin{definition}\label{def:normal_subgroup}
  ...
  \begin{defenum}
    ...
    \DItem{def:normal_subgroup/kernel} \( N \) is the kernel\Tinyref{def:group_homomorphism} of some group homomorphism (in particular, kernels are always normal subgroups).
  \end{defenum}
\end{definition}
\begin{proof}
...
\begin{definition}\label{def:category_of_groups}
  The class\Tinyref{def:set_zfc} of all groups along with all homomorphisms\Tinyref{def:group_homomorphism} between them forms a category, which we denote by \( \Bold{Grp} \).
