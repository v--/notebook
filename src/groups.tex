...
\begin{definition}\label{def:group}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:group/submodel} The set \( S \subseteq \mscrG \) is a \hyperref[thm:substructure_is_model]{submodel} of \( \mscrG \) if it is a \hyperref[def:unital_magma/submodel]{unital submagma} and if \( S^{-1} = S \), where
    \begin{equation*}
      S^{-1} = \set{ s^{-1} \given s \in S }.
    \end{equation*}

    We say that \( S \) is a \term{subgroup} of \( \mscrG \).

    As a consequence of \fullref{thm:positive_formulas_preserved_under_homomorphism}, the \hyperref[def:multi_valued_function/image]{image} of a group homomorphism \( \varphi: \mscrG \to \mscrH \) is a subgroup of \( \mscrH \).
    ...
    \thmitem{def:group/duality} The \term{dual} or \term{opposite} group of \( (\mscrG, \cdot) \) is the group \( (\mscrG, \star) \), where the group operation is defined as \( x \star y = y \cdot x \).

    The \hyperref[def:monoid_delooping]{delooping} of \( (\mscrG, \star) \) is the \hyperref[def:dual_category]{dual category} of the delooping of \( (\mscrG, \cdot) \). For this reason, we denote the dual group by \( \mscrG^{\opcat} \).
  \end{thmenum}
\end{definition}
...
\begin{proof}
  ...
  \NecessitySubProof Let the function \( \varphi: \mscrG \to \mscrH \) satisfy \eqref{eq:def:magma/homomorphism}. Then it preserves identities (i.e. is a \hyperref[def:pointed_set/homomorphism]{pointed set homomorphism}) since
  \begin{equation*}
...
\begin{definition}\label{def:normal_subgroup}
  Let \( \mscrN \) be a subgroup of \( \mscrG \). We say that \( \mscrN \) is a \term{normal subgroup} if any of the following equivalent conditions hold:
  \begin{thmenum}
...
\begin{definition}\label{def:quotient_group}
  Let \( \mscrG \) be a group and \( \mscrN \) be a normal subgroup of \( \mscrG \). We define the \term{quotient group}
  \begin{equation*}
...
\begin{proposition}\label{thm:cokernels_in_grp}
  In the category hyperref[def:group/category]{\( \cat{Grp} \)} of \hyperref[def:category_size]{small} \hyperref[def:group]{groups}, the \hyperref[def:zero_morphisms/image]{categorical image} of a \hyperref[def:group/homomorphism]{group homomorphism} \( \varphi: \mscrG \to \mscrH \) is the set-theoretic image \( \img(\varphi) \).
\end{proposition}
\begin{proof}
