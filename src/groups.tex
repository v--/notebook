...
\begin{remark}\label{remark:numbers_vs_endomorphisms_generalizations}
  ...
  Another type of objects that usually fit in the same algebraic framework are functions\Tinyref{def:function} and their composition\Tinyref{def:function/composition}. Functions from a set to itself can be composed into another function of the same type, similarly to how two integers can be added to obtain another integer. The specificity is in the non-commutativity of function composition.
...
\begin{definition}\label{def:magma}
  We study here algebraic structures\Tinyref{def:algebraic_theory} with a single binary operation. \Fullref{def:first_order_model_category} ensures the existence and well-definedness of categories\Tinyref{def:category} of all the structures.

  \begin{itemize}
    \DItem{def:magma/magma} \Def{Magmas} have a single binary operation and no axioms. The operation is denoted by juxtaposition, by \( \cdot \) or by \( + \) (see \fullref{remark:additive_group}). The \Def{order} of a magma is the number of elements in its set. The trivial magma\Tinyref{def:first_order_substructure} is the empty set.
...
\begin{definition}\label{def:unital_magma_kernel}
  The \Def{kernel} \( \ker(f) \) of a unital magma homomorphism \( f: M \to N \) is the zero locus\Tinyref{def:zero_locus} of \( f \), that is, preimage\Tinyref{def:function/preimage} \( f^{-1}(e_N) \).

  It is an instance of \fullref{def:categorical_kernel}.
\end{definition}

\begin{proposition}\label{thm:unital_magma_kernel_is_submagma}
  The kernel\Tinyref{def:unital_magma_kernel} of a unital magma homomorphism \( f: M \to N \) is a submagma\Tinyref{def:first_order_substructure} \( M \).
\end{proposition}
\begin{proof}
...
\begin{proposition}\label{thm:group_categorical_limits}
  We are interested in categorical limits\Tinyref{def:categorical_limit} and colimits\Tinyref{def:categorical_colimit} in \( \Cat{Grp} \). Fix an indexed family  \( \{ X_\al \}_{\al \in \CA} \) of groups.
  \begin{defenum}
    \DItem{thm:group_categorical_limits/product} Their categorical product\Tinyref{def:categorical_product} is their direct product\Tinyref{def:group_direct_product} \( \prod_{\al \in \CA} X_\al \), the projection morphisms being inherited from \fullref{thm:set_categorical_limits/product}.

    \DItem{thm:group_categorical_limits/coproduct} Their categorical coproduct\Tinyref{def:categorical_coproduct} is their free product\Tinyref{def:group_free_product} \( \ast_{\al \in \CA} X_\al \), the injection morphisms being
    \begin{align*}
...
\begin{definition}\label{def:monoid_completion}
  A completion of a monoid is a minimal\Tinyref{def:preordered_set/maximal_minimal_element} (under homomorphic embedding) group of that monoid.
\end{definition}
...
\begin{definition}\label{def:periodic_function}
  ...
  The \Def{base period} of a function is the least\Tinyref{def:preordered_set/largest_smallest_element} of all periods, if a minimum it exists. When referring to \enquote{the period}, the base period is meant.
\end{definition}
