...
\begin{definition}\label{def:monoid_inverse_element}
  ...
  If \( y \) is simultaneously a left and right inverse of \( x \), we call a \term{two-sided inverse} or simply an \term{inverse} of \( x \) and denote it by \( x^{-1} \). It is unique by \fullref{def:monoid_inverse_element_unique}. This notation is consistent with \fullref{def:monoid/exponentiation}
\end{definition}
...
\begin{definition}\label{def:group}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:group/submodel} The set \( S \subseteq G \) is a \hyperref[thm:substructure_is_model]{submodel} of \( G \) if it is a \hyperref[def:monoid/submodel]{submonoid} and if \( S^{-1} = S \), where
    \begin{equation*}
    ...
    \thmitem{def:group/category} The \hyperref[def:category_of_small_first_order_models]{category of \( \mscrU \)-small models} of groups \( \ucat{Grp} \) is \hyperref[def:monoid]{\( \ucat{Mon} \)}-\hyperref[def:concrete_category]{concrete}.
