...
\begin{proof}
  ...
  \begin{description}
    ...
    \Implies[def:normal_subgroup/cosets][def:normal_subgroup/kernel] The proof of correctness in \cref{def:quotient_group} only uses \cref{def:normal_subgroup/cosets}. Therefore we can take the canonical projection \( \pi(x) \coloneqq xN \) as the homomorphism without causing circular references.
