...
\begin{example}\label{ex:semirings}
  ...
  \begin{refenum}
    ...
    Another example can be given by taking a sub-semiring, but not a unital sub-semiring of the endomorphism semiring \( \End(\BbbN, \oplus) \), for example the functions
    \begin{balign*}
       & f_n: \BbbN \to \BbbN        \\
       & f_n(x) \coloneqq x + n,
    \end{balign*}
