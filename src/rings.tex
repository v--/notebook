...
\begin{example}\label{ex:semirings}
  ...
  \begin{refenum}
    ...
    \refitem{def:semiring/field} The canonical examples of fields include the rational \hyperref[def:rational_numbers]{numbers} \( \BbbQ \), the \hyperref[def:set_of_real_numbers]{real numbers} \( \BbbR \) and the \hyperref[def:complex_numbers]{complex numbers} \( \BbbC \).
...
\begin{definition}\label{def:semiring_kernel}
  The \term{kernel} \( \ker(f) \) of a semiring homomorphism \( f: R \to S \) is the zero \hyperref[def:zero_locus]{locus} of \( f \), that is, \hyperref[thm:def:function/properties/preimage]{preimage} \( f^{-1}(0_S) \).
...
\begin{proposition}\label{thm:ring_categorical_limits}
  ...
  \begin{thmenum}
    \thmitem{thm:ring_categorical_limits/product} Their \hyperref[def:discrete_category_limits]{categorical product} is their direct \hyperref[def:semiring_direct_product]{product} \( \prod_{k \in \mscrK} X_k \), the projection morphisms being inherited from \fullref{thm:set_categorical_limits/product}.
  \end{thmenum}
\end{proposition}
