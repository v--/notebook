...
\begin{definition}\label{def:semiring}
  A \Def{semiring} \( (R, +, \cdot) \) is an algebraic structure with two binary operations:
...
\begin{example}\label{ex:semirings}
  ...
  \begin{RefList}
    ...
    Assume\LEM that \( I \) is generated by the polynomial \( p(X) \in \BbbZ[X] \). Since \( 2 \in I \), then \( p(X) \) divides \( 2 \) so \( p(X) \in \{ -2, -1, 1, 2 \} \), that is \( p(X) \) is a unit of \( \BbbZ[X] \). But then \( I = \Bracket{p(X)} = \BbbZ[X] \), which contradicts the definition of \( I \).
...
\begin{proposition}\label{thm:semiring_properties}
  ...
  \begin{PropEnum}
    \ILabel{thm:semiring_properties/zero_is_absorbing} Multiplication by \( 0 \) is \Def{absorbing}, that is, \( x0 = 0x = 0 \) for any \( x \in R \).
...
\begin{proof}
  ...
  Now assume that \( R \) is nontrivial and let \( \iota: \BbbZ_{\geq 0} \to R \) be the embedding defining \( \Char(R) \). Then it is obviously also an embedding of \( \BbbZ_{\geq 0} \) into \( T \). Since \( 0_R = 0_T \), then \( \ker \iota = \ker \varphi = n\BbbZ_{\geq 0} \), where \( n = \Char(R) \).
\end{proof}
...
\begin{example}\label{ex:semiring_characteristic}
  ...
  \begin{ExEnum}
    \ILabel{ex:semiring_characteristic/nonnegative_integers} The zero-based \hyperref[def:natural_numbers]{natural numbers} \( \BbbN \) have characteristic \( \Char(\BbbN) = 0 \) because \( \iota \) is an isomorphism. Consequently, any supersemiring of \( \BbbN \) has characteristic zero, most notably the integers \( \BbbZ \) and the fields \( \BbbQ \), \( \BbbR \), \( \BbbC \).

    \ILabel{ex:semiring_characteristic/integers_modulo} The integers modulo \( n \) (see \fullref{def:ring_of_integers_modulo}) have characteristic \( \Char(\BbbZ_n) = n \) because of \fullref{thm:integers_modulo_isomorphic_to_quotient_group}.
...
\begin{definition}\label{def:ring_of_integers_modulo}
  ...
  The ring \( \BbbZ_n \) is called the \Def{ring of integers modulo} \( n \).
\end{definition}
\begin{proof}
  Note that
  \begin{BreakableAlign*}
...
\begin{definition}\label{def:semiring_direct_product}
  ...
  Analogously to \fullref{def:group_direct_product}, we define their \Def{direct product} as the ring \( \prod_{k \in \mscrK} X_k \), the operations defined componentwise as
  \begin{BreakableAlign*}
  ...
  We define their \Def{direct sum} as the subring of \( \prod_{k \in \mscrK} X_k \) (see \fullref{def:semiring_direct_product}) where only finitely many components of any ring element are different from zero.
\end{definition}
...
\begin{definition}\label{def:functions_vanish_nowhere}
  Let \( \mathcal{F} \) be a family of functions from a set \( S \) to a ring \( R \). We say that \( \mathcal{F} \) \Def{vanishes nowhere} if for every \( x \in S \) there exists a function \( f \in \mathcal{F} \) such that \( f(x) \neq 0_R \).
\end{definition}

\begin{definition}\label{def:ordered_semiring}
  Extending \fullref{def:preordered_magma} to (semi)rings, we define a \Def{preordered semiring} to be a semiring \( \BbbR \) with a magma preorder \( \leq \) that additionally satisfies
