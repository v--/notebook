...
\begin{definition}\label{def:semiring}\mcite[ch. 3, sec. 2.1]{GondranMinoux1984Graphs}
  A \term{semiring} is a \hyperref[def:magma/commutative]{commutative} \hyperref[def:monoid]{monoid} \( (R, +) \) with a second \hyperref[def:magma/associative]{associative} operation \( \cdot: R \times R \to R \) called \term{multiplication} that satisfies the following properties:
  \begin{thmenum}
    ...
    \thmitem{def:semiring/right_distributivity} Multiplication on the right distributes over addition:
    ...
    \thmitem{def:semiring/identity} There exists a multiplicative \hyperref[def:magma_identity]{identity} \( 1 \). That is, an element of \( R \) satisfying
    \begin{equation}\label{eq:def:semiring/identity}
      x \cdot 1 = 1 \cdot x = x.
    \end{equation}

    This notation is justified by \fullref{thm:semiring_characteristic_homomorphism}.

    If an identity does not exist, we call \( (R, +, \cdot) \) a \term{nonunital semiring}. We will not need nonunital semirings, but it is important to acknowledge their existence and spread. In this context, if an identity exists, we will sometimes call \( (R, + \cdot) \) a \term{unital semiring}.
  \end{thmenum}
  ...
  \begin{thmenum}
    \thmitem{def:semiring/theory} The \hyperref[def:first_order_theory]{first-order theory} for semirings extends the \hyperref[def:monoid/theory]{theory of monoids} by adding another \hyperref[rem:first_order_formula_conventions/infix]{infix} binary functional symbol \( \cdot \), a constant \( 1 \), requiring \hyperref[def:magma/commutative]{commutativity} for \( + \), \hyperref[def:magma/associative]{associativity} for \( \cdot \), \hyperref[eq:def:monoid/theory/identity]{identity} for \( 1 \), and translating \eqref{eq:def:semiring/left_distributivity}, \eqref{eq:def:semiring/right_distributivity} and \eqref{eq:def:semiring/absorption} into first-order formulas from the metalogic.
...
\begin{example}\label{ex:trivial_semiring}
  A \hyperref[def:semiring/homomorphism]{semiring} is trivial if and only if \( 0_R = 1_R \). This follows from \eqref{eq:def:semiring/absorption} and \eqref{eq:def:semiring/zero}.
