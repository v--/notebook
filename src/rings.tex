...
\begin{Definition}\label{def:semiring}
  ...
  \begin{DefEnum}[series=def:semiring]
    \ILabel{def:semiring/addition} The \Def{addition} \( + \) is \hyperref[def:algebraic_theory/associativity]{associative}, \hyperref[def:algebraic_theory/identity]{unital} and \hyperref[def:algebraic_theory/commutativity]{commutative}, i.e. \( (R, +) \) is a commutative \hyperref[def:magma/monoid]{monoid}. The additive identity is usually denoted by \( 0 \). We denote this monoid by \( R^+ \).

    \ILabel{def:semiring/multiplication} The \Def{multiplication} \( \cdot \) (usually written using juxtaposition) is \hyperref[def:algebraic_theory/associativity]{associative}, i.e. \( (R, \cdot) \) a \hyperref[def:magma/semigroup]{semigroup}. We denote this semigroup by \( R^\times \).
  \end{DefEnum}
  ...
  \begin{DefEnum}[resume=def:semiring]
    ...
    \ILabel{def:semiring/commutative_ring} \Def{Commutative rings} are \hyperref[def:semiring/ring]{rings} in which multiplication is \hyperref[def:algebraic_theory/commutativity]{commutative}, i.e. \( R, \cdot \) is a commutative semigroup.
    ...
    \ILabel{def:semiring/division_ring} \Def{Division rings} are nontrivial unital \hyperref[def:semiring/unital_ring]{rings} in which all nonzero elements are units, i.e. \( (F \setminus \{ 0 \}, \cdot) \) is a \hyperref[def:magma/group]{group}. The nontriviality is a requirement because we want \( 1 \) not to be a zero divisor (see discussion in \fullref{def:semiring/integral_domain}).
    ...
    \ILabel{def:semiring/field} \Def{Fields} are \hyperref[def:algebraic_theory/commutativity]{commutative} division \hyperref[def:semiring/division_ring]{rings}, i.e. \( (F \setminus \{ 0 \}, \cdot) \) is an \hyperref[def:abelian_group]{abelian group}. The category of fields is denoted by \( \Cat{Field} \).
  \end{DefEnum}
\end{Definition}
...
\end{Proposition}
\begin{RefListProof}
    \IRef{thm:semiring_properties/zero_is_absorbing} Follows from \fullref{def:left_module_properties/ring_zero_is_absorbing} and \fullref{def:left_module_properties/module_zero_is_absorbing}.
    \IRef{thm:semiring_properties/one_exponents} By definition, \( r \cdot 1 = r \) for any \( r \in R \). Hence for \( 1 \cdot 1 = 1 \). Proceeding by induction\IND, we can show that for \( 1^n = 1 \) for positive \( n \). For negative \( n \), since \( 1 \) is its own inverse, we also have \( 1^n = 1 \).

    \IRef{thm:semiring_properties/identities_are_equal_iff_trivial_ring}\mbox{}
    \begin{RefList}
      \ISufficiency Let \( 0 = 1 \) in a unital ring \( R \). Let \( r \in R \). Then, by \fullref{thm:semiring_properties/zero_is_absorbing},
      \begin{equation*}
        r = 1r = 0r = 0.
      \end{equation*}
      ...
      \INecessity The trivial ring only has one element, hence \( 0 = 1 \).
    \end{RefList}

    \IRef{thm:semiring_properties/cancellable_iff_not_zero_divisor}\mbox{}
    \begin{RefList}
      \ISufficiency Suppose that \( x \in R \) is not cancellable. Then there exist \( y \neq z \) for which \( xy = xz \). We have \( y - z \neq 0 \) and
      \begin{equation*}
        x(y - z) = xy - xz = 0.
      \end{equation*}
      ...
      \INecessity If \( x \) is a zero divisor, fix \( y \in R \) such that \( xy = 0 \). For any \( z \in R \) we have
      \begin{equation*}
        xy = 0 = x(yz)
      \end{equation*}
      but \( y \neq yz \) in general.
