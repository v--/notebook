...
\begin{definition}\label{def:semiring_characteristic}
  ...
  \begin{defenum}
    ...
    \DItem{def:semiring_characteristic/homomorphism} The semiring \( \Z^{\geq 0} \) of nonnegative integers can be embedded into \( R \) via the unique homomorphism
    \begin{align*}
      &\iota: \Z^{\geq 0} \to R \\
    ...
    \begin{equation*}
      n\Z^{\geq 0} \cong \ker\iota.
    \end{equation*}
  \end{defenum}
\end{definition}
\begin{proof}
  We will show that \( \iota \) is unique. Let \( \varphi: \Z^{\geq 0} \to R \) be another embedding. Obviously
  \begin{equation*}
...
\begin{proof}
  ...
  Now assume that \( R \) is nontrivial and let \( \iota: \Z^{\geq 0} \to R \) be the embedding defining \( \Char(R) \). Then it is obviously also an embedding of \( \Z^{\geq 0} \) into \( T \). Since \( 0_R = 0_T \), then \( \ker \iota = \ker \varphi = n\Z^{\geq 0} \), where \( n = \Char(R) \).
\end{proof}
