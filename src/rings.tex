...
\begin{definition}\label{def:semiring_direct_product}
  ...
  Analogously to \fullref{def:group_direct_product}, we define their \Def{direct product} as the ring \( \prod_{i \in I} X_i \), the operations defined componentwise as
  \begin{align*}
...
\begin{proposition}\label{thm:ring_categorical_limits}
  ...
  \begin{defenum}
    \DItem{thm:ring_categorical_limits/product} their categorical product\Tinyref{def:categorical_product} is their direct product\Tinyref{def:semiring_direct_product} \( \prod_{i \in I} X_i \), the projection morphisms being inherited from \fullref{thm:set_categorical_limits/product}.
  \end{defenum}
\end{proposition}
