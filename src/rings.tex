...
\begin{example}\label{ex:semirings}
  ...
  \begin{RefList}
    ...
    Assume\LEM that \( I \) is generated by the polynomial \( p(X) \in \BZ[X] \). Since \( 2 \in I \), then \( p(X) \) divides \( 2 \) so \( p(X) \in \{ -2, -1, 1, 2 \} \), that is \( p(X) \) is a unit of \( \BZ[X] \). But then \( I = \Braket{p(X)} = \BZ[X] \), which contradicts the definition of \( I \).
