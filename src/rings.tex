...
\begin{definition}\label{def:ring}
  A \textbf{ring} is an (additive\Tinyref{note:additive_group}) abelian group\Tinyref{def:group} \( (R, +) \) with an additional multiplication operation \( \cdot: R \times R \to R \) (denoted by juxtaposition), such that for all \( a, b, c \in R \) the following axioms hold
  \begin{description}
  ...
  \begin{description}
    ...
    \DItem{commutativity}{def:ring/commutativity} If \( (R, \cdot) \) is a commutative semigroup, i.e. \( ab = ba \) for all \( a, b \in R \), we say that \( (R, \cdot) \) is a \textbf{commutative ring}.
    \DItem{no zero divisors}{def:ring/no_zero_divisors} If the ring is a commutative ring and there are no zero divisors in an, we say that \( (R, \cdot) \) is an \textbf{integral domain}.
    \DItem{divisibility}{def:ring/divisibility} If all nonzero elements are units, we say that \( (R, \cdot) \) is a \textbf{division ring}.
  \end{description}
...
\begin{proposition}\label{def:ring_properties}
  ...
  \begin{defenum}
    \item\label{def:ring_properties/zero_absorbing} Multiplication by \( 0 \) is \textbf{absorbing}\Tinyref{def:group/absorbing_element}, that is, \( a0 = 0a = 0 \) for any \( a \in R \).
  \end{defenum}
\end{proposition}
\begin{proof}\mbox{}
  \begin{itemize}
