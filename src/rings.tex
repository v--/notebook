...
\begin{example}\label{ex:semiring_characteristic}
  ...
  \begin{exenum}
    ...
    \DItem{ex:semiring_characteristic/integers_modulo} The integers modulo \( n \) (see \fullref{def:ring_of_integers_modulo}) have characteristic \( \Char(\BZ_n) = n \) because of \fullref{thm:integers_modulo_isomorphic_to_quotient_group}.
...
\begin{definition}\label{def:semiring_direct_product}
  ...
  We define their \Def{direct sum} as the subring of \( \prod_{\al \in \CA} X_\al \) (see \fullref{def:semiring_direct_product}) where only finitely many components of any ring element are different from zero.
\end{definition}
