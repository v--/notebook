...
\begin{definition}\label{def:semiring}
  A \Def{semiring} \( (R, +, \cdot) \) is an algebraic structure\Tinyref{def:algebraic_theory} with two binary operation:
  \begin{description}
  ...
  \begin{defenum}
    ...
    \DItem{def:semiring/division_ring} \Def{Division rings} are nontrivial\Tinyref{def:first_order_structure/minimal} unital rings\Tinyref{def:semiring/unital_ring} in which all nonzero elements are units, i.e. i.e. \( (F \setminus \{ 0 \}, \cdot) \) is a group\Tinyref{def:magma/group}.
...
\begin{definition}\label{def:semiring_ideal}
  Let \( R \) be a ring and \( I \) be a subset of \( R \) (not necessarily a subring). We say that \( I \) is a (two-sided) \Def{ideal} of \( R \) and write \( I \unlhd R \) if any of the following equivalent conditions hold:
  \begin{defenum}
    \DItem{def:semiring_ideal/direct} \( (I, +) \) is a subgroup of \( (R, +) \) and the inclusions \( RI \subseteq I \) and \( IR \subseteq I \) hold.
    \DItem{def:semiring_ideal/kernel} \( I \) is the kernel\Tinyref{def:semiring_kernel} of some ring homomorphism.
  \end{defenum}

  We can weaken the condition in \ref{def:semiring_ideal/direct} to define \Def{left ideals} (resp \Def{right ideals}) if only \( RI \subseteq I \) (resp. \( IR \subseteq I \)) holds. If \( I \) is a subring, being either a left or right ideal is equivalent to \( I \) being a two-sided ideal. If \( R \) is a commutative ring, left and right ideals coincide with two-sided ideals.
...
\begin{definition}\label{def:ideal_generators}\cite[176]{Knapp2016BAlg}
  A set \( S \subseteq I \) is said to \Def{generate} the ideal \( I \) of a unital ring\Tinyref{def:semiring/unital_ring} \( R \) if
  \begin{equation*}
    I = \Gen{ S } = \{ x_1 s_1 + \cdots + x_n s_n \colon s_1, \cdots, s_n \in S; x_1, \ldots, x_n \in R \}.
  \end{equation*}

  If \( S \) is finite, then \( I \) is called \Def{finitely generated}.
\end{definition}
...
\begin{definition}\label{def:ring_commutator}
  ...
  The commutator ideal of \( R \) is the ideal generated\Tinyref{def:group_generators} by all the commutators in \( G \).
\end{definition}
