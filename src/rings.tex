...
\begin{definition}\label{def:semiring}
  A \Def{semiring} \( (R, +, \cdot) \) is an associative algebraic \hyperref[def:algebraic_theory]{structure} with two binary operations:
  \begin{description}
    \DItem{def:semiring/addition}{addition} \( + \) is \hyperref[def:algebraic_theory/associativity]{associative}, \hyperref[def:algebraic_theory/identity]{unital} and \hyperref[def:algebraic_theory/commutativity]{commutative}, i.e. \( (R, +) \) is a commutative \hyperref[def:magma/monoid]{monoid}. The additive identity is usually denoted by \( 0 \). We denote this monoid by \( R^+ \).

    \DItem{def:semiring/multiplication}{multiplication} \( \cdot \) (usually written using juxtaposition) is \hyperref[def:algebraic_theory/associativity]{associative}, i.e. \( (R, \cdot) \) a \hyperref[def:magma/semigroup]{semigroup}. We denote this semigroup by \( R^\times \).
  \end{description}
  ...
  \begin{defenum}
    ...
    \DItem{def:semiring/commutative_ring} \Def{Commutative rings} are \hyperref[def:semiring/ring]{rings} in which multiplication is \hyperref[def:algebraic_theory/commutativity]{commutative}, i.e. \( R, \cdot \) is a commutative semigroup.
    ...
    \DItem{def:semiring/integral_domain} \Def{Integral domains} are nontrivial commutative unital \hyperref[def:semiring/commutative_unital_ring]{rings} with no zero \hyperref[def:commutative_ring_division]{divisors}. This implies that \( 1 \neq 0 \) because otherwise \( 1 \cdot 1 = 0 \) and \( 1 \) would be a zero divisor. This in turn implies that \fullref{thm:semiring_properties/identities_are_equal_iff_trivial_ring}.
    ...
    \DItem{def:semiring/unique_factorization_domain} \Def{Unique factorization domains} are integral domains in which every element has unique \hyperref[def:factorization_in_ring]{factorization} exists.

    \DItem{def:semiring/principal_ideal_domain} \Def{Principal ideal domains} are integral \hyperref[def:semiring/integral_domain]{domains} in which every \hyperref[def:semiring_ideal]{ideal} is \hyperref[def:principal_ideal]{principal}.

    By \fullref{thm:pid_is_ufd}, every principal ideal domain is a unique factorization domain.

    \DItem{def:semiring/euclidean_domain} \Def{Euclidean domains} are integral \hyperref[def:semiring/integral_domain]{domains} which allow division with remainders (see \fullref{def:euclidean_domain}).

    By \fullref{thm:euclidean_domain_is_pid}, every Euclidean domain is a principal ideal domain.

    \DItem{def:semiring/division_ring} \Def{Division rings} are nontrivial unital \hyperref[def:semiring/unital_ring]{rings} in which all nonzero elements are units, i.e. \( (F \setminus \{ 0 \}, \cdot) \) is a \hyperref[def:magma/group]{group}. The nontriviality is a requirement because we want \( 1 \) not to be a zero divisor (see discussion in \fullref{def:semiring/integral_domain}).
    ...
    \DItem{def:semiring/field} \Def{Fields} are \hyperref[def:algebraic_theory/commutativity]{commutative} division \hyperref[def:semiring/division_ring]{rings}, i.e. \( (F \setminus \{ 0 \}, \cdot) \) is an abelian \hyperref[def:abelian_group]{group}. The category of fields is denoted by \( \Cat{Field} \).
  \end{defenum}
\end{definition}
...
\begin{example}\label{ex:semirings}
  ...
  \begin{description}
    \RItem{def:semiring/euclidean_domain} The base building block for the examples will be the ring \( (\BZ, +, \cdot) \) of \hyperref[def:integers]{integers}, which itself is an Euclidean domain (see \fullref{def:integers}).
    ...
    \RItem{def:semiring/dioid} By removing additive inverses from the integers, we obtain the dioid \( (\BN, +, \cdot) \) of natural \hyperref[def:natural_numbers]{numbers}.

    Take only the commutative monoid \( (\BN, +) \) of natural \hyperref[def:natural_numbers]{numbers} with addition. The endomorphism semiring
    \begin{equation*}
      \End(\BN)
    \end{equation*}
    ...
    \RItem{def:semiring} Simple examples of semirings (but not rings) without unity are proper semiring ideals. For example, the ideal \( 2\BN \) of positive even numbers is a non-unital semiring.
    ...
    \RItem{def:semiring/unital_ring} The endomorphism rings \( \End(G) \) for any abelian group \( G \) are unital but non-commutative rings. This includes the matrix space \( R^{n \times n} \) (see \fullref{thm:finite_dimensional_operators_are_isomorphic_to_matrices}).

    \RItem{def:semiring/ring} Consider the Banach space \( C_0(\Co) \) of complex functions vanishing at \hyperref[def:function_spaces/c0]{infinity}. If we take addition to be pointwise addition and multiplication to be composition, then \( C_0(\Co) \) becomes a non-commutative ring with no multiplicative identity because \( C_0(\Co) \) does not contain the identity function.
    ...
    \RItem{def:semiring/integral_domain}\cite[388]{Knapp2016BAlg} The integral domain \( \BZ[\sqrt{-5}] \) is not a unique factorization domain because
    \begin{equation*}
    ...
    \RItem{def:semiring/unique_factorization_domain}\cite{ProofWiki:polynomials_in_integers_is_not_principal_ideal_domain} The unique factorization domain \( \BZ[X] \) is not a principal ideal domain.
    ...
    \RItem{def:semiring/division_ring} The \hyperref[def:ring_localization]{localization} of a noncommutative ring over its nonzero cancellative elements (characterized by \fullref{thm:ring_localization_universal_property}), if it exists, forms a division ring.

    \RItem{def:semiring/field} The canonical examples of fields include the rational \hyperref[def:rational_numbers]{numbers} \( \BQ \), the real \hyperref[def:real_numbers]{numbers} \( \BR \) and the complex \hyperref[def:complex_numbers]{numbers} \( \BC \).
...
\begin{proposition}\label{thm:semiring_properties}
  ...
  \begin{propenum}
    \DItem{thm:semiring_properties/zero_is_absorbing} Multiplication by \( 0 \) is \hyperref[def:algebraic_theory/absorbing_element]{absorbing}, that is, \( x0 = 0x = 0 \) for any \( x \in R \).
    \DItem{thm:semiring_properties/one_exponents} For any integer \( n \), \( 1^n = 1 \).
    \DItem{thm:semiring_properties/identities_are_equal_iff_trivial_ring} In a unital \hyperref[def:semiring/unital_ring]{ring}, the additive and multiplicative identities are equal if and only if the ring is trivial.
...
\begin{proof}\mbox{}
  \begin{itemize}
    \RItem{thm:semiring_properties/zero_is_absorbing} Follows from \fullref{def:left_module_properties/ring_zero_is_absorbing} and \fullref{def:left_module_properties/module_zero_is_absorbing}.
    \RItem{thm:semiring_properties/one_exponents} By definition, \( r \cdot 1 = r \) for any \( r \in R \). Hence for \( 1 \cdot 1 = 1 \). Proceeding by induction\IND, we can show that for \( 1^n = 1 \) for positive \( n \). For negative \( n \), since \( 1 \) is its own inverse, we also have \( 1^n = 1 \).

    \RItem{thm:semiring_properties/identities_are_equal_iff_trivial_ring}\mbox{}
    \begin{description}
      \Implies Let \( 0 = 1 \) in a unital ring \( R \). Let \( r \in R \). Then, by \fullref{thm:semiring_properties/zero_is_absorbing},
      \begin{equation*}
        r = 1r = 0r = 0.
      \end{equation*}
...
\begin{definition}\label{def:semiring_characteristic}
  ...
  \begin{defenum}
    ...
    \DItem{def:semiring_characteristic/homomorphism} The semiring \( \BZ_{\geq 0} \) of nonnegative integers can be embedded into \( R \) via the unique homomorphism
    \begin{align*}
      &\iota: \BZ_{\geq 0} \to R \\
...
\begin{example}\label{ex:semiring_characteristic}
  ...
  \begin{exenum}
    \DItem{ex:semiring_characteristic/nonnegative_integers} The zero-based natural \hyperref[def:natural_numbers]{numbers} \( \BN \) have characteristic \( \Char(\BN) = 0 \) because \( \iota \) is an isomorphism. Consequently, any supersemiring of \( \BN \) has characteristic zero, most notably the integers \( \BZ \) and the fields \( \BQ \), \( \BR \), \( \BC \).

    \DItem{ex:semiring_characteristic/integers_modulo} The integers modulo \( n \)\Tinyref{def:ring_of_integers_modulo} have characteristic \( \Char(\BZ_n) = n \) because of \fullref{thm:integers_modulo_isomorphic_to_quotient_group}.

    \DItem{ex:semiring_characteristic/polynomial_ring} An \hyperref[def:algebra_over_ring]{algebra} \( A \) over a nontrivial commutative unital ring \( R \) has characteristic \( \Char(A) = R \) because of the canonical embedding of \( R \) in \( A \). In particular, polynomial \hyperref[def:algebra_of_polynomials]{rings} \( R[X] \) have the same characteristic as their ring.

    \DItem{ex:semiring_characteristic/galois_fields} The Galois \hyperref[thm:galois_field_existence]{field} \( \BF_{p^n} \) has characteristic \( p \) because it is a field extension of \( \BF_p \).
  \end{exenum}
\end{example}
...
\begin{definition}\label{def:ring_of_integers_modulo}
  The \hyperref[def:integers]{integers} \( \BZ \) form a ring under addition and multiplication. Fix a positive integer \( n > 1 \). We extend the group \( \BZ_n \) of integers modulo \( n \)\Tinyref{def:group_of_integers_modulo} with the operation
  \begin{equation*}
...
\begin{definition}\label{def:tropical_semiring}\cite{nLab:tropical_semiring}
  Fix a partially \hyperref[def:poset]{ordered} abelian \hyperref[def:abelian_group]{group} \( (M, +, \leq) \). Let \( \infty \) be a sentinel symbol not in \( M \). Define
  \begin{equation*}
...
\begin{definition}\label{def:semiring_direct_product}
  ...
  Analogously to \fullref{def:group_direct_product}, we define their \Def{direct product} as the ring \( \prod_{\al \in \CA} X_\al \), the operations defined componentwise as
  \begin{align*}
  ...
  We define their \Def{direct sum} as the subring of \( \prod_{\al \in \CA} X_\al \)\Tinyref{def:semiring_direct_product} where only finitely many components of any ring element are different from zero.
\end{definition}

\begin{proposition}\label{thm:ring_categorical_limits}
  We are interested in categorical \hyperref[def:categorical_limit]{limits} and \hyperref[def:categorical_colimit]{colimits} in \( \Cat{Ring} \). Fix an indexed family  \( \{ X_\al \}_{\al \in \CA} \) of rings.
  \begin{defenum}
    \DItem{thm:ring_categorical_limits/product} Their categorical \hyperref[def:categorical_product]{product} is their direct \hyperref[def:semiring_direct_product]{product} \( \prod_{\al \in \CA} X_\al \), the projection morphisms being inherited from \fullref{thm:set_categorical_limits/product}.
  \end{defenum}
\end{proposition}
