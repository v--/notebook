...
\begin{definition}\label{def:semiring_characteristic}
  Fix a unital semiring \( R \). We define its \Def{characteristic} \( \Char(R) \) via any of the equivalent definitions:
  \begin{defenum}
...
\begin{example}\label{def:semiring_characteristic}
  We find examples of semiring characteristics via the embedding \( \iota \) defined in \cref{def:semiring_characteristic}:

  \begin{exenum}
    \DItem{def:semiring_characteristic/nonnegative_integers} The zero-based natural numbers\Tinyref{def:natural_numbers} \( \N \) have characteristic \( \Char(\N) = 0 \) because \( \iota \) is an isomorphism. Consequently, any supersemiring of \( \N \) has characteristic zero, most notably the integers \( \Z \) and the fields \( \Q \), \( \R \), \( \Co \).

    \DItem{def:semiring_characteristic/integers_modulo} The integers modulo \( n \)\Tinyref{def:ring_of_integers_modulo} have characteristic \( \Char(\Z_n) = n \) because of \cref{thm:integers_modulo_isomorphic_to_quotient_group}. Therefore the finite fields that embed \( \Z_n \) (see \cref{thm:integers_modulo_isomorphic_to_quotient_group}) also have characteristic \( n \).

    \DItem{def:semiring_characteristic/polynomial_ring} An algebra\Tinyref{def:algebra_over_ring} \( A \) over a nontrivial commutative unital ring \( R \) has characteristic \( \Char(A) = R \) because of the canonical embedding of \( R \) in \( A \). In particular, polynomial rings\Tinyref{def:algebra_of_polynomials} \( R[X] \) have the same characteristic as their ring.

    \DItem{def:semiring_characteristic/galois_fields} The Galois field\Tinyref{thm:galois_field_existence} \( \F_{p^n} \) can characteristic \( p \) because it is a field extension of \( \F_p \).
  \end{exenum}
\end{example}
