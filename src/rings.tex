...
\begin{definition}\label{def:semiring_kernel}
  The \Def{kernel} \( \ker(f) \) of a semiring homomorphism \( f: R \to S \) is the zero locus\Tinyref{def:zero_locus} of \( f \), that is, preimage\Tinyref{def:function/preimage} \( f^{-1}(0_S) \).
...
\begin{definition}\label{def:tropical_semiring}\cite{nLab:tropical_semiring}
  Fix a partially ordered\Tinyref{def:poset} abelian group\Tinyref{def:abelian_group} \( (M, +, \leq) \). Let \( \infty \) be a sentinel symbol not in \( M \). Define
  \begin{equation*}
