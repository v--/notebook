...
\begin{definition}\label{def:ring}
  A \underLine{ring} is an (additive\Tinyref{note:additive_group}) abelian group\Tinyref{def:group} \( (R, +) \) with an additional multiplication operation \( \cdot: R \times R \to R \) (denoted by juxtaposition), such that for all \( a, b, c \in R \) the following axioms hold
  \begin{description}
  ...
  We say that
  \begin{itemize}
    \item\label{def:ring/trivial_group} the ring \( \{ 0 \} \) is the \underLine{zero ring} or \underLine{trivial ring}.
    \item\label{def:ring/subring} the subset \( S \subseteq R \) is a \underLine{subring of \( R \)} if \( S \) is closed under the ring operations.
    \item\label{def:ring/trivial_subgroup} the ring \( \{ 0_R \} \) is the \underLine{trivial subring} of \( R \).
    \item\label{def:ring/proper_subring} all subrings except for \( R \) itself are \underLine{proper subrings}.
    \item\label{def:ring/zero_divisor} \( a \neq 0 \) is a \underLine{(left) zero divisor} (resp. \underLine{right zero divisor}) if there exists \( b \neq 0 \) such that \( ab = 0 \) (resp. \( ba = 0 \)).
    \item\label{def:ring/unit} \( a \) is a \underLine{(left) unit} (resp. \underLine{right unit}) if there exists \( a^{-1} \) such that \( a \cdot a^{-1} = 1 \) (resp \( a^{-1} \cdot a = 1 \)).
    \item\label{def:ring/nilpotent_element} \( a \) is \underLine{nilpotent} if \( a^n \) for some nonnegative integer \( n \).
    \item\label{def:ring/idempotent_element} \( a \) is \underLine{idempotent} if \( aa = a \).
  \end{itemize}

  Additionally, the following axioms define different types of rings
  \begin{description}
    \DItem{identity}{def:ring/identity} If \( (R, \cdot) \) is a monoid, that is if there exists a multiplicative identity \( 1_R \) such that \( 1_R a = a1_R = a \) for all \( a \in R \), we say that \( (R, \cdot) \) is a \underLine{ring with identity} or \underLine{unital ring}. It is unique by~\cref{def:group_properties/unique_identity}. This is sometimes taken to be part of the definition of a ring.
    \DItem{commutativity}{def:ring/commutativity} If \( (R, \cdot) \) is a commutative semigroup, i.e. \( ab = ba \) for all \( a, b \in R \), we say that \( (R, \cdot) \) is a \underLine{commutative ring}.
    \DItem{no zero divisors}{def:ring/no_zero_divisors} If the ring is a commutative ring and there are no zero divisors in an, we say that \( (R, \cdot) \) is an \underLine{integral domain}.
    \DItem{divisibility}{def:ring/divisibility} If all nonzero elements are units, we say that \( (R, \cdot) \) is a \underLine{division ring}.
  \end{description}

  If we only require the ring to be a monoid under addition (i.e. no inverse elements), we say that \( (R, +, \cdot) \) is a \underLine{semiring}.
\end{definition}

\begin{proposition}\label{def:ring_properties}
  Any ring \( R \) has the following basic properties:
  \begin{defenum}
    \item\label{def:ring_properties/zero_absorbing} Multiplication by \( 0 \) is \underLine{absorbing}\Tinyref{def:group/absorbing_element}, that is, \( a0 = 0a = 0 \) for any \( a \in R \).
  \end{defenum}
\end{proposition}
\begin{proof}\mbox{}
  \begin{itemize}
...
\begin{definition}\label{def:ring_homomorphism}
  Let \( R \) and \( T \) be rings. We say that the function \( f: R \to T \) is a \underLine{ring homomorphism} if it is \underLine{compatible with the ring structures on \( S \) and \( T \)}, that is, it is a group homomorphism on \( (R, +) \) and a semigroup homomorphism on \( (R, \cdot) \).

  If \( R \) is a ring with identity, we additionally require \( f(1_R) = 1_T \).

  The \underLine{kernel} of \( f \) is defined as the preimage\Tinyref{def:function_preimage} \( f^{-1}(0) \).
...
\begin{definition}\label{def:ring_ideal}
  Let \( R \) be a ring and \( I \) be a subset of \( R \) (not necessarily a subring). We say that \( I \) is a \underLine{(two-sided) ideal of \( R \)} and write \( I \unlhd R \) if any of the following equivalent conditions hold:
  \begin{defenum}
  ...
  As with subrings, the \underLine{trivial ideal} of \( R \) is the trivial subring and all ideals except for \( R \) itself are called \underLine{proper ideals}.
\end{definition}
\begin{proof}
...
\begin{definition}\label{def:ring_direct_product}
  ...
  Analogously to \cref{def:group_direct_product}, we define their \underLine{direct product} as the ring \( \prod_{i \in I} X_i \), the operations defined componentwise as
  \begin{align*}
  ...
  We define their \underLine{direct sum} as the subring of \( \prod_{i \in I} X_i \)\Tinyref{def:ring_direct_product} where only finitely many components of any ring element are different from zero.
\end{definition}
