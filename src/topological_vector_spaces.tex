...
\begin{definition}\label{def:topological_vector_space}
  Let \( X \) be any vector space and let \( \mscrT \) be a topology on \( X \). The space \( (X, +, \cdot, \mscrT) \) is called a \Def{topological vector space} if the linear and topological structure agree, that is, the operations \( +: X \times X \to X \) and \( \cdot: X \times \BbbR \to X \) are continuous with respect to \( \mscrT \).
...
\begin{definition}\label{def:locally_convex_space}\MarginCite[1.8]{Rudin1991}
  We say that a \hyperref[def:topological_vector_space]{topological vector space} is \Def{locally convex} if there exists a \hyperref[def:topological_base]{topological base} of \hyperref[def:convex_set]{convex} sets.
\end{definition}
...
\begin{definition}\label{def:sublinear_functional}
  We say that \( f: X \to \BbbR \) is a \Def{sublinear functional} if it satisfies
  \begin{DefEnum}
