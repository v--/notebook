...
\begin{proposition}\label{thm:fundamental_sequence_is_bounded}
  In a metric space, any fundamental \hyperref[def:fundamental_net]{sequence} \( \{ x_k \}_{i=1}^n \) is \hyperref[def:metric_space/bounded_sequence]{bounded}.
\end{proposition}
\begin{proof}
  Since the set
  \begin{equation*}
...
\begin{proposition}\label{thm:fundamental_subsequence_convergence}
  In any metric \hyperref[def:complete_metric_space]{space}, a fundamental \hyperref[def:fundamental_net]{sequence} converges to a value if and only if it has a subsequence that converges to the same value.
\end{proposition}
\begin{proof}
