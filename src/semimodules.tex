...
\begin{definition}\label{def:free_semimodule}\mimprovised
  Fix a \hyperref[def:semiring]{semiring} \( R \). We associate with every set \( S \) its \term{free \( R \)-semimodule} \( R^{\bigoplus S} \) over \( R \) defined as the set
  \begin{equation*}
    R^{\bigoplus S} \coloneqq \bigoplus_{x \in A} R = \set{ t: A \to R \given t \T{has finite \hyperref[def:function_support]{support}} }.
  \end{equation*}

  In the case when \( R \) is the semiring \( \BbbN \) of natural numbers, \( \BbbN^{\bigoplus S} \) is the set of finite \hyperref[def:weighted_set/multiset]{multisets} over \( S \).

  We call \( S \) the \term{basis} of \( R \). As with general direct sums, we regard the function \( t \) as the indexed family \( \seq{ t_x }_{x \in S} \).

  By \fullref{thm:functions_over_model_form_model}, \( F(S) \) inherits addition and multiplication from \( R \) and is actually a semiring. Scalar multiplication can be defined as
  \begin{equation*}
    \begin{aligned}
      &\cdot: R \times F(S) \to F(S), \\
...
\begin{proposition}\label{thm:free_semimodule_universal_property}
  ...
  \begin{displayquote}
    For every semimodule \( M \) over \( R \) and every function \( f: S \to M \), there exists a unique \( R \)-semimodule homomorphism \( \widetilde{f}: F(S) \to M \) such that the following diagram commutes:
