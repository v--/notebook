...
\begin{definition}\label{def:algebra_over_semiring}\mimprovised
  An \term{algebra} \( X \) over a \hyperref[def:semiring]{semiring} \( R \) is an \( R \)-\hyperref[def:semimodule]{semimodule} with an \hyperref[def:magma/associative]{associative} \hyperref[def:multilinear_function]{bilinear} vector multiplication operation \( X \times X \to X \).
