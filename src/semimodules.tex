...
\begin{definition}\label{def:polynomial_semiring}\mcite[149]{Knapp2016BasicAlgebra}
  ...
  The \term{polynomial semiring} for the indeterminates \( \mscrX \) is the nested \hyperref[def:semimodule_direct_product]{direct product}
  \begin{equation*}
...
\begin{proposition}\label{thm:polynomial_semiring_universal_property}
  ...
  In the case of \( n \) indeterminates, this reduces to \( \Phi: R[X] \to \fun(M^n, M) \). We call the values of \( \Phi \) \term{polynomial functions}. Given values \( x_1, \ldots, x_n \) in \( M \), we write
  \begin{equation*}
    p(x_1, \ldots, x_n)
  \end{equation*}
  rather than
