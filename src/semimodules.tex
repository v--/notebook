...
\begin{definition}\label{def:subtractive_sub_semimodule}\mcite[66]{Golan2010}
  ...
  This definition also applies to more general subsets than sub-semimodules. Every \hyperref[def:module/submodel]{submodule} of a \hyperref[def:module]{module} is subtractive, so this definition will not be used much.
\end{definition}
...
\begin{definition}\label{def:semimodule_direct_product}
  The \term{direct product} of a family of \hyperref[def:semimodule]{semimodules} \( \seq{ M_k }_{k \in \mscrK} \) over the same ring is their \hyperref[def:monoid_direct_product]{monoid direct product} \( \prod_{k \in \mscrK} M_k \) with the additional componentwise scalar product
  \begin{equation*}
  ...
  As in the case of general monoids, the \term{direct sum} \( \bigoplus_{k \in \mscrK} M_k \) is the submonoid of the direct product consisting only of \hyperref[def:cartesian_product/tuple]{tuples} with \hyperref[def:set_finiteness]{finite} \hyperref[def:function_support]{support}. That is, in which only finitely many components of each tuple are nonzero.
\end{definition}
...
\end{proposition}
\begin{proof}
  Trivial refinement of \fullref{thm:monoid_categorical_limits}.
\end{proof}

\begin{definition}\label{def:free_semimodule}\mimprovised
  Fix a \hyperref[def:semiring]{semiring} \( R \). We associate with every set \( A \) its \term{free semimodule} \( F(A) \) defined as the set
  \begin{equation*}
    F(A) \coloneqq \set{ \tau: A \to R \given \tau \T{has finite \hyperref[def:function_support]{support}} }.
  \end{equation*}

  In the case when \( R \) is the semiring of natural numbers or, more generally, a semiring of cardinals, \( F(A) \) is the set of finite \hyperref[def:weighted_set/multiset]{multisets} over \( A \).
  ...
  \begin{equation*}
    ...
    \end{aligned}
  \end{equation*}
  where \( \iota_A \) is the canonical inclusion
  \begin{equation*}
    \begin{aligned}
      &\iota_A: A \to F(A), \\
...
\begin{proposition}\label{thm:free_semimodule_universal_property}
  Fix a semiring \( R \) and a set \( A \). The \hyperref[def:free_semimodule]{free semimodule} \( F(A) \) over \( R \) is the unique up to an isomorphism monoid that satisfies the following \hyperref[rem:universal_mapping_property]{universal mapping property}:
  \begin{displayquote}
    For every semimodule \( M \) over \( R \) and every function \( f: A \to M \), there exists a unique semimodule homomorphism \( \widetilde{f}: F(A) \to M \) such that the following diagram commutes:
