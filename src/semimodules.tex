...
\begin{definition}\label{def:polynomial_semiring}\mcite[149]{Knapp2016BasicAlgebra}
  ...
  \begin{equation*}
    \mscrX^\gamma \coloneqq \prod_{X \in \mscrX} X^{\gamma_X},
  \end{equation*}
  where \( \gamma_X \) are the coefficients in \( R^{\oplus \mscrX} \) of the monomial. The \term{degree} of the monomial is \( \norm{\gamma} \). In case \( \mscrX = \set{ X_1, \ldots, X_n } \), this reduces to the familiar notation
  \begin{equation*}
    X_1^{\gamma_1} X_2^{\gamma_2} \cdots X_n^{\gamma_n}.
  \end{equation*}
  ...
  \begin{equation*}
    p(\mscrX) = \sum_{k=0}^\infty \sum_{\norm{\gamma} = k} a_\gamma \mscrX^\gamma.
  \end{equation*}
  ...
  We do not ignore the structure of \( \mscrM \), but instead define multiplication in \( R[\mscrX] \) via the convolution
  \begin{equation*}
...
\begin{proof}
  ...
  \begin{equation*}
    p(\mscrX) = \sum_{k=0}^\infty \sum_{\norm{\gamma} \leq k} a_\gamma \prod_{X \in \mscrX} \iota(X)^{\gamma_X}
  \end{equation*}
  as the evaluation
  \begin{equation*}
    \Phi_f(p) \coloneqq \sum_{k=0}^\infty \sum_{\norm{\gamma} \leq k} a_\gamma \prod_{X \in \mscrX} f(X)^{\gamma_X}.
  \end{equation*}
...
\begin{proof}
  ...
  \begin{equation*}
    p(X, Y) = \sum_{k=0}^{n_X} \sum_{m=0}^{n_Y} a_{k,m} X^k Y^m.
  \end{equation*}
