...
\begin{remark}\label{rem:span_over_different_semirings}
  If \( M \) is both an \( R \)-semimodule and an \( S \)-semimodule, \fullref{thm:span_via_linear_combinations} highlights a fundamental difference between the generated \( R \)-subalgebra and the generated \( S \)-subalgebra.
\end{remark}
...
\begin{remark}\label{rem:adjoining_via_polynomials}
  Let \( R \subseteq S \) be \hyperref[def:semiring/commutative]{commutative semirings} and let \( p \in S[X_1, \ldots, X_n] \) be a \hyperref[def:polynomial_semiring]{polynomial} over \( S \).

  Let \( s_1, \ldots, s_n \) be arbitrary elements of \( S \). Consider the \hyperref[thm:polynomial_semiring_universal_property]{substitution homomorphism}
  \begin{equation*}
  ...
  The elements of this ring are the elements of \( S \) that can be obtained by evaluating polynomials over \( R \) with the values \( s_1, \ldots, s_n \). The obtained ring includes the elements of \( R \) because they correspond to constant polynomials.
...
\begin{proposition}\label{thm:generators_via_polynomials}
  ...
  When \( M = R \), the \( \BbbN \)-subalgebras correspond to \hyperref[def:semiring/submodel]{sub-semirings} and the \( R \)-subalgebras correspond to \hyperref[def:semiring_ideal/generated]{ideals}.
