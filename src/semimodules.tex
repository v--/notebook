...
\begin{example}\label{ex:def:polynomial_semiring}
  ...
  \begin{thmenum}
    ...
    \thmitem{ex:def:polynomial_semiring/tropical} Consider again the polynomial \( p(X) \coloneqq aX^2 + bX + c \), but this time over the \hyperref[ex:def:semiring/tropical]{tropical semiring} \( (\BbbN \cup \set{ \infty }, \min, +) \).
