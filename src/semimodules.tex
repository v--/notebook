...
\begin{definition}\label{def:free_semimodule}\mimprovised
  Fix a \hyperref[def:semiring]{semiring} \( R \). We associate with every set \( S \) its \term{free \( R \)-semimodule} \( R^{\bigoplus S} \) over \( R \). The free semimodule describes the formal \( R \)-\hyperref[def:linear_combination]{linear combinations} of elements of \( S \).

  We call \( S \) the \term{basis} of \( R \). As with general direct sums, we regard the function \( t \) as the indexed family \( \seq{ t_x }_{x \in S} \).

  By \fullref{thm:functions_over_model_form_model}, \( R^{\oplus S} \) inherits addition and multiplication from \( R \) and is actually a semiring. Scalar multiplication can be defined as
  \begin{equation*}
    \begin{aligned}
      &\cdot: R \times R^{\oplus S} \to R^{\oplus S}, \\
      &r \cdot \seq{ t_x }_{x \in S} \coloneqq \seq{ r \cdot t_x }_{x \in S}.
    \end{aligned}
  \end{equation*}
...
\begin{proposition}\label{thm:free_semimodule_universal_property}
  Fix a semiring \( R \) and a set \( S \). The \hyperref[def:free_semimodule]{free \( R \)-semimodule} \( R^{\oplus S} \) over \( R \) is the unique up to a unique isomorphism semimodule that satisfies the following \hyperref[rem:universal_mapping_property]{universal mapping property}:
  \begin{displayquote}
    For every semimodule \( M \) over \( R \) and every function \( f: S \to M \), there exists a unique \( R \)-semimodule homomorphism \( \widetilde{f}: R^{\oplus S} \to M \) such that the following diagram commutes:
...
\end{proposition}
\begin{proof}
  For every function \( f: S \to M \), we want
  \begin{equation*}
  ...
  \begin{equation*}
    \begin{aligned}
      &\widetilde{f}: R^{\oplus S} \to M, \\
      &\widetilde{f}(\seq{ t_x }_{x \in S}) \coloneqq \sum_{x \in S}^n t_x \cdot f(x).
    \end{aligned}
  \end{equation*}
...
\begin{definition}\label{def:algebra_over_semiring}\mimprovised
  ...
  \begin{thmenum}
    ...
    \thmitem{def:algebra_over_semiring/submodel} The set \( S \subseteq M \) is a \hyperref[thm:substructure_is_model]{submodel} of \( M \) if it is a \hyperref[def:semimodule/submodel]{sub-semimodule} of \( M \) that is closed under algebra multiplication. We say that \( S \) is a \term{subalgebra}.
...
\begin{proposition}\label{thm:functions_over_semimodule}
  For a set \( S \) and an \( R \)-\hyperref[def:semimodule]{semimodule} \( M \), the set of all functions from \( S \) to \( M \) is itself a semimodule with the following operations:
  \begin{thmenum}
  ...
  If \( S \) is also an \( R \)-semimodule, we denote this set of functions by \( \hom(S, R) \).
\end{proposition}
\begin{proof}
...
\begin{definition}\label{def:multi_index}\mimprovised
  A \term{multi-index} over the \hyperref[def:set]{plain set} \( S \) is a member of the \hyperref[def:free_semimodule]{free \( \BbbN \)-semimodule} \( S^{\oplus \BbbN} \) over \( S \). We endow \( S^{\oplus \BbbN} \) with the \hyperref[def:norm]{norm}
  \begin{equation*}
    \norm{ \alpha } \coloneqq \sum_{s \in S} \alpha_s
  \end{equation*}
  and the \hyperref[def:partially_ordered_set]{partial order}
  \begin{equation*}
    \alpha \leq \beta \T{if and only if} \qforall {s \in S} \alpha_s \leq \beta_s.
  \end{equation*}
...
\begin{definition}\label{def:polynomial_semiring}\mcite[149]{Knapp2016BasicAlgebra}
  ...
  \end{equation*}
  where \( \gamma_X \) are the coefficients in \( R^{\oplus \mscrX} \) of the monomial. The \term{degree} of the monomial is \( \norm{\gamma} \).

  The \term{polynomial semiring} \( R[\mscrX] \) for the given indeterminates is the \hyperref[def:free_semimodule]{free \( R \)-semimodule} over \( \mscrM \). That is, a polynomial \( p \in R[\mscrX] \) is an \( R \)-linear combination of monomials, and we denote polynomials by
  \begin{equation*}
...
\begin{proposition}\label{thm:polynomial_semiring_universal_property}
  ...
  The function \( f \) evaluates each indeterminate in \( R \), while \( \Phi_f \) substitutes this value in every polynomial. We call \( \Phi_f \) the \term{substitution homomorphism} corresponding to the \term{variable assignment} \( f \). We can parameterize this by the evaluation functions to obtain the functional evaluation homomorphism
  \begin{equation*}
    \begin{aligned}
  ...
  We call the values of \( \Phi \) \term{polynomial functions}. Given elements \( x_1, \ldots, x_n \) of \( A \), we write
  \begin{equation*}
    p(x_1, \ldots, x_n)
  \end{equation*}
  rather than
...
\begin{proposition}\label{thm:def:polynomial_semiring/properties}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:def:polynomial_semiring/properties/entire} If \( R \) is an \hyperref[def:divisibility/zero]{entire (commutative) semiring}, then any \hyperref[def:polynomial_semiring]{polynomial semiring} \( R[X_1, \ldots, X_n] \) in finitely many variables is also entire.
  \end{thmenum}
\end{proposition}
\begin{proof}
  ...
  \SubProofOf{thm:def:polynomial_semiring/properties/entire} The proof proceeds by induction on the number of indeterminates. We will only show the base case since the other cases reduce to it.
...
\begin{remark}\label{rem:adjoining_via_polynomials}
  ...
  This procedure is called \term{adjoining} \( x_1, \ldots, x_n \) to \( R \). It is the smallest semiring extension of \( R \) containing the adjoined elements.
\end{remark}
...
\begin{proposition}\label{thm:generators_via_polynomials}
  For a set \( S \) in an \( R \)-\hyperref[def:algebra_over_semiring]{algebra} \( A \), the \hyperref[def:algebra_over_semiring/submodel]{generated subalgebra} of \( S \), defined as the \( R \)-subalgebra generated by \( S \) in the sense of \fullref{def:first_order_generated_substructure}, equals the set
  \begin{equation*}
    \bigcup \set[\Big]{ R[a_1, \ldots, a_n] \given* a_1, \ldots, a_n \in S }
  \end{equation*}
  obtained by evaluating all multivariate polynomials over \( R \) with elements of \( S \).

  The \( \BbbN \)-subalgebras of \( A \) correspond to \hyperref[def:semiring/submodel]{sub-semirings} and the \( A \)-subalgebras correspond to \hyperref[def:semiring_ideal/generated]{ideals}.
