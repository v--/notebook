...
\begin{definition}\label{def:free_semimodule}\mimprovised
  ...
  By \fullref{thm:functions_over_model_form_model}, \( F(A) \) inherits addition and multiplication from \( R \) and is actually a (semi)ring. Scalar multiplication can be defined as
  \begin{equation*}
    \begin{aligned}
...
\begin{definition}\label{def:linear_combination}\mimprovised
  A \term{linear combination} over a \hyperref[def:semiring]{semiring} \( R \) is a finite sequence \( t_1, \ldots, t_n \) of scalars, which we call the \term{coefficients} of the combination. More abstractly, a linear combination is a member of the \hyperref[def:semimodule_direct_product]{direct sum} \( R^{\oplus \BbbN} \); as per our general convention from \fullref{rem:peano_arithmetic_zero}, however, we will start the sequence of coefficients with \( 1 \) rather than \( 0 \).
