...
\begin{definition}\label{def:algebra_over_semiring}\mimprovised
  ...
  We identify every element \( t \) of \( R \) with its canonical embedding \( t \cdot 1_M \) in \( M \), and thus we can also regard \( R \) as a subring of \( M \).
...
\begin{example}\label{ex:def:polynomial_semiring}
  ...
  \begin{thmenum}
    ...
    \thmitem{ex:def:polynomial_semiring/tropical} Consider again the polynomial \( p(X) \coloneqq aX^2 + bX + c \) over \( \BbbN \), but this time evaluated over the \hyperref[ex:def:semiring/tropical]{tropical semiring} \( (\BbbN \cup \set{ \infty }, \min, +) \).
