...
\begin{example}\label{ex:def:quiver_geometric_realization}
  ...
  \begin{thmenum}
    ...
    \begin{equation}\label{eq:ex:def:quiver_geometric_realization/k3/relization}
      \begin{aligned}
        \includegraphics{figures/eq__ex__def__graph_geometric_realization__k3__realization.pdf}
      \end{aligned}
    \end{equation}
    ...
    \begin{equation}\label{eq:ex:def:quiver_geometric_realization/k3/embedding}
      \begin{aligned}
        \includegraphics{figures/eq__ex__def__graph_geometric_realization__k3__embedding.pdf}
      \end{aligned}
    \end{equation}
...
\begin{proposition}\label{thm:quiver_geometric_realization_properties}
  ...
  \begin{thmenum}
    \thmitem{thm:quiver_geometric_realization_properties/isolated} A vertex \( v \in V \) is isolated in \( Q \), i.e. has degree zero, if and only if \( R(v) \) is an isolated point of \( \mscrX \).

    \thmitem{thm:quiver_geometric_realization_properties/t1} If \( Q \) is \hyperref[def:hypergraph/degree]{locally finite}, then the space \( (\mscrX, \mscrT) \) satisfies the \ref{def:separation_axioms/T1} separation axiom.
  \end{thmenum}
\end{proposition}
\begin{proof}
  \SubProofOf{thm:quiver_geometric_realization_properties/isolated} For every isolated vertex \( v \), the point \( R(v) = \set{ (v, v) } \) is isolated by definition.
  ...
  \SubProofOf{thm:quiver_geometric_realization_properties/t1} Let \( x \in \mscrX \).
  ...
  Both \( \set{ 0 } \) and \( \set{ 1 } \) are closed in \( [0, 1] \), hence \( \set{ 0, 1 } \) is also closed in \( [0, 1] \). Since \( Q \) is locally finite, \( R(v) \) is the union of finitely many closed sets and it itself closed.
\end{proof}
...
\begin{proposition}\label{thm:undirected_multigraph_geometric_realizations_homeomorphic}
  ...
  Let \( (\mscrX_c, \mscrT_c, R_c) \) and \( (\mscrX_d, \mscrT_d, R_d) \) be geometric realizations corresponding to the left adjoint to \( U \) functors \( D_c \) and \( D_d \) from \fullref{def:multigraph_adjunction}.
  ...
  \begin{equation*}
    \begin{aligned}
      ...
      &f(x) \coloneqq \begin{cases}
        \set[\Big]{ (0, a) \given h_d(a) = v } \cup \set[\Big]{ (1, a) \given t_d(a) = v }, &\qexists {v \in V} \deg(v) > 0 \T{and} x = R_c(v) \\
        \set[\Big]{ (e, 1 - t) },                                                           &\qexists {e \in E} x = \set{ (e, t) } \T{and} h_c(e) \neq h_d(e) \\
        x,                                                                                  &\T{otherwise.}
      \end{cases}
    \end{aligned}
  \end{equation*}
...
\begin{proposition}\label{thm:quiver_geometric_realization_paths}
  ...
  \begin{thmenum}
    \thmitem{thm:quiver_geometric_realization_paths/quiver_to_realization} If there exists an \hyperref[def:quiver_path/undirected]{undirected path} \( p = (e_1, \ldots, e_n) \) from \( s \) to \( f \), then there exists a \hyperref[def:parametric_curve]{continuous path} \( \gamma: [0, 1] \to \mscrX \) from \( R(s)  \) to \( R(f) \).
...
\begin{proof}
  ...
  \SubProofOf{thm:quiver_geometric_realization_paths/quiver_to_realization} Suppose that \( p = (e_1, \ldots, e_n) \) is an undirected path from \( s \) to \( f \). We will use \hyperref[rem:induction/well_founded]{strong induction} on the length of \( p \) to show that there exists a continuous path between the points \( R(s)  \) and \( R(f)  \) of \( \mscrX \).
  ...
  For each arc \( a \), the set \( R(a) \) is closed as a homeomorphic image of the unit interval \( [0, 1] \). From \fullref{thm:quiver_geometric_realization_properties/t1} it follows that \( \set{ R(v) } \) is a closed set for every vertex \( v \in V \). Therefore, \( \gamma^{-1}(\mscrX) \) is a union of disjoint closed sets.
