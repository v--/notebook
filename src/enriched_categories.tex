...
\begin{definition}\label{def:abelian_category}\cite{MacLane1994}[196]
  ...
  \begin{defenum}
    \item $\Bold C$ has a zero object $0$, i.e. $\Bold C$ is a pointed category\Tinyref{def:initial_final_objects}
    \item $\Bold C$ has all binary biproducts\Tinyref{def:categorical_biproduct}
    \item $\Bold C$ has a kernel and a cokernel for every morphism\Tinyref{def:categorical_kernel}; this requires $\Bold C$ to have all zero morphisms)
