...
\begin{definition}\label{def:convergent_power_series}
  Let \( \BbbK\DBracket{X} \) be the space of power series defined in \fullref{def:formal_power_series}.
  ...
  If the limit exists (as a \hyperref[def:convergent_series]{numeric series}) for a certain \( x \in \BbbK \), we say that the series \Def{converges} at \( x \).
