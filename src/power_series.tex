...
\begin{definition}\label{def:convergent_power_series}
  ...
  If the limit exists (as a numeric \hyperref[def:convergent_series]{series}) for a certain \( x \in \BK \), we say that the series \Def{converges} at \( x \).
...
\begin{proof}\mbox{}
  \begin{description}
    \RItem{thm:series_termwise_operations/differentiation} Note that the right-hand side of \fullref{thm:series_termwise_operations/derivative} is a power series. Furthermore, its radius of convergence is, by \fullref{thm:power_series_radius_of_convergence},
    \begin{equation*}
    ...
    \RItem{thm:series_termwise_operations/integration} Analogously to \fullref{thm:series_termwise_operations/differentiation}, we conclude that the right-hand side of \fullref{thm:series_termwise_operations/primitive} is a power series with radius of convergence \( R \).
