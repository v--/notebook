...
\begin{definition}[Metric space]\label{def:metric_space}\cite[248]{Engelking1989}
  ...
  \begin{defenum}[series=def:metric_space]
    \DItem{def:metric_space/identity}[M1](identity) \( \rho(x, y) = 0 \iff x = y \)
    \DItem{def:metric_space/symmetry}[M2](symmetry) \( \rho(x, y) = \rho(y, x) \)
    \DItem{def:metric_space/triangle_inequality}[M3](triangle inequality) \( \rho(x, y) \leq \rho(x, z) + \rho(z, y) \)
  \end{defenum}

  If instead of \fullref{def:metric_space/identity} we have the weaker condition
  \begin{defenum}[resume=def:metric_space]
    \DItem{def:metric_space/pseudometric_identity}[M1'](pseudometric identity) \( \forall x \in X, \rho(x, x) = 0 \),
  \end{defenum}
...
\end{proposition}
\begin{proof}
  The proof is the same as in \fullref{def:metric_topology}, except for a slight tweak in \fullref{thm:topological_local_base_axioms/BP3}, where we define \( m \) to be the the smallest positive integer such that
  \begin{equation*}
...
\begin{proof}\mbox{}
  \begin{description}
    ...
    \RItem{thm:metric_topology_properties/first_countable} Since \fullref{thm:locally_countable_metric_topology} involves generating a topology using a neighborhood system of countable local neighborhoods, \( \Cal{T} \) is first-countable.
...
\begin{proposition}\label{thm:metric_topology_convergence}
  Let \( (X, \rho) \) be a metric space and \( \Cal{T} \) be the induced metric topology\Tinyref{def:metric_topology}. The convergence\Tinyref{subsec:function_convergence} in \( \Cal{T} \) is the completely described by sequences, that is,

  \begin{defenum}
    \DItem{thm:metric_topology_convergence/single_limit} every converging net\Tinyref{def:topological_net} has a single limit.

    \DItem{thm:metric_topology_convergence/cluster_point} \( x \in X \) is a cluster point\Tinyref{def:net_convergence/cluster} of the sequence \( \{ x_i \}_{i=1}^\infty \) if and only if for every positive real number \( \varepsilon > 0 \) and every index \( i_0 \) there exists an index \( i_\varepsilon > i_0 \) such that
    \begin{equation*}
      \rho(x, x_{i_\varepsilon}) < \varepsilon.
    \end{equation*}

    \DItem{thm:metric_topology_convergence/limit_point} \( x \in X \) is a limit point\Tinyref{def:net_convergence/limit} of the sequence \( \{ x_i \}_{i=1}^\infty \) if and only if for every positive real number \( \varepsilon > 0 \) there exists an index \( i_0 \) such that
    \begin{equation*}
      i \geq i_0 \implies \rho(x, x_i) < \varepsilon.
    \end{equation*}

    \DItem{thm:metric_topology_convergence/closure} given any set \( A \subseteq X \), a point \( x \in X \) belongs to \( \Cl{A} \) if and only if there exists a sequence \( \{ x_i \}_{i=1}^\infty \subseteq A \) such that \( x_i \xrightarrow[i \to \infty]{} x \) (compare with \fullref{thm:limit_point_iff_in_closure}).
  \end{defenum}
\end{proposition}
\begin{proof}\mbox{}
  \begin{description}
    \RItem{thm:metric_topology_convergence/single_limit} Follows directly from \fullref{thm:t2_iff_singleton_limits} and \fullref{thm:metric_topology_properties/hausdorff}.

    \RItem{thm:metric_topology_convergence/cluster_point} Fix a sequence \( x_i \to x \).

    \begin{description}
      \Implies Fix a radius \( \varepsilon > 0 \) and an index \( i_0 \). Note that \( B(x, \varepsilon) \) is a neighborhood of \( x \) and hence by \fullref{def:net_convergence/cluster} there exists an index \( i_\varepsilon \geq i_0 \) such that \( x_{i_U} \in B(x, \varepsilon) \), which is the same as
      \begin{equation*}
    ...
    \RItem{thm:metric_topology_convergence/limit_point} Similar to \fullref{thm:metric_topology_convergence/cluster_point}.
    ...
    \begin{description}
      ...
      Fix \( \varepsilon > 0 \). By \fullref{thm:ordinals_are_well_ordered} there exists a least positive integer \( i_0 \) such that \( \tfrac 1 i_0 < \varepsilon \). It follows that \( x_i \to x \) since
      \begin{equation*}
...
\end{proposition}
\begin{proof}
  If\LEM \( f(x) = f(x') \), then by \fullref{def:metric_space/identity}, \( x = x' \).
\end{proof}

\begin{definition}\label{def:category_of_metric_spaces}
  We denote by \( \Cat{Met} \) the subcategory\Tinyref{def:subcategory} of \( \Bold{Unif} \)\Tinyref{def:category_of_uniform_spaces} where
  \begin{itemize}
...
\begin{proof}
  ...
  Thus \( U_i \) is the union of an at most countable amount of balls. The countable union of countable sets is again countable, hence by \fullref{def:topological_base/union}, \( \Cal{B} \) is a base for \( \Cal{T}_O \).
\end{proof}
...
\begin{definition}\label{def:complete_metric_space}
  ...
  An isometry \( f: X \to Y \) is called a \Def{completion} of \( X \) if \( Y \) is complete and \( f(X) \) is dense\Tinyref{def:topologically_dense_set/dense} in \( Y \) (see \fullref{thm:metric_space_completion_existence}).
\end{definition}

\begin{proposition}\label{thm:uniform_space_complete_iff_metric_space_complete}
  A metric space is complete in the sense of \fullref{def:complete_metric_space} if and only if it is complete as a uniform space in the sense of \fullref{def:complete_uniform_space}.
\end{proposition}

\begin{proposition}\label{thm:convergent_sequence_is_fundamental}
  In a complete metric space\Tinyref{def:complete_metric_space}, every convergent\Tinyref{def:net_convergence/limit}\Tinyref{thm:metric_topology_convergence/limit_point} sequence is fundamental\Tinyref{def:fundamental_sequence}.
\end{proposition}
\begin{proof}
...
\begin{corollary}[Cauchy's convergence criterion]\label{thm:cauchys_convergence_criterion}
  A sequence in a complete metric space is convergent\Tinyref{def:net_convergence/limit}\Tinyref{thm:metric_topology_convergence/limit_point} if and only if it is a fundamental sequence\Tinyref{def:fundamental_sequence}.

  Explicitly, a sequence \( \{ x_k \}_{k=1}^\infty \) in a metric space\Tinyref{def:metric_space} \( (X, \rho) \) is convergent\Tinyref{def:net_convergence/limit} if and only if for every \( \varepsilon > 0 \) there exists an index \( K \) such that
  \begin{equation*}
...
\begin{proof}\mbox{}
  \begin{description}
    \Implies Given by \fullref{thm:convergent_sequence_is_fundamental}
    \ImpliedBy Given by \fullref{def:complete_metric_space}
  \end{description}
\end{proof}

\begin{proposition}\label{thm:metric_local_convergence}
  ...
  \begin{propenum}
    \DItem{def:local_convergence/balls} For every \( \varepsilon > 0 \) there exists \( \delta > 0 \) such that for \( x \in D \) we have
    \begin{equation*}
      \rho_X(\Ol x, x) < \delta \implies \rho_Y(\Ol y, f(x)) < \varepsilon
    \end{equation*}
    or, equivalently,
    \begin{equation}\label{def:local_convergence/balls/inclusion}
      f(B(\Ol x, \delta) \cap D) \subseteq B(\Ol y, \varepsilon)
    \end{equation}

    \DItem{def:local_convergence/sequences} For every sequence \( \{ x_i \}_{i=1}^\infty \subseteq D \) that converges to \( \Ol x \), the corresponding sequence \( \{ f(x_i) \}_{i=1}^\infty \) converges to \( \Ol y \).
  \end{propenum}
\end{proposition}
\begin{proof}
  It is obvious that \fullref{def:local_convergence/balls} is a special case of \fullref{def:local_convergence} and by \fullref{def:metric_topology}, they are equivalent.

  \Implies[def:local_convergence/balls][def:local_convergence/sequences] Fix \( \varepsilon > 0 \) and suppose that for some \( \delta > 0 \) \fullref{def:local_convergence/balls/inclusion} holds. Let \( \{ x_i \}_{i=1}^\infty \subseteq D \) be a sequence that converges to \( \Ol x \). Thus, for some index \( i_0 \), we have
  \begin{equation*}
    i \geq i_0 \implies f(x_i) \subseteq B(\Ol y, \varepsilon).
  \end{equation*}

   Since \( \varepsilon \) was arbitrary, \fullref{thm:metric_topology_convergence/limit_point} allows us to conclude that \( \{ f(x_i) \}_{i=1}^\infty \) converges to \( \Ol y \).

   \Implies[def:local_convergence/sequences][def:local_convergence/balls] Suppose that for every sequence \( \{ x_i \}_{i=1}^\infty \subseteq D \) that converges to \( \Ol x \), the corresponding sequence \( \{ f(x_i) \}_{i=1}^\infty \) converges to \( \Ol y \).

   Fix \( \varepsilon > 0 \) and suppose that \fullref{def:local_convergence/balls/inclusion} does not hold for any \( \delta > 0 \).
   ...
   Then the sequence \( \{ x_i \}_{i=1}^\infty \) converges to \( \Ol x \) by \fullref{thm:metric_topology_convergence/limit_point}, but \(\{ f(x_i) \}_{i=1}^\infty \) does not converge to \( \Ol y \). This contradicts our assumption \fullref{def:local_convergence/sequences}.

   The obtained contradiction proves the theorem.
\end{proof}

\begin{theorem}\label{thm:metric_space_continuity}
  A function \( f: X \to Y \) between metric spaces is continuous\Tinyref{def:global_continuity} at \( \Ol x \in X \) if and only if any of the following conditions hold
  \begin{thmenum}
...
\begin{proof}\mbox{}
  \begin{description}
    \RItem{thm:metric_space_continuity/balls} By \fullref{def:metric_topology}, the balls in \( X \) and \( Y \) form a neighborhood \Tinyref{def:topological_local_base} for the metric topology. Then by \fullref{def:global_continuity/limits}, the continuity of \( f \) is equivalent to \fullref{thm:metric_space_continuity/balls}.

    \RItem{thm:metric_space_continuity/sequences} By \fullref{thm:metric_topology_convergence/closure}, this condition is equivalent to \fullref{def:global_continuity/closure}.
  \end{description}
\end{proof}

\begin{theorem}[Cantor's nested compact theorem]\label{thm:cantors_nested_compact_theorem}
  A descending sequence of nonempty compact sets \( F_1 \supseteq F_2 \supseteq \ldots \) in a complete metric space such that \( \Diam(F_i) \to 0 \) intersects at exactly one point (compare with \fullref{thm:noncompact_kuratowski_lemma}).
\end{theorem}
\begin{proof}
  ...
  The point \( x \) is contained in every set \( F_i, i = 1, 2, \ldots \) since all of the sets \( F_i \) are closed (by \fullref{thm:complete_metric_space_compact_conditions}) and contain their limit points\Tinyref{thm:limit_point_iff_in_closure}. Thus
  \begin{equation*}
...
\begin{proof}
  ...
  \begin{defenum}
    ...
    Thus the sequence \( \{ \rho(x_i, y_i) \}_{i=1}^\infty \) is fundamental and, by \fullref{def:real_numbers_complete_metric_space}, it is convergent.
