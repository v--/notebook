...
\begin{definition}\label{def:tree}
  The possibly infinite \hyperref[def:undirected_graph]{simple undirected graph} \( T: E \multto V \) is called a \term{tree} if any of the following equivalent conditions hold:
  \begin{thmenum}
...
\begin{definition}\label{def:arborescence}\mcite[ch. 4, sec. 3.1]{GondranMinoux1984Graphs}
  ...
  \begin{thmenum}
    ...
    For every node \( v \) of \( T \), we define the \term{induced sub-arborescence} of \( T \) as the subgraph \hyperref[def:hypergraph/submodel]{induced} by \( V \setminus \dom(p) \), where \( p \) is the unique path from \( r \) to \( v \).
  \end{thmenum}
\end{definition}
