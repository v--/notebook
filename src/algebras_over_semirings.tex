...
\begin{proposition}\label{thm:functions_over_semimodule}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:functions_over_semimodule/multiplication} If \( N \) is an \hyperref[def:algebra_over_semiring]{algebra} over \( R \), for example if \( N = R \), we can also define pointwise multiplication, which will make the set of functions an algebra over \( R \)
    \begin{equation*}
...
\begin{theorem}[Polynomial algebra universal property]\label{thm:polynomial_semiring_universal_property}
  ...
  \begin{displayquote}
    ...
    \begin{equation}\label{eq:thm:polynomial_semiring_universal_property/diagram}
      \begin{aligned}
...
\begin{remark}\label{rem:polynomials_over_infinitely_many_indeterminates}
  As we saw in \fullref{def:polynomial_algebra} and \fullref{thm:polynomial_semiring_universal_property}, there is no formal problem in defining polynomial algebras over infinitely many indeterminates.
