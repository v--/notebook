\subsection{Euclidean domains}\label{subsec:euclidean_domains}

\begin{definition}\label{def:commutative_ring_division}
  Let \( R \) be a commutative ring\Tinyref{def:semiring/commutative_ring}. Let \( a, b \in R \). We say that \( b \) is a \Def{divisor} of \( a \) or that \( a \) is a \Def{factor} of \( b \) and write \( b \mid a \) if there exists \( c \in R \) such that \( a = bc \).

  We say that \( b \) is a \Def{trivial} divisor of \( a \) if \( a = bc \) and either \( b \) or \( c \) is invertible.

  The \Def{greatest common denominator} of \( a \) and \( b \) is defined, if it exists, as
  \begin{equation*}
...
\begin{definition}\label{def:euclidean_domain}\cite{nLab:euclidean_domain}
  Let \( R \) be an integral domain\Tinyref{def:semiring/integral_domain}. Multiplication is not invertible in integral, but we can instead define \Def{Euclidean division} with remainders.
...
\begin{algorithm}\label{alg:euclidean_algorithm}
  ...
  \begin{algenum}
    ...
    \DItem{alg:euclidean_algorithm/step} Starting with \( i = 2 \), obtain \( q_i \) and \( r_i \) from division\Tinyref{def:euclidean_domain}
    \begin{equation*}
...
\begin{proposition}\label{thm:euclidean_domain_is_pid}
  Every Euclidean domain\Tinyref{def:euclidean_domain} is a principal ideal domain\Tinyref{def:semiring/principal_ideal_domain}.
\end{proposition}
\begin{proof}
