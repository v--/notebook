...
\begin{definition}\label{def:complex_numbers}
  We give a few equivalent definition of the \hyperref[def:field]{field} \( \BbbC \) \Def{complex numbers}. Informally, there are numbers of the form \( a + bi \), where \( a, b \in \BbbR \) and \( i = \sqrt{-1} \). In order to find the multiplicative inverse of the nonzero polynomial \( a + bi \), we assume that division is well defined and proceed as follows:
  ...
  \begin{DefEnum}
    ...
    \begin{equation*}
      \BbbC \coloneqq \BbbR[X] / \Bracket{X^2 + 1}.
    \end{equation*}
    ...
    \ILabel{def:complex_numbers/tuples} Finally, we can define \( \BbbC \) is the \hyperref[def:algebra_over_ring]{algebra} obtained from the vector space \( \BbbR^2 \) with the multiplication operation emulating \fullref{def:complex_numbers/polynomials/multiplication} as
  ...
  We define the unary \Def{complex conjugation} operation as \( \overline{a + bi} \coloneqq a - bi \) and the \Def{\hyperref[def:absolute_value]{absolute value}} as
  \begin{equation*}
