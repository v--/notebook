...
\begin{definition}\label{def:complex_numbers}
  ...
  \begin{thmenum}
    \thmitem{def:complex_numbers/polynomials} The most \enquote{algebraic} way to define complex numbers is as the \hyperref[def:polynomial]{polynomial} quotient \hyperref[thm:polynomial_quotient_rings_equinumerous_with_module_of_polynomials]{ring}
    \begin{equation*}
    ...
    \thmitem{def:complex_numbers/matrices} The complex numbers can also be defined as the matrix \hyperref[def:algebra_of_matrices]{ring}
    \begin{equation*}
      \BbbC \coloneqq \left\{
      \begin{pmatrix}
        a  & b \\
    ...
    \thmitem{def:complex_numbers/tuples} Finally, we can define \( \BbbC \) is the \hyperref[def:algebra_over_ring]{algebra} obtained from the vector space \( \BbbR^2 \) with the multiplication operation emulating \fullref{def:complex_numbers/polynomials/multiplication} as
    \begin{balign*}
