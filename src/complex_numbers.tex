...
\begin{definition}\label{def:complex_numbers}
  ...
  \begin{defenum}
    ...
    \DItem{def:complex_numbers/tuples} Finally, we can define \( \BC \) is the algebra\Tinyref{def:algebra_over_ring} obtained from the vector space \( \BR^2 \) with the multiplication operation emulating \fullref{def:complex_numbers/polynomials/multiplication} as
    \begin{align*}
...
\begin{remark}\label{remark:linear_functionals_over_c}
  \Fullref{thm:linear_functionals_over_c} allows us to identify the dual space \( X* \) of a complex vector space \( X \) with \( \Hom(X, \BR) \) in the case of an algebraic dual\Tinyref{def:dual_vector_space} or with the corresponding subspace in the case of a continuous dual space\Tinyref{def:continuous_dual_space}.
