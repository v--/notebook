...
\begin{proposition}\label{thm:def:polynomial_degree/properties}
  ...
  \begin{thmenum}
    ...
    An easy sufficient condition for \eqref{eq:thm:def:polynomial_degree/properties/product} is for the ring to be \hyperref[def:entire_semiring]{entire}, although it is sufficient for the ring to be nontrivial (so that \( 0_R \neq 1_R \)) and either \( p(X) \) or \( q(X) \) to be \hyperref[def:monic_polynomial]{monic}.
  \end{thmenum}
\end{proposition}
\begin{proof}
