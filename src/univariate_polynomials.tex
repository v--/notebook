...
\begin{proposition}\label{thm:def:polynomial_degree/properties}
  ...
  \begin{thmenum}
    ...
    An easy sufficient condition for \eqref{eq:thm:def:polynomial_degree/properties/product} is for the ring to be \hyperref[def:entire_semiring]{entire}, although it is also sufficient for the ring to be nontrivial (so that \( 0_R \neq 1_R \)) and either \( p(X) \) or \( q(X) \) to be \hyperref[def:monic_polynomial]{monic}.
  \end{thmenum}
\end{proposition}
\begin{proof}
...
\begin{algorithm}[Euclidean division of polynomials]\label{thm:euclidean_division_of_polynomials}\mcite[prop. 1.12]{Knapp2016BasicAlgebra}
  ...
  \begin{thmenum}
    \thmitem{thm:euclidean_division_of_polynomials/zero_degree} If \( \deg f = \deg g = 0 \), necessarily \( g(X) = 1_R \), and in this case we define
    \begin{align*}
      q(X) &\coloneqq f(X), \\
      r(X) &\coloneqq 0_R.
    \end{align*}

    \thmitem{thm:euclidean_division_of_polynomials/no_division} If \( f(X) \) is the zero polynomial or \( \deg f < \deg g \), define
    \begin{align*}
      q(X) &\coloneqq 0_R, \\
      r(X) &\coloneqq f(X).
    \end{align*}
    ...
    \thmitem{thm:euclidean_division_of_polynomials/positive_degree} Suppose that
    \begin{align*}
      f(X) = a_n X^n + \hat f(X), \\
      g(X) = X^m + \hat g(X),
    \end{align*}
...
\begin{proposition}\label{thm:def:algebraic_derivative/properties}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:def:algebraic_derivative/properties/degree} \( p^{(n)}(X) \) is either zero or has degree \( \deg p - n \).

    \thmitem{thm:def:algebraic_derivative/properties/product} We have the product rule
    \begin{equation}\label{eq:thm:def:algebraic_derivative/properties/product}
      (pq)' = p'q + pq'.
    \end{equation}

    \thmitem{thm:def:algebraic_derivative/properties/leibniz} Leibniz' rule holds:
    \begin{equation}\label{thm:def:algebraic_derivative/properties/product}
      (pq)^{(n)} = \sum_{k=0}^n \binom n k p^{(k)} q^{(n-k)}
    \end{equation}

    \thmitem{thm:def:algebraic_derivative/properties/affine_power} If \( m \leq n \), the \( m \)-th derivative of \( (X - u)^n \) is \( \tfrac {n!} {(n-m)!} (X - u)^{n-m} \).
  \end{thmenum}
\end{proposition}
\begin{proof}
  \SubProofOf{thm:def:algebraic_derivative/properties/linear} Trivial.

  \SubProofOf{thm:def:algebraic_derivative/properties/degree} Trivial.
  ...
  \SubProofOf{thm:def:algebraic_derivative/properties/leibniz} The proof in \fullref{thm:leibniz_rule} relies only on the product rule, hence it holds here as well.

  \SubProofOf{thm:def:algebraic_derivative/properties/affine_power} We use outer induction on \( m \) and inner induction on \( n \).
  ...
  \begin{equation*}
    ...
    \reloset {\eqref{eq:thm:def:algebraic_derivative/properties/product}} =
...
\begin{defproof}
  ...
  For the \( n < m \)-th derivative of \( p(X) \), by \fullref{thm:def:algebraic_derivative/properties/leibniz}, we have
  \begin{equation*}
    p^{(n)}(X) = \sum_{k=0}^n \binom n k \underbrace{\parens[\Big]{ (X - u)^m }^{(k)}}_{\mathclap{\frac {m!} {k!} (X - u)^{m-k} \T*{by} \ref{thm:def:algebraic_derivative/properties/affine_power}}} q^{(n-k)}(X).
  \end{equation*}
  ...
  By \fullref{thm:def:algebraic_derivative/properties/leibniz},
  \begin{equation*}
    p^{(m-1)}(X) = \sum_{k=0}^{m-1} \binom {m-1} k \underbrace{\parens[\Big]{ (X - u)^{m - 1} }^{(k)}}_{\mathclap{\frac {(m - 1)!} {k!} (X - u)^{m - k - 1} \T*{by} \ref{thm:def:algebraic_derivative/properties/affine_power}}} q^{(m - k - 1)}(X).
  \end{equation*}
  ...
  We use \fullref{thm:euclidean_division_of_polynomials} to obtain a polynomial \( s(X) \) and a constant polynomial \( r(X) = r_0 \) so that
  \begin{equation*}
    q(X) = (X - u) s(X) + r_0.
  \end{equation*}
...
\begin{proposition}\label{thm:adjoining_roots}
  Fix arbitrary commutative rings \( R \subseteq S \) and a polynomial \( p(X) \in R[X] \). If \( u \) is a \hyperref[def:polynomial_root_multiplicity]{root} of \( p(X) \) in \( S \), meaning that the \hyperref[thm:polynomial_semiring_universal_property]{evaluation homomorphism} \( \Phi_u: R[X] \to S \) sends \( p(X) \) to \( 0_S \), then we have the ring isomorphism
  \begin{equation*}
...
\begin{proof}
  ...
  \begin{equation*}
    \begin{aligned}
      &\varphi: R[X] / \braket{ p(X) } \to R[u] \\
      &\varphi\parens[\Big]{ f(X) + q(X) p(X) } \coloneqq \Phi_u(f).
    \end{aligned}
  \end{equation*}

  It is well-defined because all multiples of \( p(X) \) are in the kernel of \( \Phi_u \). This homomorphism is injective as a consequence of \fullref{thm:representatives_in_univariate_polynomial_quotient_set}. We must show that it is surjective.

  Let
  \begin{equation*}
    y = \sum_{k=0}^n a_k u^k
  \end{equation*}
  be some element of \( R[u] \). Then the polynomial
  \begin{equation*}
    p(X) \coloneqq \sum_{k=0}^n a_k X^k
  \end{equation*}
  is such that \( \varphi(p) = y \).

  Therefore, \( \varphi \) is an isomorphism.
\end{proof}

\begin{example}\label{ex:gaussian_integers}
  The \term{Gaussian integers} are complex numbers \( z = a + bi \) with integer real and imaginary components. We can define several isomorphic rings for the Gaussian integers, demonstrating \fullref{thm:adjoining_roots}.
...
\begin{example}\label{ex:integers_with_sqrt2}
  Similarly to how the Gaussian integers were defined in multiple ways in \fullref{ex:gaussian_integers}, \fullref{thm:adjoining_roots} gives us an isomorphism
  \begin{equation*}
