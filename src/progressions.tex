...
\begin{proposition}\label{thm:def:geometric_series/properties}
  The geometric series \eqref{eq:def:geometric_progression/series} has the following basic properties:
  \begin{thmenum}
    \thmitem{thm:def:geometric_series/properties/finite_sum} For all \( q \in \BbbC \setminus \{ 1 \} \), the geometric series \eqref{eq:def:geometric_progression/series} has partial sums
    ...
    \thmitem{thm:def:geometric_series/properties/degenerate} In the degenerate case \( q = 1 \), the progression itself is constant, and its partial sums are instead
    \begin{equation}\label{thm:geometric_progression/degenerate}
      \sum_{k=0}^n q^k = n + 1.
    \end{equation}

    \thmitem{thm:def:geometric_series/properties/series_sum_exterior} For \( \abs{q} \geq 1 \), the geometric series diverges.

    \thmitem{thm:def:geometric_series/properties/series_sum_interior} For \( 0 < \abs{q} < 1 \), the geometric series converges absolutely with sum
...
\end{proposition}
\begin{proof}
  \SubProofOf{thm:def:geometric_series/properties/finite_sum} Follows from \fullref{thm:xn_minus_one_factorization}.
  \SubProofOf{thm:def:geometric_series/properties/degenerate} Obvious.

  \SubProofOf{thm:def:geometric_series/properties/series_sum_exterior} For \( q = 1 \), \fullref{thm:def:geometric_series/properties/degenerate} implies that the series diverges because it grows indefinitely. If \( \abs{q} = 1 \) and \( q \neq 1 \), the integer powers \( q^k \) are rotations around the complex plane unit circle, which do not tend to a limit. Hence, the series diverges again.
  ...
  \SubProofOf{thm:def:geometric_series/properties/series_sum_interior} Fix \( q \in B(0, 1) \). Since only \( q^{n + 1} \) depends on \( n \) in \eqref{thm:geometric_progression/partial_sum}, we obtain \eqref{thm:geometric_progression/series_sum} by simply noting that \( q^n \to 0 \) when \( n \to \infty \).
\end{proof}

\begin{example}\label{ex:n_ary_decomposition}
  A simple but important practical example of a \hyperref[thm:def:geometric_series/properties/series]{geometric series} is
