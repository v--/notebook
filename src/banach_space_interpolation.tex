...
\begin{definition}\label{def:interpolated_topological_vector_space}\MarginCite[24]{Bergh1976}
  ...
  \begin{DefEnum}
    \ILabel{def:interpolated_topological_vector_space/compatibility} We say that two \hyperref[def:topological_vector_space]{topological vector spaces} \( \mscrX_0 \) and \( \mscrX_1 \) are \Def{compatible} if they can both be \hyperref[def:morphism_invertibility/monomorphism]{embedded} \hyperref[def:global_continuity]{continuously} into a \hyperref[def:separation_axioms/T2]{Hausdorff} topological vector space \( \mscrU \), in which case we can regard them as subspaces of \( \mscrU \).
    ...
    \end{equation*}
    the \Def{intersection} of \( \mscrX_0 \) and \( \mscrX_1 \) (when regarded as subspaces of \( \mscrU \)).

    \ILabel{def:interpolated_topological_vector_space/sum} Denote by
    \begin{equation*}
      \Sigma \overline{\mscrX} \coloneqq ( \mscrX_0 + \mscrX_1 )
    \end{equation*}
    the \Def{sum} of \( \mscrX_0 \) and \( \mscrX_1 \). If \( x \in \Sigma \overline{\mscrX} \), then there exist (possibly nonunique) vectors \( x_0 \in \mscrX_0 \) and \( x_1 \in \mscrX_1 \) such that \( x = x_0 + x_1 \).

    \ILabel{def:interpolated_topological_vector_space/intermediate_space} Let \( \overline{\mscrX} \) be a pair of compatible spaces. We say that the space \( \mscrX \) is an \Def{intermediate} space for \( \overline{\mscrX} \) if \( \Delta \overline{\mscrX} \subseteq \mscrX \subseteq \Sigma \overline{\mscrX} \) with continuous linear inclusions.

    \ILabel{def:interpolated_topological_vector_space/morphisms} We introduce \hyperref[def:category/C2]{morphisms} between two compatible pairs \( \overline{\mscrX} \) and \( \overline{\mscrY} \) that are, strictly speaking, not \hyperref[def:function]{functions} between the pairs themselves. We define an \Def{operator} \( T: \overline{\mscrX} \to \overline{\mscrY} \) between compatible pairs to be a function \( T \) from \( \Sigma \overline{\mscrX} \) to \( \Sigma \overline{\mscrY} \) that satisfies the additional conditions
    \begin{align*}
    ...
    \begin{RefList}
      ...
      \IRef{def:category/C2} The morphisms between two compatible pairs are the \hyperref[def:interpolated_topological_vector_space/morphisms]{continuous linear operators} \( T: \overline{\mscrX} \to \overline{\mscrY} \) between them.
      \IRef{def:category/C3} Composition of morphisms is the usual \hyperref[def:function/composition]{function composition} if we regard a morphism \( T: \overline{\mscrX} \to \overline{\mscrY} \) as a function from \( \Sigma \overline{\mscrX} \) to \( \Sigma \overline{\mscrY} \).
    \end{RefList}

    \ILabel{def:interpolated_topological_vector_space/interpolation_space} We say that the intermediate spaces \( \mscrX \) for \( \overline{\mscrX} \) and \( \mscrY \) for \( \overline{\mscrY} \) are a pair of \Def{interpolation spaces} with respect to \( \overline{\mscrX} \) and \( \overline{\mscrY} \) if, for any continuous linear \hyperref[def:interpolated_topological_vector_space/morphisms]{operator} \( T: \overline{\mscrX} \to \overline{\mscrY} \) between the compatible pairs, we have \( T(\mscrX) \subseteq \mscrY \).
  \end{DefEnum}
\end{definition}
...
\begin{proposition}\label{def:banach_space_sum_and_intersection_norms}\MarginCite[24]{Bergh1976}
  ...
  \begin{PropEnum}
    \ILabel{def:banach_space_sum_and_intersection_norms/intersection} The intersection \( \Delta \overline{\mscrX} = \mscrX_0 \cap \mscrX_1 \) is a Banach space with norm
    ...
    \ILabel{def:banach_space_sum_and_intersection_norms/sum} The sum \( \Sigma \overline{\mscrX} = \mscrX_0 + \mscrX_1 \) is a Banach space with norm
...
\end{proposition}
\begin{proof}
  \SubProofOf{def:banach_space_sum_and_intersection_norms/intersection} We will first show that \( \Norm{x}_{\Delta \overline{\mscrX}} \) is indeed a norm.
  \begin{RefList}
    ...
    \begin{BreakableAlign*}
      \Norm{x + y}_{\Delta \overline{\mscrX}}
      &=
      \max \Set{ \Norm{x + y}_{\mscrX_0}, \Norm{x + y}_{\mscrX_1} }
      \overset {\ref{def:norm/N3}} \leq \\ &\leq
      \max \Set{ \Norm{x}_{\mscrX_0} + \Norm{y}_{\mscrX_0}, \Norm{x}_{\mscrX_1} + \Norm{y}_{\mscrX_1} }
      \overset {\ref{eq:thm:preordered_magma_max_distributivity}} \leq \\ &\leq
      \max \Set{ \Norm{x}_{\mscrX_0}, \Norm{x}_{\mscrX_1} } + \max \Set{ \Norm{y}_{\mscrX_0}, \Norm{y}_{\mscrX_1} }
      = \\ &=
      \Norm{x}_{\Delta \overline{\mscrX}} + \Norm{y}_{\Delta \overline{\mscrX}}.
    \end{BreakableAlign*}
  \end{RefList}
  ...
  \SubProofOf{def:banach_space_sum_and_intersection_norms/sum} Again, we will first show that \( \Norm{x}_{\Sigma \overline{\mscrX}} \) is indeed a norm.
  \begin{RefList}
  ...
  \begin{BreakableAlign*}
    &\phantom{{}={}}
    \Norm{\left( x_0^{(k)} + x_1^{(k)} \right) - \left( \xi_0 + \xi_1 \right)}_{\Sigma \overline{\mscrX}}
    = \\ &=
    \inf \Set{ \Norm{x_0}_{\mscrX_0} + \Norm{x_1}_{\mscrX_1} : x_0 + x_1 = \left( x_0^{(k)} + x_1^{(k)} \right) - \left( \xi_0 + \xi_1 \right) }
    \leq \\ &\leq
    \Norm{x_0^{(k)} - \xi_0}_{\mscrX_0} + \Norm{x_1^{(k)} - \xi_1}_{\mscrX_1}
    <
...
\begin{definition}\label{def:lebesgue_space}\cite[6]{Bergh1976}
  Let \( \mu: U \to [0, \infty] \) be a positive measure and \( p \) be a positive real number. The \Def{Lebesgue space} \( L_p \) is defined as the set of bounded functions \( f: U \mapsto \BbbK \) such that the norm
  \begin{equation*}
...
\begin{definition}\label{def:distribution_function}\cite[6]{Bergh1976}
  ...
  \begin{DefEnum}
    \ILabel{def:distribution_function/distribution_function} Given a scalar-valued function \( f: U \mapsto \BbbK \), we define its \Def{distribution function} as
    \begin{align*}
      &m_f: [0, \infty] \to \BbbK \\
    ...
    \ILabel{def:distribution_function/lorenz_space} The \( (p, q)-\)Lorenz space, for potentially infinite positive \( q > 0 \), is the set of functions \( f: U \mapsto \BbbK \) for which the quasinorm
    \begin{equation*}
...
\begin{definition}\label{def:banach_interpolation_space_exponent}\MarginCite[27]{Bergh1976}
  ...
  \end{equation}
  holds for some constant \( C > 0 \) and \( \theta \in [0, 1] \), we say that the pair \( (\mscrX, \mscrY) \) are \Def{interpolation spaces of exponent} \( \theta \).

  If, additionally, \( C = 1 \), we say that \( (\mscrX, \mscrY) \) is an \Def{exact pair} of interpolation spaces.
\end{definition}

\begin{definition}\label{def:k_functional}\MarginCite[38]{Bergh1976}
  Let \( \overline{\mscrX} \coloneqq ( \mscrX_0, \mscrX_1 ) \) be a compatible pair of Banach spaces. Instead of the norm \( \Norm{\cdot}_{\mscrX_1} \) in \( \mscrX_1 \), we can consider \hyperref[def:equivalent_metrics]{equivalent norms} of the type \( t\Norm{\cdot}_{\mscrX_1} \) for \( t \geq 0 \). Furthermore, we can also introduce equivalent norms in \( \Sigma \overline{\mscrX} \) via the \Def{\( K \)-functional}
  \begin{AlignedEquation}\label{eq:def:k_functional}
    &K: (0, \infty) \times {\Sigma \overline{\mscrX}} \\
    &K(t, x) \coloneqq \inf \Set{ \Norm{x_0}_{\mscrX_0} + t\Norm{x_1}_{\mscrX_1} : x_0 + x_1 = x }.
  \end{AlignedEquation}
...
\begin{proposition}\label{def:k_functional_properties}\MarginCite[38]{Bergh1976}
  ...
  \begin{PropEnum}
    \ILabel{def:k_functional_properties/basic} For any fixed \( x \in \Sigma \overline{\mscrX} \), the function \( t \mapsto K(t, x) \) is positive, \hyperref[def:monotone_map]{monotone} and \hyperref[def:convex_functions]{concave}.
    ...
    \ILabel{def:k_functional_properties/equivalent_norm} For any fixed \( t > 0 \), the function \( x \mapsto K(t, x) \) is an \hyperref[def:equivalent_metrics]{equivalent norm} in the sum \( \Sigma \overline{\mscrX} \).
  \end{PropEnum}
\end{proposition}
\begin{proof}
...
\end{theorem}
\begin{proof}
  Note that \( K(s, x) \) is a norm on \( \Sigma \overline{X} \) by \fullref{def:k_functional_properties/equivalent_norm}. Therefore \( \Norm{\cdot}_{\theta,q,K} \), the composition of \( K(s, x) \) with the \hyperref[def:lorenz_quasinorm]{Lorenz quasinorm} \( \Phi_{\theta,q} \), is a norm.
...
\begin{definition}\label{def:e_functional}\MarginCite[174]{Bergh1976}
  ...
  \begin{AlignedEquation}\label{eq:def:e_functional}
    ...
    &E(t, x) \coloneqq \inf \Set{ \Norm{x - x_0}_{\mscrX_1} : \Norm{x_0}_{\mscrX_0} \leq t }.
  \end{AlignedEquation}
\end{definition}
...
\begin{proposition}\label{thm:e_functional_properties}\MarginCite[lemma 7.1.3]{Bergh1976}
  ...
  \begin{PropEnum}
    \ILabel{def:k_functional_properties/decreasing} For fixed \( x \in \Sigma\overline{\mscrX} \), the function \( t \mapsto E(t, x) \) is decreasing.
...
\begin{definition}\label{def:approximation_space}\MarginCite[def. 7.1.5]{Bergh1976}
  Let \( \overline{\mscrX} = (\mscrX_0, \mscrX_1) \) be a compatible pair of Banach spaces. We define an \Def{approximation space} \( E_{\alpha,q}(\overline{\mscrX}) \) for \( x \in \Sigma\overline{\mscrX} \) as the space of all members of \( \Sigma\overline{\mscrX} \) for which the following norm
