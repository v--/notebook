...
\begin{theorem}\label{thm:k_functional_interpolation}\MarginCite[thm. 3.1.2]{Bergh1976}
  Let \( \theta \in (0, 1) \) and \( q \in (0, \infty] \). The space \( \CX_{\theta,q,K} \) defined in \fullref{eq:def:k_functional_interpolation_space/norm} is an \hyperref[thm:banach_interpolation_space_exponent]{exact interpolation space} of exponent \( \theta \). Furthermore,
...
\begin{proof}
  ...
  Thus \( \CX \) satisfies \fullref{thm:banach_interpolation_space_exponent} for being an exact interpolating space with exponent \( \theta \).
\end{proof}
...
\begin{definition}\label{def:approximation_space}\MarginCite[def. 7.1.5]{Bergh1976}
  Let \( \Ol{\CX} = (\CX_0, \CX_1) \) be a compatible pair of Banach spaces. We define the \Def{approximation space} for \( x \in \Sigma\Ol{\CX} \) as the space of all members of \( \Sigma\Ol{\CX} \) for which the following norm
...
\begin{theorem}\label{thm:interpolation_space_and_approximation_space}\MarginCite[thm. 7.1.8]{Bergh1976}
