...
\begin{definition}\label{def:parametric_curve_length}
  ...
  \begin{itemize}
    ...
    \item Use the partition \( c \) to build linear approximation to \( \gamma \) using the \hyperref[def:line_segment]{line segments} \( [\gamma(c_{k-1}), \gamma(c_k)], k = 1, \ldots, n \).
...
\begin{proposition}\label{thm:length_of_piecewise_smooth_curves}
  For a piecewise linear parametric curve, its length is the sum of the lengths of the line segments.
\end{proposition}
...
\begin{corollary}\label{thm:length_of_function_graph}
  ...
  \begin{equation*}
    \len(\gph(f)) \coloneqq \int_a^b \sqrt{1 + [f'(x)]^2} \diff x.
  \end{equation*}
\end{corollary}
\begin{proof}
