...
\begin{definition}\label{def:riemann_partition}\mcite[def. 1]{Gordon1991}
  The concept of a partition of a nonempty \hyperref[def:real_numbers]{real} \hyperref[def:poset_interval/closed]{closed interval} \( [a, b] \) is the base for defining Riemann-style integrals.
...
\begin{definition}\label{def:riemann_integral}\mcite[def. 2]{Gordon1991}
  Let \( \mscrX \) be a real \hyperref[def:separation_axioms/T2]{Hausdorff} \hyperref[def:topological_vector_space]{topological vector space}. Fix a \hyperref[def:function/single_valued]{function} \( f: [a, b] \to X \).
...
\begin{proof}
  ...
  Note that \( f \) is \hyperref[def:bounded_function/bounded]{bounded}. Indeed, if\DNE it is unbounded on \( [a, b] \), then there exists a refinement \( (\Gamma, \Eta) \) of \( (\Delta_0, \Xi_0) \) such that
  \begin{equation*}
    S(f, \Gamma, \Eta) - I \not\in U.
  \end{equation*}
  ...
  For every \( k = 1, \ldots, n \) and every \( j = 0, \ldots, p_k \), choose\DNE an arbitrary tag
  \begin{equation*}
...
\begin{proof}
  ...
  \begin{equation*}
    ...
    \reloset {\ref{thm:function_oscillation_properties/monotone}} \leq
