...
\begin{definition}\label{def:riemann_integral}\MarginCite[def. 2]{Gordon1991}
  Let \( \CX \) be a real \hyperref[def:separation_axioms/T2]{Hausdorff} \hyperref[def:topological_vector_space]{topological vector space}. Fix a \hyperref[def:function/single_valued]{function} \( f: [a, b] \to \CX \) and a \hyperref[def:riemann_partition/tagged]{tagged partition} \eqref{eq:def:riemann_partition/tagged}.

  The \Def{Riemann sum} of \( f \) corresponding to the partition \( \Delta \) and the tags \( \Xi \) is defined as
  \begin{equation*}
...
\begin{proof}
  ...
  \begin{AlignedEquation}\label{eq:def:riemann_integral/tagged_zero}
    ...
    &\Xi_0: \xi_k^{(0)} \in [x_{k-1}^{(0)}, x_k^{(0)}], k = 1, \ldots, n.
  \end{AlignedEquation}
  ...
  Let \eqref{eq:def:riemann_partition/tagged} be a tagged partition such that \( \Diam(\Delta) \leq \Diam(\Delta_0) \).

  We introduce another partition \( \Gamma \coloneqq \Delta \cup \Delta_0 \). Since \( \Gamma \) is a refinement of \( \Delta \), we can use the splitting \eqref{def:riemann_partition/refinement/splitting}. For every \( k = 1, \ldots, n \) and every \( j = 0, \ldots, p_k \), choose\LEM an arbitrary tag
  \begin{equation*}
  ...
  In this result, \( V \) only depends on \( f \) and \( U \) only depends on \( (\Delta_0, \Xi_0) \). Therefore, given any neighborhood \( W \) of \( 0 \), we have a tagged partition \( (\Delta_0, \Xi_0) \) such that, whenever \( \Diam(\Delta) \leq \Diam(\Delta_0) \), we have \( U + (b - a) \cdot V \subseteq W \) and hence \( S(f, \Delta, \Xi) - I \in W \) holds.
...
\begin{proposition}\label{thm:darboux_integral_iff_riemann_integral}
  A real-valued function \( f: [a, b] \to \BR \) has a \hyperref[def:darboux_itegral]{Darboux integral} if and only if it has a \hyperref[def:riemann_integral]{Riemann integral}.
\end{proposition}
