...
\begin{definition}\label{def:riemann_partition}\mcite[def. 1]{Gordon1991}
  ...
  \begin{thmenum}
    \labitem{def:riemann_partition/partition} A \term{Riemann partition} of \( [a, b] \) is a set
    \begin{equation*}
    ...
    \labitem{def:riemann_partition/refinement} The partition
    \begin{equation*}
    ...
    \labitem{def:riemann_partition/diameter} Finally, the \term{diameter} of the partition \eqref{eq:def:riemann_partition/partition} is defined as
    \begin{equation}\label{eq:def:riemann_partition/diameter}
      \diam(\Delta) \coloneqq \max_{1 \leq k \leq n} (x_k - x_{k-1}).
    \end{equation}

    \labitem{def:riemann_partition/order} We can make the set \( \op{part}([a, b]) \) of all \hyperref[def:riemann_partition/partition]{Riemann partitions} of \( [a, b] \) into a \hyperref[def:directed_set]{directed set} using two common approaches:
    \begin{thmenum}
      \labitem{def:riemann_partition/order/refinement} Put \( \Delta \preceq_R \Gamma \) if and only if \( \Gamma \) is a \hyperref[def:riemann_partition/refinement]{refinement} of \( \Delta \). This actually makes \( (\op{part}([a, b]), \preceq_R) \) a \hyperref[def:poset]{poset}.
      \labitem{def:riemann_partition/order/diameter} Put \( \Delta \preceq_D \Gamma \) if and only if \( \diam(\Gamma) \leq \diam(\Delta) \).
    \end{thmenum}

    \labitem{def:riemann_partition/tagged} A \term{tagged partition} of \( [a, b] \) is a partition \eqref{eq:def:riemann_partition/partition} of \( [a, b] \) along with a choice of a \term{tag} \( \xi_k \) for each closed interval \( [x_{k-1}, x_k], k = 1, \ldots, n \). By putting \( \Xi \coloneqq \{ \xi_k \}_{k=1}^n \), we can define a tagged partition as the tuple \( (\Delta, \Xi) \). For brevity, we write
