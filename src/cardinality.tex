...
\begin{remark}\label{rem:cardinals}
  ...
  \begin{thmenum}
    \labitem{rem:cardinals/countable} \( \aleph_0 \coloneqq \card(\omega) \), the \term{cardinality of the natural numbers}.
    \labitem{rem:cardinals/continuum} \( c \coloneqq \card(\BbbR) = \card(\pow(\omega)) \), the \term{cardinality of the continuum}.
  \end{thmenum}
\end{remark}
...
\begin{definition}\label{def:cardinal_arithmetic}
  ...
  \begin{thmenum}
    \labitem{def:cardinal_arithmetic/addition}(addition) \( \xi + \eta \coloneqq \card(\xi \coprod \eta) \), where \( \coprod \) denotes disjoint \hyperref[def:disjoint_union]{unions}.
    \labitem{def:cardinal_arithmetic/multiplication}(multiplication) \( \xi \cdot \eta \coloneqq \card(\xi \times \eta) \)
    \labitem{def:cardinal_arithmetic/exponentiation}(exponentiation) \( \xi^\eta \coloneqq \card(\cat{Set}(\eta, \xi)) \) (see \fullref{def:category_of_sets})
  \end{thmenum}
\end{definition}
