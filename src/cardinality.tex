...
\begin{definition}\label{def:set_domination}\cite[145]{Enderton1977}
  We say that the set $X$ is \underLine{dominated by $Y$} and write $\Abs{X} \leq \Abs{Y}$ if there exists an injection\Tinyref{def:function_invertibility/injection} from $X$ to $Y$.
\end{definition}

\begin{definition}\label{def:equinumerous_sets}\cite[129]{Enderton1977}
  We say that the sets $X$ and $Y$ are \underLine{equinumerous} and write $X \cong Y$ if there exists a bijection\Tinyref{def:function_invertibility/bijection} between $X$ and $Y$.
\end{definition}
...
\begin{definition}\label{def:finite_set}\cite[133]{Enderton1977}
  A set $A$ is \underLine{finite} if it is isomorphic to a natural number as defined in \cref{def:natural_numbers_zfc} (using the convention that $\varnothing$ corresponds to zero).

  If $A$ is not finite, we say that it is \underLine{infinite}.
\end{definition}
...
\begin{definition}\label{def:cardinal}\cite[197]{Enderton1977}
  For each set $A$, define its \underLine{cardinal} or \underLine{cardinal number} $\Card A$ as the intersection of all ordinals that are equinumerous to $A$.
...
\begin{note}\label{note:cardinals}
  ...
  \begin{itemize}
    \item $\aleph_0 \coloneqq \Card(\omega)$, the \underLine{cardinality of the natural numbers}.
    \item $c \coloneqq \Card(\BB{R}) = \Card(\Power(\omega))$, the \underLine{cardinality of the continuum}.
  \end{itemize}
\end{note}
