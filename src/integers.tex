...
\begin{definition}\label{def:integers}
  ...
  Let \( \oplus \), \( \odot \) and \( \leq_N \) be the operations in \( \BN \) (see \fullref{thm:natural_numbers_form_dioid}). Since either \( x \in \BZ \) or \( -x \in \BZ \) is isomorphic to a natural number, we extend the operations to \( \BZ \) as follows:
  \begin{DefEnum}
