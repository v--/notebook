...
\end{definition}
\begin{defproof}
  We must prove that \( \sgn \) is a well-defined \hyperref[def:multi_valued_function/total]{total function} \( \sgn: \BbbZ \to \BbbZ \).

  Due to the trichotomy on natural numbers shown in \fullref{def:natural_number_ordering}, we have the following possibilities:
  \begin{itemize}
  ...
  Therefore, every integer is either zero, positive or negative.
\end{defproof}
...
\begin{algorithm}[Euclidean division of integers]\label{alg:euclidean_division_of_integers}
...
\begin{proposition}\label{thm:integers_are_euclidean_domain}
  The \hyperref[def:set_of_integers]{ring of integers} is an \hyperref[def:euclidean_domain]{Euclidean domain} with division given by \fullref{alg:euclidean_division_of_integers} and degree function \( \abs{\anon} \).
\end{proposition}
\begin{proof}
  By \fullref{thm:def:integer_signum/entire}, \( \BbbZ \) is an integral domain. The Euclidean domain structure is described by \fullref{alg:euclidean_division_of_integers}.
\end{proof}
...
\begin{theorem}[Euler's totient theorem]\label{thm:eulers_totient_theorem}
  For positive coprime integers \( n \) and \( x < n \), we have
  \begin{equation*}
