...
\begin{definition}\label{def:dynamical_system}\mimprovised
  ...
  \begin{thmenum}
    ...
    Due to \fullref{eq:def:monoid_action/family/compatibility}, \( \Phi_{n+1} = \Phi_n \circ \Phi_1 \) for any integer \( n \). Using \hyperref[rem:induction/peano_arithmetic]{natural number induction} and \fullref{thm:def:group/properties/negative_power}, we can show that \( \Phi_n = \Phi_1^n \) for every integer \( n \).
