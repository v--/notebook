...
\begin{proposition}\label{thm:cycle_transposition_decomposition}
  Every cycle \( \cycle{ k_1, \cdots, k_m } \) can be decomposed into the product of transpositions
  \begin{equation*}
...
\begin{definition}\label{def:permutation_cycle_decomposition}\mimprovised
  Let \( S_n \) be a \hyperref[def:symmetric_group]{symmetric group} and let \( \sigma \) be a permutation in \( S_n \). Consider the \hyperref[def:group_action]{group action}
  \begin{equation*}
    \begin{aligned}
      &\Phi^{(p)}: \braket{ p } \times \set{ 1, \ldots, n } \to \set{ 1, \ldots, n } \\
      &\Phi^{(p)}_\sigma(k) \coloneqq \sigma(k)
    \end{aligned}
  \end{equation*}
  ...
  \begin{equation*}
    O_\sigma(k) \coloneqq \set{ p^m(k) \given m \in \BbbZ }
  \end{equation*}
  of all numbers reachable from \( k \) via iterated application of \( \sigma \) or \( p^{-1} \). The family
  \begin{equation*}
  ...
  \begin{equation*}
    \cycle{o, \sigma(o), p^2(o), \ldots, p^{c-1}(o)},
  \end{equation*}
  where \( c \) is the cardinality of \( O_\sigma(o) \).

  Cycles of length \( 1 \) are not useful to us since they represent fixed points of \( \sigma \); we ignore these cycles. We obtain a unique set of disjoint cycles for every permutation \( \sigma \) in \( S_n \), which we call the \term{cycle decomposition} of \( \sigma \). We denote this set by \( C_\sigma \).
\end{definition}
\begin{defproof}
  We must prove that \( C_\sigma \) is family of disjoint sets. Let \( O_\sigma(k_1) \) and \( O_\sigma(k_2) \) be two orbits.

  Suppose that there exists a number \( k \in O_\sigma(k_2) \cap O_\sigma(k_1) \). Then there exist \( m_1, m_2 \leq n \) such that \( k = p^{m_1}(k_1) \) and \( k = p^{m_2}(k_2) \). Then
  \begin{equation*}
...
\begin{proposition}\label{thm:permutation_decomposition}
  Given a permutation \( \sigma \), for every ordering \( c_1, \ldots, c_m \) of the \hyperref[def:permutation_cycle_decomposition]{cycle decomposition} of \( \sigma \), we have
  \begin{equation*}
    p = c_1 \bincirc \cdots \bincirc c_m.
  \end{equation*}

  Via \fullref{thm:cycle_transposition_decomposition}, this permutation can further be decomposed into a composition of individual transpositions.
\end{proposition}
\begin{proof}
...
\begin{definition}\label{def:permutation_parity}
  Let \( C_\sigma \) be the \hyperref[def:permutation_cycle_decomposition]{cycle decomposition} of \( \sigma \). We say that \( \sigma \) is an \term{even permutation} if the sum
  \begin{equation*}
    \sum_{c \in C_\sigma} (\len(c) - 1)
  \end{equation*}
  is an even number; and likewise for \term{odd permutations}. In particular, a non-identity cycle \( \sigma \) is an even permutation if and only if \( \len(\sigma) \) is \hi{odd}.
...
\begin{proposition}\label{thm:parity_of_permutation_composition}
  If \( \sigma \) and \( \tau \) are both permutations from \( S_n \), the \hyperref[def:permutation_parity]{sign} of their composition is the product of their signs.

  In particular, swapping two elements changes the sign of a permutation.
\end{proposition}
\begin{proof}
  A \hyperref[def:permutation_cycle_decomposition]{cycle decomposition} of \( \tau \bincirc \sigma \) can be obtained from the decompositions of \( \tau \) and \( \sigma \) by removing cycles that are inverses of each other. This will always remove an even number of transpositions. Hence, the cycle decomposition of \( \tau \bincirc \sigma \) will have the same parity as simply combining the decompositions of \( \tau \) and \( \sigma \).

  If \( \tau \) and \( \sigma \) are both either odd or even, \( \tau \bincirc \sigma \) is even. Otherwise, \( \tau \bincirc \sigma \) is odd.
\end{proof}
