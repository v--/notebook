...
\begin{definition}\label{def:monoid_action}
  ...
  We will define monoid actions of \( \mscrM \) on \( A \), which we will sometimes call \term{left monoid actions}. There are also \term{right monoid actions}, which are defined as left monoid actions of the \hyperref[def:group/duality]{dual group} \( \mscrG^{\opcat} \).
...
\begin{definition}\label{def:group_action}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:group_action/homomorphism} A \hyperref[def:group/homomorphism]{homomorphism} from \( \mscrG \) to the \hyperref[def:automorphism_group]{automorphism group} \( \aut(A) \).

    \thmitem{def:group_action/functor} A \hyperref[def:functor]{functor} from the \hyperref[def:monoid_delooping]{delooping} \( \cat{B}_\mscrG \) to \( \cat{C} \) (right actions are \hyperref[rem:contravariant_functor]{contravariant functors}).
  \end{thmenum}
\end{definition}
\begin{proof}
...
\end{lemma}
\begin{proof}
  \SubProofOf[def:function_invertibility/injective]{injectivity} If \( y_1, y_2 \in \mscrG \) and \( \varphi_x(y_1) = \varphi_x(y_2) \), we have
  \begin{equation*}
  ...
  \SubProofOf[def:function_invertibility/surjective]{surjectivity} If \( z \in \mscrG \), then \( z = x(x^{-1} z) \). Therefore, \( z = \varphi_x(x^{-1} z) \), and thus every member of \( \mscrG \) has a preimage. Thus, \( \varphi_x \) is surjective.
\end{proof}
