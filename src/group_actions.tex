...
\begin{proposition}\label{thm:alternating_group_cardinality}
  The \hyperref[def:alternating_group]{alternating group} \( A_n \) has \( \tfrac {n!} 2 \) elements.
\end{proposition}
\begin{proof}
  The proof is similar to that of \fullref{thm:symmetric_group_cardinality}, but is a little different.

  We use induction on \( n \). The case \( n = 1 \) is trivial. Suppose that \( A_{n-1} \) has \( \tfrac {(n-1)!} 2 \) elements. Then
  \begin{equation*}
