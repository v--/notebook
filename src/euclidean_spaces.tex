...
\begin{definition}\label{def:rigid_motion}\mimprovised
  ...
  \begin{thmenum}
    ...
    \thmitem{def:rigid_motion/householder_reflection} The \term{Householder reflection} with \term{normal vector} \( v \) is, assuming \( \norm v = 1 \),
    \begin{equation*}
    ...
    \thmitem{def:rigid_motion/orthogonal_projection} The \term{orthogonal projection} onto the affine subspace \( A \) is, given an orthonormal \hyperref[def:affine_coordinate_system]{affine coordinate system} \( O e_1 \cdots e_m \) of \( A \),
    \begin{equation*}
      f(x) \coloneqq O + \sum_{k=1}^m \inprod x {e_k} \cdot e_k.
    \end{equation*}
  \end{thmenum}
\end{definition}
...
\begin{proposition}\label{thm:orthogonal_projection_minimizes_distance}
  The \hyperref[def:rigid_motion/orthogonal_projection]{orthogonal projection} \( y_0 \) of a point \( x_0 \) onto the affine subspace \( A \) is the unique point in \( A \) that solves the optimization problem
  \begin{equation*}
     \min_{a \in A} \norm{x - a} \to \min.
  \end{equation*}

  Furthermore, the vector \( \op{x_0 y_0} \) is orthogonal to every vector in the direction \( \vect A \).
\end{proposition}
