...
\begin{definition}\label{def:rigid_motion}\mimprovised
  ...
  \begin{thmenum}
    ...
    \thmitem{def:rigid_motion/householder_reflection} The \term{Householder reflection} about the \hyperref[def:geometric_ray]{ray} \( r \) with origin \( O \) and unit directional vector \( v \) is
    \begin{equation*}
    ...
    \thmitem{def:rigid_motion/point_reflection} Similarly, the \term{point reflection} about \( O \) is
    \begin{equation*}
...
\begin{definition}\label{def:orthogonal_projection}
  Let \( L \) be an affine subspace of \( \BbbR^n \) of dimension \( m \) and let \( O e_1 \cdots e_m \) be an orthogonal \hyperref[def:affine_coordinate_system]{affine coordinate system} of \( L \). Define the matrix
  \begin{equation*}
    A \coloneqq \parens*
      {
        \begin{array}{c|c|c}
  ...
  \begin{equation*}
    f(v) \coloneqq O + A^T (A A^T)^{-1} A(v - O).
  \end{equation*}

  We call \( f(v) \) the \term{orthogonal projection} or \term{shadow} onto \( L \).

  \begin{figure}[!ht]
    \hfill
    \includegraphics[align=c]{output/def__rigid_motion__orthogonal_projection.pdf}
    \hfill
    \hfill
...
\begin{proposition}\label{thm:orthogonal_projection_minimizes_distance}
  The \hyperref[def:orthogonal_projection]{orthogonal projection} \( y_0 \) of a point \( x_0 \) onto the affine subspace \( A \) is the unique point in \( A \) that solves the optimization problem
  \begin{equation*}
     \min_{a \in A} \norm{x_0 - a}.
  \end{equation*}

  Furthermore, the vector \( \vect{x_0 y_0} \) is orthogonal to every vector in the \hyperref[def:affine_subspace]{direction} \( \vect A \).
\end{proposition}
