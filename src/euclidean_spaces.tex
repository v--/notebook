...
\begin{proposition}\label{thm:def:orthogonal_projection}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:def:orthogonal_projection/orthogonal} The vector \( \vect{x,\pi(x)} = \pi(x) - x \) is orthogonal to every vector in the \hyperref[def:affine_subspace]{direction} \( \vect L \).
...
\begin{proof}
  ...
  \SubProofOf{thm:def:orthogonal_projection/orthogonal} Let \( x \) be a point in \( L \) and let \( v \) be a vector in \( \vect L \). Then
  \begin{equation*}
  ...
  \SubProofOf{thm:def:orthogonal_projection/distance} From \fullref{thm:def:orthogonal_projection/orthogonal} it follows that
  \begin{equation*}
    \norm{ x - y }^2
    =
    \norm{ x - \pi(x) + \pi(x) - y }^2
