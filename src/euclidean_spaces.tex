...
\begin{proposition}\label{thm:def:orthogonal_projection}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:def:orthogonal_projection/normal} The vector \( \vect{x,\pi(x)} = \pi(x) - x \) is \hyperref[def:normal_vector]{normal} for the \hyperref[def:affine_subspace]{direction} \( \vect L \), i.e. orthogonal to every vector in \( \vect L \).
