...
\begin{definition}\label{def:monoid_delooping}\mcite[def. 1.1.7]{Perrone2019}
  Let \( (\mscrM, \cdot, e) \) be a \hyperref[def:unital_magma/associative]{monoid}. The \term{delooping} \( \cat{B}_\mscrM \) of \( \mscrM \) is the following category:
  \begin{itemize}
...
\begin{proof}
  ...
  \SubProof*{Proof that preorders induce thin categories} Let \( (\mscrP, \leq) \) be a small preordered set. Let \( \cat{P} \) be the \hyperref[def:quiver_free_category]{free category} obtained by regarding \( (\mscrP, \leq) \) as a \hyperref[def:quiver]{quiver}. Explicitly, the category \( \cat{P} \) is built as follows:
  \begin{itemize}
...
\begin{proposition}\label{thm:order_category_isomorphism_properties}
  Let \( (\mscrP, \leq) \) be a \hyperref[def:partially_ordered_set]{partially ordered set} and let \( \cat{P} \) be its corresponding category induced by \fullref{thm:order_category_isomorphism}.

  \begin{thmenum}
    \thmitem{thm:order_category_isomorphism_properties/dual} The \hyperref[def:dual_category]{dual category} \( \cat{P}^{\opcat} \) corresponds to the \hyperref[def:partially_ordered_set/duality]{dual partially ordered set} \( (\mscrP, \geq) \).

    \thmitem{thm:order_category_isomorphism_properties/universal} If \( \cat{P} \) has an \hyperref[def:universal_objects/initial]{initial object}, since it is a skeletal category, this initial object is unique. An object \( I \) is an initial object if and only if it is the \hyperref[def:semilattice/meet]{bottom} of \( \mscrP \).
