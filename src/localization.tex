...
\begin{definition}\label{def:local_ring}
  If \( R \) has a unique maximal \hyperref[def:maximal_ring_ideal]{ideal}, we say that it is a \Def{local ring}.
\end{definition}
...
\begin{example}\label{ex:ring_localization}\cite[430]{Knapp2016BAlg}
  Let \( p \) be a prime \hyperref[def:prime_number]{number} and \( P = \Gen p \) be the corresponding prime \hyperref[def:prime_ring_ideal]{ideal}. Denote its complement by \( S \).
