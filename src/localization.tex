...
\begin{definition}\label{def:ring_localization}\MarginCite[428]{Knapp2016BAlg}
...
\begin{proposition}\label{thm:ring_localization_universal_property}\MarginCite[431]{Knapp2016BAlg}
...
\begin{proposition}\label{thm:ring_localization_preserves_ideals}\MarginCite[432]{Knapp2016BAlg}
...
\begin{definition}\label{def:local_ring}
  If \( R \) has a unique \hyperref[def:maximal_ring_ideal]{maximal ideal}, we say that it is a \Def{local ring}.
\end{definition}

\begin{proposition}\label{thm:localization_of_prime_is_local}\MarginCite[cor. 8.50]{Knapp2016BAlg}
...
\begin{example}\label{ex:ring_localization}\MarginCite[430]{Knapp2016BAlg}
  Let \( p \) be a prime \hyperref[def:prime_number]{number} and \( P = \Bracket p \) be the corresponding \hyperref[def:prime_ring_ideal]{prime ideal}. Denote its complement by \( S \).
...
\begin{definition}\label{def:field_of_fractions}
  The \Def{field of fractions} of \( R \) is defined as the \hyperref[def:ring_localization]{localization} of \( R \) by the set
  \begin{equation*}
