...
\begin{definition}\label{def:nonsmooth_derivatives}
  ...
  \begin{thmenum}
    \labitem{def:nonsmooth_derivatives/directional}\Fullref{def:differentiability} already introduced directional derivatives. Here we introduce a special notation for them:
    \begin{equation*}
      D_h^+ f(x_0) = f_+'(x_0)(h) \coloneqq \lim_{t \downarrow 0} \frac {f(x_0 + th) - f(x_0)} t.
    \end{equation*}

    \labitem{def:nonsmooth_derivatives/dini}\cite[definition 11.18]{Clarke2013} The upper (resp. lower) \term{Dini derivative} is defined as
    \begin{balign*}
    ...
    \labitem{def:nonsmooth_derivatives/clarke}\cite[section 10.1]{Clarke2013} The \term{generalized Clarke derivative} is defined as
    \begin{equation*}
      D_h^\circ f(x_0)
      =
      f^\circ(x_0)(h)
