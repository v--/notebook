...
\begin{example}\label{ex:ordinal_addition}
  The distinction between \eqref{eq:thm:monotone_map_converse/strict} and \eqref{eq:thm:monotone_map_converse/nonstrict} is important. A simple example is provided by any limit ordinal \( \lambda \), in particular by \( \omega \). The example are inconvenient to demonstrate with the recursive definition, however \fullref{thm:ordinal_addition_disjoin_union} eases us.
  ...
  As discussed in the proof of \fullref{thm:cardinal_addition_algebraic_properties}, this is only a restriction of well-orders and not of the resulting sets themselves.
\end{example}
...
\begin{proposition}\label{thm:ordinal_multiplication_algebraic_properties}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:ordinal_multiplication_algebraic_propertiess/left_cancellative} For any three ordinals \( \alpha \), \( \beta \) and \( \gamma \) such that \( \gamma \cdot \alpha = \gamma \cdot \beta \), we have \( \alpha = \beta \).
  \end{thmenum}

  Compare this with \fullref{thm:natural_number_multiplication_properties} and \fullref{thm:cardinal_multiplication_multiplication_properties}.
\end{proposition}
\begin{proof}
...
\begin{definition}\label{def:cardinal_arithmetic}
  ...
  \begin{thmenum}
    \thmitem{def:cardinal_arithmetic/sum} Based on \fullref{thm:ordinal_addition_disjoin_union}, we define their \term{sum} as
    \begin{equation*}
      \kappa + \mu \coloneqq \card(\kappa \sqcup \mu),
    \end{equation*}
    where \( \kappa \sqcup \mu \) is their \hyperref[def:disjoint_union]{disjoint union}.

    \thmitem{def:cardinal_arithmetic/product} Based on \fullref{thm:ordinal_multiplication_disjoin_union}, we define their \term{product} as
    \begin{equation*}
...
\begin{proposition}\label{thm:cardinal_multiplication_algebraic_properties}
  ...
  Compare this with \fullref{thm:natural_number_multiplication_properties} and \fullref{thm:ordinal_addition_multiplication_properties}.
\end{proposition}
\begin{proof}
...
\begin{proof}
  ...
  Clearly \( f \in \pow(A, \set{ T, F }) \) and \( T(f) = B \).
\end{proof}
...
\begin{proof}
  ...
  \end{equation*}
  by definition. Let \( \theta: 2^{\card(A)} \to \fun(\card(A), \set{ 0, 1 }) \) be a bijective function.

  Define the operator
  \begin{equation*}
    \begin{aligned}
      &T: 2^{\card(A)} \to \card(\pow(A)) \\
      &T(p) \coloneqq \psi\parens{ \set{ \varphi^{-1}(\gamma) \given \gamma \in \card(A) \T{and} \theta(p)(\gamma) = 1 } }.
    \end{aligned}
  \end{equation*}
