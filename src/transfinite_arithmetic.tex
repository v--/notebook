...
\begin{proposition}\label{thm:ordinal_multiplication_cartesian_product}
  ...
  \end{equation*}
  where \( \prec \) is the \hyperref[def:lexicographic_order]{lexicographic order} on the \hyperref[def:tuple_and_cartesian_product]{Cartesian product} \( \alpha \times \beta \).
\end{proposition}
\begin{proof}
...
\begin{definition}\label{def:cardinal_arithmetic}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:cardinal_arithmetic/product} Based on \fullref{thm:ordinal_multiplication_cartesian_product}, we define their \term{product} as
    \begin{equation*}
...
\end{proposition}
\begin{proof}
  \SubProofOf{thm:double_and_square_of_cardinal/double} Obviously \( \kappa \sqcup \kappa = 2 \times \kappa \).
