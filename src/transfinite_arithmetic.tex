...
\begin{proposition}\label{thm:ordinal_addition_disjoin_union}
  ...
  \end{equation*}
  where \( \prec \) is the \hyperref[def:lexicographic_order]{lexicographic order} on the \hyperref[def:disjoin_union]{disjoint union} \( \alpha \amalg \beta \).
\end{proposition}
\begin{proof}
...
\begin{proposition}\label{thm:def:ordinal_multiplication_algebraic/properties}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:def:ordinal_multiplication_algebraic/propertiess/left_cancellative} For any three ordinals \( \alpha \), \( \beta \) and \( \gamma \) such that \( \gamma \cdot \alpha = \gamma \cdot \beta \), we have \( \alpha = \beta \).
  \end{thmenum}
