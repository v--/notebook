...
\begin{remark}\label{rem:linear_operators_and_matrices}
  Let \( R \) be a \hyperref[def:ring/commutative]{commutative ring}. Unlike in the \( R \)-module \( R^n \) of tuples, in a general \( R \)-module of finite \hyperref[thm:commutative_module_rank]{rank}, we have no concept of a \hyperref[def:standard_basis]{standard basis}. Instead, a linear operator \( L: U \to V \) corresponds to multiple matrix.

  If \( U \) has rank \( n \), a choice of basis for \( U \) is simply a choice of isomorphism with \( R^n \). Hence, given isomorphisms \( \varphi: U \to R^n \) and via \( \psi: V \to R^m \), the operator \( \psi^{-1} \bincirc L \bincirc \varphi \) becomes a function from \( R^n \) to \( R^m \). We can now use the identification with matrices discussed in \fullref{rem:matrices_as_functions}.
...
\begin{proposition}\label{thm:}
  Let \( V \) be a module of finite rank over a commutative ring and let \( L: V \to V \) be a linear endomorphism. Then any two matrices representing \( L \) are \hyperref[def:similar_matrices]{similar}.
\end{proposition}
\begin{proof}
  Suppose that \( A \) represents \( L \) with respect to \( e_1, \ldots, e_n \) and \( B \) represents \( L \) with respect to \( f_1, \ldots, f_n \).
