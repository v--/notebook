...
\begin{definition}\label{def:array}\mimprovised
  ...
  \begin{thmenum}
    ...
    When \( S \) is a \hyperref[def:semiring]{semiring} \( R \), we often identify the set of all \( m \)-dimensional column vectors with the \hyperref[def:sequence_space]{tuple space} \( R^m \).
...
\begin{proposition}\label{thm:matrix_algebra}
  ...
  \begin{thmenum}
    ...
    With addition, \( R^{m \times n} \) becomes an \hyperref[def:magma/commutative]{commutative} \hyperref[def:monoid]{monoid} with neutral element the \term{zero matrix}
...
\begin{definition}\label{def:symmetric_function}\mimprovised
  ...
  A permutation can be decomposed into \hyperref[def:symmetric_group/cycle]{transpositions} due to \fullref{thm:permutation_decomposition_existence}. Hence, the above condition reduces to the simpler condition of \( f \) being invariant with respect to swapping any two arguments. That is,
  \begin{equation*}
...
\begin{definition}\label{def:antisymmetric_function}\mimprovised
  ...
  A permutation can be decomposed into \hyperref[def:symmetric_group/cycle]{transpositions} due to \fullref{thm:permutation_decomposition_existence}. Hence, the above condition reduces to the simpler condition of \( f \) changing sign when swapping any two arguments. That is,
  \begin{equation*}
    f(\ldots, x_{i-1}, \fbox{\( x_i \)}, x_{i+1}, \cdots, x_{j-1}, \fbox{\( x_j \)}, x_{j+1}, \ldots)
    =
    -f(\ldots, x_{i-1}, \fbox{\( x_j \)}, x_{i+1}, \cdots, x_{j-1}, \fbox{\( x_i \)}, x_{j+1}, \ldots).
...
\begin{definition}\label{def:linear_groups}\mimprovised
  The \hyperref[def:semiring]{multiplicative group} of the \hyperref[thm:matrix_algebra]{matrix algebra} \( R^{n \times n} \) is called the \term{general linear group} \( \op{GL}_R(n) \). These are the \hyperref[def:inverse_matrix]{invertible} \( n \times n \) matrices over \( R \).

  The subgroup of matrices with determinant \( 1 \) is called the \term{special linear group} \( \op{SL}_R(n) \).
\end{definition}
