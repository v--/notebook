...
\begin{definition}\label{def:propositional_language}\mcite[102]{OpenLogic20201202}
  ...
  \begin{defenum}
    \ilabel{def:propositional_language/prop} A nonempty at most \hyperref[rem:cardinals/countable]{countable} set \( \bop{Prop} \) of \term{propositional variables}.
...
\begin{definition}\label{def:propositional_syntax}
  ...
  \begin{defenum}
    ...
    \begin{bnf*}
      \bnfprod{variable}   {P \in \bop{Prop}} \\
    ...
    Note that \( \bop{Prop} \) may be infinite, in which case the grammars may have infinitely many rules. If needed, we can circumvent this by introducing an appropriate naming convention for variables, analogously to \fullref{ex:natural_arithmetic_grammar/backus_naur_form}.
    ...
    \begin{equation}\label{eq:def:propositional_syntax/varables}
      \bop{Var}(\varphi) \coloneqq \begin{cases}
        \varnothing,                              &\varphi \in \set{ \top, \bot }                     \\
        \set{ P },                                  &\varphi = P \in \bop{Prop}                      \\
        \bop{Var}(\psi),                         &\varphi = \neg \psi                              \\
        \bop{Var}(\psi) \cup \bop{Var}(\theta), &\varphi = (\psi \circ \theta), \circ \in \Sigma.
      \end{cases}
    \end{equation}

    Note that \( \bop{Var}(\varphi) \) can naturally be totally ordered by the position of the first occurrence of a variable.
  \end{defenum}
\end{definition}
...
\begin{definition}\label{def:propositional_formula_induced_function}
  Let \( \varphi \) be a propositional formula and let \( \bop{Var}(\varphi) = \set{ P_1, \ldots, P_n } \) be an ordering of the free variables of \( \varphi \). We define the \hyperref[def:boolean_function]{Boolean function}
  \begin{alignedeq}\label{eq:def:propositional_formula_induced_function}
    &\fun_\varphi: \set{ T, F }^n \to \set{ T, F } \\
    &\fun_\varphi(x_1, \ldots, x_n) \coloneqq \varphi\eval{I},
  \end{alignedeq}
...
\begin{definition}\label{def:propositional_semantics}\mcite[def. 7.14]{OpenLogic20201202}
  ...
  \begin{defenum}
    \ilabel{def:propositional_semantics/satisfiability} Given an interpretation \( I \) and a set \( \Gamma \) of formulas, we say that \( I \) \term{satisfies} \( \Gamma \) if, for every formula \( \gamma \in \Gamma \) we have \( \gamma\eval{I} = T \).
    ...
    \begin{itemize}
      \ilabel{def:propositional_semantics/equivalence/direct} We have \( \varphi\eval{I} = \psi\eval{I} \) for every interpretation \( I \).
  ...
  \begin{defenum}
    ...
    \begin{equation}\label{eq:def:propositional_substitution/single}
      \varphi[\theta \mapsto \rho] \coloneqq \begin{cases}
        \rho,                                                          &\varphi = \theta \\
        \varphi,                                                       &\varphi \neq \theta \T{and} \varphi \in \set{ \top, \bot } \cup \bop{Prop} \\
    ...
    Define
    \begin{equation*}
      \cat{Bound} \coloneqq \bop{Var}(\rho_1) \cup \ldots \cup \bop{Var}(\rho_n).
    \end{equation*}
    and, for each variable \( P_k \) in \( \cat{Bound} \), pick a variable \( Q_k \) from \( \bop{Prop} \setminus \bop{Bound} \). Let \( m \) be the \hyperref[def:cardinal]{cardinality} of \( \bop{Bound} \). The simultaneous substitution can now be defined as
    \begin{align*}
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \item If \( \varphi \in \set{ \top, \bot } \cup \bop{Prop} \), then \( \varphi[\theta \mapsto \rho] = \varphi \) and \eqref{eq:thm:propositional_substitution_equivalence} again holds trivially.
    ...
    \begin{equation*}
      \p[\Big]{ \varphi[\theta \mapsto \rho] }\eval{I}
      =
      \overline{\p[\Big]{ \psi[\theta \mapsto \rho] }\eval{I}}
      \overset {\IndHyp} =
      \overline{\psi\eval{I}}
      =
      \varphi\eval{I}.
    ...
    \begin{equation*}
      \p[\Big]{ \varphi[\theta \mapsto \rho] }\eval{I}
      =
      \p[\Big]{ \psi_1[\theta \mapsto \rho] }\eval{I} \circ \p[\Big]{ \psi_2[\theta \mapsto \rho] }\eval{I}
      \overset {\IndHyp} =
