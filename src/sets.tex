...
\begin{definition}\label{def:set_zfc}\cite[3]{Lectures:general_topology}
  ...
  \begin{enumerate}[label={\textbf{A\arabic*)}}]
    ...
    \item\label{def:set_zfc/A4}(union) If $A$ is a set, then $\bigcup A$ (see~\cref{def:set_union}) is also a set.
    ...
    \item\label{def:set_zfc/A6}(power set) If $A$ is a set, $\Power(A)$ (see~\cref{def:power_set}) is also a set.
    ...
    \item\label{def:set_zfc/A9}(choice; see~\cref{list:aoc}) Let $J \neq 0$ and for all $j \in J$, $X_j$ is a nonempty set and $X_i \cap X_j = \varnothing$ when $i \neq j$. Then there exists a set $M$ such that for every $j \in J$, the intersection $M \cap X_j$ (see~\cref{def:set_intersection}) has exactly one member.
...
\begin{note}\label{note:family_of_sets}
  In ZFC~\cref{def:set_zfc}, everything is a set. However, it is often the case that we are not interested in how a set's elements are represented and only in how they behave, e.g. when working with natural numbers (see \cref{ex:natural_numbers_zfc}) we are interested in the elements of $\BB{N}$ and not in the way every element of $\BB{N}$ is encoded as a set.
...
\begin{note}
  We give two pairs of definitions for tuples and Cartesian products. The first pair, \cref{def:kuratowski_pair,def:binary_cartessian_product}, is quite restricted and is mostly necessary for defining functions (see \cref{def:function}) and ensuring that everything along the way is indeed a set. The second pair of definitions, given in \cref{def:cartessian_product}, can then be used freely.
\end{note}
...
\begin{definition}\label{def:cartessian_product}\cite[11]{Lectures:general_topology}
  Let $\{ X_i \}_{i \in I}$ be a nonempty family of nonempty sets (see \cref{def:indexed_family}).
...
\begin{example}\label{ex:natural_numbers_zfc}
  The smallest inductive set (see~\cref{def:inductive_set}), i.e. the one that only contains $\varnothing$ and it's iterated successors, is the set $\Cal{N}$ of natural numbers.
\end{example}
...
\begin{proposition}\label{thm:finite_intersection_properties}\cite[27]{Lectures:general_topology}
  The finite intersection operator $\FI$ (see~\cref{def:finite_intersection_operator}) satisfies the following
  \begin{defenum}
