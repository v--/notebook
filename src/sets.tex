...
\begin{Definition}\label{def:set_zfc}\cite[271]{Enderton1977}
  ...
  \begin{RefList}
    ...
    \IAxiom{def:set_zfc/A7}{A7}(infinity) There exists an inductive \hyperref[def:inductive_set]{set}.
...
\end{Proposition}
\begin{RefListProof}
    \IRef{thm:set_difference_properties/intersection} Since \( a \in A \) implies \( a \in C \), we have
    \begin{align*}
      A \setminus B
      &=
      \{ a \in A \colon a \not\in B \}
    ...
    \IRef{thm:set_difference_properties/double_difference} By the law of the excluded middle,
    \begin{align*}
