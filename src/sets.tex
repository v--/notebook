...
\begin{definition}\label{def:category_of_sets}
  ...
  Furthermore, the category is locally small\Tinyref{def:category_cardinality} and concrete\Tinyref{def:concrete_category}.
\end{definition}
\begin{proof}
  ...
  It is concrete since it is equipped with the identity functor \( \Id_{\Cat{Set}} \).
\end{proof}

\begin{corollary}\label{thm:functions_over_set_form_monoid}
  Let \( X \) be a set. Then the family \( \Cat{Set}(X) \) of all functions\Tinyref{remark:category_obj_hom} is a monoid\Tinyref{def:magma/identity} with function composition as the operation.
\end{corollary}
