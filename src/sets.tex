...
\begin{definition}\label{def:set_zfc}\cite[271]{Enderton1977}
  ...
  \end{align*}
  that we call a \underLine{class}. Not all classes can be defined to have meaningful semantics (e.g. the class of all classes easily leads to paradoxes like \cref{ex:russels_paradox_sets}). We define sets in ZFC as classes with semantics given by a model for the following axioms (exclude~\ref{def:set_zfc/A8} to obtain ZF). Classes that do not satisfy these axioms are called \underLine{proper classes} and are often said to be \underLine{too big} to be sets, e.g. the class of all sets or the class of all vector spaces).
...
\begin{note}\label{note:family_of_sets}
  ...
  In order to reduce repetitiveness, sets whose elements we consider to be other sets, are often called \underLine{families of sets}. In particular if all (different) sets are disjoint\Tinyref{def:set_intersection}, we say that the family is a \underLine{disjoint family}. We usually assume that the sets are nonempty.
\end{note}

\begin{note}\label{note:singleton_sets}
  Sets with a single elements are usually called \underLine{singletons}. It is sometimes convenient, especially with connection to geometry\Tinyref{sec:geometry_of_vector_spaces} or multivalued functions\Tinyref{def:function/multivalued} (e.g. when dealing with limits\Tinyref{def:net_limit_point} or subdifferentials\Tinyref{def:subdifferentials}), to not distinguish between singleton sets and their corresponding element.
\end{note}

\begin{definition}\label{def:subset}
  We say that \( A \) is a \underLine{subset of \( B \)} and write \( A \subseteq B \) if \( x \in A \implies x \in B \).

  If \( A \subseteq B \) and \( A \neq B \), we say that \underLine{\( A \) is a proper subset of \( B \)} and write \( A \subsetneq B \).
\end{definition}

\begin{definition}\label{def:set_intersection}\cite[24]{Enderton1977}
  If \( A \) is a set, define their \underLine{intersection} as
  \begin{align*}
  ...
  For two sets \( A \) and \( B \), we define the \underLine{binary intersection} as
  \begin{align*}
  ...
  If \( A \cap B = \varnothing \), we say that \( A \) and \( B \) are \underLine{disjoint}.
\end{definition}

\begin{definition}\label{def:set_union}\cite[24]{Enderton1977}
  If \( A \) is a set, define its \underLine{union} as
  \begin{align*}
  ...
  For two sets \( A \) and \( B \), we define the \underLine{binary union} as
  \begin{align*}
...
\begin{definition}\label{def:set_difference}\cite[27]{Enderton1977}
  If \( A \) and \( B \) are sets, define their \underLine{difference} as
  \begin{align*}
...
\begin{definition}\label{def:power_set}\cite[19]{Enderton1977}
  If \( A \) is a set, define its \underLine{power set} as
  \begin{align*}
...
\begin{definition}\label{def:kuratowski_pair}\cite[36]{Enderton1977}
  If \( A \) and \( B \) are sets, define the \underLine{(binary) tuple} or \underLine{Kuratowski pair} as
  \begin{align*}
    (A, B) \coloneqq \{ \{ A \}, \{ A, B \} \}.
  \end{align*}
...
\begin{definition}\label{def:binary_cartesian_product}\cite[37]{Enderton1977}
  If \( A \) and \( B \) are sets, define their \underLine{binary Cartesian product} as
  \begin{align*}
...
\begin{definition}\label{def:cartesian_product}\cite[54]{Enderton1977}
  ...
  We define their \underLine{Cartesian product} as
  \begin{align*}
    \prod_{i \in I} X_i \coloneqq \left\{ f: I \to \bigcup_{j \in I} X_j \colon \forall j \in I, f(j) \in X_j \right\}.
  \end{align*}

  Any element of the Cartesian product is called a \underLine{tuple}.
\end{definition}

\begin{definition}\label{def:disjoint_union}
  Let \( \{ X_i \}_{i \in I} \) be a nonempty family of nonempty sets\Tinyref{def:indexed_family}.

  We define their \underLine{disjoint union} as
  \begin{align*}
...
\begin{definition}\label{def:successor_operator}\cite[68]{Enderton1977}
  For any set \( X \), we define the \underLine{successor} operation
  \begin{align*}
...
\begin{definition}\label{def:inductive_set}\cite[68]{Enderton1977}
  A set \( A \) is called \underLine{inductive} if
  \begin{defenum}
...
\begin{definition}\label{def:finite_intersection_operator}
  Let \( X \) be a nonempty set. We define the \underLine{finite intersection} operator
  \begin{align*}
...
\begin{definition}\label{def:transitive_set}\cite[71]{Enderton1977}
  A set \( A \) is called \underLine{transitive} if \( a \in A \) implies \( a \subseteq A \).
\end{definition}

\begin{definition}\label{def:ordinal}\cite[theorem 7L]{Enderton1977}
  A set \( A \) is called an \underLine{ordinal} or an \underLine{ordinal numbers} if it is well-ordered\Tinyref{def:order/partial/well_order} under set membership.
\end{definition}
