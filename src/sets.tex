...
\begin{proposition}\label{thm:subsets_form_boolean_algebra}
  Let \( X \) be an arbitrary set. Then \( \Pow(X) \) is a Boolean \hyperref[def:boolean_algebra]{algebra} with the \hyperref[def:poset]{partial order} \( \subseteq \) (see \fullref{def:subset}) and complements given by \( \Ul{A} \coloneqq X \setminus A \). More concretely,
  \begin{itemize}
