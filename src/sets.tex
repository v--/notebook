...
\begin{definition}\label{def:set_naive}\mcite\cite[chapter 1]{Enderton1977Sets}
...
\begin{definition}\label{def:set_zfc}\mcite\cite[271]{Enderton1977Sets}
  ...
  \begin{thmenum}
    \labitem[def:set_zfc/A1]{A1}(extensionality) Two sets are equal if they have the same elements (given by set membership)

    \labitem[def:set_zfc/A2]{A2}(empty set) The following class is a set
    \begin{equation*}
      \varnothing \coloneqq \{ x \colon x \neq x \}.
    \end{equation*}

    \labitem[def:set_zfc/A3]{A3}(pairing) If \( A \) and \( B \) are sets, then
    \begin{equation*}
      \{ A, B \}
    \end{equation*}
    is also a set. In particular, \( \{ A \} = \{ A, A \} \) is a set.

    \labitem[def:set_zfc/A4]{A4}(union) If \( A \) is a set, then \( \bigcup A \) (see \fullref{def:set_union}) is also a set.

    \labitem[def:set_zfc/A5]{A5}(power set) If \( A \) is a set, \( \pow(A) \) (see \fullref{def:power_set}) is also a set.

    \labitem[def:set_zfc/A6]{A6}(specification) If \( A \) is a set and \( \varphi \) is a formula, then
    \begin{equation*}
      \{ x \in A \colon \varphi(x) \}
    \end{equation*}
    is a set.

    \labitem[def:set_zfc/A7]{A7}(infinity) There exists an \hyperref[def:inductive_set]{inductive set}.

    \labitem[def:set_zfc/A8]{A8}(choice; see \fullref{thm:aoc}) Let \( \mscrM \neq 0 \) and for all \( m \in \mscrM \), let \( X_m \) be a nonempty set and \( X_k \cap X_m = \varnothing \) whenever \( m \neq k \). Then there exists a set \( M \) such that for every \( m \in \mscrM \), the intersection \( M \cap X_m \) (see \fullref{def:set_intersection}) has exactly one member.

    \labitem[def:set_zfc/A9]{A9}(replacement) Given a set \( X \) and a formula \( \varphi(x, y) \), if for every set \( x \in X \) there exists a unique set \( y \) such that \( \varphi(x, y) \) holds, then
    \begin{equation*}
      Y \coloneqq \{ y \colon \exists x \in X, \varphi(x, y) \}
    \end{equation*}
    is a set.

    \labitem[def:set_zfc/A10]{A10}(regularity) For every nonempty set \( A \), there exists a member \( a \in A \) such that
    \begin{equation*}
      a \cap A \neq \varnothing.
    \end{equation*}
  \end{thmenum}
...
\begin{definition}\label{def:set_intersection}\mcite\cite[24]{Enderton1977Sets}
...
\begin{definition}\label{def:set_union}\mcite\cite[24]{Enderton1977Sets}
...
\begin{definition}\label{def:set_difference}\mcite\cite[27]{Enderton1977Sets}
...
\begin{proposition}\label{thm:set_difference_properties}
  ...
  \begin{thmenum}
    \labitem{thm:set_difference_properties/intersection} If \( A \) and \( B \) are subsets of \( C \), then \( A \setminus B = A \cap (C \setminus B) \).
    \labitem{thm:set_difference_properties/double_difference} If \( A \subseteq B \), then \( B \setminus (B \setminus A) = A \)
  \end{thmenum}
\end{proposition}
\begin{proof}
...
\begin{definition}\label{def:power_set}\mcite\cite[19]{Enderton1977Sets}
...
\begin{definition}\label{def:kuratowski_pair}\mcite\cite[36]{Enderton1977Sets}
...
\begin{definition}\label{def:binary_cartesian_product}\mcite\cite[37]{Enderton1977Sets}
...
\begin{definition}\label{def:cartesian_product}\mcite\cite[54]{Enderton1977Sets}
...
\begin{definition}\label{def:successor_operator}\mcite\cite[68]{Enderton1977Sets}
...
\begin{definition}\label{def:inductive_set}\mcite\cite[68]{Enderton1977Sets}
...
\begin{definition}\label{def:transitive_set}\mcite\cite[71]{Enderton1977Sets}
  A set \( A \) is called \term{transitive} if \( a \in A \) implies \( a \subseteq A \).
\end{definition}

\medskip

\begin{definition}\label{def:ordinal}\mcite\cite[thm. 7L]{Enderton1977Sets}
  A set \( A \) is called an \term{ordinal} or an \term{ordinal numbers} if it is \hyperref[def:well_ordered_set]{well-ordered} under set membership.
\end{definition}

\begin{theorem}\label{thm:ordinals_are_well_ordered}\mcite\cite[thm. 7M]{Enderton1977Sets}
