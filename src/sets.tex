...
\begin{definition}\label{def:set_zfc}\cite[271]{Enderton1977}
  ...
  \begin{description}
    ...
    \DItem{def:set_zfc/A4}[A4](union) If \( A \) is a set, then \( \bigcup A \) (see \fullref{def:set_union}) is also a set.

    \DItem{def:set_zfc/A5}[A5](power set) If \( A \) is a set, \( \Pow(A) \) (see \fullref{def:power_set}) is also a set.
    ...
    \DItem{def:set_zfc/A7}[A7](infinity) There exists an inductive \hyperref[def:inductive_set]{set}.

    \DItem{def:set_zfc/A8}[A8](choice; see \fullref{thm:aoc}) Let \( \CB \neq 0 \) and for all \( \beta \in \CB \), let \( X_\beta \) be a nonempty set and \( X_\al \cap X_\beta = \varnothing \) whenever \( \beta \neq \al \). Then there exists a set \( M \) such that for every \( \beta \in \CB \), the intersection \( M \cap X_\beta \) (see \fullref{def:set_intersection}) has exactly one member.
...
\begin{remark}\label{remark:family_of_sets}
  In ZFC \fullref{def:set_zfc}, everything is a set. However, it is often the case that we are not interested in how a set's elements are represented and only in how they behave, e.g. when working with natural \hyperref[def:natural_numbers]{numbers} we are interested in the elements of \( \BN \) and not in the way every element of \( \BN \) is encoded as a set.
...
\begin{proposition}\label{thm:subsets_form_boolean_algebra}
  Let \( X \) be an arbitrary set. Then \( \Pow(X) \) is a Boolean \hyperref[def:boolean_algebra]{algebra} with the partial \hyperref[def:poset]{order} \( \subseteq \) (see \fullref{def:subset}) and complements given by \( \Ul{A} \coloneqq X \setminus A \). More concretely,
  \begin{itemize}
    \item The top element is \( X \).
    \item The bottom element is \( \varnothing \).
    \item Joins given by \( \cap \) (see \fullref{def:set_intersection}) (with the convention that the intersection of \( 0 \) elements is \( X \)).
    \item Meets given by multiplication \( \cup \) (see \fullref{def:set_union}).
  \end{itemize}
\end{proposition}
...
\begin{definition}\label{def:ordinal}\cite[theorem 7L]{Enderton1977}
  A set \( A \) is called an \Def{ordinal} or an \Def{ordinal numbers} if it is well-\hyperref[def:well_ordered_set]{ordered} under set membership.
\end{definition}
...
\begin{corollary}\label{thm:natural_numbers_are_well_ordered}
  The natural \hyperref[def:natural_numbers]{numbers} are well-ordered.
\end{corollary}
