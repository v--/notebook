...
\begin{definition}\label{def:peano_arithmetic}\mcite[exmpl. 17.6]{OpenLogicFull}
  ...
  \begin{thmenum}[resume=def:peano_arithmetic]
    ...
    See \fullref{rem:induction} for a more detailed discussion of induction in general and \fullref{natural number recursion} for the corresponding recursion principle.
  \end{thmenum}
...
\begin{definition}\label{def:natural_number_ordering}
  ...
  \end{equation}
  is then connected to \( \leq \) via \eqref{def:poset/compatibility_strict}.
