...
\begin{definition}\label{def:peano_arithmetic}\MarginCite[1]{Peano1889}
  ...
  \begin{DefEnum}
    ...
    \IAxiom{def:peano_arithmetic/PA2}{PA2} \( \not\exists \xi (s(\xi) \doteq 0) \)
    \IAxiom{def:peano_arithmetic/PA3}{PA3} The following, called the \Def{axiom schema of induction}, holds: for all formulas \( \varphi \) and all variables \( \xi \), we have:
    \begin{equation*}
      (\varphi[\xi \mapsto 0] \wedge \forall \eta (\varphi[\xi \mapsto \eta] \rightarrow \varphi[\xi \mapsto s(\eta)])) \rightarrow \forall \eta (\varphi[\xi \mapsto \eta]).
    \end{equation*}
