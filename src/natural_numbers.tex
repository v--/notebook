...
\begin{definition}\label{def:peano_arithmetic}\mcite[exmpl. 17.6]{OpenLogicFull}
  ...
  \begin{thmenum}[series=def:peano_arithmetic]
    ...
    The successor function is only a technicality used for establishing basic properties and for defining addition and multiplication, both in this subsection and in \fullref{ex:thm:omega_recursion}.
    ...
    As with all \hyperref[def:semiring]{semirings}, multiplication has higher priority than addition. In the unambiguous language defined in \fullref{ex:natural_arithmetic_grammar/backus_naur_form}, this means that we can use the shorthand \( \xi + \eta \cdot \zeta \) for \( ((\xi \cdot \eta) + \zeta) \). We often use juxtaposition for denoting multiplication.
  \end{thmenum}
...
\begin{proposition}\label{thm:natural_numbers_addition_properties}
...
\begin{proof}
  ...
  \SubProofOf[def:magma_identity]{identity} We have \( n + 0 = n \) by \eqref{eq:def:peano_arithmetic/PA4} and \( 0 + n = n \) by commutativity.
  ...
  \SubProofOf[eq:thm:natural_numbers_addition_properties/nonzero_sum]{nonzero sums} If \( n = 0 \) then \( n + m = m \) by \eqref{eq:def:peano_arithmetic/PA4} and if also \( m = 0 \), then \( n + m = 0 \).
...
\end{proposition}
\begin{proof}
  \SubProofOf[def:magma_identity]{identity} Multiplication by \( 1 \) on the right preserves any natural number:
  \begin{equation*}
     n \cdot 1
  ...
  \SubProofOf[def:semiring/distributivity]{distributivity} We will prove that \( (n + m)k = n \cdot k + n \cdot k \) with induction on \( k \).
  ...
  Conversely, let \( n \cdot m = 0 \). By induction on \( m \), either \( m = 0 \) or we have \( n \cdot m = n \cdot p(m) + n \), in which case \eqref{eq:thm:natural_numbers_addition_properties/nonzero_sum} both \( n \cdot p(m) \) and \( n \) are zero. Thus if we assume that \( m \neq 0 \), then we can conclude that \( n = 0 \).
\end{proof}
...
\begin{remark}\label{rem:natural_number_successor_via_addition}
  ...
  It is dangerous to conflate \( n + 1 \) and \( s(n) \) until we have proved the familiar properties of addition. We have already done so in \fullref{thm:natural_numbers_addition_properties}, however, and will further avoid mentioning directly the operations \( s(n) \) and \( p(n) \).
\end{remark}
...
\begin{definition}\label{def:natural_number_ordering}
  ...
  \begin{equation}\label{eq:def:natural_number_ordering/predicate}
    \xi \leq \eta \coloneqq \qexists \zeta \parens[\Big]{ \xi + \zeta \doteq \eta }.
  \end{equation}
  ...
  \begin{equation}\label{eq:def:natural_number_ordering/strict_predicate}
    \xi < \eta \coloneqq \qexists {\zeta \neq 0} \parens[\Big]{ \xi + \zeta \doteq \eta }
  \end{equation}
  is then connected to \eqref{eq:def:natural_number_ordering/predicate} via \eqref{def:poset/compatibility_strict}.

  For the specific model of \logic{PA} based on the smallest inductive set \( \omega \), latter definition is equivalent to \( \in \) as discussed in \fullref{rem:ordinal_definition}.
...
\begin{proof}
  ...
  \SubProofOf[def:binary_relation/transitive]{transitivity} Let \( n < m \) and \( m < k \). Then there exist nonzero numbers \( a \) such that \( n + a = m \) and \( b \) such that \( m + b = k \). Thus \( n + a + b = k \), which demonstrates that \( n \leq k \). Furthermore, because of \eqref{eq:thm:natural_numbers_addition_properties/nonzero_sum}, it also follows that \( a + b \neq 0 \).

  Therefore \( n < k \).

  \SubProofOf[def:binary_relation/trichotomic]{trichotomy} Let \( n \) and \( m \) be natural numbers.

  We have already shown in \fullref{def:natural_number_ordering} that due to \eqref{def:poset/compatibility_strict} the equality \( n = m \) holds if and only if neither \( n < m \) nor \( n > m \).
  ...
  \SubProofOf[def:preordered_magma]{compatibility with addition} We will show that the nonstrict order is compatible with addition in \( \BbbN \). Let \( n \leq m \) and let \( k \) be an arbitrary natural number. Since \( n \leq m \), there exists a constant \( a \) such that \( n + a = m \). Then
  \begin{equation*}
    m + k = (n + a) + k = (n + k) + a.
  \end{equation*}
  ...
  \SubProofOf[def:ordered_semiring]{compatibility with multiplication} If \( n \geq 0 \) and \( m \geq 0 \), then \( n \cdot m \geq 0 \) for the sole reason that all natural numbers are greater than or equal to zero.
\end{proof}
