...
\begin{definition}\label{def:peano_arithmetic}\mcite\cite[1]{Peano1889}
  ...
  \begin{defenum}
    ...
    \ilabel{def:peano_arithmetic/zero} A \term{zero} constant \( 0 \) (see \fullref{rem:peano_arithmetic_zero}).
    \ilabel{def:peano_arithmetic/succ} A unary functional symbol \( s \), called the \term{successor operation}.
  \end{defenum}
  ...
  \begin{defenum}
    ...
    \iaxiom{def:peano_arithmetic/PA3}{PA3} The following, called the \term{axiom schema of induction}, holds: for all formulas \( \varphi \) and all free variables \( \xi \) of \( \varphi \), we have:
    \begin{equation*}
      \paren[\Big]{ \varphi[\xi \mapsto 0] \wedge \qforall{\eta} (\varphi[\xi \mapsto \eta] \to \varphi[\xi \mapsto s(\eta)]) } \to \qforall{\eta} (\varphi[\xi \mapsto \eta]).
    \end{equation*}
