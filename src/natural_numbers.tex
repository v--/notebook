...
\begin{definition}\label{def:peano_arithmetic}\MarginCite[1]{Peano1889}
  ...
  \begin{DefEnum}
    ...
    \begin{equation*}
      \Paren[\Big]{ \varphi[\xi \mapsto 0] \wedge \Qnt{\forall}{\eta} (\varphi[\xi \mapsto \eta] \to \varphi[\xi \mapsto s(\eta)]) } \to \Qnt{\forall}{\eta} (\varphi[\xi \mapsto \eta]).
    \end{equation*}
...
\begin{remark}\label{rem:peano_arithmetic_zero}
  ...
  \begin{RemEnum}
    \ILabel{rem:peano_arithmetic_zero/nonnegative} \( 0, 1, 2, \ldots \) or \( \BbbZ_{\geq 0} \) for the nonnegative \hyperref[def:integers]{integers}.
    \ILabel{rem:peano_arithmetic_zero/positive} \( 1, 2, \ldots \) or \( \BbbZ_{> 0} \) for the positive integers.
  \end{RemEnum}
...
\begin{example}\label{ex:natural_numbers_models}
  ...
  \begin{DefEnum}
    ...
    \ILabel{ex:natural_numbers_models/vector_spaces} The \hyperref[def:first_order_model_category]{model category} \( \Cat{FinVect}_{\BbbK} \) of all \hyperref[def:vector_space_dimension]{finite-dimensional} \hyperref[def:vector_space]{vector spaces} over the \hyperref[def:field]{field} \( \BbbK \) can also be used as a model for the natural numbers. In this case,
    \begin{RefList}
      \IRef{def:peano_arithmetic/zero} \( 0 \coloneqq 0_{\BbbK} = \{ 0_{\BbbF} \} \) is the trivial (zero-dimensional) vector space.
      \IRef{def:peano_arithmetic/succ} \( s(V) \coloneqq V \oplus \BbbK \) is the \hyperref[def:left_module_direct_product]{direct sum} of the vector space \( V \) and the field \( \BbbK \) as a one-dimensional vector space.
    \end{RefList}
  \end{DefEnum}
\end{example}

\begin{definition}\label{def:natural_number_addition}
  We inductively\IND define a binary \Def{addition} operation \( +: \BbbN^2 \to \BbbN \) as
...
\begin{proof}
  ...
  \SubProofOf{def:well_ordered_set} Let \( A \subseteq \BbbN \) be a nonempty set. Aiming at a contradiction, suppose\LEM that \( A \) has no \hyperref[def:preordered_set/largest_smallest_element]{minimum}. In particular, \( 0 \not\in A \).
