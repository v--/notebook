...
\begin{definition}\label{def:category_of_sets}
  We define the category\Tinyref{def:category} \( \Cat{Set} \) as follows:
  \begin{description}
    \RItem{def:category/objects} The class\Tinyref{def:set_zfc} of objects is the class of all sets.

    \RItem{def:category/morphisms} The morphisms between two sets \( A, B \) are the functions\Tinyref{def:function} \( f: A \to B \).

    \RItem{def:category/composition} Composition of morphisms is the usual function composition\Tinyref{def:function/composition}.
  \end{description}
\end{definition}
\begin{proof}
...
\begin{proposition}\label{thm:set_is_locally_small}
  The category \( \Cat{Set} \) is locally small\Tinyref{def:category_cardinality}.
\end{proposition}
\begin{proof}
  Since a function \( f: X \to Y \) is formally\Tinyref{def:function} a subset of the Cartesian product \( X^2 \)\Tinyref{def:cartesian_product}, which is a set, it is itself a set.
...
\begin{proposition}\label{thm:set_categorical_limits}
  We are interested in categorical limits\Tinyref{def:categorical_limit} and colimits\Tinyref{def:categorical_colimit} in \( \Cat{Set} \). Fix an indexed family\Tinyref{def:indexed_family} \( \{ X_\al \}_{\al \in \CA} \) of sets.
  \begin{defenum}
    \DItem{thm:set_categorical_limits/product} Their categorical product\Tinyref{def:categorical_product} is their Cartesian product\Tinyref{def:cartesian_product} \( \prod_{\al \in \CA} X_\al \), the projection morphisms being
    \begin{align*}
      &\pi_\beta: \prod_{\al \in \CA} X_\al \to X_\beta \\
      &\pi_\beta(\{ x_\al \}_{\al \in \CA}) \coloneqq x_\beta.
    \end{align*}

    \DItem{thm:set_categorical_limits/coproduct} Their categorical coproduct\Tinyref{def:categorical_coproduct} is their disjoint union\Tinyref{def:disjoint_union} \( \coprod_{\al \in \CA} X_\al \), the injection morphisms being
    \begin{align*}
    ...
    In particular, if \( \sim \subseteq X^2 \) is an equivalence relation\Tinyref{def:equivalence_relation}) on \( X \), then the coequalizer of the two projection maps of the product \( X^2 \) is the pair \( (X / ~, \pi) \), where \( \pi \) is the quotient map
    \begin{align*}
      &\pi: X \to X / ~ \\
      &\pi(x) = [x].
    \end{align*}
