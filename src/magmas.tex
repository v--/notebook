...
\begin{definition}\label{def:magma}
  A \term{magma} is a set \( \mscrM \) equipped with a \hyperref[def:function/arity]{binary function} \( \cdot: \mscrM \times \mscrM \to \mscrM \), called the \term{magma operation}. Unless specified otherwise, we denote this operation by juxtaposition as \( xy \) instead of \( x \cdot y \). We often call the operation \term{multiplication} or \term{composition} (especially in \hyperref[def:endomorphism_monoid]{endomorphism monoids}), which contrasts which \hyperref[rem:additive_magma]{additive magmas}.

  \begin{thmenum}
    \labitem{def:magma/theory} In analogy to the \hyperref[def:pointed_set/theory]{theory of pointed sets}, we can define the \term{theory of magmas} as an empty theory over a language with a single binary function.

    \labitem{def:magma/homomorphism} A \hyperref[def:first_order_homomorphism]{homomorphism} between the magmas \( (\mscrM, \cdot_{\mscrM}) \) and \( (\mscrN, \cdot_{\mscrN}) \) is, explicitly, a function \( \varphi: \mscrM \to \mscrN \) such that
    ...
    \labitem{def:magma/substructure} The set \( S \subseteq \mscrM \) is a \hyperref[def:first_order_substructure]{submagma} of \( \mscrX \) if it is closed under the magma operation, i.e. if \( x, y \in S \) imply \( xy \in S \).

    \labitem{def:magma/category} We denote the \hyperref[def:first_order_model_category]{model category} for the theory of magmas by \( \cat{Mag} \).

    \labitem{def:magma/trivial} The \term{trivial magma} is the empty set with an empty operation. It is the unique \hyperref[def:zero_objects/initial]{initial object} in \( \cat{Mag} \).

    \labitem{def:magma/opposite} The \term{opposite magma} of \( (\mscrM, \cdot) \), also called the \term{dual magma}, is the magma \( (\mscrM, \odot) \) with multiplication reversed:
    \begin{equation*}
    ...
    \labitem{def:magma/associative} We can add the following axiom to the theory:
    ...
    \labitem{def:magma/commutative} Another common axiom is \term{commutativity}:
    ...
    \labitem{def:magma/cancellative} We say that the operation \( \cdot \) is \term{left-cancellative} (resp. \term{right-cancellative}) if, for all \( x, y \in \mscrM \), we have
    ...
    \labitem{def:magma/exponentiation} We define an additional \term{exponentiation} operation for positive integers \( n \) inductively\IND as
    ...
    \labitem{def:magma/power_set} It is customary to perform magma operations with sets. That is, if \( A \) and \( B \) are sets in the magma \( \mscrM \), it is customary to write
    \begin{equation*}
      A \cdot B \coloneqq \{ a \cdot b \colon a \in A, b \in B \}.
    \end{equation*}

    This actually turns the power set \( \pow(\mscrM) \) into a magma, which we will call the \term{power set magma} of \( \mscrM \). This is especially useful with the convention \fullref{rem:singleton_sets} since it allows us to write \( aB \) for \( a \in \mscrM \) and \( B \subseteq \mscrM \).
...
\begin{remark}\label{rem:additive_magma}
  ...
  \begin{thmenum}
    \labitem{rem:additive_magma/addition} The magma operation is denoted by \( + \) and called \term{addition}.
    \labitem{rem:additive_magma/multiplication} The magma \hyperref[def:magma/exponentiation]{exponentiation operation} is denoted by \( \cdot \) or juxtaposition called \term{multiplication}. Thus multiplication is not defined for two elements of the magma but defined for a positive integer and an element of the magma. In the case of a \hyperref[def:magma/commutative]{commutative} \hyperref[def:unital_magma/associative]{monoid}, if multiplication is extended to two elements of the monoid, we instead talk about \hyperref[def:semiring]{semirings}.

    \labitem{rem:additive_magma/identity} The \hyperref[def:magma_identity]{identity} is usually denoted by \( 0 \).
    \labitem{rem:additive_magma/inverse} If an \hyperref[def:unital_magma_inverse_element]{inverse} of \( x \) exists, it is denoted by \( -x \) rather than \( x^{-1} \).
  \end{thmenum}
\end{remark}
...
\begin{proposition}\label{thm:magma_exponentiation_properties}
  ...
  \begin{thmenum}
    \labitem{thm:magma_exponentiation_properties/commutativity} We have the following \hyperref[def:magma/commutative]{commutativity}-like property: for \( x \in \mscrM \) and \( n = 1, 2, \ldots \),
    \begin{equation}\label{eq:thm:magma_exponentiation_properties/commutativity}
      x^n = x x^{n-1} = x^{n-1} x.
    \end{equation}

    \labitem{thm:magma_exponentiation_properties/distributivity} Exponentiation distributes over multiplication: for any member \( x \in \mscrM \) and any two positive integers \( n \) and \( m \),
    \begin{equation}\label{eq:thm:magma_exponentiation_properties/multiplication}
      x^{n + m} = x^n x^m.
    \end{equation}

    \labitem{thm:magma_exponentiation_properties/repeated} For any member \( x \in \mscrM \) and any two positive integers \( n \) and \( m \),
