...
\begin{definition}\label{def:divisibility}\mimprovised
  Fix an arbitrary element \( x \) in a \hyperref[def:semiring/commutative]{commutative semiring}. If, there exist elements \( y \) and \( z \) such that \( x = lr \), we say that \( l \) is a \term{left divisor} of \( x \), \( r \) is a \term{right divisor} and that \( x \) is a \term{multiple} or \term{factor} of \( l \) and of \( r \). If \( l = r \), we say that \( l \) is a \term{two-sided} divisor and denote this by \( y \mid x \).

  As discussed in \fullref{rem:units_in_rings_etymology}, divisors of \( 1 \) are sometimes called \term{units}. \term{Zero divisors} are also important. By \hyperref[def:semiring/absorption]{absorption}, every number is a zero divisor. We are often only interested in \term{nontrivial} zero divisors, for which \( lr = 0 \) implies that both \( l \neq 0 \) and \( r \neq 0 \).
\end{definition}
...
\begin{proposition}\label{thm:natural_number_divisibility_order}
  ...
  The lattice has a \hyperref[def:partially_ordered_set_extremal_points/top_and_bottom]{bottom} element \( 1 \) and a \hyperref[def:partially_ordered_set_extremal_points/top_and_bottom]{top} element \( 0 \). Hence, the lattice is \hyperref[def:semilattice/bounded]{bounded}.
