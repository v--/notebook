...
\begin{definition}\label{def:divisibility}\mimprovised
  ...
  \begin{thmenum}
    ...
    The set of all units of \( R \) is precisely the \hyperref[def:semiring]{multiplicative group} \( R^\times \).
  \end{thmenum}
\end{definition}
...
\begin{proposition}\label{thm:semiring_multiplicative_group}
  For a \hyperref[def:semiring]{semiring} \( R \), the set \( R^\times \) of all \hyperref[def:divisibility/unit]{units} is a \hyperref[def:group]{group} with respect to multiplication.

  We call \( R^\times \) the \term{multiplicative group} of \( R \).
\end{proposition}
\begin{proof}
