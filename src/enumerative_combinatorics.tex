...
\begin{definition}\label{def:multiset}
  Let \( A \) be an arbitrary \hyperref[def:set]{set} (in the sense of \hyperref[def:zfc]{\logic{ZFC}}).

  Let \( M \) be a mapping that gives us a \hyperref[def:cardinal]{cardinal number} for every member of \( A \). This is a function by the \hyperref[def:zfc/replacement]{axiom schema of replacement}. This function is called a \term{multiset}.

  \begin{thmenum}
    \thmitem{def:multiset/universe} We call the set \( A \) the \term{universe} of \( M \).

    If \( x \in A \), we say that \( x \) belongs to \( M \) and denote this by \( x \in M \), although this is not actual set membership.

    \thmitem{def:multiset/multiplicity} For every \( x \in A \), we call the \hyperref[def:cardinality_existence]{cardinality} \( M(x) \) the \term{multiplicity} of \( x \).

    \thmitem{def:multiset/order} The \term{order} of \( M \), if it exists, is the maximum
    \begin{equation*}
...
\begin{example}\label{ex:def:multiset}
  The roots of any polynomial for a multiset with multiplicities given by \fullref{def:polynomial_root_multiplicity}.
\end{example}
