...
\begin{definition}\label{def:pointed_set}
  ...
  \begin{DefEnum}
    \ILabel{def:pointed_set/theory} Pointed sets can also be viewed as \hyperref[def:first_order_model]{models} of an \hyperref[def:first_order_theory]{empty theory} for a \hyperref[def:first_order_logic_alphabet]{first-order logic language} consisting of:
    \begin{DefEnum}
      \ILabel{def:theory_of_pointed_sets/eq} A formal equality \( \doteq \) symbol.
      \ILabel{def:theory_of_pointed_sets/point} A constant, i.e. a nullary \hyperref[def:first_order_logic_alphabet/func]{functional symbol}.
    \end{DefEnum}
