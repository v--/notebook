...
\begin{proof}
  ...
  \SubProofOf{def:binary_relation/antisymmetric} Let \( x \leq y \) and \( y \leq x \), that is, \( x \vee y = y \) and \( y \vee x = x \). By \fullref{def:algebraic_theory/commutativity}, \( x = y \vee x = x \vee y = y \).
  \SubProofOf{def:binary_relation/transitive} Let \( x \leq y \) and \( y \leq z \), that is, \( x \vee y = y \) and \( y \vee z = z \). Then, by \fullref{def:algebraic_theory/associativity},
  \begin{equation*}
