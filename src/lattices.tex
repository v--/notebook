...
\begin{example}\label{ex:lattice_polynomials}
  ...
  \begin{thmenum}
    ...
    \thmitem{ex:lattice_polynomials/boolean} A lattice polynomial for the two-element boolean algebra \hyperref[def:boolean_value]{\( \set{ T, F } \)} corresponds to a \hyperref[def:boolean_function]{boolean function}. We cannot express negation without introducing auxiliary polynomials as a consequence of \fullref{ex:thm:posts_completeness_theorem/and_or}. \hyperref[def:positive_formula]{Positive formulas} in \hyperref[def:conjunctive_disjunctive_normal_form/normal_form]{conjunctive/disjunctive normal form}, however, correspond exactly to lattice polynomials with square-free monomials.
