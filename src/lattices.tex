...
\begin{definition}\label{def:binary_join_meet}
  ...
  \begin{itemize}
    \item the \Def{join} of \( x \) and \( y \), \( x \lor y \)
    \item the \Def{meet} of \( x \) and \( y \), \( x \land y \)
  \end{itemize}
  such that,
  \begin{description}
...
\begin{proposition}\label{thm:binary_join_meet_properties}
  ...
  \begin{defenum}
    \DItem{thm:binary_join_meet_properties/idempotence} both operations are \Def{idempotent}, i.e. \( x \lor x = x = x \land x \)
  \end{defenum}
\end{proposition}
\begin{proof}
  \begin{description}
...
\begin{definition}\label{def:join_meet}
  Let \( (X, \leq) \) be a poset\Tinyref{def:poset}. We define \Def{joins} \( \lor \) and \Def{meets} \( \land \) as the partial\Tinyref{def:function/partial} functions
  \begin{align*}
    &\lor: \Power(X) \to X
    &&\land: \Power(X) \to X
    \\
...
\begin{definition}\label{def:lattice}
  ...
  \begin{description}
    \DItem{def:lattice/top}[top] a \Def{top element} \( \top \), such that \( \top = \lor X \) (in particular, \( \lor X \) exists).
    \DItem{def:lattice/bottom}[bottom] a \Def{bottom element} \( \bot \), such that \( \bot = \land X \).
...
\begin{definition}\label{def:distributive_lattice}\cite{nLab:distributive_lattice}
  A lattice \( (X, \top, \bot, \lor, \land) \) is called a \Def{distributive lattice} if any of the following two equivalent distributive axioms hold for all \( x, y, z \in X \):
  \begin{description}
...
\begin{proof}
  ...
  \end{align*}
  is isomorphic to the Galois field\Tinyref{thm:galois_field_existence} \( \Fld_2 \) with the Boolean algebra structure defined in \cref{thm:f2_is_boolean_algebra}.
