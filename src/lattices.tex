...
\end{definition}
\begin{proof}
  We first show that \( \vee \) and \( \wedge \) satisfy \cref{def:binary_join_meet}. Since suprema and infima are obviously associative and commutative, it remains only to show \ref{def:binary_join_meet/absorption}, that is, for any comparable \( x, y \in X \),
  \begin{align*}
    x = \sup \{x, \inf \{ x, y \} \}
    &&
    x = \inf \{x, \sup \{ x, y \} \}.
  ...
  We now show that if the partial order \( \leq \) was defined using binary joins and meets as in \cref{def:binary_join_meet}, then the original join \( \vee \) and meet \( \wedge \) are compatible with the binary \( \sup \) and \( \inf \).
...
\begin{theorem}[de Morgan's laws]\label{thm:demorgans_laws}
...
\begin{proof}
  We will first show
  \begin{equation}\label{thm:demorgans_laws/equation}
    \Ol{\bigvee_{\al \in \CA} x_\al} = \bigwedge_{\al \in \CA} \Ol{x_\al}
  \end{equation}

  In order for \( \wedge_{\al \in \CA} \Ol{x_\al} \) to be the complement of \( \vee_{\al \in \CA} x_\al \), the equations in \cref{def:boolean_algebra} need to be satisfied.
  ...
  This proves \cref{thm:demorgans_laws/equation}.
...
\begin{proof}
  ...
  \end{align*}
  is isomorphic to the Galois field\Tinyref{thm:galois_field_existence} \( \BF_2 \) with the Boolean algebra structure defined in \cref{thm:f2_is_boolean_algebra}.
