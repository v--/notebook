...
\begin{remark}\label{remark:infinite_join_meet}
  Suprema and infima in posets can be used to define operations named \hyperref[def:join_meet]{\Def{joins} and \Def{meets}}, however there are also axioms for binary joins and \hyperref[def:binary_join_meet]{meets}. If we are interested in infinitary joins and meets, however, we need to use the poset definition. This can be accomplished indirectly by
  \begin{enumerate}
...
\begin{definition}\label{def:binary_join_meet}
  ...
  We can use joins to define the \hyperref[def:poset]{partial order relation}
  \begin{equation*}
