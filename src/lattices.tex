...
\begin{definition}\label{def:semilattice}\mcite[3]{Gratzer1978}
  ...
  \begin{thmenum}[resume=def:semilattice]
    ...
    For bounded semilattices, the relevant constants should formally be present in any bounded sub-semilattice. This is sometimes inconvenient, however. For convenience, we will distinguish between \enquote{sublattices} and \enquote{bounded sublattices}.
...
\begin{definition}\label{def:lattice_ideal}
  We will define concepts analogous to those from \fullref{subsec:semiring_ideal}, but adapted to \hyperref[def:semilattice/lattice]{lattices}. \Fullref{thm:lattice_divisibility} provides motivation. In some cases the notions coincide --- see \fullref{thm:semilattice_ideal_as_semiring_ideal}.
...
\begin{example}\label{ex:lattice_ideals}
  ...
  \begin{thmenum}
    \thmitem{ex:lattice_ideals/lattice} Consider the (zero-based) natural number divisibility lattice from \fullref{thm:natural_number_divisibility_lattice}. For any natural number \( n \), the semiring ideal
    \begin{equation*}
