...
\begin{definition}\label{def:distributive_lattice}\MarginCite{nLab:distributive_lattice}
  A lattice \( (\CX, \vee, \wedge, \top, \bot) \) is called a \Def{distributive lattice} if any of the following two equivalent infinitary distributive axioms hold for any \hyperref[def:indexed_family]{family} \( \{ x_k \}_{k \in \CK} \subseteq \CX \) and any \( y \in \CX \):
