...
\begin{definition}\label{def:lattice_ideal}
  ...
  \begin{thmenum}
    ...
    \begin{minipage}[t]{0.45\textwidth}
      We define the \term{principal ideal} of \( x \) as the \term{downward closure}
      \begin{equation*}
    ...
    \hspace{0.02\textwidth}
    \begin{minipage}[t]{0.45\textwidth}
      Dually, we define the \term{principal filter} of \( x \) as the \term{upward closure}
      \begin{equation*}
...
\begin{definition}\label{def:centered_family}\mcite[123]{Engelking1989}
  We say that a set in a lattice is a \term{centered family} or a \term{filter subbase} if the meet of any finite subfamily is not the bottom.

  In a \hyperref[thm:boolean_algebra_of_subsets]{Boolean algebra of subsets}, this amounts to the intersection of any finite subfamily being nonempty. The term \term{finite intersection property} is also used in this context.

  Centered families are sometimes used in the context of a given filter --- see \fullref{thm:filter_from_subbase}.
\end{definition}

\begin{definition}\label{def:filter_subbase}\mimprovised
  We say that a subset \( S \) of a filter \( F \) is a \term{filter subbase} of \( F \) if, for every \( x \in F \), there exists some subfamily \( y_1, \ldots, y_n \in P \) such that
...
\begin{proposition}\label{thm:filter_from_subbase}
  Let \( S \) be a \hyperref[def:centered]{centered family} on \( L \). Then the family
  \begin{equation*}
  ...
  Unlike in \fullref{def:topological_subbase}, it is not necessary for the top element \( \top \) to belong to \( P \), hence we may avoid taking the meet of the empty family.
...
\begin{proposition}\label{thm:filterr_subbase_can_generate_filter}
  Every filter subbase is a centered family.
\end{proposition}
\begin{proof}
  The strict inequality in \eqref{eq:def:filter_subbase} implies that every subbase is a centered family.
\end{proof}
