...
\begin{definition}\label{def:binary_lattice_operations}
  ...
  \begin{thmenum}
    \labitem{def:binary_lattice_operations/join} The \term{join} of \( x \) and \( y \), denoted as \( x \vee y \).
    \labitem{def:binary_lattice_operations/meet} The \term{meet} of \( x \) and \( y \), denoted as \( x \wedge y \).
    \labitem{def:binary_lattice_operations/absorption} The following \term{absorption laws} hold:
    \begin{balign*}
      x \vee (y \wedge x) = x
       &  &
      x \wedge (y \vee x) = x.
...
\begin{proposition}\label{thm:binary_lattice_operations_properties}
  ...
  \begin{thmenum}
    \labitem{thm:binary_lattice_operations_properties/idempotence} Both operations are \term{idempotent}, i.e. \( x \vee x = x = x \wedge x \).
  \end{thmenum}
\end{proposition}
\begin{proof}
...
\begin{definition}\label{def:lattice}
  ...
  \begin{thmenum}
    \labitem{def:lattice/top} The \hyperref[def:preordered_set/largest_smallest_element]{global maximum} \( \vee \mscrX \) exists. We call it the \term{top element} \( \top \).

    \labitem{def:lattice/bottom} The \hyperref[def:preordered_set/largest_smallest_element]{global minimum} \( \wedge \mscrX \) exists. We call it the \term{bottom element} \( \bot \).

    \labitem{def:lattice/finite_join} All finite \hyperref[def:binary_lattice_operations]{joins} exist.
    \labitem{def:lattice/finite_meet} All finite \hyperref[def:binary_lattice_operations]{meets} exist.
  \end{thmenum}
...
\begin{proposition}\label{thm:lattice_homomorphism}
  ...
  \begin{thmenum}
    \labitem{thm:lattice_homomorphism/respects_joins} \( f(x \vee_{\mscrX} y) = f(x) \vee_{\mscrY} f(y) \)
    \labitem{thm:lattice_homomorphism/respects_meets} \( f(x \wedge_{\mscrX} y) = f(x) \wedge_{\mscrY} f(y) \)
    \labitem{thm:lattice_homomorphism/respects_top} \( f(\top_{\mscrX}) = \top_{\mscrY} \)
    \labitem{thm:lattice_homomorphism/respects_bot} \( f(\bot_{\mscrX}) = \bot_{\mscrY} \)
  \end{thmenum}
...
\begin{definition}\label{def:distributive_lattice}\mcite\cite{nLab:distributive_lattice}
...
\begin{definition}\label{def:boolean_algebra}\mcite\cite{nLab:boolean_algebra}
...
\begin{proposition}\label{thm:boolean_algebra_properties}
  ...
  \begin{thmenum}
    \labitem{thm:boolean_algebra_properties/unique_complement} For each \( x \in \mscrX \), there exists a unique complement \( \overline x \).
    \labitem{thm:boolean_algebra_properties/double_complement} For each \( x \in \mscrX \), we have \( x = \overline{\overline x} \).
  \end{thmenum}
\end{proposition}
\begin{proof}
...
\begin{proposition}\label{thm:functions_into_boolean_algebra_form_boolean_algebra}
  ...
  \begin{reflist}
    \refitem{def:binary_lattice_operations/join} \( f \vee_{\mscrF} g \coloneqq (x \mapsto f(x) \vee_{\mscrX} g(x)) \)
    \refitem{def:binary_lattice_operations/meet} \( f \wedge_{\mscrF} g \coloneqq (x \mapsto f(x) \wedge_{\mscrX} g(x)) \)
    \refitem{def:lattice/top} \( \top_{\mscrF} \coloneqq (x \mapsto \top_{\mscrX}) \)
    \refitem{def:lattice/bottom} \( \bot_{\mscrF} \coloneqq (x \mapsto \bot_{\mscrX}) \)
    \refitem{eq:def:boolean_algebra/complement} \( \overline{f \ }^{\mscrF} \coloneqq (x \mapsto \overline{f(x)}^{\mscrX}) \)
  \end{reflist}
