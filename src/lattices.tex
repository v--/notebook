...
\begin{proof}
  ...
  \begin{RefList}
    \IRef{def:binary_relation/reflexive} Direct consequence of \fullref{thm:binary_lattice_operations_properties/idempotence}.
    \IRef{def:binary_relation/antisymmetric} Let \( x \leq y \) and \( y \leq x \), that is, \( x \vee y = y \) and \( y \vee x = x \). By \fullref{def:algebraic_theory/commutativity}, \( x = y \vee x = x \vee y = y \).
    \IRef{def:binary_relation/transitive} Let \( x \leq y \) and \( y \leq z \), that is, \( x \vee y = y \) and \( y \vee z = z \). Then, by \fullref{def:algebraic_theory/associativity},
    \begin{equation*}
...
\end{Proposition}
\begin{RefListProof}
    \IRef{thm:binary_lattice_operations_properties/idempotence} \fullref{def:binary_lattice_operations/absorption} implies that
    \begin{equation*}
...
\begin{proof}\mbox{}
  \begin{itemize}
    \IRef{thm:boolean_algebra_properties/unique_complement} If \( y \) and \( z \) are both complements of \( x \), then
    \begin{align*}
      y
      =
      y \wedge \top
    ...
    \IRef{thm:boolean_algebra_properties/double_complement} Fix \( x \in \CX \). We have
    \begin{align*}
      \Ol{\Ol x}
      =
      \Ol{\Ol x} \wedge \top
