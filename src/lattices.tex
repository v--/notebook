...
\begin{definition}\label{def:semilattice}\mcite[3]{Gratzer1978}
  ...
  \begin{thmenum}[series=def:semilattice]
    ...
    \thmitem{def:semilattice/complete}\mcite[24]{Gratzer1978} A semilattice is said to be \term{complete} if the corresponding operation is defined for all infinite sets rather than only finite ones.
  ...
  \begin{thmenum}[resume=def:semilattice]
    ...
    We cannot proper express the \term{theory of complete (semi)lattices} as an extension of this existing theory since we must define join and meet as unary operations on subsets of \( \mscrX \) rather than binary operations on members of \( \mscrX \). Complete semilattices can instead be defined within \hyperref[def:zfc]{ZFC}.
