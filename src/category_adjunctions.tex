...
\begin{proposition}\label{thm:universal_objects_as_adjunctions}
  ...
  \begin{thmenum}
    \thmitem{thm:universal_objects_as_adjunctions/initial} The object \( I \) of \( \cat{C} \) is \hyperref[def:universal_objects]{initial} if and only if it is (the unique value of) a left adjoint to \( \Delta_{\cat{0}} \) functor.
    ...
    \thmitem{thm:universal_objects_as_adjunctions/terminal} Similarly, the \hyperref[def:universal_objects]{terminal objects} are exactly the right adjoint to \( \Delta_{\cat{0}} \) functors.
  \end{thmenum}
\end{proposition}
\begin{proof}
...
\begin{remark}\label{rem:left_and_right_adjoint_not_equivalence}
  We discussed in \fullref{ex:def:universal_objects/grp} that \enquote{the} trivial group \( \set{ e } \) is a \hyperref[def:universal_objects/zero]{zero object} of \( \cat{Grp} \). By \fullref{thm:universal_objects_as_adjunctions}, this object induces a functor that is both left adjoint and right adjoint of \( \Delta_{\cat{0}} \). Nevertheless, the categories \( \cat{Grp} \) and \( \cat{1} \) are not \hyperref[def:equivalence_of_categories]{equivalent}.
\end{remark}
