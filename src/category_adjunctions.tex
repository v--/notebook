...
\begin{proposition}\label{thm:universal_objects_as_adjunctions}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:universal_objects_as_adjunctions/terminal} Similarly, the \hyperref[def:universal_objects]{terminal objects} are exactly the right adjoint to \( \Delta_I^{\cat{1}} \) functors.
  \end{thmenum}
\end{proposition}
\begin{proof}
...
\begin{remark}\label{rem:universal_mapping_property}
  ...
  Taking into account that the functor \( F \) itself is unique up to a unique isomorphism, as per \fullref{thm:left_adjoint_uniqueness}, we have proved the following statement:
  \begin{displayquote}
