...
\begin{definition}\label{def:parametric_curve}\mimprovised
  ...
  \begin{figure}[!ht]
    \centering
    \includegraphics{output/def__parametric_curve.pdf}
...
\begin{example}\label{ex:def:simple_curve}
  ...
  \begin{thmenum}
    ...
    \begin{figure}[!ht]
      \centering
      \includegraphics{output/ex__def__simple_curve__lissajous.pdf}
...
\begin{example}\label{ex:curve_moving_backwards}
  One thing we wish to avoid is \enquote{moving backwards} during parametrization of a \hyperref[ref:parametric_curve]{parametric curve}.
  ...
  \begin{figure}[!ht]
    \centering
    \includegraphics{output/ex__curve_moving_backwards.pdf}
...
\begin{definition}\label{def:tangent_line}
  ...
  \begin{figure}[!ht]
    \centering
    \includegraphics{output/def__tangent_line.pdf}
...
\begin{definition}\label{def:closed_curve}\mimprovised
  ...
  \begin{figure}[!ht]
    \hfill
    \includegraphics{output/def__closed_curve__a.pdf}
    \hfill
    \hfill
    \includegraphics{output/def__closed_curve__b.pdf}
    \hfill
    \hfill
...
\begin{example}\label{ex:curve_staying_in_place}
  We have avoided \hyperref[def:ex:curve_moving_backwards]{\enquote{moving backwards}} by requiring that curves are \hyperref[def:smooth_curve]{smooth}, however we have not avoided \enquote{staying in place}.
...
\begin{example}\label{ex:nonregular_curve}
  ...
  \begin{figure}[!ht]
    \centering
    \includegraphics{output/ex__nonregular_curve.pdf}
...
\begin{example}\label{ex:nonregular_parametrization}\mcite[rem. 1.16]{ИвановТужилин2017}
  ...
  \begin{figure}[!ht]
    \centering
    \includegraphics{output/ex__nonsmooth_parametrization.pdf}
    \caption{Part of the curve \( (t, t^2) \) from \( t = -1 \) to \( t = 1 \).}\label{fig:ex:nonsmooth_parametrization}
  \end{figure}
\end{example}

\begin{proposition}\label{thm:natural_parametrization_existence}
  Every \hyperref[def:smooth_curve/regular]{regular} \hyperref[def:parametric_curve]{parametric curve} has a parametrization for which the \hyperref[def:curve_speed]{speed} is always \( 1 \). We call such a parametrization a \term{natural parametrization} or, due to \fullref{thm:length_of_smooth_curves}, an \term{arc length parametrization}.
\end{proposition}
\begin{proof}
...
\begin{proposition}\label{thm:natural_reparametrization_existence}
  For two \hyperref[def:smooth_curve/natural_parameter]{natural parameters} \( t_1: J_1 \to I \) and \( t_2: J_2 \to I \) of \( \gamma: I \to \BbbR^n \), there exists some constant \( a \) such that \( J_2 = a + J_1 \) and, for all \( s \in J_2 \), \( t_2(s) = t_1(a + s) \).
\end{proposition}
\begin{proof}
  Define
  \begin{equation*}
...
\begin{definition}\label{def:perimeter}
  Consider a \hyperref[rem:geometric_shape]{geometric shape}, i.e. a set of points in an Euclidean space. Suppose that the \hyperref[def:topological_boundary]{boundary} of the shape is a \hyperref[def:parametric_curve]{parametric curve}. Suppose also that the curve is \hyperref[def:arc_length]{rectifiable}.
