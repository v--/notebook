...
\begin{definition}\label{def:polynomial}
  ...
  \begin{thmenum}
    \thmitem{def:polynomial_semiring/zero_polynomial} An exception to most rules is the \term{zero polynomial}, all of whose coefficients are zeroes.

    \thmitem{def:polynomial_semiring/leading_coefficient} The last nonzero coefficient of a nonzero polynomial is called the \term{leading coefficient} and is denoted by \( \op{LC}(p) \). If \( \op{LC}(p) = 1 \), we call the polynomial \term{2}.

    \thmitem{def:polynomial_semiring/degree} The zero-based index of the leading coefficient is called the \term{degree} of the polynomial as is denoted by \( \deg(p) \). That is, if only \( a_0 \) is nonzero, then \( \deg(p) = a_0 \). The degree of the zero polynomial is left undefined.

    \thmitem{def:polynomial_semiring/monomial} A \term{monomial} is a polynomial with only one nonzero element.

    \thmitem{def:polynomial_semiring/degree_names} Polynomials of degree \( n \) with special names include
    \begin{itemize}
...
\begin{definition}\label{def:algebra_of_polynomials}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:algebra_of_polynomials/polynomial_multiplication} In order to make \( R[X] \) into an \hyperref[def:algebra_over_semiring]{algebra}, we define \term{Cauchy multiplication} \( \odot: R[X] \times R[X] \to R[X] \) as follows: if \( (a_0, a_1, \ldots) \) and \( (b_0, b_1, \ldots) \) are polynomials, their product is defined to be the polynomial with coefficients
    \begin{equation}
...
\begin{remark}\label{rem:polynomial_symbolic_expression}
  We choose a \hyperref[def:formal_language]{symbol}, usually \( X \), called an \term{indeterminate}, and give it \hyperref[def:first_order_semantics/satisfiability]{semantics} using the \hyperref[def:polynomial_semiring/monomial]{monomial}
  \begin{equation*}
  ...
  We use capital letters to highlight that this is not a function - see \fullref{def:polynomial_semiring_function}. We say that \( p(X) \) is a polynomial in one indeterminate.
\end{remark}

\begin{proposition}\label{thm:def:polynomial_semiring_degree/properties}
  The polynomial degree has the following basic properties:
  \begin{thmenum}
    \thmitem{thm:def:polynomial_semiring_degree/properties/sum} For nonzero polynomials \( p, q \in R[X] \) with \( p \neq -q \), we have
    \begin{equation*}
      \deg (p + q) \leq \max \{ \deg p, \deg q \}.
    \end{equation*}

    \thmitem{thm:def:polynomial_semiring_degree/properties/product} For nonzero polynomials \( p, q \in R[X] \) with \( pq \neq 0 \), we have
    \begin{equation*}
...
\begin{proof}
  ...
  \begin{align}
    p(X) &\coloneqq \sum_{k=0}^n a_k X^k, \label{eq:thm:def:polynomial_semiring_degree/properties/p} \\
    q(X) &\coloneqq \sum_{k=0}^m b_k X^k. \label{eq:thm:def:polynomial_semiring_degree/properties/q}
  \end{align}

  \SubProofOf{thm:def:polynomial_semiring_degree/properties/sum} Additionally assume that \( p \neq -q \) since otherwise \( p + q = 0 \) and \( \deg(p + q) \) is undefined. Thus, there exists at least one index \( k = 1, 2, \ldots \), so that \( a_k \neq b_k \). Denote by \( k_0 \) the largest such index (only finitely many are nonzero). Then
  \begin{equation*}
...
\begin{proposition}\label{thm:polynomial_algebra_basis}
  The polynomial \hyperref[def:algebra_of_polynomials]{algebra} \( R[X] \) has a Hamel \hyperref[def:module_hamel_basis]{basis} consisting of all \hyperref[def:polynomial_semiring/monomial]{monomials}
  \begin{equation*}
...
\begin{definition}\label{def:polynomial_semiring_free_module}
...
\begin{corollary}\label{thm:polynomials_over_field_are_euclidean_domain}\mcite[10]{Knapp2016BasicAlgebra}
  The polynomial \hyperref[def:integral_domain]{ring} \( \BbbK[X] \) over a field \( \BbbK \) is an \hyperref[def:euclidean_domain]{Euclidean} domain with \( \delta(p) \coloneqq \deg p \). Furthermore, the remainder and quotient are unique.
\end{corollary}
\begin{proof}
...
\begin{definition}\label{def:polynomial_semiring_function}
  Denote by \( \cat{Set}(R) \) the ring of all functions over \( R \) (see \fullref{thm:functions_over_semimodule}). Define the unital ring homomorphism
  \begin{balign*}
     & \Phi: R[X] \to \cat{Set}(R)                                                                      \\
...
\begin{definition}\label{def:polynomial_semiring_root_multiplicity}
...
\begin{proposition}\label{thm:integral_domain_polynomial_root_limit}
  If \( R \) is an integral domain, a nonzero polynomial of degree \( n \) has at most \( n \) (not necessarily \hyperref[def:polynomial_semiring_root_multiplicity]{distinct}) roots.
\end{proposition}
\begin{proof}
...
\begin{proposition}\label{thm:polynomials_with_identical_values}
  In an integral domain, two polynomials \eqref{eq:thm:def:polynomial_semiring_degree/properties/p} and \eqref{eq:thm:def:polynomial_semiring_degree/properties/q} with \( m \leq n \) are equal (i.e. have the same coefficients) if and only if their functions agree at \( n + 1 \) points.
\end{proposition}
\begin{proof}
...
\begin{theorem}\label{thm:polynomial_embedding_behavior}
  Let \( R \) be an integral domain and \( \xi \coloneqq \min \{ \card(R), \aleph_0 \} \).
...
\begin{proof}
  ...
  If \( \deg(p) < \xi \), which is automatically true if \( R \) is an infinite ring, then \( q \coloneqq p \) is the desired polynomial. It is unique by the previous paragraph.

  If \( \deg(p) \geq \xi \), which is only possible if \( R \) is a finite ring, we can easily give a non-constructive proof in the general case. \Fullref{alg:finite_field_polynomial_reduction} gives a concrete procedure for finding \( q \) in the case of certain Galois fields.

  If \( R \) is finite, both the \hyperref[def:polynomial_semiring_free_module]{polynomial free module} \( \pi_{\xi - 1} \) and the \hyperref[thm:functions_over_semimodule]{function space} \( \fun(R) \) have exactly \( \xi^\xi \) elements. Furthermore, by the first paragraph of the proof, two distinct polynomials in \( \pi_{\xi - 1} \) give rise to distinct functions. Therefore, the evaluation \( \Phi: \pi_{\xi - 1} \to \fun(R) \) is an injective function between finite sets of the same cardinality. It is therefore a bijection. That is, to each endofunction over \( R \), in particular to \( \Phi(p) \), there corresponds a unique univariate polynomial over \( R \) of degree less than \( \xi \).

  \SubProofOf{thm:polynomial_embedding_behavior/multivariate} As in the univariate case, if \( R \) is infinite, the statement trivially holds.

  Assume that \( R \) is a finite ring. A simple counting argument shows that the vector spaces \( \fun(R^n, R) \) and
  \begin{equation*}
  ...
  \begin{equation*}
    f \in R[X_1, \ldots, X_n] = R[X_1, \ldots, X_{n-1}][X_n],
  \end{equation*}
  hence
  \begin{equation*}
  ...
  Therefore, as in the univariate case, to each function from \( \fun(R^n, R) \), in particular to \( \Phi(p) \), there corresponds a unique polynomial from \( \mscrL \).
\end{proof}

\begin{definition}\label{def:rational_algebraic_function}
  We denote the field of \hyperref[def:ring_localization]{fractions} of \( R[X_1, \ldots, X_n] \) by \( R(X_1, \ldots, X_n) \) and call it the field of \term{rational algebraic functions}.
\end{definition}

\begin{definition}\label{def:laurent_polynomial}
  We generalize \hyperref[def:polynomial_semiring]{polynomials} by allowing negative coefficients.
  ...
  \begin{thmenum}
    ...
    The terms \enquote{\hyperref[def:polynomial_semiring/degree]{degree}} and \enquote{\hyperref[def:polynomial_semiring/leading_coefficient]{leading coefficient}} do not generalize naturally, so we leave them undefined.
