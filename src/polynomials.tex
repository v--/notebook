...
\begin{proposition}\label{thm:polynomial_quotient_rings}
  The quotient ring of \( R[X] \) by the ideal generated by the monic polynomial ideal \( p(X) \) is isomorphic to the ring 
  \begin{equation*}
    \Rem(R[X], p(X)) \coloneqq \{ \Rem(s(X), p(X)) \colon s(X) \in R[X] \}
  \end{equation*}
  of remainders modulo \( p(X) \).
\end{proposition}
\begin{proof}
  Euclidean division\Tinyref{thm:euclidean_division_of_polynomials} allows us to define the homomorphism
  \begin{align*}
    &\Theta: R[X] / \Gen{p(X)} \to \Rem(R[X], p(X)) \\
...
\begin{example}\label{example:polynomial_quotient_rings_gaussian_integers}
  The value of \cref{thm:polynomial_quotient_rings} is in that, like \cref{thm:integers_modulo_isomorphic_to_quotient_group}, it allows us to identify elements of the quotient rings of the form \( R[X] / \Gen{p(X)} \) for monic \( p \) with concrete polynomials.

  \Cref{thm:integers_modulo_isomorphic_to_quotient_group} tells us that we can choose a concrete polynomial from \( \Rem(R[X], p(X)) \) for every equivalence class in \( R[X] / \Gen{p(X)} \). The ring \( \Rem(R[X], p(X)) \) contains precisely the polynomials that have a zero quotient when divided by \( p(X) \). In particular, these polynomials have a lower degree than that of \( p(X) \).

  For example, consider the polynomial \( p(X) \coloneqq X^2 + 1 \) over the integers\Tinyref{def:integers} \( \Z \). Then \( \Rem(\Z[X], X^2 + 1) \) contains polynomials of the form
  \begin{equation}\label{example:polynomial_quotient_rings_gaussian_integers/linear_polynomial}
    aX + b
  \end{equation}
  with integer coefficients.

  Which polynomials of this form are contained in \( \Rem(\Z[X], X^2 + 1) \)? Polynomial division of \( s(X) \) by \( X^2 + 1 \) gives us
  \begin{equation*}
    s(X) = (X^2 + 1) q(X) + r(X),
  \end{equation*}
  hence
  \begin{equation*}
    \Floor(s(X), X^2 + 1)
    =
    \Floor((X^2 + 1) q(X) + r(X), X^2 + 1)
    =
    r(X).
  \end{equation*}

  This means that we do not impose any additional restrictions on the remainder \( r(X) \) and hence all polynomials of the form \( aX + b \) with integer coefficients are contained in \( \Rem(\Z[X], X^2 + 1) \).

  We can also identify \( \Rem(\Z[X], X^2 + 1) \) with a superring of \( \Z \). Since there is no integer \( u \) that satisfies the equation \( u^2 + 1 = 0 \), we resort to looking for solutions in superrings of \( \Z \).

  The complex number\Tinyref{def:complex_numbers} \( i \) satisfies the equation, hence, using the canonical embedding of \( \Z \) into \( \Co \), we identify
  \begin{equation*}
     \Z / \Gen{X^2 + 1} \cong \Rem(\Z[X], X^2 + 1) \cong \Z[i].
  \end{equation*}
