...
\begin{definition}\label{def:polynomial}\cite[149]{Knapp2016BAlg}
  ...
  \begin{defenum}
    ...
    \DItem{def:polynomial/leading_coefficient} The last nonzero coefficient of a nonzero polynomial is called the \Def{leading coefficient} and is denoted by \( \LC(p) \). If \( \LC(p) = 1 \), we call the polynomial \Def{monic}.
...
\begin{proof}
  ...
  \begin{align*}
    ...
    q(X) &\coloneqq \sum_{i=0}^m b_i X^i.
  \end{align*}
...
\begin{proof}
  ...
  \begin{align*}
    \hat r(X)
    &\coloneqq
    a(X) - b(X) \frac {a_n} {b_m} X^{n-m}
    = \\ &=
    a_n X^n + \hat a(X) - (b_m X^m + \hat b(X)) \frac {a_n} {b_m} X^{n-m}
    = \\ &=
    a_n X^n + \hat a(X) - a_n X^n - \hat b(X) \frac {a_n} {b_m} X^{n-m}
    = \\ &=
    \hat a(X) - \hat b(X) \frac {a_n} {b_m} X^{n-m}.
  \end{align*}
  ...
  \begin{align*}
    \hat r(X) &= a(X) - b(X) \frac {a_n} {b_m} X^{n-m} \\
    b(X) \hat q(X) + r(X) &= a(X) - b(X) \frac {a_n} {b_m} X^{n-m} \\
    b(X) \left(\hat q(X) - \frac {a_n} {b_m} X^{n-m} \right) + r(X) &= a(X).
  \end{align*}

  Define
  \begin{equation*}
    q(X) \coloneqq \hat q(X) - \frac {a_n} {b_m} X^{n-m}.
  \end{equation*}
