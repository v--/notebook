\subsection{Category of groups}\label{subsec:groups}
...
\begin{definition}\label{def:group_direct_sum}
  The \Def{direct sum} of a nonempty family of groups \( \{ \CX_k \}_{k \in \CK} \) is a subgroup of their \hyperref[def:group_direct_sum]{direct product} where, for any group element \( \{ x_k \}_{k \in \CK} \), only finitely many components are different from zero.
  ...
  \begin{DefEnum}
    ...
    \ILabel{def:group_direct_sum/external} To distinguish \( \prod_{k \in \CK} \CX_k \) from \( X \), we sometimes call it the \Def{external direct product}.
  \end{DefEnum}
\end{definition}
