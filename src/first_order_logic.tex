...
\begin{remark}\label{rem:first_order_model_notation}
  ...
  For simplicity, we will use the simpler terminology \enquote{Let \( U \subseteq \mscrV \) be a subset of the vector space \( \mscrV \)} rather than \enquote{Let \( U \subseteq V \) be a subset of the domain \( V \) of the vector space \( \mscrV \)} and \enquote{The function \( f: \mscrX \to \mscrY \) is an order-homomorphism} rather than \enquote{The function \( f: X \to Y \) is a homomorphism between the corresponding lattices}. We sometimes identify a structure with its domain, especially for special sets like \( \BbbR \).
\end{remark}
