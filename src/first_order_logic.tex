...
\begin{definition}\label{def:first_order_language}\mcite[187]{OpenLogic20201202}
  ...
  \begin{description}
    ...
    \begin{thmenum}[series=def:first_order_language]
      \labitem{def:first_order_language/propositional} The entirety of the \hyperref[subsec:propositional_logic]{propositional logic alphabet}.

      \labitem{def:first_order_language/var} A nonempty at most \hyperref[rem:cardinals/countable]{countable} alphabet of variables \( \boldop{Var} \), usually denoted by \( \xi_1, \xi_2, \ldots \) or \( \xi, \eta, \zeta \).

      \labitem{def:first_order_language/quantifiers} The quantifiers \( Q = \set{ \forall, \exists } \):
      \begin{thmenum}
        \labitem{def:first_order_language/quantifiers/universal} The \term{universal quantifier} \( \forall \).
        \labitem{def:first_order_language/quantifiers/existential} The \term{existential quantifier} \( \exists \).
        \labitem{def:first_order_language/quantifiers/dot} The dot \( . \) for separating a quantifier from its formula. The dot itself is not itself a quantifier and is not formally necessary - we use it only for readability.
      \end{thmenum}

      \labitem{def:first_order_language/equality} Optionally, an equality symbol \( \doteq \).
    \end{thmenum}

    \item[Non-logical symbols]
    \hfill
    \begin{thmenum}[resume=def:first_order_language]
      \labitem{def:first_order_language/func} A set of functional symbols, \( \boldop{Fun} \), whose elements are usually denoted by \( f_1, f_2, \ldots \) or \( f, g \) or by symbols lie \( \oplus \), in which case we use infix notation. Each functional symbol has an associated natural number called its \term{arity}, denoted by \( \# f \). Functional symbols with a zero arity are called \term{constants}.

      \labitem{def:first_order_language/pred} A set of predicate symbols, \( \boldop{Pred} \), whose elements are usually denoted by \( p_1, p_2, \ldots \) or by symbols like \( \leq \), in which case we use infix notation. Predicate symbols also have an associated arity. Predicate symbols with zero arity are called \term{propositional variables}.
    \end{thmenum}
  \end{description}
...
\begin{definition}\label{def:first_order_syntax}
  ...
  \begin{thmenum}
    \labitem{def:first_order_syntax/grammar} Consider the following \hyperref[def:backus_naur_form]{grammar schema}:
    \begin{bnf*}
      ...
      \bnfmore                  {\vdots} \\
      \bnfprod{n-ary function}  {f \in \boldop{Fun}, \#f = n \text{ (separate rule for each \( n \))}} \\
      \bnfmore                  {\vdots} \\
      \bnfprod{unary predicate} {p \in \boldop{Pred}, \#p = 1} \\
      \bnfmore                  {\vdots} \\
      \bnfprod{n-ary predicate} {p \in \boldop{Pred}, \#p = n \text{ (separate rule for each \( n \))}} \\
      \bnfmore                  {\vdots} \\
    ...
    \labitem{def:first_order_syntax/term}\mcite[189]{OpenLogic20201202}The set \( \mscrT \) of \term{terms} is the language \hyperref[def:grammar_derivation/grammar_language]{generated} by this grammar schema with \( \bnfpn{term} \) as a starting rule.

    The grammar of first-order terms is unambiguous by \fullref{thm:first_order_terms_and_formulas_are_unambiguous}.

    \labitem{def:first_order_syntax/subterm} If \( \tau \) and \( \kappa \) are terms and \( \kappa \) is a \hyperref[def:language/subword]{subword} of \( \tau \), we say that \( \kappa \) is a \term{subterm} of \( \tau \).

    \labitem{def:first_order_syntax/term_variables} For each term \( \tau \), we define its variables as
    ...
    \labitem{def:first_order_syntax/ground_term} A term \( \tau \) is called a \term{closed term} if \( \boldop{Var}(\tau) = \varnothing \).

    \labitem{def:first_order_syntax/formula} The set \( \mscrF \) of \term{formulas} is the language \hyperref[def:grammar_derivation/grammar_language]{generated} by this grammar schema with \( \bnfpn{formula} \) as a starting rule.
    ...
    \labitem{def:first_order_syntax/subformula} If \( \varphi \) and \( \psi \) are formulas and \( \psi \) is a \hyperref[def:language/subword]{subword} of \( \varphi \), we say that \( \psi \) is a \term{subformula} of \( \varphi \).

    \labitem{def:first_order_syntax/formula_terms} If \( \varphi \) is a formula, if \( \tau \) is a term and if \( \tau \) is a \hyperref[def:language/subword]{subword} of \( \varphi \), we say that \( \tau \) is a \term{term} of \( \varphi \).

    \labitem{def:first_order_syntax/formula_free_variables} The \term{free variables} of a formula are defined as
    ...
    \labitem{def:first_order_syntax/ground_formula} A formula \( \varphi \) is called a \term{closed formula} if \( \boldop{Free}(\varphi) = \varnothing \).

    \labitem{def:first_order_syntax/formula_bound_variables} Dually, the \term{bound variables} of a formula are defined as
    ...
    \labitem{def:first_order_syntax/formula_variables} Finally, the set of all variables of a formula \( \varphi \) is
...
\begin{definition}\label{def:first_order_structure}\mcite[def. 14.27]{OpenLogic20201202}
  Fix a first-order logic language \( \mscrL \). A \term{structure} for \( \mscrL \) is a pair \( (\mscrX, I) \), where

  \begin{thmenum}
    \labitem{def:first_order_structure/set} \( \mscrX \) is a nonempty set called the \term{universum} of \( \mscrX \).

    \labitem{def:first_order_structure/interpretation} \( I \) is a heterogenous (set-theoretic) \hyperref[def:function]{map} called the \term{interpretation} of \( \mscrX \) that we define partially as
    \begin{thmenum}
      \labitem{def:first_order_structure/interpretation/equality} The interpretation \( I(\doteq) \) of the equality is a \hyperref[def:binary_relation]{binary relation} on \( \mscrX \).

      \labitem{def:first_order_structure/interpretation/function} For every \( n \)-ary function symbol \( f \), its interpretation is a \hyperref[def:function]{function} of type \( I(f): \mscrX^n \to \mscrX \).

      \labitem{def:first_order_structure/interpretation/predicate} For every \( n \)-ary predicate \( p \), its interpretation is a an n-ary \hyperref[def:relation]{relation} \( I(p) \subseteq \mscrX^n \). This relation corresponds to all tuples of values that satisfy the predicate within the structure.
    \end{thmenum}
  \end{thmenum}
\end{definition}
...
\begin{definition}\label{def:first_order_valuation}
  ...
  \begin{thmenum}
    \labitem{def:first_order_valuation/variable_assignment} A \term{variable assignment} for the variables of \( \mscrL \) is any function \( v: \boldop{Var} \to \mscrX \) (loosely similar to \hyperref[def:propositional_valuation/interpretation]{propositional interpretations}).

    \labitem{def:first_order_valuation/modified_assignment} For every variable \( \xi \) and every universum element \( x \in \mscrX \) we also define the \term{modified assignment} at \( \xi \) with \( x \):
    \begin{equation*}
    ...
    \labitem{def:first_order_valuation/term_valuation} The \term{valuation} of a term \( \tau \) is a value in the universum \( \mscrX \) given by
    ...
    \labitem{def:first_order_valuation/formula_valuation} The \term{valuation} of a formula \( \varphi \) is a Boolean value (see \fullref{def:propositional_valuation}) given by
...
\begin{definition}\label{def:first_order_semantics}\mcite[208]{OpenLogic20201202}
  ...
  \begin{thmenum}
    ...
    \labitem{def:first_order_semantics/entailment} We say that the set of formulas \( \Gamma \) \term{entails} the set of formulas \( \Delta \) and write \( \Gamma \vDash \Delta \) if every model of \( \Gamma \) is also a model of \( \Delta \).

    \labitem{def:first_order_semantics/tautology} The formula \( \varphi \) is a \term{tautology} if every structure is a model of \( \varphi \).

    \labitem{def:first_order_semantics/contradiction} Dually, \( \varphi \) is a \term{contradiction} is no structure is a model of \( \varphi \).

    \labitem{def:first_order_semantics/equivalence} We say that \( \varphi \) and \( \psi \) are \term{semantically equivalent} and write \( \varphi \gleichstark \psi \) if both \( \varphi \vDash \psi \) and \( \psi \vDash \varphi \).
  \end{thmenum}
\end{definition}
...
\begin{definition}\label{def:first_order_substitution}
  ...
  \begin{thmenum}
    \labitem{def:first_order_substitution/predicate} Let \( \varphi \) be a \hyperref[def:propositional_syntax/formula]{propositional formula} with variables \( \boldop{Var}(\varphi) = \set{ P_1, \ldots, P_n } \). For brevity, denote \( V \coloneqq \boldop{Var}(\varphi) \). Let \( \Theta = \set{ \theta_1, \ldots, \theta_n } \) be a set of \hyperref[def:first_order_syntax/formula]{first order formulas}.
    ...
    \labitem{def:first_order_substitution/term_in_term} The substitution of the \hyperref[def:first_order_syntax/term]{first order term} \( \kappa \) with \( \mu \) in the term \( \tau \) is
    ...
    \labitem{def:first_order_substitution/term_in_formula} This is the only complicated case. The substitution of the term \( \kappa \) with the term \( \mu \) in the first order formula \( \varphi \) is
    ...
    The rule \eqref{eq:def:first_order_substitution/term_in_formula/quantifiers/trivial} may seem redundant but when doing inductive proofs (e.g. the proof of \fullref{thm:swapping_assignment_compatibility}), we usually need to separately consider the cases where \( \xi \in \boldop{Var}(\kappa) \) and \( \xi \not\in \boldop{Var}(\kappa) \setminus \boldop{Var}(\mu) \) and the rule \eqref{eq:def:first_order_substitution/term_in_formula/quantifiers/direct} being trivial simplifies the proofs.
...
\begin{remark}\label{rem:first_order_substitution_renaming_justification}
  ...
  \begin{thmenum}
    \labitem{rem:first_order_substitution_renaming_justification/capturing} Renaming mitigates \enquote{capturing} free variables as in
    \begin{equation*}
    ...
    \labitem{rem:first_order_substitution_renaming_justification/colliding} Renaming mitigates \enquote{colliding} multiple bound variables as in
    \begin{equation*}
...
\begin{example}\label{ex:first_order_substitution}
  ...
  \begin{thmenum}
    \labitem{ex:first_order_substitution/1} The trivial case without actual substitution:
    \begin{align*}
      &{}\phantom{=}{}
    ...
    \labitem{ex:first_order_substitution/2} A simple substitution without renaming:
    \begin{align*}
      &{}\phantom{=}{}
      \parens[\Big]{\qforall{\xi} p(\xi, \eta)}[\eta \mapsto \zeta]
    ...
    \labitem{ex:first_order_substitution/3} A simple renaming without actual substitution:
    \begin{align*}
      &{}\phantom{=}{}
    ...
    \labitem{ex:first_order_substitution/4} \Fullref{ex:first_order_substitution/3} but with \( \mu \) in \eqref{eq:def:first_order_substitution/term_in_formula} containing \( \xi \) indirectly:
    \begin{align*}
      &{}\phantom{=}{}
    ...
    \labitem{ex:first_order_substitution/5} Only renaming with multiple quantifiers which shows the limitations of \eqref{eq:def:first_order_substitution/term_in_formula/quantifiers/trivial}:
    \begin{align*}
      &{}\phantom{=}{}
    ...
    \labitem{ex:first_order_substitution/6} Both renaming and substitution with multiple quantifiers:
    \begin{align*}
      &{}\phantom{=}{}
    ...
    \labitem{ex:first_order_substitution/7} Substitution of more general terms than variables with renaming of term's variables:
    \begin{align*}
      &{}\phantom{=}{}
...
\begin{proposition}\label{thm:swapping_assignment_compatibility}
  ...
  \begin{thmenum}
    \labitem{thm:swapping_assignment_compatibility/terms} For any term \( \tau \) and any two variables \( \xi \) and \( \eta \), we have
    ...
    \labitem{thm:swapping_assignment_compatibility/formulas} For any formula \( \varphi \), any \( \eta \not\in \boldop{Var}(\varphi) \cup \boldop{Var}(\mu) \) and any variable \( \xi \) we have
...
\begin{proposition}\label{thm:first_order_substitution_equivalence}
  ...
  \begin{thmenum}
    ...
    \labitem{thm:first_order_substitution_equivalence/term_in_term} Let \( \tau \) be a \hyperref[def:first_order_syntax/term]{first order term} and let \( \kappa \) be a \hyperref[def:first_order_syntax/subterm]{subterm} of \( \tau \). Let \( \mu \) be another term such that
    ...
    \labitem{thm:first_order_substitution_equivalence/term_in_formula} Let \( \varphi \) be a \hyperref[def:first_order_syntax/formula]{first order formula} and let \( \kappa \) be a \hyperref[def:first_order_syntax/formula_terms]{term of \( \varphi \)}. Let \( \mu \) be another term such that
    ...
    \labitem{thm:first_order_substitution_equivalence/formula_in_formula} Let \( \varphi \) be a \hyperref[def:first_order_syntax/formula]{first order formula} and let \( \rho \) be a \hyperref[def:first_order_syntax/subformula]{subformula of \( \varphi \)}. Let \( \theta \) be another formula such that
