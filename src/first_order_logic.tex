...
\begin{remark}\label{rem:first_order_formula_conventions}
  ...
  \begin{thmenum}
    ...
    If needed, we can consider these new functions and predicates to be abbreviations for more verbose terms and formulas like in \fullref{thm:transfinite_induction}, where we use substitution in a certain formula \( \op{isord} \) instead of introducing a new predicate.
  \end{thmenum}
...
\begin{definition}\label{def:first_order_structure}\mcite[def. 16.1]{OpenLogicFull}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:first_order_structure/interpretation} The \term{interpretation} \( I \) of the structure \( \mscrX \) is a \hyperref[def:function/single_valued]{function} that is defined on the signature of \( \mscrL \) and satisfies the following conditions:
    \begin{thmenum}
      \thmitem{def:first_order_structure/interpretation/function} For every \( n \)-ary function symbol \( f \), its interpretation is a \hyperref[def:function]{function} of type \( I(f): X^n \to X \).

      \thmitem{def:first_order_structure/interpretation/predicate} For every \( n \)-ary predicate \( p \), its interpretation is a an n-ary \hyperref[def:relation]{relation} \( I(p) \subseteq X^n \). This relation corresponds to all tuples of values that satisfy the predicate within the structure.
    \end{thmenum}
  \end{thmenum}
\end{definition}

\begin{remark}\label{rem:first_order_model_notation}
  In first-order logic, \hyperref[def:first_order_semantics/satisfiability]{models} are defined as pairs \( (\mscrX, I) \). Outside of logic, models are usually denoted more handwavily since either a part of the interpretation or the whole interpretation is clear from the context. We say that the set \( X \) is \term{endowed} with the structure \( I \) (see \hyperref[def:category_of_first_order_models]{model categories}).

  For example, in the sense of first-order structures, the language of the \hyperref[def:group/theory]{theory of groups} has a signature consisting of three functional symbols and no predicate symbols. Specifying a structure for this language is thus the same as specifying a quadruple \( \mscrG = (G, e, (\placeholder)^{-1}, \cdot) \). Since group operations are named using a convention, we often say that \( \mscrG \) is a group and then simply use subscripts like \( e_{\mscrG} \) to avoid ambiguity.

  A popular convention is to use compatible letters like we did with \( X \) and \( \mscrX \) or \( G \) and \( \mscrG \), where the structure itself is named using calligraphic letters while the domain is named using the corresponding capital letter in normal font.

  In very simple cases we can say \enquote{Let \( \mscrP = (P, \leq) \) be a \hyperref[def:poset]{poset}}, however in the case of ordered Banach algebras, for example, this becomes \( \mscrX = (X, \BbbK, +, \cdot, \odot, \norm \placeholder, \leq) \). It is more conventional in this case to say \enquote{Let \( \mscrX = (X, \norm \placeholder, \leq) \) be an ordered Banach algebra} that everything else is implicitly defined. This is our approach to \hyperref[def:semilattice]{lattices}, where we say that \( \mscrX = (X, \leq) \) is a lattice because all operations can be defined only in terms of \( \leq \).

  For simplicity, we will use the simpler terminology \enquote{Let \( U \) be a subset of the vector space \( \mscrV \)} rather than \enquote{Let \( U \) be a subset of the domain \( V \) of the vector space \( \mscrV \)} and \enquote{The function \( f: \mscrX \to \mscrY \) is an order-homomorphism} rather than \enquote{The function \( f: X \to Y \) is a homomorphism between the corresponding lattices}.
\end{remark}

\begin{definition}\label{def:first_order_valuation}
  Fix a structure \( \mscrX \) for a first-order logic language \( \mscrL \).
...
\begin{definition}\label{def:first_order_definability}
  Fix a \hyperref[def:first_order_syntax]{first-order language} \( \mscrL \) and a \hyperref[def:first_order_structure]{structure} \( \mscrX \) on \( \mscrL \).
...
\begin{definition}\label{def:first_order_equation}
  ...
  Given a structure \( \mscrX \), we call the elements of the set defined by this formula \term{solutions}. That is, we say that the tuple \( (x_1, \ldots, x_n) \) is a solution to the equation \eqref{eq:def:first_order_equation} if
  \begin{equation*}
...
\begin{definition}\label{def:first_order_semantics}
  ...
  \begin{thmenum}
    \thmitem{def:first_order_semantics/satisfiability}\mcite[def. 16.11]{OpenLogicFull} Given a \hyperref[def:first_order_structure]{structure} \( \mscrX \), a \hyperref[def:first_order_valuation/variable_assignment]{variable assignment} \( v \) and a set \( \Gamma \) of \hyperref[def:first_order_syntax/formula]{first-order formulas}, we say that the variable assignment \( v \) \term{satisfies} \( \Gamma \) and we write \( \mscrX \vDash_v \Gamma \) if, for every formula \( \gamma \in \Gamma \) we have \( \gamma\Bracks{v} = T \).
...
\begin{proposition}\label{thm:renaming_assignment_compatibility}
  ...
  Fix a \hyperref[def:first_order_syntax]{first-order language} \( \mscrL \), a \hyperref[def:first_order_structure]{structure} \( \mscrX \) on \( \mscrL \) and a \hyperref[def:first_order_valuation/variable_assignment]{variable assignment} \( v \) in \( \mscrX \).
...
\begin{proposition}\label{thm:first_order_substitution_equivalence}
  ...
  Fix a \hyperref[def:first_order_structure]{structure} \( \mscrX \) and a \hyperref[def:first_order_valuation/variable_assignment]{variable assignment} \( v \).
...
\begin{proof}
  ...
  Let \( \mscrX \) be a structure over \( \mscrL \) and let \( v \) be a variable assignment. Then
  \begin{align*}
...
\end{proposition}
\begin{proof}
  Let \( \mscrX \) be a structure that satisfies \( \varphi \). Let \( v \) be a variable assignment in \( \mscrX \). Then for any \( x \in X \), the modified variable assignment \( v_{\xi \mapsto x} \) also satisfies \( \varphi \), i.e.
  \begin{equation*}
...
\begin{proof}
  ...
  Let \( \mscrX \) be any structure and let \( v \) be a variable assignment in that structure. Let \( t \coloneqq \tau\Bracks{v} \). Since \( \tau \) is a ground term, we have
  \begin{equation*}
...
\begin{remark}\label{rem:existential_quantifier_removal}
  Suppose that the formula \( \qexists \xi \varphi \) is satisfiable in the language \( \mscrL \). Define the extension \( \widetilde \mscrL \)  of \( \mscrL \) by adjoining a new constant \( c \). Then \fullref{thm:quantifier_satisfiability/existential} states that \( \qexists \xi \varphi \) and \( \varphi[\xi \mapsto c] \) are equisatisfiable as formulas in the language \( \widetilde \mscrL \).
...
\begin{theorem}[Semantic deduction theorem]\label{thm:semantic_deduction_theorem}\mcite[thm. 16.29]{OpenLogicFull}
  ...
  This result allows us to connect deduction in the object language and deduction in the metalanguage (see \fullref{rem:induction}). Compare with \fullref{thm:syntactic_deduction_theorem}.
\end{theorem}
