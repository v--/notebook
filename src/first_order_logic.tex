...
\begin{definition}\label{def:first_order_language}\mcite[187]{OpenLogic20201202}
  ...
  \begin{description}
    ...
    \begin{thmenum}[series=def:first_order_language]
      ...
      \thmitem{def:first_order_language/var} A nonempty at most \hyperref[rem:cardinals/countable]{countable} alphabet of variables \( \boldop{Var} \), usually denoted by \( \xi_1, \xi_2, \ldots \) or \( \xi, \eta, \zeta \).
      ...
      \begin{thmenum}
        ...
        \thmitem{def:first_order_language/quantifiers/dot} The dot \( . \) for separating a quantifier from its formula. The dot itself is not itself a quantifier and is not formally necessary - we use it only for readability.
      \end{thmenum}

      \thmitem{def:first_order_language/equality} Optionally, an equality symbol \( \doteq \).
    \end{thmenum}
...
\begin{definition}\label{def:first_order_syntax}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:first_order_syntax/term}\mcite[189]{OpenLogic20201202}The set \( \mscrT \) of \term{terms} is the language \hyperref[def:grammar_derivation/grammar_language]{generated} by this grammar schema with \( \bnfpn{term} \) as a starting rule.
    ...
    \thmitem{def:first_order_syntax/ground_term} A term \( \tau \) is called a \term{closed term} if \( \boldop{Var}(\tau) = \varnothing \).

    \thmitem{def:first_order_syntax/formula} The set \( \mscrF \) of \term{formulas} is the language \hyperref[def:grammar_derivation/grammar_language]{generated} by this grammar schema with \( \bnfpn{formula} \) as a starting rule.
    ...
    \thmitem{def:first_order_syntax/ground_formula} A formula \( \varphi \) is called a \term{closed formula} if \( \boldop{Free}(\varphi) = \varnothing \).
...
\begin{definition}\label{def:first_order_structure}\mcite[def. 14.27]{OpenLogic20201202}
  ...
  \begin{thmenum}
    \thmitem{def:first_order_structure/set} \( \mscrX \) is a nonempty set called the \term{universum} of \( \mscrX \).

    \thmitem{def:first_order_structure/interpretation} \( I \) is a heterogenous (set-theoretic) \hyperref[def:function]{map} called the \term{interpretation} of \( \mscrX \) that we define partially as
    \begin{thmenum}
      \thmitem{def:first_order_structure/interpretation/equality} The interpretation \( I(\doteq) \) of the equality is a \hyperref[def:binary_relation]{binary relation} on \( \mscrX \).
...
\begin{definition}\label{def:first_order_valuation}
  ...
  \begin{thmenum}
    \thmitem{def:first_order_valuation/variable_assignment} A \term{variable assignment} for the variables of \( \mscrL \) is any function \( v: \boldop{Var} \to \mscrX \) (loosely similar to \hyperref[def:propositional_valuation/interpretation]{propositional interpretations}).

    \thmitem{def:first_order_valuation/modified_assignment} For every variable \( \xi \) and every universum element \( x \in \mscrX \) we also define the \term{modified assignment} at \( \xi \) with \( x \):
    \begin{equation*}
    ...
    \thmitem{def:first_order_valuation/term_valuation} The \term{valuation} of a term \( \tau \) is a value in the universum \( \mscrX \) given by
    ...
    \thmitem{def:first_order_valuation/formula_valuation} The \term{valuation} of a formula \( \varphi \) is a Boolean value (see \fullref{def:propositional_valuation}) given by
...
\begin{definition}\label{def:first_order_semantics}\mcite[208]{OpenLogic20201202}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:first_order_semantics/equivalence} We say that \( \varphi \) and \( \psi \) are \term{semantically equivalent} and write \( \varphi \gleichstark \psi \) if both \( \varphi \vDash \psi \) and \( \psi \vDash \varphi \).
  \end{thmenum}
\end{definition}
...
\begin{definition}\label{def:first_order_substitution}
  ...
  \begin{thmenum}
    \thmitem{def:first_order_substitution/predicate} Let \( \varphi \) be a \hyperref[def:propositional_syntax/formula]{propositional formula} with variables \( \boldop{Var}(\varphi) = \set{ P_1, \ldots, P_n } \). For brevity, denote \( V \coloneqq \boldop{Var}(\varphi) \). Let \( \Theta = \set{ \theta_1, \ldots, \theta_n } \) be a set of \hyperref[def:first_order_syntax/formula]{first order formulas}.
    ...
    \thmitem{def:first_order_substitution/term_in_term} The substitution of the \hyperref[def:first_order_syntax/term]{first order term} \( \kappa \) with \( \mu \) in the term \( \tau \) is
    ...
    \thmitem{def:first_order_substitution/term_in_formula} This is the only complicated case. The substitution of the term \( \kappa \) with the term \( \mu \) in the first order formula \( \varphi \) is
...
\begin{proposition}\label{thm:swapping_assignment_compatibility}
  ...
  \begin{thmenum}
    \thmitem{thm:swapping_assignment_compatibility/terms} For any term \( \tau \) and any two variables \( \xi \) and \( \eta \), we have
    ...
    \thmitem{thm:swapping_assignment_compatibility/formulas} For any formula \( \varphi \), any \( \eta \not\in \boldop{Var}(\varphi) \cup \boldop{Var}(\mu) \) and any variable \( \xi \) we have
...
\begin{proposition}\label{thm:first_order_substitution_equivalence}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:first_order_substitution_equivalence/formula_in_formula} Let \( \varphi \) be a \hyperref[def:first_order_syntax/formula]{first order formula} and let \( \rho \) be a \hyperref[def:first_order_syntax/subformula]{subformula of \( \varphi \)}. Let \( \theta \) be another formula such that
