...
\begin{definition}\label{def:first_order_language}\mcite\cite[187]{OpenLogic20201202}
  ...
  \begin{description}
    ...
    \begin{thmenum}[series=def:first_order_language]
      \ilabel{def:first_order_language/propositional} The entirety of the \hyperref[subsec:propositional_logic]{propositional logic alphabet}.

      \ilabel{def:first_order_language/var} A nonempty at most \hyperref[rem:cardinals/countable]{countable} alphabet of variables \( \boldop{Var} \), usually denoted by \( \xi_1, \xi_2, \ldots \) or \( \xi, \eta, \zeta \).
...
\begin{definition}\label{def:first_order_syntax}
  ...
  \begin{thmenum}
    ...
    \ilabel{def:first_order_syntax/ground_term} A term \( \tau \) is called a \term{closed term} if \( \boldop{Var}(\tau) = \varnothing \).
    ...
    \ilabel{def:first_order_syntax/ground_formula} A formula \( \varphi \) is called a \term{closed formula} if \( \boldop{Free}(\varphi) = \varnothing \).
...
\begin{definition}\label{def:first_order_valuation}
  ...
  \begin{thmenum}
    \ilabel{def:first_order_valuation/variable_assignment} A \term{variable assignment} for the variables of \( \mscrL \) is any function \( v: \boldop{Var} \to \mscrX \) (loosely similar to \hyperref[def:propositional_valuation/interpretation]{propositional interpretations}).
...
\begin{definition}\label{def:first_order_substitution}
  ...
  \begin{thmenum}
    \ilabel{def:first_order_substitution/predicate} Let \( \varphi \) be a \hyperref[def:propositional_syntax/formula]{propositional formula} with variables \( \boldop{Var}(\varphi) = \set{ P_1, \ldots, P_n } \). For brevity, denote \( V \coloneqq \boldop{Var}(\varphi) \). Let \( \Theta = \set{ \theta_1, \ldots, \theta_n } \) be a set of \hyperref[def:first_order_syntax/formula]{first order formulas}.
