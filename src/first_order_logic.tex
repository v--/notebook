...
\begin{definition}\label{def:first_order_language}\mcite[187]{OpenLogic20201202}
  ...
  \begin{description}
    ...
    \begin{thmenum}[series=def:first_order_language]
      ...
      \thmitem{def:first_order_language/equality} Optionally, an symbol for \term{formal equality} \( \doteq \).
    \end{thmenum}
  ...
  The logical symbols, with the exception of formal equality, are common for all first-order languages. Thus first-order languages differ by their non-logical symbols. The collection of functional and predicate symbols of a language are sometimes called its \term{signature}. Whether the formal equality symbol is present is also considered a part of the signature.
\end{definition}
...
\begin{definition}\label{def:first_order_syntax}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:first_order_syntax/term}\mcite[189]{OpenLogic20201202} The set \( \boldop{Term} \) of \term{terms} in \( \mscrL \) is the language \hyperref[def:grammar_derivation/grammar_language]{generated} by this grammar schema with \( \bnfpn{term} \) as a starting rule.
    ...
    \thmitem{def:first_order_syntax/ground_term} A term \( \tau \) is called a \term{ground term} if \( \boldop{Var}(\tau) = \varnothing \).
    ...
    \thmitem{def:first_order_syntax/ground_formula} A formula \( \varphi \) is called a \term{ground formula} if \( \boldop{Free}(\varphi) = \varnothing \).
...
\begin{definition}\label{def:first_order_structure}\mcite[def. 14.27]{OpenLogic20201202}
  ...
  \begin{thmenum}
    \thmitem{def:first_order_structure/set} \( \mscrX \) is a nonempty set called the \term{universum} of the structure \( (\mscrX, I) \). See \fullref{rem:empty_models}.
...
\begin{definition}\label{def:first_order_valuation}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:first_order_valuation/formula_valuation} We extend the classical propositional valuations from \fullref{def:propositional_valuation}. The (classical) \term{valuation} of a formula \( \varphi \) is a Boolean value (see \fullref{def:propositional_valuation}) given by
...
\begin{definition}\label{def:first_order_semantics}\mcite[208]{OpenLogic20201202}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:first_order_semantics/theory_of_model} Similarly to \fullref{def:first_order_semantics/theory_of_model}, we define the \term{theory of the model} \( (\mscrX, I) \) to be the set
    \begin{equation*}
      \set{ \varphi \in \boldop{Form} \given (\mscrX, I) \vDash \varphi }.
    \end{equation*}
    ...
    \thmitem{def:first_order_semantics/theory}\mcite[def. 31.1]{OpenLogic20201202} As in \fullref{def:first_order_semantics/theory}, the (semantic) \term{theory} of the set \( \Gamma \) of formulas is the set
    \begin{equation*}
...
\begin{remark}\label{rem:propositional_logic_as_first_order_logic}
  It is now clear that \hyperref[subsec:propositional_logic]{propositional logic} can be regarded as first-order logic with no equality, no functional symbols and at most countably many nullary predicates. Thus first-order logic is indeed an extension of propositional logic.
\end{remark}

\begin{remark}\label{rem:empty_models}
   If we allow for the universum \( \mscrX \) of a structure \( (\mscrX, I) \) to be empty, we would have to reformulate a lot of important theorems (e.g. see the proof of \fullref{thm:renaming_assignment_compatibility/formulas}), which would complicate compatibility between semantics and \hyperref[def:first_order_derivation_system]{derivation systems}.
...
\begin{definition}\label{def:first_order_substitution}
  ...
  As in \fullref{def:propositional_substitution}, we define different kinds of (single) \term{substitution} in more generality that in e.g. \cite[def. 14.25]{OpenLogic20201202}. Where applicable, \term{simultaneous substitution} is defined via the same trick as in \fullref{def:propositional_substitution}.
...
\begin{proposition}\label{thm:semantic_implicit_quantification}
  For any formula \( \varphi \) and any variable \( \xi \) over any first-order language \( \mscrL \), the formulas \( \varphi \) and \( \qforall \xi \varphi \) are \hyperref[def:first_order_semantics/equivalent]{semantically equivalent}.
  ...
  Compare this result with \fullref{thm:syntactic_implicit_quantification}.
\end{proposition}
\begin{proof}
...
\begin{proposition}\label{thm:quantifier_satisfiability}
  ...
  See also \fullref{def:first_order_derivation_system/axioms/terms} for the syntactic counterpart to this proposition.
\end{proposition}
\begin{proof}
