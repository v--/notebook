...
\begin{definition}\label{def:first_order_language}\cite[definition 2.1]{Nerode2012}
  The alphabet for a \uline{first-order predicate language}\Tinyref{def:language} $L$ consists of:
  \begin{description}
...
\begin{definition}\label{def:first_order_structure}\cite[definition 4.1]{Nerode2012}
  ...
  \begin{enumerate}
    ...
    \item For every $n$-ary predicate $p$, a family of tuples\Tinyref{def:cartesian_product} $I(p) \subseteq A^n$ called the \uline{interpretation of $p$}, i.e. all tuples of values that satisfy the predicate within the structure.
  \end{enumerate}
\end{definition}
