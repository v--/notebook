...
\begin{proposition}\label{thm:def:integral_domain/properties}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:def:integral_domain/properties/polynomial_ring} The \hyperref[def:polynomial_semiring]{polynomial ring} \( R[X] \) is an integral domain if and only if \( R \) is.
  \end{thmenum}
\end{proposition}
\begin{proof}
  ...
  \SufficiencySubProof* If \( R[X] \) is an integral domain, by \fullref{thm:def:integral_domain/properties/subring}, so is \( R \).
...
\begin{proposition}\label{thm:def:domain_divisibility/properties}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:def:domain_divisibility/properties/irreducible_in_polynomial_ring} An element of the domain \( D \) is \hyperref[def:domain_divisibility/irreducible]{irreducible} in \( D \) if and only if it is irreducible in \( D[X] \).

    A similar statement about \hyperref[def:domain_divisibility/prime]{prime elements} is also true, but is more complicated --- see \fullref{thm:prime_element_iff_prime_in_polynomial_ring}.
  \end{thmenum}
\end{proposition}
\begin{proof}
  ...
  \SubProofOf{thm:def:domain_divisibility/properties/irreducible_in_polynomial_ring}

  \SufficiencySubProof* Suppose that \( x \) is irreducible in \( D \) and let \( x = y(X) z(X) \). By \fullref{thm:def:polynomial_degree/properties/product}, both \( y(X) \) and \( z(X) \) must be constant polynomials. Therefore, they are scalars, and since \( x \) is irreducible, \( y \) or \( z \) is a unit. By \fullref{thm:def:polynomial_semiring/properties/units}, if \( y \) is a unit in \( D \), it is a unit in \( D[X] \).

  Generalizing on \( x \), it follows that every irreducible element in \( D \) is also irreducible in \( D[X] \).

  \NecessitySubProof* Suppose that \( x \) is irreducible in \( D[X] \) and let \( x = yz \). Then \( y \) or \( z \) is a unit of \( D[X] \), and thus again by \fullref{thm:def:polynomial_semiring/properties/units}, it is a unit of \( D \).
...
\begin{definition}\label{def:greatest_common_divisor_domain}
...
\begin{definition}\label{def:gcd_and_lcm}\mimprovised
  Fix arbitrary elements \( x \) and \( y \) of a \hyperref[def:greatest_common_divisor_domain]{greatest common divisor domain}.
  ...
  Due to \fullref{thm:def:greatest_common_divisor_domain/properties/join}, we can extend GCDs and LCMs to finitely many elements rather than only two.
\end{definition}
...
\begin{proposition}\label{thm:def:greatest_common_divisor_domain/properties}
  \hyperref[def:greatest_common_divisor_domain]{Greatest common divisor domains} have the following basic properties:
  \begin{thmenum}
    \thmitem{thm:def:greatest_common_divisor_domain/properties/join} A lattice joins, greatest common divisors are \hyperref[def:magma/associative]{associative}, \hyperref[def:magma/commutative]{commutative} and \hyperref[def:magma/idempotent]{idempotent} when considered as a binary operation.

    \thmitem{thm:def:greatest_common_divisor_domain/properties/polynomial_ring} If the \hyperref[def:polynomial_semiring]{polynomial ring} \( D[X] \) is a greatest common divisor domain, then \( D \) also is.

    The converse to this is true, but it is more difficult to prove. See \fullref{thm:polynomial_ring_over_gcd_domain}.
  \end{thmenum}
\end{proposition}
\begin{proof}
  \SubProofOf{thm:def:greatest_common_divisor_domain/properties/join} Follows from \fullref{thm:binary_lattice_operations/semilattices}.

  \SubProofOf{thm:def:greatest_common_divisor_domain/properties/polynomial_ring} Then, given elements \( x \) and \( y \) from \( D \), \( \gcd(x, y) \) is a polynomial in \( D[X] \) that divides each of the elements. Hence, by \fullref{thm:def:polynomial_degree/properties/product}, the GCD is a constant polynomial. We may thus regard it as an element of \( D[X] \).

  Therefore, \( D \) is also a greatest common divisor domain.
\end{proof}

\begin{definition}\label{def:coprime_elements}
  We say that the nonzero elements \( x \) and \( y \) in a \hyperref[def:greatest_common_divisor_domain]{greatest common divisor domain} are \term{coprime} if any the \hyperref[def:gcd_and_lcm]{greatest common divisor} of \( x \) and \( y \) is a unit.

  In this form, the definition is unfortunately inconsistent with \hyperref[def:semiring_ideal/coprime]{coprime ideals}. See \fullref{thm:def:principal_ideal_domain/properties/coprime}.
\end{definition}
...
\begin{proof}
  ...
  \begin{equation*}
    \sum_{i+j=k} a_i b_j
  \end{equation*}
  of \( p(X) q(X) \), and hence also \( a_i b_j \) for every particular pair of indices \( i = 0, \ldots, n \) and \( j = 0, \ldots, m \).

  For any fixed \( i \), \( d \) divides \( a_i b_j \) for every \( j = 0, \ldots, m \). Since \( q(X) \) is primitive, \( d \) cannot divide \( b_j \) for every \( j \). Hence, \( d \) necessarily divides \( a_i \). Our choice of \( i \) was arbitrary, hence \( d \) divides \( a_i \) for every \( i = 1, \ldots, n \). But \( p(X) \) is also primitive. Therefore, \( d \) can only be a unit.
\end{proof}

\begin{corollary}\label{thm:content_is_multiplicative}
  The \hyperref[def:polynomial_content]{content} of a polynomial, when regarded as a function, is multiplicative, meaning that
  \begin{equation}\label{eq:thm:gauss_lemma}
    \op{cont}(p q) = \op{cont}(p) \op{cont}(q)
  \end{equation}
  modulo multiplication by a unit.
\end{corollary}
\begin{proof}
  Let \( p(X) = \op{cont}(p) p_0(X) \) and \( q(X) = \op{cont}(q) q_0(X) \). We have
  \begin{equation*}
    p(X) q(X) = \op{cont}(p) \op{cont}(q) p_0(X) q_0(X).
  \end{equation*}

  Applying \fullref{thm:gauss_lemma} to \( p_0(X) \) and \( q_0(X) \), we conclude that \( p_0(X) q_0(X) \) is a primitive polynomial, implying that \( \op{cont}(p) \op{cont}(q) \) is the content of \( p(X) q(X) \).
\end{proof}
...
\begin{proposition}\label{thm:def:irreducible_factorization/properties}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:def:irreducible_factorization/properties/polynomial_ring} For any domain \( D \), \( x = u p_1 \cdots p_n \) is an irreducible factorization in \( D \) if and only if it is an irreducible factorization of \( x \) in \( D[X] \).
  \end{thmenum}
\end{proposition}
\begin{proof}
  ...
  \begin{equation}\label{eq:def:irreducible_factorization/properties/uniqueness/proof/assumption}
    x = u p_1 \cdots p_n = v q_1 \cdots q_m.
  \end{equation}
  ...
  \begin{equation*}
    ...
    \end{cases}
  \end{equation*}
  is a permutation witnessing the equivalence of the factorizations \eqref{eq:def:irreducible_factorization/properties/uniqueness/proof/assumption}.

  \SubProofOf{thm:def:irreducible_factorization/properties/polynomial_ring}

  \SufficiencySubProof Suppose that
  \begin{equation}\label{eq:thm:def:irreducible_factorization/properties/polynomial_ring/sufficiency_assumption}
    x = u(X) p_1(X) \cdots p_n(X)
  \end{equation}
  be an irreducible factorization of \( x \in D \) in \( D[X] \). By \fullref{thm:def:polynomial_degree/properties/product}, all polynomials in this factorization are constants. By \fullref{thm:def:domain_divisibility/properties/irreducible_in_polynomial_ring}, since they are irreducible in \( D[X] \), they are also irreducible in \( D \).

  Therefore, \eqref{eq:thm:def:irreducible_factorization/properties/polynomial_ring/sufficiency_assumption} is an irreducible factorization of \( x \) in \( D \).

  \NecessitySubProof Due to \fullref{thm:def:polynomial_semiring/properties/units} and \fullref{thm:def:domain_divisibility/properties/irreducible_in_polynomial_ring}, irreducible elements and units in \( D \) are also irreducibles and units in \( D[X] \). Hence, every irreducible factorization in \( D \) is also an irreducible factorization in \( D[X] \).
\end{proof}
...
\begin{defproof}
  ...
  \ImplicationSubProof{def:unique_factorization_domain/ideals_and_primes}{def:unique_factorization_domain/factorization} Follows from \fullref{thm:def:irreducible_factorization/properties/existence} and \fullref{thm:def:irreducible_factorization/properties/uniqueness}.
\end{defproof}
...
\begin{proposition}\label{thm:def:unique_factorization_domain/properties}
  ...
  \begin{thmenum}
    \thmitem{thm:def:unique_factorization_domain/properties/polynomial_ring} If the \hyperref[def:polynomial_semiring]{polynomial ring} \( D[X] \) is a unique factorization domain, then \( D \) also is.

    The converse to this is true, but it is more difficult to prove. See \fullref{thm:polynomial_ring_over_ufd}.

    \thmitem{thm:def:unique_factorization_domain/properties/gcd} Every unique factorization domain is a \hyperref[def:greatest_common_divisor_domain]{greatest common divisor domain}.
  \end{thmenum}
\end{proposition}
\begin{proof}
  \SubProofOf{thm:def:unique_factorization_domain/properties/polynomial_ring} Suppose that \( D[X] \) is a unique factorization. By \fullref{thm:def:irreducible_factorization/properties/polynomial_ring}, every irreducible factorization of \( x \in D \) in \( D[X] \) is also an irreducible factorization in \( D[X] \). This implies both existence and uniqueness.

  \SubProofOf{thm:def:unique_factorization_domain/properties/gcd} Fix arbitrary elements \( x \) and \( y \). We will show that they have a greatest common divisor.
...
\begin{proposition}\label{thm:def:principal_ideal_domain/properties}
  ...
  \begin{thmenum}
    \thmitem{thm:def:principal_ideal_domain/properties/noetherian} Every principal ideal domain is \hyperref[def:noetherian_semiring]{noetherian}.

    \thmitem{thm:def:principal_ideal_domain/properties/ufd} Every principal ideal domain is a \hyperref[def:unique_factorization_domain]{unique factorization domain}.
    ...
    \thmitem{thm:def:principal_ideal_domain/properties/gcd} For the \hyperref[def:gcd_and_lcm]{greatest common divisor}, we have
    \begin{equation*}
    ...
    \thmitem{thm:def:principal_ideal_domain/properties/coprime} \( x \) and \( y \) are \hyperref[def:coprime_elements]{coprime elements} if and only if \( \braket{ x } \) and \( \braket{ y } \) are \hyperref[def:coprime ideal]{coprime ideals}.
  \end{thmenum}
\end{proposition}
\begin{proof}
  \SubProofOf{thm:def:principal_ideal_domain/properties/noetherian} In a principal ideal domain every ideal is generated by a single element, hence the domain satisfies \fullref{def:noetherian_semiring/generated}. Thus, it is noetherian.

  \SubProofOf{thm:def:principal_ideal_domain/properties/ufd} Suppose that \( x \) is an irreducible element of a principal ideal domain. Then \( x \) satisfies \fullref{def:domain_divisibility/irreducible/ideals}, and hence \( \braket{ x } \) is a maximal ideal. By \fullref{thm:def:semiring_ideal/properties/maximal_is_prime}, maximal ideals are prime, and hence \( x \) is a prime element.

  Therefore, every irreducible element is prime. Combined with \fullref{thm:def:principal_ideal_domain/properties/noetherian}, this implies that the domain satisfies \fullref{def:unique_factorization_domain/primes_and_ideals}, and is hence a unique factorization domain.
  ...
  \SubProofOf{thm:def:principal_ideal_domain/properties/gcd} Follows from \fullref{thm:semiring_of_ideals/lattice} by nothing that every ideal is principal.

  \SubProofOf{thm:def:principal_ideal_domain/properties/coprime} Follows from \fullref{thm:def:principal_ideal_domain/properties/gcd}.
\end{proof}
...
\end{theorem}
\begin{proof}
  Clearly \( \ifrac x {\gcd(x, y)} \) and \( \ifrac y {\gcd(x, y)} \) are \hyperref[def:coprime_elements]{coprime elements}. By \fullref{thm:def:principal_ideal_domain/properties/coprime}, there exist elements \( a \) and \( b \) such that
  \begin{equation*}
...
\begin{example}\label{ex:ufd_but_not_pid}\mcite{ProofWiki:polynomials_in_integers_is_not_principal_ideal_domain}
  ...
  Note that \( \BbbZ[X] \) is a unique factorization domain by \fullref{thm:def:unique_factorization_domain/properties/polynomial_ring}.
...
\begin{definition}\label{def:euclidean_domain}\mcite{nLab:euclidean_domain}
  ...
  \begin{equation}\label{eq:def:euclidean_domain/division}
    a = bq + r
  \end{equation}
  holds and either \( r = 0_D \) or \( \delta(r) < \delta(b) \).
...
\begin{proposition}\label{thm:def:euclidean_domain/properties}
  ...
  \begin{thmenum}
    \thmitem{thm:def:euclidean_domain/properties/pid} Every Euclidean domain is a \hyperref[def:principal_ideal_domain]{principal ideal domain}.

    Counterexamples to the converse are discussed in \cite{Anderson1988}.
  \end{thmenum}
\end{proposition}
\begin{proof}
  \SubProofOf{thm:def:euclidean_domain/properties/pid} Fix an ideal \( I \) of the Euclidean domain \( D \). By \fullref{thm:natural_numbers_are_well_ordered}, the set \( \delta(I) \) has a minimum. Choose an element \( m \in I \) such that \( \delta(m) = \min \delta(I) \). Obviously \( \braket{m} \subseteq I \). We will prove that \( I = \subseteq m \).
