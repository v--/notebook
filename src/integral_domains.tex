...
\begin{proposition}\label{thm:def:integral_domain/properties}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:def:integral_domain/polynomial_ring} The \hyperref[def:polynomial_semiring]{polynomial ring} \( R[X] \) is an integral domain if and only if \( R \) is.
  \end{thmenum}
\end{proposition}
\begin{proof}
  ...
  \SufficiencySubProof* If \( R[X] \) is an integral domain, by \fullref{thm:def:integral_domain/subring}, so is \( R \).

  \NecessitySubProof* It is sufficient to prove the statement for one indeterminate. If \( p(X) \) and \( q(X) \) are nonzero polynomials, then so is \( p(X) q(X) \) by \fullref{thm:def:polynomial_degree/product}.
\end{proof}
...
\begin{definition}\label{def:gcd_and_lcm}\mimprovised
  ...
  Due to \fullref{thm:def:greatest_common_divisor_domain/join}, we can extend GCDs and LCMs to finitely many elements rather than only two.
\end{definition}
...
\begin{proposition}\label{thm:def:greatest_common_divisor_domain/properties}
  ...
  \begin{thmenum}
    \thmitem{thm:def:greatest_common_divisor_domain/join} A lattice joins, greatest common divisors are \hyperref[def:magma/associative]{associative}, \hyperref[def:magma/commutative]{commutative} and \hyperref[def:magma/idempotent]{idempotent} when considered as a binary operation.

    \thmitem{thm:def:greatest_common_divisor_domain/polynomial_ring} If the \hyperref[def:polynomial_semiring]{polynomial ring} \( D[X] \) is a greatest common divisor domain, then \( D \) also is.

    The converse to this is true, but it is more difficult to prove. See \fullref{thm:polynomial_ring_over_gcd_domain}.
  \end{thmenum}
\end{proposition}
\begin{proof}
  \SubProofOf{thm:def:greatest_common_divisor_domain/join} Follows from \fullref{thm:binary_lattice_operations/semilattices}.

  \SubProofOf{thm:def:greatest_common_divisor_domain/polynomial_ring} Then, given elements \( x \) and \( y \) from \( D \), \( \gcd(x, y) \) is a polynomial in \( D[X] \) that divides each of the elements. Hence, by \fullref{thm:def:polynomial_degree/product}, the GCD is a constant polynomial. We may thus regard it as an element of \( D[X] \).
...
\begin{defproof}
  ...
  \ImplicationSubProof{def:unique_factorization_domain/ideals_and_primes}{def:unique_factorization_domain/factorization} Follows from \fullref{thm:def:irreducible_factorization/existence} and \fullref{thm:def:irreducible_factorization/uniqueness}.
\end{defproof}
...
\begin{proposition}\label{thm:def:unique_factorization_domain/properties}
  ...
  \begin{thmenum}
    \thmitem{thm:def:unique_factorization_domain/polynomial_ring} If the \hyperref[def:polynomial_semiring]{polynomial ring} \( D[X] \) is a unique factorization domain, then \( D \) also is.

    The converse to this is true, but it is more difficult to prove. See \fullref{thm:polynomial_ring_over_ufd}.

    \thmitem{thm:def:unique_factorization_domain/gcd} Every unique factorization domain is a \hyperref[def:greatest_common_divisor_domain]{greatest common divisor domain}.
  \end{thmenum}
\end{proposition}
\begin{proof}
...
\begin{proposition}\label{thm:def:principal_ideal_domain/properties}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:def:principal_ideal_domain/maximal_ideal_is_prime} \hyperref[def:semiring_ideal/prime]{Prime ideals} in a principal ideal domains are \hyperref[def:semiring_ideal/maximal]{maximal}.
...
\begin{proof}
  ...
  \SubProofOf{thm:def:principal_ideal_domain/maximal_ideal_is_prime} Let \( P \) be a prime ideal in a principal ideal domain. Then \( P = \braket{ p } \) for some prime element \( p \). By \fullref{thm:def:domain_divisibility/prime_is_irreducible}, \( p \) is irreducible, and hence \( \braket{ p } \) is a maximal ideal.
