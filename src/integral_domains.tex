...
\begin{definition}\label{def:gcd_and_lcm}\mimprovised
  ...
  Due to \fullref{thm:def:greatest_common_divisor_domain/properties/associativity}, we can extend GCDs and LCMs to finitely many elements rather than only two.
\end{definition}
...
\begin{proposition}\label{thm:def:greatest_common_divisor_domain/properties}
  ...
  \begin{thmenum}
    \thmitem{thm:def:greatest_common_divisor_domain/properties/associativity} We have the following \hyperref[def:magma/associative]{associativity}-like property:
    \begin{equation*}
...
\end{proposition}
\begin{proof}
  \SubProofOf{thm:def:greatest_common_divisor_domain/properties/associativity} Follows from the associativity of lattice operations proved in \fullref{thm:binary_lattice_operations/semilattices}.
...
\begin{theorem}[Bezout's identity]\label{thm:bezouts_identity}
  In a \hyperref[def:principal_ideal_domain]{principal ideal domain}, if the \hyperref[def:gcd_and_lcm]{greatest common divisor} of \( x \) and \( y \) is a unit, then there exist some elements \( a \) and \( b \) such that
...
\end{theorem}
\begin{proof}
  We state this because it is a famous theorem, but in our settings, it is a simple corollary of \fullref{thm:def:principal_ideal_domain/properties/coprime}.
\end{proof}
