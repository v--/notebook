...
\begin{definition}\label{def:set_partition}
  Let \( X \) be a set. A \textbf{partition of \( X \)} is a disjoint family\Tinyref{remark:family_of_sets} \( P \subseteq \Power(X) \) of nonempty sets such that \( X = \bigcup P \). In other words, each element of \( X \) belong to exactly one set in \( P \).
\end{definition}
