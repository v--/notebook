...
\begin{definition}\label{def:order}
  ...
  \begin{defenum}
    \DItem{def:order/preorder}\cite{nLab:preorder} The relation \( \sim \) is called a \Def{preorder} if it satisfies
    \begin{defenum}
    ...
    We define \Def{equivalence classes} or \Def{cosets} to be sets of the form
    \begin{equation*}
    ...
    \begin{align*}
      ...
      &\pi(a) \coloneqq [a].
    \end{align*}
    the canonical projection. See \fullref{thm:equivalence_partition}.
    ...
    A set with a partial order is called a \Def{partially ordered set or poset}. See \fullref{def:poset}.
    ...
    See \fullref{thm:strict_partial_order_conversion}.
  \end{defenum}
\end{definition}
...
\end{proposition}
\begin{proof}
  By \fullref{thm:first_order_equality_is_equivalence_relation}, equality itself is an equivalence relation. It is equivalent to the \enquote{diagonal} relation\Tinyref{def:relation}
  \begin{equation*}
    \{ (x, x) \colon x \in X \}.
  \end{equation*}

  This relation is a subset of any reflexive\Tinyref{def:order/equivalence/reflexivity} relation, hence it is contained in any equivalence relation and, thus, in the intersection of all equivalence relations. But it is itself an equivalence relation, hence the containment is not strict and \( = \) is the intersection of all equivalence relations.
\end{proof}

\begin{definition}\label{def:set_partition}
  Let \( X \) be a set. A \Def{cover of \( X \)} is a family\Tinyref{remark:family_of_sets} \( P \subseteq \Power(X) \) of nonempty sets such that \( X = \bigcup P \).

  A \Def{partition} of \( X \) is a pairwise disjoint cover. In other words, each element of \( X \) belong to exactly one set in a partition \( P \).
\end{definition}

\begin{proposition}\label{thm:equivalence_partition}
  Fix a set \( X \). Let \( \sim \) be a relation of \( X \). The following are equivalent:
  \begin{defenum}
    \DItem{thm:equivalence_partition/partition} There exists a partition\Tinyref{def:set_partition} \( P \) of \( X \) such that
    \begin{equation}\label{thm:equivalence_partition/partition/property}
      a \sim b \iff \exists A \in P: \{ a, b \} \subseteq A.
    \end{equation}

    \DItem{thm:equivalence_partition/equivalence} \( \sim \) is an equivalence relation\Tinyref{def:order/equivalence}.
...
\begin{proof}
  ...
  \Implies[thm:equivalence_partition/partition][thm:equivalence_partition/function] Let \( P \) be a partition of \( X \) satisfying \fullref{thm:equivalence_partition/partition/property}. Denote by \( P_x \) the set in \( P \) which contains \( x \) and define the function
  \begin{align*}
    &f: X \to P \\
    &f(x) = P_x.
  \end{align*}
  ...
  \Implies[thm:equivalence_partition/function][thm:equivalence_partition/equivalence] Let \( f: X \to Y \) be a function such that
  \begin{equation*}
    f(a) = f(b) \iff a \sim b.
  \end{equation*}

  Then \( \sum \) inherits \fullref{def:order/equivalence/reflexivity}, \fullref{def:order/equivalence/symmetry} and \fullref{def:order/equivalence/transitivity} from the equality equivalence relation \( = \)\Tinyref{thm:equality_is_smallest_equivalence_relation}.
\end{proof}
...
\begin{proof}
  ...
  \begin{description}
    \RItem{def:order/strict_partial/transitivity} Let \( x < y \) and \( y < z \). Thus \( x \leq y \) and \( y \leq z \). By \fullref{def:order/partial/transitivity}, \( x \leq z \).

    Additionally, \( x \neq y \) and \( y \neq z \). Assume\LEM that \( x = z \). By \fullref{def:order/partial/reflexivity} we have \( z \leq x \) and, since \( y \leq z \), by \fullref{def:order/partial/transitivity} we obtain \( y \leq x \). But since \( x \leq y \), by \fullref{def:order/partial/antisymmetry}, we have \( x = y \), which contradicts the assumption that \( x < y \).

    Hence \( x < z \).

    \RItem{def:order/strict_partial/irreflexivity} Fix \( x \in X \). By \fullref{def:order/partial/reflexivity}, \( x \leq x \) and by definition,
    \begin{equation*}
  ...
  \begin{description}
    \RItem{def:order/partial/reflexivity} Fix \( x \in X \) and assume\LEM that \( x \not\leq x \). Then \( x \neq x \) which contradicts the \fullref{def:order/equivalence/reflexivity} of equality (see \fullref{thm:equality_is_smallest_equivalence_relation}). Hence \( x \leq x \).

    \RItem{def:order/partial/antisymmetry} Let \( x \leq y \) and \( y \leq x \), that is, either \( x = y \) or both \( x < y \) and \( y < x \) hold. Assume\LEM the latter. By \fullref{def:order/strict_partial/transitivity}, we have \( x < x \), which contradicts \fullref{def:order/strict_partial/irreflexivity}. Hence \( x = y \).
    ...
    \RItem{def:order/partial/totality} If \( < \) is a strict total order, then \( \leq \) is a total order. Fix \( x, y \in X \). By \fullref{def:order/strict_partial/trichotomy}, either
    \begin{itemize}
