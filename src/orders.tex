...
\end{proposition}
\begin{proof}
  By \cref{thm:first_order_equality_is_equivalence_relation}, equality itself is an equivalence relation. It is equivalent to the \enquote{diagonal} relation\Tinyref{def:relation}
  \begin{equation*}
    \{ (x, x) \colon x \in X \}.
  \end{equation*}
...
\begin{proof}
  ...
  \Implies[thm:equivalence_partition/partition][thm:equivalence_partition/function] Let \( P \) be a partition of \( X \) satisfying \cref{thm:equivalence_partition/partition/property}. Denote by \( P_x \) the set in \( P \) which contains \( x \) and define the function
  \begin{align*}
    &f: X \to P \\
    &f(x) = P_x.
  \end{align*}
  ...
  Then \( \sum \) inherits \cref{def:order/equivalence/reflexivity}, \cref{def:order/equivalence/symmetry} and \cref{def:order/equivalence/transitivity} from the equality equivalence relation \( = \)\Tinyref{thm:equality_is_smallest_equivalence_relation}.
\end{proof}
