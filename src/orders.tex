...
\begin{definition}\label{def:order}
  ...
  \begin{defenum}
    ...
    \item\label{def:order/directed}\cite[8]{Engelking1989} A preordered set\Tinyref{def:order/preorder} $(X, \leq)$ is called a \ul{directed set} (there is no ubiquitous name for the relation itself) if every two elements have an upper bound\Tinyref{def:poset/upper_lower_bound}, i.e.
    \begin{description}
...
\begin{proposition}\label{thm:strict_partial_order_conversion}
  Fix a set $X$. Then the relation $\leq$ is a partial order\Tinyref{def:order/partial_order} on $X$ if and only if the relation $<$ is a strict partial order\Tinyref{def:order/strict_partial} on $X$, where
  \begin{align*}
