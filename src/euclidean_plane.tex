...
\begin{proof}
  ...
  \UniquenessSubProof Suppose that \( f(v) \) and \( g(v) \) are rigid motions sending the ray \( r(t) \) to \( s(t) \). We have already shown that \( O \) is a fixed point of both \( f(v) \) and \( g(v) \), hence, by \fullref{thm:affine_operator_fixed_point}, there exist some rotation operators \( F \) and \( G \) such that \( f(v) = O + F(v - o) \) and \( g(v) = O + G(v - o) \). Then
  \begin{equation*}
