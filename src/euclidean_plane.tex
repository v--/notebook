...
\begin{definition}\label{def:angle}
  ...
  \begin{thmenum}
    ...
    \begin{figure}[!ht]
      \centering
      \includegraphics[align=c]{output/def__angle__measure__right.pdf}
    ...
    \thmitem{def:angle/undirected} Given the transformation \( f(v) \) sending \( r \) to \( s \), its inverse \( f^{-1}(v) \) sends \( s \) to \( r \). Their composition is the identity, whose angle measure is a multiple \( 2\pi \). \Fullref{thm:plane_rotation_matrix_angle} implies that the angle measures \( \sphericalangle(r, s) \) and \( \sphericalangle(s, r) \) sum to \( 2\pi \). We call the angle with the smaller measure the \term{undirected angle} between \( r \) and \( s \).
  \end{thmenum}
\end{definition}
...
\begin{proposition}\label{thm:adjacent_angles}
  ...
  \begin{figure}[!ht]
    \centering
    \includegraphics[align=c]{output/thm__adjacent_angles.pdf}
...
\begin{definition}\label{def:perpendicularity}\mimprovised
  We say that two vectors are \term{perpendicular} if the \hyperref[def:undirected_angle]{undirected angle} between them is \hyperref[def:angle/measure/right]{right}.
...
\begin{proposition}\label{thm:angles_of_crossing}
  ...
  We call the smaller of the two undirected angle measures the \term{angle measure} \( \sphericalangle(g, h) \) of the line crossing itself.
...
\begin{proposition}\label{thm:angles_of_transversal}
  ...
  Then the angles \( \sphericalangle(\vect{PG}, \vect{PR}) \) and \( \sphericalangle(\vect{QH}, \vect{QR}) \) have equal measures. We call them \term[bg=вътрешно-кръстни ъгли]{interior alternate angles}. \Fullref{thm:angles_of_crossing} implies that other pairs of angles of the crossed lines are also equal.
...
\begin{remark}\label{rem:pythagoras_theorem_right_triangle}
  \Fullref{thm:pythagoras_theorem} generalizes a statement about right triangles --- the squared length of the hypotenuse equals the sum of the squared lengths of the legs.

  In a triangle with vertices \( A \), \( B \) and \( C \), if \( [A, B] \) is the hypotenuse, then \( \norm{CA} \) and \( \norm{CB} \) are orthogonal. \Fullref{thm:pythagoras_theorem} then implies that
  \begin{equation*}
    \norm{AB}^2 = \norm{AC}^2 + \norm{CB}^2.
  \end{equation*}
\end{remark}
...
\begin{proposition}\label{thm:trapezoid_triangulation}
  Consider a \hyperref[def:trapezoid]{trapezoid} \( T \) with vertices \( A \), \( B \), \( C \) and \( D \). Let \( T_{ABC} \) and \( T_{ADC} \) be \hyperref[def:triangle]{triangles} with the corresponding vertices. Then
  \begin{equation*}
    T = T_{ABC} \cup T_{ADC}.
  \end{equation*}

  We call this decomposition \term{trapezoid triangulation}.

  \begin{figure}[!ht]
    \centering
    \includegraphics[align=c]{output/thm__trapezoid_triangulation.pdf}
    \caption{\hyperref[thm:trapezoid_triangulation]{Trapezoid triangulation}.}\label{fig:thm:trapezoid_triangulation}
  \end{figure}
\end{proposition}
\begin{proof}
  No three vertices of the trapezoid are collinear, hence \( T_{ABC} \) and \( T_{ADC} \) are indeed triangles. Obviously every convex combination of a subset of the vertices of \( T \) belongs to \( T \), hence
  \begin{equation*}
    T_{ABC} \cup T_{ADC} \subseteq T.
  \end{equation*}
  ...
  Therefore, every point in \( T \) is either in \( T_{ABC} \) or in \( T_{ADC} \).
\end{proof}
...
\end{proposition}
\begin{proof}
  \Fullref{thm:extremal_points_of_convex_hull} implies that the extremal points are a subset of the vertices. A convex combination of only three vertices is a triangle, and \fullref{thm:trapezoid_triangulation} implies that the trapezoid minus this triangle should be another nondegenerate triangle. Hence, all four vertices are required to generate the trapezoid.

  Triangles are simplices and simplices are convex polytopes. Both of the half-planes corresponding to the diagonal line are used, one for each triangle. The union of the two triangles is the intersection of all half-planes except those two. Therefore, the trapezoid is a convex polytope.
\end{proof}
...
\end{proposition}
\begin{proof}
  \Fullref{thm:trapezoid_triangulation} and \fullref{thm:perimeter_of_triangle} imply that for a trapezoid with vertices \( A \), \( B \), \( C \) and \( D \), the perimeter is the sum of the sides of the triangles \( T_{ABC} \) and \( T_{ADC} \) minus the common side \( [A, C] \). That is,
  \begin{equation*}
    \norm{AB} + \norm{BC} + \norm{CD} + \norm{DA}.
  \end{equation*}
\end{proof}
