...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \item If \( b < 0 \), then \( \varphi \in (-\pi, 0) \) and hence \( \sin \varphi < 0 \). Thus,
    \begin{equation*}
      \sin \varphi = -\sqrt{ 1 - a^2 } = b.
    \end{equation*}
  \end{itemize}
...
\begin{definition}\label{def:trapezoid}\mimprovised
  A \term[bg=трапец,ru=трапеция]{trapezoid} is a \hyperref[def:convex_polytope]{convex polytope} with four vertices \( A \), \( B \), \( C \) and \( D \), such that (up to a renaming of the vertices) the lines through \( A \) and \( B \) and through \( C \) and \( D \) are \hyperref[def:affine_parallelism]{parallel}, and
  \begin{equation*}
    \frac {\vect{AB}} {\norm{\vect{AB}}} = -\frac {\vect{CD}} {\norm{\vect{CD}}}.
  \end{equation*}

  We call the \hyperref[def:line_segment]{line segments} \( [A, B] \), \( [B, C] \), \( [C, D] \) and \( [D, A] \) \term{sides}, and \( [A, B] \) and \( [C, D] \) --- \term{bases} of the trapezoid.
...
\end{proposition}
\begin{proof}
  Similar to \fullref{thm:perimeter_of_triangle}.
\end{proof}
