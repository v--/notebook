...
\begin{definition}\label{def:affine_space}\mcite[def. 2.1.1]{Gallier2011}
  ...
  \begin{figure}[!ht]
    \hfill
    \hfill
    \includegraphics[align=c]{output/def__affine_space__vectors.pdf}
    \hfill
    \includegraphics[align=c]{output/def__affine_space__points.pdf}
    \hfill
...
\begin{example}\label{ex:x2_as_affine_space}
  ...
  \begin{itemize}
    ...
    \begin{figure}[!ht]
      \centering
      \includegraphics{output/ex__x2_as_affine_space.pdf}
...
\begin{remark}\label{rem:affine_combination}
  ...
  \begin{equation}\label{eq:rem:affine_combination}
    \lambda x + (1 - \lambda) y.
  \end{equation}
...
\begin{definition}\label{def:affine_hull}\mimprovised
  The \term{affine hull} of the set \( S \) of points is the set of all arbitrary (i.e. not necessarily binary) \hyperref[rem:affine_combination]{affine combinations} of members of \( S \). It is a \hyperref[def:closure_operator]{closure operator}.
\end{definition}
\begin{defproof}
  We will show that the affine hull operator \( H: \pow(A) \to \pow(A) \) is a closure operator.

  \SubProofOf[def:inflationary_function]{inflation} Obviously \( S \subseteq H(S) \).
  ...
  \SubProofOf[eq:def:partially_ordered_set/homomorphism/nonstrict]{monotonicity} If \( S_1 \subseteq S_2 \), then \( H(S_2) \) contains the affine combinations of the members of \( S_1 \) in addition to others. Hence, \( H(S_1) \subseteq H(S_2) \).
\end{defproof}
...
\begin{definition}\label{def:category_of_small_affine_spaces}
  Suppose that we are given a \hyperref[def:grothendieck_universe]{Grothendieck universe} \( \mscrU \), which is safe to assume to be the smallest suitable one as explained in \fullref{def:large_and_small_sets}. We describe the \term{category of \( \mscrU \)-small affine spaces} as follows.
...
\begin{remark}\label{rem:geometric_shape}
  ...
  \begin{itemize}
    ...
    \item Manifolds, defined in \fullref{def:topological_manifold}.
  \end{itemize}
...
\begin{definition}\label{def:parametric_curve}\mimprovised
  An \term{implicit parametric curve} in an \hyperref[def:affine_space]{affine} or \hyperref[def:topological_space]{topological space} \( X \) is a set \( C \) of points for which there exists a function \( \gamma: I \to X \), where \( I \) is a nonempty interval in \( \BbbR \) with potentially infinite endpoints, such that \( \gamma(I) = C \). In the presence of a topological structure, we usually require \( \gamma \) to be \hyperref[def:global_continuity]{continuous}, making the definition much more restrictive.

  Multiple functions correspond to the same curve, the most trivial example being \( t \mapsto \gamma(-t) \). We call any concrete function \( \gamma: I \to X \) a \term{parametrization} of \( C \), an \term{explicit parametric curve} or simply a \term{parametric curve}. We prefer the interval \( I \) to be either \( [0, 1] \), \( [0, \infty) \), \( (-\infty, 0] \) or \( (-\infty, \infty) \).

  Some fundamental notions include:
  \begin{thmenum}
    \thmitem{def:parametric_curve/endpoints} If either side of \( I \) is closed, for example if \( I = [a, b] \), we call \( \gamma(a) \) is the \term{start} and \( \gamma(b) \) is the \term{end} of the curve, and we refer to them collectively as \term{endpoints}. We say that \( \gamma \) \term{connects} \( a \) and \( b \).

    \thmitem{def:parametric_curve/closed} We call a parametric curve \term{closed} if its endpoints coincide.

    \thmitem{def:parametric_curve/simple} We call a parametric curve \term{simple} if its function is injective with the possible exception of the endpoints (in which case we speak of \enquote{closed simple curves}).
  \end{thmenum}
\end{definition}

\begin{definition}\label{def:parametric_curve_reparametrization}\mcite[def. 1.13]{ИвановТужилин2017}
  A \term{reparametrization} of the \hyperref[def:parametric_curve]{parametric curve} \( \gamma: I \to C \) via the \hyperref[eq:def:partially_ordered_set/homomorphism/strict]{strictly monotone function} \( t: J \to I \) of \( \gamma \) is the parametric curve \( \delta \coloneqq \gamma \bincirc t \). That is,
  \begin{equation*}
    \delta(s) = \gamma(t(s)).
  \end{equation*}
\end{definition}

\begin{proposition}\label{thm:def:parametric_curve_reparametrization}
  The \hyperref[def:parametric_curve_reparametrization]{reparametrization} \( \delta(s) \) with \( t: J \to I \) of the \hyperref[def:parametric_curve]{parametric curve} \( \gamma: I \to X \) has the following basic properties:
  \begin{thmenum}
    \thmitem{thm:def:parametric_curve_reparametrization/image} Both \( \gamma(t) \) and \( \delta(s) \) have the same image. That is, they correspond to the same implicit parametric curve.

    \thmitem{thm:def:parametric_curve_reparametrization/endpoints} Both \( \gamma(t) \) and \( \delta(s) \) have the same beginning and end.

    \thmitem{thm:def:parametric_curve_reparametrization/continuous} The curve \( \gamma(t) \) is continuous if and only if \( \delta(s) \) is continuous.
  \end{thmenum}
\end{proposition}
\begin{proof}
  \SubProofOf{thm:def:parametric_curve_reparametrization/image} Let \( x_0 = \gamma(t_0) \). Define \( s_0 \coloneqq t^{-1}(t_0) \). Then
  \begin{equation*}
    \delta(s_0) = \gamma(t(s_0)) = \gamma(t_0) = x_0.
  \end{equation*}

  Conversely, if \( x_0 = \delta(s_0) \), then \( x_0 = \gamma(t_0) \) for \( t_0 \coloneqq t(s_0) \).

  \SubProofOf{thm:def:parametric_curve_reparametrization/endpoints} Follows from monotonicity.

  \SubProofOf{thm:def:parametric_curve_reparametrization/continuous} The composition of continuous functions is again continuous.
\end{proof}

\begin{definition}\label{def:affine_line}\mimprovised
  Lines are particularly important yet simple \hyperref[def:parametric_curve]{parametric curves}. We say that the set \( L \) of points in an \hyperref[def:affine_space]{affine space} \( (A, \vect A, \tau) \) of dimension at least two is an \term{affine line} or simply \term{line} if any of the following equivalent conditions hold:
