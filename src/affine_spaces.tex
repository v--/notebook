...
\begin{definition}\label{def:affine_hull}\mimprovised
  The \term{affine hull} of the set \( S \) of points is the set of all arbitrary (i.e. not necessarily binary) \hyperref[rem:affine_combination]{affine combinations} of members of \( S \). It is a \hyperref[def:abstract_closure_operator]{closure operator}.
\end{definition}
\begin{defproof}
  We will show that the affine hull operator \( H: \pow(A) \to \pow(A) \) is a closure operator.

  \SubProofOf[def:extensive_function]{extensivity} Obviously \( S \subseteq H(S) \).
