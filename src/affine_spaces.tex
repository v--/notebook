...
\begin{definition}\label{def:affine_coordinate_system}
  ...
  The choice of basis \( \vect E \) then gives an isomorphism between \( A \) and the \hyperref[def:free_semimodule]{free vector space} \( \BbbK^{\oplus E} \). If \( \vect E = \set{ v_1, \ldots, v_n } \), the corresponding coordinate system allows us to associate a tuple of coordinates from \( \BbbK^n \) for each point in \( A \).
  ...
  \begin{equation}\label{eq:def:affine_coordinate_system/coordinates}
    \vect{Ox} = \sum_{v \in \vect E} x_v \cdot v.
  \end{equation}
...
\begin{definition}\label{def:parametric_curve}\mimprovised
  An \term{implicit parametric curve} in an \hyperref[def:affine_space]{affine} or \hyperref[def:topological_space]{topological space} is a set \( C \) of points for which there exists a surjective function \( \gamma: I \to C \), where \( I \) is a nonempty interval in \( \BbbR \) with potentially infinite endpoints. In the presence of a topological structure, we usually require \( \gamma \) to be \hyperref[def:global_continuity]{continuous}, making the definition much more restrictive.

  Multiple functions correspond to the same curve, the most trivial example being \( t \mapsto \gamma(-t) \). We call any concrete function \( \gamma: I \to C \) an \term{explicit parametric curve} or simply a \term{parametric curve}. We prefer the interval \( I \) to be either \( [0, 1] \), \( [0, \infty) \), \( (-\infty, 0] \) or \( (-\infty, \infty) \).

  Some fundamental notions include:
  \begin{thmenum}
    \thmitem{def:parametric_curve/endpoints} If either endpoint of \( I \) is finite, for example if \( I = [a, b] \), we call \( \gamma(a) \) is the \term{start} and \( \gamma(b) \) is the \term{end} of the curve, and we refer to them collectively as \term{endpoints}. We say that \( \gamma \) \term{connects} \( a \) and \( b \).
...
\begin{proposition}\label{thm:parallel_line_through_point}
  For every point \( x \) and every \hyperref[def:affine_line]{affine line} \( l \), there exists a unique line \hyperref[def:affine_parallelism]{parallel} to \( l \) passing through \( x \).

  The case of exactly two points is a restatement of Euclid's Proposition \( \beta. \) (see \cite[8]{Fitzpatrick2008}):
  \begin{quote}
    To place a straight-line equal to a given straight-line at a given point (as an extremity).
  \end{quote}
\end{proposition}
\begin{proof}
  Suppose that \( v \) is a directional vector of \( l \). Then \( t \mapsto \tau_{tv}(x) \) is a line parallel to \( l \) passing through \( x \). Uniqueness follows from \fullref{thm:pair_of_points_is_collinear} because a unique line passes through \( x \) and \( \tau_v(x) \).
\end{proof}

\begin{definition}\label{def:transversal_line}\mimprovised
  A \term{transversal line} for two distinct \hyperref[def:affine_line]{affine lines} is a third line that intersects both of them at exactly one point.
\end{definition}
