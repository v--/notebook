...
\begin{definition}\label{def:left_module}
  Let \( R \) be a \hyperref[def:semiring/dioid]{dioid} and \( M \) be an abelian group. Analogously to \fullref{def:left_monoid_action}, we say that \( M \) is a left \( R \)-\Def{module} if it has some additional structure, which can be defined equivalently as
  \begin{DefEnum}
    \ILabel{def:left_module/homomorphism} a dioid homomorphism from \( R \) to the endomorphism \hyperref[def:endomorphism_dioid]{ring} \( \End(M) \)

    \ILabel{def:left_module/multiplication}\cite[374]{Knapp2016BAlg} an \hyperref[def:magma/associative]{associative} and \hyperref[def:unital_magma]{unital} \hyperref[remark:left_monoid_actions_as_algebraic_structures]{operation} \( \cdot: R \times M \to M \), written using juxtaposition.
    ...
    \begin{DefEnum}
      \ILabel{def:left_module/associativity}(associativity) \( s \cdot (t \cdot x) = (s t) \cdot x \).
      \ILabel{def:left_module/scalar_distributivity}(scalar distributivity) \( (s + t) \cdot x = s \cdot x + t \cdot x \).
      \ILabel{def:left_module/vector_distributivity}(vector distributivity) \( t \cdot (x + y) = t \cdot x + t \cdot y \).
      \ILabel{def:left_module/identity}(identity) \( 1_R \cdot x = x \).
    \end{DefEnum}
  \end{DefEnum}
...
\end{definition}
\begin{proof}
  \SubProofImplication{def:left_module/homomorphism}{def:left_module/multiplication} Let \( \tau: R \to \End(M) \) be a ring homomorphism. Define the operation
  ...
  \SubProofImplication{def:left_module/multiplication}{def:left_module/homomorphism} Let \( \cdot: R \times M \to M \) be a left scalar multiplication operation. We define
  \begin{BreakableAlign*}
     & \tau: R \to \End(M)                      \\
...
\begin{definition}\label{def:right_module}
  We say that \( \tau: R \to \End(A) \) is a \Def{right \( R \)-module} if the same function is a \hyperref[def:left_module]{left module} on the opposite \hyperref[def:opposite_ring]{ring} \( R^{-1} \).
\end{definition}
...
\begin{proposition}\label{thm:left_module_properties}
  Any left \( R \)-module \( M \) has the following basic properties:
  \begin{PropEnum}
    \ILabel{thm:left_module_properties/ring_zero_is_absorbing} Multiplication by \( 0_R \) is \hyperref[def:algebraic_theory/absorbing_element]{absorbing}, that is, \( 0_R x = 0_M \) for any \( x \in M \).

    \ILabel{thm:left_module_properties/module_zero_is_absorbing} Multiplication by \( 0_M \) is absorbing, that is, \( t 0_M = 0_M \) for any \( t \in R \).
  \end{PropEnum}
\end{proposition}
\begin{proof}
  \SubProofOf{thm:left_module_properties/ring_zero_is_absorbing} For any \( x \in M \) we have that \( 0_R x = (0_R + 0_R)x = 0_R x + 0_R x \), thus \( 0_R x \) is an additive identity and \( 0_R x = 0_M \).

  \SubProofOf{thm:left_module_properties/module_zero_is_absorbing} For any \( t \in R \) we have that \( t 0_M = t (0_M + 0_M) = t 0_M + t 0_M \), thus \( t 0_M \) is an additive identity and \( t 0_M = 0_M \).
\end{proof}
...
\begin{definition}\label{def:linear_operator}
  ...
  \begin{DefEnum}
    \ILabel{def:linear_operator/additivity}(additivity) \( f(x + y) = f(x) + f(y) \) for any \( x, y \in M \).
    \ILabel{def:linear_operator/homogeneity}(homogeneity) \( f(tx) = t f(x) \) for any \( t \in R \) and \( x \in M \) (see \fullref{def:homogenous_function}).
  \end{DefEnum}
\end{definition}
...
\begin{definition}\label{def:affine_operator}
  Let \( M \) and \( N \) be two left \( R \)-modules. We say that the function \( f: M \to N \) is \Def{affine} if it is a translation of a \hyperref[def:linear_operator]{linear function}, that is, if there exists a linear function \( l: M \to N \) and a constant \( a \in N \) such that \( f(x) = l(x) + a \).
\end{definition}
...
\begin{definition}\label{def:abelian_group_z_module}\MarginCite[375]{Knapp2016BAlg}
...
\begin{proposition}\label{thm:abelian_group_iff_z_module}\MarginCite[375]{Knapp2016BAlg}
...
\begin{definition}\label{def:left_module_direct_product}
  Let \( \{ X_k \}_{k \in \CK} \) be a nonempty family of left \( R \)-modules.

  Analogously to \fullref{def:group_direct_product}, we define their \Def{direct product} as the module \( \prod_{k \in \CK} X_k \), the operations defined componentwise as
  \begin{BreakableAlign*}
  ...
  We define their \Def{direct sum} as the submodule of \( \prod_{k \in \CK} X_k \) (see \fullref{def:left_module_direct_product}) where only finitely many components of any module element are different from zero.
\end{definition}
...
\begin{proposition}\label{thm:module_categorical_limits}
  We are interested in \hyperref[def:categorical_limit]{categorical limits} and \hyperref[def:categorical_colimit]{colimits} in the category \( \Cat{Mod}_R \) of left-modules over \( R \). Fix an indexed family  \( \{ X_k \}_{k \in \CK} \) of \( R \)-modules.
  \begin{DefEnum}
    \ILabel{thm:module_categorical_limits/product} Their \hyperref[def:categorical_product]{categorical product} is their direct \hyperref[def:left_module_direct_product]{product} \( \prod_{k \in \CK} X_k \), the projection morphisms being inherited from \fullref{thm:set_categorical_limits/product}.

    \ILabel{thm:module_categorical_limits/coproduct} Their \hyperref[def:categorical_coproduct]{categorical coproduct} is the \hyperref[def:group_direct_product]{direct sum} \( \oplus_{k \in \CK} X_k \), the embedding morphisms being inherited from \fullref{thm:abelian_group_categorical_limits/coproduct}.
  \end{DefEnum}
\end{proposition}
...
\begin{definition}\label{def:linear_combination}
  ...
  \begin{DefEnum}
    \ILabel{def:linear_combination/affine} an \Def{affine combination} if \( \sum_{k=1}^n t_k = 1 \).
    \ILabel{def:linear_combination/conic} a \Def{conic combination} if all of the coefficients are nonnegative real numbers.
    \ILabel{def:linear_combination/convex} a \Def{convex combination} if it is both affine and conic.
  \end{DefEnum}
\end{definition}
...
\begin{definition}\label{def:free_left_module}\MarginCite[377]{Knapp2016BAlg}
...
\begin{proposition}\label{thm:left_module_basis_cardinality}\MarginCite{ProofWiki:bases_of_free_module_have_same_cardinality}
  All bases in a free left module over a commutative unital ring have the same cardinality.
\end{proposition}

\begin{definition}\label{def:left_module_tensor_product}\MarginCite[574]{Knapp2016BAlg}
...
\begin{theorem}\label{thm:tensor_product_universal_property}\MarginCite[thm. 10.18]{Knapp2016BAlg}
...
\begin{proposition}\label{thm:tensor_product_with_underlying_ring}\MarginCite[677]{Knapp2016BAlg}
...
\begin{definition}\label{def:algebra_over_ring}\MarginCite[408]{Knapp2016BAlg}
...
\begin{proposition}\label{thm:functions_over_ring_form_algebra}
  ...
  \end{equation*}
  to be the set of all functions from \( X \) to \( R \) (see \fullref{def:category_of_sets}). Then \( A \) is an \( R \)-algebra with the operations being defined pointwise, that is,
  \begin{BreakableAlign*}
...
\begin{definition}\label{def:left_module_annihilator}\MarginCite[30]{Коцев2016}
