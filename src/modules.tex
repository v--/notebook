...
\begin{proposition}\label{thm:def:hamel_basis}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:def:hamel_basis/expansion} If \( A \) is a finite basis of the submodule \( N \) of \( M \), and if \( M \) has at least one finite basis, then there exists a finite basis \( B \) of \( M \) such that \( A \subseteq B \). That is, we expand the basis of \( N \) to a basis of \( M \).
  \end{thmenum}
\end{proposition}
\begin{proof}
  ...
  \SubProofOf{thm:def:hamel_basis/expansion} Let \( a_1, \ldots, a_n \) be a basis of \( N \) and let \( b_1, \ldots, b_m \) be a basis of \( M \). Then \( \set{ a_1, \ldots, a_n, b_1, \ldots, b_m } \) is necessarily a linearly dependent set. Since the set of also generating, \( b_m \) is a linear combination of the others, and thus
  \begin{equation*}
...
\begin{remark}\label{rem:finite_dimensional_vector_space_uniqueness}
  Because of \fullref{thm:vector_space_basis_existence}, we often regard finite-dimensional vector spaces as spaces of tuples defined in \fullref{def:module_of_tuples}.
\end{remark}
...
\begin{proposition}\label{thm:basis_of_polynomial_ring}
  ...
  \end{equation*}
  of all monomials is a \hyperref[def:hamel_basis]{basis} for \hyperref[def:polynomial_semiring]{polynomial ring} \( R[\mscrX] \).
\end{proposition}
\begin{proof}
  In \fullref{def:polynomial_semiring}, we have defined a polynomial as a \hyperref[def:free_semimodule]{free \( R \)-module} over the set of all monomials.
\end{proof}
