...
\begin{definition}\label{def:linear_combination}
  ...
  In the special case where \( R \) is a superring of \( \R \), we define the following special types of linear combinations:
  \begin{defenum}
...
\begin{definition}\label{def:left_module_linear_dependence}
  ...
  We say that the vectors \( x_1, \ldots, x_n \) are linearly independent if the corresponding set \( \{ x_1, \ldots, x_n \} \) is linearly independent and \Def{affinely independent} if the set \( \{ x_2 - x_1, \ldots, x_n - x_1 \} \) is linearly independent.
