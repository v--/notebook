...
\end{Definition}
\begin{proof}
  \IImplies{def:left_module/homomorphism}{def:left_module/multiplication} Let \( \tau: R \to \End(M) \) be a ring homomorphism. Define the operation
  \begin{align*}
    &\cdot: R \times M \to M \\
  ...
  \begin{RefList}
    \IRef{def:left_module/associativity}
    \begin{equation*}
      s \cdot (t \cdot x)
      =
      \tau(s)(\tau(t)(x))
    ...
    \IRef{def:left_module/scalar_distributivity}
    \begin{equation*}
      (s + t) \cdot x
      =
      \tau(s + t)(x)
    ...
    \IRef{def:left_module/vector_distributivity}
    \begin{equation*}
      t \cdot (x + y)
      =
      \tau(t)(x + y).
  ...
  \IImplies{def:left_module/multiplication}{def:left_module/homomorphism} Let \( \cdot: R \times M \to M \) be a left scalar multiplication operation. We define
  \begin{align*}
    &\tau: R \to \End(M) \\
...
\begin{Proposition}\label{def:left_module_properties}
  ...
  \begin{PropEnum}
    \ILabel{def:left_module_properties/ring_zero_is_absorbing} Multiplication by \( 0_R \) is \hyperref[def:algebraic_theory/absorbing_element]{absorbing}, that is, \( 0_R x = 0_M \) for any \( x \in M \).

    \ILabel{def:left_module_properties/module_zero_is_absorbing} Multiplication by \( 0_M \) is absorbing, that is, \( t 0_M = 0_M \) for any \( t \in R \).
  \end{PropEnum}
\end{Proposition}
\begin{RefListProof}
    \IRef{def:left_module_properties/ring_zero_is_absorbing} For any \( x \in M \) we have that \( 0_R x = (0_R + 0_R)x = 0_R x + 0_R x \), thus \( 0_R x \) is an additive identity and \( 0_R x = 0_M \).

    \IRef{def:left_module_properties/module_zero_is_absorbing} For any \( t \in R \) we have that \( t 0_M = t (0_M + 0_M) = t 0_M + t 0_M \), thus \( t 0_M \) is an additive identity and \( t 0_M = 0_M \).
\end{RefListProof}
...
\begin{Definition}\label{def:abelian_group_z_module}\cite[375]{Knapp2016BAlg}
...
\begin{Proposition}\label{thm:abelian_group_iff_z_module}\cite[375]{Knapp2016BAlg}
...
\begin{Definition}\label{def:free_left_module}\cite[377]{Knapp2016BAlg}
...
\end{Proposition}
\begin{RefListProof}
    \IRef{def:linear_operator/homogeneity} Let \( t \in R \) and \( x \in M \). We have the unique decompositions
    \begin{align*}
      x &= \sum_{b \in B} \pi_b(x) b, \\
    ...
    \IRef{def:linear_operator/additivity} Analogous.
\end{RefListProof}
