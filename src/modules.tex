...
\begin{definition}\label{def:left_module}
  ...
  \begin{thmenum}
    \labitem{def:left_module/homomorphism} a dioid homomorphism from \( R \) to the endomorphism \hyperref[def:endomorphism_dioid]{ring} \( \End(M) \)

    \labitem{def:left_module/multiplication}\cite[374]{Knapp2016BasicAlgebra} an \hyperref[def:magma/associative]{associative} and \hyperref[def:unital_magma]{unital} \hyperref[def:left_monoid_action/operation]{operation} \( \cdot: R \times M \to M \), written using juxtaposition.
    ...
    \begin{thmenum}
      \labitem{def:left_module/associativity}(associativity) \( s \cdot (t \cdot x) = (s t) \cdot x \).
      \labitem{def:left_module/scalar_distributivity}(scalar distributivity) \( (s + t) \cdot x = s \cdot x + t \cdot x \).
      \labitem{def:left_module/vector_distributivity}(vector distributivity) \( t \cdot (x + y) = t \cdot x + t \cdot y \).
      \labitem{def:left_module/identity}(identity) \( 1_R \cdot x = x \).
    \end{thmenum}
  \end{thmenum}
...
\begin{proposition}\label{thm:left_module_properties}
  ...
  \begin{thmenum}
    \labitem{thm:left_module_properties/ring_zero_is_absorbing} Multiplication by \( 0_R \) is \term{absorbing}, that is, \( 0_R x = 0_M \) for any \( x \in M \).

    \labitem{thm:left_module_properties/module_zero_is_absorbing} Multiplication by \( 0_M \) is absorbing, that is, \( t 0_M = 0_M \) for any \( t \in R \).
  \end{thmenum}
\end{proposition}
\begin{proof}
...
\begin{definition}\label{def:linear_operator}
  ...
  \begin{thmenum}
    \labitem{def:linear_operator/additivity}(additivity) \( f(x + y) = f(x) + f(y) \) for any \( x, y \in M \).
    \labitem{def:linear_operator/homogeneity}(homogeneity) \( f(tx) = t f(x) \) for any \( t \in R \) and \( x \in M \) (see \fullref{def:homogenous_function}).
  \end{thmenum}
\end{definition}
...
\begin{definition}\label{def:abelian_group_z_module}\mcite\cite[375]{Knapp2016BasicAlgebra}
...
\begin{proposition}\label{thm:abelian_group_iff_z_module}\mcite\cite[375]{Knapp2016BasicAlgebra}
...
\begin{proposition}\label{thm:module_categorical_limits}
  ...
  \begin{thmenum}
    \labitem{thm:module_categorical_limits/product} Their \hyperref[def:categorical_product]{categorical product} is their direct \hyperref[def:left_module_direct_product]{product} \( \prod_{k \in \mscrK} X_k \), the projection morphisms being inherited from \fullref{thm:set_categorical_limits/product}.

    \labitem{thm:module_categorical_limits/coproduct} Their \hyperref[def:categorical_coproduct]{categorical coproduct} is the \hyperref[def:group_direct_product]{direct sum} \( \oplus_{k \in \mscrK} X_k \), the embedding morphisms being inherited from \fullref{thm:abelian_group_categorical_limits/coproduct}.
  \end{thmenum}
\end{proposition}
...
\begin{definition}\label{def:linear_combination}
  ...
  \begin{thmenum}
    \labitem{def:linear_combination/affine} an \term{affine combination} if \( \sum_{k=1}^n t_k = 1 \).
    \labitem{def:linear_combination/conic} a \term{conic combination} if all of the coefficients are nonnegative real numbers.
    \labitem{def:linear_combination/convex} a \term{convex combination} if it is both affine and conic.
  \end{thmenum}
\end{definition}
...
\begin{definition}\label{def:free_left_module}\mcite\cite[377]{Knapp2016BasicAlgebra}
...
\begin{definition}\label{def:left_module_tensor_product}\mcite\cite[574]{Knapp2016BasicAlgebra}
...
\begin{theorem}\label{thm:tensor_product_universal_property}\mcite\cite[thm. 10.18]{Knapp2016BasicAlgebra}
...
\begin{definition}\label{def:algebra_over_ring}\mcite\cite[408]{Knapp2016BasicAlgebra}
...
\begin{definition}\label{def:left_module_annihilator}\mcite\cite[30]{КоцевСидеров2016}
