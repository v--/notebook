...
\begin{definition}\label{def:left_module}
  ...
  \begin{DefEnum}
    ...
    \ILabel{def:left_module/multiplication}\cite[374]{Knapp2016BAlg} an \hyperref[def:algebraic_theory/associativity]{associative} and \hyperref[def:algebraic_theory/identity]{unital} \hyperref[remark:left_group_actions_as_algebraic_structures]{operation} \( \cdot: R \times M \to M \), written using juxtaposition.
...
\begin{proposition}\label{thm:module_categorical_limits}
  ...
  \begin{DefEnum}
    ...
    \ILabel{thm:module_categorical_limits/coproduct} Their \hyperref[def:categorical_coproduct]{categorical coproduct} is the \hyperref[def:group_direct_product]{direct sum} \( \oplus_{k \in \CK} X_k \), the injection morphisms being inherited from \fullref{thm:abelian_group_categorical_limits/coproduct}.
  \end{DefEnum}
\end{proposition}
