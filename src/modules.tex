...
\begin{definition}\label{def:left_module}
  Let \( R \) be a unital ring\Tinyref{def:semiring/unital_ring} and \( M \) be an abelian group. Analogously to \cref{def:left_group_action}, we say that \( M \) is a left \( R \)-\Def{module} if it has some additional structure, which can be defined equivalently as
  \begin{defenum}
    \DItem{def:left_module/homomorphism} a unital ring homomorphism from \( R \) to the endomorphism ring\Tinyref{def:endomorphism_ring} \( \End(M) \)
...
\begin{definition}\label{def:linear_operator}
  ...
  \begin{description}
    ...
    \DItem{def:linear_operator/homogeneity}[homogeneity] \( f(tx) = t f(x) \) for any \( t \in R \) and \( x \in M \).
  \end{description}

  We say that \( f \) is \Def{semilinear} if instead of homogeneity we have
  \begin{description}
    \DItem{def:linear_operator/semihomogeneity}[semihomogeneity] there exists an automorphism \( \varphi \) on \( R \) such that \( f(tx) = \varphi(t) f(t) \) for any \( t \in R \) and \( x \in M \).
  \end{description}
\end{definition}
...
\begin{definition}\label{def:left_module_basis_projection}
  Let \( M \) be a left \( R \)-module and let \( B \) be a basis of \( M \). For each \( b \in B \), we define the \text{projection functional} \( \pi_b: M \to R \) that gives us the unique coefficient in the basis decomposition. Thus, for every \( x \in M \) we have
  \begin{equation*}
...
\begin{definition}\label{def:associative_algebra}\cite[408]{Knapp2016BAlg}
  Let \( R \) be a commutative unital ring. We say that the left \( R \)-module \( A \) is an \Def{associative \( R \)-algebra} if we define an additional bilinear \Def{vector multiplication} operation
  \begin{equation*}
    \odot: A \times A \to A
  \end{equation*}
  ...
  If \( \odot \) is commutative, we refer to \( A \) as a \Def{commutative algebra}.
\end{definition}
