...
\begin{proposition}\label{def:module_properties}
  ...
  \begin{defenum}
    \DItem{def:module_properties/zero_absorbing} Multiplication by \( 0_R \) is \textbf{absorbing}\Tinyref{def:magma/absorbing_element}, that is, \( 0_R \cdot u = 0_M \) for any \( u \in M \).
  \end{defenum}
\end{proposition}
\begin{proof}\mbox{}
  \begin{itemize}
...
\begin{definition}\label{def:abelian_group_z_module}\cite{Knapp2016BAlg}[375]
  Let \( G \) be an abelian group. Define the \textbf{\( \Z \)-module \( M \), associated with \( G \)}, with scalar multiplication
  \begin{align*}
...
\begin{definition}\label{def:free_module}[32]\cite{Kocev2016}
  ...
  The cardinality of the basis of a free module \( M \) is called the \textbf{rank} \( \Rank M \) of \( M \). \Cref{thm:free_module_basis_cardinality} tells us that this rank is unique.
\end{definition}
