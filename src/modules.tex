...
\begin{definition}\label{def:linear_combination}
  ...
  \end{equation*}
  their \Def{linear combination} with \Def{coefficients} or \Def{scalars} \( \alpha_1, \ldots, \alpha_n \) and \Def{vectors} \( x_1, \ldots, x_n \).
...
\begin{definition}\label{def:linear_dependence}
  Let \( M \) be an \( R \)-module and let \( A \subseteq M \). We say that the set \( A \) is \Def{linearly dependent} if there exists \( x \in A \) such that
  \begin{equation*}
    x \in \Span{A} \setminus \{ x \}.
  \end{equation*}

  If \( A \) is not linearly dependent, we say that it is \Def{linearly independent}.
\end{definition}

\begin{definition}\label{def:module_basis}
  The subset \( B \) of the \( R \)-module \( M \) is called a \Def{basis of \( M \)} if \( B \) is linearly independent and
  \begin{equation*}
    M = \Span{B}.
  \end{equation*}
\end{definition}
...
\begin{definition}\label{def:free_module}[32]\cite{Kocev2016}
  ...
  The cardinality of the basis of a free module \( M \) is called the \Def{rank} \( \Rank M \) of \( M \). \Cref{thm:free_module_basis_cardinality} tells us that this rank is unique.
\end{definition}
...
\begin{definition}\label{thm:functions_over_ring_form_algebra}
