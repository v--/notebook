...
\begin{definition}\label{def:linear_dependence}\mimprovised
  ...
  \begin{thmenum}
    \thmitem{def:linear_dependence/direct} A \hyperref[rem:linear_combinations]{linear combination} in \( B \) sums to zero if and only if it is \hyperref[rem:linear_combinations/trivial]{trivial}.
...
\begin{proposition}\label{thm:def:linear_dependence}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:def:linear_dependence/dependent_combination} The set \( A \cup \set{ x } \) is linearly dependent if and only if \( x \in \linspan A \).
...
\begin{proposition}\label{thm:def:vector_space}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:def:vector_space/expansion} Given a subspace \( U \) of \( V \), if \( A \) is a finite basis of \( U \) and if \( V \) has a finite basis, then we can extend \( A \) to a finite basis of \( V \). That is, there exists a finite basis \( B \) of \( V \) such that \( A \subseteq B \).
...
\begin{proposition}\label{thm:commutative_vector_space_dimension}
...
\end{corollary}
\begin{proof}
  Follows from \fullref{thm:commutative_vector_space_dimension}.
\end{proof}

\begin{example}\label{ex:field_submodules}
  For a nontrivial \hyperref[def:ring/commutative]{commutative ring} \( R \), every ideal of \( R \) is a submodule of the \hyperref[thm:commutative_vector_space_dimension]{rank-one} \hyperref[def:module]{module} \( R \). For example, both \( \BbbZ \) and \( 2\BbbZ \) are \( \BbbZ \)-modules of rank one.
...
\end{lemma}
\begin{proof}
  Let \( A \) be a basis of \( U \). By \fullref{thm:def:vector_space/expansion}, there exists a basis \( B \) of \( V \) such that \( A \subseteq B \). Then clearly \( \rank U \leq \rank V \).
\end{proof}
...
\begin{proposition}\label{thm:rank_of_direct_sum}
  Assuming that the \( R \)-modules \( M_1, \ldots, M_n \) have bases, the \hyperref[thm:commutative_vector_space_dimension]{rank} of their direct sum
  \begin{equation*}
