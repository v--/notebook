...
\begin{definition}\label{def:left_module}\cite[374]{Knapp2016BAlg}
  ...
  Related notions are
  \begin{defenum}
    \DItem{def:left_module/right_module} \Def{Right modules} are defined analogously but with the scalar on the right side. In fact, every left \( R \)-module is isomorphic to exactly one right \( R^{-1} \)-module, where \( R^{-1} \) is the opposite ring\Tinyref{def:opposite_ring}.

    \DItem{def:left_module/bimodule} An \Def{\( (R, S) \)-bimodule} \( M \) is a left \( R \)-module and a right \( S \)-module such that for \( r \in R \), \( s \in S \) and \( x \in M \) we have
    \begin{equation*}
      (rx)s = r(xs).
    \end{equation*}
  \end{defenum}
...
\begin{remark}\label{remark:module_as_ring_action}
  In analogy to group actions\Tinyref{def:group_action}, we can regard (left) modules as (left) ring actions on abelian groups.

  To see this, fix a ring \( R \) and an abelian group \( M \). Scalar multiplication \( R \times M \to M \) is then equivalent to a ring homomorphism from \( R \) to \( \End(M) \).
\end{remark}
