...
\begin{proposition}\label{def:module_properties}
  ...
  \begin{defenum}
    \item\label{def:module_properties/zero_absorbing} Multiplication by $0_R$ is \ul{absorbing}\Tinyref{def:group/absorbing_element}, that is, $0_R \cdot u = 0_M$ for any $u \in M$.
  \end{defenum}
\end{proposition}
\begin{proof}\mbox{}
  \begin{itemize}
...
\begin{definition}\label{def:module_homomorphism}\cite{Knapp2016BAlg}[375]
  ...
  The \ul{kernel} of $f$ is defined as the preimage\Tinyref{def:function_invertibility} $f^{-1}(0_N)$.
...
\begin{definition}\label{def:abelian_group_z_module}\cite{Knapp2016BAlg}[375]
  Let $G$ be an abelian group. Define the \ul{$\BB{Z}$-module $M$, associated with $G$}, with scalar multiplication
  \begin{align*}
...
\begin{definition}\label{def:module_direct_product}
  ...
  Analogously to \cref{def:group_direct_product}, we define their \ul{direct product} as the module $\prod_{i \in I} X_i$, the operations defined componentwise as
  \begin{align*}
  ...
  We define their \ul{direct sum} as the submodule of $\prod_{i \in I} X_i$\Tinyref{def:module_direct_product} where only finitely many components of any module element are different from zero.
\end{definition}
...
\begin{definition}\label{def:linear_dependence}
  Let $M$ be an $R$-module and let $A \subseteq M$. We say that the set $A$ is \ul{linearly dependent} if there exists $x \in A$ such that
  \begin{align*}
    x \in \Span{A} \setminus \{ x \}.
  \end{align*}

  If $A$ is not linearly dependent, we say that it is \ul{linearly independent}.
\end{definition}

\begin{definition}\label{def:module_basis}
  The subset $B$ of the $R$-module $M$ is called a \ul{basis of $M$} if $B$ is linearly independent and
  \begin{align*}
    M = \Span{B}.
  \end{align*}
\end{definition}
...
\begin{definition}\label{def:free_module}[32]\cite{Kocev2016}
  Let $R$ be a commutative unital ring. We say that the $R$-module $M$ is a \ul{free module} if it has a basis\Tinyref{def:module_basis}.

  If $S$ is any set, then the direct sum\Tinyref{thm:module_categorical_limits/coproduct}
  \begin{align*}
    M \coloneqq \oplus_{s \in S} R
  \end{align*}
  with injections $\{ \iota_s \}_{s \in S}$ is called the \ul{free module generated by $S$}. Define the function
  \begin{align*}
    &\varphi: S \to M \\
  ...
  If the free module $M$ has a finite basis, we say that it is \ul{finitely generated}.
\end{definition}
