...
\begin{definition}\label{def:module_over_ring}\cite[374]{Knapp2016BAlg}
  ...
  \begin{description}
    ...
    \item[scalar multiplication] \( \cdot: R \times M \to M \) (usually written using juxtaposition) is associative \Tinyref{def:algebraic_theory/associativity} and unital\Tinyref{def:algebraic_theory/identity}. In order for \cref{def:algebraic_theory} to make sense here, we define the operation
    \begin{equation*}
      \cdot^{(t)}(x): M \to M
    \end{equation*}
    for each \( t \in R \).
