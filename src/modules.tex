...
\begin{definition}\label{def:module_over_ring}
  Let \( R \) be a unital ring\Tinyref{def:semiring/unital_ring} and \( M \) be an abelian group. Analogously to \cref{def:group_action}, we say that \( M \) is an \( R \)-\Def{module} if it has some additional structure, which can be defined equivalently as
  \begin{defenum}
    \DItem{def:module_over_ring/homomorphism} a unital ring homomorphism from \( R \) to the endomorphism ring\Tinyref{def:endomorphism_ring} \( \End(M) \)

    \DItem{def:module_over_ring/left_multiplication}\cite[374]{Knapp2016BAlg} an associative \Tinyref{def:algebraic_theory/associativity} and unital\Tinyref{def:algebraic_theory/identity} operation\Tinyref{remark:group_actions_as_algebraic_structures} \( \cdot: R \times M \to M \), written using juxtaposition.
    ...
    \begin{description}
      \DItem{def:module_over_ring/associativity}[associativity] \( s \cdot (t \cdot x) = (s t) \cdot x \).
      \DItem{def:module_over_ring/scalar_distributivity}[scalar distributivity] \( (s + t) \cdot x = s \cdot x + t \cdot x \).
      \DItem{def:module_over_ring/vector_distributivity}[vector distributivity] \( t \cdot (x + y) = t \cdot x + t \cdot y \).
      \DItem{def:module_over_ring/identity}[identity] \( 1_R \cdot x = x \).
    \end{description}
  \end{defenum}

  We sometimes refer to \ref{def:module_over_ring/left_multiplication} as \Def{left modules} to distinguish them from \Def{right modules} (which are defined in the obvious way). If \( R \) is commutative, left \( R \)-modules and right \( R \)-modules coincide. An abelian group that is both a left \( R \)-module and right \( S \)-module is called an \( R, S \)-\Def{bimodule} if \( r \in R, s \in S \) and \( x \in M \) implies
  \begin{equation*}
    (rx)s = r(xs).
  \end{equation*}
...
\end{definition}
\begin{proof}
  \Implies[def:module_over_ring/homomorphism][def:module_over_ring/left_multiplication] Let \( \tau: R \to \End(M) \) be a ring homomorphism. Define the operation
  \begin{align*}
    &\cdot: R \times M \to M \\
  ...
  \begin{description}
    \RItem{def:module_over_ring/associativity}
    \begin{equation*}
      s \cdot (t \cdot x)
      =
      \tau(s)(\tau(t)(x))
    ...
    \RItem{def:module_over_ring/scalar_distributivity}
    \begin{equation*}
      (s + t) \cdot x
      =
      \tau(s + t)(x)
    ...
    \RItem{def:module_over_ring/vector_distributivity}
    \begin{equation*}
      t \cdot (x + y)
      =
      \tau(t)(x + y).
    ...
    \RItem{def:module_over_ring/identity}
    \begin{equation*}
      1_R \cdot x
      =
      \tau(1_R)(x)
  ...
  \Implies[def:module_over_ring/left_multiplication][def:module_over_ring/homomorphism] Let \( \cdot: R \times M \to M \) be a left scalar multiplication operation. We define
  \begin{align*}
    &\tau: R \to \End(M) \\
    &\tau(t) \coloneqq (x \mapsto t \cdot x).
  \end{align*}

  This function is well defined since for each \( t \in R \), the function \( \tau(t) \) is an abelian group homomorphism (due to \ref{def:module_over_ring/associativity}).
...
\begin{definition}\label{def:algebra_over_ring}\cite[4]{Kocev2016}
  Let \( R \) be a commutative unital ring. We say that the left \( R \)-module \( A \) is an \Def{\( R \)-algebra} if we define an additional \Def{vector multiplication} operation
  \begin{equation*}
    \odot: A \times A \to A
  \end{equation*}
