...
\begin{definition}\label{def:global_continuity}
  ...
  \begin{thmenum}
    \labitem{def:global_continuity/limits} \( f \) is continuous at every point of \( X \) in the sense of \fullref{def:local_continuity}.
    \labitem{def:global_continuity/open} For every open set \( V \in \mscrT \), the \hyperref[def:function/preimage]{preimage} \( f^{-1}(V) \) is open.
    \labitem{def:global_continuity/closed} For every closed set \( F \in \mscrF_{\mscrT_Y} \), the preimage \( f^{-1}(F) \) is closed.
    \labitem{def:global_continuity/base} There exists a \hyperref[def:topological_base]{base} \( \mscrB_{\mscrT_Y} \subseteq \mscrT_Y \), such that for every \( V \in \mscrB_{\mscrT_Y} \), the preimage \( f^{-1}(V) \) is open.
    \labitem{def:global_continuity/subbase} There exists a \hyperref[def:topological_subbase]{subbase} \( \mscrP_{\mscrT_Y} \subseteq \mscrT_Y \), such that for every \( V \in \mscrP_{\mscrT_Y} \), the preimage \( f^{-1}(V) \) is open.
    \labitem{def:global_continuity/closure} For every set \( A \subseteq X \), \( f(\cl(A)) \subseteq \cl(f(A)) \).
  \end{thmenum}
...
\begin{definition}\label{def:parametric_curve}
  ...
  \begin{thmenum}
    \labitem{def:parametric_curve/function} A continuous function \( \gamma: I \to X \) is called a parametric curve.

    \labitem{def:parametric_curve/image} The image \( \img(\gamma) \) of a parametric curve \( \gamma \) is also called a parametric curve.

    \labitem{def:parametric_curve/equivalence_class} The equivalence class of all continuous functions from \( I \) to \( X \) with
    \begin{equation*}
  ...
  \begin{thmenum}
    \labitem{def:parametric_curve/closed} The curve \( \gamma \) is called \term{closed} if its endpoints coincide, i.e. \( \gamma(a) = \gamma(b) \).

    \labitem{def:parametric_curve/simple} The curve \( \gamma \) is called \term{simple} if the function \( \gamma: I \to Y \) is injective with the possible exception of the endpoints (in which case we speak of \term{simple closed curves}.
  \end{thmenum}

  If \( \gamma: I \to X \) is a parametric curve, related curves are:
  \begin{thmenum}
    \labitem{def:parametric_curve/function_graph}\mcite\cite[def. 1.20]{ИвановТужилин2017}The \hyperref[def:function/graph]{graph} \( \gph(\gamma) \) of \( \gamma \) is a the image of the curve \( \overline{\gamma}(t, x) \coloneqq (t, \gamma(x)) \) in the topological space \( I \times X \).

    \labitem{def:parametric_curve/implicit}\mcite\cite[def. 1.24]{ИвановТужилин2017}If \( M \) is a subset of \( X \) and if there exists a curve \( \gamma: I \to X \) such that \( \imag(\gamma) = M \), we call \( M \) an \term{implicit parametric curve}.
  \end{thmenum}
\end{definition}
