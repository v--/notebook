...
\end{Definition}
\begin{RefListProof}
    \IImplies{def:global_continuity/limits}{def:global_continuity/open} Follows from \fullref{def:local_convergence/neighborhoods}.
    \IImplies{def:global_continuity/open}{def:global_continuity/closed} If \( F \in \CF_{\CT_Y} \) is a closed set, \( Y \setminus F \) is open, therefore \( f^{-1}(Y \setminus F) = X \setminus f^{-1}(F) \) is also open. Hence \( f^{-1}(F) \) is closed.
    \IImplies{def:global_continuity/open}{def:global_continuity/base} \( \CT \) is a base of itself.
    \IImplies{def:global_continuity/base}{def:global_continuity/subbase} Every base is also a subbase.
    \IImplies{def:global_continuity/subbase}{def:global_continuity/limits} Follows from the equivalences in \fullref{def:local_convergence}.
    \IImplies{def:global_continuity/closed}{def:global_continuity/closure} Note that
    \begin{equation*}
      A
    ...
    \IImplies{def:global_continuity/closure}{def:global_continuity/closed} Fix a closed set \( F \subseteq Y \). Then
...
\begin{Definition}\label{def:parametric_curve}
  ...
  \begin{DefEnum}
    \ILabel{def:parametric_curve/function_graph}\cite[definition 1.20]{Иванов2017} The \hyperref[def:function/graph]{graph} \( \Gph(\gamma) \) of \( \gamma \) is a the image of the curve \( \Ol{\gamma}(t, x) \coloneqq (t, \gamma(x)) \) in the topological space \( I \times X \).

    \ILabel{def:parametric_curve/implicit}\cite[definition 1.24]{Иванов2017} If \( M \) is a subset of \( X \) and if there exists a curve \( \gamma: I \to X \) such that \( \Imag(\gamma) = M \), we call \( M \) an \Def{implicit parametric curve}.
  \end{DefEnum}
\end{Definition}
