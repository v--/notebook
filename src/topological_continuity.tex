...
\begin{definition}\label{def:parametric_curve}
  ...
  \begin{thmenum}
    \ilabel{def:parametric_curve/function_graph}\mcite\cite[def. 1.20]{ИвановТужилин2017}The \hyperref[def:function/graph]{graph} \( \gph(\gamma) \) of \( \gamma \) is a the image of the curve \( \overline{\gamma}(t, x) \coloneqq (t, \gamma(x)) \) in the topological space \( I \times X \).

    \ilabel{def:parametric_curve/implicit}\mcite\cite[def. 1.24]{ИвановТужилин2017}If \( M \) is a subset of \( X \) and if there exists a curve \( \gamma: I \to X \) such that \( \imag(\gamma) = M \), we call \( M \) an \term{implicit parametric curve}.
  \end{thmenum}
\end{definition}
