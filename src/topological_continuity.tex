...
\begin{definition}\label{def:global_continuity}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:global_continuity/open} For every open set \( V \in T \), the \hyperref[def:function/preimage]{preimage} \( f^{-1}(V) \) is open.
    ...
    \thmitem{def:global_continuity/subbase} There exists a \hyperref[def:topological_subbase]{subbase} \( \mscrP_{\mscrT_Y} \subseteq T_Y \), such that for every \( V \in P_{\mscrT_Y} \), the preimage \( f^{-1}(V) \) is open.
