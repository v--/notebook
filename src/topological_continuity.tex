...
\begin{definition}\label{def:local_continuity}
  We say that the function \( f: X \to Y \) between topological spaces is \Def{continuous} at the point \( x_0 \in X \) if \( f(x_0) \) is a limit point\Tinyref{def:local_convergence} of \( f \) at \( x_0 \).
...
\begin{definition}\label{def:global_continuity}
  ...
  \begin{defenum}
    \DItem{def:global_continuity/limits} \( f \) is continuous at every point of \( X \) in the sense of \fullref{def:local_continuity}.
    \DItem{def:global_continuity/open} For every open set \( V \in \CT \), the preimage\Tinyref{def:function/preimage} \( f^{-1}(V) \) is open.
    \DItem{def:global_continuity/closed} For every closed set \( F \in \CF_{\CT_Y} \), the preimage \( f^{-1}(F) \) is closed.
    \DItem{def:global_continuity/base} There exists a base\Tinyref{def:topological_base} \( \CB_{\CT_Y} \subseteq \CT_Y \), such that for every \( V \in \CB_{\CT_Y} \), the preimage \( f^{-1}(V) \) is open.
    \DItem{def:global_continuity/subbase} There exists a subbase\Tinyref{def:topological_subbase} \( \CP_{\CT_Y} \subseteq \CT_Y \), such that for every \( V \in \CP_{\CT_Y} \), the preimage \( f^{-1}(V) \) is open.
    \DItem{def:global_continuity/closure} For every set \( A \subseteq X \), \( f(\Cl(A)) \subseteq \Cl(f(A)) \).
  \end{defenum}
...
\begin{proof}\mbox{}
  \begin{description}
    \Implies[def:global_continuity/limits][def:global_continuity/open] Follows from \fullref{def:local_convergence/neighborhoods}.
    \Implies[def:global_continuity/open][def:global_continuity/closed] If \( F \in \CF_{\CT_Y} \) is a closed set, \( Y \setminus F \) is open, therefore \( f^{-1}(Y \setminus F) = X \setminus f^{-1}(F) \) is also open. Hence \( f^{-1}(F) \) is closed.
    \Implies[def:global_continuity/open][def:global_continuity/base] \( \CT \) is a base of itself.
    \Implies[def:global_continuity/base][def:global_continuity/subbase] Every base is also a subbase.
    \Implies[def:global_continuity/subbase][def:global_continuity/limits] Follows from the equivalences in \fullref{def:local_convergence}.
    \Implies[def:global_continuity/closed][def:global_continuity/closure] Note that
    \begin{equation*}
      A
    ...
    \Implies[def:global_continuity/closure][def:global_continuity/closed] Fix a closed set \( F \subseteq Y \). Then
    ...
    From \fullref{def:global_continuity/closure_implies_closed_right} and \fullref{def:global_continuity/closure_implies_closed_left} it follows that
    \begin{equation*}
      F = f(\Cl(f^{-1}(F))).
    \end{equation*}
...
\begin{definition}\label{def:parametric_curve}
  ...
  \begin{defenum}
    \DItem{def:parametric_curve/function_graph}\cite[definition 1.20]{Иванов2017} The graph\Tinyref{def:function/graph} \( \Gph(\gamma) \) of \( \gamma \) is a the image of the curve \( \Ol{\gamma}(t, x) \coloneqq (t, \gamma(x)) \) in the topological space \( I \times X \).
