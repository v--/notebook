...
\begin{proof}
  ...
  \SubProofOf{thm:trigonometric_identities/sums} Fix some \( v, w \in \BC \) and define
  \begin{BreakableAlign*}
...
\begin{definition}\label{def:periodic_function}
  A function \( f: \CG \to \CH \) between \hyperref[def:abelian_group] abelian groups is called \Def{periodic} with \Def{period} \( p \in \CG \) if, for all \( x \in \CG \), we have \( f(x) = f(x + r) \).
  ...
  \begin{DefEnum}
    ...
    \begin{BreakableAlign*}
       ...
       & \ATanTwo(y, x) \coloneqq \begin{cases}
        \arctan(\tfrac y x),       & x > 0                \\
        \arctan(\tfrac y x) + \pi, & x < 0 \T{and} y \geq 0 \\
        \arctan(\tfrac y x) - \pi, & x < 0 \T{and} y < 0    \\
