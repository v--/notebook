...
\begin{definition}\label{def:left_monoid_action}\mcite\cite[159]{Knapp2016BasicAlgebra}
  ...
  \begin{thmenum}
    \labitem{def:left_monoid_action/homomorphism} A homomorphism from \( \mscrM \) to the \hyperref[def:endofunction]{endofunction} monoid \( \End(A) \).
    \labitem{def:left_monoid_action/indirect_homomorphism} An indexed family of endofunctions \( \{ \tau_x \}_{x \in \mscrM} \) such that
    \begin{equation}\label{eq:def:left_monoid_action/indirect_homomorphism}
      \tau_{xy} = \tau_x \circ \tau_y \T{for all} x, y \in \mscrM.
    \end{equation}

    \labitem{def:left_monoid_action/operation} A heterogeneous \hyperref[def:magma]{algebraic operation} \( \circ: \mscrM \times A \to A \), written using juxtaposition, such that
    \begin{thmenum}
      \labitem{def:left_monoid_action/operation/identity} \( e \odot a = a \) holds for any \( a \in A \).
      \labitem{def:left_monoid_action/operation/compatibility} \( (xy) \odot a = x \odot (y \odot a) \) holds whenever \( x, y \in \mscrM \) and \( a \in A \).
    \end{thmenum}
...
\begin{remark}\label{rem:theory_of_left_monoid_actions}
  ...
  \begin{thmenum}
    \labitem{rem:theory_of_left_monoid_actions/functions} For each \( x \in \mscrM \), add a unary \hyperref[def:first_order_language/func]{functional symbol} \( \varphi_x \).

    \labitem{rem:theory_of_left_monoid_actions/axiom} For each \( x, y \in \mscrM \), add the axiom
