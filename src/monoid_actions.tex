...
\begin{definition}\label{def:endomorphism_monoid}
  Given a \hyperref[def:small_and_large_categories]{locally small} \hyperref[def:category]{category} \( \cat{C} \), we call \( \cat{C}(A) \) the \term{endomorphism monoid} over \( A \) and denote it by \( \End(A) \). If \( A \) is the only object in \( \cat{C} \), we can identify the entire category \( \cat{C} \) with the monoid \( \End(A) \).
\end{definition}
