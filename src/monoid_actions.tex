...
\begin{definition}\label{def:left_monoid_action}\MarginCite[159]{Knapp2016BAlg}
  Let \( \CM \) be a \hyperref[def:unital_magma/associativity]{monoid} and let \( A \) be a set. A \Def{left monoid action} of \( \CM \) on \( A \) can be defined equivalently as:
  \begin{DefEnum}
...
\begin{proposition}\label{thm:monoid_is_action}
  Any \hyperref[def:unital_magma/associative]{monoid} \hyperref[def:left_monoid_action]{acts} on itself by endofunctions.
...
\begin{proof}
  ...
  It is a monoid homomorphism because \eqref{eq:def:pointed_set/homomorphism} holds trivially and
  \begin{equation*}
    \varphi(x_1 x_2)
    =
    x_1 x_2 \Id
  ...
  \end{equation*}
  which proves that \( \varphi \) is a monoid homomorphism.
