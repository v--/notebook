...
\begin{definition}\label{def:enriched_category}\cite[180]{MacLane1994},\cite{nLab:enriched_category}
  Enriched categories provide additional structure to the morphism sets of locally small categories. The definition can be compared with \fullref{def:category}. We say that \( \Bold C \) is an \Def{enriched category over the small monoidal category \( \Bold M \)} if
  \begin{itemize}
