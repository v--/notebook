...
\begin{definition}\label{def:poset}
  ...
  A set \( \mscrP \) with any of the two types of partial orders is called a \term{partially ordered set} or simply a \term{poset}.
  ...
  \begin{thmenum}[resume=def:poset]
    ...
    \thmitem{def:poset/submodel} As for preordered sets, any subset of a poset is itself a poset.

    \thmitem{def:poset/trivial} The \hyperref[thm:substructures_form_complete_lattice/bottom]{trivial poset} is the empty set (see \fullref{rem:empty_models} regarding allowing empty sets as first-order structures)..
...
\begin{proposition}\label{thm:totally_ordered_strict_monotone_map_is_injective}
  Let \( (\mscrP, \leq_\mscrP) \) and \( (\mscrQ, \leq_\mscrQ) \) be \hyperref[def:poset]{posets} and suppose that \( (\mscrP, \leq_\mscrP) \) is \hyperref[def:totally_ordered_set]{totally ordered}.

  Then any \hyperref[def:preordered_set/homomorphism]{strictly monotone map} from \( \mscrP \) to \( \mscrQ \) is \hyperref[def:function_invertibility/injective]{injective}.
\end{proposition}
\begin{proof}
  Suppose that \( f(x) = f(y) \) for some \( x \) and \( y \) in \( \mscrP \). We will use the \hyperref[def:binary_relation/trichotomic]{trichotomy} of \( <_\mscrP \).
  \begin{itemize}
...
\begin{proposition}\label{thm:totally_ordered_minimal_element_is_minimum}
  If \( (\mscrP, \leq) \) is a \hyperref[def:totally_ordered_set]{totally ordered set} and \( A \subseteq \mscrP \) is nonempty, then any \hyperref[def:poset_extremal_points/maximal_and_minimal_element]{minimal element} is a \hyperref[def:poset_extremal_points/maximum_and_minimum]{minimum}.
\end{proposition}
\begin{proof}
...
\begin{proposition}\label{thm:totally_ordered_segment_isomorphism}
  Let \( (\mscrP, \leq) \) be a \hyperref[def:totally_ordered_set]{totally-ordered set}. Let \( \mscrQ \) be the set of the \hyperref[def:poset_interval/ray]{strict initial segments} \( \mscrP_{<x} \) for every member \( x \) of \( \mscrP \).

  Then \( (\mscrP, \leq) \) is \hyperref[def:preordered_set/homomorphism]{strictly order-isomorphic} to \( (\mscrQ, \subseteq) \).
\end{proposition}
\begin{proof}
  ...
  We will first show that \( f \) is \hyperref[def:preordered_set/homomorphism]{strictly monotone}. If \( x < y \), then \( x \in \mscrP_{<y} \). But \( x \not\in \mscrP_{<x} \), hence \( \mscrP_{<x} \) is a strict subset of \( \mscrP_{<y} \). Thus \( f \) is strictly monotone.

  The function \( f \) is injective by \fullref{thm:totally_ordered_strict_monotone_map_is_injective} and surjective by definition. Thus \( f \) is a strict isomorphism between \( (\mscrP, \leq) \) and \( (\mscrQ, \subseteq) \).
\end{proof}

\begin{definition}\label{def:order_topology}\mcite{nLab:order_topology}
  Let \( \mscrX \) be a \hyperref[def:poset]{totally ordered set}. The \term{order topology} induced by \( \leq \) is the topology generated by the \hyperref[def:topological_subbase]{subbase} of open \hyperref[def:poset_interval/ray]{rays}
  \begin{equation*}
...
\begin{lemma}[Zorn's lemma]\label{thm:zorns_lemma}\mcite[63]{Gratzer1978}
  ...
  Zorn's lemma is usually stated and used only in a \hyperref[thm:boolean_algebra_of_subsets]{lattice of sets}.

  It is equivalent to the axiom of choice --- see \fullref{thm:axiom_of_choice_equivalences/zorn}.
\end{lemma}
