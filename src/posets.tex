...
\begin{definition}\label{def:poset_interval}\mcite{nLab:order_topology}
  Fix a \hyperref[def:poset]{poset set} \( (\mscrP, \leq) \). For any \( a, b \in \mscrP \) with \( a \leq b \), we define the following related posets:
  ...
  \begin{thmenum}
    ...
    \begin{equation*}
      \begin{aligned}
        \mathllap{ (a, \infty) }  &\coloneqq \mathrlap{ \set{ x \in \mscrP \given b > a } \eqqcolon \mscrP_{>a}, }
        \\
        \mathllap{ (-\infty, b) } &\coloneqq \mathrlap{ \set{ x \in \mscrP \given x < b } \eqqcolon \mscrP_{<b}. }
      \end{aligned}
    \end{equation*}
    ...
    \begin{equation*}
      \begin{aligned}
        \mathllap{ [a, \infty) }  &\coloneqq \mathrlap{ \set{ x \in \mscrP \given x \geq a } \eqqcolon \mscrP_{\geq a}, }
        \\
        \mathllap{ (-\infty, b] } &\coloneqq \mathrlap{ \set{ x \in \mscrP \given x \leq b } \eqqcolon \mscrP_{\leq b}. }
      \end{aligned}
    \end{equation*}

    \thmitem{def:poset_interval/closed} The \term{closed interval} with endpoints \( a \) and \( b \) is
    \begin{equation*}
      \mathllap{ [a, b] } \coloneqq \mathrlap{ \set{ x \in \mscrP \given a \leq x \leq b } = \mscrP_{\geq a} \cap \mscrP_{\leq b}. }
    \end{equation*}

    \thmitem{def:poset_interval/open} The \term{open interval} with endpoints \( a \) and \( b \) is
    \begin{equation*}
      \mathllap{ (a, b) } \coloneqq \mathrlap{ \set{ x \in \mscrP \given a < x < b } = \mscrP_{> a} \cap \mscrP_{< b}. }
    \end{equation*}

    \thmitem{def:poset_interval/half_open} The \term{half-open intervals} are
    \begin{equation*}
      \begin{aligned}
        \mathllap{ (a, b] } &\coloneqq \mathrlap{ \set{ x \in \mscrP \given a < x \leq b } = \mscrP_{> a} \cap \mscrP_{\leq b}, }
        \\
        \mathllap{ [a, b) } &\coloneqq \mathrlap{ \set{ x \in \mscrP \given a \leq x < b } = \mscrP_{\geq a} \cap \mscrP_{< b}. }
      \end{aligned}
    \end{equation*}
  \end{thmenum}
\end{definition}
