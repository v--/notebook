...
\begin{definition}\label{def:poset}
  ...
  \begin{defenum}
    ...
    \item\label{def:poset/bounded_set} The set $A \subseteq X$ is called \ul{bounded from above} (resp. \ul{bounded from below}) if it has an upper bound (resp. lower bound). If a set is bounded from both directions, we simply say that it is \uline{bounded}.
