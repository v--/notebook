...
\begin{definition}\label{def:poset}
  Fix a set \( P \). A \term{partially order} on \( P \) can be defined in the following equivalent ways:
  \begin{thmenum}[series=def:poset]
    \thmitem{def:poset/nonstrict} A \hyperref[def:preordered_set]{preorder} \( \leq \) on \( P \) such that \( \leq \) is \hyperref[def:binary_relation/antisymmetric]{antisymmetric} in addition to being \hyperref[def:binary_relation/reflexive]{reflexive} and \hyperref[def:binary_relation/transitive]{transitive}. This definition is the more common one. If we wish to distinguish it from the other definition, we call such a relation a \term{nonstrict partial order}.

    \thmitem{def:poset/strict} An \hyperref[def:binary_relation/irreflexive]{irreflexive} and \hyperref[def:binary_relation/transitive]{transitive} binary relation \( < \) on \( P \). This relation is called a \term{strict partial order}.
  \end{thmenum}
  ...
  Thus we define a \term{partially ordered set} or simply \term{poset} to be a tuple \( \mscrP = (P, \leq) \) since only one of the two relations is sufficient to define the other.

  The elements \( x, y \in P \) are called \term{comparable} if either \( x \leq y \) or \( y \leq x \).
  ...
  \begin{thmenum}[resume=def:poset]
    ...
    \thmitem{def:poset/duality} The \hyperref[def:preordered_set/duality]{principle of duality for preordered sets} holds for posets if we also swap \( < \) and \( > \).
  \end{thmenum}
\end{definition}
\begin{proof}
  ...
  \begin{itemize}
    \item To see \hyperref[def:binary_relation/reflexive]{reflexivity}, fix \( x \in P \) and assume\DNE that \( x \not\leq x \). Then \( x \neq x \) which contradicts the reflexivity of equality. Hence \( x \leq x \).
...
\begin{definition}\label{def:hasse_diagram}
  It is usually easier to define small finite posets by drawing graphs than by enumerating all relation pairs. Let \( \mscrP = (P, \leq) \) be a finite poset. The relation \( \leq \) may also be regarded as the set of edges of a \hyperref[def:directed_graph]{directed graph}. The graph \( (P, \red^T(\leq)) \), whose edges are the \hyperref[def:derived_relations/transitive]{transitive reduction} of \( \leq \), is called the \term{Hasse graph} or \term{Hasse diagram} of \( \mscrP \).
...
\begin{proposition}\label{thm:preorder_to_partial_order}
  Let \( \mscrP \) be a preordered set. Define the relation \( \cong \) by
  ...
  The pair \( (P / \cong, \preceq) \) is then a \hyperref[def:poset]{partially ordered set}.
\end{proposition}
\begin{proof}
  ...
  Thus \( (P / \cong, \preceq) \) is indeed a poset.
\end{proof}
...
\begin{definition}\label{def:poset_extremal_points}
  We introduce the following terminology for extremal elements of a poset \( \mscrP \). Analogous definition can be given for preordered sets but the nonuniqueness problems outlined in \fullref{ex:preorder_nonuniqueness} highlight that there are sometimes difficulties in doing so.
  ...
  \begin{thmenum}
    ...
    \begin{minipage}[t]{0.45\textwidth}
      An \term{upper bound} for the set \( A \subseteq P \) is an element \( x_0 \in P \) such that \( x \leq x_0 \) for every \( x \in A \). Note that \( x_0 \) does not in general belong to \( A \).
    ...
    \begin{minipage}[t]{0.45\textwidth}
      Dually, \( x_0 \in P \) is a \term{lower bound} of \( A \) if \( x_0 \leq x \) for every \( x \in A \). If \( A \) has a lower bound, it is called \term{bounded from below}.
    ...
    \begin{minipage}[t]{0.45\textwidth}
      ...
      In \cref{fig:ex:def:hasse_diagram}, the entire poset has no maximum but the set \( A = P \setminus \set{ b } = \set{ a, b, d, e } \) has \( a \) as its minimum.
    \end{minipage}

    \thmitem{def:poset_extremal_points/supremum_and_infimum}\mcite[2]{Gratzer1978}
    \begin{minipage}[t]{0.45\textwidth}
      The \term{supremum} \( \sup A \) of \( A \subseteq P \), if it exists, is its least upper bound of \( A \), i.e. the \hyperref[def:poset_extremal_points/maximum_and_minimum]{minimum} of the set of its \hyperref[def:poset_extremal_points/upper_and_lower_bounds]{upper bounds}.
    ...
    \begin{minipage}[t]{0.45\textwidth}
      If it exists, the maximum of the entire poset domain \( P \) is usually denoted by \( \top \) and called the \term{global maximum} or \term{top} element of \( \mscrP \). Since \( \top \) is the maximum of \( P \), it is also the supremum of \( P \).
    ...
    \hspace{0.02\textwidth}
    \begin{minipage}[t]{0.45\textwidth}
      Dually, the minimum of the domain \( P \) is usually denoted by \( \bot \) and called the \term{global minimum} or \term{bottom} element of \( \mscrP \).
...
\begin{definition}\label{def:poset_chain_and_antichain}\mcite[2]{Gratzer1978}
...
\begin{definition}\label{def:poset_interval}\mcite{nLab:order_topology}
  ...
  \begin{thmenum}
    \thmitem{def:poset_interval/ray} The \term{open rays}, also called the \term{open initial segment} and \term{open final segment}, are defined as
    \begin{equation*}
      \begin{aligned}
        \mathllap{ (a, \infty) }  &\coloneqq \mathrlap{ \set{ x \in P \given b > a } \eqqcolon \mscrP_{>a}, }
        \\
        \mathllap{ (-\infty, b) } &\coloneqq \mathrlap{ \set{ x \in P \given x < b } \eqqcolon \mscrP_{<b}. }
      \end{aligned}
    \end{equation*}
    ...
    \begin{equation*}
      \begin{aligned}
        \mathllap{ [a, \infty) }  &\coloneqq \mathrlap{ \set{ x \in P \given x \geq a } \eqqcolon \mscrP_{\geq a}, }
        \\
        \mathllap{ (-\infty, b] } &\coloneqq \mathrlap{ \set{ x \in P \given x \leq b } \eqqcolon \mscrP_{\leq b}. }
      \end{aligned}
    \end{equation*}

    \thmitem{def:poset_interval/closed} The \term{closed interval} with endpoints \( a \) and \( b \) is
    \begin{equation*}
      \mathllap{ [a, b] } \coloneqq \mathrlap{ \set{ x \in P \given a \leq x \leq b } = \mscrP_{\geq a} \cap \mscrP_{\leq b}. }
    \end{equation*}

    \thmitem{def:poset_interval/open} The \term{open interval} with endpoints \( a \) and \( b \) is
    \begin{equation*}
      \mathllap{ (a, b) } \coloneqq \mathrlap{ \set{ x \in P \given a < x < b } = \mscrP_{> a} \cap \mscrP_{< b}. }
    \end{equation*}

    \thmitem{def:poset_interval/half_open} The \term{half-open intervals} are
    \begin{equation*}
      \begin{aligned}
        \mathllap{ (a, b] } &\coloneqq \mathrlap{ \set{ x \in P \given a < x \leq b } = \mscrP_{> a} \cap \mscrP_{\leq b}, }
        \\
        \mathllap{ [a, b) } &\coloneqq \mathrlap{ \set{ x \in P \given a \leq x < b } = \mscrP_{\geq a} \cap \mscrP_{< b}. }
      \end{aligned}
    \end{equation*}
  \end{thmenum}
\end{definition}
...
\begin{definition}\label{def:order_topology}\mcite{nLab:order_topology}
  Let \( \mscrP \) be a \hyperref[def:poset]{totally ordered set}. The \term{order topology} induced by \( \leq \) is the topology generated by the \hyperref[def:topological_subbase]{subbase} of open \hyperref[def:poset_interval/ray]{rays}
  \begin{equation*}
    P \coloneqq \set{ (a, \infty) \given a \in P } \cup \set{ (-\infty, b) \given b \in P }.
  \end{equation*}
\end{definition}
...
\begin{lemma}[Zorn's lemma]\label{thm:zorns_lemma}\mcite[63]{Gratzer1978}
  ...
  See \fullref{thm:axiom_of_choice_equivalences/zorn}.
\end{lemma}
