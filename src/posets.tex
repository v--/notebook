...
\begin{proposition}\label{thm:small_thin_category_equivalent_to_poset}
  Any small thin category\Tinyref{def:thin_category} is equivalent to a category induced by a poset (that is, the objects are the underlying set and $f: x \to y$ is a morphism if and only if $x \leq y$).
