...
\begin{definition}\label{def:vector_space}
  A \Def{vector space} \( (V, +, \cdot) \) is a left \hyperref[def:left_module]{module} over a field \( \BK \).
...
\begin{theorem}\label{thm:all_vector_spaces_are_free_left_modules}
  All vector spaces have a \hyperref[def:left_module_hamel_basis]{basis}. Equivalently, all vector spaces are free \hyperref[def:free_left_module]{modules}. See \fullref{thm:aoc/vector_space_bases}.
\end{theorem}
\begin{proof}
...
\begin{definition}\label{def:vector_space_dimension}
  The free module \hyperref[def:free_left_module]{rank} of a vector space \( V \) is called the \Def{dimension} \( \dim V \) of \( V \). If \( U \) is a vector subspace of \( V \), we call \( \Co\dim_V U \coloneqq \dim(V/U) \) the \Def{codimension} of \( U \) relative to \( V \).
\end{definition}
