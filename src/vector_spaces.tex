...
\begin{remark}\label{remark:real_vector_space}
  Outside of algebra, we are usually only interested in vector spaces over the fields \( \R \) or \( \Co \). We call them \Def{real vector spaces} and \Def{complex vector spaces}, respectively.
\end{remark}

\begin{definition}\label{def:complex_conjucate_vector_space}
  Let \( V \) be a vector space over the complex numbers \( \Co \). Its \Def{complex conjugate vector space} \( \Ol V \) is the same space but with scalar multiplication defined as
  \begin{equation*}
...
\begin{definition}\label{def:eigenpair}
  ...
  An \Def{eigenpair} of \( f \) consists of an \Def{eigenvalue} \( \lambda \in F \) and an \Def{eigenvector} \( x \in U \) such that
  \begin{equation*}
    f(x) = \lambda x.
  \end{equation*}
\end{definition}
