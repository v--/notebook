...
\begin{definition}\label{def:functor}\cite[definitions 1.2.1, 1.2.10]{Leinster2014}
  ...
  \end{defenum}
  we call \( F \) a \Def{contravariant functor}. Equivalently, \( F: \Bold A \to \Bold B \) is contravariant if and only if \( F: \Bold{A}^{\Op} \to \Bold B \) is covariant.
...
\begin{definition}\label{def:tower_diagram}
  Let \( N \) be a subset of \( \Z \) and let \( \Bold{C} \) be any category. A \Def{tower diagram} in \( \Bold{C} \) is an injective on objects (as a function) diagram \( D: N \to \Bold{C} \) over the poset category\Tinyref{def:poset_category}, i.e.
...
\begin{definition}\label{def:opposite_functor}\cite[definition 5.2.1]{Leinster2014}
  Given a functor \( F: \Bold A \to \Bold B \), we define \Def{opposite or dual functor} \( F^{\Op}: \Bold{A}^{\Op} \to \Bold{B}^{\Op} \) as
  \begin{itemize}
    \item \( F^{\Op}(A) = F(A) \)
...
\begin{remark}\label{remark:forgetful_free_functor}\cite[examples 1.2.3, 1.2.4]{Leinster2014}
  An informal notion is that of the \Def{forgetful functor}. A functor \( F: \Bold A \to \Bold B \) is called forgetful if the images \( F(A) \) of objects \( A \in \Bold A \) have \enquote{less structure} than \( A \). For example, a functor which maps topological spaces to their underlying sets is forgetful since it \enquote{forgets} about the topological structure.

  A dual informal notion is that of a \Def{free functor}. In contrast to forgetful functors which \enquote{remove structure}, free functors \enquote{add structure}. For example, a functor which maps a set to its corresponding discrete topological space is a free functor.
\end{remark}
...
\begin{definition}\label{def:faithful_full_functor}\cite[definition 1.2.16]{Leinster2014}
  A functor \( F: \Bold A \to \Bold B \) is called \Def{faithful} (resp. \Def{full}) if the map
  \begin{align*}
    \Bold{A}(A, A) &\to \Bold{B}(F(A), F(A')) \\
    f &\mapsto F(f)
  \end{align*}
...
\begin{definition}\label{def:category_equivalence}\cite[definition 1.3.15]{Leinster2014}
  ...
  An equivalence of the form \( \Bold{A}^{\Op} \simeq \Bold{B} \) is called a \Def{duality} between \( \Bold A \) and \( \Bold B \) and we say that \Def{\( \Bold A \) is dual to \( \Bold B \)} \cite[example 1.3.22]{Leinster2014}.
\end{definition}
...
\begin{definition}\label{def:concrete_category}
  A category \( \Bold C \) is called \Def{concrete} is it is equipped with a forgetful\Tinyref{remark:forgetful_free_functor} faithful\Tinyref{def:faithful_full_functor} functor \( F: \Bold{C} \to \Bold{Set} \).
...
\begin{definition}\label{def:representable_functor}\cite[definitions 4.1.3, 4.1.16]{Leinster2014}
  ...
  We say that the functor \( F: \Bold{A} \to \Bold{Set} \) is \Def{representable} with \Def{representation} \( H^A \) if \( F \cong H^A \).
...
\begin{definition}\label{def:yoneda_embedding}\cite[definitions 4.1.15, 4.1.21]{Leinster2014}
  ...
  Analogously, we define \( H_f \) by \( H_f(C) = H^C(f), C \in \Bold A \) and the \Def{Yoneda embedding} \( H_\bullet: \Bold{A} \to [\Bold{A}^{\Op}, \Bold{Set}] \) by
  \begin{align*}
