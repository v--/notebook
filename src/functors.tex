...
\begin{definition}\label{def:natural_transformation}\mcite[def. 1.3.1]{Leinster2016Basic}
  ...
  A \term{natural transformation} \( \alpha \) from \( \cat{C} \) to \( \cat{D} \) is an \hyperref[def:tuple_and_cartesian_product/indexed_family]{indexed family} of
...
\begin{example}\label{ex:quivers_as_functors}
  ...
  This is a categorification of the conditions \eqref{eq:def:category_of_small_quivers/homomorphism/head} and \eqref{eq:def:category_of_small_quivers/homomorphism/tail} for a quiver homomorphism as defined in \fullref{def:category_of_small_quivers}.
\end{example}
