...
\begin{proposition}\label{thm:def:functor_invertibility/properties}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:def:functor_invertibility/properties/faithful_reflects_composition} A \hyperref[def:functor_invertibility/faithful]{faithful} functor reflects composition. That is, for every functor \( F: \cat{C} \to \cat{D} \), if the following diagram commutes:
    \begin{equation}\label{eq:thm:def:functor_invertibility/properties/faithful_reflects_composition/image}
      \begin{aligned}
        \includegraphics[page=1]{figures/thm__def__functor_invertibility__properties.pdf}
      \end{aligned}
    \end{equation}
    ...
    \begin{equation}\label{eq:thm:def:functor_invertibility/properties/faithful_reflects_composition/source}
      \begin{aligned}
        \includegraphics[page=2]{figures/thm__def__functor_invertibility__properties.pdf}
      \end{aligned}
    \end{equation}
...
\begin{proof}
  ...
  \SubProofOf{thm:def:functor_invertibility/properties/faithful_reflects_composition} Suppose that \eqref{eq:thm:def:functor_invertibility/properties/faithful_reflects_composition/image} commutes. Then, since \( F \) is faithful and thus injective on the morphism set \( \cat{C}(X, Z) \), the equality \( F(g \bincirc f) = F(g) \bincirc F(f) = F(h) \) implies that \( g \bincirc f = h \). Hence, \eqref{eq:thm:def:functor_invertibility/properties/faithful_reflects_composition/source} also commutes.
