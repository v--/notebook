...
\begin{example}\label{ex:topological_space_presheaf}\cite[24]{Leinster2014}
  Let \( (X, \tau) \) be a topological space. Form the category \( \Cat{C} \) from the poset \( (\tau, \subseteq) \) as in \fullref{thm:poset_iff_poset_category}. Presheaves on \( \Cat{C} \) are also called presheaves on the topological space \( (X, \tau) \).
