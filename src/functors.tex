...
\begin{proof}
  ...
  \begin{align*}
    F(f^{-1}) \circ F(f)
    \overset{\fullref{def:functor/composition_axiom}} =
    F(f^{-1} \circ f)
    =
    F(\Id_A)
    \overset{\fullref{def:functor/identity_axiom}} =
    \Id_{F(A)}.
  \end{align*}
...
\begin{example}\label{ex:topological_space_presheaf}\cite[24]{Leinster2014}
  Let \( (X, \tau) \) be a topological space. Form the category \( \Bold C \) from the poset \( (\tau, \subseteq) \) as in \fullref{thm:poset_iff_poset_category}. Presheaves on \( \Bold C \) are also called presheaves on the topological space \( (X, \tau) \).
