...
\begin{definition}\label{def:subcategory}\mcite[def. 1.2.18]{Leinster2016Basic}
  ...
  \begin{itemize}
    \item The underlying quiver \( U(\cat{D}) \) is a \hyperref[eq:def:hypergraph/submodel]{subquiver} of \( U(\cat{C}) \). That is, every object in \( \cat{D} \) is an object in \( \cat{C} \) and every morphism in \( \cat{D} \) is a morphism in \( \cat{C} \).
  ...
  \begin{thmenum}
    ...
    \thmitem{def:subcategory/full} We say that \( \cat{D} \) is a \term{full subcategory} if the underlying quiver \( U(\cat{C}) \) is a \hyperref[eq:def:hypergraph/submodel/full]{full subquiver}. That is, in case \( \cat{D}(A, B) = \cat{C}(A, B) \) for every pair of objects \( A \) and \( B \) of \( \cat{D} \).
    ...
    \thmitem{def:subcategory/induced} Every \hyperref[rem:family_of_sets]{family} \( \mscrU \) of objects in \( \cat{C} \) induces a full subcategory \( \cat{D} \) of \( \cat{C} \), whose objects are those of \( \mscrU \) and whose morphisms are restricted to those whose domain and codomain are both in \( \mscrU \).
  \end{thmenum}
\end{definition}
...
\begin{remark}\label{rem:inverting_isomorphisms_may_preserve_commutativity}
  ...
  \begin{equation}\label{eq:rem:inverting_isomorphisms_may_preserve_commutativity/ordinals_commuting}
    \begin{aligned}
      \includegraphics[page=1]{figures/rem__inverting_isomorphisms_may_preserve_commutativity.pdf}
    \end{aligned}
  \end{equation}
  but the following does not:
  \begin{equation}\label{eq:rem:inverting_isomorphisms_may_preserve_commutativity/ordinals_not_commuting}
    \begin{aligned}
      \includegraphics[page=2]{figures/rem__inverting_isomorphisms_may_preserve_commutativity.pdf}
    \end{aligned}
  \end{equation}
\end{remark}
...
\begin{definition}\label{def:diagonal_functor}\mcite[143]{Leinster2016Basic}
  ...
  \begin{equation}\label{eq:def:diagonal_functor/nat}
    \begin{aligned}
      \includegraphics[page=1]{figures/def__diagonal_functor.pdf}
    \end{aligned}
  \end{equation}
  ...
  We can now define the \term{diagonal functor}
  \begin{equation*}
    \begin{aligned}
