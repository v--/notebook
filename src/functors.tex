...
\begin{definition}\label{def:functor}\mcite\cite[def. 1.2.1 \\ def. 1.2.10]{Leinster2016Basic}
  ...
  such that
  \begin{thmenum}
    \labitem{def:functor/composition_axiom} \( A \overset f \mapsto B \overset g \mapsto C \) implies \( F(g \circ f) = F(g) \circ F(f) \).
    \labitem{def:functor/identity_axiom} \( A \in \cat{A} \) implies \( F(\id_A) = \id_{F(A)} \).
  \end{thmenum}
