...
\begin{definition}\label{def:functor}\mcite\cite[def. 1.2.1 \\ def. 1.2.10]{Leinster2014}
...
\begin{definition}\label{def:tower_diagram}
  Let \( N \) be a subset of \( \BbbZ \) and let \( \cat{C} \) be any category. A \term{tower diagram} in \( \bfop{C} \) is an injective on objects (as a function) diagram \( D: N \to \bfop{C} \) over the \hyperref[def:category_of_posets]{\( \cat{Pos} \) category}, i.e.
...
\begin{definition}\label{def:opposite_functor}\mcite\cite[def. 5.2.1]{Leinster2014}
  Given a functor \( F: \cat{A} \to \cat{B} \), we define \term{opposite or dual functor} \( F^{-1}: \cat{A}^{-1} \to \bfop{B}^{-1} \) as
  \begin{itemize}
    \item \( F^{-1}(A) = F(A) \)
...
\begin{proposition}\label{thm:functors_preserve_isomorphisms}\mcite\cite[exer. 1.2.21]{Leinster2014}
...
\begin{definition}\label{def:presheaf}\mcite\cite[def. 1.2.15]{Leinster2014}
...
\begin{example}\label{ex:topological_space_presheaf}\mcite\cite[24]{Leinster2014}
...
\begin{definition}\label{def:faithful_full_functor}\mcite\cite[def. 1.2.16]{Leinster2014}
...
\begin{example}\label{def:subcategory_functors}\mcite\cite[25]{Leinster2014}
...
\begin{definition}\label{def:natural_transformation}\mcite\cite[def. 1.3.1]{Leinster2014}
...
\begin{definition}\label{def:natural_transformation_horizontal_composition}\mcite\cite[rem. 1.3.24]{Leinster2014}
...
\begin{definition}\label{def:comma_category}\mcite\cite[def. 2.3.1]{Leinster2014}
  ...
  \begin{itemize}
    ...
    \item The morphisms from \( (A, h, B) \) to \( (A', h', B') \) are pairs \( (f, g) \in \cat{A}(A, A') \times \bfop{B}(B, B') \) such that the following diagram commutes:
