...
\begin{definition}\label{def:concrete_category}
  A \Def{concrete category} is a category \( \Cat{A} \) equipped with a faithful functor \( F: \Cat{A} \to \Cat{B} \). We say that \( \Cat{A} \) is concrete with respect to \( \Cat{B} \).
...
\begin{definition}\label{def:tower_diagram}
  Let \( N \) be a subset of \( \BbbZ \) and let \( \Cat{C} \) be any category. A \Def{tower diagram} in \( \Bold{C} \) is an injective on objects (as a function) diagram \( D: N \to \Bold{C} \) over the \hyperref[def:category_of_posets]{\( \Cat{Pos} \) category}, i.e.
