...
\begin{definition}\label{def:tower_diagram}
  Let \( N \) be a subset of \( \BbbZ \) and let \( \cat{C} \) be any category. A \term{tower diagram} in \( \bop{C} \) is an injective on objects (as a function) diagram \( D: N \to \bop{C} \) over the \hyperref[def:category_of_posets]{\( \cat{Pos} \) category}, i.e.
...
\begin{definition}\label{def:opposite_functor}\mcite\cite[def. 5.2.1]{Leinster2014}
  Given a functor \( F: \cat{A} \to \cat{B} \), we define \term{opposite or dual functor} \( F^{-1}: \cat{A}^{-1} \to \bop{B}^{-1} \) as
  \begin{itemize}
    \item \( F^{-1}(A) = F(A) \)
...
\begin{definition}\label{def:comma_category}\mcite\cite[def. 2.3.1]{Leinster2014}
  ...
  \begin{itemize}
    ...
    \item The morphisms from \( (A, h, B) \) to \( (A', h', B') \) are pairs \( (f, g) \in \cat{A}(A, A') \times \bop{B}(B, B') \) such that the following diagram commutes:
