...
\begin{definition}\label{def:functor}\mcite[def. 1.2.1 \\ def. 1.2.10]{Leinster2016Basic}
  ...
  \begin{thmenum}[series=def:functor]
    \thmitem[def:functor/CF1]{CF1} Functors must preserve identities, meaning that for any object \( A \in \cat{C} \) we must have
    \begin{equation}\label{eq:def:functor/CF2}\tag{\logic{CF2}}
      F(\id_A) = \id_{F(A)}.
    \end{equation}

    \thmitem[def:functor/CF2]{CF2} Functors must preserve composition, meaning that for any pair of morphism \( f: A \to B \) and \( g: B \to C \) in \( \cat{C} \), we have
    \begin{equation}\label{eq:def:functor/CF2}\tag{\logic{CF2}}
...
\begin{remark}\label{rem:categorical_diagram_as_functor}
  ...
  Fix a category \( \cat{I} \), called an \term{index category}. A \term{diagram} in \( \cat{C} \) of shape \( \cat{I} \) is simply a functor \( D: \cat{I} \to \cat{C} \), whose domain is \( \cat{I} \). We often identify a diagram functor with its image \( D(\cat{I}) \).
...
\begin{example}\label{ex:constant_functor_natural_transformation}\mcite{math3ma:natural_transformations}
  Let \( A \) and \( B \) be objects in some category \( \cat{C} \). A \hyperref[def:natural_transformation]{natural transformation} between two \hyperref[ex:constant_functor]{constant functors} \( F_A \) and \( F_B \) with a common domain \( \cat{I} \) is simply a morphism from \( A \) to \( B \).

  Indeed, for every object \( A \in \cat{I} \), we have \( F_A(A) = A \) and \( F_B(A) = B \). Then, if \( \alpha: F_A \Rightarrow F_B \) is any natural transformation, for every function \( f: A \to B \), the diagram \eqref{eq:def:natural_transformation/diagram} becomes
  ...
  This diagram implies that \( \alpha_A = \alpha_B \) for any two objects \( A \) and \( B \) in \( \cat{I} \). Therefore, all components of \( \alpha \) are equal to some morphism in \( \cat{D}(A, B) \).
\end{example}
