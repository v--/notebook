...
\begin{example}\label{ex:topological_space_presheaf}\cite[24]{Leinster2014}
  Let $(X, \tau)$ be a topological space. Form the category $\Bold C$ from the poset $(\tau, \subseteq)$ as in \cref{thm:small_thin_category_equivalent_to_poset}. Presheaves on $\Bold C$ are also called presheaves on the topological space $(X, \tau)$.
...
\begin{definition}\label{def:concrete_category}
  A category $\Bold C$ is called \uline{concrete} is it is equipped with a forgetful\Tinyref{note:forgetful_free_functor} faithful\Tinyref{def:faithful_full_functor} functor $F: \Bold{C} \to \Bold{Set}$.
