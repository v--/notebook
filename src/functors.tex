...
\begin{definition}\label{def:tower_diagram}
  Let $N$ be a subset of $\BB{Z}$ and let $\Bold{C}$ be any category. A \uline{tower diagram in $\Bold{C}$} is an injective on objects (as a function) diagram $D: N \to \Bold{C}$ over the poset category\Tinyref{def:poset_category}, i.e.
