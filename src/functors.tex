...
\begin{definition}\label{def:tower_diagram}
  Let \( N \) be a subset of \( \BZ \) and let \( \Cat{C} \) be any category. A \Def{tower diagram} in \( \Bold{C} \) is an injective on objects (as a function) diagram \( D: N \to \Bold{C} \) over the poset \hyperref[def:category_of_posets]{category}, i.e.
