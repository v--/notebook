...
\begin{definition}\label{def:diagonal_matrix}
  We say that the square \hyperref[def:array/matrix]{matrix} \( \{ a_{i,j} \}_{i,j=1}^{n,n} \) over a field \( \BbbK \) is \Def{diagonal} if \( a_{i,j} = 0 \) whenever \( i \neq j \), that is, only the main diagonal has nonzero elements:
  \begin{equation*}
    \begin{pmatrix}
      a_{1,1} & 0       & \cdots & 0       \\
      0       & a_{2,2} & \cdots & 0       \\
...
\begin{definition}\label{def:diagonalizable_matrix}
  A matrix over a field is \Def{diagonalizable} if it is isomorphic to a diagonal matrix.
\end{definition}

\begin{theorem}[Spectral theorem for matrices]\label{def:spectral_theorem_for_matrices}\MarginCite[thm. 6.5]{Knapp2016BAlg}
