...
\begin{example}\label{ex:categorical_product/set}
  Let $\{ X_i \}_{i \in I}$ be an indexed family in the category $\Bold{Set}$. Their product $\prod_{i \in I} X_i$ is simply the Cartessian product of the corresponding sets\Tinyref{def:cartessian_product}.
\end{example}
...
\begin{example}\label{ex:categorical_coequalizer/set}
  ...
  In particular, if $\sim \subseteq X^2$ is an equivalence relation\Tinyref{def:orders:equivalence}) on $X$, then the coequalizer of the two projection maps of the product $X^2$\Tinyref{ex:categorical_product/set} is the pair $(X / ~, \pi)$, where $\pi$ is the quotient map
  \begin{align*}
    &\pi: X \to X / ~ \\
    &\pi(x) = [x].
  \end{align*}
