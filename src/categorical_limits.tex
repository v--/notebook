...
\begin{definition}\label{def:direct_and_inverse_limits}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:direct_and_inverse_limits/inverse}\mcite[exmpl. 5.1.21(d)]{Leinster2016Basic} Now consider the dual category
...
\begin{example}\label{ex:def:direct_and_inverse_limits}
  ...
  \begin{thmenum}
    \thmitem{ex:def:direct_and_inverse_limits/vector_space_direct} Consider the chain of \hyperref[def:left_module_of_tuples]{tuple vector spaces}
    ...
    Let \( \BbbK_0^\infty \) be the vector space of all sequences in \( \BbbK \) with only finitely many nonzero elements. For each positive integer \( n \), denote by \( \iota_n^\infty: \BbbK^n \to \BbbK_0^\infty \) the canonical embedding.
