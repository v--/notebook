...
\begin{definition}\label{def:categorical_limit_preservation}\mcite\cite[def. 5.3.1, 5.3.5]{Leinster2016Basic}
  ...
  \begin{thmenum}
    ...
    \ilabel{def:categorical_limit_preservation/lift} \( F \) \term{lifts} limits of shape \( \cat{I} \) if, given a diagram \( D: \cat{I} \to \boldop B \), any limit cone \( \varprojlim D \) is the image of some limit cone in \( A \).
  \end{thmenum}
\end{definition}
