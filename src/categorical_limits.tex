...
\begin{proof}
  ...
  Now let \( (L', \lambda') \) and \( (L^\dprime, \lambda^\dprime) \) be colimit cones. By \fullref{thm:categorical_limit_duality}, \( (L', \lambda'^{\opcat}) \) and \( (L^\dprime, {\lambda^\dprime}^{\opcat}) \) are limits in the dual category and are thus isomorphic. By \fullref{thm:morphism_invertibility_duality}, the colimits are isomorphic.
\end{proof}
...
\begin{definition}\label{def:tower_diagram}\mcite{nLab:tower_diagram}
  A \term{tower diagram} is a diagram over the dual category of that \hyperref[thm:order_category_isomorphism]{induced} by the positive integers:
  \begin{equation}\label{eq:def:tower_diagram}
    \begin{aligned}
      \includegraphics[page=1]{output/def__tower_diagram.pdf}
    \end{aligned}
  \end{equation}

  \mcite[exmpl. 5.1.21(d) \\ exmpl. 5.2.15]{Leinster2016Basic} Somewhat confusingly, a \hyperref[def:category_of_cones/limit]{limit} of a tower diagram is called an \term{inverse limit} and a \hyperref[def:category_of_cones/colimit]{colimit} is called a \term{direct limit}. This terminology predates categorical limits and colimits.
\end{definition}

\begin{example}\label{ex:direct_limit_of_vector_spaces}
  Consider the \hyperref[def:tower_diagram]{tower} of \hyperref[def:vector_space]{vector spaces}
  \begin{equation}\label{eq:ex:direct_limit_of_vector_spaces/diagram}
    \begin{aligned}
      \includegraphics[page=1]{output/ex__direct_limit_of_vector_spaces.pdf}
    \end{aligned}
  \end{equation}
  where
  \begin{equation*}
    \begin{aligned}
      &\pi_k^m: \BbbK^m \to \BbbK^k \\
      &\pi_k^m(x_1, \ldots, x_k, x_{k+1}, \ldots, x_m) \coloneqq (x_1, \ldots, x_k).
    \end{aligned}
  \end{equation*}

  Let \( \BbbK^\infty \) be the vector space of all sequences in \( \BbbK \) and, for each positive integer \( k \), define \( \pi_k^\infty \) as a truncation in the obvious way.

  Then \( (\BbbK^\infty, \pi^\infty) \) is a \hyperref[def:category_of_cones/cone]{cone} of \eqref{eq:ex:direct_limit_of_vector_spaces/diagram}. We will show that it is a limit cone, i.e. that \( \BbbK^\infty \) is a \hyperref[def:tower_diagram]{direct limit} of \eqref{eq:ex:direct_limit_of_vector_spaces/diagram}.

  Let \( (A, \alpha) \) be another cocone. We want to define \( l_A: A \to \BbbK^\infty \) so that the following diagram commutes:
  \begin{equation}\label{eq:ex:direct_limit_of_vector_spaces/limit}
    \begin{aligned}
      \includegraphics[page=2]{output/ex__direct_limit_of_vector_spaces.pdf}
    \end{aligned}
  \end{equation}

  For every \( a \in A \), we must have
  \begin{equation*}
    \alpha_k(a) = \pi_k^\infty(l_A(a)),
  \end{equation*}
  which implies the obvious definition
  \begin{equation*}
    l_A(a) \coloneqq \seq{ \alpha_k(a)_k }_{k=1}^\infty.
  \end{equation*}

  That is, \( l_A(a) \) is a sequence whose \( k \)-th coordinate is the \( k \)-th coordinate \( \alpha_k(a) \). This is well-defined because, for \( m > k \), we have
  \begin{equation*}
    \pi_m^k \bincirc \alpha_m = \alpha_k.
  \end{equation*}
\end{example}

\begin{example}\label{ex:inverse_limit_as_intersection}
  In \hyperref[def:concrete_category]{concrete categories}, \hyperref[def:tower_diagram]{inverse limits} are generalizations of \hyperref[thm:zfc_existence_theorems/arbitrary_intersection]{set intersections}.

  As an example, consider the \hyperref[def:tower_diagram]{tower} of sets
  \begin{equation}\label{eq:ex:inverse_limit_as_intersection/diagram}
    \begin{aligned}
      \includegraphics[page=1]{output/ex__inverse_limit_as_intersection.pdf}
    \end{aligned}
  \end{equation}
  where \( A_{k+1} \subseteq A_k \) for every positive integer \( k \) and \( \iota_k^m: A_m \to A_k \) is simply the canonical inclusion map.

  Then the intersection \( \bigcap_{k=1}^\infty A_k \) along with its inclusion maps is a colimit of \eqref{eq:ex:inverse_limit_as_intersection/diagram}.
...
\begin{definition}\label{def:discrete_category_limits}
  ...
  It will be convenient for us to speak about the product of a subset of \( \obj(\cat{C}) \). Fix an \hyperref[def:cartesian_product/indexed_family]{indexed family} \( \mscrX = \seq{ X_k }_{k \in \mscrK} \) of objects in \( \cat{C} \).
  ...
  \hfill
  \begin{minipage}[t]{0.47\textwidth}
    We call the morphism \( \iota_k \) the \term{canonical embedding} of \( X_k \) into the coproduct, even though it may not be an injective function.
  \end{minipage}
  \medskip
  ...
  As in the case of general limits, we call \( \prod_{k \in \mscrK} X_k \) \hi{the} product of the family \( \mscrX \).
...
\begin{definition}\label{def:equalizers}\mcite[def. 5.1.11]{Leinster2016Basic}
  ...
  Note how we interchanged the notation for the projections and embeddings compared to \fullref{def:discrete_category_limits} --- \( (L, \iota) \) is a \hi{limit} of a fork, while \( (L, \pi) \) is a \hi{colimit}. The reason for this is that equalizers are usually canonical embeddings, while coequalizers are projections.
\end{definition}
...
\begin{example}\label{ex:zero_morphisms_in_grp}
  ...
  \begin{thmenum}
    ...
    Indeed, fix a group homomorphism \( \psi: A \to G \) such that \( \varphi \bincirc \psi = \tau \bincirc \psi \). We will show that \( \psi \) \hyperref[def:factors_through]{uniquely factors through} \( \ker(\varphi) \). We want to find a map \( l_A: A \to \ker(\varphi) \) such that the following diagram commutes:
    ...
    We will show that \( \psi \) \hyperref[def:factors_through]{uniquely factors through} \( Q \). We want to find a map \( l_A: Q \to A \) such that the following diagram commutes:
