...
\begin{definition}\label{def:category_of_cones}
  ...
  \begin{thmenum}
    ...
    \end{equation}
    satisfying a simplified naturality condition (compared to \eqref{eq:def:natural_transformation/diagram}). For every morphism \( u: k \to j \) in \( \cat{I} \), the following diagram must commute:
    ...
    Even more explicitly, \( (L, \pi) \) is a limit cone if it satisfies the following \hyperref[rem:limit_universal_mapping_property]{universal mapping property}:
    \begin{displayquote}
      For every cone \( (A, \alpha) \), there exists a unique morphism \( l_A: A \to L \) such that following diagram commutes for every index morphism \( u: k \to j \):
    ...
    \end{equation}
    satisfying the naturality condition that for every morphism \( u: k \to j \) in \( \cat{I} \), the following diagram must commute:
...
\begin{proof}
  ...
  Now let \( (L', \pi') \) and \( (L^\dprime, \pi^\dprime) \) be colimit cones. By \fullref{thm:categorical_limit_duality}, \( (L', \pi'^{\opcat}) \) and \( (L^\dprime, {\pi^\dprime}^{\opcat}) \) are limits in the dual category and are thus isomorphic. By \fullref{thm:morphism_knvertibility_duality}, the colimits are isomorphic.
\end{proof}
...
\begin{proof}
  ...
  \begin{equation*}
    \varepsilon_{\Delta(A)} = \seq{ \pi_k^{\Delta_A}: \lim(\Delta_A) \to \underbrace{\Delta_A(k)}_{A} }_{k \in \cat{I}}.
  \end{equation*}
...
\begin{proposition}\label{thm:limits_of_empty_diagram}
  The \hyperref[def:category_of_cones/limit]{limits} of an empty diagram over \( \cat{C} \) are the \hyperref[def:universal_object/terminal]{terminal objects} and the \hyperref[def:category_of_cones/colimit]{colimits} are the \hyperref[def:universal_object/initial]{initial object} .
...
\begin{proposition}\label{thm:limits_of_identity_functor}
  ...
  \begin{thmenum}
    \thmitem{thm:limits_of_identity_functor/initial_object_ks_limit} If \( I \) is an \hyperref[def:universal_object/initial]{initial object} of \( \cat{C} \), \( (I, \xi) \) is a limit cone of the \hyperref[eq:def:category_of_small_categories/identity]{identify functor} \( \id_{\cat{C}} \), where
    \begin{equation*}
      \xi \coloneqq \seq{ \xi_A: I \to A }_{A \in \cat{C}}
    \end{equation*}
    is the family of unique morphisms with domain \( I \).

    \thmitem{thm:limits_of_identity_functor/limit_ks_knitial_object} Conversely, if \( (L, \pi) \) is a limit cone of the identity, then \( L \) is an initial object.
  \end{thmenum}
...
\begin{proof}
  ...
  \SubProofOf{thm:limits_of_identity_functor/limit_ks_knitial_object} Let \( (L, \pi) \) be a limit cone of the identity. The component \( \pi_B \) of the natural transformation \( \pi \) is a morphism from \( L \) to \( B \). In order for \( L \) to be an initial object, these morphisms must be unique.
  ...
  Since \eqref{eq:def:thm:limits_of_identity_functor/limit_ks_knitial_object/endomorphism_uniqueness} commutes with \( \id_L \) instead of \( l_L \), by uniqueness it follows that \( l_L = \id_L \). Since, by the naturality of \( \pi \), \( \pi_L \) also satisfies this condition, \( \pi_L = \id_L \).
...
\begin{example}\label{ex:limits_of_partially_ordered_set}
  ...
  The image of a diagram \( D: \cat{I} \to \cat{P} \) is a pair \( (A, R) \), where \( A \) is the set of objects \( D(\obj(\cat{I})) \) in the image \( D(\cat{I}) \) and \( R \) is a subrelation of \( \leq \). This relation is reflexive, however it may not even be a preorder as shown in \fullref{ex:functor_kmage_not_a_category}.
...
\begin{definition}\label{def:discrete_category_limits}
  ...
  It will be convenient for us to speak about the product of a subset of \( \obj(\cat{C}) \). Fix an \hyperref[def:tuple_and_cartesian_product/indexed_family]{indexed family} \( \mscrX = \seq{ X_k }_{k \in \mscrK} \) of objects in \( \cat{C} \).
  ...
  \begin{minipage}[t]{0.47\textwidth}
    ...
    A product \hyperref[def:category_of_cones/cone]{cone} \( (L, \pi) \) of \( \mscrX \) satisfies the following \hyperref[rem:limit_universal_mapping_property]{universal mapping property}:
    \begin{displayquote}
  ...
  \begin{minipage}[t]{0.47\textwidth}
    ...
    A coproduct \hyperref[def:category_of_cones/cocone]{cocone} \( (L, \iota) \) of \( \mscrX \) satisfies the following \hyperref[rem:limit_universal_mapping_property]{universal mapping property}:
    \begin{displayquote}
      For every cone \( (A, \alpha) \), there exists a unique morphism
      \begin{equation*}
        l_A: A \to L,
  ...
  As in the case of general limits, we call \( L \) \hi{the} product (resp. coproduct) of the family \( \mscrX \).
...
\begin{proposition}\label{thm:discrete_category_limits_in_set}
  The \hyperref[def:discrete_category_limits]{product} in the \hyperref[def:category_of_small_sets]{category \( \ucat{Set} \) of \( \mscrU \)-small sets} of a family \( \mscrA = \seq{ A_k }_{k \in \mscrK} \) is their \hyperref[def:tuple_and_cartesian_product/product]{Cartesian product} \( \prod_{k \in \mscrK} A_k \) and the \hyperref[def:discrete_category_limits]{coproduct} is their \hyperref[def:disjoint_union]{disjoint union} \( \coprod_{k \in \mscrK} A_k \).
\end{proposition}
\begin{proof}
...
\begin{definition}\label{def:equalizers}\mcite[def. 5.1.11]{Leinster2016Basic}
  ...
  \begin{minipage}[t]{0.47\textwidth}
    ...
    An equalizer cone \( (L, \iota) \) satisfies the following \hyperref[rem:limit_universal_mapping_property]{universal mapping property}:
    \begin{displayquote}
      For every cone \( (A, f) \), there exists a unique morphism
      \begin{equation*}
         l_A: A \to L
      \end{equation*}
      such that the following diagram commutes:
  ...
  \begin{minipage}[t]{0.47\textwidth}
    ...
    \begin{displayquote}
      For every cocone \( (A, f) \), there exists a unique morphism
      \begin{equation*}
        l_A: L \to A
      \end{equation*}
      such that the following diagram commutes:
  ...
  Note how we interchanged our usual notation for the projections and coprojections --- \( (L, \iota) \) is a \hi{limit} of a fork, while \( (L, \pi) \) is a \hi{colimit}. The reason for this is that equalizers are usually canonical embeddings, while coequalizers are projections.
\end{definition}
...
\begin{proposition}\label{thm:equalizer_invertibility}
  ...
  \hyperref[thm:categorical_principle_of_duality]{Dually}, if \( (L, \pi) \) is a \hyperref[def:equalizers]{coequalizer}, then \( \pi \) is a \hyperref[def:morphism_invertibility/right_cancellative]{epimorphism}.
\end{proposition}
\begin{proof}
  ...
  Dually, if \( (L, \pi) \) is a coequalizer cone in \( \cat{C} \), then by \fullref{thm:categorical_limit_duality}, \( (L, \pi^{\opcat}) \) is an equalizer in \( \cat{C}^{\opcat} \). Hence, \( \pi^{\opcat} \) is a monomorphism, and by \fullref{thm:morphism_invertibility_duality}, \( \pi \) is an epimorphism.
\end{proof}

\begin{example}\label{ex:equalizers_in_set}
  For a pair of functions \( s, t: X \to Y \) in \( \cat{Set} \), define
  \begin{equation*}
    E = \set{ x \in X \given s(x) = t(x) }
  \end{equation*}
  and take the inclusion map \( \iota: E \to X \). Then \( (E, \iota) \) is an \hyperref[def:equalizers]{equalizer} cone.

  Indeed, it is obviously a cone and, for any other cone \( (A, f) \), the restriction \( f\restr_L \) is the unique map for which the defining diagram for an equalizer commutes:
  ...
  The coequalizer is more nuanced. Let \( {\sim} \) be the smallest equivalence relation on \( Y \) such that \( s(x) \sim t(x) \) for every \( x \in X \). Explicitly, this is the \hyperref[def:relation_closures/reflexive]{reflexive}, \hyperref[def:relation_closures/symmetric]{symmetric} and \hyperref[def:relation_closures/transitive]{transitive} closure of the relation
  \begin{equation*}
    \set{ (s(x), t(x)) \given x \in X }.
  \end{equation*}

  Let \( \mscrY \) be the equivalence partition \( Y / {\sim} \). Then \( \mscrY \) with the projection map
  \begin{equation*}
    \begin{aligned}
      &\pi: Y \to \mscrY \\
      &\pi(y) \coloneqq [y]
  ...
  Indeed, fix any other cocone \( (A, f) \). We want to define a map \( l_A: \mscrY \to A \) such that the defining diagram for a coequalizer
  \begin{equation}\label{eq:ex:equalizers_in_set/coequalizer}
    \begin{aligned}
      \includegraphics[page=2]{output/ex__equalizers_in_set.pdf}
    \end{aligned}
  \end{equation}
  commutes. That is, we want
  \begin{equation*}
    \underbrace{l_A(\pi(y))}_{l_A([y])} = f(y).
  \end{equation*}

  This condition uniquely determines the function \( l_A([y]) \coloneqq f(y) \).
\end{example}
...
\begin{definition}\label{def:zero_morphisms}
  ...
  \begin{thmenum}
    \thmitem{def:zero_morphisms/morphism}\mcite{nLab:zero_morphism} For every pair of objects \( A \) and \( B \) in \( \cat{C} \), there exist unique morphisms
    ...
    We denote their composition morphism by \( 0_{A,B} \) and call it the \term{zero morphism} from \( A \) to \( B \).

    \thmitem{def:zero_morphisms/kernel} The \term{kernel} of a morphism \( f: A \to B \) is the \hyperref[eq:def:equalizers/equalizer]{equalizer} cone \( (L, \iota) \) of \( f \) and \( 0_{A,B} \).

    Depending on the context, we may refer to either \( L \) or \( \iota \) as \enquote{the} kernel.
    ...
    \thmitem{def:zero_morphisms/cokernel} \hyperref[thm:categorical_principle_of_duality]{Dually}, the \term{cokernel} of a morphism \( f: A \to B \) is the \hyperref[eq:def:equalizers/coequalizer]{coequalizer} cone \( (L, \pi) \) of \( f \) and \( 0_{A,B} \).

    Depending on the context, we may refer to either \( L \) or \( \pi \) as \enquote{the} cokernel.
    ...
    \thmitem{def:zero_morphisms/image} Finally, the \term{image} of \( f \) is the cokernel of its kernel.
  \end{thmenum}
\end{definition}

\begin{example}\label{ex:zero_morphisms_in_grp}
  Consider the category \( \cat{Grp} \) of small \hyperref[def:group]{groups} and fix groups \( G \) and \( H \).

  \begin{thmenum}
    \thmitem{ex:zero_morphisms_in_grp/morphism} Note that, by \fullref{ex:def:universal_objects/grp}, \enquote{the} trivial group \( \set{ e } \) is a \hyperref[def:universal_objects/zero]{zero object}.

    The trivial homomorphism from \( G \) to \( H \) is.
    \begin{equation*}
      \tau(x) \coloneqq e_H.
    \end{equation*}

    It \( \tau \) can be represented via the composition \eqref{eq:def:zero_morphisms/morphism}, hence this is the zero morphism from \( G \) to \( H \) in \( \cat{Grp} \).

    \thmitem{ex:zero_morphisms_in_grp/kernel} Let \( \varphi: G \to H \) be a \hyperref[thm:group_homomorphism_single_condition]{group homomorphism}. As per \fullref{def:unital_magma_kernel}, its kernel is the set
    \begin{equation*}
      \ker(\varphi) \coloneqq \set{ x \in G \given \varphi(x) = e_H }.
    \end{equation*}

    The pair \( (\ker(\varphi), \iota) \), where \( \iota: \ker(\varphi) \to G \) is the canonical embedding, is an \hyperref[eq:def:equalizers/equalizer]{equalizer} of \( \varphi \) and \( \tau \), i.e. a kernel in the sense of \fullref{def:zero_morphisms/kernel}.

    Indeed, fix a group homomorphism \( \psi: A \to G \) such that \( \varphi \bincirc \psi = \tau \bincirc \psi \). We want to find a map \( l_A: A \to \ker(\varphi) \) such that the following diagram commutes:
    \begin{equation}\label{eq:ex:zero_morphisms_in_grp/kernel}
      \begin{aligned}
        \includegraphics[page=1]{output/ex__zero_morphisms_in_grp.pdf}
      \end{aligned}
    \end{equation}

    For every \( x \in A \), we must have
    \begin{equation*}
      \iota(l_A(x)) = \psi(x),
    \end{equation*}
    which uniquely determines \( l_A \) as the restriction \( \psi\restr_{\ker(\varphi)} \).

    Therefore, the group-theoretic kernel of \( \varphi \) induces a category-theoretic kernel.

    \thmitem{ex:zero_morphisms_in_grp/cokernel} Denote by \( Q \) the \hyperref[def:quotient_group]{quotient group} \( H / \img(\varphi) \). The pair \( (Q, \pi) \), where \( \pi(y) = [y] \), is a cokernel cone of \( \varphi \).

    Indeed, fix a group homomorphism \( \psi: A \to G \) such that
    \begin{equation}\label{eq:ex:zero_morphisms_in_grp/cokernel/psi}
      \psi \bincirc \varphi = \psi \bincirc \tau
    \end{equation}

    We want to find a map \( l_A: Q \to A \) such that the following diagram commutes:
    \begin{equation}\label{eq:ex:zero_morphisms_in_grp/cokernel}
      \begin{aligned}
        \includegraphics[page=2]{output/ex__zero_morphisms_in_grp.pdf}
      \end{aligned}
    \end{equation}

    For every \( y \in H \), we must have
    \begin{equation*}
       l_A(\pi(y)) = \psi(y),
    \end{equation*}
    which uniquely defines \( l_A([y]) \coloneqq \psi(y) \). This is well-defined because, by \eqref{eq:ex:zero_morphisms_in_grp/cokernel/psi}, \( \psi \) is constant on cosets.

    \thmitem{ex:zero_morphisms_in_grp/image} Finally, the kernel of the cokernel is
    \begin{equation*}
      \ker(\pi) = \set{ y \in H \given \pi(y) = e_Q } = \set{ y \in H {\given} [y] = [e_H] } = \img(\varphi).
    \end{equation*}

    Therefore, the group-theoretic image of \( \varphi \) induces a category-theoretic image.
  \end{thmenum}
\end{example}
