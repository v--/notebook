...
\begin{example}\label{ex:zero_morphisms_in_grp}
  Consider the category \( \cat{Grp} \) of small \hyperref[def:group]{groups} and fix groups \( \mscrG \) and \( \mscrH \).
  ...
  \begin{thmenum}
    ...
    The trivial homomorphism from \( \mscrG \) to \( \mscrH \) is.
    \begin{equation*}
      \tau(x) \coloneqq e_\mscrH.
    \end{equation*}

    It \( \tau \) can be represented via the composition \eqref{eq:def:zero_morphisms/morphism}, hence this is the zero morphism from \( \mscrG \) to \( \mscrH \) in \( \cat{Grp} \).

    \thmitem{ex:zero_morphisms_in_grp/kernel} Let \( \varphi: \mscrG \to \mscrH \) be a \hyperref[def:group/homomorphism]{group homomorphism}. As per \fullref{def:pointed_set_kernel}, its kernel is the set
    \begin{equation*}
      \ker(\varphi) \coloneqq \set{ x \in \mscrG \given \varphi(x) = e_\mscrH }.
    \end{equation*}

    The pair \( (\ker(\varphi), \iota) \), where \( \iota: \ker(\varphi) \to \mscrG \) is the canonical inclusion, is an \hyperref[eq:def:equalizers/equalizer]{equalizer} of \( \varphi \) and \( \tau \), i.e. a kernel in the sense of \fullref{def:zero_morphisms/kernel}.

    Indeed, fix a group homomorphism \( \psi: A \to \mscrG \) such that \( \varphi \bincirc \psi = \tau \bincirc \psi \). We will show that \( \psi \) \hyperref[def:factors_through]{uniquely factors through} \( \ker(\varphi) \). We want to find a map \( l_A: A \to \ker(\varphi) \) such that the following diagram commutes:
    ...
    \thmitem{ex:zero_morphisms_in_grp/cokernel} Denote by \( Q \) the \hyperref[def:quotient_group]{quotient group} \( \mscrH / \img(\varphi) \). The pair \( (Q, \pi) \), where \( \pi(y) = [y] \), is a cokernel cone of \( \varphi \).

    Indeed, fix a group homomorphism \( \psi: A \to \mscrG \) such that
    ...
    For every \( y \in \mscrH \), we must have
    \begin{equation*}
       l_A(\pi(y)) = \psi(y),
    \end{equation*}
    ...
    \begin{equation*}
      \ker(\pi) = \set{ y \in \mscrH \given \pi(y) = e_Q } = \set{ y \in \mscrH {\given} [y] = [e_\mscrH] } = \img(\varphi).
    \end{equation*}
