...
\begin{definition}\label{def:equalizers}\mcite[def. 5.1.11]{Leinster2016Basic}
  ...
  Note that in \fullref{def:categorical_diagram} we defined commutativity only for diagrams with nontrivial paths. That is, \eqref{eq:def:equalizers/index} cannot commute by definition since all of its paths have length either \( 0 \) or \( 1 \).
