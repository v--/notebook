...
\begin{definition}\label{def:category_of_cones}
  ...
  \begin{thmenum}
    ...
    Explicitly, \( (L, \pi) \) is limit cone if, for every cone \( (A, \alpha) \), there exists a unique \hyperref[eq:def:comma_category/fixed/right]{cone morphism} \( l_A: (A, \alpha) \to (L, \pi) \).
    ...
    In particular, \( \alpha_k \) \hyperref[def:factors_through]{uniquely factors through} \( L \) for every \( k \in \cat{I} \), but \( l_A \) is compatible with morphisms in \( \cat{I} \) and does \( l_A \) depend on \( k \).
