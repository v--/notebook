...
\begin{example}\label{ex:equalizers_in_set}
  ...
  Let \( Y \) be the equivalence partition \( Y / {\sim} \). Then \( Y \) with the projection map
  \begin{equation*}
    \begin{aligned}
      &\pi: Y \to Y \\
      &\pi(y) \coloneqq [y]
  ...
  Indeed, fix any other cocone \( (A, f) \). We want to define a map \( l_A: Y \to A \) such that the defining diagram for a coequalizer
...
\begin{example}\label{ex:zero_morphisms_in_grp}
  ...
  \begin{thmenum}
    ...
    \thmitem{ex:zero_morphisms_in_grp/kernel} Let \( \varphi: G \to H \) be a \hyperref[def:group/homomorphism]{group homomorphism}. As per \fullref{def:pointed_set_kernel}, its kernel is the set
    \begin{equation*}
      \ker(\varphi) \coloneqq \set{ x \in G \given \varphi(x) = e_H }.
    \end{equation*}

    The pair \( (\ker(\varphi), \iota) \), where \( \iota: \ker(\varphi) \to G \) is the canonical inclusion, is an \hyperref[eq:def:equalizers/equalizer]{equalizer} of \( \varphi \) and \( \tau \), i.e. a kernel in the sense of \fullref{def:zero_morphisms/kernel}.
