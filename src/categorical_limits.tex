...
\begin{definition}\label{def:categorical_cocone}\mcite[def. 5.2.1]{Leinster2016Basic}
  ...
  \end{equation*}
  is a \term{cocone} for D if it is a cone for \( D^{-1}: \cat{I} \to A^{\opcat} \).
...
\begin{definition}\label{def:categorical_coequalizer}\mcite[def. 5.2.7]{Leinster2016Basic}
  As for \hyperref[def:categorical_coequalizer]{equalizers}, assume that the index category \( \cat{I} \cong \cat{I}^{\opcat} \) consists of two objects and two unidirectional morphisms:
  \begin{equation*}
    \text{\todo{Add diagram}}\iffalse\begin{mplibcode}
      beginfig(1);
      input metapost/graphs;
...
\begin{definition}\label{def:categorical_pushout}\mcite[def. 5.2.11]{Leinster2016Basic}
  A \term{pushout} in \( \cat{A} \) is a \term{pullback} in \( \cat{A}^{\opcat} \).
...
\begin{definition}\label{def:categorical_limit_preservation}\mcite[def. 5.3.1, 5.3.5]{Leinster2016Basic}
  Let \( F: \cat{A} \to \cat{B} \) be a functor. We say that
  \begin{thmenum}
    ...
    \thmitem{def:categorical_limit_preservation/lift} \( F \) \term{lifts} limits of shape \( \cat{I} \) if, given a diagram \( D: \cat{I} \to \cat{B} \), any limit cone \( \varprojlim D \) is the image of some limit cone in \( A \).
  \end{thmenum}
\end{definition}
