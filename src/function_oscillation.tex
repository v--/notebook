...
\begin{proposition}\label{thm:function_oscillation_properties}
  ...
  \begin{thmenum}
    \labitem{thm:function_oscillation_properties/continuity_condition} \( f \) is globally \hyperref[def:uniform_continuity]{uniformly continuous} if and only if for every \( \varepsilon > 0 \) there exists \( \delta > 0 \) such that \( \omega(f, \delta) < \varepsilon \).

    \labitem{thm:function_oscillation_properties/monotone} \( \omega(f, \delta) \) is monotone in \( \delta \).

    \labitem{thm:function_oscillation_properties/cauchy_inequality}\mcite\cite[28]{Николов2020Лекции}For all \( \lambda, \delta > 0 \), we have the following analog of \fullref{thm:cauchy_bunyakovsky_schwarz_inequality}
    ...
    \labitem{thm:function_oscillation_properties/single_inequality}\mcite\cite[28]{Николов2020Лекции}For all \( \lambda, \delta > 0 \),
