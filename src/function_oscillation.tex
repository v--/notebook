...
\begin{proposition}\label{thm:def:function_oscillation/properties}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:def:function_oscillation/cauchy_inequality}\mcite[28]{Николов2020Лекции}For all \( \lambda, \delta > 0 \), we have the following analog of \fullref{thm:cauchy_bunyakovsky_schwarz_inequality}
    ...
    \thmitem{thm:def:function_oscillation/single_inequality}\mcite[28]{Николов2020Лекции}For all \( \lambda, \delta > 0 \),
...
\end{proposition}
\begin{proof}
  \SubProofOf{thm:def:function_oscillation/continuity_condition} Follows directly from \fullref{def:uniform_continuity}.
  ...
  Combining the two inequalities with \fullref{thm:def:function_oscillation/monotone}, we obtain \fullref{thm:def:function_oscillation/cauchy_inequality/inequality}.
  ...
  Otherwise, \( \lambda < 1 \) and clearly \( \lambda \delta < \delta \), which by \fullref{thm:def:function_oscillation/monotone} implies
  \begin{equation*}
