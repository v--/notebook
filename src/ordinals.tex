...
\begin{theorem}[Structural induction]\label{thm:structural_induction}\mimprovised
  ...
  Let \( G = (V, E) \) be \hyperref[def:quiver/simple]{simple directed graph}. We will call \( v \) a \term{successor vertex} if it has at least one \hyperref[def:quiver/adjacency]{predecessor} and a \term{base vertex} otherwise. We will call the graph \( G \) itself \term{unambiguously well-founded} if for every successor vertex there exists a unique \hyperref[def:quiver_path/directed]{directed path} from a base vertex.
  ...
  Suppose we want to prove some statement for every vertex of \( G \). If the graph is unambiguously well-founded, we can proceed as follows:
  \begin{thmenum}
...
\begin{remark}\label{rem:structural_recursion_and_induction}\mimprovised
  ...
  If the \hyperref[def:quiver]{quiver} happens to be unambiguously well-founded in the sense of \fullref{thm:structural_induction}, we can use \fullref{thm:structural_induction} to prove universal statements about \( A_0 \).
