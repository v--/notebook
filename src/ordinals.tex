...
\begin{definition}\label{def:well_founded_relation}
  ...
  An \term{infinitely descending sequence} is a \hyperref[def:sequence]{sequence} \( \set{ x_k }_{k=1}^\infty \) such that \( x_{k+1} \prec x_k \) for all \( k \in 1, 2, \ldots \). That is,
  \begin{equation*}
...
\begin{proof}
  ...
  \SubProofOf{thm:ordinal_properties/descending_sequence_of_ordinals_is_finite} Suppose that there exists an infinite sequence of ordinals \( \set{ \alpha_k }_{k=1}^\infty \) such that \( \alpha_{k+1} \in \alpha_k \) for all \( k \in 1, 2, \ldots \). Thus \( \alpha_1 \) itself as a set has no minimal element with respect to \( \in \), which contradicts the definition of an ordinal.
