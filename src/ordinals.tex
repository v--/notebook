...
\begin{proof}
  ...
  We will use \fullref{thm:transfinite_induction} on \( \beta \) to show that at least one of them holds. Fix an ordinal \( \beta_0 \). Our inductive hypothesis is that for every ordinal \( \alpha \) and for every \( \gamma \in \beta_0 \) at least one of \( \alpha = \gamma \), \( \alpha \in \gamma \) or \( \gamma \in \alpha \) holds.
...
\begin{theorem}[Structural recursion]\label{thm:structural_recursion}\mimprovised
  The most general recursion principle we will consider is \term{structural recursion}. It is sometimes also called \term{mutual recursion}. See \fullref{thm:structural_induction} for its corresponding induction principle.
...
\begin{theorem}[Structural induction]\label{thm:structural_induction}\mimprovised
  ...
  Let \( G = (V, E) \) be \hyperref[def:quiver/simple]{simple directed graph}. We will call \( v \) a \term{successor vertex} if it has at least one \hyperref[def:quiver/adjacency]{predecessor} and a \term{base vertex} otherwise. We will call the graph \( G \) itself \term{tree-like well-founded} if for every successor vertex there exists a unique \hyperref[def:quiver_path/directed]{directed path} from a base vertex.

  \begin{figure}
    \begin{equation*}
      \begin{aligned}
        \includegraphics[page=1]{output/thm__structural_induction.pdf}
      \end{aligned}
    \end{equation*}
    \caption{A finite unambiguously well-ordered graph}\label{fig:thm:structural_induction}
  \end{figure}

  Suppose we want to prove some statement for every vertex of \( G \). If the graph is tree-like well-founded, we can proceed as follows:
  \begin{thmenum}
    \thmitem{thm:structural_induction/base_step} The \term{base step} is to prove the statement for all base vertices of \( A \).
    \thmitem{thm:structural_induction/inductive_step} The \term{inductive step} is to fix a successor vertex \( v \), suppose that the statement holds for all predecessors of \( v \), and then prove it for \( v \) itself.
  \end{thmenum}

  Compare this principle to the more directly applicable \fullref{thm:structural_induction_on_unambiguous_grammars}.
\end{theorem}
\begin{proof}
  We prove the object statement for all base vertices of \( G \) in \fullref{thm:structural_induction/base_step} and for all successor vertices in \fullref{thm:structural_induction/inductive_step}. Therefore, statement holds for all vertices of \( G \).

  Our additional condition is justified as follows:
  \begin{itemize}
    \item It ensures \hyperref[def:well_founded_relation]{well-foundedness}, i.e. that we can reach every vertex in a finite number of steps.
    \item If \( v \) can be reached via two paths, it is possible to prove the statement for \( v \) following the first and disprove it following the second.
  \end{itemize}
\end{proof}

\begin{remark}\label{rem:structural_recursion_and_induction}\mimprovised
  We will now show the connection between \fullref{thm:structural_induction} and \fullref{thm:structural_recursion}.
  ...
  If the \hyperref[def:quiver]{quiver} happens to be tree-like well-founded in the sense of \fullref{thm:structural_induction}, we can use \fullref{thm:structural_induction} to prove universal statements about \( A_0 \).
...
\begin{remark}\label{rem:induction}
  ...
  \begin{thmenum}
    ...
    \thmitem{rem:induction/structural} A very general induction principle is \fullref{thm:structural_induction} and the corresponding recursion principle is \fullref{thm:structural_recursion}. The connection between them is explained in \fullref{rem:structural_recursion_and_induction}.

    Within this document, structural induction is used mainly via \fullref{thm:structural_induction_on_unambiguous_grammars} for \hyperref[def:propositional_syntax/formula]{propositional} and \hyperref[def:first_order_syntax/formula]{first-order formulas}.
  \end{thmenum}
\end{remark}
