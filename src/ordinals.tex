...
\begin{definition}\label{def:transitive_set}
  ...
  \begin{thmenum}
    \thmitem{def:transitive_set/relation} The relation \( {}\in{} \) is \hyperref[def:binary_relation/transitive]{transitive} on \( A \).
    \thmitem{def:transitive_set/inclusion} If \( B \in A \), then \( B \subseteq A \).
  \end{thmenum}
\end{definition}
\begin{proof}
  \ImplicationSubProof{def:transitive_set/relation}{def:transitive_set/inclusion} Let \( B \in A \) and \( C \in B \). Then by the transitivity of \( {}\in{} \), \( C \in A \) and since \( C \) was arbitrary, we can conclude that \( B \subseteq A \).

  \ImplicationSubProof{def:transitive_set/inclusion}{def:transitive_set/relation} Let \( D \in C \in B \) for some members \( B, C, D \) of \( A \). We will show that \( D \in B \), i.e. that \( {}\in{} \) is a transitive relation on \( A \).
...
\begin{remark}\label{rem:ordinal_definition}
  Let us consider some properties of the natural numbers (defined as members the smallest inductive set) so that it is clear what we want to generalize.

  Every natural number is a set of other natural numbers:
  \begin{align*}
    &0 = \varnothing \\
  ...
  Therefore \( \in \) and \( < \) are equivalent on the set \( \omega \). It follows from \fullref{thm:natural_numbers_are_well_ordered} that \( \in \) is a well-order on \( \omega \).

  What is common between any natural number \( n \) and the entire set \( \omega \) of natural numbers?
\end{remark}

\begin{definition}\label{def:ordinal}\mcite[def. 63.12]{OpenLogicFull}
  A set is called an \term{ordinal} or an \term{ordinal number} if it is \hyperref[def:totally_ordered_set]{totally ordered} by set membership.
  ...
  We use \( \gamma < \delta \) to mean \( \gamma \in \delta \).
\end{definition}

\begin{example}\label{ex:def:ordinal}
  We list several small ordinals:
  \begin{align*}
    &0 = \varnothing \\
    &1 = \set{ \varnothing } \\
    &2 = \set{ \varnothing, \set{ \varnothing } } \\
    &3 = \set{ \varnothing, \set{ \varnothing }, \set{ \varnothing, \set{ \varnothing } } }
  \end{align*}

  We will later show that the natural numbers (defined as members the smallest inductive set) are precisely the finite ordinals.
\end{example}
...
\begin{proof}
  ...
  We will first show that \( \delta \) is transitive. Let \( A \in \delta \) and \( x \in A \). Then \( A \subseteq \gamma \) since \( \gamma \) is transitive, hence \( x \in \gamma \). Suppose that \( x \not\in \delta \).

  Since \( \gamma \) is well-ordered by \( \in \), then any subset of \( \gamma \) has a minimal element. Because of the transitivity of \( \gamma \), \( \delta \) and also any subset of \( \delta \) is also a subset of \( \gamma \) and hence has a minimum with respect to \( \in \). Hence \( \delta \) is well-ordered by \( \in \).
