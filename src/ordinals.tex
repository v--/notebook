...
\begin{corollary}[\( \varepsilon \)-induction]\label{thm:epsilon_induction}
  For every formula \( \varphi \) over the language of set theory not containing \( \eta \) nor \( \zeta \) as free variables, the following is a theorem of \logic{ZFC}:
...
\begin{theorem}[Transfinite induction]\label{thm:transfinite_induction}\mcite[thm. 63.15]{OpenLogicFull}
  ...
  For every formula \( \varphi \) over the language of set theory not containing \( \eta \) nor \( \zeta \) as free variables, the following is a theorem of \logic{ZFC}:
  \small
  \begin{equation}\label{eq:def:transfinite_induction}
...
\end{theorem}
\begin{proof}
  The proof is similar to the proof of \cref{thm:epsilon_induction}.
\end{proof}
