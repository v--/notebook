...
\begin{definition}\label{def:boolean_closure}
  ...
  The \Def{closure} \( \Cl{\mscrB} \) of \( \mscrB \) is defined inductively as follows:
  \begin{itemize}
  ...
  We say that \( \mscrB \) is \Def{closed} if \( \Cl{\mscrB} = \mscrB \) and \Def{complete} if \( \Cl{\mscrB} \) is the set of all Boolean functions of arbitrary arity.
\end{definition}

\begin{definition}\label{def:zhegalkin_polynomial}
  A \Def{Zhegalkin polynomial} is a \hyperref[def:polynomial]{polynomial} in the \hyperref[thm:galois_field_existence]{Galois field} \( \BbbF_2 \).
...
\begin{definition}\label{def:boolean_functions_in_f2}
  ...
  \begin{DefEnum}
    ...
    \ILabel{def:boolean_functions_in_f2/monotone} \( f \) is \Def{monotone} if, for any two tuples of arguments \( x_1, \ldots, x_n \in \BbbF_2^n \) and \( y_1, \ldots, y_n \in \BbbF_2^n \), the inequalities \( x_k \leq y_k \) for all \( k = 1, \ldots, n \) imply that
    \begin{equation*}
