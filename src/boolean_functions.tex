...
\begin{definition}\label{def:zhegalkin_polynomial}
  A \term{Zhegalkin polynomial} is a \hyperref[def:polynomial_semiring]{polynomial} in the prime field \hyperref[thm:f2_is_boolean_algebra]{\( \BbbF_2 \)}. Due to \fullref{thm:functions_over_prime_fields}, however, we restrict ourselves to polynomials with \hyperref[def:square_free]{square-free} monomials.
...
\begin{definition}\label{def:boolean_functions_in_f2}
  Fix a \hyperref[def:boolean_function]{Boolean function} \( f(x_1, \ldots, x_n) \) in the \hyperref[thm:prime_fields]{prime field} \hyperref[thm:f2_is_boolean_algebra]{\( \BbbF_2 \)}.
...
\begin{example}\label{ex:posts_completeness_theorem}
  ...
  \begin{thmenum}
    \thmitem{ex:posts_completeness_theorem/and_or} The archetypic example of a complete set of Boolean functions is the triple \( \vee, \wedge, \overline{\anon} \) that forms the Boolean algebra structure on \( \BbbF_2 \).
    ...
    \thmitem{ex:posts_completeness_theorem/conditional_negation} Another commonly used complete family is \( \set{ \rightarrow, \overline{\anon} } \).
    We verify that the conditions of \fullref{thm:posts_completeness_theorem} are satisfied:
    \begin{refenum}
      \refitem{thm:posts_completeness_theorem/truth_preserving} \( \overline{\anon} \) is not truth-preserving.
