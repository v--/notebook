...
\begin{definition}\label{def:zhegalkin_polynomial}
  A \term{Zhegalkin polynomial} is a \hyperref[def:polynomial_semiring]{polynomial} in the finite field \hyperref[thm:f2_is_boolean_algebra]{\( \BbbZ_2 \)}.
...
\begin{definition}\label{def:boolean_functions_in_f2}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:boolean_functions_in_f2/monotone} \( f \) is \term{monotone} if, for any two tuples of arguments \( x_1, \ldots, x_n \in \BbbZ_2 \) and \( y_1, \ldots, y_n \in \BbbZ_2 \), the inequalities \( x_k \leq y_k \) for all \( k \in \set{ 1, \ldots, n } \) imply that
    \begin{equation*}
...
\begin{example}\label{ex:posts_completeness_theorem}
  ...
  \begin{thmenum}
    \thmitem{ex:posts_completeness_theorem/and_or} The archetypic example of a complete set of Boolean functions is the triple \( \vee, \wedge, \overline{\anon} \) that forms the Boolean algebra structure on \( \BbbZ_2 \).
