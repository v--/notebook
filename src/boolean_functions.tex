...
\begin{definition}\label{def:boolean_functions_in_f2}
  Fix a Boolean function\Tinyref{def:boolean_function} \( f(x_1, \ldots, x_n) \) in the Galois field\Tinyref{thm:galois_field_existence} \( \F_2 \),
  ...
  \begin{defenum}
    ...
    \DItem{def:boolean_functions_in_f2/monotone} We call \( f \) \Def{monotone} if, for any two tuples of arguments \( x_1, \ldots, x_n \in \F_2^n \) and \( y_1, \ldots, y_n \in \F_2^n \), the inequalities \( x_i \leq y_i \) for all \( i = 1, \ldots, n \) imply that
    \begin{equation*}
...
\begin{definition}\label{def:zhegalkin_polynomial}
  A \Def{Zhegalkin polynomial} is a polynomial\Tinyref{def:polynomial} in the Galois field\Tinyref{thm:galois_field_existence} \( \F_2 \). These polynomials obviously induce Boolean functions\Tinyref{def:boolean_function}.
\end{definition}
...
\begin{theorem}[Post's completeness theorem]\label{thm:posts_completeness_theorem}\cite{Pelletier1990}
  Denote by \( \Cal{B} \) the any set of Boolean functions\Tinyref{def:boolean_function} on \( \F_2 \).
...
\begin{example}\label{ex:posts_completeness_theorem}
  ...
  \begin{exenum}
    ...
    \begin{description}
      ...
      \RItem{thm:posts_completeness_theorem/self_dual} \( \neg \) is self-dual
