...
\begin{definition}\label{def:boolean_function}
  Fix any set \( S \) and a two-element set \( B = \{ T, F \} \). A function \( f: S \to B \) is called \Def{Boolean-valued} and a function \( f: B^n \to B \) is called \Def{Boolean}.

  The concrete set \( B \) does not matter much. We interpret \( T \) as a truthy value and \( F \) as a falsy value. See \fullref{remark:metalanguage}.

  Without further context, we can assume that \( B \) the \hyperref[thm:galois_field_existence]{Galois field} \( \BF_2 \) or any of the other equivalent Boolean algebras of two elements (see \fullref{thm:binary_boolean_algebras_are_isomorphic}). Unless noted otherwise, we identify \( 0 \) with \( F \) and \( 1 \) with \( T \).
\end{definition}

\begin{remark}\label{def:boolean_functions_form_boolean_algebra}
  By \fullref{thm:functions_into_boolean_algebra_form_boolean_algebra}, the set of all \hyperref[def:boolean_function]{Boolean-valued functions} over any set naturally have a \hyperref[def:boolean_algebra]{Boolean algebra} structure themselves.
\end{remark}
...
\begin{definition}\label{def:boolean_operators}
...
\begin{proposition}\label{thm:boolean_function_equivalences}
  ...
  \begin{PropEnum}
    \ILabel{def:boolean_equivalences/contrapositive} A \hyperref[def:material_implication]{material implication} is equivalent to its \hyperref[def:material_implication/contrapositive]{contrapositive}:
    \begin{equation}\label{eq:def:boolean_equivalences/contrapositive}
      x \rightarrow y = \neg y \rightarrow \neg x.
    \end{equation}

    \ILabel{def:boolean_equivalences/equivalence_via_implication} \hyperref[def:propositional_alphabet/connectives/equivalence]{Equivalence} is a \hyperref[def:propositional_alphabet/connectives/conjunction]{conjunction} of \hyperref[def:material_implication]{implications}:
...
\end{proposition}
\begin{proof}
  The proofs follow directly from the tables in \fullref{def:boolean_operators}.
\end{proof}
