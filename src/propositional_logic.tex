...
\begin{definition}\label{def:propositional_logic_language}\cite[12]{Nerode2012}
  ...
  \begin{description}
    \DItem{propositional variables}{def:propositional_logic_language/prop} A nonempty set \( \Bold{Prop} \) of \textbf{propositional variables}.
    ...
    \DItem{propositional connectives}{def:propositional_logic_language/connectives} The set \( \Sigma \) of \textbf{propositional connectives}, namely
    \begin{description}
...
\begin{definition}\label{def:propositional_interpretation}
  ...
  We say that
  \begin{defenum}
    \item\label{def:propositional_interpretation/model} \( I \) is a \textbf{Boolean model} of \( \varphi \) and write \( I \models_B \varphi \) if \( \varphi[I] = \top \).
    \item\label{def:propositional_interpretation/tautology} If all interpretations are models for \( \varphi \), we say that \( \varphi \) is a \textbf{tautology}.
    \item\label{def:propositional_interpretation/contradiction} If no interpretation is a model for \( \varphi \), we say that \( \varphi \) is a \textbf{contradiction}.
    \item\label{def:propositional_interpretation/equivalence} \( \varphi \) and \( \psi \) are \textbf{Boolean equivalent} and write \( \varphi \equiv_B \psi \) if \( \varphi[I] = \psi[I] \) for any interpretation \( I \).
  \end{defenum}
\end{definition}
