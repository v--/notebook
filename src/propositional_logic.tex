...
\begin{definition}\label{def:propositional_language}\mcite[102]{OpenLogic20201202}
  ...
  \begin{thmenum}
    \labitem{def:propositional_language/prop} A nonempty at most \hyperref[rem:cardinals/countable]{countable} set \( \boldop{Prop} \) of \term{propositional variables}.

    \labitem{def:propositional_language/constants} Two \term{propositional constants} (also known as \term{truth values}):
    \begin{thmenum}
      \labitem{def:propositional_language/constants/top} \term{top} \( \top \)
      \labitem{def:propositional_language/constants/bottom} \term{bottom} \( \bot \) (also known as \term{falsum})
    \end{thmenum}

    \labitem{def:propositional_language/negation} \term{Negation} \( \neg \)
    \labitem{def:propositional_language/connectives} The set \( \Sigma \) of \term{propositional connectives}, namely
    \begin{thmenum}
      \labitem{def:propositional_language/connectives/conjunction} \term{conjunction} \( \wedge \) (also known as \hyperref[def:standard_boolean_operators]{\term{and}} or \hyperref[def:lattice_operations]{\term{meet}})
      \labitem{def:propositional_language/connectives/disjunction} \term{disjunction} \( \vee \) (also known as \hyperref[def:standard_boolean_operators]{\term{or}} or \hyperref[def:lattice_operations]{\term{join}})
      \labitem{def:propositional_language/connectives/implication} \term{(material) implication} \( \to \) (see \fullref{def:material_implication})
      \labitem{def:propositional_language/connectives/equivalence} \term{(material) equivalence} \( \leftrightarrow \)
    \end{thmenum}

    \labitem{def:propositional_language/parentheses} Parentheses \( ( \) and \( ) \) for defining the order of operations unambiguously (see \fullref{rem:propositional_formula_parentheses}).
  \end{thmenum}
...
\begin{definition}\label{def:propositional_syntax}
  ...
  \begin{thmenum}
    \labitem{def:propositional_syntax/grammar} Consider the following \hyperref[def:backus_naur_form]{grammar schema}:
    \begin{bnf*}
    ...
    Note that \( \boldop{Prop} \) may be infinite, in which case the grammars may have infinitely many rules. If needed, we can circumvent this by introducing an appropriate naming convention for variables, analogously to \fullref{ex:natural_arithmetic_grammar/backus_naur_form}.

    We will be using the conventions in \fullref{rem:propositional_formula_parentheses}.

    \labitem{def:propositional_syntax/formula} The set of \term{propositional formulas} is the language \hyperref[def:grammar_derivation/grammar_language]{generated} by this grammar schema with \( \bnfpn{formula} \) as a starting rule.

    The grammar of propositional formulas is unambiguous by \fullref{thm:propositional_formulas_are_unambiguous}.

    \labitem{def:propositional_syntax/subformula} If \( \varphi \) and \( \psi \) are formulas and \( \psi \) is a \hyperref[def:language/subword]{subword} of \( \varphi \), we say that \( \psi \) is a \term{subformula} of \( \varphi \).

    \labitem{def:propositional_syntax/variables} For each formula \( \varphi \), we inductively\IND define its \term{variables} to be elements of the set
...
\begin{remark}\label{rem:propositional_formula_parentheses}
  ...
  \begin{thmenum}
    \labitem{rem:propositional_formula_parentheses/outermost} We may skip the outermost parentheses in formulas with top-level \hyperref[def:propositional_language/connectives]{connectives}, e.g. we may write \( P \wedge Q \) rather than \( (P \wedge Q) \).

    \labitem{rem:propositional_formula_parentheses/associative} Because of the associativity of \( \wedge \) and \( \vee \) (see \fullref{def:propositional_formula_induced_function} and \fullref{def:standard_boolean_operators}), we may skip the parentheses in chains like
    \begin{equation*}
...
\begin{definition}\label{def:material_implication}
  ...
  \begin{thmenum}
    \labitem{def:material_implication/sufficient_condition} \( P \) is a \term{sufficient condition} for \( Q \).

    \labitem{def:material_implication/necessary_condition} \( Q \) is a \term{necessary condition} for \( P \).

    \labitem{def:material_implication/antecedent} \( P \) the \term{antecedent} of \( \varphi \).

    \labitem{def:material_implication/consequent} \( Q \) the \term{consequent} of \( \varphi \).

    \labitem{def:material_implication/inverse} The formula \( \neg P \to \neg Q \) is the \term{inverse} of \( \varphi \).

    \labitem{def:material_implication/converse} The formula \( Q \to P \) is the \term{converse} of \( \varphi \).

    \labitem{def:material_implication/contrapositive} The formula \( \neg Q \to \neg P \) is the \term{contrapositive} of \( \varphi \). It is \hyperref[def:propositional_semantics/equivalence]{equivalent} to the original formula by \fullref{thm:boolean_equivalences/contrapositive}.
  \end{thmenum}
\end{definition}
...
\begin{definition}\label{def:propositional_valuation}
  \hfill
  \begin{thmenum}
    \labitem{def:propositional_valuation/interpretation} A \term{propositional interpretation} is a function \( I: \boldop{Prop} \to \set{ T, F } \). See \fullref{def:boolean_function} and \fullref{rem:metalanguage_syntax_and_semantics} for remarks regarding the \hyperref[def:boolean_algebra]{Boolean algebra} \( \set{ T, F } \) and the \hyperref[def:standard_boolean_operators]{standard Boolean operators}.

    \labitem{def:propositional_valuation/formula_valuation} Given an interpretation \( I \), we define the \term{valuation} of a formula \( \varphi \) inductively\IND as
...
\begin{definition}\label{def:propositional_semantics}\mcite[def. 7.14]{OpenLogic20201202}
  ...
  \begin{thmenum}
    \labitem{def:propositional_semantics/satisfiability} Given an interpretation \( I \) and a set \( \Gamma \) of formulas, we say that \( I \) \term{satisfies} \( \Gamma \) if, for every formula \( \gamma \in \Gamma \) we have \( \gamma\Bracks{I} = T \).
    ...
    \labitem{def:propositional_semantics/entailment} We say that the set of formulas \( \Gamma \) \term{entails} the set of formulas \( \Delta \) and write \( \Gamma \vDash \Delta \) if either of the following hold:
    \begin{itemize}
      \labitem{def:propositional_semantics/entailment/direct} Every model of \( \Gamma \) is also a model of \( \Delta \).
      \labitem{def:propositional_semantics/entailment/functional} The following \hyperref[def:function/preimage]{preimage} inclusion holds:
      \begin{equation*}
        \bigcap_{\gamma \in \Gamma} \fun_\gamma^{-1}(T) \subseteq \bigcap_{\delta \in \Delta} \fun_\delta^{-1}(T).
      \end{equation*}
    \end{itemize}

    \labitem{def:propositional_semantics/tautology} The formula \( \varphi \) is a \term{tautology} if either:
    \begin{itemize}
      \labitem{def:propositional_semantics/tautology/direct} Every interpretation satisfies \( \varphi \).
      \labitem{def:propositional_semantics/tautology/entailment} The empty set \( \Gamma = \varnothing \) of formulas entails \( \varphi \), i.e. \( \vDash \varphi \).
      \labitem{def:propositional_semantics/tautology/functional} The function \( \fun_\varphi \) is canonically true.
    \end{itemize}

    We also say that \( \varphi \) is \term{valid}.

    \labitem{def:propositional_semantics/contradiction} Dually, \( \varphi \) is a \term{contradiction} if either:
    \begin{itemize}
      \labitem{def:propositional_semantics/contradiction/direct} No interpretation satisfies \( \varphi \).
      \labitem{def:propositional_semantics/contradiction/entailment} The formula \( \varphi \) entails \( \Gamma = \varnothing \), i.e. \( \varphi \vDash \).
      \labitem{def:propositional_semantics/contradiction/functional} The function \( \fun_\varphi \) is canonically false.
    \end{itemize}

    \labitem{def:propositional_semantics/equivalence} We say that \( \varphi \) and \( \psi \) are \term{semantically equivalent} and write \( \varphi \gleichstark \psi \) if either:
    \begin{itemize}
      \labitem{def:propositional_semantics/equivalence/direct} We have \( \varphi\Bracks{I} = \psi\Bracks{I} \) for every interpretation \( I \).
      \labitem{def:propositional_semantics/equivalence/entailment} Both \( \varphi \vDash \psi \) and \( \psi \vDash \varphi \).
      \labitem{def:propositional_semantics/equivalence/functional} We have \( \fun_\varphi = \fun_\psi \) for the induced functions.
    \end{itemize}
  \end{thmenum}
\end{definition}
...
\begin{theorem}\label{thm:propositional_formula_cosets_are_boolean_functions}
  ...
  \begin{thmenum}
    \labitem{thm:propositional_formula_cosets_are_boolean_functions/equivalence} The \hyperref[def:propositional_semantics/equivalence]{semantic equivalence} \( \gleichstark \) is an equivalence relation on the set of all propositional formulas.

    \labitem{thm:propositional_formula_cosets_are_boolean_functions/bijection} The \hyperref[def:equivalence_relation/quotient]{quotient set} of all propositional formulas by \( \gleichstark \) is bijective with the set of all \hyperref[def:boolean_function]{Boolean functions} of arbitrary arity.

    \labitem{thm:propositional_formula_cosets_are_boolean_functions/boolean_algebra} The quotient set of all propositional formulas by \( \gleichstark \) forms a \hyperref[def:boolean_algebra]{Boolean algebra} with the following:
    \begin{reflist}
      \refitem{def:binary_lattice_operations/join} Finite joins are given by \hyperref[def:propositional_language/connectives/disjunction]{disjunctions \( \vee \)}.
      \refitem{def:binary_lattice_operations/meet} Finite meets are given by \hyperref[def:propositional_language/connectives/conjunction]{conjunctions \( \wedge \)}.
      \refitem{def:lattice/top} The top is the equivalence class \( [\top] \) of \hyperref[def:propositional_semantics/tautology]{tautologies}.
      \refitem{def:lattice/bottom} The bottom is the equivalence class \( [\bot] \) of \hyperref[def:propositional_semantics/contradiction]{contradictions}.
      \refitem{def:boolean_algebra} The complement of the coset \( [\varphi] \) is the \hyperref[def:propositional_language/negation]{negation} coset \( [\neg \varphi] \).
    \end{reflist}
  \end{thmenum}
\end{theorem}
\begin{proof}
...
\begin{definition}\label{def:propositional_substitution}
  ...
  \begin{thmenum}
    \labitem{def:propositional_substitution/single} We define the \term{substitution} of the propositional formula \( \psi \) with \( \rho \) in \( \varphi \) as
    ...
    \labitem{def:propositional_substitution/simultaneous} We will now define \term{simultaneous substitution} of \( \theta_1, \ldots, \theta_n \) with \( \rho_1, \ldots, \rho_n \). We wish to avoid the case where \( \theta_k \) is a subformula of \( \rho_{k-1} \) and it accidentally gets replaced during \( \varphi[\theta_{k-1} \mapsto \rho_{k-1}][\theta_k \mapsto \rho_k] \).
...
\begin{definition}\label{def:conjunctive_disjunctive_normal_form}
  \hfill
  \begin{thmenum}
    \labitem{def:conjunctive_disjunctive_normal_form/literal} A \term{literal} is either a propositional variable \( L = P \) or a negation \( L = \neg P \) of a propositional variable.

    \labitem{def:conjunctive_disjunctive_normal_form/normal_form} A propositional formula \( \varphi \) is in \term{conjunctive normal form} (resp. \term{disjunctive normal form}) if \( \varphi \) is a finite conjunction of disjunctions (resp. finite disjunction of conjunctions) of literals. That is, if \( \varphi \) is in conjunctive normal form, it has the form
    \begin{equation*}
      (L_{1,1} \vee \ldots \vee L_{1,n_1}) \wedge \cdots \wedge (L_{k,1} \vee \ldots \vee L_{k,n_k}).
    \end{equation*}

    \labitem{def:conjunctive_disjunctive_normal_form/conjunct_disjunct} A \term{disjunct} (resp. a \term{conjunct}) is a set of literals, the difference between the two being the context in which they are used. To each formula in conjunctive normal form there corresponds a set of disjuncts and to each formula in disjunctive normal form there corresponds a set of conjuncts.
  \end{thmenum}
\end{definition}
...
\begin{algorithm}\label{alg:conjunctive_normal_form_reduction}
  ...
  \begin{thmenum}
    \labitem{alg:conjunctive_normal_form_reduction/constants} Pick any variable \( P \) and substitute
    \begin{align*}
      \top \T{with} P \vee \neg P, && \bot \T{with} P \wedge \neg P
    \end{align*}
    to get rid of the \hyperref[def:propositional_language/constants]{propositional constants}.

    \labitem{alg:conjunctive_normal_form_reduction/iff} For any subformulas \( \psi \) and \( \theta \) of \( \varphi \), perform the substitution
    \begin{equation*}
    ...
    \labitem{alg:conjunctive_normal_form_reduction/implies} For any subformulas \( \psi \) and \( \theta \) of \( \varphi \), perform the substitution
    \begin{equation*}
      \psi \to \theta \T{with} \neg \psi \vee \theta.
    \end{equation*}
    ...
    \labitem{alg:conjunctive_normal_form_reduction/de_morgan} For any subformulas \( \psi \) and \( \theta \) of \( \varphi \), use \fullref{thm:de_morgans_laws} to justify the substitution
    \begin{align*}
    ...
    \labitem{alg:conjunctive_normal_form_reduction/double_negation} For any variable \( P \) of \( \varphi \), use \fullref{thm:boolean_algebra_properties/double_complement} to justify the substitution
    \begin{equation*}
      \neg \neg P \T{with} P.
    \end{equation*}

    \labitem{alg:conjunctive_normal_form_reduction/distributivity} Finally, for any subformulas \( \psi \), \( \theta \) and \( \rho \) of \( \varphi \), use \hyperref[eq:def:distributive_lattice/distributivity]{distributivity} of \( \wedge \) over \( \vee \) to justify the substitution
    \begin{equation*}
