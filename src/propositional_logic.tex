...
\begin{definition}\label{def:propositional_language}\mcite[102]{OpenLogic20201202}
  ...
  \begin{thmenum}
    \thmitem{def:propositional_language/prop} A nonempty at most \hyperref[rem:cardinals/countable]{countable} set \( \boldop{Prop} \) of \term{propositional variables}.
    ...
    \begin{thmenum}
      \thmitem{def:propositional_language/connectives/conjunction} \term{conjunction} \( \wedge \) (also known as \hyperref[def:standard_boolean_operators]{\term{and}} or \hyperref[def:lattice_operations]{\term{meet}})
      \thmitem{def:propositional_language/connectives/disjunction} \term{disjunction} \( \vee \) (also known as \hyperref[def:standard_boolean_operators]{\term{or}} or \hyperref[def:lattice_operations]{\term{join}})
      \thmitem{def:propositional_language/connectives/implication} \term{(material) implication} \( \to \) (see \fullref{def:material_implication})
      \thmitem{def:propositional_language/connectives/equivalence} \term{(material) equivalence} \( \leftrightarrow \)
    \end{thmenum}
...
\begin{definition}\label{def:propositional_syntax}
  ...
  \begin{thmenum}
    \thmitem{def:propositional_syntax/grammar} Consider the following \hyperref[def:backus_naur_form]{grammar schema}:
    \begin{bnf*}
    ...
    \thmitem{def:propositional_syntax/formula} The set of \term{propositional formulas} is the language \hyperref[def:grammar_derivation/grammar_language]{generated} by this grammar schema with \( \bnfpn{formula} \) as a starting rule.
...
\begin{definition}\label{def:material_implication}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:material_implication/converse} The formula \( Q \to P \) is the \term{converse} of \( \varphi \).
...
\begin{definition}\label{def:propositional_semantics}\mcite[def. 7.14]{OpenLogic20201202}
  ...
  \begin{thmenum}
    \thmitem{def:propositional_semantics/satisfiability} Given an interpretation \( I \) and a set \( \Gamma \) of formulas, we say that \( I \) \term{satisfies} \( \Gamma \) if, for every formula \( \gamma \in \Gamma \) we have \( \gamma\Bracks{I} = T \).
    ...
    \thmitem{def:propositional_semantics/tautology} The formula \( \varphi \) is a \term{tautology} if either:
    \begin{itemize}
      \thmitem{def:propositional_semantics/tautology/direct} Every interpretation satisfies \( \varphi \).
    ...
    \thmitem{def:propositional_semantics/contradiction} Dually, \( \varphi \) is a \term{contradiction} if either:
    \begin{itemize}
      \thmitem{def:propositional_semantics/contradiction/direct} No interpretation satisfies \( \varphi \).
      \thmitem{def:propositional_semantics/contradiction/entailment} The formula \( \varphi \) entails \( \Gamma = \varnothing \), i.e. \( \varphi \vDash \).
    ...
    \begin{itemize}
      \thmitem{def:propositional_semantics/equivalence/direct} We have \( \varphi\Bracks{I} = \psi\Bracks{I} \) for every interpretation \( I \).
...
\begin{theorem}\label{thm:propositional_formula_cosets_are_boolean_functions}
  ...
  \begin{thmenum}
    \thmitem{thm:propositional_formula_cosets_are_boolean_functions/equivalence} The \hyperref[def:propositional_semantics/equivalence]{semantic equivalence} \( \gleichstark \) is an equivalence relation on the set of all propositional formulas.

    \thmitem{thm:propositional_formula_cosets_are_boolean_functions/bijection} The \hyperref[def:equivalence_relation/quotient]{quotient set} of all propositional formulas by \( \gleichstark \) is bijective with the set of all \hyperref[def:boolean_function]{Boolean functions} of arbitrary arity.

    \thmitem{thm:propositional_formula_cosets_are_boolean_functions/boolean_algebra} The quotient set of all propositional formulas by \( \gleichstark \) forms a \hyperref[def:boolean_algebra]{Boolean algebra} with the following:
    \begin{refenum}
...
\begin{definition}\label{def:propositional_substitution}
  ...
  \begin{thmenum}
    \thmitem{def:propositional_substitution/single} We define the \term{substitution} of the propositional formula \( \psi \) with \( \rho \) in \( \varphi \) as
...
\begin{algorithm}\label{alg:conjunctive_normal_form_reduction}
  ...
  \begin{thmenum}
    ...
    \thmitem{alg:conjunctive_normal_form_reduction/double_negation} For any variable \( P \) of \( \varphi \), use \fullref{thm:boolean_algebra_properties/double_complement} to justify the substitution
    \begin{equation*}
      \neg \neg P \T{with} P.
    \end{equation*}
