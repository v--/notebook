...
\begin{definition}\label{def:propositional_logic_language}\cite[12]{Nerode2012}
  ...
  \begin{description}
    \DItem{def:propositional_logic_language/prop}[propositional variables] A nonempty set \( \Bold{Prop} \) of \Def{propositional variables}.
    ...
    \DItem{def:propositional_logic_language/parentheses}[parentheses] Parentheses \( ( \) and \( ) \) for defining the order of operations unambiguously (see \cref{remark:propositional_formula_parentheses}).
  \end{description}
...
\begin{definition}\label{def:propositional_substition}
  If \( \psi \) is a subformula of \( \varphi \) and \( \rho \) is any formula, we define the \Def{substition of \( \psi \) with \( \rho \) in \( \varphi \)} as
  \begin{align*}
...
\begin{remark}\label{remark:propositional_formula_parentheses}
  We often omit parentheses from propositional formulas when no conceptual ambiguity is possible, for example we often write \( \varphi \land \psi \land \theta \) instead of \( ((\varphi \land \psi) \land \theta) \). These are only notations shortcuts in the metalanguage\Tinyref{remark:metalanguage} and the formulas themselves (as abstract mathematical objects) are still assumed to contain parentheses to avoid syntactic ambiguity (see \cref{thm:propositional_formulas_are_unambiguous}).
\end{remark}
...
\begin{definition}\label{def:propositional_theory}
  A set of formulas is often called a \Def{propositional theory}. It is sometimes convenient, however, to have additional implicit \enquote{axioms} that are automatically part of any theory. Hence we will call a \Def{theory} the union of some set of formulas and a predefined set of axioms.
...
\begin{definition}\label{def:conjunctive_normal_form}
  ...
  We say that a propositional formula \( \varphi \) is in \Def{conjunctive (resp. disjunctive) normal form (CNF)} if \( \varphi \) is a finite conjunction of disjunctions (resp. finite disjunction of conjunctions).
\end{definition}
...
\begin{definition}\label{def:propositional_interpretation}
  ...
  We say that
  \begin{defenum}
    \DItem{def:propositional_interpretation/model} \( I \) is a \Def{Boolean model} of \( \varphi \) and write \( I \models_B \varphi \) if \( \varphi[I] = \top \).
    \DItem{def:propositional_interpretation/tautology} If all interpretations are models for \( \varphi \), we say that \( \varphi \) is a \Def{tautology}.
    \DItem{def:propositional_interpretation/contradiction} If no interpretation is a model for \( \varphi \), we say that \( \varphi \) is a \Def{contradiction}.
