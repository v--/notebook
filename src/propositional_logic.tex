...
\begin{definition}\label{def:propositional_semantics}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:propositional_semantics/theory_of_model}\mcite[def. 25.11]{OpenLogicFull} Fix an interpretation \( I \). The \term{theory} of the model \( I \) is the set
    \begin{equation*}
      \op{th}(I) \coloneqq \set{ \varphi \in \boldop{Form} \given I \vDash \varphi }.
    \end{equation*}
    ...
    \thmitem{def:propositional_semantics/theory}\mcite[def. 17.1 \\ def. 33.1]{OpenLogicFull} The \term{semantic closure} of the set \( \Gamma \) of formulas is the set
    \begin{equation*}
      \cl_\vDash(\Gamma) \coloneqq \set{ \varphi \in \boldop{Form} \given \Gamma \vDash \varphi }.
    \end{equation*}

    The set \( \Gamma \) is called \term{closed} if it equals its own closure. Closed sets of formulas are also called theories.
...
\begin{theorem}\label{thm:propositional_formula_cosets_are_boolean_functions}
  ...
  \begin{thmenum}
    ...
    If \( [\varphi] \leq [\psi] \) in this Boolean algebra, then \( \varphi \vDash \psi \).
  \end{thmenum}
\end{theorem}
\begin{proof}
  ...
  \SubProofOf{thm:propositional_formula_cosets_are_boolean_functions/boolean_algebra} Follows from \fullref{thm:propositional_formula_cosets_are_boolean_functions/bijection} and \fullref{thm:functions_over_model_of_positive_formulas_form_model}.
\end{proof}
