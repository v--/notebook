...
\begin{definition}\label{def:material_implication}
  ...
  \begin{defenum}
    ...
    \DItem{def:material_implication/contrapositive} We call the formula \( \neg Q \implies \neg P \) the \Def{contrapositive} of \( \varphi \). It is Boolean equivalent\Tinyref{def:propositional_interpretation} to the original formula by \cref{thm:boolean_equivalences}.
  \end{defenum}
\end{definition}
...
\begin{remark}\label{remark:statements_as_implications}
  Theorems in mathematics usually take the form of a material implication\Tinyref{def:material_implication} \( P \implies Q \) or even equivalence\Tinyref{def:equivalence} \( P \iff Q \). Therefore the terminology of \cref{def:material_implication} and \cref{def:equivalence} usually applies to statements about mathematics.
\end{remark}
