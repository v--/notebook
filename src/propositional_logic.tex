...
\begin{proof}
  ...
  \begin{RefList}
    \IRef{def:algebraic_theory/associativity} The \hyperref[def:truth_functions]{functions} \( H_\vee \) and \( H_\wedge \) are associative, hence the lattice operations are associative.
    \IRef{def:algebraic_theory/commutativity} The proof is analogous to \fullref{def:algebraic_theory/associativity}.
    \IRef{def:binary_lattice_operations/absorption} Let \( \varphi \) and \( \psi \) be propositional formulas and \( I \) be a propositional interpretation. Then
    \begin{equation*}
    ...
    \IRef{def:distributive_lattice} Let \( \varphi \), \( \psi \) and \( \theta \) be propositional formulas and \( I \) be a propositional interpretation. Then
    \begin{equation*}
  ...
  \begin{RefList}
    \IRef{def:lattice/top} Fix an interpretation \( I \). A formula \( \omega \) should belong to the supremum \( \sup \CF_B \) if and only if \( \varphi \vee \omega \equiv \omega \) for any formula \( \varphi \in \CF_B \). If \( \varphi \) is a tautology, \( \varphi[I] = \top \) and thus
    \begin{equation*}
    ...
    \IRef{def:lattice/bottom} The proof is analogous to \fullref{def:lattice/top}.
  \end{RefList}
\end{proof}
