...
\begin{remark}\label{remark:minimal_propositional_language}
  ...
  We are able to define the constants, negation and all other connectives so that the truth functions in \cref{def:truth_functions} are satisfied. This can simplify the study of propositional logic itself.
 ...
 or \cref{def:algebraic_theory}
  ...
  \begin{description}
    ...
    \RItem{def:propositional_logic_language/conjunction} \( (\varphi \land \psi) \iff ((\varphi \downarrow \varphi) \downarrow (\, in which case we consider them to be part of the language.si \downarrow \psi)) \)
...
\begin{remark}\label{remark:propositional_axioms}
  Entailment\Tinyref{def:propositional_interpretation/entailment} allows us to talk about \Def{axioms}. If \( \Gamma \) is a set of formulas and \( \varphi \) is a formula entailed by \( \Gamma \), we also say that the \Def{theorem} \( \varphi \) follows from the \Def{axioms} \( \Gamma \).

  Axioms are sometimes assumed to implicitly be included in every set \( \Gamma \) of formulas, e.g. in \cref{remark:minimal_propositional_language} or \cref{remark:first_order_equality} or \cref{def:algebraic_theory}, in which case we consider them to be part of the language.
\end{remark}
