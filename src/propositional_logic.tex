...
\begin{definition}\label{def:propositional_language}\mcite[sec. 7.2]{OpenLogicFull}
  ...
  \begin{thmenum}
    \thmitem{def:propositional_language/prop} A nonempty, \hyperref[def:countability/at_most_countable]{at most countable} set \( \boldop{Prop} \) of \term{propositional variables}. Technically, we can have have different languages with different variables but it is safe to assume that there is only one single language of propositional language.
