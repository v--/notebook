...
\begin{definition}\label{def:short_exact_sequence}\cite{MacLane1994}[196]
  In an abelian category\Tinyref{def:abelian_category} \( \Cat{C} \), the tower diagram\Tinyref{def:tower_diagram}
...
\begin{example}\label{ex:short_exact_sequences}
  ...
  \begin{defenum}
    ...
    \begin{itemize}
      ...
      \item \( p(x) \coloneqq [x]_n \) projects any integer into the corresponding remainder when divided by \( n \) (see \cref{def:group_of_integers_modulo}).
    \end{itemize}
    ...
    \begin{itemize}
      ...
      \item \( g(x) \coloneqq (\cos(\Frac(x)), \sin(\Frac(x)))^T \) (see \cref{def:floor_ceiling_functions} and \cref{def:quadratic_plane_curve/ellipse/parametric_equations}).
    \end{itemize}
