...
\begin{definition}\label{def:euclidean_plane_auxiliary_definitions}
  ...
  \begin{DefEnum}
    ...
    \ILabel{def:affine_plane/bound_vector} An ordered pair \( \overrightarrow{PQ} \) of points is called a \Def{bound vector}. The point \( P \) is called the \Def{beginning} of \( \overrightarrow{PQ} \) and \( Q \) is called the \Def{end} of \( \overrightarrow{PQ} \).
...
\begin{definition}\label{def:euclidean_plane_free_vector}
  We say that the bound vectors \( \overrightarrow{P_1 Q_1} \) and \( \overrightarrow{P_2 Q_2} \) in \( A_2 \) are \Def{congruent} if \( \Dist(P_1, Q_1) = \Dist(P_2, Q_2) \) and if the rays \( r_i, i = 1, 2 \) beginning at \( P_i \) and containing \( Q_i \), are unidirectional.
...
\begin{theorem}\label{thm:euclidean_plane_factorization}
  ...
  \begin{ThmEnum}
    \ILabel{thm:euclidean_plane_factorization/sum} We define the \Def{sum} of the cosets \( [\overrightarrow{PQ}] \) and \( [\overrightarrow{QR}] \) as the coset \( [\overrightarrow{PR}] \).

    \ILabel{thm:euclidean_plane_factorization/scalar_product} We define the \Def{scalar multiplication} of \( \lambda \in \BbbR \) with the coset \( [\overrightarrow{PQ}] \) to be the coset \( [\overrightarrow{PR}] \), where \( \overrightarrow{PR} \) is the unique vector that is unidirectional with \( \overrightarrow{PQ} \) and \( \Dist(P, R) = \lambda \Dist(P, Q) \).
  \end{ThmEnum}
\end{theorem}
\begin{proof}
...
\begin{definition}\label{def:euclidean_plane_coordinate_system}
  ...
  What we achieve through the choice of \( O \) is that, for each point \( P \in A_2 \), we select the bound vector \( \overrightarrow{OP} \in V_2 \), called the \Def{radius vector} of \( P \). This injects \( A_2 \) into \( V_2 \), however if we take the free vector \( [\overrightarrow{OP}] \), we instead obtain a bijection between \( A_2 \) and \( F_2 \).
