...
\begin{definition}\label{def:convergence_of_function_at_point}
  Fix two topological spaces \( (X, \Cal{T}) \) and \( (Y, \Cal{O}) \), let \( D \subseteq X \) be a nonempty open set and let \( f: D \to Y \) be any function. We say that \( f \) \Def{converges} to \( \Ol y \in Y \) at \( \Ol x \in \Cl(D) \) and write
  \begin{equation*}
    \lim_{x \to \Ol x} f(x) = \Ol y
  \end{equation*}
  if for every neighborhood \( V \in \Cal{O} \) of \( \Ol y \) there exists a neighborhood \( U \in \Cal{T} \) of \( \Ol x \) such that
  \begin{equation*}
    f(U \cap D) \subseteq V.
  \end{equation*}

  Note that \( \Ol x \) does not even need to belong to \( D \) and \( \Ol y \) does not need to belong to \( f(D) \).
\end{definition}
