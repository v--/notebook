...
\begin{definition}\label{def:first_order_derivation_system}
  ...
  \begin{itemize}
    ...
    \begin{thmenum}[resume=def:first_order_derivation_system]
      \thmitem{def:first_order_derivation_system/axioms/terms}\mcite[def 22.7]{OpenLogicFull} First, for any formula \( \varphi \), any variable \( \xi \) and any term \( \tau \), we add the axioms
      \begin{align}
      ...
      \thmitem{def:first_order_derivation_system/axioms/equality}\mcite[20.39]{OpenLogicFull} For any variable \( \xi \), any \hyperref[def:first_order_syntax/ground_term]{ground terms} \( \tau \) and \( \sigma \) any formula \( \varphi \), we also add the following axioms:
