...
\begin{definition}\label{def:first_order_proofs}
  ...
  \begin{DefEnum}
    ...
    Derivations are also called \Def{proofs}. We use the notation shortcuts from \fullref{def:propositional_semantics/satisfiability}.
...
\begin{definition}\label{def:derivability_and_satisfiability}
  ...
  \begin{DefEnum}
    ...
    \ILabel{def:derivability_and_satisfiability/completeness} Dually, if \( \vDash \varphi \) implies \( \vdash \varphi \) for formulas \( \varphi \), we say that the derivation system is \Def{complete}.
  \end{DefEnum}
\end{definition}
...
\begin{definition}\label{def:hilberts_derivation_system}\MarginCite[def. 20.5 \\ def. 20.7]{OpenLogic20201202}
  ...
  \begin{RefList}
    ...
    \begin{BreakableAlign}
      ...
      \forall \xi \centerdot \varphi                    &\rightarrow \varphi[\xi \mapsto \tau],     \hspace{0.95cm} \tau \text{ is a closed term}     \label{def:hilberts_derivation_system/axioms/forall_elim} \\
      \varphi[\xi \mapsto \tau]                         &\rightarrow \exists \xi \centerdot \varphi, \hspace{1.5cm} \tau \text{ is a closed term}     \label{def:hilberts_derivation_system/axioms/exists_intro} \\
                                        \text{equality} & \text{ axioms}                                                                              \nonumber \\
                                                        &\phantom{{}\rightarrow{}} (\xi \doteq \xi)                                                   \label{def:hilberts_derivation_system/axioms/eq_reflexivity} \\
      (\xi \doteq \eta)                                 &\rightarrow (\tau[\zeta \mapsto \xi] \doteq \tau[\zeta \mapsto \eta])                        \label{def:hilberts_derivation_system/axioms/eq_term_subst} \\
      (\xi \doteq \eta)                                 &\rightarrow (\varphi[\zeta \mapsto \xi] \rightarrow \varphi[\zeta \mapsto \eta])             \label{def:hilberts_derivation_system/axioms/eq_formula_subst} \\
                                            \text{top } & \text{definition axioms}                                                                    \nonumber \\
                                                        &\phantom{{}\rightarrow{}} \top                                                               \label{def:hilberts_derivation_system/axioms/top} \\
                                    \text{equivalence } & \text{definition axioms}                                                                    \nonumber \\
      (\varphi \leftrightarrow \psi)                    &\rightarrow (\varphi \rightarrow \psi)                                                       \label{def:hilberts_derivation_system/axioms/equiv_elim_left} \\
      (\varphi \leftrightarrow \psi)                    &\rightarrow (\psi \rightarrow \varphi)                                                       \label{def:hilberts_derivation_system/axioms/equiv_elim_right} \\
      (\varphi \rightarrow \psi)                        &\rightarrow ((\psi \rightarrow \varphi) \rightarrow (\varphi \leftrightarrow \psi))          \label{def:hilberts_derivation_system/axioms/equiv_intro} \\
                                    \text{conjunction } & \text{definition axioms}                                                                    \nonumber \\
      (\varphi \wedge \psi)                             &\rightarrow \varphi                                                                          \label{def:hilberts_derivation_system/axioms/and_elim_left} \\
      (\varphi \wedge \psi)                             &\rightarrow \psi                                                                             \label{def:hilberts_derivation_system/axioms/and_elim_right} \\
      \varphi                                           &\rightarrow (\psi \rightarrow (\varphi \wedge \psi))                                         \label{def:hilberts_derivation_system/axioms/and_intro} \\
                                    \text{disjunction } & \text{definition axioms}                                                                    \nonumber \\
      \varphi                                           &\rightarrow (\varphi \vee \psi)                                                              \label{def:hilberts_derivation_system/axioms/or_elim_left} \\
      \varphi                                           &\rightarrow (\psi \vee \varphi)                                                              \label{def:hilberts_derivation_system/axioms/or_elim_right} \\
      (\varphi \rightarrow \psi)                        &\rightarrow ((\theta \rightarrow \psi) \rightarrow ((\varphi \vee \theta) \rightarrow \psi)) \label{def:hilberts_derivation_system/axioms/or_intro} \\
      ...
      \bot                                              &\rightarrow \varphi                                                                          \label{def:hilberts_derivation_system/axioms/explosion} \\
      \neg \varphi                                      &\rightarrow (\varphi \rightarrow \psi)                                                       \label{def:hilberts_derivation_system/axioms/explosion_neg} \\
...
\begin{theorem}[Completeness of Hilbert's derivation system]\label{thm:completeness_of_hilberts_derivation_system}\MarginCite[thm. 21.8]{OpenLogic20201202}
...
\begin{remark}\label{rem:hilberts_derivation_system_axioms}
  ...
  \begin{RemEnum}
    \ILabel{rem:hilberts_derivation_system_axioms/classical_logic} The full set of axioms are called the axioms of \Def{classical logic}. Without changing the underlying language, we can remove the explosion axioms \eqref{def:hilberts_derivation_system/axioms/explosion} and \eqref{def:hilberts_derivation_system/axioms/explosion_neg} and one of the two stability axioms \eqref{def:hilberts_derivation_system/axioms/stab} or \eqref{def:hilberts_derivation_system/axioms/stab_bot} since they are provable from the rest.

    We can actually use any of the subsets of constants, negation and connectives corresponding to the complete sets of Boolean functions in \fullref{ex:posts_completeness_theorem}. If we remove some of the constants or connectives of the language, we can also remove the corresponding axioms from \fullref{def:first_order_derivation_system/axioms}. For the rest of the remark, it would be most beneficial for us to remove everything except for the falsum \( \bot \) and material implication \( \rightarrow \). The minimal subset of axioms that still correspond to classical logic is then consists of only three rules: the two base rules \eqref{def:hilberts_derivation_system/axioms/implies_intro}, \eqref{def:hilberts_derivation_system/axioms/implies_distributivity} and stability \eqref{def:hilberts_derivation_system/axioms/stab_bot}.

    \ILabel{rem:hilberts_derivation_system_axioms/intuitionistic_logic}\MarginCite[def 53.10]{OpenLogic20201202}If we instead consider the base rules \eqref{def:hilberts_derivation_system/axioms/implies_intro}, \eqref{def:hilberts_derivation_system/axioms/implies_distributivity} and the principle of explosion \eqref{def:hilberts_derivation_system/axioms/explosion}, we obtain \Def{intuitionistic logic}. By \fullref{thm:hilberts_derivation_system_properties}, we can prove the principle of explosion from stability but not vice versa, therefore we can prove less theorems in intuitionistic logic. It is said to be a \enquote{weaker} derivation system. In particular, \fullref{thm:completeness_of_hilberts_derivation_system} does not hold. As a consequence, the set \( \Set{ \rightarrow, \bot } \) is no longer sufficient to derive statements analogous to \fullref{thm:boolean_function_equivalences} involving negation \( \neg \), top \( \top \) and the rest of the connectives. The \enquote{definition} axioms in \fullref{def:hilberts_derivation_system} are designed to emulate the behavior of conjunctions and disjunctions in classical logic. Thus, if we wish to use negation or connectives other than \( \rightarrow \) in intuitionistic logic, we must include the corresponding \enquote{definition} axioms.
...
\begin{theorem}[Soundness of Hilbert's derivation system]\label{thm:soundness_of_hilberts_derivation_system}\MarginCite[thm. 1.1]{Wasilewska2010}
...
\begin{theorem}[Deduction theorem]\label{thm:deduction_theorem}\MarginCite[thm. 2.2]{Wasilewska2010}
  In Hilbert's derivation system for \hyperref[rem:hilberts_derivation_system_axioms/minimal_logic]{minimal logic}, derivability of \( \Gamma, \psi \vdash \varphi \) holds if and only if \( \Gamma \vdash \psi \rightarrow \varphi \) holds. Compare this result to \fullref{thm:semantic_deduction_theorem}.
\end{theorem}

\begin{definition}\label{def:first_order_theory}
  A \Def{theory} for a derivation system is a set of formulas.
\end{definition}
