...
\begin{definition}\label{def:derivability_and_satisfiability}
  ...
  \begin{thmenum}
    \thmitem{def:derivability_and_satisfiability/soundness} If, for any formula \( \varphi \), derivability \( \vdash \varphi \) implies satisfiability \( \vDash \varphi \), we say that the derivation system is \term{sound}.

    \thmitem{def:derivability_and_satisfiability/completeness} Dually, if satisfiability \( \vDash \varphi \) implies derivability \( \vdash \varphi \) for any formula \( \varphi \), we say that the derivation system is \term{complete}.
  \end{thmenum}
\end{definition}
...
\begin{definition}\label{def:minimal_propositional_derivation_system}\mcite[def. 53.10]{OpenLogic20201202}
  ...
  Our goal is to define the \term{minimal propositional derivation system}. We do not need new rules to express the rest of the propositional syntax. Instead, we need several axiom schemas for each connective. The only exception is \hyperref[def:propositional_language/constants/top]{\( \bot \)}, the axioms for which tend to change semantics by a lot.
  ...
  \begin{thmenum}[series=def:minimal_propositional_derivation_system]
    \thmitem{def:minimal_propositional_derivation_system/top} The simplest axiom states that the constant \hyperref[def:propositional_language/constants/top]{\( \top \)} is itself an axiom:
    \begin{equation}\label{def:minimal_propositional_derivation_system/top_intro}
      \top
    \end{equation}

    \thmitem{def:minimal_propositional_derivation_system/conjunction} Axioms for \hyperref[def:propositional_language/connectives/conjunction]{conjunction}:
    \begin{align}
      (\varphi \wedge \psi) &\rightarrow \varphi \label{def:minimal_propositional_derivation_system/and_elim_left} \\
      (\varphi \wedge \psi) &\rightarrow \psi \label{def:minimal_propositional_derivation_system/and_elim_right} \\
      \varphi               &\rightarrow (\psi \rightarrow (\varphi \wedge \psi)) \label{def:minimal_propositional_derivation_system/and_intro}
    \end{align}

    \thmitem{def:minimal_propositional_derivation_system/disjunction} Axioms for \hyperref[def:propositional_language/connectives/disjunction]{disjunction}:
    \begin{align}
      \varphi                    &\rightarrow (\varphi \vee \psi) \label{def:minimal_propositional_derivation_system/or_elim_left} \\
      \varphi                    &\rightarrow (\psi \vee \varphi) \label{def:minimal_propositional_derivation_system/or_elim_right} \\
      (\varphi \rightarrow \psi) &\rightarrow ((\theta \rightarrow \psi) \rightarrow ((\varphi \vee \theta) \rightarrow \psi)) \label{def:minimal_propositional_derivation_system/or_intro}
    \end{align}
  \end{thmenum}

  The following axioms and are usually said to be \enquote{abbreviations} and do not affect semantics:
  \begin{thmenum}[resume=def:minimal_propositional_derivation_system]
    ...
    \begin{align}
      \neg \varphi &\rightarrow (\varphi \rightarrow \bot) \label{def:minimal_propositional_derivation_system/neg_elim} \\
      (\varphi \rightarrow \bot) &\rightarrow \neg \varphi \label{def:minimal_propositional_derivation_system/neg_intro}
    \end{align}

    \thmitem{def:minimal_propositional_derivation_system/equivalence} The axioms for the biconditional is motivated via \fullref{thm:boolean_equivalences/biconditional_via_conditionals}:
    \begin{align}
      (\varphi \leftrightarrow \psi) &\rightarrow (\varphi \rightarrow \psi) \label{def:minimal_propositional_derivation_system/equiv_elim_left} \\
      (\varphi \leftrightarrow \psi) &\rightarrow (\psi \rightarrow \varphi) \label{def:minimal_propositional_derivation_system/equiv_elim_right} \\
      (\varphi \rightarrow \psi)     &\rightarrow ((\psi \rightarrow \varphi) \rightarrow (\varphi \leftrightarrow \psi)) \label{def:minimal_propositional_derivation_system/equiv_intro}
    \end{align}
  \end{thmenum}
\end{definition}
...
\begin{theorem}\label{thm:minimal_propositional_negation_laws}
  ...
  \begin{thmenum}
    \thmitem{thm:minimal_propositional_negation_laws/dne} Double negation elimination (see \cref{thm:boolean_equivalences/double_negation}):
    \begin{equation}\label{eq:thm:minimal_propositional_negation_laws/dne}
      \neg \neg \varphi \rightarrow \varphi \tag{DNE}.
    \end{equation}

    \thmitem{thm:minimal_propositional_negation_laws/efq} Ex falso quodlibet (see \cref{thm:boolean_equivalences/ex_falso_quodlibet}):
  ...
  \mcite[prop. 3]{DienerMcKubreJordens2016} Assuming the \hyperref[def:minimal_propositional_derivation_system]{minimal propositional derivation system}, we have the following derivations:
  \begin{center}
    \synttree
      [
...
\begin{definition}\label{def:intuitionistic_propositional_derivation_system}\mcite[def. 53.10]{OpenLogic20201202}
  Semantics for the \hyperref[def:minimal_propositional_derivation_system]{minimal propositional derivation system} are not very well studied. If we extend it with the axiom \eqref{eq:thm:minimal_propositional_negation_laws/efq}, we obtain the \term{intuitionistic propositional derivation system}.
...
\begin{definition}\label{def:propositional_heyting_semantics}\mcite[14]{BezhanishviliHolliday2019}
  ...
  Given an interpretation \( I \) and a formula \( \varphi \), we define \( \varphi\Bracks{I} \) via \eqref{eq:def:propositional_semantics/formula_interpretation}, the sole difference being that negation valuation is defined via the pseudocomplement:
  \begin{equation*}
  ...
  It is important that different Heyting algebras can provide different semantics --- compare \fullref{ex:heyting_semantics_lem_counterexample} with the same result in a Boolean algebra.
\end{definition}
...
\begin{theorem}\label{thm:classic_propositional_logic_is_sound_and_complete}\mcite[1.1.3]{Buss1998Proofs}
...
\begin{definition}\mcite[2.2.1]{Buss1998Proofs}\label{def:first_order_derivation_system}
  ...
  \begin{itemize}
    ...
    \begin{thmenum}[resume=def:first_order_derivation_system]
      \thmitem{def:first_order_derivation_system/axioms/terms} First, for any formula \( \varphi \), any variable \( \xi \) and any term \( \tau \), we add the axioms
      \begin{align}
      ...
      \thmitem{def:first_order_derivation_system/axioms/equality}
      If the language contains formal equality, for any variables \( \xi, \eta, \zeta \), any term \( \tau \) and any formula \( \varphi \), we also add the following axioms:
      \begin{equation}\label{eq:def:first_order_derivation_system/axioms/equality}
        \begin{aligned}
                            &\phantom{{}\rightarrow{}} (\xi \doteq \xi) \\
          (\xi \doteq \eta) &\rightarrow (\tau[\zeta \mapsto \xi] \doteq \tau[\zeta \mapsto \eta]) \\
          (\xi \doteq \eta) &\rightarrow (\varphi[\zeta \mapsto \xi] \rightarrow \varphi[\zeta \mapsto \eta])
        \end{aligned}
      \end{equation}
    \end{thmenum}
  \end{itemize}
...
\begin{proposition}\label{thm:syntactic_implicit_quantification}
  Fix a formula \( \varphi \) a variable \( \xi \) over any first-order language \( \mscrL \). The formulas \( \varphi \) and \( \qforall \xi \varphi \) are \hyperref[def:first_order_semantics/interderivable]{interderivable} in the \hyperref[def:first_order_derivation_system]{classical first-order derivation system}.

  Compare this result with \fullref{thm:semantic_implicit_quantification}.
\end{proposition}
\begin{proof}
  ...
  Now we can use the axiom \eqref{def:minimal_propositional_derivation_system/top_intro} and \eqref{eq:def:implicational_logic/rules/modus_ponens} to infer \( \qforall \xi \varphi \). Thus
  \begin{equation*}
...
\begin{theorem}\label{thm:classic_first_order_logic_is_sound_and_complete}\mcite[2.2.2 \\ 2.2.3]{Buss1998Proofs}
...
\begin{theorem}[Minimal syntactic deduction theorem]\label{thm:minimal_syntactic_deduction_theorem}\mcite[thm. 2.2]{Wasilewska2010}
  In the \hyperref[def:minimal_propositional_derivation_system]{minimal propositional derivation system}, \( \Gamma, \psi \vdash \varphi \) holds if and only if \( \Gamma \vdash \psi \rightarrow \varphi \) holds.
\end{theorem}

\begin{theorem}[Syntactic deduction theorem]\label{thm:syntactic_deduction_theorem}\mcite[thm. 20.25]{OpenLogic20201202}
  In the \hyperref[def:first_order_derivation_system]{classical first-order derivation system}, \( \Gamma, \psi \vdash \varphi \) holds if and only if \( \Gamma \vdash \psi \rightarrow \varphi \) holds. Compare this result to \fullref{thm:semantic_deduction_theorem}.
\end{theorem}
