...
\begin{theorem}[Soundness of Hilbert's derivation system]\label{thm:soundness_of_hilberts_derivation_system}\MarginCite[thm. 1.1]{Wasilewska2010}
  \hyperref[def:hilberts_derivation_system]{Hilbert's derivation system} with \hyperref[rem:hilberts_derivation_system_axioms/minimal_logic]{minimal logic} is \hyperref[def:derivability_and_satisfiability/soundness]{sound}.
\end{theorem}

\begin{theorem}[Deduction theorem]\label{thm:deduction_theorem}\MarginCite[thm. 2.2]{Wasilewska2010}
  In \hyperref[rem:hilberts_derivation_system_axioms/minimal_logic]{Hilbert-style minimal logic} (and, hence, all of its extensions like classical logic), \( \Gamma, \psi \vdash \varphi \) if and only if \( \Gamma \vdash \psi \rightarrow \varphi \). Compare this result to \fullref{thm:semantic_deduction_theorem}.
\end{theorem}
