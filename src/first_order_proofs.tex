...
\begin{theorem}[Syntactic deduction theorem]\label{thm:syntactic_deduction_theorem}\mcite[thm. 2.2]{Wasilewska2010}
  In the \hyperref[def:minimal_propositional_axiomatic_derivation_system]{minimal propositional axiomatic derivation system} and all of its extensions, \( \Gamma, \psi \vdash \varphi \) holds if and only if \( \Gamma \vdash \psi \rightarrow \varphi \) holds.

  The theorem also holds in propositional \hyperref[def:intuitionistic_propositional_axiomatic_derivation_system]{intuitionistic} and \hyperref[def:propositional_axiomatic_derivation_system]{classical} axiomatic derivation systems (see \mcite[thm. 12.20]{OpenLogicFull}), as well as in the \hyperref[def:first_order_axiomatic_derivation_system]{classical first-order axiomatic derivation system} (see \mcite[thm. 22.25]{OpenLogicFull}).
\end{theorem}

\begin{proposition}\label{thm:minimal_logic_set_is_derivable_iff_conjunction_is_derivable}
...
\begin{proof}
  ...
  \begin{equation}\label{eq:thm:minimal_logic_set_is_derivable_iff_conjunction_is_derivable/conjunction_derivation}
    \psi_1, \psi_2 \vdash (\psi_1 \wedge \psi_2).
  \end{equation}

  \Fullref{thm:derivation_system_transitivity} implies that \( \psi_1, \psi_2, (\psi_1 \wedge \psi_2) \vdash \varphi \). Applying the same theorem again but this time removing \( \psi_1 \) and \( \psi_2 \) from the from the left due to \eqref{eq:thm:minimal_logic_set_is_derivable_iff_conjunction_is_derivable/conjunction_derivation}, we conclude that
  \begin{equation*}
