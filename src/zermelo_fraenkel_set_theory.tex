...
\begin{definition}\label{def:zfc}\mcite[271]{Enderton1977Sets}
  ...
  \begin{thmenum}
    ...
    \thmitem[def:zfc/A4]{A4}(union) If \( A \) is a set, then \( \bigcup A \) (see \fullref{def:basic_set_operations/union}) is also a set.

    \thmitem[def:zfc/A5]{A5}(power set) If \( A \) is a set, \( \pow(A) \) (see \fullref{def:basic_set_operations/power_set}) is also a set.
...
\begin{definition}\label{def:smallest_inductive_set}
  ...
  \end{equation*}
  where \( S \) is the set-theoretic successor operator (see \fullref{def:basic_set_operations/successor}).
