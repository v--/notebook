...
\begin{theorem}[Axiom of choice equivalences]\label{thm:axiom_of_choice_equivalences}
  ...
  \begin{thmenum}[resume=thm:axiom_of_choice_equivalences]
    ...
    \thmitem{thm:axiom_of_choice_equivalences/hypergraph} \Fullref{thm:hypergraphs_have_minimal_transversal}: Every \hyperref[def:hypergraph]{hypergraph} has a \hyperref[def:minimal_hypergraph_transversal]{minimal transversal}.
...
\begin{proof}
  ...
  But \( A \in B \) and \( B \in A \) cannot simultaneously holds due to \fullref{thm:simple_foundation_theorems/mutual_membership}.
...
\end{theorem}
\begin{proof}
  \SubProofOf{eq:def:peano_arithmetic/PA1} Let \( n, m \in \omega \) and suppose that \( \op{succ}(n) = \op{succ}(m) \). Then by \fullref{thm:simple_foundation_theorems/ordinal_successor_is_injective} we have \( n = m \).
