...
\begin{theorem}[Recursion theorem]\label{thm:omega_recursion}\mcite[73]{Enderton1977Sets}
  Let \( A \) be a nonempty set. Suppose that we are given some member \( a_1 \) of \( A \) and some operator \( T: A \to A \). Then there exists a unique function \( f: \omega \to A \) such that
  \begin{itemize}
...
\begin{proof}
  ...
  \SubProofOf{eq:def:peano_arithmetic/PA5} For each \( n \in \omega \) the operator \( T_n \) in \fullref{def:omega_operations/addition} is defined so that if \( \oplus_n(m) = k \), then \( \oplus_n(\op{succ}(m)) = \op{succ}(k) \).
  ...
  \SubProofOf{eq:def:peano_arithmetic/PA7} Analogously to \( \oplus \), for each \( n \in \omega \) the operator \( T_n \) in \fullref{def:omega_operations/multiplication} is defined so that if \( \odot_n(m) = k \), then \( \odot_n(\op{succ}(m)) = k \oplus n \).
