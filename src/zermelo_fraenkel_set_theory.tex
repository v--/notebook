...
\begin{proposition}\label{thm:zfc_existence_theorems}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:zfc_existence_theorems/cartesian_product} For any indexed family \( \set{ A_k }_{k \in \mscrK} \), its \hyperref[def:tuple_and_cartesian_product]{Cartesian product} \( \prod_{k \in \mscrK} A_k \) exists.
...
\begin{theorem}[Surjective functions are right-invertible]\label{thm:surjective_functions_are_right_invertible}
  ...
  In \hyperref[def:zfc]{\logic{ZF}} this theorem is equivalent to the \hyperref[def:zfc/choice]{axiom of choice} --- see \fullref{thm:axiom_of_choice_equivalences/surjective}.
\end{theorem}
\begin{proof}
