\begin{definition}\label{def:functor}\cite[definitions 1.2.1, 1.2.10]{Leinster2014}
  ...
  \begin{itemize}
    ...
    \item for each $A, A' \in \Obj \Bold A$, a function
    \begin{align*}
...
\begin{definition}\label{def:forgetful_functor}\cite[example 1.2.3]{Leinster2014}
  An informal notion is that of the \uline{forgetful functor}. A functor $F: \Bold A \to \Bold B$ is called forgetful if the images $F(A)$ of objects $A \in \Obj \Bold A$ have \enquote{less structure} than $A$. For example, a functor which maps topological spaces to their underlying sets is forgetful since it \enquote{forgets} about the topological structure.
\end{definition}
...
\begin{definition}\label{def:natural_transformation_composition}
  ...
  We define the \uline{composition} (sometimes called \uline{vertical composition}) of the natural transformations $\beta$ and $\alpha$ componentwise for $A \in \Obj \Bold A$ as
  \begin{align*}
...
\begin{definition}\label{def:functor_category}
  ...
  \begin{itemize}
    \item $\Obj [\Bold A, \Bold B]$ are functors $F: \Bold A \to \Bold B$.
    \item $[\Bold A, \Bold B](F, G)$ are the natural transformations from $F$ to $G$.
  \end{itemize}
...
\begin{definition}\label{def:comma_category}\cite[definition 2.3.1]{Leinster2014}
  ...
  \begin{itemize}
    \item $\Obj(F \downarrow G) = \{ (A, h, B) \colon A \in \Obj(\Bold A), B \in \Obj(\Bold B), F(A) \overset h \to G(B) \}$
...
\begin{definition}\label{def:representable_functor}\cite[definitions 4.1.3, 4.1.16]{Leinster2014}
  ...
  Analogously, the presheaf $G: \Bold{A}^{\Op} \to \Bold{Set}$ is representable if for some $A \in \Obj \Bold{A}^{\Op}$ we have $G \cong H_A$, where
  \begin{align*}
...
\begin{definition}\label{def:yoneda_embedding}\cite[definitions 4.1.15, 4.1.21]{Leinster2014}
  Let $\Bold A$ be a locally small category. For each pair $A, B \in \Obj \Bold A$ and morphism $f: A \to B$ we define the natural transformation $H^f: H^A \to H^B$ with $C$-components (note the reversal)
  \begin{align*}
    &H^f(C): H^A(C) \to H^B(C), \\
  ...
  Analogously, we define $H_f$ by $H_f(C) = H^C(f), C \in \Obj \Bold A$ and the \uline{Yoneda embedding} $H_\bullet: \Bold{A} \to [\Bold{A}^{\Op}, \Bold{Set}]$ by
  \begin{align*}
...
\begin{proposition}\label{def:yoneda_embedding_is_injective}\cite[exercise 4.1.27]{Leinster2014}
  Let $\Bold A$ be a locally small category and let $A, A' \in \Obj \Bold A$ be such that $H_A \cong H_{A'}$. Then $A \cong A'$.
\end{proposition}
\begin{proof}
