...
\begin{definition}\label{def:category}\cite[definition 1.1.1]{Leinster2014}
  A \Def{category} \( \Cat{C} \) consists of
  \begin{itemize}
    \item a set-theoretic class\Tinyref{def:set_zfc} of \Def{objects}, where \enquote{\( A \) is an object in \( \Cat{C} \)} is denoted as \( A \in \Cat{C} \)
    \item for each pair of objects \( A, B \in \Cat{C} \), a class \( {\Cat{C}}(A, B) \) of \Def{morphisms} (also called \Def{arrows})
    \item for each triple of objects \( A, B, C \in \Cat{C} \), a function
    \begin{equation*}
      \circ: {\Cat{C}}(B, C) \times {\Cat{C}}(A, B) \to {\Cat{C}}(A, C)
    \end{equation*}
    called the composition \( g \circ f \) of \( f \in {\Cat{C}}(A, B) \) and \( g \in {\Cat{C}}(B, C) \) (the order reversal notation comes from composition of functions)
  \end{itemize}
  such that
  \begin{defenum}
    ...
    \end{equation*}
    \DItem{def:category/associativity}(associativity) composition is associative, i.e. for each \( f \in {\Cat{C}}(A, B) \), \( g \in {\Cat{C}}(B, C) \) and \( h \in {\Cat{C}}(C, D) \), we have
    \begin{equation*}
  ...
  If there are no morphisms in \( \Cat{C} \) besides identity morphisms, we say that \( \Cat{C} \) is a \Def{discrete category} \cite[example 1.1.18(b]{Leinster2014}).
...
\begin{remark}\label{remark:category_obj_hom}
  ...
  Some authors, e.g. \cite{MacLane1994}, use notations such as \( \operatorname{hom}_{\Cat{C}}(A, B) \) to denote the class of morphisms between the objects \( A, B \in \Cat{C} \). We use this notation when dealing with algebraic structures, e.g. vector spaces (see \fullref{thm:linear_map_iff_function_on_basis}).

  The notation \( \Cat{C}(A) \) is used as a shorthand for \( \Bold{C}(A, A) \) and \( \hom_{\Cat{C}}(A) \) is used as a shorthand for \( \hom_{\Cat{C}}(A, A) \).
\end{remark}
...
\begin{definition}\label{def:opposite_category}\cite[construction 1.1.9]{Leinster2014}
  The \Def{opposite or dual category} of \( \Cat{C} \) is the category \( \Cat{C}^{-1} \) such that
  \begin{itemize}
