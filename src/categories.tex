...
\begin{definition}\label{def:category}\mcite[def. 1.1.1]{Leinster2016Basic}
  ...
  \begin{thmenum}[series=def:category]
    ...
    We call the \hyperref[eq:def:quiver/arc_set]{arc set} \( \cat{C}(A, B) \) of all morphisms from \( A \) to \( B \) a \term{morphism set} or \term{\( \boldop{hom} \)-set}. We use the shorthand \( \cat{C}(A) \) for \( \cat{C}(A, A) \).
