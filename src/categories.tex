...
\begin{remark}
  The definitions here are somewhat informal because of set-theoretic difficulties (see \fullref{sec:set_theory}).
\end{remark}
...
\begin{definition}\label{def:category}\cite[definition 1.1.1]{Leinster2014}
  ...
  \begin{defenum}
    ...
    \end{equation*}
    \DItem{def:category/associativity}(associativity) composition is associative, i.e. for each \( f \in {\Bold C}(A, B) \), \( g \in {\Bold C}(B, C) \) and \( h \in {\Bold C}(C, D) \), we have
    \begin{equation*}
...
\begin{remark}\label{remark:category_obj_hom}
  ...
  Some authors, e.g. \cite{MacLane1994}, use notations such as \( \operatorname{hom}_{\Bold C}(A, B) \) to denote the class of morphisms between the objects \( A, B \in \Bold C \). We use this notation when dealing with algebraic structures, e.g. vector spaces (see \fullref{thm:linear_map_iff_function_on_basis}).
...
\begin{definition}\label{def:morphism_invertibility}
  We introduce invertibility for morphisms in some category \( \Bold C \) (compare with function invertibility, \fullref{def:function_invertibility}).
...
\begin{definition}\label{def:skeletal_category}\cite[91]{MacLane1994}
  A subcategory \( \Bold S \) of \( \Bold A \) is called \Def{skeletal} or a \Def{skeleton} of \( \Bold A \) if it is full and if each object in \( \Bold A \) is isomorphic to exactly one object in \( \Bold B \).
