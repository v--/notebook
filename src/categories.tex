...
\begin{definition}\label{def:category}\cite[definition 1.1.1]{Leinster2014}
  A category is a generalization of the class of a preordered set\Tinyref{def:preordered_set} (see \cref{thm:preorder_category_correspondence}). In particular, the subset\Tinyref{def:subset} relation in the class of all sets\Tinyref{def:set_zfc} is a preorder (see \cref{def:category_of_sets}).
  ...
  \begin{defenum}[series=def:category]
    \DItem{def:category/objects} The objects in \( \Cat{C} \) are a set-theoretic class\Tinyref{def:set_zfc}. We use the notation \( A \in \Cat{C} \) to mean \enquote{\( A \) is an object in \( \Cat{C} \)}.
    ...
    \DItem{def:category/composition} For each triple of objects \( A, B, C \in \Cat{C} \), there exists a \Def{composition} operation\Tinyref{def:function}
    \begin{equation*}
