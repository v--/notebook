...
\begin{definition}\label{def:morphism_invertibility}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:morphism_invertibility/isomorphism} The morphism \( f: X \to Y \) is \term{fully invertible} it is both left-invertible and right-invertible. By \fullref{thm:def:morphism_invertibility/properties/left_and_right}, in this case, there exists a unique morphism \( f^{-1}: Y \to X \) that is a \term{two-sided inverse}, i.e. it is both a left inverse and a right inverse.
...
\end{proposition}
\begin{proof}
  \SubProofOf{thm:def:morphism_invertibility/properties/split_monomorphism} Suppose that \( g: Y \to Z \) is left-invertible with inverse \( h: Z \to Y \). Suppose that \( f_1, f_2: X \to Y \) are morphisms such that
  \begin{equation*}
  ...
  \SubProofOf{thm:def:morphism_invertibility/properties/at_most_one_inverse} If \( f: X \to Y \) has no inverse, it vacuously has at most one inverse.
  ...
  \SubProofOf{thm:def:morphism_invertibility/properties/left_and_right} Suppose that \( f: X \to Y \) has a left-inverse \( l: Y \to X \) and a right-inverse \( r: Y \to X \). Then
  \begin{equation*}
    r
    \reloset {\eqref{eq:def:category/C1}} =
    \id_Y \bincirc r
  ...
  \SubProofOf{thm:def:morphism_invertibility/properties/cancellative_composition} Let \( g: Y \to Z \) and \( h: Z \to U \) be monomorphisms (left-cancellative).
  ...
  \SubProofOf{thm:def:morphism_invertibility/properties/invertible_composition} Let \( f: X \to Y \) and \( g: Y \to Z \) be split monomorphisms (left-invertible).
...
\begin{proposition}\label{thm:def:zero_object/properties}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:def:zero_object/properties/final} X \hyperref[def:zero_objects/initial]{terminal object} is also unique up to an isomorphism.
...
\end{proposition}
\begin{proof}
  \SubProofOf{thm:def:zero_object/properties/initial} Suppose that \( X \) and \( Y \) are both initial objects in \( \cat{C} \). Then there exist morphisms \( f: X \to Y \) and \( g: Y \to X \). Their composition \( g \bincirc f \) is an \hyperref[def:morphism_invertibility/endomorphism]{endomorphism} on \( X \).
  ...
  \SubProofOf{thm:def:zero_object/properties/zero} Suppose that \( X \) is an initial object and that \( Y \) is a final object in \( \cat{C} \). Let \( f: X \to Y \) be an isomorphism between them.

  Let \( Z \in \cat{C} \) be any other object and let \( g: Z \to Y \) be the unique morphism to \( Y \). Then \( f^{-1} \bincirc g: Z \to X \) is a morphism from \( Z \) to \( X \). The inverse \( f^{-1}: Y \to X \) is unique by \fullref{thm:def:morphism_invertibility/properties/at_most_one_inverse}, therefore its composition with \( g: Z \to Y \) is also unique. Hence any object has a unique morphism to \( X \). This makes \( X \) a terminal object and thus a zero object.
