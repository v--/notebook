...
\begin{remark}\label{rem:categorical_principle_of_duality}
  ...
  Other \enquote{dual} notions include \hyperref[def:contravariant_functor]{covariant and contravariant functors}, \hyperref[def:category_of_cones/limit]{limits} and \hyperref[def:category_of_cones/colimit]{colimits}.
\end{remark}
...
\begin{definition}\label{def:universal_objects}\mcite[def. 2.1.7]{Leinster2016Basic}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:universal_objects/terminal} \hyperref[rem:categorical_principle_of_duality]{Dually}, we call the object \( T \in \cat{C} \) a \term{terminal object} or \term{final object} if for any other object \( A \in \cat{C} \) there exists a unique morphism \( f: A \to T \).

    The initial and terminal objects are collectively called \term{universal objects}.

    \thmitem{def:universal_objects/zero}\mcite{nLab:pointed_category} If \( 0 \) is both an initial and a terminal object, we say that \( 0 \) is a \term{zero object}. A category with a zero object is called a \term{pointed category}.
  \end{thmenum}
\end{definition}
...
\begin{proposition}\label{thm:def:universal_objects/properties}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:def:universal_objects/properties/terminal} A \hyperref[def:universal_objects/initial]{terminal object} is also unique up to an isomorphism.
...
\begin{proof}
  ...
  \SubProofOf{thm:def:universal_objects/properties/terminal} The statement for terminal objects is \hyperref[rem:categorical_principle_of_duality]{dual} to that of initial objects.
