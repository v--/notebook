...
\begin{definition}\label{def:category}\mcite[def. 1.1.1]{Leinster2016Basic}
  ...
  \begin{thmenum}[series=def:category]
    ...
    We call the set \( \cat{C}(X, Y) \) of all morphisms from \( X \) to \( Y \) a \term{morphism set} or \term{\( \hom \)-set}. We use the shorthand \( \cat{C}(X) \) for \( \cat{C}(X, X) \).
...
\begin{definition}\label{def:morphism_invertibility}
  ...
  \begin{thmenum}
    ...
    A fully invertible morphism is usually called an \term{isomorphism}. If there exists an isomorphism between \( X \) and \( Y \), we say that they are \term{isomorphic} and write \( X \cong Y \).
...
\begin{example}\label{ex:def:dual_category}
  X morphism \( f: X \to Y \) in the category \( \cat{Set}^{\opcat} \) is a function from the set \( Y \) to the set \( X \). Note that this is not a function in general.
\end{example}
