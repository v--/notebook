...
\begin{definition}\label{def:subcategory}\mcite[def. 1.2.18]{Leinster2016Basic}
  ...
  We say that \( \cat{D} \) is a \term{full subcategory} if the underlying quiver \( U(\cat{C}) \) is a \hyperref[eq:def:hypergraph/submodel/full]{full subquiver}. That is, in case \( \cat{D}(A, B) = \cat{C}(A, B) \) for every pair of objects \( A \) and \( B \) of \( \cat{D} \).

  Every \hyperref[rem:family_of_sets]{family} \( \mscrU \) of objects in \( \cat{C} \) induces a full subcategory \( \cat{D} \) of \( \cat{C} \) whose objects are those of \( \mscrU \) and whose morphisms are restricted to those whose domain and codomain are both in \( \mscrU \).
\end{definition}

\begin{definition}\label{def:categorical_diagram}
  A \term{categorical diagram} over \( \cat{C} \) is a \hyperref[eq:def:hypergraph/submodel/full]{subquiver} of the underlying quiver \( U(\cat{C}) \). This is not necessarily a category as shown in \fullref{ex:functor_image_not_a_category}.

  It is often convenient to draw graphically the \hyperref[def:quiver_geometric_realization]{geometric realizations} of the quiver, in which case we add labels to nodes and labels via \fullref{def:weighted_set}, so that the quiver drawing may have the same label on different vertices.

  Other established conventions for drawing diagrams include taking the \hyperref[def:quiver_transitive_closure]{transitive reduction} of the quiver \( D(\cat{I}) \), not drawing the identity morphisms, and adding various visual aids. In this regard, categorical diagrams correspond to the everyday sense of the word \enquote{diagram}.
  ...
  See \fullref{def:morphism_invertibility} for immediate examples of diagrams that have repeated vertices and only commute sometimes. See \fullref{rem:categorical_diagram_as_functor} for how categorical diagrams can be treated as functors in a more \enquote{categorical} approach.
\end{definition}
...
\begin{definition}\label{def:dual_category}\mcite[def. 1.1.9]{Leinster2016Basic}
  ...
  \begin{itemize}
    ...
    \begin{align*}
      f_{\cat{C}} &\in \cat{C}^{\opcat}(A, B) = \cat{C}^{\opcat}(B, A) \\
...
\begin{example}\label{ex:def:dual_category}
  A morphism \( f: A \to B \) in the category \( \cat{Set}^{\opcat} \) is a function from the set \( B \) to the set \( A \). Note that this is not a function in general.
\end{example}
