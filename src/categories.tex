...
\end{definition}
    ...
    Special notations include $f: X \hookrightarrow Y$.
...
\begin{definition}\label{def:categorical_subobject}\cite{MacLane1994}[122]
  ...
  Let $u: Y \to X$ and $v: Z \to X$ be monomorphisms\Tinyref{def:morphism_invertibility}. If $u = v \circ u'$ for some monomorphism $u': Y \to Z$, we say that \uline{$u$ factors through $v$} and write $u \leq v$. If both $u \leq v$ and $v \leq u$, we say that $u$ and $v$ are equivalent and write $u \equiv v$.
