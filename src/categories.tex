...
\begin{definition}\label{def:category}\cite[definition 1.1.1]{Leinster2014}
  ...
  \begin{itemize}
    \item a set-theoretic class\Tinyref{def:set_zfc} of \uline{objects}, where \enquote{$A$ is an object in $\Bold C$} is denoted as $A \in \Bold C$
...
\begin{definition}\label{def:standard_categories}
  \begin{defenum}
    ...
    \item\label{def:standard_categories/ord} Given a preordered set $(X, \leq)$\Tinyref{def:order/preorder}, we can form the category $\Bold{C}$ by defining
    \begin{itemize}
      \item the objects to be $\Bold{C} \coloneqq X$
      \item for any $a, b \in X$, the morphisms are $f: a \to b \iff a \leq b$.
    \end{itemize}

    See~\cref{thm:small_thin_category_equivalent_to_poset}.
  \end{defenum}
\end{definition}
...
\begin{note}\label{note:skeletal_subcategory_exists}
   A skeletal subcategory $\Bold S$ of $\Bold A$ can be constructed using the axiom of choice\AOC by only selecting one object from each isomorphism class within $\Bold A$.
\end{note}
...
\begin{definition}\label{def:categorical_subobject}\cite{MacLane1994}[122]
  ...
  Let $u: Y \to X$ and $v: Z \to X$ be monomorphisms\Tinyref{def:morphism_invertability}. If $u = v \circ u'$ for some monomorphism $u': Y \to Z$, we say that \uline{$u$ factors through $v$} and write $u \leq v$. If both $u \leq v$ and $v \leq u$, we say that $u$ and $v$ are equivalent and write $u \equiv v$.
