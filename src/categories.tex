...
\begin{definition}\label{def:category}\cite[definition 1.1.1]{Leinster2014}
  A \underLine{category} \( \Bold C \) consists of
  \begin{itemize}
    \item a set-theoretic class\Tinyref{def:set_zfc} of \underLine{objects}, where \enquote{\( A \) is an object in \( \Bold C \)} is denoted as \( A \in \Bold C \)
    \item for each pair of objects \( A, B \in \Bold C \), a class \( {\Bold C}(A, B) \) of \underLine{morphisms} (also called \underLine{arrows})
  ...
  If there are no morphisms in \( \Bold C \) besides identity morphisms, we say that \( \Bold C \) is a \underLine{discrete category} \cite[example 1.1.18(b]{Leinster2014}).

  Given a morphism \( f: A \to B \), we say that \( A \) is the \underLine{domain of \( f \)} and that \( B \) is the \underLine{codomain of \( f \)}.
\end{definition}
...
\begin{definition}\label{def:category_cardinality}
  Let \( \Bold{C} \) be a category. If for each pair \( A, B \in \Bold C \) the class \( \Bold C(A, B) \) is a set, we say that \( \Bold C \) is \underLine{locally small}. If, in addition to this, the class of objects is a set, we say that \( \Bold C \) is \underLine{small}.
\end{definition}

\begin{definition}\label{def:generalized_element}\cite[definition 4.1.25]{Leinster2014}
  Let \( \Bold C \) be a category and \( A, B \in \Bold C \). We say that the morphism \( f: A \to B \) is \underLine{a generalized element of \( B \) of shape \( A \)}. In the category \( \Bold{Set} \), the morphism \( \in : 1 \to B \) is the standard element of the set \( B \) since there is a bijection between maps \( 1 \to B \) and elements of \( B \).
\end{definition}
...
\begin{definition}\label{def:morphism_invertibility}
  ...
  \begin{defenum}
    \item\label{def:morphism_invertibility/left_invertible} \( f: A \to B \) is called \underLine{left-invertible} if there exists a morphism \( g: B \to A \) such that \( g \circ f = \Id_A \). In this case we call \( g \) a \underLine{left inverse of \( f \)}.

    \item\label{def:morphism_invertibility/monomorphism} \( f: C \to B \) is called a \underLine{monomorphism} or \underLine{monic morphism} or \underLine{left-cancellative morphism} if for any \( g, h: B \to A \) the equality \( f \circ g = f \circ h \) implies \( g = h \). We sometimes denote monomorphisms by \( f: C \hookrightarrow B \).

    \item\label{def:morphism_invertibility/right_invertible} \( f: A \to B \) is called \underLine{right-invertible} if there exists a morphism \( g: B \to A \) such that \( f \circ g = \Id_B \). In this case we call \( g \) a \underLine{right inverse of \( f \)}.

    \item\label{def:morphism_invertibility/epimorphism} \( f: A \to B \) is called an \underLine{epimorphism} or \underLine{epic morphism} or \underLine{right-cancellative morphism} if for any \( g, h: B \to C \) the equality \( g \circ f = h \circ f \) implies \( g = h \). We sometimes denote epimorphisms by \( f: C \twoheadrightarrow B \).

    \item\label{def:morphism_invertibility/isomorphism} \( f: A \to B \) is called \underLine{invertible} or an \underLine{isomorphism} if there exists a morphism \( g: B \to A \) that is both a left and a right inverse of \( f \). In this case we call \( g \) a (two-sided) \underLine{inverse of \( f \)} and we say that the objects \( A \) and \( B \) are isomorphic. We sometimes denote isomorphisms by \( A \cong B \) or \( A \overset f \cong B \). We denote isomorphisms by \( f: X \cong Y \). A morphism \( f: A \to A \) from an object to itself is called an \underLine{endomorphism} and if an endomorphism is an isomorphism, we call it an \underLine{automorphism}.
  \end{defenum}
\end{definition}
...
\begin{definition}\label{def:opposite_category}\cite[construction 1.1.9]{Leinster2014}
  The \underLine{opposite or dual category} of \( \Bold C \) is the category \( \Bold C^{\Op} \) such that
  \begin{itemize}
...
\begin{definition}\label{def:subcategory}\cite[definition 1.2.18]{Leinster2014}
  We call the category \( \Bold B \) a \underLine{subcategory} of \( \Bold A \) if
  \begin{itemize}
    \item All objects in \( \Bold B \) are objects in \( \Bold A \).
    \item All morphisms \( f \in \Bold{B}(A, B) \) are morphisms in \( \Bold{A}(A, B) \).
  \end{itemize}

  In case \( \Bold{B}(A, B) = \Bold{A}(A, B) \) for all objects \( A, B \in \Bold B \), we say that \( \Bold B \) is a \underLine{full subcategory}.
\end{definition}

\begin{definition}\label{def:skeletal_category}\cite[91]{MacLane1994}
  A subcategory \( \Bold S \) of \( \Bold A \) is called \underLine{skeletal} or \underLine{a skeleton of \( \Bold A \)} if it is full and if each object in \( \Bold A \) is isomorphic to exactly one object in \( \Bold B \).

  A category \( \Bold A \) is called \underLine{skeletal} if it is its own skeleton, i.e. the only isomorphisms in \( \Bold A \) are equalities.
\end{definition}
...
\begin{definition}\label{def:product_category}\cite[exercise 1.1.14]{Leinster2014}
  Let \( \Bold A \) and \( \Bold B \) be categories. We define their \underLine{product category} \( \Bold A \times \Bold B \) component-wise as
  \begin{itemize}
...
\begin{definition}\label{def:initial_final_objects}\cite[definitions 2.1.7]{Leinster2014}
  Let \( \Bold C \) be a category. The (unique up to an isomorphism, if it exists) object \( X \in \Bold C \) is called \underLine{initial} (resp. \underLine{final} or \underLine{terminal}) if for any other object \( Y \in \Bold C \) there exists exactly one morphism \( f: X \to Y \) (resp. \( f: Y \to X \)).

  If an object is both initial and final, it is called a \underLine{zero object}. A category with a zero object is called a \underLine{pointed category}.
\end{definition}
...
\begin{definition}\label{def:categorical_subobject}\cite{MacLane1994}[122]
  ...
  Let \( u: Y \to X \) and \( v: Z \to X \) be monomorphisms\Tinyref{def:morphism_invertibility}. If \( u = v \circ u' \) for some monomorphism \( u': Y \to Z \), we say that \underLine{\( u \) factors through \( v \)} and write \( u \leq v \). If both \( u \leq v \) and \( v \leq u \), we say that \( u \) and \( v \) are equivalent and write \( u \equiv v \).

  The equivalence classes among the monomorphisms with a common codomain \( X \) are called \underLine{subobjects} of \( X \).
\end{definition}
