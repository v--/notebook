...
\begin{definition}\label{def:peano_arithmetic}\cite[1]{Peano1889}
  The Peano arithmetic (see \cref{remark:peano_arithmetic_zero}) is a first-order theory\Tinyref{def:first_order_formula} with a language with
  \begin{itemize}
  ...
  and axioms
  \begin{defenum}
    \DItem{def:peano_arithmetic/PA1}[PA1] \( \forall x, y (x \doteq y \iff s(x) \doteq s(y)) \)
    \DItem{def:peano_arithmetic/PA2}[PA2] \( \forall x \neg s(x) \)
    \DItem{def:peano_arithmetic/PA3}[PA3] (the \Def{axiom schema of induction}) for all formulas \( \varphi(x) \) (it is possible for \( \varphi \) to have other free variables), we have
    \begin{equation*}
      \varphi(0) \land \forall x (\varphi(x) \implies \varphi(s(x)))
    \end{equation*}
  \end{defenum}
...
\begin{remark}\label{remark:peano_arithmetic_zero}
  Although it is common to start the natural numbers with zero (e.g. \cite[67]{Enderton1977} or \cite[theorem 4.5]{Nerode2012}). Peano himself, however, started his natural numbers from one - see \cite[1]{Peano1889}.
...
\begin{definition}\label{def:natural_numbers}
  ...
  \begin{defenum}
    ...
    \begin{itemize}
      \item \( N \) is the set of all finite-dimensional vector spaces\Tinyref{def:vector_space} over a field\Tinyref{def:field} \( F \).
      \item \( S(V) \coloneqq V \times F \) is the module direct sum\Tinyref{def:module_direct_product} of \( V \) and \( F \).
