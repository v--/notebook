...
\begin{Theorem}[Axiom of choice]\label{thm:aoc}
  ...
  \begin{ThmEnum}
    \ILabel{thm:aoc/transversal}\cite[theorem 6M(4)]{Enderton1977} Every hypergraph has a minimal transversal (compare to \fullref{thm:finite_hypergraphs_have_minimal_transversal} that only mentions finite sets).

    \ILabel{thm:aoc/product}\cite[theorem 6M(2)]{Enderton1977} The \hyperref[def:cartesian_product]{Cartesian product} of a family of nonempty sets is nonempty.

    \ILabel{thm:aoc/choice}\cite[theorem 6M(3)]{Enderton1977} For any nonempty set \( X \) there exists a \hyperref[def:choice_function]{choice function}.

    \ILabel{thm:aoc/function}\cite[theorem 6M(1)]{Enderton1977} Every \hyperref[def:function/multivalued]{multivalued function} has a \hyperref[def:function/selection]{selection}.
