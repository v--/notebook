...
\begin{definition}\label{def:proof_derivation_system}
  ...
  \begin{thmenum}
    ...
    Some rules like \eqref{eq:def:first_order_derivation_system/exists/elim} are restrictive and depend on the \hyperref[def:derivation_system_derivability]{current derivation}. This is why we allow rules to be parameterized by some set \( \Gamma \) of formulas.
...
\begin{definition}\label{def:derivability_and_satisfiability}
  ...
  We restrict our attention to closed formulas because we wish to avoid the problems described in \fullref{rem:deduction_with_free_variables}. If we have a formula with free variables, we may simply take its \hyperref[thm:semantic_implicit_universal_quantification]{universal closure}.
\end{definition}
...
\begin{definition}\label{def:first_order_derivation_system}\mcite{LeanNaturalDeduction}
  ...
  \begin{thmenum}
    ...
    A variable \( \xi \) satisfying these conditions is called an \term{eigenvariable} of the rule. See \fullref{ex:def:first_order_derivation_system/eigenvariables} for further discussion of the necessity of these rules. The rules depend not only on their premises but on the entire derivation so they do not precisely fit \fullref{def:proof_derivation_system}. The trick is to add undischarged assumptions in the derivation (up until this point) to some set \( \Delta \) and then work with \( \Gamma \cup \Delta \) as the set of axioms. This is done to avoid polluting \( \Gamma \) with assumptions that may later be discharged.

    \mcite[def. 22.8]{OpenLogicFull} Note that we only impose this eigenvariable condition on undischarged assumptions. In order to emulate this without natural deduction, we can reformulate these rule schemas as:
    ...
    where \( \xi \) must not be free in the set \( \Gamma \) of assumptions (in the sense of \fullref{def:proof_derivation_system}).
