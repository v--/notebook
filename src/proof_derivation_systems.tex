...
\begin{definition}\label{def:derivation_system_derivability}\mcite[def. 18.1]{OpenLogicFull}
 ...
 A sequence \( \varphi_1, \ldots, \varphi_n \) of formulas is called a \term{derivation} from the set \( \Gamma \) of formulas if, for any \( k = 1, \ldots, n \), either of the following hold:
  ...
  These definitions are specifically tailored at derivation systems. See \fullref{def:first_order_theory} for the more general definition of a theory axiomatized by a set.
\end{definition}
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    The rest follows by applying \fullref{thm:syntactic_contraposition} \( \theta \rightarrow \chi \) and then using \fullref{thm:syntactic_first_order_quantifiers_are_dual}.
  \end{itemize}
\end{proof}
