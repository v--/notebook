...
\begin{definition}\label{def:intuitionistic_propositional_derivation_system}\mcite[def. 55.10]{OpenLogicFull}
  ...
  \mcite[sec. 10.2]{OpenLogicFull} The following \hyperref[thm:natural_deduction]{natural deduction} rule schema can be used interchangeably with \eqref{eq:thm:minimal_propositional_negation_laws/efq}:
...
\begin{definition}\label{def:propositional_derivation_system}
  ...
  \mcite[sec. 10.2]{OpenLogicFull} The following \hyperref[thm:natural_deduction]{natural deduction} rule schema can be used interchangeably with \eqref{eq:thm:minimal_propositional_negation_laws/dne}:
...
\begin{definition}\label{def:first_order_derivation_system}
  If we wish to work with first-order logic rather than merely propositional logic, we must extend the \hyperref[def:propositional_derivation_system]{classical propositional derivation system}. We call this, very simply, the (classical) \term{first-order derivation system}.

  \begin{itemize}
    \item\mcite[def 22.8]{OpenLogicFull} We add two \hyperref[def:proof_derivation_system/rules]{derivation rules}. Both assume that \( \varphi \) and \( \psi \) are formulas such that \( \xi \not\in \boldop{Free}(\varphi) \).
    ...
    \begin{thmenum}[resume=def:first_order_derivation_system]
      \thmitem{def:first_order_derivation_system/axioms/terms}\mcite[def 22.7]{OpenLogicFull} First, for any formula \( \varphi \), any variable \( \xi \in \boldop{Free}(\varphi) \) and any term \( \tau \), we add the axioms
      \begin{align}
      ...
      \thmitem{def:first_order_derivation_system/axioms/equality}\mcite[20.39]{OpenLogicFull} For any \hyperref[def:first_order_syntax/ground_term]{ground terms} \( \tau \) and \( \sigma \) any formula \( \varphi \) with \( \xi \in \boldop{Free}(\varphi) \), we also add the following axioms:
      \begin{align}
...
\end{proposition}
\begin{proof}
  Start with \( \varphi \). Obviously \( \xi \) is not free in \( \top \), hence we can instantiate the axiom \eqref{eq:def:positive_implicational_propositional_derivation_system/axioms/imp/intro} with \( \psi = \top \). Then
  \begin{equation*}
    \begin{prooftree}
