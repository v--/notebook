...
\begin{definition}\label{def:first_order_theory_consistency}\mcite[def. 22.6]{OpenLogicFull}
  Given a derivation system, the set \( \Gamma \) of formulas is said to be \term[bg=противоречива,ru=противоречивая]{inconsistent} if \( \Gamma \vdash \bot \) and \term{consistent} otherwise.
\end{definition}
...
\begin{definition}\label{def:category_of_first_order_models}
  Let \( \mscrL \) be a first-order language and let \( \Gamma \) be a nonempty set of \hyperref[def:positive_formula]{positive formulas}. We describe the \term{category of models} for \( \Gamma \) as the \hyperref[def:concrete_category]{concrete category}.

  Given a \hyperref[def:grothendieck_universe]{Grothendieck universe} \( \mscrU \), we define the following \hyperref[def:category]{category}:
  \begin{itemize}
    ...
    \item The \hyperref[def:category/identity]{identity morpshism} of a model is the \hyperref[def:multi_valued_function/identity]{identity function} on its universe.
  \end{itemize}
\end{definition}
\begin{proof}
...
\begin{example}\label{ex:def:category_of_first_order_models}
  ...
  \begin{itemize}
    ...
    \item The categories of simple directed and undirected graphs.
  \end{itemize}
  ...
  \begin{itemize}
    ...
    \item The category \hyperref[def:category_of_sets]{\( \cat{Set} \)} of sets with respect to either \logic{ZFC}, \logic{ZFC+U} or na\"ive set theory is not the same as the category of models of set theory. Instead, it is a category within a fixed set theory. Within the metatheory of this document, we work within a fixed model of \logic{ZFC+U} with respect to \hyperref[def:classical_logic]{classical logic}.
