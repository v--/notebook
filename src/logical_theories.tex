...
\begin{example}\label{ex:def:category_of_small_first_order_models}
  ...
  \begin{itemize}
    ...
    \item The categories \hyperref[def:ring]{\( \cat{Ring} \)} of rings, \hyperref[def:ring]{\( \cat{Mod}_\mscrR \)} of modules and \hyperref[def:vector_space]{\( \cat{Vect}_\BbbK \)} of vector spaces, which are both based on abelian groups.
