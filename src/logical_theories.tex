...
\begin{definition}\label{def:first_order_theory}\mcite[def. 17.1]{OpenLogicFull}
  ...
  As discussed in \fullref{rem:deduction_with_free_variables}, we are only interested in closed formulas. If a formula is not closed, we instead consider its \hyperref[thm:semantic_implicit_universal_quantification]{universal closure}.
...
\begin{proposition}\label{thm:formulas_unsatisfiable_iff_inconsistent}
   Assuming \hyperref[def:classical_logic]{classical logic}, a theory \( \Gamma \) is \hyperref[def:propositional_semantics/satisfiability]{unsatisfiable} if and only if it is semantically \hyperref[def:proof_derivation_system_consistency]{inconsistent}.
\end{proposition}
\begin{proof}
...
\end{theorem}
\begin{proof}
  \SufficiencySubProof Suppose that \( \Gamma \vDash \varphi \).  By \fullref{thm:classical_first_order_logic_is_sound_and_complete}, \( \Gamma \vdash \varphi \). Every \hyperref[def:derivation_system_derivability]{derivation} is finite, hence \( \varphi \) has a derivation from only finitely many formulas of \( \Gamma \). Denote these formulas by \( \Delta \). Then \( \Delta \vdash \varphi \) and, by the other direction of \fullref{thm:classical_first_order_logic_is_sound_and_complete}, we have \( \Delta \vDash \varphi \).

  \NecessitySubProof If \( \Delta \vDash \varphi \) for some finite \( \Delta \subseteq \Gamma \), then necessarily \( \Gamma \vDash \varphi \).
\end{proof}

\begin{corollary}\label{thm:first_order_compactness_theorem}\mcite[thm. 23.21]{OpenLogicFull}
...
\begin{theorem}[L\"owenheim-Skolem theorem]\label{thm:lowenheil_skolem_theorem}\mcite[thm. 23.30]{OpenLogicFull}
  Every consistent theory \( \Gamma \) has a model that is at most countably infinite.
\end{theorem}

\begin{example}[Skolem's paradox]\label{ex:skolems_paradox}\mcite[ex. 23.32]{OpenLogicFull}
  From \fullref{thm:lowenheil_skolem_theorem} it follows that \hyperref[def:zfc]{\logic{ZFC}}, if it is consistent, has a model at is at most countably infinite. The \hyperref[def:zfc/infinity]{axiom of infinity} states that no model of \logic{ZFC} is finite. Therefore there exists a model of \logic{ZFC} that is countably infinite. But we can construct uncountable sets in \logic{ZFC}.

  Therefore either:
  \begin{itemize}
    \item Uncountable sets within the metalogic are fundamentally different from those within the object logic.
...
\begin{example}\label{ex:def:category_of_first_order_models}
  ...
  \begin{itemize}
    ...
    \item The category \hyperref[def:category_of_sets]{\( \cat{Set} \)} of sets with respect to either \logic{ZFC} or na\"ive set theory is not the same as the category of models of set theory. Instead, it is a category within a fixed set theory. Within the metalogic of this document, we work within a fixed model of \logic{ZFC} with respect to \hyperref[def:classical_logic]{classical logic}.
...
\begin{proof}
  ...
  \SubProofOf{thm:intuitionistic_lindenbaum_tarski_algebra/top} The inference rule \eqref{eq:thm:minimal_natural_deduction/top/intro} shows that \( [\varphi] \leq [\top] \) for any formula \( \varphi \) and \( [\top] \) is the top element.
  ...
  But \( \varphi \) derives both \( \psi_1 \) and \( \psi_2 \), hence due to \fullref{thm:derivation_system_transitivity},
  \begin{equation*}
  ...
  From \fullref{thm:formulas_are_derivable_iff_conjunction_is_derivable},
  \begin{equation*}
