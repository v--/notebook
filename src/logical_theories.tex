...
\begin{proof}
  ...
  But from \fullref{def:first_order_valuation/formula_valuation} it follows that \( \bot\Bracks{v} = F \). The obtained contradiction shows that \( \mscrX \) cannot be a model of \( \Gamma \) and since this structure was chosen arbitrarily, we conclude that \( \Gamma \) is unsatisfiable.
\end{proof}
...
\begin{proof}
  ...
  \begin{equation*}
    \begin{prooftree}
      ...
      \hypo{ [\psi_2]^1 }
      \ellipsis {} { \varphi }
      \infer[left label=\( 1 \)]3[\ref{eq:thm:natural_deduction/or/elim}]{ \varphi }
    \end{prooftree}
  \end{equation*}
  ...
  \SubProofOf{thm:intuitionistic_lindenbaum_tarski_algebra/top} The inference rule \eqref{eq:thm:natural_deduction/top/intro} shows that \( [\varphi] \leq [\top] \) for any formula \( \varphi \) and \( [\top] \) is the top element.
