...
\begin{definition}\label{def:subobject_and_quotient}\mcite[122]{MacLane1994}
  For an object \( X \) in some \hyperref[def:category]{category}, define an equivalence relation on \hyperref[def:morphism_invertibility/left_cancellative]{monomorphisms} \( f: A \to X \) and \( g: B \to X \) with codomain \( X \) to hold if \( f \) \hyperref[def:factors_through]{factors through} \( g \) and vice versa. We call the equivalence classes of this relation \term{subobjects} of \( X \).

  \hyperref[thm:categorical_principle_of_duality]{Dually}, for the relation where epimorphisms \( f: X \to A \) and \( g: X \to B \) factors through each other, we say that the equivalence classes are \term{quotient objects} of \( X \).
\end{definition}
