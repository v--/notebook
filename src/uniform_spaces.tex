...
\begin{proposition}\label{def:uniform_limit_of_continuous_functions}
  \mbox{}
  \begin{thmenum}
    \DItem{def:uniform_limit_of_continuous_functions/continuous} A uniform limit\Tinyref{def:function_net_convergence} of continuous\Tinyref{thm:uniform_space_continuity} functions is continuous.
    \DItem{def:uniform_limit_of_continuous_functions/uniform} A uniform limit of uniformly continuous\Tinyref{def:uniform_continuity} functions is uniformly continuous.
  \end{thmenum}
\end{proposition}
\begin{proof}
  \begin{description}
    ...
    Fix uniform spaces \( (X, \U) \) and \( (Y, \V) \). Let \( \{ f_i \}_{i \in I} \) be a net\Tinyref{def:topological_net} of functions from \( S \subseteq X \) to \( (Y, \V) \) that converges to \( f: X \to Y \).

    \RItem{def:uniform_limit_of_continuous_functions/continuous} Assume that the functions \( f_i, i \in I \) are continuous and that they converge to \( f \) locally uniformly\Tinyref{def:function_net_convergence/locally_uniform}.

    Fix an entourage \( V \in \V \) and a point \( \Ol{x} \in S \). From local uniform continuity\Tinyref{def:locally_uniform_continuity},
    \begin{equation*}
      \forall i \in I \ \exists U \in \U \ \forall x_1, x_2 \in S : \Abs{x_1 - x_2} < U \implies \Abs{f_i(x_1) - f_i(x_2)} < V.
    \end{equation*}

    \RItem{def:uniform_limit_of_continuous_functions/uniform} Let \( \{ f_i \}_{i \in I} \) be a net\Tinyref{def:topological_net} of uniformly continuous on \( S \subseteq X \) functions between the uniform spaces \( (X, \U) \) to \( (Y, \V) \) that converges uniformly\Tinyref{def:function_net_convergence/uniform} to \( f: X \to Y \).

    Fix an entourage \( V \in \V \). From uniform continuity\Tinyref{def:uniform_continuity},
    \begin{equation*}
      \forall i \in I \ \exists U \in \U \ \forall x_1, x_2 \in S : \Abs{x_1 - x_2} < U \implies \Abs{f_i(x_1) - f_i(x_2)} < V.
    \end{equation*}
