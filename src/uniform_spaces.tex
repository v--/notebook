...
\begin{proposition}\label{thm:uniform_space_continuity}
  Let \( (X, \CT) \) be a topological space and \( (Y, \CU) \) be a uniform space. A function \( f: X \to Y \) is continuous at \( \Ol{x} \in X \) in the sense of \fullref{def:continuous_function} if and only if
...
\end{proposition}
\begin{proof}
  We will only prove \fullref{thm:uniform_space_continuity/uniform_source} because the proof contains a proof of \fullref{thm:uniform_space_continuity/topological_source}.
...
\begin{definition}\label{def:uniform_continuity}\cite[435]{Engelking1989}
  ...
  Compare this to \Def{pointwise continuity} on \( S \), which is defined by \fullref{thm:uniform_space_continuity/uniform_source} as convergence for any \( x_1 \in X \):
...
\begin{definition}\label{def:function_set_continuity}\cite[285]{Bouziad2004}
  ...
  \end{equation}
  and to \Def{functionwise pointwise continuity} of \( \CF \) on \( S \), i.e. regular continuity as defined by \fullref{thm:uniform_space_continuity} for all \( x_1, x_2 \in S \) and all \( f \in \CF \),
...
\begin{proof}\mbox{}
  ...
  \begin{description}
    ...
    Given an entourage \( W \in \CV \), we found a neighborhood \( A \cap B(\Ol{x}, U) \) of \( \Ol{x} \) such that \fullref{thm:uniform_space_continuity/topological_source} is satisfied. Thus \( f \) is continuous at \( \Ol{x} \).
