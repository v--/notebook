...
\begin{definition}\label{def:uniform_continuity}
  Fix two uniform spaces\Tinyref{def:uniform_space} \( (X, \U) \) and \( (Y, \V) \). We say that the function \( f: X \to Y \) if is \Def{uniformly continuous} on the set \( A \subseteq X \) if
  \begin{equation}\label{def:uniform_continuity/uniform}
    \forall V \in \V \ \underbrace{\exists U \in \U \ \forall x_1, x_2 \in A} : \Abs{x_1 - x_2} < U \implies \Abs{f(x_1) - f(x_2)} < V.
  \end{equation}

  Compare this to \Def{pointwise continuity} on \( A \), which is defined by \cref{thm:uniform_space_continuity/uniform_source} as convergence for any \( x_1 \in X \):
  \begin{equation}\label{def:uniform_continuity/pointwise}
    \forall V \in \V \ \underbrace{\forall x_1, x_2 \in A \ \exists U \in \U} : \Abs{x_1 - x_2} < U \implies \Abs{f(x_1) - f(x_2)} < V.
  \end{equation}
\end{definition}
