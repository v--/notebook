...
\begin{proof}
  ...
  \begin{itemize}
    \item Suppose that \( \beta \) is a successor ordinal, i.e. \( \beta = \op{succ}(\mu) \) for some \( \mu \), and suppose that for every \( \alpha < \mu \) we have \( V_\alpha \in V_\mu \). Clearly \( V_\mu \in V_\beta \) because \( V_\mu \) is a subset of itself.
    ...
    \item Suppose that \( \beta \) is a limit ordinal. For some fixed \( \alpha_0 \in \beta \) we have \( V_{\alpha_0} \in V_{\op{succ}(\alpha_0)} \) by what we have already proved. We also have
    \begin{equation*}
      V_\beta
      =
...
\begin{remark}\label{thm:cumulative_hierarchy_model_of_zfc}\mcite{MathSE:cumulative_hierarchy_model_of_zfc}
  ...
  \begin{thmenum}[series=thm:cumulative_hierarchy_model_of_zfc]
    \thmitem{thm:cumulative_hierarchy_model_of_zfc/power} The \hyperref[def:zfc/power_set]{axiom of power sets} is satisfied by \( V_\lambda \) for a limit ordinal \( \lambda \) if the axiom of power sets holds in the metatheory.
