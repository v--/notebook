...
\begin{definition}\label{def:preordered_set}\mcite\cite{nLab:preorder}
  ...
  \begin{thmenum}
    ...
    \ilabel{def:preordered_set/upper_lower_bound}\cite[170]{Enderton1977Sets} \( x \in X \) is an \term{upper bound} for \( A \subseteq X \) (resp. \term{lower bound} for \( A \) in the dual poset \( (X, \geq) \)) if \( y \leq x \) for any \( y \in A \).
    ...
    \ilabel{def:preordered_set/largest_smallest_element}\cite[171]{Enderton1977Sets} \( x \in X \) is a \term{largest element} or \term{maximum} \( \max X \) of \( X \) (resp. \term{smallest element} or \term{minimum} \( \min X \) of \( (X, \geq) \)) if \( y \leq x \) any \( y \in X \).

    \ilabel{def:preordered_set/maximal_minimal_element}\cite[170]{Enderton1977Sets} \( x \in X \) is a \term{maximal element} of \( X \) (resp. \term{minimal element} of \( (X, \geq) \)) if \( x \leq y \) implies \( x = y \) for any \( y \in X \).

    \ilabel{def:preordered_set/supremum_infimum}\cite[170]{Enderton1977Sets} \( x \in X \) is a \term{supremum} (resp. \term{infimum}) for \( A \subseteq X \) if \( x \) is the least upper bound (resp. greatest lower bound) of \( A \), i.e. the smallest element of the set \( U \subseteq X \) consisting of all upper bounds of \( X \).
  \end{thmenum}
\end{definition}

\begin{definition}\label{def:thin_category}\mcite\cite{nLab:thin_category}
  A \hyperref[def:category]{category} \( \cat{P} \) is called a \term{thin category} if, for every two objects \( A, B \in \bfop{P} \), whenever \( f, g \in \bfop{P}(A, B) \), we have \( f = g \).
