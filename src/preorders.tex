...
\begin{definition}\label{def:preordered_set}\mcite{nLab:preorder}
  ...
  \begin{thmenum}[series=def:preordered_set]
    \thmitem{def:preordered_set/theory} The \term{theory of preordered sets} is a theory over a language with equality and a single \hyperref[rem:order_infix_notation]{infix} binary predicate symbol with \fullref{def:binary_relation/reflexive} and \fullref{def:binary_relation/transitive} as axioms.
    ...
    \thmitem{def:preordered_set/category} We give no special name for the \hyperref[def:first_order_model_category]{model category} for the theory of preordered sets.
