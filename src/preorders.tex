...
\end{Proposition}
\begin{RefListProof}
    \ISufficiency Let \( (P, \leq) \) be a preordered set. We define the category \( \Cat{P} \) as follows:
    \begin{RefList}
      \IRef{def:category/objects} The set of objects in \( \Cat{P} \) is the set \( P \).

      \IRef{def:category/morphisms} The set of morphisms between two elements \( x, y \in \Cat{P} \) is the singleton \( \{ (x, y) \} \) when \( x \leq y \) and the empty set otherwise.

      \IRef{def:category/composition} The composition of two morphisms \( (x, y) \) and \( (y, z) \) is simply \( (x, z) \) (such a morphism exists by transitivity of \( \leq \)).
    \end{RefList}
    ...
    \INecessity Now assume that \( \Cat{P} \) is a thin small category. Define the relation \( \leq \) on the set \( \Cat{P} \) as \( x \leq y \iff \Cat{P}(x, y) \neq \varnothing \). This is a preorder because
    \begin{RefList}
      \IRef{def:binary_relation/reflexive} \( x \leq x \) because of the existence of identity morphisms.

      \IRef{def:binary_relation/transitive} If \( x \leq y \) and \( y \leq z \), then, by composition of morphisms, \( x \leq z \).
    \end{RefList}
