...
\begin{definition}\label{def:preordered_set}\mcite{nLab:preorder}
  ...
  \begin{thmenum}[series=def:preordered_set]
    \thmitem{def:preordered_set/theory} Consider a \hyperref[def:first_order_language]{first-order language} \( \mscrL \) with two \hyperref[rem:order_infix_notation]{infix} binary predicate symbol --- \( \leq \) and \( \geq \).
