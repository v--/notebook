...
\begin{definition}\label{def:preordered_set}\mcite{nLab:preorder}
  ...
  \begin{thmenum}
    \labitem{def:preordered_set/dual} The partially ordered set \( (X, \geq) \) where \( \geq \) is the converse \hyperref[def:binary_relation/converse]{relation}, is the called the \term{dual preordered set}. See \fullref{def:thin_category} for a discussion of the duality.

    \labitem{def:preordered_set/subset} For every subset \( A \subseteq X \) the pair \( (A, \leq_A) \) is a partially ordered set and the restriction
    \begin{equation*}
      \leq_A \coloneqq \{ (a, b) \in \leq \colon a \in A \T{and} b \in A \}.
    \end{equation*}
    is called the \term{subset preorder}.

    \labitem{def:preordered_set/comparability} The elements \( x, y \in X \) are called \term{comparable} if either \( x \leq y \) or \( y \leq x \).

    \labitem{def:preordered_set/upper_lower_bound}\cite[170]{Enderton1977Sets} \( x \in X \) is an \term{upper bound} for \( A \subseteq X \) (resp. \term{lower bound} for \( A \) in the dual poset \( (X, \geq) \)) if \( y \leq x \) for any \( y \in A \).

    \labitem{def:preordered_set/bounded_set} The set \( A \subseteq X \) is called \term{bounded from above} (resp. \term{bounded from below}) if it has an upper bound (resp. lower bound). If a set is bounded from both directions, we simply say that it is \term{bounded}.

    \labitem{def:preordered_set/largest_smallest_element}\cite[171]{Enderton1977Sets} \( x \in X \) is a \term{largest element} or \term{maximum} \( \max X \) of \( X \) (resp. \term{smallest element} or \term{minimum} \( \min X \) of \( (X, \geq) \)) if \( y \leq x \) any \( y \in X \).

    \labitem{def:preordered_set/maximal_minimal_element}\cite[170]{Enderton1977Sets} \( x \in X \) is a \term{maximal element} of \( X \) (resp. \term{minimal element} of \( (X, \geq) \)) if \( x \leq y \) implies \( x = y \) for any \( y \in X \).

    \labitem{def:preordered_set/supremum_infimum}\cite[170]{Enderton1977Sets} \( x \in X \) is a \term{supremum} (resp. \term{infimum}) for \( A \subseteq X \) if \( x \) is the least upper bound (resp. greatest lower bound) of \( A \), i.e. the smallest element of the set \( U \subseteq X \) consisting of all upper bounds of \( X \).
  \end{thmenum}
\end{definition}
