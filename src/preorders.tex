...
\begin{remark}\label{remark:order_infix_notation}
  \Def{Orders} are special binary relations\Tinyref{def:binary_relation}. We denote orders using symbols rather than letters because it is customary to write orders using infix notation \( a \sim b \) rather than \( (a, b) \in \sim \).
\end{remark}

\subsection{Preorders}\label{subsec:preorders}

\begin{definition}\label{def:preordered_set}\cite{nLab:preorder}
  A binary relation\Tinyref{def:binary_relation} \( \leq \) on a set \( X \) is called a \Def{preorder} if it is reflexive\Tinyref{def:binary_relation/reflexive} and transitive\Tinyref{def:binary_relation/transitive}.

  A \Def{preordered set} is a set \( X \) equipped with a preorder \( \leq \). 

  We introduce the following terminology:
  \begin{defenum}
    \DItem{def:preordered_set/dual} The partially ordered set \( (X, \geq) \) where \( \geq \) is the converse relation\Tinyref{def:binary_relation/converse}, is the called the \Def{dual preordered set}. See \fullref{def:thin_category} for a discussion of the duality.
...
\begin{proposition}\label{thm:preorder_category_correspondence}
  To every preordered set\Tinyref{def:preordered_set} there corresponds exactly one small\Tinyref{def:category_cardinality} thin\Tinyref{def:thin_category} category.

  Furthermore, infima\Tinyref{def:preordered_set/supremum_infimum} correspond to categorical products\Tinyref{def:categorical_product}, suprema to coproducts\Tinyref{def:categorical_coproduct} and dual preordered sets\Tinyref{def:preordered_set/dual} correspond to dual categories\Tinyref{def:opposite_category}.
...
\begin{proof}\mbox{}
  \begin{description}
    ...
    Note that the infimum of a set \( A \subseteq \Cat{P} \) (if it exists) has a unique morphism \( \inf A \leq x \) for any \( x \in A \). If \( b \leq x \) for all \( x \in A \) is another cone\Tinyref{def:categorical_cone}, then necessarily \( b \leq \inf A \). Therefore the infimum is the categorical product.
  \end{description}
...
\begin{definition}\label{def:monotone_map}
  Let \( (P, \leq_P) \) and \( (Q, \leq_Q) \) be two preordered sets\Tinyref{def:preordered_set}. We say that the function \( f: P \to Q \) is an \Def{order preserving map} or \Def{order homomorphism} or simply a \Def{monotone map} if
  \begin{equation*}
  ...
  In particular, if \( P \) is the set of nonnegative integers\Tinyref{def:integers}, then we speak of \Def{monotone sequences}
  \begin{equation*}
    \{ x_k \}_{k=1}^\infty,
  \end{equation*}
...
\begin{definition}\label{def:directed_set}\cite[8]{Engelking1989}
  A preordered set\Tinyref{def:preordered_set} \( (X, \leq) \) is called a \Def{directed set} (there is no established name for the relation itself) if every finite subset of \( X \) has an upper bound\Tinyref{def:preordered_set/upper_lower_bound}, i.e. for all \( x, y \in X \) there exists \( z \in X \) such that \( x \leq z \) and \( y \leq z \).

  Since the set of all upper bounds of \( \{ a, b \} \) may not have a least smallest\Tinyref{def:preordered_set/largest_smallest_element}, the upper bound condition is weaker than every two-element set having a supremum.
