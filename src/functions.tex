...
\begin{definition}\label{def:function}
  A \Def{multivalued function} from \( X \) to \( Y \) is simply a \hyperref[def:binary_relation]{binary relation} \( (F, X, Y) \), the difference being in how we treat multivalued functions and relations. We use the notation \( F: X \rightrightarrows Y \).

  \begin{DefEnum}[series=def:function]
    \ILabel{def:function/single_valued} The \hyperref[def:function/multivalued]{multivalued function} \( F: X \rightrightarrows Y \) is called a \Def{single-valued function} or simply a \Def{function} if \( F(x) \) is a \hyperref[rem:singleton_sets]{singleton set} for each \( x \in X \). In this case, we write \( F: X \to Y \) rather than \( F: X \rightrightarrows Y \).
    ...
    \ILabel{def:function/selection} A \Def{selection} of a multivalued function \( F: X \rightrightarrows Y \) is a single-valued function \( f: X \to Y \) such that \( \Gph(f) \subseteq \Gph(F) \) (see \fullref{def:function/graph}).
  ...
  \begin{DefEnum}[resume=def:function]
    ...
    \ILabel{def:function/composition} The \Def{composition} \( G \circ F \) of two functions \( F: X \rightrightarrows Y \) and \( Y \rightrightarrows Z \) is the function
    \begin{equation*}
  ...
  \begin{DefEnum}[resume=def:function]
    ...
    \ILabel{def:function/inverse} The \Def{inverse} \( F^{-1}: Y \rightrightarrows X \) of a multivalued function \( F: X \rightrightarrows Y \) is its \hyperref[def:binary_relation/converse]{converse} (rather than \hyperref[def:binary_relation/inverse]{inverse}) relation.

    \ILabel{def:function/diagonal} The function corresponding to the \hyperref[def:binary_relation/diagonal]{diagonal relation} is called the \Def{identity}. Instead, the (single-valued) \Def{diagonal function} \( f: \mscrX \to \mscrX^2 \) is defined as \( f(x) \coloneqq (x, x) \).
  \end{DefEnum}
  ...
  \begin{DefEnum}[resume=def:function]
    ...
    \ILabel{def:function/preimage} For a concrete set \( B \subseteq Y \), its \Def{large preimage} or simply \Def{preimage} under \( F: X \to Y \) is the value of \( B \) under the inverse function \( F^{-1}: Y \rightrightarrows X \). For a single value \( y \in Y \), we call \( F^{-1}(y) \) the \Def{fiber} of \( y \) under \( F \).

    We define its \Def{small preimage} as \( F_{-1}(B) \coloneqq \{ x \in X \colon F(x) \subseteq B \} \)

    \ILabel{def:function/extension} Let \( X \) and \( Y \) be sets and let \( A \subseteq X \). For the multivalued functions \( F: A \rightrightarrows Y \) and \( G: X \rightrightarrows Y \), we say that \( G \) is an \Def{extension} of \( F \) to \( X \) and that \( F \) is a \Def{restriction} of \( G \) to \( A \).
    ...
    If we are given the functions \( F_k: X \rightrightarrows Y_k, k = 1, \ldots, n \) and \( G: Y_1 \times \cdots \times Y_n \rightrightarrows Z \), their \Def{superposition} \( H: X \rightrightarrows Z \) is
    \begin{equation*}
...
\begin{proposition}\label{thm:function_image_properties}
  ...
  \begin{PropEnum}
    ...
    \ILabel{thm:function_image_properties/union} \( f(\bigcup_{k \in \mscrK} X_k) = \bigcup_{k \in \mscrK} f(X_k) \).

    \ILabel{thm:function_image_properties/intersection} \( f(\bigcap_{k \in \mscrK} X_k) \subseteq \bigcap_{k \in \mscrK} f(X_k) \) with equality holding if \( f \) is injective.
...
\begin{proposition}\label{thm:function_preimage_properties}
  ...
  \begin{PropEnum}
    ...
    \ILabel{thm:function_preimage_properties/union} \( f^{-1}(\bigcup_{k \in \mscrK} Y_k) = \bigcup_{k \in \mscrK} f^{-1}(Y_k) \).

    \ILabel{thm:function_preimage_properties/intersection} \( f^{-1}(\bigcap_{k \in \mscrK} Y_k) = \bigcap_{k \in \mscrK} f^{-1}(Y_k) \).
...
\begin{definition}\label{def:indexed_family}
  ...
  A more natural approach to indexed families is given by functions. We choose an arbitrary set \( \mscrK \), called the \Def{index set}. Every function \( f: \mscrK \to \mathcal C \) from \( \mscrK \) into some class \( \mathcal C \) of sets is then called an \Def{indexed family}. The function \( f \) maps every element \( k \) of \( \mscrK \) into a set \( X_k \coloneqq f(k) \). For convenience, this family is denoted as
  \begin{equation*}
    \{ X_k \}_{k \in \mscrK}.
  \end{equation*}
...
\begin{definition}\label{def:sequence}
  A \Def{sequence} \( \{ X_k \}_{k=1}^\infty \) is an indexed family with domain \( \mscrK = \BbbN \). Sometimes finite \( n \)-tuples are referred to as \Def{finite sequences}, in which case the usual sequences are referred to as \Def{infinite sequences}. See \fullref{def:topological_net}.
...
\begin{definition}\label{def:family_of_functions_separates_points}
  Let \( \mathcal{F} \) be a family of functions between the sets \( A \) and \( B \). We say that \( \mathcal{F} \) \Def{separates points} if for every two points \( x, y \in A \) there exists a function \( f \in \mathcal{F} \) such that \( f(x) \neq f(y) \).
\end{definition}
