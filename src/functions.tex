...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \item Suppose that both \( A \) and \( B \) are nonempty and fix two points \( x_1, x_2 \in A \). Since \( g \bincirc f = \id_A \), by definition of composition there exist points \( y_1, y_2 \in B \) such that \( f(x_k) = y_k \) and \( g(y_k) = x_k \) for \( k \in 1, 2 \). But these are precisely the values \( f(x_1) \) and \( f(x_2) \) since the function \( f \) is single-valued.
