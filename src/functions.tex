...
\begin{remark}\label{remark:function_definition}
  ...
  \begin{itemize}
    ...
    \item Several generalizations of set-theoretic functions are often used, e.g. multivalued\Tinyref{def:function} or partial functions\Tinyref{def:function/partial}, however most formalisms of set theory often only concern functions.
...
\begin{definition}\label{def:function}
  A \Def{multivalued function} from \( X \) to \( Y \) is simply a binary relation\Tinyref{def:binary_relation} \( (F, X, Y) \), the difference being in how we treat multivalued functions and relations. We use the notation \( F: X \MultTo Y \).
  ...
  \begin{defenum}[series=def:function]
    ...
    \DItem{def:function/single_valued} The multivalued function\Tinyref{def:function} \( F: X \MultTo Y \) is called a \Def{single-valued function} or simply a \Def{function} if \( F(x) \) is a singleton set\Tinyref{remark:singleton_sets} for each \( x \in X \). In this case, we write \( F: X \to Y \) rather than \( F: X \MultTo Y \).
    ...
    We denote the set of all functions from \( X \) to \( Y \) by either \( \Cat{Set}(X, Y) \) (which is consistent with \fullref{def:category_of_sets}) or by \( Y^X \) (which is consistent with cardinal arithmetic\Tinyref{def:cardinal_arithmetic}).
  ...
  \begin{defenum}[resume=def:function]
    ...
    \DItem{def:function/arity} The \Def{arity} of a function is its number of arguments\Tinyref{def:function/arity}. This should not to be confused with \cref{def:relation/arity}.

    \DItem{def:function/inverse} The \Def{inverse} \( F^{-1}: Y \MultTo X \) of a multivalued function \( F: X \MultTo Y \) is its converse\Tinyref{def:binary_relation/converse} (rather than inverse\Tinyref{def:binary_relation/inverse}) relation.
  \end{defenum}
...
\begin{definition}\label{def:currying}
  ...
  We often wish to \enquote{fix} some value \( x \), i.e. bind it using modified variable assignment\Tinyref{def:first_order_variable_assignment}, so that \( g(y): B \to C \) is well-defined.
\end{definition}
...
\begin{proposition}\label{thm:function_preimage_properties}
  Functions preimages\Tinyref{def:function/preimage} have the following basic properties (compare to \fullref{thm:function_image_properties}):
  \begin{propenum}
...
\begin{example}\label{ex:indexed_families}
  ...
  \begin{defenum}
    ...
    \item Nets\Tinyref{def:topological_net} is topology are indexed families where the domain is a directed set\Tinyref{def:directed_set}.
  \end{defenum}
\end{example}
