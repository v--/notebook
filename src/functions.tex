...
\begin{definition}\label{def:multi_valued_function}
  ...
  As discussed in \fullref{def:relation/graph}, the common practive of identifying a multi-valued function \( (F, A, B) \) with its \hyperref[def:relation/graph]{graph} \( F \) with no regard to its \hyperref[def:binary_relation/signature]{signature} \( (A, B) \) has serious drawbacks that we wish to avoid.
...
\begin{definition}\label{def:function}
  ...
  \mcite[52]{DontchevRockafellar2014} If \( f: A \to B \) is a single-valued function, \( F: A \to B \) is a multi-valued function and \( \gph(f) \subseteq \gph(F) \), we say that \( f \) is a \term{selection} of \( F \).
...
\begin{proposition}\label{thm:function_properties}
  ...
  \begin{thmenum}
    \thmitem{thm:function_properties/total} Single-valued functions are \hyperref[def:multi_valued_function/total]{total} as multi-valued functions.
...
\begin{proposition}\label{thm:existence_of_multi_valued_function_selection}
  ...
  This theorem is equivalent to \fullref{def:zfc/A8} --- see \fullref{thm:axiom_of_choice_equivalences/selection}.
\end{proposition}
...
\begin{proof}
  ...
  \SubProofOf{def:function_invertibility/bijection} It is clear that the conditions \fullref{def:function_invertibility/bijection/direct,def:function_invertibility/bijection/points,def:function_invertibility/bijection/preimage,def:function_invertibility/bijection/inverse,def:function_invertibility/bijection/categorical} are simply conjunctions of the corresponding conditions from \fullref{def:function_invertibility/injection} and \fullref{def:function_invertibility/surjection}.
...
\begin{definition}\label{def:disjoint_union}
  ...
  \begin{equation*}
    \bigsqcup_{k \in \mscrK} A_k \coloneqq \set{ (k, x) \given k \in \mscrK \T{and} x \in A_k }.
  \end{equation*}
\end{definition}
