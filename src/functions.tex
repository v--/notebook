...
\begin{definition}\label{def:function}
  A \Def{multivalued function} from \( X \) to \( Y \) is simply a binary \hyperref[def:binary_relation]{relation} \( (F, X, Y) \), the difference being in how we treat multivalued functions and relations. We use the notation \( F: X \MultTo Y \).
  ...
  \begin{defenum}[series=def:function]
    ...
    \DItem{def:function/single_valued} The multivalued \hyperref[def:function]{function} \( F: X \MultTo Y \) is called a \Def{single-valued function} or simply a \Def{function} if \( F(x) \) is a singleton \hyperref[remark:singleton_sets]{set} for each \( x \in X \). In this case, we write \( F: X \to Y \) rather than \( F: X \MultTo Y \).
...
\begin{definition}\label{def:symmetric_function}
  ...
  Note that symmetric functions are not symmetric \hyperref[def:derived_relations/symmetric]{relations}.
\end{definition}
