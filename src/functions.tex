...
\begin{definition}\label{def:multi_valued_function}
  ...
  \begin{thmenum}[resume=def:multi_valued_function]
    ...
    \mcite{nLab:corestriction} Dually, if we restrict \( B \) to its subset \( Y \), we say that the obtained function is a \term{corestriction}.
...
\begin{proposition}\label{thm:def:multivalued_function/properties}
  \hyperref[def:multi_valued_function]{Multi-valued functions} have the following basic properties:

  \begin{thmenum}
    \thmitem{thm:def:multivalued_function/properties/associative} \hyperref[def:multi_valued_function/composition]{Composition} is associative. That is, for any three functions \( F: A \to B \), \( G: B \to C \) and \( H: C \to D \) we have
    \begin{equation*}
    ...
    \thmitem{thm:def:multivalued_function/properties/composition_inverse} If \( F: A \to B \) and \( G: B \to C \) are \hyperref[def:multi_valued_function]{multi-valued functions}, then
    \begin{equation*}
...
\end{proposition}
\begin{proof}
  \SubProofOf{thm:def:multivalued_function/properties/associative} Let \( a \in A \). Then in order for \( d \in D \) to belong to \( [[H \bincirc G] \bincirc F](a) \), there must exist values \( b \in B \) and \( c \in C \) such that \( b \in F(a) \) and \( c \in G(b) \) and \( d \in H(c) \). Clearly this is also the condition for \( d \) to belong to \( [H \bincirc [G \bincirc F]](a) \).

  \SubProofOf{thm:def:multivalued_function/properties/composition_inverse} Since \( G \bincirc F \) has signature \( A \to C \), clearly \( [G \bincirc F]^{-1} \) has signature \( C \to A \). Let \( c \in C \).
...
\begin{proposition}\label{thm:def:function/properties}
  ...
  \begin{thmenum}
    \thmitem{thm:def:function/properties/total} Single-valued functions are \hyperref[def:multi_valued_function/total]{total} as multi-valued functions. As a consequence, a multi-valued function cannot have a selection unless it is \hyperref[def:multi_valued_function/total]{total}.
...
\begin{proposition}\label{thm:function_composition_invertibility}
  ...
  Compare this result to \fullref{thm:def:morphism_invertibility/properties/invertible_composition} and \fullref{thm:function_superposition_invertibility}.
\end{proposition}
\begin{proof}
...
\begin{proposition}\label{thm:def:function_image/properties}
  \hyperref[def:multi_valued_function/set_value]{Images of sets} under \( f: A \to B \) have the following basic properties:
  \begin{thmenum}
    \thmitem{thm:def:function_image/properties/monotonicity} If \( A_1 \subseteq A_2 \), then \( f[A_1] \subseteq f[A_2] \).

    \thmitem{thm:def:function_image/properties/union} For any \hyperref[def:cartesian_product/indexed_family]{indexed family} \( \seq{ A_k }_{k \in \mscrK} \subseteq A \) of subsets of \( A \) we have the equality
    \begin{equation}\label{eq:thm:def:function_image/properties/union}
      f\bracks*{ \bigcup_{k \in \mscrK} A_k } = \bigcup_{k \in \mscrK} f[A_k].
    \end{equation}

    \thmitem{thm:def:function_image/properties/intersection} For any indexed family \( \seq{ A_k }_{k \in \mscrK} \) of subsets of \( A \) we have the inclusion
    ...
    \thmitem{thm:def:function_image/properties/difference} For any two subsets \( A_1 \) and \( A_2 \) of \( A \) we have the inclusion
  ...
  Compare this result to the more well-behaved properties of \hyperref[thm:def:function/properties/preimage]{preimages} described in \fullref{thm:def:function_preimage/properties}.
\end{proposition}
\begin{proof}
  \SubProofOf{thm:def:function_image/properties/monotonicity} If \( x \in A_1 \), then \( x \in A_2 \) and hence \( f(x) \in f[A_2] \). Therefore, \( f[A_1] \subseteq f[A_2] \).

  \SubProofOf{thm:def:function_image/properties/union} If \( x_0 \in A_{k_0} \) for some \( k_0 \in \mscrK \), clearly
  \begin{equation*}
  ...
  Conversely, if \( y_0 \in f[A_{k_0}] \) for some \( k_0 \in \mscrK \), by \fullref{thm:def:function_image/properties/monotonicity} obviously
  \begin{equation*}
  ...
  \SubProofOf{thm:def:function_image/properties/intersection} If \( x_0 \) belongs to \( \bigcap_{k \in \mscrK} A_k \), then \( x_0 \) belongs to \( A_k \) for all \( k \in \mscrK \). It follows that \( f(x_0) \) belongs to \( f[A_k] \) for all \( k \in \mscrK \) and hence to their intersection. Therefore, the inclusion \eqref{eq:thm:def:function_image/properties/intersection} holds.
  ...
  \SubProofOf{thm:def:function_image/properties/difference} If \( f[A_1] \setminus f[A_2] \) is empty, \eqref{eq:thm:def:function_image/properties/difference} obviously holds. Suppose that it is nonempty and let \( y_0 \in f[A_1] \setminus f[A_2] \).
...
\begin{proposition}\label{thm:def:function_preimage/properties}
  Function \hyperref[thm:def:function/properties/preimage]{preimages} have the following basic properties:
  \begin{thmenum}
    \thmitem{thm:def:function_preimage/properties/monotonicity} If \( B_1 \subseteq B_2 \), then \( f^{-1}[B_1] \subseteq f^{-1}[B_2] \).

    \thmitem{thm:def:function_preimage/properties/union} For any \hyperref[def:cartesian_product/indexed_family]{indexed family} \( \seq{ B_k }_{k \in \mscrK} \subseteq B \) of subsets of \( B \) we have the equality
    ...
    \thmitem{thm:def:function_preimage/properties/intersection} For any indexed family \( \seq{ B_k }_{k \in \mscrK} \) of subsets of \( B \) we have the equality
    ...
    \thmitem{thm:def:function_preimage/properties/difference} For any two subsets \( B_1 \) and \( B_2 \) of \( B \) we have the equality
    \begin{equation}\label{eq:thm:def:function_preimage/properties/difference}
      f^{-1}[B_1] \setminus f^{-1}[B_2] = f^{-1}[B_1 \setminus B_2].
    \end{equation}
  \end{thmenum}

  Compare this result to the less well-behaved properties of images described in \fullref{thm:def:function_image/properties}.
\end{proposition}
\begin{proof}
  \SubProofOf{thm:def:function_image/properties/monotonicity} Analogous to \fullref{thm:def:function_image/properties/monotonicity}.

  \SubProofOf{thm:def:function_image/properties/union} Analogous to \fullref{thm:def:function_image/properties/union}.

  \SubProofOf{thm:def:function_image/properties/intersection} If \( y_0 \) belongs to \( \bigcap_{k \in \mscrK} B_k \), then \( y_0 \) belongs to \( B_k \) for all \( k \in \mscrK \). It follows that \( f(y_0) \subseteq f^{-1}[B_k] \) for all \( k \in \mscrK \) and hence it is also a subset of their intersection. Therefore,
  \begin{equation*}
    f^{-1} \bracks*{ \bigcap_{k \in \mscrK} B_k } \subseteq \bigcap_{k \in \mscrK} f^{-1}[B_k].
  \end{equation*}

  Conversely, if \( x_0 \in \bigcap_{k \in \mscrK} f^{-1}[B_k] \), it belongs to \( f^{-1}[B_k] \) for all \( k \in \mscrK \). Clearly then \( f(x_0) \in B_k \) for all \( k \in \mscrK \) and thus \( f(x_0) \in \bigcap_{k \in \mscrK} B_k \). Hence, by \fullref{thm:def:function_preimage/properties/monotonicity} we have
  \begin{equation*}
    f^{-1}(f(x_0))
    \subseteq
  ...
  \SubProofOf{thm:def:function_preimage/properties/difference} If \( y_0 \in B_1 \setminus B_2 \), there exists a point \( x_1 \in B_1 \) such that \( f(x_1) = y_0 \). Aiming at a contradiction, suppose that there exists a point \( x_2 \in f^{-1}[B_2] \) such that \( f(x_2) = y_0 \). Then \( y_0 = f(x_1) = f(x_2) \) implies that \( f^{-1}(y_0) \subseteq f^{-1}[B_1] \cap f^{-1}[B_2] \). \Fullref{thm:def:function_preimage/properties/intersection} then in turn implies that \( f^{-1}(y_0) \subseteq f^{-1}[B_1 \cap B_2] \) and hence by \fullref{thm:def:function_image/properties/monotonicity}
  \begin{equation*}
