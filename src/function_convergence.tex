...
\begin{definition}\label{def:local_convergence}
  ...
  \begin{defenum}
    ...
    \DItem{def:local_convergence/nets}(Heine-style condition) For every \hyperref[def:topological_net]{net} \( \{ x_\al \}_{\al \in \CA} \subseteq A \), for which \( x_0 \) is a limit \hyperref[def:net_convergence/limit]{point}, the corresponding net \( \{ f(x_\al) \}_{\al \in \CA} \) has \( y_0 \) as a limit point.
  \end{defenum}
\end{definition}
\begin{proof}\mbox{}
  \begin{description}
