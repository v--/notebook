...
\end{definition}
\begin{proof}
  \SubProofImplication{def:local_convergence/neighborhoods}{def:local_convergence/nets} Let \( \{ x_k \}_{k \in \CK} \subseteq U \) be a net  with limit point \( x_0 \). Consider the net \( \{ f(x_k) \}_{k \in \CK} \). Fix a neighborhood \( V \) of \( y_0 \). We need to show that \( \{ f(x_k) \}_{k \in \CK} \) is eventually in \( V \).
  ...
  \SubProofImplication{def:local_convergence/nets}{def:local_convergence/neighborhoods} Suppose that \fullref{def:local_convergence/nets} holds while \fullref{def:local_convergence/neighborhoods} does not\LEM. Let \( V \) be a neighborhood of \( y_0 \). Then there exists no neighborhood \( U \) of \( x_0 \) such that \( f(U) \subseteq V \).
