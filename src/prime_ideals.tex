...
\end{proposition}
\begin{proof}
  Let \( p \) be prime. Assume\DNE that \( p \) is reducible, that is, there exist non-invertible elements \( r_1, r_2 \in R \) such that
  \begin{equation*}
    p = r_1 r_2.
  \end{equation*}
...
\begin{proof}
  ...
  But \( p \) is prime, and thus irreducible by \fullref{thm:ufd_prime_iff_irreducible}, and hence either \( i \) or \( r \) must be a unit. If\DNE \( r \) is a unit, then \( \braket i = \braket {ir} = \braket p \), which contradicts our choice of \( I \supsetneq P \). It remains for \( i \) to be a unit.
...
\begin{proposition}\label{thm:krull_dimension_properties}
  ...
  \begin{thmenum}
    \thmitem{thm:krull_dimension_properties/monotone} If \( R = T / S \) is the quotient of some rings \( S \subseteq T \), \( \dim R \leq \dim T \).
    \thmitem{thm:krull_dimension_properties/pid} If \( R \) is a principal ideal domain, \( \dim R \in \{ 0, 1 \} \).
    \thmitem{thm:krull_dimension_properties/field} If \( R \) is a \hyperref[def:field]{field}, \( \dim R = 0 \).
    \thmitem{thm:krull_dimension_properties/polynomials_over_field}\cite[exercise 8.19]{КоцевСидеров2016} If \( R = \BbbK[X_1, \ldots, X_n] \) for some \hyperref[def:field]{field} \( \BbbK \), \( \dim R = n \).
  \end{thmenum}
\end{proposition}
