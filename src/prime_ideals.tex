...
\begin{definition}\label{def:prime_ring_ideal}\MarginCite[384]{Knapp2016BAlg}
  ...
  An element \( r \in R \) is called \Def{prime} if the ideal \( \Bracket r \) is prime.
\end{definition}
\begin{proof}
  ...
  By \fullref{thm:product_of_principal_ideals}, we have \( \Bracket{x} \Bracket{y} = \Bracket{xy} = \Bracket{0} = \{ 0 \} \).

  Since \( \Bracket{x} \Bracket{y} \subseteq P \), then either \( \Bracket{x} \subseteq P \) or \( \Bracket{y} \subseteq P \). That is, either \( [x] = 0 \) or \( [y] = 0 \).
...
\begin{proposition}\label{thm:field_maximal_ideal_representation}\MarginCite[exer. 8.1]{Коцев2016}
  If \( r_1, \ldots, r_n \) are elements of the field \( \BbbK \), then \( \Bracket{X_1 - r_1, \ldots, X_n - r_n} \) is a maximal ideal of \( \BbbK[X_1, \ldots, X_n] \).
\end{proposition}
...
\begin{proof}
  ...
  \Necessity Let \( r \) be an irreducible element and let \( p_1 p_2 \in \Bracket{r} \). We will show that either  \( p_1 \in \Bracket{r} \) or \( p_2 \in \Bracket{r} \).

  Since \( \Bracket{r} \) is an ideal, there exists an element \( q \in R \) such that \( qr = p_1 p_2 \). Because of unique factorization, there exists a unit \( u \in R \) such that \( uqr = p_1 p_2  \).

  Therefor either
  \begin{itemize}
    \item \( p_1 = 1 \), in which case \( p_2 = uqr \in \Bracket{r} \).
    \item \( p_1 = u \), in which case \( p_2 = qr \in \Bracket{r} \).
    \item \( p_1 = uq \), in which case \( p_2 = r \in \Bracket{r} \).
    \item \( p_1 = ur \in \Bracket{r} \).
    \item \( p_1 = qr \in \Bracket{r} \).
    \item \( p_1 = uqr \in \Bracket{r} \).
  \end{itemize}
\end{proof}
...
\begin{proof}
  ...
  But \( p \) is prime, and thus irreducible by \fullref{thm:ufd_prime_iff_irreducible}, and hence either \( i \) or \( r \) must be a unit. If\LEM \( r \) is a unit, then \( \Bracket i = \Bracket {ir} = \Bracket p \), which contradicts our choice of \( I \supsetneq P \). It remains for \( i \) to be a unit.

  Therefore \( I = \Bracket i = \Bracket 1 = R \). This proves that \( P \) is maximal with respect to inclusion of ideals.
\end{proof}
...
\begin{proposition}\label{thm:krull_dimension_properties}
  ...
  \begin{PropEnum}
    ...
    \ILabel{thm:krull_dimension_properties/polynomials_over_field}\cite[exercise 8.19]{Коцев2016} If \( R = \BbbK[X_1, \ldots, X_n] \) for some \hyperref[def:field]{field} \( \BbbK \), \( \dim R = n \).
  \end{PropEnum}
\end{proposition}
...
\begin{definition}\label{def:spectrum_of_ring}
  The set of all prime ideals on a ring \( R \) is called the \Def{spectrum} of \( R \) denoted by \( \Op{Spec}(R) \).
\end{definition}
