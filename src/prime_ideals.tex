...
\begin{definition}\label{def:prime_ring_ideal}\mcite\cite[384]{Knapp2016BasicAlgebra}
  An ideal \( P \) is called \term{prime} if it is proper and satisfies any of the equivalent conditions:
  \begin{thmenum}
    \labitem{def:prime_ring_ideal/direct} If \( x, y \in R \) are such that \( xy \in P \), then either \( x \in P \) or \( y \in P \).
    \labitem{def:prime_ring_ideal/ideals} If \( I, J \subseteq R \) are ideals such that \( IJ \subseteq P \), then either \( I \subseteq P \) or \( J \subseteq P \).
    \labitem{def:prime_ring_ideal/quotient} The quotient \( R / P \) is an integral domain.
  \end{thmenum}
...
\begin{proposition}\label{thm:prime_implies_irreducible}\mcite\cite[389]{Knapp2016BasicAlgebra}
...
\begin{definition}\label{def:maximal_ring_ideal}
  ...
  \begin{thmenum}
    \labitem{def:maximal_ring_ideal/maximality} \( M \) is maximal with respect to set inclusion among proper two-sided ideals.
    \labitem{def:maximal_ring_ideal/quotient} The quotient \( R / M \) is a field.
  \end{thmenum}
\end{definition}
\begin{proof}
...
\begin{theorem}[Krull's theorem]\label{thm:krulls_theorem}\mcite\cite{Hodges1979}
...
\begin{proposition}\label{thm:field_maximal_ideal_representation}\mcite\cite[exer. 8.1]{КоцевСидеров2016}
...
\begin{definition}\label{def:krull_dimension}\mcite\cite[67]{КоцевСидеров2016}
...
\begin{proposition}\label{thm:krull_dimension_properties}
  ...
  \begin{thmenum}
    \labitem{thm:krull_dimension_properties/monotone} If \( R = T / S \) is the quotient of some rings \( S \subseteq T \), \( \dim R \leq \dim T \).
    \labitem{thm:krull_dimension_properties/pid} If \( R \) is a principal ideal domain, \( \dim R \in \{ 0, 1 \} \).
    \labitem{thm:krull_dimension_properties/field} If \( R \) is a \hyperref[def:field]{field}, \( \dim R = 0 \).
    \labitem{thm:krull_dimension_properties/polynomials_over_field}\cite[exercise 8.19]{КоцевСидеров2016} If \( R = \BbbK[X_1, \ldots, X_n] \) for some \hyperref[def:field]{field} \( \BbbK \), \( \dim R = n \).
  \end{thmenum}
\end{proposition}
...
\begin{definition}\label{def:radical_ideal}\mcite\cite[15]{КоцевСидеров2016}
...
\begin{definition}\label{def:primary_ring_ideal}\mcite\cite[74]{КоцевСидеров2016}
  We call the proper ideal \( P \) of \( R \) \term{primary} if \( xy \in P \) implies that \( x \in P \) or \( y \in \sqrt P \).
\end{definition}

\begin{theorem}[Chinese remainder theorem]\label{thm:chinese_remained_theorem}\mcite\cite[thm. 8.27]{Knapp2016BasicAlgebra}
