...
\begin{definition}\label{def:zero_morphism}
  ...
  \begin{defenum}
    ...
    \item\label{def:zero_morphism/bidirectional} a \textbf{zero morphism} if it is both a left-zero and a right-zero morphism. We denote it by \( 0_{A,B} \) if it is unique (for example, in preadditive categories\Tinyref{def:preadditive_category}).
  \end{defenum}
\end{definition}
