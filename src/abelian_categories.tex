\begin{definition}\label{def:monoidal_category}\cite{MacLane1994}[158]
  A \uline{monoidal category} is a generalization of A monoid\Tinyref{def:group} from sets to categories. Formally, it is a category $\Bold M$ along with
  \begin{itemize}
...
\begin{definition}\label{def:preadditive_category}\cite{MacLane1994}[28]
  A \uline{preadditive category $\Bold C$} is any category enriched over the category $\Bold{Ab}$ of abelian groups\Tinyref{def:category_of_groups}, such that composition
  \begin{align*}
...
\begin{definition}\label{def:zero_morphism}
  ...
  \begin{defenum}
    ...
    \item\label{def:zero_morphism/bidirectional} a \uline{zero morphism} if it is both a left-zero and a right-zero morphism. We denote it by $0_{A,B}$ if it is unique (for example, in preadditive categories\Tinyref{def:preadditive_category}).
  \end{defenum}
\end{definition}
...
\begin{definition}\label{def:categorical_kernel}
  Let $\Bold C$ be a preadditive category and $f: A \to B$ be a morphism in $\Bold C$. We define the \uline{kernel} $\Ker(f)$ of $f$ as the equalizer\Tinyref{def:categorical_equalizer} of $f$ and $0_{A,B}$. Thus $\Ker(f)$ is a morphism from $L$ (the limit vertex) to $A$.
...
\begin{definition}\label{def:abelian_category}\cite{MacLane1994}[196]
  An additive category\Tinyref{def:additive_category} $\Bold C$ is called an \uline{abelian category} if:
  \begin{defenum}
...
\begin{definition}\label{def:chain_complex}\cite{nLab:chain_complex}
  In an abelian category $\Bold{C}$, the $\BB{Z}$-graded tower diagram\Tinyref{def:tower_diagram} with objects $\{ C_n \}_{n \in \BB{Z}}$ and morphisms $\partial_n: C_n \to C_{n-1}$
  \begin{equation}\label{def:chain_complex/chain_diagram}
    ...
    \end{tikzcd}
  \end{equation}
  is called a \uline{chain complex} if for every $n$,
  \begin{align*}
    \partial_n \circ \partial_{n+1} = 0_{C_{n+1},C_{n-1}}.
  \end{align*}

  Chain complexes may be finite or infinite in one or both directions. The morphisms $\partial_n$ are called \uline{boundary maps}.

  A \uline{cochain complex} is a chain complex on $\Bold{C}^{\Op}$, i.e.
