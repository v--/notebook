...
\begin{definition}\label{def:exact_morphism_pair}\MarginCite[196]{MacLane1994}
  In an abelian category \( \Cat{C} \), a composable pair of morphisms \( f: A \to B \) and \( g: B \to C \) is said to be \Def{exact} at \( B \) if \( \Img f \cong \ker g \) as subobjects of \( B \) (or, equivalently, \( \Co\ker f \cong \Co\Img g \); see \fullref{def:categorical_subobject}).
\end{definition}
