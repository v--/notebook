...
\begin{definition}\label{def:pointed_set}\mcite[26]{MacLane1994}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:pointed_set/submodel} The set \( A \subseteq X \) is a \hyperref[thm:substructure_is_model]{submodel} of \( X \) if \( e \in A \).
...
\begin{definition}\label{def:pointed_set_kernel}\mimprovised
  The \term{kernel} \( \ker(\varphi) \) of a \hyperref[def:pointed_set/homomorphism]{pointed set homomorphism} \( \varphi: X \to Y \) is the \hyperref[def:zero_locus]{zero locus} of \( \varphi \). That is, \( \ker(\varphi) \) is the \hyperref[thm:def:function/properties/preimage]{preimage} \( \varphi^{-1}(e_Y) \).
\end{definition}

\begin{proposition}\label{thm:pointed_set_kernels}
  The pointed subset \( X' \) of \( X \) is a \hyperref[def:zero_morphisms/kernel]{categorical kernel} of the morphism \( \varphi: X \to Y \) in \hyperref[def:pointed_set/category]{\( \cat{Set_*} \)} if and only if \( X' = \varphi^{-1}(e_Y) \).
\end{proposition}
\begin{proof}
  The link with categorical kernels is discussed in \fullref{ex:zero_morphisms_in_grp/kernel} for \hyperref[def:group]{groups}, where the argument only depends on groups being pointed sets.
\end{proof}
...
\begin{definition}\label{def:ordered_magma}\mimprovised\footnote{Based on \cite[224]{Golan2010}}
...
\begin{example}\label{ex:def:ordered_magma}
  ...
  \begin{thmenum}
    ...
    \thmitem{ex:def:ordered_magma/ordinal} More generally, every \hyperref[def:ordinal]{ordinal} as the set of all smaller ordinals is an ordered magma under \hyperref[def:ordinal_arithmetic/addition]{ordinal addition}.
...
\begin{definition}\label{def:monoid}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:monoid/duality} The \term{dual} or \term{opposite} monoid of \( (M, \cdot) \) is the monoid \( (M, \star) \), with the monoid operation defined as \( x \star y = y \cdot x \).

    The \hyperref[def:monoid_delooping]{delooping} of \( (M, \star) \) is the \hyperref[def:dual_category]{dual category} of the delooping of \( (M, \cdot) \). For this reason, we denote the dual monoid by \( M^{\opcat} \).
  \end{thmenum}
\end{definition}
...
\begin{proposition}\label{thm:monoid_kernels}
  The \hyperref[def:pointed_set_kernel]{kernel} of a \hyperref[def:group/homomorphism]{monoid homomorphism} \( \varphi: M \to N \) is a \hyperref[def:monoid/submodel]{submonoid} of \( M \).

  Consequently, these are the \hyperref[def:zero_morphisms/kernel]{categorical kernels} in \hyperref[def:monoid/category]{\( \cat{Mon} \)}.
\end{proposition}
\begin{proof}
  By \eqref{eq:def:pointed_set/homomorphism}, \( e_M \in \ker(\varphi) \), therefore \( \ker(\varphi) \) inherits its monoid structure from \( M \). It remains only to show that it is closed under the monoid operation. But this is trivial since, if \( x, y \in \ker(\varphi) \), then
  \begin{equation*}
    \varphi(xy) = \varphi(x) \varphi(y) = e_N e_N = e_N.
  \end{equation*}
\end{proof}
...
\begin{definition}\label{def:monoid_direct_product}
  ...
  Unlike the product, the direct sum is mostly useful for commutative monoids --- see \fullref{thm:monoid_categorical_limits}.
\end{definition}
