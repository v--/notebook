...
\begin{proposition}\label{thm:exponential_function_properties}
  ...
  \begin{PropEnum}
    ...
    \ILabel{thm:exponential_function_properties/homomorphism} \( \exp(x + y) = \exp(x) \exp(y) \). Stated in another way, \( \exp \) is a homomorphism from the additive group of \( \BbbC \) to the multiplicative group.
    ...
    \ILabel{thm:exponential_function_properties/conjugate} \( \overline{\exp(z)} = \exp(\overline{z}) \).

    \ILabel{thm:exponential_function_properties/unit_circle} For any \( c \in \BbbR \), the function \( t \mapsto \exp(it) \) is a bijection between any half-open interval \( [c, c + 2\pi) \) and the unit circle in \( \BbbC \).

    \ILabel{thm:exponential_function_properties/real_bijective} \( t \mapsto \exp(t) \) is a bijection from \( \BbbR \) to \( [0, \infty) \).

    \ILabel{thm:exponential_function_properties/bijective} For any \( c \in \BbbR \), \( \exp(z) \) is a bijection between the strip \( S \coloneqq \{ a + bi \colon c \leq b < c + 2\pi \} \) and the complex plane \( \BbbC \setminus \{ 0 \} \).
...
\begin{definition}\label{def:logarithm}
  ...
  We define the \Def{natural logarithm} \( \log(x) \) as \hyperref[def:function/inverse]{inverse function} of \( e^x \) from \( \BbbC \setminus \{ 0 \} \) to the strip \( S_c \coloneqq \{ a + bi \colon c \leq b < c + 2\pi \} \).
...
\begin{proposition}\label{thm:exponential-trigonometric_identities}
  ...
  \ILabel{thm:exponential_trigonometric_identities/eulers_formula} (Euler's formula) For any \( z \in \BbbC \),
  \begin{equation}\label{thm:exponential_trigonometric_identities/eulers_formula/identity}
    e^{iz} = \cos(z) + i \sin(z).
  \end{equation}

  \ILabel{thm:exponential_trigonometric_identities/inverse_eulers_formula} (Inverse Euler's identities) For any \( z \in \BbbC \),
  \begin{BreakableAlign}
