...
\begin{proof}
  ...
  Furthermore, this is also the minimal period. If we assume\LEM that \( \sin(x) \) has another period, say \( p \in (0, 2\pi) \), we would have \( \sin(p) = \sin(0) = 0 \) and \fullref{thm:trigonometric_function_properties/pythagorean_identity} would imply that \( \cos(p) \in \{ -1, 1 \} \). But then \( \cos(p) \) would be an extreme point for \( \cos \), which is not possible because \( \cos \) is convex in \( [0, 2\pi] \) and only has three extremal points -- \( 0, \pi, 2\pi \).
