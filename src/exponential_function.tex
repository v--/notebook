...
\begin{proposition}\label{thm:exponential_function_properties}
  The exponential function \( \exp(z) \) has the following basic properties (not that we do not use the notation \( e^z \) here in order to reduce confusion with yet-undefined power functions\Tinyref{def:power_function}):
  ...
  \begin{propenum}
    ...
    \DItem{thm:exponential_function_properties/bijective} \( \exp(z) \) is a bijection between the strip \( S \coloneqq \{ a + bi \colon 0 \leq b < 2\pi \} \) and the complex plane \( \BC \setminus \{ 0 \} \).

    \DItem{thm:exponential_function_properties/periodic} \( \exp(z) \) is \( 2i\pi \)-periodic\Tinyref{def:periodic_function}.
...
\begin{proof}
  ...
  \RItem{thm:exponential_function_properties/bijective} Fix \( a + bi \in H \), that is, \( b \in [0, 2\pi) \). By \fullref{thm:exponential_function_properties/homomorphism},
  \begin{equation*}
    e^{a + bi} = e^a e^{bi}.
  \end{equation*}
...
\begin{definition}\label{def:logarithm}
  We define the \Def{natural logarithm} \( \log(x) \) as the inverse of \( e^x \) from \( \BC \setminus \{ 0 \} \) to the strip \( S \coloneqq \{ a + bi \colon 0 \leq b < 2\pi \} \).
