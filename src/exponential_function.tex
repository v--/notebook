...
\begin{proposition}\label{thm:exponential_function_properties}
  ...
  \begin{propenum}
    \DItem{thm:exponential_function_properties/eulers_identity} (Euler's identity)
    \begin{equation*}
      \exp(i \pi) = -1.
    \end{equation*}

    \DItem{thm:exponential_function_properties/derivative} \( \exp(z) \) is its own derivative.
    ...
    \DItem{thm:exponential_function_properties/unit_circle} For any \( c \in \BR \), the function \( t \mapsto \exp(it) \) is a bijection between any half-open interval \( [c, c + 2\pi) \) and the unit circle in \( \BC \).

    \DItem{thm:exponential_function_properties/real_bijective} \( t \mapsto \exp(t) \) is a bijection from \( \BR \) to \( [0, \infty) \).

    \DItem{thm:exponential_function_properties/bijective} For any \( c \in \BR \), \( \exp(z) \) is a bijection between the strip \( S \coloneqq \{ a + bi \colon c \leq b < c + 2\pi \} \) and the complex plane \( \BC \setminus \{ 0 \} \).

    \DItem{thm:exponential_function_properties/periodic} \( \exp(z) \) is \( 2i\pi \)-\hyperref[def:periodic_function]{periodic}.
...
\end{proposition}
\begin{proof}
  \RItem{thm:exponential_function_properties/eulers_identity} By \fullref{thm:trigonometric_function_properties/zeros/pi} and \fullref{thm:trigonometric_identities/eulers_formula}, we have
  \begin{equation*}
    \exp(i\pi) = \cos(\pi) + i\sin(\pi) = -1.
  \end{equation*}

  \RItem{thm:exponential_function_properties/derivative} Follows from \fullref{thm:power_series_are_locally_uniform_convergent} and \fullref{thm:derivative_limit_exchange}.
  ...
  \RItem{thm:exponential_function_properties/interpolates_power} We use induction\IND on \( n \) to prove \( \exp(n) = e^n \). The case \( \exp(0) = 1 \) is obvious. If we assume that \( \exp(n) = e^n \), by \fullref{thm:exponential_function_properties/homomorphism}, we have
  \begin{equation*}
    \exp(n + 1)
    =
    \exp(n) \exp(1)
  ...
  \RItem{thm:exponential_function_properties/conjugate} By \fullref{thm:trigonometric_identities/eulers_formula},
  \begin{align*}
    \Ol{\exp(a + bi)}
  ...
  \RItem{thm:exponential_function_properties/periodic} By \fullref{thm:exponential_function_properties/eulers_identity},
  \begin{equation*}
  ...
  \RItem{thm:exponential_function_properties/unit_circle} For \( c, t \in \BR \) we have
  \begin{equation*}
    \Abs{\exp(it)}
    =
    \Abs{\cos(t) + i\sin(t)}
  ...
  \RItem{thm:exponential_function_properties/bijective} Fix \( a + bi \in S_c \), that is, \( b \in [c, c + 2\pi) \). By \fullref{thm:exponential_function_properties/homomorphism},
  \begin{equation*}
    e^{a + bi} = e^a e^{bi}.
  \end{equation*}
...
\begin{proposition}\label{thm:logarithm_properties}
  \mbox{}
  \begin{propenum}
    \DItem{thm:logarithm_properties/homomorphism} \( \log(xy) = \log(x) \log(y) \)
  \end{propenum}
\end{proposition}
\begin{proof}\mbox{}
  \begin{description}
...
\begin{proposition}\label{thm:power_function_properties}
  ...
  \begin{propenum}
    ...
    \DItem{thm:power_function_properties/derivative} \( D_x(x^y) = \log(x) x^y \).
  \end{propenum}
\end{proposition}
\begin{proof}\mbox{}
  \begin{description}
    ...
    \RItem{thm:power_function_properties/derivative} Using the chain rule for differentiation, we obtain
    \begin{equation*}
