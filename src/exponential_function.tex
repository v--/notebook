...
\begin{proposition}\label{thm:def:exponential_function/properties}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:def:exponential_function/properties/interpolates_power} The notation \( \exp(x) \) is consistent with iterated multiplication as defined in \fullref{def:semiring/dioid}, that is, \( \exp(n) = \underbrace{e \cdot \ldots \cdot e}_{n \text{times}} \) and for positive integers \( n \), \( \exp(n) =  \) and \( \exp(-n) =\tfrac 1 {\exp(n)} \).
