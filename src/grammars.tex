...
\begin{definition}\label{def:grammar}\cite[definition 2.2]{Sipser2013}
  Let \( \Cal{A} \) be some alphabet\Tinyref{def:language} and \( V, \Sigma \subseteq \Cal{A} \) be nonempty disjoint subsets of \( \Cal{A} \). We call elements of \( V \) \Def{variables} and the elements of \( \Sigma \) \Def{terminals}. Fix special \Def{start symbol} \( S \in V \).

  We define a binary relation\Tinyref{def:relation} \( \to \) of \Def{production rules} over \( (V \cup \Sigma)^* \), that is, rules are \enquote{transformations} that define how a language is \enquote{generated} starting from \( S \in V \) (see \cref{def:grammar_derivation} and \cref{ex:context_free_grammar/real_arithmetic}). As a shortcut, instead of listing individual rules with the same source as
  \begin{align*}
    &u \to v, \\
    &u \to w,
  \end{align*}
...
\begin{example}\label{ex:context_free_grammar/real_arithmetic}
  We will define a grammar for addition on natural numbers\Tinyref{def:natural_numbers}. Note that we only consider the number in \( \N \) only as symbols, not as the numbers themselves.

  Let \( V \coloneqq \{ A \} \) and \( \Sigma \coloneqq \N \cup \{ +, (, ) \} \). Define the rules:
  \begin{itemize}
...
\begin{proof}
  ...
  \begin{itemize}
    \item If \( \Len(w) = 1 \), then \( w = n \in \N \) and the word has been generated by the single rule \( A \to n \).
