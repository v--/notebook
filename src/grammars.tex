...
\begin{definition}\label{def:grammar}\mcite[def. 2.2]{Sipser2013}
  ...
  \begin{thmenum}
    \thmitem{def:grammar/variables} We call elements of \( V \) \term{variables} or \term{non-terminals}.

    \thmitem{def:grammar/terminals} We call elements of elements of \( \Sigma \) \term{terminals}.
    ...
    \thmitem{def:grammar/grammar} The tuple \( G \coloneqq (V, \Sigma, \to, S) \) is called a \term{formal grammar}.
...
\begin{definition}\label{def:grammar_derivation}\mcite[page 104 \\ page 108]{Sipser2013}
  ...
  \begin{thmenum}
    \thmitem{def:grammar_derivation/yields} Fix a \hyperref[def:language/word]{word} \( pvs \). If \( u \to v \) is a production rule, we say that \( pvs \) \term{yields} the word \( pws \) and write
    \begin{equation*}
      pvs \Rightarrow pws.
    \end{equation*}

    \thmitem{def:grammar_derivation/derivation} We say that \( u \) \term{derives} \( v \) and write \( u \Rightarrow v \) if there exists a finite sequence of words \( u_1, \ldots, u_n \) such that
    \begin{equation*}
