...
\begin{proposition}\label{thm:def:krull_dimension}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:def:krull_dimension/polynomial_ring}\mcite[corr. 4]{CoquandLombardi2005} For a \hyperref[def:field]{field} \( \BbbK \), the polynomial ring \( \BbbK[X_1, \ldots, X_n] \) has dimension \( n \).
  \end{thmenum}
\end{proposition}
\begin{proof}
...
\begin{definition}\label{def:zero_locus}\mimprovised
  The \term{zero locus} or \term{set of zeros} of a function \( f: S \to \BbbR^n \) from an arbitrary plain set \( S \) is the \hyperref[thm:def:function/preimage]{preimage}
  \begin{equation*}
...
\begin{remark}\label{rem:affine_varieties}
  We define \hyperref[def:affine_algebraic_set/varieties]{algebraic varieties} only for algebraically closed fields; for the purposes of \fullref{def:quadratic_plane_curve}, instead of polynomial ideals in \( \BbbR[X, Y] \) we study ideals in \( \BbbC[X, Y] \) and then use \hyperref[def:complexification]{decomplexification} on their \hyperref[def:zero_locus]{zero loci}.
\end{remark}
...
\begin{definition}\label{def:quadratic_plane_curve}\mimprovised
  ...
  Those that are \hyperref[def:affine_algebraic_set/curve]{algebraic curves} are precisely the \hyperref[def:ellipse]{ellipses}, \hyperref[def:hyperbola]{hyperbolas} and \hyperref[def:parabola]{parabolas} with their standard equations, as well as the imaginary ellipses described in \fullref{alg:canonization_of_quadratic_plane_curves/oval/imaginary_ellipse}. We call the other quadratic plane curves \term{degenerate} (we may also call the imaginary ellipse degenerate since we will have no use for it).

  \Fullref{alg:canonization_of_quadratic_plane_curves} describes a procedure for finding an \hyperref[def:affine_coordinate_system]{affine coordinate system} in \( \BbbR^2 \) in which we can easily classify the type of algebraic curve.
...
\begin{proof}
  ...
  By \fullref{thm:def:krull_dimension/polynomial_ring}, the \hyperref[def:krull_dimension]{Krull dimension} of \( \BbbK[X, Y] \) is \( 2 \). Hence, by \fullref{thm:quotient_ideal_lattice_theorem}, the chain of \hyperref[def:ring/quotient]{quotients}
  \begin{equation*}
...
\begin{example}\label{ex:real_algebraic_curves}
  ...
  \Fullref{alg:canonization_of_quadratic_curves} shows how this generalization can be used with real numbers via \hyperref[def:complexification]{complexification} and yield results that avoid degenerate cases like the one described here.
\end{example}
...
\begin{algorithm}\label{alg:canonization_of_quadratic_plane_curves}
  We can use \hyperref[def:rigid_motion]{rigid motions} to convert \hyperref[def:quadratic_plane_curve]{quadratic plane curve} into what we will call its \term{canonical form}, which is either
  \begin{itemize}
    ...
    \item An \enquote{imaginary ellipse} --- cases \cref{alg:canonization_of_quadratic_plane_curves/oval/imaginary_ellipse}.
  ...
  \begin{thmenum}
    ...
    \begin{thmenum}
      ...
      \thmitem{alg:canonization_of_quadratic_plane_curves/oval/imaginary_ellipse} If all coefficients have the same sign, then \( p(X, Y) \) is irreducible, but has no roots. Its zero locus in \( \BbbC^2 \) is also called an \term{imaginary ellipse}.
...
\begin{lemma}\label{thm:bivariate_quadratic_polynomial_irreducibility}
  The complex bivariate quadratic polynomial
  \begin{equation*}
    a X^2 + b XY + c Y^2 + d X + e Y + f
    =
    \begin{pmatrix}
    ...
    \end{pmatrix}
    \begin{pmatrix}
      a          & \ifrac b 2 & \ifrac d 2 \\
      \ifrac b 2 & b          & \ifrac e 2 \\
      \ifrac d 2 & \ifrac e 2 & c
    \end{pmatrix}
    \begin{pmatrix}
      X \\ Y \\ 1
    \end{pmatrix}
  \end{equation*}
  is irreducible if and only if its matrix is invertible.
\end{lemma}
\begin{proof}
  Define the more general polynomial
  \begin{equation*}
    p(X, Y, Z)
    =
    \begin{pmatrix}
  ...
  We are interested in the irreducibility of \( p(X, Y, 1) \). It turns out that it is equivalent to the irreducibility of \( p(X, Y, Z) \).

  Indeed, if \( q(X, Y) \) divides \( p(X, Y, 1) \), it divides \( p(X, Y, Z) \). Conversely, if \( q(X, Y, Z) \) divides \( p(X, Y, Z) \), then \( q(X, Y, 1) \) divides \( p(X, Y, 1) \). Since the units in \( \BbbC[X, Y] \) and \( \BbbC[X, Y, Z] \) are precisely the nonzero complex numbers, we conclude that \( p(X, Y, Z) \) is irreducible in \( \BbbC[X, Y, Z] \) if and only if \( p(X, Y, 1) \) is irreducible in \( \BbbC[X, Y] \).

  Denote the matrix of \( p(X, Y, Z) \) by \( A \). It is a \hyperref[def:conjugate_transpose]{Hermitian matrix}. Thus, from \fullref{thm:finite_dimensional_spectral_theorem/matrix} it follows that \( A \) is \hyperref[def:diagonalizable_matrix]{diagonalizable} via an \hyperref[def:unitary_matrix]{unitary} matrix \( P \). That is, there exist scalars \( \alpha \), \( \beta \) and \( \gamma \) such that
  \begin{equation*}
    p(X, Y, Z)
    =
    \bracks*
  ...
  From \fullref{thm:irreducible_polynomial_variable_change} it follows that \( p(X, Y, Z) \) is irreducible if and only if
  \begin{equation*}
    q(\xi, \eta, \zeta) \coloneqq \alpha \xi^2 + \beta \eta^2 + \gamma \zeta^2
  \end{equation*}
  ...
  Depending on which of \( \alpha \), \( \beta \) and \( \gamma \) is zero, we can classify the curve.


  \begin{itemize}
    \item If \( \alpha = \beta = \gamma = 0 \), then \( q(\xi, \eta, \zeta) \) and hence also \( \widehat{p}(X, Y, Z) \) are zero polynomials. This case is actually impossible, because we have assumed that \( p(X, Y) \) is quadratic.

    \item If exactly two of \( \alpha \), \( \beta \) or \( \gamma \) are zero, we are left with a homogeneous quadratic polynomial of one variable. These are always reducible. Without loss of generality, if \( \beta = \gamma = 0 \), then
    \begin{equation*}
      q(\xi, \eta, \zeta)
      =
      \alpha \xi^2
    ...
    Hence, \( q(\xi, \eta, \zeta) \) is reducible.

    \item If exactly one of \( \alpha \), \( \beta \) or \( \gamma \) is zero, we are left with a homogeneous quadratic polynomial of two variables. Without loss of generality, suppose that \( \gamma = 0 \). Then
    \begin{equation*}
      \alpha \xi^2 + \beta \eta^2 = \parens[\Big]{ \sqrt \alpha \xi - \sqrt {-\beta} \eta } \parens[\Big]{ \sqrt \alpha \xi + \sqrt {-\beta} \eta }.
    \end{equation*}

    Hence, \( q(\xi, \eta, \zeta) \) is reducible.

    This case, better discussed in \fullref{ex:real_algebraic_curves}, is the main reason we require fields to be algebraically closed in the definition of affine variety in \fullref{def:affine_algebraic_set}.

    \item If \( \alpha \), \( \beta \) and \( \gamma \) are all nonzero, \fullref{ex:def:domain_divisibility/x2_plus_y2_plus_z2} demonstrates that \( q(\xi, \eta, \zeta) \) is irreducible.
  \end{itemize}

  Therefore,
  \begin{itemize}
    \item \( p(X, Y, 1) \) is irreducible if and only if \( p(X, Y, Z) \) is.
    \item \( p(X, Y, Z) \) is irreducible if and only if \( q(\xi, \eta, \zeta) \) is.
    \item \( q(\xi, \eta, \zeta) \) is irreducible if and only if the diagonal matrix \( \op{diag}(\alpha, \beta, \gamma) \) is invertible.
    \item \( \op{diag}(\alpha, \beta, \gamma) \) is invertible if and only if \( A \) is because they have the same determinant.
  \end{itemize}
\end{proof}
