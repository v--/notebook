...
\end{theorem}
\begin{proof}
  Let \( \{ x_k \}_{k=1}^\infty \) be a bounded sequence in \( \BR \) and let \( a \leq b \) be a lower and upper bound\Tinyref{def:poset/upper_lower_bound}, respectively. Construct the sequence \( \{ F_k \}_{k=1}^\infty \) of closed intervals as follows: define \( \alpha_1 \coloneqq a \) and \( \beta_1 \coloneqq b \) and, at step \( k = 1, 2, \ldots \), put
  \begin{align*}
...
\begin{proof}
  ...
  Fix \( \varepsilon > 0 \). Then by \fullref{thm:metric_topology_convergence/limit_point}, there exist indices \( k_0 \) and \( m_0 \) such that
  \begin{align*}
...
\begin{proof}\mbox{}
  \begin{description}
    \RItem{thm:squeeze_lemma/sequences} Fix \( \varepsilon > 0 \). Then by \fullref{thm:metric_topology_convergence/limit_point}, there exist indices \( k^- \) and \( k^+ \) such that
    \begin{align*}
    ...
    \Fullref{thm:metric_topology_convergence/limit_point} is satisfied, hence \( \{ x_k \} \) converges to \( \Ol x \).

    \RItem{thm:squeeze_lemma/functions} The proof is analogous to that of \fullref{thm:squeeze_lemma/sequences}, but the machinery is different. Fix \( \varepsilon > 0 \). Then by \fullref{def:local_convergence/balls}, there exist radii \( \delta^- \) and \( \delta^+ \) such that
    \begin{align*}
    ...
    \end{equation*}
    holds and thus by \fullref{def:local_convergence/balls}, the function \( f \) converges to \( \Ol y \) at \( \Ol x \).
  \end{description}
\end{proof}
...
\begin{proof}\mbox{}
  \begin{description}
    \Implies Let \( \{ x_k \}_{k=1}^\infty \) be a convergent monotone sequence. Denote its limit by \( x \). Fix \( \varepsilon > 0 \). By \fullref{thm:metric_topology_convergence/limit_point}, there exists \( k_0 \) such that
    \begin{equation*}
    ...
    Thus \fullref{thm:metric_topology_convergence/limit_point} is satisfied and the sequence \( \{ x_k \}_{k=1}^\infty \) converges.
  \end{description}
\end{proof}
