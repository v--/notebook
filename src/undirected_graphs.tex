...
\begin{definition}\label{def:undirected_multigraph_path}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:undirected_multigraph_path/subpath} We say that the \hyperref[def:subsequence]{subsequence} \( q \) of \( p \) is a \term{subpath} of \( p \) if the edges of \( q \) are consecutive in \( p \), i.e. there exists some index \( n \geq 0 \) such that \( q_k = e_{n + k} \) for \( 0 < k < \len(q) \).
