...
\begin{definition}\label{def:free_monoid}
  We associate with every \hyperref[def:set]{plain set} \( S \) its \term{free monoid} \( (S^*, \cdot) \), where \( S^* \) is the \hyperref[def:formal_language/kleene_star]{Kleene star} and \( \cdot \) is \hyperref[def:formal_language/concatenation]{concatenation}.

  Denote by \( \iota_S: S \to F(S) \) the canonical inclusion function, which sends every member of \( S \) into the corresponding single-symbol word in the \hyperref[def:free_monoid]{free monoid} \( F(S) \).
\end{definition}
\begin{proof}
...
\begin{proposition}\label{thm:free_monoid_universal_property}
  We associate with every set \( S \) its \hyperref[def:formal_language/kleene_star]{Kleene star} \( S^* \). Denote by \( \iota_S: S \to F(S) \) the canonical inclusion function, which sends every member of \( S \) into the corresponding single-symbol word in \( S^* \).

  The Kleene star \( S^* \) with \hyperref[def:formal_language/concatenation]{concatenation} is the unique up to an isomorphism monoid that satisfies the following \hyperref[rem:universal_mapping_property]{universal mapping property}:
  \begin{displayquote}
    For every monoid \( M \) and every function \( f: S \to M \), there exists a unique monoid homomorphism \( \widetilde{f}: S^* \to M \) such that the following diagram commutes:
...
\end{proposition}
\begin{proof}
  For every function \( f: S \to M \), define the monoid homomorphism
  \begin{equation*}
    \begin{aligned}
      &\widetilde{f}: S^* \to M, \\
...
\begin{proposition}\label{thm:free_group_universal_property}
  The \hyperref[def:free_group]{free group} \( F(S) \) is the unique up to an isomorphism group that satisfies the following \hyperref[rem:universal_mapping_property]{universal mapping property}:
  \begin{displayquote}
    For every group \( G \) and every function \( f: S \to G \), there exists a unique group homomorphism \( \widetilde{f}: F(S) \to G \) such that the following diagram commutes:
...
\begin{definition}\label{def:group_presentation}
  A \term{presentation} of the \hyperref[def:group]{group} \( G \) is an \hyperref[def:group/homomorphism]{epimorphism} \( \varphi: F(S) \to M \), where \( F(S) \) is the \hyperref[def:free_group]{free group} over \( S \) (technically \( \varphi \) should be a surjective homomorphism, but in \fullref{thm:group_epimorphisms_are_surjective} it will follow that epimorphisms are surjective homomorphisms).

  By \fullref{thm:quotient_group_by_kernel},
  \begin{equation*}
    G = \img \varphi \cong F(S) / \ker \varphi.
  \end{equation*}

  \Fullref{thm:free_group_universal_property} guarantees the existence of at least one presentation for every group.

  The members of \( S \) are called \term{generators} and the members of \( \ker \varphi \) are called \term{relators}. If only a subset \( R \) of all relators is given, and if \( \ker \varphi \) is the smallest normal subgroup containing the relators, we write
  \begin{equation*}
    G \cong \braket{ S \given R }.
  \end{equation*}

  The above notation can also be used to define groups when \( S \) is a \hyperref[def:set]{plain set}. This is utilized in \fullref{def:cyclic_group}.

  We call the group \( G \) \term{finitely presented} if there exists a presentation \( \braket{ S \mid R } \) such that both \( S \) and \( R \) are \hyperref[def:set_finiteness]{finite}, \term{finitely generated} if \( S \) is finite and \term{finitely related} if \( R \) is finite.

  Compare this to \fullref{def:module_presentation} and \fullref{def:algebra_presentation}.
\end{definition}
