...
\begin{definition}\label{def:free_monoid}
  Let \( A \) be an arbitrary set. We associate with \( A \) its \term{free monoid} \( F(A) \coloneqq (A^*, \cdot) \), where \( A^* \) is the \hyperref[def:formal_language/kleene_star]{Kleene star} and \( \cdot \) is \hyperref[def:formal_language/concatenation]{concatenation}.
\end{definition}
\begin{proof}
...
\begin{definition}\label{def:group_presentation}\mcite[314]{Knapp2016BasicAlgebra}
  Let \( S \) be a set, \( F(S) \) be the \hyperref[def:free_group]{free group} and \( R \subseteq F(S) \) be a subset. Denote by \( \mscrN(R) \) the \hyperref[def:first_order_generated_substructure]{generated} by \( R \) \hyperref[def:normal_subgroup]{normal subgroup}.
  ...
  We call the pair \( (S, R) \) \eqref{eq:def:group_presentation/presentation} a \term{presentation} of \( \braket{ S \mid R } \), although we usually denote the presentation itself via \( \braket{ S \mid R } \).

  Note that the free group is generated by a set, which is different from generated subgroups defined in \fullref{def:first_order_generated_substructure}.

  We say that the group \( \mscrG \) is \term{finitely generated} if it has a presentation \( \braket{ S \mid R } \) such that \( S \) is a \hyperref[def:set_finiteness]{finite set}. If only \( R \) is finite, we call \( \mscrG \) \term{finitely presented}.
...
\begin{proof}
  ...
  By \fullref{thm:free_group_universal_property}, there exists a unique group homomorphism \( \varphi: F(S) \to \mscrG \) such that
  \begin{equation*}
    U(\varphi) \bincirc \iota_S = \id_S.
  \end{equation*}

  The kernel \( \ker \varphi \) is a normal subgroup of \( \mscrG \), hence by \fullref{thm:homomorphism_theorem_for_groups},
  \begin{equation*}
...
\begin{definition}\label{def:group_free_product}\mcite[323]{Knapp2016BasicAlgebra}
  The \term{free product} of a nonempty pairwise disjoint family of groups \( \seq{ \mscrX_k }_{k \in \mscrK} \) with \hyperref[def:group_presentation]{presentations} \( \braket{S_k \mid R_k}, k \in \mscrK \) is the group with presentation
  \begin{equation*}
    \Ast_{k \in \mscrK} \mscrX_k \coloneqq \braket*{ \bigcup_{k \in \mscrK} S_k \given[\Big] \bigcup_{k \in \mscrK} R_k }.
  \end{equation*}

  If the constituent groups are not disjoint, we may instead use \hyperref[def:disjoint_union]{disjoint unions} as
  \small
  \begin{equation*}
    \Ast_{k \in \mscrK} \mscrX_k \coloneqq \braket*{ \set[\Big]{ (k, x) \given k \in \mscrK \T{and} x \in S_k } \given[\Big] \set[\Big]{ (k, x_1) (k, x_2) \ldots (k, x_n) \colon k \in \mscrK \T{and} x_1 x_2 \ldots x_n \in R_k } }.
  \end{equation*}
  \normalsize
\end{definition}

\begin{definition}\label{def:group_direct_product}
  The \term{direct product} of a nonempty family of groups \( \seq{ \mscrX_k }_{k \in \mscrK} \) is their \hyperref[def:cartesian_product]{Cartesian product} \( \prod_{k \in \mscrK} \mscrX_k \) with the componentwise group operation
  \begin{equation*}
...
\begin{definition}\label{def:group_order}
  The \term{order} \( \ord(\mscrG) \) of a group \( \mscrG \) is its \hyperref[thm:cardinality_existence]{cardinality}.
...
\begin{proposition}\label{thm:group_categorical_limits}
  \hfill
  \begin{thmenum}
    \thmitem{thm:group_categorical_limits/product} The \hyperref[def:discrete_category_limits]{categorical product} of the family \( \seq{ \mscrG_k }_{k \in \mscrK} \) is their \hyperref[def:group_direct_product]{direct product} \( \prod_{k \in \mscrK} \mscrG_k \), the projection morphisms being
    \begin{equation*}
      \begin{aligned}
        &\pi_m: \prod_{k \in \mscrK} \mscrG_k \to \mscrG_m \\
        &\pi_m(\seq{ x_k }_{k \in \mscrK}) \coloneqq x_m.
      \end{aligned}
    \end{equation*}

    \thmitem{thm:group_categorical_limits/coproduct} The \hyperref[def:discrete_category_limits]{categorical coproduct} of the family \( \seq{ \mscrG_k }_{k \in \mscrK} \) is the \hyperref[def:group_free_product]{free product} \( \Ast_{k \in \mscrK} \mscrX_k \), the inclusion morphisms being
    \begin{equation*}
      \begin{aligned}
...
\begin{proof}
  ...
  \SubProofOf{thm:group_categorical_limits/coproduct}  Let \( (A, \alpha) \) be a \hyperref[def:category_of_cones/cocone]{cocone} for the discrete diagram \( \seq{ \mscrG_k }_{k \in \mscrK} \). We want to define a group homomorphism \( l: \Ast_{k \in \mscrK} \mscrG_k \to A \) such that, for every \( m \in \mscrK \),
  \begin{equation*}
