...
\begin{proof}
  ...
  \ExistenceSubProof Suppose that \( x \) has no normal form. Hence, it is reducible and has an immediate descendant \( x_1 \). Proceeding by \hyperref[rem:natural_number_recursion]{natural number recursion}, we can build a sequence
  \begin{equation*}
  ...
  \UniquenessSubProof Suppose that both \( y \) and \( z \) are normal forms of \( w \).
