...
\begin{proposition}\label{thm:free_monoid_universal_property}
  ...
  Via \fullref{rem:universal_mapping_property}, \( F \) becomes \hyperref[def:category_adjunction]{left adjoint} to the \hyperref[def:concrete_category]{forgetful functor} \( U: \cat{Set} \to \cat{Mon} \).
\end{proposition}
\begin{proof}
...
\begin{definition}\label{def:free_group}
  ...
  Compare this definition to free abelian groups defined in \fullref{def:free_abelian_group}.
\end{definition}
\begin{defproof}
...
\begin{proposition}\label{thm:free_group_universal_property}
  ...
  Via \fullref{rem:universal_mapping_property}, \( F \) becomes \hyperref[def:category_adjunction]{left adjoint} to the \hyperref[def:concrete_category]{forgetful functor} \( U: \cat{Set} \to \cat{Grp} \).
\end{proposition}
\begin{proof}
...
\begin{definition}\label{def:group_presentation}\mcite[314]{Knapp2016BasicAlgebra}
  ...
  The group with \term{generators} \( A \) and \term{relators} \( R \) is
  ...
  Note that the free group is generated by a set, which is different from generated subgroups defined in \fullref{def:group/submodel}. The former defines a new group operation from scratch, while the latter uses an existing group operation and is restricted by this operation.

  We say that the group \( G \) is \term{finitely generated} if it is isomorphic to \( \braket{ S \mid R } \), where \( A \) is a \hyperref[def:set_finiteness]{finite set}. If only \( R \) is finite, we call \( G \) \term{finitely presented}.
...
\begin{definition}\label{def:group_direct_product}
  The \term{direct product} of a family of groups \( \seq{ G_k }_{k \in \mscrK} \) is their \hyperref[def:cartesian_product]{Cartesian product} \( \prod_{k \in \mscrK} G_k \) with the componentwise group operation
  \begin{equation*}
...
\begin{proposition}\label{thm:group_categorical_limits}
  \hfill
  \begin{thmenum}
    \thmitem{thm:group_categorical_limits/product} The \hyperref[def:discrete_category_limits]{categorical product} of the family \( \seq{ G_k }_{k \in \mscrK} \) in \( \cat{Grp} \) is their \hyperref[def:group_direct_product]{direct product} \( \prod_{k \in \mscrK} G_k \), the projection morphisms being
    \begin{equation*}
      \begin{aligned}
    ...
    \thmitem{thm:group_categorical_limits/coproduct} The \hyperref[def:discrete_category_limits]{categorical coproduct} of the family \( \seq{ G_k }_{k \in \mscrK} \) in \( \cat{Grp} \) is their \hyperref[def:group_free_product]{free product} \( \Ast_{k \in \mscrK} X_k \), the inclusion morphisms being
    \begin{equation*}
      \begin{aligned}
    ...
    Compare this to \fullref{thm:abelian_group_categorical_limits/coproduct}.
  \end{thmenum}
\end{proposition}
\begin{proof}
