...
\begin{definition}\label{def:minimal_hypergraph_transversal}
  ...
  \begin{thmenum}
    \thmitem{def:minimal_hypergraph_transversal/order} \( T \) is a minimal \hyperref[def:preordered_set/maximal_and_minimal_element]{element} under \hyperref[rem:subset_and_membership_relations]{set inclusion} in the set of all transversals of \( H \).
...
\begin{proof}
  ...
  Assume\DNE that \( T \) is not minimal and let \( y \in T \) be such that \( T \setminus \{ y \} \) is a transversal. But our assertion gives us an edge \( E_y \in \mathcal{F} \) such that \( T \cap E_y = \{ y \} \). Clearly the set \( T \setminus \{ y \} \) cannot be a transversal of \( H \) since
  \begin{equation*}
...
\end{proposition}
\begin{proof}
  A minimal transversal must exist\DNE because the vertex set is a transversal and we can only remove finitely many elements from it.
\end{proof}
