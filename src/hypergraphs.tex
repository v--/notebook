...
\begin{definition}\label{def:hypergraph}
  Fix a \hyperref[def:set]{set} \( V \), whose members we will call \term{vertices} or \term{nodes}, and a set \( E \), whose members we will call \term{hyperedges}.
  ...
  \begin{figure}[h]
    \begin{equation*}
      \begin{aligned}
        \includegraphics{figures/fig__def__hypergraph.pdf}
      \end{aligned}
    \end{equation*}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:hypergraph/submodel} A \term{sub-hypergraph} of \( H: E \multto V \) is a \hyperref[def:multi_valued_function/restriction]{function restriction} of \( H \). That is, \( H': E' \multto V' \) is a sub-hypergraph of \( H \) if \( V' \subseteq V \), \( E' \subseteq E \) and
    \begin{equation}\label{eq:def:theory_of_graphs/submodel}
      \img(H\restr_{E'}) = \set{ v \in V \given \qexists {e \in E'} e \T{is incident to} v } \subseteq V'.
    \end{equation}

    We say that sub-hypergraph \( H' \) is \term{full} if equality holds in \eqref{eq:def:theory_of_graphs/submodel}. We also say that \( V' \) \term{induces} the sub-hypergraph \( H' \).
    ...
    An unambiguous concept is that of an \term{edgeless hypergraph}, in which the set of hyperedges is empty, but the set of vertices may or may not be empty. Every hypergraph \( H: E \to V \) has \( 2^{\ord(V)} \) edgeless subhypergraphs (one for each subset of \( V \)).
...
\begin{example}\label{ex:hypergraphs_not_induced_by_family}
  The hypergraph \cref{fig:def:hypergraph} is a counterexample to the converse of \fullref{thm:family_of_sets_induces_hypergraph}. It cannot be represented as a family of sets because the edges \( e_2 \) and \( \widehat{e_2} \) are \hyperref[def:hypergraph/parallel_hyperedges]{parallel} and would be represented by the same set.
...
\begin{definition}\label{def:category_of_small_hypergraphs}
  ...
  \begin{itemize}
    \item The \hyperref[def:category/objects]{set of objects} \( \obj(\cat{HypGph}) \) is the set of all \( \mscrU \)-small hypergraphs, i.e. the hypergraphs \( H: E \multto V \) such that \( V \) and \( E \) are both members of \( \mscrU \).
    ...
    \item The \hyperref[def:category/identity]{identity morphism} of the hypergraph \( H: E \multto V \) is the pair of \hyperref[def:multi_valued_function/identity]{identity functions} \( (\id_V, \id_H) \).
  \end{itemize}
\end{definition}

\begin{definition}\label{def:hypergraph_incidence_matrix}
  Let \( H: E \multto V \) be a finite hypergraph. Its \term{incidence matrix}
  \begin{equation*}
...
\begin{definition}\label{def:hypergraph_vector_spaces}
  Let \( H: E \multto V \) be a \hyperref[def:graph]{hypergraph}. We introduce several \hyperref[def:vector_space]{vector spaces} that allow us to study hypergraphs using linear algebra.
  ...
  \begin{thmenum}
    ...
    The space is motivated by \hyperref[def:undirected_graph_path]{undirected paths} and their \hyperref[def:undirected_graph_path/characteristic_vector]{characteristic vectors}.
  \end{thmenum}
\end{definition}

\begin{proposition}\label{thm:graphs_as_linear_transformations}
  Let \( V = \set{ 1, \ldots, m } \) and \( E = \set{ 1, \ldots, n } \). Then there is a bijection between the \( m \times n \) \hyperref[def:matrix]{matrices} over \( \BbbF_2 \) and the \hyperref[def:hypergraph]{hypergraphs} \( H: E \multto V \).
\end{proposition}
\begin{proof}
  The \hyperref[def:hypergraph_incidence_matrix]{incidence matrix} of every hypergraph is a matrix in \( \BbbF_2^{m \times n} \).

  Conversely, let \( M = \seq{ M_{ve} } \in \BbbF_2^{m \times n} \) and define the hypergraph
  \begin{equation*}
    \begin{aligned}
      &H: E \multto V \\
      &H(e) \coloneqq \set{ v \in 1, \ldots, n \given M_{ve} = 1 }.
    \end{aligned}
  \end{equation*}
...
\begin{definition}\label{def:hypergraph_transversal}\mcite[32]{GondranMinoux1984Graphs}
  Let \( H: E \multto V \) be a \hyperref[def:hypergraph]{hypergraph}. We say that a set \( T \subseteq V \) of vertices is a \term{transversal} of \( H \) if it is incident to every edge of \( H \).
\end{definition}
...
\begin{definition}\label{def:hypergraph_minimal_transversal}
  A transversal \( T \) of the hypergraph \( H: E \multto V \) is said to be \term{minimal} if any of the following equivalent conditions hold:
  \begin{thmenum}
...
\begin{proof}
  ...
  Fix a vertex \( v \in T \). Since \( T \) is minimal, the set \( T \setminus \set{ v } \) is not a transversal. So there exists a hyperedge \( e_v \in E \) such that \( (T \setminus \set{ v }) \cap H(e_v) = \varnothing \).

  Now since \( T \) is a transversal for \( H \), the set \( T \cap H(e_v) \) is nonempty and thus
  \begin{align*}
    T \cap H(e_x)
    &=
    \parens[\Big]{ (T \setminus \set{ v }) \cup \set{ v } } \cap H(e_v)
    \reloset {\eqref{eq:def:semilattice/distributive_lattice/finite/meet_over_join}} = \\ &=
    \underbrace{\parens[\Big]{ (T \setminus \set{ v }) \cap H(e_v) }}_{\varnothing} \cup \parens[\Big]{ \set{ v } \cap H(e_v) }
    = \\ &=
    \set{ v }.
  \end{align*}

  \ImplicationSubProof{def:hypergraph_minimal_transversal/singleton}{def:hypergraph_minimal_transversal/order} Now suppose that for every vertex \( v \in T \) there exists a hyperedge \( e_v \in E \) such that \( T \cap H(e_v) = \set{ v } \).

  Suppose that \( T \) is not minimal. Then there exists some vertex \( u \in T \) be such that \( T \setminus \set{ u } \) is a transversal. But our assertion gives us an edge \( e_u \in E \) such that \( T \cap H(e_u) = \set{ u } \). Clearly the set \( T \setminus \set{ u } \) cannot be a transversal of \( H \) since
  \begin{equation*}
    (T \setminus \set{ u }) \cap H(e_u) = \varnothing.
  \end{equation*}
...
\begin{proof}
  ...
  \ImplicationSubProof[def:zfc/choice]{the axiom of choice}[thm:hypergraphs_have_minimal_transversal]{minimal transversal existence} Let \( H: E \multto V \) be a hypergraph. Then \( E \) is a family of nonempty sets and thus there exists a set \( B \) such that \( B \cap H(e) \) for every hyperedge \( e \in E \). This is a minimal transversal by \fullref{def:hypergraph_minimal_transversal/singleton}.

  \ImplicationSubProof[thm:hypergraphs_have_minimal_transversal]{minimal transversal existence}[def:zfc/choice]{axiom of choice} Let \( \mscrA \) be an arbitrary family of nonempty sets. Then \( H \coloneqq \parens*{ \bigcup \mscrA, \mscrA } \) is a hypergraph. Let \( T \) be a minimal transversal of \( H \). Then, by definition, for every hyperedge \( e \in \mscrA \), the intersection \( T \cap H(e) \) is a singleton set. Hence, \( T \) is the image of a \hyperref[def:choice_function]{choice function} for \( \mscrA \).
\end{proof}
