...
\begin{definition}\label{def:hypergraph_vector_spaces}
  ...
  \begin{thmenum}
    \thmitem{def:hypergraph_vector_spaces/vertex} The \term{vertex space} \( \BbbZ_2^V \) is the \hyperref[thm:functions_over_semimodule]{function space} \( \BbbZ_2^V \), where \( \BbbZ_2 \) is the \hyperref[thm:ring_of_integers_modulo_zero_divisors]{two-element field}.
    ...
    This vector is called the \term{characteristic vector} of \( U \). Conversely, every vector in \( \BbbZ_2^V \) induces a set of vertices. If \( U \) consists of a single vertex \( u \), we write \( \vect{u} \) rather than \( \vect{\set{u}} \).
    ...
    \thmitem{def:hypergraph_vector_spaces/edge} Analogously, the \term{hyperedge space} \( \BbbZ_2^E \) is the \hyperref[thm:functions_over_semimodule]{function space} \( \BbbZ_2^E \). Every subset of \( E \) induces a unique characteristic vector in \( \BbbZ_2^E \) and vice versa.
