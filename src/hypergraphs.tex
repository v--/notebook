...
\begin{definition}\label{def:hypergraph}
  ...
  \begin{thmenum}
    \thmitem{def:hypergraph/rank} The \term{rank} of a hyperedge \( e \) is the \hyperref[thm:cardinality_existence]{cardinality}
    \begin{equation*}
      \rank(e) \coloneqq \card(H(e))
    \end{equation*}
    ...
    \thmitem{def:hypergraph/parallel_hyperedges} We say that the hyperedges \( e_1 \) and \( e_2 \) are parallel if they have the same endpoints.
    ...
    Similarly, we say that \( e_1 \) and \( e_2 \) are \term{adjacent hyperedges} if they have a common endpoint.
...
\begin{definition}\label{def:category_of_small_hypergraphs}
  ...
  We denote the \hyperref[def:category]{category} of \( \mscrU \)-small \hyperref[def:hypergraph]{hypergraphs} by \( \cat{\mscrU-HypGraph} \) or, if the universe is clear from the context, simply by \( \cat{HypGraph} \).

  \begin{itemize}
    \item The \hyperref[def:category/objects]{set of objects} \( \obj(\cat{HypGraph}) \) is the set of all \( \mscrU \)-small hypergraphs, i.e. the hypergraphs \( H: E \multto V \) such that \( E \) and \( V \) are members of \( \mscrU \).

    \item The \hyperref[def:category/morphisms]{set of morphisms} \( \cat{Set}(H, G) \) from \( H \) to \( G \) is the set hypergraph homomorphisms. Given two hypergraphs \( H: E_H \multto V_H \) and \( G: E_G \multto V_G \), a \term{hypergraph homomorphism} is a pair of functions
    ...
    \begin{equation*}
      (f_V, f_E) \in \cat{HypGraph}(A, B) \T{and} (g_V, g_E) \in \cat{HypGraph}(B, C)
    \end{equation*}
    is the morphism
    \begin{equation*}
      (g_V \bincirc f_V, g_E \bincirc f_E) \in \cat{HypGraph}(A, C).
    \end{equation*}
