...
\begin{definition}\label{def:hypergraph_vector_spaces}
  ...
  \begin{thmenum}
    \thmitem{def:hypergraph_vector_spaces/vertex} The \term{vertex space} \( \BbbF_2^V \) is the \hyperref[thm:functions_over_semimodule]{function space} \( \BbbF_2^V \), where \( \BbbF_2 \) is the \hyperref[thm:ring_of_integers_modulo_zero_divisors]{two-element field}.
    ...
    \thmitem{def:hypergraph_vector_spaces/edge} Analogously, the \term{hyperedge space} \( \BbbF_2^E \) is the \hyperref[thm:functions_over_semimodule]{function space} \( \BbbF_2^E \). Every subset of \( E \) induces a unique characteristic vector in \( \BbbF_2^E \) and vice versa.
