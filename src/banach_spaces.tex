...
\begin{definition}\label{def:banach_space_support_function}\mcite\cite[exmpl. 3.2(a)]{Phelps1993})
  ...
  \begin{balign*}
     ...
     & \sigma_{A^*}(x) \coloneqq \sup \{ \inner {x^*} x \colon x^* \in A^* \}
  \end{balign*}

  and the \term{weak* support function \( \sigma^*_A \) for the set of points \( A \subseteq X \)} by
  \begin{balign*}
     & \sigma^*_A: X^* \to \BbbR \cup \{ \infty \}                          \\
     & \sigma^*_A(x^*) \coloneqq \sup \{ \inner {x^*} x \colon x \in A \}.
  \end{balign*}
\end{definition}

\begin{definition}\label{def:banach_space_slice}\mcite\cite[def. 2.17]{Phelps1993}
  ...
  \begin{balign*}
     ...
     & S(x^*, A, \alpha) \coloneqq \{ x \in A \colon \inner {x^*} x > \sigma_A^*(x^*) - \alpha \}.
  \end{balign*}

  We define a weak* slice of \( A^* \subseteq X^* \) as \( S^*(x, A^*, \alpha) \), where
  \begin{balign*}
     & S^*: X \times \pow(X) \times \BbbR_{>0} \mapsto \pow(A)                                            \\
     & S^*(x, A^*, \alpha) \coloneqq \{ x^* \in A^* \colon \inner {x^*} x > \sigma_{A^*}(x) - \alpha \}.
  \end{balign*}
