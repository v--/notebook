...
\begin{example}\label{ex:noncomplete_normed_space}\cite{MathCounterExamples:noncomplete_normed_space}
  Consider the polynomial algebra\Tinyref{def:algebra_of_polynomials} \( \BR[x] \) as a vector space with the supremum norm. We will show that it is not complete. Define the sequence
  \begin{equation*}
...
\begin{definition}\label{def:banach_space_slice}\cite[definition 2.17]{Phelps1993}
  ...
  \begin{align*}
    &S: X^* \times \Power(X) \times \BR_{>0} \mapsto \Power(A) \\
...
\begin{proposition}
  If \( \{ a_k \}_{k=1}^\infty \) and \( \{ b_k \}_{k=1}^\infty \) are sequences a in a Banach\Tinyref{def:banach_space} algebra\Tinyref{def:algebra_over_ring} \( X \), that converge to \( a \) and \( b \), correspondingly, then \( a_k b_k \to a b \).
\end{proposition}
\begin{proof}
