...
\begin{proposition}\label{thm:finite_extensions_are_algebraic}
...
\begin{proof}
  ...
  By \fullref{thm:finite_extensions_are_algebraic}, \( Q \) is an algebraic extension of \( \BbbK \). Since \( \BbbK \) has no nontrivial algebraic extensions, it follows that \( \BbbK = Q \). Thus, \( Q \) is unidimensional, and we have already discussed that \( \dim Q = \deg p \). Therefore, \( p \) is a linear polynomial.
...
\end{proposition}
\begin{proof}
  \SubProof{Proof of uniqueness} Let \( \BbbK \) and \( \BbbL \) be two algebraically closed algebraic extensions of \( \Bbbk \).
...
\end{proposition}
\begin{proof}
  Follows from \fullref{thm:finite_extensions_are_algebraic} applied to \fullref{def:algebraically_closed_field/trivial_algebraic_extensions}.
\end{proof}
