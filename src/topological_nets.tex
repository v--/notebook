...
\begin{definition}\label{def:topological_net}
  A \Def{net} or \Def{generalized \hyperref[def:sequence]{sequence}} or \Def{Moore-Smith sequence} in a nonempty set \( S \) is a family of elements of \( S \) indexed by a nonempty \hyperref[def:indexed_family]{directed set}, i.e. a function from a nonempty directed \hyperref[def:directed_set]{set} \( (\CK, \leq) \) to \( S \). We use the conventional notation for indexed sets (with the caveats described in \fullref{remark:indexed_family_notation}):
  \begin{equation*}
    \{ x_k \}_{k \in \CK},
  \end{equation*}
