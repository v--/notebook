...
\begin{definition}\label{def:topological_net}
  A \Def{net} or \Def{generalized sequence}\Tinyref{def:sequence} or \Def{Moore-Smith sequence} in a nonempty set \( S \) is a family of elements of \( S \) indexed by a directed set\Tinyref{def:indexed_family}, i.e. a function from a directed set\Tinyref{def:order/directed} \( (\CA, \leq) \) to \( S \). We use the conventional notation for indexed sets (with the caveats described in \fullref{remark:indexed_family_notation}):
  \begin{equation*}
    \{ x_\al \}_{\al \in \CA},
  \end{equation*}
...
\begin{proof}
  ...
  \begin{description}
    ...
    If \( x \in \Bd{A} \), by \fullref{def:topological_boundary/neighborhoods}, every neighborhood of \( x \) contains points from \( A \). Therefore we can build reverse inclusion net in the style of \fullref{ex:reverse_inclusion_net} that converges to \( x \).
    ...
    Let \( F \supseteq A \) be a closed set. Denote \( U \coloneqq X \setminus F \). Suppose\LEM that \( x \in U \). Then \( U \) is a neighborhood \( x \) and, by \fullref{def:net_convergence/cluster}, the net \( \{ x_\al \}_{\al \in \CA} \subseteq A \) is eventually in \( U \). But \( U \) does not contains \( A \).
