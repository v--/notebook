...
\begin{proof}
  ...
  \begin{description}
    ...
    If \( x \in \partial{A} \), by \cref{def:topological_boundary/neighborhoods}, every neighborhood of \( x \) contains points from \( A \). Therefore we can build a net \( (x_U)_{U \in \CT(x)} \) by choosing\AOC an element \( x \in U \cap A \) for every neighborhood \( U \) of \( x \). It is then immediate that \( (x_U)_{U \in \CT(x)} \) converges to \( x \).
