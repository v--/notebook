...
\begin{definition}\label{def:topological_net}
  A \Def{net} or \Def{generalized \hyperref[sequence]{def:sequence}} or \Def{Moore-Smith sequence} in a nonempty set \( S \) is a family of elements of \( S \) indexed by a nonempty directed \hyperref[def:indexed_family]{set}, i.e. a function from a nonempty directed \hyperref[def:directed_set]{set} \( (\CA, \leq) \) to \( S \). We use the conventional notation for indexed sets (with the caveats described in \fullref{remark:indexed_family_notation}):
  \begin{equation*}
    \{ x_\al \}_{\al \in \CA},
  \end{equation*}
...
\begin{proposition}\label{thm:topological_net_properties}
  ...
  \begin{propenum}
    \DItem{thm:topological_net_properties/eventually_in_implies_frequently_in} \enquote{\hyperref[def:topological_net/eventually_in]{Eventually in}} implies \enquote{\hyperref[def:topological_net/frequently_in]{frequently in}}.
...
\begin{proof}\mbox{}
  \begin{description}
    ...
    \begin{description}
      ...
      By the definition of \( \varphi \) (see \fullref{def:topological_net/subnet}), given an index \( \alpha_0 \in \CA \), there exist \( \beta_0 \in \CB \) such that \( \varphi(\beta_0) \geq \alpha_0 \)
