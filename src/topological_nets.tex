...
\begin{definition}\label{def:topological_net}
  A \Def{net} or \Def{generalized sequence}\Tinyref{def:sequence} or \Def{Moore-Smith sequence} in a nonempty set \( S \) is a family of elements of \( S \) indexed by a nonempty directed set\Tinyref{def:indexed_family}, i.e. a function from a nonempty directed set\Tinyref{def:directed_set} \( (\CA, \leq) \) to \( S \). We use the conventional notation for indexed sets (with the caveats described in \fullref{remark:indexed_family_notation}):
  \begin{equation*}
    \{ x_\al \}_{\al \in \CA},
  \end{equation*}
  ...
  \begin{defenum}
    ...
    \begin{defenum}
      \Ditem{def:topological_net/subnet/directed} To every \( \al \in \CA \) there corresponds \( \beta \in \CB \) such that \( \varphi(\beta) \geq \al \).
      \Ditem{def:topological_net/subnet/identity} For every \( \beta \in \CB \) we have \( x_{\varphi(\beta)} = y_\beta \).
    \end{defenum}
  \end{defenum}
\end{definition}
...
\begin{proposition}\label{thm:cluster_point_of_set_iff_limit_point_of_net}
  The point \( x_0 \in X \) is a cluster point\Tinyref{def:topological_derived_set/cluster_point} of the set \( A \) if and only if it is a limit point\Tinyref{def:net_convergence/cluster} of some net in \( A \setminus \{ x_0 \} \) (or, equivalently, a cluster point of some net in \( A \setminus \{ x_0 \} \)).
\end{proposition}
\begin{proof}\mbox{}
  \begin{description}
...
\begin{definition}\label{def:sequential_space}
  A topological space is called \Def{sequential} if every sequentially closed\Tinyref{def:sequential_closure_operator} set is closed.
\end{definition}

\begin{remark}\label{remark:sequential_spaces}
  By \cref{thm:limit_point_iff_in_closure}, in a sequential space\Tinyref{def:sequential_space}, a set is closed if and only if it is sequentially closed.
  ...
  This allows us to restrict ourselves only to sequences rather than arbitrary nets in certain spaces like metric spaces\Tinyref{def:metric_space}.
\end{remark}
...
\begin{proof}
  ...
  Since \( X \) is a first countable space, we can fix a countable local base\Tinyref{def:topological_local_base} \( \{ U_k \}_{k=1}^\infty \) at \( x_0 \). For each \( k = 1, 2, \ldots \), define the neighborhood \( V_k \coloneqq \bigcap_{m=1}^k U_m \) so that \( V_k \subseteq V_m \) whenever \( k \geq m \).
  ...
  Thus we obtain a sequence \( \{ x_{\al_k} \}_{k=1}^\infty \) that is eventually in every neighborhood of the local base \( \{ V_k \}_{k=1}^\infty \) of \( x_0 \), which by \fullref{thm:net_convergence_via_subbases} is sufficient for \( x_0 \) to be a limit point of the sequence.
\end{proof}
...
\end{proposition}
\begin{proof}
  Let \( X \) be a first-countable space and let \( A \subseteq X \) be a sequentially closed\Tinyref{def:sequential_closure_operator} set. We must show that it is closed.
