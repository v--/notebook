...
\begin{definition}\label{def:clarke_gradient}\cite[definition 10.3]{Clarke2013}
  ...
  We use the Clarke generalized derivative\Tinyref{def:nonsmooth_derivatives/clarke},
  \begin{equation*}
    f^\circ(x)(h) \coloneqq \limsup_{\substack{y \to x \\ t \downarrow 0}} \frac {f(y + th) - f(y)} t
  \end{equation*}

  We define the \Def{generalized gradient of \( f \) at \( x \)} to be the set
  \begin{equation*}
    \partial_C f(x) \coloneqq \{ x^* \in X^* \colon \forall h \in X, f^\circ(x)(h) \geq \Prod {x^*} h \}.
  \end{equation*}

  We say that the vector \( h \) is a \Def{descent direction of \( f \) at \( x \)} if
  \begin{equation*}
