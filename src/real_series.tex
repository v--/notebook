...
\begin{theorem}[Riemann's series permutation theorem]\label{thm:riemanns_series_permutation_theorem}\MarginCite[\textnumero 247]{Фихтенгольц1968/2}
  If the real series
  \begin{equation*}
    \sum_{k=0}^\infty a_k
  \end{equation*}
...
\begin{proof}
  ...
  as follows:
  \begin{AlgEnum}
    \ILabel{thm:riemanns_series_theorem/positive} Assign to \( b_n \) only nonnegative elements of the sequence \( \{ a_k \}_{k=0}^\infty \) until \( \sum_{k=0}^n b_k \geq x \). Then go to \fullref{thm:riemanns_series_theorem/negative}.
    \ILabel{thm:riemanns_series_theorem/negative} Assign to \( b_n \) only negative elements of the sequence \( \{ a_k \}_{k=0}^\infty \) until \( \sum_{k=0}^n b_k \geq x \). Then go to \fullref{thm:riemanns_series_theorem/positive}.
  \end{AlgEnum}
...
\begin{proposition}\label{thm:positive_series_comparison}\MarginCite[\textnumero 237]{Фихтенгольц1968/2}
  ...
  Then
  \begin{PropEnum}
    \ILabel{thm:positive_series_comparison/b_converges} If \fullref{def:positive_series_comparison/b} converges, so does \fullref{def:positive_series_comparison/a}.

    \ILabel{thm:positive_series_comparison/a_diverges} If \fullref{def:positive_series_comparison/a} diverges, so does \fullref{def:positive_series_comparison/b}.
  \end{PropEnum}
\end{proposition}
\begin{proof}
...
\begin{proposition}[Cauchy's root test]\label{thm:cauchys_root_test}\MarginCite[thm. 3.33]{Rudin1976}
...
\begin{proposition}[d'Alambert's ratio test]\label{thm:dalamberts_ratio_test}\MarginCite[thm. 3.33]{Rudin1976}
  Consider the nonnegative series \fullref{def:positive_series_comparison/a}. Put
  \begin{equation*}
...
\begin{theorem}\label{thm:weierstrass_series_criterion_nessessity}\MarginCite[\textnumero 268]{Фихтенгольц1968/2}
