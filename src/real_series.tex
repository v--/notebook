...
\begin{proof}
  ...
  as follows:
  \begin{algenum}
    \DItem{thm:riemanns_series_theorem/positive} Assign to \( b_n \) only nonnegative elements of the sequence \( \{ a_k \}_{k=0}^\infty \) until \( \sum_{k=0}^n b_k \geq x \). Then go to \cref{thm:riemanns_series_theorem/negative}.
    \DItem{thm:riemanns_series_theorem/negative} Assign to \( b_n \) only negative elements of the sequence \( \{ a_k \}_{k=0}^\infty \) until \( \sum_{k=0}^n b_k \geq x \). Then go to \cref{thm:riemanns_series_theorem/positive}.
  \end{algenum}
...
\begin{proposition}\label{thm:positive_series_comparison}\cite[\textnumero 237]{Фихтенгольц1968/2}
  ...
  We say that the series \cref{def:positive_series_comparison/b} \Def{dominates} the series \cref{def:positive_series_comparison/a}.
...
\begin{proof}
  ...
  The statement of the theorem now follows from comparison (\cref{thm:positive_series_comparison}) with the geometric series\Tinyref{def:geometric_progression/series}.
\end{proof}
...
\begin{proof}
  ...
  Suppose that the limit exists. Therefore there exists an index \( N \) such that
  \begin{equation*}
    a_{k+1} \leq q a_k \quad\forall n \geq K.
  \end{equation*}

  Thus
  \begin{equation*}
    a_{K + m} \leq q^m a_K \quad\forall m \geq \Z^{\geq 0}.
  \end{equation*}

  The statement of the theorem now follows from comparison (\cref{thm:positive_series_comparison}) of the geometric series\Tinyref{def:geometric_progression/series} with the subseries of \cref{def:positive_series_comparison/a} obtained by trimming the first \( K \) elements with.
\end{proof}
