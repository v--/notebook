...
\begin{definition}\label{def:first_order_model}\MarginCite[14.35]{OpenLogic20201202}
  A structure \( \CA \) for a first-order theory \( \Gamma \) in the first-order logic language \( \CL \) is called a \Def{model} if there exists a single variable assignment \( v \) so that for every formula \( \gamma \in \Gamma \), we have \( \CA \models_v \gamma \). We write \( \CA \models_v \Gamma \) or simply \( \CA \models \Gamma \).
...
\begin{definition}\label{def:first_order_substructure}
  If \( \CA = (A, I) \) is a structure and \( B \subseteq A \), then we say that \( \CB = (B, I) \) is a \Def{substructure} of \( \CA \) if it is closed under function application, that is, for any function symbol \( f \) with arity \( n \), we have \( I(f)(B^n) \subseteq B \).
  ...
  A structure \( \CO \) is called a \Def{trivial} or \Def{zero} structure if it can be \hyperref[def:first_order_homomorphism/embedding]{embedded} into any structure for \( \CL \).
\end{definition}
...
\begin{theorem}\label{thm:first_order_generated_substructure}
  Let \( \CA = (A, I) \) be a structure over \( \CL \) and let \( \CB = (B, I) \) be a substructure. Let \( S \subseteq B \) be any set. Then the following are equivalent:
  \begin{ThmEnum}
    \ILabel{thm:first_order_generated_substructure/smallest} Out of all substructures of \( \CA \) whose universum contains \( S \), the universum \( B \) is the smallest with respect to \hyperref[def:subset]{set inclusion}.

    \ILabel{thm:first_order_generated_substructure/intersection} \( B \) is the intersection of the universums of all substructures of \( \CA \) that contain \( S \).

    \ILabel{thm:first_order_generated_substructure/induction} \( B \) is the \hyperref[def:preordered_set/largest_smallest_element]{smallest} \hyperref[def:fixed_point]{fixed point} of the operator
    \begin{AlignedEquation}\label{eq:thm:first_order_generated_substructure/induction/operator}
      &T: \Pow(A) \to \Pow(A) \\
      &T(C) \coloneqq C \cup \{ a \in A \mid \exists f \in \Func_{\CL}, \exists x_1, \ldots, x_{\#f} \in C \colon I(f)(x_1, \ldots, x_{\#f}) = a \}.
    \end{AlignedEquation}
  \end{ThmEnum}

  In all of the above cases, the set \( S \) is said to \Def[bg=поражда,ru=порождает]{generate} the substructure \( \CB \).
\end{theorem}
\begin{proof}
  Let \( \{ \CB_k \}_{k \in \CK} \) be the family of all substructures of \( \CA \) whose universums \( B_k, k \in \CK \) contain \( S \).

  \SubProofImplication{thm:first_order_generated_substructure/smallest}{thm:first_order_generated_substructure/intersection} Assume that \( \CB = (B, I) \) is the substructure in \( \{ \CB_k \}_{k \in \CK} \) such that \( B \) is minimal in \( \{ B_k \}_{k \in \CK} \). We have that
  \begin{equation*}
    \bigcap_{k \in \CK} B_k \subseteq B.
  \end{equation*}

  But since every other universum in \( \{ B_k \}_{k \in \CK} \) strictly contains \( B \), we also have the reverse inclusion
  \begin{equation*}
    \bigcap_{k \in \CK} B_k \supseteq B.
  \end{equation*}

  Therefore \( B \) is exactly the intersection of all the universums \( \{ B_k \}_{k \in \CK} \).

  \SubProofImplication{thm:first_order_generated_substructure/induction}{thm:first_order_generated_substructure/smallest} Let \( B \) be the least fixed point of the operator \( T \) defined by \eqref{eq:thm:first_order_generated_substructure/induction/operator}. Such a point exists by \fullref{thm:knaster_tarski}. We will first show that \( \CB \coloneqq (B, I) \) is a substructure of \( \CA \).

  Indeed, by definition of \( T \), \( B \) if a fixed point if and only if
  \begin{equation*}
    \{ a \in A \mid \exists f \in \Func_{\CL}, \exists x_1, \ldots, x_{\#f} \in B \colon I(f)(x_1, \ldots, x_{\#f}) = a \} \subseteq B.
  \end{equation*}

  In other words, if \( f \in \Func_{\CL} \), then for any tuple \( x_1, \ldots, x_{\#f} \), the value \( I(f)(x_1, \ldots, x_{\#f}) \) belongs to \( B \). This is precisely the \hyperref[def:first_order_substructure]{condition} for \( \CB \) to be a substructure of \( \CA \).

  Up until now, we only showed that a fixed point of \( T \) is the universum of some substructure of \( \CA \). To show the minimality of \( B \)



  \SubProofImplication{thm:first_order_generated_substructure/intersection}{thm:first_order_generated_substructure/induction} Let
  \begin{equation*}
    B \coloneqq \bigcap_{k \in \CK} B_k.
  \end{equation*}

  By \fullref{thm:first_order_substructure_properties}, \( \CB \coloneqq (B, I) \) is a substructure of \( \CA \). By \hyperref[def:first_order_substructure]{definition of a substructure}, for any functional symbol \( f \) in \( \CL \) with arity \( n \), we have
  \begin{equation*}
    I(f)(B^n) \subseteq B.
  \end{equation*}

  Therefore \( B \) is a fixed point of the induction operator \( T \) defined by \eqref{eq:thm:first_order_generated_substructure/induction/operator}.

  Let \( C \) be the smallest fixed point of \( T \). Such a point exists by \fullref{thm:knaster_tarski}. We know that
  \begin{equation*}
    S \subseteq C \subseteq B.
  \end{equation*}
...
\begin{definition}\label{def:first_order_homomorphism}\MarginCite[def. 23.8]{OpenLogic20201202}
  Let \( \CA = (A, I_{\CA}) \) and \( \Cal{B} = (B, I_{\Cal{B}}) \) be structures over a common language. We say that the \hyperref[def:function]{function} \( \varphi: A \to B \) is a \Def{homomorphism} between \( \CA \) and \( \Cal{B} \) if it preserves all functions and relations, that is, for any \( f \in \Bold{Func} \) and any \( x_1, \ldots, x_{\#f} \in A \),
  \begin{equation*}
