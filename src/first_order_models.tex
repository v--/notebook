...
\begin{proposition}\label{thm:substructures_form_complete_lattice}
  ...
  The set of all substructures of a \( (\mscrX, I) \) form a complete lattice with respect to set inclusion of domains. It is a complete \hyperref[def:semilattice/submodel]{sublattice} of the Boolean algebra \( \pow(\mscrX) \) described in \fullref{thm:boolean_algebra_of_subsets}.
...
\begin{example}\label{ex:thm:functions_over_model_of_positive_formulas_form_model}
  ...
  \begin{itemize}
    ...
    Then by \fullref{thm:functions_over_model_form_model}, the set \( \mscrB = \fun(\mscrS, \set{ T, F }) \) of all Boolean functions of arbitrary arities is again a Boolean algebra. See \fullref{thm:lindenmaum_tarski_algebra_of_full_propositional_logic/bijection} for further discussion.
