...
\begin{definition}\label{def:real_numbers}
  The \Def{real numbers} \( \BbbR \) are the metric space \hyperref[def:complete_metric_space]{completion} of \( \BbbQ \) with respect to the absolute value. Unfortunately, real numbers are used for defining metric spaces, so we cannot rely on the theory of metric spaces. This can be circumvented by
  \begin{enumerate}
...
\begin{definition}\label{def:floor_ceiling_functions}
  Let \( x \in \BbbR \) be a real number. In analogy with \fullref{def:commutative_ring_division}, we define its \Def{floor}
  \begin{equation*}
...
\begin{definition}\label{def:signum}
  We define the \Def{signum} function \( \Sign: \BbbR \to \{ -1, 0, 1 \} \) as
  \begin{equation*}
