...
\begin{definition}\label{def:set_of_rational_numbers}
  The \term{rational numbers} \( \BbbQ \) are the field of \hyperref[def:ring_localization]{fractions} of the \hyperref[def:set_of_integers]{integers}. Both operations from \( \BbbZ \) are inherited in \( \BbbQ \) and all nonzero elements in \( \BbbQ \) are now invertible, which makes \( \BbbQ \) a field.
\end{definition}
