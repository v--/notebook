...
\begin{definition}\label{def:relation}
  ...
  \end{equation*}
  are called \Def{relations}. If \( \CA \) is a finite set of cardinality \( n \)\Tinyref{remark:cardinals}, the relation is called \Def{n-ary} (\Def{binary} for \( n = 2 \), \Def{ternary} for \( n = 3 \)). If all \( X_\al \) are the same set, we say that \Def{\( \sim \) is relation on X}.
