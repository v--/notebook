...
\begin{definition}\label{def:relation}
  ...
  \end{equation*}
  are called \textbf{relations}. If \( I \) is a finite set of cardinality \( n \)\Tinyref{remark:cardinals}, the relation is called \textbf{n-ary (binary for \( n = 2 \), ternary for \( n = 3 \))}. If all \( X_i \) are the same set, we say that \textbf{\( \sim \) is relation on X}.
