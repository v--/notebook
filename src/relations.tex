...
\begin{definition}\label{def:relation}
  ...
  \begin{thmenum}
    \labitem{def:relation/graph} \( R \) is called the \term{graph} of the relation. In case the family \( \{ X_k \}_{k \in \mscrK} \) is clear from the context, we say that the graph \( R \) itself is a relation.

    \labitem{def:relation/arity} When \( \mscrK \) is a finite set of \hyperref[rem:cardinals]{cardinality} \( n \), the relation is called \term{n-ary}. In particular,
    \begin{itemize}
    ...
    \labitem{def:relation/single_set} If all \( X_k \) are equal to some set \( X \), \( X \) is clear from the context and we usually say that \( R \subseteq X^{\mscrK} \) is a \term{relation on} \( X \).
  \end{thmenum}
\end{definition}
...
\begin{definition}\label{def:binary_relation}
  ...
  \begin{thmenum}[series=def:binary_relation]
    \labitem{def:binary_relation/domain} We define the \term{domain} of \( R \) as the set
    \begin{equation*}
      \dom(R) \coloneqq \{ x \in X \colon \exists y: (x, y) \in R \}
    \end{equation*}
    of all members of \( X \) that belong to at least one tuple.

    \labitem{def:binary_relation/image} Similarly, we define the \term{image} of \( R \) as the set
    \begin{equation*}
      \img(R) \coloneqq \{ y \in Y \colon \exists x: (x, y) \in R \}
    \end{equation*}
    of all members of \( Y \) that belong to at least one tuple.

    \labitem{def:binary_relation/range} The set \( Y \) is called the \term{range} of \( R \). There is no similar established terminology for \( X \).

    \labitem{def:binary_relation/inverse} We define \term{inverse relation} of \( R \) as
    \begin{equation*}
      \neg R \coloneqq (X \times Y) \setminus R.
    \end{equation*}

    \labitem{def:binary_relation/converse} We define \term{converse relation} of \( R \) as
    \begin{equation*}
      R^{-1} \coloneqq \{ (y, x) \colon (x, y) \in R \}.
    \end{equation*}

    \labitem{def:binary_relation/diagonal} A very special relation is the \term{diagonal relation} on a set \( X \):
    \begin{equation*}
      \Delta_X \coloneqq \{ (x, x) \colon x \in X \}.
    \end{equation*}

    \labitem{def:binary_relation/composition} Given two relations \( R \subseteq X \times Y \) and \( T \subseteq Y \times Z \), we define their composition as
    \begin{equation*}
  ...
  \begin{thmenum}[resume=def:binary_relation]
    \labitem{def:binary_relation/reflexive} \( R \) is \term{reflexive} if \( \Delta_X \subseteq R \).

    \labitem{def:binary_relation/irreflexive} \( R \) is \term{irreflexive} if \( \Delta_X \cap R = \varnothing \).

    \labitem{def:binary_relation/symmetric} \( R \) is \term{symmetric} if \( R = R^{-1} \).

    \labitem{def:binary_relation/antisymmetric} \( R \) is \term{antisymmetric} if \( R \cap R^{-1} = \Delta_X \).

    \labitem{def:binary_relation/transitive} \( R \) is \term{transitive} if
    \begin{equation*}
      (x, y) \in R \T{and} (y, z) \in R \T{implies} (x, z) \in R.
    \end{equation*}

    \labitem{def:binary_relation/total} \( R \) is \term{total} if, for all \( x, y \in X \), either \( (x, y) \in R \) or \( (y, x) \in R \). This is not to be confused with \fullref{def:function/total}.

    \labitem{def:binary_relation/trichotomic} \( R \) is \term{trichotomic} if, for all \( x, y \in X \), either \( x = y \), \( (x, y) \in R \) or \( (y, x) \in R \).
  \end{thmenum}
\end{definition}
...
\begin{definition}\label{def:derived_relations}
  ...
  \begin{thmenum}
    \labitem{def:derived_relations/reflexive} The \term{reflexive closure} of \( R \) is defined as
    \begin{equation*}
      \cl^R(R) \coloneqq R \cup \Delta_X.
    \end{equation*}

    \labitem{def:derived_relations/symmetric} The \term{symmetric closure} of \( R \) is defined as
    \begin{equation*}
      \cl^S(R) \coloneqq R \cup R^{-1}.
    \end{equation*}

    \labitem{def:derived_relations/transitive} The \term{transitive closure} of \( R \) is defined as
    \begin{equation*}
...
\begin{definition}\label{def:equivalence_relation}
  ...
  \begin{thmenum}
    \labitem{def:equivalence_relation/coset} We define \term{equivalence classes} or \term{cosets} to be sets of the form
    \begin{equation*}
      [x] \coloneqq \{ y \in X \colon x \cong y \}.
    \end{equation*}

    \labitem{def:equivalence_relation/quotient} We define the \term{quotient set} of \( X \) by \( \cong \) as
    \begin{equation*}
      X / \cong \ \coloneqq \{ [x] \colon x \in X \}.
    \end{equation*}

    \labitem{def:equivalence_relation/projection} We call the function
    \begin{balign*}
       & \pi: X \to X / \cong  \\
       & \pi(a) \coloneqq [a].
    \end{balign*}
...
\begin{proposition}\label{thm:equivalence_partition}
  ...
  \begin{thmenum}
    \labitem{thm:equivalence_partition/equivalence} \( \sim \) is an \hyperref[def:equivalence_relation]{equivalence relation}.

    \labitem{thm:equivalence_partition/partition} There exists a \hyperref[def:set_partition]{partition} \( \mscrP \) of \( X \) such that
    \begin{equation}\label{thm:equivalence_partition/partition/property}
      x \sim y \iff \exists P \in \mscrP: \{ x, y \} \subseteq P.
    \end{equation}

    \labitem{thm:equivalence_partition/function} There exists a set \( Y \) and a function \( f: X \to Y \) such that \( f(a) = f(b) \iff a \sim b \).
  \end{thmenum}
\end{proposition}
\begin{proof}
