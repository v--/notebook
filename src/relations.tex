...
\begin{definition}\label{def:relation}
  ...
  \begin{DefEnum}
    ...
    \ILabel{def:relation/arity} When \( \CK \) is a finite set of \hyperref[rem:cardinals]{cardinality} \( n \), the relation is called \Def{n-ary}. In particular,
    \begin{itemize}
...
\begin{definition}\label{def:set_partition}
  Let \( X \) be a set. A \Def{cover of \( X \)} is a \hyperref[rem:family_of_sets]{family} \( \CP \subseteq \Pow(X) \) of nonempty sets such that \( X = \bigcup \CP \).
...
\begin{proof}
  ...
  \SubProofImplication{thm:equivalence_partition/partition}{thm:equivalence_partition/function} Let \( \CP \) be a partition of \( X \) satisfying \fullref{thm:equivalence_partition/partition/property}. Denote by \( P_x \) the set in \( \CP \) which contains \( x \) and define the function
  \begin{BreakableAlign*}
     & f: X \to \CP \\
     & f(x) = P_x.
  \end{BreakableAlign*}
