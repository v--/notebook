...
\begin{definition}\label{def:relation}
  ...
  \end{equation*}
  are called \Def{relations}. If \( I \) is a finite set of cardinality \( n \)\Tinyref{remark:cardinals}, the relation is called \Def{n-ary (binary for \( n = 2 \), ternary for \( n = 3 \))}. If all \( X_i \) are the same set, we say that \Def{\( \sim \) is relation on X}.
