...
\begin{definition}\label{def:compact_space}\cite[123]{Engelking1989}
  ...
  \begin{defenum}
    ...
    \DItem{def:compact_space/centered_family} Every centered family\Tinyref{def:centered_family} \( \CF \) of closed subsets of \( X \) has a nonempty intersection.
    \DItem{def:compact_space/convergent_nets} Every net\Tinyref{def:topological_net} has a cluster point or, equivalently\Tinyref{thm:net_convergence_properties/cluster_point_iff_subnet_limit_point}, a convergent\Tinyref{def:net_convergence} subnet. This property is also called \enquote{generalized sequential compactness} or, when restricted to sequences instead of general nets, simply \enquote{sequential compactness}.
  \end{defenum}
\end{definition}
\begin{proof}\mbox{}
  \begin{description}
...
\begin{proposition}\label{thm:compact_space_properties}
  Compact spaces\Tinyref{def:compact_space} have the following basic properties:
  \begin{propenum}
...
\begin{proof}
  \begin{description}
    \RItem{thm:compact_space_properties/closed} If \( A \) is compact, by \cref{def:compact_space/convergent_nets} every net in \( A \) has a cluster point in \( A \). By \cref{thm:derived_set_properties/closed_iff_contains_all_cluster_points}, \( A \) is closed in \( X \).

    \RItem{thm:compact_space_properties/closed_subspace} Let \( X \) be compact\Tinyref{def:compact_space} and let \( X' \subseteq X \) be a closed subspace.

    Fix an open cover \( \{ U_\al \}_{\al \in \CA} \) of \( X' \). By definition of the subspace topology\Tinyref{def:topological_subspace} for each \( U_\al \) there exists a set \( V_\al \) that is open in \( X \) and \( U_\al = V_\al \cap X' \). Since \( X' \) is closed, \( X \setminus X' \) is open and hence the family \( \{ V_\al \}_{\al \in \CA} \) together with \( X \setminus X' \) is an open cover of the space \( X \).
...
\begin{theorem}[Tychonoff's product theorem]\label{thm:tychonoffs_product_theorem}\cite[theorem 3.2.4]{Engelking1989}
  Let \( { (X_\al, \CT_\al) }_{\al \in \CA} \) be a family of topological spaces. Their product\Tinyref{def:topological_product} \( (\prod_{\al \in \CA} X_\al, \prod_{\al \in \CA} \CT_\al) \) is compact\Tinyref{def:compact_space} if and only if \( (X_\al, \CT_\al) \) is compact for every \( \al \in \CA \).
\end{theorem}
...
\begin{theorem}[Weierstrass' extreme value theorem]\label{thm:weierstrass_extreme_value_theorem}\cite[corollary 3.2.9]{Engelking1989}
  Let \( X \) be a compact topological space and let \( f: X \to \BR \) be a continuous function into the real numbers\Tinyref{def:real_numbers}.
