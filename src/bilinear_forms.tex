...
\begin{definition}\label{def:bilinear_form}\mcite[249]{Knapp2016BAlg}
...
\begin{definition}\label{def:bilinear_form_radicals}\mcite[250]{Knapp2016BAlg}
...
\begin{definition}\label{def:nondegenerate_bilinear_form}\mcite[249]{Knapp2016BAlg}
...
\begin{definition}\label{def:sesquilinear_form}\mcite[258]{Knapp2016BAlg}
...
\begin{definition}\label{def:duality_pairing}
  Let \( M \) and \( N \) be left \( R \)-modules. A \term{duality pairing} \( \inner \cdot \cdot: M \times N \to R \) is a \hyperref[def:nondegenerate_bilinear_form]{nondegenerate} bilinear form.
...
\begin{definition}\label{def:inner_product_space}
  An \term{inner product space} is a vector space \( V \) over \( F \) equipped with a positive \hyperref[def:quadratic_form_definiteness]{definite} \hyperref[def:bilinear_form/symmetric]{symmetric} bilinear form \( \inner \cdot \cdot: V \times V \to F \).
...
\begin{definition}\label{def:symplectic_vector_space}
  A \term{symplectic vector space} is a vector space \( V \) over \( F \) equipped with a \hyperref[def:bilinear_form/symmetric]{nondegenerate}  \hyperref[def:bilinear_form/alternating]{alternating} bilinear form \( \inner \cdot \cdot: V \times V \to F \).
\end{definition}
...
\end{theorem}
\begin{proof}
  Note that we use this theorem to prove that the induced norm is a norm so we cannot use the norm here. Associate with \( \inner \cdot \cdot \) the function \( Q(x) \coloneqq \inner x x \). By \fullref{thm:inner_product_quadratic_form_is_positive_definite}, \( Q \) is positive definite.
  ...
  \begin{balign*}
    ...
    \inner {x + ty} {x + ty}
    =    \\ &=
    Q(x) + \overline t \inner x y + t \inner y x + \abs{t}^2 Q(y)
    =    \\ &=
    Q(x) + 2\real t \overline{\inner x y} + \abs{t}^2 Q(y)
  \end{balign*}
  ...
  \begin{balign*}
    Q(x) - \frac {\abs{\inner x y}^2} {Q(y)} & \geq 0                  \\
    Q(x) Q(y)                               & \geq \abs{\inner x y}^2.
  \end{balign*}
...
\begin{proof}
  ...
  \SubProofOf{def:norm/N1} Follows from the positive definiteness of \( \inner \cdot \cdot \)
  ...
  \begin{balign*}
    ...
    =                                                            \\ &=
    \norm{x}^2 + 2 \real \inner x y + \norm{y}^2
    \leq                                                         \\ &\leq
    \norm{x}^2 + 2 \abs{\real \inner x y} + \norm{y}^2
