...
\end{Proposition}
\begin{RefListProof}
    \ISufficiency Let \( L \) be alternating. Then
    \begin{align*}
  ...
  \INecessity Let \( L \) be skew-symmetric. Then
  \begin{equation*}
    L(x, x) = -L(x, x),
  \end{equation*}
...
\begin{proof}
  ...
  \begin{RefList}
    \IRef{def:norm/identity} Follows from the positive definiteness of \( \Prod \cdot \cdot \)

    \IRef{def:norm/absolute_homogeneity} For \( t \in \BC \) and \( x \in V \) we have
    \begin{equation*}
      \Norm{tx} = \sqrt{\Prod{tx} {tx}} = \Abs{t} \sqrt{\Prod x x} = \Abs t \Norm x.
    \end{equation*}

    \IRef{def:norm/subadditivity} For \( x, y \in V \) we have
    \begin{align*}
      \Norm{x + y}^2
      &=
      \Prod{x + y} {x + y}
