...
\begin{definition}\label{def:sesquilinear_form}\cite[258]{Knapp2016BAlg}
  Let \( V \) be a complex vector space and let \( \Ol V \) be its conjugate transpose\Tinyref{def:complex_conjucate_vector_space}. We call the bilinear form \( L: V \times \Ol V \to \BB{C} \) a \Def{sesquilinear form} (we say that \( L \) is \enquote{semilinear} in its second argument and \enquote{sesqui} means \enquote{one and a half} is Latin).
...
\begin{definition}\label{def:inner_product_space}
  ...
  In the special case where \( F = \BB{C} \), we require \( V \) to be equipped with a positive definite Hermitian\Tinyref{def:sesquilinear_form/hermitian} sesquilinear form instead.
\end{definition}
...
\begin{proof}
  ...
  If \( x \) and \( y \) are linearly dependent, equality obviously holds. Conversely, suppose that equality holds for \( x \) and \( y \). This implies that
  \begin{equation*}
    Q(x + ty) = 0,
  \end{equation*}
...
\begin{definition}\label{def:bilinear_form_topology}
  Let \( V \) be a real or complex inner product space\Tinyref{def:inner_product_space} with product \( \Prod \cdot \cdot \). We define
  \begin{defenum}
    \DItem{def:bilinear_form_topology/norm} its induced norm\Tinyref{def:norm} as
    \begin{align*}
      &\Norm \cdot : V \to \R^{\geq 0} \\
      &\Norm x \coloneqq \sqrt{\Prod x x}.
    \end{align*}

    Thus, if \( V \) is a real inner product space, the induced norm is a square root of the induced quadratic form\Tinyref{def:quadratic_form} of \( \Prod \cdot \cdot \).

    \DItem{def:bilinear_form_topology/metric} its induced metric\Tinyref{def:metric_space} as
    \begin{align*}
      &\mu: V \times V \to \R^{\geq 0} \\
      &\mu(x, y) \coloneqq \Norm{x - y}.
    \end{align*}

    \DItem{def:bilinear_form_topology/topology} its induced topology\Tinyref{def:topological_space} as the metric topology\Tinyref{def:metric_topology} generated by \( \mu \), that is, the topology generated by the neighborhood system of open balls.
  \end{defenum}
\end{definition}
\begin{proof}
  ...
  \begin{description}
    \RItem{def:bilinear_form_topology/norm} \( \Norm \cdot \) is well-defined (that is, positive definite) by \cref{thm:inner_product_quadratic_form_is_positive_definite}.

    Now we will show that it is a norm.
    \begin{description}
      \RItem{def:norm/identity} Follows from the positive definiteness of \( \Prod \cdot \cdot \)

      \RItem{def:norm/absolute_homogeneity} For \( t \in \BB{C} \) and \( x \in V \) we have
      \begin{equation*}
      ...
      \RItem{def:bilinear_form_topology/metric} Define \( \mu(x, y) \coloneqq \Norm{x - y} \). The function is positive definite since \( \Norm \cdot \) is positive definite; we will show that the function is a metric.

      \begin{description}
        \RItem{def:metric_space/identity} Follows from \cref{def:norm/identity}.

        \RItem{def:metric_space/symmetry} By \ref{def:norm/absolute_homogeneity},
        \begin{equation*}
          \mu(x, y) = \Norm{ x - y } = \Norm{ (-1) (y - x) } = \Abs{-1} \Norm{y - x} = \mu(y, x).
        \end{equation*}

        \RItem{def:metric_space/triangle_inequality}
        \begin{equation*}
          \mu(x, y) + \mu(y, z) = \Norm{x - y} + \Norm{y - z} \geq \Norm{x - z} = \mu(x, z).
        \end{equation*}
      \end{description}

      \RItem{def:bilinear_form_topology/topology} This is proved in \cref{def:metric_topology}.
    \end{description}
  \end{description}
\end{proof}
