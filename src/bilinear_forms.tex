...
\begin{definition}\label{def:sesquilinear_form}\cite[258]{Knapp2016BAlg}
  Let \( V \) be a complex vector space and let \( \Ol V \) be its conjugate transpose\Tinyref{def:complex_conjucate_vector_space}. We call the bilinear form \( L: V \times \Ol V \to \C \) a \Def{sesquilinear form} (we say that \( L \) is \enquote{semilinear} in its second argument and \enquote{sesqui} means \enquote{one and a half} is Latin).
...
\begin{definition}\label{def:duality_pairing}
  ...
  See \cref{def:canonical_duality_pairing} and \cref{def:locally_convex_duality_pairing}.
\end{definition}
...
\begin{definition}\label{def:inner_product_space}
  ...
  In the special case where \( F = \C \), by convention we require \( V \) to instead be equipped with a positive definite Hermitian\Tinyref{def:sesquilinear_form/hermitian} sesquilinear form instead.
\end{definition}
...
\begin{proof}
  ...
  \begin{description}
    ...
    \RItem{def:norm/absolute_homogeneity} For \( t \in \C \) and \( x \in V \) we have
    \begin{equation*}
