...
\begin{definition}\label{def:bilinear_form}\cite[249]{Knapp2016BAlg}
  ...
  \begin{defenum}
    \DItem{def:bilinear_form/symmetric} If \( L \) is a symmetric \hyperref[def:symmetric_function]{function}, we say that is is a \Def{symmetric bilinear form}
...
\begin{proposition}\label{thm:skew_symmetric_iff_alternating}
  Let \( 1 + 1 = 2 \) be a unit in the ring \( R \). Let \( M \) be a left module over \( R \). Then the bilinear form \( L: M \times M \to R \) is \hyperref[def:bilinear_form/alternating]{alternating} if and only if it is skew-\hyperref[def:bilinear_form/skew_symmetric]{symmetric}.
...
\begin{definition}\label{def:quadratic_form_definiteness}
  We say that a bilinear form is positive or negative definite or semidefinite if the corresponding quadratic form has the same \hyperref[def:function_definiteness]{definiteness} (as a function).
\end{definition}

\begin{proposition}\label{thm:bilinear_forms_vs_to_quadratic_forms}
  A \Def{quadratic form} \( Q: M \to R \) is a homogeneous \hyperref[def:homogenous_function]{function} of degree \( 2 \). In particular, \( Q(x) = Q(-x) \).
\end{proposition}
\begin{proof}
...
\begin{definition}\label{def:orthogonality}
  ...
  Let \( \CA \) be an index set and \( \{ x_\al \}_{\al \in \CA} \subseteq M \), \( \{ y_\al \}_{\al \in \CA} \subseteq N \) be two families of vectors indexed by \( I \). We say that these families form a \Def{biorthogonal system} with respect to \( L \) if
  \begin{equation*}
    L(x_\al, y_\beta) = 0 \text{ follows from } \al \neq \beta
  \end{equation*}

  If \( M = N \), we usually consider \Def{orthogonal systems} \( \{ x_\al \}_{\al \in \CA} \subseteq M \) where
  \begin{equation*}
...
\begin{lemma}\label{thm:inner_product_quadratic_form_is_positive_definite}
  Let \( V \) be a real or complex inner product \hyperref[def:inner_product_space]{space} with product \( \Prod \cdot \cdot \). The function \( Q(x) \coloneqq \Prod x x \) (which is not a quadratic form in the complex case) is positive definite.
\end{lemma}
\begin{proof}
...
\begin{theorem}[Cauchy-Bunyakovsky-Schwarz inequality]\label{thm:cauchy_bunyakovsky_schwarz_inequality}
  Let \( V \) be a real or complex inner product \hyperref[def:inner_product_space]{space} with product \( \Prod \cdot \cdot \). For every \( x, y \in V \) it holds that
...
\begin{definition}\label{def:bilinear_form_induced_norm}
  Let \( V \) be a real or complex inner product \hyperref[def:inner_product_space]{space} with product \( \Prod \cdot \cdot \). We define its induced \hyperref[def:norm]{norm} as
  \begin{align*}
