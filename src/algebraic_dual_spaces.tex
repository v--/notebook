...
\begin{definition}\label{def:canonical_duality_pairing}
  We denote by \( \Prod \cdot \cdot \) the \Def{canonical duality pairing}\Tinyref{def:duality_pairing}
  \begin{align*}
...
\begin{remark}\label{remark:finite_dimensional_dual_space_isomorphism}
  ...
  For example, if we have the differentiable\Tinyref{def:differentiability} function \( f(x, y) = xy \), we can regard its gradient at the point \( (\Ol x, \Ol y) \) as the row vector
  \begin{align*}
    f'(\Ol x, \Ol y) =
    \begin{pmatrix}
      \Ol y & \Ol x
