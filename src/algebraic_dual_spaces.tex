...
\begin{definition}\label{def:dual_vector_space}
  Let \( V \) be a vector space over \( F \). The vector \hyperref[thm:functions_over_ring_form_algebra]{space} of all linear \hyperref[def:linear_operator]{functions} from \( V \) to \( F \) is called the \Def{algebraic dual space} of \( V \) and is denoted by \( V^* \). The functions themselves are called \Def{linear functionals}.
\end{definition}
