...
\begin{definition}\label{def:convergent_series}
  ...
  We say that the series \fullref{def:convergent_series/series} \Def{converges} to a value \( x \) if \( \lim_{n \to \infty} S_n = x \) in the sense of \fullref{thm:metric_topology_convergence}. The value \( x \) is called the \Def{sum} of the series.
  ...
  \end{equation}
  converges, we say that \fullref{def:convergent_series/series} is \Def{absolutely convergent}.
\end{definition}
...
\end{proposition}
\begin{proof}
  Suppose that \fullref{def:convergent_series/absolute_series} converges.
  ...
  Therefore the series \fullref{def:convergent_series/series} is convergent.
\end{proof}

\begin{remark}\label{remark:establish_series_convergence_by_absolute_series}
  Convergence of the series \fullref{def:convergent_series/series} can be established using the convergence of the nonnegative series \fullref{def:convergent_series/absolute_series}.
...
\begin{proposition}\label{thm:infinitary_triangle_inequality}
  For every series \fullref{def:convergent_series/series} we have
...
\begin{proof}
  ...
  \fullref{def:convergent_series/series} exists.
...
\begin{proposition}\label{thm:convergent_series_terms_vanish}
  The terms of the convergent series \fullref{def:convergent_series/series} vanish as \( k \to \infty \), that is,
  \begin{equation*}
...
\begin{proposition}[Cauchy's series convergence criterion]\label{thm:cauchy_series_convergence_criterion}\cite[3.22]{Rudin1976}
  The series \fullref{def:convergent_series/series} converges if and only if for every \( \varepsilon > 0 \) there exists an index \( K \) such that
  \begin{equation*}
    \Norm{\sum_{k=m}^n x_k} < \varepsilon \quad\forall m, n \geq K.
  \end{equation*}
\end{proposition}
\begin{proof}
  This is simply a restatement of \fullref{thm:cauchys_convergence_criterion}.
\end{proof}
...
\end{proposition}
\begin{proof}
  This is simply a restatement of \fullref{def:uniform_limit_of_continuous_functions} in the style of \fullref{thm:cauchy_series_convergence_criterion}.
\end{proof}
...
\begin{proof}
  ...
  The rest of the theorem follows from \fullref{def:uniform_limit_of_continuous_functions}.
\end{proof}
...
\begin{theorem}\label{thm:uniform_limit_exchange}\cite[\textnumero 268]{Фихтенгольц1968/2}
  ...
  \begin{thmenum}
    ...
    \DItem{thm:uniform_limit_exchange/series} If the series \fullref{thm:cauchy_series_continuity_criterion/function} converges uniformly on \( S \), we can exchange the limits
    \begin{equation*}
...
\begin{corollary}\label{thm:riemann_intergral_limit_exchange}\cite[\textnumero 269]{Фихтенгольц1968/2}
  ...
  \begin{corenum}
    ...
    \DItem{thm:riemann_intergral_limit_exchange/series} If the series \fullref{thm:cauchy_series_continuity_criterion/function} converges uniformly, then
    \begin{equation*}
...
\begin{proof}\mbox{}
  \begin{description}
    \RItem{thm:riemann_intergral_limit_exchange/sequence} Assume that the sequence \( \{ f_k \}_{k=0}^\infty \) converges uniformly to \( f \). Then by \fullref{thm:uniform_limit_exchange}, we note that for any index \( k \), the difference \( r_k(x) \coloneqq f(x) - f_k(x) \) is continuous, hence integrable, and
    \begin{equation*}
    ...
    Thus \fullref{thm:metric_topology_convergence/limit_point} is satisfied and equality holds.

    \RItem{thm:riemann_intergral_limit_exchange/series} This is a special case of \fullref{thm:riemann_intergral_limit_exchange/sequence}.
  \end{description}
\end{proof}

\begin{corollary}\label{thm:derivative_limit_exchange}\cite[theorem 7.17]{Rudin1976}
  ...
  \begin{corenum}
    \DItem{thm:derivative_limit_exchange/sequence} If the sequence \( \{ D f_k \}_{k=0}^\infty \) of derivatives converges uniformly, then \( \{ f_k \}_{k=0}^\infty \) also converges uniformly, its limit is differentiable in \( (a, b) \) and
    \begin{equation*}
...
\begin{proof}\mbox{}
  \begin{description}
    ...
    \RItem{thm:derivative_limit_exchange/series} This is a special case of \fullref{thm:riemann_intergral_limit_exchange/sequence}.
  \end{description}
\end{proof}
