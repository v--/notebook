...
from ....logic.structure import evaluate_formula
from ....logic.variables import get_formula_free_variables
...
@pytest_parametrize_kwargs(
    dict(fol_formula='⊤', hol_term='H⊤'),
    dict(fol_formula='(x ≤ y)', hol_term='((≤x)y)'),
    dict(fol_formula='((x + y) ≤ z)', hol_term='((≤((+x)y))z)'),
    dict(fol_formula='¬(x ≤ y)', hol_term='(H¬((≤x)y))'),
    dict(fol_formula='(⊤ ∧ ⊥)', hol_term='((H∧H⊤)H⊥)'),
    dict(fol_formula='∀x.∃y.(x ≤ y)', hol_term='(H∀(λx:ι.(H∃(λy:ι.((≤x)y)))))'),
)
