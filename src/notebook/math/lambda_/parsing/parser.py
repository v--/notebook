        ...
        match head.kind:
            case 'SIGNATURE_SYMBOL':
                ...
                if not isinstance(symbol, BaseTypeSymbol):
                    raise self.annotate_token_error(f'Unexpected {symbol.get_kind_string()} {symbol} while parsing type')
        ...
        match head.kind:
            case 'SIGNATURE_SYMBOL':
                ...
                if not isinstance(symbol, ConstantTermSymbol):
                    raise self.annotate_token_error(f'Unexpected {symbol.get_kind_string()} {symbol} while parsing term')
