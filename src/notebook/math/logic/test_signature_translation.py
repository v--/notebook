...
from .signature import FormalLogicSignature, FunctionSymbol, PredicateSymbol, SignatureTranslationError
...
def test_translation_arity_mismatch(dummy_signature: FormalLogicSignature) -> None:
    with pytest.raises(SignatureTranslationError, match="Mismatch between function symbol 'f¹' of arity 1 and function symbol 'g²' of arity 2"):
...
def test_translate_term(dummy_signature: FormalLogicSignature) -> None:
    ...
    target_signature.add_symbol(FunctionSymbol('ф²', 2, infix=False))
    target_signature.add_symbol(FunctionSymbol('г¹', 1, infix=False))
...
def test_translate_term_exchange_prefix_and_infix(dummy_signature: FormalLogicSignature) -> None:
    translation = SignatureMorphism(dummy_signature, {
        dummy_signature['f²']: dummy_signature['+'],
        dummy_signature['×']: dummy_signature['g²'],
    })
...
def test_translate_formula(dummy_signature: FormalLogicSignature) -> None:
    target_signature = FormalLogicSignature()
    target_signature.add_symbol(FunctionSymbol('ф⁰', 0, infix=False))
    target_signature.add_symbol(PredicateSymbol('п¹', 1, infix=False))
