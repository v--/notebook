...
def parse_prop_formula(source: str, signature: PropLogicSignature = DEFAULT_PROP_SIGNATURE) -> PropFormula:
    return convert_to_prop_formula(parse_formula(source, signature))


def parse_prop_variable(source: str, signature: PropLogicSignature = DEFAULT_PROP_SIGNATURE) -> PropVariable:
    formula = parse_prop_formula(source, signature)
...
def parse_prop_schema(source: str, signature: PropLogicSignature = DEFAULT_PROP_SIGNATURE) -> PropFormulaSchema:
    return convert_to_prop_schema(parse_formula_schema(source, signature))
