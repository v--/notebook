...
\begin{definition}\label{def:quiver_transitive_closure}
  Let \( Q = (V, A, h, t) \) be a \hyperref[def:quiver]{quiver}. Let \( P \) be the set of all finite nonempty \hyperref[def:quiver_path/directed]{directed paths} in \( Q \). We want the paths to be nonempty in order to avoid loops and preserve an analogy with the much better-established concept of a transitive closure of a binary relation defined in \fullref{def:relation_closures/transitive}.

  For every path we have endpoints defined in \fullref{def:quiver_path}. Denote the head of the path \( p \) by \( \vect{h} \) and the tail by \( \vect{t} \). Both are defined for every path in \( P \).
...
\begin{definition}\label{def:quiver_free_category}
  ...
  \begin{itemize}
    ...
    \item The \hyperref[def:category/identity]{identity morphism} on the vertex \( v \) is the \hyperref[def:undirected_multigraph_path/empty]{empty path} at \( v \). This is the primary motivation for having non-unique empty paths.
  \end{itemize}

  Unlike in the \hyperref[def:quiver_transitive_closure]{transitive closure} of \( Q \), empty paths are allowed and are actually an important part of this definition.
