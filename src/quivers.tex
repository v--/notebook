...
\begin{definition}\label{def:quiver}
  ...
  \begin{figure}
    \begin{equation}\label{eq:fig:def:quiver}
      \begin{aligned}
        \includegraphics[page=1]{figures/def__quiver.pdf}
      \end{aligned}
    \end{equation}
...
\begin{example}\label{ex:tower_diagram_graph}
  ...
  \begin{equation}\label{eq:ex:tower_diagram_graph/forward}
    \begin{aligned}
      \includegraphics[page=1]{figures/ex__tower_diagram_graph.pdf}
    \end{aligned}
  \end{equation}
  ...
  \begin{equation}\label{eq:ex:tower_diagram_graph/backward}
    \begin{aligned}
      \includegraphics[page=2]{figures/ex__tower_diagram_graph.pdf}
    \end{aligned}
  \end{equation}
  ...
  \begin{equation}\label{eq:ex:tower_diagram_graph/two_sided}
    \begin{aligned}
      \includegraphics[page=3]{figures/ex__tower_diagram_graph.pdf}
    \end{aligned}
  \end{equation}
...
\begin{definition}\label{def:category_of_small_quivers}
  ...
  We denote the \hyperref[def:category]{category} of \( \mscrU \)-small \hyperref[def:hypergraph]{quivers} by \( \cat{\mscrU-Quiv} \) or, if the universe is clear from the context, simply by \( \cat{Quiv} \). See \fullref{def:category_size} for a further discussion of universes and categories.
...
\begin{example}\label{ex:def:quiver_path}
  ...
  \begin{equation}\label{fig:ex:def:quiver_path}
    \begin{aligned}
      \includegraphics[page=1]{figures/ex__def__quiver_path.pdf}
    \end{aligned}
  \end{equation}
...
\begin{example}\label{ex:def:quiver_condensation}
  ...
  \begin{equation}\label{eq:ex:def:quiver_condensation/uncondensed}
    \begin{aligned}
      \includegraphics[page=1]{figures/ex__def__graph_condensation.pdf}
    \end{aligned}
  \end{equation}
  ...
  \begin{equation}\label{eq:ex:def:quiver_condensation/condensed}
    \begin{aligned}
      \includegraphics[page=2]{figures/ex__def__graph_condensation.pdf}
    \end{aligned}
  \end{equation}
...
\begin{definition}\label{def:quiver_transitive_closure}
  ...
  For every path we have endpoints defined in \fullref{def:quiver_path/endpoints}. Denote the head of the path \( p \) by \( \vect{h} \) and the tail by \( \vect{t} \). Both are defined for every path in \( P \).

  Therefore, \( \cl^T(Q) \coloneqq (V, P, \vect{h}, \vect{t}) \) is a quiver. It is called the \term{transitive closure} of \( Q \).

  A \term{transitive reduction} of \( Q \) is a \hyperref[def:partially_ordered_set_extremal_points/maximal_and_minimal_element]{minimal} \hyperref[def:hypergraph/submodels]{subquiver} \( R \subseteq Q \) such that \( \cl^T(R) = \cl^T(Q) \). If there exists a smallest such relation, it is the unique transitive reduction, and we denote it by \( \red^T(Q) \).

  This is a generalization of the transitive closure and transitive reductions of a relation defined in \fullref{def:relation_closures/transitive}.
\end{definition}

\begin{definition}\label{def:quiver_free_category}
  Let \( Q = (V, A, h, t) \) be a \hyperref[def:quiver]{quiver}. We define the \term{free category} generated by \( Q \) based on the \hyperref[def:quiver_transitive_closure]{transitive closure} of \( Q \), albeit with certain differences regarding loops.

  We will see in \fullref{thm:free_quiver_adjunction} that \( F: \cat{\mscrU-Quiv} \to \cat{\mscrU-Cat} \) is an adjoint functor to the forgetful functor \( U: \cat{\mscrU-Cat} \to \cat{\mscrU-Quiv} \).
  ...
  \begin{itemize}
    ...
    If at least one vertex either has no loops or multiple loops in \( Q \), in order to avoid technicalities, we simply add a new loop for each vertex and let this be the identity morphism \( \id_v \).

    \item The \hyperref[def:category/composition]{composition of the morphisms} \( p \) and \( q \) is the concatenation \( p \cdot q \) defined in \fullref{def:undirected_multigraph_path/concatenation}.

    \item The \hyperref[def:category/identity]{identity morphism} on the object \( v \) is a bit more tricky.

    If every vertex has exactly one loop in \( Q \) (\hi{not in \( \cl^T(Q) \)}), we reuse those loops as identity morphisms.
