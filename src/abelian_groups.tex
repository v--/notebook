...
\begin{definition}\label{def:group_of_integers_modulo}
  ...
  The group \( \BbbZ_n \) is called the \Def{group of integers modulo} \( n \).
\end{definition}
\begin{proof}
...
\begin{proposition}\label{thm:abelian_group_categorical_limits}
  ...
  \begin{DefEnum}
    ...
    \ILabel{thm:abelian_group_categorical_limits/coproduct} Their \hyperref[def:categorical_coproduct]{categorical coproduct} is the \hyperref[def:group_direct_product]{direct sum} \( \oplus_{k \in \mscrK} \mscrX_k \), the embedding morphisms being
    \begin{align*}
