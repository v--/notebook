...
\begin{proof}
  ...
  \SubProofOf{def:magma/semigroup} Addition in \( \BZ_n \) is associative since
  \begin{BreakableAlign*}
    (x \oplus y) \oplus z
     & =
    \Rem((x \oplus y) + z, n)
  ...
  \SubProofOf{def:magma/monoid} The zero is obviously the identity.

  \SubProofOf{def:magma/group} Fix \( x \in \BZ_n \). If \( x = 0 \), its inverse is \( 0 \). If \( x > 0 \), its inverse is \( n - x \) since \( n - x \in \BZ_n \) and
  \begin{equation*}
    x \oplus (n - x) = x + (n - x) - n = 0.
  \end{equation*}

  \SubProofOf{def:abelian_group} Commutativity follows from
  \begin{equation*}
    x \oplus y
    =
    \Rem(x + y, n)
