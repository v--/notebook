...
\begin{proposition}\label{thm:abelian_group_categorical_limits}
  We are interested in categorical \hyperref[def:categorical_limit]{limits} and \hyperref[def:categorical_colimit]{colimits} in \( \Cat{Ab} \). Fix an indexed family  \( \{ X_\al \}_{\al \in \CA} \) of abelian groups.
  \begin{defenum}
    \DItem{thm:abelian_group_categorical_limits/product} Their categorical \hyperref[def:categorical_product]{product} is the direct product as inherited from \fullref{thm:group_categorical_limits}.

    \DItem{thm:abelian_group_categorical_limits/coproduct} Their categorical \hyperref[def:categorical_coproduct]{coproduct} is the direct \hyperref[def:group_direct_product]{sum} \( \oplus_{\al \in \CA} X_\al \), the injection morphisms being
    \begin{align*}
    ...
    Since \( \Cat{Ab} \) is a subcategory of \( \Cat{Grp} \), by \fullref{thm:group_categorical_limits} we have that for abelian groups the notions of free \hyperref[def:group_free_product]{product} and direct sum coincide.
  \end{defenum}
\end{proposition}
...
\begin{proposition}\label{thm:ab_is_monoidal}
  ...
  \begin{itemize}
    \item the direct \hyperref[def:group_direct_product]{sum} acting as a monoidal product
  ...
  and with
  \begin{itemize}
    \item the tensor \hyperref[def:module_tensor_product]{product} (see \fullref{thm:abelian_group_iff_z_module}) acting as a monoidal product
...
\begin{proposition}\label{thm:ab_is_abelian}
  The category \( \Cat{Ab} \) of abelian \hyperref[def:abelian_group]{group} is \hyperref[def:abelian_category]{abelian} (enriched with the direct sum monoidal \hyperref[thm:ab_is_monoidal]{structure}).
\end{proposition}
\begin{proof}
