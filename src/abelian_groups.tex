...
\begin{definition}\label{def:abelian_group}
  A \hyperref[def:magma/commutative]{commutative} \hyperref[def:group]{group} is usually called an \Def{abelian group}. It is conventional to use \hyperref[remark:additive_magma]{additive notation} for abelian groups.
...
\begin{example}\label{ex:lagranges_theorem_for_groups/direct_product_zn}
  \Fullref{ex:lagranges_theorem_for_groups} and \fullref{thm:integers_modulo_isomorphic_to_quotient_group} imply that, for any positive integer \( n \), there exists a bijection between \( n \BZ \times \BZ_n \) and \( \BZ \).
...
\begin{proposition}\label{thm:monoid_completion_to_abelian_group}\MarginCite{nLab:grothendieck_group_of_a_commutative_monoid}
  Every \hyperref[def:magma/commutative]{commutative} \hyperref[def:unital_magma/associative]{monoid} can be \hyperref[def:first_order_homomorphism/embedding]{embedded} into an abelian group using the \Def{Grothendieck completion} presented in the proof.
\end{proposition}
\begin{proof}
