...
\begin{definition}\label{def:abelian_group}
  ...
  By \fullref{thm:abelian_group_is_module}, the abelian groups are precisely the \hyperref[def:module]{modules} over \( \BbbZ \), and we have an \hyperref[rem:category_similarity/isomorphism]{isomorphism of categories} \( \cat{Ab} \cong \cat{Mod}_\BbbZ \).
\end{definition}
...
\begin{proposition}\label{thm:group_abelianization_universal_property}\mcite[prop. 7.4]{Knapp2016BasicAlgebra}
  ...
  \begin{displayquote}
    For every abelian group \( H \) and every group homomorphism \( \varphi: G \to H \), there exists a unique group homomorphism \( \widetilde{\varphi}: G / [G, G] \to H \) such that the following diagram commutes:
  ...
  Via \fullref{rem:universal_mapping_property}, the abelianization functor becomes \hyperref[def:category_adjunction]{left adjoint} to the \hyperref[def:concrete_category]{forgetful functor} \( U: \cat{Ab} \to \cat{Grp} \).
...
\begin{proposition}\label{thm:free_abelian_group_as_quotient}
  The \hyperref[def:free_semimodule]{free abelian group} over a set is (isomorphic to) the \hyperref[thm:group_abelianization_universal_property]{abelianization} of the \hyperref[def:free_group]{free group} over the same set.
\end{proposition}
\begin{proof}
  By the universal mapping property \fullref{thm:free_semimodule_universal_property} for abelian groups, it is unique up to a unique isomorphism. The universal properties \fullref{thm:free_group_universal_property} for free groups and \fullref{thm:group_abelianization_universal_property} for group abelianization give us another abelian group with the same universal property. Hence, the two are isomorphic.
\end{proof}
