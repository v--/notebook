...
\begin{proof}
  ...
  \begin{RefList}
    \IRef{def:magma/semigroup} Addition in \( \BZ_n \) is associative since
    \begin{align*}
      (x \oplus y) \oplus z
      &=
      \Rem((x \oplus y) + z, n)
    ...
    \IRef{def:magma/monoid} The zero is obviously the identity.

    \IRef{def:magma/group} Fix \( x \in \BZ_n \). If \( x = 0 \), its inverse is \( 0 \). If \( x > 0 \), its inverse is \( n - x \) since \( n - x \in \BZ_n \) and
    \begin{equation*}
      x \oplus (n - x) = x + (n - x) - n = 0.
    \end{equation*}

    \IRef{def:abelian_group} Commutativity follows from
    \begin{equation*}
      x \oplus y
      =
      \Rem(x + y, n)
...
\begin{Proposition}\label{thm:abelian_group_categorical_limits}
  ...
  \begin{DefEnum}
    ...
    \ILabel{thm:abelian_group_categorical_limits/coproduct} Their \hyperref[def:categorical_coproduct]{categorical coproduct} is the \hyperref[def:group_direct_product]{direct sum} \( \oplus_{k \in \CK} X_k \), the injection morphisms being
    \begin{align*}
...
\begin{proof}
  ...
  \begin{RefList}
    \IRef{def:binary_relation/reflexive}
    \begin{equation*}
      (x_1, x_2) \cong (x_1, x_2) \iff x_1 + x_2 + 0 = x_1 + x_2 + 0
    \end{equation*}

    \IRef{def:binary_relation/symmetric} By commutativity,
    \begin{align*}
    ...
    \IRef{def:binary_relation/transitive} Let \( (x_1, x_2) \cong (y_1, y_2) \) and \( (y_1, y_2) \cong (z_1, z_2) \). Thus there exist \( a, b \in \BN \) such that
    \begin{equation*}
