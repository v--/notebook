...
\begin{definition}\label{def:grothendieck_universe}\mcite{nLab:grothendieck_universe}
  ...
  \begin{thmenum}
    \thmitem[def:grothendieck_universe/inductive]{GU1} It contains an \hyperref[def:inductive_set]{inductive set}. We specifically mean the \hyperref[thm:smallest_inductive_set_existence]{smallest inductive set \( \omega \)}, however is sufficient only to state the existence of one inductive set.
...
\begin{proof}
  ...
  Now that we have defined \( \widehat \mscrU \), it remains to verify that it is itself a universe. Note that every universe contains an inductive set, hence the \hyperref[thm:smallest_inductive_set_existence]{smallest inductive set \( \omega \)} belongs to all universes and thus also to their \enquote{intersection} \( \widehat \mscrU \). This rest of the verification is trivial.
\end{proof}
...
\begin{definition}\label{def:large_and_small_sets}\mcite{nLab:grothendieck_universe}
  ...
  Without further context (i.e. in \enquote{ordinary mathematics}), we assume that \( \mscrU \) refers to the \hyperref[thm:smallest_grothendieck_universe_existence]{smallest Grothendieck universe} and instead of the terms \( \mscrU \)-large and \( \mscrU \)-small, we simply use the terms \term{large} and \term{small}.
\end{definition}
...
\begin{theorem}\label{thm:grothendieck_universe_iff_strongly_inaccessible}\mcite{Williams1969}
  ...
  Conversely, for every Grothendieck universe \( \mscrU \), there exists a strongly inaccessible cardinal \( \kappa \) such that \( \mscrU = V_\kappa \).
\end{theorem}
\begin{proof}
  ...
  \SubProof*{Proof that \( V_\beta \in \mscrU \) for \( \beta \in \mscrU \)} We will use \fullref{thm:bounded_transfinite_induction} on \( \beta < \alpha \).
  \begin{itemize}
    \item \ref{def:grothendieck_universe/inductive} ensures that \( \mscrU \) contains an inductive set \( A \). \ref{def:grothendieck_universe/transitive} ensures that \( A \subseteq \mscrU \) and since \( \varnothing \) is a member of every inductive set, we have \( \varnothing \in \mscrU \). Therefore, \( V_0 = \varnothing \in \mscrU \).
    ...
    We will come back to this step a bit later, but for now note that \( V_{\beta + 1} \in \mscrU \) irregardless of whether \( \beta + 1 \in \mscrU \).
  ...
  \SubProof*{Proof that \( \kappa \) is a strong limit} For every \( \beta < \kappa \) by \ref{def:grothendieck_universe/power_set} we have \( \pow(\beta) \in \mscrU \). We have already show that \( \card(\pow(\beta)) \in \mscrU \) and, by \fullref{thm:cardinal_exponentiation_power_set}, we have
  \begin{equation*}
  ...
  \SubProof*{Proof that \( \kappa \) is uncountable} In the beginning of the proof we noted that \( \omega \subseteq \mscrU \) since \( \mscrU \) contains some inductive set \( A \). Thus,
  \begin{equation*}
    \omega = \bigcup\set[\Big]{ \set{ n } \given* n \in \omega } \cup \bigcup\set[\Big]{ \varnothing \given* A \setminus \omega }.
  \end{equation*}

  By \ref{def:grothendieck_universe/union}, \( \omega \in \mscrU \). Therefore, \( \omega < \kappa \) and since \( \kappa \) is a cardinal, we conclude that \( \kappa \) is necessary uncountable.
...
\end{corollary}
\begin{proof}
  Follows from \fullref{thm:grothendieck_universe_iff_strongly_inaccessible} and \fullref{thm:cumulative_hierarchy_model_of_zfc}.
\end{proof}
