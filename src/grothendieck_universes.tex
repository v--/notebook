...
\begin{example}\label{ex:large_and_small_sets}
  ...
  \begin{itemize}
    ...
    \item If \( \kappa < \mu \) are regular strong limit cardinals, the stage \( V_\kappa \) of von Neumann's hierarchy is \( V_\mu \)-small by \fullref{thm:cumulative_hierarchy_properties/membership}.
  \end{itemize}
\end{example}

\begin{theorem}\label{thm:grothendieck_universe_iff_regular_strong_limit}\mcite{Williams1969}
...
\begin{proof}
  ...
  \SubProofOf*{def:grothendieck_universe/nonempty} Since \( \kappa \) is infinite and thus \( 0 < \kappa \), from \fullref{thm:cumulative_hierarchy_properties/membership} it follows that \( V_0 \in V_\kappa \).
