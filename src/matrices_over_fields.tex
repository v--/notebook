...
\begin{definition}\label{def:column_and_row_spaces}
  The \term{column space} of the \( m \times n \) matrix \( A \) is the \hyperref[def:module/submodel]{linear span} of the columns vectors of \( A \), regarded as a subspace of \( \BbbK^n \). It is precisely the image of \( A \) regarded as a linear operator.
...
\begin{proposition}\label{thm:matrix_rank}
  The \hyperref[def:column_and_row_spaces]{column space} of a matrix is isomorphic to its \hyperref[def:column_and_row_spaces]{row space}.

  We call the dimension of these spaces the \term{rank} of the matrix. We say that the matrix has \term{full rank} if its rank matches the smaller of its dimensions.
\end{proposition}
\begin{proof}
  Let \( A \) be an \( m \times n \) matrix over \( \BbbK \) and let \( r \) be the dimension of the image of \( A \). By \fullref{thm:quotient_module_universal_property}, as a linear operator, \( A \) \hyperref[def:factors_through]{factors through}
  \begin{equation*}
    \BbbK^m / \ker A \cong \img A \cong \BbbK^r.
  \end{equation*}

  More precisely, there exists an \( m \times r \) matrix \( B \) and an \( r \times n \) matrix \( C \) such that \( A = BC \).

  If \( A = BC \), then \( A^T = C^T B^T \). We cannot, at this point, rule out that \( r \) is larger than the dimension of \( \img A^T \). Hence,
  \begin{equation*}
    \dim \img A^T \leq \dim \img A.
  \end{equation*}

  Conversely,
  \begin{equation*}
    \dim \img A = \dim \img (A^T)^T \leq \dim \img A^T.
  \end{equation*}

  Therefore, the dimensions of the images of \( A \) and \( A^T \) are equal, and hence the images are isomorphic.
\end{proof}
...
\begin{remark}\label{rem:system_of_equations}
  ...
  A \term{solution} to this system is an \( n \)-dimensional \hyperref[def:array/column_vector]{column vector} \( x \). When regarding \( x \) as a free variable in the sense of \fullref{def:first_order_equation}, the entries of \( x \) are called the \term{variables} of the system. When regarding \( T \) has a column vector of functions (via the \hyperref[def:basis_decomposition]{projection functionals}), each individual row is called a \term{equation} of the system. The above is thus a system with \( n \) variables and \( m \) equations.

  The system is called \term{homogeneous} if \( b \) is the zero vector. To every non-homogeneous system, there corresponds a homogeneous system
  \begin{equation}\label{eq:rem:system_of_linear_equation/homogeneous}
    T(x) = \vect 0.
  \end{equation}

  In the case where \( T \) is a \hyperref[def:semimodule/homomorphism]{linear function}, we say that the system itself is linear. We can, in that case, rewrite the system via the matrix of \( T \) with respect to the canonical bases. A linear system is often written in scalar form as
...
\begin{theorem}[Kroneker-Capelli theorem]\label{thm:kroneker_capelli}
  The \hyperref[rem:system_of_equations]{system}
  \begin{equation*}
    Ax = b
  \end{equation*}
  of \( n \) variables and \( m \) linear equations has a solution if and only if the \hyperref[thm:matrix_rank]{rank} of its coefficient matrix \( A \) is equal to the rank of its augmented matrix
  \begin{equation*}
    \parens*
    {
      \begin{array}{c|c}
