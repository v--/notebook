...
\begin{remark}\label{rem:change_of_basis}
  Suppose that \( e_1, \ldots, e_n \) and \( f_1, \ldots, f_n \) are both bases of some vector space \( V \) over \( \BbbK \). Any vector \( v \) can be decomposed along \( e_1, \ldots, e_n \) to form the tuple \( x_1, \ldots, x_n \) and along \( f_1, \ldots, f_n \) to form \( y_1, \ldots, y_n \).
...
\begin{remark}\label{rem:system_of_equations}
  ...
  \begin{equation}\label{eq:rem:system_of_linear_equation/vector_form}
    T(x) = b.
  \end{equation}
  ...
  When regarding \( x \) as a free variable in the sense of \fullref{def:first_order_equation}, the entries of \( x \) are themselves called the \term{variables} of the system. When regarding \( x \) as a bound variable, it is called a \term{solution} if \eqref{eq:rem:system_of_linear_equation/vector_form} is satisfied.

  We can write the system \eqref{eq:rem:system_of_linear_equation/vector_form} in scalar form as
...
\begin{theorem}[Kroneker-Capelli theorem]\label{thm:kroneker_capelli}
  ...
  \begin{equation*}
    Ax = b
  \end{equation*}
  of \( n \) variables and \( m \) equations has a solution if and only if the \hyperref[thm:matrix_rank]{rank} of its coefficient matrix \( A \) is equal to the rank of its augmented matrix
  \begin{equation*}
    \parens*
    {
      \begin{array}{c|c}
