...
\begin{proof}
  ...
  Therefore \( A \in V_{\rho + 2} \). Since \( \kappa \) is an infinite cardinal, from \fullref{thm:cardinal_is_infinite_iff_limit_ordinal} it follows that \( \rho + 1 < \kappa \) and from \fullref{thm:cumulative_hierarchy_properties/subsets} it follows that \( A \in V_\kappa \).

  \NecessitySubProof Follows from \fullref{thm:cumulative_hierarchy_properties/transitive}.
\end{proof}

\begin{proposition}\label{thm:strongl_regular_cardinal_stages}
...
\begin{proof}
  ...
  \end{equation*}
  where the last inequality follows from \fullref{thm:strongl_regular_cardinal_stages}.
\end{proof}
...
\begin{remark}\label{rem:universe_of_hereditary_finite_sets}
  From \fullref{thm:aleph_zero_is_strong_limit} and \fullref{thm:aleph_zero_is_regular} is follows that \( \omega = \aleph_0 \) is a regular strong limit cardinal. Then by \fullref{thm:strong_regular_cardinal_stage_cardinality}, every member of \( V_\omega \) is finite. Because \( V_\omega \) is a transitive set, every member of every member of \( V_\omega \) is also finite. So is every member of every member of every member.

  For this reason, \( V_\omega \) is known as the \term{universe of hereditary finite sets}.

  Actually \( V_\omega \) is a standard model of \logic{ZFC} without the \hyperref[def:zfc/infinity]{axiom of infinity}. Indeed, from the proof of \fullref{thm:cumulative_hierarchy_model_of_z} is follows that \( V_\omega \) is a model of \logic{ZFC} without the axiom of infinity and the axiom schema of replacement. \Fullref{rem:cumulative_hierarchy_model_of_zfc} shows that being a countable regular strong limit cardinal is sufficient for \( V_\omega \) to satisfy the axiom of replacement.
\end{remark}
