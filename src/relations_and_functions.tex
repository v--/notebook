...
\begin{definition}\label{def:binary_relation}
  ...
  The nested tuple \( (R,  (A, B)) \) is called a \term{binary relation}. We say that the tuple \( (x, y) \in A \times B \) is \term{related} with respect to \( R \) when \( (x, y) \in R \). Later (in \fullref{def:relation}) we will introduce relations that are not necessarily binary, but we must first define arbitrary Cartesian products.
  ...
  \begin{thmenum}[series=def:binary_relation]
    ...
    \thmitem{def:binary_relation/range} The set \( B \) is called the \term{range} or \term{codomain} of \( R \). There is no similar terminology established for \( A \).
  ...
  \begin{thmenum}[resume=def:binary_relation]
    ...
    The following formula is used as an axiom for \hyperref[def:equivalence_relation]{equivalence relations}, \hyperref[def:graph/undirected]{undirected graphs} and \hyperref[def:entourage]{entourages}:
...
\begin{example}\label{ex:def:binary_relation}
  ...
  \begin{itemize}
    ...
    \item \hyperref[def:graph/directed]{Directed graphs} are binary relations over finite sets.
...
\begin{definition}\label{def:multi_valued_function}
  ...
  \begin{thmenum}[resume=def:multi_valued_function]
    ...
    Symmetric functions should not be confused with symmetric relations as defined in \fullref{def:relation_closures/symmetric}.
...
\begin{proposition}\label{thm:function_composition_invertibility}
  ...
  Compare this result to \fullref{thm:function_superposition_invertibility}.
\end{proposition}
\begin{proof}
...
\begin{definition}\label{def:sequence}
  ...
  A \term{finite sequence} is a instead a function whose domain is some \hyperref[def:set_finiteness]{finite} \hyperref[def:ordinal]{ordinal}.
\end{definition}
...
\begin{definition}\label{def:cartesian_product}
  ...
  The definition also makes sense when any of the sets is empty because the product itself is then empty. The Cartesian product of identical sets \( A \) is denoted for short by \( A^n \).
...
\begin{definition}\label{def:superposition}
  ...
  If we are given the functions \( f_k: A \multto B_k, k = 1, \ldots, n \) and \( g: B_1 \times \cdots \times B_n \multto C \), their \term{superposition} is
  \begin{equation*}
    \begin{aligned}
      &h: A \to C
...
\begin{definition}\label{def:relation_closures}
  ...
  \begin{thmenum}
    ...
    \end{equation*}
    where \( R^k \) is iterated \hyperref[def:binary_relation/composition]{composition} of \( R \) regarded as a \hyperref[def:multi_valued_function]{multi-valued function}.
...
\begin{proposition}\label{thm:relation_closures_properties}
  ...
  \begin{thmenum}
    \thmitem{thm:relation_closures_properties/reflexive_set} The symmetric and transitive closures of a reflexive relation are symmetric.
    \thmitem{thm:relation_closures_properties/symmetric_set} The reflexive and transitive closures of a symmetric set are symmetric.
    \thmitem{thm:relation_closures_properties/symmetric_and_transitive} For the \hyperref[def:relation_closures/transitive]{transitive} and \hyperref[def:relation_closures/symmetric]{symmetric} closures of \( A \) we have
    \begin{equation}\label{eq:thm:relation_closures_properties/symmetric_and_transitive}
      \cl^S \cl^T(R) \subseteq \cl^T \cl^S(R).
    \end{equation}

    The converse holds if \( R \) is symmetric but not in general --- see \fullref{thm:relation_closures_properties/symmetric_and_transitive}.
  \end{thmenum}
\end{proposition}
\begin{proof}
  \SubProofOf{thm:relation_closures_properties/reflexive_set} Trivial.
  \SubProofOf{thm:relation_closures_properties/symmetric_set} Trivial.
  \SubProofOf{thm:relation_closures_properties/symmetric_and_transitive} It is clear that
  \begin{equation*}
    R \subseteq \cl^S(R) \subseteq \cl^T(\cl^S(R)).
  \end{equation*}
  ...
  \begin{equation*}
    ...
    =
    \cl^T(R)
    \reloset {\ref{thm:relation_closures_properties/symmetric_set}} =
    \cl^S \cl^T(R).
  \end{equation*}
\end{proof}
...
\begin{example}\label{ex:thm:relation_closures_properties/symmetric_and_transitive}
  ...
  \begin{equation*}
    \cl^S(\cl^T(R)) = \cl^S(R) = R \cup \set{ (b, a), (b, a) }.
  \end{equation*}

  But then
  \begin{equation*}
    \cl^T(\cl^S(R)) = \cl^T(R) \cup \set{ (a, c), (c, a) } = A \times A.
  \end{equation*}
