...
\begin{definition}\label{def:multi_valued_function}
  ...
  \begin{thmenum}[series=def:multi_valued_function]
    ...
    The notation \( f[A'] \) is sometimes used instead, for example in \cite[def. 2.31]{OpenLogicFull}. This has the benefit of avoiding the nasty ambiguity described in \fullref{rem:function_on_ordinal}. Outside of set theory, however, this notation is not commonly accepted. Our disambiguation strategy is, in case \( A' \) is both a member and a subset of \( A \), to always regard \( A' \) as a member of \( A \).
  \end{thmenum}
...
\begin{definition}\label{def:function}
  ...
  \begin{thmenum}
    ...
    See the notes in \fullref{def:multi_valued_function/set_value} regarding notational ambiguity.
