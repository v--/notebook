...
\begin{definition}\label{def:preordered_magma}
  A \term{preordered magma} is a magma \( M \) equipped with a \hyperref[def:preordered_set]{preorder} \( \leq \) such that \( x \leq y \) implies \( xz \leq yz \) and \( zx \leq zy \) for all \( z \in M \).
...
\begin{definition}\label{def:unital_magma}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:unital_magma/submodel} The set \( S \subseteq X \) is a \hyperref[thm:substructure_is_model]{submodel} if \( X \) if \( e \in S \). This is equivalent to \( S \) being a pointed subset.

    We say that \( S \) is a \term{unital submagma}.

    As a consequence of \fullref{thm:positive_formulas_preserved_under_homomorphism}, the \hyperref[def:multi_valued_function/image]{image} of a unital magma homomorphism \( \varphi: X \to Y \) is a unital submagma of \( Y \).
    ...
    \thmitem{def:unital_magma/power_set} The \hyperref[def:magma/power_set]{power set magma} \( \pow(M) \) of a unital magma \( M \) with identity \( e \) is again a unital magma with identity \( \set{ e } \).
  \end{thmenum}
\end{definition}
