...
\begin{definition}\label{def:graph}\mcite[ch. 1, sec. 1.1]{GondranMinoux1984Graphs}
  ...
  \begin{thmenum}[resume=def:graph]
    ...
    The term \enquote{graph embedding} usually refers to families of paths in topological spaces, and we thus avoid using the term for injective graph homomorphisms.
...
\begin{example}\label{ex:def:graph}
  ...
  It should be noted that when defining a concrete graph, it is impractical to enumerate the vertices and edges. It is instead more understandable to work with a diagram like it is done here.
\end{example}
