...
\begin{remark}\label{remark:directed_and_undirected_graphs}
  ...
  We define undirected graphs as a special case of directed graphs. This approach makes some definitions more awkward. In programming, however, implementing undirected graphs as a special case of directed graphs is often more versatile (see \cite[sec. 5.4]{Erickson2019} and \cite[ch. 1, sec. 2.4]{Gondran1984}).
\end{remark}

\begin{definition}\label{def:directed_graph}\MarginCite[ch. 1, sec. 1.1]{Gondran1984}
  A \Def{directed graph} \( G = (V, E) \) is a pair where
  \begin{itemize}
    \item \( V \) is a set, whose elements are called \Def{vertices} or \Def{nodes}.
    \item \( E \subseteq V^2 \) is a \hyperref[def:relation]{relation} over \( V \), whose elements are called \Def{arcs}. If \( u, v \in V \) are vertices and \( e = (u, v) \) is an arc, we say that \( u \) is the \Def{head} or \Def{initial endpoint} and \( v \) is the \Def{tail} or \Def{terminal endpoint} of the arc. We denote $u = h(e)$ and $v = t(e)$.
  \end{itemize}
  ...
  \begin{DefEnum}
    ...
    \ILabel{def:directed_graph/empty} a graph \( G = (V, E) \) is \Def{empty} if \( E = \varnothing \).
    \ILabel{def:directed_graph/subgraph} the graph \( G' = (V', E') \) is a \Def{subgraph} of \( G = (V, E) \) if we have both \( V' \subseteq V \) and \( E' \subseteq E \).
    \ILabel{def:directed_graph/loop} the arc \( u \to v \) is a \Def{loop} if \( u = v \).
    \ILabel{def:directed_graph/simple}\MarginCite[ch. 1, sec. 1.3]{Gondran1984}the graph \( G \) is \Def{simple} if it has no loops.
  \end{DefEnum}
\end{definition}
...
\begin{definition}\label{def:graph_matrices}
  ...
  \begin{DefEnum}
    \ILabel{def:graph_matrices/incidence}\MarginCite[ch. 1, sec. 2.1]{Gondran1984} The \Def{incidence matrix} \( I = \{ a_{ve} \}_{v \in V, e \in E} \) of \( G \) is defined as
    \begin{equation*}
    ...
    \ILabel{def:graph_matrices/adjacency}\MarginCite[ch. 1, sec. 2.3]{Gondran1984} The \Def{adjacency matrix} \( A = \{ a_{ve} \}_{u, v \in V} \) of \( G \) is defined as
    \begin{equation*}
...
\begin{definition}\label{def:graph_paths}
  ...
  \begin{DefEnum}
    \ILabel{def:graph_paths/adjacent_vertices} Two vertices \( u \)  and \( v \)  are called \Def{adjacent} if there exists an arc from \( u \)  to \( v \) .

    \ILabel{def:graph_paths/adjacent_arcs}\MarginCite[ch. 1, sec. 1.4]{Gondran1984} Two arcs are called \Def{adjacent} if they have a common endpoint. Thus the first three pairs of arcs are adjacent and the fourth is not (assuming all vertices are distinct):
    \begin{enumerate}
    ...
    \ILabel{def:graph_paths/undirected_path}\MarginCite[ch. 1, sec. 3.1]{Gondran1984} An \Def{undirected path} or \Def{chain} is a sequence of distinct arcs
    \begin{equation*}
    ...
    Some authors (e.g. \cite[sec. 5.2]{Erickson2019}) call undirected paths \Def{walks} and reserve the term \enquote{path} for simple undirected paths.
    ...
    \ILabel{def:graph_paths/directed_path}\MarginCite[ch. 1, sec. 3.2]{Gondran1984}If the tail of each non-endpoint arc in a path coincides with the head of the next arc, we say that the path is a \Def{directed path}.
    ...
    \ILabel{def:graph_paths/dag}\MarginCite[231]{Erickson2019}A \Def{directed acyclic graph} or \Def{dag} is a directed graph without directed cycles.

    \medskip

    \ILabel{def:graph_paths/eulerian_path}\MarginCite[ch. 8, sec. 1.1]{Gondran1984}A path (either directed or undirected) is called \Def{Eulerian} if it contains every arc of the graph exactly once, that is, the path induces an ordering of the arcs. A graph with an Eulerian cycle is called an \Def{Eulerian graph}.

    \ILabel{def:graph_paths/hamiltonian_path}\MarginCite[ch. 8, sec. 3.1]{Gondran1984}A simple path (either directed or undirected) is called \Def{Hamiltonian} if it contains every vertex of the graph, that is, it induces an ordering of the vertices. A graph with a Hamiltonian cycle is called a \Def{Hamiltonian graph}.
  \end{DefEnum}
\end{definition}
...
\begin{definition}\label{def:graph_incidence}
  ...
  \begin{DefEnum}
    \ILabel{def:graph_incidence/incident_arcs} The arc \( e \) is said to be \Def{incident} with the vertex \( v \) if \( e \in w(v) \), that is, if \( v \) is an endpoint of \( e \).

    \ILabel{def:graph_incidence/degree}\MarginCite[ch. 1, sec. 1.4]{Gondran1984}Given a vertex \( v \), the \Def{degree} \( d(v) \) (resp. \Def{in-degree} \( d^+(v) \) and \Def{out-degree} \( d^-(v) \)) of the vertex is defined as
    \begin{equation*}
...
\begin{definition}\label{def:graph_connectivity}
  ...
  \begin{DefEnum}
    \ILabel{def:graph_connectivity/reachable_vertices}The vertex \( v \) is \Def{reachable} from the vertex \( u \) if there exists a directed \hyperref[def:graph_paths/directed_path]{path} from \( u \) to \( v \).

    \ILabel{def:graph_connectivity/strongly_connected_graph}\MarginCite[ch. 1, sec. 3.5]{Gondran1984}The graph \( G \) is \Def{strongly connected} if every pair of distinct vertices are reachable, that is, if there exists a directed path between every pair of distinct vertices.

    \ILabel{def:graph_connectivity/weakly_connected_graph}\MarginCite[ch. 1, sec. 3.3]{Gondran1984}The graph \( G \) is \Def{weakly connected} if there exists an undirected path between every pair of distinct vertices.

    \ILabel{def:graph_connectivity/connected_component}\MarginCite[ch. 1, sec. 3.3 \\ ch. 1, sec. 3.5]{Gondran1984}The subgraph \( G' \) of \( G \) is a \Def{connected component} (resp. \Def{strongly connected component}) if it is connected (resp. strongly connected) and there exists no connected (resp. strongly connected) subgraph of \( G \) that properly contains \( G' \).

    \ILabel{def:graph_connectivity/connectivity_number}\MarginCite[ch. 1, sec. 3.3 \\ ch. 1, sec. 3.5]{Gondran1984}\( G \) has \Def{connectivity number} (resp. \Def{strong connectivity number}) $n$ if it has $n$ connected (resp. strongly connected) components.

    \ILabel{def:graph_connectivity/cut}\MarginCite[ch. 1, sec. 3.4]{Gondran1984}The set \( U \subseteq V \) of vertices is a \Def{cut} (resp. \Def{directed cut}) if removing \( T \) from the graph would increase the connectivity number (resp. strong connectivity number) of the graph.

    \ILabel{def:graph_connectivity/cocycle}\MarginCite[ch. 1, sec. 4.4]{Gondran1984}The set \( F \subseteq E \) of arcs is a \Def{cocycle} (resp. \Def{cocircuit}) if there exists a set \( U \subseteq V \) of vertices such that \( F = w(T) \) (resp. \( F \in \{ w^+(T), w^-(T) \} \)).
  \end{DefEnum}
\end{definition}
...
\begin{definition}\label{def:graph_adjacency}
  ...
  \begin{DefEnum}
    \ILabel{def:graph_adjacency/clique}\MarginCite[ch. 1, sec. 1.4]{Gondran1984}The set \( U \subseteq V \) is called a \Def{clique} if all two vertices in \( U \) are adjacent.

    \medskip

    \ILabel{def:graph_adjacency/complete_graph}\MarginCite[ch. 1, sec. 1.4]{Gondran1984}If \( V \) itself is a clique, we say that \( G \) is a \Def{complete graph}.

    \medskip

    \ILabel{def:graph_adjacency/anticlique}\MarginCite[120]{Erickson2019}Dually, \( U \subseteq V \) is an \Def{anticlique} or \Def{independent set} of vertices if no two vertices in \( U \) are adjacent.

    \ILabel{def:graph_adjacency/matching}\MarginCite[ch. 5, exer. 11]{Gondran1984}The set \( F \subseteq E \) of arcs is a \Def{matching} or \Def{independent set} of arcs if no two arcs in \( F \) are adjacent.

    \ILabel{def:graph_adjacency/bipartite_graph}\MarginCite[7]{Gondran1984}The graph is called \Def{bipartite} if there exists a partition \( \{ A, B \} \) of \( V \) such that both \( A \) and \( B \) are anticliques. We also write \( G = (A, B, E) \).
...
\begin{example}\label{ex:petersen_graph}\MarginCite[347]{Gondran1984}
  The Petersen graph
