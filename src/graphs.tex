...
\begin{definition}\label{def:graph}
  ...
  \begin{thmenum}[resume=def:graph]
    ...
    \thmitem{adef:hypergraph/adjacency} We say that \( u \) and \( v \) are \term{adjacent vertices} if there exists an edge \( e \) such that both \( u \) and \( v \) are endpoints of \( e \).

    \thmitem{adef:hypergraph/adjacency} We say that \( e_1 \) and \( e_2 \) are \term{adjacent edges} if they have a common endpoint.
