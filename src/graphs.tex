...
\begin{definition}\label{def:graph_spaces}
  Let \( G = (V, E) \) be a graph. Let \( \mscrR \) be the \hyperref[def:field]{field} \hyperref[thm:f2_is_boolean_algebra]{\BbbF_2} if the graph is undirected and \hyperref[def:semiring/ring]{ring} \( \BbbZ \) of \hyperref[def:set_of_integers]{integers} otherwise. We introduce two \hyperref[def:free_left_module]{free} \( \mscrR \)-\hyperref[def:left_module]{modules} (or, in the undirected case, two vector spaces over \( \BbbF_2 \)).

  \begin{thmenum}
    \thmitem{def:graph_spaces/vertex} The \term{vertex space} \( \mscrR^V \) is the free \( \mscrR \)-module of dimension \( \card(V) = \ord(G) \).

    Every subset \( A \subseteq V \) of vertices induces a unique vector \( x = \seq{ x_v }_{v \in V} \) in the \hyperref[def:graph_spaces/vertex]{vertex space} \( \mscrR^V \) such that
    \begin{equation*}
      x_v = \begin{cases}
        1, &v \in A \\
        0, &v \not\in A
    ...
    This vector is called the \term{characteristic vector} of \( A \).

    \thmitem{def:graph_spaces/edge} Analogously, the \term{edge space} \( \mscrR^E \) is the free \( \mscrR \)-module of dimension \( \card(E) \). Every subset \( B \subseteq E \) of arcs induces a unique characteristic vector \( x = \seq{ x_e }_{e \in E} \) in the \hyperref[def:graph_spaces/edge]{edge space} \( \mscrR^E \).
  \end{thmenum}
\end{definition}

\begin{proposition}\label{thm:undirected_graph_spaces}
  For an undirected graph \( G = (V, E) \), every vector in the \hyperref[def:graph_spaces/vertex]{vertex space}  \( \BbbF_2^V \) induces a set of vertices and every vector in the \hyperref[def:graph_spaces/edge]{edge space}  \( \BbbF_2^E \) induces a set of edges.
\end{proposition}
\begin{proof}
  Trivial.
\end{proof}
