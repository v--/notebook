...
\begin{definition}\label{def:undirected_graph}
  An \Def{undirected graph} is a directed graph \( G = (V, E) \) where \( E \) is a symmetric relation (see \fullref{rem:directed_and_undirected_graphs}). When dealing with undirected graphs, instead of speaking about the arcs \( u \to v \) and \( v \to u \), we speak about the \Def{edges} \( \{ u, v \} \). Thus, we can also define an undirected graph to be the tuple \( G = (V, E) \), where
  \begin{itemize}
...
\begin{definition}\label{def:category_of_graphs}
  ...
  We denote by \( \Cat{UGraph} \) the category of undirected graphs. Given the convention established in \fullref{rem:directed_and_undirected_graphs} about undirected graphs being a special case of directed graphs, we can regard \( \Bold{UGraph} \) as a subcategory of \( \Bold{DGraph} \).
