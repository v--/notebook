...
\begin{Definition}\label{def:directed_graph}\cite[1-graphs from chapter 1, section 1.1]{Gondran1984}
  ...
  \begin{DefEnum}
    ...
    \ILabel{def:directed_graph/simple}\cite[chapter 1, section 1.3]{Gondran1984} the graph \( G \) is \Def{simple} if it has no loops.
  \end{DefEnum}
\end{Definition}
...
\begin{Definition}\label{def:graph_matrices}
  ...
  \begin{DefEnum}
    \ILabel{def:graph_matrices/incidence}\cite[chapter 1, section 2.1]{Gondran1984} The \Def{incidence matrix} \( I = \{ a_{ve} \}_{v \in V, e \in E} \) of \( G \) is defined as
    \begin{equation*}
    ...
    \ILabel{def:graph_matrices/adjacency}\cite[chapter 1, section 2.3]{Gondran1984} The \Def{adjacency matrix} \( A = \{ a_{ve} \}_{u, v \in V} \) of \( G \) is defined as
    \begin{equation*}
...
\begin{Definition}\label{def:graph_paths}
  ...
  \begin{DefEnum}
    ...
    \ILabel{def:graph_paths/adjacent_arcs}\cite[chapter 1, section 1.4]{Gondran1984} Two arcs are called \Def{adjacent} if they have a common endpoint. Thus the first three pairs of arcs are adjacent and the fourth is not (assuming all vertices are distinct):
    \begin{enumerate}
    ...
    \ILabel{def:graph_paths/undirected_path}\cite[chapter 1, section 3.1]{Gondran1984} An \Def{undirected path} or \Def{chain} is a sequence of distinct arcs
    \begin{equation*}
    ...
    \ILabel{def:graph_paths/directed_path}\cite[chapter 1, section 3.2]{Gondran1984} If the tail of each non-endpoint arc in a path coincides with the head of the next arc, we say that the path is a \Def{directed path}.
    ...
    \ILabel{def:graph_paths/dag}\cite[231]{Erickson2019} A \Def{directed acyclic graph} or \Def{dag} is a directed graph without directed cycles.

    \ILabel{def:graph_paths/eulerian_path}\cite[chapter 8, section 1.1]{Gondran1984} A path (either directed or undirected) is called \Def{Eulerian} if it contains every arc of the graph exactly once, that is, the path induces an ordering of the arcs. A graph with an Eulerian cycle is called an \Def{Eulerian graph}.

    \ILabel{def:graph_paths/hamiltonian_path}\cite[chapter 8, section 3.1]{Gondran1984} A simple path (either directed or undirected) is called \Def{Hamiltonian} if it contains every vertex of the graph, that is, it induces an ordering of the vertices. A graph with a Hamiltonian cycle is called a \Def{Hamiltonian graph}.
  \end{DefEnum}
\end{Definition}
...
\begin{Definition}\label{def:graph_incidence}
  ...
  \begin{DefEnum}
    ...
    \ILabel{def:graph_incidence/degree}\cite[chapter 1, section 1.4]{Gondran1984} Given a vertex \( v \), the \Def{degree} \( d(v) \) (resp. \Def{in-degree} \( d^+(v) \) and \Def{out-degree} \( d^-(v) \)) of the vertex is defined as
    \begin{equation*}
...
\begin{Definition}\label{def:graph_connectivity}
  ...
  \begin{DefEnum}
    ...
    \ILabel{def:graph_connectivity/strongly_connected_graph}\cite[chapter 1, section 3.5]{Gondran1984} the graph \( G \) is \Def{strongly connected} if every pair of distinct vertices are reachable, that is, if there exists a directed path between every pair of distinct vertices.

    \ILabel{def:graph_connectivity/weakly_connected_graph}\cite[chapter 1, section 3.3]{Gondran1984} the graph \( G \) is \Def{weakly connected} if there exists an undirected path between every pair of distinct vertices.

    \ILabel{def:graph_connectivity/connected_component}\cite[chapter 1, sections 3.3 and 3.5]{Gondran1984} the subgraph \( G' \) of \( G \) is a \Def{connected component} (resp. \Def{strongly connected component}) if it is connected (resp. strongly connected) and there exists no connected (resp. strongly connected) subgraph of \( G \) that properly contains \( G' \).

    \ILabel{def:graph_connectivity/connectivity_number}\cite[chapter 1, sections 3.3 and 3.5]{Gondran1984} \( G \) has \Def{connectivity number} (resp. \Def{strong connectivity number}) $n$ if it has $n$ connected (resp. strongly connected) components.

    \ILabel{def:graph_connectivity/cut}\cite[chapter 1, section 3.4]{Gondran1984} the set \( U \subseteq V \) of vertices is a \Def{cut} (resp. \Def{directed cut}) if removing \( T \) from the graph would increase the connectivity number (resp. strong connectivity number) of the graph.

    \ILabel{def:graph_connectivity/cocycle}\cite[chapter 1, section 4.4]{Gondran1984} the set \( F \subseteq E \) of arcs is a \Def{cocycle} (resp. \Def{cocircuit}) if there exists a set \( U \subseteq V \) of vertices such that \( F = w(T) \) (resp. \( F \in \{ w^+(T), w^-(T) \} \)).
  \end{DefEnum}
\end{Definition}
...
\begin{Definition}\label{def:graph_adjacency}
  ...
  \begin{DefEnum}
    \ILabel{def:graph_adjacency/clique}\cite[chapter 1, section 1.4]{Gondran1984} The set \( U \subseteq V \) is called a \Def{clique} if all two vertices in \( U \) are adjacent.

    \ILabel{def:graph_adjacency/complete_graph}\cite[chapter 1, section 1.4]{Gondran1984} If \( V \) itself is a clique, we say that \( G \) is a \Def{complete graph}.

    \ILabel{def:graph_adjacency/anticlique}\cite[120]{Erickson2019} Dually, \( U \subseteq V \) is an \Def{anticlique} or \Def{independent set} of vertices if no two vertices in \( U \) are adjacent.

    \ILabel{def:graph_adjacency/matching}\cite[chapter 5, exercise 11]{Gondran1984} The set \( F \subseteq E \) of arcs is a \Def{matching} or \Def{independent set} of arcs if no two arcs in \( F \) are adjacent.

    \ILabel{def:graph_adjacency/bipartite_graph}\cite[7]{Gondran1984} The graph is called \Def{bipartite} if there exists a partition \( \{ A, B \} \) of \( V \) such that both \( A \) and \( B \) are anticliques. We also write \( G = (A, B, E) \).
