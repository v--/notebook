...
\begin{definition}\label{def:category_of_groups}
  The class\Tinyref{def:set_zfc} of all graphs forms the category\Tinyref{def:category} \( \Bold{Graph} \), where for every two graphs \( X, Y \in \Bold{Graph} \), the morphisms \( \Bold{Graph}(X, Y) \) are the homomorphisms\Tinyref{def:graph_homomorphism} from \( X \) to \( Y \) and composition is the usual function composition\Tinyref{def:function_composition}.

  Furthermore, \( \Bold{Graph} \) is locally small\Tinyref{def:category_cardinality}.
\end{definition}
