...
\begin{remark}\label{rem:directed_and_undirected_graphs}
  ...
  \begin{itemize}
    \item Graphs of functions (see \fullref{def:multi_valued_function/graph})
...
\begin{definition}\label{def:graph_incidence}
  Let \( G = (V, E) \) be a directed graph. We define the multi-valued \hyperref[def:multi_valued_function]{functions} with signature \( \pow(V) \rightrightarrows E \):
  \begin{balign*}
...
\begin{definition}\label{def:category_of_graphs}
  ...
  \begin{itemize}
    ...
    \item the morphisms between two graphs the homomorphisms between them, with morphism composition being the usual \hyperref[def:multi_valued_function/composition]{function composition}.
  \end{itemize}
