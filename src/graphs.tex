...
\begin{definition}\label{def:graph_directed_path}
  ...
  \begin{thmenum}
    ...
    We say that the vertex \( u \) is \term{reachable} from \( v \) if there exists a path that connects them. Clearly reachability is the \hyperref[def:derived_relations/transitive]{transitive closure} of \( \to \).
...
\begin{definition}\label{def:graph_condensation}
  ...
  It is the \hyperref[def:derived_relations/transitive]{transitive} and \hyperref[def:derived_relations/symmetric]{symmetric} closure of \( E \). Thus, \( \sim \) is an equivalence relation.
...
\begin{proposition}\label{thm:undirected_graph_condensation}
  The \hyperref[def:graph_condensation]{condensation} of an undirected graph is an edgeless graph with one vertex.
\end{proposition}
\begin{proof}
  Reachability is a symmetric relation in an undirected graph. Therefore, the relation \( {\sim} \) in \fullref{def:graph_condensation} is only the transitive closure of \( E \), hence the relation \( {\sim} \) is the reachability relation. Thus, the quotient \( \widetilde{V} \) is a singleton set.

  There cannot be a loop by definition of \( \widetilde{E} \), hence \( \widetilde{E} \) is empty.
\end{proof}
...
\begin{definition}\label{def:graph_embedding}\mcite{nLab:topological_map}
  ...
  We will say that a functor \( F: \cat{G} \to \cat{X} \) is a \term{graph embedding} if it satisfies the following conditions:
  \begin{thmenum}
    \thmitem{def:graph_embedding/injective} \( F \) is injective on objects. That is, \( F(u) = F(v) \) implies \( u = v \).

    \thmitem{def:graph_embedding/intersection} For every two arcs \( e_1 \) and \( e_2 \), the intersection \( \img(F(e_1)) \cap \img(F(e_2)) \) of their curves contains only the images of the common endpoint of \( e_1 \) and \( e_2 \).

    \thmitem{def:graph_embedding/complement} The complement
    \begin{equation*}
      \mscrX \setminus \set{ \img(F(e)) \given e \in E }
    \end{equation*}
    is a \hyperref[def:disjoint_union]{disjoint union} of \hyperref[def:connected_space]{connected sets}. We call these connected sets \term{faces}.
  \end{thmenum}
\end{definition}

\begin{definition}\label{def:planar_graph}
  A directed graph \( G = (V, E) \) is \term{planar} if there exists a \hyperref[def:graph_embedding]{graph embedding} into the plane \( \BbbR^2 \).
\end{definition}
