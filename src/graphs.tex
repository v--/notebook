...
\begin{definition}\label{def:directed_graph}\mcite\cite[ch. 1, sec. 1.1]{Gondran1984}
  ...
  \begin{thmenum}
    ...
    \ilabel{def:directed_graph/simple}\mcite\cite[ch. 1, sec. 1.3]{Gondran1984}the graph \( G \) is \term{simple} if it has no loops.
  \end{thmenum}
\end{definition}
...
\begin{definition}\label{def:graph_matrices}
  ...
  \begin{thmenum}
    \ilabel{def:graph_matrices/incidence}\mcite\cite[ch. 1, sec. 2.1]{Gondran1984} The \term{incidence matrix} \( I = \{ a_{ve} \}_{v \in V, e \in E} \) of \( G \) is defined as
    \begin{equation*}
    ...
    \ilabel{def:graph_matrices/adjacency}\mcite\cite[ch. 1, sec. 2.3]{Gondran1984} The \term{adjacency matrix} \( A = \{ a_{ve} \}_{u, v \in V} \) of \( G \) is defined as
    \begin{equation*}
...
\begin{definition}\label{def:graph_paths}
  ...
  \begin{thmenum}
    ...
    \ilabel{def:graph_paths/adjacent_arcs}\mcite\cite[ch. 1, sec. 1.4]{Gondran1984} Two arcs are called \term{adjacent} if they have a common endpoint. Thus the first three pairs of arcs are adjacent and the fourth is not (assuming all vertices are distinct):
    \begin{enumerate}
    ...
    \ilabel{def:graph_paths/undirected_path}\mcite\cite[ch. 1, sec. 3.1]{Gondran1984} An \term{undirected path} or \term{chain} is a sequence of distinct arcs
    \begin{equation*}
    ...
    \ilabel{def:graph_paths/directed_path}\mcite\cite[ch. 1, sec. 3.2]{Gondran1984}If the tail of each non-endpoint arc in a path coincides with the head of the next arc, we say that the path is a \term{directed path}.
    ...
    \ilabel{def:graph_paths/eulerian_path}\mcite\cite[ch. 8, sec. 1.1]{Gondran1984}A path (either directed or undirected) is called \term{Eulerian} if it contains every arc of the graph exactly once, that is, the path induces an ordering of the arcs. A graph with an Eulerian cycle is called an \term{Eulerian graph}.

    \ilabel{def:graph_paths/hamiltonian_path}\mcite\cite[ch. 8, sec. 3.1]{Gondran1984}A simple path (either directed or undirected) is called \term{Hamiltonian} if it contains every vertex of the graph, that is, it induces an ordering of the vertices. A graph with a Hamiltonian cycle is called a \term{Hamiltonian graph}.
  \end{thmenum}
\end{definition}
...
\begin{definition}\label{def:graph_incidence}
  ...
  \begin{thmenum}
    ...
    \ilabel{def:graph_incidence/degree}\mcite\cite[ch. 1, sec. 1.4]{Gondran1984}Given a vertex \( v \), the \term{degree} \( d(v) \) (resp. \term{in-degree} \( d^+(v) \) and \term{out-degree} \( d^-(v) \)) of the vertex is defined as
    \begin{equation*}
...
\begin{definition}\label{def:graph_connectivity}
  ...
  \begin{thmenum}
    ...
    \ilabel{def:graph_connectivity/strongly_connected_graph}\mcite\cite[ch. 1, sec. 3.5]{Gondran1984}The graph \( G \) is \term{strongly connected} if every pair of distinct vertices are reachable, that is, if there exists a directed path between every pair of distinct vertices.

    \ilabel{def:graph_connectivity/weakly_connected_graph}\mcite\cite[ch. 1, sec. 3.3]{Gondran1984}The graph \( G \) is \term{weakly connected} if there exists an undirected path between every pair of distinct vertices.

    \ilabel{def:graph_connectivity/connected_component}\mcite\cite[ch. 1, sec. 3.3 \\ ch. 1, sec. 3.5]{Gondran1984}The subgraph \( G' \) of \( G \) is a \term{connected component} (resp. \term{strongly connected component}) if it is connected (resp. strongly connected) and there exists no connected (resp. strongly connected) subgraph of \( G \) that properly contains \( G' \).

    \ilabel{def:graph_connectivity/connectivity_number}\mcite\cite[ch. 1, sec. 3.3 \\ ch. 1, sec. 3.5]{Gondran1984}\( G \) has \term{connectivity number} (resp. \term{strong connectivity number}) \( n \) if it has \( n \) connected (resp. strongly connected) components.

    \ilabel{def:graph_connectivity/cut}\mcite\cite[ch. 1, sec. 3.4]{Gondran1984}The set \( U \subseteq V \) of vertices is a \term{cut} (resp. \term{directed cut}) if removing \( T \) from the graph would increase the connectivity number (resp. strong connectivity number) of the graph.

    \ilabel{def:graph_connectivity/cocycle}\mcite\cite[ch. 1, sec. 4.4]{Gondran1984}The set \( F \subseteq E \) of arcs is a \term{cocycle} (resp. \term{cocircuit}) if there exists a set \( U \subseteq V \) of vertices such that \( F = w(T) \) (resp. \( F \in \{ w^+(T), w^-(T) \} \)).
  \end{thmenum}
\end{definition}
...
\begin{definition}\label{def:graph_adjacency}
  ...
  \begin{thmenum}
    \ilabel{def:graph_adjacency/clique}\mcite\cite[ch. 1, sec. 1.4]{Gondran1984}The set \( U \subseteq V \) is called a \term{clique} if all two vertices in \( U \) are adjacent.

    \medskip

    \ilabel{def:graph_adjacency/complete_graph}\mcite\cite[ch. 1, sec. 1.4]{Gondran1984}If \( V \) itself is a clique, we say that \( G \) is a \term{complete graph}.
    ...
    \ilabel{def:graph_adjacency/matching}\mcite\cite[ch. 5, exer. 11]{Gondran1984}The set \( F \subseteq E \) of arcs is a \term{matching} or \term{independent set} of arcs if no two arcs in \( F \) are adjacent.

    \ilabel{def:graph_adjacency/bipartite_graph}\mcite\cite[7]{Gondran1984}The graph is called \term{bipartite} if there exists a partition \( \{ A, B \} \) of \( V \) such that both \( A \) and \( B \) are anticliques. We also write \( G = (A, B, E) \).
...
\begin{example}\label{ex:petersen_graph}\mcite\cite[347]{Gondran1984}
  The Petersen graph
...
\begin{definition}\label{def:category_of_graphs}
  ...
  We denote by \( \cat{UGraph} \) the category of undirected graphs. Given the convention established in \fullref{rem:directed_and_undirected_graphs} about undirected graphs being a special case of directed graphs, we can regard \( \bfop{UGraph} \) as a subcategory of \( \bfop{DGraph} \).
