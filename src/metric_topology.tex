...
\begin{proposition}\label{rem:bounded_set_metric_order_equivalence}
  A set \( A \) in a metric space \( (X, \rho) \) is \hyperref[def:metric_space/bounded_set]{bounded} if and only if the set \( \{ \rho(a, b) \colon a, b \in A \} \) is bounded as a \hyperref[def:poset_extremal_points/upper_and_lower_bounds]{poset}.
\end{proposition}
...
\begin{proof}
  ...
  \begin{refenum}
    ...
    Note that \( m \) exists by \fullref{thm:ordinal_properties/set_of_ordinals_has_minimum}.
...
\begin{proposition}\label{thm:metric_topology_properties}
  ...
  \begin{thmenum}
    \thmitem{thm:metric_topology_properties/ball_is_open} For every point \( x \in X \) and any radius \( r > 0 \), the ball \( B(x, r) \) is an open set and, hence, a neighborhood of \( x \).
    \thmitem{thm:metric_topology_properties/first_countable} \( \mscrT \) is first-countable.
    \thmitem{thm:metric_topology_properties/sequential} \( \mscrT \) is sequential.
    \thmitem{thm:metric_topology_properties/hausdorff} \( \mscrT \) is Hausdorff.
  \end{thmenum}
\end{proposition}
\begin{proof}
...
\end{proposition}
\begin{proof}
  If\DNE \( f(x) = f(x') \), then by \fullref{def:metric_space/M1}, \( x = x' \).
\end{proof}
