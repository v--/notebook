...
\begin{definition}\label{def:metric_space}\mcite\cite[248]{Engelking1989}
  ...
  \begin{thmenum}[series=def:metric_space]
    \labitem[def:metric_space/M1]{M1} \( \rho(x, y) = 0 \iff x = y \)
    \labitem[def:metric_space/M2]{M2}(symmetry) \( \rho(x, y) = \rho(y, x) \)
    \labitem[def:metric_space/M3]{M3}(triangle inequality) \( \rho(x, y) \leq \rho(x, z) + \rho(z, y) \)
  \end{thmenum}

  If instead of \ref{def:metric_space/M1} we have the weaker condition
  \begin{thmenum}[resume=def:metric_space]
    \labitem[def:metric_space/pseudometric_identity]{M1'} \( \forall x \in X, \rho(x, x) = 0 \),
  \end{thmenum}
  we call \( \rho \) a \term{pseudometric} and \( (X, \rho) \) a \term{pseudometric space}.

  \begin{thmenum}
    \labitem{def:metric_space/subspace} If \( A \subseteq X \) is a set, then the restriction \( (A, \rho{\rvert_A}) \) is a metric space and it is called a \term{subspace} of \( X \).

    \labitem{def:metric_space/ball} Define the function
    \begin{balign*}
    ...
    \labitem{def:metric_space/closed_ball} The set
    \begin{equation*}
      \overline{B(x, r)} \coloneqq \cl(B(x, r))
    \end{equation*}
    is called the \term{closed ball} with center \( x \) and radius \( r \).

    \labitem{def:metric_space/sphere} The set
    \begin{equation*}
      S(x, r) \coloneqq \fr{B(x, r)}
    \end{equation*}
    is called the \term{sphere} with center \( x \) and radius \( r \).

    \labitem{def:metric_space/bounded_set} A set \( A \subseteq X \) is called \term{bounded} if it is contained in some ball \( B(x, r) \).

    \labitem{def:metric_space/bounded_sequence} A \hyperref[def:sequence]{sequence} \( \{ x_k \}_{k=1}^\infty \subseteq X \) is called \term{bounded} if the corresponding set \( \{ x_k \colon k = 1, 2, \ldots \} \) is \hyperref[def:metric_space/bounded_set]{bounded}.

    \labitem{def:metric_space/bounded_metric} If every set is bounded, we say that the metric itself is bounded.

    \labitem{def:metric_space/bounded_function} We say that a function \( f: S \to X \) from a set \( S \) to a metric space \( (X, \rho) \) is \term{bounded} if its image \( f(S) \) is a bounded set in \( (X, \rho) \).

    \labitem{def:metric_space/diameter} Define the function
    \begin{balign*}
    ...
    \labitem{def:metric_space/distance} Define the function
    \begin{balign*}
...
\begin{definition}\label{def:metric_topology}\mcite\cite[249]{Engelking1989}
...
\begin{proposition}\label{thm:metric_topology_properties}
  ...
  \begin{thmenum}
    \labitem{thm:metric_topology_properties/ball_is_open} For every point \( x \in X \) and any radius \( r > 0 \), the ball \( B(x, r) \) is an open set and, hence, a neighborhood of \( x \).
    \labitem{thm:metric_topology_properties/first_countable} \( \mscrT \) is first-countable.
    \labitem{thm:metric_topology_properties/sequential} \( \mscrT \) is sequential.
    \labitem{thm:metric_topology_properties/hausdorff} \( \mscrT \) is Hausdorff.
  \end{thmenum}
\end{proposition}
\begin{proof}
  \SubProofOf{thm:metric_topology_properties/ball_is_open} Obvious from \fullref{def:metric_topology}.

  \SubProofOf{thm:metric_topology_properties/first_countable} Since \fullref{def:metric_topology} involves generating a topology using a neighborhood system of countable local neighborhoods, \( \mscrT \) is first-countable.
...
\begin{proof}
  ...
  \begin{reflist}
    ...
    \refitem{thm:uniform_space_base_axioms/BU2} Fix \( V_n \in \mscrV \) and \( m \coloneqq 2n \). By the triangle inequality, we have that if \( \rho(x, y) < m \) and \( \rho(y, z) < m \), then
    \begin{equation*}
...
\begin{theorem}\label{thm:countable_uniform_base_implies_metrizable}\mcite\cite[thm. 8.1.21]{Engelking1989}
  A uniform space \( X \) is metrizable if and only \( w(X) \leq \aleph_0 \).
\end{theorem}

\medskip

\begin{definition}\label{def:isometry}\mcite\cite[253]{Engelking1989}
