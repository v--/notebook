...
\begin{definition}\label{def:metric_space}\MarginCite[248]{Engelking1989}
  ...
  \begin{DefEnum}
    \ILabel{def:metric_space/subspace} If \( A \subseteq X \) is a set, then the restriction \( (A, \rho{\rvert_A}) \) is a metric space and it is called a \Def{subspace} of $X$.
...
\begin{definition}\label{def:metric_topology}\MarginCite[249]{Engelking1989}
  Let \( (X, \rho) \) be a metric space. We define the \Def{metric topology}\( \mscrT \) , also called the \Def{induced topology}, as the \hyperref[def:topological_space]{topology} generated by the \hyperref[def:topological_local_base]{neighborhood system}
  \begin{equation}\label{def:metric_topology/integer_base}
    \mathcal{B}(x) \coloneqq \{ B(x, \tfrac 1 n) \colon n = 1, 2, 3, \ldots \}.
  \end{equation}

  If for some topological space \( (X, \mscrT) \) there exists a metric such that \( \mscrT \) is its induced topology, we say that the topology \( \mscrT \) is \Def{metrizable}.
...
\begin{proposition}\label{thm:metric_topology_properties}
  ...
  \begin{DefEnum}
    ...
    \ILabel{thm:metric_topology_properties/first_countable} \( \mscrT \) is first-countable.
    \ILabel{thm:metric_topology_properties/sequential} \( \mscrT \) is sequential.
    \ILabel{thm:metric_topology_properties/hausdorff} \( \mscrT \) is Hausdorff.
  \end{DefEnum}
\end{proposition}
\begin{proof}
...
\begin{definition}\label{def:metric_uniformity}
  ...
  We define the \Def{metric uniformity} \( \mscrV \), also called the \Def{induced uniformity}, as the \hyperref[def:uniform_space]{uniformity} generated by the countable \hyperref[thm:uniform_space_base_axioms]{base}
...
\begin{proof}
  ...
  \begin{RefList}
    ...
    \IRef{thm:uniform_space_base_axioms/BU2} Fix \( V_n \in \mscrV \) and \( m \coloneqq 2n \). By the triangle inequality, we have that if \( \rho(x, y) < m \) and \( \rho(y, z) < m \), then
    \begin{equation*}
...
\begin{definition}\label{def:equivalent_metrics}
  Two metrics \( \rho \) and \( \nu \) on the set \( X \) are said to be \Def{equivalent} if \( \rho \) and \( \nu \) have the same \hyperref[def:metric_topology]{metric topology}. They are said to be \Def{strongly equivalent} if there exist constants \( \alpha, \beta \in \BbbR \) such that for every \( x, y \in X \) we have
  \begin{equation*}
...
\begin{definition}\label{def:translation_invariant_metric}
  A \hyperref[def:metric_space]{metric} \( \rho \) on a \hyperref[def:magma]{magma} \( G \) is said to be \Def{left translation-invariant} if
  \begin{equation*}
