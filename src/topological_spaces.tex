...
\begin{remark}\label{rem:constructing_topologies}
  ...
  \begin{thmenum}
    ...
    \thmitem{rem:constructing_topologies/subbase} By specifying only some family of open sets --- \fullref{thm:topology_from_subbase}.
  \end{thmenum}
\end{remark}
...
\begin{definition}\label{def:neighborhood_system}\mimprovised
...
\begin{proposition}\label{thm:topology_from_base}\mcite[12]{Engelking1989}
  ...
  Furthermore, \( \mscrB \) satisfies \fullref{def:topological_base/union} is thus a \hyperref[def:topological_base]{base} of \( \mscrT \).
\end{proposition}
\begin{proof}
...
\begin{example}\label{ex:def:topological_space_weight}
  ...
  \begin{thmenum}
    ...
    \thmitem{ex:def:topological_base/discrete} Let \( \mscrB \) be an arbitrary base of the \hyperref[def:discrete_topology]{discrete topology} \( \pow(X) \) on \( X \).
...
\begin{example}\label{ex:def:topological_subbase}
  ...
  \begin{thmenum}
    ...
    \thmitem{ex:def:topological_subbase/sierpinski} The family \( \set{ \set{ \top } } \) is a subbase of the \hyperref[def:sierpinski_space]{Sierpinski space}.
    ...
    \begin{equation*}
      \mscrP = \set{ \set{ x, y } \given x \in X \T{and} y \in X \T{and} x \neq y }.
    \end{equation*}
...
\begin{proposition}\label{thm:topology_from_subbase}
  ...
  Furthermore, \( \mscrP \) is a \hyperref[def:topological_subbase]{subbase} of the topology.
\end{proposition}
\begin{proof}
...
\begin{proposition}\label{thm:lattice_of_topologies}
  The family \( \mfrakT \) of all \hyperref[def:topological_space]{topologies} on a set \( X \) is a \hyperref[def:semilattice/lattice]{complete bounded lattice}. Explicitly:
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:lattice_of_topologies/top} The \hyperref[def:partially_ordered_set_extremal_points/top_and_bottom]{top element} is the \hyperref[def:discrete_topology]{discrete topology} on \( X \).
    ...
    \thmitem{thm:lattice_of_topologies/bottom} The \hyperref[def:partially_ordered_set_extremal_points/top_and_bottom]{bottom element} is the \hyperref[def:indiscrete_topology]{indiscrete topology} on \( X \).
  \end{thmenum}
\end{proposition}
\begin{proof}
  \SubProofOf{thm:lattice_of_topologies/join} Minimality is shown in \fullref{thm:topology_from_subbase}.
...
\begin{proposition}\label{thm:topological_local_base_axioms}\mcite[13]{Engelking1989}
  ...
  \end{equation*}
  is the \hyperref[thm:topology_from_base]{base} of some topology \( \mscrT \) on \( X \). Furthermore, \( \{ \mscrB(x) \subseteq \pow(X) \colon x \in X \} \) is a \hyperref[def:topological_local_base]{neighborhood system} for \( (X, \mscrT) \).
