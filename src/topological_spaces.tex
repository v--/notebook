...
\begin{remark}\label{rem:topological_space_countability}
  Among others, we have the following \hyperref[def:set_countability]{countability} conditions on topological spaces:
  \begin{thmenum}
    \thmitem{rem:topological_space_countability/cardinality} The cardinality of the space itself can be \hyperref[def:set_countability/at_most_countable]{at most countable}.

    \thmitem{rem:topological_space_countability/weight} The \hyperref[def:topological_space_weight]{weight} of a topology, under which spaces can be second-countable.

    \thmitem{rem:topological_space_countability/character} The \hyperref[def:topological_space_character]{character} of a topology, under which spaces can be first-countable.

    \thmitem{rem:topological_space_countability/density} The \hyperref[def:topological_space_density]{density} of a topology, under which spaces can be separable.
  \end{thmenum}
\end{remark}
...
\begin{example}\label{ex:def:topological_subbase}
  ...
  \begin{thmenum}
    ...
    \thmitem{ex:def:topological_subbase/indiscrete} The family \( \set[\Big]{ X } \) is a subbase for the \hyperref[def:indiscrete_topology]{indiscrete topology} on any set \( X \).
...
\begin{definition}\label{def:cluster_point}\mcite[24]{Engelking1989}
  ...
  Points of \( A \) that are not cluster points, i.e. points in \( A \setminus \drv(A) \), are said to be \term{isolated points} of \( A \).
\end{definition}
...
\begin{example}\label{ex:def:topologically_dense_set}
  ...
  \begin{thmenum}
    ...
    Every set in the base \eqref{def:metric_topology/real_base} is uncountable, hence, \( A \) does not contain any of them. Then \( A \) does not contain any open set. Since \( A \) is closed, \( A = \cl(A) \), and \( \Int(A) = \varnothing \) implies that \( \Int(\cl(A)) = \varnothing \). Therefore, \( A \) is nowhere dense.
  \end{thmenum}
\end{example}
...
\begin{proposition}\label{thm:properties_via_bases}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:properties_via_bases/isolated} \( x \) is an \hyperref[def:cluster_point/isolated_point]{isolated point} of \( A \) if and only \( \mscrB(x) \) contains a set disjoint from \( A \setminus \set{ x } \).
