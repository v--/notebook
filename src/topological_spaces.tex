...
\begin{definition}\label{def:topological_space}\mcite[11]{Engelking1989}
  A \term{topology} on a set \( X \) is a \hyperref[def:semilattice/submodel]{bounded sublattice} of the \hyperref[thm:boolean_algebra_of_subsets]{Boolean algebra of subsets} that is closed under arbitrary unions.
...
\begin{example}\label{ex:boolean_subalgebras_as_topological_spaces}
  Every \hyperref[def:boolean_algebra/submodel]{complete Boolean subalgebra} of \( \pow(X) \) is a topology on \( X \). Important examples are the discrete topology defined in \hyperref{def:discrete_topology}, the indiscrete topology defined in \hyperref{def:indiscrete_topology} and the topology of the Sierpinski space defined in \fullref{def:sierpinski_space}.
\end{example}
...
\begin{definition}\label{def:indiscrete_topology}
  The \term{indiscrete topology} on the set \( X \) is simply the trivial subalgebra \( \set{ 0, X } \) of \( \pow(X) \).
\end{definition}

\begin{definition}\label{def:sierpinski_topology}
  The initial lattice \( \set{ \top, \bot } \) endowed with the topology \( \set{ \varnothing, \set{ \top }, \set{ \top, \bot } } \) is called the \term{Sierpinski space}.

  \begin{figure}[!ht]
    \hfill
    \includegraphics[page=1]{output/def__sierpinski_space.pdf}
    \hfill\hfill
    \caption{The \hyperref[def:hasse_diagram]{Hasse diagram} of the topology of the \hyperref[def:sierpinski_space]{Sierpinski space}.}
...
\begin{definition}\label{def:topological_neighborhood}\mimprovised
...
\begin{proposition}\label{thm:neighborhood_filter}
  The \hyperref[def:topological_neighborhood]{neighborhood system} \( \mscrT(A) \) is a \hyperref[def:lattice_ideal/ideal]{filter} in \( \mscrT \).
\end{proposition}
\begin{proof}
...
\begin{proposition}\label{thm:set_open_iff_neighborhood_is_contained}
  A set \( A \) is open if and only if every point of \( A \) has a \hyperref[def:topological_neighborhood]{neighborhood} contained in \( A \).
\end{proposition}
\begin{proof}
...
\begin{proposition}\label{thm:topology_from_base}\mcite[12]{Engelking1989}
  ...
  \begin{equation}\label{eq:thm:topology_from_base/topology}
    \mscrT \coloneqq \set*{ \bigcup \mscrB' \colon \mscrB' \subseteq B }
  \end{equation}
  is a topology on \( X \).
...
\begin{example}\label{ex:topological_bases_with_same_cardinality}
  For an \hyperref[def:euclidean_space]{Euclidean space}, the \hyperref[def:metric_topology]{metric topology}
\end{example}
