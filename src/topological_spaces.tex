...
\begin{definition}\label{def:standard_topologies}
  ...
  \begin{defenum}
    ...
    \item\label{def:standard_topologies/co_cardinal} For any cardinal\Tinyref{def:cardinal} \( \xi \), the \textbf{co-\( \xi \) topology} \( \Cal{T} \coloneqq \{ A \subseteq X \colon \Card A < \xi \} \) and, in particular, \textbf{cofinite} (\( \xi = \aleph_0 \)) and \textbf{cocountable} (\( \xi = c \)) topologies.
  \end{defenum}
