...
\begin{proof}
  ...
  \begin{description}
    \RItem{def:topological_space/O1} \( \varnothing = \bigcup \varnothing \in \tau \) and \( X = \bigcup \CB \in \CT \) (by~\ref{thm:topological_base_axioms/B1})
    ...
    Define \( \CB' \coloneqq \bigcup_{\alpha \in A} \CB_\alpha \). Obviously \( \CB' \subseteq \CB \) and thus, by~\ref{thm:topological_base_axioms/B1}, \( \bigcup \CB \in \CT \).
...
\begin{proposition}\label{thm:subbase_from_arbitrary_family}
  Fix a set \( X \) and a family of subsets \( \CP \subseteq \Power(X) \). The family \( \CP' \coloneqq \CP \cup X \) is then a subbase\Tinyref{def:topological_subbase} of some topology on \( X \).
\end{proposition}
...
\begin{proposition}\label{thm:topological_local_base_axioms}\cite[13]{Engelking1989}
  ...
  \end{equation*}
  is the base\Tinyref{thm:topological_base_axioms} of some topology \( \CT \) on \( X \). Furthermore, \( \{ \CB(x) \subseteq \Power(X) \colon x \in X \} \) is a neighborhood system\Tinyref{def:topological_local_base} for \( (X, \CT) \).
...
\begin{proposition}\label{thm:derived_set_properties}
  ...
  \begin{propenum}
    ...
    \DItem{thm:derived_set_properties/closed_iff_contains_all_cluster_points} A set is closed if and only if it contains all of its cluster points. Compare this result to \fullref{thm:cluster_point_iff_in_closure}.
