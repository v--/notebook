...
\begin{remark}\label{rem:constructing_topologies}
  ...
  \begin{thmenum}
    \thmitem{rem:constructing_topologies/open} By specifying all open sets --- \fullref{def:topological_space}.
    \thmitem{rem:constructing_topologies/closed} By specifying all closed sets --- \fullref{thm:topology_from_closed_sets}.
    \thmitem{rem:constructing_topologies/base} By specifying only a \hyperref[def:topological_base]{base} of open sets --- \fullref{thm:topology_from_base}.
    \thmitem{rem:constructing_topologies/subbase} By specifying only some family of open sets --- \fullref{thm:topology_from_subbase}. This is the approach we use for order topologies in \fullref{def:order_topology}.
    \thmitem{rem:constructing_topologies/local_base} By specifying a \hyperref[def:topological_local_base]{local base} of open sets at every point --- \fullref{thm:topology_from_local_base}. This is the approach we use for metric spaces in \fullref{def:metric_topology} and uniform spaces in \fullref{def:uniform_topology}.
    \thmitem{rem:constructing_topologies/closure} By specifying a \hyperref[def:topological_closure_operator]{closure operator} --- \fullref{thm:topology_from_closure_operator}.
    \thmitem{rem:constructing_topologies/interior} By specifying an \hyperref[def:topological_interior_operator]{interior operator} --- \fullref{thm:topology_from_interior_operator}.
  \end{thmenum}
\end{remark}
...
\begin{proposition}\label{thm:set_open_iff_neighborhood_is_contained}
  A set \( A \) is open if and only if every point of \( A \) has a \hyperref[def:neighborhood_system]{neighborhood} contained in \( A \).
\end{proposition}
\begin{proof}
  ...
  \begin{equation*}
    B \coloneqq \bigcup_{x \in A} U_x.
  \end{equation*}

  Obviously \( B \subseteq A \). Aiming at a contradiction, suppose that there exists some point \( y_0 \in A \setminus B \). Then \( y_0 \) has a neighborhood \( U_{y_0} \) such that \( U_{y_0} \setminus B \) is nonempty. But this is impossible by the definition of \( B \). The obtained contradiction proves \( B = A \).
\end{proof}
...
\begin{proposition}\label{thm:topology_from_base}\mcite[12]{Engelking1989}
  ...
  \begin{thmenum}
    \thmitem[thm:topology_from_base/B1]{B1} \( \bigcup \mscrB = X \)
...
\begin{proposition}\label{thm:topological_base_can_generate_topology}
...
\begin{definition}\label{def:topological_space_weight}
  ...
  \begin{equation*}
    w(X, \mscrT) \coloneqq \min \set{ \card(\mscrB) \given \mscrB \text{ is a base for } \mscrT }.
  \end{equation*}
...
\begin{example}\label{ex:def:topological_space_weight}
  ...
  \begin{thmenum}
    ...
    \thmitem{ex:def:topological_space_weight/discrete} Let \( \mscrB \) be an arbitrary base of the \hyperref[def:discrete_topology]{discrete topology} \( \pow(X) \) on \( X \).
    ...
    \Fullref{thm:cardinal_exponentiation_comparison} then implies that \( \card X \leq \card \mscrB \). The base discussed in \fullref{ex:def:topological_base/discrete} has the cardinality of \( X \) itself, hence it is possible to attain equality.
...
\begin{definition}\label{def:topological_subbase}\mcite[12]{Engelking1989}
  ...
  \begin{equation}\label{eq:def:topological_subbase/base}
    \mscrB \coloneqq \set{ X } \cup \set*{ \bigcap \mscrP' \colon \mscrP' \T{is a finite nonempty subfamily of} \mscrP }
  \end{equation}
  of finite intersections of \( P \) is a \hyperref[def:topological_base]{base} of \( \mscrT \).

  Unlike in \fullref{thm:filter_from_prefilter}, we need the top element \( X \) to belong to \( \mscrB \).
\end{definition}

\begin{remark}\label{rem:subbase_and_empty_intersection}
  In the context of an ambient topological space, the intersection of empty sets discussed in \fullref{def:basic_set_operations/intersection} is safe to use. It can be utilized, for example, in \fullref{def:topological_subbase} to avoid the union with \( \set{ X } \).
...
\begin{example}\label{ex:def:topological_subbase}
  ...
  \begin{thmenum}
    ...
    \thmitem{ex:def:topological_subbase/superset} If \( \mscrP \) is a subbase of \( \mscrT \), then any set between \( \mscrP \) and \( \mscrT \) is also a subbase.

    \thmitem{ex:def:topological_subbase/indiscrete} The empty family is a subbase for the \hyperref[def:indiscrete_topology]{indiscrete topology} on any set.

    \thmitem{ex:def:topological_subbase/sierpinski} The family \( \set[\Big]{ \set{ \top } } \) is a subbase of the \hyperref[def:sierpinski_space]{Sierpinski space}.
    ...
    \begin{equation*}
      \mscrP = \set[\Big]{ \set{ x, y } \given x \in X \T{and} y \in X \T{and} x \neq y }.
    \end{equation*}
    ...
    \thmitem{ex:def:topological_base/order} \hyperref[def:order_topology]{Order topologies} have a base \eqref{eq:def:order_topology/base} constructed from the subbase \eqref{eq:def:order_topology/subbase}.
  \end{thmenum}
\end{example}

\begin{proposition}\label{thm:topology_from_subbase}
  Fix a set \( X \) and a family of subsets \( \mscrP \) of \( X \). Then the base \eqref{eq:def:topological_subbase/base} generates the coarsest topology on \( X \) containing \( \mscrP \).

  Furthermore, \( \mscrP \) is a \hyperref[def:topological_subbase]{subbase} of the topology. We say that the subbase \( \mscrP \) \term{generates} the topology.
\end{proposition}
\begin{proof}
...
\begin{proposition}\label{thm:topological_base_to_local_base}
  For any base \( \mscrB \), the subfamily \( \mscrB(x) \) of all members of \( \mscrB \) containing \( x \) is a local base at \( x \).
\end{proposition}
\begin{proof}
  Trivial.
\end{proof}
...
\begin{proposition}\label{thm:neighborhood_iff_union_in_topological_local_base}
  A set \( A \) containing \( x \) is a neighborhood of \( x \) if and only if \( A \) is a union of elements of some \hyperref[def:topological_local_base]{local base} \( \mscrB(x) \).

  Compare this to \fullref{def:topological_base/union}.
\end{proposition}
\begin{proof}
...
\begin{proposition}\label{thm:topology_from_local_base}\mcite[13]{Engelking1989}
  ...
  \end{equation*}
  is a \hyperref[thm:topology_from_base]{base} of the coarsest topology \( \mscrT \) containing \( \mscrB(x) \) for every \( x \in X \).

  Furthermore, \( \mscrB(x) \) is a \hyperref[def:topological_local_base]{local base} for \( \mscrT \) at \( x \).
\end{proposition}
\begin{proof}
  \SubProof{Proof that \( \mscrB \) is a base}
...
\begin{proposition}\label{thm:topological_local_base_can_generate_topology}
...
\end{proposition}
\begin{proof}
  Let \( (X, \mscrT) \) be a topological space and let \( \mscrB(x) \) be a local base for \( \mscrT \) at \( x \). As a \hyperref[def:prefilter]{prefilter} of \( \mscrT(x) \), \( \mscrB(x) \) consists of neighborhoods of \( x \) and every neighborhood of \( x \) has an open subset in \( \mscrB(x) \).
...
\begin{definition}\label{def:topological_space_character}
  ...
  \begin{equation*}
    \chi(x) \coloneqq \min \set{ \card \mscrB(x) \given \mscrB(x) \T{is a local base for} \mscrT \T{at} x }.
  \end{equation*}
...
\begin{definition}\label{def:topological_boundary}
  ...
  \begin{equation*}
    \fr: \pow(X) \to \pow(X)
  \end{equation*}
  via the equivalent definitions:
  \begin{thmenum}
    \thmitem{def:topological_boundary/closure} \( \fr(A) \coloneqq \cl(A) \setminus \Int(A) \)
    \thmitem{def:topological_boundary/neighborhoods} \( \fr(A) \) is the set of all points for which every neighborhood intersects both \( A \) and \( X \setminus A \).
  \end{thmenum}
\end{definition}
\begin{proof}
  The equivalence of the definitions is trivial when \( \fr(A) = \varnothing \). Assume that \( \fr(A) \neq \varnothing \).

  \ImplicationSubProof{def:topological_boundary/closure}{def:topological_boundary/neighborhoods} Let \( x \in \cl(A) \setminus \Int(A) \).

  Aiming for a contradiction, suppose that there is a neighborhood \( U \) of \( x \) that does not intersect \( A \). Then \( U \subseteq X \setminus A \). Hence, \( A \subseteq X \setminus U \). Since \( X \setminus U \) is closed, it follows that \( \cl(A) \subseteq X \setminus U \). But \( x \not\in X \setminus U \), therefore \( x \not\in \cl(A) \), which contradicts our choice of \( x \in \cl(A) \).
  ...
  \ImplicationSubProof{def:topological_boundary/neighborhoods}{def:topological_boundary/closure} Suppose that every neighborhood of \( x \) intersects both \( A \) and \( X \setminus A \). Therefore, no neighborhood of \( x \) is contained in neither \( A \) not \( X \setminus A \), and \( x \) belongs to neither \( \Int(A) \) nor \( \Int(X \setminus A) \). Hence,
  \begin{equation*}
    x
    \in
    \parens[\Big]{ X \setminus \Int(X \setminus A)) \setminus \Int(A) }
    \reloset {\ref{thm:interior_closure_complement}} =
    \cl(A) \setminus \Int(A).
  \end{equation*}
\end{proof}

\begin{proposition}\label{thm:def:topological_closure_operator}
  For any point \( x \in X \), \( x \in \cl A \) if and only if every neighborhood of \( x \) intersects \( A \).
\end{proposition}
\begin{proof}
  \SufficiencySubProof Fix \( x \in \cl{A} \), and let \( U \) be a neighborhood of \( x \).

  If \( x \in A \), then \( x \in U \cap A \), and \( U \) intersects \( A \).

  If \( x \not\in A \), then \( x \in \cl(A) \setminus \Int(A) \), and the equivalences in \fullref{def:topological_boundary} imply that every neighborhood of \( x \) intersects both \( \cl(A) \) and \( \Int(A) \). In particular, \( U \) intersects \( \Int(A) \subseteq A \).

  \NecessitySubProof Fix \( x \in X \) and assume that every neighborhood of \( x \) intersects \( A \). Since the case \( x \in A \) is trivial, suppose that \( x \not\in A \). By \fullref{thm:set_open_iff_neighborhood_is_contained}, every neighborhood \( U \) of \( x \) does not entirely belong to \( A \). Then \( x \in \fr A \subseteq \cl A \).
\end{proof}

\begin{proposition}\label{thm:def:topological_boundary}
...
\begin{proof}
  ...
  \SubProofOf{thm:def:topological_boundary/not_open} The set \( \fr(A) \) is either empty or is not open because \fullref{def:topological_boundary/neighborhoods} is incompatible with \fullref{thm:set_open_iff_neighborhood_is_contained}.

  \SubProofOf{thm:def:topological_boundary/complement} \Fullref{thm:interior_closure_complement} implies that
  \begin{balign*}
    \fr(X \setminus A)
    &=
    \cl(X \setminus A) \setminus \Int(X \setminus A)
    = \\ &=
    \cl(X \setminus A) \cap (X \setminus \Int(X \setminus A))
    \reloset {\ref{thm:interior_closure_complement}} = \\ &=
    (X \setminus \Int(A)) \cap \cl(A)
    = \\ &=
    \cl(A) \setminus \Int(A)
    = \\ &=
    \fr(A).
...
\begin{proposition}\label{thm:def:derived_set}
...
\begin{definition}\label{def:topologically_dense_set}\mcite[25]{Engelking1989}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:topologically_dense_set/codense} \term{codense} in \( X \) if \( X \setminus A \) is dense: \( \cl(X \setminus A) = X \).

    \thmitem{def:topologically_dense_set/nowhere_dense} \term{nowhere dense} in \( X \) if \( \cl(A) \) is codense: \( X = \cl(X \setminus \cl A) \reloset {\ref{thm:interior_closure_complement}} = \cl(\Int(X \setminus A)) \).

    \thmitem{def:topologically_dense_set/dense_in_itself} \term{dense in itself} if \( A \subseteq \derived(A) \): if \( A \) has no isolated points.
  \end{thmenum}
...
\begin{proposition}\label{thm:def:dense_set}
...
\begin{proposition}\label{thm:def:nowhere_dense}
