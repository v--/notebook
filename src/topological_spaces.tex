...
\begin{definition}\label{def:topological_space}\mcite\cite[11]{Engelking1989}
  ...
  \begin{thmenum}
    \labitem[def:topological_space/O1]{O1} \( \varnothing, X \in \mscrT \)
    \labitem[def:topological_space/O2]{O2} \( U, V \in \mscrT \implies U \cap V \in \mscrT \)
    \labitem[def:topological_space/O3]{O3} \( \mscrT' \subseteq \mscrT \implies \bigcap \mscrT' \in \mscrT \)
  \end{thmenum}
  ...
  \begin{thmenum}
    \labitem[def:topological_space/F1]{F1} \( \varnothing, X \in \mscrF \)
    \labitem[def:topological_space/F2]{F2} \( U, V \in \mscrF \implies U \cup V \in \mscrF \)
    \labitem[def:topological_space/F3]{F3} \( \mscrF' \subseteq \mscrF \implies \bigcup \mscrF' \in \mscrF \)
  \end{thmenum}
...
\begin{definition}\label{def:standard_topologies}
  ...
  \begin{thmenum}
    \labitem{def:standard_topologies/discrete} The \term{discrete topology} \( \mscrT \coloneqq \pow(X) \).
    \labitem{def:standard_topologies/indiscrete} The \term{indiscrete topology} \( \mscrT \coloneqq \{ \varnothing, X \} \).
    \labitem{def:standard_topologies/co_cardinal} For any \hyperref[def:cardinal]{cardinal} \( \xi \), the \term{co-\( \xi \) topology} \( \mscrT \coloneqq \{ A \subseteq X \colon \card A < \xi \} \) and, in particular, \term{cofinite} (\( \xi = \aleph_0 \)) and \term{cocountable} (\( \xi = c \)) topologies.
  \end{thmenum}
...
\begin{definition}\label{def:topological_base}\mcite\cite[12]{Engelking1989}
  ...
  \begin{thmenum}
    \labitem{def:topological_base/union} Every open set \( U \in \mscrT \) is the union \( U = \bigcup \mscrB' \) of some subset \( \mscrB' = \mscrB \)
    \labitem{def:topological_base/subset} For any point \( x \in X \) and for any neighborhood \( U \) of \( x \) there exists a set \( V \in \mscrB \) in the base such that \( x \in V \subseteq U \)
  \end{thmenum}
\end{definition}
\begin{proof}
...
\begin{proposition}\label{thm:topological_base_axioms}\mcite\cite[12]{Engelking1989}
  ...
  \begin{thmenum}
    \labitem[thm:topological_base_axioms/B1]{B1} \( \bigcup \mscrB = X \)
    \labitem[thm:topological_base_axioms/B2]{B2} \( \forall U, V \in \mscrB, \forall x \in U \cap V, \exists W \in \mscrB: x \in W \subseteq U \cap V \)
  \end{thmenum}
...
\begin{proposition}\label{thm:topological_local_base_axioms}\mcite\cite[13]{Engelking1989}
  ...
  \begin{thmenum}
    \labitem[thm:topological_local_base_axioms/BP1]{BP1} For every \( x \in X \), \( \mscrB(x) \neq \varnothing \) and \( x \in U \) for every \( U \in \mscrB(x) \).
    \labitem[thm:topological_local_base_axioms/BP2]{BP2} For every \( x \in X \) and for all \( U, V \in \mscrB(x) \), \( \exists W \in \mscrB(x): W \subseteq U \cap V \).
    \labitem[thm:topological_local_base_axioms/BP3]{BP3} For all \( x, y \in X \), \( x \in U \in \mscrB(y) \) implies that there exists \( V \in \mscrB(x) \) such that \( U \subseteq V \).
  \end{thmenum}
...
\begin{proposition}\label{thm:closure_operator_properties}
  ...
  \begin{thmenum}
    \labitem{thm:closure_operator_properties/closed} The set \( A \) is closed if and only if \( A = \cl A \).
    \labitem{thm:closure_operator_properties/neighborhood_intersection} For any \( x \in X \), \( x \in \cl A \) if and only if every neighborhood of \( x \) intersects \( A \).
    \labitem{thm:closure_operator_properties/monotone} \( \cl \) is \hyperref[def:monotone_map]{monotone}, i.e. if \( A \subseteq B \), then \( \cl(A) \subseteq \cl(B) \).
  \end{thmenum}
\end{proposition}
\begin{proof}
...
\begin{proposition}\label{thm:closure_operator_axioms}\mcite\cite[14]{Engelking1989}
  ...
  \begin{thmenum}
    \labitem[thm:closure_operator_axioms/CO1]{CO1} \( \cl(\varnothing) = \varnothing \)
    \labitem[thm:closure_operator_axioms/CO2]{CO2} \( \forall A \in \pow(X), A \subseteq \cl(A) \)
    \labitem[thm:closure_operator_axioms/CO3]{CO3} \( \forall A, B \in \pow(X), \cl(A \cup B) = \cl(A) \cup \cl(B) \)
    \labitem[thm:closure_operator_axioms/CO4]{CO4} \( \forall A \in \pow(X), \cl(\cl(A)) = \cl(A) \)
  \end{thmenum}
...
\begin{proposition}\label{thm:interior_operator_properties}
  ...
  \begin{thmenum}
    \labitem{thm:interior_operator_properties/open} A set \( A \) is a topological space is open if and only if \( A = \int A \).
    \labitem{thm:interior_operator_properties/monotone} \( \int \) is \hyperref[def:monotone_map]{monotone}, i.e. if \( A \subseteq B \), then \( \inter(A) \subseteq \inter(B) \).
  \end{thmenum}
\end{proposition}
\begin{proof}
...
\begin{proposition}\label{thm:interior_operator_axioms}
  ...
  \begin{thmenum}
    \labitem[thm:interior_operator_axioms/IO1]{IO1} \( \inter(X) = X \)
    \labitem[thm:interior_operator_axioms/IO2]{IO2} \( \forall A \in \pow(X), \inter(A) \subseteq A \)
    \labitem[thm:interior_operator_axioms/IO3]{IO3} \( \forall A, B \in \pow(X), \inter(A \cap B) = \inter(A) \cap \inter(B) \)
    \labitem[thm:interior_operator_axioms/IO4]{IO4} \( \forall A \in \pow(X), \inter(\inter(A)) = \inter(A) \)
  \end{thmenum}
...
\begin{definition}\label{def:topological_boundary}
  ...
  \begin{thmenum}
    \labitem{def:topological_boundary/closure} \( \fr(A) \coloneqq \cl(A) \setminus \inter(A) \)
    \labitem{def:topological_boundary/neighborhoods} \( \fr(A) \) is the set of all points \( x \in X \) such that every neighborhood of \( x \) intersects both \( A \) and \( X \setminus A \).
  \end{thmenum}
\end{definition}
\begin{proof}
...
\begin{proposition}\label{thm:topological_boundary_properties}
  ...
  \begin{thmenum}
    \labitem{thm:topological_boundary_properties/closed} \( \fr(A) \) is a closed set.
    \labitem{thm:topological_boundary_properties/not_open} If \( \fr(A) \) is not empty, it is not an open set.
    \labitem{thm:topological_boundary_properties/complement} \( \fr(A) = \fr(X \setminus A) \).
  \end{thmenum}
\end{proposition}
\begin{proof}
...
\begin{definition}\label{def:topological_derived_set}\mcite\cite[24]{Engelking1989}
  ...
  \begin{thmenum}
    \labitem{def:topological_derived_set/cluster_point} We say that the point \( x_0 \in X \) is a \term{cluster point} or an \term{accumulation point} of the set \( A \subseteq X \) if \( x \in \cl(A \setminus \{ x \}) \). It is not necessary for \( x_0 \) to belong to \( A \).

    \labitem{def:topological_derived_set/derived_set} The set of all cluster points of \( A \) is called the \term{derived set} of \( A \) and is denoted by \( \derived(A) \).

    \labitem{def:topological_derived_set/perfect_set} If a set equals its derived set, we call it a \term{perfect set}.

    \labitem{def:topological_derived_set/isolated_point} Points in \( A \setminus \derived(A) \) are said to be \term{isolated points} of \( A \).

    \labitem{def:topological_derived_set/discrete_set} If \( \derived(A) = \varnothing \), that is, if \( A \) consists of only discrete points, we say that \( A \) is a \term{discrete set}.
  \end{thmenum}
\end{definition}
...
\begin{proposition}\label{thm:derived_set_properties}
  ...
  \begin{thmenum}
    \labitem{thm:derived_set_properties/cluster_via_neighborhoods} \( x \) is a cluster point of \( A \) if and only if every neighborhood of \( x \) intersects \( A \setminus \{ x \} \)
    \labitem{thm:derived_set_properties/isolated_via_neighborhoods} \( x \) is an isolated point of \( A \) if and only if there exists a neighborhood of \( x \) that does not intersect \( A \setminus \{ x \} \)
    \labitem{thm:derived_set_properties/closed} \( \derived(A) \) is a closed set.
    \labitem{thm:derived_set_properties/closure} \( A \cup \derived(A) = \cl(A) \).
    \labitem{thm:derived_set_properties/closed_iff_contains_all_cluster_points} A set is closed if and only if it contains all of its cluster points. Compare this result to \fullref{thm:limit_point_iff_in_closure}.
    \labitem{thm:derived_set_properties/closed_iff_only_isolated_and_cluster_points} A set if closed if and only if every point is either a cluster point or an isolated point.
  \end{thmenum}
\end{proposition}
\begin{proof}
...
\begin{definition}\label{def:topologically_dense_set}\mcite\cite[25]{Engelking1989}
  ...
  \begin{thmenum}
    \labitem{def:topologically_dense_set/dense} \term{dense} in \( X \) if \( \cl{A} = X \) (if \( X \) is assumed from the context, we simply say that \( A \) is dense).

    \labitem{def:topologically_dense_set/codense} \term{codense} in \( X \) if \( X \setminus A \) is dense, i.e. \( \cl(X \setminus A) = X \).

    \labitem{def:topologically_dense_set/nowhere_dense} \term{nowhere dense} in \( X \) if \( \cl(A) \) is codense, i.e. \( X = \cl(X \setminus \cl A) \overset {\ref{thm:interior_closure_complement}} = \cl(\inter(X \setminus A)) \).

    \labitem{def:topologically_dense_set/dense_in_itself} \term{dense in itself} if \( A \subseteq \derived(A) \), i.e. if \( A \) has no isolated points.
  \end{thmenum}
...
\begin{proposition}\label{thm:dense_set_properties}
  ...
  \begin{thmenum}
    \labitem{thm:dense_set_properties/open_intersection}\mcite\cite[prop. 1.3.5]{Engelking1989} The set \( A \) is dense if and only if every nonempty open set intersects \( A \).
  \end{thmenum}
\end{proposition}
\begin{proof}
...
\begin{proposition}\label{thm:nowhere_dense_properties}
  ...
  \begin{thmenum}
    \labitem{thm:nowhere_dense_properties/empty_interior} Nowhere dense sets have an empty interior
    \labitem{thm:nowhere_dense_properties/contained_in_boundary} Nowhere dense sets are entirely contained in their boundaries.
    \labitem{thm:nowhere_dense_properties/interior_of_closure} The set \( A \) is nowhere dense if and only if \( \inter(\cl(A)) = \varnothing \).
    \labitem{thm:nowhere_dense_properties/closure_contains_no_open_set} The set is nowhere dense if and only if its closure does not contain a nonempty open set.
    \labitem{thm:nowhere_dense_properties/open_subset}\mcite\cite[prop. 1.3.5]{Engelking1989}The set \( A \) is nowhere dense if and only if every open set contains a nonempty open subset disjoint from \( A \).
    \labitem{thm:nowhere_dense_properties/subset} A subset of a nowhere dense set is nowhere dense.
    \labitem{thm:nowhere_dense_properties/homeomorphism} The \hyperref[def:homeomorphism]{homeomorphic} image of a nowhere dense set is nowhere dense.
    \labitem{thm:nowhere_dense_properties/complement_dense} A set is closed and nowhere dense if and only if its complement is open and dense.
  \end{thmenum}
\end{proposition}
\begin{proof}
