...
\begin{proposition}\label{thm:interior_operator_properties}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:interior_operator_properties/monotone} \( \int \) is \hyperref[def:partially_ordered_set/homomorphism]{monotone}, i.e. if \( A \subseteq B \), then \( \inter(A) \subseteq \inter(B) \).
  \end{thmenum}
\end{proposition}
\begin{proof}
...
\begin{definition}\label{def:topologically_dense_set}\mcite[25]{Engelking1989}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:topologically_dense_set/nowhere_dense} \term{nowhere dense} in \( X \) if \( \cl(A) \) is codense, i.e. \( X = \cl(X \setminus \cl A) \reloset {\ref{thm:interior_closure_complement}} = \cl(\inter(X \setminus A)) \).
