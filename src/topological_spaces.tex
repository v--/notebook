...
\begin{definition}\label{def:topological_space}\cite[11]{Engelking1989}
  Let \( X \) be any set and \( \Cal{T} \subseteq \Power(X) \) be a family of subsets of \( X \). \( \Cal{T} \) is called a \underLine{topology on \( X \)} and the tuple \( (X, \Cal{T}) \) is said to be a \underLine{topological space} if the following axioms are satisfied:
  \begin{description}
  ...
  Elements of the set \( X \) are called \underLine{points of the topological space}, elements of \( \Cal{T} \) are called \underLine{open sets} and set-theoretic complements of open sets are called \underLine{closed sets}.

  If \( x \in U \in \Cal{T} \), we say that \( U \) is a \underLine{neighborhood of \( x \)}. Note that some authors (e.g.~\cite[38]{Kelley1955}) alternatively define neighborhoods as arbitrary sets that contain an open set that contains \( x \).
...
\begin{definition}\label{def:standard_topologies}
  ...
  \begin{defenum}
    \item\label{def:standard_topologies/discrete} The \underLine{discrete topology} \( \Cal{T} \coloneqq \Power(X) \).
    \item\label{def:standard_topologies/indiscrete} The \underLine{indiscrete topology} \( \Cal{T} \coloneqq \{ \varnothing, X \} \).
    \item\label{def:standard_topologies/co_cardinal} For any cardinal\Tinyref{def:cardinal} \( \xi \), the \underLine{co-\( \xi \) topology} \( \Cal{T} \coloneqq \{ A \subseteq X \colon \Card A < \xi \} \) and, in particular, \underLine{cofinite} (\( \xi = \aleph_0 \)) and \underLine{cocountable} (\( \xi = c \)) topologies.
  \end{defenum}
...
\begin{definition}\label{def:topological_base}\cite[12]{Engelking1989}
  Fix a topological space \( (X, \Cal{T}) \). We say that the family \( \Cal{B} \subseteq \Cal{T} \) is a \underLine{base for the topology \( \Cal{T} \)} if \( \Cal{B} \) satisfies any of the equivalent conditions
  \begin{defenum}
...
\begin{definition}\label{def:topological_space_weight}
  We define the \underLine{weight of \( (X, \Cal{T}) \)} as the cardinal
  \begin{align*}
  ...
  Spaces for which \( w(X) \leq \aleph_0 \) are said to be \underLine{second-countable}.
\end{definition}
\begin{proof}
...
\begin{definition}\label{def:topological_subbase}\cite[12]{Engelking1989}
  Fix a topological space \( (X, \Cal{T}) \). We say that the family \( \Cal{P} \subseteq \Cal{T} \) is a \underLine{subbase for the topology \( \Cal{T} \)} if \( \FI(\Cal{P}) \)\Tinyref{def:finite_intersection_operator}) is a base\Tinyref{def:topological_base} of \( \Cal{T} \).
\end{definition}
...
\begin{definition}\label{def:topological_local_base}\cite[12]{Engelking1989}
  Fix a topological space \( (X, \Cal{T}) \) and a point \( x \in X \). We say that the family \( \Cal{B}(x) \) is a \underLine{local base for \( \Cal{T} \) at \( x \)} if for every neighborhood \( U \) of \( x \) there exists a set \( V \in \Cal{B}(x) \) such that \( x \in V \subseteq U \).

  The indexed family of local bases \( \{ \Cal{B}(x) \colon x \in X \} \) is called a \underLine{neighborhood system} of \( \Cal{T} \).
\end{definition}
...
\begin{definition}\label{def:topological_space_character}
  We define the \underLine{character of the point \( x \in X \)} as the cardinal
  \begin{align*}
    \chi(x) \coloneqq \min \{ \Abs{\Cal{B(x)}} \colon \Cal{B(x)} \text{ is a local base for } \Cal{T} \text{ at } x \}.
  \end{align*}

  We define the \underLine{character of of \( (X, \Cal{T}) \)} as
  \begin{align*}
  ...
  Spaces for which \( \chi(X) \leq \aleph_0 \) are said to be \underLine{first-countable}.
\end{definition}
\begin{proof}
...
\begin{definition}\label{def:closure_operator}\cite[33]{Engelking1989}
  Let \( (X, \Cal{T}) \) be a topological space. Define the \underLine{closure operator}
  \begin{align*}
...
\begin{definition}\label{def:interior_operator}\cite[15]{Engelking1989}
  Let \( (X, \Cal{T}) \) be a topological space. Define the \underLine{interior operator}
  \begin{align*}
...
\begin{definition}\label{def:topological_boundary}\cite[24]{Engelking1989}
  For a subset \( A \) of a topological space we define the \underLine{boundary}
  \begin{align*}
...
\begin{definition}\label{def:topologically_derived_set}\cite[24]{Engelking1989}
  ...
  \begin{defenum}
    \item\label{def:topologically_derived_set/accumulation_point} We say that the point \( x \in X \) is an \underLine{accumulation point of the set \( A \subseteq X \)} if \( x \in \Cl(A \setminus \{ x \}) \). It is not necessary for \( x \) to belong to \( A \).

    \item\label{def:topologically_derived_set/derived_set} The set of all accumulation points of \( A \) is called the \underLine{derived set of \( A \)} and is denoted by \( A^d \).

    \item\label{def:topologically_derived_set/isolated_point} Points in \( A \setminus A^d \) are said to be \underLine{isolated points of \( A \)}.
  \end{defenum}
\end{definition}
...
\begin{definition}\label{def:topologically_dense_set}\cite[25]{Engelking1989}
  ...
  \begin{defenum}
    \item\label{def:topologically_dense_set/dense} \underLine{dense in \( X \)} if \( \Cl{A} = X \) (if \( X \) is assumed from the context, we simply say that \( A \) is dense).

    \item\label{def:topologically_dense_set/codense} \underLine{codense in \( X \)} if \( X \setminus A \) is dense, i.e. \( \Cl{X \setminus A} = X \).

    \item\label{def:topologically_dense_set/nowhere_dense} \underLine{nowhere dense in \( X \)} if \( \Cl{A} \) is codense, i.e. \( \Cl{X \setminus \Cl{A}} = X \).

    \item\label{def:topologically_dense_set/dense_in_itself} \underLine{dense in itself} if \( A \subseteq A^d \), i.e. if \( A \) has no isolated points.
  \end{defenum}

  We define the \underLine{density \( d(X) \) of \( X \)} to be the minimum cardinality\Tinyref{def:cardinal} of all dense sets. If \( d(X) \leq \aleph_0 \), we say that the space is separable.
\end{definition}
