...
\begin{definition}\label{def:topological_space}\mcite[11]{Engelking1989}
  ...
  Elements of \( X \) are called \term{points} of the topological space, elements of \( \mscrT \) are called \term{open sets} and \hyperref[thm:boolean_algebra_of_subsets/complement]{set-theoretic complements} of open sets are called \term{closed sets}.
\end{definition}
...
\begin{proposition}\label{thm:topology_from_closed_sets}
  ...
  \begin{thmenum}
    \thmitem[def:topological_space/F1]{F1} \( \mscrF \) contains both \( \varnothing \) and \( X \).
    \thmitem[def:topological_space/F2]{F2} \( \mscrF \) is closed under finite unions: \( U, V \in \mscrF \) implies \( U \cup V \in \mscrF \).
    \thmitem[def:topological_space/F3]{F3} \( \mscrF \) is closed under arbitrary intersections: \( \mscrF' \subseteq \mscrF \) implies \( \bigcap \mscrF' \in \mscrF \).
  \end{thmenum}
\end{proposition}
\begin{proof}
  \SubProofOf{def:topological_space/O1} Follows directly from \ref{def:topological_space/F1}.
  \SubProofOf{def:topological_space/O2} If \( F, G \in \mscrF \), then \ref{def:topological_space/F2} implies \( F \cup G \in \mscrF \).
  ...
  \SubProofOf{def:topological_space/O3} Follows from \ref{def:topological_space/F3} via \fullref{thm:de_morgans_laws_for_sets}.
\end{proof}
...
\begin{proposition}\label{thm:def:interior_operator}
