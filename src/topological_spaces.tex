...
\begin{definition}\label{def:topological_space}\cite[11]{Engelking1989}
  Let \( X \) be any set and \( \Cal{T} \subseteq \Power(X) \) be a family of subsets of \( X \). \( \Cal{T} \) is called a \Def{topology on \( X \)} and the tuple \( (X, \Cal{T}) \) is said to be a \Def{topological space} if the following axioms are satisfied:
  \begin{description}
  ...
  Elements of the set \( X \) are called \Def{points of the topological space}, elements of \( \Cal{T} \) are called \Def{open sets} and set-theoretic complements of open sets are called \Def{closed sets}.

  If \( x \in U \in \Cal{T} \), we say that \( U \) is a \Def{neighborhood} of \( x \). Note that some authors (e.g. \cite[38]{Kelley1955}) alternatively define neighborhoods as arbitrary sets that contain an open set that contains \( x \). For simplicity, we define the subfamily
  \begin{equation*}
...
\begin{definition}\label{def:standard_topologies}
  ...
  \begin{defenum}
    \DItem{def:standard_topologies/discrete} The \Def{discrete topology} \( \Cal{T} \coloneqq \Power(X) \).
    \DItem{def:standard_topologies/indiscrete} The \Def{indiscrete topology} \( \Cal{T} \coloneqq \{ \varnothing, X \} \).
    \DItem{def:standard_topologies/co_cardinal} For any cardinal\Tinyref{def:cardinal} \( \xi \), the \Def{co-\( \xi \) topology} \( \Cal{T} \coloneqq \{ A \subseteq X \colon \Card A < \xi \} \) and, in particular, \Def{cofinite} (\( \xi = \aleph_0 \)) and \Def{cocountable} (\( \xi = c \)) topologies.
  \end{defenum}
...
\begin{proof}
  ...
  \begin{description}
    \Implies Assume that \( A \) is open and \( x \in A \). Then \( A \) is a neighborhood of \( x \) and the theorem holds trivially.
...
\begin{definition}\label{def:topological_base}\cite[12]{Engelking1989}
  Fix a topological space \( (X, \Cal{T}) \). We say that the family \( \Cal{B} \subseteq \Cal{T} \) is a \Def{base for the topology \( \Cal{T} \)} if \( \Cal{B} \) satisfies any of the equivalent conditions
  \begin{defenum}
...
\begin{definition}\label{def:topological_space_weight}
  We define the \Def{weight of \( (X, \Cal{T}) \)} as the cardinal
  \begin{equation*}
...
\begin{definition}\label{def:topological_subbase}\cite[12]{Engelking1989}
  Fix a topological space \( (X, \Cal{T}) \). We say that the family \( \Cal{P} \subseteq \Cal{T} \) is a \Def{subbase} for the topology \( \Cal{T} \) if the family
  \begin{equation*}
    \Cal{B} \coloneqq \left\{ \bigcap P' \colon P' \text{ is a nonempty finite\Tinyref{def:finite_set} subset of } P \right\}
  \end{equation*}
  of finite intersections of \( \Cal{P} \) is a base\Tinyref{def:topological_base} of \( \Cal{T} \).
\end{definition}
...
\begin{definition}\label{def:topological_local_base}\cite[12]{Engelking1989}
  Fix a topological space \( (X, \Cal{T}) \) and a point \( x \in X \). We say that the family \( \Cal{B}(x) \) is a \Def{local base for \( \Cal{T} \) at \( x \)} if for every neighborhood \( U \) of \( x \) there exists a set \( V \in \Cal{B}(x) \) such that \( x \in V \subseteq U \).

  The indexed family of local bases \( \{ \Cal{B}(x) \colon x \in X \} \) is called a \Def{neighborhood system} of \( \Cal{T} \).
\end{definition}
...
\begin{definition}\label{def:topological_space_character}
  ...
  We define the \Def{character of of \( (X, \Cal{T}) \)} as
  \begin{equation*}
...
\begin{definition}\label{def:closure_operator}\cite[33]{Engelking1989}
  Let \( (X, \Cal{T}) \) be a topological space. Define the \Def{closure operator}
  \begin{align*}
...
\begin{definition}\label{def:interior_operator}\cite[15]{Engelking1989}
  Let \( (X, \Cal{T}) \) be a topological space. Define the \Def{interior operator}
  \begin{align*}
...
\begin{proof}
  ...
  \begin{align*}
    X \setminus \Int(A)
    &=
    X \setminus \bigcup \{ U : U \in \Cal{T}, U \subseteq A \}
    = \\ &=
    X \setminus \bigcup \{ F : F \in \Cal{F}_{\Cal{T}}, X \setminus A \subseteq F \}
    \overset {X \setminus (X \setminus A) = A} = \\ &=
    \bigcup \{ F : F \in \Cal{F}_{\Cal{T}}, F \subseteq A \}.
    = \\ &=
    \Cl(A).
  \end{align*}
...
\begin{definition}\label{def:topological_boundary}
  For a subset \( A \) of a topological space we define its \Def{boundary} \( \partial(A) \) equivalently as
  \begin{defenum}
    \DItem{def:topological_boundary/closure} \( \partial(A) \coloneqq \Cl(A) \setminus \Int(A) \)
    \DItem{def:topological_boundary/neighborhoods} \( \partial(A) \) is the set of all points \( x \in X \) such that every neighborhood of \( x \) intersects both \( A \) and \( X \setminus A \).
  \end{defenum}
\end{definition}
\begin{proof}
  The equivalence of the definitions is trivial when \( \partial(A) = \varnothing \). We assume that \( \partial(A) \neq \varnothing \).
  ...
  \begin{description}
    ...
    \Implies[def:topological_boundary/neighborhoods][def:topological_boundary/closure] Suppose that every neighborhood of \( x \in \partial(A) \) intersects both \( A \) and \( X \setminus A \). Therefore no neighborhood of \( x \) is contained in neither \( A \) not \( X \setminus A \) and \( x \) belongs to neither \( \Int(A) \) nor \( \Int(X \setminus A) \). Hence
    \begin{equation*}
...
\begin{proposition}\label{thm:topological_boundary_properties}
  ...
  \begin{thmenum}
    \DItem{thm:topological_boundary_properties/closed} \( \partial(A) \) is a closed set.
    \DItem{thm:topological_boundary_properties/not_open} If \( \partial(A) \) is not empty, it is not an opens set.
    \DItem{thm:topological_boundary_properties/complement} \( \partial(A) = \partial(X \setminus A) \).
  \end{thmenum}
\end{proposition}
\begin{proof}
  \begin{description}
    ...
    \begin{equation*}
      \partial(A) = \Cl(A) \setminus \Int(A) = \Cl(A) \cap (X \setminus \Int(A)),
    \end{equation*}
    which is the intersection of two closed sets. Hence \( \partial(A) \) is itself a closed set.

    \RItem{thm:topological_boundary_properties/not_open} Note that \( \partial(A) \) is either empty or is not open because \cref{def:topological_boundary/neighborhoods} is incompatible with \cref{thm:set_open_iff_neighborhood_is_contained}.
...
\begin{definition}\label{def:topologically_derived_set}\cite[24]{Engelking1989}
  ...
  \begin{defenum}
    \DItem{def:topologically_derived_set/accumulation_point} We say that the point \( x \in X \) is a \Def{cluster point} of the set \( A \subseteq X \) if \( x \in \Cl(A \setminus \{ x \}) \). It is not necessary for \( x \) to belong to \( A \).

    \DItem{def:topologically_derived_set/derived_set} The set of all cluster points of \( A \) is called the \Def{derived set} of \( A \) and is denoted by \( A^d \).

    \DItem{def:topologically_derived_set/isolated_point} Points in \( A \setminus A^d \) are said to be \Def{isolated points} of \( A \).

    \DItem{def:topologically_derived_set/discrete_set} If a set \( A \) consists of only discrete points, we say that \( A \) is a \Def{discrete set}.
  \end{defenum}
\end{definition}
...
\begin{definition}\label{def:topologically_dense_set}\cite[25]{Engelking1989}
  ...
  \begin{defenum}
    ...
    \DItem{def:topologically_dense_set/dense_in_itself} \Def{dense in itself} if \( A \subseteq A^d \), i.e. if \( A \) has no isolated points.
  \end{defenum}
