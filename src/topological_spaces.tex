...
\begin{definition}\label{def:topological_space}\cite[11]{Engelking1989}
  ...
  \begin{equation*}
    \T(x) \coloneqq \{ U \in \Cal{T} \colon x \in U \}.
  \end{equation*}
...
\begin{definition}\label{def:topological_subbase}\cite[12]{Engelking1989}
  Fix a topological space \( (X, \Cal{T}) \). We say that the family \( \Cal{P} \subseteq \Cal{T} \) is a \Def{subbase for the topology \( \Cal{T} \)} if \( \FI(\Cal{P}) \)\Tinyref{def:finite_intersection_operator} is a base\Tinyref{def:topological_base} of \( \Cal{T} \).
\end{definition}
...
\begin{definition}\label{def:topologically_derived_set}\cite[24]{Engelking1989}
  ...
  \begin{defenum}
    \DItem{def:topologically_derived_set/accumulation_point} We say that the point \( x \in X \) is an \Def{accumulation point of the set \( A \subseteq X \)} if \( x \in \Cl(A \setminus \{ x \}) \). It is not necessary for \( x \) to belong to \( A \).

    \DItem{def:topologically_derived_set/derived_set} The set of all accumulation points of \( A \) is called the \Def{derived set of \( A \)} and is denoted by \( A^d \).
...
\begin{definition}\label{def:topologically_dense_set}\cite[25]{Engelking1989}
  ...
  We define the \Def{density \( d(X) \) of \( X \)} to be the minimum cardinality\Tinyref{def:cardinal} of all dense sets. If \( d(X) \leq \aleph_0 \), we say that the space is separable.
\end{definition}
