...
\begin{proposition}\label{thm:closure_operator_properties}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:closure_operator_properties/monotone} \( \cl \) is \hyperref[def:partially_ordered_set/homomorphism]{monotone}, i.e. if \( A \subseteq B \), then \( \cl(A) \subseteq \cl(B) \).
  \end{thmenum}
\end{proposition}
\begin{proof}
...
\begin{proof}
  ...
  Now we will prove the reverse inclusion. Let \( x \in \cl(A) \). Then either \( x \in A \) or \( x \in \fr(A) \). Assume the latter. By \fullref{def:topological_boundary/neighborhoods}, every neighborhood \( U \) of \( x \) has points both in \( A \) and outside \( A \), therefore \( U \cap (A \setminus \{ x \}) \) is nonempty. By \fullref{thm:closure_operator_properties/neighborhood_intersection}, \( x \in \cl(A \setminus \{ x \}) \), that is, \( x \in \derived(A) \).
