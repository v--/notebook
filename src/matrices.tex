...
\end{theorem}
\begin{proof}
  Let \( V \) be an arbitrary \( n \)-dimensional vector space over \( \BbbK \). Since a basis of \( V \) exists by \fullref{thm:vector_space_basis_existence}, fix a basis and fix an ordering \( b_1, \ldots, b_n \) of the basis vectors. Denote the projection \hyperref[def:module_basis_projection]{maps} by \( \pi_{b_i} \).
