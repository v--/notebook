...
\begin{definition}\label{def:array}
  ...
  \begin{defenum}
    ...
    \DItem{def:array/square_matrix} If \( n = m \), we call the matrix a \Def{square matrix}. The elements \( a_{1,1}, \ldots, a_{n,n} \) of a square matrix are called its \Def{main diagonal}.
...
\begin{theorem}\label{thm:finite_dimensional_spaces_are_isomorphic}
  Every \( n \)-dimensional\Tinyref{def:vector_space_dimension} vector space\Tinyref{def:vector_space} over the field \( F \) is isomorphic to \( F^n \)\Tinyref{def:module_of_tuples}.
\end{theorem}
\begin{proof}
  Let \( V \) be an arbitrary \( n \)-dimensional vector space over \( F \). Since a basis of \( V \) exists\AOC by \cref{thm:all_vector_spaces_are_free_left_modules}, fix a basis and fix an ordering \( b_1, \ldots, b_n \) of the basis vectors. Denote the projection maps\Tinyref{def:free_left_module_basis_projection} by \( \pi_{b_i} \).
  ...
  \end{align*}
  that maps a vector \( x \in V \) into an \( n \)-tuple of the projections of \( x \) along the ordered basis \( b_1, \ldots, b_n \). It is linear since, by \cref{thm:free_left_module_basis_projections_are_linear}, the projections are linear.
...
\begin{definition}\label{def:algebra_of_matrices}
  ...
  \begin{description}
    ...
    \DItem{def:algebra_of_matrices/matrix_multiplication} We define \Def{matrix multiplication} in two steps. First, if \( a \in R^{1,n} \) is a row vector\Tinyref{def:array/row_vector} and \( b \in R^{n,1} \) is a column vector\Tinyref{def:array/column_vector}, we define their \Def{inner product} to be
    \begin{equation}
  ...
  If \( n = m \), we can add matrix multiplication\Tinyref{def:algebra_of_matrices/matrix_multiplication} to the module \( R^{n \times n} \) so that it becomes an algebra\Tinyref{def:algebra_over_a_ring}. The identity matrix of order \( n \) is the identity with respect to matrix multiplication.
\end{definition}
...
\begin{proposition}\label{thm:finite_dimensional_operators_are_isomorphic_to_matrices}
  The vector space of all linear maps \( \hom(\R^n, \R^m) \) is isomorphic to the matrix vector space\Tinyref{def:algebra_of_matrices} \( \R^{n \times m} \).

  If \( n = m \), the algebra \( \End(\R^n) \) with function composition as vector multiplication is isomorphic to \( \R^{n \times n} \) with matrix multiplication as vector multiplication.
\end{proposition}
