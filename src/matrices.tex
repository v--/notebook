...
\begin{definition}\label{def:array}
  ...
  \begin{defenum}
    ...
    \DItem{def:array/vector} one-dimensional arrays are called simply \Def{vectors} or \hyperref[def:cartesian_product]{tuples} and are usually written as either column vectors or row vectors.
  \end{defenum}
\end{definition}
...
\begin{definition}\label{def:left_module_of_tuples}
  ...
  In particular, if \( R \) is a \hyperref[def:semiring/field]{field}, \( R^n \) is a vector \hyperref[def:vector_space]{space} and we refer to it as a \Def{tuple space}. We are usually only concerned with the vector spaces \( \BR^n \) and \( \BC^n \).
\end{definition}
...
\begin{theorem}\label{thm:finite_dimensional_spaces_are_isomorphic}
  Every \( n \)-\hyperref[def:vector_space_dimension]{dimensional} vector \hyperref[def:vector_space]{space} over the field \( \BK \) is isomorphic to \( \BK^n \)\Tinyref{def:left_module_of_tuples}.
\end{theorem}
\begin{proof}
...
\begin{proposition}\label{def:matrix_determinant_properties}\cite[proposition 5.1]{Knapp2016BAlg}
  ...
  \begin{propenum}
    ...
    \DItem{def:matrix_determinant_properties/transpose} For the transpose \hyperref[def:matrix_transpose]{matrix} \( A^T \) of \( A \in R^n \), we have
    \begin{equation*}
      \det(A^T) = \det(A).
    \end{equation*}
