...
\begin{definition}\label{def:array}
  ...
  In particular,
  \begin{thmenum}
    \labitem{def:array/matrix} two-dimensional arrays of shape \( n, m \) are usually called \term{matrices}. An \( n, m \)-matrix \( A \) is denoted as
    \begin{equation*}
    ...
    \labitem{def:array/square_matrix} If \( n = m \), we call the matrix a \term{square matrix} of order \( n \).

    \labitem{def:array/column_vector} matrices with only one column are called \term{column matrices}:
    \begin{equation*}
      \begin{pmatrix}
        a_{1,1} \\
        \vdots  \\
    ...
    \labitem{def:array/row_vector} matrices with only one row are called \term{row matrices}:
    \begin{equation*}
      \begin{pmatrix}
        a_{1,1} & \cdots & a_{1,m}
      \end{pmatrix}.
    ...
    \labitem{def:array/vector} one-dimensional arrays are called simply \term{vectors} or \hyperref[def:cartesian_product]{tuples} and are usually written as either column vectors or row vectors.
  \end{thmenum}
\end{definition}
...
\begin{definition}\label{def:algebra_of_matrices}
  ...
  \begin{thmenum}
    \labitem{def:algebra_of_matrices/addition} We define \term{matrix addition} as
    \begin{balign*}
    ...
    \labitem{def:algebra_of_matrices/scalar_multiplication} We define \term{scalar multiplication} as
    \begin{balign*}
...
\begin{definition}\label{def:matrix_determinant}\mcite\cite[215]{Knapp2016BasicAlgebra}
...
\begin{proposition}\label{thm:matrix_determinant_properties}\mcite\cite[prop. 5.1]{Knapp2016BasicAlgebra}
  Matrix determinants over the commutative unital ring \( R \) have the following basic properties:
  \begin{thmenum}
    \labitem{thm:matrix_determinant_properties/identity} For the identity matrix \( E_n \in R^n \) we have
    \begin{equation*}
      \det(E_n) = 1.
    \end{equation*}

    \labitem{thm:matrix_determinant_properties/transpose} For the transpose \hyperref[def:matrix_transpose]{matrix} \( A^T \) of \( A \in R^n \), we have
    \begin{equation*}
      \det(A^T) = \det(A).
    \end{equation*}

    \labitem{thm:matrix_determinant_properties/product} For matrices \( A, B \in R^n \) we have
    \begin{equation*}
      \det(A) \det(B) = \det(AB).
    \end{equation*}
  \end{thmenum}
...
\begin{proposition}\label{thm:matrix_invertible_iff_nonsingular}\mcite\cite[cor. 5.5]{Knapp2016BasicAlgebra}
