...
\begin{definition}\label{def:magma_ideal}
  Let \( M \) be a magma\Tinyref{def:magma/magma} and \( I \) be a subset of \( M \). We say that \( I \) is a \Def{left ideal} (resp. \Def{right ideal}) of \( G \) if the inclusion \( IM \subseteq I \) holds, where we use the convention in \fullref{remark:vector_space_set_operations}, that is,
  \begin{equation*}
...
\begin{example}\label{ex:subgroup_is_not_ideal}
  We explicitly give a counterexample to the converse of \fullref{thm:magma_ideal_is_submagma}. Define \( G \coloneqq \BZ \times \BZ \) to be the abelian group\Tinyref{def:abelian_group} given by pointwise addition. Define
  \begin{equation*}
