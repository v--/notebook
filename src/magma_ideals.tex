...
\begin{definition}\label{def:magma_ideal}\mimprovised
  Let \( M \) be a \hyperref[def:magma]{magma} and \( I \) be a subset of \( M \). We say that \( I \) is a \term{left ideal} of \( M \) if the inclusion \( IM \subseteq I \) holds in the \hyperref[def:magma/power_set]{power set magma}. That is,
  \begin{equation*}
...
\begin{example}\label{ex:subgroup_is_not_ideal}
  We explicitly give a counterexample to the converse of \fullref{thm:magma_ideal_is_submagma}. Define \( G \coloneqq \BbbZ \times \BbbZ \) as a \hyperref[def:group_direct_sum]{direct sum} of two copies of the \hyperref[def:set_of_integers]{integers}. Define also the subset
  \begin{equation*}
