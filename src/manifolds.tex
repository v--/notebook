...
\begin{definition}\label{def:smooth_curve}\mcite[8]{ИвановТужилин2017}
  ...
  \begin{thmenum}
    \thmitem{def:smooth_curve/speed} If \( \gamma \) does not intersect itself at the point \( \gamma(t_0) \), we call the norm \( \norm{\gamma'(t_0)} \) the \term{speed} of \( \gamma \) at the point \( \gamma(t_0) \).
    ...
    \thmitem{def:smooth_curve/natural_parameter} We say that the \hyperref[def:reparametrization]{reparametrization} \( s: J \to I \) is a \term{natural parameter} if, at any point \( s \in J \), we have \( \norm{\gamma'(s)} = 1 \).

    Of course, \( s \) can be the identity, in which case we say that the original parametrization is natural.

    \thmitem{def:smooth_curve/curvature} If the naturally parametrized curve \( \gamma: I \to \BbbR^n \) does not intersect itself at the point \( \gamma(t_0) \), we call the norm \( \norm{\gamma^\dprime(t_0)} \) the \term{curvature} of \( \gamma \) at \( \gamma(t_0) \).
