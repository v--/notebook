...
\begin{definition}\label{def:magma_identity}
  An element \( e \) of a magma \( \mscrM \) is called a \Def{left identity} (resp. \Def{right identity}) if
  \begin{align}\label{eq:def:magma_identity}
    ex = x \T{for all} x \in \mscrM
    &&
    (\text{resp. } xe = x \T{for all} x \in \mscrM)
  \end{align}

  If \( e \) is simultaneously a left and right identity, we call a \Def{two-sided identity} or simply \Def{identity} of \( \mscrM \).
\end{definition}
...
\begin{definition}\label{def:unital_magma_kernel}
  The \Def{kernel} \( \ker(\varphi) \) of a unital magma homomorphism \( \varphi: \mscrM \to \mscrN \) is the zero \hyperref[def:zero_locus]{locus} of \( \varphi \), that is, \hyperref[def:function/preimage]{preimage} \( \varphi^{-1}(e_{\mscrN}) \).
