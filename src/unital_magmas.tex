...
\begin{definition}\label{def:unital_magma}
  ...
  \begin{DefEnum}
    ...
    \ILabel{def:unital_magma/category} The \hyperref[def:first_order_model_category]{model category} \( \Cat{Mag}_* \) of unital magmas has obviously defined \hyperref[def:forgetful_functor]{forgetful functors} into \hyperref[def:pointed_set/category]{\( \Cat{Set}_* \)} and into \hyperref[def:magma/category]{\( \Cat{Mag} \)}.
    ...
    \ILabel{def:unital_magma/exponentiation} The identity allows us to define an additional \Def{exponentiation} operation for nonnegative integers \( n \) inductively\IND as
    \begin{equation*}
