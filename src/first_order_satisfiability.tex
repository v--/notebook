...
\begin{proposition}\label{thm:implicit_universal_quantification}
  ...
  See \fullref{ex:def:first_order_derivation_system/eigenvariables/invalid_universal_closure} for how this fails for derivability.
\end{proposition}
\begin{proof}
...
\begin{proposition}\label{thm:quantifier_satisfiability}
  ...
  See also \fullref{def:first_order_derivation_system/terms} for derivation rules corresponding to this proposition.
\end{proposition}
\begin{proof}
  ...
  \SubProofOf{thm:quantifier_satisfiability/universal} If \( v \) satisfies \( \qforall \xi \varphi \), then for any variable \( x \), the valuation \( v_{\xi \mapsto x} \) satisfies \( \varphi \). Since \( t \) is a variable, we have
...
\begin{theorem}[First-order semantic deduction theorem]\label{thm:first_order_semantic_deduction_theorem}\mcite[thm. 16.29]{OpenLogicFull}
  ...
  Compare this result with \fullref{thm:propositional_semantic_deduction_theorem} and \fullref{thm:first_order_syntactic_deduction_theorem}.
\end{theorem}
\begin{proof}
...
\begin{remark}\label{rem:deduction_with_free_variables}
  In order to highlight the importance of closed formulas in certain theorems, we will take a closed look at the proof of \fullref{thm:first_order_semantic_deduction_theorem}.
