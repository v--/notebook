...
\begin{definition}\label{def:differentiability}
  ...
  We only use the operator \( \partial \) inside this definition. See \cref{remark:derivative_notation} for a discussion of derivative notation.
  ...
  \begin{defenum}
    ...
    Within its domain of definition of \( \delta \), which is stricter than that of \( \partial \), the operator \( \delta \) is a currying\Tinyref{def:function_arguments} of \( \partial \). We are interested in how the operator \( \delta f(x) \) varies as \( x \) varies.
    ...
    If \( \Lambda \) exists, we usually denote it by \( D_G f(x_0) \) or \( f_G'(x_0) \) and call it the \Def{Gateaux derivative} of \( f \) at \( x_0 \). See \cref{remark:derivative_notation} for a discussion of the notation.
    ...
    If \( \Lambda \) exists, we usually denote it by \( D f(x_0) \) or \( f(x_0) \) and call it the \Def{Frechet derivative} of \( f \) at \( x_0 \). See \cref{remark:derivative_notation} for a discussion of the notation.

    Note that \cref{def:differentiability/gateaux/condition} uses convergence in the topology of \( Y \) while \cref{def:differentiability/frechet/condition} uses convergence in \( \BR \). We discuss in \cref{remark:gateaux_vs_frechet} how Frechet differentiability is a special \enquote{uniform} case of Gateaux differentiability.
...
\begin{remark}\label{remark:gateaux_vs_frechet}
  ...
  By comparing \cref{remark:gateaux_vs_frechet/gateaux} to \cref{def:differentiability/frechet}, we conclude that \( f \) is Frechet differentiable at \( x_0 \) if \( \inf_{h \in S_X} \delta^h_G > 0 \), that is, if \( f \) is Gateaux differentiable and the convergence of the Gateaux derivative is uniform on \( h \in S_X \).
