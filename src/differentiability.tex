...
\begin{definition}\label{def:differentiability}
  ...
  \begin{defenum}
    ...
    \DItem{def:differentiability/two-sided}\cite[0.2.1]{Йоффе1974} If the two-sided directional derivative \( f'(x)(h) \) exists for all directions \( h \in S_X \), we call it the \Def{first variation at \( x \)} and denote the corresponding linear operator \( \delta f(x): X \to Y \).
