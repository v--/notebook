...
\begin{definition}\label{def:differentiability}
  ...
  \begin{defenum}
    \item We do not introduce a special name for functions that have a one-sided derivative at $x$ in the direction $h$, although some authors call these functions \ul{Gateaux differentiable at $x$ in the direction $h$}. % TODO: Give examples of such authors
    ...
    \item\label{def:differentiability/strong}\cite[33]{Dontchev2014} We say that \ul{$f$ is strictly differentiable at $x$} if there exists a continuous linear operator $f'(x): X \to Y$ such that
    \begin{align*}
