...
Let \( X \) and \( Y \) be Hausdorff topological vector \hyperref[def:topological_vector_space]{spaces} and let \( U \subseteq X \) be an open set.
...
\begin{definition}\label{def:differentiability}
  ...
  \begin{defenum}
    ...
    \DItem{def:differentiability/frechet}\cite[section 0.2.1]{Йоффе1974} We now restrict our attention to Banach \hyperref[def:banach_space]{spaces}. We say that \( f \) is \Def{Frechet differentiable} or \Def{strongly differentiable} at \( x_0 \) if there exists a continuous linear operator \( \Lambda: X \to Y \) such that
