...
\begin{proof}
  ...
  Therefore, by \fullref{thm:proper_ideals_containing_identity}, \( I = \BbbK \).
\end{proof}

\begin{lemma}\label{thm:ring_of_integers_modulo_zero_divisors}
  In the ring \hyperref[thm:ring_of_integers_modulo]{\( \BbbZ_n \)} of integers modulo \( n \), a nonzero number \( m \) is a \hyperref[def:semiring_division]{zero divisor} if and only if \( m \) is not \hyperref[def:coprime_numbers]{coprime} to \( n \).
\end{lemma}
\begin{proof}
