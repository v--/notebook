...
\begin{definition}\label{def:norm}\cite[3]{Rudin1991}
  Let \( V \) be a vector space over the field \( F \) with absolute value \( \Abs{\cdot} \). We say that the function \( \Norm{\cdot}: V \to \R^{>0} \) is a \Def{norm} if
  \begin{description}
    \DItem{def:norm/identity}[identity] \( x = 0_V \) if and only if \( \Norm x = 0 \)
    \DItem{def:norm/absolute_homogeneity}[absolute homogeneity]
    \begin{equation*}
      \Norm{\alpha x} = \Abs{\alpha} \Norm{x} \text{ for all } \alpha \in F \text{ and } x \in V
    \end{equation*}

    \DItem{def:norm/subadditivity}[subadditivity] 
    \begin{equation*}
      \Norm{x + y} \leq \Norm{x} + \Norm{y} \text{ for all } x, y \in V
    \end{equation*}
  \end{description}

  If we remove \ref{def:norm/identity}, then \( \Norm{\cdot} \) is called a \Def{seminorm}.

  If instead \( V \) is an associative\Tinyref{def:associative_algebra} and \( \Norm{\cdot} \) satisfies the additional axiom
  \begin{description}
    \DItem{def:norm/multiplicativity}[multiplicativity]
    \begin{equation*}
      \Norm{xy} = \Norm{x} \cdot \Norm{y} \text{ for all } x, y \in V,
    \end{equation*}
  \end{description}
