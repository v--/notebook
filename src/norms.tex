...
\begin{proof}
  ...
  \begin{description}
    \RItem{def:metric_space/M1} Follows from \fullref{def:norm/identity}.

    \RItem{def:metric_space/M2} By \fullref{def:norm/absolute_homogeneity},
    \begin{equation*}
      \rho(x, y) = \Norm{ x - y } = \Norm{ (-1) (y - x) } = \Abs{-1} \Norm{y - x} = \rho(y, x).
    \end{equation*}

    \RItem{def:metric_space/M3}
    \begin{equation*}
