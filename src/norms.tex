...
\begin{proof}
  ...
  \begin{RefList}
    \IRef{def:metric_space/M1} Follows from \fullref{def:norm/identity}.

    \IRef{def:metric_space/M2} By \fullref{def:norm/absolute_homogeneity},
    \begin{equation*}
      \rho(x, y) = \Norm{ x - y } = \Norm{ (-1) (y - x) } = \Abs{-1} \Norm{y - x} = \rho(y, x).
    \end{equation*}

    \IRef{def:metric_space/M3}
    \begin{equation*}
...
\end{Definition}
\begin{RefListProof}
    \IImplies{def:rotund_norm/no_sphere_segments}{def:rotund_norm/least_norm} Let the norm in \( E \) be rotund and let \( C \subseteq E \) be a (potentially empty) convex set. We will prove that \( C \) contains at most one point of least norm.
    ...
    \IImplies{def:rotund_norm/least_norm}{def:rotund_norm/no_sphere_segments} Let every convex set \( C \subseteq E \) have at most one element of least norm.
    ...
    \IImplies{def:rotund_norm/no_sphere_segments}{def:rotund_norm/linearly_dependent} Let \( E \) be rotund let \( x, y \in E \) be distinct vectors such that
    ...
    \IImplies{def:rotund_norm/linearly_dependent}{def:rotund_norm/no_sphere_segments} Let \fullref{def:rotund_norm/linearly_dependent/equation} hold and fix \( x, y \in S_E, t \in (0, 1) \). Define \( z \coloneqq tx + (1-t)y \).
