...
\begin{definition}\label{def:absolute_value}\mcite{nLab:absolute_value}
  ...
  \begin{thmenum}
    \labitem[def:absolute_value/RN1]{RN1}(identity) \( x = 0_R \) if and only if \( \abs{x} = 0 \)
    \labitem[def:absolute_value/RN2]{RN2}(multiplicativity) For any \( x, y \in V \),
    \begin{equation*}
      \abs{xy} = \abs{x} \cdot \abs{y}
    \end{equation*}

    \labitem[def:absolute_value/RN3]{RN3}(subadditivity) For any \( x, y \in V \),
    \begin{equation*}
...
\begin{definition}\label{def:norm}
  ...
  \begin{thmenum}
    \labitem[def:norm/N1]{N1}(identity) \( x = 0_M \) if and only if \( \norm x = 0_{\BbbR} \)

    \labitem[def:norm/N2]{N2}(absolute homogeneity)
    \begin{equation*}
      \norm{t x} = \abs{t} \norm{x} \text{ for all } t \in R \text{ and } x \in M
    \end{equation*}

    \labitem[def:norm/N3]{N3}(subadditivity)
    \begin{equation*}
  ...
  \begin{thmenum}
    \labitem{def:norm/multiplicativity}(multiplicativity)
    \begin{equation*}
      \norm{xy} = \norm{x} \cdot \norm{y} \text{ for all } x, y \in M,
    \end{equation*}
  \end{thmenum}
...
\begin{definition}\label{def:rotund_norm}\mcite[def. 2.36]{Phelps1993}
  ...
  \begin{thmenum}
    \labitem{def:rotund_norm/no_sphere_segments} There are no line segments in the unit sphere \( S_X \).
    \labitem{def:rotund_norm/least_norm} Every convex subset of \( X \) has at most one point of least norm.
    \labitem{def:rotund_norm/linearly_dependent}
