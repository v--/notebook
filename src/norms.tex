...
\begin{definition}\label{def:absolute_value}\cite{nLab:absolute_value}
  Let \( R \) be a semiring\Tinyref{def:semiring}. We say that the function \( \Abs{\cdot}: V \to \R_{>0} \) is an \Def{absolute value} if
  \begin{description}
    \DItem{def:absolute_value/identity}[identity] \( x = 0_R \) if and only if \( \Abs{x} = 0 \)
    \DItem{def:absolute_value/multiplicativity}[multiplicativity] For any \( x, y \in V \),
    \begin{equation*}
      \Abs{xy} = \Abs{x} \cdot \Abs{y}
    \end{equation*}

    \DItem{def:absolute_value/subadditivity}[subadditivity] For any \( x, y \in V \),
    \begin{equation*}
...
\begin{definition}\label{def:norm}
  Let \( M \) be an \( R \)-module with absolute value \( \Abs{\cdot} \). We say that the function \( \Norm{\cdot}: M \to \R_{\geq 0} \) is a \Def{norm} if
  \begin{description}
    \DItem{def:norm/identity}[identity] \( x = 0_M \) if and only if \( \Norm x = 0_{\R} \)

    \DItem{def:norm/absolute_homogeneity}[absolute homogeneity]
    \begin{equation*}
      \Norm{t x} = \Abs{t} \Norm{x} \text{ for all } t \in R \text{ and } x \in M
    \end{equation*}

    \DItem{def:norm/subadditivity}[subadditivity] 
    \begin{equation*}
      \Norm{x + y} \leq \Norm{x} + \Norm{y} \text{ for all } x, y \in M
    \end{equation*}
  \end{description}

  If we remove \ref{def:norm/identity}, then \( \Norm{\cdot} \) is called a \Def{seminorm}.

  If instead \( V \) is an associative\Tinyref{def:algebra_over_ring} and \( \Norm{\cdot} \) satisfies the additional axiom
  \begin{description}
    \DItem{def:norm/multiplicativity}[multiplicativity]
    \begin{equation*}
...
\begin{definition}\label{def:norm_induced_metric}
  A norm \( \Norm \cdot \) on a real or complex vector space \( V \) induces the metric\Tinyref{def:vector_space}
  \begin{align*}
...
\begin{definition}\label{def:duality_mapping}\cite[Example 2.26]{Phelps1993}
...
\begin{proof}\mbox{}
  \begin{enumerate}
    ...
    Fix \( x \in S_X \). Since \( D(x) \) is nonempty (by \cref{thm:hahn_banach_implies_duality_mapping_nonempty}) and since \( \Norm{\cdot} \) is not smooth, then there exist two different functionals \( x^*, y^* \in D(x) \), such that
    \begin{align*}
      \Prod {x^*} x
      =
      \Prod {y^*} x
