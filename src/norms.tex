...
\begin{definition}\label{def:absolute_value}\MarginCite{nLab:absolute_value}
  Let \( R \) be a \hyperref[def:semiring]{semiring}. We say that the function \( \Abs{\cdot}: V \to \BR_{>0} \) is an \Def{absolute value} or a \Def{semiring norm} if
  \begin{DefEnum}
...
\begin{definition}\label{def:norm}
  ...
  \begin{DefEnum}
    \IAxiom{def:norm/N1}{N1}(identity) \( x = 0_M \) if and only if \( \Norm x = 0_{\BR} \)
