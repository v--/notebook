...
\begin{definition}\label{def:absolute_value}\cite{nLab:absolute_value}
  Let \( R \) be a semiring\Tinyref{def:semiring}. We say that the function \( \Abs{\cdot}: V \to \BR_{>0} \) is an \Def{absolute value} if
  \begin{description}
...
\begin{definition}\label{def:norm}
  ...
  \begin{description}
    \DItem{def:norm/identity}[identity] \( x = 0_M \) if and only if \( \Norm x = 0_{\BR} \)
