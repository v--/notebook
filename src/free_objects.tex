...
\begin{definition}\label{def:forgetful_functor}\mcite[exmpl. 1.2.3]{Leinster2016Basic}
...
\begin{example}\label{ex:forgetful_functors}
  ...
  \begin{thmenum}
    ...
    \item More generally, given a \hyperref[def:first_order_semantics/theory]{first-order theory} \( \Gamma \) and its corresponding category \( \cat{Model}_\Gamma \), the functor \( U: \cat{Model}_\Gamma \to \cat{Set} \) forgets any additional conditions imposed on the underlying sets of the models.
  \end{thmenum}
\end{example}

\begin{definition}\label{def:free_functor}\mcite{nLab:free_object}
...
\begin{definition}\label{def:free_object}\mcite{nLab:free_object}
