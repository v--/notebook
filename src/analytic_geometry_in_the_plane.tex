...
\begin{definition}\label{def:quadratic_plane_curve}
  ...
  \begin{defenum}
    ...
    If \( a = b \), we say that the ellipse is a \Def{circle}. Circles generalize to \hyperref[def:metric_space/sphere]{spheres} in metric spaces.

    We are often interested in defining ellipses via \Def{scalar parametric equations} using trigonometric \hyperref[def:trigonometric_functions]{functions} as follows:
    ...
    Similarly to ellipses, we are can define hyperbolas via \Def{scalar parametric equations} using hyperbolic trigonometric \hyperref[def:hyperbolic_trigonometric_functions]{functions} as follows:
