...
\begin{figure}
  ...
  \begin{minipage}[b]{0.45\textwidth}
    ...
    \begin{mplibcode}
      ...
      endfig;
    \end{mplibcode}
    \Caption{def:angle/figure}{An \hyperref[def:angle]{angle} with its measurement segments dashed.}
  \end{minipage}
\end{figure}
...
\begin{definition}\label{def:angle}
  ...
  The measure of a directed angle, often called the angle itself, is defined as the number (see \fullref{def:geometric_trigonometric_functions} for a justification)
  \begin{equation*}
...
\begin{definition}\label{def:triangle}
  ...
  A \Def{triangle} is a triple \( (A, B, C) \) of \hyperref[def:point]{points}, no two of which are \hyperref[def:collinear_complanar]{collinear} (see \fullref{def:simplex/triangle} for a more general definition).
...
\begin{definition}\label{def:geometric_trigonometric_functions}
  ...
  To see that \( \sin_G \) and \( \cos_G \) are somewhat analogous to \( \sin \) and \( \cos \), notice that by \fullref{thm:arctantwo}, there exists a unique \( t_0 \coloneqq \arctantwo(y_0, x_0) \).
  \begin{align*}
    \sin(t_0) = x_0,
    &&
    \cos(t_0) = y_0.
