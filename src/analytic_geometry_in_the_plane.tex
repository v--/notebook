...
\begin{definition}\label{def:plane_line_equations}
  ...
  \begin{thmenum}
    \ilabel{def:plane_line_equations/vector_parametric} When regarding a line as a parametric curve as in \fullref{def:geometric_line/parametric}, the \hyperref[def:first_order_syntax/formula]{formula}
    \begin{equation}\label{def:plane_line_equations/parametric_equation}
      l(t) = tx + a
    \end{equation}
    is called a \term{vector parametric equation}.

    \ilabel{def:plane_line_equations/scalar_parametric} Given \fullref{def:plane_line_equations/parametric_equation}, the \term{scalar parametric equations} of the line are
    ...
    \ilabel{def:plane_line_equations/normal} If \( A^2 + B^2 = 1 \), we call \fullref{def:plane_line_equations/general_equation} a \term{normal equation}. This leaves us with only two representatives of \( \braket{p} \).

    \ilabel{def:plane_line_equations/cartesian} Given \( k, m \in \BbbR \) and  \( k \neq 0 \), we define the \term{Cartesian equation} of a line:
    ...
    \ilabel{def:plane_line_equations/intercept} Given nonzero \( a, b \in \BbbR \), we define the \term{intercept equation} of a line:
...
\begin{figure}
  \begin{minipage}[b]{0.40\textwidth}
    ...
    \todo{Add diagram}\iffalse\begin{mplibcode}
      ...
      endfig;
    \end{mplibcode}\fi
    \Caption{def:plane_line_equations/cartesian_equation_drawing}{A \hyperref[def:geometric_line]{line} in \( \BbbR^2 \) defined using its \hyperref[def:plane_line_equations/cartesian]{Cartesian equation}.}
  \end{minipage}
...
\begin{definition}\label{def:angle}
  ...
  \begin{thmenum}
    \ilabel{def:angle/zero} \term{zero} if \( \alpha = 0 \),
    \ilabel{def:angle/acute} \term{acute} if \( \alpha \in (0, \tfrac \pi 2) \),
    \ilabel{def:angle/right} \term{right} if \( \alpha = \tfrac \pi 2 \),
    \ilabel{def:angle/obtuse} \term{obtuse} if \( \alpha \in (\tfrac \pi 2, \pi) \),
    \ilabel{def:angle/straight} \term{straight} if \( \alpha = \pi \), in which case the angle is actually a line,
    \ilabel{def:angle/reflex} \term{reflex} if \( \alpha > \pi \).
  \end{thmenum}
...
\begin{definition}\label{def:triangle}
  ...
  \begin{thmenum}
    \ilabel{def:triangle/isosceles} \term{isosceles} if at least two of its sides have equal length
    \ilabel{def:triangle/equilateral} \term{equilateral} if all of its sides have equal length
  \end{thmenum}
  or based on their angles as
  \begin{thmenum}
    \ilabel{def:triangle/acute} \term{acute} if all of its angles are \hyperref[def:angle/acute]{acute}.
    \ilabel{def:triangle/right} \term{right} if at least one of the angles is \hyperref[def:angle/straight]{straight}.
    \ilabel{def:triangle/obtuse} \term{obtuse} if at least one of its angles is \hyperref[def:angle/obtuse]{obtuse}.
  \end{thmenum}
\end{definition}
...
\begin{definition}\label{def:quadratic_plane_curve}
  ...
  \begin{thmenum}
    \ilabel{def:quadratic_plane_curve/ellipse} An \term{ellipse} is a quadratic curve whose canonical equation has the form
    ...
    \ilabel{def:quadratic_plane_curve/hyperbola} A \term{hyperbola} is a quadratic curve whose canonical equation has the form
    ...
    \ilabel{def:quadratic_plane_curve/parabola} A \term{parabola} is a quadratic curve whose canonical equation has the form
...
\begin{definition}\label{def:geometric_trigonometric_functions}
  ...
  \begin{figure}
    ...
    \begin{minipage}[b]{0.4\textwidth}
      ...
      \end{mplibcode}\fi
    \end{minipage}
    \Caption{def:geometric_trigonometric_functions/triangle}{An \enquote{abstract} right triangle in the \hyperref[def:euclidean_plane]{Euclidean plane} with legends for geometric sines and cosines and the same triangle in \( \BbbR^2 \) connecting the origin to a point \( (x_0, y_0) \) on the unit circle.}
  \end{figure}
