...
\begin{proof}
  ...
  \begin{RefList}
    \IRef{def:metric_space/M1} Obviously \( h(E, E) = 0 \). If \( h(E, F) = 0 \), then there exists no point of \( E \) outside of \( F \) and vice versa, hence \( E = F \).

    \IRef{def:metric_space/M2} This follows from the symmetry of the \( \max \) function.

    \IRef{def:metric_space/M3} For any point \( y \in X \), we have
    \begin{align*}
      \Dist(x, G)
      =
      \inf_{z \in G} \mu(x, z)
