\documentclass{notebook}

% Bibliography
\addbibresource{bib/books.bib}
\addbibresource{bib/papers.bib}
\addbibresource{bib/articles.bib}
\addbibresource{bib/nlab.bib}
\addbibresource{bib/proofwiki.bib}
\addbibresource{bib/mathcounterexamples.bib}
\addbibresource{bib/mathse.bib}
\addbibresource{bib/mathof.bib}
\addbibresource{bib/misc.bib}
\addbibresource{bib/code.bib}

% Document
\title{Notebook}
\subtitle{\boldtt{\url{https://github.com/v--/notebook}}}
\author{Ianis Vasilev}

% Failsafe means "use defauls if no file is found", see common/git_commit_info.sty
\GitCommitInfoReadFailsafe {git-commit-info}
{
  \date
  {
    \normalsize
    \textbf{Date:} \GitCommitInfoDate \\
    \textbf{Commit:} \texttt{\GitCommitInfoHash}
  }
}

\begin{document}

\maketitle

\begin{abstract}
  This ever-expanding document started as a set of study notes and exercises and gradually outgrew itself to become more encyclopedic. Having all these notes in one place is quite helpful for both expressing my own thoughts clearly and for later reference. It is also helpful for tracking connections between seemingly unrelated concepts --- the entire document is inter-hyperlinked. Even though I aim at understanding every concept in the way it is meant to be used, most concepts are presented insomuch as they are relevant to probability and optimization.

  Since these are study notes, they will naturally have a lot of errors, so read them with caution. The document claims no referential nor pedagogical value. Everything is written at the level of abstraction I am comfortable with. Furthermore, some sections in the document are much more polished than others. Feel free to contact me if something in this document happens to distress you.

  I tried putting citations on as many things as possible. The citations themselves are usually put in the left margin. If there is no citation on a definition or theorem, that means that I have either recalled it from memory or discovered it on my own. Many of the unoriginal definitions and theorems are restated. The simple proofs are mostly original, and the difficult ones are, often loosely, based on proofs from the places cited. Some proofs simply state \enquote{Trivial.} in order to distinguish themselves from the proofs that are omitted for other reasons.

  See the remark in the introduction of the sections on \hyperref[sec:mathematical_logic]{mathematical logic} and \hyperref[sec:set_theory]{set theory} for clarification regarding seemingly arbitrary conventions.
\end{abstract}

\newpage
\addcontentsline{toc}{section}{Contents}
\tableofcontents
\newpage

% Numbers
\include{src/natural_numbers}
\include{src/integers}
\include{src/rational_numbers}
\include{src/real_numbers}
\include{src/complex_numbers}

% Real analysis
\include{src/topology_of_euclidean_spaces}
\include{src/real_valued_functions}
\include{src/real_convergence}
\include{src/real_differentiability}
\include{src/real_series}
\include{src/riemann_integration}
\include{src/line_integrals}
\include{src/total_variation}

% Complex analysis
\include{src/complex_functions}
\include{src/series}
\include{src/power_series}
\include{src/trigonometric_functions}
\include{src/exponential_function}
\include{src/trigonometric_polynomials}
\include{src/special_functions}
\include{src/norms}

% Functional analysis
\include{src/topological_groups}
\include{src/topological_vector_spaces}
\include{src/hahn_banach}
\include{src/frechet_spaces}
\include{src/banach_spaces}
\include{src/hilbert_spaces}
\include{src/asplund_spaces}
\include{src/minkowski_functionals}
\include{src/dentable_sets}
\include{src/differentiability}
\include{src/banach_space_interpolation}

% Nonsmooth analysis
\include{src/nonsmooth_derivatives}
\include{src/convex_functions}
\include{src/subdifferentials}
\include{src/clarke_gradients}

% Approximation theory
\include{src/lagrange_polynomials}
\include{src/bernstein_inequalities}

% Differential equations
\include{src/ordinary_differential_equations}

% General topology
\include{src/topological_spaces}
\include{src/topological_nets}
\include{src/function_convergence}
\include{src/topological_continuity}
\include{src/initial_final_topologies}
\include{src/separation_axioms}
\include{src/connected_spaces}
\include{src/compact_spaces}
\include{src/baire_spaces}
\include{src/uniform_spaces}

% Metric spaces
\include{src/metric_topology}
\include{src/complete_metric_spaces}
\include{src/hausdorff_distance}
\include{src/totally_bounded_sets}
\include{src/noncompactness_measures}
\include{src/lipschitz_continuity}
\include{src/function_oscillation}

% Geometry
\include{src/affine_coordinate_systems}
\include{src/vector_space_geometry}
\include{src/analytic_geometry_in_the_plane}
\include{src/manifolds}
\include{src/affine_varieties}

% Group theory
\include{src/pointed_sets}
\include{src/magmas}
\include{src/unital_magmas}
\include{src/involutions}
\include{src/magma_ideals}
\include{src/monoid_actions}
\include{src/groups}
\include{src/group_presentations}
\include{src/group_actions}
\include{src/category_of_groups}
\include{src/abelian_groups}

% Ring theory
\include{src/rings}
\include{src/ring_ideals}
\include{src/fields}
\include{src/modules}

% Commutative algebra
\include{src/euclidean_division}
\include{src/polynomials}
\include{src/prime_ideals}
\include{src/localization}
\include{src/noetherian_rings}

% Linear algebra
\include{src/vector_spaces}
\include{src/algebraic_dual_spaces}
\include{src/matrices}
\include{src/diagonalization}
\include{src/bilinear_forms}

\include{src/mathematical_logic}
\include{src/formal_languages}
\include{src/boolean_functions}
\include{src/propositional_logic}
\include{src/first_order_logic}
\include{src/first_order_satisfiability}
\include{src/first_order_models}
\include{src/proof_derivation_systems}
\include{src/logical_theories}

\include{src/set_theory}
\include{src/naive_set_theory}
\include{src/relations}
\include{src/functions}
\include{src/zermelo_fraenkel_set_theory}
\include{src/well_ordered_sets}
\include{src/ordinals}
\include{src/cardinals}
\include{src/transfinite_arithmetic}
\include{src/von_neumanns_cumulative_hierarchy}
\include{src/grothendieck_universes}

\include{src/category_theory}
\include{src/categories}
\include{src/functors}
\include{src/category_equivalences}
\include{src/category_adjunctions}
\include{src/categorical_limits}

\include{src/order_theory}
\include{src/preordered_sets}
\include{src/partially_ordered_sets}
\include{src/totally_ordered_sets}
\include{src/lattices}
\include{src/boolean_algebras}

\include{src/combinatorics}
\include{src/enumerative_combinatorics}
\include{src/progressions}
\include{src/hypergraphs}
\include{src/undirected_graphs}
\include{src/quivers}
\include{src/trees}
\include{src/graph_embedding}

\begin{sloppypar}
  \printbibliography
\end{sloppypar}

\end{document}
