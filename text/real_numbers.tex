...
\begin{definition}\label{def:real_numbers}\mcite[\S V.]{Dedekind2007ContinuityAndIrrationalNumbers}
...
\begin{proposition}\label{thm:field_of_real_numbers}
  ...
  \begin{thmenum}[series=thm:field_of_real_numbers]
    ...
    \thmitem{thm:field_of_real_numbers/addition_neutral}  \( O \) acts as a \hyperref[def:semigroup_neutral_element]{neutral element} of addition:
  ...
  \begin{thmenum}[resume=thm:field_of_real_numbers]
    ...
    \thmitem{thm:field_of_real_numbers/field_embedding} The \hyperref[def:partially_ordered_set/morphism]{order embedding} \( \iota(a) = \BbbQ_{<a} \) from \cref{def:lower_cut_completion} is a \hyperref[def:field/morphism]{field embedding} of \( \BbbQ \) into \( \BbbR \).
  \end{thmenum}
\end{proposition}
\begin{proof}
  ...
  Fix lower cuts \( A \subseteq B \) and \( C \), and suppose that \( C \) is \emph{positive}.
  ...
  \SubProofOf{thm:field_of_real_numbers/field_embedding} Clearly the map \( \iota(a) = \BbbQ_{\leq a} \) is injective as an order embedding. We must show that it is a \hyperref[def:field/morphism]{field homomorphism}. Clearly \( O = \iota(0) \) and \( I = \iota(1) \).
...
\begin{definition}\label{def:principal_nonnegative_nth_root}\mimprovised
  Given an integer \( n \geq 2 \), for every \emph{nonnegative} \hyperref[def:real_numbers]{real number} \( A \) we define its \term{principal \( n \)-th root} as
...
\begin{comments}
  ...
  \item Although not for lower sets, \textcite[def. 5.6.4]{Tao2022AnalysisI} defines \( n \)-th roots for nonnegative real numbers similarly.
\end{comments}
\begin{defproof}
...
\begin{definition}\label{def:irrational_number}\mcite[7]{Dedekind2007ContinuityAndIrrationalNumbers}
