...
\begin{definition}\label{def:real_number_arithmetic}\mimprovised
  ...
  \begin{thmenum}
    ...
    \thmitem{def:real_number_arithmetic/additive_inverse}\mcite[117]{Enderton1977Sets} Additive inverses:
...
\begin{proof}
  ...
  \begin{itemize}
    \item If \( A \supsetneq O \), since, by \fullref{thm:def:rational_numbers_ordering/archimedean}, \( \BbbQ \) is \hyperref[def:archimedean_field]{Archimedean}, hence there exists a largest nonnegative integer \( n \) such that \( a \coloneqq (-q/2)n \) is in \( A \).
  ...
  \begin{figure}[!ht]
    \begin{subcaptionblock}{\textwidth}
      \centering
      \includegraphics[page=1]{output/thm__real_numbers_are_a_field__addition_invertible}
    \end{subcaptionblock}

    \begin{subcaptionblock}{\textwidth}
      \centering
      \includegraphics[page=2]{output/thm__real_numbers_are_a_field__addition_invertible}
    \end{subcaptionblock}
  ...
  \SubProofOf{thm:field_of_real_numbers/multiplication_ordering} We will demonstrate that multiplication is compatible with the subset order.
  ...
  \begin{itemize}
    ...
    \item Otherwise, \( 0 < i < 1 \). As in \fullref{thm:field_of_real_numbers/addition_invertible}, we will use that \( \BbbQ \) is \hyperref[def:archimedean_field]{Archimedean}.
...
\begin{proposition}\label{thm:real_number_positive_power}
  ...
  \begin{equation}\label{eq:thm:real_number_positive_power}
    A^n = O \cup \set{ a^n \given a \in A \setminus O }.
  \end{equation}
\end{proposition}
\begin{proof}
  Define
  ...
  For a fixed lower cut \( A \), we will use induction on \( n \) to show that \( P(A, n) = A^n \).
...
\begin{definition}\label{def:real_nth_root}\mimprovised
  For any \hyperref[def:integers]{integer} \( n \geq 2 \) and any \hi{nonnegative} \hyperref[def:real_numbers]{real number} \( A \), we define the \( n \)-th \term{root} of the \( A \) as the lower cut
...
\end{definition}
\begin{comments}
  \item \Fullref{thm:def:real_nth_root/polynomial} implies that \( \sqrt[n]{ A } \) is a \hyperref[def:polynomial_root]{root} of the \hyperref[def:polynomial_algebra/polynomials]{polynomial} \( X^n - A \); hence the name. The polynomial may also have other roots.
\end{comments}
\begin{proof}
...
\begin{proposition}\label{thm:def:real_nth_root}
  The \hyperref[def:real_nth_root]{real \( n \)-th root} satisfies the following basic properties:
  \begin{thmenum}
    \thmitem{thm:def:real_nth_root/power_of_root} For any \( A \supsetneq O \), we have
    \begin{equation}\label{eq:thm:def:real_nth_root/power_of_root}
      \parens[\Big]{ \sqrt[n]{ A } }^n = A.
    \end{equation}

    \thmitem{thm:def:real_nth_root/root_of_power} For any \( A \supsetneq O \), we have
    \begin{equation}\label{eq:thm:def:real_nth_root/root_of_power}
      \sqrt[n]{ A^n } = A.
    \end{equation}

    \thmitem{thm:def:real_nth_root/polynomial} \( \sqrt[n]{ A } \) is a \hyperref[def:polynomial_root]{polynomial root} of the \hyperref[def:polynomial_algebra/polynomials]{polynomial} \( X^n - A \) over the real numbers.

    \thmitem{thm:def:real_nth_root/multiplicative} Taking the \( n \)-th root is \hyperref[def:multiplicative_function]{multiplicative}:
...
\end{proposition}
\begin{proof}
  \SubProofOf{thm:def:real_nth_root/power_of_root} We have
  \begin{equation*}
    \parens[\Big]{ \sqrt[n]{ A } }^n
  ...
  \SubProofOf{thm:def:real_nth_root/root_of_power} We have
  \begin{equation*}
    \sqrt[n]{ A^n }
    \reloset{\eqref{eq:def:real_nth_root}} =
    O \cup \set{ s \geq 0 \given s^n \in A^n }
    \reloset{\eqref{eq:thm:real_number_positive_power}} =
  ...
  \SubProofOf{thm:def:real_nth_root/polynomial} Follows from \fullref{thm:def:real_nth_root/power_of_root}.

  \SubProofOf{thm:def:real_nth_root/multiplicative} If \( A = O \) or \( B = O \), then both sides of \eqref{eq:thm:def:real_nth_root/multiplicative} are equal to \( O \).
...
\begin{definition}\label{def:extended_real_numbers}\mimprovised
  ...
  We will extend the algebraic operations of \( \BbbR \) based on \cite[\S 1.1.17]{Schechter1997AnalysisHandbook} and \cite[11]{Folland1999RealAnalysis}. We leave \( \infty - \infty \) undefined and take precautions as not to encounter this situation (see for example \fullref{def:measure/positive}). The absorbing property of \( 0 \) when multiplied by either \( -\infty \) or \( \infty \) is controversial, however it is quite useful in \fullref{ch:measure_theory} and \fullref{ch:convex_analysis}. The rest of the conventions are motivated by \incite[sec. IV.4]{Bourbaki1995Topology1to4}.
