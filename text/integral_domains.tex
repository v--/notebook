...
\begin{proposition}\label{thm:def:integral_domain}
  ...
  \begin{thmenum}
    \thmitem{thm:def:integral_domain/subring} Any nonzero \hyperref[def:ring/submodel]{subring} of an integral domain is also an integral domain.
...
\begin{remark}\label{rem:prime_and_irreducible_terminology}
  ...
  \begin{itemize}
    ...
    Later Russian authors, for example \textcite[def. 3.5]{Винберг2014КурсАлгебры} and \textcite[30]{Шафаревич1999ОсновныеПонятияАлгебры}, call irreducible elements \enquote{простые} (which is used as a translation for \enquote{prime} elsewhere in the book, e.g. for prime ideals). This also transfers to Bulgarian books --- for example, \enquote{прост елемент} is used by \textcite[def. VI.5]{ГеновМиховскиМоллов1991Алгебра} --- but \textcite[413]{Обрешков1962ВисшаАлгебра} uses both \enquote{прост елемент} and \enquote{неразложим елемент} as synonyms for irreducible elements in general integral domains. \Textcite[125]{Тыртышников2017ОсновыАлгебры} instead defines irreducible elements as \enquote{\textru{неприводимые елементы}} and avoid introducing prime elements (but uses prime ideals).
    ...
    \textcite[19]{Шафаревич1999ОсновныеПонятияАлгебры}
