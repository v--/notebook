...
\begin{proposition}\label{thm:def:integral_domain}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:def:integral_domain/polynomial_invertible} A polynomial is \hyperref[def:divisibility/invertible]{invertible} in \( D[X] \) if and only if it is (the embedding of) an invertible element of \( D \).
  \end{thmenum}
\end{proposition}
\begin{proof}
...
\begin{proposition}\label{thm:ring_entire_iff_unique_quotient}
  A \hyperref[def:ring/commutative]{commutative ring} is \hyperref[def:entire_semiring]{entire} if and only if, whenever \( y \) \hyperref[def:divisibility]{divides} \( x \), there exists a \emph{unique} element \( z \) such that \( x = yz \).
\end{proposition}
\begin{proof}
  ...
  \NecessitySubProof Suppose instead that uniqueness holds and let \( 0 = yz \). By absorption, we have \( 0 = y0 \), and thus \( z = 0 \), showing that \( y \) has no zero divisors.

  Since \( y \) was arbitrary, we conclude that the ring is entire.
\end{proof}

\begin{example}\label{ex:nonunique_divisor}
  The requirement in \cref{thm:ring_entire_iff_unique_quotient} for the ring to be entire is essential --- \cref{thm:idempotent_division} implies that (nontrivial) multiplicatively idempotent elements are zero divisor.
