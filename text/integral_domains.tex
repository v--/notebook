...
\begin{definition}\label{def:irreducible_factorization}\mcite[def. V.1.8]{Aluffi2009Algebra}
  ...
  \begin{thmenum}
    ...
    \end{equation*}
    are \term{equivalent} if \( n = m \) and if there exists a \hyperref[def:symmetric_group]{permutation} \( \pi \in S_n \) such that \( p_k \) and \( q_{\pi(k)} \) are \hyperref[def:domain_divisibility/associates]{associated} for every \( k = 1, \ldots, n \).
