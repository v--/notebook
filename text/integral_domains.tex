...
\begin{proof}
  ...
  \SufficiencySubProof* It is sufficient to prove the statement for one indeterminate. If \( p(X) \) and \( q(X) \) are nonzero polynomials, then so is \( p(X) q(X) \) by \fullref{thm:polynomial_degree_arithmetic/product}.
  ...
  If \( p(X) \) has a root \( u \), by the equivalence in \fullref{def:multiple_root}, \( (X - u) \) divides \( p(X) \). Then \( p(X) / (X - u) \) has degree \( n - 1 \) by \fullref{thm:polynomial_degree_arithmetic/product}. Let \( M \) be the multiset of roots of \( p(X) / (X - u) \). After adding \( u \) to \( M \), we have incremented its total cardinality by \( 1 \), thus making it at most \( n \).
...
\begin{remark}\label{rem:prime_and_irreducible_terminology}
  ...
  \begin{itemize}
    ...
    \incite[343]{Jacobson1989AlgebraI},
...
\begin{definition}\label{def:irreducible_factorization}\mcite[def. V.1.8]{Aluffi2009Algebra}
  An \term{irreducible factorization} or simply \term{factorization} of a nonzero element \( x \) in an arbitrary \hyperref[def:integral_domain]{integral domain} is a finite sequence \( p_1, \ldots, p_r \) of \hyperref[def:domain_divisibility/irreducible]{irreducible elements} such that, for some \hyperref[def:divisibility/invertible]{invertible element} \( u \),
  \begin{equation*}
    x = u p_1 \cdots p_r.
  \end{equation*}
...
\begin{comments}
  ...
  \begin{itemize}
    ...
    \item \incite[392]{Knapp2016BasicAlgebra} and \incite[def 2.5]{Jacobson1989AlgebraI} use the term \enquote{Euclidean domain}, however they both require \( \delta \) to be defined for zero elements and neither states \fullref{def:euclidean_domain/multiplication}.
  \end{itemize}
\end{comments}
...
\begin{definition}\label{def:domain_primitive_polynomial}\mcite[108]{Jacobson1989AlgebraI}
