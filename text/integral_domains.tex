...
\begin{proof}
  ...
  If \( f(X) \) has a root \( u \), by the equivalence in \cref{def:multiple_root}, \( (X - u) \) divides \( f(X) \). Then \( f(X) / (X - u) \) has degree \( n - 1 \) by \cref{thm:polynomial_degree_arithmetic/product}. Let \( M \) be the multiset of roots of \( f(X) / (X - u) \). After adding \( u \) to \( M \), we have incremented its total cardinality by \( 1 \), thus making it at most \( n \).
