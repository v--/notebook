...
\begin{definition}\label{def:integral_domain}\mcite[def. III.1.10]{Aluffi2009Algebra}
  An \term[bg=област на цялостност (\cite[393]{Обрешков1962ВисшаАлгебра}), ru=область целостности (\cite[def. 3.5.1]{Винберг2014КурсАлгебры})]{integral domain} is an \hyperref[def:ring/trivial]{nonzero} \hyperref[def:entire_semiring]{entire} \hyperref[def:ring/commutative]{commutative (unital) ring}.
\end{definition}
...
\begin{proposition}\label{thm:def:integral_domain}
  ...
  \begin{thmenum}
    \thmitem{thm:def:integral_domain/subring} Any nonzero \hyperref[def:ring/subobject]{subring} of an integral domain is also an integral domain.
    ...
    \thmitem{thm:def:integral_domain/polynomial_invertible} A polynomial is \hyperref[def:semiring_divisibility/invertible]{invertible} in \( D[X] \) if and only if it is (the embedding of) an invertible element of \( D \).
  \end{thmenum}
\end{proposition}
\begin{proof}
  ...
  \SufficiencySubProof* It is sufficient to prove the statement for one indeterminate. If \( f(X) \) and \( g(X) \) are nonzero polynomials, then so is \( f(X) g(X) \) by \cref{thm:univariate_polynomial_product/degree}.
  ...
  Since we are working over a domain and there are no proper zero divisors, there exists only one pair of nonnegative integers \( m \) and \( l \) such that \( a_m b_l = 1 \). It follows that \( f(X) = a_m X^m \) and \( g(X) = a_l X^l \).

  \SubProofOf{thm:def:integral_domain/polynomial_invertible}
  \SufficiencySubProof* Suppose that \( f(X) \) is invertible in \( D[X] \). Then there exists some \( g(X) \) such that \( f(X) \cdot g(X) = 1 \).

  \Cref{thm:univariate_polynomial_product/degree} implies that \( \deg f = \deg g = 0 \), hence both are constant polynomials whose leading coefficients multiply to \( 1 \).

  \NecessitySubProof* If \( f(X) = a \), where \( a \) is invertible in \( D[X] \) with \( ab = 1 \), then \( g(X) \coloneqq b \) is such that \( f(X) \cdot g(X) = 1 \).
\end{proof}
...
\end{comments}
\begin{proof}
  \SufficiencySubProof Suppose that \( P \) is a prime ideal. Clearly \( R / P \) is a commutative ring. Since \( P \) is a proper ideal, \( R / P \) must be nonzero. We will show that it is an \hyperref[def:entire_semiring]{entire ring}.
  ...
  \NecessitySubProof Suppose that \( R / P \) is an integral domain. Since \( R / P \) is nonzero, \( P \) must be a proper ideal. We will show that it satisfies \cref{thm:def:semiring_ideal/prime_pointwise}.

  Let \( xy \in P \). We have
  \begin{equation*}
    P = [0] = [xy] = [x] [y],
  \end{equation*}
  hence \( [x] \) and \( [y] \) are proper zero divisors in \( R / P \). But \( R / P \) is entire, hence either \( [x] \) or \( [y] \) must be zero. That is, either \( x \in P \) or \( y \in P \).
...
\begin{proposition}\label{thm:ring_entire_iff_unique_quotient}
  A \hyperref[def:ring/commutative]{commutative ring} is \hyperref[def:entire_semiring]{entire} if and only if, whenever \( y \) \hyperref[def:semiring_divisibility]{divides} \( x \), there exists a \emph{unique} element \( z \) such that \( x = yz \).
\end{proposition}
\begin{proof}
  ...
  \NecessitySubProof Suppose instead that uniqueness holds and let \( 0 = yz \). By absorption, we have \( 0 = y0 \), and thus \( z = 0 \), showing that \( y \) has no proper zero divisors.

  Since \( y \) was arbitrary, we conclude that the ring is entire.
\end{proof}

\begin{example}\label{ex:nonunique_divisor}
  The requirement in \cref{thm:ring_entire_iff_unique_quotient} for the ring to be entire is essential --- \cref{thm:idempotent_division} implies that (nontrivial) multiplicatively idempotent elements are proper zero divisor.

  For example, if \( R \) is nonzero, consider the \hyperref[thm:matrix_algebra]{matrix algebra} \( R^{2 \times 2} \) and the idempotent matrix
  \begin{equation*}
    \begin{pmatrix}
      1 & 0 \\
      0 & 0
...
\begin{definition}\label{def:domain_divisibility}
  We will introduce several notions related to \hyperref[def:semiring_divisibility]{divisibility} in \hyperref[def:integral_domain]{integral domains}.
  ...
  \begin{thmenum}
    ...
    \begin{thmenum}
      ...
      \thmitem{def:domain_divisibility/associates/invertible}\mcite[393]{Knapp2016BasicAlgebra} There exists an \hyperref[def:semiring_divisibility/invertible]{invertible element} \( u \) such that \( x = uy \).
...
\begin{remark}\label{rem:prime_and_irreducible_terminology}
  ...
  \begin{itemize}
    \item \Textcite[111]{Lang2002Algebra} uses the term \enquote{irreducible} for what we call \enquote{prime}, and later notes that, in factorial domains, the principal ideal of an irreducible element is prime, and thus it makes sense to call irreducible elements prime in this context. In the Russian translation of the book, in \cite[89]{Ленг1968Алгебра}, irreducible elements are called \enquote{неприводимые элементы}, and prime elements are called \enquote{простые елементы}.

    Later Russian authors, for example \textcite[def. 3.5]{Винберг2014КурсАлгебры} and \textcite[30]{Шафаревич2019ОсновныеПонятияАлгебры}, call irreducible elements \enquote{простые} (which is used as a translation for \enquote{prime} elsewhere in the book, e.g. for prime ideals). This also transfers to Bulgarian books --- for example, \enquote{прост елемент} is used by \textcite[def. VI.5]{ГеновМиховскиМоллов1991Алгебра} --- but \textcite[413]{Обрешков1962ВисшаАлгебра} uses both \enquote{прост елемент} and \enquote{неразложим елемент} as synonyms for irreducible elements in general integral domains. \Textcite[125]{Тыртышников2017ОсновыАлгебры} instead defines irreducible elements as \enquote{\textru{неприводимые елементы}} and avoid introducing prime elements (but uses prime ideals).

    In the context of polynomials over fields, however,
    \textcite[161]{Курош1968КурсВысшейАлгебры},
    \textcite[121]{Винберг2014КурсАлгебры},
    \textcite[190]{Кострикин2020АлгебраЧасть1} and
    \textcite[19]{Шафаревич2019ОсновныеПонятияАлгебры}
    use \enquote{неприводимый многочлен} and \textcite[409]{Обрешков1962ВисшаАлгебра} and \textcite[def. VI.3]{ГеновМиховскиМоллов1991Алгебра} use \enquote{неразложим полином} for what we call an irreducible polynomial, and avoid mentioning prime polynomials.
    ...
    \textcite[343]{Jacobson1985BasicAlgebraI},
    \textcite[388; 389]{Knapp2016BasicAlgebra},
    \textcite[27; 29]{Eisenbud1995CommutativeAlgebra} and
    \textcite[def. V.1.6]{Aluffi2009Algebra}.

    \textcite[67]{Rotman2015AdvancedModernAlgebraPart1} only defines irreducible elements, mentioning \enquote{prime elements} in \cite[105]{Rotman2015AdvancedModernAlgebraPart1} as a remark.
  \end{itemize}
\end{remark}
...
\begin{proof}
  ...
  \SufficiencySubProof* Suppose that \( x \) is irreducible in \( D \) and let \( x = y(X) z(X) \). By \cref{thm:univariate_polynomial_product/degree}, both \( y(X) \) and \( z(X) \) must be constant polynomials. Therefore, they are scalars, and since \( x \) is irreducible, \( y \) or \( z \) is invertible. By \cref{thm:def:domain_divisibility/irreducible_in_polynomial_ring}, if \( y \) is invertible in \( D \), it is invertible in \( D[X] \).

  Generalizing on \( x \), it follows that every irreducible element in \( D \) is also irreducible in \( D[X] \).

  \NecessitySubProof* Suppose that \( x \) is irreducible in \( D[X] \) and let \( x = yz \). Then \( y \) or \( z \) is invertible in \( D[X] \), and thus again by \cref{thm:def:domain_divisibility/irreducible_in_polynomial_ring}, it is invertible in \( D \).
...
\begin{example}\label{ex:def:domain_divisibility}
  ...
  \begin{thmenum}
    ...
    \Cref{thm:integer_multiplicative_invertibility} implies that the invertible elements of \( \BbbZ \) are \( 1 \) and \( -1 \).
    ...
    \begin{equation}\label{eq:ex:def:domain_divisibility/irreducible_not_prime/decomposition}
      n = \parens*{ a + b \sqrt{-5} }\parens*{ c + d \sqrt{-5}}.
    \end{equation}
    ...
    \begin{equation}\label{eq:ex:def:domain_divisibility/irreducible_not_prime/abs}
      \abs{n}^2
      =
      \abs*{a + b \sqrt{-5}}^2 \cdot \abs*{c + d \sqrt{-5}}^2
      =
      (a^2 + 5b^2) (c^2 + 5d^2)
      =
    ...
    \begin{equation*}
      6 = 2 \cdot 3 = \parens*{ 1 + \sqrt{-5} } \cdot \parens*{ 1 - \sqrt{-5}}.
    \end{equation*}
...
\begin{remark}\label{rem:choice_of_associates}
  ...
  \begin{itemize}
    ...
    \item In a polynomial ring over the integers \( \BbbZ \), by \cref{thm:def:domain_divisibility/irreducible_in_polynomial_ring}, the invertible elements are again \( 1 \) and \( -1 \), and we can choose the leading coefficient to be positive.
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \Cref{thm:univariate_polynomial_product/degree} implies that \( q(X) \) has degree \( n \). Let \( M \) be its multiset of roots. By the inductive hypothesis, \( M \) has total cardinality at most \( n - 1 \). After adding \( \alpha \) to \( M \), we increment its total cardinality by \( 1 \).
...
\begin{theorem}[Polynomial comparison principle]\label{thm:polynomial_comparison_principle}
  In an \hyperref[def:integral_domain]{integral domain}, two univariate polynomials of degree \( n \) are equal if and only if they agree on (at least) \( n + 1 \) values for their arguments.
\end{theorem}
\begin{comments}
  \item The name of the theorem is based on the Bulgarian phrase \enquote{принцип за сравняване на коефициентите при полиномите} (\enquote{principle for comparing the coefficients of polynomials}), which is used to describe the theorem in some books like \cite[192]{Тагамлицки1971ДиференциалноСмятане}.
\end{comments}
\begin{proof}
  \SufficiencySubProof Trivial.
  \NecessitySubProof Let \( f(X) \) and \( g(X) \) be polynomials of degree \( n \) over the domain \( D \). Consider their difference \( f(X) - g(X) \). By \cref{thm:univariate_polynomial_sum/degree}, this difference is either the zero polynomial or its degree is at most \( n \). In the latter case, \cref{thm:polynomial_degree_bounds_root_count} implies that \( f(X) - g(X) \) has at most \( n \) roots, counting multiplicities.
...
\begin{corollary}\label{thm:parametrized_polynomial_evaluation_isomorphism}
  Consider the \hyperref[def:polynomial_algebra]{polynomial algebra} \( D[\mscrX] \), where \( \mscrX \) contains \emph{finitely} many indeterminates.

  Then, for any \hyperref[def:algebra_over_ring/commutative]{commutative \( D \)-algebra} \( M \), the \hyperref[con:free_construction/evaluation]{parametrized evaluation homomorphism} \( \Phi_M: D[\mscrX] \to \fun(D^\mscrX, D) \) is an embedding if and only if \( D \) is infinite.
\end{corollary}
\begin{comments}
  \item In particular, if \( D \) is infinite, every polynomial corresponds to a unique polynomial function.
\end{comments}
\begin{proof}
  Let \( n \) be the cardinality of \( \mscrX \).

  Note that \( D[\mscrX] \) is necessarily infinite because it embeds the monomials, and \cref{thm:cardinality_exponentiation_rule} implies that the set of monomials \( \BbbN^{\oplus \mscrX} \) has cardinality \( \aleph_0^n \).

  \SufficiencySubProof Suppose that \( \Phi_M \) is bijective.

  If \( M \) has finitely many elements, say \( m \), \cref{thm:cardinality_exponentiation_rule} implies that \( M^\mscrX \) has \( m^n \) elements and \( \fun(M^\mscrX, M) \) has \( m^{m^n} \) elements.

  But then \( \Phi_M \) cannot be bijective because, as we saw, \( D[\mscrX] \) has infinitely many elements.

  \NecessitySubProof Suppose that \( D \) is infinite. Since \( D \) embeds into \( M \), the latter is also infinite.

  We will show that \( \Phi_M \) is injective. Suppose that \( \Phi_M(f) = \Phi_M(g) \). Due to linearity of \( \Phi_M \), the function \( \Phi_M(f - g) \) is canonically zero.

  Then all infinitely many elements of \( M \) are roots of \( f - g \). \Cref{thm:polynomial_comparison_principle} implies that \( f = g \).
\end{proof}

\begin{example}\label{ex:polynomial_root_uniqueness}
  We list some examples related to \fullref{thm:polynomial_comparison_principle}:
  \begin{thmenum}
    ...
    So, as long as \( R \) is big enough, we can easily find counterexamples to the comparison principle --- namely, we can add \( f(X) \) to any polynomial of degree greater than \( 2 \). A simple example is the pair \( X^3 \) and \( X^3 + X^2 - X \).
...
\end{proposition}
\begin{proof}
  Consider \( f(X) = X + a \). \Cref{thm:univariate_polynomial_product/degree} implies that if
  \begin{equation*}
    f(X) = g(X) \cdot h(X),
  \end{equation*}
...
\begin{proposition}\label{thm:small_degree_polynomial_without_roots_irreducible}
  A \hyperref[def:monic_polynomial]{\emph{monic}} quadratic or cubic univariate polynomial over an integral domain \( D \) without \hyperref[def:polynomial_root]{roots} in \( D \) is \hyperref[def:domain_divisibility/irreducible]{irreducible} over \( D \).
\end{proposition}
\begin{proof}
  Let \( f(X) \) be a polynomial over \( D \) of degree at most \( 3 \) that has no roots in \( D \).

  Let \( f(X) = g(X) \cdot h(X) \) and suppose that \( g(X) \) is not invertible. \Cref{thm:univariate_polynomial_product/nonzero_leading} implies that the leading coefficients or \( g(X) \) and \( h(X) \) multiply to \( 1 \) since \( f(X) \) is monic, hence the leading coefficient of \( g(X) \) is invertible. But \( g(X) \) itself is not invertible, and \cref{thm:def:domain_divisibility/irreducible_polynomial_coefficients} implies it is not constant. Then \( g(X) \) has positive degree.

  Then \cref{thm:univariate_polynomial_product/degree} gives us three possibilities:
  \begin{itemize}
    \item If \( g(X) \) is linear, we already saw that the leading coefficient of \( g(X) \) is invertible, and \cref{thm:linear_polynomial_over_field_has_root} implies that \( g(X) \) has a root in \( D \). This is also a root of \( f(X) \), contradicting our initial assumption that \( f(X) \) has no root in \( D \).
...
\begin{example}\label{ex:common_polynomial_divisors}\mcite{MathSE:polynomials_without_gcd}
  ...
  Let \( C \) be the set of all common divisors of \( f(X) \) and \( g(X) \). Every invertible element of the domain is itself a common divisor, so \( C \) is necessarily nonempty. Consider the \hyperref[thm:semiring_divisibility_order]{divisibility (pre)order} in \( C \). A \hyperref[def:extremal_points/maximum_and_minimum]{greatest element} of \( C \) with respect to divisibility (i.e. a multiple of all elements) must contain all common roots of \( f(X) \) and \( g(X) \), and thus it makes sense to search for greatest common divisors.
  ...
  \begin{thmenum}
    ...
    This general problem comes from the lack of antisymmetry in preorders --- see \cref{ex:def:preordered_set}.
...
\begin{definition}\label{def:gcd}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:gcd/infimum} \( g \) is a \hyperref[def:extremal_points/supremum_and_infimum]{infimum} of \( x \) and \( y \) with respect to the \hyperref[thm:semiring_divisibility_order]{divisibility order}.

    Here we use that infima can be generalized to preorders by dropping the uniqueness assumption.
...
\end{definition}
\begin{comments}
  \item There may be distinct greatest common divisors that divide each other --- see \cref{ex:common_polynomial_divisors/distinct} for a simple example or \cref{ex:def:preordered_set} for a discussion of this problem for general preordered sets. For the \hyperref[def:natural_numbers]{natural numbers}, where the GCD of \( x \) and \( y \) is unique, we denote it via \( \gcd(x, y) \).
  ...
  \item Some authors like \textcite[111]{Lang2002Algebra}, \textcite[144]{Jacobson1985BasicAlgebraI} and \textcite[2]{Knapp2016BasicAlgebra} leave greatest common divisors of \( 0 \) and \( 0 \) undefined, while others like \textcite[304]{Rotman2015AdvancedModernAlgebraPart1}, \textcite[def. V.2.2]{Aluffi2009Algebra}, \textcite[119]{Винберг2014КурсАлгебры} and \textcite[143]{ГеновМиховскиМоллов1991Алгебра} do not handle them as a special case.
\end{comments}
\begin{defproof}
...
\begin{definition}\label{def:lcm}
  Dually to \cref{def:gcd}, we say that, in a \hyperref[def:integral_domain]{integral domain}, \( l \) is a \term[bg=най-малко общо кратно (на числа) (\cite[381]{ГеновМиховскиМоллов1991Алгебра}), ru=наименьшее общее кратное (\cite[exerc. 3.6.3]{Винберг2014КурсАлгебры}), en=least common multiple (\cite[32]{Knapp2016BasicAlgebra})]{least common multiple} (LCM) of \( x \) and \( y \) if it satisfies the following equivalent conditions:
...
\begin{proof}
  ...
  \begin{equation*}
    xy = \parens*{ d \cdot \frac x d } \cdot y = d \cdot \frac {xy} d = dm.
  \end{equation*}
...
\begin{remark}\label{rem:gcd_but_no_lcm}
  ...
  \textcite[thm. 4]{Khurana2003GCD} provides counterexamples --- in \( \BbbZ[\sqrt{-3}] \), the elements \( x = 2 \) and \( y = 1 + \sqrt{-3} \) have a GCD, but not a LCM.

  Fortunately, if \emph{every} pair of elements has a GCD, then every pair also has a LCM.
\end{remark}
