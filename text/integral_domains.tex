...
\begin{corollary}\label{thm:parametrized_polynomial_evaluation_isomorphism}
  Consider the \hyperref[def:polynomial_algebra]{polynomial algebra} \( D[\mscrX] \), where \( \mscrX \) contains \hi{finitely} many indeterminates.
...
\begin{proposition}\label{thm:small_degree_polynomial_without_roots_irreducible}
  A \hyperref[def:monic_polynomial]{\hi{monic}} quadratic or cubic univariate polynomial over an integral domain \( D \) without \hyperref[def:polynomial_root]{roots} in \( D \) is \hyperref[def:domain_divisibility/irreducible]{irreducible} over \( D \).
\end{proposition}
\begin{proof}
