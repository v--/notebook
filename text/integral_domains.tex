...
\begin{proof}
  ...
  \SufficiencySubProof* Suppose that \( x \) is irreducible in \( D \) and let \( x = y(X) z(X) \). By \cref{thm:univariate_polynomial_product/degree}, both \( y(X) \) and \( z(X) \) must be constant polynomials. Therefore, they are scalars, and since \( x \) is irreducible, \( y \) or \( z \) is invertible. By \cref{thm:def:polynomial_algebra/invertible}, if \( y \) is invertible in \( D \), it is invertible in \( D[X] \).
...
\begin{proof}
  ...
  Let \( f(X) = g(X) \cdot h(X) \) and suppose that \( g(X) \) is not invertible. \Cref{thm:univariate_polynomial_product/leading} implies that the leading coefficients or \( g(X) \) and \( h(X) \) multiply to \( 1 \) since \( f(X) \) is monic, hence the leading coefficient of \( g(X) \) is invertible. But \( g(X) \) itself is not invertible, and \cref{thm:def:domain_divisibility/irreducible_polynomial_coefficients} implies it is not constant. Then \( g(X) \) has positive degree.
