...
\begin{definition}\label{def:euclidean_domain}\mcite[392]{Knapp2016BasicAlgebra}
  ...
  \end{equation*}
  which we call the \term{Euclidean degree}, such that for every pair \( x \) and \( y \) of elements of \( D \) in which \( y \) is nonzero, there exists a pair \( q \) and \( r \), where \( r \) is either zero or \( \delta(r) < \delta(q) \), such that
...
\begin{algorithm}[Euclidean algorithm]\label{alg:euclidean_algorithm}
  ...
  \begin{thmenum}
    \thmitem{alg:euclidean_algorithm/guard} If \( y \) is zero, halt the algorithm with \( g \coloneqq x \).
    \thmitem{alg:euclidean_algorithm/initialization} Otherwise, define \( r_{-1} \coloneqq x \) and \( r_0 \coloneqq y \).
