...
\end{proposition}
\begin{proof}
  \SufficiencySubProof Suppose that \( R \) is entire. Let \( x = yz = yz' \). \Fullref{thm:def:ring/cancellative_iff_no_zero_divisors} implies that we can cancel \( y \) to obtain \( z = z' \). This demonstrates uniqueness.
...
\end{proposition}
\begin{comments}
  \item The premise always holds in \hyperref[eq:def:bezout_domain]{Bezout domains}.
\end{comments}
\begin{proof}
...
\end{proposition}
\begin{proof}
  \SubProofOf{thm:def:coprime_elements/monotone} Suppose that \( x \) and \( y \) are coprime and let \( z \) be a divisor of \( y \).
