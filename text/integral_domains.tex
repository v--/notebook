...
\end{proposition}
\begin{proof}
  Consider \( f(X) = X + a \). \Cref{thm:polynomial_degree_arithmetic/product} implies that if
  \begin{equation*}
    f(X) = g(X) \cdot h(X),
  \end{equation*}
...
\begin{proof}
  ...
  Let \( f(X) = g(X) \cdot h(X) \) and suppose that \( g(X) \) is not invertible. \Cref{thm:leading_coefficient_of_product} implies that the leading coefficients or \( g(X) \) and \( h(X) \) multiply to \( 1 \) since \( f(X) \) is monic, hence the leading coefficient of \( g(X) \) is invertible. But \( g(X) \) itself is not invertible, and \cref{thm:def:domain_divisibility/irreducible_polynomial_coefficients} implies it is not constant. Then \( g(X) \) has positive degree.

  Then \cref{thm:polynomial_degree_arithmetic/product} gives us three possibilities:
  \begin{itemize}
