...
\begin{proof}
  ...
  \SubProofOf[rem:well_founded_relation]{well-foundedness} We will show by induction on \( n \) that an infinitely descending chain ending at \( n \) cannot exist.
...
\begin{proposition}\label{thm:natural_number_divisibility_lattice}
  ...
  \begin{thmenum}
    \thmitem{thm:natural_number_divisibility_lattice/join} The \hyperref[def:semilattice/join]{join} of \( n \) and \( m \) is their \hyperref[def:lcm]{least common multiple} \( \lcm\set{ n, m } \) (defined via \fullref{alg:euclidean_algorithm} and \fullref{thm:gcd_and_lcm}).
    ...
    \thmitem{thm:natural_number_divisibility_lattice/meet} Dually, the \hyperref[def:semilattice/meet]{meet} of \( n \) and \( m \) is their \hyperref[def:gcd]{greatest common divisor} \( \gcd\set{ n, m } \) (defined via \fullref{alg:euclidean_algorithm}).
