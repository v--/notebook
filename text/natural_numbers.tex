...
\begin{definition}\label{def:peano_arithmetic}\mcite[def. 2.7.7]{Jech2003}
...
\begin{proof}
  ...
  We have already shown in \fullref{def:natural_numbers_ordering} that due to \eqref{def:partially_ordered_set/compatibility_strict} the equality \( n = m \) holds if and only if neither \( n < m \) nor \( n > m \).
