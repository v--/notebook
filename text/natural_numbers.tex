...
\end{definition}
\begin{comments}
  \item See \fullref{ex:natural_number_arithmetic_grammar/rules} for a simple \hyperref[def:formal_grammar]{grammar} that produces numeric symbols in their decimal notation.
\end{comments}
...
\begin{proof}
  ...
  \SubProofOf[rem:well_founded_relation]{well-foundedness} We will show by induction on \( n \) that an strictly descending sequence ending at \( n \) cannot exist.
...
\begin{proposition}\label{thm:natural_numbers_confinal_equivalence}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:natural_numbers_confinal_equivalence/unbounded} It is \hyperref[def:extremal_points/upper_and_lower_bounds]{unbounded from above}.
...
\begin{proposition}\label{thm:natural_number_divisibility_lattice}
  ...
  \begin{thmenum}
    \thmitem{thm:natural_number_divisibility_lattice/join} The \hyperref[def:semilattice/join]{join} of \( n \) and \( m \) is their unique \hyperref[def:lcm]{least common multiple} \( \lcm(n, m) \), denoted by \( \lcm(n, m) \).
    ...
    \thmitem{thm:natural_number_divisibility_lattice/meet} Dually, the \hyperref[def:semilattice/meet]{meet} of \( n \) and \( m \) is their unique \hyperref[def:gcd]{greatest common divisor}, denoted by \( \gcd(n, m) \) and constructed via \fullref{alg:euclidean_algorithm}.
