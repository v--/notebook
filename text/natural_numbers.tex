...
\begin{example}\label{ex:peano_arithmetic_nonstandard_models}
  ...
  \begin{thmenum}
    ...
    Using \fullref{thm:upward_lowenheim_skolem_theorem}, we can construct an \hyperref[def:elementary_embedding]{elementary extension} \( \mscrM \) of \( \mscrN \) of any desired cardinality, say \hyperref[def:aleph_hierarchy]{\( \aleph_1 \)}.
    ...
    Let \( \Sigma^+ \) be the extension of \( \Sigma_{\logic{PA}} \) by a single individual constant, \( \sync \). Let \( T \) be the extension of \( \logic{PA}^+ \) with the following axioms:
    \begin{align*}
    ...
    \Fullref{thm:fol_semantic_compactness} implies that \( \logic{PA}^+ \) is \hyperref[def:fol_semantics/satisfaction]{satisfiable}. Then there exists a \( \mscrM \) of \( \logic{PA}^+ \), which is incidentally also a model of \( \logic{PA} \).

    \Cref{thm:peano_arithmetic_standard_segment} implies that \( \mscrN \) embeds into \( \mscrM \). Because of the new axioms imposed on \( \mscrM \), we can show by \fullref{thm:induction_on_natural_numbers} on \( \mscrN \) that the denotation of \( \sync \) in \( \mscrM \) differs from the denotation of any term in \( \mscrN \).
...
\begin{proof}
  ...
  \begin{itemize}
    \item The axiom \eqref{eq:def:peano_arithmetic/theory/PA1} states that the function \( s \) is injective. By the equivalences in \cref{def:function_invertibility/injective}, its \hyperref[def:set_valued_map/inverse]{inverse set-valued map} is actually a \hyperref[def:set_valued_map/partial]{single-valued partial function}. Denote this inverse by \( p \).

    \item The axiom \eqref{eq:def:peano_arithmetic/theory/PA2} states that the function \( s \) is not surjective. By the equivalences in \cref{def:function_invertibility/surjective}, the inverse \( p \) is not a \hyperref[def:set_valued_map/partial]{total function}.
  \end{itemize}
  ...
  This is precisely the antecedent of the instance of \eqref{eq:def:peano_arithmetic/theory/PA3} with \( \varphi = \theta \). By \cref{thm:syntactic_deduction_theorem} we have
  \begin{equation*}
    \eqref{eq:def:peano_arithmetic/theory/PA3} \vdash \qforall \syny \theta[\synx \mapsto \syny].
  \end{equation*}
  ...
  We can use only \eqref{eq:def:peano_arithmetic/theory/PA1} to show that \( s \) is injective and then pick \( p \) to be any of its left inverses. We also want \( p \) to be as close as possible to a right inverse, however. The latter, as we have seen, is more tricky.
\end{proof}
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \item If \( m \neq 0 \) and if the inductive hypothesis holds for \( p(m) \), \eqref{eq:def:peano_arithmetic/theory/PA1} yields that \( n + m = m + n \) if and only if \( n + p(m) = p(m) + n \). But the last equality is satisfied because of the inductive hypothesis, hence commutativity of \( n \) and \( m \) follows.
  \end{itemize}
  ...
  If \( k \neq 0 \), the proof follows directly from \eqref{eq:def:peano_arithmetic/theory/PA1} as in our proof of commutativity.
  ...
  \begin{equation*}
    ...
    m + s(p(k)),
  \end{equation*}
  which by \eqref{eq:def:peano_arithmetic/theory/PA5} is equivalent to \( s(n + p(k))) = s(m + p(k))) \).

  By \eqref{eq:def:peano_arithmetic/theory/PA1}, we have \( n + p(k) = m + p(k) \). The inductive hypothesis implies that \( n = m \).
  ...
  \begin{itemize}
    ...
    \item If \( m > 0 \), then \( n + m = s(n + p(m)) \) by \eqref{eq:def:peano_arithmetic/theory/PA5}, and by \eqref{eq:def:peano_arithmetic/theory/PA2}, \( n + m \neq 0 \).
  \end{itemize}
...
\begin{proof}
  ...
  If \( m = 0 \), then \( n \cdot m = 0 \) by \eqref{eq:def:peano_arithmetic/theory/PA6}. If \( n = 0 \), then by induction on \( m \) we can easily show that \( n \cdot m = n \cdot p(m) + n = 0 + 0 = 0 \).
...
\begin{proof}
  ...
  If \( n = 0 \), by \eqref{eq:def:peano_arithmetic/theory/PA2} \( n \) has no predecessor and thus there cannot exist a natural number \( m \) such that \( m < n \).
