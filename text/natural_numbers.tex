...
\begin{remark}\label{rem:peano_arithmetic_zero}
  It is common to consider the first natural numbers to be \( 0 \). This is done by Peter Hinman in \incite[sec. 4.1]{Hinman2005Logic} and by \incite[71]{Enderton1977SetTheory}. Peano himself, however, considered \( 1 \) to be the first natural number - see \cite[1]{Peano1889PA}.
...
\end{comments}
\begin{proof}
  \SubProofOf{thm:peano_arithmetic_models/omega} \Fullref{thm:omega_is_model_of_pa} states that \( \omega \) is a model of \( \logic{PA} \).

  \SubProofOf{thm:peano_arithmetic_models/monoid} \Fullref{thm:cyclic_monoid_classification/infinite_monoid} implies that any infinite cyclic monoid is isomorphic to the \hyperref[def:formal_language/kleene_star]{Kleene star} \( \set{ a }^* \) over one symbol. There is a unique string corresponding to every natural number, thus we can use the different strings as numbers.

  \SubProofOf{thm:peano_arithmetic_models/initial} \Fullref{thm:category_of_semirings_properties/initial} implies that any model of \( \logic{PA} \) is an initial object in the category of unital semirings, and \fullref{thm:def:universal_objects/initial} implies that any other initial object is isomorphic to it.
\end{proof}
...
\begin{proof}
  ...
  If either \( n \) or \( m \) is zero, since, by \fullref{thm:natural_number_multiplication_properties}, the natural numbers are an entire semiring, their product \( nm \) is also zero; so they cannot be zero.
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    Note that \( 1 \leq m \). We have shown in \fullref{thm:natural_number_addition_properties} that addition is cancellative, hence \fullref{thm:def:ordered_semigroup/cancellative} implies that \( n + 1 < km + m \). By distributivity, we have \( (k + 1)m = km + m \), thus, as desired,
    \begin{equation*}
      n + 1 < (k + 1)m.
    \end{equation*}
  \end{itemize}
...
\begin{proof}
  ...
  \SubProofOf[def:binary_relation/antisymmetric]{antisymmetry} If \( n \mid m \) and \( m \mid n \), there exist numbers \( a \) and \( b \) such that \( n = ay \) and \( m = bx \). Then \( n = abx \), and we can cancel \( n \) to obtain that \( ab = 1 \). But, by \fullref{thm:natural_number_multiplicative_invertibility}, \( 1 \) is the only invertible element in \( \BbbN \), hence \( a = b = 1 \), and thus \( n = m \).
...
\begin{proposition}\label{thm:sums_of_powers}
  ...
  \begin{thmenum}
    ...
    These are precisely the triangular numbers, which we define in \fullref{def:triangular_number}; we also discuss there how triangular numbers relate to \hyperref[def:triangular_point_configuration]{triangular point configurations}.
...
\begin{proof}
  ...
  Subtracting the two\fnote{Even though we don't have subtraction for natural numbers, we have shown in \fullref{thm:natural_number_addition_properties} that addition of natural numbers is cancellative, which justifies this step.}, we obtain
  \begin{equation*}
