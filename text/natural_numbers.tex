...
\begin{definition}\label{def:peano_arithmetic}\mcite[sec. 4.1]{Hinman2005Logic}
  ...
  \begin{thmenum}[series=def:peano_arithmetic]
    ...
    \thmitem{def:peano_arithmetic/succ} A unary \hyperref[def:fol_signature/functions]{functional symbol} \( s \), called the \term{successor operation}.
    ...
    \thmitem{def:peano_arithmetic/plus} An \hyperref[def:fol_signature/notation]{infix} binary functional symbol \( + \) for denoting \term{addition}.

    See \cref{thm:natural_number_addition_properties} for the algebraic properties of natural number addition.

    \thmitem{def:peano_arithmetic/mult} Another infix binary functional symbol \( \cdot \) for denoting \term{multiplication}. Outside the \hyperref[con:metalanguage]{object language} we usually use juxtaposition instead.
  ...
  We impose the following base \hyperref[def:fol_theory]{axioms}:
  \begin{thmenum}[resume=def:peano_arithmetic]
    ...
    To describe this formally, we state that, for any variables \( \synx \) and \( \syny \) and any formula \( \varphi \) not containing \underline{\( \syny \)} as a \hyperref[def:fol_variable_freeness]{free variable}, the following is an axiom:
...
\begin{remark}\label{rem:standard_models_of_arithmetic}
  ...
  We want the two sets of natural numbers to coincide. This is important when talking about, for example, \hyperref[def:sequence]{sequences}. If a number in \( \BbbN \) is not a natural number within the metatheory, we say that it is \term{nonstandard}. The existence of nonstandard models is guaranteed by \fullref{thm:upward_lowenheim_skolem_theorem_theorem}. There cannot be numbers in the metatheory that are not in \( \BbbN \) because a model of \logic{PA} cannot have a finite domain and the natural numbers are the smallest metalogical infinite set.
