...
\end{definition}
\begin{comments}
  \item We will prefer representations of rational numbers \hyperref[def:lowest_terms]{in lowest terms} with positive denominators. \Cref{def:lowest_terms} implies that such a representation is unique.
...
\begin{definition}\label{def:rational_numbers_ordering}\mcite[lemma 5QH]{Enderton1977SetTheory}
...
\begin{proposition}\label{thm:def:rational_numbers_ordering}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:def:rational_numbers_ordering/archimedean} It makes \( \BbbQ \) an \hyperref[def:archimedean_semiring]{Archimedean field}.
...
\end{proposition}
\begin{proof}
  \SubProofOf{thm:def:rational_numbers_ordering/inverse} Let \( a / b \leq c / d \). Then \( ad \leq_\BbbZ bc \). \Cref{thm:def:integer_ordering/inverse} implies that \( -bc \leq_\BbbZ -ad \), and hence \( -c / d \leq - a / b \).

  The converse direction of the proof is identical.

  \SubProofOf{thm:def:rational_numbers_ordering/total} All necessary conditions for \( \leq \) to be a total order follow from the analogous property for integers --- \cref{thm:def:integer_ordering/total} --- similarly to our proof in \cref{thm:def:rational_numbers_ordering/inverse}.
  ...
  \begin{equation}\label{eq:thm:def:rational_numbers_ordering/archimedean_exponentiation/aux}
    \parens*{ \frac a b }^n > n\parens*{ \frac a b - 1 }.
  \end{equation}
  ...
  \small
  \begin{equation*}
    \parens*{ \frac a b }^{n+1} = \parens*{ \frac a b }^n\parens*{ \frac a b - 1 } + \parens*{ \frac a b }^n > \parens*{ \frac a b }^n\parens*{ \frac a b - 1 } + n\parens*{ \frac a b - 1 } > \parens*{ \frac a b - 1 } + n\parens*{ \frac a b - 1 } = (n + 1)\parens*{ \frac a b - 1 }.
  \end{equation*}
  \normalsize

  Having that in mind, \cref{thm:def:rational_numbers_ordering/archimedean} implies that, for some positive integer \( n \), we have
  \begin{equation*}
    n\parens*{\frac a b - 1} > \frac c d.
  \end{equation*}
...
\begin{algorithm}[Rational number power bisection]\label{alg:rational_number_power_bisection}
  ...
  \begin{thmenum}
    ...
    \begin{thmenum}
      ...
      We have
      \begin{equation*}
        l_0^n = \parens*{ \frac a b }^n < \frac a b
      \end{equation*}
      and
      \begin{equation*}
...
\end{algorithm}
\begin{comments}
  \item This algorithm can be found as \identifier{math.arithmetic.rational.power_bisection} in \cite{notebook:code}.
\end{comments}
\begin{proof}
...
\end{proposition}
\begin{comments}
  \item \Cref{thm:real_nth_root_is_irrational} establishes that this fails for \hyperref[def:real_numbers]{real numbers}.
...
\begin{proof}
  ...
  Then \( a^n = m \cdot b^n \). \Cref{thm:def:coprime_elements/gcd_power} implies that \( a^n \) and \( b^n \) are coprime since \( a \) and \( b \) are. But \( b^n \) divides \( a^n \), which is only possible if \( b^n = 1 \) and hence \( b = 1 \).
...
\begin{proof}
  ...
  \begin{align*}
    A \coloneqq \set*{ \frac a b \given* \parens*{ \frac a b }^2 \leq m }
    &&
    B \coloneqq \set*{ \frac a b \given* \parens*{ \frac a b }^2 \geq m }.
  \end{align*}

  \Cref{thm:nth_root_is_not_rational} implies that neither \( A \) has a minimum nor \( B \) has a maximum.
