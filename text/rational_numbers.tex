...
\begin{proposition}\label{thm:def:rational_number_ordering}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:def:rational_number_ordering/archimedean_exponentiation} For every positive rational numbers \( u \) and \( s > 1 \), there exists some integer \( n \) such that \( s^n > u \).
...
\begin{proof}
  ...
  \SubProofOf{thm:def:rational_number_ordering/archimedean_exponentiation} We will use an auxiliary statement: for any nonnegative integer \( n \), we have \( s^n > n(s - 1) \).
  ...
  Having that in mind, \fullref{thm:def:rational_number_ordering/archimedean} implies that \( n(s - 1) > u \) for some positive integer \( n \), and our auxiliary statement implies that \( s^n > u \).
