...
\begin{definition}\label{def:natural_transformation}\mcite[def. 7.6; def. 7.10]{Awodey2010CategoryTheory}
  ...
  We say that \( \alpha \) is a \term{natural isomorphism} if all its components are \hyperref[def:morphism_invertibility/isomorphism]{isomorphisms}, and similarly for left/right cancellable/invertible morphisms. This will be justified in \cref{thm:natural_transformation_invertibility}.
\end{definition}
...
\end{proposition}
\begin{comments}
  \item In case the components are function, this holds for both interpretation of function smallness from \cref{rem:small_functions}.

  Similarly, if the components are functors, this holds for both interpretations of functor smallness from \cref{rem:small_functors}.
\end{comments}
\begin{proof}
...
\begin{proposition}\label{thm:natural_transformation_invertibility}
\end{proposition}
