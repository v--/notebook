  ...
  \begin{thmenum}
    ...
    \begin{equation*}
      \begin{aligned}
        ...
        &R(f^\oppos: A \to X, g^\oppos: B \to Y, h: C \to Z) \coloneqq \parens[\big]{ \underbrace{s}_{\mathclap{A \to \fun(B, C)}} \mapsto \underbrace{\parens[\big]{ x \mapsto \parens[\big]{ y \mapsto h\parens[\big]{ \smash{ \overbrace{s(f(x))}^{B \to C}(f(y)) } } } }}_{X \to \fun(Y, Z)} }.
      \end{aligned}
    \end{equation*}
