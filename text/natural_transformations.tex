...
\begin{remark}\label{rem:opposite_natural_transformation}
  ...
  For the \hyperref[def:opposite_functor]{opposite functor} \( F^* \), given \( f: A \to B \), we have defined \( F^*(f) \) as \( F(f)^\oppos \), thus,
  \begin{equation*}
    F^*(f): F^*(B) \to F^*(A).
  \end{equation*}
  ...
  This demonstrates the naturality of \( \alpha^*: G^* \to F^* \), the family of morphisms defined by inverting the components of \( \alpha \).
\end{remark}

\begin{definition}\label{def:opposite_natural_transformation}\mcite[144]{ЦаленкоШульгейфер1974ОсновыТеорииКатегорий}
  We define the \term[ru=двойственное (естественное преобразование)]{opposite} of a \hyperref[def:natural_transformation]{natural transformation} \( \alpha: F \to G \) by taking the opposites of all its components, thus obtaining \( \alpha^*: G^* \to F^* \).
\end{definition}
\begin{comments}
  ...
  \end{comments}
\begin{defproof}
  We show in \cref{rem:opposite_natural_transformation} that the naturality diagram commutes for \( \alpha^* \).
\end{defproof}
...
\begin{proposition}\label{thm:opposite_natural_transformation}
  ...
  Then \( \alpha \) is a \hyperref[def:natural_transformation]{natural transformation} from \( F \) to \( G \) if and only if its \hyperref[def:opposite_natural_transformation]{opposite} \( \alpha^* \) is natural transformation from \( G^* \) to \( F^* \).
\end{proposition}
\begin{proof}
...
\begin{remark}\label{rem:opposite_functors_and_transformations}
  ...
  Similarly, the \hyperref[def:opposite_natural_transformation]{opposite natural transformation} \( \alpha^*: G^* \to F^* \) of \( \alpha: F \to G \) is different from the opposite morphism \( \alpha^\oppos: G \to F \) in the \hyperref[def:functor_category]{functor category} \( [\cat{C}, \cat{D}] \).
    ...
    \begin{rightcolumn}
      \begin{equation*}
        \begin{aligned}
          ...
          &\underbrace{H^f}_{\mathclap{f: A \to B}} \coloneqq \seq{ \underbrace{s}_{\mathclap{B \to C}} \mapsto \underbrace{s \bincirc f}_{\mathclap{A \to C}} }_{C \in \obj(\cat{C})}.
        \end{aligned}
      \end{equation*}
    \end{rightcolumn}
  \end{paracol}
...
\begin{comments}
  ...
  \item We show that \( H_{\Anon} \) and \( H^{\Anon} \) are \hyperref[def:functor_invertibility/injective]{injective on objects} (i.e. categorical embeddings) in \cref{thm:yoneda_embeddings_are_embeddings}, and show that they are \hyperref[def:functor_invertibility/full]{full functors} in \cref{thm:yoneda_embeddings_are_full}.
\end{comments}
\begin{defproof}
  We claim that \( H_f \) is a well-defined natural transformation for every morphism \( f \) in \( \cat{C} \), that \( H_{\Anon} \) is a well-defined functor and, furthermore, that \( Y \) an \hyperref[def:functor_invertibility/injective]{embedding} (i.e. is injective on objects).
...
\begin{proposition}\label{thm:yoneda_embeddings_are_embeddings}
  The \hyperref[def:yoneda_embeddings]{Yoneda embeddings} are \hyperref[def:functor_invertibility/injective]{categorical embedding} (i.e. is injective on objects).
\end{proposition}
\begin{proof}
    ...
    \begin{paracol}{2}
      ...
      \begin{rightcolumn}
        ...
        \begin{align*}
          ...
          &L'(A, F) \coloneqq F(A), \\
          &L'(\underbrace{f}_{A \to B}, \underbrace{\varphi}_{F \Rightarrow G}) \coloneqq \underbrace{\varphi_B \bincirc F(f^\oppos)}_{\T*{or} G(f^\oppos) \bincirc \varphi_A}
        \intertext{and}
          &R': \cat{C} \times [\cat{C}^\oppos, \cat{Set}] \to \cat{Set}, \\
          &R'(A, F) \coloneqq \hom(H_A, F), \\
          &R'(\underbrace{f}_{A \to B}, \underbrace{\varphi}_{F \Rightarrow G}) \coloneqq \parens[\big]{ \underbrace{\psi}_{\mathclap{H_A \Rightarrow F}} \mapsto \underbrace{\varphi \bincirc \psi \bincirc H_{f^\oppos}}_{H_B \Rightarrow G}}.
        \end{align*}
      \end{rightcolumn}
    \end{paracol}
...
\end{lemma}
\begin{comments}
  \item We obtain \( L' \) and \( R' \) by replacing \( \cat{C} \) with \( \cat{C}^\oppos \) in \( L \) and \( R \), and then taking the opposite of the first argument.
\end{comments}
\begin{proof}
