...
\begin{proposition}\label{thm:yoneda_embedding}
  ...
  We claim that \( Y(f) \) is a well-defined natural transformation, that \( Y \) is a well-defined functor and, furthermore, that \( Y \) an \hyperref[def:functor_invertibility/injective]{embedding} (i.e. injective on objects).
\end{proposition}
\begin{proof}
...
\begin{proposition}\label{thm:isomorphisms_via_yoneda_embedding}
  In a \hyperref[def:small_category]{locally small} \hyperref[def:category]{category} \( \cat{C} \), the objects \( A \) and \( B \) are \hyperref[def:morphism_invertibility/isomorphism]{isomorphic} if and only if the functors \( \hom(\anon*, A) \) and \( \hom(\anon*, B) \) are \hyperref[def:natural_isomorphism]{naturally isomorphic}.
\end{proposition}
\begin{proof}
  \SufficiencySubProof Fix an isomorphism \( f: A \to B \) in \( \cat{C} \). The \hyperref[def:yoneda_embedding]{Yoneda embedding} \( {Y: \cat{C} \to [\cat{C}^\oppos, \cat{Set}]} \) gives us natural transformations \( Y(f): \hom(\anon*, A) \Rightarrow \hom(\anon*, B) \), which is an isomorphism by \cref{thm:def:functor/two_sided_inverses}.
  ...
  Indeed, the naturality of \( \varphi^{-1} \) implies that the following diagram commutes:
  \begin{equation*}
    \begin{aligned}
...
\begin{proof}
  ...
  \begin{equation*}
    \coloneqq \seq{ \alpha_{(A, Y(B))} }_{(A, B) \in \obj(\cat{C})^2}
  \end{equation*}
