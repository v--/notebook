...
\begin{remark}\label{rem:metalogic}
  ...
  \Cref{rem:metalogic} schematically represents the relationships between these concepts.

  \begin{figure}[!ht]
    \begin{equation*}
      \includegraphics[page=1]{output/rem__metalogic}
    \end{equation*}
    \caption{The relationship between the concepts in \fullref{rem:metalogic}}\label{fig:rem:metalogic}
  \end{figure}
\end{remark}
\begin{comments}
...
\end{remark}
\begin{comments}
  \item Intuitionistic logic can also be called \enquote{constructive logic} due to the \hyperref[rem:brouwer_heyting_kolmogorov_interpretation]{Brouwer-Heyting-Kolmogorov interpretation}. See \fullref{rem:brouwer_heyting_kolmogorov_interpretation_compatibility} for further discussion of the topic.
\end{comments}
