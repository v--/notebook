...
Mathematical logic uses mathematics to study logic and vice versa. We try to give very general formal definitions in \fullref{sec:logical_consequence}. Nevertheless, in order to build a theory to study theories, if we are to use \hyperref[def:well_founded_relation]{well-founded} notions, we must accept some notions as fundamental without being able to properly define them. We will call such an informal definition a \enquote{concept}.

\begin{concept}\label{con:judgment}
  Our first fundamental notion is that of a \term{judgment}. \Textcite[9]{MartinLöf1996LogicalLaws} writes
  \begin{displayquote}
...
\begin{concept}\label{con:hypothetical_judgment}
  \textcite[30]{MartinLöf1984IntuitionisticTypeTheory} calls a \hyperref[con:judgment]{judgment} \term{hypothetical} if it is, in his own words, \enquote{made under assumptions}.
\end{concept}
\begin{comments}
...
\begin{concept}\label{con:proposition}
  ...
  In the artificial languages we will consider here, the sentences are expressed symbolically as what we call \term[en=formula (\cite[4]{Kleene2002Logic})]{formulas}. It is important to note that formulas with \hyperref[con:variable_binding]{free variables} may have ambiguous \hyperref[con:denotation]{denotations}, in which case they are not considered sentences; see \cref{rem:implicit_quantification_and_deduction}.
\end{concept}
\begin{comments}
  \item \Textcite[4]{Kleene2002Logic} introduces the above notions as follows:
  \begin{displayquote}
  ...
  \textcite[8]{Эдельман1975Логика} calls a \enquote{высказывание} (\enquote{proposition}) a statement which is certainly true or false.
...
\begin{comments}
  ...
  \item Within \hyperref[def:higher_order_logic]{(simply typed) higher-order logic}, the expressions correspond to \( \muplambda \)-terms. \hyperref[con:judgment]{Judgments} --- specific types of expressions --- correspond to formulas --- specific typed \( \muplambda \)-terms.

  In \hyperref[def:first_order_logic]{first-order logic}, terms are instead distinct constructs compared to formulas. Even so, in a first-order metalanguage in which we restrict the well-formed expressions to first-order terms, little is lost by requiring judgments and expressions to be distinct constructs.

  \item We list several sources in the literature that validate the above terminology:
  \begin{itemize}
    \item \Textcite[3]{Shoenfield1967MathematicalLogic} writes
    \begin{displayquote}
      Any finite sequence of symbols of a language is called an \textit{expression} of that language.
    \end{displayquote}

    \item \Textcite[217]{Kleene2002Logic} remarks
    \begin{displayquote}
    ...
    \item \Textcite[27]{CurryFeysCraig1958CombinatoryLogicVol1} claim that the phrase \enquote{well-formed} is introduced in \cite[352]{Church1932LambdaCalculus} and is \enquote{finding wide acceptance}.

    \item \Textcite[29]{Andrews2002Logic} describes \enquote{well-formed formulas} as \enquote{meaningful expressions}.

    \item On a more informal level, \textcite[266]{Tao2022AnalysisI} remarks
    \begin{displayquote}
    ...
    \item On a similar note, when introducing Scheme, \textcite[7]{AbelsonSussman2012eSICP} write
    \begin{displayquote}
...
\begin{remark}\label{rem:mathematical_logic_conventions}
  ...
  \begin{thmenum}
    ...
    \thmitem{rem:mathematical_logic_conventions/propositional_constants} We define the propositional constants for truth and falsity via \( \syntop \) and \( \synbot \) in the object language. Unlike the other improper symbols from logic, we cannot avoid the basic truth values in the metalanguage, so we will use the dedicated symbols \( \semtop \) and \( \sembot \).

    We want to highlight their importance, so we prefer dedicated notation compared to \( \top \) and \( \bot \) used for general \hyperref[def:lattice]{lattices} through \fullref{ch:order_theory}.

    \thmitem{rem:mathematical_logic_conventions/shorthands} We additionally use syntactic shorthands like \cref{rem:propositional_formula_notation_conventions} and \cref{rem:fol_formula_conventions} when writing formulas. We will also find handy various \hyperref[con:syntactic_abbreviation]{abbreviations} like the \hyperref[con:description_operator/unique_existence]{unique existence quantifier}.

    These are only notational shorthands in the metalanguage and do not affect the formulas themselves.

    \thmitem{rem:mathematical_logic_conventions/concepts} Some definitions are not completely formal --- beginning with fundamental concepts like the judgments discussed in \cref{con:judgment} and all the way to the Backus normal form discussed in \cref{con:backus_normal_form}. We name such definitions \enquote{concepts} to distinguish them from completely formal ones.

    \thmitem{rem:mathematical_logic_conventions/models_as_sets} Unless we are working with explicit \hyperref[con:proposition]{sentences}, we will generally reason about \hyperref[con:logical_model]{models} in the ambient metatheory and treat their universes, elements, relations and operations as sets.

    \thmitem{rem:mathematical_logic_conventions/structure_pairs} We often conflate structures in the metatheory (i.e. sets with functions and/or relations defined on them) with their universe --- see \cref{rem:fol_structure_notation} for a further discussion.

    \thmitem{rem:mathematical_logic_conventions/quantification} Instead of writing excessive \hyperref[con:predicate_logic/quantifiers]{quantifiers}, we sometimes rely on \hyperref[def:fol_quantifier_closure]{universal closures}. For example, half of the axioms of Peano arithmetic are presented without quantifiers in \cref{def:peano_arithmetic}, but are still assumed to have quantifiers.
  \end{thmenum}
\end{remark}
