...
\begin{example}\label{ex:def:metric_space}
  ...
  \begin{thmenum}
    ...
    \thmitem{ex:def:metric_space/norm} As shown in \cref{def:norm_induced_metric}, every \hyperref[def:norm]{(semi)norm} induces a (pseudo)metric.

    \thmitem{ex:def:metric_space/levenshtein} As shown in \cref{thm:levenshtein_distance_metric}, the \hyperref[def:levenshtein_distance]{Levenshtein distance} on any \hyperref[def:formal_language/language]{formal language} is a metric.

    \thmitem{ex:def:metric_space/graph_geodesic} As shown in \cref{thm:graph_geodesic_distance_is_metric}, \hyperref[def:graph_geodesic_distance]{geodesic distance} in a \hyperref[def:graph_connectedness/undirected]{connected} \hyperref[def:undirected_graph]{undirected graph} or \hyperref[def:hypergraph]{hypergraph} is a metric on the vertices.

    Similarly, as shown in \cref{thm:cayley_distance_is_metric}, the \hyperref[def:cayley_distance]{Cayley distance} in a \hyperref[def:cayley_graph]{Cayley graph} is a metric on the vertices.
  \end{thmenum}
\end{example}
...
\end{proposition}
\begin{proof}
  \SubProofOf{thm:def:metric_topology/ball_is_open} Obvious from \cref{def:metric_topology}.

  \SubProofOf{thm:def:metric_topology/first_countable} Since \cref{def:metric_topology} involves generating a topology using a neighborhood filter of countable local neighborhoods, \( \mscrT \) is first-countable.
...
\end{proposition}
\begin{proof}
  If \( f(x) = f(x') \), then by \cref{def:metric_space/M1}, \( x = x' \).
\end{proof}

\begin{definition}\label{def:category_of_metric_spaces}
  Metric spaces and monotone maps form a subcategory of \( \cat{Unif} \) (see \cref{def:category_of_uniform_spaces}). We denote this category by \( \cat{Met} \).
\end{definition}
...
\begin{remark}\label{rem:metric_space_convergence}
  ...
  It is conventional to prefer the bases \cref{def:metric_topology/real_base} and \cref{def:metric_uniformity/real_base} to the bases \cref{def:metric_topology/integer_base} and \cref{def:metric_topology/integer_base}.

  For example, given two metric spaces \( X \) and \( Y \), continuity of \( f: X \to Y \) at \( x_0 \in X \) (see \cref{def:local_continuity}) is usually written using the \enquote{epsilon-delta notation} as
  \begin{equation*}
