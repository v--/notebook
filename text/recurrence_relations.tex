...
\begin{definition}\label{def:recurrence_relation}\mimprovised
  ...
  \begin{thmenum}
    ...
    \thmitem{def:recurrence_relation/initial} We call the \( 0 \)-th state vector the \term{initial vector}. If such a vector is specified in addition to \eqref{eq:def:recurrence_relation}, we call the obtained \hyperref[def:system_of_equations]{system of equations} an \term{initial value problem}.
...
\begin{comments}
  ...
  \textcite[21]{Юмагулов2015ДинамическиеСистемы} calls recurrence relations \enquote{рекуррентные уравнения} (\enquote{recurrent equations}), while \textcite[def. 2.4.4]{Rosen2019DiscreteMathematics} defines a recurrence relation as an equation. The former term suggests that recurrent equations are variations rather than subtypes of equations, but the latter is flawed --- it would, in our case, suggest that a solution to \eqref{eq:def:recurrence_relation} should, in accordance with \cref{def:fol_equation/semantic_solution}, be an \( (n + 1) \)-tuple \( (x_n, x_{n+1}, \ldots, x_{n+k}) \). Not only do we want the equation satisfied by any contiguous subsequence of length \( (n + 1) \), but also we regard \( x_k \) as a value to be defined rather than something that has been given.

  For this reason, we highlight that a recurrence relation is defined by an equation, but is itself not an equation.

  \item We avoid referring to recurrent sequences as \enquote{solutions} to recurrences because \enquote{solving a recurrence} can also refer to \fullref{thm:linear_recurrence_unwinding}. \Textcite[def. 2.4.4]{Rosen2019DiscreteMathematics} calls recurrent sequences \enquote{solutions} even though section 8.2 is called \enquote{Solving Linear Recurrence Relations} and describes what we call unwinding.

  \item Recurrence relations generalize \fullref{thm:omega_recursion} to \( n > 1 \).

  \item Although we generally start indexing from \( 1 \), as discussed in \cref{rem:peano_arithmetic/zero}, here we index the indeterminates from \( 0 \) to \( k - 1 \) --- as we shall see, this turns out to be important for \hyperref[def:linear_recurrence_characteristic_polynomial]{characteristic polynomials}. This also allows us to keep compatibility with \textcite[395]{LidlNiederreiter1997FiniteFields}), \textcite[def. 2.4.4]{Rosen2019DiscreteMathematics} and \textcite[21]{Юмагулов2015ДинамическиеСистемы}.
...
\begin{remark}\label{rem:unrestricted_recurrence_relation}
  In \cref{def:recurrence_relation} we defined recurrence relations of \emph{finite} order \( k \) with equations of the form
  \begin{equation*}
    X_{N+k} = F(N, X_{N+k-1}, \ldots, X_N).
  \end{equation*}
...
\begin{example}\label{ex:def:recurrence_relation}
  ...
  \begin{thmenum}
    ...
    \thmitem{ex:def:recurrence_relation/triangular_numbers} Similarly, triangular numbers, which we have characterized algebraically in \cref{def:triangular_number/arithmetic} as
    \begin{equation*}
      \sum_{k=1}^n k
    \end{equation*}
...
\end{definition}
\begin{comments}
  \item We could use \enquote{unit sequence} instead of \enquote{impulse response sequences}, but this would be more ambiguous. As pointed out in \cite[402]{LidlNiederreiter1997FiniteFields}, whose term we use,
  \begin{displayquote}
...
\begin{definition}\label{def:linear_recurrence}\mcite[395]{LidlNiederreiter1997FiniteFields}
  We will mostly consider \hyperref[def:recurrence_relation/autonomous]{autonomous} \hyperref[def:recurrence_relation]{recurrence relation} given by a \hyperref[tab:def:polynomial_degree]{linear} \hyperref[def:polynomial_equation]{algebraic equation}. We will unsurprisingly call such recurrence relations \term{linear}.
...
\begin{comments}
  ...
  \item When describing such recurrences, \textcite[def. 8.2.1]{Rosen2019DiscreteMathematics} states that they have \enquote{constant coefficients}, emphasizing that it is possible for the coefficients \( a_{k-i} \) to depend on \( N \).

  Examples of \enquote{non-constant coefficients} can be found in the linear recurrences studied by \textcite{AkraBazzi1998Recurrences}, in which the order \( 0 \) coefficient \( a \) is allowed to vary with \( N \), i.e. to not be a constant. We briefly mention these recurrences in \cref{rem:unrestricted_recurrence_relation}.
...
\end{definition}
\begin{comments}
  \item We take the transpose of the matrix defined by \textcite[401]{LidlNiederreiter1997FiniteFields}.
...
\begin{comments}
  ...
  \item The generalization to abelian groups can also be found in \cite[595]{ButlerEtAl2016RainbowArithmeticProgressions}.
...
\begin{proposition}\label{thm:arithmetic_progression_unwinding}
  ...
  \begin{equation*}
    x_n = a_0 + nd,
  \end{equation*}
  where \( kd \) is \hyperref[def:additive_semigroup/iterated]{iterated semigroup addition}.
\end{proposition}
\begin{proof}
...
\begin{remark}\label{rem:fibonacci_sequence_initial_value}
  ...
  \begin{itemize}
    \item In Fibonacci's own convention, which we discuss in \cref{ex:fibonacci_rabbits}, we have \( F_0 = 1 \) and \( F_1 = 2 \).
    ...
    \item \Textcite[198]{Яблонский2003ДискретнаяМатематика} instead starts with \( F_0 = F_1 = 1 \).
  \end{itemize}
\end{remark}
...
\begin{comments}
  ...
  \item The sequence is presented starting with \( L_0 = 2 \) by \textcite[112]{ConwayGuy1998BookOfNumbers}.

  The more common practice is to avoid defining \( L_0 \); it is done by \textcite[exerc. III.2.9]{Aigner1997CombinatorialTheory}, \textcite[example 1.1.6]{Stanley2012EnumerativeCombinatoricsVol1}, \textcite[example 15]{Rosen2019DiscreteMathematics}, \textcite[273]{Deza2012FigurateNumbers} and \textcite[240]{Strang2023LinearAlgebra}.
\end{comments}
...
\end{theorem}
\begin{comments}
  \item The theorem is attributed to Binet by \textcite[273]{Deza2012FigurateNumbers} and \textcite[exerc. 7.18]{ГашковЧубариков2005СложностьВычисления}.
