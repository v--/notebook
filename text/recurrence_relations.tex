...
\begin{definition}\label{def:characteristic_polynomial_of_linear_recurrence}\mcite[404]{LidlNiederreiter1997FiniteFields}
  ...
  \begin{equation}\label{eq:def:characteristic_polynomial_of_linear_recurrence}
    f(X) = X^n - \sum_{k=1}^n a_k X^{k-1}.
  \end{equation}
\end{definition}

\begin{proposition}\label{thm:maximal_period_linear_recurrence}
  If the \hyperref[def:characteristic_polynomial_of_linear_recurrence]{characteristic polynomial} of a \hyperref[def:recurrence_relation/linear]{homogeneous linear} \hyperref[def:recurrence_relation]{recurrence relation} over \( \BbbF_q \) is a \hyperref[def:finite_field_primitive_polynomial]{primitive polynomial}, its \hyperref[def:recurrence_relation/solution]{solution} is \hyperref[def:periodic_function]{periodic} with base period \( q^n - 1 \), where \( n \) is the order of the recurrence.
\end{proposition}
\begin{proof}
  \todo{Prove}.
\end{proof}
