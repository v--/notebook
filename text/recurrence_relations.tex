...
\begin{definition}\label{def:autonomous_recurrence_relation}\mimprovised
  In analogy with autonomous systems of differential equations, we say that the recurrence is \term{autonomous} if it does not depend on \( N \) --- that is, if evaluating the function with different values of \( N \) gives identical results.
\end{definition}
\begin{comments}
  \item Autonomous recurrences can be regarded as \hyperref[def:dynamical_system]{dynamical systems} --- see \fullref{thm:autonomous_recurrence_as_dynamical_system}.
\end{comments}
...
\end{corollary}
\begin{comments}
  \item Thus, the matrix is simply an associated function in the sense of \fullref{def:autonomous_recurrence_associated_function}.
\end{comments}
\begin{proof}
  Follows from \fullref{thm:homogeneous_linear_recurrence_matrix_action} and \fullref{thm:autonomous_recurrence_as_dynamical_system}.
\end{proof}
...
\begin{comments}
  ...
  \item See \fullref{thm:geometric_progression_unwinding} and \fullref{thm:binets_fibonacci_number_formula} for examples of how this theorem can be used and \fullref{ex:arithmetic_progression_unwinding_as_recurrence} for how its generalizations can fail.
\end{comments}
\begin{proof}
...
\begin{proposition}\label{thm:linear_recurrence_finite_field_periodic}
  ...
  \begin{thmenum}
    \thmitem{thm:linear_recurrence_finite_field_periodic/general}\mcite[thm. 8.7]{LidlNiederreiter1997FiniteFields} Every sequence satisfying this recurrence is \hyperref[def:eventually_periodic_function]{eventually periodic} with fundamental period at most \( q^k \).
...
\begin{proof}
  ...
  \SubProofOf{thm:linear_recurrence_finite_field_periodic/homogeneous} In case the recurrence is homogeneous, either \( \seq{ x_n }_{n=0}^\infty \) has a zero state vector, which makes the sequence vacuously eventually periodic, or otherwise the zero vector does not occur, and we have at most \( \BbbF_q^k - 1 \) state vectors.
...
\end{proposition}
\begin{comments}
  \item \Fullref{thm:linear_recurrence_finite_field_periodic/homogeneous} implies that every recurrent sequence over a finite field is \hyperref[def:eventually_periodic_function]{eventually periodic}. We are only concerned with showing that the impulse response sequence has the largest possible period.
\end{comments}
\begin{proof}
  ...
  Let \( \seq{ d_n }_{n=0}^\infty \) be the impulse response sequence. \Fullref{thm:linear_recurrence_finite_field_periodic/homogeneous} implies that it is eventually periodic. Denote by \( t_0 \) and \( t \) the preperiod and fundamental period and by \( v_n \) its \( n \)-th \hyperref[def:recurrence_relation/state]{state vector}.
  ...
  It follows that \( t \) is a period of \( \seq{ x_n }_{n=0}^\infty \). \Fullref{thm:def:eventually_periodic_function/least} implies that its fundamental period divides \( t \).
\end{proof}
...
\begin{proof}
  ...
  Since the characteristic period is primitive, it is irreducible, and, by \fullref{thm:def:domain_divisibility/irreducible_polynomial_no_constant_term}, its free term \( a_0 \) is nonzero. \Fullref{thm:linear_recurrence_finite_field_periodic/periodic} then implies that all eventually periodic sequences are periodic.

  \SubProofOf{thm:linear_recurrences_with_maximal_period/impulse} \Fullref{thm:linear_recurrence_finite_field_periodic/homogeneous} implies that the impulse response sequence \( \seq{ d_n }_{n=0}^\infty \) is eventually periodic (and hence periodic) with period at most \( q^k - 1 \).

  Since the characteristic period is primitive, it does have at least one root by assumption, say \( \alpha \). \Fullref{thm:linear_recurrences_with_maximal_period/root} implies that the sequence \( \seq{ \alpha^k }_{k=0}^\infty \) satisfies the recurrence and is periodic with fundamental period \( q^k - 1 \).

  \Fullref{thm:impulse_response_sequence_period} then implies that \( q^k - 1 \) divides the fundamental period of \( \seq{ d_n }_{n=0}^\infty \). We have already shown in the beginning that its fundamental period cannot exceed \( q^k - 1 \); thus, it must coincide with \( q^k - 1 \).
\end{proof}
...
\begin{remark}\label{rem:fibonacci_sequence_initial_value}
  ...
  \begin{itemize}
    \item Fibonacci's own convention, discussed in \fullref{ex:fibonacci_rabbits}, where \( F_0 = 1 \) and \( F_1 = 2 \) is also used by \incite[exerc. 5.5]{Treil2017LinearAlgebraDoneWrong}.

    \item The modern convention from \fullref{def:fibonacci_numbers}, focusing on computation, is used by
    \incite[98]{Aigner1997CombinatorialTheory},
    \incite[467]{LidlNiederreiter1997FiniteFields},
    \incite[def. 2.4.5]{Rosen2019DiscreteMathematics},
    \incite[112]{ConwayGuy1998BookOfNumbers},
    \incite[122]{Mimram2020ProgramEqualsProof},
    \incite[98]{Erickson2019Algorithms},
    \incite[13]{Knuth1997ArtVol1},
    \incite[exerc. B-5.14(ii)]{Rotman2015AdvancedModernAlgebraPart1},
    \incite[59]{Мальцев1986Алгоритмы} and
    ...
    \incite[example 1.1.12]{Stanley2012EnumerativeCombinatoricsVol1}
    \incite[exerc. 7.15]{ГашковЧубариков2005СложностьВычисления})
...
\begin{comments}
  ...
  \item The sequence is presented starting with \( L_0 = 2 \) by \incite[112]{ConwayGuy1998BookOfNumbers}, and also in \cite[142]{RosenEtAl2018DiscreteMathematicsHandbook}.
...
\end{definition}
\begin{comments}
  \item It follows from \fullref{thm:quadratic_extension_conjugate} that \( \psi \) is the unique \hyperref[def:conjugate_algebraic_element]{conjugate algebraic element} of \( \varphi \) over \( \BbbQ \).
\end{comments}
