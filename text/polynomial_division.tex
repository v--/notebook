...
\begin{proposition}\label{thm:alg:euclidean_division_of_polynomials}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:alg:euclidean_division_of_polynomials/linear} Both functions \( \rem(\anon*, g) \) and \( \quot(\anon*, g) \) are \hyperref[def:semimodule/morphism]{linear}.
...
\begin{theorem}[Polynomial remainder theorem]\label{thm:polynomial_remainder_theorem}\mcite[corr. 2.1]{Jacobson1985BasicAlgebraI}
  Over a nontrivial commutative ring, for any univariate polynomial \( f(X) \) and any value \( \alpha \) we have
...
\begin{theorem}[Polynomial factor theorem]\label{thm:polynomial_factor_theorem}\mcite[corr. 2.2]{Jacobson1985BasicAlgebraI}
  Over a nontrivial commutative ring, \( \alpha \) is a \hyperref[def:polynomial_root]{root} of the univariate polynomial \( f(X) \) if and only if \( (X - \alpha) \) divides \( f(X) \).
\end{theorem}
\begin{comments}
...
\begin{proposition}\label{thm:adjoint_roots_and_quotients}
  Fix arbitrary nontrivial commutative rings \( R \subseteq S \) and some element \( u \) of \( S \). Consider the \hyperref[con:free_construction/evaluation]{evaluation homomorphism} \( \Phi_u: R[X] \to S \) sending \( X \) to \( u \). Additionally, suppose that the kernel of \( \Phi_u \) is a principal ideal with generator \( f(X) \).
