...
\begin{algorithm}[Euclidean division of polynomials]\label{alg:euclidean_division_of_polynomials}
  ...
  The algorithm will be used in \cref{thm:def:euclidean_domain/polynomials} to show that the polynomial algebra \( R[X] \) is a \hyperref[def:euclidean_domain]{Euclidean domain} when \( R \) is a field. Even more generally, we will reuse the Euclidean domain notation \( \quot(f, q) \) and \( \quot(f, g) \) for \( q(X) \) and \( r(X) \).
...
\begin{proof}
  ...
  If either \( \quot(f, g) - q \) or \( \rem(f, g) - r \) is nonzero, both are nonzero. Indeed, if \( \quot(f, g) - q \) is nonzero, \cref{thm:univariate_polynomial_product/invertible_leading} implies that \( \rem(f, g) - r \) is. Conversely, if \( \rem(f, g) - r \) is nonzero, \cref{thm:univariate_polynomial_product/degree} implies that \( \quot(f, g) - q \) is nonzero.

  In such case, \cref{thm:univariate_polynomial_product/degree} implies that
  \begin{equation*}
  ...
  \begin{equation*}
    f = \quot(f, g) \cdot g + f,
  \end{equation*}
  hence \( \quot(f, g) \cdot g \) is zero. Because \( g \) is nonzero, \cref{thm:univariate_polynomial_product/invertible_leading} implies that \( \quot(f, g) \) must be zero.
...
\begin{remark}\label{rem:alg:horners_rule}
  ...
  \incite{Horner1819SolvingNumericalEquations} describes a method for solving equations. Variations of method are described by \incite[144]{Курош1968КурсВысшейАлгебры}, \incite[97]{Винберг2014КурсАлгебры} and \incite[209]{Кострикин2000АлгебраЧасть1} as \enquote{схема Горнера} (\enquote{Horner's scheme}) and \incite[\S III.V.2]{Обрешков1962ВисшаАлгебра} as \enquote{правилото на Хорнер} (\enquote{Horner's rule}). We roughly follow their discussion in \fullref{alg:horners_rule}.
...
\begin{remark}\label{rem:root_terminology}
  ...
  \begin{itemize}
    ...
    \item The \hyperref[def:equation_solution]{solutions} of an \hyperref[def:algebraic_equation]{algebraic equation} are also called \enquote{roots}. An explicit definition with this usage can be found in \incite[2]{Обрешков1962ВисшаАлгебра}.
