\section{Polynomial division}\label{sec:polynomial_division}
...
\begin{algorithm}[Euclidean division of polynomials]\label{alg:euclidean_division_of_polynomials}
  In a nonzero commutative ring \( R \), fix two univariate polynomials
  \begin{align*}
    f(X) = \sum_{k=0}^n a_k X^k, &&
    g(X) = \sum_{k=0}^m b_k X^k.
  \end{align*}

  Suppose that the leading coefficient \( b_m \) of \( g(X) \) is invertible. We will build polynomials \( q(X) \) and \( r(X) \), where \( \deg r < \deg g = m \), such that
  \begin{equation*}
  ...
  The algorithm will be used in \cref{thm:def:euclidean_domain/polynomials} to show that the polynomial algebra \( R[X] \) is a \hyperref[def:euclidean_domain]{Euclidean domain} when \( R \) is a field. Even more generally, we will reuse the Euclidean domain notation \( \quot(f, q) \) and \( \rem(f, g) \) for \( q(X) \) and \( r(X) \).

  \begin{thmenum}
    \thmitem{alg:euclidean_division_of_polynomials/no_division} If \( \deg f < \deg g \), let
    \begin{align*}
      q(X) \coloneqq 0_R, &&
      r(X) \coloneqq f(X).
    \end{align*}

    \thmitem{alg:euclidean_division_of_polynomials/division} Otherwise, \( \deg f \geq \deg g \). In this case \( f(X) \) is nonzero. Let
    \begin{equation*}
      \widehat r(X) \coloneqq f(X) - (a_n / b_m) X^{n-m} \cdot g(X).
    \end{equation*}

    We use the algorithm recursively to divide \( \widehat r(X) \) by \( g(X) \), and obtain \( \widehat q(X) \) and \( r(X) \), where \( r(X) \) is of lower degree than \( g \), such that
    \begin{equation*}
...
\begin{comments}
  ...
  \begin{CodeRefDisplay}
    \GetCodeRef{alg:euclidean_division_of_polynomials}
  \end{CodeRefDisplay}
\end{comments}
\begin{defproof}
  Via induction on \( n \), we will show that the algorithm terminates.

  \begin{itemize}
    \item If \( n = 0 \), either \( m > 0 \) and the algorithm terminates at \fullref{alg:euclidean_division_of_polynomials/no_division}, or \( m = 0 \) and we use the recursive step \fullref{alg:euclidean_division_of_polynomials/division}.

    In the latter case, \( \widehat r(X) \) evaluates to \( 0_R \), so the recursive application uses \fullref{alg:euclidean_division_of_polynomials/no_division} and then the algorithm terminates.

    \item Suppose that \( n > 0 \) and that the inductive hypothesis holds for \( n - 1 \).

    Again, we use either \fullref{alg:euclidean_division_of_polynomials/no_division} or the recursive step \fullref{alg:euclidean_division_of_polynomials/division}.

    In the latter case, to use the inductive hypothesis, we only need to show that \( \widehat r(X) \) has degree lower than \( n \). Let
    \begin{align*}
    ...
    \begin{equation*}
      \widehat r(X)
      =
      f(X) - (a_n / b_m) X^{n-m} \cdot g(X)
      =
      \widehat f(X) - (a_n / b_m) X^{n-m} \cdot \widehat g(X).
    \end{equation*}

    \Cref{thm:univariate_polynomial_sum} implies that
    \begin{equation*}
      \deg\parens[\big]{ (a_n / b_m) X^{n-m} \cdot \widehat g(X) }
      =
      \deg X^{n - m} + \deg \widehat g
      <
      (n - m) + m
      =
      n,
    ...
    hence
    \begin{equation*}
      \deg \widehat r = \max\set{ \deg \widehat f, \deg \widehat g } < n.
    \end{equation*}

    Then the inductive hypothesis is applicable to \( \deg \widehat r \).
  \end{itemize}
\end{defproof}

\begin{proposition}\label{thm:alg:euclidean_division_of_polynomials}
  In a nonzero commutative ring \( R \), fix a polynomial \( g(X) \) of degree \( n \) whose leading coefficient is invertible. \Fullref{alg:euclidean_division_of_polynomials} gives us maps \( \quot(\anon*, g) \) and \( \rem(\anon*, g) \) on \( R[X] \).

  These quotients and remainders have the following basic properties:
  \begin{thmenum}
    \thmitem{thm:alg:euclidean_division_of_polynomials/unique} They are unique in the following sense: if \( f = q \cdot g + r \), where \( \deg r < \deg g \), then \( r = \rem(f, g) \) and \( q = \quot(f, g) \).
    ...
    \thmitem{thm:alg:euclidean_division_of_polynomials/quot_zeros} The kernel of \( \quot(\anon*, g) \) and the f\hyperref[def:fol_fixed_point_substructure]{fixed point subspace} of \( \rem(\anon*, g) \) coincide.

    We will show in \cref{thm:truncated_polynomial_algebra/quot_zeros} that both sets coincide with the \hyperref[def:truncated_polynomial_algebra]{truncated polynomial module} \( \Pi_{n-1}(R[X]) \).
  \end{thmenum}
\end{proposition}
\begin{proof}
  ...
  \Cref{thm:univariate_polynomial_product/degree} implies that
  \begin{equation*}
    \deg(\rem(f, g) - r) = \deg(\quot(f, g) - q) + \deg g.
  \end{equation*}

  \Cref{thm:univariate_polynomial_sum/degree} implies that
  \begin{equation*}
    \deg(\rem(f, g) - r) \leq \underbrace{\max\set{ \deg \rem(f, g), \deg r }}_{< \deg g},
  \end{equation*}
  so
  \begin{equation*}
    \deg(\quot(f, g) - q) + \deg g < \deg g.
  \end{equation*}

  This is only possible if the left side is \( -\infty \), which happens precisely when \( \quot(f, g) - q \) is the zero polynomial. Hence, \( \rem(f, g) - r \) is also zero.
  ...
  The sum \( \rem(f, g) + \rem(p, g) \) has degree less than \( n \), so the uniqueness shown in \cref{thm:alg:euclidean_division_of_polynomials/unique} implies that \( \rem(f + p, g) = \rem(f, g) + \rem(p, g) \).
  ...
  \SufficiencySubProof* If \( f \) is a fixed point of \( \rem(\anon*, g) \), then
  \begin{equation*}
    f = \quot(f, g) \cdot g + f,
  \end{equation*}
  hence \( \quot(f, g) \cdot g \) is zero. Because \( g \) is invertible, \( \quot(f, g) \) must be zero since otherwise we would contradict \cref{thm:univariate_polynomial_product/invertible_leading}.

  \NecessitySubProof* If \( f \) is in the kernel of \( \quot(\anon*, g) \), then \( \quot(f, g) \) is zero, and obviously \( f = \rem(f, g) \).
\end{proof}

\begin{theorem}[Polynomial remainder theorem]\label{thm:polynomial_remainder_theorem}\mcite[corr. 2.1]{Jacobson1985BasicAlgebraI}
  Over a nonzero commutative ring, for any univariate polynomial \( f(X) \) and any value \( \alpha \) we have
  \begin{equation}\label{eq:thm:polynomial_remainder_theorem}
    f(X) = (X - \alpha) \cdot \quot(f, X - \alpha) + f(\alpha).
  \end{equation}
\end{theorem}
\begin{proof}
  The uniqueness of remainders from \cref{thm:alg:euclidean_division_of_polynomials/unique} implies that \( \rem(f, X - \alpha) = f(\alpha) \).
\end{proof}

\begin{theorem}[Polynomial factor theorem]\label{thm:polynomial_factor_theorem}\mcite[corr. 2.2]{Jacobson1985BasicAlgebraI}
  Over a nonzero commutative ring, \( \alpha \) is a \hyperref[def:polynomial_root]{root} of the univariate polynomial \( f(X) \) if and only if \( (X - \alpha) \) divides \( f(X) \).
\end{theorem}
\begin{comments}
  \item This is a special case of the equivalence of definitions of multiple roots in \cref{def:polynomial_root_multiplicity}.
...
\end{comments}
\begin{proof}
  Follows directly from \cref{thm:polynomial_remainder_theorem}.
\end{proof}
...
\begin{algorithm}[Horner's rule]\label{alg:horners_rule}\mcite[\S III.V.2]{Обрешков1962ВисшаАлгебра}
  ...
  The coefficients of the quotient of \( f \) and \( g \) with respect to \fullref{alg:euclidean_division_of_polynomials} can be computed recursively as follows:
  ...
  Furthermore, \( \rem(f, g) \) is a constant polynomial that equals
...
\begin{comments}
  \item This algorithm is implemented in \cite{notebook:code} as
  \begin{CodeRefDisplay}
    \GetCodeRef{alg:horners_rule}
  \end{CodeRefDisplay}
\end{comments}
\begin{proof}
  Since \( \deg \rem(f, g) < \deg g = 1 \), the remainder is obviously constant.

  Let \( c_k \) be the \( k \)-th coefficient of \( \quot(f, g) \). For every \( k > 0 \), we have
  \begin{equation*}
...
\begin{remark}\label{rem:alg:horners_rule}
  ...
  \textcite{Horner1819SolvingNumericalEquations} describes a method for solving \hyperref[def:polynomial_equation]{algebraic equations}. Variations of his method are described in \cite[144]{Курош1968КурсВысшейАлгебры}, \cite[97]{Винберг2014КурсАлгебры} and \cite[208]{Кострикин2020АлгебраЧасть1} as \enquote{\textru{схема Горнера}} (\enquote{Horner's scheme}) and in \cite[\S III.V.2]{Обрешков1962ВисшаАлгебра} as \enquote{\textbg{правилото на Хорнер}} (\enquote{Horner's rule}). We roughly follow their discussion in \fullref{alg:horners_rule}.

  On the other hand, \textcite[486]{Knuth1997ArtVol2} calls \enquote{Horner's rule} the representation
  \begin{equation}\label{eq:rem:alg:horners_rule/knuth}
    f(X) = \sum_{k=0}^n a_k X^k = a_0 + X (a_1 + \cdots + X(a_{n-1} + X a_n) + \cdots),
  \end{equation}
  considered from the point of view of computational complexity --- at allows computing \( f(x) \) via \( n \) multiplications and \( n \) additions, while direct evaluation requires  \( {n(n+1)} / 2 \) multiplications and \( n \) additions.

  \textcite[exerc. 3.3.14]{Rosen2019DiscreteMathematics} calls this procedure \enquote{Horner's method}.
\end{remark}

\paragraph{Multiple roots}

\begin{definition}\label{def:algebraic_derivative}\mcite[434]{Aluffi2009Algebra}
  Generalizing \cref{def:differentiability} from analysis, we define the \term[ru=производная (\cite[163]{Тыртышников2017ОсновыАлгебры}), en=derivative (\cite[434]{Aluffi2009Algebra})]{algebraic derivative} of a univariate polynomial
  \begin{equation*}
  ...
  Via \fullref{thm:omega_recursion}, we can extend algebraic derivatives to order \( m \) as
  \begin{equation*}
...
\begin{proof}
  ...
  \SubProofOf{thm:def:algebraic_derivative/product} Due to the linearity shown in \cref{thm:def:algebraic_derivative/linear}, it is enough to consider the case where both \( f \) and \( g \) are monomials.
...
\begin{defproof}
  ...
  Let \( \Phi_\alpha: R[X] \to R \) be the \hyperref[con:free_construction/evaluation]{evaluation homomorphism} at \( \alpha \). Then
  \begin{equation*}
  ...
  We have
  \begin{equation*}
    g(X) = (X - \alpha) \cdot \quot(g, X - \alpha) + \rem(g, X - \alpha).
  \end{equation*}

  Since \( \Phi_\alpha(g) = 0_R \), the remainder is necessarily zero. Therefore,
  \begin{equation*}
    f(X) = (X - \alpha)^{m-1} \cdot g(X) = (X - \alpha)^m \cdot \quot(g, X - \alpha).
  \end{equation*}
\end{defproof}
...
\begin{definition}\label{def:truncated_polynomial_algebra}\mimprovised
  Corresponding to the \hyperref[def:polynomial_algebra]{polynomial algebra} \( R[X] \) (over an arbitrary commutative semiring), we define the \term{truncated polynomial module} of degree \( d \) as the \( R \)-submodule of polynomials of degree at most \( d \), i.e. the linear span
  \begin{equation*}
    \Pi_d(R[X]) \coloneqq \lin\set{ 1, X, X^2, \ldots, X^d }.
  \end{equation*}

  In a nonzero commutative ring, by the argumentation in \cref{ex:def:hamel_basis/polynomial_algebra}, the monomials \( 1, X, \ldots, X^d \) form a \hyperref[def:hamel_basis]{Hamel basis} of \( \Pi_d(R[X]) \). Furthermore, due to the isomorphism in \cref{thm:truncated_polynomial_algebra/isomorphism}, we identify \( \Pi_d(R[X]) \) with the \hyperref[def:algebra_over_ring/quotient]{quotient algebra} \( R[X] / \braket{ X^{d+1} } \).
\end{definition}

\begin{proposition}\label{thm:truncated_polynomial_algebra}
  In a nonzero commutative ring \( R \), fix a polynomial \( g(X) \) of degree \( n \) whose leading coefficient is invertible.

  We will now develop a correspondence between the \hyperref[def:algebra_over_ring/quotient]{quotient algebra} \( R[X] / \braket{ g(X) } \) and the \hyperref[def:truncated_polynomial_algebra]{truncated polynomial algebra} \( \Pi_{n-1}(R[X]) \).

  \begin{thmenum}
    \thmitem{thm:truncated_polynomial_algebra/quot_zeros} The kernel of \( \quot(\anon*, g) \) and the \hyperref[def:fol_fixed_point_substructure]{fixed point subspace} of \( \rem(\anon*, g) \) both coincide with \( \Pi_{n-1}(R[X]) \).
    ...
    \end{equation*}
    the truncated \( R \)-module \( \Pi_{n-1}(R[X]) \) becomes a commutative unital associative \( R \)-algebra.

    \thmitem{thm:truncated_polynomial_algebra/isomorphism} The map
    \begin{equation*}
      \varphi([f]) \coloneqq \rem(f, g)
    \end{equation*}
    is a well-defined unital \( R \)-algebra isomorphism between \( R[X] / \braket{ g(X) } \) and \( \Pi_{n-1}(R[X]) \).
...
\end{proposition}
\begin{proof}
  \SubProofOf{thm:truncated_polynomial_algebra/quot_zeros} \Cref{thm:alg:euclidean_division_of_polynomials/quot_zeros} implies that the zeros of \( \quot(\anon*, g) \) coincide with the fixed points of \( \rem(\anon*, g) \). We will now show that \( \rem(r, g) = r \) if and only if \( r \) is in \( \Pi_{n-1}(R[X]) \).

  \SufficiencySubProof* Suppose that \( \rem(r, g) = r \). Then \( \deg r < n \). In both cases, \( r \) is in \( \Pi_{n-1}(R[X]) \).

  \NecessitySubProof* If \( r \in \Pi_{n-1}(R[X]) \), then division must use \fullref{alg:euclidean_division_of_polynomials/no_division}, which defines \( \rem(r, g) \) as \( r \).
  ...
  \SubProofOf[def:function_invertibility/surjective/existence]{surjectivity} For a polynomial \( r \) in \( \Pi_{n-1}(R[X]) \).,\cref{thm:truncated_polynomial_algebra/quot_zeros} implies that \( \rem(r, g) = r \), so \( [r] \) is the desired preimage.

  \SubProofOf[def:semimodule/morphism]{linearity} Follows from \cref{thm:alg:euclidean_division_of_polynomials/linear}.

  \SubProof{Proof that the supposed isomorphism \hyperref[def:algebra_over_ring/homomorphism]{preserves multiplication}} We have
  \begin{equation*}
...
\begin{proposition}\label{thm:adjoint_roots_and_quotients}
  Fix arbitrary nonzero commutative rings \( R \subseteq S \) and some element \( u \) of \( S \). Consider the \hyperref[con:free_construction/evaluation]{evaluation homomorphism} \( \Phi_u: R[X] \to S \) sending \( X \) to \( u \). Additionally, suppose that the kernel of \( \Phi_u \) is a principal ideal with generator \( f(X) \).
...
\begin{comments}
  ...
  When \( R \) is a \hyperref[def:field]{field}, by \cref{thm:def:principal_ideal_domain/field_polynomials}, \( R[X] \) is a \hyperref[def:principal_ideal_domain]{principal ideal domain}, and thus the kernel of \( \Phi_u \) is always principal. If \( u \) is an \hyperref[def:algebraic_element]{algebraic element}, i.e. if \( \ker \Phi_u \) is nonzero and thus has a monic generator, this generator is called the \hyperref[def:algebraic_element_minimal_polynomial]{minimal polynomial} of \( u \).
...
\end{comments}
\begin{proof}
  \Cref{thm:homomorphism_theorem_for_rings} implies that
  \begin{equation*}
...
\begin{example}\label{ex:thm:adjoint_roots_and_quotients}
  ...
  \begin{thmenum}
    ...
    \begin{thmenum}
      ...
      Conversely, consider the \hyperref[con:free_construction/evaluation]{evaluation homomorphism} \( \Phi_i: \BbbZ[X] \to \BbbC \) for the imaginary unit. Let \( f(X) \in \BbbZ[X] \). Then
      \begin{equation*}
        f(i)
        =
        \Phi_i(f)
