...
\begin{defproof}
  ...
  \begin{itemize}
    ...
    In the latter case, \( \widehat r(X) \) evaluates to \( 0_R \), so the recursive application uses \cref{alg:euclidean_division_of_polynomials/no_division} and then the algorithm terminates.
...
\begin{proof}
  ...
  \NecessitySubProof* If \( r \in \Pi_{n-1}(R[X]) \), then division must use \cref{alg:euclidean_division_of_polynomials/no_division}, which defines \( \rem(r, g) \) as \( r \).
