...
\begin{definition}\label{def:parse_tree}\mimprovised
  ...
  \begin{thmenum}
    ...
    We handle the special case \( l(x) \to \varepsilon \) by introducing a single child for \( x \) with a label \( \varepsilon \). This is done in order to distinguish \( \varepsilon \) rules from \enquote{dangling} nonterminals.
  \end{thmenum}
\end{definition}
\begin{comments}
...
\begin{algorithm}[Leftmost derivation from parse tree]\label{alg:leftmost_derivation_from_parse_tree}
  ...
  \begin{thmenum}
    ...
    \begin{itemize}
      ...
      \item Otherwise, let \( j \) be the smallest index such that \( l(k_j) \) is a nonterminal, and let \( k'_1 < \cdots < k'_{n'_j} \) be the children of \( k_j \). Then define
      \begin{equation*}
...
\begin{algorithm}[Brute force parsing]\label{alg:brute_force_parsing}
  ...
  Suppose that we are given the rule \( A \to v_1 \cdots v_m \), where each \( v_k \) is a single symbol (either a terminal or nonterminal). The algorithm relies on partitioning \( w \) into \( m \) parts - one for each symbol in \( v \) - by which we mean
  \begin{equation*}
...
\begin{example}\label{ex:evaluation_via_trees}
  ...
  Let \( v \) be a symbol, either a terminal or nonterminal, and let \( T \) be the tree
  \begin{equation*}
    \begin{aligned}
  ...
  \begin{equation*}
    F(T) \coloneqq \begin{cases}
      ...
      v,                    &v \T{is a terminal}, \\
      F(T_1) \cdots F(T_n), &v \T{is a nonterminal}
    \end{cases}
  \end{equation*}
