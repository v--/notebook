...
\end{proposition}
\begin{proof}
  Let \( w \) be a word in the Dyck language. We will use \cref{thm:induction_on_rooted_trees} on the \hyperref[def:parse_tree]{parse tree} of \( w \).
...
\begin{proof}
  ...
  \SubProof{Proof that \( V \subseteq W \)} Let \( v \) be a string in \( V \). We will use \cref{thm:induction_on_rooted_trees} to show that \( v \) belongs to \( W \).
