...
\begin{definition}\label{def:parse_tree_word}\mcite[46]{Aho2006}
  Let \( T = (X, A, r, \leq, l) \) be a \hyperref[def:parse_tree]{parse tree}. Let \( x_1, \ldots, x_n \) be the nodes in the \hyperref[def:traversal_ordering]{pre-order traversal} of \( T \) whose labels are terminals. We say that \( T \) \term{yields} the word
  \begin{equation*}
    l(x_1) \cdots l(x_n).
  \end{equation*}
...
\begin{example}\label{ex:def:parse_tree}
  ...
  \begin{thmenum}
    ...
    We will discuss abstract syntax trees in \fullref{rem:abstract_syntax_tree}, which are notably tidier.
...
\begin{algorithm}[Parse tree from derivation]\label{alg:parse_tree_from_derivation}
  ...
  \begin{thmenum}
    ...
    \thmitem{alg:parse_tree_from_derivation/step} For every \( i > 0 \), suppose that we have already built \( T_{i-1} \) for \( w_{i-1} \). The tree \( T_{i-1} \) has the form
    \begin{equation*}
      \begin{aligned}
  ...
  \thmitem{alg:parse_tree_from_derivation/finish} The tree \( T_m \) is the desired parse tree for \( w \).
\end{algorithm}
\begin{comments}
...
\begin{algorithm}[Leftmost derivation from parse tree]\label{alg:leftmost_derivation_from_parse_tree}
  ...
  \begin{thmenum}
    ...
    \begin{itemize}
      ...
      \begin{equation*}
        u_i \coloneqq r_1 \cdots r_{j-1} \overline{ k'_1 \cdots k'_{n'_j} } r_{j+1} \cdots r_{n_j}.
      \end{equation*}

      Define also
      \begin{equation*}
        w_i \coloneqq l(r_1) \cdots l(r_{j-1}) \overline{ l(k'_1) \cdots l(k'_{n'_j}) } l(r_{j+1}) \cdots l(r_{n_j}).
      \end{equation*}
...
\end{proposition}
\begin{proof}
  \Fullref{alg:parse_tree_from_derivation} allows us to construct a parse tree from any derivation. Multiple derivations can lead to the same parse tree, but our choice of nonterminal in \fullref{alg:parse_tree_from_derivation/step} ensures that a leftmost derivation leads to a unique parse tree.
...
\begin{comments}
  ...
  \item We allow different starting nonterminals because, if \( F \) calls itself recursively, it must be able to process words generated in different ways.
\end{comments}

\begin{example}\label{ex:natural_number_arithmetic_grammar/evaluation}
  We will define \hyperref[rem:evaluation]{evaluation} for the binary natural number grammar \eqref{eq:ex:natural_number_arithmetic_grammar/schema/shorthand} from \fullref{ex:natural_number_arithmetic_grammar/schema}. Let
  \begin{equation*}
...
\begin{remark}\label{rem:abstract_syntax_tree}
  ...
  \begin{thmenum}
    ...
    Any words produced via the lexical rules are called \term[ru=лексемы (\cite[329]{Гладкий1973})]{lexemes}. The goal is to isolate atoms of a language from arbitrarily complicated recursive structures. This distinction is important for evaluation and becomes apparent in more complicated use cases like first-order formula substitution in \fullref{def:first_order_substitution}.
