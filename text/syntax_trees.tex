...
\begin{example}\label{ex:def:parse_tree}
  ...
  \begin{thmenum}
    ...
    The first tree corresponds to \( (\syn1\syn0 \syntimes (\syn1 \synplus \syn1\syn0)) \), which evaluates to \( 6 \), while the second one corresponds to \( ((\syn1 \syn0 \syntimes \syn1) \synplus \syn1\syn0) \), which evaluates to \( 4 \).\fnote{\( \syn1\syn0 \) in binary notation is \( 2 \) is decimal notation, \( \syn1\syn0\syn0 \) in binary is \( 4 \) in decimal and \( \syn1\syn1\syn0 \) in binary is \( 6 \) in decimal.}
...
\begin{comments}
  ...
  \item This algorithm is implemented in \cite{notebook:code} as
    \WriteCodeRef{alg:brute_force_parsing}
\end{comments}
...
\begin{comments}
  \item This algorithm is implemented in \cite{notebook:code} as
    \WriteCodeRef{alg:derivation_to_parse_tree}
\end{comments}
...
\begin{comments}
  \item This algorithm is implemented in \cite{notebook:code} as
    \WriteCodeRef{alg:parse_tree_to_leftmost_derivation}
\end{comments}
