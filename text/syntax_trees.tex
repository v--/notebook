...
\begin{definition}\label{def:parse_tree}\mcite[\S 2.2.3]{AhoEtAl2006Compilers}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:parse_tree/leaves} If a leaf is labeled by a nonterminal \( N \), there exists a rule \( N \to \bnfves \).
...
\begin{example}\label{ex:def:parse_tree}
  ...
  \begin{thmenum}
    ...
    \thmitem{ex:def:parse_tree/arithmetic} We continue the binary natural number grammar example from \cref{ex:natural_number_arithmetic_grammar/schema} with the grammar \eqref{eq:ex:natural_number_arithmetic_grammar/schema/shorthand}.

    The only possible parse tree for the expression \( (\syn1 \syn0 \synmult (\syn1 \synplus \syn1 \syn0)) \) is
    \begin{equation*}
    ...
    The corresponding expression \( \syn1 \syn0 \synplus \syn1 \synmult \syn1 \syn0 \) has more than one parse tree:
    \begin{equation*}
      \begin{aligned}
    ...
    The first tree corresponds to \( (\syn1\syn0 \synmult (\syn1 \synplus \syn1\syn0)) \), which evaluates to \( 6 \), while the second one corresponds to \( ((\syn1 \syn0 \synmult \syn1) \synplus \syn1\syn0) \), which evaluates to \( 4 \).\fnote{\( \syn1\syn0 \) in binary notation is \( 2 \) is decimal notation, \( \syn1\syn0\syn0 \) in binary is \( 4 \) in decimal and \( \syn1\syn1\syn0 \) in binary is \( 6 \) in decimal.}
...
\begin{comments}
  ...
  \begin{CodeRefDisplay}
    \GetCodeRef{alg:brute_force_parsing}
  \end{CodeRefDisplay}
\end{comments}
...
\begin{comments}
  \item This algorithm is implemented in \cite{notebook:code} as
  \begin{CodeRefDisplay}
    \GetCodeRef{alg:derivation_to_parse_tree}
  \end{CodeRefDisplay}
\end{comments}
...
\begin{algorithm}[Parse tree to leftmost derivation]\label{alg:parse_tree_to_leftmost_derivation}
  ...
  \begin{thmenum}
    ...
    Let \( s_1, \ldots, s_p \) be the labels of the children of \( v_k \). \Cref{def:parse_tree/children} ensures that there exists a production rule
    \begin{equation*}
...
\begin{comments}
  \item This algorithm is implemented in \cite{notebook:code} as
  \begin{CodeRefDisplay}
    \GetCodeRef{alg:parse_tree_to_leftmost_derivation}
  \end{CodeRefDisplay}
\end{comments}
...
\end{definition}
\begin{comments}
  \item \Cref{thm:derivations_and_parse_trees} ensures that every generated string has at least one parse tree.
  \item \Cref{thm:leftmost_derivation_existence} ensures that every generated string has at least one leftmost derivation.
\end{comments}
\begin{proof}
  \EquivalenceSubProof{def:grammar_ambiguity/tree}{def:grammar_ambiguity/leftmost} Follows from \cref{thm:derivations_and_parse_trees}.
...
\begin{comments}
  ...
  \item In a more abstract setting, evaluation is performed via evaluation morphisms --- see \cref{con:free_construction/evaluation}.
\end{comments}
...
\begin{concept}\label{con:abstract_syntax_tree}
  ...
  \begin{thmenum}
    ...
    Any strings produced via the lexical rules are called \term[ru=лексемы (\cite[329]{Гладкий1973ГрамматикиИЯзыки})]{lexemes}. The goal is to isolate atoms of a language from arbitrarily complicated recursive structures. This distinction is important for evaluation and becomes apparent in more complicated use cases like first-order formula substitution in \cref{def:atomic_fol_substitution}.
...
\end{concept}
\begin{comments}
  \item \Fullref{thm:recursively_defined_abstract_syntax} provides an alternative approach --- defining abstract syntax trees without an underlying formal language.
\end{comments}
...
\end{definition}
\begin{comments}
  \item The term \enquote{paired delimiter} is based on \texttt{DeclarePairedDelimiter} command from the \texttt{mathtools} \LaTeX{} package. The command is used in this monograph to define convenience macros for different kinds of parentheses and brackets.

  \item The condition for balanced delimiters is based on the discussion of \hyperref[def:dyck_language]{Dyck languages} by \textcite[example 6.6.6]{Stanley2023EnumerativeCombinatoricsVol2}.
...
\begin{example}\label{ex:def:paired_delimiters}
  ...
  \begin{thmenum}
    ...
    \thmitem{ex:def:paired_delimiters/lambda} \hyperref[def:lambda_term]{\( \muplambda \)-terms} have balanced parentheses.
...
\begin{proposition}\label{thm:paired_delimiters_not_regular}
  If a formal language with \emph{distinct} \hyperref[def:paired_delimiters]{balanced delimiters} has, for every positive integer \( p \), a word whose first \( p \) symbols are left delimiters, it is not a \hyperref[def:chomsky_hierarchy/regular]{regular language}.
\end{proposition}
\begin{proof}
...
\end{proposition}
\begin{proof}
  Let \( w \) be a word in the Dyck language. We will use \fullref{thm:induction_on_abstract_syntax} on the \hyperref[def:parse_tree]{parse tree} of \( w \).
...
\begin{proof}
  ...
  \SubProof{Proof that \( V \subseteq W \)} Let \( v \) be a string in \( V \). We will use \fullref{thm:induction_on_abstract_syntax} to show that \( v \) belongs to \( W \).
...
\begin{corollary}\label{thm:dyck_words_and_binary_trees}
  \begin{figure}[!ht]
    ...
    \begin{subcaptionblock}{0.19\textwidth}
      ...
      \caption{\texttt{()()()}.}
    \end{subcaptionblock}
    \caption{A visualization of \cref{thm:dyck_words_and_binary_trees}: \hyperref[def:nary_tree]{full binary trees} and their corresponding \hyperref[def:dyck_language]{Dyck words}.}\label{fig:thm:dyck_words_and_binary_trees}
  \end{figure}

  \hyperref[def:nary_tree]{Full binary trees} and \hyperref[def:dyck_language]{Dyck words} are related as follows:
  \begin{thmenum}
    ...
    \begin{thmenum}
      \thmitem{thm:dyck_words_and_binary_trees/word_to_tree/empty} If \( w = \bnfves \), we define \( T_w \) as the \hyperref[def:canonical_singleton_tree]{canonical singleton tree}.
...
\end{corollary}
\begin{proof}
  \Cref{thm:dyck_language_alternative_grammar} implies that \eqref{eq:thm:dyck_words_and_binary_trees/tree_to_word} describes precisely all Dyck words. This process is clearly reversible.
\end{proof}
