...
\begin{comments}
  ...
  \item The condition for balanced delimiters is based on the discussion of \hyperref[def:dyck_language]{Dyck languages} by \incite[example 6.6.6]{Stanley2023EnumCombinatoricsVol2}.
...
\begin{definition}\label{def:dyck_language}\mcite[example 6.6.6]{Stanley2023EnumCombinatoricsVol2}
  The \term[ru=язык Дика (\cite[213]{Гладкий1973Языки})]{Dyck language} is the language \hyperref[def:formal_grammar/language]{generates} by the \hyperref[def:formal_grammar]{grammar}
  \begin{bnf*}
    \bnfprod{expr} \bnfor \bnftsq{(} \bnfsp \bnfpn{expr} \bnfsp \bnftsq{(} \bnfor \bnfpn{expr} \bnfpn{expr} \bnfor \varepsilon
  \end{bnf*}
\end{definition}

\begin{proposition}\label{thm:dyck_language_balanced}
  The \hyperref[def:dyck_language]{Dyck language} has \hyperref[def:paired_delimiters]{balanced parentheses}.
\end{proposition}
\begin{proof}
