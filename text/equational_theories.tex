...
\begin{proposition}\label{thm:pure_equality_isomorphism}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:pure_equality_isomorphism/structure} Every \hi{nonempty} set determines a structure over \( \Sigma_= \) (with an empty interpretation).
    ...
    \thmitem{thm:pure_equality_isomorphism/n_categorical} The theory \( T_= \) is \hyperref[def:fol_categorical_theory]{\( \kappa \)-categorical} for every \hyperref[def:cardinal]{cardinal} \( \kappa \).
...
\begin{proof}
  ...
  \SubProofOf{thm:pure_equality_isomorphism/n_categorical} Let \( \mscrX = (X, I) \) and \( \mscrY = (Y, J) \) be two structures over \( \Sigma_= \).
  ...
  \NecessitySubProof* \Cref{thm:pure_equality_isomorphism/n_categorical} implies that, if two structures have the same cardinality, they are isomorphic. Then \cref{thm:def:elementary_embedding/isomorphism} implies that they are elementarily equivalent.
\end{proof}
...
\begin{definition}\label{def:categorical_kernels}
  ...
  \begin{thmenum}
    \thmitem{def:categorical_kernels/kernel}\mcite[36]{ЦаленкоШульгейфер1974ОсновыТеорииКатегорий} We define the \term[ru=ядро, en=kernel (\cite[191]{MacLane1998CategoryTheory})]{kernel} of a morphism \( f: A \to B \) as the \hyperref[def:equalizers/equalizer]{equalizer} of \( f \) and \( 0_{A,B} \). It is by definition a \hyperref[def:categorical_subobject]{subobject} of \( A \).

    We denote the vertex by \( \ker f \) and the inclusion by \( \iota: \ker f \to A \), however we may also ambiguously use \( \ker f \) for the morphism in some cases like \cref{def:categorical_kernels/image}.

    \thmitem{def:categorical_kernels/cokernel}\mcite[37]{ЦаленкоШульгейфер1974ОсновыТеорииКатегорий} Dually, we define the \term[ru=коядро, en=cokernel (\cite[191]{MacLane1998CategoryTheory})]{cokernel} \( \co\ker f \) of a morphism \( f: A \to B \) as the \hyperref[def:equalizers/coequalizer]{coequalizer} of \( f \) and \( 0_{A,B} \). It is by definition a \hyperref[def:categorical_quotient_object]{quotient object} of \( A \).

    We denote the vertex by \( \co\ker f \) and the projection by \( \pi: B \to \co\ker f \), with the same caveat as above --- we may ambiguously use \( \co\ker f \) for the morphism when appropriate.

    \thmitem{def:categorical_kernels/image}\mcite[def. IX.1.16]{Aluffi2009Algebra} We define the \term{image} of a morphism \( f: A \to B \) as \( \ker \co\ker f \).

    \thmitem{def:categorical_kernels/coimage}\mcite[def. IX.1.16]{Aluffi2009Algebra} Dually, we define the \term{coimage} of a morphism \( f: A \to B \) as \( \co\ker \ker f \).
  \end{thmenum}
\end{definition}

\begin{theorem}[Image-coimage factorization]\label{thm:image_coimage_factorization}
  In a \hyperref[def:category]{category} with \hyperref[def:zero_morphism]{zero morphisms}, for every morphism \( f: A \to B \), if both its \hyperref[def:categorical_kernels/image]{image} and \hyperref[def:categorical_kernels/coimage]{coimage} exist, there exists a unique morphism \( \widetilde{f}: \co\img f \to \img f \) such that the following diagram commutes:
  \begin{equation*}
    \begin{aligned}
...
\begin{proof}
  ...
  The projection \( \pi: A \to \co\img f \) is the cokernel morphism of \( \iota' \) above, which is in turn the kernel morphism of \( \ker f \). Another coequalizer cocone for \( \iota' \) is \( (B, f) \). Indeed, \( \iota' \) itself satisfies
...
\begin{proposition}\label{thm:equational_theory_zero_morphisms}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:equational_theory_zero_morphisms/image} The \hyperref[def:categorical_kernels/image]{categorical image} of a morphism \( h: X \to Y \) is the coset \( \pi(J(c)) \) with the inclusion \( \iota: \pi(J(c)) \to Y \), where \( \pi: Y \to Y / {\cong} \) is the projection from \cref{thm:equational_theory_zero_morphisms/cokernel}.

    The image \( \pi(J(c)) \) contains the \hyperref[def:set_valued_map/image]{set-theoretic image} \( h[X] \) by definition, but does not necessarily coincide with it (for example, \( h[X] \) may not be a normal subgroup, even though, as a kernel, \( \pi(J(c)) \) should be).

    \thmitem{thm:equational_theory_zero_morphisms/coimage} Dually, the \hyperref[def:categorical_kernels/coimage]{categorical coimage} of a morphism \( h: X \to Y \) is the quotient of \( \mscrX \) by the \hyperref[def:fol_congruence]{congruence} \( {\cong} \) \hyperref[def:fol_generated_congruence]{generated} by
    \begin{equation}\label{eq:thm:equational_theory_zero_morphisms/coimage/relation}
      a \sim I(c) \T{if and only if} h(a) = J(c)
    \end{equation}
    with the canonical projection \( \pi: X \to X / {\cong} \).
  \end{thmenum}
\end{proposition}
\begin{proof}
  ...
  \SubProofOf{thm:equational_theory_zero_morphisms/image} We have already shown in \cref{thm:equational_theory_zero_morphisms/cokernel} that the cokernel of \( h \) is \( (\mscrY / {\cong}, \pi) \), where \( {\cong} \) is the congruence generated by \eqref{eq:thm:equational_theory_zero_morphisms/cokernel/relation}.

  The kernel of \( \pi \) is, according to \cref{thm:equational_theory_zero_morphisms/kernel}, determined by the set
  \begin{equation*}
    L \coloneqq \set{ b \in Y \given \underbrace{\pi(b) = \pi(J(c))}_{b \cong J(c)} } = \pi(J(c)).
  \end{equation*}

  The congruence \( {\cong} \) is generated by \( {\sim} \), which relates all elements of \( h[X] \) to \( J(c) \). Hence, \( h[X] \) is a subset of \( L = \pi(J(c)) \).

  Conversely, if \( b \cong J(c) \), then \( b \) belongs to \( h[X] \).

  \SubProofOf{thm:equational_theory_zero_morphisms/coimage} We have shown in \cref{thm:equational_theory_zero_morphisms/kernel} that the kernel of \( h \) is \( \ker h = h^{-1}(J(c)) \) with the inclusion \( \iota: \ker h \to X \).

  The cokernel of \( \iota \), according to \cref{thm:equational_theory_zero_morphisms/cokernel}, is the quotient of \( \mscrX \) by the relation
  \begin{equation*}
    a \sim I(c) \T{if and only if} a \in \iota[\ker h].
  \end{equation*}

  By definition, \( a \) is in \( \ker h \) if and only if \( h(a) = J(c) \), which leads to \eqref{eq:thm:equational_theory_zero_morphisms/coimage/relation}.
\end{proof}
