...
\begin{proof}
  ...
  The inverse function \( U(h)^{-1}: Y \to X \) is a candidate for an inverse of \( h \). We only need to show that it is a homomorphism.

  Indeed, fix some \( n \)-ary function symbol \( f \) and values \( y_1, \ldots, y_n \) from \( Y \). To avoid confusion, denote \( U(h)^{-1} \) by \( g \). The notation \( h^{-1} \) would be incorrect since we are yet to show that it is a morphism in \( \cat{C} \). Then
  \begin{equation*}
    J(f)\parens[\big]{ \underbrace{h(g(y_1))}_{y_1}, \ldots, \underbrace{h(g(y_n))}_{y_n} }
    =
    h\parens[\big]{ I(f)(g(y_1), \ldots, g(y_n)) }
  \end{equation*}
  and hence, applying \( g \) to both sides, we obtain
  \begin{equation*}
    g\parens[\big]{ J(f)\parens[\big]{ y_1, \ldots, y_1 } }
    =
    I(f)(g(y_1), \ldots, g(y_n)).
  \end{equation*}

  Therefore, \( g \) is a homomorphism, and hence an inverse of \( f \) in \( \cat{C} \).
\end{proof}
