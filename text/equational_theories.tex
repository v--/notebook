...
\begin{definition}\label{def:fol_equational_theory}\mimprovised
  We say that a \hyperref[def:fol_theory]{first-order theory} \( \Gamma \) is \term{equational} if it has an axiomatization consisting of \hyperref[def:fol_quantifier_closure]{universal closures} of \hyperref[def:fol_equation]{equations}.
\end{definition}
\begin{comments}
...
\begin{proposition}\label{thm:pure_equality_isomorphism}
  The \hyperref[def:pure_equality]{theory of pure equality} \( \Gamma_= \) (over the empty signature \( \Sigma_= \)) has the following basic properties:
  \begin{thmenum}
    \thmitem{thm:pure_equality_isomorphism/initial_theory} The theory \( \Gamma_= \) is an \hyperref[def:universal_objects/initial]{initial object} in any \hyperref[def:fol_theory/category]{category of theories} \( \cat{Th} \): every theory over every signature is an \hyperref[def:fol_theory/extension]{extension} of \( \Gamma_= \).

    \thmitem{thm:pure_equality_isomorphism/structure} Every set induces a structure over \( \Sigma_= \) (with an empty interpretation).
    ...
    \thmitem{thm:pure_equality_isomorphism/model} Every structure over \( \Sigma_= \) is a model of \( \Gamma_= \).
...
\begin{proof}
  ...
  \SubProofOf{thm:pure_equality_isomorphism/model} Since \( \Gamma_= \) is the consequence closure of the empty set, \cref{thm:institutional_satisfaction_closure} implies that every structure \( \mscrX \) over \( \Sigma_= \) satisfying \( \varnothing \) also satisfies \( \Gamma_= \).
...
\begin{definition}\label{def:fol_free_term_model}\mimprovised
  ...
  \begin{thmenum}
    ...
    \thmitem{def:fol_free_term_model/quotient} The desired free term model \( \mscrT_\Gamma(V) \) is then the \hyperref[def:fol_quotient_structure]{quotient structure} \( \mscrP / {\cong} \).
  \end{thmenum}
\end{definition}
