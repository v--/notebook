...
\begin{remark}\label{rem:categorical_image_and_coimage}
  Based on categorical kernels and cokernels, which we discuss in \cref{def:categorical_kernels}, \incite[def. IX.1.15]{Aluffi2009Algebra} defines the \term{image} of a morphism \( f: A \to B \) as \( \img f \coloneqq \ker \co\ker f \) and the \term{coimage} \( \co\img f \coloneqq \co\ker \ker f \).
  ...
  For these reasons, we avoid giving a general definition for images and coimages. Still, we will consider some generalizations of the image-coimage factorization, which gives us, for every \( f: A \to B \), a unique morphism \( \widetilde{f}: \co\img f \to \img f \) such that the following diagram commutes:
  \begin{equation*}
    \begin{aligned}
