...
\begin{remark}\label{rem:prime_numbers_and_prime_elements}
  ...
  \begin{thmenum}
    ...
    \begin{displayquote}
      We say that \( n \) is an \hi{irreducible element} of \( \BbbZ \) if \( \abs{n} > 1 \) and, whenever \( n = ab \) either \( \abs{a} = 1 \) or \( \abs{b} = 1 \) (or both).
    \end{displayquote}
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \item If \( a > b \), we can argue by \hyperref[thm:preorder_duality]{duality} to conclude that \cref{thm:int_sqrt_leq/b_leq} holds.
  \end{itemize}
\end{proof}
