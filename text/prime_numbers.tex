...
\begin{definition}\label{def:prime_number}\mcite[2]{Apostol1976AnalyticNumberTheory}
  A \term[ru=простое (число) (\cite[256]{Хинчин1951ЭлементыТеорииЧисел})]{prime number} is an integer greater than \( 1 \) whose only proper \emph{positive} \hyperref[def:semiring_divisibility]{divisor} is \( 1 \). Non-prime integers greater than \( 1 \) are called \term[ru=составное (число) (\cite[256]{Хинчин1951ЭлементыТеорииЧисел})]{composite numbers}.
\end{definition}
\begin{comments}
...
\begin{remark}\label{rem:prime_numbers_and_prime_elements}
  ...
  \begin{thmenum}
    ...
    \begin{displayquote}
      We say that \( n \) is an \emph{irreducible element} of \( \BbbZ \) if \( n \) is nonzero and non-invertible and, whenever \( n = ab \), either \( a \) or \( b \) or both are invertible.
    \end{displayquote}
    ...
    \begin{displayquote}
      We say that \( n \) is an \emph{irreducible element} of \( \BbbZ \) if \( \abs{n} > 1 \) and, whenever \( n = ab \) we have \( \abs{a} = 1 \) or \( \abs{b} = 1 \) (or both).
    \end{displayquote}
    ...
    \begin{displayquote}
      We say that \( n \) is a \emph{prime element} of \( \BbbZ \) if \( n \) is nonzero and non-invertible and, whenever \( n \) divides \( ab \), it divides \( a \) or \( b \) (or possibly both).
    \end{displayquote}

    We can again simplify the first condition on \( n \) to \( \abs{n} > 1 \), but the divisibility condition is less obvious.

    \Fullref{thm:euclids_lemma} demonstrates that every prime \emph{number} is also a prime \emph{element} of \( \BbbZ \). Thus, a prime element \( p \) of \( \BbbZ \) is a prime number if and only if \( p \) is positive.
  \end{thmenum}
\end{remark}
\begin{comments}
...
\begin{theorem}[Euclid's prime infinitude theorem]\label{thm:euclids_prime_infinitude_theorem}\mcite[62]{Кострикин2020АлгебраЧасть1}
...
\Textcite[255]{Aluffi2009Algebra} remarks that \enquote{Entire libraries have been written on the subject of valuations}, yet as we shall see, there is no consensus on a precise definition. We provide \cref{def:exponential_valuation} and otherwise try to avoid the term as much as possible.
...
\begin{comments}
  ...
  \begin{itemize}
    \item \Textcite[def. 9.4']{Jacobson1989BasicAlgebraII} defines an \enquote{exponential valuation} on fields with values in an abelian group having an additional value adjoined, \( \infty \), such that \( v(x) = \infty \) if and only if \( x = 0 \).

    \item \Textcite{Винберг2014КурсАлгебры} defines an \enquote{нормирование} (\enquote{norming}) on integral domains as surjective functions with positive integer values.
  \end{itemize}
\end{comments}
...
\end{definition}
\begin{comments}
  \item \Textcite[465]{Lang2002Algebra} calls such functions \enquote{valuations}, while \cite[def. 9.4']{Jacobson1989BasicAlgebraII} generalizes them to Abelian groups and calls them \enquote{additive valuations}.
\end{comments}
...
\begin{theorem}[Fundamental theorem of arithmetic]\label{thm:fundamental_theorem_of_arithmetic}
  ...
  Furthermore, every \hyperref[def:irreducible_factorization]{irreducible factorization} of \( n \) into \emph{positive numbers} is a \hyperref[def:multiset_permutation]{multiset permutation} of the prime number multiplicands above, where \( p \) occurs \( v_p(n) \) times.
\end{theorem}
\begin{comments}
...
\begin{definition}\label{def:int_sqrt}\mimprovised
  ...
  \begin{equation*}
    \op{isqrt}(n) \coloneqq \max\set{ k = 1, 2, \ldots \given k^2 \leq n }.
  \end{equation*}
...
\begin{proposition}\label{thm:int_sqrt_leq}
  If \( n = ab \) for some positive integers, we have the following possibilities:
  \begin{thmenum}
...
\begin{corollary}\label{thm:prime_number_sqrt_prime}
  An integer \( n > 1 \) is \hyperref[def:prime_number]{prime} if and only if no \emph{prime} \( p \) with \( p \leq \op{isqrt}(n) \) divides \( n \).
\end{corollary}
\begin{proof}
...
\begin{comments}
  ...
  \begin{CodeRefDisplay}
    \GetCodeRef{alg:sieve_of_eratosthenes}
  \end{CodeRefDisplay}
\end{comments}
...
\begin{example}\label{ex:thm:inclusion_exclusion_eratosthenes}
  ...
  \begin{equation*}
    \begin{array}{r *{23}{c}}
      1  & \underline{2} & \underline{3} & 4       & \underline{5} & 6       & \underline{7} & 8  & 9  & 10      & \underline{11} & 12 & \underline{13} \\
      14 & 15     & 16     & \underline{17} & 18     & \underline{19} & 20     & 21 & 22 & \underline{23} & 24      & 25 &
    \end{array}
  \end{equation*}
\end{example}
...
\begin{definition}\label{def:coprime_numbers}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:coprime_numbers/concrete}\mcite[45]{Зорич2019АнализЧасть1} Their only \emph{positive} common divisor is \( 1 \).
...
\begin{theorem}[Modular Chinese remainder theorem]\label{thm:modular_chinese_remainder_theorem}
  Fix pairwise \hyperref[def:coprime_numbers]{coprime} integers \( m_1, \ldots, m_n \) and let \( m \coloneqq m_1 \cdots m_n \) be their product. Then any system of \hyperref[con:modular_arithmetic]{modular} equations
  \begin{equation}\label{eq:thm:modular_chinese_remainder_theorem}
    \begin{aligned}
  ...
  In particular, when \( a_1 = \cdots = a_n \), if \( x \) is a solution modulo \( m_k \) for every \( k = 1, \ldots, n \), it is a solution modulo \( m \).
\end{theorem}
\begin{comments}
  \item According to \textcite[\S 5.2]{Stillwell2010MathHistory}, this theorem was known to Sun Zi in the third century AD. Hence the name of the theorem.
\end{comments}
\begin{proof}
  Since \( m_1, \ldots, m_n \) are coprime, their least common multiple is \( m \), hence
  \begin{equation*}
    \BbbZ_{m_1} \cap \cdots \cap \BbbZ_{m_n} = \BbbZ_m.
  \end{equation*}

  \Fullref{thm:chinese_remainder_theorem} implies that the map
  \begin{equation*}
    \begin{aligned}
      &\Phi: \BbbZ_m \to \BbbZ_{m_1} \times \cdots \times \BbbZ_{m_n}, \\
      &\Phi(x + \BbbZ_m) \coloneqq (x + \BbbZ_{m_1}, \ldots, x + \BbbZ_{m_n})
    \end{aligned}
  \end{equation*}
  is an isomorphism. Therefore, the solutions to the system \eqref{eq:thm:modular_chinese_remainder_theorem} belong to the coset
  \begin{equation*}
    \Phi^{-1}(a_1 + \BbbZ_{m_1}, \ldots, a_n + \BbbZ_{m_n}).
  \end{equation*}

  This coset has a unique representative modulo \( m \), which is the solution we seek.
\end{proof}

\begin{algorithm}[Chinese remainder theorem iteration]\label{alg:chinese_remainder_theorem_iteration}
  \Fullref{thm:modular_chinese_remainder_theorem} guarantees the existence of a solution, but only gives a hint at how to find it. The proof hints at how to find it, but in our more abstract presentation, the details are obscured away.

  We present a simple recursive algorithm for solving the system \eqref{eq:thm:modular_chinese_remainder_theorem}.

  \begin{thmenum}
    \thmitem{alg:chinese_remainder_theorem_iteration/bottom} If \( n = 1 \), the equation \( x \equiv a_1 \pmod {m_1} \) has a trivial solution --- \( \rem(a_1, m_1) \).

    \thmitem{alg:chinese_remainder_theorem_iteration/reduction} If \( n > 1 \), we solve the last two equations
    \begin{alignat*}{2}
      x &\equiv a_{n-1} &&\pmod {m_{n-1}}, \\
      x &\equiv a_n     &&\pmod {m_n}.
    \end{alignat*}

    \Fullref{alg:extended_euclidean_algorithm} gives us coefficients \( k_{n-1} \) and \( k_n \) such that
    \begin{equation*}
      k_{n-1} m_{n-1} + k_n m_n = \gcd(m_{n-1}, m_n) = 1.
    \end{equation*}

    We claim that \( b \coloneqq (k_{n-1} m_{n-1}) a_n + (k_n m_n) a_{n-1} \) is a solution since
    \begin{equation*}
      b = (1 - k_n m_n) a_n + (k_n a_n) a_{n-1} = a_n + (k_n m_n) (a_{n-1} - a_n),
    \end{equation*}
    so \( b \equiv a_n \pmod {m_n} \), and similarly \( b \equiv a_{n-1} \pmod {m_{n-1}} \).

    Note that, since \( m_{n-1} \) and \( m_n \) are both coprime with \( m_i \) for \( i < n - 1 \), their product is also coprime. Thus, we obtain a smaller system of equations
    \begin{alignat*}{2}
      x &\equiv a_1     &&\pmod {m_1}, \\
        &\vdots \\
      x &\equiv a_{n-2} &&\pmod {m_{n-2}}, \\
      x &\equiv b       &&\pmod {m_{n-1} m_n}
    \end{alignat*}
    that we can apply the algorithm recursively to.
  \end{thmenum}
\end{algorithm}
\begin{comments}
  \item This algorithm is implemented in \cite{notebook:code} as
  \begin{CodeRefDisplay}
    \GetCodeRef{alg:chinese_remainder_theorem_iteration}
  \end{CodeRefDisplay}
\end{comments}

\begin{example}\label{ex:thm:modular_chinese_remainder_theorem}
  We list examples of how \fullref{thm:modular_chinese_remainder_theorem} can be used:
  \begin{thmenum}
    \thmitem{ex:thm:modular_chinese_remainder_theorem/pirates} A popular problem goes as follows.

    A group of \( n \) pirates, where \( n \) is odd, finds a treasure. They split the coins among themselves evenly, with \( a \) coins left.

    While arguing how split these coins, one of the pirates is killed in the turmoil. The rest split the coins evenly, only to find that there are \( b \) coins left.

    After another pirate gets killed, the rest are able to split the treasure evenly.

    The total number of coins, \( x \), is then a solution to the following system of equations:
    \begin{align*}
      x &\equiv a \pmod n, \\
      x &\equiv b \pmod {n - 1}, \\
      x &\equiv 0 \pmod {n - 2}.
    ...
    \Fullref{thm:modular_chinese_remainder_theorem} implies that a unique solution modulo \( n(n-1)(n-2) \) exists. To find a solution, we can use \fullref{alg:chinese_remainder_theorem_iteration}.

    For example, with \( n = 7 \), \( a = 3 \) and \( b = 4 \), the unique solution modulo \( 210 \) is \( 10 \). Indeed,
    \begin{align*}
      10 &\equiv 3 \pmod 7, \\
      10 &\equiv 4 \pmod 6, \\
      10 &\equiv 0 \pmod 5.
    ...
    \thmitem{ex:thm:modular_chinese_remainder_theorem/proof} It is important in the proof of correctness of \fullref{alg:rsa}.
  \end{thmenum}
\end{example}
...
\end{definition}
\begin{comments}
  \item In particular, we have \( \varphi(1) = 1 \). It is for this reason that we count integers smaller than \emph{or equal to} \( n \).

  \item \Fullref{thm:totient_function_product_formula} provides a useful expression in terms of prime factors.

  \item This function is an important part of \fullref{alg:rsa}.
\end{comments}
...
\begin{definition}\label{def:multiplicative_arithmetic_function}\mcite[33]{Apostol1976AnalyticNumberTheory}
  ...
  If \eqref{eq:def:multiplicative_arithmetic_function} is satisfied for \emph{all} positive integers \( n \) and \( m \), we say that \( f \) is \term[ru=вполне мультипликативная (функция) (\cite[def. 37]{Бухштаб1966ТеорияЧисел})]{completely multiplicative}.
\end{definition}
\begin{comments}
...
\begin{proof}
  ...
  Let \( m \) be the quotient of \( \varphi(n) \) and \( k \). We have \( y^k \equiv 1 \pmod n \) since \( k \) is the (multiplicative) order of \( y \). Thus,
  \begin{equation*}
    y^{\varphi(n)}
    =
    y^{km}
...
\begin{corollary}\label{thm:division_modulo}
  ...
  \begin{equation*}
    x^n \equiv x^r \pmod m,
  \end{equation*}
\end{corollary}
\begin{proof}
  By \fullref{thm:eulers_totient_theorem}, \( x^{\varphi(m)} \equiv 1 \pmod m \). Then
  \begin{equation*}
    x^n = (x^{\varphi(m)})^q x^r \equiv x^r \pmod m.
  \end{equation*}
\end{proof}
...
\begin{proof}
  ...
  Otherwise, by \fullref{thm:eulers_totient_theorem}, we have \( x^{\varphi(p) + 1} \equiv x \pmod p \), and by \cref{thm:def:eulers_totient_function/prime}, we have \( \varphi(p) + 1 = p \).
\end{proof}
