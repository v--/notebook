...
\begin{theorem}[Euclid's prime infinitude theorem]\label{thm:euclids_prime_infinitude_theorem}\mcite[62]{Кострикин2000АлгебраТом1}
...
\begin{definition}\label{def:non_archimedean_absolute_value}\mcite[561]{Jacobson1989AlgebraPart2}
...
\end{definition}
\begin{comments}
  \item \incite[465]{Lang2002Algebra} calls such functions \enquote{valuations}, while \cite[def. 9.4']{Jacobson1989AlgebraPart2} generalizes them to Abelian groups and calls them \enquote{additive valuations}.
\end{comments}

\begin{definition}\label{def:exponential_valuation}
  An \term[ru=нормирование (\cite[437]{Винберг2014Алгебра}), en=exponential valuation (\cite[def. 9.4']{Jacobson1989AlgebraPart2})]{exponential valuation} on an \hyperref[def:integral_domain]{integral domain} \( D \) is a map \( v: D \setminus \set{ 0 } \to \BbbZ_{>0} \) that satisfies the following properties:
  \begin{thmenum}
...
\begin{comments}
  ...
  \begin{itemize}
    \item \incite[def. 9.4']{Jacobson1989AlgebraPart2}, where Jacobson defines an \enquote{exponential valuation} on fields with values in an abelian group having an additional value adjoined, \( \infty \), such that \( v(x) = \infty \) if and only if \( x = 0 \).

    \item \incite[437]{Винберг2014Алгебра}, where Vinberg defines an \enquote{нормирование} (\enquote{norming}) on integral domains as surjective functions with positive integer values.
  \end{itemize}
\end{comments}
...
\begin{definition}\label{def:padic_valuation}\mcite[437]{Винберг2014Алгебра}
...
\begin{theorem}[Fundamental theorem of arithmetic]\label{thm:fundamental_theorem_of_arithmetic}
  ...
  \begin{equation}\label{thm:fundamental_theorem_of_arithmetic}
...
\end{comments}
\begin{proof}
  Let \( u r_1 \cdots r_m \) be an irreducible factorization of \( n \). All irreducible factors are either prime numbers or additive inverses of prime numbers, thus, without loss of generality, we can assume that \( u = 1 \) and that all irreducible factors are prime numbers.

  Furthermore, every irreducible factorization of \( n \) is simply a permutation of this one.

  Then \( v_p(n) \) is the number of occurrences of \( p \) among \( r_1, \ldots, r_m \).
...
\end{definition}
\begin{comments}
  \item In terms of the real number \( n \)-th root function, which we will discuss in \fullref{def:real_nth_root}, we can define the integer square root as
  \begin{equation*}
...
\end{definition}
\begin{comments}
  \item In particular, we have \( \varphi(n) = 1 \). It is for this reason that we count integers smaller than \hi{or equal to} \( n \).
...
\end{proposition}
\begin{comments}
  \item We will discuss decimal expansions in \fullref{sec:positional_number_systems}. For the purpose of this proposition, a decimal expansion of the positive integer \( n \) is a list \( a_0, \cdots, a_m \) of nonnegative integers less than \( 10 \) such that \( a_m \neq 0 \) and
  \begin{equation*}
...
\begin{theorem}[Totient function product formula]\label{thm:totient_function_product_formula}
  Let \( n = p_1^{a_1} \cdots p_r^{a_r} \) be a prime factorization of \( n \). Then \hyperref[def:eulers_totient_function]{Euler's totient function} can be expressed as
...
\begin{comments}
  ...
  To resolve this issue, we use \enquote{completely multiplicative} whenever confusion is possible.
\end{comments}

\begin{proposition}\label{thm:totient_function_multiplicative}
  The \hyperref[def:eulers_totient_function]{Euler's totient function} is \hyperref[def:multiplicative_arithmetic_function]{multiplicative}.
\end{proposition}
\begin{proof}
