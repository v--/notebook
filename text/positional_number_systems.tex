...
\begin{comments}
  ...
  \item Although integers are (embedded into) real numbers, we will sometimes find it convenient to handle them separately --- see \fullref{def:nonnegative_integer_radix_expansion}.
\end{comments}
...
\begin{proof}
  ...
  The latter is a \hyperref[def:geometric_series]{geometric series} with quotient \( 1 / b \), which converges by \fullref{thm:def:geometric_series/series_sum_interior}. \Fullref{thm:positive_series_comparison/b_converges} implies that \eqref{eq:thm:radix_expansion_converges/proof} also converges.
\end{proof}
...
\begin{example}\label{ex:def:real_number_radix_expansion}
  ...
  \begin{thmenum}
    ...
    \thmitem{ex:def:real_number_radix_expansion/geometric} For any integer \( n \), the following \hyperref[eq:def:geometric_progression/recursive]{geometric series} is a base \( n \) expansion of \( n \):
...
\end{definition}
\begin{comments}
  \item These interpretations are justified by \fullref{thm:nonnegative_integer_radix_expansion} and \fullref{alg:rational_number_to_positional_string}.
\end{comments}
...
\begin{comments}
  ...
  \item This algorithm can be found as \identifier{arithmetic.bases.get_number_expansion} in \cite{notebook:code}.
\end{comments}
\begin{defproof}
...
\begin{proposition}\label{thm:nonnegative_integer_radix_expansion}
...
\begin{corollary}\label{thm:nonnegative_integer_radix_expansion_via_algorthm}
...
\end{corollary}
\begin{proof}
  Follows from \fullref{thm:nonnegative_integer_radix_expansion} and \fullref{thm:real_number_radix_expansion_zeros}.
\end{proof}
...
\end{algorithm}
\begin{comments}
  \item This algorithm can be found as \identifier{arithmetic.bases.get_rational_number_expansion} in \cite{notebook:code}.
\end{comments}
\begin{defproof}
...
\end{algorithm}
\begin{comments}
  \item This algorithm can be found as \identifier{arithmetic.bases.get_fraction} in \cite{notebook:code}.
\end{comments}

\paragraph{Integer radix expansions}

\begin{definition}\label{def:nonnegative_integer_radix_expansion}
  Similarly to how we have defined real number radix expansions in \fullref{def:real_number_radix_expansion}, we define a radix \( b \) expansion for the nonnegative integer \( n \) as an \hyperref[def:univariate_polynomial]{univariate polynomial}
  \begin{equation*}
...
\begin{proposition}\label{thm:integer_and_real_radix_expansions}
  ...
  \begin{equation}\label{eq:thm:integer_and_real_radix_expansions/polynomial}
    \sum_{k=0}^m a_k b^k
  \end{equation}
  is an \hyperref[def:nonnegative_integer_radix_expansion]{integer radix expansion}.
\end{proposition}
\begin{comments}
  \item The gist here, discussed in \fullref{def:endianness_and_numbers}, is that the real number expansion gives the \hyperref[def:endianness_and_numbers]{little-endian order} of the coefficients while integer expansion \hyperref[def:endianness_and_numbers]{big-endian order}.
\end{comments}
\begin{proof}
  Trivial.
\end{proof}

\begin{algorithm}[Integer radix expansion]\label{alg:nonnegative_integer_radix_expansion}
  We will give a simplified form of \fullref{alg:real_number_radix_expansion} for \hyperref[def:nonnegative_integer_radix_expansion]{integer radix expansions}. Fix an integer \( n \) and a radix \( b \geq 2 \).

  \begin{thmenum}
    \thmitem{alg:nonnegative_integer_radix_expansion/negative} As in \fullref{alg:real_number_radix_expansion/negative}, if \( n < 0 \) we build an expansion for \( -n \) and then multiply it by \( -1 \).

    \thmitem{alg:nonnegative_integer_radix_expansion/base} If \( 0 \leq n < b \), halt the algorithm with \( n \) regarded as a constant polynomial.

    \thmitem{alg:nonnegative_integer_radix_expansion/recursive} Finally, if \( n > b \), let \( n = bq + r \) be the result of \fullref{alg:integer_division} and let \( p(X) \) be the expansion of \( q \). Then
    \begin{equation*}
      X \cdot p(X) + r
    \end{equation*}
...
\begin{algorithm}\label{alg:addition_with_carrying}
  Fix a radix \( b \) and consider the \hyperref[def:nonnegative_integer_radix_expansion]{integer expansions}
  \begin{equation*}
    n = \sgn(n) \cdot \sum_{k=0}^p a_k b^k
  \end{equation*}
  and
  \begin{equation*}
    m = \sgn(m) \cdot \sum_{k=0}^r c_k b^k.
  \end{equation*}

  We will find the digits of the expansion
  \begin{equation*}
    n + m = \sgn(n + m) \cdot \sum_{k=0}^{\max\set{ p, r } + 1} d_k b^k.
  \end{equation*}

  \begin{thmenum}
    \thmitem{alg:addition_with_carrying/n_guard} If \( n < 0 \), we can apply the algorithm to \( -n \) and \( -p \) since
    \begin{equation*}
      n + p = -(-n - p).
    \end{equation*}
...
\begin{defproof}
  ...
  \begin{itemize}
    ...
    \begin{equation*}
      ...
      \reloset{\T{ind.}} =
      \sum_{k=0}^s a_k b^k + \sgn(m) \cdot \sum_{k=0}^s c_k b^k.
    \end{equation*}
  \end{itemize}
  ...
  \SubProof{Proof that the expansion polynomial degree is at most \( \max\set{ p, r } + 1 \)} \Fullref{alg:addition_with_carrying/m_guard} ensures that \( r \leq p \), thus \( \max\set{ p, r } = p \). We must show that \( d_k = 0 \) when \( k > p + 1 \).
...
\paragraph{Fixed-size integer encoding}
...
\begin{definition}\label{def:endianness}\mimprovised
  ...
  \begin{thmenum}
    \thmitem{def:endianness/big} The \term{big-endian} order is based on the implicit \hyperref[def:enumeration]{enumerations} \( d_0, \ldots, d_{m-1} \).
...
\end{definition}
\begin{comments}
  \item Endianness is discussed in \fullref{ex:little_endian_motivation}.

  \item Digit significance allows us to work with both big-endian and little-endian orders simultaneously by abstracting away the implicit order of a string. This is illustrated in \fullref{thm:nonnegative_integer_expansion_divisibility}.

  \item This terminology is based on Danny Cohen's article \cite{Cohen1981Endianness} on the order in which computers and communication protocols handle \hyperref[def:bit_string]{bit strings}. Cohen in turn based his terms on Jonathan Swift's \cite{Swift2005GulliverTravels}, where Lilliputs fight wars for which end the eggs should be broken at.
...
\begin{definition}\label{def:fixed_length_nonnegative_integer_encoding}\mimprovised
  ...
  We say that the string \( s_{a_0} \ldots s_{a_{m-1}} \) \term{encodes} the nonnegative integer \( n \) if
  \begin{equation*}
...
\end{definition}
\begin{comments}
  \item This is an extension of the positional natural number strings whose connection to real number expansions is discussed in \fullref{def:positional_notation_as_radix_expansion/natural_numbers}. It allows both varying endianness and relaxes the requirement that the most significant digit must be nonzero. The global uniqueness lost from this is brought back by using fixed-length strings --- see \fullref{thm:def:fixed_length_nonnegative_integer_encoding}.
\end{comments}

\begin{proposition}\label{thm:def:fixed_length_nonnegative_integer_encoding}
...
\begin{proof}
  ...
  For a nonnegative integer \( n < b^m \), \fullref{alg:nonnegative_integer_radix_expansion} gives us the radix \( b \) expansion
  \begin{equation*}
  ...
  \Fullref{thm:cardinality_product_rule} implies that there is a total of \( b^m \) digit strings of length \( m \), hence we have described all of them.
\end{proof}
...
\begin{example}\label{ex:little_endian_motivation}
  ...
  \begin{MemoryLine}{12}
    \anon & \anon & \MemoryLineArrow 0 & 0 & 0 & 0 & 0 & 4 & 3 & 2 & \anon & \anon
  \end{MemoryLine}
  ...
  \begin{MemoryLine}{12}
    \anon & \anon & \anon & \anon & \anon & \anon & \MemoryLineArrow 0 & 4 & 3 & 2 & \anon & \anon
  \end{MemoryLine}

  Another option is to move the least significant \( 4 \) digits and discard the rest:
  \begin{MemoryLine}{12}
    \anon & \anon & \MemoryLineArrow 0 & 4 & 3 & 2 & \anon & \anon & \anon & \anon & \anon & \anon
  \end{MemoryLine}
