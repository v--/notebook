...
\begin{definition}\label{def:positional_number_system}\mimprovised
  ...
  We list several established positional systems, out of which we will almost exclusively use decimal notation\fnote{As per \cref{rem:object_language_dots}, for disambiguation, we place dots on top of the symbols in the object language.}:
  \begin{thmenum}
    \thmitem{def:positional_number_system/binary} The \term[ru=двоичная (система) (\cite[60]{Зорич2019АнализЧасть1}), en=binary (\cite[195]{Knuth1997ArtVol2})]{binary} system with \( b = 2 \) and digits \( \syn0, \syn1 \). We also call binary digits \term{bits}.

    \thmitem{def:positional_number_system/ternary} The \term[ru=троичная (система) (\cite[60]{Зорич2019АнализЧасть1}), en=ternary (\cite[195]{Knuth1997ArtVol2})]{ternary} system with \( b = 3 \) and digits \( \syn0, \syn1, \syn2 \).

    \thmitem{def:positional_number_system/octal} The \term[ru=восьмичная (система) (\cite[60]{Зорич2019АнализЧасть1}), en=octal (\cite[\S 4.2.2]{Rosen2019DiscreteMathematics})]{octal} system with \( b = 8 \) and digits \( \syn0, \syn1, \ldots, \syn7 \).

    \thmitem{def:positional_number_system/decimal} The ubiquitous \term[ru=десетичная (система) (\cite[60]{Зорич2019АнализЧасть1}), en=decimal (\cite[195]{Knuth1997ArtVol2})]{decimal} system with \( b = 10 \) and digits \( \syn0, \syn1, \ldots, \syn9 \).

    \thmitem{def:positional_number_system/hexadecimal}\mcite[195]{Knuth1997ArtVol2} The \term[en=hexadecimal (\cite[195]{Knuth1997ArtVol2})]{hexadecimal} system with \( b = 16 \) and digits \( \syn0, \syn1, \ldots, \syn9, \synA, \synB, \synC, \synD, \synE, \synF \).
  \end{thmenum}
\end{definition}
\begin{comments}
...
\begin{comments}
  ...
  \begin{CodeRefDisplay}
    \GetCodeRef{alg:real_number_radix_expansion}
  \end{CodeRefDisplay}
\end{comments}
\begin{defproof}
...
\begin{proposition}\label{thm:alg:real_number_radix_expansion}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:alg:real_number_radix_expansion/rational_coefficient} For a nonnegative rational number \( p / q \), the \( n \)-th \emph{nonnegative} coefficient in \fullref{alg:real_number_radix_expansion} is
...
\begin{comments}
  \item This algorithm is implemented in \cite{notebook:code} as
  \begin{CodeRefDisplay}
    \GetCodeRef{alg:rational_number_to_positional_string}
  \end{CodeRefDisplay}
\end{comments}
\begin{defproof}
...
\begin{comments}
  \item This algorithm is implemented in \cite{notebook:code} as
  \begin{CodeRefDisplay}
    \GetCodeRef{alg:positional_string_to_rational_number}
  \end{CodeRefDisplay}
\end{comments}
