...
\begin{definition}\label{def:zfc}\mcite[def. 6.1.6]{Hinman2005}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:zfc/choice}\mcite[def. 6.4.1]{Hinman2005} The \term{axiom of choice} states that a \hyperref[def:choice_function]{choice function} exists for any family of nonempty sets. In symbols:
      ...
      \qforall \sigma \qexists \rho \parens[\Bigg]
        {
          ...
          \wedge
          \qforall {\synx \in \sigma} \parens[\Big]{ \neg\ref{eq:def:empty_set/predicate}[\synx] \synimplies \qexists \syny (\syny \in \rho \synimplies \qExists \synz (\synz \in \synx \synwedge \ref{eq:def:cartesian_product/kuratowski_pair_predicate}[\syny, \synx, \synz]) }
        }
    \end{equation}
...
\begin{theorem}[Diaconescu-Goodman-Myhill theorem]\label{thm:diaconescu_goodman_myhill_theorem}\mcite{GoodmanMyhill1978}
...
\end{theorem}
\begin{comments}
  \item \incite[corr. 2]{Diaconescu1975} formulated a more abstract result, which \incite{GoodmanMyhill1978} later vastly simplified for the case of intuitionistic set theory. Our proof is based on the one given by \incite[thm. 5.3.3.6]{Mimram2020}, which in turn is based on the later.
\end{comments}
\begin{proof}
