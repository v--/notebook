...
\begin{definition}\label{def:zfc}\mcite[def. 6.1.6]{Hinman2005Logic}
  ...
  \begin{thmenum}
    ...
    \end{equation*}
    where \( \Bracks{\varphi}(\cdots) \) is the formula valuation function from \cref{def:propositional_valuation/formula_valuation_function}.
...
\begin{proof}
  ...
  \end{align*}
  where \( T \) and \( F \) are the Boolean values discussed in \cref{con:boolean_value}, which we may, for simplicity, regard as \( \set{ \varnothing } \) and \( \varnothing \).
