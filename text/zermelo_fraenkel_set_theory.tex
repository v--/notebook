...
\begin{proof}
  ...
  \SufficiencySubProof Let \( f: A \to B \) be a nonempty injective function. It thus satisfies \cref{def:function_invertibility/injective/inverse}, hence the \hyperref[def:set_valued_map/inverse]{inverse} \( f^{-1}: B \to A \) is a partial single-valued function.
  ...
  \SufficiencySubProof* The case with an empty function is handled in \cref{thm:function_invertibility_categorical/empty}, and we assume that it is nonempty.
  ...
  \NecessitySubProof* Conversely, if \( f \) is injective, it is left-invertible by \cref{thm:function_invertibility_categorical/nonempty_left_invertible} and left-cancellative by \cref{thm:def:morphism_invertibility/split_monomorphism}.
  ...
  \SubProofOf{thm:function_invertibility_categorical/right_cancellative} The case with an empty function is handled in \cref{thm:function_invertibility_categorical/empty}, and we assume that it is nonempty.
  ...
  it is right-invertible by \fullref{thm:surjective_functions_are_right_invertible} and right-cancellative by \cref{thm:def:morphism_invertibility/split_epimorphism}.

  \SubProofOf{thm:function_invertibility_categorical/fully_invertible}

  \SufficiencySubProof* If \( f: \varnothing \to B \) is a bijective empty function, then it is surjective and, by \cref{thm:function_invertibility_categorical/right_invertible}, it is right-invertible. By \cref{thm:function_invertibility_categorical/empty}, it is also left-invertible. Thus, it is fully invertible.

  \NecessitySubProof* Conversely, if an empty function \( f: \varnothing \to B \) is fully invertible, by \cref{thm:function_invertibility_categorical/empty} we have \( A = B = \varnothing \) and hence it is bijective.

  Finally, if \( f: A \to B \) is \hi{nonempty} function, then by \cref{thm:function_invertibility_categorical/nonempty_left_invertible} and \cref{thm:function_invertibility_categorical/right_invertible} it is bijective if and only if it is fully invertible.
