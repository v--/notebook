...
\begin{definition}\label{def:well_founded_relation}
  ...
  \begin{thmenum}
    \thmitem{def:well_founded_relation/minimal}\mcite[241]{Enderton1977SetTheory} \Fullref{def:chain_condition/maximal} can be restated as follows: every nonempty subset of \( P \) has a minimal element \( m \) in the sense that there does not exist any other element \( x \) from \( P \) such that \( x \prec m \).
