...
\begin{definition}\label{def:zfc}\mcite[def. 6.1.6]{Hinman2005Logic}
  The \hyperref[def:fol_theory]{first-order theory} commonly abbreviated as \term{\logic{ZFC}} is based on the same language as \hyperref[def:naive_set_theory]{na\"ive set theory}, but with different axioms. The three letters refer to:
  \begin{itemize}
  ...
  \begin{thmenum}
    \thmitem{def:zfc/extensionality} The \term{axiom of extensionality}, as defined in \cref{def:naive_set_theory/extensionality}. This is also the only axiom of the theory that does not deal with existence.
    ...
    As explained in \cref{def:naive_set_theory/unrestricted_comprehension} and \cref{def:set_builder_notation}, this set may depend on parameters, which are other sets. We must formally take the \hyperref[def:fol_quantifier_closure]{universal closure} of this formula to quantify over all possible values for the parameters.

    Compare this axiom to \hyperref[def:naive_set_theory/unrestricted_comprehension]{unrestricted comprehension}. Informally, restricted comprehension axiom can be obtained by taking the result of unrestricted comprehension and intersecting it with some set \( A \). As mentioned in \cref{def:set_builder_notation}, in set-builder notation such a set can be denoted via
    \begin{equation*}
      \set[\big]{ x \in B \given* \Bracks{\varphi}(x, u_1, \ldots, u_n) },
    \end{equation*}
    where \( \Bracks{\varphi}(\cdots) \) is the function from \fullref{def:parametrized_propositional_denotation}.
    ...
    As will be shown in \cref{thm:cumulative_hierarchy_model_of_zfc_without_infinity}, no other axioms of \logic{ZFC} ensure the existence of at least one infinite set.
    ...
    \todo{show that, as long as one choice function exists, we can find another one that suits our needs.}
    ...
    As is the case with the \hyperref[def:zfc/specification]{axiom schema of specification}, the formula \( \varphi \) may depend on parameters, in which case we use its \hyperref[def:fol_quantifier_closure]{universal closure}.
    ...
    This is a powerful axiom because it shows that set membership in \logic{ZFC} is well-founded --- see \cref{thm:set_membership_is_well_founded}. It is equivalent to \cref{thm:axiom_of_regularity} and is often itself called the \term{axiom of regularity}.
  \end{thmenum}
\end{definition}
...
\begin{proposition}\label{thm:zfc_existence_theorems}
  ...
  For example, \cref{thm:zfc_existence_theorems/set_of_functions} shows that the set \( \fun(A, B) \) of functions exists within the object theory for any two sets \( A \) and \( B \) in the object theory. Therefore, every single function between \( A \) and \( B \) is a set within the object theory because it is a member of \( \fun(A, B) \).
  ...
  \begin{thmenum}
    ...
    Only \hyperref[def:fol_definability]{definable subsets} of \( A \) can be described in this way, however. See \cref{thm:zfc_existence_theorems/power_set}.
    ...
    \thmitem{thm:zfc_existence_theorems/arbitrary_intersection} For any \emph{nonempty} family \( \mscrA \), the \hyperref[def:basic_set_operations/intersection]{intersection} \( \bigcap \mscrA \) exists.
    ...
    As a consequence, even subsets of \( A \) which are not \hyperref[def:fol_definability]{definable} exist.
    ...
    \thmitem{thm:zfc_existence_theorems/set_of_functions} For any two sets \( A \) and \( B \), the set \hyperref[def:set_of_all_functions]{\( \fun(A, B) \)} of functions between them exists.
...
\begin{proof}
  ...
  As a consequence of the \term{axiom of extensionality}, this empty set is unique. As discussed in \cref{def:empty_set}, we denote this unique empty set by \( \varnothing \).
  ...
  \SubProofOf{thm:zfc_existence_theorems/binary_intersection} For sets \( A \) and \( B \), by the \hyperref[def:zfc/pairing]{axiom of pairing} the set \( \set{ A, B } \) exists. Then by \cref{thm:zfc_existence_theorems/arbitrary_intersection}, the binary intersection
  \begin{equation*}
  ...
  \SubProofOf{thm:zfc_existence_theorems/binary_union} Similarly to \cref{thm:zfc_existence_theorems/binary_intersection}, for sets \( A \) and \( B \), by the \hyperref[def:zfc/pairing]{axiom of pairing} the set \( \set{ A, B } \) exists and by the \hyperref[def:zfc/union]{axiom of unions}, the binary union
  \begin{equation*}
  ...
  Its existence follows from \cref{thm:zfc_existence_theorems/singleton} and \cref{thm:zfc_existence_theorems/binary_union}.
  ...
  We can apply the \hyperref[def:zfc/power_set]{axiom of power sets} one more time in addition to those in \cref{thm:zfc_existence_theorems/indexed_family} to obtain the set of all indexed by \( \mscrK \) families of members of this union. Then we can apply the \hyperref[def:zfc/specification]{axiom schema of specification} to restrict only to those families that satisfy the condition of \cref{def:cartesian_product}.

  \SubProofOf{thm:zfc_existence_theorems/set_of_relations} All relations between \( A \) and \( B \) are subsets of \( A \times B \), hence elements of \( \pow(A \times B) \). The latter exists by \cref{thm:zfc_existence_theorems/cartesian_product} and \cref{thm:zfc_existence_theorems/power_set}.

  \SubProofOf{thm:zfc_existence_theorems/set_of_functions} The set of single-valued functions from \( A \) to \( B \) is a subset of \( \pow(A \times B) \), hence it exists by \cref{thm:zfc_existence_theorems/set_of_relations} and \cref{thm:zfc_existence_theorems/power_set}.

  \SubProofOf{thm:zfc_existence_theorems/quotient_set} Let \( A \) be an arbitrary set and \( \cong \) be a binary relation over \( A \). Then \( A / {\cong} \) is a subset of \( \pow(A) \) and hence it exists as a consequence of the \hyperref[def:zfc/power_set]{axiom of power sets} and the \hyperref[def:zfc/specification]{axiom schema of specification} --- see \cref{thm:equivalence_partition/partition}.

  \SubProofOf{thm:zfc_existence_theorems/function_evaluation} The set \( \seq{ f_k }_{k \in \mscrK} \) exists because it is a member of \( \fun(\mscrK, \fun(A, B)) \), which set exists by \cref{thm:zfc_existence_theorems/set_of_functions}.
...
\end{theorem}
\begin{comments}
  \item Within \hyperref[def:zfc]{\logic{ZF}}, this theorem is equivalent to the \hyperref[def:zfc/choice]{axiom of choice}. We list equivalent results in \cref{thm:axiom_of_choice_equivalences}.
\end{comments}
\begin{proof}
  ...
  \begin{equation*}
    ...
    \end{aligned}
  \end{equation*}
  that sends each set in \( \mscrA \) to the corresponding subset of \( \bigcup \mscrA \). In terms of relations, we have \( (A, x) \in \gr(F) \) if and only if \( x \in A \). This is a total set-valued map because every set in \( \mscrA \) is nonempty.
...
\begin{theorem}[Surjective functions are right invertible]\label{thm:surjective_functions_are_right_invertible}
  Every \hyperref[def:function_invertibility/surjective]{surjective function} is \hyperref[def:morphism_invertibility/right]{right invertible}.
\end{theorem}
\begin{comments}
  \item Within \hyperref[def:zfc]{\logic{ZF}}, this theorem is equivalent to the \hyperref[def:zfc/choice]{axiom of choice}. We list equivalent results in \cref{thm:axiom_of_choice_equivalences}.
\end{comments}
\begin{proof}
  ...
  \ImplicationSubProof[def:zfc/choice]{the axiom of choice}[thm:epimorphisms_split_in_set]{right-invertibility} Suppose that \( f \) is surjective and thus satisfies \cref{def:function_invertibility/surjective/inverse}, stating that its inverse is total.
  ...
  For a fixed set \( A \) from the family \( \mscrA \), \( g(A) \) is a pair \( (A, x) \) with \( x \in A \). This allows us to define the choice function
  \begin{equation*}
    \begin{aligned}
...
\begin{proposition}\label{thm:function_invertibility_categorical}
  ...
  \begin{thmenum}
    \thmitem{thm:function_invertibility_categorical/empty} An \hyperref[def:set_valued_map/empty]{empty function} is always \hyperref[def:function_invertibility/injective]{injective} and \hyperref[def:morphism_cancellation/left]{left cancellable}, however only if its codomain is empty is it \hyperref[def:morphism_invertibility/left]{left invertible}, \hyperref[def:function_invertibility/surjective]{surjective}, \hyperref[def:morphism_cancellation/right]{right cancellable} and \hyperref[def:morphism_invertibility/right]{right invertible}.

    \thmitem{thm:function_invertibility_categorical/left_cancellable} A function is \hyperref[def:morphism_cancellation/left]{left cancellable} if and only if it is \hyperref[def:function_invertibility/injective]{injective}.

    \thmitem{thm:function_invertibility_categorical/right_cancellable} A function is \hyperref[def:morphism_cancellation/right]{right cancellable} if and only if it is \hyperref[def:function_invertibility/surjective]{surjective}.

    \thmitem{thm:function_invertibility_categorical/nonempty_left_invertible} A nonempty function is \hyperref[def:morphism_invertibility/left]{left invertible} if and only if it is \hyperref[def:function_invertibility/injective]{injective}.

    \thmitem{thm:function_invertibility_categorical/right_invertible} A function is \hyperref[def:morphism_invertibility/right]{right invertible} if and only if it is \hyperref[def:function_invertibility/surjective]{surjective}.

    \thmitem{thm:function_invertibility_categorical/fully_invertible} A function is \hyperref[def:morphism_invertibility/isomorphism]{fully invertible} if and only if it is \hyperref[def:function_invertibility/bijective]{bijective}.
  \end{thmenum}
\end{proposition}
\begin{comments}
...
\end{comments}
\begin{proof}
  \SubProofOf{thm:function_invertibility_categorical/empty} Let \( g: B \to C \) be the empty function to \( C \). Clearly \( B = \varnothing \).

  \begin{itemize}
    \item It is vacuously injective.

    \item For left-cancellation, note that, for any set \( A \), any function \( f: A \to B \) is empty because \( B \) is. Thus, all functions that can be composed with \( g \) on the right are empty.

    This makes \( g \) left cancellable irrespective of \( C \).

    \item For left-invertibility, note that \( h \bincirc g = \id_B \) requires the left inverse \( h: C \to B \) to be empty because \( B \) is.

    But \( h \) is well-defined if and only if \( C \) is empty.

    \item For surjectivity, clearly \( C = \varnothing \) is necessary and sufficient.

    \item For right-cancellation, note that, for any set \( D \) and any pair of functions \( h_1: C \to D \) and \( h_2: C \to D \), the equality \(h_1 \bincirc g = h_2 \bincirc g \) holds irrespective of \( g \) since both \( h_1 \bincirc g: B \to D \) and \( h_2 \bincirc g: B \to D \) are empty.

    But \( h_1 \) and \( h_2 \) may differ unless \( C \) is empty. So this is a necessary and sufficient condition for right-cancellation.

    \item For right-invertibility, note that \( g \bincirc f = \id_C \) requires the right inverse \( f: C \to B \) to be empty because \( B \) is.

    As in the case of left invertibility, \( f \) is well-defined if and only if \( C \) is empty.
  \end{itemize}

  \SubProofOf{thm:function_invertibility_categorical/left_cancellable} Empty functions are handled in \cref{thm:function_invertibility_categorical/empty}, so here we will only discuss the nonempty case.

  \SufficiencySubProof* Fix a nonempty left cancellable function \( g: B \to C \).

  Let \( g(y_1) = g(y_2) \) and let \( f: B \to B \) be \( \id_B: B \to B \) modified as to exchange \( y_1 \) and \( y_2 \). Then
  \begin{equation*}
    g(f(y_2)) = g(\id_B(y_1)) = g(\id_B(y_2)) = g(f(y_1))
  \end{equation*}
  and \( g(f(y)) = g(\id_B(y)) \) for all other \( y \) in \( B \).

  Since \( g \) is left cancellable, it follows that \( f = \id_B \). In particular, \( y_1 = y_2 \), which demonstrates that \( g \) is injective.

  \NecessitySubProof* Conversely, fix a nonempty injective function \( g: B \to C \).

  Suppose that \( g \bincirc f_1 = g \bincirc f_2 \) for some functions \( f_1: A \to B \) and \( f_2: A \to B \). From \( g(f_1(x)) = g(f_2(x)) \) it follows that \( f_1(x) = f_2(x) \), so, generalizing on \( x \), we conclude that \( f_1 = f_2 \). Thus, \( g \) is left invertible.

  \SubProofOf{thm:function_invertibility_categorical/right_cancellable} Empty functions are handled in \cref{thm:function_invertibility_categorical/empty}, so here we will only discuss the nonempty case.

  \todo{Simplify proof}

  \SufficiencySubProof* Fix a nonempty right cancellable function \( g: B \to C \).

  Aiming at a contradiction, suppose that it is not surjective.

  Let \( \widetilde f: \img g \to B \) be a \hyperref[def:function/selection]{single-valued selection} of the \hyperref[def:set_valued_map/inverse]{inverse} of \( g \). Such a selection exists by \fullref{thm:existence_of_single_valued_selections}.

  Since \( g \) is nonempty, its image is nonempty. Fix an element \( z_1 \) from this set. Fix an element \( z_2 \) of \( C \) not in \( \img g \). Let \( f_1: C \to B \) and \( f_2: C \to B \) be extensions of \( \widetilde f \) that send all elements not in \( \img g \) to \( z_1 \) and \( z_2 \), correspondingly.

  Clearly \( f_1 \bincirc g = f_2 \bincirc g \). Since \( g \) is right cancellable, this implies that \( f_1 = f_2 \), and in particular \( z_1 = z_2 \). But we have picked \( z_1 \) and \( z_2 \) from disjoint sets.

  The obtained contradiction shows that \( g \) must be surjective.

  \NecessitySubProof* Conversely, fix a nonempty surjective function \( g: B \to C \).

  Suppose that \( h_1 \bincirc g = h_2 \bincirc g \) for some functions \( h_1: C \to D \) and \( h_2: C \to D \).

  For any \( z \) in \( C \), there exists \( y \) be an element of \( B \) such that \( g(y) = z \); then \( h_1(z) = h_1(g(y)) = h_2(g(y)) = h_2(z) \). We conclude that \( h_1 = h_2 \).

  Therefore, \( g \) is right cancellative.

  \SubProofOf{thm:function_invertibility_categorical/nonempty_left_invertible}

  \SufficiencySubProof* Fix a nonempty left invertible function \( g: B \to C \).

  Fix a left inverse \( h: C \to B \). If \( g(y_1) = g(y_2) \), then \( y_1 = h(g(y_1)) = h(g(y_2)) = y_2 \), implying that \( g \) is injective.

  \NecessitySubProof* Conversely, fix a nonempty injective function \( g: B \to C \).

  Let \( \widehat h: C \to B \) be the \hyperref[def:set_valued_map/inverse]{inverse} of \( g \). It is defined only on the image of \( g \), where it is single-valued.

  Fix a value \( y_0 \) from \( B \) and let \( h: C \to B \) be the extension of \( \widehat h \) that sends all elements outside \( \img g \) to \( y_0 \). Then \( h \) is a total single-valued function such that \( h \bincirc f = \id_B \).

  \SubProofOf{thm:function_invertibility_categorical/right_invertible} Empty functions are handled in \cref{thm:function_invertibility_categorical/empty}, so here we will only discuss the nonempty case.

  \NecessitySubProof* Fix a nonempty right invertible function \( g: B \to C \).

  Let \( f: C \to B \) be a right inverse of \( g \). For any value \( z \) in \( C \), we have \( g(f(z)) = z \), so \( f(z) \) is in the preimage of \( z \) under \( g \).

  It follows that \( g \) is surjective.

  \SufficiencySubProof* For the converse, we have \fullref{thm:surjective_functions_are_right_invertible}.

  \SubProofOf{thm:function_invertibility_categorical/fully_invertible}

  \SufficiencySubProof* Fix a fully invertible function \( g: B \to C \).

  \Cref{thm:def:morphism_invertibility/left_and_right} implies that there exists a function \( f: C \to B \) that is both a left and right inverse.

  If \( g \) is empty, then \( B \) is empty, and hence so are \( f \) and \( C \). In this case the function is vacuously bijective by \cref{thm:function_invertibility_categorical/empty}.

  Otherwise, \cref{thm:function_invertibility_categorical/nonempty_left_invertible} implies that \( g \) is injective and \cref{thm:function_invertibility_categorical/right_invertible} implies that it is surjective.

  \NecessitySubProof* Fix a bijective function \( g: B \to C \).

  If \( g \) is empty, then \( C = \varnothing \) since \( g \) is surjective, and \cref{thm:function_invertibility_categorical/empty} implies that \( g \) is both left invertible and right invertible.

  Otherwise, \cref{thm:function_invertibility_categorical/nonempty_left_invertible} implies that it is left invertible.

  \Fullref{thm:surjective_functions_are_right_invertible} implies that \( g \) is right invertible. Furthermore, since \( g \) is injective, the proof can be modified as not to rely on the axiom of choice.
\end{proof}

\begin{corollary}\label{thm:category_of_sets_is_balanced}
  The \hyperref[def:category_of_small_sets]{category of small sets} is \hyperref[def:balanced_category]{balanced}.
\end{corollary}
\begin{proof}
  \Cref{thm:function_invertibility_categorical} shows that every bimorphism is both injective and surjective, hence bijective, and that every bijective function is invertible.
\end{proof}
...
\begin{theorem}[Axiom of choice equivalences]\label{thm:axiom_of_choice_equivalences}
  ...
  \begin{thmenum}[resume=thm:axiom_of_choice_equivalences]
    ...
    \thmitem{thm:axiom_of_choice_equivalences/surjective} \Fullref{thm:surjective_functions_are_right_invertible}: Every \hyperref[def:function_invertibility/surjective]{surjective function} is \hyperref[def:morphism_invertibility/right]{right invertible}.

    \thmitem{thm:axiom_of_choice_equivalences/epimorphisms} \Fullref{thm:epimorphisms_split_in_set}: Every \hyperref[def:morphism_cancellation/right]{epimorphism} in \hyperref[def:category_of_small_sets]{\( \cat{Set} \)} splits.

    \thmitem{thm:axiom_of_choice_equivalences/fully_faithful_essentially_surjective} \Fullref{thm:categorical_equivalence_characterization}: Every \hyperref[def:functor_invertibility/fully_faithful]{fully faithful} and \hyperref[def:functor_invertibility/essentially_surjective]{essentially surjective} functor induces an \hyperref[def:equivalence_of_categories]{equivalence of categories}.
...
\end{theorem}
\begin{comments}
  \item \Textcite[corr. 2]{Diaconescu1975AOC} formulated a more abstract result, which \textcite{GoodmanMyhill1978AOC} later vastly simplified for the case of intuitionistic set theory. Our proof is based on the one given by \textcite[thm. 5.3.3.6]{Mimram2020ProgramEqualsProof}, which in turn is based on the later.
\end{comments}
\begin{proof}
  ...
  \end{align*}
  where \( T \) and \( F \) are the Boolean values discussed in \cref{def:truth_value_algebra}, which we may, for simplicity, regard as \( \set{ \varnothing } \) and \( \varnothing \).
...
\begin{definition}\label{def:well_founded_relation}
  If \( \prec \) is simply a binary relation without any additional conditions imposed, we can still make sense of the chain conditions from \cref{def:chain_condition}. We will restrict ourselves to the descending chain condition --- we call the relation \( \prec \) \term{well-founded} if any of the following equivalent conditions hold:

  \begin{thmenum}
    \thmitem{def:well_founded_relation/minimal}\mcite[241]{Enderton1977SetTheory} \Cref{def:chain_condition/maximal} can be restated as follows: every nonempty subset of \( P \) has a minimal element \( m \) in the sense that there does not exist any other element \( x \) from \( P \) such that \( x \prec m \).

    \thmitem{def:well_founded_relation/stabilization} \Cref{def:chain_condition/stabilization} can be restated as follows: every nonstrict descending sequence
    ...
    \thmitem{def:well_founded_relation/infinite} \Cref{def:chain_condition/infinite} can be restarted as follows: there exists no strictly infinitely ascending descending sequence in \( P \), i.e. a sequence of the form \eqref{eq:def:well_founded_relation/stabilization} such that no two members are equal.
  \end{thmenum}
\end{definition}
...
\begin{corollary}\label{thm:simple_foundation_theorems}
  We list several statements, which are consequences of the \hyperref[def:zfc/foundation]{axiom of foundation} via \cref{thm:set_membership_is_well_founded}:
...
\end{theorem}
\begin{comments}
  \item See the comments in \cref{def:peano_arithmetic/theory/PA3} regarding variables and quantification in axiom schemas and \cref{con:induction} for a general discussion of induction.
  \item In the special case where \( X = \BbbN \), this is called \term{strong induction} compared to the usual natural number induction \eqref{eq:def:peano_arithmetic/theory/PA3}. This is discussed in \cref{con:induction/well_founded}.
\end{comments}
\begin{proof}
  ...
  Since \( v \) was chosen arbitrarily, this is true for all variable assignments in \( X \). Then the following formula is \hyperref[def:fol_semantics/model]{valid} in \( \mscrX \):
...
\end{theorem}
\begin{comments}
  \item This induction schema is called \enquote{\( \varepsilon \)-induction} because the set membership symbol \( \in \) is derived from \( \varepsilon \) as explained in \cref{rem:epsilon_and_set_membership}.

  \item See the comments in \cref{def:peano_arithmetic/theory/PA3} regarding variables and quantification in axiom schemas and \cref{con:induction} for a general discussion of induction.
\end{comments}
\begin{proof}
  Every model of \logic{ZFC} is well-founded by \( \in \) as a consequence of \cref{thm:set_membership_is_well_founded}. The corollary then follows from \fullref{thm:well_founded_induction}.
\end{proof}

\begin{definition}\label{def:godel_bernays_set_theory}
