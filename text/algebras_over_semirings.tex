...
\begin{definition}\label{def:algebra_over_semiring}\mimprovised
  ...
  \begin{thmenum}
    ...
    \thmitem{def:algebra_over_semiring/submodel} The set \( A \subseteq M \) is a \hyperref[def:fol_substructure/submodel]{submodel} of \( M \) if it is a \hyperref[def:monoid/subobject]{submodule} of \( M \) that is closed under algebra multiplication. We say that \( A \) is a \term{subalgebra}.

    As for general submodules, \cref{ex:def:semimodule/natural_number_span} shows how it is important to be unambiguous about over which semiring we consider the subalgebra.

    As a consequence of \cref{thm:fol_equational_theory_models/image}, the image of an \( R \)-algebra homomorphism is a subalgebra of its codomain.

    \thmitem{def:algebra_over_semiring/generated} For an arbitrary set \( A \), we denote the \hyperref[def:fol_generated_substructure]{generated submodel} by \( \lin{ A } \).
