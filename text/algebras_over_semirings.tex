...
\begin{defproof}
  ...
  \SubProofOf[eq:def:semiring_absorbing_element/left]{left absorption} Since \( \ast \) is homogeneous in the first argument,
  \begin{equation*}
    0_M \ast x
  ...
  \SubProofOf[eq:def:semiring_absorbing_element/left]{right absorption} Analogous.
\end{defproof}
...
\begin{definition}\label{def:algebra_over_semiring}\mimprovised
  ...
  \begin{thmenum}[resume=def:algebra_over_semiring]
    ...
    \begin{align*}
      \qforall \synx \qforall \syny (\synscalar_r(\synx \synmult \syny) &= \synscalar_r(\synx) \synmult \syny), \\
      \qforall \synx \qforall \syny (\synscalar_r(\synx \synmult \syny) &= \synx \synmult \synscalar_r(\syny)).
    \end{align*}

    \Cref{thm:bilinear_function_in_semimodule} implies that this theory also contains the absorption axioms \eqref{eq:def:semiring_absorbing_element/left} and \eqref{eq:def:semiring_absorbing_element/right}.
    ...
    \thmitem{def:algebra_over_semiring/category} For a fixed \hyperref[def:grothendieck_universe]{Grothendieck universe} \( \mscrU \) and a fixed semiring \( R \), the first-order theories of \( R \)-algebras gives rise to a \hyperref[def:fol_theory_model_functor/obj]{categories of \( \mscrU \)-small models}.
...
\end{theorem}
\begin{comments}
  \item Via \cref{rem:universal_mapping_property}, \( G \mapsto R[G] \) becomes \hyperref[def:category_adjunction]{left adjoint} to the \hyperref[def:concrete_category]{forgetful functor} which sends \( R \)-algebras to their multiplicative semigroup.
\end{comments}
\begin{proof}
