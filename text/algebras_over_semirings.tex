...
\begin{example}\label{ex:def:semigroup_algebra}
  ...
  \begin{thmenum}
    ...
    This is the conventional syntax for \hyperref[def:univariate_polynomial]{univariate polynomials} used through this monograph (see e.g. \fullref{sec:univariate_polynomials}). In fact, we define general polynomials in \cref{def:polynomial_algebra} using direct products of free monoids.
...
\begin{definition}\label{def:polynomial_algebra}\mimprovised
  ...
  \begin{thmenum}
    ...
    \thmitem{def:polynomial_algebra/polynomials} We define the \term[ru=алгебра многочленов (\cite[92]{Винберг2014КурсАлгебры}), en=polynomial algebra (\cite[473]{Bourbaki1998Algebra1to3})]{polynomial algebra} \( R[\mscrX] \) as the \hyperref[def:semigroup_algebra]{monoid algebra} \( R[\BbbN^{\oplus \mscrX}] \).
...
\begin{remark}\label{rem:polynomials_over_infinitely_many_indeterminates}
  As we saw in \cref{def:polynomial_algebra} and \fullref{thm:polynomial_algebra_universal_property}, it is possible to define polynomial algebras over infinitely many indeterminates.

  There is a problem in practice, however. Polynomials in one indeterminate, which we will call univariate based on \cref{def:operation_arity_terminology}, have a \hyperref[def:well_ordered_set]{well-ordering} on their monomials, induced by the degree of their monomials. This is defined and discussed in \fullref{sec:univariate_polynomials}.
...
\begin{definition}\label{def:polynomial_degree_terminology}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:polynomial_degree_terminology/quadratic} \term[bg=квадратна (функция) (\cite[1]{Обрешков1962ВисшаАлгебра}), ru=квадратичная (форма) (\cite[315]{Курош1968КурсВысшейАлгебры}), en=quadratic (equation) (\cite[30]{GowersEtAl2008PrincetonCompanion})]{Quadratic} when \( \deg(f) = 2 \),
    \thmitem{def:polynomial_degree_terminology/cubic} \term[bg=кубична (форма) (\cite[1]{Обрешков1962ВисшаАлгебра}), ru=кубичная (форма) (\cite[315]{Курош1968КурсВысшейАлгебры}), en=cubic (equation) (\cite[30]{GowersEtAl2008PrincetonCompanion})]{Cubic} when \( \deg(f) = 3 \),
    \thmitem{def:polynomial_degree_terminology/quartic} \term[en=quartic (equation) (\cite[30]{GowersEtAl2008PrincetonCompanion})]{Quartic} when \( \deg(f) = 4 \),
    \thmitem{def:polynomial_degree_terminology/quintic} \term[en=quintic (equation) (\cite[30]{GowersEtAl2008PrincetonCompanion})]{Quintic} when \( \deg(f) = 5 \)
  \end{thmenum}
\end{definition}
\begin{comments}
...
\begin{proof}
  ...
  \SubProofOf{thm:def:polynomial_algebra/invertible} As in \cref{thm:def:polynomial_algebra/entire}, it is sufficient to prove the statement for one indeterminate.
...
\begin{proposition}\label{thm:univariate_polynomial_sum}
  ...
  Their sum \( f(X) + g(X) \) has the following properties:
  \begin{thmenum}
    \thmitem{thm:univariate_polynomial_sum/degree} Assuming both \( f(X) \) and \( g(X) \) are nonzero, as well as their sum \( f(X) + g(X) \), we have
    \begin{equation}\label{eq:thm:univariate_polynomial_sum/degree}
      \deg(f + g) \leq \max\set{ \deg f, \deg g }.
    \end{equation}

    Furthermore, equality holds unless \( \deg f = \deg g \) and \( a_n + b_m = 0 \).
  \end{thmenum}
\end{proposition}
\begin{proof}
  Trivial.
...
\begin{proposition}\label{thm:univariate_polynomial_product}
  ...
  Note that we do not require \( f(X) \) and \( g(X) \) to be nonzero.

  Their product \( f(X) \cdot g(X) \) has the following properties:
  \begin{thmenum}
    \thmitem{thm:univariate_polynomial_product/zero_product} The product \( f(X) \cdot g(X) \) is zero if and only if both \( f(X) \) and \( g(X) \) are zero.

    \thmitem{thm:univariate_polynomial_product/leading} If the product \( a_n \cdot b_m \) of leading coefficients is nonzero (in \( R \)), the product \( f(X) \cdot g(X) \) is also nonzero (in \( R[X] \)), and its leading coefficient is \( a_n \cdot b_m \).

    \thmitem{thm:univariate_polynomial_product/invertible_leading} If \( R \) is nontrivial and if \( a_n \) or \( b_m \) is invertible, then the product \( f(X) \cdot g(X) \) is nonzero.

    \thmitem{thm:univariate_polynomial_product/degree} If the product \( f(X) \cdot g(X) \) is nonzero, then so are \( f(X) \) and \( g(X) \), and hence
    \begin{equation}\label{eq:thm:univariate_polynomial_product/degree}
      \deg (fg) \leq \deg f + \deg g.
    \end{equation}

    Equality holds if the product of leading coefficients is nonzero.
  \end{thmenum}
\end{proposition}
\begin{proof}
  All statements are consequences of the following observation.

  The coefficient \( c_{n + m} \) of \( f(X) g(X) \) is \( a_n b_m \) by definition. For \( k > n + m \), if \( i + j = k \), either \( i > n \) or \( j > m \), so \( c_k \) is zero. In other words, either \( c_{n + m} \) is zero or is last nonzero coefficient of \( f(X) g(X) \).
\end{proof}
...
\end{definition}
\begin{comments}
  \item The evaluation homomorphism defined in \fullref{thm:polynomial_algebra_universal_property} is problematic since algebraic operations are finitary by nature. This is discussed in \cref{rem:binary_operation_syntax_trees/infinite}, along with how sometimes we can make sense of infinitary algebraic operations.
\end{comments}
