...
\begin{definition}\label{def:polynomial_algebra}\mimprovised
  ...
  \begin{thmenum}
    ...
    \thmitem{def:polynomial_algebra/algebra} The \term[ru=алгебра многочленов (\cite[92]{Винберг2014})]{polynomial algebra} \( R[\mscrX] \) for the given indeterminates is the \hyperref[def:free_semimodule]{free \( R \)-semimodule} over \( \mscrM \).

    \thmitem{def:polynomial_algebra/polynomial} A \term[bg=полином (\cite[22]{ГеновМиховскиМоллов1991}), ru=многочлен (\cite[92]{Винберг2014}; многочлен/полином (\cite[sec. 11.3]{Тыртышников2007})]{polynomial} \( p \in R[\mscrX] \) is an \( R \)-linear combination of monomials, and we denote polynomials by
