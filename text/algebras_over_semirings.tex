...
\begin{concept}\label{con:evaluation_homomorphism}\mimprovised
  ...
  To each polynomial \( p \) in \( R[\mscrX] \), the homomorphism \( \Phi_M \) sends \( p \) to a function that, given a value in \( M \) for each indeterminate from \( \mscrX \), evaluates \( p \) in \( M \). We call \( \Phi_M(f) \) the \term{polynomial function} corresponding to \( p \).
...
\begin{definition}\label{def:root_of_polynomial}\mimprovised
  A \term[bg=корен (на уравнение) (\cite[2]{Обрешков1962ВисшаАлгебра}), ru=корень (многочлена) (\cite[99]{Винберг2014Алгебра}), en=root (of polynomial) (\cite[281]{Aluffi2009Algebra})]{root} in the \( R \)-module \( M \) of the polynomial \( p \) from \( R[\mscrX] \) is an evaluation \( e: \mscrX \to M \) such that \( \Phi_e(f) = 0_M \).
\end{definition}
\begin{comments}
...
\begin{remark}\label{rem:polynomials_over_infinitely_many_indeterminates}
  ...
  There is a problem in practice, however. Polynomials in one indeterminate, which we will call univariate based on \fullref{def:operation_arity_terminology}, have a \hyperref[def:well_ordered_set]{well-ordering} on their monomials, induced by the degree of their monomials. This is defined and discussed in \fullref{sec:univariate_polynomials}.
...
\begin{definition}\label{def:polynomial_degree_terminology}
  ...
  \begin{thmenum}
    \thmitem{def:polynomial_degree_terminology/constant} \term[ru=константа (\cite[118]{Тыртышников2007ЛинейнаяАлгебра})]{constant} when \( \deg(f) = 0 \) or \( f \) is the zero polynomial,
    \thmitem{def:polynomial_degree_terminology/linear} \term[bg=линейна (функция) (\cite[1]{Обрешков1962ВисшаАлгебра}), ru=линейная (форма) (\cite[315]{Курош1968ВысшаяАлгебра})]{linear} when \( \deg(f) = 1 \),
    \thmitem{def:polynomial_degree_terminology/quadratic} \term[bg=квадратна (функция) (\cite[1]{Обрешков1962ВисшаАлгебра}), ru=квадратичная (форма) (\cite[315]{Курош1968ВысшаяАлгебра}), en=quadratic (equation) (\cite[360]{Rosen2018DiscreteHandbook})]{quadratic} when \( \deg(f) = 2 \),
    \thmitem{def:polynomial_degree_terminology/cubic} \term[bg=кубична (форма) (\cite[1]{Обрешков1962ВисшаАлгебра}), ru=кубичная (форма) (\cite[315]{Курош1968ВысшаяАлгебра}), en=cubic (equation) (\cite[360]{Rosen2018DiscreteHandbook})]{cubic} when \( \deg(f) = 3 \),
    \thmitem{def:polynomial_degree_terminology/quartic} \term[en=quartic (equation) (\cite[360]{Rosen2018DiscreteHandbook})]{quartic} when \( \deg(f) = 4 \),
    \thmitem{def:polynomial_degree_terminology/quintic} \term[en=quintic (equation) (\cite[360]{Rosen2018DiscreteHandbook})]{quintic} when \( \deg(f) = 5 \)
  \end{thmenum}
\end{definition}
\begin{comments}
  \item A linear polynomial corresponds to an \hyperref[def:affine_operator]{affine function}, not a \hyperref[def:linear_function]{linear function}. Thus, we must be precise when using the word \enquote{linear}. This also leads to a distinction between homogeneous and inhomogeneous linear equations characterized in \fullref{thm:homogeneous_linear_equation}.
\end{comments}
...
\begin{proposition}\label{thm:def:polynomial_algebra}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:def:polynomial_algebra/union}\mcite[119]{Jacobson1985AlgebraI} The algebras \( R[\mscrX \cup \mscrY] \) and \( R[\mscrX][\mscrY] \) are isomorphic.
...
\begin{example}\label{ex:def:polynomial_algebra}
  ...
  \begin{thmenum}
    \thmitem{ex:def:polynomial_algebra/natural_numbers} Consider the polynomial \( f(X) \coloneqq aX^2 + bX + c \) in \( \BbbN[X] \). A function from the set \( \set{ X } \) to \( \BbbN \) corresponds to an element of \( \BbbN \), and hence evaluating the polynomial is done by simply replacing \( X \) symbolically in \( p \) and then evaluating the obtained \hyperref[rem:binary_operation_syntax_trees]{syntax tree}.
    ...
    \thmitem{ex:def:polynomial_algebra/tropical} Consider again the polynomial \( f(X) \coloneqq aX^2 + bX + c \) over \( \BbbN \), but this time evaluated over the \hyperref[def:tropical_semiring]{\( \min \)-plus semiring} \( (\BbbN \cup \set{ \infty }, \min, +) \).
...
\begin{definition}\label{def:formal_power_series}\mcite[127]{Jacobson1985AlgebraI}
...
\end{definition}
\begin{comments}
  \item \incite[exerc. B-1.7]{Rotman2015AlgebraPart1} and \incite[127]{Aluffi2009Algebra} discuss the ring of Laurent polynomials, but restrict themselves to one variable.
\end{comments}

\begin{definition}\label{def:formal_laurent_series}\mcite[425]{Jacobson1989AlgebraII}
...
\begin{proposition}\label{thm:semiring_adjunction_tower}\mcite[119]{Jacobson1985AlgebraI}
...
\begin{definition}\label{def:noncommutative_polynomial_algebra}\mcite[169]{Aluffi2009Algebra}
  We define the \term[ru=алгебра некоммутативных многочленов (\cite[92]{Шафаревич1999Алгебра})]{noncommutative polynomial algebra} \( R\braket{ \mscrX } \) over the semiring \( R \) with \hyperref[con:indeterminate]{indeterminates} \( \mscrX \) as the \hyperref[def:semigroup_algebra]{monoid algebra} \( R[\mscrX^*] \), where \( \mscrX^* \) is the
