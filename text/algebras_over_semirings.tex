...
\begin{definition}\label{def:algebra_over_semiring}\mimprovised
  Fix a \hyperref[def:semiring/commutative]{commutative semiring} \( R \). An \( R \)-\term[bg=линейна асоциативна алгебра (над поле) (\cite[298]{ГеновМиховскиМоллов1991Алгебра}), ru=алгебра с единицей (над полем) (\cite[def. 1.7.1]{Винберг2014КурсАлгебры}), en=algebra (over ring) (\cite[def. 3.1.1]{Bourbaki1998Algebra1to3})]{algebra} is an \( R \)-\hyperref[def:semimodule]{semimodule} \( M \) with an \( R \)-\hyperref[def:multilinear_function]{bilinear} vector multiplication operation. We denote this operation via either juxtaposition, \( {\cdot} \) or \( {\ast} \)\fnote{The asterisk notation generalizes \hyperref[def:semigroup_algebra]{convolution products}.}.

  As with semirings, we assume by default that a multiplicative unit exists and we generally distinguish between \term[en=unital algebra (over ring) (\cite[428]{Bourbaki1998Algebra1to3})]{unital} and \term{nonunital} \( R \)-algebras. We identify every element \( r \) of \( R \) with its inclusion \( r \cdot 1_M \) in \( M \).

  \Cref{thm:bilinear_function_in_semimodule} implies that, with such a multiplication operation, \( M \) becomes a nonassociative semiring. As discussed in \cref{rem:ring_unitality}, additional assumptions like associativity and commutativity depend on the author.
  ...
  \begin{thmenum}
    ...
    The \hyperref[def:additive_semigroup/theory]{theory of additive commutative monoids} underlying that of semirings is already part of the theory of semimodules.
    ...
    For unital \( R \)-algebras, we also need the \hyperref[def:monoid/theory]{theory of monoids} without associativity.

    \thmitem{def:algebra_over_semiring/morphism}\mcite[def. 3.1.2]{Bourbaki1998Algebra1to3} We say that the function \( \varphi: M \to N \) between \( R \)-algebras is \( R \)-\term[en=homomorphism (\cite[def. 3.1.2]{Bourbaki1998Algebra1to3})]{algebra homomorphism} if it is an \( R \)-\hyperref[def:semimodule/morphism]{semimodule homomorphism} that also preserves vector multiplication.

    These are precisely the \hyperref[def:fol_homomorphism]{first-order homomorphisms} of nonunital \( R \)-algebras, which makes the concepts from \fullref{sec:first_order_structures} applicable.

    If both \( M \) and \( N \) have multiplicative units and if \( \varphi \) preserves them, we call \( \varphi \) \term{unital}.

    \thmitem{def:algebra_over_semiring/category} As with semirings, for a fixed semiring \( R \), the first-order theories of \( R \)-algebras gives rise to several \hyperref[def:fol_theory_model_functor/obj]{categories of small models}.

    These categories differ by whether the algebras and/or the homomorphisms are unital.

    We list several properties:
    \begin{itemize}
      \item The various theories of \( R \)-algebras are equational and thus have \hyperref[def:fol_free_term_model]{free term models}; \Fullref{thm:fol_free_term_model_universal_property} provides \hyperref[def:adjoint_functor]{left adjoints} to the \hyperref[def:fol_theory_forgetful_functor/set]{forgetful functors} from \( R \)-algebras to their underlying sets.
    ...
    \thmitem{def:algebra_over_semiring/subobject} An \( R \)-\term[en=subalgebra (over ring) (\cite[429]{Bourbaki1998Algebra1to3})]{subalgebra} of \( M \) is an \( R \)-\hyperref[def:semimodule/subobject]{sub-semimodule} that is also closed under vector multiplication.
    ...
    \thmitem{def:algebra_over_semiring/generated}\mcite[429]{Bourbaki1998Algebra1to3} We define the \term{generated \( R \)-subalgebra}of the a subset \( A \) of \( M \) as the intersection of \( R \)-subalgebras of \( M \) containing \( A \).

    This coincides with the first-order submodel of \( M \) \hyperref[def:fol_generated_substructure]{generated} by \( A \).

    \thmitem{def:algebra_over_semiring/trivial} The \hyperref[def:semimodule/trivial]{zero \( R \)-semimodule} is vacuously an algebra, and we call it the \term{zero \( R \)-algebra}.

    \thmitem{def:algebra_over_semiring/ideal}\mcite[430]{Bourbaki1998Algebra1to3} We call the \( R \)-subalgebra \( I \) of \( M \) a (left, right or two-sided) \( R \)-\term[en=ideal (\cite[430]{Bourbaki1998Algebra1to3})]{algebra ideal} if it is closed under (left, right or two-sided) vector multiplication with members of \( M \).

    \Cref{thm:algebra_over_semiring_ideal_is_submodule} implies that, if \( M \) is unital, \( I \) is automatically an \( R \)-sub-semimodule if it is a \hyperref[def:semiring_ideal]{semiring ideal}\fnote{Generally, unless \( M \) is not associative, we cannot freely use the definitions and theorems from \fullref{sec:semiring_ideals}.} (i.e. if it is closed under addition and multiplication with members of \( M \)). For associative unital \( R \)-algebras, the distinction between semiring ideals and \( R \)-algebra ideals becomes meaningless.

    Different approaches towards defining algebra ideals are reviewed in \Cref{rem:ring_unitality/algebras}.
    ...
    \thmitem{def:algebra_over_semiring/associative}\mcite[def. 3.1.1]{Bourbaki1998Algebra1to3} We call an \( R \)-algebra \term{associative} if its multiplication is associative.

    An associative unital \( R \)-algebra is itself a semiring and can be regarded as an extension of \( R \).

    \thmitem{def:algebra_over_semiring/commutative}\mcite[def. 3.1.1]{Bourbaki1998Algebra1to3} Similarly to general (semi)rings, we call an \( R \)-algebra \term{commutative} if its multiplication is commutative.
  \end{thmenum}
\end{definition}
\begin{comments}
  \item We have adjusted this definition based on \cref{def:algebra_over_ring} for algebras over rings. The references are for algebras over commutative rings.
\end{comments}

\begin{proposition}\label{thm:algebra_over_semiring_ideal_is_submodule}
  In a unital \( R \)-\hyperref[def:algebra_over_semiring]{algebra} \( M \), any \hyperref[def:semiring_ideal]{semiring ideal} \( I \) of \( M \) is an \( R \)-submodule.
\end{proposition}
\begin{comments}
  \item Nota that we have used the definition of semiring ideals without assuming that \( M \) is associative.
\end{comments}
\begin{proof}
  If \( x \) belongs to \( I \), then so does
  \begin{equation*}
    r \cdot x
...
\begin{proposition}\label{thm:algebra_over_semiring_via_homogeneity}
  In a (possibly nonunital) \( R \)-\hyperref[def:algebra_over_semiring]{algebra} \( M \), scalar and vector multiplication satisfy
  \begin{equation}\label{eq:thm:algebra_over_semiring_via_homogeneity}
    r \cdot (x \ast y) = (r \cdot x) \ast y = x \ast (r \cdot y).
  \end{equation}

  Conversely, if \( M \) is both an \( R \)-module and a semiring \eqref{eq:thm:algebra_over_semiring_via_homogeneity}, then \( M \) is a unital associative \( R \)-algebra.
\end{proposition}
\begin{comments}
  \item With the same proof, this also holds if \( R \) and \( M \) are \hyperref[def:ring]{rings}.
\end{comments}
\begin{proof}
  \SufficiencySubProof Suppose that \( M \) is a unital \( R \)-algebra. Then
  \begin{equation*}
    r \cdot (x \ast y)
    \reloset {\eqref{eq:def:homogeneous_function}} =
    (r \cdot x) \ast y
  ...
  \NecessitySubProof Conversely, if \( M \) is an \( R \)-semimodule and a semiring satisfying \eqref{eq:thm:algebra_over_semiring_via_homogeneity}, then \( {\ast} \) is \( R \)-bilinear because it is by definition additive and
  \begin{align*}
    (r \cdot x) \ast y
    &\reloset {\eqref{eq:thm:algebra_over_semiring_via_homogeneity}} =
    r \cdot (x \ast y)
    \\
    x \ast (r \cdot y)
    &\reloset {\eqref{eq:thm:algebra_over_semiring_via_homogeneity}} =
...
\begin{proposition}\label{thm:algebra_over_semiring_via_homomorphism}
  In an associative \( R \)-\hyperref[def:algebra_over_semiring]{algebra} \( M \), the inclusion \( \varphi(r) \coloneqq r \cdot 1_M \) is a \hyperref[def:semiring/morphism]{semiring homomorphism}.

  Conversely, given unital semirings \( R \) and \( M \) and a homomorphism \( \varphi: R \to M \), the operation
  \begin{equation}\label{eq:thm:algebra_over_semiring_via_homomorphism}
    r \cdot x \coloneqq \varphi(r) \ast x
  \end{equation}
  makes \( M \) an \( R \)-\hyperref[def:semimodule]{semimodule} (and hence a unital associative \( R \)-algebra).
\end{proposition}
\begin{comments}
  \item With the same proof, this also holds if \( R \) and \( M \) are \hyperref[def:ring]{rings}.
\end{comments}
\begin{proof}
  \SufficiencySubProof Suppose that \( M \) is a unital \( R \)-algebra. Let \( \varphi(r) \coloneqq r \cdot 1_M \). Then
  \begin{align*}
    \varphi(0_R)
    &=
    0_R \cdot 1_M
  ...
  and
  \begin{equation*}
    \underbrace{(r \cdot_R s) \cdot 1_M}_{\varphi(r \cdot_R s)}
    \reloset {\eqref{eq:def:semimodule/operation/action/compatibility}} =
    r \cdot (s \cdot 1_M)
    \reloset {\eqref{eq:def:semigroup_neutral_element/left}} =
    1_M \ast (r \cdot (s \cdot 1_M))
    \reloset {\eqref{eq:def:homogeneous_function}} =
    \underbrace{(r \cdot 1_M) \ast (s \cdot 1_M)}_{\varphi(r) \ast \varphi(s)}.
  \end{equation*}

  \NecessitySubProof Suppose that \( \varphi: R \to M \) is a semiring homomorphism. Define a scalar product on \( M \) via \eqref{eq:thm:algebra_over_semiring_via_homomorphism}. This makes \( M \) an \( R \)-semimodule because
  \begin{align*}
    1_R \cdot x
    &=
    \varphi(1_R) \ast x
    \reloset {\eqref{eq:def:pointed_set/morphism}} =
    1_M \ast x
    \reloset {\eqref{eq:def:semigroup_neutral_element/left}} =
    x,
    \\
    (r \cdot_R s) \cdot x
    &=
    \varphi(r \cdot_R s) \ast x
    \reloset {\eqref{eq:def:semigroup/morphism}} =
    (\varphi(r) \ast \varphi(s)) \ast x
    ...
    \varphi(r) \ast x +_M \varphi(r) \ast y
    =
...
\begin{proposition}\label{thm:algebra_over_semiring_is_semimodule}
  If \( S \) is a \hyperref[def:semiring/subobject]{sub-semiring} of the commutative semiring \( R \), then \( R \) is an \( S \)-\hyperref[def:algebra_over_semiring]{algebra} with scalar multiplication given by multiplication in \( R \).
\end{proposition}
\begin{proof}
...
\begin{proposition}\label{thm:semiring_is_natural_number_algebra}
  The \hyperref[def:category]{categories} \hyperref[def:semiring/category]{of semirings} and \hyperref[def:semimodule/category]{of associative \( \BbbN \)-algebras} are \hyperref[def:isomorphism_of_categories]{isomorphic}.
\end{proposition}
\begin{comments}
...
\begin{definition}\label{def:semigroup_algebra}
  ...
  We define the corresponding \term{semiring algebra} \( R[G] \) as the \hyperref[def:free_semimodule]{free \( R \)-semimodule} on \( G \) endowed with a vector multiplication operation \( \ast \) called \term[ru=свёртка (\cite[62]{Боровков1999ТеорияВероятностей}) / конволюция (\cite[179]{ИоффеТихомиров1974ЭкстремальныеЗадачи}), en=\( \ast \)-convolution (\cite[29]{Golan1999Semirings})]{convolution product} and defined as follows:
  ...
  This algebra is associative, but not necessarily unital. It also comes with the embedding
...
\begin{theorem}[Semigroup algebra universal property]\label{thm:semigroup_algebra_universal_property}\mcite[381]{Knapp2016BasicAlgebra}
  The \hyperref[def:semigroup_algebra]{semigroup algebra} \( R[G] \) is the unique up to a unique isomorphism associative nonunital \( R \)-\hyperref[def:algebra_over_semiring]{algebra} \( R \) that satisfies the following \hyperref[con:universal_property]{universal property}:
  \begin{displayquote}
    For every associative nonunital \( R \)-algebra \( M \) and every \hyperref[def:semigroup/morphism]{semigroup homomorphism} \( e: G \to M \) into the \hyperref[def:algebra_over_semiring/monoids]{multiplicative semigroup} of \( M \), there exists a unique \hyperref[def:algebra_over_semiring]{\( R \)-algebra homomorphism} \( \Phi_e: R[G] \to M \) such that the following diagram commutes:
...
\end{theorem}
\begin{comments}
  \item \Cref{thm:hom_adjunction_from_universal_morphism/unit} implies that \( G \mapsto R[G] \) is \hyperref[def:adjoint_functor]{left adjoint} to the \hyperref[con:forgetful_functor]{forgetful functor} which sends associative \( R \)-algebras to their multiplicative semigroup.
\end{comments}
\begin{proof}
...
\begin{theorem}[Polynomial algebra universal property]\label{thm:polynomial_algebra_universal_property}
  Fix a \hyperref[def:semiring/commutative]{commutative semiring} \( R \) and a set \( \mscrX \) of indeterminates. The \hyperref[def:polynomial_algebra]{polynomial algebra} \( R[\mscrX] \) is the unique up to a unique isomorphism of \hyperref[def:algebra_over_semiring/commutative]{commutative associative \( R \)-algebras} that satisfies the following \hyperref[con:universal_property]{universal property}:
  \begin{displayquote}
    For every commutative associative \( R \)-algebra \( M \) and every function \( e: \mscrX \to M \), there exists a unique \hyperref[def:algebra_over_semiring/morphism]{\( R \)-algebra homomorphism} \( \Phi_e: R[\mscrX] \to M \) such that the following diagram commutes:
    \begin{equation*}
      \begin{aligned}
...
\end{theorem}
\begin{comments}
  \item \Cref{thm:hom_adjunction_from_universal_morphism/unit} implies that \( R[\anon*] \) is \hyperref[def:adjoint_functor]{left adjoint} to the \hyperref[con:forgetful_functor]{forgetful functor} sending \hyperref[def:algebra_over_semiring/commutative]{commutative associative \( R \)-algebras} to their underlying set. The action of \( R[\anon*] \) on morphisms is provided by \( \Phi \).
...
\begin{corollary}\label{thm:entire_polynomial_algebra}
  The univariate \hyperref[def:polynomial_algebra]{polynomial algebra} \( R[X] \) is \hyperref[def:entire_semiring]{entire} if and only if \( R \) is entire.
\end{corollary}
\begin{proof}
...
\begin{proposition}\label{thm:univariate_polynomial_algebra_cardinality}
  For the univariate \hyperref[def:polynomial_algebra]{polynomial algebra} \( R[X] \), we have
  \begin{equation*}
    \card(R[X]) = \max\set{ \aleph_0, \card(R) }.
  \end{equation*}
\end{proposition}
\begin{proof}
  Let \( F_k \) be the set of polynomials with exactly \( k \) nonzero coefficients. Then \( R[X] \) is the disjoint union
  \begin{equation*}
    R[X] = \bigcup_{k=0}^\infty F_k.
  \end{equation*}
  ...
  \begin{itemize}
    \item If \( R \) is finite, then \( \card(F_k) \) is also finite for every \( k \), and \cref{thm:cardinality_of_disjoint_union} implies that
    \begin{equation*}
      \card(R[X])
      \leq
      \aleph_0 \cdot \aleph_0
      \reloset {\ref{thm:simplified_cardinal_arithmetic/infinite}} =
      \aleph_0.
    \end{equation*}

    On the other hand, \cref{thm:countably_infinite_disjoin_union_of_nonempty_sets} implies that \( \card(R[X]) \geq \aleph_0 \).

    \item If \( R \) is infinite, we instead have
    \begin{equation*}
      \card(R)^k = \begin{cases}
        1,       &k = 0, \\
        \card(R) &k > 0.
      \end{cases}
    \end{equation*}

    Thus, \cref{thm:cardinality_of_disjoint_union} implies that
    \begin{equation*}
      \card(R[X] \setminus \set{ F_0 }) = \aleph_0 \cdot \card(R) = \max\set{ \aleph_0, \card(R) } = \card(R)
    \end{equation*}
    and hence
    \begin{equation*}
      \card(R[X]) = \card(F_0 \cup R[X] \setminus \set{ F_0 }) = 1 + \card(R) = \card(R).
    \end{equation*}
  \end{itemize}
...
\begin{proposition}\label{thm:generators_via_polynomials}
  For a set \( A \) in an associative \( R \)-\hyperref[def:algebra_over_semiring]{algebra} \( M \), the \( R \)-subalgebra \hyperref[def:algebra_over_semiring/generated]{generated} by \( A \) coincides with the set
  \begin{equation*}
...
\begin{comments}
  ...
  \item Compare this result to \cref{thm:generated_submodule} for linear spans in (semi)modules.
\end{comments}
\begin{proof}
  Similar to \cref{thm:generated_submodule}.
\end{proof}
...
\begin{theorem}[Noncommutative polynomial algebra universal property]\label{thm:noncommutative_polynomial_algebra_universal_property}
  Fix a \hyperref[def:semiring/commutative]{commutative semiring} \( R \) and a set \( \mscrX \) of indeterminates. The \hyperref[def:noncommutative_polynomial_algebra]{noncommutative polynomial algebra} \( R\braket{\mscrX} \) is the unique up to a unique isomorphism associative \hyperref[def:algebra_over_semiring]{\( R \)-algebra} that satisfies the following \hyperref[con:universal_property]{universal property}:
  \begin{displayquote}
    For every associative \( R \)-algebra \( M \) and every function \( e: \mscrX \to M \), there exists a unique \( R \)-algebra homomorphism \( \Phi_e: R\braket{\mscrX} \to M \) such that the following diagram commutes:
    \begin{equation*}
      \begin{aligned}
