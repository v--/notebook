...
\begin{proposition}\label{thm:def:polynomial_algebra}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:def:polynomial_algebra/cardinality} If \( \mscrX \) is finite, the \hyperref[thm:cardinality_existence]{cardinality} of \( R[\mscrX] \) is at most \( \max\set{ \aleph_0, \card(R) } \).
...
\begin{proof}
  ...
  \SubProofOf{thm:def:polynomial_algebra/cardinality} Note that \( R[\mscrX] \) is defined as the \hyperref[def:semigroup_algebra]{monoid algebra} \( R[\BbbN^{\oplus \mscrX}] \), which is in turn the \hyperref[def:free_semimodule]{free \( R \)-semimodule} on \( \BbbN^{\oplus \mscrX} \).

  But \( \BbbN^{\oplus \mscrX} \) is itself the free \( \BbbN \)-semimodule on \( \mscrX \). Since \( \mscrX \) is by assumption finite, \cref{thm:free_semimodule_cardinality} implies that \( \BbbN^{\oplus \mscrX} \) has cardinality at most \( \aleph_0 \).

  By the same proposition, for \( R[\mscrX] \) itself, the cardinality is at most \( \max\set{ \aleph_0, \card(R) } \).
