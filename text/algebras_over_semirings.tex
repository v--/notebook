...
\begin{definition}\label{def:polynomial_algebra}\mimprovised
  ...
  \begin{thmenum}
    \thmitem{def:polynomial_algebra/monomials} Let \( \mscrM \) be the \hyperref[def:free_semimodule]{free \( R \)-semimodule} over \( \mscrX \), written \hyperref[rem:additive_semigroup]{multiplicatively}. We will call the members of \( \mscrM \) \term[bg=едночлен (\cite[page 1 in \textparagraph 8]{НеновБожилов2007}), ru=одночлен/моном (\cite[sec. 11.3]{Тыртышников2007})]{monomials}. Using a \hyperref[def:multi_index]{multi-index} \( \gamma \) over \( \mscrX \), every monomial can be written as
    \begin{equation*}
    ...
    \thmitem{def:polynomial_algebra/polynomial} A \term[bg=полином (\cite[page 1 in \textparagraph 8]{НеновБожилов2007}), ru=многочлен/полином (\cite[sec. 11.3]{Тыртышников2007})]{polynomial} \( p \in R[\mscrX] \) is an \( R \)-linear combination of monomials, and we denote polynomials by
    \begin{equation}\label{eq:def:polynomial_algebra/p}
      p(\mscrX) = \sum_\gamma a_\gamma \prod_{X \in \mscrX} X^{\gamma_X}.
    \end{equation}

    We call \( a_\gamma \) the \term[bg=коефициенти (\cite[page 1 in \textparagraph 8]{НеновБожилов2007})]{coefficients} of the polynomial. We use the components of the multi-index as powers in the monomials, but we use \( \gamma \) itself as an index for the coefficient \( a_\gamma \). Unfortunately, multi-indices are sometimes confusing, but often their brevity outweighs the possible confusion.
