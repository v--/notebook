...
\begin{definition}\label{def:semigroup_algebra}\mcite[29]{Golan2010}
  ...
  We define the corresponding \term{semiring algebra} or \term[ru=групповая алгебра (\cite[504]{Винберг2014}), en=group ring (\cite[127]{Aluffi2009})]{semigroup semiring} \( R[G] \) as the \hyperref[def:free_semimodule]{free semimodule} endowed with a vector multiplication operation \( \odot \) called \term[ru=свёртка (\cite[62]{Боровков1999Вероятности}) / конволюция (\cite[179]{ИоффеТихомиров1974})]{convolution product} and defined as follows:
  \begin{equation}\label{eq:def:semigroup_algebra/multiplication}
    \parens[\Big]{ \Bbbsum_{a \in G} x_a \cdot a } \odot \parens[\Big]{ \Bbbsum_{b \in G} y_b \cdot b }
    \coloneqq
    \Bbbsum_{g \in G} \parens[\Big]{ \sum_{ab = g} x_a y_b } \cdot g.
  \end{equation}
...
\end{definition}
\begin{comments}
  \item More simply put, the coefficient before \( g \) in the product is the sum of products \( x_a y_b \) where \( ab = g \). This definition is justified in our proof of \fullref{thm:semigroup_algebra_universal_property}.
...
\begin{proof}
  ...
  \begin{equation*}
    \begin{aligned}
      &\Phi: R[G] \to M, \\
      &\Phi\parens[\Big]{ \Bbbsum_{a \in G} x_a \cdot a } \coloneqq \sum_{a \in G} x_a \cdot_M \varphi(a).
    \end{aligned}
  \end{equation*}

  We will show that \( \Phi \) preserves products, that is, that \( \Phi \) also a homomorphism of \( R \)-algebras.

  Fix some vector \( \Bbbsum_{b \in G} y_b \cdot b \). We will use induction on the number of nonzero coefficients in this sum to show that
  \begin{equation}\label{eq:thm:semigroup_algebra_universal_property/hypothesis}
    \Phi\parens[\Big]{ \Bbbsum_{a \in G} x_a \cdot a } \odot_M \Phi\parens[\Big]{ \Bbbsum_{b \in G} y_b \cdot b }
    =
    \Bbbsum_{g \in G} \parens[\Big]{ \sum_{a b = g} x_a y_b } \cdot_M \varphi(g).
  \end{equation}

  \begin{itemize}
    \item If \( y_b = 0 \) for every \( b \in G \), then \( x_a y_b = 0 \) and thus the product \eqref{eq:def:semigroup_algebra/multiplication} with any other vector is necessarily zero. Thus,
    \begin{equation*}
      \Phi\parens[\Big]{ \Bbbsum_{a \in G} x_a \cdot a } \odot_M \Phi\parens[\Big]{ \Bbbsum_{b \in G} \underbrace{y_b}_{\mathclap{\T*{zero}}} \cdot b }
      =
      \Bbbsum_{g \in G} \underbrace{\parens[\Big]{ \sum_{a b = g} x_a y_b }}_{\T{zero}} \cdot_M \varphi(g).
    \end{equation*}
    ...
    \begin{equation*}
      \sum_{k=1}^{m+1} y_{b_k} \cdot b_k.
    \end{equation*}

    Since \( \odot_M \) is additive in its second argument, we have
    \begin{align}
      &\phantom{{}={}}
      \Phi\parens[\Big]{ \Bbbsum_{a \in G} x_a \cdot a } \odot_M \Phi\parens[\Big]{ \Bbbsum_{b=1}^{m+1} y_{b_k} \cdot b_k }
      = \nonumber \\ &=
      \Phi\parens[\Big]{ \Bbbsum_{a \in G} x_a \cdot a } \odot_M \Phi\parens[\Big]{ \Bbbsum_{b=1}^m y_{b_k} \cdot b_k } + \Phi\parens[\Big]{ \Bbbsum_{a \in G} x_a \cdot a } \odot_M \Phi\parens[\Big]{ y_{b_{m+1}} \cdot b_{m+1} }. \label{eq:thm:semigroup_algebra_universal_property/split}
    \end{align}

    We will study the second term. Because \( \odot_M \) is homogeneous in both arguments, and since \( \Phi \) is also homogeneous, we have
    \begin{align*}
      &\phantom{{}={}}
      \Phi\parens[\Big]{ \Bbbsum_{a \in G} x_a \cdot a } \odot_M \Phi\parens[\Big]{ y_{b_{m+1}} \cdot b_{m+1} }
      = \\ &=
      y_{b_{m+1}} \cdot \bracks[\Bigg]{ \Phi\parens[\Big]{ \Bbbsum_{a \in G} x_a \cdot a } \odot_M \Phi\parens[\Big]{ b_{m+1} } }
      = \\ &=
      \Phi\parens[\Big]{ \Bbbsum_{a \in G} x_a y_{b_{m+1}} \cdot a } \odot_M \Phi\parens[\Big]{ b_{m+1} }
      = \\ &=
      \Bbbsum_{a \in G} x_a y_{b_{m+1}} \cdot_M \underbrace{\varphi(a) \odot_M \varphi(b_{m+1})}_{\varphi(ab_{m+1})}.
    \end{align*}
    ...
    \begin{equation}\label{eq:thm:semigroup_algebra_universal_property/second}
      \Bbbsum_{g \in G} \parens[\Big]{ \sum_{ab_{m+1} = g} x_a y_{b_{m+1}} } \cdot_M \varphi(g).
    \end{equation}
    ...
    \begin{align*}
      &\phantom{{}={}}
      \Phi\parens[\Big]{ \Bbbsum_{a \in G} x_a \cdot a } \odot_M \Phi\parens[\Big]{ \Bbbsum_{b=1}^m y_{b_k} \cdot b_k } + \Phi\parens[\Big]{ \Bbbsum_{a \in G} x_a \cdot a } \odot_M \Phi\parens[\Big]{ y_{b_{m+1}} \cdot b_{m+1} }
      = \\ &=
      \Bbbsum_{g \in G} \parens[\Big]{ \sum_{\substack{a b_k = g \\ 1 \leq k \leq m}} x_a y_{b_k} } \cdot_M \varphi(g) + \Bbbsum_{g \in G} \parens[\Big]{ \sum_{ab_{m+1} = g} x_a y_{b_{m+1}} } \cdot_M \varphi(g)
      = \\ &=
      \Bbbsum_{g \in G} \parens[\Big]{ \sum_{\substack{a b_k = g \\ 1 \leq k \leq m}} x_a y_{b_k} + \sum_{ab_{m+1} = g} x_a y_{b_{m+1}} } \cdot_M \varphi(g)
      = \\ &=
      \Bbbsum_{g \in G} \parens[\Big]{ \sum_{\substack{a b_k = g \\ 1 \leq k \leq m + 1}} x_a y_{b_k} } \cdot_M \varphi(g)
      = \\ &=
      \Bbbsum_{g \in G} \parens[\Big]{ \sum_{a b = g} x_a y_b } \cdot_M \varphi(g).
    \end{align*}
  \end{itemize}
...
\begin{example}\label{ex:def:semigroup_algebra}
  ...
  \begin{thmenum}
    ...
    \thmitem{ex:def:semigroup_algebra/z} The group algebra of the \hyperref[def:semimodule_direct_sum]{direct sum} \( \BbbN^n \) over \( R \) is the \hyperref[def:polynomial_algebra]{polynomial algebra} in \( n \) indeterminates over \( R \).
...
\begin{definition}\label{def:polynomial_algebra}\mimprovised
  ...
  \begin{thmenum}
    ...
    \thmitem{def:polynomial_algebra/algebra} We define the \term[ru=алгебра многочленов (\cite[92]{Винберг2014})]{polynomial algebra} \( R[\mscrX] \) as the \hyperref[def:semigroup_algebra]{monoid semiring} \( R[\BbbN^{\oplus \mscrX}] \).
...
\begin{definition}\label{def:formal_power_series}\mimprovised
  If we extend the concept of \hyperref[def:polynomial_algebra]{polynomials} to allow countably many monomials with nonzero coefficients, we obtain a set \( R\Bracks{\mscrX} \) which we call the \term{formal power series} over \( R \) with indeterminates from the set \( \mscrX \).
