...
\begin{comments}
  ...
  \item \Fullref{ex:def:semigroup_algebra/cyclic} shows how \eqref{eq:def:semigroup_algebra/multiplication} is merely a formalization of bracket expansion with grouping.
...
\begin{example}\label{ex:def:semigroup_algebra}
  ...
  \begin{thmenum}
    ...
    This is the conventional syntax for \hyperref[def:univariate_polynomial]{univariate polynomials} used through this monograph (see e.g. \fullref{sec:univariate_polynomials}). In fact, we define general polynomials in \fullref{def:polynomial_algebra} using direct products of free monoids.
    ...
    \thmitem{ex:def:semigroup_algebra/trivial} In the special case of the trivial group \( C_1 = \set{ e } \), by \fullref{ex:def:semigroup_algebra/cyclic}, all powers of \( X \) in \( R[C_1] \) are identified, and thus the convolution product reduces to the usual product in \( R \).
    ...
    \thmitem{ex:def:semigroup_algebra/noncommutative} The (semi)group is not necessarily commutative. For example, we define noncommutative polynomials in \fullref{def:noncommutative_polynomial_algebra}. In the univariate case, it coincides with ordinary polynomials, but for bivariate noncommutative polynomials \( XY \) and \( YX \) are distinct.

    We define the quaternion algebra in \fullref{def:quaternion_algebra} using noncommutative polynomials.
  \end{thmenum}
\end{example}
...
\begin{definition}\label{def:polynomial_algebra}\mimprovised
  ...
  \begin{thmenum}
    ...
    As discussed in \fullref{rem:free_commutative_monoid_as_quotient}, we can regard elements of this monoid as congruence classes of strings of indeterminates. We call these congruence classes \term[ru=одночлен/моном (\cite[\S 11.3]{Тыртышников2007ЛинейнаяАлгебра})]{monomials}.
    ...
    \thmitem{def:polynomial_algebra/polynomials} We define the \term[ru=алгебра многочленов (\cite[92]{Винберг2014КурсАлгебры}), en=polynomial algebra (\cite[473]{Bourbaki1998Algebra1to3})]{polynomial algebra} \( R[\mscrX] \) as the \hyperref[def:semigroup_algebra]{monoid semiring} \( R[\BbbN^{\oplus \mscrX}] \).
...
\begin{concept}\label{con:evaluation_homomorphism}\mimprovised
  ...
  In the case of \( n \) indeterminates, \( \Phi_M(f) \) is a function from \( M^n \) to \( M \). Polynomials and their functions are thus an instance of the syntax-semantics duality described in \fullref{con:syntax_semantics_duality}.
...
\begin{example}\label{ex:def:root_of_polynomial}
  We list examples of \hyperref[def:root_of_polynomial]{polynomial roots}:
  \begin{thmenum}
    \thmitem{ex:def:root_of_polynomial/infinite}\mcite{MathCounterExamples:infinitely_many_roots} Consider the \hyperref[thm:boolean_algebra_of_subsets]{power set Boolean algebra} \( \pow(A) \) of some set \( A \).

    \Fullref{thm:boolean_algebra_as_ring} allows us to regard it as a \hyperref[def:boolean_ring]{Boolean ring}, with \hyperref[def:symmetric_difference]{symmetric difference} as addition and intersection as multiplication.
...
\begin{remark}\label{rem:zero_polynomial_degree}
  In \fullref{def:polynomial_degree} we have left the degree of the zero polynomial \hyperref[con:undefinedness]{undefined}.
  ...
  \begin{thmenum}
    ...
    \incite[155]{Knapp2016BasicAlgebra} defines the degree as \( -\infty \) for the rest of the section, but otherwise leaves the degree undefined.

    This convention is convenient when comparing degrees.

    \thmitem{rem:zero_polynomial_degree/minus_one} The degree of \( f(X) = 0 \) is defined as \( -1 \) in
...
\begin{definition}\label{def:polynomial_degree_terminology}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:polynomial_degree_terminology/linear} \term[bg=линейна (функция) (\cite[1]{Обрешков1962ВисшаАлгебра}), ru=линейная (форма) (\cite[315]{Курош1968КурсВысшейАлгебры})]{Linear} when \( \deg(f) = 1 \),
    \thmitem{def:polynomial_degree_terminology/quadratic} \term[bg=квадратна (функция) (\cite[1]{Обрешков1962ВисшаАлгебра}), ru=квадратичная (форма) (\cite[315]{Курош1968КурсВысшейАлгебры}), en=quadratic (equation) (\cite[360]{RosenEtAl2018DiscreteMathematicsHandbook})]{Quadratic} when \( \deg(f) = 2 \),
    \thmitem{def:polynomial_degree_terminology/cubic} \term[bg=кубична (форма) (\cite[1]{Обрешков1962ВисшаАлгебра}), ru=кубичная (форма) (\cite[315]{Курош1968КурсВысшейАлгебры}), en=cubic (equation) (\cite[360]{RosenEtAl2018DiscreteMathematicsHandbook})]{Cubic} when \( \deg(f) = 3 \),
    \thmitem{def:polynomial_degree_terminology/quartic} \term[en=quartic (equation) (\cite[360]{RosenEtAl2018DiscreteMathematicsHandbook})]{Quartic} when \( \deg(f) = 4 \),
    \thmitem{def:polynomial_degree_terminology/quintic} \term[en=quintic (equation) (\cite[360]{RosenEtAl2018DiscreteMathematicsHandbook})]{Quintic} when \( \deg(f) = 5 \)
  \end{thmenum}
\end{definition}
\begin{comments}
...
\begin{definition}\label{def:formal_power_series}\mcite[127]{Jacobson1985BasicAlgebraI}
