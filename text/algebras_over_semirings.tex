...
\begin{proof}
  ...
  \begin{equation*}
    \begin{aligned}
      &\Phi: R[G] \to M, \\
      &\Phi\parens*{ \Bbbsum_{x \in G} a_x \cdot x } \coloneqq \sum_{x \in G} a_x \cdot_M \varphi(x).
    \end{aligned}
  \end{equation*}
  ...
  \begin{equation}\label{eq:thm:semigroup_algebra_universal_property/hypothesis}
    \Phi\parens*{ \Bbbsum_{x \in G} a_x \cdot x } \ast_M \Phi\parens*{ \Bbbsum_{y \in G} b_y \cdot y }
    =
  ...
  \begin{itemize}
    ...
    \begin{equation*}
      \Phi\parens[\Big]{ \Bbbsum_{x \in G} a_x \cdot x } \ast_M \Phi\parens[\Big]{ \Bbbsum_{y \in G} \underbrace{b_y}_{\mathclap{\T*{zero}}} \cdot y }
      =
    ...
    \begin{align}
      &\phantom{{}={}}
      \Phi\parens[\Big]{ \Bbbsum_{x \in G} a_x \cdot x } \ast_M \Phi\parens[\Big]{ \Bbbsum_{i=1}^{m+1} b_{y_i} \cdot y_i }
      = \nonumber \\ &=
      \Phi\parens[\Big]{ \Bbbsum_{x \in G} a_x \cdot x } \ast_M \Phi\parens[\Big]{ \Bbbsum_{i=1}^m b_{y_i} \cdot y_i } + \Phi\parens[\Big]{ \Bbbsum_{x \in G} a_x \cdot x } \ast_M \Phi\parens[\Big]{ b_{y_{m+1}} \cdot y_{m+1} }. \label{eq:thm:semigroup_algebra_universal_property/split}
    \end{align}

    We will study the second term. Because \( \ast_M \) is homogeneous in both arguments, and since \( \Phi \) is also homogeneous, we have
    \begin{align*}
      &\phantom{{}={}}
      \Phi\parens[\Big]{ \Bbbsum_{x \in G} a_x \cdot x } \ast_M \Phi\parens[\Big]{ b_{y_{m+1}} \cdot y_{m+1} }
      = \\ &=
      b_{y_{m+1}} \cdot \bracks[\Bigg]{ \Phi\parens[\Big]{ \Bbbsum_{x \in G} a_x \cdot x } \ast_M \Phi\parens[\Big]{ y_{m+1} } }
      = \\ &=
      \Phi\parens[\Big]{ \Bbbsum_{x \in G} a_x b_{y_{m+1}} \cdot x } \ast_M \Phi\parens[\Big]{ y_{m+1} }
      = \\ &=
    ...
    \begin{align*}
      &\phantom{{}={}}
      \Phi\parens[\Big]{ \Bbbsum_{x \in G} a_x \cdot x } \ast_M \Phi\parens[\Big]{ \Bbbsum_{i=1}^m b_{y_i} \cdot y_i } + \Phi\parens[\Big]{ \Bbbsum_{x \in G} a_x \cdot x } \ast_M \Phi\parens[\Big]{ b_{y_{m+1}} \cdot y_{m+1} }
      = \\ &=
...
\begin{definition}\label{def:polynomial_algebra}\mimprovised
  Fix a \hyperref[def:semiring/commutative]{commutative semiring} \( R \) and an \hyperref[def:indexed_family]{indexed family} \( \mscrX = \seq{ X_k }_{k \in \mscrK} \) of \hyperref[def:formal_language/symbol]{symbols}. These will act as indeterminates in the sense of \cref{con:free_construction/indeterminate}.
...
\begin{remark}\label{rem:polynomials_over_infinitely_many_indeterminates}
  As will be shown in \cref{thm:def:polynomial_algebra/union}, the algebras \( R[X, Y] \) and \( R[X][Y] \) are isomorphic. Thus, if we prove a statement about the algebra \( R[X] \) in one indeterminate, oftentimes it automatically extends via induction to the algebra \( R[X_1, \ldots, X_n] \) over finitely many indeterminates.
...
\end{definition}
\begin{comments}
  \item Different conventions for handling the degree of the zero polynomial are discussed in \cref{ex:def:extended_real_numbers/formal_order}.
\end{comments}
...
\begin{remark}\label{rem:zero_polynomial_degree}
  ...
  \begin{thmenum}
    ...
    \thmitem{rem:zero_polynomial_degree/undefined} The degree of \( f(X) = 0 \) is left \hyperref[con:undefinedness]{undefined} in
...
\begin{definition}\label{def:polynomial_degree_terminology}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:polynomial_degree_terminology/quadratic} \term[bg=квадратна (функция) (\cite[1]{Обрешков1962ВисшаАлгебра}), ru=квадратичная (форма) (\cite[315]{Курош1968КурсВысшейАлгебры}), en=quadratic (polynomial) (\cite[44]{Rotman2015AdvancedModernAlgebraPart1})]{Quadratic} when \( \deg(f) = 2 \),
    \thmitem{def:polynomial_degree_terminology/cubic} \term[bg=кубична (форма) (\cite[1]{Обрешков1962ВисшаАлгебра}), ru=кубичная (форма) (\cite[315]{Курош1968КурсВысшейАлгебры}), en=cubic (polynomial) (\cite[44]{Rotman2015AdvancedModernAlgebraPart1})]{Cubic} when \( \deg(f) = 3 \),
    \thmitem{def:polynomial_degree_terminology/quartic} \term[en=quartic (polynomial) (\cite[44]{Rotman2015AdvancedModernAlgebraPart1})]{Quartic} when \( \deg(f) = 4 \),
    \thmitem{def:polynomial_degree_terminology/quintic} \term[en=quintic (polynomial) (\cite[44]{Rotman2015AdvancedModernAlgebraPart1})]{Quintic} when \( \deg(f) = 5 \)
  \end{thmenum}
\end{definition}
\begin{comments}
...
\begin{example}\label{ex:polynomial_root}
  ...
  \begin{thmenum}
    \thmitem{ex:polynomial_root/natural_numbers} Consider the polynomial \( f(X) \coloneqq aX^2 + bX + c \) in \( \BbbN[X] \). A function from the set \( \set{ X } \) to \( \BbbN \) corresponds to an element of \( \BbbN \), and hence evaluating the polynomial is done by simply replacing \( X \) symbolically in \( p \) and then evaluating the obtained \hyperref[rem:binary_operation_syntax_trees]{syntax tree}.
