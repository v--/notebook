...
\begin{theorem}[Semigroup algebra universal property]\label{thm:semigroup_algebra_universal_property}\mcite[381]{Knapp2016BasicAlgebra}
  The \hyperref[def:semigroup_algebra]{semigroup algebra} \( R[G] \) is the unique up to a unique isomorphism associative nonunital \( R \)-\hyperref[def:algebra_over_semiring]{algebra} \( R \) that satisfies the following \hyperref[rem:universal_mapping_property]{universal mapping property}:
  \begin{displayquote}
...
\end{theorem}
\begin{comments}
  \item Via \cref{rem:universal_mapping_property}, \( G \mapsto R[G] \) becomes \hyperref[def:category_adjunction]{left adjoint} to the \hyperref[con:forgetful_functor]{forgetful functor} which sends associative \( R \)-algebras to their multiplicative semigroup.
\end{comments}
\begin{proof}
