...
\end{definition}
\begin{comments}
  \item We generalize terminology traditionally used for \hyperref[def:semimodule/homomorphism]{linear maps} --- for example by \incite[66]{Knapp2016BasicAlgebra}, \incite[522]{Aluffi2009Algebra} or \incite[239]{Treil2017LinAlgebra}.
\end{comments}
...
\begin{definition}\label{def:semigroup_algebra}\mcite[29]{Golan1999Semirings}
  ...
  We define the corresponding \term[ru=групповая алгебра (\cite[504]{Винберг2014Алгебра})]{semiring algebra} as the \hyperref[def:free_semimodule]{free semimodule} endowed with a vector multiplication operation \( \odot \) called \term[ru=свёртка (\cite[62]{Боровков1999Вероятности}) / конволюция (\cite[179]{ИоффеТихомиров1974ЭкстремЗадачи})]{convolution product} and defined as follows:
...
\begin{definition}\label{def:polynomial_algebra}\mimprovised
  ...
  \begin{thmenum}
    ...
    As discussed in \fullref{rem:free_commutative_monoid_as_quotient}, we can regard elements of this monoid as congruence classes of strings of indeterminates. We call these congruence classes \term[ru=одночлен/моном (\cite[sec. 11.3]{Тыртышников2007ЛинАлгебра})]{monomials}.
    ...
    We call elements of \( R[\mscrX] \) \term[bg=полиноми (\cite[22]{ГеновМиховскиМоллов1991Алгебра}), ru=многочлены (\cite[92]{Винберг2014Алгебра}; многочлены/полиномы (\cite[sec. 11.3]{Тыртышников2007ЛинАлгебра})]{polynomials}. A polynomial is thus a linear combination of monomials with the \hyperref[def:semigroup_algebra]{convolution product}.
...
\begin{concept}\label{con:substitution_homomorphism}
  ...
  In the case of \( n \) indeterminates, \( \Phi(p) \) is a function from \( M^n \) to \( M \). Polynomials and their functions are thus an instance of the \hyperref[con:syntax_semantics_duality]{syntax-semantics duality}.
...
\begin{definition}\label{def:semiring_adjunction}\mcite[119]{Jacobson1985AlgebraPart1}
  ...
  \begin{thmenum}
    \thmitem{def:semiring_adjunction/smallest} \( R[A] \) is the (unique) smallest super-semiring of \( R \) containing \( A \).

    \thmitem{def:semiring_adjunction/evaluation} \( R[A] \) is the image of the \hyperref[con:substitution_homomorphism]{evaluation homomorphism} \( \Phi_e: R[\mscrX] \to S \), where \( e: \mscrX \to S \) is a bijective function assigning the elements of \( A \) to indeterminates.
  \end{thmenum}
