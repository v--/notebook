...
\begin{definition}\label{def:polynomial_algebra}\mimprovised
  ...
  \begin{thmenum}
    ...
    \thmitem{def:polynomial_algebra/polynomials} We define the \term[ru=алгебра многочленов (\cite[92]{Винберг2014Алгебра})]{polynomial algebra} \( R[\mscrX] \) as the \hyperref[def:semigroup_algebra]{monoid semiring} \( R[\BbbN^{\oplus \mscrX}] \).

    We call elements of \( R[\mscrX] \) \term[bg=полиноми (\cite[22]{ГеновМиховскиМоллов1991Алгебра}), ru=многочлены (\cite[92]{Винберг2014Алгебра}; многочлены/полиномы (\cite[sec. 11.3]{Тыртышников2007ЛинейнаяАлгебра})]{polynomials}.
...
\begin{definition}\label{def:univariate_polynomial}\mimprovised
  ...
  We call \( a_n \) the \term[bg=старши коефициент (\cite[23]{ГеновМиховскиМоллов1991Алгебра})]{leading coefficient} and \( a_0 \) the \term{constant coefficient} of the polynomial.
\end{definition}
...
\begin{definition}\label{def:semiring_adjunction}
  Let \( R \) be a \hyperref[def:ring/submodel]{sub-semiring} of the \hyperref[def:semiring/commutative]{commutative semiring} \( S \) and let \( A \) be a subset of \( S \). Denote by \( R[A] \) the ring generated by \term{adjoining} \( A \), which we define via any of the following equivalent definitions:
  \begin{thmenum}
