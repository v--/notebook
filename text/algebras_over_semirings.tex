...
\begin{definition}\label{def:algebra_over_semiring}\mimprovised
  ...
  \begin{thmenum}[resume=def:algebra_over_semiring]
    ...
    \thmitem{def:algebra_over_semiring/ideal}\mcite[430]{Bourbaki1998Algebra1to3} In case \( M \) is a unital algebra, left, right and two-sided ideals in \( R \)-algebras can be defined as for general semirings in \cref{def:semiring_ideal}.
...
\begin{theorem}[Semigroup algebra universal property]\label{thm:semigroup_algebra_universal_property}\mcite[381]{Knapp2016BasicAlgebra}
  The \hyperref[def:semigroup_algebra]{semigroup algebra} \( R[G] \) is the unique up to a unique isomorphism \( R \)-\hyperref[def:algebra_over_semiring]{algebra} \( R \) that satisfies the following \hyperref[rem:universal_mapping_property]{universal mapping property}:
  \begin{displayquote}
    For every \( R \)-algebra \( M \) and every \hyperref[def:semigroup/morphism]{semigroup homomorphism} \( e: G \to M \) into the \hyperref[def:algebra_over_semiring/monoids]{multiplicative semigroup} of \( M \), there exists a unique \hyperref[def:algebra_over_semiring]{\( R \)-algebra homomorphism} \( \Phi_e: R[G] \to M \) such that the following diagram commutes:
  ...
  If \( G \) is a monoid, the above holds for monoids and monoid homomorphisms.
\end{theorem}
\begin{comments}
