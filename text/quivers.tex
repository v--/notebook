...
\begin{definition}\label{def:quiver}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:quiver/graph} If there are no parallel and opposing arcs or loops, we call the quiver a \term{directed graph}.
    ...
    \thmitem{def:quiver/duality}\mcite[372]{Bollobas1998} The \term{opposite} or \term{dual quiver} of \( Q \) is obtained by swapping \( h \) and \( t \).
  \end{thmenum}
...
\begin{definition}\label{def:category_of_small_quivers}
  ...
  \begin{itemize}
    ...
    Given two quivers \( Q = (V_Q, A_Q, h_Q, t_Q) \) and \( R = (V_R, A_R, h_R, t_R) \), a \term{quiver homomorphism} is a pair of functions
