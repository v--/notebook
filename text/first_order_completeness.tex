...
\begin{proof}
  ...
  \SubProof{Proof that \( \Gamma^+ \) is satisfiable} Let \( \mscrX \) be an infinite model of \( T \) of cardinality \( \lambda \). Fix a family \( \seq{ a_\alpha }_{\alpha < \lambda} \) of distinct elements of \( \mscrX \). Consider the expansion \( \mscrX^+ \) of \( \mscrX \) to \( \Sigma^+ \) in which \( c_\alpha \) is interpreted as \( a_\alpha \).
  ...
  \SubProof{Proof that \( \Gamma \) has a model of cardinality \( \lambda \)} Since \( \Gamma^+ \) is satisfiable, by \cref{thm:classical_propositional_satisfiable_implies_consistent} implies that it is syntactically consistent.

  Then \fullref{thm:fol_model_existence_theorem} implies that \( \Gamma^+ \) has a model \( \mscrY \) of cardinality at most \( \lambda \), the number of new constants in \( \Sigma^+ \). Because of the axioms included in \( \Gamma^+ \), there must be at least \( \lambda \) different elements in \( \mscrY \). By antisymmetry, it follows that the cardinality of \( \mscrY \) is exactly \( \lambda \).

  The reduct of \( \mscrY \) to \( \Sigma \) is then the desired model of \( \Gamma \).
\end{proof}
