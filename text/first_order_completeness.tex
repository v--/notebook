...
\begin{theorem}[Theorem on constants]\label{thm:theorem_on_constants}\mcite[33]{Shoenfield1967MathematicalLogic}
  Over some \hyperref[def:fol_signature]{signature} \( \Sigma \), fix some set \( \Gamma \) of \hyperref[def:fol_closed_formula]{closed formulas} \hi{over} \( \Sigma \) and a formula \( \varphi \) with free variables \( x_1, \ldots, x_n \).
...
\begin{theorem}[Downward L\"owenheim-Skolem theorem]\label{thm:downward_lowenheim_skolem_theorem}\mcite[thm. 2.3.34]{Hinman2005Logic}
  ...
  More generally, for every structure \( \mscrX = (X, I) \) over \( \Sigma \) of cardinality \( \mu \) and every subset \( A \) of \( X \) of cardinality \hi{at most} \( \kappa \), there exists an elementary substructure of \( \mscrX \) of cardinality \hi{exactly} \( \kappa \) containing \( A \).
\end{theorem}
\begin{comments}
...
\begin{proof}
  ...
  First, since we can add arbitrary elements of \( X \) to \( A \), we can assume without loss of generality that the cardinality of \( A \) is \hi{exactly} \( \kappa \).
...
\begin{proof}
  ...
  \SubProof{Proof that \( \mscrX \) has an elementary extension of cardinality \( \mu \)} Let \( \mscrY = (Y, J) \) be a model of \( \Gamma^+ \). Because of the number of new constants in \( \Sigma^+ \), the cardinality of \( \mscrY \) is \hi{at least} \( \mu \). Since we can use \fullref{thm:downward_lowenheim_skolem_theorem}, without loss of generality, we can assume that \( \mscrY \) has cardinality \hi{exactly} \( \mu \).
