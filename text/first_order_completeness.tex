...
\begin{remark}\label{rem:henkin_theories}
   ...
   The differences are subtle but important. For example, Gerasimov imposes a weaker condition than in \cref{def:fol_henkin_completeness} --- if \( \qexists x \varphi \) belongs to \( \Gamma \), there must exist a closed term \( \tau \) such that \( \varphi[x \mapsto \tau] \) belongs to \( \Gamma \). This requires modifying how \hyperref[def:fol_henkin_extension]{Henkin extensions} interact with other extensions, and requires also adapting \fullref{thm:fol_lindenbaums_lemma}.
\end{remark}
...
\begin{definition}\label{def:fol_henkin_extension}\mcite[def. 3.1.6]{VanDalen2004LogicAndStructure}
  ...
  We define the (one-step) \term{Henkin extension} \( \Gamma^* \) as the consequence closure of
...
\end{proposition}
\begin{proof}
  Let \( \Gamma \) be a theory over \( \Sigma \). Let \( \Gamma^* \) be the Henkin extension of \( \Gamma \) with signature \( \Sigma^+ \). Denote by \( \Delta \) the set of additional axioms from \( \Gamma^* \), so that \( \Gamma^* = \op*{Th}(\Gamma \cup \Delta) \).
  ...
  Here \( P_{k+1} \) derives \( \varphi \) from \( \Gamma \cup \Delta_{k+1} \), as desired.
\end{proof}
...
\begin{definition}\label{def:fol_henkin_completion}\mimprovised
  With respect to some \hyperref[def:consequence_relation/compactness]{compact} \hyperref[def:consequence_relation]{consequence relation}, consider some \hyperref[def:fol_theory]{first-order theory} \( \Gamma \) over \( \Sigma \) as follows.
  ...
  \begin{equation*}
    \Gamma^{*\alpha} \coloneqq \begin{cases}
      \Gamma,                                &\alpha = 0, \\
      (\Gamma^{*\beta})^*,                   &\alpha = \op{sc}(\beta), \\
      \bigcup_{\beta < \alpha} \Gamma^\beta, &\alpha \T{is a limit ordinal.}
    \end{cases}
  \end{equation*}

  For the signatures, we define an analogous sequence \( \seq{ \Sigma^{*\alpha} }_{\alpha < \mu} \).

  As we will show in \cref{thm:fol_henkin_completion_is_complete}, this sequence stabilizes at \( \alpha = \omega \). We thus call \( \Gamma^{*\omega} \) the \term{Henkin completion} of \( \Gamma \).
\end{definition}
\begin{comments}
...
\end{lemma}
\begin{proof}
  Let \( \seq{ \Gamma_k }_{k \in \mscrK} \) be an upward-directed family of theories and let \( \Gamma \) be their union (we take the union of signature symbols also). \Cref{thm:upward_union_of_theories} shows that \( \Gamma \) is also a theory.
...
\end{proposition}
\begin{proof}
  Consider the theory \( \Gamma \) over the signature \( \Sigma \).

  \SubProof{Proof that \( \Gamma^{*\omega} \) is conservative} If \( \varphi \) is a formula over \( \Sigma \) that belongs to \( \Gamma^{*\omega} \), it also belongs to some finite \( \Gamma^{*k_0} \). Using \cref{thm:def:fol_henkin_extension/conservative}, by induction on \( k_0 \) we can show that \( \Gamma^{*k_0} \) is conservative over \( \Gamma \). Therefore, \( \varphi \) belongs to \( \Gamma \).

  \SubProof{Proof that \( \Gamma^{*\omega} \) is a fixed point} \Cref{thm:upward_union_of_theories} implies that the completion \( \Gamma^{*\omega} \) is a theory. Since the Henkin extension operator is \hyperref[def:scott_continuity]{Scott-continuous} by \cref{thm:fol_henkin_extensions_scott_continuous}, \fullref{thm:knaster_tarski_iteration/continuous} implies that \( \Gamma^{*\omega} \) is a fixed point.

  \SubProof{Proof that \( \Gamma^{*\omega} \) is a Henkin complete} Every existential formula \( \qexists x \varphi \) over \( \Sigma^{*\omega} \) is a formula over some signature \( \Sigma^{*k_0} \) of finite index. In this case, \( \Gamma^{*k_0} \), and hence also \( \Gamma^{*\omega} \), contains the formula
  \begin{equation*}
    (\qexists x \varphi) \synimplies \varphi[x \mapsto c_{x,\varphi}].
  \end{equation*}

  It follows that \( c_{x,\varphi} \) is a witness of \( \qexists x \varphi \) in \( \Gamma^{*\omega} \). Generalizing, we conclude that \( \Gamma^{*\omega} \) is Henkin complete.
\end{proof}
