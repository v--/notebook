...
\begin{remark}\label{rem:fol_henkin_theories}
   We will present a proof of \fullref{thm:fol_completeness} based on Leon Henkin's \cite{Henkin1949FOLCompleteness}. The first proof of this theorem is attributed to Kurt G\"odel, thus it is also called \enquote{Go\"del's completeness theorem}.

   Our presentation follows the modern adaptation of \textcite[\S 3.1]{VanDalen2004LogicAndStructure}. Other adaptations are given in \cite[\S 4.2]{Shoenfield1967MathematicalLogic}, \cite[\S 4.5]{ШеньВерещагин2017ЯзыкиИИсчисления} and \cite[\S 2.6.3]{Герасимов2014Вычислимость}. Similar constructions we will use for proving \fullref{thm:fol_semantic_compactness} are given in \cite[ch. 3]{Hinman2005Logic}.
   ...
   We also present Henkin's construction for higher-order logic --- see \cref{rem:hol_henkin_theories}.
\end{remark}
...
\begin{definition}\label{def:fol_henkin_completeness}\mcite[def. 3.1.4(iii)]{VanDalen2004LogicAndStructure}
  We say that the \hyperref[def:fol_theory]{first-order theory} \( T \) over \( \Sigma \) is \term[en=Henkin-complete (theory) (\cite[def. 3.1.7]{Hinman2005Logic}); Henkin theory (\cite[def. 3.1.4(iii)]{VanDalen2004LogicAndStructure})]{Henkin-complete} if, for every closed existentially quantified formula \( \qexists x \varphi \) over \( \Sigma \) (not necessarily in \( T \)), there exists a constant \( c \) such that
...
\begin{comments}
  ...
  This allows us to define completions in \cref{thm:fol_henkin_completion_is_complete} and ensure that they are compatible with further extensions. This is one of several possible definitions of Henkin theories --- see \cref{rem:fol_henkin_theories}.

  \item Since \( \qexists x \varphi \) is assumed to be closed, \( x \) is the only variable possibly free in \( \varphi \).

  \item We generalize this to higher-order logic in \cref{def:hol_henkin_completeness}.
\end{comments}
...
\begin{definition}\label{def:fol_henkin_extension}\mcite[def. 3.1.6]{VanDalen2004LogicAndStructure}
  ...
  Consider the \hyperref[def:fol_signature_extension]{extension} \( \Sigma^* \) of \( \Sigma \) where, for every existentially quantified formula \( \qexists x \varphi \) over \( \Sigma \) (not necessarily in \( T \)), we add a constant \( c_{x,\varphi} \). For the sake of determinism, suppose that \( c_{x,\varphi} \) is a symbol entirely determined by \( x \) and \( \varphi \). Thus, if such a constant already exists in \( \Sigma \), a new one does not need to be added.
  ...
  \begin{equation}\label{eq:def:fol_henkin_extension}
    T \cup \set[\big]{ (\qexists x \varphi) \synimplies \varphi[x \mapsto c_{x,\varphi}] \given* \qexists x \varphi \T{is a closed formula over} \Sigma }.
  \end{equation}
\end{definition}
\begin{comments}
  ...
  \item The determinism condition, although not present in \cite[def. 3.1.6]{VanDalen2004LogicAndStructure}, is important for some intermediate results like \cref{thm:fol_henkin_extensions_scott_continuous}.

  \item This construction is necessary for syntactic theories, but can be avoided for semantic theories --- see \cref{ex:thm:fol_completeness/skolemization}.
\end{comments}

\begin{theorem}[Theorem on constants]\label{thm:theorem_on_constants}\mcite[33]{Shoenfield1967MathematicalLogic}
  Over some \hyperref[def:fol_signature]{signature} \( \Sigma \), fix some set \( \Gamma \) of \hyperref[def:fol_closed_formula]{closed formulas} \emph{over} \( \Sigma \) and a formula \( \varphi \) with free variables \( x_1, \ldots, x_n \).
...
\begin{proof}
  ...
  \Fullref{thm:fol_syntactic_deduction_theorem} gives us a proof tree \( P_k' \) deriving
  \begin{equation*}
...
\begin{definition}\label{def:fol_henkin_completion}\mimprovised
  With respect to some \hyperref[def:consequence_relation/compactness]{compact} \hyperref[def:consequence_relation]{consequence relation}, consider some \hyperref[def:fol_theory]{first-order theory} \( T \) over \( \Sigma \).
...
\begin{proof}
  ...
  Indeed, let \( \psi = (\qexists x \varphi) \synimplies \varphi[x \mapsto c_{x,\varphi}] \) be a new axiom introduced in \( T^* \). Then there exists some signature \( \Sigma_{k_0} \) to which \( \qexists x \varphi \) belongs. Then \( \psi \) is also present in \( T_{k_0}^* \). It follows that
  \begin{equation*}
    T^* \subseteq \bigcup_{k \in \mscrK} T_k^*.
  \end{equation*}

  Conversely, let \( \psi = (\qexists x \varphi) \synimplies \varphi[x \mapsto c_{x,\varphi}] \) be a new axiom introduced in some \( T_{k_0}^* \) for some index \( k_0 \). Then \( \qexists x \varphi \) is a formula over \( \Sigma_{k_0} \). Since \( \Sigma^* \) extends \( \Sigma_{k_0} \), the formula \( \psi \) belongs to \( T^* \).
\end{proof}
...
\begin{proof}
  ...
  \SubProof{Proof that \( T^{*\omega} \) is conservative} If \( \varphi \) is a formula over \( \Sigma \) that belongs to \( T^{*\omega} \), it also belongs to some finite \( T^{*k_0} \). Using \cref{thm:fol_henkin_extension_is_conservative}, by induction on \( k_0 \) we can show that \( T^{*k_0} \) is conservative over \( T \). Therefore, \( \varphi \) belongs to \( T \).
...
\begin{definition}\label{def:fol_term_model}\mcite[107]{VanDalen2004LogicAndStructure}
  ...
  \begin{thmenum}
    \thmitem{def:fol_term_model/intermediate} Let \( C \) be the set of all closed terms over \( \Sigma \). This set is nonempty because \( T \) contains a witness of \( \qexists x \syntop \).
...
\end{definition}
\begin{comments}
  \item Compare this to its counterpart in higher-order logic, \cref{def:hol_term_model}, which is also due to Henkin.

  \item This construction resembles free term models of \hyperref[def:fol_equational_theory]{equational theories}, which we discuss in \cref{def:fol_free_term_model}.
\end{comments}

\begin{definition}\label{def:fol_frugal_structure}\mcite[128]{Andrews2002Logic}
  We say that a \hyperref[def:fol_structure]{first-order structure} \( \mscrX = (X, I) \) over \( \Sigma \) is \term{frugal} if the cardinality of \( X \) does not exceed the number of terms over \( \Sigma \).
\end{definition}
\begin{comments}
  \item Compare this to \cref{def:hol_frugal_structure}, where for higher-order structures frugality restricts all universes of functions.
\end{comments}

\begin{proposition}\label{thm:fol_term_model_is_frugal}
  \hyperref[def:fol_term_model]{First-order term models} are \hyperref[def:fol_frugal_structure]{frugal}.
\end{proposition}
\begin{proof}
  A term model is defined as a quotient set of the set of all terms, so its cardinality cannot exceed the number of terms.
\end{proof}

\begin{proposition}\label{thm:fol_frugality_characterization}
  A \hyperref[def:fol_structure]{first-order structure} \( \mscrX = (X, I) \) over \( \Sigma \) is \hyperref[def:fol_frugal_structure]{frugal} if and only if
  \begin{equation*}
    \card(X) \leq \max\set{ \aleph_0, \kappa },
  \end{equation*}
...
\begin{proof}
  There are two cases:
  \begin{itemize}
    \item If \( \kappa \) is finite, \cref{thm:def:formal_language/finite_alphabet_cardinality} implies that the set of terms, as a language over \( \Sigma \), is countably infinite. In this case \( \card(X) \) must not exceed \( \aleph_0 \).

    \item If \( \kappa \) is infinite, \cref{thm:def:formal_language/infinite_alphabet_cardinality} implies that the set of all terms also has cardinality \( \kappa \). In this case \( \card(X) \) must not exceed \( \kappa \).
  \end{itemize}
\end{proof}
...
\begin{corollary}\label{thm:fol_consistent_implies_satisfiable}
  If a set of closed formulas is \hyperref[def:fol_theory/consistent]{consistent} with respect to \hyperref[def:fol_natural_deduction_system]{classical natural deduction}, it has a \hyperref[def:fol_frugal_structure]{frugal} \hyperref[def:fol_semantics/model]{model}.
\end{corollary}
\begin{comments}
  \item This statement has an analog for both \hyperref[def:propositional_logic]{propositional logic} and \hyperref[def:higher_order_logic]{higher-order logic} --- see \cref{thm:propositional_consistent_implies_satisfiable} and \cref{thm:hol_consistent_implies_satisfiable}, respectively.
\end{comments}
\begin{proof}
  ...
  This allows us to construct a \hyperref[def:fol_term_model]{term model} \( \mscrT \) satisfying \( T^+ \). Its \hyperref[def:fol_reduct_along_morphism]{reduct} along the inclusion \( \iota: T \to T^+ \) is then a model of \( T \).

  Furthermore, \cref{thm:fol_term_model_is_frugal} shows that \( \mscrT \) is frugal over the Henkin completion \( \Sigma^{*\omega} \). To show that it is also frugal over \( \Sigma \), we will use \cref{thm:fol_frugality_characterization} and prove that \( \Sigma^{*\omega} \) has at most \( \max\set{ \aleph_0, \kappa } \) symbols, where \( \kappa \) is the number of symbols in \( \Sigma \).
  ...
  \begin{itemize}
    ...
    \item If \( \kappa \) is infinite, \cref{thm:def:formal_language/infinite_alphabet_cardinality} implies that each of the Henkin extensions \( \Sigma^{*k} \) have \( \kappa \) symbols, and hence so does \( \Sigma^{*\omega} \).
  \end{itemize}
\end{proof}
...
\end{theorem}
\begin{comments}
  \item This theorem is attributed to both Kurt G\"odel and to Leon Henkin; see \cref{rem:fol_henkin_theories}.

  \item This is the first-order counterpart of \fullref{thm:classical_propositional_completeness} and \fullref{thm:hol_weak_completeness}.
\end{comments}
\begin{proof}
...
\begin{example}\label{ex:thm:fol_completeness}
  We list examples related to \fullref{thm:fol_completeness}:
  \begin{thmenum}
    \thmitem{ex:thm:fol_completeness/skolemization} We showed in \fullref{thm:skolems_theorem} that, given a semantic theory \( T \) over \( \Sigma \), we can utilize \fullref{alg:fol_skolemization} to generate a conservative extension \( T^+ \) with a \hyperref[def:fol_prenex_normal_form/universal]{universal} axiomatization.

    By completeness, both \( T \) and \( T^+ \) are also syntactic theories. However, \( T^+ \) is not conservative over \( T \)

    The construction of \( T^+ \) simply modifies the axioms of \( T \) without introducing new ones. Thus, there is no reason for a formula in \( T^+ \) over \( \Sigma \) to be derivable from \( T \).

    A strategy for adapting this to syntactic consequence is to follow the approach used for \hyperref[def:fol_henkin_extension]{Henkin extensions} --- adding axioms which would allow us to build, for every formula \( \theta \) over \( \Sigma \), a proof tree in \( T \) from one in \( T^+ \). Choosing the right axioms and modifying the proof trees is not at all straightforward, as witnessed by the simpler case shown in \cref{thm:fol_henkin_extension_is_conservative}.
  \end{thmenum}
\end{example}
...
\begin{theorem}[First-order semantic compactness]\label{thm:fol_semantic_compactness}
  ...
  \begin{thmenum}
    \thmitem{thm:fol_semantic_compactness/relation} The relation is \hyperref[def:consequence_relation/compactness]{compact}: if \( \Gamma \vDash \varphi \), there exists a finite subset \( \Gamma_0 \) of \( \Gamma \) such that \( \Gamma_0 \vDash \varphi \).

    \thmitem{thm:fol_semantic_compactness/satisfiable} A set of sentences is \hyperref[def:fol_semantics/satisfaction]{satisfiable} (has a model) if and only if it is \hyperref[def:finitely_satisfiable_set_of_sentences]{finitely satisfiable} (every finite subset has a model).

    \thmitem{thm:fol_semantic_compactness/consistent} A set of sentences is \hyperref[def:fol_theory/consistent]{consistent} (does not entail \( \synbot \)) if and only if it is \hyperref[def:finitely_consistent_set_of_sentences]{finitely consistent} (no finite subset entails \( \synbot \)).
  \end{thmenum}
\end{theorem}
\begin{comments}
...
\end{comments}
\begin{proof}
  \Cref{thm:fol_semantic_compactness/relation} Follows from \fullref{thm:fol_completeness} via \cref{thm:completeness_implies_compactness}.
...
\begin{theorem}[Tarski-Vaugh criterion]\label{thm:tarski_vaugh_criterion}\mcite[thm. 2.3.33]{Hinman2005Logic}
  Consider the \hyperref[def:fol_structure]{first-order structure} \( \mscrX = (X, I) \) over \( \Sigma \).

  The subset \( A \) \hyperref[def:fol_substructure/determines]{induces} an \hyperref[def:elementary_embedding]{elementary substructure} of \( \mscrX \) if and only if, for every formula \( \varphi \) over \( \Sigma \) and every variable assignment \( v \) into \( A \), there exists a value \( a_\varphi \) in \( A \) such that
  \begin{equation*}
    \Bracks{\qexists x \varphi}_\mscrX^v = \Bracks{\varphi}_\mscrX^{v_{x \mapsto a_\varphi}}.
  \end{equation*}
\end{theorem}
\begin{comments}
  \item We take the name of the theorem from \cite[lemma 1.15.22]{Kunen2013SetTheory}, and the proof is based on \cite[thm. 2.3.33]{Hinman2005Logic}.
\end{comments}
\begin{proof}
  \SufficiencySubProof Suppose that \( \mscrA = (A, I\restr_A) \) is an elementary substructure of \( \mscrX \). Fix a formula \( \varphi \) over \( \Sigma \) and a variable assignment \( v \) into \( A \).
  \begin{itemize}
    \item If \( \Bracks{\qexists x \varphi}_\mscrX^v = \semtop \), then \( \Bracks{\qexists x \varphi}_\mscrA^v = \semtop \) by elementary equivalence, so there exists a value \( a \) in \( A \) such that \( \Bracks{\varphi}_\mscrX^{v_{x \mapsto a}} = \semtop \).

    \item Otherwise, \( \Bracks{\qexists x \varphi}_\mscrX^v = \sembot \), so for every value \( a \) in \( A \) we have \( \Bracks{\varphi}_\mscrX^{v_{x \mapsto a}} = \sembot \).
  \end{itemize}

  \NecessitySubProof Suppose that, for every formula \( \varphi \) over \( \Sigma \) and every variable assignment \( v \) into \( A \), we have
  \begin{equation*}
    \Bracks{\qexists x \varphi}_\mscrX^v = \Bracks{\varphi}_\mscrX^{v_{x \mapsto a_\varphi}}.
  \end{equation*}

  Clearly \( A \) is closed under function application because, if \( f \) is an \( n \)-ary function and \( a_1, \ldots, a_n \) are in \( A \), then there must exist a value \( b \) in \( A \) such that
  \begin{equation*}
    \Bracks{\qexists \syny f(\synx_1, \ldots, \synx_n) \syneq \syny}(a_1, \ldots, a_n) = \Bracks{f(\synx_1, \ldots, \synx_n) \syneq \syny}(a_1, \ldots, a_n, b).
  \end{equation*}

  Thus, \( \mscrA = (A, I\restr_A) \) is a substructure of \( \mscrX \). We will use \fullref{thm:induction_on_abstract_syntax} on \( \varphi \) to show that \( \Bracks{\varphi}_\mscrX^v = \Bracks{\varphi}_\mscrA^v \) for every assignment \( v \) into \( A \).
  \begin{itemize}
    \item There is nothing to prove for atomic formulas.
    \item For negations and propositional connective formulas, the induction is straightforward.
    \item Suppose that \( \varphi = \qexists x \psi \), where the inductive hypothesis holds for \( \psi \).

    Fix an assignment \( v \) into \( A \).
    \begin{itemize}
      \item If \( \Bracks{\qexists x \psi}_\mscrX^v = \semtop \), by assumption there exists an element \( a_\psi \) in \( A \) such that
      \begin{equation*}
        \underbrace{\Bracks{\qexists x \psi}_\mscrX^v}_{\semtop} = \Bracks{\psi}_\mscrX^{v_{x \mapsto a_\psi}}.
      \end{equation*}

      By the inductive hypothesis, we have \( \Bracks{\psi}_\mscrA^{v_{x \mapsto a_\psi}} = \semtop \), so
      \begin{equation*}
        \Bracks{\qexists x \psi}_\mscrX^v = \bigvee \set[\big]{ \Bracks{\psi}_\mscrA^{v_{x \mapsto a}} \given a \in A } = \semtop.
      \end{equation*}

      \item Otherwise, \( \Bracks{\qexists x \psi}_\mscrX^v = \sembot \), so \( \Bracks{\psi}_\mscrX^{v_{x \mapsto a_\psi}} = \sembot \) for every \( a \) in \( X \), and in particular every \( a \) in \( A \).

      By the inductive hypothesis, we have \( \Bracks{\psi}_\mscrA^{v_{x \mapsto a_\psi}} = \sembot \), so \( \Bracks{\qexists x \psi}_\mscrX^v = \sembot \).
    \end{itemize}

    \item Finally, suppose that \( \varphi = \qforall x \psi \), where the inductive hypothesis holds for \( \psi \).

    Fix an assignment \( v \) into \( A \). \Cref{thm:fol_quantifier_duality} implies that
    \begin{equation*}
      \Bracks{\qforall x \psi}_\mscrX^v = \oline {\Bracks{\qexists x \synneg \psi}_\mscrX^v},
    \end{equation*}
    which by assumption equals
    \begin{equation*}
      \oline{\Bracks{\synneg \psi}_\mscrX^{v_{x \mapsto a_{\synneg \psi}}}}
      =
      \Bracks{\psi}_\mscrX^{v_{x \mapsto a_{\synneg \psi}}}
    \end{equation*}
    for some \( a_{\synneg \psi} \) in \( A \).

    We can continue by using the inductive hypothesis on \( \psi \), like we did above.
  \end{itemize}
\end{proof}

\begin{theorem}[Downward L\"owenheim-Skolem theorem]\label{thm:downward_lowenheim_skolem_theorem}\mcite[thm. 2.3.34]{Hinman2005Logic}
  Fix infinite cardinals \( \kappa < \mu \) and a \hyperref[def:fol_signature]{first-order signature} \( \Sigma \) with at most \( \kappa \) symbols.

  Every \hyperref[def:fol_structure]{structure} over \( \Sigma \) of cardinality \( \mu \) has an \hyperref[def:elementary_embedding]{elementary substructure} of cardinality \( \kappa \).

  More generally, for every structure \( \mscrX = (X, I) \) over \( \Sigma \) of cardinality \( \mu \) and every subset \( A \) of \( X \) of cardinality \emph{at most} \( \kappa \), there exists an elementary substructure of \( \mscrX \) of cardinality \emph{exactly} \( \kappa \) containing \( A \).
\end{theorem}
\begin{comments}
  \item This theorem is closely related to \fullref{thm:upward_lowenheim_skolem_theorem}.
\end{comments}
\begin{proof}
  We will use Skolem functions, similar to the ones in \fullref{alg:fol_skolemization}.

  First, since we can add arbitrary elements of \( X \) to \( A \), we can assume without loss of generality that the cardinality of \( A \) is \emph{exactly} \( \kappa \).

  For every existential formula \( \qexists x \varphi \) with \( n \) free variables, pick a function \( h_{x,\varphi}: X^n \to X \) such that
  \begin{equation*}
    \Bracks{\qexists x \varphi}_\mscrX(a_1, \ldots, a_n) = \Bracks{\varphi}_\mscrX(a_1, \ldots, a_n, h_{x,\varphi}(a_1, \ldots, a_n)).
  \end{equation*}

  In case \( \Bracks{\qexists x \varphi}_\mscrX(a_1, \ldots, a_n) = \sembot \), we can choose \( h_{x,\varphi}(a_1, \ldots, a_n) \) to be arbitrary.

  Let \( Y \) be the closure of \( A \) under application of signature functions and of \( h_{x,\varphi} \) for every \( x \) and \( \varphi \).

  Then \( \mscrY \coloneqq (Y, I\restr_Y) \) is by construction a substructure of \( \mscrX \). Its cardinality is at least \( \kappa \) because it is a superset of \( A \). Furthermore, since \( \Sigma \) has at most \( \kappa \) symbols, \cref{thm:def:formal_language/infinite_alphabet_cardinality} implies that there are at most \( \kappa \) formulas over \( \Sigma \), and hence the cardinality of \( Y \) must be exactly \( \kappa \).

  It remains to show that \( \mscrY \) is an elementary substructure of \( \mscrX \), i.e. that the inclusion \hyperref[def:fol_function_respect]{respects} all formulas. This follows from \fullref{thm:tarski_vaugh_criterion} because \( Y \) is closed under the application of the specifically constructed Skolem functions.
\end{proof}

\begin{definition}\label{def:fol_structure_diagram}\mcite[74]{Shoenfield1967MathematicalLogic}
  Fix a \hyperref[def:fol_structure]{first-order structure} \( \mscrX = (X, I) \) over \( \Sigma \). Extend the signature to \( \Sigma^{D_\mscrX} \) by adding a new constant \( d_a \) for every element \( a \) of \( X \). This induces an obvious \hyperref[def:fol_structure_expansion]{expansion} \( \mscrX^D \) of \( \mscrX \) in which \( d_a \) is interpreted as \( a \).

  We refer to the \hyperref[def:fol_theory/model]{theory} \( \cat{Th}(\mscrX^D) \) as the \term{diagram} of \( \mscrX \).
\end{definition}

\begin{proposition}\label{thm:fol_structure_diagram_model}
  Any \hyperref[def:fol_semantics/model]{model} \( \mscrY = (Y, J) \) of the \hyperref[def:fol_structure_diagram]{diagram} \( \cat{Th}(\mscrX^D) \) of \( \mscrX = (X, I) \) induces an \hyperref[def:elementary_embedding]{elementary embedding} \( h(a) \coloneqq \Bracks{d_a}_\mscrY \) of \( \mscrX \) into (the reduct of) \( \mscrY \).
\end{proposition}
\begin{proof}
  Fix a formula \( \varphi \). Let \( x_1, \ldots, x_n \) be an enumeration of the free variables of \( \psi \). Then, for any variable assignment \( v \), we have
  \begin{equation*}
    \Bracks{\varphi}_\mscrX^v
    =
    \Bracks{\varphi[\underbrace{x_1 \mapsto d_{v(x_1)}, \ldots, x_n \mapsto d_{v(x_n)}}_\Bbbs]}_\mscrX^v.
  \end{equation*}

  \begin{itemize}
    \item If \( \Bracks{\varphi}_\mscrX^v = \semtop \), then \( \varphi[\Bbbs] \) belongs to the diagram of \( \mscrX \), hence \( \Bracks{\varphi[\Bbbs]}_\mscrY^{h \bincirc v} = \semtop \).

    \item Otherwise, \( \Bracks{\varphi[\Bbbs]}_\mscrX^v = \sembot \), and so \( \synneg \varphi[\Bbbs] \) belongs to the diagram. In this case, we have \( \Bracks{\varphi[\Bbbs]}_\mscrY^{h \bincirc v} = \sembot \).
  \end{itemize}
  ...
  \begin{equation*}
    ...
    =
    \Bracks{\varphi[\Bbbs}_\mscrY^{h \bincirc v}
    =
    \Bracks{\varphi}_\mscrY^{h \bincirc v}.
  \end{equation*}
\end{proof}

\begin{theorem}[Upward L\"owenheim-Skolem theorem]\label{thm:upward_lowenheim_skolem_theorem}\mcite[thm. 2.4.22]{Hinman2005Logic}
  The following dual of \fullref{thm:downward_lowenheim_skolem_theorem} holds: whenever \( \mu > \kappa \geq \aleph_0 \) and \( \Sigma \) has at most \( \kappa \) symbols, every structure over \( \Sigma \) of cardinality \( \kappa \) has an \hyperref[def:elementary_embedding]{elementary extension} of cardinality \( \mu \).
\end{theorem}
\begin{proof}
  Fix a structure \( \mscrX = (X, I) \) of cardinality \( \kappa \) over \( \Sigma \). We will work with the \hyperref[def:fol_structure_diagram]{diagram} \( T \coloneqq \cat{Th}(\mscrX^D) \) of \( \mscrX = (X, I) \).

  Let \( \Sigma^+ \) be a further extension of the diagram signature \( \Sigma^D \) with a new constant \( c_\alpha \) for every \( \alpha < \mu \), and let \( \Gamma^+ \) be an extension of \( T \) with a new axiom \( \synneg (c_\alpha \syneq c_\beta) \) for every pair \( \alpha \neq \beta \).

  \SubProof{Proof that \( \Gamma^+ \) is satisfiable} Fix a finite subset \( \Delta \) of \( \Gamma^+ \). Let \( c_{\alpha_1}, \dots, c_{\alpha_m} \) be the new constants that occur in \( \Delta \), and define
  \begin{equation*}
    \Gamma_0 \coloneqq T \cup \set{ \synneg (c_{\alpha_i} \syneq c_{\alpha_j}) \given i, j = 1, \ldots, m }.
  \end{equation*}

  Let \( \Sigma_0 \) be the signature extending \( \Sigma^D \) with only these constants. Fix distinct elements \( a_1, \ldots, a_m \) of \( X \), and let \( \mscrX_0 \) be the expansion of \( \mscrX' \) to \( \Sigma_0 \) in which we assign \( a_i \) to \( c_{\alpha_i} \).

  Then \( \mscrX_0 \) satisfies the new axioms in \( \Gamma_0 \) in addition to those in \( T \).

  Generalizing on \( \Delta \), we conclude that \( \Gamma^+ \) is \hyperref[def:finitely_satisfiable_set_of_sentences]{finitely satisfiable}. Satisfiability of \( \Gamma^+ \) itself follows from \fullref{thm:fol_semantic_compactness/satisfiable}.

  \SubProof{Proof that \( \mscrX \) has an elementary extension of cardinality \( \mu \)} Let \( \mscrY = (Y, J) \) be a model of \( \Gamma^+ \). Because of the number of new constants in \( \Sigma^+ \), the cardinality of \( \mscrY \) is \emph{at least} \( \mu \). Since we can use \fullref{thm:downward_lowenheim_skolem_theorem}, without loss of generality, we can assume that \( \mscrY \) has cardinality \emph{exactly} \( \mu \).

  Clearly (the reduct of) \( \mscrY \) is a model of the diagram \( T \) of \( \mscrX \). \Cref{thm:fol_structure_diagram_model} implies that \( h(a) \coloneqq \Bracks{d_a}_\mscrY \) is an elementary embedding of \( \mscrX \) into \( \mscrY \).

  Finally, by replacing \( h(a) \) with \( a \) in the image of \( h \), we can turn \( \mscrY \) into an elementary extension of \( \mscrX \).
\end{proof}

\begin{corollary}\label{thm:lowenheim_skolem_and_categoricity}
  A \hyperref[def:fol_theory]{first-order theory} with an infinite model cannot be \hyperref[def:fol_theory/categorical]{categorical}.
\end{corollary}
\begin{proof}
  We can use \fullref{thm:upward_lowenheim_skolem_theorem} to build a non-isomorphic model.
\end{proof}

\begin{example}\label{ex:lowenheim_skolem}
  We list examples of \fullref{thm:downward_lowenheim_skolem_theorem} and \fullref{thm:upward_lowenheim_skolem_theorem}:
  \begin{thmenum}
    \thmitem{ex:lowenheim_skolem/pa} \Cref{thm:peano_arithmetic_standard_models} lists several isomorphic models of \hyperref[def:peano_arithmetic]{Peano arithmetic}. All of them are standard in the sense of \cref{rem:simulating_second_order_logic}, and in particular they are countably infinite.

    We can use \fullref{thm:upward_lowenheim_skolem_theorem} to construct models of higher cardinality, with additional elements, which we call \enquote{nonstandard natural numbers} in accordance with \cref{def:peano_arithmetic_standard_model}.

    In \cref{ex:peano_arithmetic_nonstandard_models}, we present a construction of nonstandard natural numbers that uses \fullref{thm:fol_semantic_compactness} directly.

    \thmitem{ex:lowenheim_skolem/zfc} We discuss concrete models of \hyperref[def:zfc]{Zermelo-Fraenkel set theory} in \fullref{sec:von_neumanns_cumulative_hierarchy} and \fullref{sec:grothendieck_universes}. They are again standard in the sense of \cref{rem:simulating_second_order_logic}.

    Using \fullref{thm:downward_lowenheim_skolem_theorem}, we can find a countably infinite elementary submodel of any of them. This is further discussed in \fullref{thm:skolems_paradox}.
  \end{thmenum}
\end{example}
