...
\begin{theorem}[Tarski-Vaugh criterion]\label{thm:tarski_vaugh_criterion}\mcite[thm. 2.3.33]{Hinman2005Logic}
  ...
  The subset \( A \) \hyperref[def:fol_substructure/induces]{induces} an \hyperref[def:elementary_embedding]{elementary substructure} of \( \mscrX \) if and only if, for every formula \( \varphi \) over \( \Sigma \) and every variable assignment \( v \) into \( A \), there exists a value \( a_\varphi \) in \( A \) such that
  \begin{equation*}
