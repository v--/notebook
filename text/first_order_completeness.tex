...
\begin{theorem}[Downward L\"owenheim-Skolem theorem]\label{thm:downward_lowenheim_skolem}\mcite[thm. 3.2.3]{VanDalen2004LogicAndStructure}
  Consider a \hyperref[def:fol_theory]{first-order theory} \( T \) over a \hyperref[def:fol_signature]{signature} \( \Sigma \) with \( \kappa \) symbols.

  If \( T \) has a \hyperref[def:fol_semantics/model]{model} of \hi{infinite} \hyperref[thm:cardinality_existence]{cardinality} \( \mu > \kappa \), then it has a model of cardinality \( \lambda \) whenever
  \begin{equation*}
    \max\set{ \kappa, \aleph_0 } \leq \lambda < \mu.
  \end{equation*}
\end{theorem}
\begin{comments}
...
\end{comments}
\begin{proof}
  Let \( \Sigma^+ \) be the extension of \( \Sigma \) by a family of constants \( \seq{ c_\alpha }_{\alpha < \lambda} \). Let \( \Gamma \) be a set of axioms for \( T \) and let \( \Gamma^+ \) be an extension of \( \Gamma \) with a new axiom \( c_\alpha \neq c_\beta \) for every pair \( \alpha \neq \beta \).

  \SubProof{Proof that \( \Gamma^+ \) is satisfiable} Let \( \mscrX \) be an infinite model of \( T \) of cardinality \( \mu \). Fix a family \( \seq{ a_\alpha }_{\alpha < \lambda} \) of distinct elements of \( \mscrX \). Consider the expansion \( \mscrX^+ \) of \( \mscrX \) to \( \Sigma^+ \) in which \( c_\alpha \) is interpreted as \( a_\alpha \).

  Clearly \( \mscrX \) is a model of \( \Gamma \), and, since \( \Gamma \) does not contain any of the new symbols in \( \Sigma^+ \), the expansion \( \mscrX^+ \) is also a model of \( \Gamma \).

  Furthermore, by construction \( \mscrX^+ \) also satisfies the axiom \( c_\alpha \neq c_\beta \) whenever \( \alpha \neq \beta \). Therefore, \( \mscrX^+ \) is a model of the extension \( \Gamma^+ \).

  \SubProof{Proof that \( \Gamma \) has a model of cardinality \( \lambda \)} Since \( \Gamma^+ \) is satisfiable, \cref{thm:classical_propositional_satisfiable_implies_consistent} implies that it is syntactically consistent.

  Then \fullref{thm:fol_model_existence_theorem} implies that the \( \Gamma^+ \) has a \hyperref[def:fol_term_model]{term model} \( \mscrT \) of cardinality at most \( \lambda \), the number of new constants in \( \Sigma^+ \). Because of the axioms included in \( \Gamma^+ \), there must be at least \( \lambda \) different elements in \( \mscrT \). By antisymmetry, it follows that the cardinality of \( \mscrT \) is exactly \( \lambda \).

  The reduct of \( \mscrT \) to \( \Sigma \) is then the desired model of \( \Gamma \).
\end{proof}

\begin{theorem}[Upward L\"owenheim-Skolem theorem]\label{thm:upward_lowenheim_skolem}\mcite[thm. 3.2.4]{VanDalen2004LogicAndStructure}
  As in \fullref{thm:downward_lowenheim_skolem}, consider a \hyperref[def:fol_theory]{first-order theory} \( T \) over a \hyperref[def:fol_signature]{signature} \( \Sigma \) with \( \kappa \) symbols.

  If \( T \) has a \hyperref[def:fol_semantics/model]{model} of \hi{infinite} \hyperref[thm:cardinality_existence]{cardinality} \( \mu > \kappa \), then it has a model of cardinality \( \lambda \) whenever \( \lambda > \mu \).
\end{theorem}
\begin{proof}
  As in our proof of \fullref{thm:downward_lowenheim_skolem}, let \( \Sigma^+ \) be the extension of \( \Sigma \) by a family of constants \( \seq{ c_\alpha }_{\alpha < \lambda} \). Let \( \Gamma \) be a set of axioms for \( T \) and let \( \Gamma^+ \) be an extension of \( \Gamma \) with a new axiom \( c_\alpha \neq c_\beta \) for every pair \( \alpha \neq \beta \).

  \SubProof{Proof that \( \Gamma^+ \) is satisfiable} Let \( \mscrX \) be a model of \( T \) of cardinality \( \mu \).

  Fix a finite subset \( \Delta \) of \( \Gamma^+ \). Let \( c_{k_1}, \dots, c_{k_m} \) be the new constants that occur in \( \Delta \), and define
  \begin{equation*}
    \Gamma_0 \coloneqq \Gamma \cup \set{ c_{k_i} \neq c_{k_j} \given i, j = 1, \ldots, m }.
  \end{equation*}

  Let \( \Sigma_0 \) be the signature extending \( \Sigma \) only with these constants. Let \( \mscrX \) be a model of \( T \) of cardinality \( \mu \). Fix distinct elements \( a_1, \ldots, a_m \) of \( \mscrX \), and let \( \mscrX_0 \) be the expansion of \( \mscrX \) to \( \Sigma_0 \) in which we assign \( a_i \) to \( c_{k_i} \).

  Then \( \mscrX_0 \) satisfies the new axioms in \( \Gamma_0 \) in addition to those in \( \Gamma \) satisfied by \( \mscrX \).
  ...
  \SubProof{Proof that \( \Gamma \) has a model of cardinality \( \lambda \)} Because of the number of new constants in \( \Sigma^+ \), the cardinality of any model of \( \Gamma^+ \) is at least \( \lambda \).

  We can use \fullref{thm:downward_lowenheim_skolem} to construct a model of cardinality exactly \( \lambda \).
\end{proof}
