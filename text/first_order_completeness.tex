...
\begin{proof}
  ...
  \SubProof{Proof that \( \mscrX \) has an elementary extension of cardinality \( \mu \)} Let \( \mscrY = (Y, J) \) be a model of \( \Gamma^+ \). Because of the number of new constants in \( \Sigma^+ \), the cardinality of \( \mscrY \) is \hi{at least} \( \mu \). Since we can use \fullref{thm:downward_lowenheim_skolem}, without loss of generality, we can assume that \( \mscrY \) has cardinality \hi{exactly} \( \mu \).
