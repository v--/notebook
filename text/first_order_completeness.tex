...
\end{theorem}
\begin{proof}
  Fix a structure \( \mscrX = (X, I) \) of cardinality \( \kappa \) over \( \Sigma \), and denote \hyperref[def:fol_theory/model]{its theory} by \( T \).

  First, let \( \Sigma' \) be the extension of \( \Sigma \) with a new constant \( d_a \) for every element \( a \) of \( X \), and let \( \mscrX' = (X, I') \) be an expansion of \( \mscrX \) to \( \Sigma \) in which \( d_a \) is interpreted as \( a \). Let \( T' \) be the theory of \( \mscrX' \). By \cref{thm:def:fol_theory/model_theory_consistent_complete}, both \( T \) and \( T' \) are consistent and complete, so \( T' \) must be a \hyperref[def:fol_theory/conservative]{conservative extension} of \( T \).

  Let \( \Sigma^\dprime \) be a further extension of \( \Sigma' \) with a new constant \( c_\alpha \) for every \( \alpha < \mu \), and let \( \Gamma^\dprime \) be an extension of \( T' \) with a new axiom \( \synneg (c_\alpha \syneq c_\beta) \) for every pair \( \alpha \neq \beta \).

  \SubProof{Proof that \( \Gamma^\dprime \) is satisfiable} Fix a finite subset \( \Delta \) of \( \Gamma^\dprime \). Let \( c_{k_1}, \dots, c_{k_m} \) be the new constants that occur in \( \Delta \), and define
  \begin{equation*}
    \Gamma_0 \coloneqq T' \cup \set{ \synneg (c_{k_i} \syneq c_{k_j}) \given i, j = 1, \ldots, m }.
  \end{equation*}

  Let \( \Sigma_0 \) be the signature extending \( \Sigma' \) with only these constants. Fix distinct elements \( a_1, \ldots, a_m \) of \( X \), and let \( \mscrX_0 \) be the expansion of \( \mscrX' \) to \( \Sigma_0 \) in which we assign \( a_i \) to \( c_{k_i} \).

  Then \( \mscrX_0 \) satisfies the new axioms in \( \Gamma_0 \) in addition to those in \( T' \).

  Generalizing on \( \Delta \), we conclude that \( \Gamma^\dprime \) is \hyperref[def:finitely_satisfiable_set_of_sentences]{finitely satisfiable}. Satisfiability of \( \Gamma^\dprime \) itself follows from \fullref{thm:fol_semantic_compactness/satisfiable}.

  \SubProof{Proof that \( \mscrX \) has an elementary extension of cardinality \( \mu \)} Let \( \mscrY = (Y, J) \) be a model of \( \Gamma^\dprime \). Because of the number of new constants in \( \Sigma^\dprime \), the cardinality of \( \mscrY \) is at least \( \mu \).

  Let \( \mscrY' = (Y, I') \) be the reduct of \( \mscrY \) to \( \Sigma' \). Consider the function \( h: X' \to Y \) which sends \( a \) to \( \Bracks{d_a}_{\mscrY'} \). It is an elementary embedding (over \( \Sigma' \)) because the behavior of \( h \) on \( X' \) (and on \( X \)) is determined by the equalities in \( T' \) involving the constant \( d_a \).

  Define \( \mscrZ \) as the superstructure of \( \mscrX' \) obtained from \( \mscrY \) by replacing \( h(a) \) with \( a \) in the image \( h[X'] \). Thus constructed, \( \mscrZ \) is an elementary extension of \( \mscrX' \).

  It remains to take the reduct of \( \mscrZ \) to \( \Sigma \) and use \fullref{thm:downward_lowenheim_skolem} to obtain an elementary extension \( \mscrZ' \) of \( \mscrX' \) of cardinality \( \mu \). Since the theory \( T' \) of \( \mscrX' \) is conservative over \( T \), it follows that \( \mscrZ' \) is an elementary extension of \( \mscrX \).
\end{proof}
