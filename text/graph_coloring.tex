...
\begin{remark}\label{rem:estimating_ramsey_numbers}
  ...
  Searching for a monochrome triangle may require enumerating all triangles; and there are \( \binom 5 3 = 10 \) triangles in \( K_5 \).

  Thus, if we are to use na\"ive search, we need to perform thousands of checks to compute even the smallest Ramsey numbers.

  Luckily, we know about an explicit edge coloring of \( K_5 \) without monochrome triangles --- see \cref{fig:thm:ramsey_number_33}. This allows us to conclude that \( R(3, 3) = 6 \).

  For slightly larger numbers like \( R(5, 5) \), \eqref{eq:thm:def:ramsey_number/binary_bound} gives an upper bound of \( 70 \). In order to check whether some coloring of \( K_{69} \) has no monochromatic complete subgraphs of order \( 5 \), we need to inspect
  \begin{equation*}
    2^{\binom {69} 2} = 2^{2346}
  \end{equation*}
  binary edge colorings, and for each of them possibly check all \( \binom {69} 5 = \numprint{11 238 513} \) complete subgraphs of order \( 5 \).

  Therefore, other techniques must be used to give more precise bounds. The survey article \cite[4]{Radziszowski2021} states that, while an exact value for \( R(5, 5) \) is now known, we have the inequalities
  \begin{equation*}
    43 \leq R(5, 5) \leq 48.
  \end{equation*}
