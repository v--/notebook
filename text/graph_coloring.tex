...
\end{comments}
\begin{proof}
  We will use induction on \( \kappa \).
  \begin{itemize}
...
\begin{proposition}\label{thm:complete_graph_chromatic_number}
  ...
  Conversely, for a \hyperref[def:directed_graph]{simple directed graph} \( G \) of \hi{finite} order \( k \), if \( G \) is \( k \)-chromatic, then it is \hyperref[def:complete_subgraph]{complete}\fnote{By \enquote{\( G \) is complete} we mean that \( G \) is a complete subgraph of itself, i.e. it is isomorphic to \( K_k \)}.
\end{proposition}
\begin{proof}
  \SufficiencySubProof By \fullref{thm:def:graph_coloring/order}, \( \chi(K_\kappa) \leq \kappa \). Furthermore, for every \( \mu < \kappa \) and every \( \mu \)-coloring, by \fullref{thm:pigeonhole_principle}, there will be two adjacent vertices with the same color, making the coloring improper.

  Therefore, \( \chi(K_\kappa) = \kappa \).

  \NecessitySubProof Suppose that \( G = (V, E) \) is a \( k \)-chromatic graph of order \( k \). Since \( G \) has order \( k \), there exists a bijective function \( c: V \to k \), which also happens to be a \( k \)-coloring. It is proper because no two vertices have the same color.

  By \fullref{thm:graph_coloring_as_homomorphism/simple_undirected}, \( c \) is a homomorphism from \( G \) to \( K_\kappa \). We will show that it satisfies \eqref{eq:thm:graph_isomorphisms/simple_undirected}, which will make it an isomorphism.

  Given the edge \( \set{ i, j } \) in \( K_\kappa \), \fullref{thm:chromatic_number_color_pairs} implies that \( \set{ c^{-1}(i), c^{-1}(j) } \) is an edge in \( G \).

  Therefore, \( c \) is a bijective homomorphism satisfying \eqref{eq:thm:graph_isomorphisms/simple_undirected}, and \fullref{thm:graph_isomorphisms/simple_undirected} implies that it is an isomorphism between \( G \) and \( K_\kappa \).
\end{proof}
