...
\begin{example}\label{ex:ramsey_party_problem}
  ...
  Then the statement is that \( K_6 \) has a \hyperref[def:set_coloring/monochrome]{monochrome} \hyperref[def:triangle_subgraph]{triangle subgraph}. This can be seen in \cref{fig:def:graph_coloring/triangle}.
...
\begin{comments}
  ...
  The theorem has later been restated and re-proved multiple times. We generalized the statement and proof from \incite[thm. VI.1]{Bollobas1998} to \( r \) colors rather than only \( 2 \).
\end{comments}
\begin{proof}
...
\end{definition}
\begin{comments}
  \item We avoid defining Ramsey numbers for \( r = 1 \) due to possible ambiguity. \incite[183]{Bollobas1998} defines \( R(s) \) to be \( R(s, s) \), while our general definition would suggest that \( R(s) \) should instead be the degenerate case of monochromatic coloring.
\end{comments}
...
\end{proposition}
\begin{proof}
  \SubProofOf{thm:def:ramsey_number/one} There are no edges in the complete graph \( K_1 \), thus its edges are monochrome with respect to any possible edge coloring.
...
\begin{remark}\label{rem:estimating_ramsey_numbers}
  Providing a better estimate for \hyperref[def:ramsey_number]{Ramsey numbers} than the ones obtained via \fullref{thm:def:ramsey_number/binary_bound} and \fullref{thm:def:ramsey_number/superposition} can be cumbersome.

  For example, by \eqref{eq:thm:def:ramsey_number/binary_bound}, we have \( R(3, 3) \leq 6 \). If we want to verify that equality holds by enumerating all possible binary colorings of smaller graphs and searching for triangle subgraphs, we would have to traverse all edge colorings of \( K_5 \), and, if that fails, of \( K_4 \).
  ...
  For \( K_5 \), this number is \( 2^{10} = 1024 \).

  Searching for a monochrome triangle may require enumerating all triangles. There are \( \binom 5 3 = 10 \) triangles in \( K_5 \).

  It is sufficient to only find a counterexample once. We already know about an explicit edge coloring of \( K_5 \) without monochrome triangles --- \cref{fig:thm:ramsey_number_33} --- and this allows us to conclude that \( R(3, 3) = 6 \). Furthermore, if we are lucky, na\"ive search can yield a result early. In \cite{code}, the function \identifier{combinatorics.ramsey.compute_ramsey_naive} needs to only check \( 1150 \) triangles to conclude that \( R(3, 3) = 6 \).

  But in general, if we are to use na\"ive search, we need to perform an immense amount of checks. For slightly larger numbers like \( R(5, 5) \), \eqref{eq:thm:def:ramsey_number/binary_bound} gives an upper bound of \( 70 \). In order to check whether some coloring of \( K_{69} \) has no monochromatic complete subgraphs of order \( 5 \), we need to inspect
  \begin{equation*}
    2^{\binom {69} 2} = 2^{2346}
  \end{equation*}
  binary edge colorings, and for each of them possibly check all \( \binom {69} 5 = \numprint{11 238 513} \) complete subgraphs of order \( 5 \).

  Attempting to compute this na\"ively turns our not to be fruitful. Other techniques must be used to give more precise bounds. The survey article \cite[4]{Radziszowski2021} states that, while an exact value for \( R(5, 5) \) is now known, we have the inequalities
  \begin{equation*}
    43 \leq R(5, 5) \leq 48.
  \end{equation*}
...
\end{proposition}
\begin{proof}
  \Cref{fig:thm:ramsey_number_33} illustrates an edge coloring of \( K_{5,5} \) without monochrome triangle subgraphs, thus \( R(3, 3) > 5 \). By \fullref{thm:def:ramsey_number/triangle}, we have \( R(3, 3) \leq 6 \).
\end{proof}
