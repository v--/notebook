...
\begin{definition}\label{def:graph_coloring}\mcite[395]{Erickson2019Algorithms}
  A \term{vertex coloring} (resp. \term{edge coloring} or \term{arc coloring}) of a \hyperref[rem:graph_of_unspecified_type]{graph of unspecified type or hypergraph} is, unsurprisingly, a \hyperref[def:set_coloring]{coloring} of its vertices (resp. edges or arcs).
  ...
  \begin{thmenum}
    ...
    \thmitem{def:graph_coloring/colorable}\mcite[119]{Diestel2017GraphTheory} If a graph has a \( \rho \)-coloring of its \emph{vertices}, we say that it is \( \rho \)-\term[bg=\( \rho \)-оцветим (граф) (\cite[141]{Мирчев2001Графи}), ru=\( \rho \)-раскрашиваемый (граф) (\cite[\S 53]{ЕмеличевИПр1990ТеорияГрафов})]{colorable}.
  \end{thmenum}
\end{definition}
\begin{comments}
  ...
  \item A useful characterization of colorable graphs is \cref{thm:graph_coloring_as_homomorphism}.

  \item Some authors not distinguish between proper and improper colorings by requiring all colorings to be proper. This includes
  \cite[119]{Diestel2017GraphTheory},
  \cite[126]{Harary1969GraphTheory},
  \cite[145]{Bollobás1998ModernGraphTheory},
  \cite[def. 10.8.1]{Rosen2019DiscreteMathematics} and
  \cite[374]{Новиков2013ДискретнаяМатематика}.

  We prefer the more explicit terminology of
  \cite[395]{Erickson2019Algorithms},
  \cite[\S 53]{ЕмеличевИПр1990ТеорияГрафов} and
...
\begin{proposition}\label{thm:graph_coloring_as_homomorphism}
  ...
  \begin{thmenum}
    \thmitem{thm:graph_coloring_as_homomorphism/simple_undirected} In a \hyperref[def:undirected_graph]{simple undirected graph} \( G = (V, E) \), a function \( c: V \to \rho \) is a proper coloring if and only if it is a \hyperref[def:undirected_graph/morphism]{homomorphism} into the \hyperref[def:complete_graph]{complete graph} \( K_\rho \).

    \thmitem{thm:graph_coloring_as_homomorphism/simple_directed} In a \hyperref[def:directed_graph]{simple directed graph} \( G = (V, A) \), a function \( c: V \to \rho \) is a proper coloring if and only if it is a \hyperref[def:directed_graph/morphism]{homomorphism} into the \hyperref[def:graph_functors/simple_doubling]{doubling} \( D_S(K_\rho) \).

    \thmitem{thm:graph_coloring_as_homomorphism/hypergraph} In a \hyperref[def:hypergraph]{hypergraph} \( G = (V, E, \mscrE) \), a function \( c: V \to \rho \) is a proper coloring if and only if the pair \( (c, \mscrE) \) is a \hyperref[def:hypergraph/morphism]{homomorphism} into the \hyperref[def:graph_functors/undirected_inclusion]{inclusion} \( I_U(K_\rho) \).

    \thmitem{thm:graph_coloring_as_homomorphism/directed_multigraph} Finally, in a \hyperref[def:directed_multigraph]{directed multigraph} \( G = (V, E, t, h) \), a function \( c: V \to \rho \) is a proper coloring if and only if pairing \( c \) with \( e \mapsto (t(h), e(h)) \) gives a \hyperref[def:directed_multigraph/morphism]{homomorphism} into the \hyperref[def:graph_functors/directed_inclusion]{inclusion} of the \hyperref[def:graph_functors/simple_doubling]{doubling} \( I_D(D_S(K_\rho)) \).
  \end{thmenum}
\end{proposition}
\begin{proof}
  We will only prove \cref{thm:graph_coloring_as_homomorphism/simple_undirected} since the rest are obvious corollaries.
...
\begin{proposition}\label{thm:hypergraph_representation}
  ...
  \begin{thmenum}
    ...
    \begin{figure}
      ...
      \hfill
      \hfill
      \caption{A \hyperref[def:hypergraph]{hypergraph} and its associated \hyperref[def:graph_coloring/proper]{properly \( 2 \)-colored} \hyperref[def:undirected_graph]{simple undirected graph} from \cref{thm:hypergraph_representation}.}\label{fig:thm:hypergraph_representation}
    \end{figure}
    ...
    \thmitem{thm:hypergraph_representation/equivalence} The category of hypergraphs and hypergraph homomorphisms is \hyperref[rem:category_similarity/isomorphism]{isomorphic} to the category of small \( 2 \)-colored simple undirected graphs and color-preserving graph homomorphisms.
  \end{thmenum}
\end{proposition}
\begin{comments}
  \item The construction \cref{thm:hypergraph_representation/hypergraph_to_graph} is often stated via \hyperref[def:multipartite_graph]{bipartite graphs} --- for example by \textcite[187]{DörflerWaller1980Hypergraphs} and \textcite[300]{ЕмеличевИПр1990ТеорияГрафов}. Bipartite graphs may have different partitions into independent sets, thus we do not generally know what is supposed to be the vertex set in the inverse construction \cref{thm:hypergraph_representation/graph_to_hypergraph}. For this reason, we decided to explicitly rely on \( 2 \)-colorings.
\end{comments}
\begin{proof}
  Straightforward.
\end{proof}
...
\begin{definition}\label{def:graph_chromatic_number}\mcite[127]{Harary1969GraphTheory}
  The (vertex) \term[bg=хроматично число (\cite[142]{Мирчев2001Графи}), ru=хроматическое число (\cite[235]{ЕмеличевИПр1990ТеорияГрафов})]{chromatic number} \( \chi(G) \) of the \hyperref[rem:graph_of_unspecified_type]{graph of unspecified type or hypergraph} \( G \) is the minimum nonzero cardinal \( \rho \) for which \( G \) is \( \rho \)-\hyperref[def:graph_coloring/colorable]{colorable}.

  If \( \chi(G) = \rho \), we say that \( G \) is \term[ru=\( \rho \)-хроматический (граф) (\cite[236]{ЕмеличевИПр1990ТеорияГрафов})]{\( \rho \)-chromatic}.
\end{definition}
\begin{comments}
  \item Different authors use a different notation for chromatic numbers. We use \enquote{\( \chi \)} similarly to
  \cite[119]{Diestel2017GraphTheory},
  \cite[145]{Bollobás1998ModernGraphTheory},
  \cite[152]{Knauer2019AlgebraicGraphTheory},
  \cite[def. 10.8.2]{Rosen2019DiscreteMathematics},
  \cite[235]{ЕмеличевИПр1990ТеорияГрафов} and
  \cite[374]{Новиков2013ДискретнаяМатематика}.

  \Textcite[28]{GondranMinoux1984GraphsAndAlgorithms} use \enquote{\( \gamma \)}, while \textcite[142]{Мирчев2001Графи} uses \enquote{\( \varkappa \)}.
\end{comments}
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    If no arc/edge exists whose endpoints are colored \( p \) and \( q \), \cref{thm:def:graph_coloring/prec} implies that the graph has a proper \( \rho \)-coloring, which contradicts the assumption that \( G \) is \( (r + 1) \)-chromatic.
  \end{itemize}
\end{proof}

\begin{corollary}\label{thm:chromatic_number_coloring_splits}
  For every positive integer \( r \), for every \hyperref[def:graph_chromatic_number]{\( r \)-chromatic} graph \( G = (V, E) \) and for every proper coloring \( c: V \to \set{ 0, \ldots, r - 1 } \), the function \( c \) is a \hyperref[def:morphism_cancellation/right]{categorical split epimorphism} from \( G \) to the \hyperref[def:complete_graph]{complete graph} \( K_r \).
\end{corollary}
\begin{proof}
  By \cref{thm:graph_coloring_as_homomorphism/simple_undirected}, \( c \) is a homomorphism from \( G \) to \( K_r \).

  By \cref{thm:chromatic_number_color_pairs}, the function \( { u, v } \mapsto \set{ c(u), c(v) } \) is surjective on edges. Then \( c \) is surjective on vertices, hence, by \cref{thm:function_invertibility_categorical/right_invertible}, it has a right inverse
  \begin{equation*}
  ...
  Obviously \( d \) is a right inverse of \( c \). Therefore, \( c \) is right invertible in the category of small simple undirected graphs.
\end{proof}
...
\begin{proposition}\label{thm:complete_graph_chromatic_number}
  ...
  Conversely, for a \hyperref[def:directed_graph]{simple directed graph} \( G \) of \emph{finite} order \( r \), if \( G \) is \( r \)-chromatic, then it is \hyperref[def:complete_subgraph]{complete}\fnote{By \enquote{\( G \) is complete} we mean that \( G \) is a complete subgraph of itself, i.e. it is isomorphic to \( K_r \)}.
\end{proposition}
\begin{proof}
  \SufficiencySubProof By \cref{thm:def:graph_coloring/order}, \( \chi(K_\rho) \leq \rho \). Furthermore, for every \( \rho < \rho \) and every \( \rho \)-coloring, by \cref{thm:pigeonhole_principle/simple}, there will be two adjacent vertices with the same color, making the coloring improper.
  ...
  By \cref{thm:graph_coloring_as_homomorphism/simple_undirected}, \( c \) is a homomorphism from \( G \) to \( K_\rho \). We will show that it satisfies \eqref{eq:thm:graph_isomorphisms/simple_undirected}, which will make it an isomorphism.

  Given the edge \( \set{ i, j } \) in \( K_\rho \), \cref{thm:chromatic_number_color_pairs} implies that \( \set{ c^{-1}(i), c^{-1}(j) } \) is an edge in \( G \).

  Therefore, \( c \) is a bijective homomorphism satisfying \eqref{eq:thm:graph_isomorphisms/simple_undirected}, and \cref{thm:graph_isomorphisms/simple_undirected} implies that it is an isomorphism between \( G \) and \( K_\rho \).
\end{proof}
...
\end{proposition}
\begin{proof}
  First note that \( K_{V_1, \ldots, V_k} \) is \( r \)-colorable due to \cref{thm:k_colorable_iff_multipartite}.

  Furthermore, if \( V_1, \ldots, V_r \) are nonempty, again via \cref{thm:pigeonhole_principle/simple} we can conclude that every coloring with fewer colors cannot be proper.
...
\begin{proof}
  ...
  \Cref{thm:graph_coloring_as_homomorphism/simple_undirected} implies that \( c \) is a homomorphism from \( G \) to \( K_r \). By \cref{thm:complete_graph_edge_count}, \( K_r \) has \( \binom r 2 \) edges. They represent color classes. By \cref{thm:chromatic_number_color_pairs}, each of the color classes has a preimage in \( G \), i.e. an edge whose endpoints have the colors of the class.
  ...
  By \cref{thm:real_quadratic_polynomial_roots}, the roots of the polynomial are
  \begin{equation*}
  ...
  The other bound \eqref{eq:thm:chromatic_number_bound/vertices} follows from \cref{thm:complete_graph_edge_count}.
\end{proof}
...
\begin{example}\label{ex:def:graph_chromatic_number}
  ...
  \begin{thmenum}
    \thmitem{ex:def:graph_chromatic_number/edgeless} As shown in \cref{thm:chromatic_number_edgeless}, a graph is arcless/edgeless if and only if its chromatic number is \( 1 \).

    \thmitem{ex:def:graph_chromatic_number/complete} As shown in \cref{thm:complete_graph_chromatic_number}, the chromatic number of a \hyperref[def:complete_graph]{complete graph} is its order.

    \thmitem{ex:def:graph_chromatic_number/compete_multipartite} As shown in \cref{thm:complete_multipartite_graph_chromatic_number}, the chromatic number of a \hyperref[def:complete_multipartite_graph]{complete \( r \)-partite} graph with nonempty partitions is \( r \).

    \thmitem{ex:def:graph_chromatic_number/path} As per the discussion in \cref{ex:def:multipartite_graph/path}, the \hyperref[def:path_graph]{path graph} \( P_n \) has chromatic number \( 2 \).

    \thmitem{ex:def:graph_chromatic_number/cycle} As per the discussion in \cref{ex:def:multipartite_graph/cycle}, the \hyperref[def:cycle_graph]{cycle graph} \( C_n \) has chromatic number \( 2 \) if \( n \) is odd and \( 3 \) if \( n \) is even.
    ...
    Per its definition, it contains the cycle graph \( C_5 \), and hence it has an odd-length cycle. \Cref{thm:bipartite_iff_no_odd_cycles} then implies that \( P_{5,2} \) is not bipartite, and thus not \( 2 \)-colorable.

    Therefore, \( \chi(P_{5,2}) = 3 \).
  \end{thmenum}
\end{example}

\begin{theorem}[Erd\H{o}s-De Bruijn theorem]\label{thm:erdos_de_bruijn_theorem}\mcite[thm. 2.5.5]{Harzheim2005OrderedSets}
  For a \hyperref[def:undirected_graph]{simple directed graph} \( G = (V, E) \), if the positive integer \( k \) bounds the \hyperref[def:graph_chromatic_number]{chromatic number} of every finite \hyperref[def:induced_subgraph]{induced subgraph} of \( G \), then \( k \) bounds the chromatic number of \( G \) itself.
\end{theorem}
\begin{proof}
  \SubProof{Construction of maximal edge set \( M \)} Let \( F \) be the \enquote{full} set of all possible edges over \( V \), excluding loops.

  Consider the family
  \begin{equation*}
    \mscrA \coloneqq \set{ A \subseteq F \given E \subseteq A \T{and} \qforall*{U \underset {\mathclap{\T{finite}}} \subseteq V} \chi(U, A \cap U^2) \leq k }.
  \end{equation*}

  For each set \( A \) from \( \mscrA \), the graph \( (V, A) \) must satisfy the condition of the proposition --- the chromatic number of its finite induced subgraphs must not exceed \( k \).

  For every chain \( \seq{ A_k }_{k \in \mscrK} \) in \( \mscrA \), where \( A_k \subseteq A_m \) whenever \( k \leq m \), the union \( \bigcup_{k \in \mscrK} A_k \) also belongs to \( \mscrA \).

  Indeed, given a finite subset \( U \) of vertices, each edge in \( U^2 \cap \bigcup_{k \in \mscrK} A_k \) belongs to some \( A_k \). There are finitely many such edges, so there is a maximal index \( m \). Then
  \begin{equation*}
    U^2 \cap A_m = U^2 \cap \bigcup_{k \in \mscrK} A_k,
  \end{equation*}
  so, by definition of \( \mscrA \), the graph \( (U^2, U^2 \cap \bigcup_{k \in \mscrK} A_k) \) is at most \( k \)-colorable.

  \SubProof{Equivalence relation based on \( M \)} By \fullref{thm:zorns_lemma}, the set \( \mscrA \) has a maximal element \( M \). Consider the following relation on \( V \):
  \begin{equation*}
    a \sim b \T{if and only if} \set{ a, b } \not\in M.
  \end{equation*}

  This relation is symmetric by definition, and reflexive since we have disallowed loops. We must show that it is transitive.

  If \( a \sim b \) and \( b \sim c \), then neither \( \set{ a, b } \) nor \( \set{ b, c } \) are in \( M \). By the maximality of \( M \), there exist finite subsets \( M' \) and \( M^\dprime \) of \( M \) such that neither \( (V, M' \cup \set{ a, b }) \) nor \( (V, M^\dprime \cup \set{ b, c }) \) is \( k \)-colorable. In particular, any proper \( k \)-coloring \( l \) of \( (V, M' \cup M^\dprime) \) does not distinguish between \( a \) and \( b \) and \( b \) and \( c \), i.e. \( l(a) = l(b) \) and \( l(b) = l(c) \). But then \( l(a) = l(c) \), so, generalizing on \( l \), we conclude that \( (V, M' \cup M^\dprime \cup \set{ a, c }) \) is not \( k \)-colorable.

  It follows that \( (V, M \cup \set{ a, c }) \) is also not \( k \)-colorable, so \( \set{ a, c } \) is not in \( M \), i.e. we have \( a \sim c \).

  This shows that \( {\sim} \) is an equivalence relation on \( V \).

  \SubProof{Coloring of \( G \)} It remains to show that the quotient \( V / {\sim} \) has at most \( k \) equivalence classes, which can be used to color \( G \).

  If \( a \sim b \), then \( \set{ a, b } \not\in M \), and in particular \( \set{ a, b } \not\in E \). So the vertices of a single equivalence class are disconnected.

  Furthermore, any set of \( k + 1 \) edges in \( E \) form a finite subset, which is by definition \( k \)-colorable, so there are at most \( k \) equivalence classes.

  Then the projection map \( \pi: V \to V / {\sim} \) is a proper coloring of \( G \) with at most \( k \) colors.
\end{proof}
...
\begin{example}\label{ex:ramsey_party_problem}
  ...
  Compare this to the considerably easier problem from \cref{ex:thm:pigeonhole_principle/party}, in which we are interested in how many people know \emph{everybody else}.
  ...
  This example is generalized by \cref{thm:ramseys_theorem_single}, which is in turn a special case of \fullref{thm:ramseys_theorem}.
\end{example}
...
\begin{comments}
  ...
  The theorem has later been restated and re-proved multiple times. We generalized the statement and proof from \cite[thm. VI.1]{Bollobás1998ModernGraphTheory} to \( r \) colors rather than only \( 2 \).
\end{comments}
\begin{proof}
  Let us denote by \( R(s_1, \ldots, s_r) \) the corresponding Ramsey number, defined in \cref{def:ramsey_number} as the minimal positive integer \( n \) satisfying the theorem. Then our proof amounts to proving that \( R(s_1, \ldots, s_r) \) is well-defined. We will do this by induction on \( s_1 + \ldots + s_r \).
  ...
  \begin{itemize}
    ...
    Fix any vertex \( u \) of \( K_n \) and consider the partition of its other vertices into sets \( V_1, \ldots, V_r \), where \( v \) is in \( V_k \) if the edge from \( u \) to \( v \) has color \( k \). By \cref{thm:ramsey_number_lemma}, there exists an index \( k_0 \) such that \( V_{k_0} \) has at least \( r_{k_0} \) elements.
...
\begin{definition}\label{def:ramsey_number}\mcite[183]{Bollobás1998ModernGraphTheory}
  Given an \( r \)-tuple \( s_1, \ldots, s_r \), where \( r \geq 2 \), we define the \term{Ramsey number} \( R(s_1, \ldots, s_r) \) as the least positive integer for which \fullref{thm:ramseys_theorem} holds.
\end{definition}
\begin{comments}
  \item We avoid defining Ramsey numbers for \( r = 1 \) due to possible ambiguity. \Textcite[183]{Bollobás1998ModernGraphTheory} defines \( R(s) \) to be \( R(s, s) \), while our general definition would suggest that \( R(s) \) should instead be the degenerate case of monochromatic coloring.
\end{comments}
...
\begin{proof}
  ...
  The other case \( R(2, s) \) follows from \cref{thm:def:ramsey_number/symmetric}.
  ...
  \begin{itemize}
    \item If \( s = 1 \), \cref{thm:def:ramsey_number/one} implies that \( R(s, t) = 1 \), while
    \begin{equation*}
...
\end{proposition}
\begin{proof}
  \Cref{fig:thm:ramsey_number_33} illustrates an edge coloring of \( K_{5,5} \) without monochromatic triangle subgraphs, thus \( R(3, 3) > 5 \). By \cref{thm:def:ramsey_number/triangle}, we have \( R(3, 3) \leq 6 \).
\end{proof}

\begin{remark}\label{rem:estimating_ramsey_numbers}
  Even though \cref{thm:def:ramsey_number/binary_bound} and \cref{thm:def:ramsey_number/superposition} provide upper bounds for \hyperref[def:ramsey_number]{Ramsey numbers}, obtaining the numbers themselves can be cumbersome. The survey article \cite{Radziszowski2021SmallRamseyNumbers} provides a lot of upper and lower bounds since only a few Ramsey numbers are known.
  ...
  By \cref{thm:complete_graph_edge_count}, there are \( \binom n 2 \) edges in the complete graph \( K_n \), hence the number of \( r \)-colorings is
  \begin{equation*}
    r^{\binom n 2}.
  \end{equation*}
  ...
  If we are lucky, exhaustive search can yield a result early. This can be verified by naive computation that we have implemented in \cite{notebook:code} as
  \begin{CodeRefDisplay}
    \GetCodeRef{alg:fol_term_signature_translation}
  \end{CodeRefDisplay}

  The calculation needs to only check \( \numprint{221} \) colorings and a total of \( \numprint{729} \) triangles to conclude that \( R(3, 3) = 6 \).
  ...
  \Textcite[thm. 1.1]{AngeltveitMcKay2026Ramsey5} improve the upper bound to \( 46 \).
