...
\begin{definition}\label{def:array}\mimprovised
  ...
  \begin{thmenum}
    ...
    When \( S \) is a \hyperref[def:semiring]{semiring} \( R \), we often identify the set of all \( m \)-dimensional column vectors with the \hyperref[def:coordinate_space]{coordinate space} \( R^m \).
...
\begin{proposition}\label{thm:matrix_algebra}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:matrix_algebra/matrix_multiplication} We define \term{matrix multiplication} in two steps. The definition is justified by \cref{thm:matrix_and_linear_function_algebras}. First, if \( \seq{ a_j }_{j=1}^n \) is a \hyperref[def:array/row_vector]{row vector} and \( \seq{ b_i }_{j=1}^n \) is a \hyperref[def:array/column_vector]{column vector}, we define their \term{inner product} as
    \begin{equation*}
...
\begin{proposition}\label{thm:matrix_and_linear_function_algebras}
  For a \hyperref[def:ring/commutative]{commutative ring} \( R \), the \hyperref[thm:matrix_algebra]{matrix algebra} \( R^{m \times n} \) is \hyperref[def:algebra_over_semiring/morphism]{isomorphic} to the \hyperref[def:fol_structure_of_functions]{linear function algebra} \( \hom(R^n, R^m) \)\fnote{Note that the maps are from \( R^n \) to \( R^m \) and not vice versa}.
\end{proposition}
\begin{proof}
  Follows from our discussion in \cref{rem:matrices_as_functions} due to linearity.
\end{proof}

\begin{remark}\label{rem:double_index_maps}
  We want to be able to map single indices to double indices and vice versa, for example for the purpose of \cref{thm:matrix_spaces_are_free_modules}. As an example, we want to be able to \enquote{linearize} an \( m \times n \) matrix such as the \( 2 \times 3 \) matrix
  ...
  We can easily verify that \( \sharp \) is a \hyperref[def:morphism_invertibility/left]{left inverse} of \( \flat \) (note that \( j < m \)):
  \begin{align*}
    \sharp(\flat(i, j))
    &=
  ...
  We can just as easily verify that \( \flat \) is a \hyperref[def:morphism_invertibility/right]{right inverse} of \( \sharp \):
  \begin{align*}
    \flat(\sharp(k))
    &=
  ...
  Hence, \( \sharp \) is fully invertible with inverse \( \flat \). By \cref{thm:function_invertibility_categorical/fully_invertible}, it is bijective.
\end{remark}
...
\end{proposition}
\begin{proof}
  \Cref{rem:double_index_maps} gives us a semimodule isomorphism between \( m \times n \) matrices and \( mn \)-dimensional column vectors when extended to linear maps via \fullref{thm:free_semimodule_universal_property}.
\end{proof}
...
\begin{definition}\label{def:matrix_determinant}\mcite[215]{Knapp2016BasicAlgebra}
  ...
  Following \textcite[493]{Eisenbud1995CommutativeAlgebra}, we will define the determinant for the \hyperref[ex:def:array/zero_dimensional]{zero-dimensional matrix} as \( 1 \).

  \Cref{thm:similar_matrices_and_determinants} allows us to define determinants for linear endomorphisms rather than square matrices.

  See our proof of \cref{thm:determinant_on_columns} for a justification of the definition.
\end{definition}
...
\begin{definition}\label{def:symmetric_function}\mcite[def. 2.11.1]{Savage2008ModelsOfComputation}
  ...
  A permutation can be decomposed into \hyperref[def:cyclic_permutation]{transpositions} due to \cref{thm:permutation_decomposition_existence}. Hence, the above condition reduces to the simpler condition of \( f \) being invariant with respect to swapping any two arguments. That is,
  \begin{equation*}
  ...
  Symmetric functions should not be confused with symmetric binary relations defined in \cref{def:binary_relation/symmetric}.
\end{definition}

\begin{definition}\label{def:antisymmetric_function}\mimprovised
  Given a function \( f: X^n \to Y \), where \( X \) is a \hyperref[def:set]{plain set} and \( Y \) is an \hyperref[def:additive_semigroup]{additive group}, we say that \( f \) is \term{antisymmetric} if, for any \hyperref[def:symmetric_group]{permutation} \( \sigma \in S_n \), we have
  \begin{equation*}
    f(x_1, \ldots, x_n) = \sgn(\sigma) \cdot f(x_{\sigma(1)}, \ldots, x_{\sigma(n)}).
  \end{equation*}

  A permutation can be decomposed into \hyperref[def:cyclic_permutation]{transpositions} due to \cref{thm:permutation_decomposition_existence}. Hence, the above condition reduces to the simpler condition of \( f \) changing sign when swapping any two arguments. That is,
  \begin{equation*}
    f(\ldots, x_{i-1}, \fbox{\( x_i \)}, x_{i+1}, \cdots, x_{j-1}, \fbox{\( x_j \)}, x_{j+1}, \ldots)
    =
    -f(\ldots, x_{i-1}, \fbox{\( x_j \)}, x_{i+1}, \cdots, x_{j-1}, \fbox{\( x_i \)}, x_{j+1}, \ldots).
  ...
  Antisymmetric functions should not be confused with antisymmetric binary relations defined in \cref{def:binary_relation/antisymmetric}.
\end{definition}
...
\begin{proof}
  ...
  Finally, since \( f \) is \hyperref[def:antisymmetric_function]{antisymmetric} due to \cref{thm:alternating_multilinear_is_antisymmetric},
  \begin{equation*}
...
\begin{proposition}\label{thm:def:matrix_determinant}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:def:matrix_determinant/homomorphism}\mcite[\S 6.7]{Тыртышников2007ЛинейнаяАлгебра} \( \det(AB) = \det(A) \cdot \det(B) \).

    That is, \( \det: R^{n \times n} \to R \) is a \hyperref[def:monoid/morphism]{monoid homomorphism} from the \hyperref[def:semiring]{multiplicative monoid} of the ring \( R^{n \times n} \) to the multiplicative monoid of \( R \).
  \end{thmenum}
\end{proposition}
\begin{proof}
  ...
  \SubProofOf{thm:def:matrix_determinant/homogeneous} Follows from \cref{thm:determinant_on_columns}.
...
\begin{theorem}[Laplace expansion]\label{thm:laplace_expansion}\mcite[prop. 2.36]{Knapp2016BasicAlgebra}
  ...
  By \cref{thm:def:matrix_determinant/transpose}, we can also expand along a column rather than a row.
\end{theorem}
\begin{proof}
...
\begin{proof}
  ...
  Then \eqref{eq:thm:3x3_determinant} follows via \cref{thm:2x2_determinant}.
\end{proof}

\begin{remark}\label{rem:3x3_determinant}
  The expression \eqref{eq:thm:3x3_determinant} for calculating the \( 3 \times 3 \) determinant in \cref{thm:3x3_determinant} can easily be remembered via a mnemonic.
...
\begin{definition}\label{def:inverse_matrix}
  We say that \( B \in R^{n \times m} \) is a \term{left inverse matrix} of \( A \in R^{m \times n} \) if \( BA \) is the identity matrix \( I_n \) and a \term{right inverse matrix} if \( AB \) is \( I_m \). These are precisely the left and right inverse linear maps in the correspondence described in \cref{thm:matrix_and_linear_function_algebras}.

  Due to \cref{thm:square_matrix_left_invertible_iff_right_invertible}, for square matrices, the two notions coincide, and we say that \( B \) is simply an \term{inverse} of \( A \). An inverse matrix, if it exists, is unique. We denote this inverse of \( A \) by \( A^{-1} \).

  We say that \( A \) is \term{invertible} if an inverse exists, and \term{singular} otherwise.
\end{definition}
\begin{defproof}
  The inverse is unique by \cref{thm:def:monoid_inverse/unique}.
\end{defproof}
...
\end{proposition}
\begin{proof}
  \Cref{thm:group_homomorphism_trivial_kernel} implies that \( A \) is invertible if and only if its kernel is trivial.
\end{proof}
...
\end{proposition}
\begin{proof}
  \NecessitySubProof Suppose that \( A \) is a right invertible matrix. When regarding \( A \) as a linear map via the identification from \cref{rem:matrices_as_functions}, this implies that \( A \), as a linear map from \( R^n \) to \( R^n \), is right invertible. Then it is surjective and, by \cref{thm:surjective_endomorphism_in_free_module}, an isomorphism. Therefore, \( A \) is a fully invertible matrix.
...
\begin{proposition}\label{thm:matrix_invertibility_via_determinants}\mcite[corr. 5.5]{Knapp2016BasicAlgebra}
  Over a \hyperref[def:ring/commutative]{commutative ring} \( R \), the matrix \( A \) is \hyperref[def:inverse_matrix]{invertible} if and only if its \hyperref[def:matrix_determinant]{determinant} is \hyperref[def:semiring_divisibility/invertible]{invertible}.
\end{proposition}
\begin{proof}
...
\end{corollary}
\begin{proof}
  Suppose that \( A \) is an invertible matrix in \( R^{n \times n} \). \Cref{thm:matrix_invertibility_via_determinants} implies that \( \det A \) is invertible in \( R \) and \cref{thm:def:matrix_determinant/transpose} implies that \( \det A^T \) is invertible as well. Then \cref{thm:matrix_invertibility_via_determinants} implies that \( A^T \) is also invertible in \( R^{n \times n} \).
\end{proof}
...
\end{proposition}
\begin{proof}
  \SufficiencySubProof As in \cref{thm:determinant_on_columns}, regard \( \det(v_1, \ldots, v_n) \) are an alternating multilinear function on the columns of a matrix.

  Suppose that \( A \) is invertible. Then \cref{thm:matrix_invertibility_via_determinants} implies that \( \det(v_1, \ldots, v_n) \) is invertible.
  ...
  But we have assumed that \( \det( v_1, \ldots, v_{k-1}, v_k, v_{k+1}, \ldots, v_n ) \) is invertible and that \( t_k \) is nonzero. Hence, the determinant can only be a proper zero divisor if the ring is trivial, which we have assumed it is not. The obtained contradiction shows that \( v_1, \ldots, v_n \) are linearly independent.
  ...
  Since the columns are linearly independent, only \( x_1 = \cdots = x_n \) is a solution to this equation. Thus, when regarding \( A \) as the linear map \( x \mapsto Ax \), the \hyperref[def:module/kernel]{kernel} of \( A \) becomes trivial. By \cref{thm:group_homomorphism_trivial_kernel}, this map is injective. As discussed in \cref{def:module/category}, the injective linear maps are exactly the left invertible linear maps. Hence, there exists a left inverse of \( A \). Since \( A \) is a square matrix, by \cref{thm:square_matrix_left_invertible_iff_right_invertible}, this implies that \( A \) is invertible.
\end{proof}
...
\end{proposition}
\begin{proof}
  Follows from \cref{thm:inverse_via_adjunction}.
\end{proof}
...
\end{definition}
\begin{comments}
  \item The special linear group \( \grp{SL}_R(n) \) is \hyperref[def:normal_subgroup]{normal} in \( \grp{GL}_R(n) \) as the kernel of the determinant.
\end{comments}
...
\end{proposition}
\begin{proof}
  By \cref{thm:def:matrix_determinant/homomorphism} and \cref{thm:determinant_of_inverse},
  \begin{equation*}
...
\begin{remark}\label{rem:linear_operators_and_matrices}
  Let \( R \) be a \hyperref[def:ring/commutative]{commutative ring}. Unlike in the \( R \)-module \( R^n \) of tuples, in a general \( R \)-module of finite \hyperref[def:module_rank]{rank}, we have no concept of a \hyperref[def:coordinate_space]{standard basis}. Instead, a linear operator \( T: U \to V \) corresponds to multiple matrix.

  If \( U \) has rank \( n \), a choice of basis for \( U \) is simply a choice of isomorphism with \( R^n \). Hence, given isomorphisms \( \varphi: U \to R^n \) and via \( \psi: V \to R^m \), the operator \( \psi^{-1} \bincirc T \bincirc \varphi \) becomes a function from \( R^n \) to \( R^m \). We can now use the identification with matrices discussed in \cref{rem:matrices_as_functions}.

  This requires a choice of bases for \( U \) and for \( V \). If we have a matrix representing an operator and if we wish to use different bases, we must multiply it with the corresponding change of basis matrices discussed in \cref{rem:matrices_as_functions}.
\end{remark}
