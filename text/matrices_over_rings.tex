...
\begin{definition}\label{def:array}\mimprovised
  ...
  In particular:
  \begin{thmenum}
    \thmitem{def:array/matrix} A two-dimensional array of shape \( m \times n \) is usually called a \term[ru=матрица (\cite[\S 1.1.2]{ПетровЗяпков2000ЛААГТом1}), ru=матрица (\cite[sec. 1.1]{Тыртышников2007ЛинАлгебра})]{matrix}. Let \( A \) be an \( m \times n \)-matrix. We will denote \( A \) as
    \begin{equation*}
...
\begin{definition}\label{def:symmetric_function}\mcite[def. 2.11.1]{Savage1998Computability}
...
\begin{proposition}\label{thm:def:matrix_determinant}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:def:matrix_determinant/homomorphism}\mcite[sec. 6.7]{Тыртышников2007ЛинАлгебра} \( \det(AB) = \det(A) \cdot \det(B) \).
