...
\begin{definition}\label{def:array}\mimprovised
  ...
  In particular:
  \begin{thmenum}
    \thmitem{def:array/matrix} A two-dimensional array of shape \( m \times n \) is usually called a \term[ru=матрица (\cite[\S 1.1.2]{ПетровЗяпков2000ЛААГТом1}), ru=матрица (\cite[sec. 1.1]{Тыртышников2007ЛинейнаяАлгебра})]{matrix}. Let \( A \) be an \( m \times n \)-matrix. We will denote \( A \) as
    \begin{equation*}
