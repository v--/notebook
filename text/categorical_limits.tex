...
\begin{definition}\label{def:comma_category}\mcite[46]{MacLane1998CategoryTheory}
  Comma categories allow us to define morphisms between morphisms, which becomes useful in, for example, the concise definition of a limit in \cref{def:category_of_cones/limit}.
...
\begin{definition}\label{def:factors_through}\mimprovised
  ...
  If \( g \) and \( g \) are uniquely determined by \( f \) and \( X \), we say that \( f \) \term{uniquely factors through} \( X \).
\end{definition}
\begin{comments}
...
\begin{definition}\label{def:category_of_cones}
  ...
  \begin{thmenum}
    ...
    From \cref{thm:def:universal_objects/unique} and \cref{thm:universal_objects_as_adjunctions/terminal} it follows that a limit, if it exists, is unique up to a unique isomorphism.
    ...
    \thmitem{def:category_of_cones/cocone} Dually, a \( D \)-\term{cocone} with vertex \( A \) is a family of morphisms
    ...
    The analogous diagram to \eqref{eq:def:category_of_cones/limit} is exactly its opposite:
...
\begin{lemma}\label{thm:categorical_limit_uniqueness_lemma}
  ...
  We prove a stronger result in \cref{thm:categorical_limit_uniqueness}.
\end{lemma}
\begin{proof}
  ...
  Now let \( (L', \lambda') \) and \( (L^\dprime, \lambda^\dprime) \) be colimit cocones. By \cref{thm:categorical_limit_duality}, \( (L', \lambda'^\oppos) \) and \( (L^\dprime, {\lambda^\dprime}^\oppos) \) are limits in the opposite category and are thus isomorphic. By \cref{thm:morphism_invertibility_duality}, the colimits are isomorphic.
\end{proof}
...
\end{proposition}
\begin{proof}
  It is sufficient to prove this for limits since the statement for colimits follows from the duality principles \cref{thm:category_adjunction_duality} and \cref{thm:categorical_limit_duality}. The unit \( \eta: \id_{\cat{C}} \to [{\lim} \bincirc \Delta] \) of the adjunction
  \begin{equation*}
    \Delta \dashv \lim
  \end{equation*}
  ...
  It follows that both \( \lim(D) \) and \( [{\lim} \bincirc \Delta \bincirc {\lim}](D) \) are limits over the same diagram. By \cref{thm:categorical_limit_is_adjoint}, they are isomorphic, and hence \eqref{eq:thm:categorical_limit_is_adjoint/ic_triangle} commutes.
...
\begin{corollary}\label{thm:categorical_limit_uniqueness}
  ...
  This statement strengthens \cref{thm:categorical_limit_uniqueness_lemma}.
\end{corollary}
\begin{proof}
  Follows from \cref{thm:functor_adjoint_uniqueness} and \cref{thm:categorical_limit_is_adjoint}.
\end{proof}

\begin{remark}\label{rem:categorical_limit_uniqueness}
  \todo{}.
\end{remark}

\begin{remark}\label{rem:limit_universal_mapping_property}
  The limit diagram \eqref{eq:def:category_of_cones/limit} may seem unrelated to the universal mapping properties discussed in \cref{rem:universal_mapping_property}, however it is actually a special case.
  ...
  \begin{align*}
    ...
    \Delta: &\cat{C} \to [\cat{I}, \cat{C}]
  \end{align*}
  discussed in \cref{thm:categorical_limit_is_adjoint}. We have established that \( \Delta \dashv \lim \).
...
\begin{proposition}\label{thm:limits_of_empty_diagram}
  ...
  Compare this result with \cref{thm:limits_of_identity_functor}.
\end{proposition}
\begin{proof}
  ...
  The statement for colimits follows by \fullref{thm:category_duality}.
\end{proof}
...
\begin{proposition}\label{thm:limits_of_identity_functor}
  ...
  \begin{thmenum}
    \thmitem{thm:limits_of_identity_functor/initial_object_ks_limit} If \( I \) is an \hyperref[def:universal_objects/initial]{initial object} of \( \cat{C} \), \( (I, \xi) \) is a limit cone of the \hyperref[def:category_of_small_categories/identity]{identity functor} \( \id_{\cat{C}} \), where
    \begin{equation*}
  ...
  Dually, by \cref{thm:universal_object_duality} and \cref{thm:categorical_limit_duality}, the cocones (and colimits) of the identity functor are the terminal objects.

  Compare this result with \cref{thm:limits_of_empty_diagram}.
\end{proposition}
\begin{proof}
...
\begin{example}\label{ex:limits_of_partially_ordered_set}
  ...
  Let \( (P, \leq) \) be a \hyperref[def:partially_ordered_set]{partially ordered set} and \( \cat{P} \) be its corresponding \hyperref[def:skeletal_category]{skeletal} \hyperref[def:thin_category]{thin category}. The correspondence is discussed in \fullref{thm:order_category_isomorphism}.

  The image of a diagram \( D: \cat{I} \to \cat{P} \) is a pair \( (A, R) \), where \( A \) is the set of objects \( D(\obj(\cat{I})) \) in the image \( D(\cat{I}) \) and \( R \) is a subrelation of \( \leq \). This relation is reflexive, however it may not even be a preorder as shown in \cref{ex:def:functor/image_no_category}.
  ...
  Dually, cocones are upper bounds and colimits are suprema.
...
\begin{definition}\label{def:direct_and_inverse_limits}
  We will define limits and colimits of diagrams over infinite \hyperref[def:partial_order_connectedness]{chains} of integers. These notions predate limits and colimits, which explains why their names may seem inconsistent with other limits and colimits.

  \begin{thmenum}
    \thmitem{def:direct_and_inverse_limits/direct}\mcite[example 5.2.15]{Leinster2014BasicCategoryTheory} Consider the category \hyperref[thm:order_category_isomorphism]{induced} by the positive integers
    ...
    \thmitem{def:direct_and_inverse_limits/inverse}\mcite[example 5.1.21(d)]{Leinster2014BasicCategoryTheory} Now consider the opposite category
...
\begin{example}\label{ex:def:direct_and_inverse_limits}
  We list several examples of \hyperref[def:direct_and_inverse_limits/direct]{direct} and \hyperref[def:direct_and_inverse_limits/inverse]{inverse} limits.
  ...
  \begin{thmenum}
    ...
    \thmitem{ex:def:direct_and_inverse_limits/vector_space_inverse} Dually, consider the chain
    ...
    Then the intersection \( \bigcap_{k=1}^\infty A_k \) along with its inclusion maps is a limit of \eqref{eq:ex:inverse_limit_as_intersection/diagram}. This is a consequence of the discussion in \cref{ex:limits_of_partially_ordered_set}.
...
\begin{concept}\label{con:inclusion_and_projection}
  \todo{}
\end{concept}

\begin{definition}\label{def:discrete_category_limits}
  Fix an arbitrary category \( \cat{C} \). A \term{product} in \( \cat{C} \) is a \hyperref[def:category_of_cones/limit]{limit} over a diagram \( D: \cat{I} \to \cat{C} \), whose domain \( \cat{I} \) is a \hyperref[def:discrete_category]{discrete category}. Dually, a \term{coproduct} or \term{sum} in \( \cat{C} \) is a colimit of \( D \).

  It will be convenient for us to speak about the product of an \hyperref[def:indexed_family]{indexed family} \( \seq{ X_k }_{k \in \mscrK} \) of objects in \( \cat{C} \).
  ...
  From \cref{thm:limits_of_empty_diagram} it follows that the product (resp. coproduct) of an empty family is a terminal (resp. initial) object of \( \cat{C} \).

  As in the case of general limits, we call \( \prod_{k \in \mscrK} X_k \) \emph{the} product of the family \( X \).
...
\begin{remark}\label{rem:sum_and_coproduct}
\end{remark}

\begin{proposition}\label{thm:product_category_is_product}
\end{proposition}
...
\begin{definition}\label{def:coproduct_category}\mimprovised
  ...
  \begin{thmenum}[series=def:coproduct_category]
...
\end{definition}
\begin{comments}
  \item Coproduct categories are thus named because they are \hyperref[def:discrete_category_limits]{coproducts} in the \hyperref[def:category_of_small_categories]{category of small categories} --- see \cref{thm:coproduct_category_is_coproduct}.
...
\begin{proposition}\label{thm:coproduct_category_is_coproduct}
  The \hyperref[def:discrete_category_limits]{categorical coproduct} of the family \( \seq{ \cat{C}_k }_{k \in \mscrK} \) in the category \hyperref[def:category_of_small_categories]{\( \cat{Cat} \)} of \hyperref[def:small_set]{small} categories is their \hyperref[def:coproduct_category]{disjoint union} \( \coprod_{k \in \mscrK} \cat{C}_k \).
\end{proposition}
\begin{proof}
...
\begin{definition}\label{def:equalizers}\mcite[def. 5.1.11, def. 5.2.7]{Leinster2014BasicCategoryTheory}
  ...
  A \hyperref[def:category_of_cones/limit]{limit} of a this diagram is called an \term[ru=уравнитель (\cite[\S 3.2.4]{КусраевКутателадзе2005БулевозначныйАнализ})]{equalizer} and a \hyperref[def:category_of_cones/colimit]{colimit} --- a \term[ru=коуравнитель (\cite[\S 3.2.4]{КусраевКутателадзе2005БулевозначныйАнализ})]{coequalizer}. Note the special condition from \cref{def:categorical_diagram} that does not require parallel morphisms to be equal in a commuting diagram.

  To analyze these definitions in more detail, let us label the diagram as follows:
  \begin{equation}\label{eq:def:equalizers/labeled_index}
    \begin{aligned}
  ...
  \begin{thmenum}
    \thmitem{def:equalizers/equalizer} A \hyperref[def:category_of_cones/cone]{cone} with vertex \( X \) over this diagram has a pair of morphisms \( e: X \to A \) and \( e': X \to B \) such that the following diagram commutes:
    \begin{equation}\label{eq:def:equalizers/equalizer/cone}
      \begin{aligned}
        \includegraphics[page=3]{output/def__equalizers}
      \end{aligned}
    \end{equation}

    This requires \( e' \), \( g \bincirc e \) and \( f \bincirc e \) to coincide. The morphism \( e' \) becomes redundant; hence, we will we regard the cone as a pair \( (X, e) \). This convention is used by \cite[20]{Johnstone1982StoneSpaces}, \cite[def. 5.1.11]{Leinster2014BasicCategoryTheory} and \cite[\S 3.2.4]{КусраевКутателадзе2005БулевозначныйАнализ}, among others.

    \Cref{thm:equalizer_invertibility} implies that the map \( e: X \to A \) is a \hyperref[def:morphism_cancellation/left]{monomorphism}. This enables us to view a cone as a \hyperref[def:categorical_subobject]{subobject} with an associated inclusion map.

    A limit cone \( (L, \iota) \) must satisfy the following \hyperref[rem:limit_universal_mapping_property]{universal mapping property}:
    \begin{displayquote}
      For every cone \( (X, e) \) of \eqref{eq:def:equalizers/labeled_index}, \( e \) \hyperref[def:factors_through]{uniquely factors through} \( L \): there exists a unique morphism \( l_X: X \to L \) such that the following diagram commutes:
      \begin{equation}\label{eq:def:equalizers/equalizer/property}
        \begin{aligned}
    ...
    \thmitem{def:equalizers/coequalizer} A \hyperref[def:category_of_cones/cocone]{cocone} with vertex \( Y \) over this diagram has a single morphism \( e: B \to Y \) such that the following diagram commutes:
    \begin{equation}\label{eq:def:equalizers/coequalizer/cocone}
      \begin{aligned}
        \includegraphics[page=5]{output/def__equalizers}
      \end{aligned}
    \end{equation}

    \Cref{thm:equalizer_invertibility} implies that the map \( e: B \to Y \) is an \hyperref[def:morphism_cancellation/right]{epimorphism}. This enables us to view a cocone as a \hyperref[def:categorical_quotient_object]{quotient object} with an associated projection map.

    A limit cocone \( (L, \pi) \) satisfies the following \hyperref[rem:limit_universal_mapping_property]{universal mapping property}:
    \begin{displayquote}
      For every cocone \( (Y, e) \) of \eqref{eq:def:equalizers/labeled_index}, \( e \) \hyperref[def:factors_through]{uniquely factors through} \( L \): there exists a unique morphism \( l_X: L \to X \) such that the following diagram commutes:
      \begin{equation}\label{eq:def:equalizers/coequalizer/property}
        \begin{aligned}
...
\end{definition}
\begin{comments}
  \item Some authors like \textcite[def. 21.4.1]{Schubert1972Categories} call the coequalizer diagram \eqref{eq:def:equalizers/coequalizer/cocone} a \enquote{fork}.

  \item In connection with \cref{thm:equalizer_invertibility}, we have interchanged the notation for the projections and inclusions compared to \cref{def:discrete_category_limits} --- \( (L, \iota) \) is a \emph{limit}, while \( (L, \pi) \) is a \emph{colimit}.
\end{comments}

\begin{proposition}\label{thm:equalizer_invertibility}
  If \( (L, \iota) \) is an \hyperref[def:equalizers]{equalizer cone} in the category \( \cat{C} \), then \( \iota \) is a \hyperref[def:morphism_cancellation/left]{monomorphism}.

  Dually, if \( (L, \pi) \) is a \hyperref[def:equalizers]{coequalizer cone}, then \( \pi \) is an \hyperref[def:morphism_cancellation/right]{epimorphism}.
\end{proposition}
\begin{proof}
  Fix an equalizer cone \( (L, \iota) \) of \eqref{eq:def:equalizers/labeled_index}. Fix any object \( X \) in \( \cat{C} \) and any two parallel morphisms \( h_1, h_2: X \to A \) such that
  \begin{equation*}
  ...
  \end{equation*}
  and thus \( (X, \iota \bincirc h_1) \) is a cone. Similarly, \( (X, \iota \bincirc h_2) \) is a cone.
  ...
  Therefore, \( h_1 = h_2 \) and, since \( h_1 \) and \( h_2 \) were arbitrary, it follows that \( \iota \) is a monomorphism.

  Dually, if \( (L, \pi) \) is a colimit cocone, by \cref{thm:categorical_limit_duality}, \( (L, \pi^\oppos) \) is an equalizer in \( \cat{C}^\oppos \). Hence, \( \pi^\oppos \) is a monomorphism, and by \cref{thm:morphism_invertibility_duality}, \( \pi \) is an epimorphism.
\end{proof}

\begin{example}\label{ex:equalizers_in_set}
  We will analyze \hyperref[def:equalizers]{equalizers and coequalizers} in a \hyperref[def:category_of_small_sets]{category of small sets} \( \cat{Set} \).

  \begin{thmenum}
    \thmitem{ex:equalizers_in_set/equalizer} For a pair of functions \( f, g: A \to B \) in \( \cat{Set} \), define the set
    \begin{equation*}
      L \coloneqq \set{ x \in A \given f(x) = g(x) }.
    \end{equation*}

    With the inclusion map \( \iota: L \to A \), this is an \hyperref[def:equalizers]{equalizer} cone for \( f \) and \( g \).

    We will now prove that it is a limit cone. For any cone \( (X, e) \), we must find a map \( l_X: X \to L \) such that the universal property diagram \eqref{eq:def:equalizers/equalizer/property} commutes.

    In order for \( (X, e) \) to be a cone, the set-theoretic image \( e[X] \) must be a subset of \( L \), i.e. we must define \( l_A \) as \( e \) with its codomain restricted to \( L \).

    It follows that \( (L, \iota) \) is a limit cone.

    This example will be important for us when discussing kernels in certain categories of first-order models in \cref{thm:equational_theory_zero_morphisms}.

    \thmitem{ex:equalizers_in_set/coequalizer} The coequalizer of \( f, g: A \to B \) is more nuanced.

    Let \( {\cong} \) be the equivalence relation on \( B \) generated, in accordance with \cref{thm:generated_equivalence_relation}, by the relation \( {\sim} \) such that \( f(a) \sim g(a) \) for every \( a \) in \( A \).
    ...
    Fix a cocone \( (Y, e) \). We will now construct a map \( l_Y: L \to Y \) such that the universal property diagram \eqref{eq:def:equalizers/coequalizer/property} commutes. In particular, we must have \( l_Y(\pi(b)) = e(b) \). We can use this as a definition as long as it is well-defined.

    We will use \fullref{thm:induction_on_recursively_defined_relations} on \( b \cong b' \) to show that \( e(b) = e(b') \).
    \begin{itemize}
      \item If \( b \cong b' \) due to \ref{inf:thm:generated_equivalence_relation/intro}, there exists some \( a \) such that \( b = f(a) \) and \( b' = g(a) \). Then \( e(f(a)) = e(g(a)) \) since \( (Y, e) \) is a cocone.

      \item If \( b \cong b' \) due to \ref{inf:thm:generated_equivalence_relation/refl}, then \( b = b' \) and clearly \( e(b) = e(b') \).
      ...
      Then \( e(b') = e(b) \), so naturally \( e(b) = e(b') \).
      ...
      Then \( e(b) = e(b^\dprime) = e(b') \).
    \end{itemize}

    Therefore, we can indeed define \( l_Y(\pi(b)) \) as \( e(b) \), and this map would satisfy the universal property. Uniqueness of \( l_Y \) follows from the requirement that \( l_Y(\pi(b)) \) must equal \( e(b) \).
...
\begin{definition}\label{def:complete_category}\mcite[109]{MacLane1998CategoryTheory}
  We say that a category is \term{complete} if every \hyperref[def:small_set]{small} \hyperref[def:categorical_diagram]{diagram} has a limit.
\end{definition}
\begin{comments}
...
\begin{definition}\label{def:exponential_object}
\end{definition}
