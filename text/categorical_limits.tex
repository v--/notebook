...
\begin{proposition}\label{thm:limits_of_identity_functor}
  ...
  \hyperref[thm:categorical_principle_of_duality]{Dually}, by \cref{thm:universal_object_duality} and \cref{thm:categorical_limit_duality}, the cocones (and colimits) of the identity functor are the terminal objects.
...
\begin{proposition}\label{thm:equalizer_invertibility}
  ...
  \hyperref[thm:categorical_principle_of_duality]{Dually}, if \( (L, \pi) \) is a \hyperref[def:equalizers]{coequalizer cone}, then \( \pi \) is an \hyperref[def:morphism_invertibility/right_cancellative]{epimorphism}.
\end{proposition}
\begin{proof}
...
\begin{definition}\label{def:compete_category}\mcite[109]{MacLane1998CategoryTheory}
  We say that a category is \term{complete} with respect to a \hyperref[def:grothendieck_universe]{Grothendieck universe} \( \mscrU \) if every \( \mscrU \)-small \hyperref[def:categorical_diagram]{diagram} has a limit.
\end{definition}
\begin{comments}
