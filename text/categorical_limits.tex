...
\begin{definition}\label{def:category_of_cones}
  ...
  \begin{thmenum}
    ...
    From \cref{thm:def:universal_objects/terminal} and \cref{thm:universal_objects_as_adjunctions/terminal} it follows that a limit, if it exists, is unique up to a unique isomorphism.
...
\begin{example}\label{ex:limits_of_partially_ordered_set}
  ...
  The image of a diagram \( D: \cat{I} \to \cat{P} \) is a pair \( (A, R) \), where \( A \) is the set of objects \( D(\obj(\cat{I})) \) in the image \( D(\cat{I}) \) and \( R \) is a subrelation of \( \leq \). This relation is reflexive, however it may not even be a preorder as shown in \cref{ex:functor_image_not_a_category}.
...
\begin{definition}\label{def:equalizers}\mcite[def. 5.1.11, def. 5.2.7]{Leinster2014BasicCategoryTheory}
  ...
  \begin{thmenum}
    ...
    \Cref{thm:equalizer_invertibility} implies that the map \( e: X \to A \) is a \hyperref[def:morphism_invertibility/left_cancellative]{monomorphism}. This enables us to view a cone as a \hyperref[def:categorical_subobject]{subobject} with an associated inclusion map.
    ...
    \Cref{thm:equalizer_invertibility} implies that the map \( e: B \to Y \) is an \hyperref[def:morphism_invertibility/right_cancellative]{epimorphism}. This enables us to view a cocone as a \hyperref[def:categorical_quotient_object]{quotient object} with an associated projection map.
...
\begin{proposition}\label{thm:equalizer_invertibility}
  If \( (L, \iota) \) is an \hyperref[def:equalizers]{equalizer cone} in the category \( \cat{C} \), then \( \iota \) is a \hyperref[def:morphism_invertibility/left_cancellative]{monomorphism}.

  Dually, if \( (L, \pi) \) is a \hyperref[def:equalizers]{coequalizer cone}, then \( \pi \) is an \hyperref[def:morphism_invertibility/right_cancellative]{epimorphism}.
\end{proposition}
\begin{proof}
...
\begin{definition}\label{def:compete_category}\mcite[109]{MacLane1998CategoryTheory}
