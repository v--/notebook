...
\begin{definition}\label{def:boolean_algebra_difference}\mcite[\S 2.1.5]{КусраевКутателадзе2005БулевозначныйАнализ}
  ...
  In case of ambiguity, following \textcite[20]{Halmos1976MeasureTheory}, we will refer to them as \term{proper differences}.
\end{definition}
\begin{comments}
  \item This notion is a generalization of set differences as defined in \cref{def:basic_set_operations/difference}.
\end{comments}
...
\end{definition}
\begin{comments}
  \item This is incidentally a generalization of both the set-theoretic symmetric difference defined in \cref{def:boolean_algebra_difference} and the \enquote{exclusive or} Boolean function defined in \cref{def:standard_boolean_functions}.
\end{comments}
...
\begin{proposition}\label{thm:symmetric_difference_abelian_group}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:symmetric_difference_abelian_group/neutral} The bottom acts as a \hyperref[def:semigroup_neutral_element]{neutral element}.
...
\begin{proposition}\label{thm:def:boolean_ring}
  ...
  \begin{thmenum}
    \thmitem{thm:def:boolean_ring/involution} Addition is \hyperref[def:categorical_involution]{involutive}, that is, \( x + x = 0 \) for every element \( x \).
...
\begin{proposition}\label{thm:boolean_ring_characteristic}
  Every \hyperref[def:boolean_ring]{Boolean ring} has \hyperref[def:ring_characteristic]{characteristic} \( 2 \).
\end{proposition}
\begin{comments}
  \item By definition of characteristic, the \hyperref[def:finite_field]{finite field} \( \BbbF_2 \) embeds into any Boolean ring, thus making every Boolean ring a \( \BbbF_2 \)-\hyperref[def:algebra_over_ring]{algebra}.
\end{comments}
\begin{proof}
  Fix a Boolean ring \( R \) with top \( \top \) and bottom \( \bot \). We must show that mapping sending \( \bot \) to \( 0 \) and \( \top \) to \( 1 \) is compatible with addition and multiplication.
...
\end{proposition}
\begin{proof}
  Fix a Boolean algebra \( X \). We have already shown in \cref{thm:symmetric_difference_abelian_group} that \( (X, \symdiff) \) is an abelian group. Furthermore, meets are associative, commutative and idempotent by definition. \Cref{thm:def:bounded_lattice/neutral} then implies that \( (X, \wedge) \) is an idempotent monoid with neutral element \( \top \).
...
\begin{proof}
  ...
  \SubProof{Proof that \( (R, \vee, \wedge) \) is a lattice} We must show that \( {\vee} \) and \( {\wedge} \) are both associative and commutative and satisfy the absorption properties from \cref{def:lattice/theory/absorption}.
  ...
  \begin{equation*}
    (x \vee y) \vee z
    =
    (\underline{xy} + \underline{x} + y)z + (\underline{xy} + x + y) + z
    =
    x(\underline{yz} + \underline{y} + \underline{z}) + (yz + y + z) + x
    =
    x \vee (y \vee z).
  \end{equation*}

  To conclude that \( (R, \vee, \wedge) \) is a \hyperref[def:lattice]{lattice}, we must show that the absorption laws \eqref{eq:def:lattice/theory/absorption/join} and \eqref{eq:def:lattice/theory/absorption/meet} hold.
  ...
  \begin{align*}
    ...
    (xz + x + z)(yz + y + z)
    = \\ &=
    xyz^2 + xzy + xz^2 + \underline{xyz} + \underline{xy} + xz + z^2y + zy + \underline{z^2}
    = \\ &=
    xyz + xy + z
    = \\ &=
    (x \wedge y) \vee z.
...
\begin{proposition}\label{thm:categories_of_boolean_algebras_and_ring}
  The \hyperref[def:category]{categories} of Boolean algebras and Boolean rings are \hyperref[def:isomorphism_of_categories]{isomorphic}.
\end{proposition}
\begin{proof}
  Denote the category of small Boolean rings by \( \cat{BRing} \). \Cref{thm:boolean_algebra_as_ring} provides a functor \( F: \cat{Bool} \to \cat{BRing} \), while \cref{thm:boolean_ring_as_algebra} provides a functor \( G: \cat{BRing} \to \cat{Bool} \).
