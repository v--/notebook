...
\end{comments}
\begin{proof}
  Fix a Boolean ring \( R \). Let us denote the elements of \( \BbbF_2 \) by \( \top \) and \( \bot \). We must show that mapping sending \( \bot \) to \( 0 \) and \( \top \) to \( 1 \) is compatible with addition and multiplication.
...
\begin{proof}
  ...
  \SubProof{Proof that \( (R, \vee, \wedge) \) is a lattice} WWe must show that \( {\vee} \) and \( {\wedge} \) are both associative and commutative and satisfy the absorption properties from \cref{def:lattice/theory/absorption}.
