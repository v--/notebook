...
\begin{example}\label{ex:countable_indeterminates_non_noetherian}
  \Fullref{thm:hilberts_basis_theorem} implies that if \( R \) is a noetherian commutative ring, so is \( R[X_1, \ldots, X_n] \). If we instead consider the polynomial algebra \( R[X_1, X_2, X_3, \ldots] \) in countably many indeterminates, it will not be noetherian because we have the following non-stabilizing chain of ideals:
  \begin{equation*}
...
\begin{definition}\label{def:quaternions}\mimprovised
  ...
  \begin{figure}[!ht]
    ...
    \caption{A triangle illustrating the congruence in \fullref{def:quaternions} --- traversing an edge clockwise, we obtain the opposite vertex. Traversing an edge counter-clockwise, we obtain the negation of the vertex.}\label{fig:def:quaternions}
  \end{figure}
...
\begin{proposition}\label{thm:def:quaternions}
  \hyperref[def:quaternions]{Quaternions} have the following basic properties:
  \begin{thmenum}
    \thmitem{thm:def:quaternions/commutative} Multiplication of quaternions is not commutative.
    \thmitem{thm:def:quaternions/inverse} \( \BbbH \) is a \hyperref[def:division_ring]{division ring} --- every nonzero quaternion \( a + bi + cj + dk \) has a (two-sided) multiplicative inverse:
    \begin{equation*}
...
\end{proposition}
\begin{proof}
  \SubProofOf{thm:def:quaternions/commutative} We have \( ij = k = -ji \).
  \SubProofOf{thm:def:quaternions/inverse} We have
  \begin{align*}
    &\phantom{{}={}}
