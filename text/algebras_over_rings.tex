...
\begin{definition}\label{def:algebra_over_ring}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:algebra_over_ring/trivial} Any single-element \( R \)-algebra, that is, every zero object in \( \ucat{Alg}_R \), is as trivial object in the sense of \fullref{def:trivial_object}. We often call it \enquote{the} trivial \( R \)-algebra.
    ...
    \thmitem{def:algebra_over_ring/kernel} The \hyperref[def:zero_morphisms/kernel]{kernel} of an \( R \)-algebra homomorphism \( \varphi: M \to N \) is its \hyperref[def:zero_locus]{zero locus} \( \varphi^{-1}(0_N) \).
...
\begin{theorem}[Quotient algebra universal property]\label{thm:quotient_algebra_universal_property}
  ...
  Compare this result to \fullref{thm:quotient_structure_universal_property} and \fullref{thm:quotient_structure_universal_property}.
\end{theorem}
\begin{proof}
  Simple refinement of \fullref{thm:quotient_structure_universal_property}.
\end{proof}
...
\begin{theorem}[Quotient ideal lattice theorem]\label{thm:lattice_theorem_for_rings}
  ...
  Compare this result to \fullref{thm:lattice_theorem_for_subgroups} and \fullref{thm:lattice_theorem_for_submodules}.
\end{theorem}
\begin{proof}
  \SubProof{Proof for general ideals} Simple refinement of \fullref{thm:lattice_theorem_for_substructures}.
  ...
  \SubProof{Proof for radical ideals} Correspondence of radical ideals follows from correspondence of prime ideals since, by \fullref{thm:radical_ideal_is_intersection}, a radical ideal is the intersection of all prime ideals containing it.
\end{proof}
...
\end{corollary}
\begin{proof}
  Since \( M \) is maximal, only \( M \) and \( R \) are ideals of \( R \) containing \( M \). Therefore, by \fullref{thm:lattice_theorem_for_rings}, \( R / M \) has only two ideals. The converse also follows from the lattice theorem.
\end{proof}
...
\begin{proof}
  ...
  \SubProofOf{thm:def:noetherian_semimodule/quotient} By \fullref{thm:lattice_theorem_for_submodules}, every chain of submodules of \( M / N \) corresponds to a chain of submodules in \( M \). Thus, if \( M \) is noetherian, clearly \( M / N \) also is.
