...
\begin{definition}\label{def:algebra_over_ring}\mcite[15]{Kaplansky1974CommRings}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:algebra_over_ring/homomorphism}\mcite[28]{Eisenbud1995CommAlgebra} A \hyperref[def:first_order_homomorphism]{first-order homomorphism} between two \( R \)-algebras is a \hyperref[def:linear_function]{linear map} that preserves multiplication. This is the same as for semirings.

    \thmitem{def:algebra_over_ring/submodel}\mcite[28]{Eisenbud1995CommAlgebra}) The set \( A \subseteq M \) is a \hyperref[def:first_order_submodel]{submodel} of \( M \) if it is a \hyperref[def:monoid/submodel]{submodule} of \( M \) that contains \( 1 \) and is closed under algebra multiplication. We say that \( A \) is an \( R \)-\term{subalgebra} of \( M \).
...
\end{proposition}
\begin{proof}
  Let \( f(X) \) be an arbitrary polynomial. \Fullref{alg:euclidean_division_of_polynomials} gives us polynomials \( q(X) \) and \( r(X) \) such that
  \begin{equation*}
    f(X) = p(X) q(X) + r(X),
  \end{equation*}
...
\begin{definition}\label{def:gaussian_integers}\mcite[\S V.6.2]{Aluffi2009Algebra}
  A \term[ru=целые гауссовые числа (\cite[example 3.5.1]{Винберг2014Алгебра})]{Gaussian integer} is a \hyperref[def:complex_number]{complex number} whose real and imaginary part are \hyperref[def:integers]{integers}.
\end{definition}
...
\begin{definition}\label{def:noetherian_semimodule}\mcite[prop. 6.16]{Golan1999Semirings}
  We say that an \( R \)-\hyperref[def:semimodule]{semimodule} is \term[bg=ньотеров (\cite[41]{КоцевСидеров2016КомАлгебра}), ru=нётеровый (\cite[def. 9.4.1]{Винберг2014Алгебра})]{noetherian} if any of the following equivalent conditions hold:
  \begin{thmenum}
...
\begin{proposition}\label{thm:def:noetherian_semimodule}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:def:noetherian_semimodule/quotient}\mcite[prop. 6.3b)]{КоцевСидеров2016КомАлгебра} Let \( N \) be an \( R \)-\hyperref[def:module/submodel]{submodule} of \( M \). Then \( M \) is noetherian if and only if both \( N \) and their \hyperref[def:module/quotient]{quotient} \( M / N \) are.
  \end{thmenum}
\end{proposition}
\begin{proof}
...
\begin{theorem}[Hilbert's basis theorem]\label{thm:hilberts_basis_theorem}\mcite[thm. 7.4]{КоцевСидеров2016КомАлгебра}
...
\begin{definition}\label{def:quaternion_algebra}\mimprovised
  We define the \term[bg=кватерниони (\cite[454]{Обрешков1962ВисшаАлгебра}), ru=алгебра кватернионов (\cite[41]{Винберг2014Алгебра}), en=quaternion algebra (\cite[exer. III.1.12]{Aluffi2009Algebra})]{quaternion algebra} \( \BbbH \) as the \hyperref[def:algebra_over_ring/quotient]{quotient} of the \hyperref[def:noncommutative_polynomial_algebra]{noncommutative polynomial algebra} \( \BbbR\braket{ i, j, k } \) by the congruence \hyperref[def:first_order_generated_congruence]{generated} by the following relation:
  \begin{align*}
