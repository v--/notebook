...
\begin{definition}\label{def:algebra_over_ring}\mcite[15]{Kaplansky1974CommutativeRings}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:algebra_over_ring/homomorphism}\mcite[28]{Eisenbud1995CommutativeAlgebra} A \hyperref[def:fol_homomorphism]{first-order homomorphism} between two \( R \)-algebras is a \hyperref[def:semimodule/morphism]{linear map} that preserves multiplication. This is the same as for semirings.
    ...
    \thmitem{def:algebra_over_ring/category}\mimprovised For a fixed ring \( R \), we denote the \hyperref[def:fol_theory_model_functor/obj]{category of \( \mscrU \)-small models} by \( \ucat{Alg}_R \). It is concrete with respect to both \( \ucat{CRing} \) and \( \ucat{Mod}_R \).
