...
\begin{definition}\label{def:algebra_over_ring}\mcite[15]{Kaplansky1974CommutativeRings}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:algebra_over_ring/homomorphism}\mcite[28]{Eisenbud1995CommutativeAlgebra} A \hyperref[def:fol_homomorphism]{first-order homomorphism} between two \( R \)-algebras is a \hyperref[def:linear_function]{linear map} that preserves multiplication. This is the same as for semirings.

    \thmitem{def:algebra_over_ring/submodel}\mcite[28]{Eisenbud1995CommutativeAlgebra}) The set \( A \subseteq M \) is a \hyperref[def:fol_substructure/submodel]{submodel} of \( M \) if it is a \hyperref[def:monoid/subobject]{submodule} of \( M \) that contains \( 1 \) and is closed under algebra multiplication. We say that \( A \) is an \( R \)-\term{subalgebra} of \( M \).

    If \( A \) does not contain \( 1 \), we may instead refer to nonunital \( R \)-subalgebras. We will use them for quotients.

    As a consequence of \cref{thm:fol_equational_theory_models/image}, the image of an \( R \)-algebra homomorphism is an \( R \)-subalgebra of its codomain.

    \thmitem{def:algebra_over_ring/category}\mimprovised For a fixed ring \( R \), we denote the \hyperref[def:fol_theory_model_functor/obj]{category of \( \mscrU \)-small models} by \( \ucat{Alg}_R \). It is concrete with respect to both \( \ucat{CRing} \) and \( \ucat{R\mathdash Mod} \).
    ...
    \thmitem{def:algebra_over_ring/kernel}\mimprovised \Cref{thm:equational_theory_zero_morphisms/kernel} implies that the \hyperref[def:categorical_kernels/kernel]{categorical kernel} of a homomorphism \( \varphi: M \to N \) between \emph{nonunital} \( R \)-algebras is the additive group kernel
    \begin{equation*}
...
\begin{remark}\label{rem:algebra_terminology}
\end{remark}
