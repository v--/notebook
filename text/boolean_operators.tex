...
\begin{definition}\label{def:boolean_closure}\mcite[30]{Яблонский1986}
  ...
  \begin{thmenum}
    \thmitem{def:boolean_closure/closed}\mcite[33]{Яблонский1986} We say that a subset \( B \) of \( \mscrB \) is \term{closed} if whenever \( g(x_1, \ldots, x_m) \) is in \( B \) and \( f_k(x_1, \ldots, x_n) \) is in \( B \) for \( k = 1, \ldots, m \), then their \hyperref[rem:function_superposition]{superposition}
    \begin{equation*}
