...
\begin{definition}\label{def:boolean_closure}
  ...
  \begin{thmenum}
    \thmitem{def:boolean_closure/closed}\mcite[33]{Яблонский2003} We say that a subset \( B \) of \( \mscrB \) is \term{closed} if, whenever \( g(x_1, \ldots, x_m) \) is in \( B \) and \( f_k(x_1, \ldots, x_n) \) is in \( B \) for \( k = 1, \ldots, m \), then their \hyperref[rem:function_superposition]{superposition}
    \begin{equation*}
...
\end{definition}
\begin{comments}
  \item \Fullref{thm:functions_over_prime_fields} implies that to every Boolean operator there corresponds exactly one \hyperref[def:square_free_element]{square-free} Zhegalkin polynomial.
...
\begin{proposition}\label{thm:complete_sets_of_boolean_operators}
  ...
  \begin{thmenum}
    \thmitem{thm:complete_sets_of_boolean_operators/zhegalkin} \( \set{ \oplus, \wedge, F, T } \).
    \thmitem{thm:complete_sets_of_boolean_operators/or_not} \( \set{ \vee, \oline{\anon} } \).
    \thmitem{thm:complete_sets_of_boolean_operators/and_not} \( \set{ \wedge, \oline{\anon} } \).
    \thmitem{thm:complete_sets_of_boolean_operators/nand} \( \set{ \uparrow } \), where \( (x \uparrow y) \coloneqq \oline{x \wedge y} \)\fnote{This operation is called \term{Sheffer's stroke} or \term{nand} (\enquote{not and}).}
...
\begin{proof}
  ...
  \SubProofOf{thm:complete_sets_of_boolean_operators/or_not} By inspecting \fullref{def:standard_boolean_operators}, we can conclude that we can express the operators from \fullref{thm:complete_sets_of_boolean_operators/zhegalkin} via \( \vee \) and \( \oline{\anon} \):
  \begin{itemize}
  ...
  \Fullref{thm:complete_sets_of_boolean_operators/and_not} implies that \( \set{ \wedge, \oline{\anon} } \) is complete. Then so is \( \set{ \uparrow } \).
