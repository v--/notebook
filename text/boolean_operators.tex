...
\begin{definition}\label{def:standard_boolean_operators}\mcite[14]{Яблонский2003}
  ...
  \begin{center}
    \begin{tabular}{c | c || c c | c c c c c c}
      \( x \) & \( \overline{x} \) & \( x \) & \( y \) & \( x \vee y \)             & \( x \oplus y \)    & \( x \wedge y \)        & \( x \implies y \)   & \( x \syniff y \) \\
      \hline
...
\begin{comments}
  ...
  \item The operations \( \implies \) and \( \syniff \) are also defined in any \hyperref[def:boolean_algebra]{Boolean algebra}.
...
\begin{proposition}\label{thm:complete_sets_of_boolean_operators}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:complete_sets_of_boolean_operators/nand} \( \set{ \uparrow } \), where \( (x \uparrow y) \coloneqq \overline{x \wedge y} \)\fnote{This operation is called \term{Sheffer's stroke} or \term{nand} (\enquote{not and}).}
    \thmitem{thm:complete_sets_of_boolean_operators/conditional_falsum} \( \set{ \implies, F } \).
  \end{thmenum}
\end{proposition}
\begin{proof}
  ...
  \begin{equation*}
    x \uparrow y = x \implies (y \implies F).
  \end{equation*}
\end{proof}
