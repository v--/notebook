...
\begin{definition}\label{def:endianness}\mimprovised
  ...
  \begin{thmenum}
    \thmitem{def:endianness/big} The \term{big-endian} order is based on the implicit \hyperref[def:set_enumeration]{enumeration} \( d_0, \ldots, d_{m-1} \).
...
\begin{definition}\label{def:ring_of_unsigned_integers}\mimprovised
  ...
  \Fullref{alg:addition_with_carrying} and \fullref{alg:addition_with_carrying}, limited to \( w \) digits and adapted with respect to endianness, define operations that turn \( U_w \) into a \hyperref[def:ring]{ring} isomorphic to \hyperref[def:ring_of_integers_modulo]{\( \BbbZ_{2^w} \)}.
...
\begin{example}\label{ex:little_endian_arithmetic}
  ...
  \begin{MemoryLine}
    \MP \MP \MH{0} 0 0 0 0 2 3 4 \MP \MP
  \end{MemoryLine}

  We have marked the beginning of the segment. Clearly \( 234 \) can be encoded with a smaller \( 4 \)-digit segment.

  If we depend on reading the memory line sequentially, we need to scan the entire segment to determine the least significant digit. For algorithms that depend on reading the least significant bit first, like \fullref{alg:addition_with_carrying} or even the evenness check \cref{thm:nonnegative_integer_binary_expansion_parity}, this requires scanning the whole segment and then possibly going backwards.
...
\begin{definition}\label{def:digit_shift}
  In the ring \hyperref[def:ring_of_unsigned_integers]{\( U_w \)} of \hyperref[def:endianness/big]{\emph{big-endian}} unsigned \( w \)-digit integers, we can define the \term{left shift} by \( m \) as
  \begin{align}
...
\end{proposition}
\begin{proof}
  We will only consider big-endian encoding in the proof; for little-endianness, the proof then follows by \fullref{thm:order_duality}.
