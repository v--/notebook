...
\begin{definition}\label{def:ordinal}\mcite[subsec. 6.3.1(i)]{Hinman2005}
  ...
  Finally, we introduce the following \hyperref[con:predicate_formula]{predicate formula}
...
\end{theorem}
\begin{proof}
  This theorem is a special case of \fullref{thm:epsilon_induction} with the formula \( \xi \in \tau \synimplies \varphi \) that is explicitly universally quantified by the parameter \( \tau \) which ranges over all ordinals.
...
\begin{concept}\label{con:induction}
  ...
  \begin{thmenum}
    ...
    \thmitem{con:induction/grammars} Within this monograph, we use \fullref{thm:induction_on_syntax_trees}, for example \hyperref[def:propositional_syntax/formula]{propositional} and \hyperref[def:first_order_syntax/formula]{first-order formulas}. It is not an established principle like the others.
    ...
    \thmitem{con:induction/lattices} \Fullref{thm:knaster_tarski_theorem} and \fullref{thm:knaster_tarski_iteration} hold in \hyperref[def:complete_lattice]{complete lattices}, and enable the very general \fullref{thm:least_fixed_point_recursion} and \fullref{thm:least_fixed_point_induction}.
  \end{thmenum}
\end{concept}
