...
\begin{remark}\label{rem:ordinal_definition}
  ...
  For an arbitrary set \( A \), set membership is not even a \hyperref[thm:strict_partial_order]{strict partial order} --- irreflexivity is implied by \fullref{thm:set_membership_is_well_founded}, but transitivity of \( \in \) as a binary relation on \( A \) fails to hold in general, not speaking about trichotomy.
...
\begin{proposition}\label{thm:initial_segment_of_ordinal}
  Let \( \alpha \) be an ordinal. For any \( \beta \in \alpha \), the \hyperref[def:order_interval/ray]{open initial segment} \( \alpha_{<\beta} \) equals \( \beta \).
...
\begin{theorem}[Bounded transfinite recursion]\label{thm:bounded_transfinite_recursion}\mcite[177]{Enderton1977Sets}
  Fix an \hyperref[def:ordinal]{ordinal} \( \alpha \) and a nonempty set \( A \). Suppose that we are given some transformation \( T: \pow(\alpha \times A) \to A \), which sends every \hyperref[def:multi_valued_function]{multi-valued function} with signature \( \alpha \multto A \) to an element of \( A \). Then there exists a unique \( \alpha \)-indexed \hyperref[def:transfinite_sequence]{transfinite sequence} \( f: \alpha \to A \) such that for any \( \beta \in \alpha \) we have \( f(\beta) = T(f\restr_\beta) \).
\end{theorem}
\begin{comments}
...
\begin{definition}\label{def:transitive_closure_of_a_set}\mcite[lemma 6.1]{Jech2003}
...
\begin{theorem}[Structural recursion]\label{thm:structural_recursion}\mimprovised
  Let \( X \) be a set and let \( T: \pow(X) \to \pow(X) \) be a transformation. Then \( T \) has a \hyperref[def:extremal_points/maximum_and_minimum]{smallest} \hyperref[def:fixed_point]{fixed point}. That is, there exists a unique subset \( A_0 \subseteq X \) such that \( T(A_0) = A_0 \) and no proper subset of \( A_0 \) has this property.
\end{theorem}
\begin{comments}
  \item This is the most general recursion principle we consider.
  \item See \fullref{thm:well_founded_induction} for its corresponding induction principle.
  \item Structural recursion is sometimes also called \term{mutual recursion}.
  \item See \fullref{rem:structural_recursion_and_induction} for how this theorem is used.
\end{comments}
\begin{proof}
...
\begin{remark}\label{rem:structural_recursion_and_induction}\mimprovised
  ...
  Define a binary relation \( \prec \) on \( A_0 \) by declaring that, for every function \( f \) in \( \mscrF \), every sequence \( x_1, \ldots, x_{\#f} \) and every index \( k = 1, \ldots, \#f \), we have
  \begin{equation*}
    x_k \prec f(x_1, \ldots, x_{\#f}).
  \end{equation*}

  This relation can be defined by taking unions of smaller relations rather than via recursion.

  If \( \prec \) happens to be \hyperref[def:well_founded_relation]{well-founded}, we can use \fullref{thm:well_founded_induction} to prove universal statements about \( A_0 \).
\end{remark}
...
\begin{theorem}\label{thm:well_ordered_order_type_existence}
  Any \hyperref[def:well_ordered_set]{well-ordered set} \( (P, \leq) \) is \hyperref[def:order_homomorphism/isomorphism]{order-isomorphic} to a unique ordinal. This ordinal is called the \term{order type} of \( (P, \leq) \) and is denoted by \( \ord(P, \leq) \) or simply \( \ord(P) \).
