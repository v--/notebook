...
\begin{definition}\label{def:institution}\mcite[def. 1]{GoguenBurstall1992Institutions}
  ...
  \begin{thmenum}
    ...
    \begin{thmenum}
      ...
      \thmitem{def:institution/models/hom} \mcite[94]{MossakowskiKrumnackMaibaum2015DerivedSignatureMorphisms} For a given signature morphism \( s: \Sigma \to \Theta \), we call \( \cat{Mod}(s): \cat{Mod}(\Theta) \to \cat{Mod}(\Sigma) \) the \term{model reduct} functor.
    \end{thmenum}

    \thmitem{def:institution/satisfaction} A family of \hyperref[def:relation]{relations} indexed by \( \obj(\cat{Sign}) \), where, for a given signature \( \Sigma \), \( \vDash_\Sigma \) is a relation between \( \obj(\op*{Mod}(\Sigma)) \) and \( \cat{Sen}(\Sigma) \) such that, for each signature morphism \( s: \Sigma \to \Theta \), each sentence \( \varphi \) in \( \op*{Sen}(\Sigma) \) and each model \( \mscrY \) in \( \op*{Mod}\mscrI_\Theta \), the following condition holds:
    \begin{equation}\label{eq:def:institution/satisfaction}
      \mscrY \vDash_\Theta \underbrace{\op*{Sen}(s)}_{\mathclap{\op*{Sen}(\Sigma) \to \op*{Sen}(\Theta)}}(\varphi)
      \quad\T{if and only if}\quad
      \underbrace{\op*{Mod}(s)}_{\mathclap{\op*{Mod}(\Theta) \to \op*{Mod}(\Sigma)}}(\mscrY) \vDash_\Sigma \varphi
    \end{equation}
