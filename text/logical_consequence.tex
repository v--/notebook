...
\end{remark}
\begin{comments}
  \item We only show non-compactness in \fullref{thm:hol_entailment_not_compact}.
\end{comments}
...
\begin{concept}\label{con:minimal_logic}\mimprovised
  ...
  We will only sketch semantics for \hyperref[def:propositional_formula]{propositional formulas} in \cref{rem:minimal_logic_semantics}, but will present a full-fledged natural deduction system in \cref{def:propositional_natural_deduction}, and extend it to \hyperref[def:fol_formula]{first-order formulas} in \cref{def:fol_natural_deduction} and \hyperref[def:hol_expression/formula]{higher-order formulas} in \cref{def:hol_natural_deduction}.
\end{concept}
