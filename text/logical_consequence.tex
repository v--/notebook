...
\begin{definition}\label{def:variable_identifier}\mimprovised
  ...
  Furthermore, in definitions like \cref{def:atomic_lambda_term_substitution/sharp}, we would like to find the \enquote{identifier} digit not present in a given set of variables. This can be done via the \hyperref[thm:def:well_ordered_set/lexicographic]{reverse lexicographic order} on letter-number pairs:
  \begin{equation*}
...
\begin{definition}\label{def:sequent}\mimprovised
  ...
  Unless specified otherwise, we assume that the sequent relation is \enquote{\( \synvdash \)} (with a dot on top inside the object language, in accordance with \cref{rem:mathematical_logic_conventions/terminal_dots}).
\end{definition}
\begin{comments}
...
\begin{remark}\label{rem:logical_compactness_theorems}
  ...
  \begin{thmenum}
    ...
    \thmitem{rem:logical_compactness_theorems/propositional_syntactic} \Cref{thm:propositional_natural_deduction_derivation_compact}.
    \thmitem{rem:logical_compactness_theorems/propositional_semantic} \Cref{thm:classical_propositional_compactness}.
    \thmitem{rem:logical_compactness_theorems/first_order_syntactic} \Cref{thm:first_order_natural_deduction_entailment_compact}.
  \end{thmenum}
\end{remark}
...
\begin{definition}\label{def:interderivability}\mimprovised
  We refer to \hyperref[def:logical_equivalence]{logical equivalence} in \hyperref[def:entailment_system]{entailment systems} as \term{interderivability}, especially when the entailment system is the syntactic part of a \hyperref[def:general_logic]{general logic}.
\end{definition}
...
\begin{definition}\label{def:theory_of_institutional_model}\mimprovised
  In an \hyperref[def:institution]{institution}, we denote by \( \op*{Th}(\mscrX) \) the set of all sentences \hyperref[def:institutional_satisfaction/satisfiable]{valid} in \( \mscrX \). We call it the \term{theory} \hi{of} \( \mscrX \).
\end{definition}
\begin{defproof}
...
\begin{definition}\label{def:tautology}\mimprovised
  In an \hyperref[def:institution]{institution}, we say that the sentence is a \term[en=tautology (\cite[12]{Kleene2002Logic})]{tautology} if it is \hyperref[def:institutional_satisfaction/satisfiable]{satisfied} in every model (over the same signature). Dually, we call a sentence a \term{contradiction} if no model satisfies it.
\end{definition}
\begin{comments}
