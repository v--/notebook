...
\begin{definition}\label{def:logical_theory}\mcite[96]{CitkinMuravitsky2022ConsequenceRelations}
  ...
  We say that \( \Gamma \) \term{generates} or \term{axiomatizes} the theory \( \op*{Cn}(\Gamma) \). This justifies the notation \( \cat{Th}(\Gamma) \) instead of \( \op*{Cn}(\Gamma) \) for the consequence closure of \( \Gamma \).
\end{definition}
\begin{comments}
...
\begin{definition}\label{def:entailment_system_theory}\mimprovised
  ...
  \begin{thmenum}
    ...
    \thmitem{def:entailment_system_theory/conservative}\mcite[def. 23]{GoguenBurstall1992Institutions} We say that the theory morphism \( t: (\Sigma, \Gamma) \to (\Theta, \Delta) \) is \term{conservative} when \( \Gamma \vdash_\Sigma \varphi \) if and only if \( \Delta \vdash_\Theta \op*{Sen}(t)(\varphi) \).
  \end{thmenum}
\end{definition}
\begin{comments}
...
\begin{proof}
  ...
  Suppose that some model \( \mscrT \) in \( \cat{Mod}(\Theta) \) satisfies \( t[\Gamma] \). Due to \eqref{eq:def:institution/satisfaction}, \( \cat{Mod}(t)(\mscrT) \) is a model in \( \cat{Mod}(\Sigma) \) that satisfies \( \Gamma \), and hence also \( \varphi \). Again via \eqref{eq:def:institution/satisfaction} we conclude that \( \mscrT \) satisfies \( t(\varphi) \).
