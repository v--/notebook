...
\begin{concept}\label{con:logical_system}
  ...
  This may also encompass different \hyperref[def:rewriting_system]{rewriting systems} --- for example, \textcite{Barendregt1974SurjectivePairing} assumes that what he calls a \enquote{logical system} has a syntax allowing \hyperref[def:lambda_term]{\( \muplambda \)-application}.
...
\begin{remark}\label{rem:named_logical_systems}
  ...
  \begin{itemize}
    \thmitem{rem:named_logical_systems/gentzen} \Textcite[176]{Gentzen1935LogischeSchließen} introduces \hyperref[def:fol_natural_deduction]{first-order natural deduction systems} (which he called \enquote{\textde{Kalkül des natürlichen Schliessens}}), and called the corresponding \hyperref[con:intuitionistic_logic]{intuitionistic} and \hyperref[con:classical_logic]{classical} variants \logic{NJ} and \logic{NK}. We formulate them in \cref{def:propositional_natural_deduction}.

    In the same paper he also introduces \hyperref[def:abstract_sequent_calculus_system]{sequent calculus}, whose variants he called \logic{LJ} (\enquote{\textde{Intuitionistische Prädikatenlogik}}) and \logic{LK} (\enquote{\textde{Klassik Prädikatenlogik}}). We formulate them in \cref{def:classical_propositional_sequent_calculus}.

    \thmitem{rem:named_logical_systems/q0} \Textcite[\S 51]{Andrews2002Logic} introduces the system \( \logic{Q}_0 \), which we use as an inspiration for our \hyperref[def:higher_order_logic]{higher-order logic} --- see \cref{rem:higher_order_logic_and_type_theory/terms}.

    \thmitem{rem:named_logical_systems/peano_robinson} \Textcite[def. 4.1.1]{Hinman2005Logic} calls \hyperref[def:peano_arithmetic]{Peano arithmetic} \( \logic{P} \) (which we prefer to abbreviate as \logic{PA}), and its weaker form, the \hyperref[def:peano_arithmetic]{Robinson arithmetic}, \( \logic{Q} \).

    \thmitem{rem:named_logical_systems/barendregt} \Textcite{Barendregt1992LambdaCalculiWithTypes} introduces several \hyperref[def:abstract_type_system]{type systems} for \( \muplambda \)-calculus, which revolve around the letter \( \muplambda \). For example, he denotes his of \hyperref[con:simple_type_theory]{simple type theory} by \( \muplambda_{\rightarrow} \) and \hyperref[def:lambda_cube/type_on_term]{his flavor} of \hyperref[def:polymorphic_typed_lambda_calculus]{second-order polymorphic \( \muplambda \)-calculus} by \( \synlambda 2 \). Some others are listed in \cref{def:lambda_cube}.
...
\begin{definition}\label{def:consequence_operator}\mimprovised
  Fix a set \( S \), whose elements we will call \term[ru=предложения (\cite[103]{КолмогоровДрагалин2006Логика}), en=sentences (\cite[62]{Tarski1983MethodologyOfDeductiveSciences})]{sentences}, as per \cref{con:proposition}. If \( \op*{Cn} \) is a \hyperref[def:abstract_closure_operator]{closure operator} on the power set of \( S \), we say that it is a \term{Tarski consequence operator} on \( S \).
...
\begin{comments}
  ...
  \item The notation \( \op*{Cn} \) is due to Tarski. We also use \( \cat{Th} \) when dealing with \hyperref[def:logical_theory]{logical theories}.

  \item Sentences themselves are not, in practice, atoms. In propositional logic, they are formulas, which themselves have an elaborate structure discussed in \cref{def:propositional_formula}. In first-order logic their structure is even more elaborate --- see \cref{def:fol_closed_formula}.
...
\end{definition}
\begin{comments}
  \item This definition is based on \cite[290]{Gentzen1964LogicalDeduction}, but formalized via grammars.
  ...
  \begin{itemize}
    ...
    \item General contexts on the right are used in \fullref{sec:propositional_sequent_calculus}.
  \end{itemize}
\end{comments}
...
\begin{definition}\label{def:consequence_relation}\mcite[def. 4.1.1]{CitkinMuravitsky2022ConsequenceRelations}
  ...
  \begin{thmenum}
    ...
    This corresponds to Gentzen's \hyperref[inf:def:abstract_sequent_calculus_system/ax]{initialization rule} in sequent calculus.
    ...
    This corresponds to \hyperref[def:abstract_sequent_calculus_system/weak]{weakening rules} in sequent calculus.
    ...
    This corresponds to the \hyperref[def:abstract_sequent_calculus_system/cut]{cut rule} in sequent calculus.
...
\begin{remark}\label{rem:logical_compactness_theorems}
  ...
  \begin{thmenum}
    \thmitem{rem:logical_compactness_theorems/propositional} \Fullref{thm:classical_propositional_semantic_compactness}, with syntactic counterparts \cref{thm:propositional_natural_deduction_derivation_compact} for \hyperref[def:propositional_natural_deduction]{propositional natural deduction}, \cref{thm:propositional_axiomatic_derivation_entailment_compact} for \hyperref[def:propositional_axiomatic_derivation_system]{propositional axiomatic derivations} and \cref{thm:sequent_calculus_consequence_compact} for \hyperref[def:classical_propositional_sequent_calculus]{propositional sequent calculus}.

    \thmitem{rem:logical_compactness_theorems/fol_semantic} \Fullref{thm:fol_semantic_compactness}, with syntactic counterpart \cref{thm:fol_natural_deduction_derivation_compact} for \hyperref[def:fol_natural_deduction_proof_tree]{first-order natural deduction}.

    \thmitem{rem:logical_compactness_theorems/hol_semantic} \Fullref{thm:hol_weak_semantic_compactness}, with syntactic counterpart \cref{thm:hol_natural_deduction_derivation_compact} for \hyperref[def:hol_natural_deduction_proof_tree]{higher-order natural deduction}.
  \end{thmenum}
\end{remark}
\begin{comments}
  \item We only show non-compactness in \fullref{thm:hol_semantic_not_compact}.
\end{comments}

\begin{definition}\label{def:logical_theory}\mcite[96]{CitkinMuravitsky2022ConsequenceRelations}
  Fix a compatible \hyperref[def:consequence_operator]{consequence operator} \( \op*{Cn} \) and \hyperref[def:consequence_relation]{consequence relation} \( {\vdash} \) on the same set of sentences \( S \). We say that a set of sentences \( T \) is a \term[ru=теория (\cite[def. 3.1.1]{Герасимов2014Вычислимость})]{theory} with respect to \( \op*{Cn} \) if any of the following equivalent conditions hold:
  \begin{thmenum}[series=def:logical_theory]
    \thmitem{def:logical_theory/operator} \( T \) is closed, that is, the set \( \op*{Cn}(T) \) coincides with \( T \).

    \thmitem{def:logical_theory/relation} \( \varphi \in T \) if and only if \( T \vdash \varphi \)
  \end{thmenum}

  We say that \( T \) \term{generates} or \term{axiomatizes} the theory \( \op*{Cn}(T) \). This justifies the notation \( \cat{Th}(T) \) instead of \( \op*{Cn}(T) \) for the consequence closure of \( T \).
\end{definition}
\begin{comments}
  \item We will mostly rely on the more structured approach to theories in \hyperref[def:entailment_system]{entailment systems} --- \cref{def:entailment_system_theory} and \cref{def:category_of_theories} --- or in \hyperref[def:institution]{institutions} --- see \cref{def:theory_of_institutional_model}. In these cases theories depend on signatures.

  For \hyperref[def:propositional_logic]{propositional logic} we will refine several related notions in \cref{def:propositional_theory}, while for \hyperref[def:first_order_logic]{first-order logic} we will do so in \cref{def:fol_theory}.
...
\begin{remark}\label{rem:logical_theory_closed}
  We have defined a logical theory in \cref{def:logical_theory} as a set of sentences closed under consequence. This is due to \textcite[96]{CitkinMuravitsky2022ConsequenceRelations}, who, like us, consider arbitrary consequence operators.

  Similarly, theories are required to be closed under consequence by \textcite[def. 1.4.5]{Hinman2005Logic} (for propositional logic), \textcite[def. 3.1.4(i)]{VanDalen2004LogicAndStructure} and \textcite[21]{Andrews2002Logic}. \Textcite{Эдельман1975Логика} even distinguishes between syntactic and semantic theories of first-order logic.

  On the other hand, the requirement that a theory is closed is sometimes lifted, for example by \textcite[7]{TroelstraSchwichtenberg2000BasicProofTheory}, \textcite[def. 1.3.1]{TentZiegler2012ModelTheory}, \textcite[def. 3.1.1]{Герасимов2014Вычислимость} and \textcite[146]{ШеньВерещагин2017ЯзыкиИИсчисления}.
  ...
  \textcite[def. 2]{GoguenBurstall1992Institutions} take the opposite approach --- they distinguish between closed theories and possibly non-closed presentations of theories.

  We assume that theories are closed and try to be explicit about how a given theory is \hyperref[def:logical_theory]{axiomatized}.
\end{remark}

\begin{definition}\label{def:logical_independence}\mimprovised
  With respect to a given \hyperref[def:consequence_operator]{consequence operator}, we say that the sentence \( \varphi \) is \term[en=independent (formula) (\cite[exerc. 1.5.3]{VanDalen2004LogicAndStructure})]{independent} from the set \( \Gamma \) if neither \( \varphi \) nor \( \synneg \varphi \) follow from \( \Gamma \).
\end{definition}
\begin{comments}
  \item We generalize the first-order definition from \cite[exerc. 1.5.3]{VanDalen2004LogicAndStructure} to arbitrary consequence operators.
\end{comments}
...
\begin{definition}\label{def:logical_equivalence}\mimprovised
  ...
  \begin{thmenum}[series=def:logical_equivalence]
    \thmitem{def:logical_equivalence/operators}\mcite[def. 2]{Tarski1983FundamentalConceptsOfMetamathematics} The \hyperref[def:logical_theory]{logical theories} \hyperref[def:logical_theory]{axiomatized} by \( \Gamma \) and \( \Delta \) coincide, i.e. \( \op*{Cn}(\Gamma) = \op*{Cn}(\Delta) \).
...
\begin{concept}\label{con:logical_system_signature}
  A single \hyperref[con:logical_system]{logical system} is generally designed to encode multiple \hyperref[con:metalanguage]{object languages}\fnote{An exception is propositional logic, which we discuss in \fullref{sec:propositional_logic}. It provides only a way to study logical relations between \hyperref[con:proposition]{propositions}, irrespective of how these propositions are structured.}. A particular symbol may or may not be common for all languages of the system. For example, the addition symbol \( {\synplus} \) appears in \hyperref[def:peano_arithmetic]{Peano arithmetic} or the \hyperref[def:semiring/theory]{theory of semirings}, while some \hyperref[con:improper_symbol]{improper symbols} such as the conditional connective \( {\synimplies} \) is part of the \hyperref[def:propositional_alphabet]{propositional alphabet} and may thus appear in all formulas of \hyperref[def:propositional_logic]{propositional logic}, \hyperref[def:first_order_logic]{first-order logic} and \hyperref[def:higher_order_logic]{higher-order logic}.

  We call a \term[ru=сигнатура (логики первого порядка) (\cite[def. 2.1.1]{Герасимов2014Вычислимость}), en=signature (\cite[97]{GoguenBurstall1992Institutions})]{signature} the collection of symbols specific to an object language but not the entire system. Thus, in our example with Peano arithmetic, the signature consists of \( {\synplus} \), \( {\synmult} \), \( \syn0 \) and \( \syns \).

  Signatures may also specify some properties of their symbols. \hyperref[def:fol_signature]{First-order signatures}, for example, specify the kind and arity of the symbols --- in Peano arithmetic, \( {\synplus} \) and \( {\synmult} \) are binary operation, \( \syns \) is a unary operation, while \( \syn0 \) is a nullary operation.
...
\end{concept}
\begin{comments}
  \item \Textcite[97]{GoguenBurstall1992Institutions} describe a signature as \enquote{vocabularies for use in constructing sentences in a logical system}.

  \item Some authors use \enquote{language} for what we call \enquote{signature}, for example \textcite[def. 2.1.2]{Hinman2005Logic} and \textcite[270]{Farmer2008STTVirtues}.
...
\begin{definition}\label{def:entailment_system}\mcite[def. 1]{Meseguer1989GeneralLogics}
  ...
  \begin{thmenum}
    ...
    For a signature morphism \( t: \Sigma \to \Sigma' \), we refer to the both the action of \( t \) and of the induced function \( \op*{Sen}(t) \) as \term{translation}.

    \thmitem{def:entailment_system/entailment} For every signature \( \Sigma \), a \hyperref[def:consequence_relation]{consequence relation} \( \vdash_\Sigma \) on \( \op*{Sen}(\Sigma) \) such that, for every signature translation \( t: \Sigma \to \Sigma' \), the following compatibility condition holds:
    \begin{equation}\label{eq:def:entailment_system/entailment}
      \Gamma \vdash_\Sigma \varphi \T{implies} \op*{Sen}(t)[\Gamma] \vdash_{\Sigma'} \op*{Sen}(t)(\varphi).
    \end{equation}
...
\begin{definition}\label{def:entailment_system_theory}\mimprovised
  ...
  First and foremost, the definition only makes sense over a fixed signature \( \Sigma \). A theory is thus a set \( T \) of sentences over \( \Sigma \) that are closed under \( {\vdash_\Sigma} \), i.e. \( \varphi \) must belong to \( T \) whenever \( T \vdash_\Sigma \varphi \).

  The signature  \( \Sigma \) may be ambiguous considering \hyperref[def:fol_signature_extension]{first-order signature extensions} and the like; thus, it is sometimes convenient to make \( \Sigma \) explicit by regarding a theory as a pair \( (\Sigma, T) \).

  \begin{thmenum}
    \thmitem{def:entailment_system_theory/morphism}\mcite[def. 5]{GoguenBurstall1992Institutions} A signature morphism \( t: \Sigma \to \Sigma' \) allows us to translate sentences.

    We say that \( t \) is a \term{theory morphism} from \( (\Sigma, T) \) to \( (\Sigma', T') \) if the translation \( \op*{Sen}(t)[T] \) of the sentences from \( T \) is a subset of \( T' \).

    \thmitem{def:entailment_system_theory/conservative}\mcite[def. 23]{GoguenBurstall1992Institutions} We say that the theory morphism \( t: (\Sigma, T) \to (\Sigma', T') \) is \term{conservative} when \( T \vdash_\Sigma \varphi \) if and only if \( T' \vdash_{\Sigma'} \op*{Sen}(t)(\varphi) \).
  \end{thmenum}
\end{definition}
\begin{comments}
  \item We have defined what \cite[def. 2]{Meseguer1989GeneralLogics} calls \enquote{axiom-preserving morphisms}. He also suggests a weaker alternative --- merely requiring \( T' \vdash \op*{Sen}(t)(\varphi) \) for every sentence \( \varphi \) in \( T \).

  \item In accordance with \cref{rem:implicit_quantification_and_deduction}, in \hyperref[con:predicate_logic]{predicate logic}, we require theories to consist of closed formulas (i.e. formulas without \hyperref[con:variable_binding]{free variables}).
\end{comments}

\begin{definition}\label{def:category_of_theories}\mcite[def. 2]{Meseguer1989GeneralLogics}
  For a fixed \hyperref[def:entailment_system]{entailment system} \( \mscrE \), we define the \term{\hyperref[def:category]{category} of \hyperref[def:small_set]{small} theories} \( \cat{Th}(\mscrE) \) as follows:
  \begin{thmenum}
    \thmitem{def:category_of_theories/objects} The objects are pairs \( (\Sigma, T) \), where \( T \) is a small \hyperref[def:entailment_system_theory]{theory} over the (small) signature \( \Sigma \).

    \thmitem{def:category_of_theories/morphisms} A morphism \( t: (\Sigma, T) \to (\Sigma', T') \) is a compatible signature translation, defined as in \cref{def:entailment_system_theory/morphism}.
    ...
    \thmitem{def:category_of_theories/identity} The theory identity on \( (\Sigma, T) \) is the underlying identity signature morphism on \( \Sigma \).
  \end{thmenum}
\end{definition}
...
\begin{concept}\label{con:logical_model}
  A \term[ru=модель (\cite[71]{КолмогоровДрагалин2006Логика}), en=model (\cite[25]{Kleene1971Metamathematics})]{model} of a \hyperref[def:logical_theory]{logical theory} is a \hyperref[con:metalogic]{metatheoretic} object that satisfies them.

  The models themselves are not isolated, and the connections between them are described by \term[en=structure-preserving map (\cite[25]{DaveyPriestley2002LatticeTheory})]{structure preserving maps}, also called \term[en=morphism (\cite[25]{DaveyPriestley2002LatticeTheory})]{morphisms} and \term[en=homomorphism (\cite[108]{Schwartzman1996WordsOfMathematics})]{homomorphisms}. These terms are discussed in \cref{rem:morphism_etymology}.

  The precise connection between a theory, its models and their homomorphisms gives rise to a particular \hyperref[def:institution]{institution}.
\end{concept}
\begin{comments}
  \item \enquote{Structure-preserving maps} are sometimes collections of functions, like \hyperref[def:hol_homomorphism]{homomorphisms of higher-order logic}.
\end{comments}
...
\begin{definition}\label{def:institution}\mcite[def. 1]{GoguenBurstall1992Institutions}
  ...
  \begin{thmenum}
    ...
    \begin{thmenum}
      \thmitem{def:institution/models/obj} We call the objects of the category \( \cat{Mod}(\Sigma) \) \term{models} and the morphisms --- \term{homomorphisms}.

      \thmitem{def:institution/models/hom} \mcite[94]{MossakowskiKrumnackMaibaum2015DerivedSignatureMorphisms} For a given signature morphism \( t: \Sigma \to \Sigma' \), we call \( \cat{Mod}(t): \cat{Mod}(\Sigma') \to \cat{Mod}(\Sigma) \) the \term{model reduct} functor.
    \end{thmenum}

    \thmitem{def:institution/satisfaction} For every signature \( \Sigma \), a \hyperref[def:relation]{relation} \( {\vDash_\Sigma} \) between models from \( \cat{Mod}(\Sigma) \) and sentences from \( \op*{Sen}(\Sigma) \) such that, for every signature morphism \( t: \Sigma \to \Sigma' \), every sentence \( \varphi \) in \( \op*{Sen}(\Sigma) \) and every model \( \mscrY \) in \( \cat{Mod}(\Sigma') \), the following condition holds:
    \begin{equation}\label{eq:def:institution/satisfaction}
      \mscrY \vDash_{\Sigma'} \underbrace{\op*{Sen}(t)(\varphi)}_{\mathclap{\T*{translation of} \varphi}}
      \T{if and only if}
      \underbrace{\cat{Mod}(t)(\mscrY)}_{\mathclap{\T*{reduct of} \mscrY}} \vDash_\Sigma \varphi
    \end{equation}
...
\begin{comments}
  ...
  \item An institution provides a particular instance of the \hyperref[con:syntax_semantics_duality]{syntax-semantics duality}.
\end{comments}
...
\begin{proof}
  ...
  Fix a signature translation \( t: \Sigma \to \Sigma' \) and suppose that \( \Gamma \vDash_\Sigma \varphi \).

  Suppose that some model \( \mscrT \) in \( \cat{Mod}(\Sigma') \) satisfies \( t[\Gamma] \). Due to \eqref{eq:def:institution/satisfaction}, \( \cat{Mod}(t)(\mscrT) \) is a model in \( \cat{Mod}(\Sigma) \) that satisfies \( \Gamma \), and hence also \( \varphi \). Again via \eqref{eq:def:institution/satisfaction} we conclude that \( \mscrT \) satisfies \( t(\varphi) \).
...
\begin{corollary}\label{thm:institutional_satisfaction_closure}
  In an \hyperref[def:institution]{institution}, if \( \mscrX \vDash \Gamma \), then \( \mscrX \vDash \cat{Th}(\Gamma) \).
\end{corollary}
\begin{proof}
...
\begin{definition}\label{def:theory_of_institutional_model}\mimprovised
  In an \hyperref[def:institution]{institution}, we denote by \( \cat{Th}(\mscrX) \) the set of all sentences \hyperref[def:propositional_semantics/satisfaction]{valid} in \( \mscrX \). We call it the \term{theory} \emph{of} \( \mscrX \).
\end{definition}
\begin{defproof}
  We must justify the name by showing that \( \cat{Th}(\mscrX) \) is closed under entailment.

  Let \( \Sigma \) be the underlying signature. Suppose that \( \cat{Th}(\mscrX) \vDash_\Sigma \varphi \), i.e. every model \( \mscrY \) that satisfies \( \cat{Th}(\mscrX) \) must also satisfy \( \varphi \). One such model is \( \mscrX \) itself. So, by definition of \( \cat{Th}(\mscrX) \) must contain \( \varphi \).
\end{defproof}
...
\begin{definition}\label{def:equisatisfiability}\mimprovised
  we say that two sets \( \Gamma \) and \( \Delta \) of sentences over possibly different signatures are \term[en=equisatisfiable (\cite[237]{Mimram2020ProgramEqualsProof})]{equisatisfiable} if \( \Gamma \) and \( \Delta \) either both have (possibly different) models or if both do not.
\end{definition}
...
\begin{definition}\label{def:categorical_logical_theory}
  With respect to a given \hyperref[def:institution]{institution}, we say that a set of sentences is \term[ru=категоричная (система аксиом) (\cite[248]{Мальцев1970АлгебраическиеСистемы}), en=categorical (theory) (\cite[def. 2.5.2]{Hinman2005Logic})]{categorical} if all its models are isomorphic.
\end{definition}
\begin{comments}
  \item The term \enquote{categorical} is unrelated to \fullref{ch:category_theory} --- see \cref{rem:categorical_theory_terminology}.

  \item We generalize the definitions given for \hyperref[def:fol_theory]{first-order theories} in \cite[def. 2.5.2]{Hinman2005Logic} and \cite[88]{Shoenfield1967MathematicalLogic} and for \hyperref[def:hol_theory]{higher-order theories} in \cite[exerc. X5502]{Andrews2002Logic}.

  This definition technically makes unsatisfiable theories categorical. Unlike the aforementioned authors, \textcite[def. 3.3.15]{VanDalen2004LogicAndStructure} explicitly requires the theory to be satisfiable.
\end{comments}

\begin{remark}\label{rem:categorical_theory_terminology}
  Categorical theories in the sense of \cref{def:categorical_logical_theory} are attributed by \textcite[174]{Tarski1983ConceptOfTruth} to \textcite[346]{Veblen1904AxiomsForGeometry}. Veblen's paper predates \fullref{ch:category_theory} by nearly four decades; see \cref{rem:category_as_graph} for the latter.

  We use categories to define many concepts across this chapter, for example the \hyperref[def:category_of_theories]{category of small theories} of an \hyperref[def:entailment_system]{entailment system}. Even though the phrase \enquote{categorical theory} may be interpreted as \enquote{theory related to categories}, this risk is negligible.

  Indeed, explicit definitions of theories that are categorical in our sense can be found in
  \cite[def. 2.5.2(i)]{Hinman2005Logic},
  \cite[88]{Shoenfield1967MathematicalLogic},
  while the related notion of \( \kappa \)-categorical theories, where only models of cardinality \( \kappa \) should be isomorphic, is defined in
  \cite[def. 3.3.15]{VanDalen2004LogicAndStructure},
  \cite[exerc. X5502]{Andrews2002Logic} and
  \cite[def. 2.3.3]{TentZiegler2012ModelTheory}.

  An explicit definition can also be found in Russian as \enquote{\textru{категоричная теория}}, for example in \cite[249]{Мальцев1970АлгебраическиеСистемы}, however there is less ambiguity because \enquote{theory related to categories} in Russian would be \enquote{\textru{категорная теория}}.
\end{remark}
...
\begin{comments}
  ...
  The term \enquote{logical framework} is a fine candidate, but is unfortunately also ambiguous --- according to \textcite{Pfenning2002LogicalFrameworks},
  \begin{displayquote}
...
\begin{remark}\label{rem:soundness_and_completeness_theorem_list}
  ...
  \begin{itemize}
    ...
    \item \Fullref{thm:fol_completeness}.
    \item \Fullref{thm:hol_natural_deduction_soundness}.
    \item \Fullref{thm:hol_weak_completeness}.
  \end{itemize}
\end{remark}
\begin{comments}
  \item We only show incompleteness in \fullref{thm:hol_incompleteness}.
\end{comments}
...
\begin{concept}\label{con:classical_logic}\mimprovised
  By \term[ru=классическая логика (\cite[58]{ШеньВерещагин2017ЯзыкиИИсчисления}), en=classical logic (\cite[8]{TroelstraSchwichtenberg2000BasicProofTheory})]{classical logic} we will mean either of the following:
  \begin{itemize}
    \item The \hyperref[def:general_logic]{general logic} defined in \cref{def:propositional_logic} for \hyperref[def:propositional_formula]{propositional formulas}, with \hyperref[def:truth_value_algebra/classical]{Boolean} \hyperref[def:propositional_institution]{propositional institution} and the corresponding \hyperref[def:propositional_natural_deduction]{classical natural deduction system}

    \item The general logic defined in \cref{def:first_order_logic} for \hyperref[def:fol_formula]{first-order formulas}, with the \hyperref[def:fol_institution]{first-order institution} and the corresponding \hyperref[def:fol_natural_deduction]{natural deduction system}.

    \item The general logic defined in \cref{def:higher_order_logic} for \hyperref[def:hol_expression/formula]{higher-order formulas}, with the (standard or general) \hyperref[def:hol_institution]{higher-order institution} and the corresponding \hyperref[def:hol_natural_deduction]{natural deduction system}.
  \end{itemize}
\end{concept}
\begin{comments}
...
\begin{concept}\label{con:intuitionistic_logic}\mimprovised
  ...
  We will define a \hyperref[def:general_logic]{general logic} for \hyperref[def:propositional_formula]{propositional formulas} in \cref{def:propositional_logic}, with the \hyperref[def:truth_value_algebra/intuitionistic]{intuitionistic} \hyperref[def:propositional_institution]{propositional institution} and the corresponding \hyperref[def:propositional_natural_deduction]{natural deduction system}. We will extend intuitionistic natural deduction to \hyperref[def:fol_formula]{first-order formulas} in \cref{def:fol_natural_deduction} and to \hyperref[def:hol_expression/formula]{higher-order formulas} in \cref{def:hol_natural_deduction}.
\end{concept}
\begin{comments}
...
\begin{concept}\label{con:minimal_logic}\mimprovised
  By (Johansson's) \term[en=minimal logic (\cite[1]{VanDerMolen2016MinimalLogic}]{minimal logic} we will mean the generalization of \hyperref[con:intuitionistic_logic]{intuitionistic logic} where we reject both \fullref{thm:propositional_semantic_lem} and the strictly weaker \fullref{thm:propositional_semantic_efq}. More technically, we ignore the special role of the \hyperref[def:propositional_alphabet/constants/falsum]{falsum} \( \synbot \) in hope to avoid the \enquote{paradoxes of \hyperref[con:material_implication]{material implication}}.

  We will only sketch semantics for \hyperref[def:propositional_formula]{propositional formulas} in \cref{rem:minimal_propositional_semantics}, but will present a full-fledged natural deduction system in \cref{def:propositional_natural_deduction}, and extend it to \hyperref[def:fol_formula]{first-order formulas} in \cref{def:fol_natural_deduction} and \hyperref[def:hol_expression/formula]{higher-order formulas} in \cref{def:hol_natural_deduction}.
\end{concept}
