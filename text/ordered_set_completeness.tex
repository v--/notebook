...
\begin{proposition}\label{thm:def:dedekind_macnielle_closure}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:def:dedekind_macnielle_closure/closed_elements} If a set is \hyperref[def:dedekind_macnielle_closure]{Dedekind-MacNeille closed}, it is \hyperref[def:closed_ordered_subset]{downward closed} and if it contains the \hyperref[def:extremal_points/supremum_and_infimum]{suprema} of all its nonempty subsets that have suprema.

    The converse holds for totally ordered sets but may fail in general --- see \ref{fig:def:dedekind_macnielle_closure}.
  \end{thmenum}
\end{proposition}
\begin{proof}
  ...
  \SubProofOf{thm:def:dedekind_macnielle_closure/closed_elements}
  ...
  \NecessitySubProof* Suppose that the order is total. Suppose also that \( A \) is downward closed and that, if a nonempty subset of \( A \) has a supremum, the latter belongs to \( A \).
