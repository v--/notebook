...
\end{corollary}
\begin{proof}
  Follows from \cref{thm:directed_multigraph_geometric_realization_paths}.
\end{proof}
...
\end{corollary}
\begin{proof}
  Follows from \cref{thm:directed_multigraph_geometric_realization_connectedness}.
\end{proof}
...
\end{proposition}
\begin{proof}
  Follows from \cref{ex:vandermonde_matrix}.
\end{proof}
...
\begin{definition}\label{def:planar_graph}\mcite[102]{Harary1969GraphTheory}
  If an \hyperref[rem:graph_of_unspecified_type]{graph of unspecified type} can be embedded into \( \BbbR^2 \), we say that it is \term[bg=планарен (граф) (\cite[129]{Мирчев2001Графи}), ru=планарный (граф) (\cite[150]{ЕмеличевИПр1990ТеорияГрафов}), en=planar (graph) (\cite[89]{Diestel2017GraphTheory})]{planar}.
\end{definition}
