...
\begin{example}\label{ex:def:directed_multigraph_geometric_realization}
  ...
  \begin{thmenum}
    \thmitem{ex:def:directed_multigraph_geometric_realization/order_zero} The geometric realization of an \hyperref[def:hypergraph/trivial]{edgeless} directed multigraph is the discrete topological space on its vertices.
...
\begin{proposition}\label{thm:def:directed_multigraph_geometric_realization}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:def:directed_multigraph_geometric_realization/t1} If \( Q \) is \hyperref[def:hypergraph/degree]{locally finite}, then the space \( (X, \mscrT) \) satisfies the \ref{def:separation_axioms/T1} separation axiom.
  \end{thmenum}
\end{proposition}
\begin{proof}
...
\begin{proposition}\label{thm:directed multigraph_geometric_realization_paths}
  ...
  \begin{thmenum}
    \thmitem{thm:directed multigraph_geometric_realization_paths/directed multigraph_to_realization} If there exists an \hyperref[def:directed_multigraph_path/undirected]{undirected path} \( p = (v, e_1, \ldots, e_n) \) from \( s \) to \( f \), then there exists a \hyperref[def:parametric_curve]{continuous path} \( \gamma: [0, 1] \to X \) from \( R_V(s)  \) to \( R_V(f) \).
