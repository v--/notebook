...
\begin{proposition}\label{thm:directed multigraph_geometric_realization_paths}
  ...
  \begin{thmenum}
    \thmitem{thm:directed multigraph_geometric_realization_paths/directed multigraph_to_realization} If there exists an \hyperref[def:graph_walk/path/undirected]{undirected path} \( p = (v, e_1, \ldots, e_n) \) from \( s \) to \( f \), then there exists a \hyperref[def:parametric_curve]{continuous path} \( \gamma: [0, 1] \to X \) from \( R_V(s)  \) to \( R_V(f) \).
