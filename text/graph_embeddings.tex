...
\begin{definition}\label{def:graph_geometric_realization}\mimprovised
  ...
  \begin{thmenum}
    ...
    Every finite simple graph can be embedded into \( \BbbR^3 \) as shown in \fullref{thm:directed_multigraph_can_be_embedded_into_r3}.

    \thmitem{def:graph_geometric_realization/planar} If a graph can be embedded into \( \BbbR^2 \), we say that it is \term[bg=планарен (граф) (\cite[129]{Мирчев}), ru=планарный (граф) (\cite[313]{Зыков2004}), en=planar (graph) (\cite[93]{Diestel2005})]{planar}.
...
\begin{proposition}\label{thm:moment_curve}\mcite[833]{Rosen1999}
...
\begin{theorem}[Wagner's theorem]\label{thm:wagners_theorem}\mcite[thm. 18]{Bollobas1998}
  A \hyperref[def:undirected_graph]{simple undirected graph} is \hyperref[def:graph_geometric_realization/planar]{planar} if and only if it contains neither the \hyperref[def:complete_graph]{complete graph} \( K_5 \) nor the \hyperref[def:complete_graph]{complete bipartite graph} \( K_{3,3} \) as a \hyperref[def:graph_minor]{minor}.
\end{theorem}
