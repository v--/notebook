\section{Propositional formula transformations}\label{sec:propositional_formula_transformations}
...
\begin{definition}\label{def:propositional_substitution}\mimprovised
  We can define \hyperref[con:syntactic_substitution]{simultaneous substitution} for \hyperref[def:propositional_formula]{propositional formulas} as follows.

  Consider a map \( \Bbbs: \op*{Prop} \to \op*{Form} \), which allows us to replace propositional variables with arbitrary propositional formulas. In the case where only finitely many variables are not \hyperref[def:function_fixed_point]{fixed points}, we will call the map \( \Bbbs \) itself a \term{substitution}.
...
\begin{proposition}\label{thm:propositional_substitution_equivalence}
  For a \hyperref[def:propositional_interpretation]{propositional interpretation} \( I \) and \hyperref[def:propositional_substitution]{simultaneous substitution} \( \Bbbs \), we have \( \Bracks[\big]{\varphi[\Bbbs]}_I = \Bracks{\varphi}_I \) if and only if \( \Bracks{\Bbbs(p)}_I = I(p) \) for every variable \( p \) in \( \op*{Var}(\varphi) \).
\end{proposition}
\begin{proof}
  \SufficiencySubProof Suppose that \( \Bracks[\big]{\varphi[\Bbbs]}_I = \Bracks{\varphi}_I\). We will use \fullref{thm:induction_on_rooted_trees} on the abstract syntax tree of \( \varphi \) to show that \( \Bracks{\Bbbs(p)}_I = I(p) \) for every variable \( p \) in \( \op*{Var}(\varphi) \).
  ...
  \NecessitySubProof Suppose that \( \Bracks{\Bbbs(p)}_I = I(p) \) for every variable \( p \) in \( \op*{Var}(\varphi) \). We will use \fullref{thm:induction_on_rooted_trees} on \( \varphi \) to show that \( \Bracks[\big]{\varphi[\Bbbs]}_I = \Bracks{\varphi}_I \).
...
\begin{definition}\label{def:propositional_subformula_substitution}\mcite[def. 1.3.13]{Hinman2005Logic}
  ...
  \begin{equation}\label{eq:def:propositional_subformula_substitution}
    \varphi[\chi \mapsto \omega] \coloneqq \begin{cases}
      \omega,                                                         &\varphi = \chi \\
      \varphi,                                                        &\varphi \neq \chi \T{and} \varphi \in \op*{PConst} \cup \op*{Prop} \\
...
\end{proposition}
\begin{proof}
  Suppose that \( \Bracks{\chi}_I = \Bracks{\omega}_I \). We will use \fullref{thm:induction_on_rooted_trees} on the abstract syntax tree of \( \varphi \) to show that
  ...
  \begin{thmenum}
    ...
    \begin{equation*}
      T_{\logic{cc}}(\psi \syniff \theta) \coloneqq \begin{cases}
        ...
        \synbot,                   &\T{otherwise if} \psi' = \syntop \T{and} \theta' = \synbot \\
        \synbot,                   &\T{otherwise if} \psi' = \synbot \T{and} \theta' = \syntop \\
...
\end{algorithm}
\begin{comments}
  \item This algorithm can be found as \identifier{math.logic.transformation.constants.collapse_constants} in \cite{notebook:code}.
\end{comments}
\begin{defproof}
...
\begin{algorithm}[Propositional constant expansion]\label{alg:propositional_constant_expansion}
  We introduce an operator \( T_{\logic{ce}} \) on \hyperref[def:propositional_formula]{propositional formulas} that removes propositional constants at the cost of possibly introducing new variables.

  Unlike the collapsing of constants, this algorithm is much simpler. Denote by \( p_\varphi \) the lexicographically smallest variable in \( \varphi \), if such exists, and let \( p_\varphi \) be the default variable \( \synp \) otherwise. Define
  \begin{equation*}
...
\end{algorithm}
\begin{comments}
  \item This algorithm can be found as \identifier{math.logic.transformation.constants.expand_constants} in \cite{notebook:code}.
\end{comments}
\begin{proof}
...
\begin{definition}\label{def:cnf_and_dnf}\mimprovised
  ...
  \begin{longbnf*}
    ...
    \bnfprod{negative literal}       {\synneg \bnfpn{propositional variable}} \\
...
\begin{comments}
  ...
  \item With only positive literals, this becomes a particular syntax for \hyperref[rem:lattice_polynomials]{lattice polynomials}.
...
\begin{remark}\label{rem:straightforward_traversal}
  ...
  In computer programming, this exact problem is solved via the visitor design pattern --- generic visitors for formulas of first-order logic can be found in the module \identifier{math.logic.formulas.formula_visitor} in \cite{notebook:code}.
\end{remark}
...
\paragraph{Dual formulas}

\begin{algorithm}[Dual propositional formulas]\label{alg:dual_propositional_formulas}
  We introduce several operators on \hyperref[def:propositional_formula]{propositional formulas} that produce dual formulas. With logical operations regarded as Boolean algebra operations, these correspond exactly to the transformations from \fullref{thm:boolean_algebra_duality}. These substitutions are only sound with respect to \hyperref[def:truth_value_algebra/intuitionistic]{classical semantics}.

  \begin{thmenum}
    \thmitem{alg:opposite_propositional_formula/literals} The following only exchanges every variable with its negation:
    \begin{equation*}
      \varphi^{\oppos_L} \coloneqq \begin{cases}
        \varphi,                                    &\varphi \in \op*{PConst}, \\
        \synneg \varphi,                            &\varphi \in \op*{Prop}, \\
        \psi,                                       &\varphi = \synneg \psi \T{and} \psi \in \op*{Prop}, \\
        \synneg \psi^{\oppos_L},                    &\varphi = \synneg \psi \T{and} \psi \not\in \op*{Prop}, \\
        \psi^{\oppos_L} \syncirc \theta^{\oppos_L}, &\varphi = \psi \syncirc \theta. \\
      \end{cases}
    \end{equation*}

    The letter \enquote{L} comes from \enquote{literal} since propositional variables and their negations are called so in the context of \hyperref[def:cnf_and_dnf]{conjunctive and disjunctive normal forms}.

    \thmitem{alg:opposite_propositional_formula/connectives} The following instead exchanges dual connectives and swaps propositional constants:
    \begin{equation*}
      \varphi^{\oppos_C} \coloneqq \begin{cases}
        \synbot,                                       &\varphi = \syntop, \\
        \syntop,                                       &\varphi = \synbot, \\
        \varphi,                                       &\varphi \in \op*{Prop}, \\
        \synneg \psi^{\oppos_C},                       &\varphi = \synneg \psi, \\
        \psi^{\oppos_C} \synvee \theta^{\oppos_C},     &\varphi = \psi \synwedge \theta, \\
        \psi^{\oppos_C} \synwedge \theta^{\oppos_C},   &\varphi = \psi \synvee \theta, \\
        \theta^{\oppos_C} \synimplies \psi^{\oppos_C}, &\varphi = \psi \synimplies \theta, \\
        \psi^{\oppos_C} \syniff \theta^{\oppos_C},     &\varphi = \psi \syniff \theta. \\
      \end{cases}
    \end{equation*}

    Note that we exchange every conditional formula with its converse, but preserve the biconditionals.

    \thmitem{alg:opposite_propositional_formula/combined} Finally, since the two commute, we can compose them in any order to obtain \( \varphi^{\oppos_{LC}} \).
  \end{thmenum}
\end{algorithm}
\begin{comments}
  \item This algorithm can be found as \identifier{math.logic.transformation.negation.to_dual_formula} in \cite{notebook:code}.
\end{comments}

\begin{proposition}\label{thm:opposite_propositional_formula_semantics}
  Under \hyperref[def:truth_value_algebra/intuitionistic]{classical semantics}, the dual formula operators from \fullref{alg:opposite_propositional_formula} behave as follows with respect to a \hyperref[def:propositional_interpretation]{propositional interpretation} \( I \):
  \begin{equation*}
    \Bracks{\varphi}_I = \Bracks{\varphi^{\oppos_{LC}}}_I = \oline{\Bracks{\varphi^{\oppos_L}}_I} = \oline{\Bracks{\varphi^{\oppos_C}}_I}.
  \end{equation*}
\end{proposition}
\begin{comments}
  \item Follows from \fullref{thm:boolean_algebra_duality}.
\end{comments}
