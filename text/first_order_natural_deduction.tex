...
\begin{definition}\label{def:fol_natural_deduction_proof_tree}
  ...
  \begin{thmenum}[resume=def:fol_natural_deduction_proof_tree]
    ...
    Let \( \BbbI \) be an \hyperref[def:atomic_fol_instantiation]{atomic schema instantiation} and a list \( (P_1, \ldots, P_n) \) of proof trees such that, for \( i = 1, \ldots, n \), the conclusion of \( P_i \) is \( \Phi_i[\BbbI] \).
...
\end{definition}
\begin{comments}
  \item The treatment of eigenvariables are generalized here from the two quantifier rules discussed in detail in \cite[\S 2.1]{TroelstraSchwichtenberg2000BasicProofTheory}. We present these rules in \cref{def:fol_natural_deduction}.
...
\begin{definition}\label{def:fol_natural_deduction}
\end{definition}
\begin{comments}
  \item These precise rules are presented in the module \identifier{logic.classical_logic} in \cite{notebook:code}.
\end{comments}
