...
\begin{corollary}\label{thm:fol_formula_semantics_of_single_substitution}
  Fix a \hyperref[def:fol_formula]{first-order formula} \( \tau \), an \hyperref[def:atomic_fol_substitution]{atomic substitutions} \( \Bbbs \) and a \hyperref[def:fol_variable_assignment]{variable assignments} \( v \) in some \hyperref[def:fol_structure]{structure} \( \mscrX = (X, I) \).

  If \( v(x) = \Bracks{\Bbbs(x)}_\mscrX^v \) for every \hi{free} variable \( x \) in \( \varphi \), then
  \begin{equation}\label{eq:thm:fol_formula_semantics_of_single_substitution}
    \Bracks{\varphi[\Bbbs]}_\mscrX^v = \Bracks{\varphi}_\mscrX^v.
  \end{equation}
\end{corollary}
\begin{comments}
  \item An analogous statement holds for formulas --- see \cref{thm:fol_formula_semantics_of_single_substitution}.
\end{comments}
\begin{proof}
  Follows from \cref{thm:fol_term_semantics_of_compatible_substitutions} with \( \Bbbt = \id \).
\end{proof}
