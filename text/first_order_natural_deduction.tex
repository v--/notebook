...
\begin{proposition}\label{thm:alg:fol_formula_substitution}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:alg:fol_formula_substitution/quant_single_rule} For any quantifier formula \( \varphi = \quantifier Q x \psi \) and any substitution \( \Bbbs \), there exists a variable \( n \not\in \op*{Free}_\Bbbs(\varphi) \) such that
    \begin{equation}\label{eq:thm:alg:fol_formula_substitution/quant_single_rule}
      \varphi[\Bbbs] = \quantifier Q n \psi[\Bbbs_{x \mapsto v}].
    \end{equation}
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \begin{equation*}
      ...
      \reloset {\eqref{eq:thm:fol_term_and_assignment_substitution}} =
...
\begin{corollary}\label{thm:fol_formula_and_assignment_substitution}
  ...
  \begin{equation}\label{eq:thm:fol_formula_and_assignment_substitution}
    \Bracks{\varphi}_\mscrX^{v[\Bbbs]} = \Bracks{\varphi[\Bbbs]}_\mscrX^v
  \end{equation}
\end{corollary}
\begin{comments}
  \item An analogous statement holds for terms --- see \cref{thm:fol_term_and_assignment_substitution}.
\end{comments}
\begin{proof}
  We can use \cref{thm:fol_formula_semantics_of_compatible_substitutions} with \( \Bbbt = \id \).
\end{proof}
...
\begin{definition}\label{def:fol_formula_length}\mimprovised
  We will extend the \term{length} defined in \cref{def:fol_term_length} for \hyperref[def:fol_term]{first-order terms} to \hyperref[def:fol_formula]{formula} as
  \begin{equation*}
    \len(\varphi) \coloneqq \begin{cases}
      1,                                        &\varphi \in \op*{PConst}, \\
      3 + \len(\tau) + \len(\sigma),            &\varphi = (\tau \syneq \sigma), \\
      1 + \len(\tau_1) + \cdots + \len(\tau_n), &\varphi = p(\tau_1, \ldots, \tau_n), \\
      1 + \len(\psi),                           &\varphi = \synneg \psi, \\
      3 + \len(\psi) + \len(\theta),            &\varphi = (\psi \syncirc \theta), {\syncirc} \in \op*{Conn}, \\
      3 + \len(\psi),                           &\varphi = \quantifier Q x \psi, Q \in \op*{Quant}.
    \end{cases}
  \end{equation*}
\end{definition}
\begin{comments}
  \item If all predicate symbols use prefix notation and if the variable names in \( \varphi \) contain no numeric indices, then \( \len(\varphi) \) is exactly the \hyperref[def:formal_language/string_length]{string length} of \( \varphi \).
\end{comments}

\begin{proposition}\label{thm:def:fol_formula_length}
  \hyperref[def:fol_formula]{First-order formulas} have the following basic properties regarding their \hyperref[def:fol_formula_length]{length}:
  \begin{thmenum}
    \thmitem{thm:def:fol_formula_length/subformula} Every strict subformula of a formula is strictly shorter.

    \thmitem{thm:def:fol_formula_length/substitution} For any atomic substitution \( \Bbbs \) and formula \( \varphi \), we have
    \begin{equation}\label{eq:thm:def:fol_formula_length/substitution}
      \len(\varphi[\Bbbs]) = \len(\varphi) + \sum_{\mathclap{u \in \op*{Var}(\varphi)}} (\len(\Bbbs(u)) - 1).
    \end{equation}

    In particular, if \( \Bbbs \) is a \hyperref[def:lambda_renaming_substitution]{renaming}, then \( \len(\varphi[\Bbbs]) = \len(\varphi) \).

    \thmitem{thm:def:fol_formula_length/equivalent} \hyperref[def:fol_formula_alpha_equivalence]{\( \alpha \)-equivalent} formulas have the same length.
  \end{thmenum}
\end{proposition}
\begin{comments}
  \item See the corresponding statements for \( \muplambda \)-terms in \cref{thm:def:lambda_term_length} and for first-order terms in \cref{thm:def:fol_term_length}.
\end{comments}
\begin{proof}
  \SubProofOf{thm:def:fol_formula_length/subformula} Trivial.
  \SubProofOf{thm:def:fol_formula_length/substitution} Similar to \cref{thm:def:lambda_formula_length/substitution}.
  \SubProofOf{thm:def:fol_formula_length/equivalent} Similar to \cref{thm:def:lambda_formula_length/equivalent}.
\end{proof}
...
\end{proposition}
\begin{proof}
  We will use induction on the \hyperref[thm:def:fol_formula_length]{length} of \( \varphi \) to show, simultaneously on all assignments \( v \) in \( \mscrX = (X, I) \), that \( \varphi \aequiv \psi \) implies \( \Bracks{\varphi}_\mscrX^v = \Bracks{\psi}_\mscrX^v \).

  The only formula kinds of fixed length are the propositional constants, for which the proposition is obvious. Suppose that \( \varphi \aequiv \psi \) and that the statement holds for all formulas shorter than \( \varphi \).
