...
\begin{definition}\label{def:fol_formula_schema}\mimprovised
  ...
  \begin{longbnf*}
    \bnfprod{eigenvariable}           {\bnfpn{variable placeholder} \bnfsp \bnftsq{\( \ast \)}} \\
    \bnfprod{formula placeholder}     {\bnfpn{Small Greek identifier}} \\
    \bnfprod{formula schema}          {\bnfpn{formula placeholder} \bnfor \cdots} \\
    \bnfprod{substitution subject}    {\bnfpn{term schema} \bnfor \bnfpn{eigenvariable}} \\
...
\begin{comments}
  ...
  \item The eigenvariable rules only show intention; we will use them to restrict certain constructions when defining proof trees in \cref{def:fol_natural_deduction_proof_tree}.
\end{comments}
...
\begin{algorithm}[First-order formula instantiation]\label{alg:fol_formula_schema_instantiation}
  ...
  \begin{equation}\label{eq:alg:lambda_term_schema_instantiation}
    \Phi[\BbbI] \coloneqq \begin{cases}
      ...
      \Phi[\BbbI][\BbbI(x) \mapsto \tau[\BbbI]],  &\Phi = \Phi[x \synsubst \tau] \T{and} \tau \T{is not an eigenvariable}.
    \end{cases}
  \end{equation}
