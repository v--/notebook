...
\begin{proposition}\label{thm:fol_formula_semantics_of_compatible_substitutions}
  ...
  If \( \Bracks{\Bbbs(x)}_\mscrX^v = \Bracks{\Bbbt(x)}_\mscrX^w \) for every \emph{free} variable \( x \) in \( \varphi \), then \( \Bracks{\varphi[\Bbbs]}_\mscrX^v = \Bracks{\varphi[\Bbbt]}_\mscrX^w \).
\end{proposition}
\begin{comments}
  \item An analogous statement holds for first-order terms --- see \cref{thm:fol_term_semantics_of_compatible_substitutions} --- and for higher-order expressions --- see \cref{thm:hol_expression_semantics_of_compatible_substitutions}.
\end{comments}
\begin{proof}
...
\begin{corollary}\label{thm:fol_formula_semantics_of_assignment_substitution}
  \Fullref{alg:fol_formula_substitution} and \fullref{alg:fol_assignment_substitution} are compatible for \emph{quantifierless} formulas:
...
\begin{proposition}\label{thm:def:fol_formula_alpha_equivalence}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:def:fol_formula_alpha_equivalence/substitution_composition} For the \hyperref[def:fol_substitution_composition]{composition} \( \Bbbt \Bbbs \) of the \hyperref[def:atomic_fol_substitution]{substitutions} \( \Bbbt \Bbbs \), we have
...
\begin{definition}\label{def:fol_formula_schema}\mimprovised
  ...
  \begin{longbnf*}
    ...
    \bnfprod{extended formula placeholder} {\bnfpn{formula placeholder} \bnfsp \bnfpn{substitution}} \\
    \bnfprod{formula schema}               {\bnfpn{formula placeholder} \bnfor \cdots} \\
    \bnfprod{extended formula schema}      {\bnfpn{extended formula placeholder} \bnfor \bnfpn{formula schema} \bnfor \cdots}
  \end{longbnf*}
...
\end{definition}
\begin{comments}
  \item Extended schemas possibly have substitution specifications after formula placeholders.
  ...
  \item It is possible to place a substitution schema on formula placeholders subformulas, like in the axiom schema \eqref{eq:def:peano_arithmetic/theory/PA3}. For natural deduction, we will only need standalone placeholders with substitutions.
...
\begin{definition}\label{def:fol_natural_deduction_proof_tree}
  ...
  In addition to the conclusion, rule name and open and discharged assumptions, we will also need several sets of variables:
  ...
  \begin{thmenum}[resume=def:fol_natural_deduction_proof_tree]
    ...
    \begin{thmenum}[series=def:fol_natural_deduction_proof_tree/application]
      \thmitem{def:fol_natural_deduction_proof_tree/application/main} If the rule premise \( \Phi_k \) is an extended placeholder with an eigenvariable specification, then \( \Phi_k = \Lambda[\xi \synsubst \eta^*] \) for some formula placeholder \( \Lambda \) and variable placeholders \( \xi \) and \( \eta \). For convenience, denote \( \BbbI(\xi) \) and \( \BbbI(\eta) \) by \( x \) and \( y \).
      ...
      \begin{thmenum}
        ...
        This condition will automatically be satisfied for the only rule, \ref{inf:def:fol_natural_deduction/forall/intro}, in which we will have such a situation. We list the condition here only for completeness, since a similar condition is listed in the other case below.

        \thmitem{def:fol_natural_deduction_proof_tree/application/main/assumptions} Additionally, \( y \) must not be open in \( P_k \).
      \end{thmenum}

      \thmitem{def:fol_natural_deduction_proof_tree/application/attached} If the premise \( \Phi_k \) has a dischargeable schema \( \Theta_{k,i} \) attached, and if \( \Theta_{k,i} \) is a placeholder with an eigenvariable specification, then, as above, \( \Theta_{k,i} = \Lambda[\xi \synsubst \eta^*] \). Again, denote \( \BbbI(\xi) \) and \( \BbbI(\eta) \) by \( x \) and \( y \).
    ...
    \begin{thmenum}[resume=def:fol_natural_deduction_proof_tree/application]
      ...
      \thmitem{def:fol_natural_deduction_proof_tree/application/open_variables} We will now utilize the set \( D_d \) of discharged assumption markers.
      ...
      \begin{equation*}
        F_d(D, E) \coloneqq \bigcup_{k=1}^n F_k(D \cup D_d, E \cup E_d).
      \end{equation*}

      Thus, have taken the union of all open variables of the subtrees, excluding those discharged at the current application.
    \end{thmenum}
  \end{thmenum}
\end{definition}
\begin{comments}
  ...
  \item The term \enquote{eigenvariable} comes from \cite{Gentzen1935LogischeSchließen}, where Gentzen introduces natural deduction. In the corresponding English translation, \cite[293]{Gentzen1964LogicalDeduction}, eigenvariables are instead called \enquote{proper variables}. Both terms are currently used in English (e.g. \Textcite[\S 5.1.10]{Mimram2020ProgramEqualsProof} uses \enquote{eigenvariable}, while \textcite[38]{TroelstraSchwichtenberg2000BasicProofTheory} uses \enquote{proper variable}).
...
\begin{proposition}\label{thm:def:fol_natural_deduction_proof_tree}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:def:fol_natural_deduction_proof_tree/not_open_var} If the variable \( v \) is \emph{not} in \( F(D, \varnothing) \), it is not free in any open assumption of \( P \) except those with markers in \( D \).
...
\begin{definition}\label{def:fol_natural_deduction_consequence}\mimprovised
  For every \hyperref[def:fol_natural_deduction_system]{first-order natural deduction system}, we define a \hyperref[def:consequence_relation]{consequence relation} on \emph{closed} formulas as follows: we let \( \Gamma \vdash \varphi \) if there exists a \hyperref[def:fol_natural_deduction_proof_tree]{proof tree} with conclusion \( \varphi \) whose \hyperref[def:propositional_natural_deduction_proof_tree/open_assumptions]{open assumptions} are all in \( \Gamma \).

  We may write \( \Gamma \vdash_\Sigma \varphi \) to highlight that all formulas in the proof tree have the same \hyperref[def:fol_signature]{signature} \( \Sigma \).

  In accordance with \cref{def:general_logic}, we say that \( \varphi \) is \term{derivable} from \( \Gamma \) if this relation holds.
\end{definition}
\begin{defproof}
  We can prove that \( {\vdash} \) is indeed a consequence relation analogously to the propositional case --- see \cref{def:propositional_natural_deduction_consequence}.
\end{defproof}

\begin{proposition}\label{thm:fol_natural_deduction_derivation_compact}
  Every \hyperref[def:fol_natural_deduction_consequence]{first-order natural deduction consequence relation} is \hyperref[def:consequence_relation/compactness]{compact}.
\end{proposition}
\begin{comments}
...
\begin{definition}\label{def:fol_natural_deduction}\mimprovised
  ...
  \begin{thmenum}
    \thmitem{def:fol_natural_deduction/equality} Rules for introducing and eliminating the \hyperref[def:predicate_logic_alphabet/equality]{equality}:
    \begin{paracol}{2}
      \begin{leftcolumn}
        \ParacolAlignmentHack
    ...
    For both rules, we often need to perform several successive applications. In accordance with \cref{rem:inference_rule_multiple_application}, in such cases we denote the rules by \( \ref{inf:def:fol_natural_deduction/forall/intro}* \) and \( \ref{inf:def:fol_natural_deduction/forall/elim}* \).
...
\begin{example}\label{ex:def:fol_natural_deduction}
  ...
  \begin{thmenum}
    ...
    \thmitem{ex:def:fol_natural_deduction/implicit_premises} \hyperref[ex:def:propositional_natural_deduction/implicit]{Implicit premises} allows introducing \enquote{implicit} free variables that do not come from any open assumption.

    For example, consider the proof tree
    \begin{equation*}
      \begin{prooftree}
        \infer0[\ref{inf:def:propositional_natural_deduction/top/intro}]{ \syntop }
        \infer1[\ref{inf:def:propositional_natural_deduction/or/intro_left}]{ \syntop \synvee p(x) }
      \end{prooftree}
    \end{equation*}

    Here \( p(x) \) is an implicit premise, in which \( x \) is a free variable. Hence, \( x \) is free in the conclusion, but it is not open in the proof tree. In particular, it satisfies the eigenvariable condition \cref{def:fol_natural_deduction_proof_tree/application/main/assumptions}:
    \begin{equation*}
      \begin{prooftree}
        \infer0[\ref{inf:def:propositional_natural_deduction/top/intro}]{ \syntop }
        \infer1[\ref{inf:def:propositional_natural_deduction/or/intro_left}]{ \syntop \synvee p(x) }
        \infer1[\ref{inf:def:fol_natural_deduction/forall/intro}]{ \qforall x (\syntop \synvee p(x)) }
      \end{prooftree}
    \end{equation*}

    Free variables from implicit premises are explicitly forbidden for \ref{inf:def:propositional_natural_deduction/or/intro_left}, \ref{inf:def:propositional_natural_deduction/or/intro_right} and \ref{inf:def:propositional_natural_deduction/imp/intro} (when no premise is discharged) in a passing remark in \cite[\S 2.1]{TroelstraSchwichtenberg2000BasicProofTheory}. Other sources like \cite{VanDalen2004LogicAndStructure} and \cite[fig. 2.1]{Mimram2020ProgramEqualsProof} do not mention implicit premises at all.

    \thmitem{ex:def:fol_natural_deduction/implicit_variables} Another source of implicit free variables is elimination of universal quantifiers:
    \begin{equation*}
      \begin{prooftree}
        \hypo{ [\qforall x p(x)]^u }
        \infer1[\ref{inf:def:fol_natural_deduction/forall/elim}]{ p(f(y, z)) }
      \end{prooftree}
    \end{equation*}

    Here \( y \) and \( z \) are also free in the conclusion, but are again not open.
  \end{thmenum}
\end{example}
\begin{comments}
...
\begin{remark}\label{rem:fol_empty_universe/natural_deduction}
  ...
  \begin{thmenum}
    ...
    \thmitem{rem:fol_empty_universe/non_eigenvariable} An alternative is suggested by \textcite[\S 5.1.10]{Mimram2020ProgramEqualsProof}. He suggests keeping an explicit \hyperref[def:logical_context]{context} of variables that are allowed to be used in term substitutions (i.e. in \( \tau \) in \ref{inf:def:fol_natural_deduction/forall/elim} and \ref{inf:def:fol_natural_deduction/exists/intro}). Discharging an eigenvariable removes it from this context.
...
\begin{remark}\label{rem:sequent_calculus_eigenvariables}
  \textcite[def. 3.1.1]{TroelstraSchwichtenberg2000BasicProofTheory} give the following \hyperref[def:sequent_calculus_rule]{sequent calculus rule} for \hyperref[def:predicate_logic_alphabet/quantifiers/universal]{universal quantifiers}:
  ...
  \begin{thmenum}
    ...
    \end{equation*}
    but let it fail if \( \BbbI(\eta) \) is \emph{not} in \( E \).
  ...
  \begin{itemize}
    \item In its first occurrence, due to \ref{rem:sequent_calculus_eigenvariables/atomic}, \( \synp(\syny) \) requires \( \syny \) to \emph{not} be in \( E \).
...
\begin{theorem}[First-order syntactic deduction theorem]\label{thm:fol_syntactic_deduction_theorem}
  With respect to \hyperref[def:fol_natural_deduction]{minimal first-order natural deduction}, we have
  \begin{equation*}
...
\end{comments}
\begin{proof}
  Follows by translating the proof trees from \fullref{thm:propositional_syntactic_deduction_theorem} via \fullref{alg:fol_propositional_proof_tree_translation}.
\end{proof}
