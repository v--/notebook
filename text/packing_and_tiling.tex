...
\end{definition}
\begin{comments}
  \item We base our definition on the definition of tilings by \incite[611]{Arenas1999Tiling}, but without the requirement of covering, analogously to how \incite[16]{GrünbaumShephard1987Tilings} define a packing in the Euclidean plane as a tiling that is not a covering.
\end{comments}
...
\begin{remark}\label{rem:sphere_packing}
  ...
  John Conway and Neil Sloane have dedicated an entire book on sphere packing, \cite{ConwaySloane1998SpherePackings}, while \incite[ch. 7]{Rogers1964Packing} discusses \enquote{packings of spheres}. The term \enquote{ball packing} is used by \incite[327]{Gruber2007Geometry}.
...
\end{definition}
\begin{comments}
  \item The density of more general packings is discussed by \incite{Rogers1964Packing}.
\end{comments}
...
\begin{definition}\label{def:topological_space_tiling}\mcite[611]{Arenas1999Tiling}
...
\end{definition}
\begin{comments}
  \item We use the definition of tiling by \incite[611]{Arenas1999Tiling}, but split it in two parts -- we define packing in \fullref{def:topological_space_packing} and them, based on the more concrete case in \incite[16]{GrünbaumShephard1987Tilings}, we define a tiling as a packing that covers the entire space.
...
\begin{remark}\label{rem:tiling_definition}
  ...
  \begin{thmenum}
    \thmitem{rem:tiling_definition/topological} \incite[611]{Arenas1999Tiling} defines tilings in general topological spaces. We use his definition. At this level of generality meaningful regularity conditions are difficult to introduce.

    \thmitem{rem:tiling_definition/grunbaum} \incite[16]{GrünbaumShephard1987Tilings} restrict themselves to the Euclidean plane, and then proceed to classify tilings based on how they are constructed.

    \thmitem{rem:tiling_definition/berger} \incite[596]{Berger2010HigherGeometry} restricts himself to the Euclidean plane and requires all tiles to be compact and to have a nonempty interior. In his earlier book, in \cite[\S 1.7.3]{Berger1987GeometryVol1}, he initially restricts himself to tilings generated from a single connected tile by an \hyperref[def:isometry_group]{isometry group}. These restrictions allow him to prove classification theorems.
  \end{thmenum}
\end{remark}
...
\begin{definition}\label{def:prototile}\mimprovised
  ...
  We say that the subfamily \( \mscrP \) of \( \mscrT \) is a system of \term[en=prototile (\cite[20]{GrünbaumShephard1987Tilings})]{prototiles} if every tile in \( \mscrT \) is \hyperref[def:isometry]{isometric} to \hi{exactly one} tile in \( \mscrP \).
\end{definition}
\begin{comments}
  \item We extend the definition by \incite[20]{GrünbaumShephard1987Tilings} that holds for only one tile. The authors rely on an implicit generalization when defining \( n \)-hedral tilings several pages later.
\end{comments}

\begin{definition}\label{def:k_hedral_tiling}\mcite[23]{GrünbaumShephard1987Tilings}
...
\begin{definition}\label{def:parallelogram_tiling}\mcite[exerc. 1.1.10]{GrünbaumShephard1987Tilings}
...
\begin{comments}
  ...
  \begin{itemize}
    \item \incite[74]{Rogers1964Packing} additionally requires \( \mscrP \) to be countable, and additionally assume the existence of a radius \( r \) such that the balls \( B(p, r) \) with \( p \in \mscrP \) cover \( \BbbR^n \).
