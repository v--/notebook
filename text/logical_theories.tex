...
\begin{definition}\label{def:first_order_theory}\mcite[def. 1.4.5]{VanDalen2004}
  ...
  The set \( \Gamma \) is called \term{closed} if it equals its own closure. A closed set of closed formulas is also sometimes called a \term{theory} in e.g. \cite[48]{VanDalen2004}, but we will not put this restriction because it is not clear what are the axioms of an arbitrary closed set of formulas. Instead, we will use the term \enquote{theory} to mean the set of axioms itself.

  As discussed in \fullref{rem:deduction_with_free_variables}, we are only interested in closed formulas. If a formula is not closed, we instead consider its \hyperref[def:universal_closure]{universal closure}.
  ...
  \begin{thmenum}
    \thmitem{def:first_order_theory/axiomatized}\mcite[def. 3.2.8]{VanDalen2004} We say that the set \( \Gamma \) is \term{axiomatized} by \( \Delta \) if \( \Delta \) is finite and \( \Gamma = \cl(\Delta) \).

    \medskip

    \thmitem{def:first_order_theory/complete}\mcite[def. 3.3.14]{VanDalen2004} The set \( \Gamma \) of formulas is said to be \term{complete} if every for every formula in \( \varphi \), either \( \Gamma \vDash \varphi \) or \( \Gamma \vDash \neg \varphi \).
...
\begin{definition}\label{def:first_order_theory_consistency}\mcite[def. 1.5.2]{VanDalen2004}
...
\begin{proposition}\label{thm:formulas_unsatisfiable_iff_inconsistent}
   Assuming \hyperref[rem:classical_logic]{classical logic}, a theory \( \Gamma \) is \hyperref[def:propositional_semantics/satisfies]{unsatisfiable} if and only if it is semantically \hyperref[def:first_order_theory_consistency]{inconsistent}.
\end{proposition}
\begin{proof}
...
\begin{theorem}[First-order compactness theorem]\label{thm:first_order_compactness_theorem}\mcite[thm. 3.2.1]{VanDalen2004}
...
\begin{theorem}[Downward L\"owenheim-Skolem theorem]\label{thm:downward_lowenheim_skolem_theorem}\mcite[thm. 3.3.12]{VanDalen2004}
...
\begin{example}[Skolem's paradox]\label{ex:skolems_paradox}\mcite[112]{VanDalen2004}
...
\begin{theorem}[Upward L\"owenheim-Skolem theorem]\label{thm:upward_lowenheim_skolem_theorem}\mcite[thm. 3.3.13]{VanDalen2004}
...
\begin{definition}\label{def:category_of_small_first_order_models}
  Let \( \mscrL \) be a first-order language and let \( \Gamma \) be a nonempty set of formulas. See \fullref{rem:positive_formulas_in_category_of_models} for why it is a reasonable restriction for \( \Gamma \) to be a set of \hyperref[def:positive_formula]{positive formulas}.
...
\begin{example}\label{ex:def:category_of_small_first_order_models}
  ...
  \begin{itemize}
    ...
    \item The categories \hyperref[def:partially_ordered_set]{\( \cat{Pos} \)} in order theory and the related \hyperref[def:preordered_set/category]{\( \cat{PreOrd} \)}, \hyperref[def:totally_ordered_set]{\( \cat{Tos} \)}, \hyperref[def:semilattice/category]{\( \cat{Lat} \)}, \hyperref[def:heyting_algebra/category]{\( \cat{Heyt} \)} and \hyperref[def:boolean_algebra/category]{\( \cat{Bool} \)} in lattice theory.
  \end{itemize}
...
\begin{proposition}\label{thm:first_order_categorical_invertibility}
  ...
  \begin{thmenum}
    \thmitem{thm:first_order_categorical_invertibility/injective} Every \hyperref[def:multi_valued_function/empty]{\hi{nonempty}} \hyperref[def:first_order_isomorphism]{structure embedding} (injective homomorphism) is a \hyperref[def:morphism_invertibility/left_cancellative]{categorical monomorphism}.
    ...
    \thmitem{thm:first_order_categorical_invertibility/bijective} Every \hyperref[def:first_order_isomorphism]{structure isomorphism} is a \hyperref[def:morphism_invertibility/isomorphism]{categorical isomorphism} and vice versa.
  \end{thmenum}
\end{proposition}
\begin{proof}
...
\begin{definition}\label{def:lindenbaum_tarski_algebra}\mcite[232]{Wasilewska2018}
  Assume some fixed \hyperref[def:deductive_system]{deductive system} in propositional or first-order logic. Let \( \Gamma \) be a \hyperref[def:first_order_theory]{closed set} of \hyperref[def:first_order_syntax/closed_formula]{closed formulas} within the corresponding logic.
...
\begin{proposition}\label{thm:filters_intuitionistic_lindenbaum_tarski_algebra}
  ...
  Given a \hyperref[def:first_order_theory/complete]{complete theory} \( T \), the set of (equivalence classes of) all formulas derivable from \( T \) is an \hyperref[def:ultrafilter]{ultrafilter} in \( L \).
\end{proposition}
\begin{proof}
  Simply a reformulation of the completeness of a theory.
\end{proof}
