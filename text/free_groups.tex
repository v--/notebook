...
\begin{definition}\label{def:cyclic_group}\mcite[def. II.4.7]{Aluffi2009Algebra}
  For a singleton alphabet \( \set{ a } \), we define the \term[ru=(бесконечная) циклическая группа (\cite[sec. 2.11]{Тыртышников2007ЛинейнаяАлгебра})]{infinite cyclic group} as
...
\begin{definition}\label{def:group_element_order}\mcite[130]{Knapp2016BasicAlgebra}
  The \term[ru=степень (\cite[sec. 2.10]{Тыртышников2007ЛинейнаяАлгебра})]{order} \( \ord(x) \) of a member \( x \) of a group is the smallest positive integer \( n \) such that \( x^n = e \), i.e. order of the \hyperref[def:cyclic_group]{cyclic subgroup} \hyperref[def:group/submodel]{generated} by \( x \).
\end{definition}
