...
\end{theorem}
\begin{comments}
  \item \Cref{thm:hom_adjunction_from_universal_morphism/unit} implies that\( (\Anon*)^* \) is a \hyperref[def:adjoint_functor]{left adjoint} to the \hyperref[con:forgetful_functor]{forgetful functor}
  \begin{equation*}
    U: \cat{Mon} \to \cat{Set}.
  \end{equation*}
\end{comments}
...
\begin{definition}\label{def:monoid_free_product}\mimprovised
  ...
  Finally, we define the free product itself as the \hyperref[def:fol_quotient_structure]{quotient}
...
\begin{proposition}\label{thm:monoid_coproduct}
  The \hyperref[def:discrete_category_limits]{categorical coproduct} of the family \( \seq{ M_k }_{k \in \mscrK} \) in the category \hyperref[def:monoid/category]{\( \cat{Mon} \)} of \hyperref[def:small_set]{small} monoids is their \hyperref[def:monoid_free_product]{free product} \( \Ast_{k \in \mscrK} M_k \).
\end{proposition}
\begin{proof}
  ...
  \begin{itemize}
    ...
    \item Otherwise, if \( v \cong w \) due to \ref{inf:def:fol_generated_congruence/fun}, and there exist elements \( v_1 \), \( v_2 \), \( w_1 \) and \( w_2 \) of \( F(\coprod_{k \in \mscrK} M_k) \) such that \( v = v_1 \cdot v_2 \) and \( v_1 \cong v_2 \), and similarly \( w = w_1 \cdot w_2 \) and \( w_1 \cong w_2 \).
...
\end{theorem}
\begin{comments}
  \item \Cref{thm:hom_adjunction_from_universal_morphism/unit} implies that \( F \) is \hyperref[def:adjoint_functor]{left adjoint} to the \hyperref[con:forgetful_functor]{forgetful functor}
  \begin{equation*}
    U: \cat{Grp} \to \cat{Set}.
  \end{equation*}
\end{comments}
