...
\begin{proof}
  ...
  By \fullref{def:normal_subgroup/kernel}, the kernel \( \ker \varphi \) is a normal subgroup of \( A \), hence by \fullref{thm:quotient_group_universal_property},
  \begin{equation*}
...
\begin{proposition}\label{thm:def:group_order}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:def:group_order/product_of_cyclic} The \hyperref[def:first_order_direct_product]{direct sum} \( C_m \oplus C_n \) of two \hyperref[def:cyclic_group]{cyclic groups} is cyclic if and only if \( m \) and \( n \) are \hyperref[def:coprime_elements]{coprime}.
  \end{thmenum}
\end{proposition}
\begin{proof}
  \SubProofOf{thm:def:group_order/divides} Follows from \fullref{thm:lagranges_theorem_for_arbitrary_subgroups}.
  \SubProofOf{thm:def:group_order/prime_order} Let \( N \) be a proper normal subgroup of \( G \), where \( \ord(G) \) is a prime number.

  From \fullref{thm:lagranges_theorem_for_arbitrary_subgroups} it follows that \( \ord(N) \) divides \( p \). But \( p \) is prime, hence \( N \) is either the trivial group or the full group.

  Therefore, \( G \) is a simple group.

  \SubProofOf{thm:def:group_order/subgroups_of_cyclic} If \( H \) is a subgroup of \( C_n = \braket{ a \given a^n } \), by \fullref{thm:lagranges_theorem_for_arbitrary_subgroups}, its order divides \( n \). Conversely, let \( m \mid n \). Then the following is a cyclic subgroup of exactly \( m \) elements:
  \begin{equation*}
