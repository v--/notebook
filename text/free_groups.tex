...
\begin{definition}\label{def:monoid_free_product}\mimprovised
  ...
  Finally, we define the free product itself as the \hyperref[def:monoid/quotient]{quotient}
...
\begin{definition}\label{def:cyclic_group}\mcite[43]{Jacobson1985AlgebraI}
  We say that a \hyperref[def:group]{group} is \term[ru=циклична группа (\cite[379]{Обрешков1962ВисшаАлгебра}), ru=циклическая группа (\cite[97]{Тыртышников2017Алгебра})]{cyclic} if it can be \hyperref[def:object_presentation]{generated} by a single element.
...
\begin{definition}\label{def:cyclic_monoid}\mcite[exerc. II.4.6]{MacLane1998Categories}
  In analogy with \hyperref[def:cyclic_group]{cyclic groups}, we say that a \hyperref[def:monoid]{monoid} is \term{cyclic} if it can be \hyperref[def:object_presentation]{generated} by a single element.
\end{definition}
\begin{comments}
...
\begin{proof}
  ...
  \SubProofOf{thm:cyclic_monoid_classification/finite_group} Denote the group \eqref{eq:thm:cyclic_monoid_classification/finite} by \( C_n \).
