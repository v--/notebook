...
\begin{proposition}\label{thm:group_categorical_limits}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:group_categorical_limits/coproduct} The \hyperref[def:discrete_category_limits]{categorical coproduct} of the family \( \seq{ G_k }_{k \in \mscrK} \) in the category \hyperref[def:group/category]{\( \cat{Grp} \)} of groups is their \hyperref[def:group_free_product]{free product} \( \Ast_{k \in \mscrK} G_k \).
  \end{thmenum}
\end{proposition}
\begin{comments}
  \item Compare \fullref{thm:group_categorical_limits/coproduct} to the non-commutative case for monoids discussed in \fullref{thm:monoid_categorical_limits/coproduct}.
\end{comments}
\begin{proof}
