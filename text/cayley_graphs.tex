...
\begin{proof}
  ...
  \NecessitySubProof Conversely, let \( G \) be group of finite order \( n \) and suppose that, for some generating set \( C \), there exists a \hyperref[def:directed_graph/morphism]{graph isomorphism} \( \varphi: \Gamma(G, C) \to H \), where \( H \) is a cycle graph with \( n \) vertices oriented so that \( (k, m) \) is an arc if \( m = k + 1 \pmod n \).
...
\begin{definition}\label{def:graph_geodesic}\mcite[1]{Bollobás1984GeodesicsInOrientedGraphs}
  Among all \hyperref[def:graph_walk/directed]{directed walks} between two vertices in an \hyperref[rem:graph_of_unspecified_type]{(hyper)graph of unspecified type}, the ones having shortest length are called \term[ru=геодезические (цепи) (\cite[\S 7.2.5]{Новиков2013ДискретнаяМатематика})]{geodesic}.
\end{definition}
\begin{comments}
