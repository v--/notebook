...
\begin{example}\label{ex:def:cayley_graph}
  ...
  \begin{thmenum}
    ...
    This is used when visualizing \hyperref[def:cellular_automaton]{automata}; see \fullref{rem:automata_on_square_grid}.
  \end{thmenum}
\end{example}
...
\end{definition}
\begin{comments}
  \item We avoid defining a geodesic distance for directed graphs because it is not clear whether we should consider directed or generalized walks. Specifically for \hyperref[def:cayley_graph]{Cayley graphs}, we introduce the Cayley metric in \fullref{def:cayley_distance} based on generalized walks.

  \item We regard \( \infty \) as the corresponding \hyperref[def:extended_real_numbers]{extended real number}, but it is only important for it to be larger than any integer. For the general usage of the symbol \( \infty \), see \fullref{rem:lemniscate_symbol}.
...
\begin{example}\label{ex:def:cayley_graph_neighborhood}
  ...
  \begin{thmenum}
    \thmitem{ex:def:cayley_graph_neighborhood/finite_cyclic} In \( \Gamma(C_n, \set{ a }) \), the example we considered in \fullref{ex:def:cayley_graph/finite_cyclic}, only \( a \) and \( a^{-1} = a^{n-1} \) are adjacent to the origin, so the von Neumann neighborhood is \( \set{ e, a, a^{-1} } \).
    ...
    \thmitem{ex:def:cayley_graph_neighborhood/free} In \( \Gamma(F(\set{ a, b }, \set{ a, b }) \), the example we considered in \fullref{ex:def:cayley_graph/free}, the neighbors of the origin are \( a \), \( b \), \( a^{-1} \) and \( b^{-1} \).
