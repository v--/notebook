...
\begin{proposition}\label{thm:geometric_series_properties}
  ...
  \begin{thmenum}
    \thmitem{thm:geometric_series_properties/finite_sum} For all \( q \in \BbbC \setminus \set{ 1 } \), the geometric series \eqref{eq:def:geometric_progression/series} has partial sums
...
\begin{example}\label{ex:n_ary_decomposition}
  ...
  Another application of \eqref{eq:ex:n_ary_decomposition/binary} is showing that \( 0.\oline{1} = 2 \) in the binary number system. More generally, for the \( n \)-ary number system we have
  \begin{equation}\label{eq:ex:n_ary_decomposition/general}
    \sum_{k=0}^\infty \parens*{ \frac {n-1} n }^k = \frac 1 {1 - (n-1) / n} = n.
  \end{equation}
\end{example}
...
\begin{definition}\label{def:harmonic_progression}
  ...
  \begin{itemize}
    \item \( d = 0 \) and \( a_0 \neq 0 \), which turns \eqref{eq:def:harmonic_progression} into the constant sequence \( \seq{ 1 / a_0 }_{k=0}^\infty \).
    \item \( d \neq 0 \), in which case
    \begin{equation*}
      a_k = \frac d {a_0 / d + k}.
    \end{equation*}

    Thus, if \( d \neq 0 \), then \( a_0 / d \) must not be a negative integer unless we are satisfied with only the first \( * / (-a_0) d \) terms of the progression existing.
  \end{itemize}
...
\begin{remark}\label{rem:harmonic_progression_recursive_form}
  ...
  \begin{equation*}
    a_k \coloneqq \begin{cases}
      1 / a_0,                   & k = 0, \text{ only defined if } a_0 \neq 0, \\
      1 / (a_0 + d),               & k = 1,                                      \\
      \frac 1 {1 / a_{k-1} + d}, & k > 0.
    \end{cases}
  \end{equation*}
\end{remark}
