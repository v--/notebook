...
\begin{example}\label{ex:n_ary_decomposition}
  ...
  \begin{equation}\label{eq:ex:n_ary_decomposition/binary}
    \sum_{k=0}^\infty \frac 1 {2^k} = \frac 1 {1 - \ifrac 1 2} = 2.
  \end{equation}
...
\begin{definition}\label{def:harmonic_progression}
  ...
  \begin{itemize}
    \item \( d = 0 \) and \( a_0 \neq 0 \), which turns \eqref{eq:def:harmonic_progression} into the constant sequence \( \seq{ \ifrac 1 {a_0} }_{k=0}^\infty \).
    \item \( d \neq 0 \), in which case
    \begin{equation*}
      a_k = \frac d {\ifrac {a_0} d + k}.
    \end{equation*}

    Thus, if \( d \neq 0 \), then \( \ifrac {a_0} d \) must not be a negative integer unless we are satisfied with only the first \( \ifrac* {-a_0} d \) terms of the progression existing.
  \end{itemize}
