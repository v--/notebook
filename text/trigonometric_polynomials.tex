\section{Trigonometric polynomials}\label{sec:trigonometric_polynomials}
...
\begin{definition}\label{def:trigonometric_polynomial}
  ...
  \end{equation}
  or, using \hyperref[thm:exponential_trigonometric_identities/eulers_formula]{Euler's formula}, rewritten in the more conventional notation (see \cite[1]{Боянов2008ЧислениМетоди} or \cite[def. 4.23]{Rudin1987RealAndComplexAnalysis}):
  ...
  In particular, when using \cref{def:trigonometric_polynomial/trigonometric}, we may regard the coefficients \( \{ a_k \}_{k=0}^\infty \) and \( \{ b_k \}_{k=1}^\infty \) as either real or complex, which is a downside of \cref{def:trigonometric_polynomial/exponential}.
