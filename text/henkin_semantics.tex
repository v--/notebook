...
\begin{definition}\label{def:hol_term_denotation}\mimprovised
  ...
  If the denotation \( \Bracks{ M^\tau }_\mscrX^v \) coincides for any variable assignment \( v \), we use the simplified notation \( \Bracks{ \varphi }_\mscrX \). This is the case for \hyperref[def:hol_term/closed]{closed logical terms}, but it is also more general --- see \cref{ex:universal_validity_and_lem}.
\end{definition}
\begin{comments}
