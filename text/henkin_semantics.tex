...
\begin{remark}\label{rem:hol_structure_notation}
  ...
  This is especially useful for one-sorted first-order logic, where only one universe is needed --- that of individuals --- and we can say \enquote{Let \( \mscrP = (P, \leq) \) be a \hyperref[def:partially_ordered_set]{partially ordered set}}.
...
\begin{definition}\label{def:hol_term_valuation}\mimprovised
  ...
  \begin{itemize}
    \item In \eqref{eq:def:hol_term_valuation/formula/eq}, \( \delta \) is the \hyperref[def:kronecker_delta]{Kronecker delta function} (written with prefix notation).
