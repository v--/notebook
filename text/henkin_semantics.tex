...
\begin{comments}
  ...
  \item We generalize structures as defined for first-order logic in \cref{def:fol_structure}.
...
\begin{definition}\label{def:hol_expression_denotation}\mimprovised
  ...
  First, suppose that \( \tau = \syn\omicron \) and \( M = \varphi \) is a \hyperref[def:hol_expression/formula]{logical formula} typed by the rules of \cref{def:hol_typing_rules}. In this case the denotation should be an element of \( \BbbT \). We define
  \begin{empheq}[left={\Bracks{\varphi}_\mscrX^v} \coloneqq \empheqlbrace]{align}
    &T,                                                                                           &&\varphi = \synH_{\syntop},                               && (\syntop)                  \label{eq:def:hol_expression_denotation/formula/top} \\
    &F,                                                                                           &&\varphi = \synH_{\synbot},                               && (\synbot)                  \label{eq:def:hol_expression_denotation/formula/bot} \\
    &\oline{\Bracks{\psi}_\mscrX^v},                                                              &&\varphi = \synH_{\synneg} \psi,                          && (\synneg \psi)             \label{eq:def:hol_expression_denotation/formula/neg} \\
    &\Bracks{\psi}_\mscrX^v \bincirc \Bracks{\theta}_\mscrX^v,                                    &&\varphi = \synH_{\syncirc} \psi \theta,                && (\psi \syncirc \theta)     \label{eq:def:hol_expression_denotation/formula/conn} \\
    &\delta\parens[\big]{ \Bracks{N^\sigma}_\mscrX^v, \Bracks{K^\sigma}_\mscrX^v },               &&\varphi = \synH_{\syneq} N^\sigma K^\sigma,              && (M^\sigma \syneq N^\sigma) \label{eq:def:hol_expression_denotation/formula/eq} \\
    &\bigwedge\set[\big]{ \Bracks{\psi}_\mscrX^{v^{x^\sigma \mapsto a}} \given* a \in X_\sigma }, &&\varphi = \synH_{\synforall} (\qabs {x^\sigma} \psi),  && (\qforall {x^\sigma} \psi) \label{eq:def:hol_expression_denotation/formula/forall} \\
    &\bigvee\set[\big]{ \Bracks{\psi}_\mscrX^{v^{x^\sigma \mapsto a}} \given* a \in X_\sigma },   &&\varphi = \synH_{\synexists} (\qabs {x^\sigma} \psi),  && (\qexists {x^\sigma} \psi) \label{eq:def:hol_expression_denotation/formula/exists}
  \end{empheq}

  Most of the cases above are familiar from the propositional denotations defined in \fullref{alg:propositional_denotation}.
  \begin{itemize}
    \item In \eqref{eq:def:hol_expression_denotation/formula/eq}, \( \delta \) is the \hyperref[def:kronecker_delta]{Kronecker delta function} (written with prefix notation).

    \item The cases \eqref{eq:def:hol_expression_denotation/formula/forall} and \eqref{eq:def:hol_expression_denotation/formula/exists} evaluates \( \psi \) using a \hyperref[def:hol_variable_assignment/modified]{modified assignment} parameterized by the elements of \( X_\sigma \).
  \end{itemize}
  ...
  \begin{empheq}[left={\Bracks{ M^\tau }_\mscrX^v} \coloneqq \empheqlbrace]{align}
    &I(M),                                                                                          &&M \in \op*{NLConst},                                                     \label{eq:def:hol_expression_denotation/const} \\
    &v(M^\tau),                                                                                     &&M \in \op*{Var},                                                         \label{eq:def:hol_expression_denotation/var} \\
    &\parens[\big]{ a \mapsto \Bracks{ N^\rho }_\mscrX^{v_{x^\sigma \mapsto a}} },                  &&\tau = \sigma \synimplies \rho \T{and} M^\tau = \qabs {x^\sigma} N^\rho, \label{eq:def:hol_expression_denotation/abs} \\
    &\Bracks{ N^{\sigma \synimplies \tau} }_\mscrX^v \parens[\big]{ \Bracks{ K^\sigma }_\mscrX^v }, &&M^\tau = N^{\sigma \synimplies \tau} K^\sigma.                           \label{eq:def:hol_expression_denotation/app}
  \end{empheq}

  Here \eqref{eq:def:hol_expression_denotation/abs} determines a function whose value for \( a \in X_\sigma \) is the denotation of \( N^\rho \) with respect to \( v \) modified at \( x^\sigma \) with \( a \). This ensures that the denotation of an arrow type is a function, so in \eqref{eq:def:hol_expression_denotation/app} we apply the denotation of the left (function) subterm to the denotation of the right subterm.

  If the denotation \( \Bracks{ M^\tau }_\mscrX^v \) coincides for any variable assignment \( v \), we use the simplified notation \( \Bracks{ \varphi }_\mscrX \). This is the case for \hyperref[def:hol_closed_expression]{closed logical terms}, but it is also more general --- see \cref{rem:implicit_quantification_and_deduction}.
\end{definition}
\begin{comments}
...
\begin{remark}\label{rem:fol_empty_universe}
  ...
  \begin{thmenum}
    \thmitem{rem:fol_empty_universe/function_set} An immediate peculiarity is that the function universe \( X_{\tau \synimplies \syn\iota} \) is empty if and only if \( X_\tau \) is \hi{not} empty.
    ...
    \thmitem{rem:fol_empty_universe/interpretation} As a consequence, it is not possible to interpret nonlogical constants of certain types.
    ...
    Therefore, allowing empty universes is only meaningful when dealing with constants of types whose universe are guaranteed to be nonempty. Such is the case for \hyperref[def:hol_expression/predicate]{predicates}, for example.

    \thmitem{rem:fol_empty_universe/descriptions}

    \thmitem{rem:fol_empty_universe/variable_assignment} Even if all constants have a proper interpretation, variable assignments are impossible to define.

    It is easy to avoid free variables in \hyperref[def:logical_theory]{logical theories} by only considering closed formulas and using \hyperref[def:fol_quantifier_closure]{universal closures} where appropriate. It is also easy to avoid other logical terms that would otherwise inhabit an empty type.

    Thus, when computing denotations using \eqref{eq:def:hol_expression_denotation/formula/forall} or \eqref{eq:def:hol_expression_denotation/formula/exists}, we must rely on lattice meets and joins behaving trivially for empty sets.

    \thmitem{rem:fol_empty_universe/eigenvalues} Consider the \hyperref[def:hol_natural_deduction_proof_tree]{proof tree}
    \begin{equation*}
      \begin{prooftree}
    ...
    \begin{equation*}
      ...
      \reloset {\eqref{eq:def:hol_expression_denotation/formula/forall}} =
      \bigwedge\varnothing
      =
      T
      \neq
      ...
      =
      \bigvee\varnothing
      \reloset {\eqref{eq:def:hol_expression_denotation/formula/exists}} =
