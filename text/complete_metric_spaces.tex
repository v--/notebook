...
\begin{definition}\label{def:complete_metric_space}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:complete_metric_space/uniform} It is complete as a uniform space in the sense of \cref{def:complete_uniform_space}
  \end{thmenum}
\end{definition}
\begin{proof}
  The equivalence is due to \cref{thm:def:metric_topology/first_countable}, \cref{thm:first_countable_space_limit_points} and \cref{thm:def:metric_topology/hausdorff}.
\end{proof}
...
\end{theorem}
\begin{proof}
  Let \( (X, \rho) \) be a metric space. Uniqueness of the completion follows from \cref{thm:metric_space_completion_uniqueness}. We will only show existence.
  ...
  \begin{thmenum}
    ...
    Thus, the sequence \( \{ \rho(x_k, y_k) \}_{k=1}^\infty \) is fundamental and, by \cref{def:real_numbers_complete_metric_space}, it is convergent.
...
\begin{proof}
  ...
  The point \( x \) is contained in every set \( F_k, i = 1, 2, \ldots \) since all of the sets \( F_k \) are closed (by \cref{thm:complete_metric_space_compact_conditions}) and contain their limit \hyperref[thm:cluster_point_iff_in_closure]{points}. Thus,
  \begin{equation*}
