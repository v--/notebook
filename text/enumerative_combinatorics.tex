...
\begin{theorem}[Cardinality sum rule]\label{thm:cardinality_sum_rule}
  The \hyperref[thm:cardinality_existence]{cardinality} of the union of finitely many \emph{\hyperref[def:subset_ordering]{disjoint}} \hyperref[def:set]{sets} is the sum of their individual cardinalities.
\end{theorem}
\begin{comments}
...
\begin{theorem}[Cardinality exponentiation rule]\label{thm:cardinality_exponentiation_rule}
  The \hyperref[thm:cardinality_existence]{cardinality} of the set of functions \hyperref[def:set_of_all_functions]{set of all functions} \( B^A \) from \( A \) to \( B \) is the \hyperref[def:cardinal_arithmetic/exponentiation]{cardinal exponent} \( \card(B)^{\card(A)} \).
\end{theorem}
\begin{proof}
  Every function \( f: A \to B \) uniquely corresponds to a function from \( \card(A) \) to \( \card(B) \).
\end{proof}
...
\begin{comments}
  ...
  \item \Textcite*[195]{Stanley2012EnumerativeCombinatoricsVol1} uses a linear-algebraic generalization of this theorem to handle integer-valued functions other than the cardinality. Our statement makes use only of the finite additivity of \( \mu \).
\end{comments}
\begin{proof}
...
\begin{concept}\label{con:pigeonhole_principle}
  ...
  \Textcite[sec. 6.2]{Rosen2019DiscreteMathematics} dedicates entire sections to nontrivial applications of this principle.
  ...
  \begin{itemize}
    \item The \enquote{pigeonhole principle} is used, among others, in \cite[289]{Knuth1997ArtVol2}, \cite[sec. 6.2]{Rosen2019DiscreteMathematics}, \cite[66]{HopcroftEtAl2001Computability}, \cite[exerc. A-4.1]{Rotman2015AdvancedModernAlgebraPart1}, \cite[134]{Enderton1977SetTheory} and \cite[16]{Savage2008ModelsOfComputation}.

    \item The \enquote{drawer principle} is used in \cite[222]{Арнольд2014ОДУ} (as \enquote{\textru{принцип ящиков Дирихле}}), Goncharov in \cite[156]{Гончаров1952ФункцииДействительногоПеременного} (as \enquote{\textru{принцип Дирихле}}), \cite[224]{Проданов1982ФункционаленАнализЧаст1} (as \enquote{\textbg{принцип за чекмеджетата}}).

    \item \enquote{Dirichlet's principle} is used by \cite[64]{Гуров2013ТеорияРешёток} (as \enquote{\textru{принцип Дирихле}}), \cite[9]{Мирчев2001Графи} (as \enquote{\textbg{принцип на Дирихле}}).
  \end{itemize}
\end{concept}
...
\begin{theorem}[Dirichlet's pigeonhole principle]\label{thm:pigeonhole_principle}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:pigeonhole_principle/finitary} Fix nonempty \emph{finite} sets \( A \) with \( n \) elements and \( B \) with \( m \) elements. For every function \( f: A \to B \) between them, there exists some element \( b \) in \( B \) such that \( f(a) = b \) for at least \( n / m \) elements \( a \) of \( A \).
...
\begin{example}\label{ex:thm:pigeonhole_principle}
  ...
  \begin{thmenum}
    \thmitem{ex:thm:pigeonhole_principle/lagrange} Given a \hyperref[def:group/morphism]{group homomorphism} \( \varphi: G \to H \), by \fullref{thm:homomorphism_theorem_for_groups},
    \begin{equation*}
    ...
    \thmitem{ex:thm:pigeonhole_principle/boolean} Again via \cref{thm:pigeonhole_principle/finitary} we conclude that, for every \hyperref[def:boolean_function]{Boolean-valued function} \( f: A \to \set{ T, F } \) on a finite set \( A \) with \( n \) elements, either \( f \) holds for at least \( n / 2 \) members of \( A \) or \( f \) doesn't hold for (at least) \( n / 2 \) members.
    ...
    Compare this example to \cref{ex:ramsey_party_problem}, where we are interested how many people know \emph{each other}. This leads to the considerably harder problem of estimating \hyperref[def:ramsey_number]{Ramsey numbers}.
  \end{thmenum}
\end{example}
...
\begin{proposition}\label{thm:alternating_group_cardinality}
  For \( n > 1 \), the \hyperref[def:alternating_group]{alternating group} \( A_n \) has \( n! / 2 \) elements.
\end{proposition}
\begin{proof}
  \Cref{thm:permutation_parity_homomorphism} implies \( \sgn: S_n \to C_2 \) is a homomorphism. Its kernel is \( A_n \).

  \Cref{thm:homomorphism_theorem_for_groups} implies that the quotient \( S_n / A_n \) is isomorphic to \( C_2 \).
...
\begin{remark}\label{rem:combinatorial_permutation_terminology}
  ...
  When comparing his terminology to that of others, \textcite[29]{Aigner1997CombinatorialTheory} states that injective functions from a set of \( r \) elements to a set of \( n \) elements are also called \enquote{\( n \)-permutations of \( r \) objects}. He then remarks that, in older literature, \enquote{\( n \)-permutations} are called \enquote{\( n \)-variations}. This terminology is still used in Bulgarian, for example in \cite[27]{ДимитровЯнев2007ВероятностиИСтатистика}.

  In Russophone literature, \enquote{\textru{размещение}} (\enquote{arrangement}) is used instead. This usage can be found in \cite[171]{Яблонский2003ДискретнаяМатематика} and \textcite[100]{БелоусовТкачёв2004ДискретнаяМатематика}.
  ...
  In the context of combinatorics, the term \enquote{permutation} does indeed often refer to what we call \enquote{variation}. \Textcite[1]{Hall1986CombinatorialTheory} provides the following definition:
  \begin{displayquote}
    A permutation is an ordered selection of objects from a set \( S \).
  \end{displayquote}

  \textcite[429]{Rosen2019DiscreteMathematics} provides two related definition:
  \begin{displayquote}
  ...
  \begin{itemize}
    \item For elements of the finite symmetric group \( S_n \), it is used in
    \cite[31]{Jacobson1985BasicAlgebraI},
    \cite[78]{Винберг2014КурсАлгебры} (as \enquote{\textru{перестановка}}) and
    \cite[82]{Тыртышников2017ОсновыАлгебры} (as \enquote{\textru{подстановка}})

    \item In the same sense, but without explicit discussion of symmetric groups, it is used in
    \cite[28]{Knuth1997ArtVol1},
    \cite[20]{Stanley2012EnumerativeCombinatoricsVol1} and
    \cite[def. 2.11.1]{Savage2008ModelsOfComputation}.

    \item For elements of a general symmetric group \( S_A \), it is used in
    \cite[8]{Lang2002Algebra},
    \cite[116]{Rotman2015AdvancedModernAlgebraPart1},
    \cite[def. II.2.1]{Aluffi2009Algebra} and
    \cite[\S 2.2.5]{Новиков2013ДискретнаяМатематика} (as \enquote{\textru{перестановка}}).
  \end{itemize}
\end{remark}
...
\begin{proposition}\label{thm:combinatorial_variation_count}
  ...
  \begin{thmenum}
    \thmitem{thm:combinatorial_variation_count/no_repetition} The number of variations of \( A \) of size \( k \) \emph{without repetition} is \( 0 \) if \( k > n \) and
    \begin{equation}\label{eq:thm:combinatorial_variation_count/no_repetition}
      \frac {n!} {(n - k)!} = \prod_{m=0}^{k - 1} (n - m).
    \end{equation}
    otherwise.

    \thmitem{thm:combinatorial_variation_count/repetition} The number of \hyperref[def:combinatorial_variation]{variations} of \( A \) of size \( k \) \emph{with repetition} is \( n^k \).
  \end{thmenum}
\end{proposition}
\begin{comments}
  \item \Textcite[74]{Aigner1997CombinatorialTheory} calls \eqref{eq:thm:combinatorial_variation_count/no_repetition} a \enquote{falling factorial}.
\end{comments}
\begin{proof}
...
\begin{proposition}\label{thm:combinatorial_combination_count}
  ...
  \begin{thmenum}
    \thmitem{thm:combinatorial_combination_count/no_repetition} The number of combinations of \( A \) of size \( k \) \emph{without repetition} is given by the \hyperref[def:binomial_coefficient]{binomial coefficient} \( \binom n k \).

    \thmitem{thm:combinatorial_combination_count/repetition} The number of \hyperref[def:combinatorial_combination]{combinations} of \( A \) of size \( k \) \emph{with repetition} is given by the \hyperref[def:binomial_coefficient]{negative binomial coefficient}
...
\end{definition}
\begin{comments}
  \item Our definition is a formalization of \textcite[\S 5.1.2]{Knuth1998ArtVol3}. \Textcite[28]{ДимитровЯнев2007ВероятностиИСтатистика} call them \enquote{пермутации с повторение} (\enquote{permutations with repetition}), however the terminology doesn't seem to be established and may not fit well with \hyperref[def:combinatorial_variation]{variations} and \hyperref[def:combinatorial_combination]{combinations} with repetition, especially considering the ambiguity of the term \enquote{permutation} discussed in \cref{rem:combinatorial_permutation_terminology}.
\end{comments}

\begin{proposition}\label{thm:multiset_permutation_count}
  Fix a nonempty \hyperref[def:multiset]{multiset} \( M \) with finite \hyperref[def:multiset_cardinality]{multiset cardinality} \( n \). Let \( a_1, \ldots, a_m \) be the distinct elements of \( M \), and denote by \( k_1, \ldots, k_m \) their cardinalities in \( M \).
