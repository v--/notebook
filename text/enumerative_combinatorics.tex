...
\begin{concept}\label{con:pigeonhole_principle}
  ...
  \begin{itemize}
    ...
    \item The \enquote{drawer principle} is used by \incite[222]{Арнольд2014ОДУ} (as \enquote{\textru{принцип ящиков Дирихле}}), Goncharov in \cite[156]{Гончаров1952ФункцииДействительногоПеременного} (as \enquote{\textru{принцип Дирихле}}), \incite[224]{Проданов1982ФункционаленАнализЧаст1} (as \enquote{\textbg{принцип за чекмеджетата}}).
    \item \enquote{Dirichlet's principle} is used by \incite[64]{Гуров2013ТеорияРешёток} (as \enquote{\textru{принцип Дирихле}}), \incite[9]{Мирчев2001Графи} (as \enquote{\textbg{принцип на Дирихле}}).
  \end{itemize}
\end{concept}
...
\begin{example}\label{ex:thm:pigeonhole_principle}
  ...
  \begin{thmenum}
    \thmitem{ex:thm:pigeonhole_principle/lagrange} Given a \hyperref[def:group/morphism]{group homomorphism} \( \varphi: G \to H \), by \cref{thm:fundamental_theorem_of_homomorphisms},
    \begin{equation*}
...
\begin{proof}
  ...
  \Cref{thm:fundamental_theorem_of_homomorphisms} implies that the quotient \( S_n / A_n \) is isomorphic to \( C_2 \).
...
\begin{remark}\label{rem:combinatorial_permutation_terminology}
  ...
  \begin{itemize}
    ...
    \incite[78]{Винберг2014КурсАлгебры} (as \enquote{\textru{перестановка}}) and
    \incite[82]{Тыртышников2017ОсновыАлгебры} (as \enquote{\textru{подстановка}})
    ...
    \incite[\S 2.2.5]{Новиков2013ДискретнаяМатематика} (as \enquote{\textru{перестановка}}).
  \end{itemize}
\end{remark}
