...
\begin{definition}\label{def:propositional_schema_substitution}\mimprovised
  A \term{simultaneous schema substitution} is a \hyperref[def:set_valued_map/partial]{partial map} from \hyperref[def:propositional_formula_schema]{propositional formula placeholders} to \hyperref[def:propositional_formula]{propositional formulas} having a finite domain.

  We extend this to an operation on arbitrary formula schemas via \hyperref[rem:straightforward_traversal]{straightforward traversal} similarly to how it is done in \fullref{def:propositional_substitution} for propositional substitution. Of course, we use the notation from \fullref{rem:uniform_substitution_notation}.

  Given a substitution \( \sigma \) and a schema \( \Phi \), we call \( \Phi[\sigma] \) an \term{instance} of \( \Phi \).
\end{definition}
\begin{comments}
  \item This definition was adapted from \fullref{def:propositional_substitution}, which is in turn based on \cite[def. 3.1.3]{CitkinMuravitsky2021}.
...
\begin{comments}
  ...
  \begin{itemize}
    ...
    \item We avoid adding any rules except for \eqref{inf:thm:axiomatic_derivation_as_natural_deduction/mp}.
  \end{itemize}
