...
\begin{definition}\label{def:propositional_schema_instantiation}\mimprovised
  A \term{schema instantiation} is a \hyperref[def:set_valued_map/partial]{partial map} from \hyperref[def:propositional_formula_schema]{propositional formula placeholders} to \hyperref[def:propositional_syntax/formula]{propositional formulas}. For simplicity, we will assume that it has a finite domain.
