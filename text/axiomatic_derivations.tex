...
\begin{definition}\label{def:propositional_schema_substitution}\mimprovised
  A \term{simultaneous schema substitution} is a map from \hyperref[def:propositional_formula_schema]{propositional formula placeholders} to \hyperref[def:propositional_formula]{propositional formulas} such that only finitely many placeholders are not fixed.

  We extend this to arbitrary formula schemas via \hyperref[rem:straightforward_traversal]{straightforward traversal} similarly to how it is done in \fullref{def:propositional_substitution} for propositional substitution. Of course, we use the notation from \fullref{rem:uniform_substitution_notation}.

  Given a substitution \( \sigma \) and a schema \( \Phi \), we call \( \sigma(\Phi) \) an \term{instance} of \( \Phi \).
\end{definition}
\begin{comments}
...
\begin{definition}\label{def:axiomatic_derivation_system}\mimprovised
  ...
  For any axiom schema \( \Phi \) and any \hyperref[def:propositional_schema_substitution]{substitution} \( \sigma \), we call the formula \( \Phi[\sigma] \) a \term{logical axiom}.
\end{definition}
\begin{comments}
