...
\begin{definition}\label{def:propositional_schema_substitution}\mimprovised
  A \term{simultaneous schema substitution} is a \hyperref[def:set_valued_map/partial]{partial map} from \hyperref[def:propositional_formula_schema]{propositional formula placeholders} to \hyperref[def:propositional_syntax/formula]{propositional formulas} having a finite domain.

  We extend this to an operation on arbitrary formula schemas via \hyperref[rem:straightforward_traversal]{straightforward traversal} similarly to how it is done in \fullref{def:propositional_substitution} for propositional substitution. Of course, we use the notation from \fullref{rem:substitution_notation}.

  Given a substitution \( \sigma \) and a schema \( \Phi \), we call \( \Phi[\sigma] \) an \term{instance} of \( \Phi \).
\end{definition}
\begin{comments}
  \item This definition was adapted from \fullref{def:propositional_substitution}, which is in turn based on \cite[def. 3.1.3]{CitkinMuravitsky2021ConsequenceRelations}.
...
\begin{definition}\label{def:axiomatic_derivation_system}\mimprovised
  ...
  For any axiom schema \( \Phi \) and any \hyperref[def:propositional_schema_substitution]{substitution} \( \sigma \) defined for the placeholders of \( \Phi \), we call the formula \( \Phi[\sigma] \) a \term{logical axiom}.
\end{definition}
\begin{comments}
...
\begin{definition}\label{def:axiomatic_derivation}\mcite[51]{TroelstraSchwichtenberg2000Proofs}
  ...
  \begin{thmenum}
    \thmitem{def:axiomatic_derivation/axiom} \( \syn\psi_k \) is a logical axiom.
    \thmitem{def:axiomatic_derivation/premise} \( \syn\psi_k \) is a premise, that is, it belongs to \( \Gamma \).
    \thmitem{def:axiomatic_derivation/mp} There exist indices \( i \) and \( j \), both smaller than \( k \), such that \( \syn\psi_j = (\syn\psi_i \synimplies \syn\psi_k) \).
  \end{thmenum}
\end{definition}
\begin{comments}
  \item \incite[51]{TroelstraSchwichtenberg2000Proofs} use the term \enquote{axiomatic deduction}. We prefer \enquote{derivation} to \enquote{deduction} based on an analogy with \hyperref[def:formal_grammar/derivation]{derivations} on \hyperref[def:formal_grammar]{formal grammars}. \incite[117]{CitkinMuravitsky2021ConsequenceRelations} also prefer the term \enquote{derivation}.
