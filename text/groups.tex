...
\begin{definition}\label{def:group}\mcite[29]{Ляпин1960Полугруппы}
  A \term[bg=група (\cite[def. IV.2]{ГеновМиховскиМоллов1991Алгебра}), ru=группа (\cite[def. 4.1.2]{Винберг2014Алгебра})]{group} is a \hyperref[def:monoid]{monoid} in which every element has a \hyperref[def:monoid_inverse]{two-sided inverse}. They are the most well-studied and most well-behaved semigroups.
  ...
  \begin{thmenum}
    ...
    \thmitem{def:group/homomorphism} A \hyperref[def:first_order_homomorphism]{first-order homomorphism} between groups is simply a \hyperref[def:semigroup/homomorphism]{semigroup homomorphism}.
    ...
    \thmitem{def:group/opposite} The \hyperref[def:monoid/opposite]{opposite monoid} \( G^\oppos \) of a group \( G \) is again a group, which we will call the \term{opposite group}.
...
\begin{proposition}\label{thm:ordered_group}
  \hyperref[def:ordered_semiring]{Ordered} \hyperref[def:groups]{groups} have the following properties:
  \begin{thmenum}
