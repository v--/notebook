...
\begin{proof}
  ...
  \SubProofOf{thm:subgroup_cosets/inside} \Fullref{thm:subgroup_cosets/translation_congruence} implies that \( e H = a H \) if and only if \( e \sim a \), i.e. \( e^{-1} a \in H \).
...
\end{definition}
\begin{comments}
  \item The adjectives \enquote{left} and \enquote{right} reflect the fact that \( x \mapsto g x g^{-1} \) is a \hyperref[def:group_action]{left group action}, while \( x \mapsto g^{-1} x g \) is a \hyperref[def:group_action]{right group action} --- see \fullref{thm:group_conjugation_action}.
\end{comments}
...
\begin{proof}
  ...
  \begin{equation*}
    z = (hg) x (hg)^{-1},
  \end{equation*}
  where \( (hg)^{-1} = g^{-1} h^{-1} \) due to \fullref{thm:def:group/inverse_composition}.
\end{proof}
...
\begin{example}\label{ex:image_of_normal_subgroup}
  ...
  Consider the \hyperref[def:symmetric_group]{symmetric group} \( S_3 \) discussed in \fullref{ex:s3} and the \hyperref[def:cyclic_group]{cyclic group} \( C_2 \).
  ...
  Based on the \hyperref[def:cayley_table]{multiplication table} in \fullref{ex:s3}, we can conclude that \( \cycle{ 1, 2 } \bincirc \cycle{ 1, 2 } = \id \). Hence, \( \varphi \) is indeed a homomorphism.
...
\begin{comments}
  ...
  \item The term \enquote{involution} is also used more generally for self-inverse morphisms (including earlier in this section); see \fullref{def:morphism_invertibility/involution}.
\end{comments}
\begin{defproof}
