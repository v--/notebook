...
\begin{definition}\label{def:group}\mcite[29]{Ляпин1960}
  ...
  \begin{thmenum}
    ...
    \begin{equation}\label{eq:def:group/theory/inverse_axiom}
      \qforall \xi (\xi \cdot \xi^{-1} = e \synwedge \xi^{-1} \cdot \xi = e).
    \end{equation}
    ...
    \thmitem{def:group/opposite} The \hyperref[def:monoid/opposite]{opposite monoid} \( G^{\opcat} \) of a group \( G \) is again a group, which we will call the \term{opposite group}.
...
\begin{definition}\label{def:normal_closure}\mcite[78]{Скорняков1990}
  The \term[ru=нормальное замыкание]{normal closure} of a subset \( A \) of a group \( G \) is the intersection of all \hyperref[def:normal_subgroup]{normal subgroups} of \( G \) containing \( A \).
