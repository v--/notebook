...
\begin{definition}\label{def:group}\mcite[29]{Ляпин1960Полугруппы}
  ...
  \begin{thmenum}
    \thmitem{def:group/theory}\mimprovised To obtain a \hyperref[def:first_order_theory]{first-order theory} for groups, we can extend the \hyperref[def:monoid/theory]{theory of monoids} by adding a unary \hyperref[def:fol_signature/functions]{functional symbol} \( (\anon)^{-1} \) to the language and then adding the axiom
    ...
    \thmitem{def:group/category}\mcite[12]{MacLane1998CategoryTheory} We denote the \hyperref[def:category_of_small_first_order_models]{category of \( \mscrU \)-small models} of groups by \( \ucat{Grp} \).
...
\end{definition}
\begin{defproof}
  The fact that the normal closure is a closure operator follows from \cref{thm:intersection_of_normal_subgroups} via \cref{thm:closure_operator_from_set_semilattice}.
\end{defproof}
