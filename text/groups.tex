...
\begin{definition}\label{def:group}\mcite[29]{Ляпин1960Полугруппы}
  ...
  \begin{thmenum}
    ...
    \begin{itemize}
      \item It is \hyperref[def:concrete_category]{concrete} over \hyperref[def:monoid/category]{\( \ucat{Mon} \)} and hence over \hyperref[def:pointed_set/category]{\( \ucat{Set_*} \)}. The latter allows us to use \fullref{thm:image_coimage_factorization} and \cref{thm:zero_morphisms_pointed}, as we will soon do --- see \cref{thm:group_zero_morphisms}.
    ...
    \thmitem{def:group/kernel}\mimprovised \Cref{thm:group_zero_morphisms/kernel} implies that the \hyperref[def:zero_morphisms/kernel]{categorical kernel} of a group homomorphism \( \varphi: G \to H \) is the set
    \begin{equation*}
...
\end{comments}
\begin{proof}
  \SubProofOf{thm:group_zero_morphisms/kernel} Follows from \cref{thm:zero_morphisms_pointed/kernel}.
  \SubProofOf{thm:group_zero_morphisms/cokernel} \Cref{thm:zero_morphisms_pointed/cokernel} implies that the categorical cokernel of \( \varphi \) is the quotient of \( H \) by the \hyperref[def:fol_congruence]{congruence} \hyperref[def:fol_generated_congruence]{generated} by the relation \( {\sim} \) defined as
  \begin{equation*}
  ...
  \SubProofOf{thm:group_zero_morphisms/image} Follows from \cref{thm:zero_morphisms_pointed/image}.
