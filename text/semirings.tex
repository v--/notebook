...
\begin{remark}\label{rem:semiring_etymology}
  ...
  \begin{itemize}
    \item \incite*[ch. 3]{GondranMinoux1984Graphs} suggest using \enquote{dioid} (short for \enquote{double monoid}) instead of \enquote{semiring}. \incite[xi]{Golan1999Semirings} describes how the term \enquote{dioid} may refer to semirings with idempotent addition, i.e. a general form of the tropical semirings defined in \fullref{def:tropical_semiring}.

    \item \incite[exerc. II.2.24]{Savage2008Computability} suggests using \enquote{near-ring}.
  \end{itemize}
...
\begin{example}\label{ex:def:semiring}
  ...
  \begin{thmenum}
    ...
    \thmitem{ex:def:semiring/boolean} As a consequence of \fullref{thm:symmetric_algebra_as_ring}, \hyperref[def:boolean_algebra]{Boolean algebras} are (semi)rings with \hyperref[def:symmetric_difference]{symmetric difference} acting as addition and meets acting as multiplication.
...
\end{definition}
\begin{comments}
  \item According to \incite{Pin1998TropicalSemirings}, the name \enquote{tropical semiring} is a dedication to the Brazilian Imre Simon. The paper also introduces the terms \enquote{tropical integers}, \enquote{tropical reals}, etc. \incite[3]{Golan1999Semirings} refers to the more general notion of additively-idempotent semirings. Both reserve the term \enquote{tropical semiring} for the \( \min \)-plus semiring. \incite[ch. 3]{GondranMinoux1984Graphs} does not explicitly use the word \enquote{tropical}, but instead refers to semirings as \enquote{dioids}, and the latter term sometimes refers to additively-idempotent semirings.
...
\begin{definition}\label{def:divisibility}\mcite[11]{Golan1999Semirings}
  ...
  \begin{thmenum}
    \thmitem{def:divisibility/zero}\mcite[4]{Golan1999Semirings} Divisors of \( 0 \) are called \term[bg=делител на нулата (\cite[def. V.2]{ГеновМиховскиМоллов1991Алгебра}), ru=делитель нуля (\cite[20]{Винберг2014Алгебра})]{zero divisors}.
    ...
    \thmitem{def:divisibility/invertible}\mcite[84]{Lang2002Algebra} Divisors of \( 1 \) are called \term[bg=обратим (\cite[def. V.3]{ГеновМиховскиМоллов1991Алгебра}), ru=обратимый (\cite[20]{Винберг2014Алгебра})]{invertible}, since they are precisely the \hyperref[def:monoid_inverse]{monoid inverses} under multiplication.
  \end{thmenum}
\end{definition}
\begin{comments}
...
\begin{definition}\label{def:signum}\mcite[def. A.VI.2]{Bourbaki2003Algebra4to7}
  In a \hyperref[def:totally_ordered_set]{totally} \hyperref[def:ordered_semiring]{ordered} \hyperref[def:semiring]{semiring} \( R \), we can define the \term[ru=знак (\cite[208]{АлександровМаркушевичХинчин1952ЭнциклопедияТом3}) / сигнум (\cite[100]{Зорич2019АнализЧасть1}), en=sign (function) (\cite[\S 2.2(c)]{Schechter1997AnalysisHandbook})]{signum} function
  \begin{equation*}
    \begin{aligned}
      &\sgn: R \to \BbbZ \\
...
\begin{comments}
  ...
  \incite[208]{АлександровМаркушевичХинчин1952ЭнциклопедияТом3}
...
\end{definition}
\begin{comments}
  \item Although the Archimedean property is commonly defined only for fields, for example by \incite[192]{Mendelson2008NumberSystems} and \incite[450]{Lang2002Algebra}, \incite[A.VI.35]{Bourbaki2003Algebra4to7} generalize it to groups. For our purposes it will be appropriate to generalize Mendelson's definition to semirings.
\end{comments}
