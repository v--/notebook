...
\begin{definition}\label{def:semiring}\mcite[1]{Golan2010}
  ...
  \begin{thmenum}
    ...
    \begin{thmenum}
      ...
      \thmitem{def:semiring/identity} We also restate the identity axiom \eqref{eq:def:monoid/theory/neutral} for the multiplicative unit \( 1 \) to highlight its connection with \eqref{eq:def:semiring/absorption}:
    ...
    \thmitem{def:semiring/power_set} Similarly to power set semigroups defined in \fullref{def:power_semigroup}, the power set \( \pow(R) \) of a semiring is also a semiring with the operations
    \begin{align*}
...
\begin{example}\label{ex:def:semiring}
  ...
  \begin{thmenum}
    \thmitem{ex:def:semiring/trivial} A semiring has one unique element if and only if \( 0_R = 1_R \). This follows from \eqref{eq:def:semiring/absorption} and \eqref{eq:def:semiring/identity}.

    As a consequence, if \( \varphi: \set{ 0 } \to R \) is a \hyperref[def:semiring/homomorphism]{semiring} homomorphism, \( R \) is has one element. This is further strengthened by \fullref{thm:ring_embedding_preserves_characteristic}.
...
\begin{definition}\label{def:ordered_semiring}\mcite[224]{Golan2010}
  A (partially) \term{ordered semiring} is a semiring \( R \) with a \hyperref[def:partially_ordered_set]{partial order} \( \leq \) such that \( (R, +) \) is an \hyperref[def:ordered_semigroup]{ordered semigroup} and, additionally, \( x \leq y \) and \( 0 \leq z \) together imply \( xz \leq yz \).
...
\end{proposition}
\begin{comments}
  \item Divisibility is not a partial order in general. To avoid the nonuniqueness problems described in \fullref{ex:preorder_nonuniqueness}, we instead prefer working with ideals. See \fullref{rem:lattice_of_principal_ideals} and \fullref{rem:lattice_of_principal_ideals} for the general approach.
\end{comments}
\begin{proof}
  Fix a semiring \( R \).
