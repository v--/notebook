...
\begin{example}\label{ex:def:semiring}
  ...
  \begin{thmenum}
    ...
    Since the lattice is bounded, \( \bot \) is absorbing with respect to \( \wedge \) and \( \top \) with respect to \( \vee \). Therefore, if the lattice is \hyperref[def:distributive_lattice]{distributive}, as a consequence of \fullref{thm:def:lattice/bounded_absorption}, both \( (X, \vee, \wedge) \) and \( (X, \wedge, \vee) \) are semirings.
...
\begin{definition}\label{def:divisibility}\mcite[11]{Golan1999Semirings}
  ...
  \begin{thmenum}
    \thmitem{def:divisibility/zero}\mcite[4]{Golan1999Semirings} Divisors of \( 0 \) are called \term[bg=делител на нулата (\cite[def. V.2]{ГеновМиховскиМоллов1991Алгебра}), ru=делитель нуля (\cite[20]{Винберг2014Алгебра})]{zero divisors}. Due to \hyperref[def:semiring/absorption]{absorption}, every semiring element is a zero divisor. If \( lr = 0 \) for nonzero \( l \) and \( r \), we say that \( l \) (resp. \( r \)) is a \term{nontrivial} left (resp. right) zero divisor.
