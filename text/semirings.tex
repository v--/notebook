...
\begin{definition}\label{def:semiring}\mcite[1]{Golan2010}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:semiring/category} We denote the corresponding \hyperref[def:category_of_small_first_order_models]{category of \( \mscrU \)-small models} by \( \ucat{SRing} \).
...
\begin{example}\label{ex:def:semiring}
  ...
  \begin{thmenum}
    ...
    As a consequence, if \( \varphi: \set{ 0 } \to R \) is a \hyperref[def:semiring/homomorphism]{semiring} homomorphism, \( R \) has one element. This is further strengthened by \fullref{thm:ring_embedding_preserves_characteristic}.
...
\begin{proposition}\label{thm:category_of_semirings_properties}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:category_of_semirings_properties/terminal} The one-element semiring \( \set{ 0 } \) is a \hyperref[def:universal_objects/terminal]{terminal object}.
  \end{thmenum}
\end{proposition}
\begin{proof}
...
\end{proposition}
\begin{comments}
  \item Divisibility is not a partial order in general. To avoid the nonuniqueness problems described in \fullref{ex:preorder_nonuniqueness}, we instead prefer working with ideals. See \fullref{rem:lattice_of_principal_ideals} for the general approach.
\end{comments}
\begin{proof}
  Fix a semiring \( R \).
...
\begin{definition}\label{def:zerosumfree}\mcite[4]{Golan2010}
  We say that an \hyperref[rem:additive_semigroup]{additive} \hyperref[def:monoid]{monoid} is \term[ru=антикольцо (\cite[7]{Мальцев1970})]{zerosumfree} if the \hyperref[thm:invertible_submonoid_is_group]{additive group} is trivial. That is, if \( x + y = 0 \) implies \( x = y = 0 \).
\end{definition}
