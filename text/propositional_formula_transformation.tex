...
For propositional logic, We will also find useful the ability to replace an entire subformula with another formula. We will use this, for example, to justify the substitutions in \fullref{alg:propositional_formula_to_cnf_and_dnf}.
...
\end{algorithm}
\begin{comments}
  \item A variation suitable for \fullref{alg:propositional_formula_to_cnf_and_dnf} is to replace \( \syntop \) with \( p_\varphi \synvee \synneg p_\varphi \). This has the downside of only being sound with respect to classical semantics.
...
\begin{algorithm}[Propositional formula to CNF or DNF]\label{alg:propositional_formula_to_cnf_or_dnf}
  ...
  \begin{thmenum}
    \thmitem{alg:propositional_formula_to_cnf_and_dnf/atomic} If \( \varphi \) is atomic, it is either a constant or a positive literal, and thus it is already in both conjunctive and disjunctive normal form.

    In this case, let \( T_{\anon}(\varphi) \coloneqq \varphi \) for both \( {\anon} = \wedge \) and \( {\anon} = \vee \).

    \thmitem{alg:propositional_formula_to_cnf_and_dnf/negation} If \( \varphi = \synneg \psi \), let
    \begin{align*}
    ...
    \thmitem{alg:propositional_formula_to_cnf_and_dnf/conditional} If \( \varphi = \psi \synimplies \theta \), let
    \begin{equation*}
      T_{\anon}(\varphi) \coloneqq T_{\anon}(\synneg \psi \synvee \theta).
    \end{equation*}

    \thmitem{alg:propositional_formula_to_cnf_and_dnf/biconditional} If \( \varphi = \psi \syniff \theta \), let
    \begin{align*}
    ...
    \thmitem{alg:propositional_formula_to_cnf_and_dnf/outer} If \( \varphi = \psi \syncirc \theta \), let
    \begin{equation*}
      T_{\syncirc}(\varphi) \coloneqq T_{\syncirc}(\psi) \syncirc T_{\syncirc}(\theta).
    \end{equation*}

    \thmitem{alg:propositional_formula_to_cnf_and_dnf/inner} If \( \varphi = \psi \syncirc^{\oppos} \theta \), let \( \psi' \coloneqq T_{\syncirc}(\psi) \) and \( \theta' \coloneqq T_{\syncirc}(\theta) \) and let
    \begin{equation*}
