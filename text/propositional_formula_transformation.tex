...
\begin{algorithm}[Propositional substitution]\label{alg:propositional_substitution}\mimprovised
  Given an \hyperref[def:atomic_propositional_substitution]{atomic substitution map} \( \Bbbs \), we define via \hyperref[rem:straightforward_traversal]{straightforward traversal} the following operation on any formula \( \varphi \) for whose variables \( \Bbbs \) is defined:
  \begin{equation*}
...
\begin{comments}
  ...
  \item We focus on simultaneous substitutions directly instead of defining substituting a single variable. The latter is often done similarly to \fullref{alg:propositional_subformula_substitution}. See \cref{rem:simulating_simultaneous_substitution} for an alternative approach.

  \item This algorithm is a special case of the analogous first-order logic algorithm \fullref{alg:fol_formula_substitution}, implemented in \cite{notebook:code} as
...
\begin{remark}\label{rem:substitution_symbol}
  Using \( \Bbbs \) and adjacent letters to denote an instantiation function is inspired by Hindley's \enquote{type-substitutions} defined in \cite[def. 3A1]{Hindley1997BasicSTT}. An alternative used by \textcite[\S 5.4]{Mimram2020ProgramEqualsProof} and \textcite[def. 3.1.3]{CitkinMuravitsky2022ConsequenceRelations} is \( \sigma \).

  We find Hindley's notation useful because:
  \begin{itemize}
    \item Plain Latin letters adjacent to \( p \), like \( s \), are used for both propositional variables and \hyperref[def:fol_signature/predicate]{first-order predicates}.
    \item Greek letters adjacent to \( \tau \), like \( \sigma \), are used for both \hyperref[def:fol_term]{first-order terms} and for \hyperref[rem:type_theory]{types}.
  \end{itemize}
\end{remark}
...
\begin{remark}\label{rem:substitution_notation}
  ...
  A similar variation, used by \textcite[def. 2.1.15]{Barendregt1984LambdaCalculus} in the context of \hyperref[def:atomic_lambda_term_substitution]{\( \muplambda \)-term substitution}, is
  \begin{equation*}
  ...
  It is used for logical formulas by \textcite[19]{VanDalen2004LogicAndStructure} and \textcite[\S 3.1.4]{Mimram2020ProgramEqualsProof} and for \( \muplambda \)-terms by \textcite[def. 1A7]{Hindley1997BasicSTT}.

  Moreover, some authors like \textcite[4]{TroelstraSchwichtenberg2000BasicProofTheory} exchange the variables and formulas in this notation.

  Other authors use more esoteric notations --- \textcite[47]{КолмогоровДрагалин2006Логика} use
  \begin{equation*}
    \varphi(p_1, \ldots, p_n \mathrel{\Vert} \omega_1, \cdots, \omega_n),
  \end{equation*}
  while \textcite[131]{Эдельман1975Логика} uses
  \begin{equation*}
    S_{b_{p_1, \ldots, p_n}}(\varphi; \omega_1, \ldots, \omega_n).
  \end{equation*}

  \textcite[16]{Andrews2002Logic} instead uses the very concise notation
  \begin{equation*}
    \varphi^{p_1, \ldots, p_n}_{\omega_1, \ldots, \omega_n},
  \end{equation*}
  which \textcite[def. 2.2.19]{Hinman2005Logic} writes only for a single variable as
  \begin{equation*}
    \varphi_p(\omega).
  \end{equation*}
\end{remark}
...
\begin{definition}\label{def:substitution_schema}\mimprovised
  ...
  \begin{bnf*}
    \bnfprod{replacement}      {\bnfpn{variable} \bnfsp \bnftsq{\( \synsubst \)} \bnfsp \bnfpn{substitution target}}, \\
    \bnfprod{replacement list} {\bnfpn{replacement} \bnfor \bnfpn{replacement} \bnfsp \bnftsq{,} \bnfsp \bnfpn{replacement list}}, \\
    \bnfprod{substitution}     {\bnfpn{substitution target} \bnfsp \bnftsq{\( [ \)} \bnfsp \bnfpn{replacement list} \bnfsp \bnftsq{\( ] \)}}.
  \end{bnf*}

  Here the rules \( \bnfpn{variable} \) and \( \bnfpn{substitution target} \) must be provided externally. The targets can be \hyperref[def:propositional_formula]{propositional formulas}, \hyperref[def:lambda_term]{\( \muplambda \)-terms}, or any other syntactic construct where substitution makes sense.
\end{definition}
\begin{comments}
...
For propositional logic, We will also find useful the ability to replace an entire subformula with another formula. We will use this, for example, to justify the substitutions in \fullref{alg:propositional_formula_to_cnf_or_dnf}.
...
\begin{comments}
  ...
  \begin{CodeRefDisplay}
    \GetCodeRef{alg:propositional_subformula_substitution}
  \end{CodeRefDisplay}
\end{comments}
...
\begin{algorithm}[Propositional constant collapse]\label{alg:propositional_constant_collapse}
  We introduce an operator \( M_{\logic{cc}} \) on \hyperref[def:propositional_formula]{propositional formulas} that attempts to simplify subformulas involving logical constants. The result \( M_{\logic{cc}}(\varphi) \) will be \hyperref[def:propositional_semantics/equivalence]{semantically equivalent} with \( \varphi \).

  \begin{thmenum}
    \thmitem{alg:propositional_constant_collapse/atomic} If \( \varphi \) is atomic, i.e. a constant or variable, there is nothing we can do, so let \( M_{\logic{cc}} \coloneqq \varphi \).

    \thmitem{alg:propositional_constant_collapse/negation} If \( \varphi = \synneg \psi \), let \( \psi' \coloneqq M_{\logic{cc}}(\psi) \) and proceed as follows:
    \begin{equation*}
      M_{\logic{cc}}(\synneg \psi) \coloneqq \begin{cases}
        \synbot,       &\psi' = \syntop \\
        \syntop,       &\psi' = \synbot \\
    ...
    \thmitem{alg:propositional_constant_collapse/conjunction} If \( \varphi = \psi \synwedge \theta \), let \( \psi' \coloneqq M_{\logic{cc}}(\psi) \) and \( \theta' \coloneqq M_{\logic{cc}}(\theta) \) and proceed as follows:
    \begin{equation*}
      M_{\logic{cc}}(\psi \synwedge \theta) \coloneqq \begin{cases}
    ...
    \begin{equation*}
      M_{\logic{cc}}(\psi \synvee \theta) \coloneqq \begin{cases}
    ...
    \begin{equation*}
      M_{\logic{cc}}(\psi \synimplies \theta) \coloneqq \begin{cases}
    ...
    \begin{equation*}
      M_{\logic{cc}}(\psi \syniff \theta) \coloneqq \begin{cases}
...
\begin{comments}
  \item This algorithm is implemented in \cite{notebook:code} as
  \begin{CodeRefDisplay}
    \GetCodeRef{alg:propositional_constant_collapse}
  \end{CodeRefDisplay}
\end{comments}
\begin{defproof}
...
\end{remark}
\begin{comments}
  \item In computer programming, this exact problem is solved via the visitor design pattern --- a generic visitor for formulas of first-order logic is implemented in \cite{notebook:code} as
  \begin{CodeRefDisplay}
    \ManualCodeRef{logic.formulas.formula_visitor.FormulaVisitor}
  \end{CodeRefDisplay}
\end{comments}

\begin{algorithm}[Propositional constant expansion]\label{alg:propositional_constant_expansion}
  While \fullref{alg:propositional_constant_collapse} aims to remove unnecessary occurrences of the propositional constants, we may want to dispose of them altogether. For this, we introduce an operator \( M_{\logic{ce}} \) on \hyperref[def:propositional_formula]{propositional formulas} that replaces propositional constants with equivalent constructs expressed via other connective.
  ...
  \begin{equation*}
    M_{\logic{ce}}(\varphi) \coloneqq \begin{cases}
...
\end{algorithm}
\begin{comments}
  \item A variation suitable for \fullref{alg:propositional_formula_to_cnf_or_dnf} is to replace \( \syntop \) with \( p_\varphi \synvee \synneg p_\varphi \). This has the downside of only being sound with respect to classical semantics.

  \item This algorithm is implemented in \cite{notebook:code} as
  \begin{CodeRefDisplay}
    \GetCodeRef{alg:propositional_constant_expansion}
  \end{CodeRefDisplay}
\end{comments}
\begin{proof}
...
\begin{algorithm}[Propositional repeated negation collapse]\label{alg:propositional_repeated_negation_collapse}
  ...
  \begin{equation*}
    M_{\logic{DNE}}(\varphi) = \begin{cases}
      p,                                                      &\varphi = p \in \op*{Prop}, \\
      \theta,                                                 &\varphi = \synneg \psi \T{and} M_{\logic{DNE}}(\psi) = \synneg \theta, \\
      \synneg M_{\logic{DNE}}(\psi),                          &\varphi = \synneg \psi \T{and} M_{\logic{DNE}}(\psi) \T{is not a negation}, \\
      M_{\logic{DNE}}(\psi) \syncirc M_{\logic{DNE}}(\theta), &\varphi = \psi \syncirc \theta, {\syncirc} \in \op*{Conn}.
    \end{cases}
  \end{equation*}

  For any \hyperref[def:propositional_interpretation]{propositional interpretation} \( I \), we have \( \Bracks{\varphi}_I \leq \Bracks{M_{\logic{DNE}}(\varphi)}_I \) as a consequence of \cref{thm:def:heyting_algebra/dni}. Equality holds in \hyperref[def:truth_value_algebra/classical]{classical semantics}.
\end{algorithm}
\begin{comments}
  \item This algorithm is implemented in \cite{notebook:code} as
  \begin{CodeRefDisplay}
    \GetCodeRef{alg:propositional_repeated_negation_collapse}
  \end{CodeRefDisplay}
\end{comments}

\begin{proposition}\label{thm:alg:propositional_repeated_negation_collapse}
  \Fullref{alg:propositional_repeated_negation_collapse} has the following basic properties:
  \begin{thmenum}
    \thmitem{thm:alg:propositional_repeated_negation_collapse/no_double_negation} The result of the algorithm contains no double negations.

    \thmitem{thm:alg:propositional_repeated_negation_collapse/double_dual} If \( \varphi \) does not contain double negations, then \( \varphi \) coincides with its double opposite \( M_{\logic{DNE}}(M_{\logic{DNE}}(\varphi)) \).
   \end{thmenum}
\end{proposition}
\begin{proof}
  \SubProofOf{thm:alg:propositional_repeated_negation_collapse/no_double_negation} We will use \fullref{thm:induction_on_abstract_syntax} on \( \varphi \) to show that \( M_{\logic{DNE}}(\varphi) \) contains no double negations.

  \begin{itemize}
    \item If \( \varphi \) is atomic, there is nothing to prove.
    \item If \( \varphi = \synneg \psi \), where the inductive hypothesis holds for \( \psi \), there are two possibilities:
    \begin{itemize}
      \item If \( M_{\logic{DNE}}(\psi) = \synneg \theta \), then, by the inductive hypothesis, \( \theta \) is not a negation and does not contain double negations. Then neither does \( M_{\logic{DNE}}(\varphi) = \theta \)

      \item If \( M_{\logic{DNE}}(\psi) \) is not a negation, by the inductive hypothesis it does not contain double negations, and thus neither does
      \begin{equation*}
        M_{\logic{DNE}}(\varphi) = \synneg M_{\logic{DNE}}(\psi).
      \end{equation*}
    \end{itemize}

    \item If \( \varphi = \psi \syncirc \theta \), where the inductive hypothesis holds for \( \psi \) and \( \theta \), then
    \begin{equation*}
      M_{\logic{DNE}}(\varphi) = M_{\logic{DNE}}(\psi) \syncirc M_{\logic{DNE}}(\theta)
    \end{equation*}
    because its immediate subformulas do not.
  \end{itemize}

  \SubProofOf{thm:alg:propositional_repeated_negation_collapse/double_dual} Follows easily by induction.
\end{proof}
...
\begin{algorithm}[Propositional formula dualization]\label{alg:propositional_formula_dualization}
  ...
  Our goal is to build a formula \( \oline{M}(\varphi) \) such that \( \Bracks{\oline{M}(\varphi)}_I = \oline{\Bracks{\varphi}_I} \) for every \hyperref[def:propositional_interpretation]{propositional interpretation} \( I \).

  \begin{thmenum}
    \thmitem{alg:propositional_formula_dualization/const} If \( \varphi \) is a constant formula, let \( \oline{M}(\varphi) \) be the \hyperref[def:bounded_lattice/opposite]{dual constant} of \( \varphi \).

    \thmitem{alg:propositional_formula_dualization/prop} For every variable \( p \), let \( \oline{M}(p) \coloneqq \synneg p \).

    \thmitem{alg:propositional_formula_dualization/neg} If \( \varphi = \synneg \psi \), let
    \begin{equation*}
      \oline{M}(\varphi) \coloneqq M_{\logic{DNE}}(\synneg \oline{M}(\psi)),
    \end{equation*}
    where \( M_{\logic{DNE}} \) is the operator from \fullref{alg:propositional_repeated_negation_collapse}.

    \thmitem{alg:propositional_formula_dualization/conn} Finally, if \( \varphi = \psi \syncirc \theta \), let
    \begin{equation*}
      \oline{M}(\varphi) \coloneqq \begin{cases}
        \oline{M}(\psi) \synvee \oline{M}(\theta),                                          &{\syncirc} = {\synwedge}, \\
        \oline{M}(\psi) \synwedge \oline{M}(\theta),                                        &{\syncirc} = {\synvee}, \\
        \synneg \parens[\big]{ M_{\logic{DNE}}(\psi) \synimplies M_{\logic{DNE}}(\theta) }, &{\syncirc} = {\synimplies}, \\
        \synneg \parens[\big]{ M_{\logic{DNE}}(\psi) \syniff M_{\logic{DNE}}(\theta) },     &{\syncirc} = {\syniff}.
      \end{cases}
    \end{equation*}
...
\end{algorithm}
\begin{comments}
  \item This algorithm is a special case of the analogous first-order logic algorithm \fullref{alg:fol_formula_dualization}, implemented in \cite{notebook:code} as
  \begin{CodeRefDisplay}
    \GetCodeRef{alg:fol_formula_dualization}
  \end{CodeRefDisplay}

  \item The other Boolean polynomial operations from \cref{def:dual_boolean_polynomial} also give rise to transformations of propositional formulas. For example, \textcite{Gottschalk1953Quaternality} discusses contradual and dual formulas. \Textcite[114]{Church1956LogicVol1} and \textcite[192]{Andrews2002Logic} call the latter formulas \enquote{principal dual}.
  ...
  \item It seems simpler to define \( \oline{M}(\synneg \psi) \) as \( \psi \) rather than \( \synneg \oline{M}(\psi) \) (modulo double negation elimination). Unfortunately, this would require handling recursive cases separately.
...
\begin{proposition}\label{thm:alg:propositional_formula_dualization}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:alg:propositional_formula_dualization/double_dual} If \( \varphi \) does not contain double negations, then \( \varphi \) coincides with its double opposite \( \oline{M}(\oline{M}(\varphi)) \).
   \end{thmenum}
\end{proposition}
\begin{proof}
  \SubProofOf{thm:alg:propositional_formula_dualization/no_double_negation} We will use \fullref{thm:induction_on_abstract_syntax} on \( \varphi \) to show that \( \oline{M}(\varphi) \) contains no double negations.

  \begin{itemize}
    \item If \( \varphi \) is atomic, there is nothing to prove.
    \item If \( \varphi = \synneg \psi \), then \( \oline{M}(\varphi) \coloneqq M_{\logic{DNE}}(\synneg \oline{M}(\psi)) \). We explicitly remove double negations here.

    \item If \( \varphi = \psi \synwedge \theta \), where the inductive hypothesis holds for \( \psi \) and \( \theta \), we have
    \begin{equation*}
      \oline{M}(\varphi) = \oline{M}(\psi) \synvee \oline{M}(\theta).
    \end{equation*}

    No additional negations are introduced in \( \oline{M}(\varphi) \) compared to \( \oline{M}(\psi) \) or \( \oline{M}(\theta) \), so the inductive hypothesis implies that \( \oline{M}(\varphi) \) does not contain double negation.

    \item If \( \varphi = \psi \synvee \theta \), the result follows by duality.
    \item If \( \varphi = \psi \synimplies \theta \), then
    \begin{equation*}
      \oline{M}(\varphi) = \synneg \parens[\big]{ M_{\logic{DNE}}(\psi) \synimplies M_{\logic{DNE}}(\theta) }.
    \end{equation*}

    We explicitly remove double negations from \( \psi \) and \( \theta \), so \( \oline{M}(\varphi) \) also contains none.

    \item If \( \varphi = \psi \syniff \theta \), we proceed analogously.
  \end{itemize}

  \SubProofOf{thm:alg:propositional_formula_dualization/double_dual} Suppose that \( \varphi \) contains no double negations. We will use \fullref{thm:induction_on_abstract_syntax} on \( \varphi \) to show that \( \oline{M}(\oline{M}(\varphi)) \).

  \begin{itemize}
    \item If \( \varphi \) is a constant, then \( \oline{M}(\varphi) \) is by definition the dual constant, and \( \oline{M}(\oline{M}(\varphi)) \) thus coincides with \( \varphi \).

    \item If \( \varphi \) is a propositional variable, say \( p \), then \( \oline{M}(p) = \synneg p \)
    \begin{equation*}
      \oline{M}(\oline{M}(p)) = \oline{M}(\synneg p) = p.
    \end{equation*}
    ...
    \begin{equation*}
      ...
      \oline{M}(\synneg \psi)
      =
      M_{\logic{DNE}}(\synneg \oline{M}(\psi)).
    \end{equation*}

    We have two possibilities:
    \begin{itemize}
      \item If \( \oline{M}(\psi) \) is a negation, then \( \oline{M}(\psi) = \synneg \psi = \varphi \).

      Indeed, we have the following possibilities:
      \begin{itemize}
        \item If \( \psi \) is a variable, say \( p \), then \( \oline{M}(p) = \synneg p \).

        \item If \( \psi = \theta \synimplies \chi \), then
        \begin{equation*}
          \oline{M}(\psi)
          =
          \oline{M}(\theta \synimplies \chi)
          =
          \synneg (M_{\logic{DNE}}(\theta) \synimplies M_{\logic{DNE}}(\chi))
          =
      ...
      \begin{equation*}
        ...
        M_{\logic{DNE}}(\synneg \oline{M}(\psi))
        =
        M_{\logic{DNE}}(\synneg \synneg \psi)
        =
        \psi,
      \end{equation*}
      thus
      \begin{equation*}
        \oline{M}(\oline{M}(\varphi)) = \oline{M}(\psi) = \synneg \psi = \varphi.
      \end{equation*}

      \item Otherwise, \( \oline{M}(\psi) \) is not a negation, thus
      \begin{equation*}
        \oline{M}(\varphi) = M_{\logic{DNE}}(\synneg \oline{M}(\psi) = \synneg \oline{M}(\psi)
      \end{equation*}
      and
      \begin{equation*}
        \oline{M}(\oline{M}(\varphi))
        =
        M_{\logic{DNE}}(\synneg \oline{M}(\oline{M}(\psi)))
        \reloset {\T{ind.}} =
        M_{\logic{DNE}}(\synneg \psi)
        =
        M_{\logic{DNE}}(\varphi)
        =
        \varphi.
    ...
    \begin{equation*}
      \oline{M}(\oline{M}(\varphi))
      =
      \oline{M}(\oline{M}(\psi) \synvee \oline{M}(\theta))
      =
      \oline{M}(\oline{M}(\psi)) \synwedge \oline{M}(\oline{M}(\theta))
      \reloset {\T{ind.}} =
      \psi \synwedge \theta
      =
      \varphi,
    ...
    \begin{equation*}
      \oline{M}(\oline{M}(\varphi))
      =
      \oline{M}(M_{\logic{DNE}}(\psi) \synimplies M_{\logic{DNE}}(\theta))
      =
      \oline{M}(\psi \synimplies \theta)
      =
      \varphi,
    \end{equation*}
...
\end{definition}
\begin{comments}
  \item We can use \fullref{alg:propositional_constant_collapse} to remove unnecessary propositional constants in formulas like \( \synp \wedge \syntop \).
  ...
  \item Via \fullref{alg:propositional_formula_to_heyting_term}, this becomes a particular syntax for \hyperref[def:free_boolean_algebra/polynomial]{Boolean polynomials}.
...
\begin{proposition}\label{thm:propositional_cnf_and_dnf_dualization}
  Suppose that the \hyperref[def:propositional_formula]{propositional formula} \( \varphi \) has no double negations. Consider its opposite \( \oline{M}(\varphi) \) obtained via \fullref{alg:propositional_formula_dualization}.

  \begin{thmenum}
    \thmitem{thm:propositional_cnf_and_dnf_dualization/cnf} The formula \( \varphi \) is in \hyperref[def:propositional_cnf_and_dnf]{conjunctive normal form} if and only if \( \oline{M}(\varphi) \) is in disjunctive normal form.

    \thmitem{thm:propositional_cnf_and_dnf_dualization/dnf} Conversely, \( \varphi \) is in disjunctive normal form if and only if \( \oline{M}(\varphi) \) is in conjunctive normal form.
  \end{thmenum}
\end{proposition}
\begin{proof}
  We will only show \cref{thm:propositional_cnf_and_dnf_dualization/cnf} because \cref{thm:propositional_cnf_and_dnf_dualization/dnf} follows by \fullref{thm:lattice_duality} (via \fullref{alg:propositional_formula_to_heyting_term}).

  \SufficiencySubProof Suppose that \( \varphi \) is in CNF.

  We will use \fullref{thm:induction_on_abstract_syntax} on \( \varphi \) to show that \( \oline{M}(\varphi) \) is in DNF:
  \begin{itemize}
    \item If \( \varphi \) is atomic, it is either a variable (i.e. a positive literal) or a constant, so it is in both CNF and DNF.

    Then \( \oline{M}(\varphi) \) is either a negative literal or a constant; hence it is also both in CNF and DNF.

    \item If \( \varphi = \synneg \psi \), then \( \psi \) must be a propositional variable. In this case \( \oline{M}(\varphi) = \psi \), which is again both in CNF and DNF.

    \item If \( \varphi = \psi \synvee \theta \), then \( \psi \) and \( \theta \) are elementary disjuncts.

    Then \( \oline{M}(\psi) \) and \( \oline{M}(\theta) \) are elementary conjuncts, and thus so is their conjunction.
    ...
    Since \( \varphi \) is in CNF, so are \( \psi \) and \( \theta \). By the inductive hypothesis, \( \oline{M}(\psi) \) and \( \oline{M}(\theta) \) are in DNF; then so is their disjunction \( \oline{M}(\varphi) \).
  \end{itemize}

  \NecessitySubProof Let \( \varphi \) be a formula without double negation and suppose that \( \oline{M}(\varphi) \) is in DNF.

  Sufficiency in \cref{thm:propositional_cnf_and_dnf_dualization/dnf} implies that \( \oline{M}(\oline{M}(\varphi)) \) is in CNF. Moreover, since we have assumed that \( \varphi \) contains no double negations, \cref{thm:alg:propositional_formula_dualization/double_dual} implies that \( \oline{M}(\oline{M}(\varphi)) = \varphi \).
...
\begin{algorithm}[Propositional formula to CNF or DNF]\label{alg:propositional_formula_to_cnf_or_dnf}
  ...
  \begin{thmenum}
    \thmitem{alg:propositional_formula_to_cnf_or_dnf/atomic} If \( \varphi \) is atomic, it is either a constant or a positive literal, and thus it is already in both conjunctive and disjunctive normal form.

    In this case, let \( T_{\anon}(\varphi) \coloneqq \varphi \) for both \( {\anon} = \wedge \) and \( {\anon} = \vee \).

    \thmitem{alg:propositional_formula_to_cnf_or_dnf/negation} If \( \varphi = \synneg \psi \), let
    \begin{align*}
      T_\wedge(\varphi) \coloneqq \oline{M}(T_\vee(\psi))
      &&
      \T{and}
      &&
      T_\vee(\varphi) \coloneqq \oline{M}(T_\wedge(\psi)),
    \end{align*}
    where \( \oline{M} \) is the dualization operator from \fullref{alg:propositional_formula_dualization}.

    These cases are justified by \cref{thm:propositional_cnf_and_dnf_dualization}.

    \thmitem{alg:propositional_formula_to_cnf_or_dnf/conditional} If \( \varphi = \psi \synimplies \theta \), let
    \begin{equation*}
      T_{\anon}(\varphi) \coloneqq T_{\anon}(\synneg \psi \synvee \theta).
    \end{equation*}

    \thmitem{alg:propositional_formula_to_cnf_or_dnf/biconditional} If \( \varphi = \psi \syniff \theta \), let
    \begin{align*}
    ...
    \thmitem{alg:propositional_formula_to_cnf_or_dnf/outer} If \( \varphi = \psi \syncirc \theta \), let
    \begin{equation*}
      T_{\syncirc}(\varphi) \coloneqq T_{\syncirc}(\psi) \syncirc T_{\syncirc}(\theta).
    \end{equation*}

    \thmitem{alg:propositional_formula_to_cnf_or_dnf/inner} If \( \varphi = \psi \syncirc^\oppos \theta \), let \( \psi' \coloneqq T_{\syncirc}(\psi) \) and \( \theta' \coloneqq T_{\syncirc}(\theta) \) and let
    \begin{equation*}
...
\begin{comments}
  \item This algorithm is implemented in \cite{notebook:code} as
  \begin{CodeRefDisplay}
    \GetCodeRef{alg:propositional_formula_to_cnf_or_dnf}
  \end{CodeRefDisplay}
\end{comments}
...
\begin{definition}\label{def:full_propositional_cnf_and_dnf}\mimprovised
  ...
  \begin{thmenum}
    ...
    Otherwise, either \( \varphi = \sembot \) (resp. \( \varphi = \semtop \)) or \( \varphi \) contains no constants.
...
\begin{algorithm}[Boolean function to perfect DNF]\label{alg:function_to_dnf}\mcite[thm. I.1.3]{Яблонский2003ДискретнаяМатематика}
  ...
  If we wish to obtain a perfect conjunctive normal form instead, we can take the opposite \( \oline{M}(\varphi_f) \) obtained via \fullref{alg:propositional_formula_dualization}.
...
\begin{comments}
  \item This algorithm is implemented in \cite{notebook:code} as
  \begin{CodeRefDisplay}
    \GetCodeRef{alg:function_to_dnf}
  \end{CodeRefDisplay}
\end{comments}
\begin{defproof}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      \item If \( f = \semtop \), then \( \varphi_f = \syntop \) by \fullref{alg:function_to_dnf/true}.
      \item If \( f = \sembot \), then \( \varphi_f = \synbot \) by \fullref{alg:function_to_dnf/false}.
    \end{itemize}
    ...
    \item Let \( n = 1 \).
    \begin{itemize}
      \item If \( f(x) = \sembot \), then \( \varphi_f = \synbot \), again by \fullref{alg:function_to_dnf/false}.

      \item If \( f(x) = \semtop \), then \fullref{alg:function_to_dnf/elementary} provides us with the elementary disjunctions \( p_1^\semtop = p_1 \) and \( p_1^\sembot = \synneg p_1 \) and \fullref{alg:function_to_dnf/total} gives us \( \varphi_f = p_1 \synwedge \synneg p_1 \).
...
\begin{proposition}\label{thm:full_cnf_and_dnf_uniqueness}
  Every \hyperref[def:propositional_formula]{propositional formula} is \hyperref[def:propositional_semantics/equivalence]{semantically equivalent} to a unique formula in \hyperref[def:full_propositional_cnf_and_dnf]{perfect disjunctive normal form} (resp. perfect conjunctive normal form).
\end{proposition}
\begin{proof}
