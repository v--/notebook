...
\begin{comments}
  ...
  \item This algorithm is a special case of the analogous first-order logic algorithm \fullref{alg:fol_substitution}, implemented in \cite{notebook:code} as
...
\begin{algorithm}[Propositional formula to CNF or DNF]\label{alg:propositional_formula_to_cnf_or_dnf}
  ...
  \begin{thmenum}
    ...
    \thmitem{alg:formula_to_cnf_and_dnf/inner} If \( \varphi = \psi \syncirc^{\oppos} \theta \), let \( \psi' \coloneqq T_{\syncirc}(\psi) \) and \( \theta' \coloneqq T_{\syncirc}(\theta) \) and let
    \begin{equation*}
