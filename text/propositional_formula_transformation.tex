...
\begin{algorithm}[Propositional constant collapse]\label{alg:propositional_constant_collapse}
  We introduce an operator \( T_{\logic{cc}} \) on \hyperref[def:propositional_formula]{propositional formulas} that attempts to simplify subformulas involving logical constants. The result \( T_{\logic{cc}}(\varphi) \) will be \hyperref[def:propositional_semantics/equivalence]{semantically equivalent} with \( \varphi \).
...
\begin{algorithm}[Propositional repeated negation collapse]\label{alg:propositional_repeated_negation_collapse}
  ...
  \begin{equation*}
    T_{\logic{DNE}}(\varphi) = \begin{cases}
      ...
      \theta,                                                 &\varphi = \synneg \psi \T{and} T_{\logic{DNE}}(\psi) = \synneg \theta, \\
      \synneg T_{\logic{DNE}}(\psi),                          &\varphi = \synneg \psi \T{and} T_{\logic{DNE}}(\psi) \T{is not a negation}, \\
      T_{\logic{DNE}}(\psi) \syncirc T_{\logic{DNE}}(\theta), &\varphi = \psi \syncirc \theta, {\syncirc} \in \op*{Conn}.
    \end{cases}
  \end{equation*}

  For any \hyperref[def:propositional_interpretation]{propositional interpretation} \( I \), we have \( \Bracks{\varphi}_I \leq \Bracks{T_{\logic{DNE}}(\varphi)}_I \) as a consequence of \cref{thm:def:heyting_algebra/dni}. Equality holds in \hyperref[def:truth_value_algebra/classical]{classical semantics}.
\end{algorithm}
\begin{comments}
...
\begin{proposition}\label{thm:alg:propositional_repeated_negation_collapse}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:alg:propositional_repeated_negation_collapse/double_dual} If \( \varphi \) does not contain double negations, then \( \varphi \) coincides with its double opposite \( T_{\logic{DNE}}(T_{\logic{DNE}}(\varphi)) \).
   \end{thmenum}
\end{proposition}
\begin{proof}
  \SubProofOf{thm:alg:propositional_repeated_negation_collapse/no_double_negation} We will use \fullref{thm:induction_on_abstract_syntax} on \( \varphi \) to show that \( T_{\logic{DNE}}(\varphi) \) contains no double negations.
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      \item If \( T_{\logic{DNE}}(\psi) = \synneg \theta \), then, by the inductive hypothesis, \( \theta \) is not a negation and does not contain double negations. Then neither does \( T_{\logic{DNE}}(\varphi) = \theta \)

      \item If \( T_{\logic{DNE}}(\psi) \) is not a negation, by the inductive hypothesis it does not contain double negations, and thus neither does
      \begin{equation*}
        T_{\logic{DNE}}(\varphi) = \synneg T_{\logic{DNE}}(\psi).
      \end{equation*}
    \end{itemize}

    \item If \( \varphi = \psi \syncirc \theta \), where the inductive hypothesis holds for \( \psi \) and \( \theta \), then
    \begin{equation*}
      T_{\logic{DNE}}(\varphi) = T_{\logic{DNE}}(\psi) \syncirc T_{\logic{DNE}}(\theta)
    \end{equation*}
...
\begin{comments}
  ...
  \item It seems simpler to define \( \oline{T}(\synneg \psi) \) as \( \psi \) rather than \( \synneg \oline{T}(\psi) \) (modulo double negation elimination). Unfortunately, this would require handling recursive cases separately.
...
\end{proposition}
\begin{proof}
  \SubProofOf{thm:alg:propositional_formula_dualization/no_double_negation} We will use \fullref{thm:induction_on_abstract_syntax} on \( \varphi \) to show that \( \oline{T}(\varphi) \) contains no double negations.
...
\begin{algorithm}[Propositional formula to CNF or DNF]\label{alg:propositional_formula_to_cnf_or_dnf}
  ...
  \begin{thmenum}
    ...
    \end{align*}
    where \( \oline{T} \) is the dualization operator from \fullref{alg:propositional_formula_dualization}.
