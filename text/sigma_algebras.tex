\section{\texorpdfstring{\( \sigma \)}{σ}-algebras}\label{sec:sigma_algebras}
...
\begin{proposition}\label{thm:boolean_subalgebra}
  ...
  \begin{thmenum}
    \thmitem{thm:boolean_subalgebra/subalgebra} It is a Boolean sublattice in the sense of \cref{def:boolean_algebra/subobject}, that is, it contains the top and bottom elements and is closed under binary joins, binary meets and complements.
    \thmitem{thm:boolean_subalgebra/subring} It is a (Boolean) subring in the sense of \cref{def:ring/subobject}, that is, it contains the top and bottom elements and is closed under \hyperref[def:symmetric_difference]{symmetric differences} and meets.
    \thmitem{thm:boolean_subalgebra/finite} It is closed under (possibly empty) \emph{finite} joins (resp. meets) and complements.
    \thmitem{thm:boolean_subalgebra/binary} It is \emph{nonempty} and is closed under binary joins (resp. meets) and complements.
    \thmitem{thm:boolean_subalgebra/difference} It contains the \emph{top element} and is closed under binary joins (resp. meets) and \hyperref[def:boolean_algebra_difference]{\emph{proper differences}}.
  \end{thmenum}
\end{proposition}
\begin{comments}
  ...
  \textcite[def. 3.1]{Драганов2022ТеорияНаМярката},
  \textcite[15]{Yoshida1980FunctionalAnalysis} (which only defines \( \sigma \)-algebras),
  \textcite[def. 4.9]{Jech2006SetTheory},
  \textcite[21]{Folland1999RealAnalysis},
  \textcite[2]{Çinlar2011ProbabilityAndStochastics},
  \textcite[def. 1.4.1]{Tao2011MeasureTheory},
  \textcite[\S 5.25]{Schechter1997AnalysisHandbook},
  \textcite[3]{Bobrowski2005FunctionalAnalysis},
  \textcite[2]{Malliavin1995IntegrationAndProbability},
  \textcite[294]{WheedenZygmund2015MeasureAndIntegral},
  \textcite[50]{КанторовичАкилов1984ФункциональныйАнализ},
  \textcite[def. 2.1]{Боровков1999ТеорияВероятностей} and
  \textcite[30]{Ширяев2007ВероятностьТом1}.
  ...
  \textcite[def. 1.2.1]{Богачёв2003ОсновыТеорииМерыТом1},
  \textcite[def. 1.2]{ДьяченкоУльянов1998МераИИнтеграл} (who require closure under intersections and \emph{symmetric differences}) and
  \textcite[30]{Ширяев2007ВероятностьТом1}.
  Otherwise, we obtain Halmos' strictly weaker notion of a \enquote{ring of sets} (although Halmos discusses sets specifically, his results are easily generalizable). These rings are discussed in \cite[ch. I]{Halmos1976MeasureTheory}.

  \item The simplified condition \cref{thm:boolean_subalgebra/binary} is often stated, in the context of \hyperref[def:algebra_of_subsets]{algebras of subsets}, so that \( Y \) must contain \( \top \) (e.g. \Textcite[def. 1.3]{Rudin1987RealAndComplexAnalysis}, \textcite[\S 5.25]{Schechter1997AnalysisHandbook}, \textcite[3]{Bobrowski2005FunctionalAnalysis} and \textcite[def. 2.1]{Боровков1999ТеорияВероятностей}), or alternatively so that \( Y \) must contain \( \bot \) (e.g. \Textcite[def. 1.4.1]{Tao2011MeasureTheory}) or even both (e.g. \Textcite[def. 1.1]{ДимитровЯнев2007ВероятностиИСтатистика})
\end{comments}
\begin{proof}
  ...
  \ImplicationSubProof{thm:boolean_subalgebra/subring}{thm:boolean_subalgebra/finite} \Cref{thm:boolean_ring_as_algebra} expresses binary meets, joins and complements via ring operations on, thus a subring must also be closed under finite meets, finite joins and complements.
  ...
  \ImplicationSubProof{thm:boolean_subalgebra/binary}{thm:boolean_subalgebra/difference} Suppose that \( Y \) is closed under complements and either binary joins or binary meets. Then, due to \cref{thm:boolean_subalgebra_binary_de_morgan}, it is closed under both joins and meets.
...
\begin{definition}\label{def:algebra_of_subsets}\mcite[294]{WheedenZygmund2015MeasureAndIntegral}
  We say that a family of subsets of a set \( \Omega \) is an \term[bg=булева алгебра (\cite[def. 3.1]{Драганов2022ТеорияНаМярката}), ru=алгебра подмножеств (\cite[def. II.1.1]{Ширяев2007ВероятностьТом1})]{algebra of subsets} if it is a \hyperref[def:boolean_algebra/subobject]{Boolean subalgebra} of its \hyperref[thm:boolean_algebra_of_subsets]{power set Boolean algebra} \( \pow(\Omega) \).
\end{definition}
\begin{comments}
...
\begin{definition}\label{def:sigma_algebra}
  ...
  \begin{thmenum}
    \thmitem{def:sigma_algebra/subalgebra}\mimprovised It is a \hyperref[def:countably_complete_lattice]{countably-complete} \hyperref[def:boolean_algebra/subobject]{Boolean subalgebra} of its \hyperref[thm:boolean_algebra_of_subsets]{power set Boolean algebra} \( \pow(\Omega) \).

    \thmitem{def:sigma_algebra/direct}\mcite[49]{WheedenZygmund2015MeasureAndIntegral} It is \emph{nonempty} and closed under \hyperref[def:set_countability/countably_infinite]{countably infinite} joins (resp. meets) and complements.
...
\begin{comments}
  ...
  \item Terminology for algebras and \( \sigma \)-algebras and related concepts is discussed in \cref{rem:sigma_algebra_terminology}.
  \item \Cref{def:sigma_algebra/monotone} is usually stated as a theorem claiming that \( \sigma \)-algebras are \hyperref[def:algebra_of_subsets]{algebras of subsets} that are also \hyperref[def:monotone_class]{monotone classes}, for example by
  \textcite[thm. 1.4]{Malliavin1995IntegrationAndProbability},
  \textcite[thm. 1.2.7]{Bobrowski2005FunctionalAnalysis},
  \textcite[lemma 1.7.14]{Tao2011MeasureTheory},
  \textcite[exerc. 1.6(b)]{Cohn2013MeasureTheory},
  \textcite[thm. 2.35]{Folland1999RealAnalysis} and
  \textcite[thm. 1.9.3(i)]{Богачёв2003ОсновыТеорииМерыТом1}.
...
\begin{remark}\label{rem:sigma_algebra_terminology}
  ...
  \begin{table}[!ht]
    ...
    \begin{tabular}{l l l}
      ...
      \cite[50]{КанторовичАкилов1984ФункциональныйАнализ}                    & Алгебра                                               & \( \sigma \)-алгебра                               \\
                                                                             & \footnotesize (algebra)                               & \footnotesize (\( \sigma \)-algebra)               \\
      \cite[def. I.5.1; def. I.5.3]{КолмогоровФомин2004ФункциональныйАнализ} & Алгебра множеств                                      & \( \sigma \)-алгебра                               \\
  ...
  \begin{itemize}
    \item \Textcite[77]{Hausdorff1935Mengenlehre} uses \enquote{Ring} for families of sets closed under binary unions and intersections, and defines \enquote{K\"{o}rper} (translated as \enquote{fields} in \cite[90]{Hausdorff1962SetTheory}) as rings closed under set differences.

    \item \Textcite[20]{Halmos1976MeasureTheory} uses \enquote{ring} for what Hausdorff calls \enquote{field}, and avoid the term \enquote{field}.

    \item \Textcite[86]{Вулих1973ВещественныйАнализ} uses \enquote{кольцо} (\enquote{ring}) for analogously.

    \item \Textcite[268]{HalmosGivant2009BooleanAlgebras} use \enquote{field} for nonempty families closed under binary intersections, unions and complements, which due to \cref{thm:boolean_subalgebra} coincides with our notion of algebras of subsets. Unlike in Halmos' aforementioned book, the author's avoid defining and discussing \enquote{rings}, and define a \enquote{\( \sigma \)-algebra} as a \hyperref[def:countably_complete_lattice]{countably-complete} Boolean algebra, which is a generalization of our usage.

    \item \Textcite[15]{Yoshida1980FunctionalAnalysis} avoids defining rings of sets, but refers to \( \sigma \)-algebras a \enquote{\( \sigma \)-rings}.

    \item \Textcite[12]{Birkhoff1967LatticeTheory} and \textcite[113]{Grätzer2011LatticeTheory} use \enquote{ring} to refer to sublattices of the power set, i.e. families closed under unions and intersections. Both define \enquote{fields} as rings closed under complements, which, if nonempty, are equivalent to our notion of algebras of subsets due to \cref{thm:boolean_subalgebra}.

    \item \Textcite[def. I.5.1]{КолмогоровФомин2004ФункциональныйАнализ} and \textcite[def. 1.2]{ДьяченкоУльянов1998МераИИнтеграл} use \enquote{кольцо} (\enquote{ring}) to refer to families closed under symmetric differences and intersections, and \enquote{algebra} for a ring with a top element. Again, due to \cref{thm:boolean_subalgebra}, this notion of algebras coincides with our usage.

    \item \Textcite[def. 1.2.13]{Богачёв2003ОсновыТеорииМерыТом1} uses \enquote{кольцо} (\enquote{ring}) to refer to families closed under binary unions, binary intersections and set differences. The author defines algebras and \( \sigma \)-algebras prior to rings and rarely mentions rings later in the book.
  \end{itemize}
\end{remark}
