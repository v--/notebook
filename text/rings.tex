...
\begin{definition}\label{def:ring}\cite[141]{Knapp2016BasicAlgebra}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:ring/trivial} Any single-element ring is as trivial object in \( \ucat{Rng} \) in the sense of \fullref{def:trivial_object}. It is not a zero object in \( \ucat{Ring} \), although we may refer to \( \set{ 0 } \) as \enquote{the trivial ring}.

    \thmitem{def:ring/kernel} \Fullref{thm:zero_morphisms_pointed/kernel} implies that the \hyperref[def:zero_morphisms/kernel]{categorical kernel} of a morphism \( \varphi: R \to T \) in \( \ucat{Rng} \) as the (nonunital) subring
    \begin{equation*}
    ...
    \thmitem{def:ring/quotient} Similarly to groups, \hyperref[def:first_order_congruence]{first-order congruences} for rings are well-behaved. \Fullref{thm:ideals_and_congruences} shows that congruences correspond exactly to \hyperref[def:semiring_ideal]{ideals}.
  \end{thmenum}
\end{definition}
...
\begin{proof}
  ...
  \SubProofOf{thm:def:ring/ring_entire_iff_cancellative} Follows from \fullref{thm:semiring_cancellative_iff_no_zero_divisors}.
...
\begin{proposition}\label{thm:ideals_and_congruences}
  ...
  \begin{thmenum}
    \thmitem{thm:ideals_and_congruences/cong_to_ideal} For every congruence in \( R \), the coset \( [0] \) of the zero element is an ideal of \( R \).

    \thmitem{thm:ideals_and_congruences/ideal_to_cong} Conversely, for every ideal \( I \), the relation \( x \cong y \) defined to hold if \( x - y \) is in \( I \), is a congruence.

    \thmitem{thm:ideals_and_congruences/inverse} The procedures for obtaining a congruence from an ideal and vice versa are inverses.
  \end{thmenum}
\end{proposition}
\begin{comments}
  ...
  \item This is based on a similar statement holds for groups --- see \fullref{thm:normal_subgroups_and_congruences}.
\end{comments}
\begin{proof}
  ...
  Since \( I \) is closed under multiplication with anything and addition, we conclude that \( ax - by \in I \) and hence \( ax \cong by \).
