...
\begin{definition}\label{def:ring}\mcite[def. 2.1]{Jacobson1985BasicAlgebraI}
  ...
  \begin{thmenum}
    \thmitem{def:ring/theory}\mimprovised We can construct a \hyperref[def:first_order_theory]{first-order theory} for rings by adding a unary functional symbol \( - \) and the involution axiom \eqref{eq:def:group/theory/inverse_axiom} to the \hyperref[def:semiring/theory]{theory of semirings}.
    ...
    \thmitem{def:ring/category}\mimprovised We denote the corresponding \hyperref[def:category_of_small_first_order_models]{category of \( \mscrU \)-small models} by \( \ucat{Ring} \) and the category of nonunital rings by \( \ucat{Rng} \).
    ...
    \thmitem{def:ring/quotient}\mimprovised Similarly to groups, \hyperref[def:first_order_congruence]{first-order congruences} for rings are well-behaved. \Cref{thm:ideals_and_congruences} shows that congruences correspond exactly to \hyperref[def:semiring_ideal]{two-sided ideals}.
  \end{thmenum}
\end{definition}
...
\begin{proposition}\label{thm:integer_polynomials_and_logical_terms}
  ...
  Let \( {\sim} \) be the \hyperref[def:equivalence_relation]{equivalence relation} on \( \op*{Term}_\Sigma \) defined as \( \tau \sim \sigma \) if and only if \( \tau \) and \( \sigma \) have the same \hyperref[def:fol_denotation/terms]{denotation} for every variable assignment into over \( \BbbZ \).
...
\begin{definition}\label{def:field}
  ...
  \begin{thmenum}
    \thmitem{def:field/theory}\mimprovised We can construct a \hyperref[def:first_order_theory]{first-order theory} for fields by adding to the \hyperref[def:semiring/theory]{theory of rings} the axioms \( \synneg (0 \syneq 1) \) and
