...
\begin{definition}\label{def:ring}\mcite[def. 2.1]{Jacobson1985BasicAlgebraI}
  ...
  The absorption condition \eqref{eq:def:semiring/absorption} becomes redundant due to \cref{thm:def:ring/zero_absorbing}.
  ...
  \begin{thmenum}
    ...
    \thmitem{def:ring/homomorphism}\mcite[def. 2.3]{Jacobson1985BasicAlgebraI} A \hyperref[def:fol_homomorphism]{first-order homomorphism} between the rings \( R \) and \( T \) is a \hyperref[def:semiring/homomorphism]{semiring homomorphism} \( \varphi: R \to T \) that additionally preserves additive inverses.
    ...
    \thmitem{def:ring/trivial} Similarly to semirings, any single-element ring is as trivial object in \( \ucat{Rng} \) in the sense of \cref{def:trivial_object}. It is not a zero object in \( \ucat{Ring} \), although we may refer to \( \set{ 0 } \) as \enquote{the trivial ring}.
...
\begin{definition}\label{def:field}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:field/submodel}\mcite[231]{Jacobson1985BasicAlgebraI} If \( \Bbbk \) and \( \BbbK \) are fields such that \( \Bbbk \subseteq \BbbK \), we say that \( \BbbK \) is a \term[bg=разширение на поле (\cite[194]{ГеновМиховскиМоллов1991Алгебра}), ru=расширение поля (\cite[190]{Кострикин2001АлгебраЧасть3})]{field extension} of \( \Bbbk \) and that \( \Bbbk \) is a \term[bg=подполе (\cite[192]{ГеновМиховскиМоллов1991Алгебра}), ru=подполе (\cite[def. 1.3.3]{Винберг2014КурсАлгебры})]{subfield} of \( \BbbK \). In particular, if \( \BbbK = \Bbbk \), we say that the extension is trivial.
