...
\begin{proof}
  ...
  \SubProofOf{thm:ring_zero_morphisms/isomorphism} \Cref{thm:fundamental_theorem_of_homomorphisms} implies that \( \psi \) as defined in \eqref{eq:thm:ring_zero_morphisms/isomorphism} is an isomorphism between the the additive groups of \( R / \ker \varphi \) and \( \img \varphi \). Furthermore, \( \psi \) is a ring isomorphism because \( \varphi \) is.
\end{proof}
...
\begin{comments}
  ...
  \item This result is an explicit proof of the equivalence between the \hyperref[def:polynomial_algebra]{polynomial algebra} \( \BbbZ[mscrX] \) and the corresponding \hyperref[def:free_term_model]{free term model}. This point is made more explicit in \cref{ex:def:fol_free_term_model}.
\end{comments}
\begin{proof}
