...
\begin{definition}\label{def:ring}\mcite[def. 2.1]{Jacobson1985BasicAlgebraI}
  ...
  \begin{thmenum}
    \thmitem{def:ring/theory}\mimprovised We can construct a \hyperref[def:fol_theory]{first-order theory} for rings by adding a unary functional symbol \( - \) and the involution axiom \eqref{eq:def:group/theory/inverse_axiom} to the \hyperref[def:semiring/theory]{theory of semirings}.
...
\begin{proof}
  ...
  \SubProofOf{thm:ring_zero_morphisms/isomorphism} \Cref{thm:group_zero_morphisms/isomorphism} implies that \( \psi \) as defined in \eqref{eq:thm:ring_zero_morphisms/isomorphism} is an isomorphism between the the additive groups of \( R / \ker \varphi \) and \( \img \varphi \). Furthermore, \( \psi \) is a ring isomorphism because \( \varphi \) is.
\end{proof}
...
\begin{proposition}\label{thm:def:signum}
  \hyperref[def:signum]{The signum function} on the totally ordered \hi{\hyperref[def:entire_semiring]{entire}} ring \( R \) is a \hyperref[def:group/homomorphism]{group homomorphism} from the multiplicative group of \( R \) to the group \( \set{ -1, 0, 1 } \) under integer multiplication.
