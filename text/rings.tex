...
\begin{proposition}\label{thm:def:ring_localization}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:def:ring_localization/prime_ideals}\mcite[exer. 4.3]{КоцевСидеров2016} The map \( I \mapsto S^{-1} I \) is a \hyperref[def:preordered_set/homomorphism]{strict order isomorphism} between the set of \hyperref[def:ring/submodel]{prime ideals} of \( R \) not intersecting \( S \) and the set of all prime ideals of \( S^{-1} R \).
...
\begin{definition}\label{def:division_ring}\mcite[91]{Jacobson1985Vol1}
...
\begin{definition}\label{def:field}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:field/division_ring}\mcite[91]{Jacobson1985Vol1} \( \BbbK \) is a \hyperref[def:ring/commutative]{commutative} \hyperref[def:division_ring]{division ring}.
  \end{thmenum}
