...
\begin{definition}\label{def:polynomial_equation}\mimprovised
  \hyperref[def:fol_equation]{Equations of first-order logic} in the \hyperref[def:ring/theory]{theory of rings} are called \term{algebraic}.

  As shown in \cref{thm:integer_polynomials_and_logical_terms}, these correspond naturally to polynomials over integers. \Fullref{alg:logical_term_to_integer_polynomial} allows transforming the equation \( \tau \syneq \sigma \) in the object logic to the metalingual \term{polynomial equation}
  ...
  When not only the coefficients but also the solutions are intended to be integers, we call the equation \term[ru=Диофантово уравнение (\cite[\S 1.9]{ШеньВерещагин2017ВычислимыеФункции}), en=Diophantine equation (\cite[5]{Apostol1976AnalyticNumberTheory})]{Diophantine}.

  More generally we can consider polynomial equations over any \hyperref[def:semiring]{semiring}. This aligns with \cref{con:free_construction}, where we discussed the role of polynomials as abstract syntax for (semi)rings.
...
\begin{definition}\label{def:field}
  We will call the \hyperref[def:ring/trivial]{nontrivial} \hyperref[def:ring]{ring} \( \BbbK \) a \term[bg=поле (\cite[394]{Обрешков1962ВисшаАлгебра}), ru=поле (\cite[def. 1.3.2]{Винберг2014КурсАлгебры}), field=field (\cite[91]{Jacobson1985BasicAlgebraI})]{field} if any of the following equivalent conditions hold:
  \begin{thmenum}
