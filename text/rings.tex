...
\begin{definition}\label{def:ring}\mcite[def. 2.1]{Jacobson1985BasicAlgebraI}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:ring/kernel}\mimprovised \Fullref{thm:ring_zero_morphisms/kernel} implies that the \hyperref[def:zero_morphisms/kernel]{categorical kernel} of a morphism \( \varphi: R \to T \) in \( \ucat{Rng} \) is the additive group kernel
    \begin{equation*}
    ...
    \thmitem{def:ring/quotient}\mimprovised Similarly to groups, \hyperref[def:first_order_congruence]{first-order congruences} for rings are well-behaved. \Fullref{thm:ideals_and_congruences} shows that congruences correspond exactly to \hyperref[def:semiring_ideal]{two-sided ideals}.
  \end{thmenum}
\end{definition}
...
\begin{defproof}
  ...
  If \( n = 0 \), \( \ker\iota \) is a trivial group and \fullref{thm:group_homomorphism_trivial_kernel} implies that \( \iota \) is an embedding. Then there cannot exist a positive integer \( n \) such that
  \begin{equation*}
    n \cdot 1_R = 0_R.
  \end{equation*}
...
\begin{proof}
  ...
  The characteristic morphism \( n \mapsto n \cdot 1_R \) thus has a trivial kernel, and \fullref{thm:group_homomorphism_trivial_kernel} implies that it is injective. Then \( R \) is infinite since it contains an embedding of every integer.
  ...
  We will use \fullref{def:ring_characteristic/direct}, according to which \( n \) is the additive order of \( 1 \) if it is finite and \( 0 \) if the order is infinite.
...
\begin{example}\label{ex:def:ring_characteristic}
  ...
  \begin{thmenum}
    ...
    \thmitem{ex:def:ring_characteristic/boolean_ring} \Fullref{thm:boolean_ring_characteristic} implies that the characteristic of a \hyperref[def:boolean_ring]{Boolean ring} is \( 2 \).
  \end{thmenum}
\end{example}

\paragraph{Endomorphism rings}

\begin{definition}\label{def:endomorphism_ring}\mcite{Jacobson1985BasicAlgebraI}
  Let \( X \) be an \hyperref[def:abelian_group]{abelian group} or, more generally, an object in a \hyperref[def:category]{category} \( \cat{C} \) \hyperref[def:concrete_category]{concrete} over the category \( \cat{Ab} \) of abelian groups.

  Then the \hyperref[def:endomorphism_monoid]{endomorphism monoid} \( \End(X) \) is a \hyperref[def:ring]{ring} with addition inherited from \( X \) and multiplication given by composition. We refer to \( \End(X) \) as the \term[ru=кольцо эндоморфизмов (\cite[107]{Шафаревич1999ОсновныеПонятияАлгебры})]{endomorphism ring}.
\end{definition}
\begin{defproof}
...
\end{proposition}
\begin{comments}
  \item Also note the properties in \fullref{thm:def:ordered_semiring} of ordered semirings.

  \item Results regarding \hyperref[def:ordered_semigroup]{ordered semigroups} also hold for the additive and multiplicative semigroup --- see \fullref{thm:def:ordered_semigroup}.

  \item Similarly, results regarding \hyperref[def:ordered_semigroup]{ordered} \hyperref[def:group]{groups} also hold --- see \fullref{thm:ordered_group}.
\end{comments}
\begin{proof}
  \SubProofOf{thm:ordered_ring/additive_inversion} Follows from \fullref{thm:ordered_group/inversion}.
  ...
  \begin{itemize}
    ...
    \item If both \( x \) and \( y \) are nonzero, \fullref{thm:def:ring/ring_entire_iff_cancellative} implies that we can cancel \( z \) in either \( xz = yz \) or \( zx = zy \) to obtain \( x = y \), thus obtaining a contradiction.
  \end{itemize}

  In all three cases, we concluded that the inequalities \( zx \leq zy \) and \( zx \leq yz \) must be strict.

  \SubProofOf{thm:ordered_ring/negative_prod} If \( z < 0 \), \fullref{thm:ordered_ring/additive_inversion} implies that \( -z > 0 \). Then \( x < y \) implies that \( x(-z) \leq y(-z) \), and we can again use \fullref{thm:ordered_ring/additive_inversion} to conclude that \( xz \geq yz \). We can analogously prove that \( zx \geq zy \).

  If the ring is entire, strict inequalities follow from \fullref{thm:ordered_ring/strict_prod}.
  ...
  \Fullref{thm:ordered_ring/strict_prod} implies that, if the ring is entire, then \( y^{-1} < x^{-1} \).
  ...
  \Fullref{thm:ordered_ring/strict_prod} implies that, if the ring is entire, the inequality is strict.
  ...
  Now suppose that the ring is entire. Aiming at a contradiction, suppose that \( x^{n+1} = y^{n+1} \). \Fullref{thm:xn_minus_yn_factorization} implies that
  \begin{equation*}
    0
    =
    y^{n+1} - x^{n+1}
  ...
  \Fullref{thm:ordered_ring/nonnegative_power} implies that both \( x^k > 0 \) and \( y^{n-k} > 0 \) for every \( k = 0, \ldots, n \). \Fullref{thm:ordered_ring/strict_prod} implies that \( x^k y^{n-k} > 0 \cdot y^{n-k} = 0 \), and \fullref{thm:def:ordered_semiring/strict_sum} implies that
  \begin{equation*}
...
\begin{proof}
  \hfill
  \begin{itemize}
    \item If both \( x \) and \( y \) are positive, the proposition follows from \fullref{thm:def:ordered_semiring_positivity/positive_prod}.
    \item If \( x \) and \( y \) have different signs, the proposition follows from \fullref{thm:def:ordered_semiring_positivity/alternating_prod}.
    \item If \( x \) and \( y \) are both negative, then \( -x \) is positive, and the previous case implies that \( (-x)y \) is negative. \Fullref{thm:ordered_ring/additive_inversion} then implies that \( xy \) is positive.
  \end{itemize}
\end{proof}
...
\end{corollary}
\begin{proof}
  Follows from \fullref{thm:def:signum} by induction on \( n \).
\end{proof}
...
\begin{proof}
  ...
  \SubProofOf[def:order_function]{monotonicity} Follows from \fullref{thm:def:order_function/injective_implies_strict}.
