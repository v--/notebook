...
\begin{definition}\label{def:ring}\mcite[def. 2.1]{Jacobson1985BasicAlgebraI}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:ring/submodel}\mcite[105]{Jacobson1985BasicAlgebraI} The set \( A \subseteq R \) is a \hyperref[def:first_order_submodel]{submodel} of \( R \) if it is a \hyperref[def:semiring]{sub-semiring} of \( R \).

    As a consequence of \cref{thm:positive_formulas_preserved_under_homomorphism}, the image of a ring homomorphism is a subring of its codomain.
    ...
    \thmitem{def:ring/kernel}\mimprovised \Cref{thm:ring_zero_morphisms/kernel} implies that the \hyperref[def:zero_morphisms/kernel]{categorical kernel} of a morphism \( \varphi: R \to T \) in \( \ucat{Rng} \) is the additive group kernel
    \begin{equation*}
...
\end{comments}
\begin{proof}
  \SubProofOf{thm:ring_zero_morphisms/kernel} Follows from \cref{thm:zero_morphisms_pointed/kernel}.
  \SubProofOf{thm:ring_zero_morphisms/cokernel} Analogously to \cref{thm:group_zero_morphisms/cokernel}, the statement follows from \cref{thm:zero_morphisms_pointed/cokernel} by noting that \( \braket{ \varphi[A] } \) is the smallest ideal whose congruence \( \cong \) satisfies \( y \cong 0_T \) if and only if \( y \in \varphi[R] \).

  \SubProofOf{thm:ring_zero_morphisms/image} Follows from \cref{thm:zero_morphisms_pointed/image}.
...
\begin{algorithm}[Logical term to integer polynomial]\label{alg:logical_term_to_integer_polynomial}
  ...
  \begin{equation}\label{eq:alg:logical_term_to_polynomial}
    f_\tau \coloneqq \begin{cases}
      ...
      f_\tau \cdot f_\rho, &\tau = \sigma \syncdot \rho.
    \end{cases}
  \end{equation}
\end{algorithm}
\begin{comments}
  ...
  \item This algorithm can be found as \identifier{math.logic.theories.semirings.term_to_polynomial.term_to_polynomial} in \cite{notebook:code}.
\end{comments}
\begin{defproof}
...
\begin{algorithm}[Integer polynomial to logical term]\label{alg:integer_polynomial_to_logical_term}
  ...
  \begin{thmenum}
    ...
    \begin{equation}\label{eq:alg:logical_term_to_polynomial/number}
    ...
    \begin{equation}\label{eq:alg:logical_term_to_polynomial/monomial}
    ...
    \begin{equation}\label{eq:alg:logical_term_to_polynomial/polynomial}
      \tau_f \coloneqq \begin{cases}
        \tau_a,                              &f(\mscrX) = a \in \BbbZ, \\
        \tau_{\mscrX^\nu},                   &f(\mscrX) = a \mscrX^\nu \T{and} a = 1, \\
        \tau_a \syncdot \tau_{\mscrX^\nu},   &f(\mscrX) = a \mscrX^\nu \T{and} a \neq 1, \\
...
\begin{comments}
  ...
  \item This algorithm can be found as \identifier{math.logic.theories.semirings.polynomial_to_term.polynomial_to_term} in \cite{notebook:code}.
\end{comments}
\begin{defproof}
