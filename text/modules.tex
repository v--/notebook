...
\begin{definition}\label{def:module}\mcite[def. III.5.2]{Aluffi2009Algebra}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:module/category}\mimprovised For a fixed ring \( R \), we denote the \hyperref[def:category_of_first_order_models]{category of \( \mscrU \)-small models} by \( \ucat{Mod}_R \).
    ...
    \begin{itemize}
      ...
      \item The \hyperref[def:free_semimodule]{free semimodules} over a ring are modules, and \fullref{thm:free_semimodule_universal_property} ensures that this is left adjoint to the forgetful functor \( U: \ucat{Mod}_R \to \ucat{Set} \). Therefore, by \cref{thm:first_order_categorical_invertibility}, the monomorphisms are exactly the injective homomorphisms, and that the \hyperref[def:subobject_and_quotient]{categorical subobjects} correspond to submodules.
    ...
    \thmitem{def:module/quotient}\mimprovised Similarly to groups, \hyperref[def:first_order_congruence]{first-order congruences} for modules are well-behaved. \Cref{thm:submodules_and_congruences} shows that congruences correspond exactly to submodules.
