...
\begin{definition}\label{def:module}\mcite[def. III.5.2]{Aluffi2009Algebra}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:module/homomorphism}\mcite[160]{Aluffi2009Algebra} A \hyperref[def:fol_homomorphism]{first-order homomorphism} between two \( R \)-modules \( M \) and \( N \) is simply a \hyperref[def:linear_function]{linear map}.
...
\end{definition}
\begin{comments}
  \item Since the members of \( R^n \) are \hyperref[def:ordered_tuple]{ordered tuples}, and we often regard them as \hyperref[def:array/column_vector]{column vectors} or, less often, \hyperref[def:array/row_vector]{row vectors}. See \cref{rem:vector_etymology} for a more detailed discussion of terminology for vectors.
\end{comments}
