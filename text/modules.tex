...
\begin{definition}\label{def:module}
  A \term[bg=модул (\cite[27]{КоцевСидеров2016}, ru=модуль (\cite[def. 9.3.1]{Винберг2014})]{module} is a \hyperref[def:semimodule]{semimodule} over a \hyperref[def:ring]{ring} rather than a \hyperref[def:semiring]{semiring}.
...
\begin{proof}
  ...
  To show that it is an \( R \)-module congruence, we must show that \( x \cong y \) implies \( rx \cong ry \) every scalar \( r \in R \)\footnote{This is conceptually different from \fullref{thm:submodules_and_congruences/submodule_to_cong} because, in terms of first-order logic, we quantify by functional symbols rather than individuals}. This holds because of distributivity:
  \begin{equation*}
    rx - ry
...
\begin{definition}\label{def:module_presentation}\mimprovised
  A \term{presentation} of the \( R \)-\hyperref[def:module]{module} \( M \) is an \hyperref[def:module/homomorphism]{epimorphism} \( \varphi: R^{\oplus A} \to M \), where \( R^{\oplus A} \) is the \hyperref[def:free_semimodule]{free semimodule} on \( A \).
...
\begin{proof}
  ...
  By \fullref{thm:free_semimodule_universal_property}, there exists a unique \( R \)-linear map \( \varphi: R^{\oplus A} \to M \) such that
  \begin{equation*}
  ...
  \Fullref{thm:first_order_categorical_invertibility/surjective} then implies that \( \varphi \) is an epimorphism.
\end{proof}
...
\begin{definition}\label{def:hamel_basis}\mimprovised
  Let \( M \) be a left \( R \)-\hyperref[def:module]{module} and fix a subset \( E \subseteq M \). We say that \( E \) is a \term{Hamel basis} or simply \term[bg=базис (\cite[\textnumero 2.1.5]{ПетровЗяпков2010}), ru=базис (\cite[sec. 3.7]{Тыртышников2007})]{basis} of \( M \) if any of the following equivalent conditions hold:
...
\begin{definition}\label{def:sequence_space}\mimprovised
  ...
  We call the \hyperref[def:free_semimodule]{free module} \( R^\alpha \) a \term{sequence space} or, in case \( \alpha \) is finite, also a \term[bg=координатно пространство (\cite[\textnumero 2.1.2]{ПетровЗяпков2010})]{coordinate space}.
...
\begin{definition}\label{def:vector_space}\mcite[def. III.5.5]{Aluffi2009}
  A \term[ru=линейно пространство (\cite[\textnumero 4.1.3]{ПетровЗяпков2010}), ru=векторное/линейное пространство (\cite[def. 1.6.1]{Винберг2014})]{vector space} is a \hyperref[def:module]{left module} over a \hyperref[def:field]{field}.
...
\begin{definition}\label{def:vector_space_dimension}\mimprovised
  For a \( \BbbK \)-\hyperref[def:vector_space]{vector space} \( V \), we define its \term[bg=размерност (\cite[\textnumero 4.2.3]{ПетровЗяпков2010}), ru=размерность (\cite[sec. 3.7]{Тыртышников2007})]{dimension} \( \dim V \) as the \hyperref[thm:cardinality_existence]{cardinality} of any of its bases.
\end{definition}
\begin{defproof}
  \item \Fullref{thm:vector_space_basis_existence} shows every vector space has a basis.
...
\begin{definition}\label{def:module_rank}\mimprovised
  If an \( R \)-module has at least one base, we define its \term{rank} as the \hyperref[thm:cardinality_existence]{cardinality} of any of its bases.
\end{definition}
\begin{comments}
