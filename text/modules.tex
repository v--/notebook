...
\begin{definition}\label{def:module}
  ...
  \begin{thmenum}[series=def:module]
    ...
    \Cref{thm:module_absorption_conditions} implies that the absorption law \eqref{eq:def:semimodule/operation/absorption} becomes redundant.
  \end{thmenum}
  ...
  \begin{thmenum}
    ...
    \begin{itemize}
      ...
      \item With the above left adjoint, \cref{thm:concrete_category_function_invertibility/injective} implies that an \( R \)-linear map is a \hyperref[def:morphism_invertibility/left_cancellative]{monomorphism} if and only if it is \hyperref[def:function_invertibility/injective]{injective}.
...
\begin{proposition}\label{thm:submodules_and_congruences}
  Fix an \( R \)-\hyperref[def:module]{module} \( M \). \hyperref[def:fol_congruence]{First order congruences} on \( M \) and \( R \)-\hyperref[def:module/subobject]{submodules} of \( M \) are related as follows:
  \begin{thmenum}
...
\begin{proposition}\label{thm:kernel_is_submodule}
  The \hyperref[def:module/kernel]{kernel} of an \( R \)-\hyperref[def:linear_function]{linear map} is an \( R \)-submodule of its domain.
\end{proposition}
\begin{proof}
...
\begin{proposition}\label{thm:module_zero_morphisms}
  Let \( M \) and \( N \) be \( R \)-modules and let \( \varphi: M \to N \) be an \( R \)-\hyperref[def:linear_function]{linear map}.
