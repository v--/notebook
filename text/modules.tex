...
\begin{example}\label{ex:def:hamel_basis}
  ...
  \begin{thmenum}
    ...
    \thmitem{ex:def:hamel_basis/polynomial_ring} In \cref{def:polynomial_algebra}, we have defined a polynomial as a \hyperref[def:free_semimodule]{free \( R \)-module} over the set of all monomials.
...
\begin{definition}\label{def:ordered_hamel_basis}\mcite[79]{FriedbergInselSpence2018LinearAlgebra}
  We refer to any  \hyperref[def:enumeration]{enumeration} of a \hyperref[def:hamel_basis]{Hamel basis} as an \term{ordered basis}.
\end{definition}
...
\begin{proof}
  ...
  Suppose that it is not a spanning set and let \( x \in V \setminus \linspan A \). By \cref{thm:def:vector_space/span_independent}, the set \( A \cup \set{ x } \) is linearly independent, contradicting the maximality of \( A \).
