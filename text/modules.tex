...
\begin{definition}\label{def:module}\mcite[def. III.5.2]{Aluffi2009Algebra}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:module/kernel}\mimprovised \Cref{thm:module_zero_morphisms/kernel} implies that the \hyperref[def:zero_morphisms/kernel]{categorical kernel} of an \( R \)-linear map \( \varphi: M \to N \) between \( R \)-modules is the additive group kernel
    \begin{equation*}
...
\end{comments}
\begin{proof}
  \SubProofOf{thm:module_zero_morphisms/kernel} Follows from \cref{thm:zero_morphisms_pointed/kernel}.
  \SubProofOf{thm:module_zero_morphisms/cokernel} Analogously to \cref{thm:group_zero_morphisms/cokernel}, the statement follows from \cref{thm:zero_morphisms_pointed/cokernel} by noting that \( \braket{ \varphi[A] } \) is the submodule whose congruence \( \cong \) satisfies \( y \cong 0_N \) if and only if \( y \in \varphi[M] \).

  \SubProofOf{thm:module_zero_morphisms/image} Follows from \cref{thm:zero_morphisms_pointed/image}.
