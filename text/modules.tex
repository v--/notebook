...
\begin{definition}\label{def:sequence_space}\mimprovised
  ...
  We call the \hyperref[def:free_semimodule]{free module} \( R^\alpha \) a \term{sequence space} or, in case \( \alpha \) is finite, also a \term{coordinate space}.
...
\begin{proposition}\label{thm:commutative_module_rank}
  ...
  We define the \term{rank} of a module as the \hyperref[thm:cardinality_existence]{cardinality} of any of its bases. This is a generalization of \hyperref[thm:vector_space_dimension]{vector space dimensions}. Unlike vector space dimensions, however, module ranks may not exist --- see \fullref{ex:def:hamel_basis/module_without_basis}. Modules with at least one basis are will called \term{free}, however we will prefer to use the term for modules with a concrete bases, as in \fullref{def:free_semimodule}.
\end{proposition}
\begin{proof}
...
\begin{theorem}[Rank-nullity theorem]\label{thm:rank_nullity_theorem}
  ...
  The dimension of the kernel of \( \varphi \) is will called the \term{nullity} of \( \varphi \) and the dimension of the image - the \term{rank} of \( \varphi \)
\end{theorem}
\begin{proof}
