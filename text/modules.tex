...
\begin{definition}\label{def:module}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:module/category}\mcite[158]{Aluffi2009Algebra} For a fixed semiring \( R \), the first-order theories of left and right \( R \)-modules give rise to \hyperref[def:fol_theory_model_functor/obj]{categories of small models}. We denote them by \( \cat[R]{Mod} \) and \( \cat{Mod}[R] \) respectively.

    It is a very well-behaved category, even more than the category \hyperref[def:group/category]{\( \cat{Grp} \)} of small groups.
    \begin{itemize}
      \item Just as with \( R \)-semimodules, \fullref{thm:free_semimodule_universal_property} implies that \hyperref[def:free_semimodule]{free modules} provide a \hyperref[def:adjoint_functor]{left adjoint} to the \hyperref[def:fol_theory_forgetful_functor/set]{forgetful functor}
      \begin{equation*}
        U: \cat[R]{Mod} \to \cat{Set}.
      \end{equation*}

      \item With the above left adjoint, \cref{thm:concrete_category_function_invertibility/injective} implies that an \( R \)-module homomorphism is a \hyperref[def:morphism_cancellation/left]{monomorphism} if and only if it is \hyperref[def:function_invertibility/injective]{injective}.

      \item Every epimorphism in \( \cat[R]{Mod} \) is surjective. This will be shown in \cref{thm:module_epimorphisms_are_surjective}.

      \Cref{thm:category_of_modules_is_balanced} then implies that the category \( \cat[R]{Mod} \) is \hyperref[def:balanced_category]{balanced}.

      \item The zero \( R \)-semimodules, which we will define in \cref{def:semimodule/trivial}, are \hyperref[def:universal_objects/zero]{zero objects} in the categories \( \cat[R]{Mod} \) and \( \cat{Mod}[R] \).
      ...
      \item Another consequence of the theory being equational is that, by \cref{thm:fol_equational_category_of_models_is_complete}, \( \cat[R]{Mod} \) is \hyperref[def:complete_category]{complete}.

      Furthermore, \cref{thm:category_of_small_semimodules_is_cocomplete} implies that \( \cat[R]{Mod} \) is also \hyperref[def:cocomplete_category]{cocomplete}.
    \end{itemize}
    ...
    In accordance with \cref{def:trivial_object}, due to \cref{thm:equational_theory_zero_morphisms/trivial}, it is a \hyperref[def:universal_objects/zero]{zero object} in the categories \( \cat[R]{Mod} \) and \( \cat{Mod}[R] \).
    ...
    \thmitem{def:module/simple}\mcite[212]{Bourbaki1998Algebra1to3} We say that a \emph{nonzero} \( R \)-module is \term{simple} if its only \( R \)-submodules are the module itself and its zero submodule.
...
\begin{remark}\label{rem:category_of_modules_notation}
  To distinguish the categories of left and right \( R \)-modules, it is convenient to write \( \cat[R]{Mod} \) and \( \cat[R]{Mod} \). This notation is used in
  \cite[94]{Jacobson1989BasicAlgebraII},
  \cite[158]{Aluffi2009Algebra} and
  \cite[12]{MacLane1998CategoryTheory}.

  These categories are also denoted by \( \cat{{}_R Mod} \) and \( \cat{Mod_R} \), for example in
  \cite[446]{Rotman2015AdvancedModernAlgebraPart1} and
  \cite[12]{Schubert1972Categories}.
\end{remark}
...
\begin{proposition}\label{thm:abelian_group_is_module}
  The \hyperref[def:category]{categories} \hyperref[def:abelian_group]{of abelian groups} and \hyperref[def:module/category]{of \( \BbbZ \)-semimodules} are \hyperref[def:isomorphism_of_categories]{isomorphic}.
...
\begin{proposition}\label{thm:module_categorical_kernels}
  In the category \hyperref[def:module/category]{\( \cat[R]{Mod} \)} of left \( R \)-modules or \hyperref[def:ring/category]{\( \cat{Rng} \)}), \hyperref[def:module/kernel]{kernels} and \hyperref[def:module_cokernel]{cokernels} related to the corresponding categorical notions from \cref{def:categorical_kernels} as follows:
  \begin{thmenum}
...
\begin{proposition}\label{thm:module_epimorphisms_are_surjective}
  Every \hyperref[def:fol_epimorphism]{epimorphism} in \hyperref[def:module/category]{\( \cat[R]{Mod} \)} is \hyperref[def:function_invertibility/surjective]{surjective}.
\end{proposition}
\begin{proof}
...
\begin{corollary}\label{thm:category_of_modules_is_balanced}
  The category \( \hyperref[def:module/category]{\cat[R]{Mod}} \) of small left \( R \)-modules is \hyperref[def:balanced_category]{balanced}.
\end{corollary}
\begin{proof}
  Follows from \cref{thm:fol_equational_theory_balanced_category} in light of \cref{thm:module_epimorphisms_are_surjective}.
\end{proof}
...
\begin{example}\label{ex:def:module_torsion}
  ...
  \begin{thmenum}
    ...
    \begin{equation*}
      2 \cdot 3 \equiv 3 \cdot 2 \equiv 0 \pmod 6,
    \end{equation*}
    ...
    \begin{equation*}
      3 \cdot 2 \equiv 3 \cdot 4 \equiv 0 \pmod 6.
    \end{equation*}
  \end{thmenum}
\end{example}
