...
\begin{definition}\label{def:endomorphism_ring}\mcite[102]{Bourbaki1998Algebra1to3}
  In the \hyperref[def:endomorphism_semiring]{endomorphism semiring} \( \End(G) \) over an \hyperref[def:abelian_group]{abelian group} \( G \), we can extend additive inverses from \( G \) to \( \End(G) \).

  We call \( \End(X) \) as the \term[ru=кольцо эндоморфизмов (\cite[130]{Курош1973ОбщаяАлгебра})]{endomorphism ring}.
\end{definition}
...
\begin{definition}\label{def:module}\mcite[def. III.5.2]{Aluffi2009Algebra}
  ...
  This makes the identity law \eqref{eq:def:semimodule/operation/action/identity} redundant.
  ...
  \begin{thmenum}
    ...
    \thmitem{def:module/homomorphism}\mcite[160]{Aluffi2009Algebra} A \hyperref[def:fol_homomorphism]{first-order homomorphism} between two \( R \)-modules \( M \) and \( N \) is simply a \hyperref[def:semimodule/morphism]{linear map}.

    \thmitem{def:module/submodel}\mcite[162]{Aluffi2009Algebra} The set \( A \subseteq M \) is a \hyperref[def:fol_substructure/submodel]{first-order submodel} of \( M \) if it is a sub-semimodule of \( M \), i.e. a subgroup of \( M \) that is closed under scalar multiplication. We say that \( A \) is an \( R \)-\term{submodule} of \( M \).

    As a consequence of \cref{thm:fol_equational_theory_models/image}, the image of a module homomorphism is a submodule of its codomain.

    \thmitem{def:module/category}\mimprovised For a fixed ring \( R \), we denote the \hyperref[def:fol_theory_model_functor/obj]{category of \( \mscrU \)-small models} by \( \ucat{Mod}_R \).

    Other notations are in use, for example \( R-\cat{Mod} \) for \( R \)-modules by \textcite[158]{Aluffi2009Algebra}, that better highlight whether we are considering left or right (semi)modules. We will prefer \( \cat{Mod}_R^\oppos \) for the category of right \( R \)-modules.
    ...
    \begin{itemize}
      ...
      \item The \hyperref[def:free_semimodule]{free semimodules} over a ring are modules, and \fullref{thm:free_semimodule_universal_property} ensures that this is left adjoint to the forgetful functor \( U: \ucat{Mod}_R \to \ucat{Set} \). Therefore, by \cref{thm:first_homomorphism_invertibility}, the monomorphisms are exactly the injective homomorphisms, and that the \hyperref[def:subobject_and_quotient]{categorical subobjects} correspond to submodules.

      \item Every epimorphism in \( \ucat{Mod}_R \) is surjective. This will be proved in \cref{thm:module_epimorphisms_are_surjective}. Along with \cref{thm:surjective_group_homomorphisms_are_cokernels}, this shows that the \hyperref[def:subobject_and_quotient]{categorical quotient objects} correspond to \hyperref[def:module/quotient]{quotient modules}, which we will discuss shortly.
    \end{itemize}

    \thmitem{def:module/trivial}\mimprovised Any single-element \( R \)-module, that is, every zero object in \( \ucat{Mod}_R \), is as trivial object in the sense of \cref{def:trivial_object}. We will call it \enquote{the} trivial \( R \)-module.

    \thmitem{def:module/kernel}\mimprovised \Cref{thm:module_zero_morphisms/kernel} implies that the \hyperref[def:categorical_kernels/kernel]{categorical kernel} of an \( R \)-linear map \( \varphi: M \to N \) between \( R \)-modules is the additive group kernel
    \begin{equation*}
      \ker \varphi \coloneqq \varphi^{-1}(0_T) = \set{ x \in R \given \varphi(x) = 0_T }.
    \end{equation*}

    It is a submodule as a consequence of \cref{thm:kernel_is_submodule}. It is also called the \term[ru=нуль-пространство (матрицы) (\cite[112]{Тыртышников2007ЛинейнаяАлгебра}), en=null space (\cite[67]{FriedbergInselSpence2018LinearAlgebra})]{null space} of \( \varphi \), especially for \hyperref[def:vector_space]{vector spaces}, where it is a linear subspace.

    \thmitem{def:module/quotient}\mimprovised Similarly to groups, \hyperref[def:fol_congruence]{first-order congruences} for modules are well-behaved. \Cref{thm:submodules_and_congruences} shows that congruences correspond exactly to submodules.
...
\begin{proposition}\label{thm:abelian_group_is_module}
  ...
  More concretely, every abelian group \( G \) is a left module over \( \BbbZ \) with scalar multiplication given by \hyperref[def:additive_semigroup/iterated]{recursively defined multiplication}
...
\begin{proposition}\label{thm:submodules_and_congruences}
  Fix an \( R \)-\hyperref[def:module]{module} \( M \). \hyperref[def:fol_congruence]{First order congruences} on \( M \) and \( R \)-\hyperref[def:module/submodel]{submodules} of \( M \) are related as follows:
  \begin{thmenum}
...
\end{comments}
\begin{proof}
  \SubProofOf{thm:submodules_and_congruences/cong_to_submodule} Since module congruences are also congruences of the additive group, \cref{thm:normal_subgroups_and_congruences/cong_to_subgroup} implies that \( [0] \) is a \hyperref[def:normal_subgroup]{normal subgroup} of the additive group of \( M \).

  \Cref{def:semimodule/operation/absorption} implies that \( 0 \) absorbs scalars, i.e. \( r \cdot 0 = 0 \) for every \( r \) in \( R \), thus \( [0] \) is indeed an \( R \)-submodule.
  ...
  To show that it is an \( R \)-module congruence, we must show that \( x \cong y \) implies \( rx \cong ry \) every scalar \( r \in R \)\fnote{This is conceptually different from \cref{thm:submodules_and_congruences/submodule_to_cong} because, in terms of first-order logic, we quantify by functional symbols rather than individuals}. This holds because of distributivity:
  \begin{equation*}
    rx - ry
  ...
  \SubProofOf{thm:submodules_and_congruences/inverse} This is a special case of \cref{thm:normal_subgroups_and_congruences/inverse}.
\end{proof}
...
\begin{proposition}\label{thm:module_zero_morphisms}
  ...
  \begin{thmenum}
    \thmitem{thm:module_zero_morphisms/kernel} The \hyperref[def:categorical_kernels/kernel]{categorical kernel} \( \ker \varphi \) of \( \varphi \) is the preimage of \( 0_N \).

    \thmitem{thm:module_zero_morphisms/cokernel} The \hyperref[def:categorical_kernels/kernel]{categorical cokernel} \( \co\ker \varphi \) of \( \varphi \) is the \hyperref[def:group/quotient]{quotient} of \( N \) by the the \hyperref[def:set_valued_map/image]{set-theoretic image} \( \varphi[M] \).

    \thmitem{thm:module_zero_morphisms/image} The \hyperref[def:categorical_kernels/image]{categorical image} \( \img \varphi \) of \( \varphi \) is the \hyperref[def:set_valued_map/image]{set-theoretic image} \( \varphi[M] \).

    \thmitem{thm:module_zero_morphisms/coimage} The \hyperref[def:categorical_kernels/coimage]{categorical coimage} of \( \varphi \) is the \hyperref[def:module/quotient]{quotient} of \( M \) by the \hyperref[def:module/kernel]{kernel} \( \ker \varphi \).
...
\end{proposition}
\begin{comments}
  \item This is based on a similar statement for \hyperref[def:group]{groups} --- see \cref{thm:group_zero_morphisms}. Another similar statement holds for (nonunital) \hyperref[def:ring]{rings} --- see \cref{thm:ring_zero_morphisms}.
\end{comments}
\begin{proof}
  \SubProofOf{thm:module_zero_morphisms/kernel} Follows from \cref{thm:equational_theory_zero_morphisms/kernel}.
  \SubProofOf{thm:module_zero_morphisms/cokernel} Analogously to \cref{thm:group_zero_morphisms/cokernel}, the statement follows from \cref{thm:equational_theory_zero_morphisms/cokernel} by noting that \( \braket{ \varphi[A] } \) is the submodule whose congruence \( \cong \) satisfies \( y \cong 0_N \) if and only if \( y \in \varphi[M] \).

  \SubProofOf{thm:module_zero_morphisms/image} Follows from \cref{thm:equational_theory_zero_morphisms/image}.
  \SubProofOf{thm:module_zero_morphisms/coimage} Analogously to \cref{thm:group_zero_morphisms/coimage}, we conclude that the coimage, defined as the cokernel of the kernel of \( \varphi \), is the quotient of \( M \) by \( \ker \varphi \), the latter being a submodule as a consequence of \cref{thm:kernel_is_ideal}.

  \SubProofOf{thm:module_zero_morphisms/isomorphism} \Cref{thm:homomorphism_theorem_for_groups} implies that \( \psi \) as defined in \eqref{eq:thm:module_zero_morphisms/isomorphism} is an isomorphism between the additive groups of \( R / \ker \varphi \) and \( \img \varphi \). Furthermore, \( \psi \) is a ring isomorphism because \( \varphi \) is.
\end{proof}

\begin{proposition}\label{thm:module_epimorphisms_are_surjective}
  Every \hyperref[def:fol_epimorphism]{epimorphism} in \hyperref[def:group/category]{\( \cat{Mod}_R \)} is \hyperref[def:function_invertibility/surjective]{surjective}.
\end{proposition}
\begin{proof}
  Let \( \varphi: M \to N \) be an \( R \)-module epimorphism. Consider the canonical projection \( \pi: N \to N / \img \varphi \) and the parallel \hyperref[def:zero_morphism]{zero morphism} \( z: N \to N / \img \varphi \). Clearly
  \begin{equation*}
    \pi \bincirc \varphi = z \bincirc \varphi.
  \end{equation*}

  Since \( \varphi \) is an epimorphism, it follows that \( \pi = z \). Thus, the image of \( \pi \) is trivial. But the image of \( \pi \) is the quotient \( N / \img \varphi \). \Cref{thm:group_homomorphism_zero_cokernel} then implies that \( \varphi \) is surjective.
\end{proof}
...
\begin{definition}\label{def:internal_module_direct_sum}
  \todo{}
\end{definition}

\begin{proposition}\label{thm:internal_binary_semimodule_direct_sum}
  Fix \( R \)-submodules \( A \) and \( B \) of the \( R \)-\hyperref[def:module]{module} \( M \), and, on for \hyperref[def:external_semimodule_direct_sum]{direct sum} \( A \oplus B \), consider the map
  \begin{equation*}
    \begin{aligned}
      &\Phi: A \oplus B \to M, \\
...
\end{proposition}
\begin{comments}
  \item Here \( A + B \) is the \hyperref[def:power_semigroup]{Minkowski sum} \( \set{ a + b \given a \in A \T{and} b \in B } \).
...
\begin{proof}
  ...
  This is clearly a \hyperref[def:semimodule/morphism]{linear map}, i.e. a homomorphism. We will show that it is bijective, i.e. an isomorphism.

  \SubProofOf[def:function_invertibility/injective/equality]{injectivity} If \( \Phi(a, b) = \Phi(a', b') \), then \( a + b = a' + b' \) and \( a - a' = b' - b \).
  ...
  \SubProofOf[def:function_invertibility/surjective/existence]{surjectivity} Fix some vector \( x \) in \( M \).
...
\begin{proof}
  ...
  \SubProofOf{thm:def:linear_dependence/span_dependent} By \cref{thm:span_via_linear_combinations}, there exists a linear combination of members of \( E \) such that
  \begin{equation*}
...
\begin{definition}\label{def:hamel_basis}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:hamel_basis/free} \( M \) is \hyperref[def:semimodule/morphism]{linearly isomorphic} to the \hyperref[def:free_semimodule]{free module} \( R^{\oplus E} \).
  \end{thmenum}
\end{definition}
\begin{comments}
...
\begin{definition}\label{def:coordinate_space}\mimprovised
  ...
  \Cref{thm:basis_of_direct_sum} shows that the vectors \( e_1, \ldots, e_n \), in which the \( i \)-th component of the tuple \( e_i \) is \( 1 \) and all others are \( 0 \), is an \hyperref[def:ordered_hamel_basis]{ordered} \hyperref[def:hamel_basis]{Hamel basis}, which we call the \term{standard basis} of \( R^n \).
\end{definition}
\begin{comments}
...
\begin{remark}\label{rem:vector}
  ...
  \begin{thmenum}
    ...
    In the aforementioned books, free vectors are defined with respect to an informal ambient \hyperref[rem:euclidean_plane]{Euclidean plane}. The construction is meaningless in affine spaces, where free vectors are taken as part of the definition. In fact, when introducing affine spaces. \textcite[\S 2.1.3]{Berger1987GeometryI} defines the \enquote{free vector} \( \vect{xy} \) associated with the \enquote{bound vector} \( (x, y) \) as the unique element of \( \vect A \) such that \( \tau_{\vect{xy}}(x) = y \).

    The term \enquote{free vector} is also used for elements of \( \vect A \) in \cite[def. 2.9.1]{Bourbaki1998Algebra1to3}.
    ...
    \cite[def. 2.1.2]{Bourbaki1998Algebra1to3} and
    ...
    \cite[def. 1.1]{Stewart1998MatrixAlgorithmsVol1},
    \cite[3]{BoydVandenberghe2018AppliedLinearAlgebra} and
    \cite[37]{Тыртышников2007ЛинейнаяАлгебра}.
...
\begin{proposition}\label{thm:modules_with_same_rank_are_isomorphic}
  Two \( R \)-modules having the same \hyperref[def:module_rank]{rank} are \hyperref[def:semimodule/morphism]{linearly isomorphic}.
\end{proposition}
\begin{proof}
...
\begin{definition}\label{def:rank_and_nullity}\mcite[70]{FriedbergInselSpence2018LinearAlgebra}
  For a \hyperref[def:semimodule/morphism]{linear map} \( \varphi: U \to V \), we call the dimension of the kernel of \( \varphi \) the \term[ru=дефект (\cite[250]{Тыртышников2007ЛинейнаяАлгебра})]{nullity} of \( \varphi \) and the dimension of the image - the \term[ru=ранг (\cite[250]{Тыртышников2007ЛинейнаяАлгебра})]{rank} of \( \varphi \)
\end{definition}

\begin{theorem}[Rank-nullity theorem]\label{thm:rank_nullity_theorem}
  For every \hyperref[def:semimodule/morphism]{linear map} \( \varphi: U \to V \) between \hyperref[thm:vector_space_dimension]{finite-dimensional} \hyperref[def:vector_space]{vector spaces}, we have
...
\end{theorem}
\begin{proof}
  By \cref{thm:module_zero_morphisms/isomorphism},
  \begin{equation*}
