...
\begin{definition}\label{def:dedekind_cut}\mimprovised
  A \term[ru=сечение (\cite[34]{Тагамлицки1971ДиференциалноСмятане}), ru=сечение (\cite[34]{Александров1977Топология}), en=Dedekind cut (\cite[39]{Jech2006Sets})]{Dedekind cut} in an \hyperref[def:extremal_points/bounds]{unbounded} \hyperref[def:totally_ordered_set]{totally ordered set} \( (P, \leq) \) is pair \( (A, B) \) of \hi{nonempty} sets that \hyperref[def:set_partition]{partitions} \( P \), such that \( a < b \) for every \( a \in A \) and \( b \in B \).
