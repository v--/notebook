...
\begin{comments}
  ...
  \item We use the original definition given by \incite[sec. I.IV]{Beman1901DedekindEssays} (although in greater generality). His definition has been refined, for example by \incite[113]{Enderton1977Sets} and \incite[17]{Rudin1976Principles}, which defines a cut as a \hyperref[def:closed_ordered_subset]{downward closed} nonempty set \( A \) with no maximum (which uniquely defines \( B \) as its complement). We will not have use for such refinements.
\end{comments}
