...
\begin{definition}\label{def:dedekind_cut}\mimprovised
  A \term[ru=сечение (\cite[34]{Тагамлицки1971ДиференциалноСмятане}), ru=сечение (\cite[34]{Александров1977ОбщаяТопология}), en=Dedekind cut (\cite[39]{Jech2006SetTheory})]{Dedekind cut} in an \hyperref[def:extremal_points/bounds]{unbounded} \hyperref[def:totally_ordered_set]{totally ordered set} \( (P, \leq) \) is pair \( (A, B) \) of \emph{nonempty} sets that \hyperref[def:set_partition]{partitions} \( P \), such that \( a < b \) for every \( a \in A \) and \( b \in B \).
...
\begin{proposition}\label{thm:dedekind_completeness_unbounded_characterization}
  An unbounded totally ordered set is \hyperref[def:dedekind_completeness]{Dedekind complete} if and only if every \emph{bounded} \emph{nonempty} subset has both a supremum and an infimum.
\end{proposition}
\begin{proof}
  ...
  We can further assume that \( S \) has no maximum, and thus all upper bounds of \( S \) are strict. Consider the set \( B \) of all upper bounds of \( S \) and also the set \( A \) of all \emph{strict} lower bounds of \( B \).
...
\begin{definition}\label{def:macnielle_closure}\mimprovised
  ...
  This allows us to introduce the \hyperref[def:abstract_closure_operator]{closure operator} \( A \mapsto A^{UL} \), which we call the \term{MacNeille closure} of \( A \).
...
\end{definition}
\begin{comments}
  \item Like Dedekind, MacNeille developed a theory of completion based on cuts. \Textcite[48]{MacNeille1936OrderCompletion}, he generalized \hyperref[def:dedekind_cut]{Dedekind cuts} by considering pairs \( (A, B) \) of subsets of a partially ordered set in which every element in \( A \) is less than every element of \( B \), and requiring that every \hyperref[def:extremal_points/bounds]{lower bound} of \( B \) be in \( A \) and every upper bound of \( A \) be in \( B \).
...
\begin{proof}
  ...
  \SubProofOf{thm:def:macnielle_closure/ideal} Let \( A \) be an arbitrary subset of the lattice \( X \). \Cref{thm:def:macnielle_closure/downward_closed} shows that \( A^{UL} \) is downward closed. We will show that \( A^{UL} \) is upward directed. Then from \cref{thm:def:lattice_ideal/directed_and_closed} it will follow that \( A^{UL} \) is a lattice ideal.
...
\begin{theorem}[Dedekind-MacNeille completion universal property]\label{thm:dedekind_macneille_completion_universal_property}
  The \hyperref[def:dedekind_macnielle_completion]{Dedekind-MacNeille completion} \( D(P) \) of a \hyperref[def:partially_ordered_set]{partially ordered set} \( (P, \leq) \) satisfies the following \hyperref[con:universal_property]{universal property}:
  \begin{displayquote}
    For every \hyperref[def:lattice]{complete lattice} \( L \), every \hyperref[def:order_function/preserving]{order-preserving map} \( \varphi: P \to L \) \hyperref[def:factors_through]{uniquely factors through} \( D(P) \). More precisely, there exists a unique \hyperref[def:complete_lattice/morphism]{complete lattice homomorphism} \( \widetilde{\varphi}: D(P) \to L \) such that the following diagram commutes:
    \begin{equation*}
      \begin{aligned}
...
\end{theorem}
\begin{comments}
  \item \Cref{thm:hom_adjunction_from_universal_morphism/unit} implies thatthis functor becomes \hyperref[def:adjoint_functor]{left adjoint} to the \hyperref[con:forgetful_functor]{forgetful functor} from complete lattices to partially ordered sets.
\end{comments}
\begin{proof}
...
\begin{comments}
  ...
  Some other authors, for example \textcite[113]{Enderton1977SetTheory} (who bases his construction on Mendelson's) and \textcite[17]{Rudin1976AnalysisPrinciples}, define cuts similarly.
...
\begin{theorem}[Lower cut completion]\label{thm:unbounded_lower_cut_completion}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:unbounded_lower_cut_completion/dual_macneille} The complementation map \( A \mapsto P \setminus A \) is an \hyperref[def:partially_ordered_set/morphism]{order isomorphism} between the bounded completion \( L(P, \leq) \) of \( (P, \leq) \) and the \hyperref[def:dedekind_macnielle_completion]{Dedekind-MacNeille completion} \( D(P, \geq) \) of the \hyperref[def:partially_ordered_set/opposite]{dual ordered set} \( (P, \geq) \).
...
\begin{proof}
  ...
  Furthermore, \cref{thm:set_difference/complement} implies that the correspondence is a \hyperref[def:partially_ordered_set/morphism]{strict order homomorphism}, and \cref{thm:def:totally_ordered_set/embedding_iff_strict} implies that it is an \hyperref[def:partially_ordered_set/morphism]{order embedding}. Since it is surjective, we conclude that it is an order isomorphism.
