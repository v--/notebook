...
\begin{comments}
  ...
  We follow this construction since it is simpler to work with than both \hyperref[def:dedekind_cut]{Dedekind cuts}, which are pairs of sets for which equivalence need to be defined, and \hyperref[def:macneille_closure]{MacNeille-closed subsets}, which require case analysis based on whether they contain a supremum or not.
...
\begin{definition}\label{def:lower_cut_closure}\mimprovised
  In order to highlight the connection between \hyperref[def:macneille_closure]{MacNeille-closed} sets and \hyperref[def:lower_cut]{lower cuts}, we define the following mutually inverse operations:
...
\begin{proposition}\label{thm:def:lower_cut_closure}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:def:lower_cut_closure/closed} \hyperref[def:macneille_closure]{MacNille-closed} sets are fixed points of \( \sharp \).
...
\begin{proposition}\label{thm:lower_cut_characterization}
  ...
  Then \( \flat(A) \) is a \hyperref[def:lower_cut]{lower cut} if and only if \( \sharp(A) \) is \hyperref[def:macneille_closure]{MacNeille-closed}.
\end{proposition}
\begin{proof}
