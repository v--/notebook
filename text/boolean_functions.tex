...
\begin{definition}\label{def:boolean_closure}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:boolean_closure/complete}\mcite[30]{Яблонский2003ДискретнаяМатематика} If the closure \( \cl{B} \) is the entire set \( \mscrB \), we say that \( B \) is \term[ru=полная система, en=functionally complete (\cite[857]{Rosen2019DiscreteMathematics})]{complete}.
  \end{thmenum}
\end{definition}
\begin{comments}
...
\begin{proposition}\label{thm:complete_sets_of_boolean_functions}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:complete_sets_of_boolean_functions/nand} \( \set{ \uparrow } \), where \( (x \uparrow y) \coloneqq \oline{x \wedge y} \) is called \term[ru=штрих Шеффера (\cite[29]{Эдельман1975Логика}), en=Sheffer's stroke (\cite[40]{Hinman2005Logic})]{Sheffer's stroke} or \term[en=nand (\cite[40]{Hinman2005Logic})]{nand} (\enquote{not and}).
