...
\end{proposition}
\begin{comments}
  \item \enquote{XOR} is an abbreviation for \enquote{exclusive or}, while \enquote{iff} is an abbreviation for \enquote{if and only if}. Both can be found, for example, in \cite[14]{Rosen2018DiscreteHandbook}.
...
\begin{proposition}\label{thm:complete_sets_of_boolean_functions}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:complete_sets_of_boolean_functions/nand} \( \set{ \uparrow } \), where \( (x \uparrow y) \coloneqq \oline{x \wedge y} \) is called \term[ru=штрих Шеффера (\cite[29]{Эдельман1975Логика}), en=Sheffer's stroke (\cite[45]{Birkhoff1967Lattices})]{Sheffer's stroke} or \term[en=NAND (\cite[369]{Rosen2018DiscreteHandbook})]{nand} (\enquote{not and}).
