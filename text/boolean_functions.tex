...
\end{proposition}
\begin{comments}
  \item \Cref{ex:def:boolean_algebra/f2} shows how \( \BbbF_2 \) can be regarded as a \hyperref[def:boolean_algebra]{Boolean algebra}.
\end{comments}
\begin{proof}
...
\end{proposition}
\begin{proof}
  Existence is provided by \fullref{alg:infer_zhegalkin_polynomial}, while uniqueness is provided by \cref{thm:functions_over_finite_fields}.
\end{proof}
...
\end{definition}
\begin{comments}
  \item The last rows of the two tables contain \hyperref[def:zhegalkin_polynomial]{Zhegalkin polynomials} of the corresponding functions. \Cref{thm:unary_boolean_function_zhegalkin_polynomial} and \cref{thm:binary_boolean_function_zhegalkin_polynomial} were used to determine the coefficients.
