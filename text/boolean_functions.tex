...
\begin{remark}\label{rem:boolean_valued_functions_and_relations}
  ...
  We use this when we generalize equivalence relations beyond Boolean values in \cref{def:generalized_equivalence_relation}.
\end{remark}
...
\end{proposition}
\begin{proof}
  This is simply a restatement of \cref{alg:infer_zhegalkin_polynomial/step}.
\end{proof}
...
\begin{proof}
  ...
  The expressions for the concrete values for the coefficients follow from \cref{thm:unary_boolean_function_zhegalkin_polynomial} and \cref{alg:infer_zhegalkin_polynomial/step}.
\end{proof}
...
\begin{proof}
  ...
  For the relative pseudocomplement, \cref{thm:def:heyting_algebra/extrema_complement} implies that \( \oline{\bot} = \top \) and \( \oline{\top} = \bot \).

  \SubProofOf{thm:def:standard_boolean_functions/biconditional} By definition,
  \begin{equation*}
    (x \leftrightarrow y) = (x \rightarrow y) \wedge (y \rightarrow x).
  \end{equation*}

  \begin{itemize}
    \item Based on our discussion of \( \wedge \), \( (x \leftrightarrow y) = \semtop \) if and only if \( (x \rightarrow y) = (y \rightarrow x) = \semtop \).

    \item Based on our discussion of \( \rightarrow \), \( (x \rightarrow y) = (y \rightarrow x) \) is only possible if \( x = y \).
  \end{itemize}

  So \( (x \leftrightarrow y) = \semtop \) if and only if \( x = y \).
