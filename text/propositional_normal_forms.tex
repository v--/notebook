...
\begin{remark}\label{rem:uniform_substitution_notation}
  ...
  \begin{equation*}
    \varphi[\upsilon_1 \mapsto \sigma(\upsilon_1), \cdots, \upsilon_n \mapsto \sigma(\upsilon_n)]
  \end{equation*}

  When the variables \( \upsilon_1, \ldots, \upsilon_n \) are clear from the context, we write
  \begin{equation*}
    \varphi[\omega_1, \cdots, \omega_n]
  \end{equation*}
  as a shorthand for
  \begin{equation*}
    \varphi[\upsilon_1 \to \omega_1, \cdots, \upsilon_n \to \omega_n].
  \end{equation*}
...
\begin{proof}
  ...
  \end{equation*}
  where, for every \( k = 1, \ldots, n \), \( L_k \) is either \( \upsilon_k \) or \( \neg \upsilon_k \).
...
\begin{definition}\label{def:perfect_cnf_and_dnf}\mimprovised
  ...
  \begin{thmenum}
    ...
    More explicitly, if the variables are \( \upsilon_1, \ldots, \upsilon_n \), every elementary disjunction has the form
    \begin{equation*}
      L_1 \synvee L_2 \synvee \cdots \synvee L_n,
    \end{equation*}
    where, for every \( k = 1, \ldots, n \), \( L_k \) is either \( \upsilon_k \) or \( \neg \upsilon_k \).

    \thmitem{def:perfect_cnf_and_dnf/elementary} The elementary disjunctions (resp. conjunctions) are ordered \hyperref[def:lexicographic_order]{lexicographically} so that \( \upsilon_k < \neg \upsilon_k \) for every variable \( \upsilon_k \).
  \end{thmenum}
\end{definition}
\begin{comments}
