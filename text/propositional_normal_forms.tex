...
\begin{comments}
  ...
  \begin{itemize}
    ...
    This allows us to stay consistent with \cref{def:propositional_denotation}, where we make a distinction between interpretations and valuations
...
\begin{remark}\label{rem:substitution_symbol}
  ...
  \begin{itemize}
    ...
    \item Greek letters adjacent to \( \tau \), like \( \sigma \), are used for both \hyperref[def:fol_term]{first-order terms} and \hyperref[def:simple_type]{types}.
  \end{itemize}
\end{remark}
...
\begin{algorithm}[Formula to CNF or DNF]\label{alg:cnf_and_dnf}
  ...
  \begin{thmenum}
    ...
    \Cref{thm:propositional_substitution_equivalence} implies that, since \( \synp \synvee \synneg \synp \gleichstark \syntop \) and \( \synp \synwedge \synneg \synp \gleichstark \synbot \), then \( C_1(\varphi) \gleichstark \varphi \).
...
\end{algorithm}
\begin{comments}
  \item This algorithm can be found as \identifier{math.logic.cnf.to_cnf} in \cite{notebook:code}.
\end{comments}
\begin{defproof}
...
\begin{algorithm}[Boolean function to perfect CNF or DNF]\label{alg:full_cnf_and_dnf}\mcite[thm. I.1.3]{Яблонский2003ДискретнаяМатематика}
  ...
  We will build a formula \( \varphi \) in \hyperref[def:full_cnf_and_dnf]{perfect} \hyperref[def:cnf_and_dnf]{conjunctive normal form} whose \hyperref[def:propositional_denotation/function]{induced function} under \hyperref[def:truth_value_algebra/classical]{classical semantics} will be \( f \).
...
\end{algorithm}
\begin{comments}
  \item This algorithm can be found as \identifier{math.logic.cnf.function_to_cnf} in \cite{notebook:code}.
\end{comments}
\begin{defproof}
