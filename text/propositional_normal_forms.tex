...
\end{definition}
\begin{comments}
  \item The terms \enquote{literal}, \enquote{positive literal} and \enquote{negative literal} are used in \cite[7]{Rosen2018DiscreteHandbook}. The adjective \enquote{elementary} for conjunctions and disjunctions is based on \cite[36]{Эдельман1975Логика}. \incite[13]{Smullyan1995FOL} instead uses \enquote{basic conjunction}.
...
\begin{definition}\label{def:perfect_cnf_and_dnf}\mimprovised
  We say that the \hyperref[def:cnf_and_dnf]{CNF} (resp. \hyperref[def:cnf_and_dnf]{DNF}) is \term[ru=совершенная (\cite[def. 6.3; def. 6.4]{Эдельман1975Логика}), en=full (\cite[5]{Rosen2018DiscreteHandbook}) / strict (\cite[def. 1.3.10]{Hinman2005Logic})]{perfect} if the following hold:
  \begin{thmenum}
