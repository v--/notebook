...
\begin{comments}
  ...
  \begin{itemize}
    ...
    \item The \enquote{minimal logic} fragment, in which only the falsum \( \synbot \) is disallowed from the propositional syntax. We will find it simpler to generalize our semantics for minimal logic than to restrict the syntax. See \cref{def:minimal_propositional_denotation}.
  \end{itemize}
...
\begin{theorem}[Minimal implicational logic soundness]\label{thm:minimal_implicational_logic_soundness}
  \hyperref[def:minimal_implication_logic]{Minimal implicational logic} is \hyperref[def:general_logic]{sound} with respect to \hyperref[def:minimal_propositional_denotation]{minimal semantics}.
\end{theorem}
\begin{comments}
