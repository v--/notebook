...
\end{concept}
\begin{comments}
  \item \Textcite[\S 3.1]{CitkinMuravitsky2022ConsequenceRelations} describe a similar idea --- they first define \enquote{metaformulas} that roughly correspond to what we call formula schemas, and later define an instantiation map. \Textcite[20]{Эдельман1975Логика} only briefly mentions schemas without formalizing instantiations. A vaguely similar idea is described in the context of type inference by \textcite[\S 4.4.4]{Mimram2020ProgramEqualsProof}. \Textcite[def. 2.1.18]{Barendregt1984LambdaCalculus} defines a \enquote{context} as a term with \enquote{holes}, which roughly corresponds to \( \muplambda \)-term schemas with \( \muplambda \)-term placeholders.
...
\begin{comments}
  ...
  \item Schemas provide little benefit over formulas in propositional logic, but become useful in \( \muplambda \)-calculus and first-order logic, where substitutions on terms and formulas are defined for variables only. See \cref{def:lambda_term_schema} and \cref{def:fol_formula_schema}.
\end{comments}
...
\begin{comments}
  ...
  \item This algorithm is a special case of the analogous first-order logic algorithm \fullref{alg:fol_formula_schema_instantiation}, implemented in \cite{notebook:code} as
...
\end{definition}
\begin{comments}
  \item Similar general definitions appear in \cite[6D1]{Hindley1997BasicSTT} and \textcite[46]{Герасимов2014Вычислимость}.
  ...
  \item The term \enquote{logical axiom} may have different meaning depending on the author. \Textcite[6D1]{Hindley1997BasicSTT} uses the term \enquote{axiom} for our notion of \enquote{logical axiom}. \Textcite[204]{Hinman2005Logic} gives an ad-hoc definition of derivability from a set of \enquote{logical axioms}. \Textcite{TroelstraSchwichtenberg2000BasicProofTheory} uses both \enquote{logical axiom} and \enquote{nonlogical axiom} but do not define the terms, while \textcite{Kleene2002Logic} only uses the latter term, again without defining it.
...
\end{definition}
\begin{comments}
  \item \Textcite[51]{TroelstraSchwichtenberg2000BasicProofTheory} use the term \enquote{axiomatic deduction}. We prefer \enquote{derivation} to \enquote{deduction} based on an analogy with \hyperref[def:formal_grammar/derivation]{derivations} on \hyperref[def:formal_grammar]{formal grammars}. \Textcite[115]{CitkinMuravitsky2022ConsequenceRelations} also prefer the term \enquote{derivation}.

  \item \Cref{def:propositional_axiomatic_derivation/mp} is an adaptation of \fullref{thm:propositional_semantic_mp}.
\end{comments}
...
\end{concept}
\begin{comments}
  \item This description is loosely based on \cite[\S 2.2.6]{Mimram2020ProgramEqualsProof}, where Mimram writes
  \begin{displayquote}
  ...
  Other authors also use the word \enquote{fragment}, albeit even more loosely. For example, \textcite[def. 2.3.1]{TroelstraSchwichtenberg2000BasicProofTheory} say that a \hyperref[def:fol_formula]{first-order formula} \( \varphi \) belongs to the \enquote{negative fragment} if \( \varphi \) contains no \( \synvee \) or \( \synexists \) and if atomic formulas in \( \varphi \) are negated.
\end{comments}
...
\end{example}
\begin{comments}
  \item \hyperref[def:proof_tree]{Proof trees} allow a more demonstrative proof --- see \eqref{ex:minimal_implication_logic_identity/trees}.
\end{comments}
...
\begin{comments}
  ...
  \begin{CodeRefDisplay}
    \GetCodeRef{alg:derivation_conclusion_hypothesis_introduction}
  \end{CodeRefDisplay}
\end{comments}
