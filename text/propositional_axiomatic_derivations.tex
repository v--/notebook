...
\begin{concept}\label{con:syntax_fragment}\mimprovised
  ...
  Such a fragment is easily obtained by removing \hyperref[def:formal_grammar]{formal grammar rules}. For example, in the grammar of propositional formulas from \cref{def:propositional_formula}, we have the following four rules (after we expand the \( \bnpn{atomic formula} \) rule):
  \begin{bnf*}
  ...
  The result would be a fragment that is as expressive as the original since \( \syntop \) can be replaced by \( \synp \syn\vee \synneg \synp \) and \( \synbot \) --- by \( \synp \syn\wedge \synneg \synp \). This is the approach we use in \fullref{alg:cnf_and_dnf}.
\end{concept}
\begin{comments}
...
\begin{definition}\label{def:implicational_propositional_fragment}\mcite[49]{Mimram2020ProgramEqualsProof}
  The \term{implicational \hyperref[con:syntax_fragment]{fragment}} of propositional logic is the \hyperref[con:syntax_fragment]{fragment} obtained via the following grammar (based on the \hyperref[def:propositional_formula]{usual schema}):
  \begin{bnf*}
