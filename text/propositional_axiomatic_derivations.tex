...
\begin{algorithm}[Derivation conclusion hypothesis introduction]\label{alg:derivation_conclusion_hypothesis_introduction}
  We will present an algorithm that, given an \hyperref[def:axiomatic_derivation]{axiomatic derivation} of \( \psi \) from \( \Gamma \cup \set{ \varphi } \), will provide a derivation of \( \varphi \synimplies \psi \) from \( \Gamma \).
...
\end{algorithm}
\begin{comments}
  \item This algorithm can be found as \identifier{logic.derivation.minimal_implicational_logic.introduce_conclusion_hypothesis} in \cite{notebook:code}.
\end{comments}
