...
\begin{proposition}\label{thm:def:partial_order_chain}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:def:partial_order_chain/unbounded} A chain is \hyperref[def:extremal_points/upper_and_lower_bounds]{unbounded from above} (resp. below) if, for every element, it contains a strictly larger (resp. smaller) element.
...
\begin{proof}
  ...
  Expanding the definition \fullref{def:extremal_points/upper_and_lower_bounds} of upper bound, we conclude that
  \begin{displayquote}
...
\end{proposition}
\begin{comments}
  \item An analogous result holds for total orders (\fullref{thm:def:totally_ordered_set/lexicographic}) and well-ordered sets (\fullref{thm:well_ordered_lexicographic_order_is_well_ordered}).
\end{comments}
\begin{proof}
...
\begin{definition}\label{def:chain_condition}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:chain_condition/unbounded} There exists no \hyperref[def:extremal_points/upper_and_lower_bounds]{unbounded from above} (resp. from below) \hyperref[def:partial_order_chain]{chain}.
  \end{thmenum}
\end{definition}
\begin{defproof}
...
\begin{example}\label{ex:def:cofinal_set}
  ...
  \begin{thmenum}
    \thmitem{ex:def:cofinal_set/finite} In a finite set like \( \set{ 0, 1, 2 } \), the set \( \set{ 2 } \) containing the maximum is cofinal. This is generalized by \fullref{def:cofinal_set/top} and \fullref{def:cofinal_set/maximal}.
...
\begin{proposition}\label{thm:def:cofinal_set}
  ...
  \begin{thmenum}
    \thmitem{def:cofinal_set/top} If a preordered set is bounded from above, a subset is cofinal if and only if it contains a \hyperref[def:extremal_points/top_and_bottom]{top element}.

    \thmitem{def:cofinal_set/maximal} A cofinal subset of a \hi{partially ordered set} contains all \hyperref[def:extremal_points/maximal_and_minimal_element]{maximal elements}.

    \thmitem{def:cofinal_set/acc} In a partially ordered set satisfying the \hyperref[def:chain_condition]{ascending chain condition}, if a set contains all \hyperref[def:extremal_points/maximal_and_minimal_element]{maximal elements}, it is cofinal.
  \end{thmenum}
\end{proposition}
\begin{proof}
  \SubProofOf{def:cofinal_set/top} Let \( (P, \leq) \) be a preordered set with top element \( \top \).
  ...
  \SubProofOf{def:cofinal_set/maximal} Suppose that \( A \) is cofinal in \( (P, \leq) \), where \( \leq \) is a partial order. Let \( m \) be a maximal element of \( P \).
  ...
  \SubProofOf{def:cofinal_set/acc} Suppose that \( A \) contains all maximal elements.
