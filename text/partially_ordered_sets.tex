...
\begin{definition}\label{def:partial_order_covering}\mcite[11]{DaveyPriestley2002LatticeTheory}
...
\end{definition}
\begin{comments}
  \item The definition is based on \bycite[113]{DaveyPriestley2002LatticeTheory}, where atoms are defined in lattices. We additionally define coatoms.
\end{comments}
...
\begin{definition}\label{def:stabilizing_sequence}\mcite[244]{Aluffi2009Algebra}
  Consider the \hyperref[def:order_function/preserving]{ascending sequence}
  \begin{equation*}
...
\end{definition}
\begin{comments}
  \item \incite[40]{Harzheim2005OrderedSets} prefers the term \enquote{extensional operator}, \incite[186]{DaveyPriestley2002LatticeTheory} prefer \enquote{increasing map} while \incite[def. 2.12]{Гуров2013ТеорияРешёток} uses \enquote{рефлексивный оператор} (\enquote{reflexive operator}).
\end{comments}
