...
\begin{definition}\label{def:formal_language}\mcite[9]{Savage2008ModelsOfComputation}
  ...
  \begin{thmenum}
    ...
    Unless explicitly noted otherwise, like in \fullref{sec:free_groups}, we will assume that \( \mscrA \) is finite. Even in the cases where it is not, we require the symbols to be \hyperref[def:universe_of_hereditary_finite_sets]{hereditarily finite sets}. This is discussed in \cref{rem:language_alphabet_cardinality}.
...
\begin{remark}\label{rem:language_alphabet_cardinality}
  ...
  \begin{thmenum}
    ...
    A signature consists of one or two alphabets, so forming a category requires knowing which \hyperref[def:grothendieck_universe]{Grothendieck universe} these alphabets belong to.

    The symbols of an alphabet are technically sets, even though in our \hyperref[con:metalogic]{metalanguage} they have glyphs assigned. Unless restricted, they can belong to an arbitrary universe. If the symbols are \hyperref[def:universe_of_hereditary_finite_sets]{hereditarily finite sets}, which belongs to the smallest Grothendieck universe \( V_\omega \), and, due to \cref{thm:grothendieck_universe_contains_finite_sets}, to every Grothendieck universe.

    So the signature belongs to the universe \( \mscrU \) if the alphabets do. In particular, all finite signatures belong to \( V_\omega \).

    We will make use of this in \cref{def:fol_signature_category}, for example.
  \end{thmenum}
\end{remark}
