...
\begin{definition}\label{def:formal_language}\mcite[3]{Salomaa1987}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:formal_language/word} A \hyperref[def:sequence]{finite sequence} of symbols over is called a \term{word} or \term{string}. If \( (a, b, c) \) is a word, for convenience we write it as \( abc \). This notation only makes sense if each symbol of the language is actually represented by one typographic symbol.

    \thmitem{def:formal_language/empty_word} We denote the empty word by \( \varepsilon \)\footnote{This notation is used, for example, by \cite[9]{Savage1998}. Other authors like Arto Salomaa in \cite[3]{Salomaa1987} use \( \lambda \) instead.}.
...
\end{definition}
\begin{comments}
  \item Note that in some contexts like \hyperref[subsec:propositional_logic]{propositional logic} or \hyperref[subsec:first_order_logic]{first-order logic}, the term \enquote{language} may refer to the alphabet itself, as noted in \fullref{rem:propositional_language_is_alphabet}.
\end{comments}
...
\begin{example}\label{ex:def:formal_language}
  ...
  \begin{thmenum}
    ...
    We can identify the language with the \hyperref[def:weighted_set]{edge-labeled} \hyperref[def:quiver/graph]{directed graph}
    \begin{equation}\label{def:formal_language/an}
      \begin{aligned}
        \includegraphics[page=1]{output/ex__def__formal_language.pdf}
      \end{aligned}
    \end{equation}

    Each word in \( \mscrL \) corresponds to a \hyperref[def:quiver_path/directed]{directed path} in \eqref{eq:fig:def:formal_language/an} and vice versa.
    ...
    It can encode natural number operations just as well. It cannot, however, be represented via a directed graph like \eqref{eq:fig:def:formal_language/an/quiver}. This will be made precise and proved in \fullref{ex:def:finite_automaton/anbn}.
...
\begin{definition}\label{def:finite_automaton}\mcite[def. 4.1.1]{Savage1998}
  ...
  \begin{figure}[!ht]
    \hfill
    \includegraphics[page=1]{output/def__finite_automaton.pdf}
    \hfill
    \includegraphics[page=2]{output/def__finite_automaton.pdf}
    \hfill\hfill
...
\begin{definition}\label{def:reverse_automaton}\mimprovised
...
\begin{proposition}\label{thm:reverse_finite_automaton_graph}
  The \hyperref[def:finite_automaton/graph]{graph} of the \hyperref[def:reverse_automaton]{reverse automaton} \( \op{rev}(F) \) is the \hyperref[def:quiver/duality]{opposite graph}.
\end{proposition}
\begin{proof}
  Trivial.
\end{proof}
...
\begin{example}\label{ex:def:finite_automaton}
  ...
  \begin{thmenum}
    ...
    \begin{equation*}
      \includegraphics[page=2]{output/ex__def__finite_automaton.pdf}
    \end{equation*}
    ...
    \begin{equation*}
      \includegraphics[page=1]{output/ex__def__finite_automaton.pdf}
    \end{equation*}
    ...
    \begin{equation*}
      \includegraphics[page=3]{output/ex__def__finite_automaton.pdf}
    \end{equation*}
    ...
    \begin{equation*}
      \includegraphics[page=4]{output/ex__def__finite_automaton.pdf}
    \end{equation*}
    or
    \begin{equation*}
      \includegraphics[page=5]{output/ex__def__finite_automaton.pdf}
    \end{equation*}
    ...
    \begin{equation*}
      \includegraphics[page=6]{output/ex__def__finite_automaton.pdf}
    \end{equation*}
...
\begin{definition}\label{def:formal_grammar}\mcite[9]{Salomaa1987}
  ...
  We call the quadruple \( (V, \Sigma, \to, S) \) a \term{phrase-structure grammar} or simply a \term{formal grammar}.
  ...
  \begin{thmenum}
    ...
    \thmitem{def:formal_grammar/graph}\mimprovised We can regard the derivation relation \( \Rightarrow \) as a set of \hyperref[def:quiver/arcs]{arcs} over the grammar language \( \mscrL(G) \). Thus, \( (\mscrL(G), \Rightarrow) \) is a \hyperref[def:quiver/simple]{directed graph}, whose nonempty paths are precisely the derivations in \( G \). Furthermore, we can \hyperref[def:weighted_set]{label} each arc with the rule applied.
...
\begin{proposition}\label{thm:derivation_graph_connected}
  Every \hyperref[def:formal_grammar/graph]{derivation graph} is \hyperref[def:quiver_connectedness/strong]{strongly connected}.
\end{proposition}
\begin{proof}
...
\begin{example}\label{ex:natural_number_arithmetic_grammar/rules}
  ...
  \begin{figure}[!ht]
    \centering
    \includegraphics[page=1]{output/ex__natural_number_arithmetic_grammar__rules.pdf}
...
\begin{example}\label{ex:thm:length_increasing_grammar}
  ...
  \Fullref{alg:epsilon_rule_removal} suggests instead the essentially length-increasing grammar:
  \begin{equation*}
    \begin{aligned}
      S &\to \varepsilon \\\
      S &\to A, \\
...
\begin{comments}
  ...
  \item Compare this to the much more complicated \fullref{alg:recursive_descent_parsing} that works in more restricted cases.
\end{comments}
\begin{proof}
...
\begin{definition}\label{def:chomsky_hierarchy}\mcite[15]{Salomaa1987}
  We can classify \hyperref[def:formal_grammar]{formal grammars} and \hyperref[def:language]{languages} to form the \term{Chomsky hierarchy}. Chomsky himself in \cite[def. 6]{Chomsky1959} defined a hierarchy of grammars consisting of four types --- \enquote{type 0} through \enquote{type 3}. He also defined a parallel hierarchy of languages, in which \( \mscrL \) is a \enquote{type \( i \) language} if there exists a type \( i \) grammar \hyperref[def:grammar_derivation/grammar_language]{generating it}.
...
\begin{example}\label{ex:def:chomsky_hierarchy}
  ...
  \begin{thmenum}
    ...
    Neither of these grammars is \hyperref[def:epsilon_free_grammar]{essentially \( \varepsilon \)-free}. \Fullref{alg:epsilon_rule_removal} suggests the essentially length-increasing grammar
    \begin{equation*}
      \begin{aligned}
...
\begin{algorithm}[Length-increasing to context-sensitive grammar]\label{alg:length_increasing_to_context_sensitive}\mcite[thm. 9.2]{Salomaa1987}
  ...
  \begin{thmenum}
    ...
    \begin{thmenum}
      \thmitem{alg:length_increasing_to_context_sensitive/intermediate/new} Let \( \Sigma_I \) is a set of new non-terminals, one for each terminal in \( \Sigma \), and let \( \varphi: \Sigma \to \Sigma_I \) be an explicit bijective mapping between the two.
      \thmitem{alg:length_increasing_to_context_sensitive/intermediate/non_terminals} Let \( V_I \coloneqq V \cup \Sigma_I \).
      \thmitem{alg:length_increasing_to_context_sensitive/intermediate/new_rules} For each terminal \( a \in \Sigma \), add to \( G_I \) a new rule \( \varphi(a) \to a \).
      \thmitem{alg:length_increasing_to_context_sensitive/intermediate/substitution} For each rule \( v \to w \), we add a corresponding rule to \( G_I \) by replacing all terminals in \( v \) and \( w \) with the corresponding new non-terminals.
    \end{thmenum}
...
\begin{remark}\label{rem:chomsky_hierarchy_failure}
  ...
  Unfortunately, for an arbitrary context-free grammar, \fullref{thm:length_increasing_grammar} does not hold, and neither does \fullref{alg:length_increasing_grammar}. There is a well-developed theory (and practice) for parsing context-free languages (which are mostly restricted to certain classes of context-free grammars; see e.g. \cite[ch. 6]{Salomaa1987}). We only present \fullref{alg:recursive_descent_parsing}.

  For theoretical purposes, we can use \fullref{alg:epsilon_rule_removal} to adapt context-free languages to \fullref{alg:length_increasing_grammar}.
\end{remark}
  ...
  \begin{thmenum}
    ...
    \begin{equation*}
      U_k \coloneqq \begin{cases}
        ...
        U_{k-1} \cup \set{ A \in V \given \qexists{w \in U_{k-1}^*} A \to w }, &k > 0.
      \end{cases}
    \end{equation*}
    ...
    Let \( S' \) be an entirely new start symbol and let \( V' \coloneqq V \cup \set{ S' } \). Define the production relation \( A \to' w \) to hold if \( w \) is nonempty and there exist words \( w_0, \ldots, w_n \) over \( V \cup \Sigma \) and non-terminals \( B_1, \ldots, B_n \) from \( U_m \) such that
    \begin{equation*}
...
\begin{algorithm}[Renaming rule collapse]\label{alg:rewriting_rule_collapse}
  Fix a \hyperref[def:chomsky_hierarchy/context_free]{context-free} grammar \( G = (V, \Sigma, \to, S) \). We will build an \hyperref[def:formal_grammar/equivalent]{equivalent} context-free grammar \( G' = (V, \Sigma, \to', S) \) in which there are no \enquote{renaming} rules of the form \( A \to B \) (except possibly when \( A = S \)).

  \begin{thmenum}
    \thmitem{alg:rewriting_rule_collapse/init} Recursively define
    \begin{equation*}
      U_k \coloneqq \begin{cases}
        \set{ (A, A) \given A \in V },                                                          &k = 0, \\
        U_{k-1} \cup \set{ (A, C) \given (A, B) \in U_{k-1} \T{and} B \to C \T{and} C \neq A }, &k > 0.
      \end{cases}
    \end{equation*}

    Since \( V \) has only finitely many non-terminals, the sequence stabilizes at some index \( m \).

    \thmitem{alg:rewriting_rule_collapse/rules} Let \( S \to' w \) if \( S \to w \) and, for \( A \neq S \), \( A \to' w \) if \( B \to w \), where
    \begin{itemize}
      \item \( (A, B) \in U_m \),
      \item the word \( w \) is \hi{not} a single non-terminal.
    \end{itemize}
  \end{thmenum}
\end{algorithm}
...
\begin{algorithm}[Finite automaton from regular grammar]\label{alg:finite_automaton_from_regular_grammar}
  ...
  \begin{thmenum}
    ...
    \thmitem{alg:finite_automaton_from_regular_grammar/collapse} Let \( G_3 = (V_2, \Sigma, \to_3, S_2) \) be the grammar obtained from \( G_2 \) by collapsing renaming rules via \fullref{alg:rewriting_rule_collapse}.
...
\begin{algorithm}[Right-linear grammar from finite automaton]\label{alg:right_linear_grammar_from_finite_automaton}\mcite[thm. 4.10.1]{Savage1998}
  Let \( F = (\Sigma, Q, \delta, I, T) \) be a \hyperref[def:finite_automaton]{finite automaton}. We will build a \hyperref[def:chomsky_hierarchy/regular]{right-linear grammar} \( G = (V, \Sigma, \to, S) \) that \hyperref[def:grammar_derivation/grammar_language]{generates} \( \mscrL(F) \).
