...
\begin{definition}\label{def:formal_language}\mcite[9]{Savage2008Computability}
  \hfill
  \begin{thmenum}
    ...
    \thmitem{def:formal_language/empty_string} We denote the empty string via \( \varepsilon \)\fnote{This notation is used, for example, by \incite[9]{Savage2008Computability}. \incite[3]{Salomaa1973Languages} uses \( \lambda \) instead, while \incite[19]{Гладкий1973Языки} uses \( \Lambda \).}.
...
\begin{definition}\label{def:formal_grammar}\mcite[def. 4.9.1]{Savage2008Computability}
...
\begin{definition}\label{def:chomsky_hierarchy}\mcite[15]{Salomaa1973Languages}
  ...
  \begin{thmenum}
    \thmitem{def:chomsky_hierarchy/unrestricted} When no additional restrictions are imposed on the rules of the grammar, we call it \term{unrestricted}. We call the corresponding languages \term{recursively enumerable} following \cite[thm 5.4.1; thm 5.4.2]{Savage2008Computability}.
...
\begin{proposition}\label{thm:non_recursively_enumerable_language}\mcite[thm. 5.7.4]{Savage2008Computability}
