...
\begin{remark}\label{rem:language_alphabet_cardinality}
  ...
  \begin{thmenum}
    ...
    As shown in \cref{thm:hol_small_model_characterization} and \cref{thm:fol_small_model_characterization}, it is important for a signature to be \hyperref[def:small_set]{small} so that it is possible to determine whether a \hyperref[def:fol_semantics/model]{model} is small.

    The symbols of an alphabet are technically sets, even though in our \hyperref[con:metalogic]{metalanguage} they have glyphs assigned. Unless restricted, alphabets can go beyond the (smallest) \hyperref[def:grothendieck_universe]{Grothendieck universe}, and fail to be small sets.

    Thus, even if the alphabets are, for some reason, infinite, we assume that they do not go beyond basic \hyperref[def:zfc]{\logic{ZFC}} and thus belong to every \hyperref[def:grothendieck_universe]{Grothendieck universe}.
  \end{thmenum}
\end{remark}
