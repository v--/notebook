...
\begin{remark}\label{rem:language_alphabet_cardinality}
  ...
  \begin{thmenum}
    ...
    On the other side, for some theoretical needs like proving \fullref{thm:classical_fol_completeness}, it is necessary to create alphabets with infinitely many symbols.
...
\begin{comments}
  ...
  \item This algorithm is implemented in \cite{notebook:code} as
    \WriteCodeRef{alg:wagner_fisher}
\end{comments}
...
\begin{comments}
  \item Term schemas are implemented in \cite{notebook:code}, in the module
    \WriteCodeRefText{math.grammars.grammar}

  A parser is implemented in
    \WriteCodeRefText{math.grammars.parsing}
\end{comments}
...
\begin{comments}
  ...
  \item This algorithm is implemented in \cite{notebook:code} as
    \WriteCodeRef{alg:context_sensitive_string_membership}
\end{comments}
...
\begin{example}\label{ex:def:chomsky_hierarchy}
  ...
  \thmitem{ex:def:chomsky_hierarchy/anbncn}
\end{example}
...
\begin{comments}
  \item This algorithm is implemented in \cite{notebook:code} as
    \WriteCodeRef{alg:length_increasing_to_context_sensitive}
\end{comments}
...
\begin{comments}
  \item This algorithm is implemented in \cite{notebook:code} as
    \WriteCodeRef{alg:epsilon_rule_removal}
\end{comments}
...
\begin{comments}
  ...
  \item This algorithm is implemented in \cite{notebook:code} as
    \WriteCodeRef{alg:renaming_rule_collapse}
\end{comments}
