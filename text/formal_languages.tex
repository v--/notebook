...
\begin{example}\label{ex:def:formal_language}
  ...
  \begin{thmenum}
    ...
    We can identify the language with the \hyperref[def:labeled_set]{edge-labeled} \hyperref[def:directed_multigraph/graph]{directed graph}
    ...
    Each word in \( \mscrL \) corresponds to a \hyperref[def:directed_multigraph_path/directed]{directed path} in \eqref{eq:def:formal_language/an} and vice versa.
...
\begin{proposition}\label{thm:reverse_finite_automaton_graph}
  The \hyperref[def:finite_automaton/graph]{graph} of the \hyperref[def:reverse_finite_automaton]{reverse automaton} \( \op{rev}(F) \) is the \hyperref[def:directed_multigraph/duality]{opposite graph}.
\end{proposition}
\begin{proof}
  Trivial.
\end{proof}
