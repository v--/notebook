...
\begin{example}\label{ex:def:formal_language}
  ...
  \begin{thmenum}
    ...
    Each word in \( \mscrL \) corresponds to a \hyperref[def:quiver_path/directed]{directed path} in \eqref{eq:def:formal_language/an} and vice versa.
...
\begin{proposition}\label{thm:reverse_finite_automaton_graph}
  The \hyperref[def:finite_automaton/graph]{graph} of the \hyperref[def:reverse_finite_automaton]{reverse automaton} \( \op{rev}(F) \) is the \hyperref[def:quiver/duality]{opposite graph}.
\end{proposition}
\begin{proof}
  Trivial.
\end{proof}
  ...
  \begin{thmenum}
    ...
    \begin{equation*}
      U_k \coloneqq \begin{cases}
        ...
        U_{k-1} \cup \set{ A \in V \given \qexists{B \in U_{k-1}^*} A \to B }, &k > 0.
      \end{cases}
    \end{equation*}
