...
\begin{definition}\label{def:formal_language}\mcite[9]{Savage1998}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:formal_language/occurrence}\mimprovised An \term[ru=вхождение (\cite[64]{Герасимов2011}), en=occurrence (\cite[9A2]{Hindley1997})]{occurrence} of a nonempty string \( v \) in \( w \) is a triple of strings \( (p, v, s) \), such that \( w = pvs \).

    If \( (p, v, s) \) is an occurrence in \( w \), we call \( v \) a \term[ru=подцепочка (\cite[21]{Гладкий1973Языки})]{substring}, \( p \) --- \term[ru=начало (\cite[20]{Гладкий1973Языки})]{prefix} and \( s \) --- a \term[ru=конец (\cite[20]{Гладкий1973Языки})]{suffix} of \( w \).
