...
\begin{definition}\label{def:formal_language}\mcite[9]{Savage1998Computability}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:formal_language/string} We call a \hyperref[def:sequence]{finite sequence} of symbols a \term[ru=слово (\cite[19]{Гладкий1973Языки}), en=word (\cite[3]{Salomaa1987Languages})]{word} or \term[ru=цепочка (\cite[19]{Гладкий1973Языки})]{string}. If \( (a, b, c) \) is a string, for convenience we use the notation \( abc \). This notation only makes sense if each symbol of the language is actually represented by one typographic symbol.

    \thmitem{def:formal_language/empty_string} We denote the empty string via \( \varepsilon \)\fnote{This notation is used, for example, by \incite[9]{Savage1998Computability}. \incite[3]{Salomaa1987Languages} uses \( \lambda \) instead, while \incite[19]{Гладкий1973Языки} uses \( \Lambda \).}.
...
\begin{example}\label{ex:def:formal_language}
  ...
  \begin{thmenum}
    ...
    We can redefine all operations from \fullref{subsec:natural_numbers} to hold for strings in \( \mscrL \). For example, addition of \( a^n \) and \( a^m \) is their concatenation \( a^{n + m} \), while the exponentiation \( (a^n)^m \) corresponds to \( m \) repetitions of the string \( a^n \), that is, to \( a^{nm} \).
...
\begin{definition}\label{def:hamming_distance}\mcite[993]{Rosen1999DiscreteHandbook}
  The \term{Hamming distance} between two \hyperref[def:formal_language/string]{strings} \( a_1 \cdots a_n \) and \( b_1 \cdots b_n \) of the same length is simply the number of indices for which \( a_k \neq b_k \).
\end{definition}
...
\begin{definition}\label{def:epsilon_free_grammar}\mcite[54]{Salomaa1987Languages}
...
\begin{definition}\label{def:length_increasing_grammar}\mcite[15]{Salomaa1987Languages}
...
\begin{definition}\label{def:chomsky_hierarchy}\mcite[15]{Salomaa1987Languages}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:chomsky_hierarchy/regular}\mcite[44]{Salomaa1987Languages} Finally, we call the grammar \term{left linear} if every rule has one of the forms
    \begin{itemize}
      \item \( A \to w \),
      \item \( A \to B w \),
    \end{itemize}
...
\begin{algorithm}[Length-increasing to context-sensitive grammar]\label{alg:length_increasing_to_context_sensitive}\mcite[thm. 9.2]{Salomaa1987Languages}
...
\begin{remark}\label{rem:chomsky_hierarchy_failure}
  ...
  \begin{itemize}
    ...
    \item For practical purposes, parsing context-free languages is a topic in itself. These algorithms are mostly restricted to certain classes of context-free grammars; see e.g. \cite[ch. 6]{Salomaa1987Languages}. General algorithms for parsing all context-free grammars are scarcer; several are discussed in \cite{Economopoulos2006Parsing}. We present \fullref{alg:brute_force_parsing}, which handles arbitrary context-free grammars, but is too inefficient to use in practice.
  \end{itemize}
\end{remark}

\paragraph{Context-free languages}

\begin{algorithm}[Epsilon rule removal]\label{alg:epsilon_rule_removal}\mcite[thm. 6.2]{Salomaa1987Languages}
