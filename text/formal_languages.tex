...
\begin{definition}\label{def:formal_language}\mcite[9]{Savage1998}
  \hfill
  \begin{thmenum}
    ...
    \thmitem{def:formal_language/string} We call a \hyperref[def:sequence]{finite sequence} of symbols a \term[ru=слово (\cite[19]{Гладкий1973Языки}), en=word (\cite[3]{Salomaa1987})]{word} or \term[ru=цепочка (\cite[19]{Гладкий1973Языки})]{string}. If \( (a, b, c) \) is a string, for convenience we use the notation \( abc \). This notation only makes sense if each symbol of the language is actually represented by one typographic symbol.

    \thmitem{def:formal_language/empty_string} We denote the empty string via \( \varepsilon \)\fnote{This notation is used, for example, by \incite[9]{Savage1998}. \incite[3]{Salomaa1987} uses \( \lambda \) instead, while \incite[19]{Гладкий1973Языки} uses \( \Lambda \).}.
...
\begin{example}\label{ex:def:formal_language}
  ...
  \begin{thmenum}
    ...
    \thmitem{ex:def:formal_language/leucine}\mcite[ch. 1]{Waterman1995} DNA and RNA molecules are composed of smaller molecules that are linked together. These smaller molecules are called nucleotides.
...
\begin{comments}
  ...
  \item This algorithm can be found as \identifier{lang.distance.wagner_fisher} in \cite{code}.
\end{comments}
...
\begin{remark}\label{rem:length_increasing_grammar}
  ...
  Noam Chomsky in \cite[361]{MathPsychology1963Vol2} defines \enquote{type 1} grammars as those satisfying the inequality \( \len(v) \leq \len(w) \) for every rule \( v \to w \). These grammars still have cycles like \( A \to B \to A \), but they avoid more convoluted cases and lead to \Fullref{alg:context_sensitive_string_membership}.
...
\begin{comments}
  ...
  \item Chomsky introduced length-increasing grammars in \cite[360]{MathPsychology1963Vol2} and on the same page hinted at this property of theirs, which we will use in \fullref{alg:context_sensitive_string_membership}.
\end{comments}
\begin{proof}
...
\end{algorithm}
\begin{comments}
  \item This algorithm can be found as \identifier{grammars.renaming_rules.collapse_renaming_rules} in \cite{code}.
  \item Our goal is to \enquote{collapse} derivations like \( A \to B \to C \to w \) into \( A \to' w \).
\end{comments}

\begin{remark}\label{con:backus_normal_form}\mcite[14]{Backus1958}
