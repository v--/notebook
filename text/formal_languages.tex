...
\begin{definition}\label{def:formal_language}\mcite[9]{Savage2008ModelsOfComputation}
  \hfill
  \begin{thmenum}
    \thmitem{def:formal_language/alphabet} Fix a nonempty set \( \mscrA \), which we will call an \term[ru=алфавит (\cite[19]{Гладкий1973ГрамматикиИЯзыки})]{alphabet}.

    Unless explicitly noted otherwise, like in \fullref{sec:free_groups}, we will assume that \( \mscrA \) is finite. Even in the cases where it is not, we require \( \mscrA \) to be a \hyperref[def:small_set]{small set} and its symbols to be \hyperref[def:universe_of_hereditary_finite_sets]{hereditarily finite sets}. This is discussed in \cref{rem:language_alphabet_cardinality}.
    ...
    \thmitem{def:formal_language/empty_string} We denote the empty string via \( \bnfves \)\fnote{This notation is used, for example, by \textcite[9]{Savage2008ModelsOfComputation}. \Textcite[3]{Salomaa1973FormalLanguages} uses \( \lambda \) instead, while \textcite[19]{Гладкий1973ГрамматикиИЯзыки} uses \( \Lambda \).}.
    ...
    We abbreviate \( \smash{\overbrace{w \ldots w}^{k \T*{times}}} \) as \( w^k \).\fnote{This is only a notational shortcut within the \hyperref[con:metalogic]{metalanguage}. We do not distinguish, formally, between the strings \( aaabbaa \) and \( a^3 b^2 a^2 \), nor between \( a \varepsilon b \) and \( ab \).}
    ...
    \thmitem{def:formal_language/prefix}\mimprovised We say that the string \( p = (p_1, \ldots, p_m) \) is a \term[ru=начало (\cite[20]{Гладкий1973ГрамматикиИЯзыки}); префикс (\cite[261]{Яблонский2003ДискретнаяМатематика})]{prefix} of \( w = (w_1, \ldots, w_n) \) if
    \begin{equation*}
    ...
    \thmitem{def:formal_language/kleene_star}\mcite[158]{Savage2008ModelsOfComputation} We define the \term{Kleene star}\fnote{The Kleene star is naturally a monoid --- see \cref{def:free_monoid}.} \( \mscrA^* \) of \( \mscrA \) as the set of all strings over \( \mscrA \).

    \thmitem{def:formal_language/kleene_plus}\mimprovised If we wish to exclude the empty string, we instead obtain the \term{Kleene plus} \( \mscrA^+ \).
...
\begin{remark}\label{rem:language_alphabet_cardinality}
  There are several reasons the cardinality of a language's alphabet is important to us.

  \begin{thmenum}
    \thmitem{rem:language_alphabet_cardinality/parsing} We want to be able to parse a language. For \hyperref[def:chomsky_hierarchy/context_free]{context-free} languages, this amounts to an algorithm like \fullref{alg:brute_force_parsing} for constructing \hyperref[def:parse_tree]{parse trees}.

    If there are infinitely many symbols, such a parsing algorithm cannot exist because it would have an infinite description.

    On the other side, for some theoretical needs like proving \fullref{thm:fol_completeness}, it is necessary to create alphabets with infinitely many symbols.

    \thmitem{rem:language_alphabet_cardinality/category} In some places like in \hyperref[def:entailment_system]{entailment systems} and \hyperref[def:institution]{institutions}, we need to be able to work with a \hyperref[def:category]{category} of \hyperref[con:logical_system_signature]{logical system signatures}.

    As shown in \cref{thm:hol_small_model_characterization} and \cref{thm:fol_small_model_characterization}, it is important for a \hyperref[con:logical_system_signature]{logical signature} to be \hyperref[def:small_set]{small} so that it is possible to determine whether a \hyperref[def:fol_semantics/model]{model} is small.

    The symbols of an alphabet are technically sets, even though in our \hyperref[con:metalogic]{metalanguage} they have glyphs assigned.
    Even if the alphabets are, for some reason, infinite, we assume that they do not go beyond basic \hyperref[def:zfc]{\logic{ZFC}} and thus belong to the \hyperref[def:smallest_grothendieck_universe]{smallest Grothendieck universe}.
  \end{thmenum}
\end{remark}

\begin{proposition}\label{thm:def:formal_language}
  \hyperref[def:formal_language]{Formal languages} have the following basic properties:
  \begin{thmenum}
    \thmitem{thm:def:formal_language/finite_alphabet_cardinality} For any \emph{nonempty finite} \hyperref[def:formal_language/alphabet]{alphabet} \( A \), the \hyperref[def:formal_language/kleene_star]{Kleene star} \( A^* \) is countably infinite.

    This gives an upper bound to the cardinality of languages over \( A \).

    \thmitem{thm:def:formal_language/infinite_alphabet_cardinality} For any alphabet \( A \) of infinite cardinality \( \kappa \), the cardinality of the Kleene star \( A^* \) is \( \kappa \).
  \end{thmenum}
\end{proposition}
\begin{proof}
  \SubProofOf{thm:def:formal_language/finite_alphabet_cardinality} Let \( \kappa \) be the cardinality of \( A \). \fullref{thm:cardinality_product_rule} implies that the cardinality of all words over \( A \) of length \( i \) is \( \kappa^i \), and \fullref{thm:cardinality_sum_rule} implies that the cardinality of \( A^* \) is the supremum
  \begin{equation}\label{eq:thm:def:formal_language/finite_alphabet_cardinality/proof/sup}
    \sup_{k=0}^\infty \kappa^i.
  \end{equation}

  Since \( \kappa \) is finite, \cref{thm:simplified_cardinal_arithmetic/finite} implies that \( \kappa^i \) is finite for every \( k \); hence, the supremum over all \( k \) is \( \aleph_0 \).

  \SubProofOf{thm:def:formal_language/infinite_alphabet_cardinality} If \( \kappa \) is infinite, \cref{thm:simplified_cardinal_arithmetic/infinite} implies that \( \kappa^i = \kappa \) for every \( k \); hence, the supremum \eqref{eq:thm:def:formal_language/finite_alphabet_cardinality/proof/sup} is also \( \kappa \).
\end{proof}

\paragraph{String distance}

\begin{definition}\label{def:hamming_distance}\mcite[24]{Golan1999Semirings}
  We define the \term[ru=расстояние Хэмминга (\cite[\S 6.3.3]{Новиков2013ДискретнаяМатематика}), en=Hamming distance]{Hamming distance} between two \hyperref[def:formal_language/string]{strings} \( v = v_1 \cdots v_n \) and  \( w = w_1 \cdots w_n \) of the same length is simply the number of positions for which \( v_k \neq w_k \).
\end{definition}

\begin{proposition}\label{thm:hamming_distance_metric}
  As an operator on \( \Sigma^n \) for some alphabet \( \Sigma \), the \hyperref[def:hamming_distance]{Hamming distance} is a \hyperref[def:metric_space]{metric}.

  \begin{figure}
    \hfill
    \begin{subcaptionblock}{0.45\textwidth}
      \centering
      \includegraphics[page=1]{output/thm__hamming_distance_metric}
      \caption{The \hyperref[def:p_norm]{\( 1 \)-norm}.}
    \end{subcaptionblock}
    \hfill
    \begin{subcaptionblock}{0.45\textwidth}
      \centering
      \includegraphics[page=2]{output/thm__hamming_distance_metric}
      \caption{The \hyperref[def:hamming_distance]{Hamming distance}.}
    \end{subcaptionblock}
    \hfill
    \caption{The \hyperref[def:metric_space/ball]{unit circles} under different metrics in a plane over a \hyperref[def:finite_field]{finite field}.}
    \label{fig:thm:hamming_distance_metric}
  \end{figure}
\end{proposition}
\begin{proof}
  \SubProofOf{def:metric_space/M1} The Hamming distance is zero if and only if \( v = w \).

  \SubProofOf{def:metric_space/M2} Trivial.

  \SubProofOf{def:metric_space/M3} For any three strings \( u = u_1 \ldots u_n \), \( v = v_1 \ldots v_n \) and \( w = w_1 \ldots w_n \), if \( u_i \neq w_i \), then either \( u_i \neq v_i \) or \( v_i \neq w_i \).
\end{proof}
...
\begin{definition}\label{def:levenshtein_distance}\mcite[4]{Левенштейн1965ДвоичныеКоды}
  ...
  In this context, following \textcite[168]{WagnerFischer1974LevenshteinDistance}, we will refer to these operations as \enquote{edit operations}.
\end{definition}
\begin{comments}
  \item We state Levenshtein's definition, but we will conflate it with the recursively-defined function from \cref{thm:levenshtein_distance_characterization}.
  ...
  \item \Textcite[2]{Левенштейн1965ДвоичныеКоды} also discusses a similar distance function, but without allowing substitutions. We use here the more established version, called \enquote{edit distance} by \textcite[168]{WagnerFischer1974LevenshteinDistance}.
\end{comments}
...
\begin{example}\label{ex:def:levenshtein_distance}
  ...
  \begin{thmenum}
    ...
    while
    \begin{equation*}
      l(\underline{\texttt{con}}\texttt{unction}, \oline{\texttt{j}}\texttt{unction}) = 1 + l(\underline{\texttt{co}}\oline{\texttt{j}}\texttt{unction}, \oline{\texttt{j}}\texttt{unction}) = 3
    \end{equation*}
    and
    \begin{equation*}
      l(\underline{\texttt{con}}\texttt{unction}, \underline{\texttt{dis}}\oline{\texttt{j}}\texttt{unction}) = 1 + l(\underline{\texttt{con}}\oline{\texttt{j}}\texttt{unction}, \underline{\texttt{dis}}\oline{\texttt{j}}\texttt{unction}) = 4.
    \end{equation*}

    On the other hand, \( \texttt{subjunction} \) is at a distance of \( 3 \) from all three strings, so neither of them is \enquote{closest}.
  \end{thmenum}
\end{example}
...
\begin{algorithm}[Wagner-Fisher algorithm for Levenshtein distance]\label{alg:wagner_fisher}\mcite[171; 172]{WagnerFischer1974LevenshteinDistance}
  Fix two strings
  \begin{align*}
    v = v_1 \cdots v_n && w = w_1 \cdots w_m.
  \end{align*}

  The \hyperref[def:levenshtein_distance]{Levenshtein distance} from the prefix \( v_1 \cdots v_j \) to the prefix \( w_1 \cdots w_i \) can be computed recursively as follows:
  \begin{equation*}
    d_{i,j} = \begin{cases}
      j,                                                         &i = 0, \\
      i,                                                         &j = 0, \\
      \min\set{ 1 + d_{i-1,j}, 1 + d_{i,j-1}, 0 + d_{i-1,j-1} }, &i > 0 \T{and} j > 0 \T{and} v_i = w_i, \\
      \min\set{ 1 + d_{i-1,j}, 1 + d_{i,j-1}, 1 + d_{i-1,j-1} }, &i > 0 \T{and} j > 0 \T{and} v_i \neq w_i.
    \end{cases}
  \end{equation*}
...
\begin{comments}
  ...
  \begin{CodeRefDisplay}
    \ManualCodeRef{grammars.grammar}
  \end{CodeRefDisplay}

  A parser is implemented in
  \begin{CodeRefDisplay}
    \ManualCodeRef{grammars.parsing}
  \end{CodeRefDisplay}
\end{comments}
...
\begin{example}\label{ex:natural_number_arithmetic_grammar/schema}
  ...
  Let \( V \coloneqq \set{ N, O, E } \) and \( \Sigma \coloneqq \set{ \syn0, \syn1, \synplus, \synmult, (,) } \). Consider the derivation rules
  \begin{equation}\label{eq:ex:natural_number_arithmetic_grammar/schema/simple}
    \begin{aligned}
      ...
      N &\to \syn1   & \quad B &\to \syn0 B & \quad O &\to \synmult & \quad E &\to (E O E) \\
...
\begin{remark}\label{rem:length_increasing_grammar}
  ...
  \Textcite[361]{Chomsky1963Grammars} defines \enquote{type 1} grammars as those satisfying the inequality \( \len(v) \leq \len(w) \) for every rule \( v \to w \). These grammars still have cycles like \( A \to B \to A \), but they avoid more convoluted cases and lead to \Fullref{alg:context_sensitive_string_membership}.

  This restriction excludes the empty string from the grammar's language. \Textcite[15]{Salomaa1973FormalLanguages} additionally allows the rule \( S \to \varepsilon \), but only if \( S \) does not occur on the right side of any derivation. \Cref{ex:unboudned_grammar_derivation_length} highlights the importance of the latter assumption.
  ...
  (What we call) context-sensitive \emph{grammars} is a strict subset of the essentially length-increasing grammars, but \cref{thm:context_sensitive_languages} shows that context-sensitive \emph{languages} and length-increasing languages coincide. Hence, the term \enquote{type 1 language} makes sense, although we prefer \enquote{context-sensitive language}.
\end{remark}
...
\begin{definition}\label{def:length_increasing_grammar}\mcite[15]{Salomaa1973FormalLanguages}
  We say that the grammar \( G = (V, \Sigma, \to, S) \) is \term[ru=неукорачивающая (грамматика) (\cite[83]{Гладкий1973ГрамматикиИЯзыки})]{length-increasing} if \( \len(v) \leq \len(w) \) for every rule \( v \to w \) and \term[ru=почти неукорачивающая (грамматика\cite[88]{Гладкий1973ГрамматикиИЯзыки}]{essentially length-increasing} if it is \hyperref[def:epsilon_free_grammar]{essentially \( \varepsilon \)-free} and if \( \len(v) \leq \len(w) \) for any non-\( \varepsilon \) rule.
\end{definition}
...
\begin{comments}
  ...
  \item This algorithm is implemented in \cite{notebook:code} as
  \begin{CodeRefDisplay}
    \GetCodeRef{alg:context_sensitive_string_membership}
  \end{CodeRefDisplay}
\end{comments}
...
\begin{definition}\label{def:chomsky_hierarchy}\mcite[15]{Salomaa1973FormalLanguages}
  ...
  \begin{thmenum}
    ...
    We call the corresponding languages \term{context-free}. While it is possible for a context-free \emph{grammar} to not be \hyperref[def:epsilon_free_grammar]{essentially \( \varepsilon \)-free} and hence not context-sensitive, a context-free \emph{language} is context-sensitive. This is shown in \cref{thm:context_free_languages_are_context_sensitive} and discussed in \cref{rem:chomsky_hierarchy_failure}.
...
\begin{example}\label{ex:def:chomsky_hierarchy}
  ...
  \begin{thmenum}
    ...
    \thmitem{ex:def:chomsky_hierarchy/anbncn} Now consider the following grammar, which we shall denote by \( G \):
    \begin{equation*}
      \begin{aligned}
        S &\to abc \mid aSBc \mid \varepsilon, \\
        cB &\to Bc, \\
        bB &\to bb.
      \end{aligned}
    \end{equation*}

    It allows the following derivations:
    \begin{itemize}
      \item \( S \Rightarrow \varepsilon \).
      \item \( S \Rightarrow abc \).
      \item \( S \Rightarrow aSBc \Rightarrow a\underline{abc}Bc \Rightarrow aab\underline{Bc}c \Rightarrow a^2 b^2 c^2 \).
      \item \( S \Rightarrow aSBc \Rightarrow a\underline{aSBc}Bc \Rightarrow aa\underline{abc}BcBc \Rightarrow \cdots \Rightarrow a^3 b B^2 c^3 \Rightarrow a^3 b^3 c^3 \).
    \end{itemize}

    We can show by induction on string length that the language of \( G \) is
    \begin{equation*}
      \mscrL = \set{ a^n b^n c^n \given n \geq 0 }.
    \end{equation*}

    Since \( G \) is essentially length-increasing, its language is context-sensitive\fnote{\Textcite[example 6.2]{Salomaa1973FormalLanguages} the language \( \mscrL \) cannot be context-free.}. Because of the rule \( cB \to bC \), however, the grammar itself is not context-sensitive.
...
\begin{algorithm}[Length-increasing to context-sensitive grammar]\label{alg:length_increasing_to_context_sensitive}\mcite[thm. 9.2]{Salomaa1973FormalLanguages}
  ...
  \begin{thmenum}
    ...
    \end{equation*}
    where \( r_1 \cdots r_{m_k} \) and \( s_1 \cdots s_{l_k} \) are either terminal or nonterminal symbols. This rule comes from the original grammar \( G \).
    ...
    \begin{align*}
      ...
      C_1 C_2 \cdots C_{m_k-1} C_{m_k} s_{m_k+1}' \cdots s_{l_k}'    &\to'_k s_1' C_2 \cdots C_{m_k-1} C_{m_k} s_{m_k+1}' \cdots s_{l_k}', \\
                                                                     &\vdots, \\
      s_1' s_2' \cdots s_{m_k-1}' C_{m_k} s_{m_k+1}' \cdots s_{l_k}' &\to'_k s_1' s_2' \cdots s_{m_k-1}' C_{m_k} s_{m_k+1}' \cdots s_{l_k}'.
    \end{align*}
...
\begin{comments}
  \item This algorithm is implemented in \cite{notebook:code} as
  \begin{CodeRefDisplay}
    \GetCodeRef{alg:length_increasing_to_context_sensitive}
  \end{CodeRefDisplay}
\end{comments}
...
\begin{remark}\label{rem:chomsky_hierarchy_failure}
  ...
  Fortunately, context-free \emph{languages} are context-sensitive as a consequence of \cref{thm:context_free_languages_are_context_sensitive}.
...
\begin{algorithm}[Epsilon rule removal]\label{alg:epsilon_rule_removal}\mcite[thm. 6.2]{Salomaa1973FormalLanguages}
  ...
  \begin{thmenum}
    ...
    Let \( S' \) be an entirely new start symbol and let \( V' \coloneqq V \cup \set{ S' } \). Define the production relation \( A \to' w \) to hold if \( w \) is \emph{nonempty} and if there exist strings \( w_0, \ldots, w_n \) over \( V \cup \Sigma \) and nonterminals \( B_1, \ldots, B_n \) from \( U \) such that
    \begin{equation*}
...
\begin{comments}
  \item This algorithm is implemented in \cite{notebook:code} as
  \begin{CodeRefDisplay}
    \GetCodeRef{alg:epsilon_rule_removal}
  \end{CodeRefDisplay}
\end{comments}
...
\begin{proposition}\label{thm:context_free_languages_are_context_sensitive}
  \hyperref[def:chomsky_hierarchy/context_free]{Context-free} \emph{languages} are \hyperref[def:chomsky_hierarchy/context_sensitive]{context-sensitive}.
\end{proposition}
\begin{proof}
...
\begin{comments}
  ...
  \item This algorithm is implemented in \cite{notebook:code} as
  \begin{CodeRefDisplay}
    \GetCodeRef{alg:renaming_rule_collapse}
  \end{CodeRefDisplay}
\end{comments}
...
\begin{remark}\label{con:backus_normal_form}\mcite[5]{Backus1959Syntax}
  ...
  For \hyperref[def:chomsky_hierarchy/context_free]{context-free grammars}, we can use the \term{Backus normal form} (BNF; used by \textcite[61]{Salomaa1973FormalLanguages}), sometimes also called \term{Backus-Naur form} (also BNF; used by \textcite[40]{AhoEtAl2006Compilers}). Backus himself in \cite[5]{Backus1959Syntax} described it via examples.
  ...
  \begin{bnf*}
    ...
    \bnfprod{operation}      {\bnftsq{\( \synmult \)} \bnfor \bnftsq{\( \synmult \)}} \\
  ...
  \begin{itemize}
    ...
    \item Different rules with the same source are combined as in \eqref{eq:ex:natural_number_arithmetic_grammar/schema/shorthand}. This is also discussed in \cref{con:syntax_fragment}.
...
\end{remark}
\begin{comments}
  \item We will use Backus normal forms in more complicated grammars like those across \fullref{ch:mathematical_logic} and \fullref{ch:lambda_calculus}, while preferring more the primitive notation \eqref{eq:ex:natural_number_arithmetic_grammar/schema/simple} for generic examples relating to grammars.

  \item Note that we have placed the (only) \hyperref[con:abstract_syntax_tree/syntactic]{syntactic rule} on top.

  \item There are several established formal grammars for variations of BNF, for example \enquote{extended BNF} (\enquote{EBNF}; see \cite{ISO:14977:1996}) and \enquote{augmented BNF} (\enquote{ABNF}; see \cite{IETF:RFC5234}).
\end{comments}
