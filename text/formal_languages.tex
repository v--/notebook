...
\begin{example}\label{ex:def:chomsky_hierarchy}
  ...
  \begin{thmenum}
    ...
    Since \( G \) is essentially length-increasing, its language is context-sensitive\fnote{\tcite{#1 shows in #2 that}[example 6.2]{Salomaa1973FormalLanguages} the language \( \mscrL \) cannot be context-free.}. Because of the rule \( cB \to bC \), however, the grammar itself is not context-sensitive.
