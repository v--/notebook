\section{Nonsmooth derivatives}\label{sec:nonsmooth_derivatives}

\begin{remark}\label{rem:nonsmooth_analysis}
  Nonsmooth analysis studies generalized differentiability for functions which are not necessarily differentible. The generalized derivatives (see \fullref{sec:nonsmooth_derivatives}) are not linear, which motivates the study of subdifferentials (see \fullref{sec:subdifferentials}).
...
\begin{definition}\label{def:nonsmooth_derivatives}
  ...
  \begin{thmenum}
    \thmitem{def:nonsmooth_derivatives/directional}\Cref{def:differentiability} already introduced directional derivatives. Here we introduce a special notation for them:
    \begin{equation*}
      D_h^+ f(x_0) = f_+'(x_0)(h) \coloneqq \lim_{t \downarrow 0} \frac {f(x_0 + th) - f(x_0)} t.
    \end{equation*}

    \thmitem{def:nonsmooth_derivatives/dini}\mcite[definition 11.18]{Clarke2013OptimalControl} The upper (resp. lower) \term{Dini derivative} is defined as
    \begin{balign*}
      \oline{D}_h f(x_0) = \oline{f'}(x_0)(h) & \coloneqq \limsup_{t \downarrow 0} \frac {f(x_0 + th) - f(x_0)} t
      \\
    ...
    \thmitem{def:nonsmooth_derivatives/clarke}\mcite[section 10.1]{Clarke2013OptimalControl} The \term{generalized Clarke derivative} is defined as
    \begin{equation*}
      D_h^\circ f(x_0)
      =
      f^\circ(x_0)(h)
    ...
    Refer to \fullref{sec:clarke_gradients} for their usefulness.
  \end{thmenum}
\end{definition}
