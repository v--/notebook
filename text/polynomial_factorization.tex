...
\begin{definition}\label{def:graded_ring}\mcite[def. II.11.2]{Bourbaki1998Algebra1to3}
  ...
  An \( M \)-\term[bg=градуиран (пръстен) (\cite[56]{КоцевСидеров2016КомутативнаАлгебра}), ru=градуированное (кольцо) (\cite[63]{Шафаревич2019ОсновныеПонятияАлгебры}), en=graded (ring) (\cite[29]{Eisenbud1995CommutativeAlgebra})]{graded ring} is a commutative ring equipped with an \( M \)-grading.

  The additive subgroups of an \hyperref[def:algebra_over_ring]{algebra} are its \hyperref[def:module/subobject]{submodules}, which motivates \textcite[exerc. B-5.2]{Rotman2015AdvancedModernAlgebraPart1} and \textcite[197]{Винберг2014КурсАлгебры}) to define a \term[ru=градуированная (алгебра) (\cite[197]{Винберг2014КурсАлгебры}), en=graded algebra (\cite[exerc. B-5.2]{Rotman2015AdvancedModernAlgebraPart1})]{graded algebra} as an algebra equipped with a ring grading.
...
\begin{definition}\label{def:homogeneous_element}\mcite[63]{Шафаревич2019ОсновныеПонятияАлгебры}
  ...
  If \( x \) has only one nonzero homogeneous component \( x_d \), we say that it is a \term[bg=еднороден елемент (\cite[57]{КоцевСидеров2016КомутативнаАлгебра}), ru=однородный элемент]{homogeneous element} of degree \( d \). In particular, we leave the degree of \( 0 \) \hyperref[con:undefinedness]{undefined}.
\end{definition}
...
\end{comments}
\begin{proof}
  A homogeneous element of degree \( n \) is a linear combination of monomials of the degree \( n \). By definition, the product of monomials of degree \( n \) and \( m \) is a monomial of degree \( n + m \). Then, due to linearity of the convolution product, it follows that the product of homogeneous elements of degree \( n \) and \( m \) is either zero or of degree \( n + m \) (depending on whether the leading coefficients multiply to zero; see \cref{thm:univariate_polynomial_product/degree}).
...
\end{definition}
\begin{comments}
  \item Some authors use \enquote{form} as a secondary term for homogeneous polynomials, for example \textcite[13]{Eisenbud1995CommutativeAlgebra}, \textcite[314]{Курош1968КурсВысшейАлгебры} and \textcite[2]{Обрешков1962ВисшаАлгебра}. We only use the term for \hyperref[def:bilinear_form]{bilinear} and \hyperref[def:quadratic_form]{quadratic forms}.
...
\begin{proposition}\label{thm:homogeneous_polynomial_iff_homogeneous_function}
  Fix a \hyperref[def:semiring]{(semi)ring} \( R \), an arbitrary \hyperref[def:algebra_over_semiring/commutative]{commutative associative unital \( R \)-algebra} \( M \) and a \hyperref[def:polynomial_algebra]{polynomial algebra} \( R[\mscrX] \).

  If a polynomial \( f \) in \( R[\mscrX] \) is \hyperref[def:homogeneous_polynomial]{homogeneous} of degree \( d \), then the \hyperref[con:free_construction/evaluation]{evaluation} \( \Phi_M(f) \) is a \hyperref[def:homogeneous_function]{homogeneous function} of degree \( d \). The converse holds if \( R \) is an \hyperref[def:integral_domain]{integral domain}.
\end{proposition}
\begin{proof}
...
\begin{definition}\label{def:homogeneous_equation}\mimprovised
  If the polynomial corresponding to an \hyperref[def:polynomial_equation]{algebraic equation} is \hyperref[def:homogeneous_polynomial]{homogeneous}, we say that the equation itself is \term{homogeneous}.
\end{definition}

\begin{proposition}\label{thm:homogeneous_linear_equation}
  For a \hyperref[tab:def:polynomial_degree]{linear} \hyperref[def:polynomial_equation]{algebraic equation}, the following are equivalent:
  \begin{thmenum}
    \thmitem{thm:homogeneous_linear_equation/homogeneous} It is \hyperref[def:homogeneous_equation]{homogeneous}.

    \thmitem{thm:homogeneous_linear_equation/trivial_solution} It has a trivial solution.
...
\end{proposition}
\begin{proof}
  Follows from \cref{thm:univariate_polynomial_product/degree} by induction on the number of variables.
\end{proof}
...
\begin{proof}
  ...
  \begin{equation*}
    f(X_1, \ldots, X_n, Y) = Y^d \cdot g\parens*{ \frac {X_1} Y, \ldots, \frac {X_n} Y }.
  \end{equation*}

  Then
  \begin{equation*}
    f(X_1, \ldots, X_n, Y) = Y^d \cdot p\parens*{ \frac {X_1} Y, \ldots, \frac {X_n} Y } \cdot q\parens*{ \frac {X_1} Y, \ldots, \frac {X_n} Y }.
  \end{equation*}

  Hence,
  \begin{equation*}
    \widehat{p}(X_1, \ldots, X_n, Y) \coloneqq Y^{\deg p} \cdot p\parens*{ \frac {X_1} Y, \ldots, \frac {X_n} Y }
  \end{equation*}
  ...
  \begin{equation*}
    p(X_1, \ldots, X_n, Y) = Y^{\deg p} \underbrace{p\parens*{ \frac {X_1} Y, \ldots, \frac {X_n} Y, 1 }}_{\T{invertible constant}}
  \end{equation*}
...
\begin{definition}\label{def:symmetric_polynomial}\mcite[139]{Тыртышников2017ОсновыАлгебры}
  We say that a polynomial \( f(X_1, \ldots, X_n) \) over the \hyperref[def:semiring/commutative]{commutative (semi)ring} \( R \) is \term[bg=симетричен (полином) (\cite[58]{ГеновМиховскиМоллов1991Алгебра}), ru=симметрический (многочлен), en=symmetric (polynomial) (\cite[190]{Lang2002Algebra})]{symmetric} if, for any \hyperref[def:algebra_over_semiring/commutative]{commutative associative unital \( R \)-algebra} \( M \), the \hyperref[con:free_construction/evaluation]{evaluation} \( \Phi_M(f) \) is a \hyperref[def:symmetric_function]{symmetric function}, i.e. \( \Phi_M(f) \) is invariant under permutations of \( X_1, \ldots, X_n \).
\end{definition}
...
\begin{proof}
  ...
  \begin{balign*}
    a_k
    &=
    a_n \cdot \parens*{ (-1)^{n-k} \sigma_{n-k}(\alpha_1, \ldots, \alpha_{n-1}) - \alpha_n \cdot (-1)^{n-1-k} \cdot \sigma_{n-1-k}(\alpha_1, \ldots, \alpha_{n-1}) }
    = \\ &=
    a_n \cdot (-1)^{n-k} \cdot \parens*{ \sigma_{n-k}(\alpha_1, \ldots, \alpha_{n-1}) + \alpha_n \sigma_{n-1-k}(\alpha_1, \ldots, \alpha_{n-1}) }
...
\begin{definition}\label{def:sylvester_matrix}\mcite[136]{Тыртышников2017ОсновыАлгебры}
  We define the \term[ru=матрица Сильвестра, en=Sylvester's matrix (\cite[exercise 4.6.1.12]{Knuth1997ArtVol2})]{Sylvester matrix} of the non-constant polynomials
  \begin{align*}
...
\begin{definition}\label{def:resultant}\mcite[136]{Тыртышников2017ОсновыАлгебры}
  We define the \term[bg=резултанта (\cite[198]{Обрешков1962ВисшаАлгебра}), ru=результанта, en=resultant (\cite[674]{Knuth1997ArtVol2})]{resultant} of the non-constant univariate polynomials \( f(X) \) and \( g(X) \) as the \hyperref[def:matrix_determinant]{determinant}
  \begin{equation*}
...
\end{definition}
\begin{comments}
  \item Discriminants are often defined via the roots of \( f(X) \) and later this property is proven to be equivalent, for example in
  \cite[227]{Кострикин2020АлгебраЧасть1},
  \cite[258]{Jacobson1985BasicAlgebraI},
  \cite[192]{Lang2002Algebra} and
  \cite[223]{Rotman2015AdvancedModernAlgebraPart1}.
...
\begin{proof}
  ...
  \begin{equation}\label{eq:thm:derivative_at_polynomial_root/product_derivative}
    f'(X) = \parens*{ \prod_{k=1}^{n-1} (X - \alpha_k) }' \cdot (X - \alpha_n) + \prod_{k=1}^{n-1} (X - \alpha_k) \cdot \underbrace{(X - \alpha_n)'}_{1}.
  \end{equation}
  ...
  \begin{itemize}
    ...
    \begin{equation*}
      f'(\alpha_m) = \parens*{ \prod_{\substack{k \neq m \\ k \neq n}} (\alpha_m - \alpha_k) } \cdot (\alpha_m - \alpha_n) + \underbrace{\prod_{k=1}^{n-1} (\alpha_m - \alpha_k)}_{0}.
    \end{equation*}
  \end{itemize}
\end{proof}

\begin{proposition}\label{thm:discriminant_as_product}
  Fix an \hyperref[def:algebraically_closed_field]{\emph{algebraically closed}} \hyperref[def:field]{field} \( \BbbK \) and a polynomial
  \begin{equation*}
    f(X) = \sum_{k=0}^n a_k X^k.
  \end{equation*}
...
\begin{proof}
  ...
  Then
  \begin{equation*}
    a_n p^n = -q \parens*{ \sum_{k=0}^{n-1} a_k p^k q^{n-k} }.
  \end{equation*}
...
\begin{proof}
  ...
  \begin{align*}
    ...
    \frac {(-1)} a \cdot \parens*{ ab^2 + 0 + 4a^2c - 0 - 0 - 2ab^2 }
    = \\ &=
    b^2 - 4ac.
  \end{align*}
\end{proof}
...
\begin{proof}
  ...
  Aiming at a contradiction, suppose that both are not invertible. Then both have positive degree. \Cref{thm:divisors_of_homogeneous_polynomial} implies that both \( g(X, Y) \) and \( h(X, Y) \) are homogeneous, and \cref{thm:univariate_polynomial_product/degree} leaves only the possibility that
  \begin{align*}
    q(X, Y) &= c X^m + d Y^m + e \\
