...
\begin{definition}\label{def:graded_module}\mcite[def. II.11.3]{Bourbaki1998Algebra1to3}
  ...
  An \( M \)-\term{grading} of the left (resp. right) \( R \)-\hyperref[def:module]{module} \( A \) compatible with \( \seq{ R_d }_{d \in M} \) is a family \( \seq{ A_d }_{d \in M} \) of \hyperref[def:module/submodule]{submodules} such that
  \begin{equation*}
...
\begin{definition}\label{def:trivial_module_grading}\mimprovised
  Similarly to how we defined trivial ring gradings in \fullref{def:trivial_ring_grading}, we can define, for every \( R \)-module \( A \) and every commutative monoid \( M \), a trivial \hyperref[def:graded_module]{module grading} of \( A \) compatible with any grading of \( R \).
\end{definition}

\begin{definition}\label{def:graded_algebra}\mcite[def. III.3.1]{Bourbaki1998Algebra1to3}
...
\begin{definition}\label{def:homogeneous_element}\mcite[63]{Шафаревич1999Алгебра}
  A \term[ru=разложение на однородные составляющие]{homogeneous decomposition} of an element \( x \) in an \( M \)-\hyperref[def:graded_ring]{graded ring} (resp. \hyperref[def:graded_module]{graded module}) \( R \) is the indexed family \( \seq{ x_d }_{d \in M} \) such that \( x_d \) is in \( R_d \) and
  \begin{equation}
    x = \sum_{d \in M} x_d.
  \end{equation}
...
\begin{definition}\label{def:homogeneous_polynomial}\mcite[139]{Jacobson1985AlgebraI}
...
\begin{comments}
  ...
  \item Homogeneous polynomials are precisely the \hyperref[def:homogeneous_element]{homogeneous elements} of the polynomial algebra with respect to the degree \hyperref[def:algebra_grading]{grading} discussed in \fullref{thm:polynomial_algebra_grading}. The zero polynomial being homogeneous of every degree agrees with this grading.
\end{comments}
...
\begin{comments}
  ...
  \incite[258]{Jacobson1985AlgebraI},
  \incite[192]{Lang2002Algebra} and
  \incite[223]{Rotman2015AlgebraPart1}.
