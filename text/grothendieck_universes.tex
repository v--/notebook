...
\begin{proposition}\label{thm:grothendieck_universe_closure}
  Every \hyperref[def:grothendieck_universe]{Grothendieck universe} \( \mscrU \) has the following closure properties:
  \begin{thmenum}
    \thmitem{thm:grothendieck_universe_closure/member} If \( A \) belongs to \( \mscrU \), so do all members of \( A \).

    The converse is not true --- all members of \( \mscrU \) belong to \( \mscrU \), but it cannot belong to itself.

    \thmitem{thm:grothendieck_universe_closure/hereditarily_finite} All \hyperref[def:universe_of_hereditary_finite_sets]{hereditarily finite sets} belong to \( \mscrU \).

    \thmitem{thm:grothendieck_universe_closure/power_set} A belongs to \( \mscrU \) if and only if its \hyperref[def:basic_set_operations/power_set]{power set} does.

    \thmitem{thm:grothendieck_universe_closure/subset} The set \( A \) belongs to \( \mscrU \) if and only if all its subsets do.

    \thmitem{thm:grothendieck_universe_closure/binary_union} The \hyperref[def:basic_set_operations/union]{binary union} \( A \cup B \) belongs to \( \mscrU \) if and only if both \( A \) and \( B \) do.

    \thmitem{thm:grothendieck_universe_closure/kuratowski_pair} The \hyperref[def:kuratowski_pair]{Kuratowski pair}
    \begin{equation*}
      \braket{ A, B } = \set{ \set{ A }, \set{ A, B } }
    \end{equation*}
    belong to \( \mscrU \) if and only if both \( A \) and \( B \) do.

    \thmitem{thm:grothendieck_universe_closure/unordered_pair} The unordered pair \( \set{ A, B } \) belongs to \( \mscrU \) if and only if both \( A \) and \( B \) do.

    \thmitem{thm:grothendieck_universe_closure/cartesian_product} The \hyperref[def:cartesian_product]{Cartesian product} \( \prod_{k \in \mscrK} A_k \) belongs to \( \mscrU \) if and only if both \( \mscrK \) and each set \( A_k \) belong to \( \mscrU \).

    \thmitem{thm:grothendieck_universe_closure/indexed_family} The \hyperref[def:indexed_family]{indexed family} \( \seq{ A_k }_{k \in \mscrK} \) belongs to \( \mscrU \) if and only if both \( \mscrK \) and each set \( A_k \) belong to \( \mscrU \).

    In particular, the \hyperref[def:ordered_tuple]{ordered tuple} \( (A_1, \ldots, A_n) \) belongs to \( \mscrU \) if and only if \( A_1, \ldots, A_n \) do.

    \thmitem{thm:grothendieck_universe_closure/function} The \hyperref[def:set_valued_map/graph]{graph} \( \gph f \) of the \hyperref[def:function]{function} \( f: A \to B \) belongs to \( \mscrU \) if and only if both its domain \( A \) and image \( f[A] \) are. Equivalently, both \( A \) and \( f(a) \) for every \( a \) in \( A \) must belong to \( \mscrU \).

    If, in accordance with \cref{rem:function_definition}, we encode \( f: A \to B \) as the triple \( (\gph f, A, B) \), then this triple belongs to \( \mscrU \) if and only if \( A \) and \( B \) (not \( f[A] \)) do.

    See \cref{rem:small_functions} for a discussion of this ambiguity.
  \end{thmenum}
\end{proposition}
\begin{proof}
  \SubProofOf{thm:grothendieck_universe_closure/member} Restatement of \ref{def:grothendieck_universe/transitive}.

  \SubProofOf{thm:grothendieck_universe_closure/hereditarily_finite} By \ref{def:grothendieck_universe/inductive}, \( \mscrU \) contains an inductive set, and hence the smallest inductive set \( \omega \).
  ...
  Then, by \ref{def:grothendieck_universe/union}, the union \( V_\omega = \bigcup_{n \in \omega} V_n \) also belongs to \( \mscrU \).

  \SubProofOf{thm:grothendieck_universe_closure/power_set}

  \SufficiencySubProof* If \( A \) belongs to \( \mscrU \), by \ref{def:grothendieck_universe/power_set}, so does \( \pow(A) \).

  \NecessitySubProof* Conversely, if \( \pow(A) \) belongs to \( \mscrU \), then, by \ref{def:grothendieck_universe/transitive}, so does \( A \) as a member of \( \pow(A) \).

  \SubProofOf{thm:grothendieck_universe_closure/subset}

  \SufficiencySubProof* If \( A \) belongs to \( \mscrU_0 \) and \( B \) is a subset of \( A \), then \( B \) is a member of \( \pow(A) \). Then \( \pow(A) \) belongs to \( \mscrU \) by \ref{def:grothendieck_universe/power_set}, and \( B \) belongs to \( \mscrU \) by \ref{def:grothendieck_universe/transitive}.

  \NecessitySubProof* Conversely, since \( A \) is a subset of itself, if every subset of \( A \) belongs to \( \mscrU \), so does \( A \).

  \SubProofOf{thm:grothendieck_universe_closure/binary_union}

  \SufficiencySubProof* \Cref{thm:grothendieck_universe_closure/hereditarily_finite} implies that the set \( \set{ 0, 1 } \) belongs to \( \mscrU \). By \ref{def:grothendieck_universe/union}, the union \( A \cup B \) of the indexed family \( \set{ \braket{ 0, A }, \braket{ 1, B } } \) also belongs to \( \mscrU \).

  Note that \ref{def:grothendieck_universe/union} says nothing about the Kuratowski pairs \( \braket{ 0, A } \) and \( \braket{ 1, B } \), nor about the set containing them.

  \NecessitySubProof* Conversely, \cref{thm:grothendieck_universe_closure/subset} implies that, if \( A \cup B \) belongs to \( \mscrU \), so do its subsets \( A \) and \( B \).

  \SubProofOf{thm:grothendieck_universe_closure/kuratowski_pair}

  \SufficiencySubProof* Suppose that \( A \) and \( B \) belong to \( \mscrU \). \Cref{thm:grothendieck_universe_closure/binary_union} implies that so does the union \( A \cup B \).
  ...
  \begin{equation*}
    \braket{ A, B } = \set{ \set{ A }, \set{ A, B } }
  \end{equation*}
  belongs to \( \pow^3(A \cup B) \). Since the latter belongs to \( \mscrU \), by the transitivity axiom \ref{def:grothendieck_universe/transitive}, so does the pair \( \braket{ A, B } \) itself.

  \NecessitySubProof* Conversely, if \( \braket{ A, B } \) belongs to \( \mscrU \), so does its member \( \set{ A, B } \), and its members \( A \) and \( B \).

  \SubProofOf{thm:grothendieck_universe_closure/unordered_pair}

  \SufficiencySubProof* \Cref{thm:grothendieck_universe_closure/kuratowski_pair} implies that the Kuratowski pair \( \braket{ A, B } \) belongs to \( \mscrU \).

  Since \( \set{ A, B } \) is a subset of \( \braket{ A, B } \), by \cref{thm:grothendieck_universe_closure/subset}, the unordered pair \( \set{ A, B } \) also belongs to \( \mscrU \).

  \NecessitySubProof* Conversely, if \( \set{ A, B } \) belongs to \( \mscrU \), so do its members \( A \) and \( B \).

  \SubProofOf{thm:grothendieck_universe_closure/cartesian_product}

  \SufficiencySubProof* Suppose that \( \mscrK \) and \( A_k \) for each \( k \) in \( \mscrK \) belong to \( \mscrU \).

  By the union schema \ref{def:grothendieck_universe/union}, the set
  \begin{equation*}
    \mscrA \coloneqq \bigcup_{k \in \mscrK} A_k
  \end{equation*}
  belongs to \( \mscrU \).

  The Cartesian product \( \prod_{k \in \mscrK} A_k \) consists of indexed families \( \seq{ a_k }_{k \in \mscrK} \), each of which is a set of Kuratowski pairs \( \braket{ k, a_k } \), where \( k \) is from \( \mscrK \) and \( a_k \) is from \( A_k \subseteq \mscrA \).

  Since \( k \) and \( a_k \) belong to \( \mscrK \cup \mscrA \), the sets \( \set{ k } \) and \( \set{ k, a_k } \) belong to \( \pow(\mscrK \cup \mscrA) \), and
  \begin{equation*}
    \braket{ k, a_k } = \set{ \set{ k }, \set{ k, a_k } }
  \end{equation*}
  belongs to \( \pow^2(\mscrK \cup \mscrA) \).

  A single indexed family \( \seq{ a_k }_{k \in \mscrK} \) is thus a subset of \( \pow^2(\mscrK \cup \mscrA) \), i.e. a member of \( \pow^3(\mscrK \cup \mscrA) \). Consequently, the product \( \prod_{k \in \mscrK} A_k \) belongs to \( \pow^4(\mscrK \cup \mscrA) \).
  ...
  Therefore, by \ref{def:grothendieck_universe/transitive}, the product \( \prod_{k \in \mscrK} A_k \) belongs to \( \mscrU \).

  \NecessitySubProof* Conversely, suppose that \( \prod_{k \in \mscrK} A_k \) belongs to \( \mscrU \).

  We can regard the product as a family indexed by itself, and \ref{def:grothendieck_universe/union} allows us to take its union. Denote this union by \( \mscrA_1 \).

  This union \( \mscrA_1 \) consists of all Kuratowski pairs \( \braket{ k, a_k } \), where \( k \) is in \( \mscrK \) and \( a_k \) is in \( A_k \).

  The union \( \mscrA_2 \) of \( \mscrA_1 \) then consists of all singleton sets \( \set{ k } \) and unordered pairs \( \set{ k, a_k } \).

  The union \( \mscrA_3 \) of \( \mscrA_2 \) must then contain all members of \( \mscrK \) and of \( A_k \) for every \( k \) in \( \mscrK \).

  Since \( \mscrA_3 \) belongs to \( \mscrU \), by \cref{thm:grothendieck_universe_closure/subset}, so do its subsets \( \mscrK \) and \( A_k \) for \( k \) in \( \mscrK \).

  \SubProofOf{thm:grothendieck_universe_closure/indexed_family}

  \SufficiencySubProof* Suppose that \( \mscrK \) belongs to \( \mscrU \), and so does \( A_k \) for every \( k \) in \( \mscrK \).

  By \cref{thm:grothendieck_universe_closure/binary_union}, for every set \( A_k \) from \( \mscrU \), the singleton \( \set{ A_k } \) also belongs to \( \mscrU \).
  ...
  \NecessitySubProof* Conversely, suppose that \( \seq{ A_k }_{k \in \mscrK} \) belongs to \( \mscrU \).

  As a set, it consists of Kuratowski pairs \( \braket{ k, A_k } \). By \ref{def:grothendieck_universe/union}, its union, which we denote by \( \mscrA_1 \), then consists of singleton sets \( \set{ k } \) and pairs \( \set{ k, A_k } \).

  The union \( \mscrA_2 \) of \( \mscrA \) then contains \( k \) and \( A_k \) for every \( k \) in \( \mscrK \). Since it belongs to \( \mscrU \), so does \( A_k \) by \ref{def:grothendieck_universe/transitive}, and so does \( \mscrK \) by \cref{thm:grothendieck_universe_closure/subset}.

  \SubProofOf{thm:grothendieck_universe_closure/function} For the graph \( \gph f \), \cref{thm:grothendieck_universe_closure/indexed_family} implies that \( \gph f \) is in \( \mscrU \) if and only if \( A \) and \( f(a) \) for every \( a \) in \( A \) are in \( \mscrU \).

  For the triple \( f = (A, B, \gph f) \), \cref{thm:grothendieck_universe_closure/indexed_family} implies that \( f \) is in \( \mscrU \) if and only if \( A \), \( B \) are \( \gph f \) are. The last condition is redundant since \( \gph f \subseteq A \times B \).
\end{proof}
