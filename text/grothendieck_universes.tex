...
\begin{proposition}\label{thm:grothendieck_universe_closure}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:grothendieck_universe_closure/function} The \hyperref[def:set_valued_map/graph]{graph} \( \gr f \) of the \hyperref[def:function]{function} \( f: A \to B \) belongs to \( \mscrU \) if and only if both its domain \( A \) and image \( f[A] \) are. Equivalently, both \( A \) and \( f(a) \) for every \( a \) in \( A \) must belong to \( \mscrU \).

    If, in accordance with \cref{rem:function_definition}, we encode \( f: A \to B \) as the triple \( (\gr f, A, B) \), then this triple belongs to \( \mscrU \) if and only if \( A \) and \( B \) (not \( f[A] \)) do.
...
\begin{proof}
  ...
  \SubProofOf{thm:grothendieck_universe_closure/function} For the graph \( \gr f \), \cref{thm:grothendieck_universe_closure/indexed_family} implies that \( \gr f \) is in \( \mscrU \) if and only if \( A \) and \( f(a) \) for every \( a \) in \( A \) are in \( \mscrU \).

  For the triple \( f = (A, B, \gr f) \), \cref{thm:grothendieck_universe_closure/indexed_family} implies that \( f \) is in \( \mscrU \) if and only if \( A \), \( B \) are \( \gr f \) are. The last condition is redundant since \( \gr f \subseteq A \times B \).
\end{proof}
