...
\begin{lemma}\label{thm:grothendieck_universe_contains_finite_sets}
  Every Grothendieck universe is a superset of universe of hereditarily finite sets \hyperref[def:universe_of_hereditary_finite_sets]{\( V_\omega \)}.
\end{lemma}
\begin{proof}
...
\begin{proof}
  ...
  Now that we have defined \( \widehat \mscrU \), it remains to verify that it is itself a universe. To show \ref{def:grothendieck_universe/nonempty}, note that \( V_\omega \in \mscrU \) by \cref{thm:grothendieck_universe_contains_finite_sets} and hence, by \ref{def:grothendieck_universe/transitive}, \( \omega \in \mscrU \).
...
\begin{definition}\label{def:large_and_small_sets}\mcite[22]{MacLane1998CategoryTheory}
  Suppose \( \mscrV = (V, I) \) is a \hyperref[def:fol_semantics/model]{model} of \logic{ZFC+U}. Let \( \mscrU \) be a fixed Grothendieck universe.

  We say that a set \( A \) is \( \mscrU \)-\term{small} if \( A \in \mscrU \) and \( \mscrU \)-\term{moderate} if \( A \subseteq \mscrU \). This situation resembles the difference between sets and proper classes described in \cref{def:set_builder_notation}.
...
\begin{example}\label{ex:large_and_small_sets}
  ...
  \begin{itemize}
    ...
    \item If \( \kappa < \mu \) are regular strong limit cardinals, the stage \( V_\kappa \) of von Neumann's hierarchy is \( V_\mu \)-small by \cref{thm:def:cumulative_hierarchy/membership}.
  \end{itemize}
\end{example}
...
\begin{proof}
  ...
  \SubProofOf*{def:grothendieck_universe/nonempty} Since \( \kappa \) is infinite and thus \( 0 < \kappa \), from \cref{thm:def:cumulative_hierarchy/membership} it follows that \( V_0 \in V_\kappa \).
  ...
  \begin{itemize}
    \item From \cref{thm:grothendieck_universe_contains_finite_sets} it follows that \( \varnothing \in \mscrU \).
