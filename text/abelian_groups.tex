...
\begin{concept}\label{con:additive_semigroup}\mimprovised
  ...
  Multiplicative semigroups use the notation of \cref{def:semigroup}, however additive semigroups differ as follows:
  \begin{thmenum}
    \thmitem{con:additive_semigroup/addition} The operation is denoted by \( + \) and called \term[en=addition (\cite[180]{Schechter1997AnalysisHandbook})]{addition}.
    ...
    In the case of a \hyperref[def:binary_operation/commutative]{commutative} \hyperref[def:monoid]{monoid}, if multiplication is extended to two elements of the monoid, we instead obtain a \hyperref[def:semiring]{semiring}.
...
\begin{remark}\label{rem:additive_function_terminology}
  ...
  \begin{itemize}
    ...
    \item \Textcite[def. 2.2.1]{HillePhillips1996FunctionalAnalysis} use \enquote{additive transformation} for functions between \hyperref[con:additive_semigroup]{additive groups} satisfying \eqref{eq:def:additive_function}. Later in \cite[def. 2.5.1]{HillePhillips1996FunctionalAnalysis}, they use \enquote{subadditive functional} for real-valued linear functionals satisfying \eqref{eq:def:additive_function/sub}.
...
\begin{definition}\label{def:multiplicative_function}\mimprovised
  We say that a function between \hyperref[con:additive_semigroup]{multiplicative (semi)groups} is \term{multiplicative} if it is a \hyperref[def:semigroup/morphism]{semigroup homomorphism}. Explicitly, \( f: G \to H \) is multiplicative if, for all \( x \) and \( y \) in \( G \), we have
...
\begin{remark}\label{rem:multiplicative_function_terminology}
  ...
  \begin{itemize}
    \item \Textcite[342]{ЛюстерникСоболев1965ФункциональныйАнализ} and \textcite{Кострикин2000АлгебраЧасть1} refer to \eqref{eq:def:multiplicative_function} as \enquote{multiplicativity}.
...
\begin{definition}\label{def:abelian_group}\mcite[44; 62]{Aluffi2009Algebra}
  \hyperref[def:binary_operation/commutative]{Commutative} \hyperref[def:group]{groups} are called \term[bg=абелева группа (\cite[390]{Обрешков1962ВисшаАлгебра}), ru=абелева группа (\cite[81]{Тыртышников2017ОсновыАлгебры})]{abelian}. They turn out to be important enough for their \hyperref[def:category]{category} to have an established notation --- \( \cat{Ab} \).
\end{definition}
\begin{comments}
...
\begin{proposition}\label{thm:derivative_abelian_groups}
...
\begin{definition}\label{def:group_commutator}\mcite[245]{Jacobson1985BasicAlgebraI}
  Let \( G \) be an arbitrary group. We define the \term[ru=коммутатор (\cite[448]{Винберг2014КурсАлгебры})]{commutator} of the elements \( x \) and \( y \) as
  \begin{equation*}
    [x, y] \coloneqq \underbrace{xy(yx)^{-1}}_{xyx^{-1}y^{-1}}.
  \end{equation*}

  We call the subgroup \hyperref[def:group/subobject]{generated} by all the commutators in \( G \) the \term[ru=коммутант (группы) (\cite[448]{Винберг2014КурсАлгебры})]{commutator subgroup} and denote it by \( [G, G] \).
\end{definition}
\begin{comments}
