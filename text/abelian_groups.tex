...
\begin{remark}\label{rem:additive_function_terminology}
  ...
  \begin{itemize}
    ...
    \item \Textcite[def. 2.2.1]{HillePhillips1996FunctionalAnalysis} uses \enquote{additive transformation} for functions between \hyperref[con:additive_semigroup]{additive groups} satisfying \eqref{eq:def:additive_function}. Later in \cite[def. 2.5.1]{HillePhillips1996FunctionalAnalysis}, they use \enquote{subadditive functional} for real-valued linear functionals satisfying \eqref{eq:def:additive_function/sub}.
...
\begin{remark}\label{rem:multiplicative_function_terminology}
  ...
  \begin{itemize}
    \item The property \eqref{eq:def:multiplicative_function} is referred to as \enquote{multiplicativity} in \cite[342]{ЛюстерникСоболев1965ФункциональныйАнализ} and \cite{Кострикин2000АлгебраЧасть1}.

    \item Multiplicative complex-valued functions, as used in \cite[141]{Jacobson1985BasicAlgebraI}.

    \item Completely multiplicative arithmetic functions, as used in \cite[33]{Apostol1976AnalyticNumberTheory},
    \cite[exerc. 1.2.4.29]{Knuth1997ArtVol1},
    \cite[def. 37]{Бухштаб1966ТеорияЧисел} and \cite[exerc. 4.28]{ГашковЧубариков2005СложностьВычисления}.

    \item Multiplicative linear functionals, as used in \cite[240]{Engelking1989GeneralTopology},
    \cite[def. 4.14.1]{HillePhillips1996FunctionalAnalysis} and \cite[364]{Rudin1987RealAndComplexAnalysis}.
  \end{itemize}
  ...
  \textcite[def. 18.1]{Rudin1987RealAndComplexAnalysis} and \textcite[prop. 4]{Хелемский2014ФункциональныйАнализ} refer to \eqref{eq:def:multiplicative_function/sub} as \enquote{multiplicative inequality}.
\end{remark}
...
\begin{definition}\label{def:abelian_group}\mcite[119]{Knapp2016BasicAlgebra}
  ...
  \begin{thmenum}
    \thmitem{def:abelian_group/category} \todo{}
  \end{thmenum}
\end{definition}
...
\begin{definition}\label{def:group_commutator}\mcite[313]{Knapp2016BasicAlgebra}
  ...
  We call the subgroup \hyperref[def:group/subobject]{generated} by all the commutators in \( G \) the \term{commutator subgroup} and denote it by \( [G, G] \).
\end{definition}
\begin{comments}
