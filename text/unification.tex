...
\begin{remark}\label{rem:identity_of_indiscernibles}
  Stephen Kleene suggests in \cite[163]{Kleene2002Logic} that, in \hyperref[def:bounded_hol]{second order logic}, the \hyperref[con:equality]{equality} formula \( \synx \syneq \syny \) can be regarded as an abbreviation for the \hyperref[con:schemas_and_instances]{formula schema}
...
\begin{definition}\label{def:equation}\mimprovised
  ...
  \begin{thmenum}
    ...
    Within the formalisms of \cref{def:equation_solution}, for an identity the solution set coincides with the entirety of \( X \).
  \end{thmenum}
\end{definition}
\begin{comments}
...
\begin{definition}\label{def:equation_solution}\mimprovised
  ...
  We can easily formalize this at the level of generality of \hyperref[def:higher_order_logic]{higher-order logic}. Let \( M \) and \( N \) be two terms that have the same type with respect to the \hyperref[def:type_context]{type context} \( x_1: \tau_1, \ldots, x_n: \tau_n \). Then, in for a fixed model, the solution set is simply the \hyperref[def:equalizers]{equalizer set} of \( \Bracks{M} \) and \( \Bracks{N} \):
  \begin{equation}\label{eq:def:equation_solution}
    \set[\Big]{ (a_1, \ldots, a_n) \in \Bracks{\tau_1} \times \cdots \times \Bracks{\tau_n} \given* \Bracks{M}(a_1, \ldots, a_n) = \Bracks{N}(a_1, \ldots, a_n) }.
  \end{equation}

  More often than not, we are working over \hyperref[def:ring]{rings}, \hyperref[def:module]{modules} or other \hyperref[def:abelian_group]{abelian groups}, where the zero element has a special role.

  If the zero is a solution to an equation (or a system), we call it the \term{trivial solution}.
\end{definition}

\begin{definition}\label{def:system_of_equations}
  We refer to nonempty sets of \hyperref[def:equation]{equations} as \term{systems}.

  We define the solution set of a system of equations as the intersection of the individual solution sets.
\end{definition}

\begin{definition}\label{def:fol_equation}
\end{definition}
