...
\begin{remark}\label{rem:identity_of_indiscernibles}
  ...
  This principle is related to \enquote{univalence} --- see \fullref{con:univalence_principle}.
\end{remark}
...
\begin{concept}\label{con:equality}
  ...
  In \cite[\enquote{When is One Thing Equal to Some Other Thing?}, page 222]{GoldSimonsEtAl2008ProofAndOtherDilemmas}, Barry Mazur writes
  \begin{displayquote}
...
\begin{example}\label{ex:def:equation}
  ...
  \begin{thmenum}
    ...
    \thmitem{ex:def:equation/fixed} \hyperref[def:function_fixed_point]{Fixed points} of functions are studied in different branches of mathematics. See \fullref{sec:lattice_fixed_points} and \fullref{thm:banach_fixed_point_theorem}.
    ...
    \thmitem{ex:def:equation/recurrence} \hyperref[def:recurrence_relation]{Recurrence relations} describe combinatorial sequences. See \fullref{sec:recurrence_relations}.
  \end{thmenum}
\end{example}
