...
\end{remark}
\begin{comments}
  \item This principle is related to \enquote{univalence} --- see \fullref{con:univalence_principle}.
\end{comments}
...
\begin{concept}\label{con:equality}
  ...
  Within the metalanguage, we use the symbol \( {\coloneqq} \) for definitional equality, \( {=} \) for propositional equality. In accordance with \fullref{rem:object_language_dots}, we place dots on top of the symbols when dealing with equality in the object language.
...
\begin{example}\label{ex:def:equation}
  ...
  \begin{thmenum}
    ...
    \thmitem{ex:def:equation/dynamical} \hyperref[def:abstract_dynamical_system]{Dynamical systems} are often specified via equations, for example \hyperref[def:recurrence_relation]{recurrence relations}, \hyperref[def:difference_equation]{difference equations} or differential equations. See \fullref{ch:dynamical_system}.
