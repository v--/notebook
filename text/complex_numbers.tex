...
\Cref{thm:ordered_field_not_algebraically_closed} implies that the \hyperref[def:field]{field} \( \BbbR \) of \hyperref[def:real_numbers]{real numbers} is not \hyperref[def:algebraically_closed_field]{algebraically closed}. This motivates the introduction of complex numbers.
...
\begin{definition}\label{def:complex_numbers}\mimprovised
  ...
  It is indeed a field because, by \cref{thm:axn_byn_irreducible} and \cref{thm:homogeneous_polynomial_constant}, the polynomial \( i^2 + 1 \) is irreducible in \( \BbbR \) and, by \cref{thm:quotient_by_irreducible_polynomial}, \( \BbbC \) is a \hyperref[def:field/subobject]{field extension} of \( \BbbR \) of \hyperref[def:field_extension_degree]{degree} \( 2 \).

  Here \( i \) is an indeterminate with no inherent semantics --- its semantics stem from the algebraic properties of the defined quotient. \Cref{thm:truncated_polynomial_algebra/isomorphism} allows us to unambiguously identify each coset in \( \BbbC \) with its \hyperref[def:quotient_polynomial_algebra_as_truncated]{principal representative}; thus, we regard members of \( \BbbC \) as linear polynomials with real coefficients:
...
\begin{comments}
  ...
  \item The definition as a quotient ring is given after knowledge of complex numbers is already assumed, for which reason complex numbers are often defined in a more elementary form --- by endowing \( \BbbR^2 \) with multiplication defined by \eqref{eq:def:complex_numbers/multiplication}. The quotient ring construction is given in \cite[example III.4.8]{Aluffi2009Algebra}, where it is shown to be equivalent to the vector space construction.

  We discuss an alternative definition via matrices in \cref{thm:complex_numbers_as_matrices} and as the \hyperref[def:semigroup_algebra]{group algebra} \( \BbbR[C_2] \) in \cref{ex:def:semigroup_algebra/cyclic}.
\end{comments}
...
\begin{proposition}\label{thm:complex_numbers_as_matrices}
  ...
  \begin{equation}\label{eq:thm:complex_numbers_as_matrices}
    ...
    \end{pmatrix}
  \end{equation}
  is a \hyperref[def:algebra_over_ring/subobject]{unital subalgebra} and is isomorphic to the \( \BbbR \)-algebra \( \BbbC \) of \hyperref[def:complex_numbers]{complex numbers}.
\end{proposition}
\begin{proof}
...
\end{corollary}
\begin{proof}
  Since \( 1 \) is by itself linearly independent over \( \BbbQ \), \fullref{thm:lindemann_weierstrass} implies that \( e \) is by algebraically independent over \( \BbbQ \). Hence, there exists no rational polynomial whose root is \( e \), and thus \( e \) satisfies \cref{def:transcendental_element}.
\end{proof}
...
\begin{example}\label{ex:polynomials_over_pi}
  \Cref{thm:pi_is_transcendental} implies that the polynomial ring \( \BbbQ[X] \) can be embedded into \( \BbbR \) via \( \Phi_\pi: \BbbQ[X] \to \BbbR \). We can thus identify a rational polynomial
  \begin{equation*}
...
\begin{definition}\label{def:principal_real_square_root}\mimprovised
  We extend \hyperref[def:semigroup_power]{principal square roots} as defined in \cref{def:principal_nonnegative_nth_root} to arbitrary \hyperref[def:real_numbers]{real numbers} by defining, for any \( x > 0 \),
  \begin{equation}\label{eq:def:principal_real_square_root/negative}
    \sqrt {-x} \coloneqq i \sqrt x.
  \end{equation}
\end{definition}
\begin{comments}
  \item We avoid defining square roots for arbitrary complex numbers, or every higher degree roots for negative real numbers, since it is difficult to make a decision which root should be principal. \Textcite[16]{Маркушевич1967АналитическиеФункцииТом1} defines principal roots for any complex number based on their \hyperref[def:complex_numbers_trigonometric_form]{trigonometric form}, however we prefer to avoid such a choice.
\end{comments}
...
\begin{proposition}\label{thm:real_quadratic_polynomial_roots}
  The \hyperref[def:polynomial_root]{roots} of the \hyperref[def:real_numbers]{real} \hyperref[tab:def:polynomial_degree]{quadratic} \hyperref[def:polynomial_algebra]{polynomial} \( f(X) = a X^2 + b X + c \) are
...
\begin{remark}\label{rem:conjugate_quadratic_roots}\mimprovised
  Consider a \hyperref[def:rational_numbers]{rational} \hyperref[tab:def:polynomial_degree]{quadratic} \hyperref[def:polynomial_algebra]{polynomial} \( f(X) = a X^2 + b X + c \).
  ...
  \Cref{thm:quadratic_extension_conjugate} then implies that the roots
  \begin{equation*}
...
\end{corollary}
\begin{proof}
  \Cref{thm:real_quadratic_polynomial_roots} implies that the roots of \( f(X) \) are
  \begin{align*}
...
\begin{proposition}\label{thm:def:complex_conjugation}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:def:complex_conjugation/involution} Conjugation is an \hyperref[def:morphism_invertibility/involution]{involution} of the complex numbers.

    \thmitem{thm:def:complex_conjugation/automorphism} Conjugation is a \hyperref[def:field/morphism]{field automorphism} of the complex numbers.
    ...
    \thmitem{thm:def:complex_conjugation/linear} As an endofunction on the vector space \( \BbbC \) over \( \BbbR \), complex conjugation is linear.
  \end{thmenum}
\end{proposition}
\begin{proof}
  ...
  \SubProofOf{thm:def:complex_conjugation/involution} Trivial.
  ...
  \SubProofOf{thm:def:complex_conjugation/linear} Additivity follows from \cref{thm:def:complex_conjugation/automorphism}. Then, if \( a \) is a real number, \cref{thm:def:complex_conjugation/fixed} implies that \( \oline a = a \), thus
  \begin{equation*}
    \oline {az} = \oline a \cdot \oline z = a \cdot \oline z.
  \end{equation*}
\end{proof}
...
\begin{proposition}\label{thm:def:complex_absolute_value}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:def:complex_absolute_value/integers} Complex absolute values extend integer absolute values as defined in \cref{def:integer_absolute_value}: \( \abs{n} = \abs{n}_\BbbZ \) for any integer \( n \).
  \end{thmenum}
\end{proposition}
\begin{proof}
  \SubProofOf{thm:def:complex_absolute_value/idempotent} By positive definiteness, we have \( \abs z \geq 0 \). \Cref{thm:def:complex_conjugation/fixed} then implies that \( \abs z \) is a fixed point of conjugation, hence \( \abs z = \oline{\abs z} \) and
  \begin{equation*}
    \abs{\abs z}
    =
  ...
  \SubProofOf{thm:def:complex_absolute_value/negative} Follows from \cref{thm:def:complex_absolute_value/nonnegative} by noting that, by \cref{thm:def:ordered_semiring_positivity/additive_inverse}, a real number is positive if and only if its additive inverse is negative.

  \SubProofOf{thm:def:complex_absolute_value/integers} Follows from \cref{thm:def:complex_absolute_value/nonnegative} and \cref{thm:def:complex_absolute_value/negative}.
\end{proof}
