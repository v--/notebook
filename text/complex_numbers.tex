...
\begin{proof}
  ...
  \SubProofOf{thm:def:complex_conjugation/linear} Additivity follows from \fullref{thm:def:complex_conjugation/automorphism}. Then, if \( a \) is a real number, \fullref{thm:def:complex_conjugation/fixed} implies that \( \oline a = a \), thus
  \begin{equation*}
...
\end{proposition}
\begin{comments}
  \item If, as in \fullref{thm:real_quadratic_discriminant/positive} or \fullref{thm:real_quadratic_discriminant/zero}, a polynomial has only real roots, each of them is its own inverse under complex conjugation. In this case the proposition gives us no information.
\end{comments}
\begin{proof}
