...
\end{definition}
\begin{comments}
  \item Across our cited literature, \enquote{delooping} is used only in \bycite[10]{Perrone2024StartingCategoryTheory}. The same construction with the same notation but with no explicit name can be found in \bycite[exerc. 1.3.9]{Riehl2016CategoryTheory}.
\end{comments}
...
\begin{comments}
  ...
  \item As shown in \cref{ex:preorder_nonuniqueness} and discussed in \fullref{thm:order_category_isomorphism}, a preorder category may not be \hyperref[def:skeletal_category]{skeletal}.
\end{comments}
...
\begin{proof}
  ...
  \SubProof{Proof for partial orders} For a preorder category, there is an isomorphism between \( x \) and \( y \) if and only if there is both a morphism from \( x \) to \( y \) and one from \( y \) to \( x \). This isomorphism may not be unique as a consequence of \cref{ex:preorder_nonuniqueness}. Uniqueness requires \( x = y \) to hold in this case, which is in turn equivalent to partial order \hyperref[def:binary_relation/antisymmetric]{antisymmetry}.
\end{proof}
