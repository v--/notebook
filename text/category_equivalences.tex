\section{Category equivalences}\label{sec:category_equivalences}
...
\begin{remark}\label{rem:category_similarity}
  ...
  \begin{thmenum}
    ...
    Examples of isomorphic categories include \fullref{thm:order_category_isomorphism} and \cref{ex:isomorphism_of_directed_multigraph_categories}.

    \thmitem{rem:category_similarity/equivalence} A weaker but very useful notion is \term{category equivalence} defined in \cref{def:category_equivalence}.
  \end{thmenum}
\end{remark}

\begin{example}\label{ex:set_discr_cat_isomorphism}
  There is an isomorphism between the category \( \cat{Set} \) of small sets and \( \cat{DiscrCat} \) of all small \hyperref[def:discrete_category]{discrete categories}.

  Consider the pair of functors
  \begin{equation*}
    U: \cat{DiscrCat} \to \cat{Set},
  \end{equation*}
  which for any small category \( \cat{C} \) gives us its set of objects \( \obj(\cat{C}) \) and
  \begin{equation*}
    D: \cat{Set} \to \cat{DiscrCat},
  \end{equation*}
  which for any small set \( A \) gives us the \hyperref[def:discrete_category]{discrete category} whose set of objects is \( A \).

  It is obvious that for any small category \( \cat{C} \), the functor \( D \bincirc U \) is bijective on objects. We need to verify that it is bijective on morphisms, however, in order to prove that \( D \bincirc U \) is the identity functor on \( \cat{DiscrCat} \).
...
\begin{definition}\label{def:category_equivalence}\mcite[def. 1.3.15]{Leinster2014BasicCategoryTheory}
  ...
  If \( (F, G, \eta, \varepsilon) \) is an equivalence, we say that \( \cat{C} \) and \( \cat{D} \) are equivalent categories. This is justified because equivalence of categories is an equivalence relation --- see \cref{thm:category_equivalence_is_equivalence_relation}.

  Note that an equivalence is not necessarily an \hyperref[def:category_adjunction]{adjunction}, they simply have a common setup. This is discussed in \cref{thm:adjoint_equivalence}.
\end{definition}

\begin{proposition}\label{thm:category_equivalence_is_equivalence_relation}
  \hyperref[def:category_equivalence]{Category equivalence} is an \hyperref[def:equivalence_relation]{equivalence relation} on the set \( \obj(\cat{Cat}) \)
\end{proposition}
\begin{proof}
...
\begin{proposition}\label{thm:discrete_category_equivalence}
  Let \( \cat{C} \) and \( \cat{D} \) be \hyperref[def:small_category]{discrete categories} \hyperref[def:discrete_category]{discrete categories}. Then \( \cat{C} \) and \( \cat{D} \) are \hyperref[def:category_equivalence]{equivalent} if and only if the underlying sets \( \obj(\cat{C}) \) and \( \obj(\cat{D}) \) are \hyperref[def:equinumerosity]{equinumerous}.
\end{proposition}
\begin{proof}
  ...
  \end{equation*}
  for every object \( A \) of \( \cat{C} \). Since the only morphisms in \( \cat{C} \) are the identities, it follows that \( \eta_A = \id_A \) and hence \( [G \bincirc F](A) = A \). In particular, this implies that \( \eta \) is the identity natural transformation on \( \id_{\cat{C}} \) and that the restriction \( G\restr_{\obj(D)} \) is a left inverse of \( F\restr_{\obj(\cat{C})} \).
  ...
  \NecessitySubProof Suppose that \( F: \obj(\cat{C}) \to \obj(D) \) is a bijective function. Then it is an isomorphism in the category \( \cat{Cat} \), hence it induces an equivalence between \( \cat{C} \) and \( \cat{D} \).
\end{proof}
...
\begin{proposition}\label{thm:equivalence_induces_fully_faithful_and_essentially_surjective_functor}
  In any \hyperref[def:category_equivalence]{category equivalence} \( (F, G, \eta, \varepsilon) \), the functor \( F \) is \hyperref[def:functor_invertibility/fully_faithful]{fully faithful} and \hyperref[def:functor_invertibility/essentially_surjective]{essentially surjective on objects}.
...
\begin{proof}
  ...
  Therefore, \( \eta_B \bincirc f_1 = \eta_B \bincirc f_2 \) and, since \( \eta_B \) is left cancellable, \( f_1 = f_2 \).
  ...
  We have already shown that \( F \) is faithful and, by \cref{thm:category_equivalence_is_equivalence_relation}, \( G \) is also faithful. Since \( g \) and \( F(f) \) are parallel, it follows that they are equal.

  Therefore, \( F \) is full.
\end{proof}

\begin{remark}\label{rem:adjoint_equivalence_induces_fully_faithful_and_essentially_surjective_functor}
  In the fullness proof of \cref{thm:equivalence_induces_fully_faithful_and_essentially_surjective_functor}, we can use another argument if \( (F, G, \eta, \varepsilon) \) is an \hyperref[def:adjoint_equivalence]{adjoint equivalence}.
...
\begin{theorem}[Fully faithful and essentially surjective functor induces equivalence]\label{thm:fully_faithful_and_essentially_surjective_functor_induces_equivalence}
  ...
  In \hyperref[def:zfc]{\logic{ZF}}, this theorem is equivalent to the \hyperref[def:zfc/choice]{axiom of choice}. We list equivalent results in \cref{thm:axiom_of_choice_equivalences}.

  We prove the converse of this statement separately in \cref{thm:equivalence_induces_fully_faithful_and_essentially_surjective_functor}.
\end{theorem}
\begin{proof}
  ...
  We have implicitly used \cref{ex:def:categorical_diagram/inverting_isomorphisms} above.
  ...
  \begin{equation*}
    ...
    \end{aligned}
  \end{equation*}
  so that \( F(\eta_A) = \varepsilon_{F(A)}^{-1} \). By \cref{thm:def:functor_invertibility/fully_faithful_reflects_invertible}, since \( \varepsilon_{F(A)} \) is an isomorphism, \( \eta_A \) is also an isomorphism.
...
\begin{example}\label{ex:skeleton_of_set}
  The \hyperref[def:small_set]{small} \hyperref[def:cardinal]{cardinal numbers} are a \hyperref[def:skeletal_category]{skeletal subcategory} of \( \cat{Set} \).
\end{example}
...
\begin{proof}
  ...
  In order for \( \Iota \) to be a full functor, \( \cat{S} \) must be a full subcategory. Therefore, when building \( \cat{S} \), we can only remove objects and must preserve the \( \hom \)-sets for the remaining objects.
...
\begin{definition}\label{def:monoid_delooping}\mcite[10]{Perrone2024StartingCategoryTheory}
  Let \( (M, \cdot, e) \) be a \hyperref[def:monoid]{monoid}. The \term{delooping} \( \cat{B} M \) of \( M \) is the following category:
  \begin{itemize}
    \item The \hyperref[def:category/objects]{set of objects} \( \obj(\cat{B} M) \) is the singleton set \( \set{ \Anon } \), where \( \Anon \) is any set not in the set of all morphisms \( M \).

    \item The only \hyperref[def:category/morphisms]{set of morphisms} \( \cat{B} M(\Anon) \) is the underlying set \( M \) of the monoid.
...
\end{definition}
\begin{comments}
  \item Across our cited literature, the term \enquote{delooping} is used only in \cite[10]{Perrone2024StartingCategoryTheory}.
\end{comments}

\begin{proposition}\label{thm:delooping_of_opposite_monoid}
  The \hyperref[def:monoid_delooping]{delooping} of the \hyperref[def:monoid/opposite]{opposite monoid} \( M^\oppos \) of \( M \) is the \hyperref[def:opposite_category]{opposite category} of the delooping of \( M \).
\end{proposition}
\begin{proof}
  Trivial.
\end{proof}
...
\begin{definition}\label{def:preorder_category}\mcite[11]{MacLane1998CategoryTheory}
...
\begin{comments}
  ...
  \item As shown in \cref{thm:preordered_set_antisymmetry} and discussed in \fullref{thm:order_category_isomorphism}, a preorder category may not be \hyperref[def:skeletal_category]{skeletal}.
\end{comments}

\begin{theorem}[Ordered sets as categories]\label{thm:order_category_isomorphism}
  The category \( \cat{PreOrd} \) of small \hyperref[def:preordered_set]{preordered sets} and (nonstrict) \hyperref[def:order_function/preserving]{order-preserving maps} is \hyperref[rem:category_similarity/isomorphism]{isomorphic} to the category of small \hyperref[def:preorder_category]{preorder categories} and their functors.
...
\begin{proof}
  ...
  \SubProof{Proof for partial orders} For a preorder category, there is an isomorphism between \( x \) and \( y \) if and only if there is both a morphism from \( x \) to \( y \) and one from \( y \) to \( x \). This isomorphism may not be unique as a consequence of \cref{thm:preordered_set_antisymmetry}. Uniqueness requires \( x = y \) to hold in this case, which is in turn equivalent to partial order \hyperref[def:binary_relation/antisymmetric]{antisymmetry}.
\end{proof}
...
\begin{proposition}\label{thm:category_coproduct}
  The \hyperref[def:discrete_category_limits]{categorical coproduct} of the family \( \seq{ \cat{C}_k }_{k \in \mscrK} \) in the category \hyperref[def:category_of_small_categories]{\( \cat{Cat} \)} of \hyperref[def:small_set]{small} categories is their \hyperref[def:category_disjoint_union]{disjoint union} \( \coprod_{k \in \mscrK} \cat{C}_k \).
\end{proposition}
\begin{proof}
...
\begin{proposition}\label{thm:order_category_isomorphism_properties}
  ...
  \begin{thmenum}
    \thmitem{thm:order_category_isomorphism_properties/opposite} The \hyperref[def:opposite_category]{opposite category} \( \cat{P}^\oppos \) corresponds to the \hyperref[thm:order_duality]{opposite partially ordered set} \( (P, \geq) \).
    ...
    Dually, \( T \) is a \hyperref[def:universal_objects/terminal]{terminal object} if and only if it is the \hyperref[def:extremal_points/top_and_bottom]{top} of \( P \).

    \thmitem{thm:order_category_isomorphism_properties/coproduct} The coproduct of partially ordered sets is their disjoint union in the sense of \cref{def:category_disjoint_union}.
  \end{thmenum}
\end{proposition}
\begin{proof}
  ...
  \SubProofOf{thm:order_category_isomorphism_properties/coproduct} Follows from \cref{thm:category_coproduct}.
\end{proof}
