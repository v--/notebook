\subsection{Deduction systems}\label{subsec:deduction_systems}

\paragraph{Proof trees}

\begin{concept}\label{con:inference_rule}\mimprovised
  An \term[ru=правило вывода (\cite[31]{Герасимов2011}), en=inference rule (\cite[2]{MartinLof1996Meanings})]{inference rule} is a specification of when a judgment placeholder \( J \) can follow from the placeholders \( I_1, I_2, \cdots, I_n \).

  \incite[291]{Szabo1964Gentzen} introduced the following convenient notation for an inference rule with name \enquote{\( R \)}:
  \begin{equation*}
    \begin{prooftree}
      \hypo{ I_1 }
      \hypo{ \cdots }
  ...
  For convenience in our definitions, where we will refer to rules only by name, we will suppose that names are derived from the \( \bnfpn{Latin string} \) nonterminal from \fullref{rem:grammar_rules_for_variables}.
...
\end{concept}
\begin{comments}
  \item We can use inference rules to specify entailment of sentences in an object logic rather than entailment of judgments in the metalogic. The epistemology of inference rules is discussed \incite[2]{MartinLof1996Meanings}.

  \item Inference rules can easily be formalized as \hyperref[def:relation]{relations}, as is done in \incite[31]{Герасимов2011}, but this approach makes it difficult to rely on additional metalogical context, as we will need to do in \fullref{def:first_order_natural_deduction_system} with regard to eigenvariables or in \fullref{subsec:simple_type_theory} with regard to typing rules.
\end{comments}

\begin{definition}\label{def:proof_tree}\mimprovised
  Let \( S \) be a nonempty set, whose elements we will call \enquote{entries} as may be may expect them to be formulas or sequents. A \term{proof tree} for \( S \) is an \hyperref[def:ordered_tree]{ordered tree} whose nodes are \hyperref[def:labeled_set]{labeled} by a pair consisting of an element of \( S \) and the name of an \hyperref[con:inference_rule]{inference rule}. We impose the restriction that nodes whose rule name is the empty string \( \varepsilon \) cannot have children.

  This allows us to extend the notation we introduced for inference rules --- if a node with entry \( \varphi \) and rule name \( R \) has \( T_1, \ldots, T_n \) as subtrees, we may denote this as
  \begin{equation*}
    \begin{prooftree}
      \hypo{ T_1 }
      \hypo{ \cdots }
  ...
  \begin{thmenum}
    \thmitem{def:proof_tree/conclusion} We will call the root of the tree its \term[en=conclusion (\cite[22]{TroelstraSchwichtenberg2000})]{conclusion}.

    \thmitem{def:proof_tree/premises} We will call the nodes with an empty rule name \term[en=premises (\cite[22]{TroelstraSchwichtenberg2000})]{premises} or, in the context of \hyperref[def:natural_deduction_system]{natural deduction}, \term[en=assumption (\cite[23]{TroelstraSchwichtenberg2000})]{assumptions}. Because of our aforementioned restriction, these are necessarily \hyperref[def:rooted_tree/leaf]{leaves}.

    \thmitem{def:proof_tree/subproof} A \term{subproof} is simply a subtree of a proof tree.
  \end{thmenum}
\end{definition}
\begin{comments}
  \item Of course, we expect each node to either be a premise or an application of an actual rule from the metalogic. The concrete rules depend on the deductive system.
  \item This definition somewhat resembles \enquote{prooftrees} from \incite[22]{TroelstraSchwichtenberg2000}, but is more rigorous and explicitly encodes which inference rules have been used.
\end{comments}

\paragraph{Axiomatic deduction systems}

\begin{definition}\label{def:axiomatic_deductive_system}\mimprovised
  Fix a nonempty set \( S \), whose elements we will call \term{formulas}\fnote{We allow these formulas to have free variables, so we avoid calling them sentences.}. An \term[en=axiomatic system (\cite[6D1]{Hindley1997})]{axiomatic deduction system} for \( S \), also called a \term[ru=исчисление высказываний гильбертовского типа (\cite[35]{Герасимов2011}), en=Hilbert-style system (\cite[6D1]{Hindley1997}); Hilbert system (\cite[33]{TroelstraSchwichtenberg2000}); Hilbert calculus (\cite[103]{Mimram2020})]{Hilbert-style system}, is simply a subset \( \mscrA \) of \( S \), whose elements we call \term{logical axioms}.

  We rely only on a single \hyperref[con:judgment/inference_rule]{inference rule}, \term[en=modus ponens (\cite[6D1]{Hindley1997})]{modus ponens}:
...
\end{definition}
\begin{comments}
  \item Our definition mostly resembles that of \incite[6D1]{Hindley1997}, but with some important differences --- we do not require axioms to be tautologies and avoid adding additional rules. We also use somewhat different terminology.

  \item The term \enquote{logical axiom} may have different meaning depending on the author. \incite[6D1]{Hindley1997} uses the term \enquote{axiom} for our notion of \enquote{logical axiom}. \incite[205]{Hinman2005} gives an ad-hoc definition of derivability from a set of \enquote{logical axioms} via \( \logic{MP} \). \incite{TroelstraSchwichtenberg2000} uses both \enquote{logical axiom} and \enquote{nonlogical axiom} but does not define the terms, while \incite{Kleene2002Logic} only uses the latter term, again without defining it.

  \item We may alternatively encode the axioms as inference rules without premises, as it is done in \cite[103]{Mimram2020}.
\end{comments}
...
\end{theorem}
\begin{comments}
  \item This theorem also holds for propositional deduction systems which extend the positive implication system with compatible rules, as in the case of \fullref{def:minimal_propositional_natural_deduction_system} or \fullref{def:first_order_natural_deduction_system}.
\end{comments}
\begin{proof}
...
\end{definition}
\begin{comments}
  \item In the case of \hyperref[def:first_order_syntax/formula]{first-order formulas} and not merely \hyperref[def:propositional_grammar_schema/formula]{propositional formulas}, we restrict ourselves to \hyperref[def:first_order_syntax/closed_formula]{closed formulas}. This avoids any possible ambiguity on how to deal with free variables. As in the case of \fullref{def:first_order_natural_deduction_system}, it makes sense to discuss derivations for formulas with free variables because of intermediate formulas in proofs, which may lack outermost quantifiers.
\end{comments}
