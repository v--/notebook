...
\begin{definition}\label{def:consequence_operator}\mimprovised
  ...
  \begin{thmenum}
    \thmitem{def:consequence_operator/compactness}\mcite[95]{CitkinMuravitsky2021} We say that \( \op*{Cn} \) is \term{compact} if the following condition holds:
...
\begin{comments}
  ...
  In \cite[30]{Woodger1983Tarski}, which chronologically precedes the aforementioned definition, Tarski additionally requires that there exists a sentence \( \varphi \) such that \( S = \op*{Cn}(\set{ \varphi }) \).
...
\begin{definition}\label{def:variable_identifier}\mimprovised
  ...
  \begin{bnf*}
    \bnfprod{Small Latin letter}     {\bnftsq{\( \syn a \)} \bnfor \bnftsq{\( \syn b \)} \bnfor \cdots \bnfor \bnftsq{\( \syn y \)} \bnfor \bnftsq{\( \syn z \)}}, \\
    \bnfprod{Small Latin identifier} {\bnfpn{Small Latin letter} \bnfor} \\
    \bnfmore                         {\bnfpn{Small Latin letter} \bnfsp \bnfpn{natural number}}, \\
  \end{bnf*}
  where grammar rules for natural numbers can be taken from \fullref{rem:decimal_notation_grammar} (but without dots on top of the numerals). We have introduced dots on top of the letters in accordance with the disambiguation convention discussed in \fullref{rem:object_language_dots/terminals}.
...
\begin{remark}\label{rem:logical_context_semantics}
  ...
  We may, depending on the application, choose to interpret contexts as either sets of entries or as lists of entries. Both possibilities are discussed in \incite[rem. 4.1.7.2]{Mimram2020} in the context of sequent calculus.
...
\end{definition}
\begin{comments}
  \item This definition is based on the one by \incite[290]{Szabo1964Gentzen}, but formalized via grammars.
...
\end{definition}
\begin{comments}
  \item Regarded as a binary relation between sentences on a fixed signature, entailment is a \hyperref[def:preordered_set]{preorder}. This motivates Lindenbaum-Tarski algebras --- see \fullref{def:lindenbaum_tarski_algebra}.
\end{comments}
...
\begin{definition}\label{def:logical_theory}\mcite[96]{CitkinMuravitsky2021}
  Fix a compatible \hyperref[def:consequence_operator]{consequence operator} \( \op*{Cn} \) and \hyperref[def:consequence_relation]{consequence relation} \( {\vdash} \) on the same set of sentences \( S \). We say that a set of sentences \( \Gamma \) is a \term[ru=теория (\cite[3.1.1]{Герасимов2011})]{theory} with respect to them if any of the following equivalent conditions hold:
  ...
  \begin{thmenum}[resume=def:logical_theory]
    ...
    \thmitem{def:logical_theory/equivalent}\mcite[def. III.2]{Woodger1983Tarski} We say that two sets of sentences are \term{equivalent} if they generate the same theory, that is, if \( \varphi \vdash \psi \) and \( \psi \vdash \varphi \).
  \end{thmenum}
\end{definition}
\begin{comments}
  \item \incite[70]{Woodger1983Tarski} instead calls these theories \enquote{deduction systems}, but we avoid this term to prevent any confusion with \hyperref[def:axiomatic_derivation_system]{axiomatic deductive systems} and \hyperref[def:propositional_natural_deduction_systems]{natural deduction systems}.
\end{comments}
...
\begin{definition}\label{def:entailment_system}\mcite[def. 1]{Meseguer1989}
...
\begin{definition}\label{def:institutional_satisfaction}\mcite[def. 2(1)]{GoguenBurstall1992}
...
\end{definition}
\begin{comments}
  \item This definition is generalized from \incite[def. 1.4.8(i)]{Hinman2005}.
\end{comments}

\begin{definition}\label{def:institutional_entailment}\mcite[def. 2(9)]{GoguenBurstall1992}
  Fix a signature \( \Sigma \) in some \hyperref[def:institution]{institution} and suppose that all sentences and models correspond to \( \Sigma \).

  We say that the set of sentences \( \Gamma \) \term{semantically entails} the sentence \( \varphi \) and write \( \Gamma \vDash_\Sigma \varphi \) if, whenever a model \( \mscrX \) satisfies \( \Gamma \), it also satisfies \( \varphi \). If \( \vdash_\Sigma \varphi \), we say that \( \varphi \) is \term[en=valid (\cite[def. 2.2.1]{Hinman2005})]{valid}.
\end{definition}
\begin{comments}
...
\end{corollary}
\begin{comments}
  \item Although every institution is an entailment system, we will, following \cite{Meseguer1989}, use entailment systems for syntactic entailment that complement institutions.
\end{comments}
\begin{proof}
...
\begin{definition}\label{def:logical_framework}\mimprovised
  ...
  \begin{thmenum}
    \thmitem{def:logical_framework/soundness}\mcite[def. 6]{Meseguer1989} We say that the logical framework is \term[ru=корректная (система) (\cite[37]{Герасимов2011})]{sound} if, whenever a formula is derivable from a set of premises, these premises entail the formula.

    \thmitem{def:logical_framework/completeness}\mcite[def. 6]{Meseguer1989} We say that the logical framework is \term[ru=полная (система) (\cite[44]{Герасимов2011})]{complete} if, whenever a set of premises semantically entails a formula, then the formula is derivable from those premises.
  \end{thmenum}
\end{definition}
\begin{comments}
...
\begin{definition}\label{def:intuitionistic_logic}\mimprovised
  ...
  We will only discuss one logical framework --- that of \hyperref[def:propositional_syntax/formula]{propositional formulas}, \hyperref[def:propositional_semantics]{Heyting algebra semantics} and the \hyperref[def:propositional_natural_deduction_systems]{intuitionistic propositional natural deduction system}.
\end{definition}
\begin{comments}
