...
\begin{definition}\label{def:variable_identifier}\mimprovised
  ...
  \end{bnf*}
  where grammar rules for natural numbers can be taken from \fullref{rem:decimal_notation_grammar} (but without dots on top of the numerals). We have introduced dots on top of the letters in accordance with the disambiguation convention discussed in \fullref{rem:object_language_dots/variables}.
...
\begin{remark}\label{rem:logical_compactness_theorems}
  ...
  \begin{itemize}
    ...
    \item \Fullref{thm:propositional_natural_deduction_entailment_compact} for \hyperref[def:propositional_natural_deduction_system]{propositional natural deduction systems}.
...
\end{definition}
\begin{comments}
  \item \incite[70]{Woodger1983Tarski} instead calls these theories \enquote{deduction systems}, but we avoid this term to prevent any confusion with \hyperref[def:axiomatic_derivation_system]{axiomatic deductive systems} and \hyperref[def:propositional_natural_deduction_system]{natural deduction systems}.
\end{comments}
