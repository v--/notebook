...
\begin{remark}\label{rem:named_logical_systems}
  ...
  \begin{itemize}
    ...
    \thmitem{rem:named_logical_systems/q0} \incite*[\S 51]{Andrews2002Logic} introduced the system \( \logic{Q}_0 \), upon which we base our \hyperref[def:simple_type_system]{simple type systems}.
...
\begin{definition}\label{def:variable_identifier}\mimprovised
  ...
  Furthermore, in definitions like \fullref{def:lambda_term_substitution/sharp}, we would like to find the \enquote{identifier} digit not present in a given context. This can be done via the \hyperref[thm:def:well_ordered_set/lexicographic]{reverse lexicographic order} on letter-number pairs:
  \begin{equation*}
...
\begin{definition}\label{def:logical_context_semantics}
  ...
  We may, depending on the application, choose to interpret contexts as either sets of entries or as lists of entries. Both possibilities are discussed in \incite[rem. 4.1.7.2]{Mimram2020ProgramEqualsProof} in the context of \hyperref[rem:sequent_calculus]{sequent calculus}.
...
\begin{comments}
  ...
  \item Although we have not put restrictions for the context on the right, we may assume that it consists of a single entry. General contexts on the right are used in sequent calculus (described in \cite[sec. 2.6]{Mimram2020ProgramEqualsProof}), while empty contexts are used in dependent type theory (described in \cite[ch. 8]{Mimram2020ProgramEqualsProof}).
