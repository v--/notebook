...
Sets are ubiquitous in mathematics, yet set theory itself is quite complicated. Attention needs to be put to define a \hyperref[def:fol_theory]{logical theory} of sets that is both useful and \hyperref[def:fol_theory/consistent]{consistent}.
...
We later introduce the more sophisticated \hyperref[def:zfc]{Zermelo-Fraenkel set theory} (\logic{ZFC}) and enhance it with Grothendieck's \hyperref[def:zfcu]{axiom of universes} to obtain \logic{ZFC+U}. The consistency of the latter theory is discussed in \cref{rem:set_definition_recursion}.
...
\begin{remark}\label{rem:set_definition_recursion}
  ...
  \begin{itemize}
    \item We define set theory as a \hyperref[def:fol_theory]{theory} of first-order logic.
...
\begin{remark}\label{rem:first_order_theories_in_zfc}
  ...
  This is a natural approach, and we will use it implicitly. Furthermore, it makes no sense to speak about concepts like the \hyperref[thm:fol_lattice_of_substructures]{lattice of subgroups} or the \hyperref[def:cardinal]{cardinality} of a group otherwise.
