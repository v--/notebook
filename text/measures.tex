...
\begin{remark}\label{rem:set_function}
  Some authors refer to \enquote{set functions} --- functions defined on a family of sets. This is the definition given by \incite[30]{Halmos1976Measures}. \incite[def. 11.1]{Rudin1976AnalysisPrinciples} defines \enquote{set functions} as function from a \enquote{rings of sets}, closed under unions and differences, to the extended real numbers.
...
\begin{definition}\label{def:measurable_space}\mcite[def. 1.3]{Rudin1987RealAndComplexAnalysis}
  A \term[bg=измеримо пространство (\cite[def. 3.6]{Драганов2022Мерки}), ru=измеримое пространство (\cite[def. 1.2.3]{Богачёв2003МерыТом1})]{measurable space} is a pair \( (\Omega, \mscrA) \), where \( \Omega \) is a set and \( \mscrA \) is a \hyperref[def:sigma_algebra]{\( \sigma \)-algebra} on \( \Omega \). We call the sets in \( \mscrA \) \term[bg=измеримо множество (\cite[def. 3.6]{Драганов2022Мерки}), ru=измеримое (множество) (\cite[52]{КанторовичАкилов1984ФункАнализ})]{measurable}.
\end{definition}
\begin{comments}
...
\begin{definition}\label{def:measure}\mimprovised
  ...
  We say that the function \( \mu: \mscrA \to Y \) is an \( Y \)-valued \term[bg=мярка (\cite[def. 3.7]{Драганов2022Мерки}), ru=мера (\cite[def. I.1.6]{Ширяев2007ВероятностиТом1}), en=measure (\cite[7]{Cohn2013Measures})]{measure} if it is \hyperref[def:boolean_algebra_additive_function/countable]{countably-additive}. We call the triple \( (\Omega, \mscrA, \mu) \) a \term[bg=пространство с мярка (\cite[def. 3.9]{Драганов2022Мерки}), ru=пространство с мерой (\cite[def. 1.3.5]{Богачёв2003МерыТом1}), en=measure space (\cite[8]{Cohn2013Measures})]{measure space}.
  ...
  \begin{thmenum}
    ...
    \thmitem{def:measure/signed}\mcite[118]{Halmos1976Measures} If \( Y \) is one of the extended real intervals in \fullref{thm:extended_real_semigroup} containing negative numbers, we call \( Y \) a \term[bg=знакопроменлива мярка (\cite[def. 10.1]{Драганов2022Мерки}), ru=знакопеременная мера (\cite[def. 20.1]{ДьяченкоУльянов1998Меры})]{signed measure}.
...
\begin{remark}\label{rem:measure_terminology}
  ...
  \begin{itemize}
    ...
    \item \incite[30]{Halmos1976Measures} provides a similar definition, but instead of defining measures on \( \sigma \)-algebras, he defines them on his concept of \enquote{ring} discussed in \fullref{rem:sigma_algebra_terminology}, that is, families of sets closed under unions and set differences. This generalizes our positive measures.

    Later, in \incite[118]{Halmos1976Measures}, he defines \enquote{signed measures} as functions from \( \sigma \)-rings to either \( (-\infty, \infty] \) or \( [\infty, \infty) \). This again generalizes our usage, but only by considering a more general domain than \( \sigma \)-algebras.
    ...
    \item \incite[def. 2.1]{ДьяченкоУльянов1998Меры} define \enquote{measures} (\enquote{меры}) as finitely-additive nonnegative-valued functions on what they call \enquote{semirings of sets}. They also define \enquote{\( \sigma \)-additive measures} (\enquote{\( \sigma \)-аддитивные меры}) as secondary notions.
    ...
    Later, in \cite[def. 20.1]{ДьяченкоУльянов1998Меры}, they define \enquote{charges} (\enquote{заряды}) as countably-additive real-valued functions on \( \sigma \)-algebras. The latter coincides with our notion of signed measures.
...
\begin{definition}\label{def:counting_measure}\mcite[def. 1.20(a)]{Rudin1987RealAndComplexAnalysis}
  The \term[ru=считающая мера (\cite[exerc. 4.7.78]{Богачёв2003МерыТом1})]{counting measure} on a set \( S \) is the \hyperref[def:measure/positive]{positive measure}
