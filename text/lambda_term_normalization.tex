...
\end{definition}
\begin{comments}
  \item \Fullref{def:lambda_reduction_normal_form/redex} is based on \bycite[def. 2.1.34]{Barendregt1984LambdaCalculus} and \bycite[def. 1B6]{Hindley1997BasicSTT}, who discuss only \( \beta \)-normal forms.
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \item If \( T \) is an assumption tree for \ref{inf:def:base_typing_rules/arrow/intro/exp}, then \( \tau = \sigma \synimplies \rho \) and \( M = \qabs x N \). The tree \( T \) must have a discharge assertion \( x: \sigma \) and subtree \( T_N \) deriving \( N: \rho \).
