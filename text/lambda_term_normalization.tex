...
\begin{proposition}\label{thm:beta_eta_redexes}
  ...
  \begin{thmenum}
    \thmitem{thm:beta_eta_redexes/substitution} If \( M \) contains a redex, so does \( M[\Bbbs] \) for any atomic substitution \( \Bbbs \).
...
\end{proposition}
\begin{proof}
  \SubProofOf{thm:beta_eta_redexes/substitution} We will use \fullref{thm:induction_on_abstract_syntax} on of \( M \) to show, simultaneously on all substitutions \( \Bbbs \), that, if \( M \) contains a redex, so does \( M[\Bbbs] \), and that if \( M \) is a redex, so is \( M[\Bbbs] \).
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \end{equation*}
      and \cref{thm:lambda_substitution_chain_contraction} implies that, since \( b \) is not free in \( B[b \mapsto a] \) (due to \cref{thm:def:lambda_term_alpha_equivalence/free}),
      \begin{equation*}
  ...
  \NecessitySubProof* We will use \fullref{thm:induction_on_abstract_syntax} on of \( M \) to show that, if \( M \) contains a redex, then there exists a term \( N \) such that \( M \pred N \).
...
\end{definition}
\begin{comments}
  \item \Cref{def:lambda_term_normal_form/redex} is based on \cite[def. 2.1.34]{Barendregt1984LambdaCalculus} and \cite[def. 1B6]{Hindley1997BasicSTT}, where only \( \beta \)-normal forms are discussed.
...
\begin{definition}\label{def:lambda_term_reduction_chain}\mimprovised
  ...
  We call a \term{\( \Anon \)-reduction chain} any \hyperref[def:graph_walk/directed]{directed walk} in the graph \hyperref[def:directed_graph_induced_by_relation]{induced} by \( {\pred} \).
\end{definition}
\begin{comments}
  \item The \enquote{reduction graphs} can be considered as standalone objects, but in our formulation are somewhat ill-behaved. This is discussed in \cref{rem:lambda_term_reduction_graphs}.
\end{comments}

\begin{remark}\label{rem:lambda_term_reduction_graphs}
  It is possible to define the \term{\( \Anon \)-reduction graph} \( G_{\Anon}(M) \) of the \( \muplambda \)-term \( M \) as the \hyperref[def:directed_graph]{directed graph} \hyperref[def:directed_graph_induced_by_relation]{induced} by \( {\pred} \).

  Unfortunately, studying such a graph requires making several choices which are not evident without a clear goal in mind. Consider \cref{fig:rem:lambda_term_reduction_graphs}. All \( \beta \)-reducts in the figure are \( \alpha \)-equivalent \hyperref[def:lambda_term_normal_form]{normal forms} (validating \cref{thm:lambda_term_normal_form_uniqueness}), but they are distinct \( \muplambda \)-terms. Having countably many \( \alpha \)-equivalent normal forms does not carry important information for us.

  \begin{figure}
    \centering
    \includegraphics[page=1]{output/rem__lambda_term_reduction_graphs}
    \caption{A fragment of the \hyperref[rem:lambda_term_reduction_graphs]{reduction graph} of a \( \muplambda \)-term.}\label{fig:rem:lambda_term_reduction_graphs}
  \end{figure}

  In line with Barendregt's convention described in \cref{rem:barendregt_convention}, we can consider cosets under \( \alpha \)-equivalence rather than concrete \( \muplambda \)-terms. Furthermore, Barendregt makes another non-evident choice in \cite[def. 2.3.18]{Barendregt1992LambdaCalculiWithTypes}, where he defines reduction multigraphs so that reductions obtained using different strategies correspond to different arcs.

  We avoid discussing reduction graphs since we do not need to study concrete reduction chains.
\end{remark}

\begin{proposition}\label{thm:term_normalizing_iff_exists_finite_reduction_chain}
  A \( \muplambda \)-term is \hyperref[def:lambda_term_normal_form]{\( \Anon \)-normalizing} if and only if it has a (possibly empty) \hyperref[def:lambda_term_reduction_chain]{reduction chain} ending with a \( \Anon \)-normal form.
\end{proposition}
\begin{proof}
...
\begin{definition}\label{def:strongly_normalizing_lambda_term}\mimprovised
  Let \enquote{\( {\Anon} \)} be a combination of \enquote{\( \beta \)}, \enquote{\( \eta \)} and possibly \enquote{\( \delta \)}. We say that the \( \muplambda \)-term \( M \) is \term{strongly \( \Anon \)-normalizing} if all \hyperref[def:lambda_term_reduction_chain]{reduction chains} starting at \( M \) are finite.
\end{definition}
\begin{comments}
  \item This definition is based on \cite[\S 4.3]{Barendregt1992LambdaCalculiWithTypes} and \cite[\S 4.2]{Mimram2020ProgramEqualsProof}, both of whom provide no formal definition.
...
\end{proposition}
\begin{proof}
  Suppose that \( N \) has an infinite \hyperref[def:lambda_term_reduction_chain]{reduction chain}
  \begin{equation*}
...
\end{definition}
\begin{comments}
  \item This definition is based on \cite[172]{Mimram2020ProgramEqualsProof}, which in turn refines \cite[41]{GirardEtAl1989ProofsAndTypes}. In our definition we allow not only \( \beta \)-, but also \( \eta \)- and \( \beta\eta \)-reduction.
\end{comments}
...
\end{lemma}
\begin{proof}
  We will use \fullref{thm:induction_on_abstract_syntax} on \( \tau \):
  \begin{itemize}
...
\end{proposition}
\begin{proof}
  We will use \fullref{thm:induction_on_abstract_syntax} on \( \tau \) to prove the statements simultaneously.
...
\end{lemma}
\begin{comments}
  \item We base this lemma on \cite[thm. 4.2.2.5]{Mimram2020ProgramEqualsProof}, but also handle \( \eta \)- and \( \beta\eta \)-reduction.
\end{comments}
\begin{proof}
...
\begin{lemma}\label{thm:reducibility_candidate_substitution}\mcite[lemma 4.2.2.3]{Mimram2020ProgramEqualsProof}
  Suppose we are given the \hyperref[def:simple_type_derivation]{derivation} \( \Gamma \vdash M: \tau \).

  For every \hyperref[def:atomic_lambda_term_substitution]{atomic substitution} \( \Bbbs \) such that \( \Bbbs(v) \) is in \( R_\sigma \) for every assumption \( v: \sigma \) in \( \Gamma \), the term \( M[\Bbbs] \) belongs to \( R_\tau \).
\end{lemma}
\begin{proof}
  ...
  We will use \fullref{thm:induction_on_abstract_syntax} on \( T \), simultaneously on all substitutions, to conclude that \( M[\Bbbs] \) belongs to \( R_\tau \).
  \begin{itemize}
    ...
    \end{equation*}
    for some variable \( v \) such that \( v \notin \op*{Free}(M) \cup \op*{Free}(M[\Bbbs]) \).
    ...
    We use \fullref{alg:simply_typed_substitution} to construct from \( T_N \) a tree \( S \) for \( N[\Bbbs_{x \mapsto v}] \). Compared to \( T_N \), in \( S \) the assumption \( x: \sigma \) gets replaced with \( v: \sigma \). Since \( v \) is free in \( N \), \cref{thm:lambda_substitution_chain_contraction} implies that
    \begin{equation*}
...
\end{theorem}
\begin{comments}
  \item The proof of this theorem with all the lemmas is based on \cite[thm. 4.2.2.5]{Mimram2020ProgramEqualsProof}, but we additionally handle \( \eta \)- and \( \beta\eta \)-reduction. The work for this is done in \cref{thm:reducibility_candidate_abstraction}.
...
\end{comments}
\begin{proof}
  If \( \Gamma \vdash M: \tau \), \cref{thm:reducibility_candidate_adequacy} implies that \( M \) belongs to \( R_\tau \). \ref{thm:def:strong_normalization_reducibility_candidate/normalizing} then implies that \( M \) is strongly normalizing.
\end{proof}
