...
Proving \fullref{thm:simply_typable_terms_are_strongly_normalizing} requires considerable effort, so we will restrict ourselves to arrow types, with variables and arbitrary base types (as in \fullref{con:simple_type_theory/arrow}).
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      Since \( A \aequiv B \), we have \( E \aequiv F \). \Fullref{thm:def:lambda_term_alpha_equivalence/free} implies that \( a \) is not free in \( F \), hence both \( M \) and \( N \) are \( \eta \)-redexes.
    ...
    We have the following possibilities:
    \begin{itemize}
      \item If \( A = Ea \), then \( B[b \mapsto a] \) is an application term whose second subterm is \( a \). It follows that \( B = Fb \) for some \( \muplambda \)-term \( F \). \Fullref{thm:def:lambda_term_alpha_equivalence/free} implies that \( a \) is not free in \( B[b \mapsto a] \), and \fullref{thm:lambda_substitution_free_variables_single} implies that \( b \) is not free in \( B \).
...
\end{definition}
\begin{comments}
  \item \Fullref{def:lambda_term_normal_form/redex} is based on \bycite[def. 2.1.34]{Barendregt1984LambdaCalculus} and \bycite[def. 1B6]{Hindley1997BasicSTT}, both of whom however only discuss only \( \beta \)-normal forms.
...
\end{proposition}
\begin{proof}
  Follows from \fullref{thm:beta_eta_redexes/alpha}.
\end{proof}
...
\begin{definition}\label{def:lambda_term_reduction_graph}\mimprovised
  Fix a \hyperref[def:lambda_term_reduction]{reduction relation} \( {\pred} \) for \( \muplambda \)-terms (with the modified rules from \fullref{def:typed_term_reduction} in case the terms are typed).
...
\begin{proof}
  ...
  \begin{itemize}
    \item If \( \tau \) is an atomic type, \( R_\tau \) contains all strongly \( \Anon \)-normalizing terms. If \( M \) is in \( R_\tau \) and \( M \aequiv N \), \fullref{thm:strong_normalization_of_alpha_equivalent_term} implies that \( N \) is also in \( R_\tau \).
...
\begin{theorem}[Simply typable terms are strongly normalizing]\label{thm:simply_typable_terms_are_strongly_normalizing}\mcite[thm. 4.2.2.5]{Mimram2020ProgramEqualsProof}
  ...
  Then every (\hyperref[def:typed_lambda_term]{typed} or \hyperref[def:lambda_term]{untyped}) \( \muplambda \)-term \hyperref[def:typability]{typable} via the arrow typing rules from \fullref{def:arrow_type} is \hyperref[def:strongly_normalizing_lambda_term]{\( \Anon \)-strongly normalizing}.
\end{theorem}
\begin{comments}
  \item The proof of this theorem with all the lemmas is based on \bycite[thm. 4.2.2.5]{Mimram2020ProgramEqualsProof}, but we additionally handle \( \eta \)- and \( \beta\eta \)-reduction. The work for this is done in \fullref{thm:reducibility_candidate_abstraction}.

  \item \Fullref{thm:strongly_normalizing_term_is_normalizing} implies that every strongly normalizing term is \hyperref[def:lambda_term_normal_form]{normalizing}, and \Fullref{thm:lambda_term_normal_form_uniqueness} implies that its normal form is unique up to (possibly typed) \( \alpha \)-equivalence. Thus, every typable term has a unique (up to \( \alpha \)-equivalence) normal form.
\end{comments}
\begin{proof}
