...
\begin{proposition}\label{thm:simply_typed_church_rosser_theorem}
  ...
  More precisely, let \enquote{\( {\Anon} \)} be a combination of \enquote{\( \beta \)} and \enquote{\( \eta \)}. Let \( \Lambda \) be the set of either \hyperref[def:lambda_term]{untyped} or \hyperref[def:typed_lambda_term]{typed \( \synlambda \)-terms}, typable via the base rules from \fullref{def:base_typing_rules}. In particular, \( \Lambda \) has no constant terms.
