...
\begin{remark}\label{rem:first_order_satisfiability_bivalence}
  ...
  Given a \hyperref[def:propositional_syntax/formula]{propositional formula} \( \varphi \) and an \hyperref[def:propositional_valuation/interpretation]{interpretation} \( I \), the latter \hyperref[def:propositional_model]{satisfies} either \( \varphi \) or \( \neg \varphi \). Then \( I \) necessarily satisfies \( \varphi \vee \neg \varphi \), which is the principle of bivalence (a.k.a. law of the excluded middle) discussed in \fullref{rem:classical_logic}.

  Similarly, given a \hyperref[def:first_order_valuation/variable_assignment]{first-order variable assignment} \( v \) in the \hyperref[def:first_order_structure]{structure} \( \mscrX \) and a \hyperref[def:first_order_syntax/formula]{first-order formula} \( \varphi \), we have \( \Bracks{\varphi \vee \neg \varphi}_v = T \).
...
\end{definition}
\begin{comments}
  \item Since the formulas are closed, the result does not depend on the assignment. We only use it as a technical tool because we avoid defining entailment within a structure. The problems of doing the latter are discussed in \fullref{rem:first_order_satisfiability_bivalence}.
\end{comments}
...
\begin{definition}\label{def:first_order_substructure}\mcite[def. 3.3.4]{VanDalen2004}
...
\begin{proposition}\label{thm:substructures_form_complete_lattice}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:substructures_form_complete_lattice/top} The \hyperref[def:extremal_points/top_and_bottom]{top element} is the substructure \( \mscrX \) itself. Any substructures that are different from \( \mscrX \) are called \term{proper}.
    ...
    As discussed in \fullref{rem:empty_first_order_structures}, the empty set is not allowed to be the domain of a structure by definition. Nevertheless, for the sake of having a bottom element we allow structures with empty domains in this lattice.
  \end{thmenum}
\end{proposition}
\begin{proof}
...
\begin{definition}\label{def:first_order_homomorphism}\mcite[def. 3.3.1(i)]{VanDalen2004}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:first_order_homomorphism/predicates} \( h \) preserves relations. That is, for any predicate symbol \( p \in \boldop{Pred} \) of arity \( n \) and any \( x_1, \ldots, x_n \in X \),
    \begin{equation*}
...
\begin{proposition}\label{thm:def:first_order_homomorphism}
  ...
  \begin{thmenum}
    ...
    \begin{equation}\label{thm:def:first_order_homomorphism/submodel/canonical_embedding}
      ...
      \end{aligned}
    \end{equation}
    is indeed a homomorphism (and thus an embedding in the sense of \fullref{def:first_order_isomorphism}).
...
\begin{definition}\label{def:first_order_isomorphism}\mcite[def. 3.3.1(ii)]{VanDalen2004}
  We say that an \hyperref[def:function_invertibility/injective]{injective} \hyperref[def:first_order_homomorphism]{homomorphism} \( h: \mscrX \to \mscrY \) is an \term{embedding} if its inverse is also a homomorphism.
  ...
  It is a strengthening of the usual condition \fullref{def:first_order_homomorphism/predicates} on predicates.
\end{definition}
\begin{comments}
...
\begin{definition}\label{def:first_order_automorphism}
  We define an \term{endomorphism} on a \hyperref[def:first_order_structure]{first-order structure} to be a \hyperref[def:first_order_homomorphism]{homomorphism} from the structure to itself and an \term{automorphism} to be an \hyperref[def:first_order_isomorphism]{isomorphism} from the structure to itself.
\end{definition}
