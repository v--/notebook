...
\begin{remark}\label{rem:first_order_satisfiability_bivalence}
  ...
  Therefore, the principle of bivalence does not hold for universal validity. In order for it to hold, what we will do in \cref{def:first_order_model} is restrict to \hyperref[def:first_order_syntax/closed_formula]{closed formulas}, which depend only on the underlying structure and not on the variable assignments.
\end{remark}
...
\begin{theorem}[Lattice theorem for substructures]\label{thm:lattice_theorem_for_substructures}
  ...
  We will give a verbose formulation as a buildup for \cref{thm:lattice_theorem_for_substructures/isomorphism}; the theorem is summarized in \ref{fig:thm:lattice_theorem_for_substructures}.
