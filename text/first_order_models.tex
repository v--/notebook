...
\begin{example}\label{ex:def:first_order_direct_product}
  ...
  \begin{thmenum}
    ...
    \thmitem{ex:def:first_order_direct_product/order} The direct product of the family of \hyperref[def:preoredered_set]{preordered sets} \( \seq{ (P_k, \leq_k) }_{k \in \mscrK} \) is the first-order structure with domain
    \begin{equation*}
    ...
    Furthermore, if \( \leq_k \) is a \hyperref[def:partially_ordered_set]{partial order} for every \( k \in \mscrK \), the product order \( \leq \) is also a partial order because the axiom \eqref{eq:def:binary_relation/antisymmetry} for antisymmetry also satisfies \fullref{thm:direct_product_preserves_positive_formulas}.
