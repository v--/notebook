...
\begin{remark}\label{rem:def:category_of_first_order_models}
  It is important that in \cref{def:category_of_first_order_models} we have required the subcategory \( \ucat{Mod}(\Gamma) \) to be full. This means that a structure from \( \ucat{Mod}(\Gamma) \) is required to be either be a model of \( \Gamma \), but a homomorphism between models of \( \Gamma \) only depends on the signature.
\end{remark}
