...
\begin{definition}\label{def:first_order_substructure}\mcite[def. 2.3.12]{Hinman2005Logic}
  Fix two \hyperref[def:fol_structure]{structures} \( \mscrX = (X, I) \) and \( \mscrY = (Y, J) \) over the \hyperref[def:fol_signature]{first-order signature} \( \Sigma \).

  We say that \( \mscrY \) is a \term{substructure} of \( \mscrX \) if \( Y \subseteq X \) and either of the following equivalent conditions hold:
  \begin{thmenum}
    \thmitem{def:first_order_substructure/inclusion} The inclusion \( \iota: Y \subseteq X \) is a \hyperref[def:fol_homomorphism]{homomorphism}.
    \thmitem{def:first_order_substructure/universe} The universe \( Y \) satisfies either of the equivalent conditions:

    \begin{thmenum}
      \thmitem{def:first_order_substructure/universe/deductive} It is closed under function application, that is, for any functional symbol \( f \) in \( \Sigma \) with arity \( n \), we have \( I(f)(Y^n) \subseteq Y \).
      ...
      \begin{equation*}
        ...
        \end{aligned}
      \end{equation*}
      which enlarges \( A \) with the union of all image of \( A \) under functions of the signature \( \Sigma \).
    \end{thmenum}
  \end{thmenum}
\end{definition}
\begin{comments}
...
\begin{defproof}
  ...
  This condition is clearly satisfied if \( B \) satisfies \cref{def:first_order_substructure/universe/deductive}.

  \ImplicationSubProof{def:first_order_substructure/universe/inductive}{def:first_order_substructure/universe/deductive} If, instead \( Y \) is a fixed point of \( T \), for the \( n \)-ary functional symbol \( f \in \op*{Fun} \) and for any tuple \( x_1, \ldots, x_n \), the value \( I(f)(x_1, \ldots, x_n) \) belongs to \( Y \). Therefore, \cref{def:first_order_substructure/universe/deductive} is satisfied.
\end{defproof}

\begin{remark}\label{rem:questions_regarding_structures}
  Within this section, we are interested in the following questions regarding \hyperref[def:fol_structure]{first-order structure} and \hyperref[def:fol_semantics/model]{models}:

  \begin{itemize}
    \item Which subsets of a structure form a \hyperref[def:first_order_substructure]{substructure}?

    This is answered by \cref{def:first_order_substructure} and by \cref{def:first_order_generated_substructure}. Vacuously, if the signature contains no functional symbols, every subset of (the domain of) a structure is a substructure. Such is the case with \hyperref[def:set]{sets} themselves, with \hyperref[def:partially_ordered_set]{partially ordered sets} or with \hyperref[def:metric_space]{metric} and \hyperref[def:topological_space]{topological spaces}.

    \Cref{thm:substructures_form_complete_lattice} shows that the family of all substructures of a structure is worth studying in itself.

    \item Given a model of some set \( \Gamma \) of formulas, which substructures and \hyperref[def:fol_homomorphism]{homomorphic} images of the model are again models of \( \Gamma \)?

    This is answered by \cref{thm:positive_formulas_preserved_under_homomorphism}, \cref{thm:arbitrary_formulas_preserved_under_isomorphisms} and \cref{thm:functions_over_model_form_model}.
  \end{itemize}
\end{remark}
...
\begin{example}\label{ex:def:first_order_substructure/vector_space}
  ...
  See \cref{thm:span_via_linear_combinations} for a further discussion.
\end{example}

\begin{remark}\label{rem:topological_first_order_structures}
  Let \( \mscrX = (X, I) \) be a structure over some signature \( \Sigma \) without predicate symbols.
...
\end{proposition}
\begin{comments}
  \item In case the substructures are disjoint, their intersection is empty, which is technically not the domain of a structure. If we allow empty domains, as per \cref{rem:empty_first_order_structures}, then the condition for the substructures not to be disjoint is unnecessary.
\end{comments}
\begin{proof}
  For any functional symbol \( f \) in \( \Sigma \) with arity \( n \), we have
  \begin{equation*}
...
\begin{theorem}[Induction on generated substructures]\label{thm:induction_on_generated_substructures}
  Fix a \hyperref[def:fol_structure]{first-order structure} \( \mscrX = (X, I) \) over \( \Sigma \) and a subset \( A \) of \( X \). Let \( \varphi \) be a formula over \( \Sigma \) with a single free variable \( \xi \).
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:induction_on_generated_substructures/step} For every functional symbol \( f \) in \( \Sigma \), prove that, given that \( n \) is the arity of \( f \), if there exists a tuple \( x_1, \ldots, x_n \) such that \( \Bracks{\varphi}(x_k) = T \) for every \( k = 1, \ldots, n \), then
    \begin{equation*}
...
\end{theorem}
\begin{proof}
  Suppose that both \cref{thm:induction_on_generated_substructures/base} and \cref{thm:induction_on_generated_substructures/step} hold. Let \( x \) be an arbitrary member of \( \braket{ A } \).

  \begin{itemize}
    \item If \( x \in A \), then \cref{thm:induction_on_generated_substructures/base} ensures that \( \Bracks{\varphi}(x) = T \).
    \item Otherwise, there exists a functional symbol \( f \) and a tuple \( x_1, \ldots, x_n \) such that \( x = f(x_1, \ldots, x_n) \).

    Indeed, aiming at a contradiction, suppose not. Then \( \braket{ A } \setminus \set{ x } \) is also closed under application of all functions in \( \Sigma \), and hence is itself the domain of a substructure of \( \mscrX \). This contradicts the minimality of \( \braket{ A } \), however.

    Now that we have shown the existence of \( f \) and \( x_1, \ldots, x_n \), \cref{thm:induction_on_generated_substructures/step} implies that
    \begin{equation*}
      \Bracks{\varphi}\parens[\big]{ \underbrace{ I(f)(x_1, \ldots, x_n) }_{x} } = T.
    \end{equation*}
  \end{itemize}

  We have shown that \cref{thm:induction_on_generated_substructures/base} and \cref{thm:induction_on_generated_substructures/step} together imply that \( \Bracks{\varphi}(x) = T \) for every member \( x \) of \( \braket{ A } \).
\end{proof}

\begin{example}\label{ex:def:first_order_generated_substructure}
  Common examples of generated substructures are the \hyperref[def:semimodule/submodel]{linear span} discussed in \cref{thm:span_via_linear_combinations} and the \hyperref[def:semiring_ideal/generated]{generated ring ideals}.
\end{example}

\begin{proposition}\label{thm:substructures_form_complete_lattice}
  Fix a structure \( \mscrX = (X, I) \) for the signature \( \Sigma \).
...
\begin{proof}
  ...
  \SubProofOf{thm:substructures_form_complete_lattice/meet} The domain of the intersection substructure of the family \( \seq{ (X_k, I_k) }_{k \in \mscrK} \) of substructures of \( \mscrX \) is the infimum of the family as a consequence of the equivalence in \cref{def:first_order_generated_substructure}.

  \SubProofOf{thm:substructures_form_complete_lattice/bottom} It follows from \cref{thm:substructures_form_complete_lattice/meet} that the bottom element is the intersection of all substructures of \( \mscrX \).
\end{proof}

\paragraph{Positive formulas}

\begin{definition}\label{def:positive_formula}\mcite[def. 3.5.67]{Hinman2005Logic}
  The following grammar rule, extending the \hyperref[def:first_order_formula]{grammar of first-order formulas}, describes what we call \term{positive formulas}:
  \begin{bnf*}
    ...
    \bnfmore                   {\bnfpn{quantifier} \bnfsp \bnfpn{individuals variable} \bnfsp \bnftsq{.} \bnfsp \bnfpn{formula}}
  \end{bnf*}
\end{definition}
\begin{comments}
  \item The point of positive formulas is to avoid \hyperref[def:propositional_alphabet/negation]{negation \( \synneg \)}. We also avoid \( \synimplies \) because, assuming classical logic, \cref{thm:classical_equivalences/conditional_as_disjunction} would then allow us to introduce negation.

  \item Positive formulas are used in \cref{thm:positive_formulas_preserved_under_homomorphism}, which fails to hold for some non-positive formulas --- see \cref{ex:monoid_cancellation_not_preserved_by_homomorphism}.

  \item When dealing with first-order logic, we simply use \hyperref[thm:first_order_substitution_equivalence/propositional]{substitution} to replace propositional variables with atomic formulas. This way we obtain positive first-order formulas with \hyperref[thm:implicit_universal_quantification]{implicit universal quantification}. Of course, we can always add explicit universal quantifiers, but we avoid existential quantifiers because of \cref{thm:first_order_quantifiers_are_dual}.
\end{comments}

\begin{definition}\label{def:first_order_submodel}
  Given a \hyperref[def:fol_semantics/model]{model} \( \mscrX \) of the set of \hyperref[def:closed_fol_formula]{closed formulas} \( \Gamma \), we say that a \hyperref[def:first_order_substructure]{substructure} \( \mscrY \) of \( \mscrX \) is a \term{submodel} (with respect to \( \Gamma \)) if \( \mscrY \) is also a model of \( \Gamma \).
\end{definition}
\begin{comments}
...
\end{proposition}
\begin{comments}
  \item See \cref{ex:replacing_functional_symbols_via_relations} for an example of how this may fail if there are existential quantifiers.
\end{comments}
\begin{proof}
  This proposition can be proven by a straightforward application of \fullref{thm:induction_on_abstract_syntax}. In each case of the induction, we do not require any elements in addition to those already present in the substructure.
\end{proof}
...
\begin{proposition}\label{thm:functions_over_model_form_model}
  ...
  \begin{itemize}
    \item For each \( n \)-ary functional symbol \( f \) in \( \Sigma \), define the interpretation of the functions \( y_1, \ldots, y_n \) componentwise as
    \begin{equation*}
      \begin{aligned}
        &J(f): Y^n \to Y \\
    ...
    \item For each \( n \)-ary predicate symbol \( p \) in \( \Sigma \), define \( J(p) \subseteq Y^n \) via
    \begin{equation*}
      \begin{aligned}
        &J(p): Y^n \to \set{ T, F } \\
...
\begin{proof}
  ...
  Let \( \varphi \) be a positive formula and suppose that it is valid in \( \mscrX \). We will use \fullref{thm:induction_on_abstract_syntax} to show that \( \Bracks{\varphi}_w = T \) for a fixed variable assignment \( w \) in \( \mscrY \).
  \begin{itemize}
    ...
    \item Analogous to our proof of \cref{thm:positive_formulas_preserved_under_homomorphism}, conjunction and disjunction formulas that are valid in \( \mscrX \) are valid in \( \mscrY \).
...
\begin{example}\label{ex:thm:functions_over_model_of_positive_formulas_form_model}
  While the statement of \cref{thm:functions_over_model_form_model} may be a little cryptic, a few examples show that it is actually obvious.
  \begin{itemize}
    ...
    Then from \cref{thm:functions_over_model_form_model} it follows that the set \( B = \fun(S, \set{ T, F }) \) of all Boolean functions of arbitrary arities is again a Boolean algebra. See \cref{thm:propositional_formulas_and_boolean_functions/bijection} for further discussion.

    \thmitem{ex:thm:functions_over_model_of_positive_formulas_form_model/ring} If \( R \) is a \hyperref[def:ring]{ring} and \( A \) is any set, then \( \fun(A, R) \) is again a ring with componentwise operations --- see \cref{thm:functions_over_algebra}.
...
\begin{proposition}\label{thm:def:fol_homomorphism}
  \hyperref[def:fol_homomorphism]{First-order homomorphisms} have the following basic properties:
  \begin{thmenum}
    \thmitem{thm:def:fol_homomorphism/inclusion} If \( \mscrX = (X, I) \) is a structure and \( \mscrY = (Y, I) \) is a \hyperref[def:first_order_substructure]{substructure} of \( \mscrX \), then the \term{canonical inclusion} function
    \begin{equation}\label{thm:def:fol_homomorphism/inclusion/canonical_inclusion}
      \begin{aligned}
        &\iota: Y \to X \\
        &\iota(y) \coloneqq y
      \end{aligned}
    \end{equation}
    is indeed a homomorphism (and thus an embedding in the sense of \cref{def:first_order_embedding}).

    \thmitem{thm:def:fol_homomorphism/image_is_substructure} Given a homomorphism \( h: \mscrX \to \mscrY \), the \hyperref[def:set_valued_map/image]{image} of \( \mscrX \) under \( h \) is a substructure of \( \mscrY \).

    \thmitem{thm:def:fol_homomorphism/preimage_is_substructure} Given a homomorphism \( h: \mscrX \to \mscrY \) and a substructure \( \mscrZ \) of \( \mscrY \), the \hyperref[def:set_valued_map/inverse]{preimage} of \( \mscrZ \) under \( h \) is a substructure of \( \mscrX \).

    \thmitem{thm:def:fol_homomorphism/composition} The \hyperref[def:set_valued_map/composition]{composition} of two homomorphisms is again a homomorphism.

    \thmitem{thm:def:fol_homomorphism/term_valuation} Fix a homomorphism \( h: X \to Y \) and a term \( \tau \). For any variable assignments \( v \) and \( w \) such that \( w(\xi) = h(v(\xi)) \) for all \( \xi \in \op*{Var}(\tau) \), we have
    \begin{equation*}
...
\end{proposition}
\begin{proof}
  \SubProofOf{thm:def:fol_homomorphism/inclusion} The interpretation in the substructure \( \mscrY \) is the \hyperref[def:set_valued_map/restriction]{restriction} \( J \) of \( I \) to \( Y \). Thus, \( \mscrY = (Y, J) \) is a structure. Conditions \cref{def:fol_homomorphism/functions} and \cref{def:fol_homomorphism/predicates} are thus satisfied. Hence, \( \iota \) is a homomorphism.

  \SubProofOf{thm:def:fol_homomorphism/image_is_substructure} We must show that the image \( h[X] \) satisfies \cref{def:first_order_substructure/universe/deductive} and thus the domain of a substructure of \( \mscrY \).

  Indeed, due to \cref{def:fol_homomorphism/functions}, for any \( n \)-ary functional symbol and any \( n \)-tuple \( x_1, \ldots, x_n \) from \( X \), we have that
  \begin{equation*}
    J(f) \parens[\big]{ h(x_1), \ldots, h(x_n) }
    \reloset {\ref{def:fol_homomorphism/functions}} =
    h\parens[\big]{ I(f)(x_1, \ldots, x_n) }
    \reloset {\ref{def:first_order_substructure/universe/deductive}} \in
    h[X].
  \end{equation*}

  \SubProofOf{thm:def:fol_homomorphism/preimage_is_substructure} We must show that \( h^{-1}[Z] \) satisfies \cref{def:first_order_substructure/universe/deductive}.
  ...
  \SubProofOf{thm:def:fol_homomorphism/composition} Let \( h: \mscrX \mapsto \mscrY \) and \( l: \mscrY \mapsto \mscrZ \) both be homomorphisms. Then \( l \bincirc h: \mscrX \to \mscrY \) is a homomorphism:

  \begin{itemize}
    \item \Cref{def:fol_homomorphism/functions} is satisfied because for any \( n \)-ary functional symbol \( f \) and any tuple \( x_1, \ldots, x_n \in X \),
    \small
    \begin{equation*}
    ...
    \item \Cref{def:fol_homomorphism/predicates} is satisfied because for any \( n \)-ary predicate symbol \( p \) and any tuple \( x_1, \ldots, x_n \in X \),
    \begin{equation*}
      I(p) (x_1, \ldots, x_n)
      \reloset {\ref{def:fol_homomorphism/predicates}} =
  ...
  \SubProofOf{thm:def:fol_homomorphism/term_valuation} We use induction on the structure of \( \tau \).
  \begin{itemize}
...
\begin{definition}\label{def:fol_fixed_point_substructure}
\end{definition}

\begin{proposition}\label{thm:positive_formulas_preserved_under_homomorphism}
  A \hyperref[def:fol_homomorphism]{first-order homomorphism} preserves models of closed \hyperref[def:positive_formula]{positive formulas}.

  More concretely, given some signature \( \Sigma \), if \( \Gamma \) is a set of closed positive formulas and \( h: \mscrX \to \mscrY \) is a homomorphism between \hyperref[def:fol_semantics/model]{models} of \( \Gamma \), then the image \( h[X] \) is also (the domain of) a model of \( \Gamma \).
\end{proposition}
\begin{comments}
  \item Compare this to \cref{thm:arbitrary_formulas_preserved_under_isomorphisms}, which places restrictions on the homomorphism rather than the formulas.
\end{comments}
\begin{proof}
  Since the formulas in \( \Gamma \) are closed, we are free to choose any variable assignment we like. Via \fullref{thm:induction_on_abstract_syntax} on an arbitrary positive formula \( \varphi \), we will show that, for every variable assignment \( w \) in \( \mscrY \), there exists an assignment \( v \) in \( \mscrX \) such that \( \Bracks{\varphi}_v = T \) implies \( \Bracks{\varphi}_w = T \).
  ...
  \begin{itemize}
    ...
    \begin{equation*}
      \Bracks{\tau_1}_w
      \reloset {\ref{thm:def:fol_homomorphism/term_valuation}} =
      h(\Bracks{\tau_1}_v)
      =
      h(\Bracks{\tau_2}_v)
      \reloset {\ref{thm:def:fol_homomorphism/term_valuation}} =
    ...
    Now \( \Bracks{p(\tau_1, \ldots, \tau_n)}_w = T \) follows from \cref{thm:def:fol_homomorphism/term_valuation}.
...
\begin{example}\label{ex:thm:positive_formulas_preserved_under_homomorphism/proof_failure}
  To see how our proof of \cref{thm:positive_formulas_preserved_under_homomorphism} fails for conditionals, consider the formula \( \varphi = \psi_1 \synimplies \psi_2 \), where the inductive hypothesis holds for \( \psi_1 \) and \( \psi_2 \).
  ...
  What if we change our inductive hypothesis to \( \Bracks{\varphi}_v = \Bracks{\varphi}_w \)\fnote{This is precisely what we will do in the case of \hyperref[def:first_order_embedding]{embeddings} in \cref{thm:arbitrary_formulas_preserved_under_isomorphisms}}? This fails for atomic formulas because \( \Bracks{\tau_1 \syneq \tau_2}_v = F \) doesn't imply \( \Bracks{\tau_1 \syneq \tau_2}_w = F \) --- consider the case of a group homomorphism from \( (\BbbR, +) \) to \( (\set{ 0 }, +) \). We can this of this alternative inductive hypothesis as a semantic way of introducing negation, which turns our to cause problems.

  See \cref{ex:monoid_cancellation_not_preserved_by_homomorphism} for an example where a conditional is not preserved by a homomorphism.
\end{example}
...
\begin{example}\label{ex:replacing_functional_symbols_via_relations}
  ...
  If we choose to work only with the relation, the signature would not have any functional symbols, and every subset of (the domain of) \( \mscrX \) would be a substructure.
  ...
  \end{equation*}
  where we have used the unique existence shorthand from \cref{ex:replacing_functional_symbols_via_relations}.
...
\begin{definition}\label{def:first_order_direct_product}\mcite[def. 2.3.54]{Hinman2005Logic}
  Fix a family of \hyperref[def:fol_structure]{first-order structures} \( \seq{ \mscrX_k }_{k \in \mscrK} \) over a common signature \( \Sigma \), where \( \mscrX_k = (X_k, I_k) \).
  ...
  \begin{thmenum}
    ...
    \thmitem{def:first_order_direct_product/functions} For every \( n \)-ary functional symbol \( f \) in \( \Sigma \), we define its interpretation componentwise:
    \begin{equation*}
      \begin{aligned}
    ...
    \thmitem{def:first_order_direct_product/predicates} For every \( n \)-ary predicate symbol \( p \) in \( \Sigma \), we define its interpretation componentwise:
    \begin{equation*}
      \begin{aligned}
...
\end{definition}
\begin{comments}
  \item The direct product is surprisingly useful --- we use it to characterize homomorphisms and congruences as substructures. See \cref{thm:first_order_homomorphism_as_substructure}.
\end{comments}

\begin{proposition}\label{thm:direct_product_projections}
  Given the \hyperref[def:first_order_direct_product]{first-order direct product} \( \prod_{k \in \mscrK} \mscrX_k \), for every \( m \in \mscrK \), the following \term{canonical projection} function is a \hyperref[def:fol_homomorphism]{first-order homomorphism}:
  \begin{equation*}
    \begin{aligned}
...
\end{proposition}
\begin{proof}
  Let \( I \) be the interpretation of the product and \( I_k \) --- of \( \mscrX_k \). For every \( n \)-ary functional symbol \( f \) in \( \Sigma \), we have
  \begin{align*}
    &\phantom{{}={}}
  ...
  This shows \cref{def:fol_homomorphism/functions}.

  Now fix an \( n \)-ary predicate symbol \( p \) in \( \Sigma \) and suppose that
  \begin{equation*}
    I(p)\parens[\big]{ \seq{ x_{1,k} }_{k \in \mscrK}, \ldots, \seq{ x_{n,k} }_{k \in \mscrK} } = T.
  \end{equation*}

  Then \cref{def:first_order_direct_product/predicates} implies that
  \begin{equation*}
  ...
  This shows \cref{def:fol_homomorphism/predicates}.

  Therefore, \( \pi_m \) is a first-order homomorphism.
\end{proof}

\begin{proposition}\label{thm:first_order_homomorphism_as_substructure}
  Let \( \mscrX = (X, I) \) and \( \mscrY = (Y, J) \) be structures over \( \Sigma \). The \hyperref[def:function]{set-theoretic function} \( h: X \to Y \) preserves logical functions, i.e. satisfies \cref{def:fol_homomorphism/functions}, if and only if it is the domain of a \hyperref[def:first_order_substructure]{substructure} of the \hyperref[def:first_order_direct_product]{direct product} \( \mscrX \times \mscrY \).
\end{proposition}
\begin{proof}
  \SufficiencySubProof Suppose that \( h: X \to Y \) is a function that satisfies \cref{def:fol_homomorphism/functions}. We must show that \( h \) is closed under function application in \( \mscrX \times \mscrY \). Denote by \( K \) the interpretation in the structure \( \mscrX \times \mscrY \).

  First note that \( (x, y) \in h \) if and only if \( y = h(x) \).

  Let \( f \) be an \( n \)-ary function symbol from \( \Sigma \). Then, for any \( n \)-tuple of pairs \( (x_1, y_1), \ldots, (x_n, y_n) \) from \( h \), we have
  \begin{align*}
  ...
  \begin{equation*}
    h\parens[\big]{ I(f)(x_1, \ldots, x_n) } = J(f)\parens[\big]{ h(x_1), \ldots, h(x_n) }
  \end{equation*}
  and \( h \) satisfies \cref{def:fol_homomorphism/functions}.
\end{proof}

\begin{proposition}\label{thm:direct_product_preserves_positive_formulas}
  Let \( \Gamma \) be a closed set of \hyperref[def:positive_formula]{positive formulas} \hi{without disjunctions} over \( \Sigma \) and let \( \seq{ \mscrX_k }_{k \in \mscrK} \) be a family of structures over \( \Sigma \), where \( \mscrX_k = (X_k, I_k) \) for every \( k \in \mscrK \).

  If, for every \( k \in \mscrK \), the structure \( \mscrX_k \) is a \hyperref[def:fol_semantics/model]{model} of \( \Gamma \), then so is the \hyperref[def:first_order_direct_product]{direct product} \( \prod_{k \in \mscrK} \mscrX_k \).
\end{proposition}
\begin{comments}
  \item \Cref{ex:def:first_order_direct_product/algebraic} demonstrates that the requirement for positivity is crucial, while \cref{ex:def:first_order_direct_product/order} demonstrates that the requirement for the formulas to have no disjunction is crucial.
...
\begin{proof}
  ...
  First, we will use \fullref{thm:induction_on_abstract_syntax} on a term \( \tau \) to show that the following extension of \cref{def:first_order_direct_product/functions} holds:
  ...
  \end{equation}
  where \( v \) is any \hyperref[def:fol_variable_assignment]{variable assignment} in \( \prod_{k \in \mscrK} \mscrX_k \) and \( v^k \) be the \( k \)-th component of \( v \).
  ...
  Now, we will use \fullref{thm:induction_on_abstract_syntax} on a positive formula \( \varphi \) to show that the following extension of \cref{def:first_order_direct_product/predicates} holds:
...
\begin{example}\label{ex:def:first_order_direct_product}
  ...
  \begin{thmenum}
    \thmitem{ex:def:first_order_direct_product/algebraic} Direct products for algebraic structures are simple --- there are no predicates, and \cref{thm:direct_product_preserves_positive_formulas} is usually satisfied.
    ...
    The axioms \eqref{eq:def:binary_relation/reflexive} for reflexivity and \eqref{eq:def:binary_relation/transitive} for transitivity are positive and have no disjunctions, thus the product \( (P, \leq) \) is also preordered as a consequence of \cref{thm:direct_product_preserves_positive_formulas}.

    Furthermore, if \( \leq_k \) is a \hyperref[def:partially_ordered_set]{partial order} for every \( k \in \mscrK \), the product order \( \leq \) is also a partial order because the axiom \eqref{eq:def:binary_relation/antisymmetric} for antisymmetry also satisfies \cref{thm:direct_product_preserves_positive_formulas}.
...
\begin{definition}\label{def:first_order_embedding}\mcite[def. 2.3.26(ii)]{Hinman2005Logic}
  Let \( \mscrX = (X, I) \) and \( \mscrY = (Y, J) \) be structures over \( \Sigma \).

  We say that an \hyperref[def:function_invertibility/injective]{injective} \hyperref[def:fol_homomorphism]{homomorphism} \( h: \mscrX \to \mscrY \) is an \term{embedding} if any of the following equivalent conditions hold:

  \begin{thmenum}
    \thmitem{def:first_order_embedding/inverse} The inverse of \( h \) on its image is also a homomorphism.
    \thmitem{def:first_order_embedding/predicates} For any predicate symbol \( p \) in \( \Sigma \),
...
\end{definition}
\begin{comments}
  \item In particular, if there are no predicate symbols in the signature, an embedding is simply an injective homomorphism.
...
\begin{defproof}
  ...
  Requiring \( h^{-1} \) to satisfy the condition \eqref{eq:def:first_order_embedding/predicates} on homomorphisms for predicate symbols is identical to requiring \( h \) (or \( h^{-1} \)) to satisfy the condition \eqref{eq:def:fol_homomorphism/predicates} on embeddings.

  The condition on functional symbols is simpler to handle. We will show that \( h^{-1} \) always satisfies the condition \eqref{eq:def:fol_homomorphism/functions} on homomorphisms for functional symbols.
  ...
  \begin{align*}
    ...
    \reloset {\eqref{eq:def:fol_homomorphism/functions}} = \\ &=
...
\begin{proposition}\label{thm:arbitrary_formulas_preserved_under_isomorphisms}
  ...
  More concretely, given some signature \( \Sigma \), if \( \Gamma \) is a set of closed formulas and \( h: \mscrX \to \mscrY \) is an embedding between \hyperref[def:fol_semantics/model]{models} of \( \Gamma \), then the image \( h[X] \) is also (the domain of) a model of \( \Gamma \).
\end{proposition}
\begin{comments}
  \item Compare this to \cref{thm:positive_formulas_preserved_under_homomorphism}, which places restrictions on the formulas rather than the homomorphism.
  \item We say that embeddings preserve the validity arbitrary formulas.
\end{comments}
\begin{proof}
  The embedding condition allows us to extend the induction in our proof of \cref{thm:arbitrary_formulas_preserved_under_isomorphisms} to the case of arbitrary formulas by instead requiring that \( \Bracks{\varphi}_v = \Bracks{\varphi}_w \) with the fixed variable assignments \( v \) and \( w \) from our proof of \cref{thm:arbitrary_formulas_preserved_under_isomorphisms}.
\end{proof}

\begin{proposition}\label{thm:isomorphism_preserves_validity}\mcite[prop. 2.3.39]{Hinman2005Logic}
  Let \( \mscrX = (X, I) \) and \( \mscrY = (Y, J) \) be structures over \( \Sigma \). Let \( h: \mscrX \to \mscrY \) be a \hyperref[def:first_order_isomorphism]{first-order isomorphism}. Fix a formula \( \varphi \) in \( \Sigma \) with (at most) \( n \) free variables. Then
  \begin{equation*}
    \Bracks{\varphi}_\mscrX(x_1, \ldots, x_n) = \Bracks{\varphi}_\mscrY\parens[\big]{ h(x_1), \ldots, h(x_n) },
  \end{equation*}
  where \( \Bracks{\varphi} \) denotes the denotation function defined in \cref{def:parametrized_propositional_denotation}.
\end{proposition}
\begin{proof}
  We will show via \fullref{thm:induction_on_abstract_syntax} that
  \begin{equation}\label{eq:thm:isomorphism_preserves_validity/hypothesis_formulas}
    \Bracks{\varphi}_v = \Bracks{\varphi}_w,
  \end{equation}
  where \( \varphi \) is a formula over \( \Sigma \), \( v \) is a variable assignment in \( \mscrX \) and \( w(\xi) \coloneqq h(v(\xi)) \) is an assignment in \( \mscrY \).
  ...
  First, we will need to prove the following auxiliary result: for any term \( \tau \) over \( \Sigma \), we have
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \begin{equation*}
        ...
        \reloset {\eqref{eq:def:fol_homomorphism/predicates}} =
        T.
      \end{equation*}
    ...
    \begin{equation*}
      \Bracks{\varphi}_v
      =
      \Bracks{\psi_1}_v \bincirc \Bracks{\psi_2}_v
      \reloset {\T{ind.}} =
      \Bracks{\psi_1}_w \bincirc \Bracks{\psi_2}_w
      =
      \Bracks{\varphi}_w
    \end{equation*}
...
\begin{definition}\label{def:first_order_congruence}\mcite[46]{Мальцев1970АлгебраическиеСистемы}
  Let \( \mscrX = (X, I) \) be a \hyperref[def:fol_structure]{first-order structure} over some signature.
...
\end{definition}
\begin{comments}
  \item In particular, as a consequence of \cref{thm:closure_operator_minimality}, the generated congruence is the smallest congruence containing \( {\sim} \).
\end{comments}
...
\begin{definition}\label{def:first_order_quotient}\mcite[62]{Мальцев1970АлгебраическиеСистемы}
  ...
  Then \( \mscrX / {\cong} \coloneqq (X / {\cong}, I_\cong) \) is a structure over the same signature and, furthermore, the projection \( \pi \) is a \hyperref[def:fol_homomorphism]{homomorphism}.
\end{definition}
...
\begin{proposition}\label{thm:quotient_preserves_positive_formulas}
  Fix a set \( \Gamma \) of closed \hyperref[def:positive_formula]{positive formulas} over a signature without predicate symbols.

  If \( \mscrX = (X, I) \) is a \hyperref[def:fol_semantics/model]{model} of \( \Gamma \) and \( \cong \) is a \hyperref[def:first_order_congruence]{first-order congruence} on \( \mscrX \), then the \hyperref[def:first_order_quotient]{quotient structure} \( \mscrX / {\cong} \) defined as \( (X / {\cong}, I_\cong) \) is also a model of \( \Gamma \).
\end{proposition}
\begin{defproof}
  Fix a positive formula \( \varphi \). Via \fullref{thm:induction_on_abstract_syntax}, we will show that if any variable assignment \( v \) satisfies \( \varphi \) in \( \mscrX \), the following variable assignment satisfies \( \varphi \) in \( \mscrX / {\cong} \):
  \begin{equation*}
    \begin{aligned}
...
\begin{theorem}[Lattice theorem for substructures]\label{thm:lattice_theorem_for_substructures}
  Let \( \mscrX = (X, I) \) be a \hyperref[def:fol_structure]{structure} over \hyperref[def:first_order_language]{first-order signature} \( \Sigma \) and let \( {\cong} \) be a \hyperref[def:first_order_congruence]{congruence} on \( \mscrX \).
  ...
  \begin{thmenum}
    \thmitem{thm:lattice_theorem_for_substructures/direct} If \( (S, I) \) is a substructure of \( (X, I) \) compatible with \( {\cong} \), then \( (S / {\cong}, I_\cong) \) is a substructure of \( (X / {\cong}, I_\cong) \), that is, a structure for \( \Sigma \) satisfying \( S / {\cong} \subseteq X / {\cong} \).
    ...
    \thmitem{thm:lattice_theorem_for_substructures/reverse} If \( (Q, I_\cong) \) is a substructure of \( (X / {\cong}, I_\cong) \), then \( (\bigcup Q, I) \) is a substructure of \( (X, I) \) compatible with \( {\cong} \), that is, a structure over \( \Sigma \) satisfying \( \bigcup Q \subseteq X \) and \eqref{eq:thm:lattice_theorem_for_substructures/compatibility}.
...
\begin{proof}
  ...
  Let \( f \) be an \( n \)-ary functional symbol from \( \Sigma \). If \( [x_1], \ldots, [x_n] \) are members of \( S / {\cong} \), then \( x_1, \ldots, x_n \) are members of \( S \). Since \( S \) is closed under function application, it contains
  \begin{equation*}
    I(f)(x_1, \ldots, x_n).
  \end{equation*}
  ...
  Again, let \( f \) be an \( n \)-ary functional symbol from \( \Sigma \). If \( x_1, \ldots, x_n \) are members of \( \bigcup Q \), then the cosets \( [x_1], \ldots, [x_n] \) belong to \( Q \) and since \( Q \) is closed under application of \( I_\cong(f) \), it contains
  \begin{equation*}
    I_\cong(f)\parens{ [x_1], \ldots, [x_n] }
    =
    [I(f)(x_1, \ldots, x_n)].
  ...
  In one direction, if \( [q] \in Q \), then \( q \in \bigcup Q \) and, since we have shown in \cref{thm:lattice_theorem_for_substructures/reverse} that \( (\bigcup Q, I) \) is a structure satisfying the compatibility condition \eqref{eq:thm:lattice_theorem_for_substructures/compatibility}, we conclude that \( [q] \in (\bigcup Q) / {\cong} \).
  ...
  Therefore, \( T / {\cong} \) is a subset \( S / {\cong} \). \Cref{thm:lattice_theorem_for_substructures/direct} ensures that both are substructures of \( (X, I) \) and \cref{thm:substructure_relation_is_transitive} ensures that \( (T / {\cong}, I_\cong) \) is a substructure of \( (S / {\cong}, I_\cong) \).
  ...
  Furthermore, \cref{thm:closure_operator_from_set_semilattice} shows that the following is a \hyperref[def:moore_closure_operator]{Moore closure operator}:
  \begin{equation*}
    \begin{aligned}
  ...
  From \cref{thm:closure_operator_minimality} it follows that this is indeed the smallest suitable substructure.
  ...
  \SubProofOf{thm:lattice_theorem_for_substructures/isomorphism} The map \( (S, I) \mapsto (S / {\cong}, I_\cong) \) is left invertible as per \cref{thm:lattice_theorem_for_substructures/left_invertible} and right invertible as per \cref{thm:lattice_theorem_for_substructures/right_invertible}. It is also order-preserving as per \cref{thm:lattice_theorem_for_substructures/direct_order}, and is order-reflecting as per \cref{thm:lattice_theorem_for_substructures/reverse_order}.

  Then \cref{thm:def:complete_lattice/embedding} is satisfied, implying that the aforementioned map is an isomorphism of complete lattices.
\end{proof}

\paragraph{First-order definability}

\begin{definition}\label{def:first_order_definability}\mcite[def. 2.3.37]{Hinman2005Logic}
  Fix a \hyperref[def:first_order_language]{first-order signature} \( \Sigma \) and a \hyperref[def:fol_structure]{structure} \( \mscrX = (X, I) \) for \( \Sigma \).

  To every \hyperref[def:first_order_syntax/formula]{formula} \( \varphi \) whose \hyperref[def:fol_variable_freeness]{free variables} are among \( \xi_1, \ldots, \xi_n, \eta_1, \ldots, \eta_m \), and to every \( m \)-tuple \( u_1, \ldots, u_m \) of members of \( A \), which we call \term{parameters}, there corresponds a set \( A \subseteq X^n \) such that
  \begin{equation*}
...
\begin{proposition}\label{thm:automorphism_of_definable_set}
  Let \( \mscrX = (X, I) \) be a structure over some signature \( \Sigma \).
...
\end{proposition}
\begin{proof}
  \SubProofOf{thm:automorphism_of_definable_set/direct} If \( A \) is definable via \( \varphi \), then \cref{thm:isomorphism_preserves_validity} implies that,if \( (x_1, \ldots, x_n) \in A \), then
  \begin{equation*}
  ...
  \SubProofOf{thm:automorphism_of_definable_set/contrapositive} This is the contrapositive of \cref{thm:automorphism_of_definable_set/direct}.
\end{proof}
