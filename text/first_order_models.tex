...
\begin{comments}
  ...
  \item Since \( \cat{Mod}(\Gamma) \) generally has less objects than \( \cat{Mod}(\Delta) \) if \( \Gamma \subseteq \Delta \), it is possible for \( h \) to be an epimorphism in \( \cat{Mod}(\Delta) \) but not in \( \cat{Mod}(\Gamma) \).
\end{comments}
