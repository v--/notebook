...
\begin{proposition}\label{thm:fol_bijective_homomorphism}
\end{proposition}

\begin{proposition}\label{thm:implicit_universal_quantification}
  Every first-order formula is semantically equivalent to its \hyperref[def:fol_quantifier_closure]{universal closure}.
\end{proposition}
\begin{comments}
  \item See \cref{ex:def:first_order_natural_deduction_system/eigenvariables/invalid_universal_closure} for how this fails for derivability rather than entailment.
\end{comments}
\begin{proof}
...
\begin{definition}\label{def:first_order_model}\mcite[def. 2.2.13(ii)]{Hinman2005Logic}
  We say that the \hyperref[def:fol_structure]{first-order structure} \( \mscrX = (X, I) \) is a \term{first-order model} of the set of \hyperref[def:first_order_syntax/formula]{first-order formulas} \( \Gamma \) if, for an arbitrary variable assignment \( v \) in \( \mscrX \), \( \Bracks{\varphi}_v = T \) for every formula \( \varphi \in \Gamma \).
...
\end{definition}
\begin{comments}
  \item Since the formulas are closed, the result does not depend on the assignment. We only assignments as a technical tool because we avoid defining entailment within a structure. The problems of doing the latter are discussed in \cref{rem:implicit_quantification_and_deduction}.
\end{comments}

\begin{remark}\label{rem:questions_regarding_structures}
  Within this section, we are interested in the following questions regarding \hyperref[def:fol_structure]{first-order structure} and \hyperref[def:first_order_model]{models}:
...
\begin{theorem}[Induction on generated substructures]\label{thm:induction_on_generated_substructures}
  Fix a \hyperref[def:fol_structure]{first-order structure} \( \mscrX = (X, I) \) over \( \mscrL \) and a subset \( A \) of \( X \). Let \( \varphi \) be a formula over \( \mscrL \) with a single free variable \( \xi \).
...
\begin{definition}\label{def:first_order_submodel}
  Given a \hyperref[def:first_order_model]{model} \( \mscrX \) of the set of \hyperref[def:closed_fol_formula]{closed formulas} \( \Gamma \), we say that a \hyperref[def:first_order_substructure]{substructure} \( \mscrY \) of \( \mscrX \) is a \term{submodel} (with respect to \( \Gamma \)) if \( \mscrY \) is also a model of \( \Gamma \).
\end{definition}
\begin{comments}
...
\begin{definition}\label{def:first_order_direct_product}\mcite[def. 2.3.54]{Hinman2005Logic}
  Fix a family of \hyperref[def:fol_structure]{first-order structures} \( \seq{ \mscrX_k }_{k \in \mscrK} \) over a common language \( \mscrL \), where \( \mscrX_k = (X_k, I_k) \).
...
\begin{definition}\label{def:first_order_congruence}\mcite[46]{Мальцев1970АлгебраическиеСистемы}
  Let \( \mscrX = (X, I) \) be a \hyperref[def:fol_structure]{first-order structure} over some language.
...
\begin{definition}\label{def:first_order_quotient}\mcite[62]{Мальцев1970АлгебраическиеСистемы}
  ...
  Then \( \mscrX / {\cong} \coloneqq (X / {\cong}, I_\cong) \) is a structure over the same language and, furthermore, the projection \( \pi \) is a \hyperref[def:fol_homomorphism]{homomorphism}.
\end{definition}
...
\begin{theorem}[Lattice theorem for substructures]\label{thm:lattice_theorem_for_substructures}
  Let \( \mscrX = (X, I) \) be a \hyperref[def:fol_structure]{structure} over \hyperref[def:first_order_language]{first-order language} \( \mscrL \) and let \( {\cong} \) be a \hyperref[def:first_order_congruence]{congruence} on \( \mscrX \).
...
\begin{definition}\label{def:first_order_definability}\mcite[def. 2.3.37]{Hinman2005Logic}
  Fix a \hyperref[def:first_order_language]{first-order language} \( \mscrL \) and a \hyperref[def:fol_structure]{structure} \( \mscrX = (X, I) \) for \( \mscrL \).
