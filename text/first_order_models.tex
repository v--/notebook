...
\end{definition}
\begin{comments}
  \item This allows us to skip quantifiers when writing formulas without changing their validity. This is useful for notational brevity and is used all across the monograph whenever we want \hi{closed} formulas, for example in \cref{def:semiring/theory}, \cref{def:semiring/theory} or \cref{def:binary_relation}.
\end{comments}
...
\end{comments}
\begin{proof}
  This proposition can be proven by a straightforward application of \cref{thm:induction_on_rooted_trees}. In each case of the induction, we do not require any elements in addition to those already present in the substructure.
\end{proof}
...
\begin{proof}
  ...
  Let \( \varphi \) be a positive formula and suppose that it is valid in \( \mscrX \). We will use \cref{thm:induction_on_rooted_trees} to show that \( \Bracks{\varphi}_w = T \) for a fixed variable assignment \( w \) in \( \mscrY \).
  \begin{itemize}
...
\end{comments}
\begin{proof}
  Since the formulas in \( \Gamma \) are closed, we are free to choose any variable assignment we like. Via \cref{thm:induction_on_rooted_trees} on an arbitrary positive formula \( \varphi \), we will show that, for every variable assignment \( w \) in \( \mscrY \), there exists an assignment \( v \) in \( \mscrX \) such that \( \Bracks{\varphi}_v = T \) implies \( \Bracks{\varphi}_w = T \).
...
\begin{proof}
  ...
  First, we will use \cref{thm:induction_on_rooted_trees} on a term \( \tau \) to show that the following extension of \cref{def:first_order_direct_product/functions} holds:
  ...
  Now, we will use \cref{thm:induction_on_rooted_trees} on a positive formula \( \varphi \) to show that the following extension of \cref{def:first_order_direct_product/predicates} holds:
...
\end{proposition}
\begin{proof}
  We will show via \cref{thm:induction_on_rooted_trees} that
...
\end{proposition}
\begin{defproof}
  Fix a positive formula \( \varphi \). Via \cref{thm:induction_on_rooted_trees}, we will show that if any variable assignment \( v \) satisfies \( \varphi \) in \( \mscrX \), the following variable assignment satisfies \( \varphi \) in \( \mscrX / {\cong} \):
  \begin{equation*}
    \begin{aligned}
