...
\begin{remark}\label{rem:first_order_satisfiability_bivalence}
  ...
  Similarly, given a \hyperref[def:first_order_valuation/variable_assignment]{first-order variable assignment} \( v \) in the \hyperref[def:first_order_structure]{structure} \( \mscrX \) and a \hyperref[def:first_order_syntax/formula]{first-order formula} \( \varphi \), either \( \Bracks{\varphi} = T \) or \( \Bracks{\synneg \varphi}_v = T \).
  ...
  We just saw that \( \xi \syneq \eta \) is not universally valid. But neither is \( \xi \syneq \eta \) because
  \begin{equation*}
    \Bracks{\synneg (\xi \syneq \eta)}_v = \overline{\Bracks{\xi \syneq \eta}_v} = F
  \end{equation*}
...
\end{definition}
\begin{comments}
  \item The point of positive formulas is to avoid \hyperref[def:propositional_language/negation]{negation \( \synneg \)}. We also avoid \( \synimplies \) because, assuming classical logic, \fullref{thm:boolean_equivalences/conditional_as_disjunction} would then allow us to introduce negation.
