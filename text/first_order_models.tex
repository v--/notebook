...
\begin{definition}\label{def:hol_universal_validity}\mimprovised
  If, for some \hyperref[def:hol_term/formula]{higher-order logical formula} \( \varphi \), the \hyperref[def:hol_term_valuation]{valuation} \( \Bracks{ \varphi }_\mscrX^v \) equals \( T \) for any variable assignment \( v \), we say that \( \varphi \) is \term{universally valid} in \( \mscrX \).
\end{definition}
\begin{comments}
...
\begin{definition}\label{def:hol_universal_closure}\mimprovised
  ...
  \end{equation*}
  where \( x_1^{\tau_1}, \ldots, x_n^{\tau_n} \) is an \hyperref[def:set_enumeration]{enumeration} of the free variables of \( \varphi \).
...
\begin{proof}
  ...
  Let \( \varphi \) be a formula and let \( x_1^{\tau_1}, \ldots, x_n^{\tau_n} \) be any ordering of its variables.
  ...
  Suppose that, for some assignment \( v_0 \), we have \( \Bracks{ \varphi }_\mscrX^{v_0} = F \). Then
  \begin{equation*}
    \Bracks{ \qforall {x_n^{\tau_n}} \varphi }_\mscrX^v
    \reloset {\eqref{eq:def:hol_term_valuation/formula/forall}} =
    \bigwedge\set[\big]{ \Bracks{ \varphi }_\mscrX^{v^{x^{\tau_n} \mapsto a}} \given* a \in X_{\tau_n} }
  ...
  By induction, we conclude that  \( \Bracks{ \qforall {x_1^{\tau_1}} \ldots \qforall {x_n^{\tau_n}} \varphi }_\mscrX^v = F \).
...
\begin{remark}\label{rem:first_order_satisfiability_bivalence}
  We will now show why \cref{def:propositional_semantics/model} requires some adjustments for first-order formulas.

  Given a \hyperref[def:propositional_formula]{propositional formula} \( \varphi \) and an \hyperref[def:propositional_interpretation]{interpretation} \( I \), the latter \hyperref[def:propositional_semantics/model]{satisfies} either \( \varphi \) or \( \synneg \varphi \). This is the principle of bivalence discussed in \cref{con:classical_logic}.

  Similarly, given a \hyperref[def:fol_variable_assignment]{first-order variable assignment} \( v \) in the \hyperref[def:first_order_structure]{structure} \( \mscrX \) and a \hyperref[def:first_order_syntax/formula]{first-order formula} \( \varphi \), either \( \Bracks{\varphi}_v = T \) or \( \Bracks{\synneg \varphi}_v = T \).
  ...
  Therefore, the principle of bivalence does not hold for universal validity. In order for it to hold, what we will do in \cref{def:first_order_model} is restrict to \hyperref[def:closed_fol_formula]{closed formulas}, which depend only on the underlying structure and not on the variable assignments.
\end{remark}

\begin{definition}\label{def:fol_universal_validity}\mcite[rem. 2.2.17]{Hinman2005Logic}
\end{definition}

\begin{definition}\label{def:fol_universal_closure}\mcite[23]{CitkinMuravitsky2022ConsequenceRelations}
  We call a \term[ru=универсальное замыкание (\cite[65]{Герасимов2011Вычислимость})]{universal closure} of the \hyperref[def:first_order_syntax/formula]{first-order logical formula} \( \varphi \) any formula of the form
  \begin{equation*}
...
\begin{comments}
  ...
  \item Note that the condition \cref{def:first_order_homomorphism/predicates} for predicates is strictly weaker than
  \begin{equation*}
...
\end{proposition}
\begin{proof}
  \SubProofOf{thm:def:first_order_homomorphism/inclusion} The interpretation in the substructure \( \mscrY \) is the \hyperref[def:set_valued_map/restriction]{restriction} \( J \) of \( I \) to \( Y \). Thus, \( \mscrY = (Y, J) \) is a structure. Conditions \cref{def:first_order_homomorphism/functions} and \cref{def:first_order_homomorphism/predicates} are thus satisfied. Hence, \( \iota \) is a homomorphism.

  \SubProofOf{thm:def:first_order_homomorphism/image_is_substructure} We must show that the image \( h[X] \) satisfies \cref{def:first_order_substructure/universe/deductive} and thus the domain of a substructure of \( \mscrY \).

  Indeed, due to \cref{def:first_order_homomorphism/functions}, for any \( n \)-ary functional symbol and any \( n \)-tuple \( x_1, \ldots, x_n \) from \( X \), we have that
  \begin{equation*}
  ...
  \SubProofOf{thm:def:first_order_homomorphism/preimage_is_substructure} We must show that \( h^{-1}[Z] \) satisfies \cref{def:first_order_substructure/universe/deductive}.
  ...
  \begin{itemize}
    \item \Cref{def:first_order_homomorphism/functions} is satisfied because for any \( n \)-ary functional symbol \( f \) and any tuple \( x_1, \ldots, x_n \in X \),
    \small
    \begin{equation*}
    ...
    \item \Cref{def:first_order_homomorphism/predicates} is satisfied because for any \( n \)-ary predicate symbol \( p \) and any tuple \( x_1, \ldots, x_n \in X \),
    \begin{equation*}
      I(p) (x_1, \ldots, x_n)
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    Now \( \Bracks{p(\tau_1, \ldots, \tau_n)}_w = T \) follows from \cref{thm:def:first_order_homomorphism/term_valuation}.
...
\begin{proof}
  ...
  This shows \cref{def:first_order_homomorphism/functions}.
...
\begin{proof}
  ...
  Since \( h \) satisfies \cref{def:first_order_homomorphism/functions}, it follows that
  \begin{equation*}
    h\parens[\Big]{ I(f)(x_1, \ldots, x_n) } = J(f)\parens[\Big]{ h(x_1), \ldots, h(x_n) },
  \end{equation*}
  and
  ...
  \begin{equation*}
    h\parens[\Big]{ I(f)(x_1, \ldots, x_n) } = J(f)\parens[\Big]{ h(x_1), \ldots, h(x_n) }
  \end{equation*}
  and \( h \) satisfies \cref{def:first_order_homomorphism/functions}.
\end{proof}
...
\begin{proposition}\label{thm:isomorphism_preserves_validity}\mcite[prop. 2.3.39]{Hinman2005Logic}
  ...
  \end{equation*}
  where \( \Bracks{\varphi} \) denotes the denotation function defined in \cref{def:propositional_denotation/function}.
\end{proposition}
\begin{proof}
