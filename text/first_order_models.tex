...
\begin{proposition}\label{thm:algebraic_theory_bijective_homomorphism}
\end{proposition}

\begin{proposition}\label{thm:implicit_universal_quantification}
  Every first-order formula is semantically equivalent to its \hyperref[def:fol_universal_closure]{universal closure}.
\end{proposition}
\begin{comments}
...
\begin{definition}\label{def:fol_homomorphism}\mcite[def. 2.3.26(i); rem. 2.3.27]{Hinman2005Logic}
  ...
  \begin{thmenum}
    \thmitem{def:fol_homomorphism/functions} For any functional symbol \( f \) of arity \( n \) and any tuple \( x_1, \ldots, x_n \in X \) we have
    \begin{equation}\label{eq:def:fol_homomorphism/functions}
      h\parens[\big]{ I(f)(x_1, \ldots, x_n) } = J(f) \parens[\big]{ h(x_1), \ldots, h(x_n) }
    \end{equation}

    \thmitem{def:fol_homomorphism/predicates} For any predicate symbol \( p \) of arity \( n \) and any \( x_1, \ldots, x_n \in X \),
...
\begin{comments}
  ...
  \item \Cref{thm:first_order_homomorphism_as_substructure} provides a condition equivalent to \cref{def:fol_homomorphism/functions}.
  \item Note that the condition \cref{def:fol_homomorphism/predicates} for predicates is strictly weaker than
  \begin{equation*}
    I(p) (x_1, \ldots, x_n) = J(p) \parens[\big]{ h(x_1), \ldots, h(x_n) },
  \end{equation*}
  which is used in \cref{def:first_order_embedding} to define embeddings.
...
\begin{proof}
  ...
  \SubProofOf{thm:def:fol_homomorphism/composition} Let \( h: \mscrX \mapsto \mscrY \) and \( l: \mscrY \mapsto \mscrZ \) both be homomorphisms. Then \( l \relcirc h: \mscrX \to \mscrY \) is a homomorphism:
...
\begin{proof}
  ...
  First, we will use \fullref{thm:induction_on_rooted_trees} on a term \( \tau \) to show that the following extension of \cref{def:first_order_direct_product/functions} holds:
  ...
  Now, we will use \fullref{thm:induction_on_rooted_trees} on a positive formula \( \varphi \) to show that the following extension of \cref{def:first_order_direct_product/predicates} holds:
