...
\begin{definition}\label{def:function_application_syntax}\mimprovised
  Fix a \hyperref[def:formal_language/symbol]{symbol} \( f \) intended to denote a function of \hyperref[con:function_arguments]{arity} \( n \). The following is a simple \hyperref[def:formal_grammar]{formal grammar rule} for \( f \), specifically for \hyperref[con:predicate_logic]{predicate logic}:
  \begin{bnf*}
    \bnfprod{function application} {\bnfts{\( f \)} \bnfsp \bnftsq{(} \bnfsp \underbrace{\bnfpn{term} \bnfsp \bnftsq{,} \bnfsp \cdots \bnfsp \bnftsq{,} \bnfsp \bnfpn{term}}_{n \T*{terms}} \bnfsp \bnftsq{)}}
  \end{bnf*}

  Here the \( \bnfpn{term} \) rule is taken from the ambient \hyperref[def:formal_grammar/schema]{grammar schema}; for example, in \hyperref[def:first_order_logic]{first-order logic} and \hyperref[def:higher_order_logic]{higher-order logic}, these will be the different.
  ...
  \begin{bnf*}
    \bnfprod{postfix application} {\bnftsq{(} \bnfsp \underbrace{\bnfpn{term} \bnfsp \bnftsq{,} \bnfsp \cdots \bnfsp \bnftsq{,} \bnfsp \bnfpn{term}}_{n \T*{term}} \bnfsp \bnftsq{)} \bnfsp \bnfts{\( f \)}}
  \end{bnf*}
  ...
  \begin{bnf*}
    \bnfprod{infix application} {\bnftsq{(} \bnfsp \bnfpn{term} \bnfsp \bnfts{\( \anon \)} \bnfsp \bnfpn{term} \bnfsp \bnftsq{)}}
  \end{bnf*}
\end{definition}
\begin{comments}
...
\begin{remark}\label{rem:def:function_application_syntax}
  ...
  \begin{thmenum}
    ...
    \thmitem{rem:def:function_application_syntax/polish} \incite*[38]{Church1956LogicVol1} calls prefix notation the \enquote{parenthesis-free notation of Jan \L{}ukasiewicz}, who used it for \hyperref[def:propositional_alphabet/connectives]{propositional connectives}. It is also called \enquote{Polish notation} in his honor, for example in \cite[338]{Knuth1997ArtVol1} and \cite[45]{Andrews2002Logic}.

    In prefix notation, the formula
    \begin{equation*}
...
\begin{concept}\label{con:description_operator}
  ...
  \begin{thmenum}
    ...
    Unlike the \( \rotiota \) operator with its complexities, this quantifier can easily be defined as an abbreviation via other logical connectives --- see \cref{rem:exists_unique_abbreviation}.
  \end{thmenum}
\end{concept}
