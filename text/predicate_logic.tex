...
\begin{concept}\label{con:predicate_logic}
  ...
  \begin{thmenum}
    \thmitem{con:predicate_logic/predicate} \textcite{Matthews2007DictionaryOfLinguistics} defines a linguistic predicate as follows:
    \begin{displayquote}
...
\begin{remark}\label{rem:def:function_application_notation}
  ...
  \begin{thmenum}
    ...
    Specifically for propositional connectives, \textcite[38]{Church1956LogicVol1} calls this the \enquote{parenthesis-free notation of Jan \L{}ukasiewicz}. More generally, is also called \enquote{Polish notation} in his honor, for example in \cite[338]{Knuth1997ArtVol1} and \cite[45]{Andrews2002Logic}.
...
\begin{concept}\label{con:description_operator}
  ...
  \begin{center}
    \begin{tabular}{l c c c c}
      \toprule
                                                        & \( \qexists* x p(x) \) & \( \quantifier \varepsilon x p(x) \) & \( \qdesc* x p(x) \) & \( \qExists* x p(x) \) \\
      \midrule
  ...
  \begin{thmenum}
    ...
    \thmitem{con:description_operator/epsilon} David Hilbert's \( \varepsilon \) operator, described in \cite{Leisenring1969MathematicalLogic}, produces a particular value of \( x \) satisfying \( p \), if one exists, and otherwise produces a value \emph{not} satisfying \( p \).
    ...
    In this case, the particular value given by \( \varepsilon \) does not matter, but the nondeterminism may be undesirable in general. The semantics suggested by \textcite[\S 3.3]{Leisenring1969MathematicalLogic} for \( \quantifier \varepsilon x p(x) \) requires a \hyperref[def:choice_function]{choice function} for determining a particular value of \( x \) satisfying \( p(x) \), and a fixed value (independent of \( p \)) in case no value satisfies \( p(x) \). These choices are in general not obvious.

    \textcite[\S 8.3]{Farmer2008STTVirtues} calls the \( \varepsilon \) operator \term{indefinite description}.

    \thmitem{con:description_operator/iota} Bertrand Russell's \( \turnediota \) operator is by intention left undefined wherever \( \varepsilon \) has an ambiguous value.

    \Textcite[30]{WhiteheadRussell1927PrincipiaMathematicaVol1} call it a \term{description} operator. \Textcite{Farmer2008STTVirtues} suggests calling it \term{definite description} to distinguish it from indefinite descriptions. We discuss description operators in \hyperref[def:higher_order_logic]{higher-order logic} in \cref{rem:hol_definite_description}.

    The symbol \( \turnediota \) is an \enquote{inverted} (rotated) Greek iota. The inversion is perhaps reminiscent of how Russell calls \enquote{inverted commas} the quotes that abstract away the meaning of a sentence.

    Unlike Hilbert's \( \varepsilon \) operator, by intention the description is well-defined only if a unique unambiguous value \( x = a \) satisfies \( p(x) \). We list two suggestions for the semantics of  \( \quantifier \turnediota x p(x) \) in the other cases:
    \begin{thmenum}
      \thmitem{con:description_operator/iota/error} We can, as with \( \varepsilon \), provide a dedicated \enquote{error value}. This is done by \textcite[\S 3.2]{Farmer2008STTVirtues}, and also by \textcite[\S 54]{Andrews2002Logic}.
      ...
      \thmitem{con:description_operator/iota/undefined} Alternatively, we can leave \( \quantifier \turnediota x p(x) \) \hyperref[con:undefinedness]{undefined} in the case of nonuniqueness. This is done by \textcite{Farmer1990PartialFunctionSTT}, and, for first-order logic, by \textcite{Hamkins2022DefiniteDescriptions} (in several variants).

      This however requires every formula depending on \( \quantifier \turnediota x p(x) \) to possibly be undefined, which leads to elaborate rules of when a formula may even have a truth value.
    \end{thmenum}
    ...
    Unlike the \( \turnediota \) operator with its complexities, this quantifier can easily be defined as an abbreviation via other logical connectives --- see \cref{rem:fol_formula_conventions/unique_existence}.
  \end{thmenum}
\end{concept}
