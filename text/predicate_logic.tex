...
\begin{definition}\label{def:hol_term}\mimprovised
  ...
  \begin{thmenum}[series=def:hol_term]
    ...
    It is important here that we regard the arrow type \( \tau \synimplies \sigma \) as a standalone entity rather than an abbreviation for \( \qprod {x^\tau} \sigma \)\fnote{Formally, this can be achieved by extending the formal grammar of Martin-L\"of type theory pseudoterms from \cref{def:mltt_pseudoterm} with the rules for simple type connectives from \cref{def:simple_type}.}. Technically, \( x \) is a bound variable and \( \tau \) should be quantifiable (i.e. should inhabit \( \BbbQ \)), but we want \( \tau \) to be a formula (i.e. to inhabit \( \syn\omicron \)).
  ...
  \begin{thmenum}[resume=def:hol_term]
    \thmitem{def:hol_term/over} If \( \Gamma \) is a subcontext of \( \Delta \), we say that \( (\Gamma, M, \tau) \) is a term \term{over} \( \Delta \).

    \thmitem{def:hol_term/closed} If \( M \) has no free variables, we call \( (\varnothing, M, \tau) \) \term[en=closed expression (\cite[270]{Farmer2008STTVirtues})]{closed}.
  \end{thmenum}
\end{definition}
\begin{comments}
...
\begin{definition}\label{def:hol_entailment}
  ...
  Define the entailment \( \Gamma \vdash \varphi \) to hold if there exists a \hyperref[def:type_derivation_tree]{type derivation tree} whose \hyperref[def:type_derivation_tree]{open assumptions} are all in \( C \) or \( \Gamma \)\fnote{Here \( C \) is a list of type assertions, while \( \Gamma \) is a set of triples. The assumption \( M: \tau \) can \enquote{be in \( C \)} if it is an item in \( C \), or it can \enquote{be in \( \Gamma \)} if \( \tau = \syn\omicron \) and \( \Gamma \) contains the logical formula \( (C, M, \syn\omicron) \)} and whose conclusion is \( \varphi: \syn\omicron \).
...
\begin{proposition}\label{thm:hol_natural_deduction_rules}
  ...
  \begin{thmenum}
    \thmitem{thm:hol_natural_deduction_rules/propositional} The propositional \hyperref[def:propositional_natural_deduction_systems]{natural deduction rules} for \hyperref[con:intuitionistic_logic]{intuitionistic logic} from \cref{def:propositional_natural_deduction_systems}.
  \end{thmenum}
\end{proposition}
\begin{comments}
...
\begin{proof}
  ...
  \SubProofOf{thm:hol_natural_deduction_rules/propositional}
  \SubProofOf*{inf:def:propositional_natural_deduction_systems/top/intro}
  \begin{equation*}
    \begin{prooftree}
  ...
  \SubProofOf*{inf:def:propositional_natural_deduction_systems/bot/efq} Fix a formula \( \varphi \)
  \begin{equation*}
    \begin{prooftree}
      \hypo{ x: \syn\Bbbzero }
