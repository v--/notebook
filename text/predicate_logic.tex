...
\begin{definition}\label{def:simply_typed_hol_proof_tree}\mimprovised
  ...
  \begin{thmenum}[series=def:simply_typed_hol_proof_tree]
    ...
    \thmitem{def:simply_typed_hol_proof_tree/proof} \hyperref[con:proof_tree]{Proof trees} for \hyperref[def:simply_typed_hol_formula]{formulas of simply typed higher-order logic} are rather simple since no discharging or eigenvalues are involved, but the intricacies of the rules makes them hard to formalize.
    ...
    \Fullref{thm:simply_typed_hol_equal} shows that the conclusion of any rule application is itself a formula.
...
\begin{proposition}\label{thm:simply_typed_hol_equal}
...
\begin{proof}
  ...
  We will use \fullref{thm:induction_on_rooted_trees} on \( T \) to show that \( M \) is well-formed.
  \begin{itemize}
    ...
    \Fullref{alg:simply_typed_reduction} shows how \( K \) has the same type as \( N \), so \( M \) inhabits \( \syn\omicron \) due to \ref{inf:def:simply_typed_hol_typing/equality}.
    ...
    Since \( A \syneq B \) is well-formed, \( A \) and \( B \) inhabit the same type; then, by an extension of \fullref{alg:simply_typed_substitution}, we can conclude that \( M \) inhabits the same type as \( N \).
...
\begin{proposition}\label{thm:simply_typed_hol_equality}
  ...
  \begin{thmenum}
    \thmitem{thm:simply_typed_hol_equality/reflexivity}\mcite[prop. 5200]{Andrews2002Logic} \hyperref[def:relation_closures/reflexive]{Reflexivity}: for any (freely typed) \( \muplambda \)-term \( M \), we have \( \vdash M \syneq M \).

    \thmitem{thm:simply_typed_hol_equality/symmetry} \hyperref[def:relation_closures/symmetric]{Symmetry}: we have \( M \syneq N \vdash N \syneq M \).

    \thmitem{thm:simply_typed_hol_equality/transitivity} \hyperref[def:relation_closures/transitive]{Transitivity}: we have \( M \syneq N, N \syneq K \vdash M \syneq K \).
  \end{thmenum}
\end{proposition}
\begin{proof}
  \SubProofOf{thm:simply_typed_hol_equality/reflexivity} Let \( \tau \) be the type of \( M \). Then
  \begin{equation*}
    \begin{prooftree}
  ...
  \SubProofOf{thm:simply_typed_hol_equality/symmetry}
  \begin{equation*}
    \begin{prooftree}
      \hypo{}
  ...
  \SubProofOf{thm:simply_typed_hol_equality/transitivity}
  \begin{equation*}
    \begin{prooftree}
      \hypo{ M \syneq N }
      \hypo{ N \syneq K }
...
\begin{proposition}\label{thm:simply_typed_hol_derivations}
  ...
  \begin{thmenum}
    \thmitem{thm:simply_typed_hol_derivations/substitution_of_equal_terms} Fix \( \muplambda \)-terms \( M \) and \( N \) inhabiting the same type and suppose that \( K \) inhabits \( \tau \). Then, for any variable \( x^\tau \), we have
    \begin{equation}\label{eq:thm:simply_typed_hol_derivations/substitution_of_equal_terms}
      \vdash M \syneq N \thinspace \T{entails} {}\vdash M[x^\tau \mapsto K] \syneq N[x^\tau \mapsto K].
    \end{equation}

    \thmitem{thm:simply_typed_hol_derivations/refl_iff_top}\mcite[prop. 5210]{Andrews2002Logic} For any \( \muplambda \)-term \( M \), we have
    \begin{equation}\label{eq:thm:simply_typed_hol_derivations/refl_iff_top}
      \syntop \syniff (M \syneq M)
    \end{equation}

    \thmitem{thm:simply_typed_hol_derivations/top_elimination}\mcite[prop. 5218]{Andrews2002Logic} For any formula \( \varphi \), we have
...
\end{proposition}
\begin{proof}
  \SubProofOf{thm:simply_typed_hol_derivations/substitution_of_equal_terms} Suppose that \( T_{M \syneq N} \) derives \( M \syneq N \) (without assumptions). Then the following tree derives \( M[x^\tau \mapsto K] \syneq N[x^\tau \mapsto K] \):
  \small
  \begin{equation*}
  ...
  \SubProofOf{thm:simply_typed_hol_derivations/refl_iff_top} Let \( \tau \) be the type of \( M \). Consider be the instance of the axiom schema \eqref{eq:def:simply_typed_hol_proof_tree/axioms/a3} where both \( \synX^{\syn\tau} \) and \( \synY^{\syn\tau} \) map to \( \qabs {\syny^\tau} \syny \) and the variable placeholder \( \syny \) maps to the variable \( \synx \):
  \begin{equation*}
  ...
  \SubProofOf{thm:simply_typed_hol_derivations/top_elimination} Consider the instance of the axiom schema \eqref{eq:def:simply_typed_hol_proof_tree/axioms/a1} where \( \synP^{\syn\omicron \synimplies \syn\omicron} \) is
  \begin{equation*}
...
\end{proposition}
\begin{proof}
  \SubProofOf{inf:def:propositional_natural_deduction_systems/top/intro} Special case of \cref{thm:simply_typed_hol_equality/reflexivity} with \( M = \qabs {\synp^{\syn\omicron}} \synp \).
