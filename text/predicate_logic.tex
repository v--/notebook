...
\begin{definition}\label{def:simply_typed_hol_proof_tree}\mimprovised
  ...
  \begin{thmenum}[series=def:simply_typed_hol_proof_tree]
    ...
    \thmitem{def:simply_typed_hol_proof_tree/consequence} Finally, we let \( \Gamma \vdash \varphi \) if there exists a proof tree with conclusion \( \varphi \) whose open assumptions are in \( \Gamma \).
  \end{thmenum}
\end{definition}
\begin{defproof}
...
\begin{proof}
  ...
  We will use \fullref{thm:induction_on_rooted_trees} on \( P \) to show that \( M \) is well-formed.
  \begin{itemize}
...
\end{proposition}
\begin{proof}
  \SubProofOf{inf:thm:simply_typed_hol_equality_derivation/reflexivity} Let \( \tau \) be the type of \( M \). Then
  \begin{equation*}
    \begin{prooftree}
...
\begin{proposition}\label{thm:simply_typed_hol_derivations}
  ...
  \begin{thmenum}[resume=thm:simply_typed_hol_derivations]
    \thmitem{thm:simply_typed_hol_derivations/substitution_of_equal_terms} We can replace the proof of an equality \( \muplambda \)-term with a proof of its valid substitutions without changing the assumptions:
...
\begin{proof}
  ...
  \SubProofOf{thm:simply_typed_hol_derivations/alpha} If \( M \aequiv N \), then \( M \bred* N \), hence we can use \ref{inf:def:simply_typed_hol_proof/rules/a4}:
  \begin{equation*}
    \begin{prooftree}
      \hypo{ M }
  ...
  \SubProofOf{thm:simply_typed_hol_derivations/beta} If \( M \bred N \), then
  \begin{equation*}
    \begin{prooftree}
      \hypo{ M }
      \infer0[\ref{inf:def:simply_typed_hol_proof/rules/a4}]{ M \bred N }
  ...
  \SubProofOf{thm:simply_typed_hol_derivations/substitution_of_equal_terms}\mcite[prop. 5209]{Andrews2002Logic} Suppose that \( P_{M \syneq N} \) derives \( M \syneq N \) from \( \Gamma \). Then the following tree derives \( M[x^\tau \mapsto K] \syneq N[x^\tau \mapsto K] \) from \( \Gamma \):
  \begin{equation*}
  ...
  \SubProofOf{eq:thm:simply_typed_hol_derivations/refl_iff_top}\mcite[prop. 5210]{Andrews2002Logic} Let \( \tau \) be the type of \( M \). Denote by \( P \) the tree
  \begin{equation*}
    \begin{prooftree}
  ...
  \SubProofOf{thm:simply_typed_hol_derivations/top_intro} Special case of \cref{thm:simply_typed_hol_equality_derivation/reflexivity} with \( M = \qabs {\synp^{\syn\omicron}} \synp \).
