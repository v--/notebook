...
\begin{remark}\label{rem:predicate_logic_formalization}
  ...
  Still, we will not attempt to fully formalize the syntax of inference rules and derivations like we did with propositional logic in \fullref{sec:axiomatic_derivations} and \fullref{sec:natural_deduction}.
...
\begin{comments}
  ...
  \item We will justify the use of \hyperref[def:beta_eta_reduction]{\( \beta \)-reduction} in  \cref{def:simply_typed_hol_derivation}, which will allow us to regard \( p(x_1, \ldots, x_n) \) as function application in the sense of \fullref{alg:untyped_lambda_term_to_function}.
...
\begin{definition}\label{def:simply_typed_hol_derivation}\mimprovised
  We will now define a \hyperref[def:consequence_relation]{consequence relation} for \hyperref[def:simply_typed_hol_formula]{formulas} of \hyperref[def:simply_typed_hol]{simply-typed higher-order logic} based on axiomatic derivations for propositional logic from \fullref{sec:axiomatic_derivations}.

  \begin{thmenum}[series=def:simply_typed_hol_derivation]
    \thmitem{def:simply_typed_hol_derivation/axioms} We will need the following \( \muplambda \)-terms, intended to be used as axiom schemas:
    \begin{align}
    ...
    \thmitem{def:simply_typed_hol_derivation/reduction} We will need ad-hoc \( \muplambda \)-term reduction relations. Namely, for every pair of \( \muplambda \)-terms \( A \) and \( B \), we define a reduction relation based on the following rule:
    ...
    \thmitem{def:simply_typed_hol_derivation/rules} We will rely on the following \hyperref[def:inference_rule]{inference rules}:
    \begin{paracol}{2}
      \begin{leftcolumn}
      ...
      \begin{rightcolumn}
        ...
        \begin{equation*}\taglabel[\ensuremath{ \beta }]{inf:def:simply_typed_hol_derivation/rules/beta}
          \begin{prooftree}
            \hypo{ M }
            \infer1[\ref{inf:def:simply_typed_hol_derivation/rules/beta}]{ N },
          \end{prooftree}
        \end{equation*}
        where \( M \bred N \).
      \end{rightcolumn}
    ...
    \thmitem{def:simply_typed_hol_derivation/derivation} Finally, it remains to define an \term{axiomatic derivation}. Similarly to \cref{def:axiomatic_derivation}, we define a derivation of the \hyperref[def:simply_typed_hol_formula]{formula} \( \varphi \) from \( \Gamma \) as a nonempty list \( \psi_1, \ldots, \psi_n \) of formulas such that \( \psi_n = \varphi \) and, for every \( k = 1, \ldots, n \), any of the following hold:
    \begin{thmenum}
      \thmitem{def:simply_typed_hol_derivation/derivation/axiom} \( \psi_k \) is an instance of any of the axiom schemas from \cref{def:simply_typed_hol_derivation/axioms}.

      \thmitem{def:simply_typed_hol_derivation/derivation/premise} \( \psi_k \) is a premise, that is, it belongs to \( \Gamma \).

      \thmitem{def:simply_typed_hol_derivation/derivation/rules} \( \phi_k \) is the conclusion of either \ref{inf:def:simply_typed_hol_derivation/rules/r} or \ref{inf:def:simply_typed_hol_derivation/rules/beta} with premises from \( \psi_1, \ldots, \psi_{k-1} \).
    \end{thmenum}

    \thmitem{def:simply_typed_hol_derivation/consequence} Finally, we let \( \Gamma \vdash \varphi \) if there exists an axiomatic derivation of \( \varphi \) from \( \Gamma \).
  \end{thmenum}
\end{definition}
\begin{defproof}
...
\begin{comments}
  ...
  All the above feature another axiom allowing \( \beta \)-reduction. We could not easily adapt this axiom to our underlying formalisms, but fortunately our extensive discussion of \( \beta \)-reduction in \fullref{sec:lambda_term_reductions} and \fullref{sec:simply_typed_subject_reduction} allowed us to very simply state the rule \ref{inf:def:simply_typed_hol_derivation/rules/beta} instead.

  Additionally, all the above systems feature \enquote{rule R}. For example, \incite[213]{Andrews2002Logic} postulates the following:
  \begin{displayquote}
