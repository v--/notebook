...
\begin{proposition}\label{thm:simply_typed_hol_r_application_to_subformulas}
  ...
  Let \( P_\varphi \) be a proof tree of \( \varphi \) and let \( P_{A \syneq B} \) be a proof tree of \( A^\gamma \syneq B^\gamma \). Let \( \Gamma \) be the set of all their open assumptions. Suppose that we can apply \ref{inf:def:simply_typed_hol_proof/rules/r} to \( \varphi \) and \( A^\gamma \syneq B^\gamma \), i.e. that \( \varphi \redrel{A \mapsto_V B} \varphi' \) for some formula \( \varphi' \), where
  \begin{equation*}
    V = \parens[\Big]{ \op*{Free}(A^\gamma) \cup \op*{Free}(B^\gamma) } \cap \bigcup_{\rho \in \Gamma} \op*{Free}(\rho).
  \end{equation*}
  ...
  \begin{equation*}
    \begin{prooftree}
      \hypo{}
      \ellipsis { \( P_\varphi \) } { \varphi }

      \hypo{}
      \ellipsis { \( P_{A \syneq B} \) } { A^\gamma \syneq B^\gamma }

      \infer2[\ref{inf:def:simply_typed_hol_proof/rules/r}]{ \varphi' }
    \end{prooftree}
  \end{equation*}

  \begin{thmenum}
    \thmitem{thm:simply_typed_hol_r_application_to_subformulas/connective} Now let \( P_{\varphi \bincirc \psi} \) be a proof tree of \( \varphi \bincirc \psi \), where \( \bincirc \) is a propositional connective.

    Let \( \Gamma' \) be the set of all open assumptions of \( P_{\varphi \bincirc \psi} \) and \( P_{A \syneq B} \), and let \( V' \) be the corresponding set of free variables.

    We claim that \( \varphi \bincirc \psi \redrel{A \mapsto_{V'} B} \varphi' \bincirc \psi \), and thus the following is a valid rule application:
    \begin{equation*}
      \begin{prooftree}
        \hypo{}
        \ellipsis { \( P_{\varphi \bincirc \psi} \) } { \varphi \bincirc \psi }

        \hypo{}
        \ellipsis { \( P_{A \syneq B} \) } { A^\gamma \syneq B^\gamma }

        \infer2[\ref{inf:def:simply_typed_hol_proof/rules/r}]{ \varphi' \bincirc \psi }
      \end{prooftree}
    \end{equation*}

    Analogously, \( \psi \bincirc \varphi \redrel{A \mapsto_{V'} B} \psi \bincirc \varphi' \).

    \thmitem{thm:simply_typed_hol_r_application_to_subformulas/quantifier} Given a quantifier \( Q \), substituting in \( \quantifier Q {x^\tau} \varphi \) gives \( \quantifier Q {x^\tau} \varphi' \) if \( x^\tau \) additionally satisfies the free variable conditions of the rule.
  \end{thmenum}
\end{proposition}
\begin{proof}
  \SubProofOf{thm:simply_typed_hol_r_application_to_subformulas/connective} By definition,
  \begin{equation*}
    \varphi \synwedge \psi = (\qabs {f^{\syn\omicron \synimplies \syn\omicron \synimplies \syn\omicron}} f \syntop \syntop) \syneq (\qabs {f^{\syn\omicron \synimplies \syn\omicron \synimplies \syn\omicron}} f \varphi \psi),
  \end{equation*}
  where \( f \) is a deterministically chosen variable not free in \( \varphi \) nor \( \psi \) (we skip its verbose annotation outside of abstractors).

  To conclude that
  \begin{equation*}
    \qabs {f^{\syn\omicron \synimplies \syn\omicron \synimplies \syn\omicron}} f \varphi \psi \redrel{A \mapsto_{V'} B} \qabs {f^{\syn\omicron \synimplies \syn\omicron \synimplies \syn\omicron}} f \varphi' \psi,
  \end{equation*}
  we must use \ref{inf:def:simply_typed_hol_proof/reduction/abs}, which in turn requires \( f \) not to be in \( V' \).
\end{proof}
