...
\begin{example}\label{ex:thm:regular_pumping_lemma}
  ...
  \begin{thmenum}
    ...
    Consider the binary natural number grammar example from \fullref{rem:backus_normal_form}, in which every \( \bnfpn{expression} \) is either a single \( \bnfpn{number} \) or has the form
    \begin{equation*}
    ...
    Assume, as in \fullref{rem:backus_normal_form}, that numbers are single symbol strings. Then the only strings longer than one symbol must be expressions.
