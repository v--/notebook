...
\begin{definition}\label{def:finite_automaton}\mcite[27]{Salomaa1973Languages}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:finite_automaton/recognition}\mcite[def. 4.1.1]{Savage1998Computability} We say that the automaton \term{accepts} or \term{recognizes} the \hyperref[def:formal_language/string]{string} \( a_1 \cdots a_n \) over \( \mscrA \) if there exists a \hyperref[def:graph_walk/directed]{walk}
    \begin{equation*}
    ...
    \thmitem{def:finite_automaton/equivalent}\mcite[152]{Savage1998Computability} We say that two finite automata are \term{equivalent} if they recognize the same language.
  \end{thmenum}
\end{definition}
...
\begin{definition}\label{def:reverse_grammar}\mcite[17]{Salomaa1973Languages}
...
\begin{algorithm}[Regular grammar to finite automaton]\label{alg:regular_grammar_to_automaton}
  ...
  \begin{thmenum}
    ...
    \thmitem{alg:regular_grammar_to_automaton/automaton}\mcite[thm. 4.10.1]{Savage1998Computability} Let \( F \) be some new nonterminal symbol. Then the following is a finite automaton that accepts \( \mscrL(G_4) \), and hence also \( G_2 \) and \( G_1 \):
    \begin{itemize}
...
\begin{algorithm}[Finite automaton to right-linear grammar]\label{alg:finite_automaton_to_right_linear_grammar}\mcite[thm. 4.10.1]{Savage1998Computability}
...
\begin{lemma}[Pumping lemma for regular languages]\label{thm:regular_pumping_lemma}\mcite[lemma 4.5.1]{Savage1998Computability}
...
\begin{proof}
  ...
  \Fullref{thm:pigeonhole_principle/basic} implies that, since \( n \geq p \), at least one state is visited twice. Let \( i \) be the smallest index such that \( q_i \) is repeated, and let \( j > i \) be the smallest index such that \( q_j = q_i \). Then the states \( q_0, \ldots, q_{j-1} \) are distinct by construction, implying \( j - 1 \leq p \).
