...
\begin{defproof}
  ...
  Let \( \Phi_u: R[X] \to R \) be the \hyperref[rem:substitution_homomorphism]{evaluation homomorphism} at \( u \). Then
  \begin{equation*}
...
\end{corollary}
\begin{comments}
  \item As shown in \fullref{ex:gaussian_integers} and \fullref{ex:integers_with_sqrt2}, the vector multiplication operation may differ.
\end{comments}
\begin{proof}
...
\begin{proposition}\label{thm:adjoint_roots_and_quotients_general}
  Fix arbitrary commutative rings \( R \subseteq S \) and some element \( u \) of \( S \). Consider the \hyperref[rem:substitution_homomorphism]{evaluation homomorphism} \( \Phi_u: R[X] \to S \) sending \( X \) to \( u \). Furthermore, suppose that the \hyperref[def:ring/kernel]{kernel} of \( \Phi_u \) is generated by a single polynomial \( p(X) \) (this condition will be improved in \fullref{thm:adjoint_roots_and_quotients}).
...
\begin{example}\label{ex:gaussian_integers}
  The \term{Gaussian integers} are \hyperref[def:complex_numbers]{complex numbers} with integer real and imaginary components. We can define several isomorphic rings for the Gaussian integers, demonstrating \fullref{thm:adjoint_roots_and_quotients_general}.
...
\begin{example}\label{ex:integers_with_sqrt2}
  \Fullref{thm:adjoint_roots_and_quotients_general} gives us the isomorphism
  \begin{equation*}
