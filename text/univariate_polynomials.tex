...
\begin{definition}\label{def:polynomial_degree}\mcite[10]{Knapp2016BasicAlgebra}
  The \term[bg=степен (\cite[22]{ГеновИПр1991Алгебра}), ru=степень (\cite[sec. 11.3]{Тыртышников2007ЛинАлгебра})]{degree} of the nonzero univariate \hyperref[def:polynomial_algebra]{monomial} \( X^k \) is its power \( k \). More generally, the degree of a multivariate monomial \( X_1^{k_1} \cdots X_n^{k_n} \) is the sum of powers \( k_1 + \cdots + k_n \).
  ...
  This notation also subsumes the zero polynomial. We call \( a_n \) the \term[bg=старши коефициент (\cite[23]{ГеновИПр1991Алгебра}), ru=старший коэффициент (\cite[93]{ГеновИПр1991Алгебра})]{leading coefficient} and \( a_0 \) the \term{constant coefficient} of the polynomial.
...
\begin{definition}\label{def:monic_polynomial}
  We say that the nonzero univariate polynomial \( p(X) \) is \term[bg=нормиран (\cite[29]{ГеновИПр1991Алгебра}), ru=нормированный/приведенный (\cite[102]{Винберг2014Алгебра})]{monic} if its leading coefficient is \( 1 \).
\end{definition}
...
\begin{definition}\label{def:polynomial_root}\mimprovised
  ...
  We say that the value \( u \in R \) is a \term[bg=корен (\cite[33]{ГеновИПр1991Алгебра}), ru=корень (\cite[99]{Винберг2014Алгебра})]{root} of multiplicity \( m \) for the univariate polynomial \( p(X) \in R[X] \) of degree \( n \geq m \) if any of the following equivalent conditions hold:
  \begin{thmenum}
