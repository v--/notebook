...
\begin{definition}\label{def:directed_multigraph_free_category}\mcite[48]{MacLane1998CategoryTheory}
  ...
  \begin{thmenum}[series=def:directed_multigraph_free_category]
    ...
    \thmitem{def:directed_multigraph_free_category/composition} The \hyperref[def:category/composition]{composition of the morphisms} \( p: u \to v \) and \( q: v \to w \) is their \hyperref[def:graph_walk/concatenation]{concatenation} (in reverse order):
    \begin{equation*}
  ...
  \begin{thmenum}[resume=def:directed_multigraph_free_category]
    \thmitem{def:directed_multigraph_free_category/functor} For every small category \( \cat{C} \) and every directed multigraph homomorphism \( \varphi: G \to U(\cat{C}) \), consider the following functor, which \enquote{evaluates} walks from \( G \) inside \( \cat{C} \):
