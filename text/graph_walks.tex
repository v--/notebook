...
\begin{definition}\label{def:graph_walk}\mimprovised
  We will define several related notions, all of which are commonly called \term[bg=маршрут (\cite[11]{Мирчев2001}), ru=маршрут (\cite[128]{Зыков2004}), en=walk (\cite[10]{Diestel2005})]{walks}. The relationship between them is discussed in \fullref{rem:undirected_walks} and \fullref{thm:walk_in_undirected_counterpart}.
  ...
  \begin{thmenum}[series=def:graph_walk]
    ...
    \thmitem{def:graph_walk/undirected} In an \hyperref[def:undirected_multigraph]{undirected multigraph}, trying to adapt both \hyperref[def:graph_walk/directed]{directed walks} and \hyperref[def:graph_walk/generalized]{generalized walks} leads to the same definition. We say that \eqref{eq:def:graph_walk} is a \term{walk} if, for every index \( k \), \( e_k \) is an edge with endpoints \( v_{k-1} \) and \( v_k \).
  ...
  \begin{figure}[!ht]
    \begin{equation}\label{eq:fig:def:graph_walk}
      ...
      \end{aligned}
    \end{equation}
    \caption{A \hyperref[def:graph_walk/path]{path}'s \hyperref[def:graph_walk/induced]{induced graph} in an \hyperref[def:undirected_multigraph]{undirected multigraph}.}\label{fig:def:graph_walk}
  \end{figure}
  ...
  \begin{thmenum}[resume=def:graph_walk]
    ...
    \thmitem{def:graph_walk/path}\mcite[191]{Erickson2019} We say that the walk is a \term[bg=път (\cite[11]{Мирчев2001}), ru=путь (\cite[278]{БелоусовТкачёв2004})]{path} if it has no repeated vertices\fnote{Having no repeated vertices also implies having no repeated edges, but not vice versa.}. Unless the path is closed, it can be fully recovered from its induced subgraph, for which reason we can conflate the two.

    \thmitem{def:graph_walk/closed}\mcite[def. 1.1.3]{Knauer2011} We say that a finite walk is \term{closed} if the two endpoints coincide.

    \thmitem{def:graph_walk/reachable}\incite[191]{Erickson2019} We say that the vertex \( v \) is \term[ru=достижимая (вершина) (\cite[278]{БелоусовТкачёв2004})]{reachable} from \( u \) if there exists a (possibly trivial) path from \( u \) to \( v \).
...
\end{definition}
\begin{comments}
  \item It may seem tempting to define walks as sequences of arcs (without explicitly mentioning any vertices). This does, however, introduce an additional difficulty --- we can no longer distinguish different empty paths. An alternative is to define walks as a starting vertex followed by a sequence of arcs or edges, but the definition given above has the advantage of being more widely accepted --- see \fullref{rem:graph_walk_terminology/walk}.
\end{comments}

\begin{definition}\label{def:graph_adjacency}\mcite[3]{Diestel2005}
  In an \hyperref[rem:arbitrary_graph]{arbitrary graph}, if two vertices are \hyperref[def:graph_incidence]{incident} to a common arc/edge, we say that they are \term[bg=съседни (върхове) (\cite[7]{Мирчев2001}), ru=смежные (вершины) (\cite[15]{Зыков2004})]{adjacent}.
\end{definition}
...
\begin{proposition}\label{thm:def:graph_walk}
  ...
  \begin{thmenum}
    \thmitem{thm:def:graph_walk/finite_graph_finite_path} In a \hyperref[def:graph_cardinality/order]{finite-order graph}, every \hyperref[def:graph_walk/path]{path} is finite (however walks can be arbitrarily long).
  \end{thmenum}
\end{proposition}
\begin{proof}
  \SubProofOf{thm:def:graph_walk/finite_graph_finite_path} By \fullref{thm:pigeonhole_principle}, an infinite path must pass through infinitely many vertices, which contradicts the assumption that the graph has finite order.
\end{proof}
...
\begin{remark}\label{rem:graph_walk_terminology}
  ...
  \begin{thmenum}
    ...
    \begin{itemize}
      \item \incite[21]{Diestel2005}, \incite[4]{Bollobas1998}, \incite[572]{Rosen1999}, \incite[128]{Зыков2004} and \incite[\textnumero 7.2.3]{Новиков2013} (as \enquote{маршрут}) for a finite alternating sequence of vertices and incident edges in simple undirected graphs. We generalize this definition to directed graphs and optionally allow walks to be infinite.
      \item \incite[191]{Erickson2019} and \incite[10]{Savage1998} for a sequence of adjacent vertices. This coincides with our usage in the case of simple undirected graphs.
      \item \incite[12]{Мирчев2001} (as \enquote{маршрут}) for a sequence of (undirected) edges where consecutive arcs have an endpoint in common. This coincides with our usage in the case of nonempty finite walks in undirected graphs.
    \end{itemize}

    \thmitem{rem:graph_walk_terminology/directed_walk} The term \enquote{directed walk} is used by
    \begin{itemize}
      \item \incite[564]{Rosen1999} for a sequence of arcs where the tail of each arc is the head of the next. This coincides with our usage for finite nonempty walks.
    \end{itemize}

    \thmitem{rem:graph_walk_terminology/path} The term \enquote{path} is used by
    \begin{itemize}
      \item \incite[377]{Knuth1997Vol1}, \incite[10]{Savage1998} and \incite[191]{Erickson2019} for walks with distinct vertices. We use this definition, although our definition of \enquote{walk} differs from theirs.

      \item \incite[6]{Diestel2005} and \incite[4]{Bollobas1998} for a subgraph with an implicit ordering of the vertices and one arc for every pair of consecutive vertices. This also roughly coincides with our usage in the case of undirected graphs.

      \item \incite[def. 1.1.3]{Knauer2011}, \incite[13]{GondranMinoux1984Graphs} and \incite[11]{Мирчев2001} for a sequence of arcs where the tail of each arc is the head of the next. This corresponds to what we call finite nonempty directed walks.

      \item \incite[278]{БелоусовТкачёв2004} for a sequence of pairwise adjacent vertices in simple directed graphs. This corresponds to what we call finite paths.
    \end{itemize}

    \thmitem{rem:graph_walk_terminology/simple_path} The term \enquote{simple path} is used by
    \begin{itemize}
      \item \incite[def. 1.1.3]{Knauer2011}, \incite[279]{БелоусовТкачёв2004}, \incite[128]{Зыков2004} and \incite[11]{Мирчев2001} for walks with no repeated vertices --- what we call \enquote{paths}. \incite[32]{GondranMinoux1984Graphs} use \enquote{elementary path} instead. \incite[279]{БелоусовТкачёв2004} allow the endpoints to coincide.
    \end{itemize}

    \thmitem{rem:graph_walk_terminology/semipath} The term \enquote{semipath} is used by
    \begin{itemize}
      \item \incite[def. 1.1.3]{Knauer2011} for what we call nonempty finite generalized walks.
    \end{itemize}

    \thmitem{rem:graph_walk_terminology/trail} The term \enquote{trail} is used by
    \begin{itemize}
      \item \incite[5]{Bollobas1998} and \incite[572]{Rosen1999} for walks with no repeated arcs.
    \end{itemize}

    \thmitem{rem:graph_walk_terminology/chain} The term \enquote{chain} is used by
    \begin{itemize}
      \item \incite[128]{Зыков2004} (as \enquote{цепь}) and \incite[\textnumero 7.2.3]{Новиков2013} (as \enquote{цепь}) for walks with no repeated arcs.

      \item \incite[12]{GondranMinoux1984Graphs} and \incite[10]{Мирчев2001} as (as \enquote{верига}) for a sequence of arcs where consecutive arcs have an endpoint in common. This corresponds to what we call finite nonempty generalized walks.

      \item \incite[278]{БелоусовТкачёв2004} as (as \enquote{цепь}) for possibly infinite sequences of pairwise adjacent vertices in simple undirected graphs. This corresponds to what we call paths.
    \end{itemize}
  \end{thmenum}
\end{remark}
...
\begin{definition}\label{def:graph_cycle}
  We say that the \hyperref[def:walk/closed]{closed} \hyperref[def:walk]{walk}
  \begin{equation}\label{eq:def:graph_cycle}
    v_0 \reloset {e_1} \to v_1 \reloset {e_2} \to \cdots \reloset {e_{n-1}} \to v_{n-1} \reloset {e_n} \to v_n
  \end{equation}
  is a \term[bg=цикъл (\cite[11]{Мирчев2001}), ru=цикл (\cite[279]{БелоусовТкачёв2004}), en=cycle (\incite[572]{Rosen1999})]{cycle} if any of the following equivalent conditions hold:
  \begin{thmenum}
    \thmitem{def:graph_cycle/direct}\mcite[279]{БелоусовТкачёв2004} The walk is nonempty, the vertices \( v_1, \ldots, v_n \) are distinct, and so are the arcs/edges \( e_1, \ldots, e_n \).
...
\begin{defproof}
  ...
  Since in a trivial path the vertex has zero degree, we conclude that the path is not trivial --- that is, we hae \( n > 0 \).
...
\begin{remark}\label{rem:graph_cycle_terminology}
  ...
  \begin{thmenum}
    ...
    \begin{itemize}
      \item \incite[5]{Bollobas1998} for closed walks with no repeated arcs.
      \item \incite[13]{GondranMinoux1984Graphs} for what we call closed nonempty walks.
      \item \incite[def. 1.1.3]{Knauer2011} and \incite[279]{БелоусовТкачёв2004} (as \enquote{контур}) for nonempty closed walks where only the endpoints may repeat, but only for simple directed graphs.
      \item \incite[11]{Мирчев2001} (as \enquote{контур}) for closed nonempty generalized walks.
    \end{itemize}

    \thmitem{rem:graph_cycle_terminology/cycle} The term \enquote{cycle} is used by
    \begin{itemize}
      \item \incite[279]{БелоусовТкачёв2004} (as \enquote{цикл}) for \fullref{def:graph_cycle/direct}.

      \item \incite[532]{Rosen1999} and \incite[\textnumero 7.2.3]{Новиков2013} (as \enquote{цикл}) for closed walks with no repeated arcs, which generalizes our usage.

      \item \incite[11]{Мирчев2001} (as \enquote{цикъл}) for closed directed walks, which generalizes our usage in the case of directed graphs.

      \item \incite[550]{Rosen1999} for an undirected graph isomorphic to some \hyperref[def:cycle_graph]{cycle graph}. This usage coincides with ours for undirected graphs.

      \item \incite[191]{Erickson2019} for closed walks that \enquote{enter and leave each vertex at most once}. This coincides with our usage.

      \item \incite[8]{Diestel2005}, \incite[5]{Bollobas1998} and \incite[377]{Knuth1997Vol1} for graphs constructed from a path with its head connecting to its tail, with at least three vertices. This coincides with our usage for simple directed graphs.
      ...
      \item \incite[def. 1.1.3]{Knauer2011} for closed nonempty directed walks.
    \end{itemize}
  \end{thmenum}
\end{remark}
...
\begin{example}\label{ex:def:graph_cycle}
  ...
  \begin{thmenum}
    \thmitem{ex:def:graph_cycle/trivial} We require cycles to be nontrivial, hence \hyperref[def:graph_walk/trivial]{trivial walk} at a vertex is not a cycle. This would prevent any graph from possibly being \hyperref[def:acyclic_graph]{acyclic}.
...
\begin{algorithm}[Cycle removal]\label{alg:cycle_removal}
  Fix a \hyperref[def:undirected_multigraph]{directed multigraph} and consider the following walk:
  ...
  \begin{thmenum}
    \thmitem{alg:cycle_removal/start} We start with \( k = 0 \) and the trivial path at \( v_0 \).
...
\end{algorithm}
\begin{comments}
  \item This algorithm can be found as \identifier{graphs.cycle.remove_cycles} in \cite{code}.
\end{comments}

\begin{proposition}\label{thm:path_iff_walk}
  In any \hyperref[def:undirected_multigraph]{directed multigraph}, there exists a \hyperref[def:graph_walk/directed]{directed walk} between two vertices if and only if there exists a \hyperref[def:graph_walk/path]{path} between them.
\end{proposition}
\begin{proof}
...
\begin{algorithm}[Closed walk to cycle]\label{alg:closed_walk_to_cycle}
  Fix a \hyperref[def:undirected_multigraph]{directed multigraph} and consider the \hi{nontrivial} \hyperref[def:graph_walk/closed]{closed} walk
...
\begin{definition}\label{def:acyclic_graph}
  We say that a \hyperref[rem:arbitrary_graph]{graph} is \term[bg=ацикличен (ориентиран граф) (\cite[14]{Мирчев2001}), ru=ацикличный (неориентированный граф); безконтурный (ориентированный граф) (\cite[280]{БелоусовТкачёв2004})]{acyclic} if it satisfies any of the following equivalent properties:
  \begin{thmenum}
    \thmitem{def:acyclic_graph/cycles}\mcite[21]{Bollobas1998} It has no \hyperref[def:graph_cycle]{cycles}.
    \thmitem{def:acyclic_graph/walks} It has no nontrivial closed walks.
  \end{thmenum}
\end{definition}
\begin{comments}
  \item Every acyclic graph is a union of \hyperref[def:tree]{trees}, for which reason acyclic graphs are also called \enquote{forests}. This usage is mentioned by \incite[13]{Diestel2005}, \incite[21]{Bollobas1998}, \incite[6]{Knauer2011}, \incite[191]{Erickson2019}, \incite[550]{Rosen1999}, \incite[168]{Зыков2004} (as \enquote{лес}), \incite[\textnumero 9.1.1]{Новиков2013} (as \enquote{лес}) and \incite[21]{Мирчев2001} (as \enquote{гора}).
\end{comments}
\begin{defproof}
  \ImplicationSubProof{def:acyclic_graph/cycles}{def:acyclic_graph/walks} Suppose that \( G \) has no cycles. If \( G \) has a nontrivial closed walk, we obtain a cycle via \fullref{alg:closed_walk_to_cycle}, which is a contradiction. Hence, \( G \) has no nontrivial closed walks.

  \ImplicationSubProof{def:acyclic_graph/walks}{def:acyclic_graph/cycles} Special case.
\end{defproof}
...
\begin{proposition}\label{thm:acyclic_undirected_graph_orientation}
  Any \hyperref[def:graph_orientation]{orientation} of an \hyperref[def:acyclic_graph]{acyclic} \hyperref[def:undirected_multigraph]{undirected multigraph} is also \hyperref[def:acyclic_graph]{acyclic}.
\end{proposition}
\begin{comments}
...
\begin{proposition}\label{thm:acyclic_undirected_multigraph}
  An \hyperref[def:acyclic_graph]{acyclic} \hyperref[def:directed_multigraph]{directed multigraph} has no \hyperref[def:directed_multigraph/loop]{loops} or \hyperref[def:directed_multigraph/opposite]{oppositely directed arcs} (but may have \hyperref[def:directed_multigraph/parallel]{parallel arcs}).
\end{proposition}
\begin{proof}
...
\begin{proposition}\label{thm:acyclic_undirected_multigraph}
  An \hyperref[def:acyclic_graph]{acyclic} \hyperref[def:undirected_multigraph]{undirected multigraph} is \hyperref[def:undirected_graph]{simple}, that is, has no \hyperref[def:undirected_multigraph/loop]{loops} or \hyperref[def:undirected_multigraph/parallel]{parallel edges}.
\end{proposition}
\begin{proof}
...
\begin{proposition}\label{thm:acyclic_graph_paths}
  In an \hyperref[def:acyclic_graph]{acyclic} \hyperref[def:undirected_multigraph]{undirected multigraph}, there exists at most one \hyperref[def:graph_walk/path]{path} between any two vertices.
\end{proposition}
\begin{proof}
...
\begin{proposition}\label{thm:def:well_founded_graph}
  ...
  \begin{thmenum}
    \thmitem{thm:def:well_founded_graph/simple} A simple directed graph with loops is well-founded in the sense of \fullref{def:well_founded_graph} if and only if it is well-founded as a \hyperref[def:binary_relation]{binary relation} in the sense of \fullref{def:well_founded_relation}.
...
\begin{proof}
  ...
  By \fullref{thm:pigeonhole_principle}, since \( G \) is finite, there exists an index \( n \) such that \( v_0 = v_n \). Thus, the following walk is closed:
  \begin{equation*}
    v_0 \reloset {e_1} \to v_1 \reloset {e_2} \to \cdots \reloset {e_{n-1}} \to v_{n-1} \reloset {e_n} \to v_n.
  \end{equation*}
...
\begin{definition}\label{def:graph_connectedness}\mcite[def. 1.2.1]{Knauer2011}
  \hyperref[rem:arbitrary_graph]{Graphs} have the following notions of connectedness:
  \begin{thmenum}
    \thmitem{def:graph_connectedness/strong} We say that a \hyperref[def:directed_multigraph]{directed multigraph} is \term[ru=сильно связаный (граф) (\cite[def. 5.2]{БелоусовТкачёв2004})]{strongly connected} if there exists a \hyperref[def:graph_walk/directed]{directed walk} between every pair of distinct vertices.

    Due to \fullref{thm:path_iff_walk}, it is immaterial whether we consider walks or paths.

    \thmitem{def:graph_connectedness/weak} If there exists a \hyperref[def:graph_walk/generalized]{generalized walk} between every pair of distinct vertices, we say that the graph is \term[ru=слабо связный (граф) (\cite[def. 5.4]{БелоусовТкачёв2004})]{weakly connected}.

    \thmitem{def:graph_connectedness/undirected} We say that an \hyperref[def:undirected_multigraph]{undirected multigraph} is \term[bg=свързан (граф) (\cite[13]{Мирчев2001}), ru=свързный (граф) (\cite[285]{БелоусовТкачёв2004})]{connected} if there exists a \hyperref[def:graph_walk/undirected]{walk} between every pair of distinct vertices.
...
\begin{proposition}\label{thm:connected_graph_edge_removal}
  Fix a \hyperref[def:graph_connectedness/undirected]{connected} undirected multigraph \( G = (V, E, \mscrE) \). If one of the endpoints of an edge \( e \) has odd degree, then the graph
  \begin{equation*}
    G' = (V \setminus \set{ v \in \mscrE(e) \given \deg(v) = 1 }, E \setminus \set{ e }, \mscrE)
  \end{equation*}
  is also connected.
\end{proposition}
\begin{proof}
  Let \( e \) be an edge with endpoints \( u \) and \( v \), and suppose that either \( u \) or \( v \) or both have odd degrees.
...
\begin{proof}
  ...
  \begin{itemize}
    \item If \( u_{m-1} = v_{k-1} \), then \( w = v_k \) and we can append to \eqref{eq:thm:connected_graph_cycle_removal/proof/ind} the walk
    \begin{equation*}
    ...
    \begin{figure}[!ht]
      \begin{equation}\label{eq:fig:thm:connected_graph_cycle_removal}
        \begin{aligned}
          \includegraphics[page=1]{output/thm__connected_graph_cycles}
        \end{aligned}
      \end{equation}
    ...
    \item If \( u_{m-1} = v_k \), then \( w = v_{k-1} \) and we can append to \eqref{eq:thm:connected_graph_cycle_removal/proof/ind} the walk
    \begin{equation*}
...
\begin{definition}\label{def:graph_connected_component}\mcite[def. 1.4.11]{Knauer2011}
...
\end{definition}
\begin{comments}
  \item \incite[425]{БелоусовТкачёв2004} and \incite[425]{Зыков2004} use the term \enquote{bicomponent} (\enquote{бикомпонента}) for strongly connected components. This is unrelated to \hyperref[def:r_partite_graph]{bipartite graphs}, which \incite[\textnumero 7.3.3]{Новиков2013} calls \enquote{bigraph} (\enquote{биграф}). \incite[\textnumero 8.5.2]{Новиков2013} prefers the term \enquote{strongly connected component} (\enquote{компоненты сильной связи}) to \enquote{bicomponent}.
\end{comments}
...
\begin{definition}\label{def:directed_graph_condensation}\mcite[example 1.6.6]{Knauer2011}
  ...
  Irreflexive relations over sets can be regarded as \hyperref[def:directed_graph]{simple directed graphs}. Therefore, the pair \( \widetilde{G} \coloneqq (\widetilde{V}, \widetilde{A}) \) is a directed graph. We call it the \term[bg=кондензация (\cite[61]{Мирчев2001}), ru=конденсация (\cite[\textnumero 8.5.2]{Новиков2013})]{condensation} of the directed multigraph \( G \).
\end{definition}
\begin{comments}
  \item Another term for condensations from Russian origin is \enquote{Herz's graph} (\enquote{граф Герца}). It is used by \incite[\textnumero 8.5.2]{Новиков2013} and \incite[426]{Зыков2004}.
\end{comments}
\begin{defproof}
...
\begin{example}\label{ex:def:directed_graph_condensation}
  ...
  \begin{equation}\label{eq:ex:def:directed_graph_condensation/uncondensed}
    \begin{aligned}
      \includegraphics[page=1]{output/ex__def__directed_graph_condensation}
    \end{aligned}
  \end{equation}
  ...
  \begin{equation}\label{eq:ex:def:directed_graph_condensation/condensed}
    \begin{aligned}
      \includegraphics[page=2]{output/ex__def__directed_graph_condensation}
    \end{aligned}
  \end{equation}
...
\begin{definition}\label{def:eulerian_walk}\mcite[16]{Bollobas1998}
  We say that a \hyperref[def:graph_walk]{graph walk} is \term[bg=Ойлеров (цикыл) (\cite[110]{Мирчев2001}), ru=Ойлеров (цикл) (\cite[358]{Новиков2013})]{Eulerian} if each arc/edge in the graph occurs exactly once.
\end{definition}

\begin{theorem}[Euler's theorem for graphs]\label{thm:eulers_theorem_for_graphs}
  Fix a nonempty \hyperref[def:graph_connectedness/undirected]{connected} \hyperref[def:undirected_multigraph]{undirected multigraph} \( G = (V, E, \mscrE) \) with finitely many edges.
...
\begin{proof}
  ...
  \begin{equation*}
    V' \coloneqq \begin{cases}
      V \setminus \set{ v_{n+1} }, &\deg(v_{n+1}) = 1, \\
      V,                           &\deg(v_{n+1}) > 1
    \end{cases}
  \end{equation*}
  and
  ...
  Then \( G' \) is again nonempty and connected, hence the inductive hypothesis holds. Hence, \( \deg'(v_k) \) is even for \( k = 1, \ldots, n - 1 \), where \( \deg'(v_k) \) the degree of \( v_k \) in \( G' \).
  ...
  \SubProof*{Proof of inductive step} Suppose that the statement holds for all graphs with \( n \) edges, and let \( G \) be a graph with exactly \( n + 1 \) edges.

  Fix two vertices \( s \) and \( f \), suppose that all others have even degree, and that \( s = f \) if and only if \( s \) and \( f \) have even degrees. Let \( v_{n+1} \coloneqq f \).
  \begin{itemize}
    \item If \( s \) and \( f \) have even degrees, all vertices of \( G \) have even degrees. Choose \( v_k \) arbitrarily from all vertices adjacent to \( f \).

    \item Otherwise, \( s \) or \( f \) have odd degrees, and, by \fullref{thm:odd_degree_vertices}, both have.
    \begin{itemize}
      \item If \( f \) has a vertex of even degree adjacent to it (possibly \( f \) itself), let \( v_k \) be one of them.

      This choice is essential because, if there is exactly one arc between \( v_k \) and \( f \), we would not be able to leave \( v_k \) by another edge once reached from \( f \). If \( v_k \) has even degree, then there is another edge incident with \( v_k \). More formally, this ensures that the graph \( G' \) defined next will be connected.

      \item Otherwise, only \( s \) and possibly \( f \) are adjacent to \( f \). Let \( v_k \coloneqq s \).
    \end{itemize}
  \end{itemize}

  Let \( e_{k+1} \) be any edge between them \( v_k \) and \( v_{k+1} \). As in the proof of sufficiency, consider the graph \( G' = (V', E', \mscrE) \) obtained by removing \( e_{n+1} \), and denote the degree of \( v \) in \( G' \) by \( \deg'(v) \). The graph \( G' \) is connected as can be seen from the remarks made earlier.

  The following variation of \eqref{eq:thm:eulers_theorem_for_graphs/proof/sufficiency/deg} holds:
  \begin{equation}\label{eq:thm:eulers_theorem_for_graphs/proof/necessity/deg}
    \deg'(v) = \begin{cases}
      \deg(v) - 2, &v = v_k \T{or} v = v_{k+1} \T{and} e_{n+1} \T{is a loop}, \\
      \deg(v) - 1, &v = v_k \T{or} v = v_{k+1} \T{and} e_{n+1} \T{is not a loop}, \\
      \deg(v),     &\T{otherwise.}
    \end{cases}
  \end{equation}

  Note that \( \deg'(v) \) is even for all \( v \) except possibly \( s \), \( v_k \) and \( f = v_{k+1} \). We will show that either all vertices have even degree in \( G' \) or only \( s \) and \( v_n \) have odd degree:
  \begin{itemize}
    \item If \( \deg(s) \) or \( \deg(f) \) are even, all vertices have even degree in \( G \).
    \begin{itemize}
      \item If \( e_{n+1} \) is a loop, then, by \eqref{eq:thm:eulers_theorem_for_graphs/proof/sufficiency/deg}, \( \deg'(v_n) = \deg'(v_{n+1}) \) is again even.

      \item Otherwise, again by \eqref{eq:thm:eulers_theorem_for_graphs/proof/sufficiency/deg}, \( \deg'(v_n) \) and \( \deg'(v_{n+1}) = \deg'(s) \) are odd.
    \end{itemize}

    \item If \( \deg(s) \) or \( \deg(f) \) is odd, by \fullref{thm:odd_degree_vertices} they both are, while all other vertices have even degree in \( G \).
    \begin{itemize}
      \item If \( e_{n+1} \) is a loop, then \( \deg'(v_n) = \deg'(v_{k+1}) \) is again odd, and, by \fullref{thm:odd_degree_vertices}, so is \( \deg'(s) \).

      \item Otherwise, \( \deg'(v_{k+1}) \) is even.
      \begin{itemize}
        \item If \( \deg(v_n) \) is odd, by construction \( v_n = s \), and \( \deg'(s) = \deg'(v_n) \) is even.
        \item If \( \deg(v_n) \) is even, then \( v_n \neq s \) by construction, however \( \deg'(v_n) \) is odd, and, by \fullref{thm:odd_degree_vertices}, so is \( \deg'(s) \).
      \end{itemize}
    \end{itemize}
  \end{itemize}

  In all cases, at most \( s \) and \( v_n \) have odd degree, therefore, we can apply the inductive hypothesis for \( G' \) to obtain an Eulerian walk
  \begin{equation*}
    s = v_0 \reloset {e_1} \to v_1 \reloset {e_2} \to \cdots \reloset {e_{n-1}} \to v_{n-1} \reloset {e_n} \to v_n.
  \end{equation*}

  We can now append \( e_{n+1} \) to obtain an Eulerian walk in \( G \). It remains to show that \( s = f \) if and only if \( \deg(s) \) and \( \deg(f) \) are even.

  If \( s = f \), by \fullref{thm:odd_degree_vertices}, \( \deg(s) = \deg(f) \) must be even. Conversely, suppose that \( \deg(s) \) and \( \deg(f) \) are even.
  \begin{itemize}
    \item If \( e_{n+1} \) is a loop, then \( \deg'(v_n) \) is even, and hence so is \( \deg'(s) \). Then \( s = v_n \), hence \( s = f \).

    \item Otherwise, \( \deg'(v_n) \) is odd and hence so is \( \deg'(s) \). By assumption, \( \deg(s) \) is even, which, due to \eqref{eq:thm:eulers_theorem_for_graphs/proof/necessity/deg}, is only possible if \( s = v_n \) or \( s = v_{n+1} \).

    But, by the inductive hypothesis, since both \( \deg'(s) \) and \( \deg'(v_n) \) are odd, we have \( s \neq v_n \).

    It remains for \( s = v_{n+1} \) to hold.
  \end{itemize}
\end{proof}
