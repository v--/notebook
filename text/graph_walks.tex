...
\begin{remark}\label{rem:graph_walk_terminology}
  ...
  \begin{thmenum}
    ...
    \begin{itemize}
      \item \incite[21]{Diestel2005}, \incite[13]{Harary1969}, \incite[4]{Bollobas1998}, \incite[572]{Rosen1999}, \incite[22]{Емеличев1990} (as \enquote{маршрут}) and \incite[\S 7.2.3]{Новиков2013} (as \enquote{маршрут}) for a finite alternating sequence of vertices and incident edges in simple undirected graphs. We generalize this definition to directed graphs and optionally allow walks to be infinite.
    ...
    \begin{itemize}
      \item \incite[22]{Емеличев1990} (as \enquote{цепь}) and \incite[\S 7.2.3]{Новиков2013} (as \enquote{цепь}) for walks with no repeated arcs. In case of no repeated vertices, both use \enquote{simple chain} (\enquote{простая цепь}).
...
\begin{remark}\label{rem:graph_cycle_terminology}
  ...
  \begin{thmenum}
    ...
    \begin{itemize}
      ...
      \item \incite[532]{Rosen1999}, \incite[22]{Емеличев1990} (as \enquote{цикл}) and \incite[\S 7.2.3]{Новиков2013} (as \enquote{цикл}) for closed walks with no repeated arcs, which generalizes our usage.
...
\end{definition}
\begin{comments}
  \item \incite[425]{БелоусовТкачёв2004} use the term \enquote{bicomponent} (\enquote{бикомпонента}) for strongly connected components. This is unrelated to \hyperref[def:r_partite_graph]{bipartite graphs}, which \incite[\S 7.3.3]{Новиков2013} calls \enquote{bigraph} (\enquote{биграф}). \incite[\S 8.5.2]{Новиков2013} prefers the term \enquote{strongly connected component} (\enquote{компоненты сильной связи}) to \enquote{bicomponent}.
...
\begin{definition}\label{def:graph_bridge}
  We say that an edge in an \hyperref[def:undirected_multigraph]{undirected multigraph} is a \term[bg=мост (\cite[134]{Мирчев2001}, ru=мост \cite[\S 8.1.2]{Новиков2013}]{bridge} if it satisfies any of the following equivalent definitions:
  \begin{thmenum}
...
\end{definition}
\begin{comments}
  \item Another term for condensations from Russian origin is \enquote{Herz's graph} (\enquote{граф Герца}). It is used by \incite[\S 8.5.2]{Новиков2013}.
\end{comments}
\begin{defproof}
...
\begin{definition}\label{def:eulerian_walk}\mcite[65]{Harary1969}
  We say that a \hyperref[def:graph_walk]{graph walk} is \term[bg=Ойлеров (цикъл) (\cite[110]{Мирчев2001}), ru=Эйлеровый (маршрут) (\cite[192]{Емеличев1990})]{Eulerian} if each arc/edge in the graph occurs exactly once.
\end{definition}
\begin{comments}
  \item See \fullref{ex:konigsberg_bridges} for a broad discussion of the origin of this concept.
\end{comments}

\begin{theorem}[Euler's theorem for graphs]\label{thm:eulers_theorem_for_graphs}\mcite[\S 20]{Euler1741}
