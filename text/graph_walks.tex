...
\begin{definition}\label{def:graph_walk}\mimprovised
  ...
  In an \hyperref[rem:graph_of_unspecified_type]{(hyper)graph of unspecified type}, consider a (finite or infinite) nonempty alternating sequence
  ...
  \begin{thmenum}[resume=def:graph_walk]
    ...
    See \cref{ex:def:extended_real_numbers/walk_length} for a discussion of alternative definitions.
...
\end{proposition}
\begin{proof}
  \SubProofOf{thm:def:graph_walk/finite_graph_infinite_walk} Follows from \cref{thm:pigeonhole_principle/infinitary}.
...
\begin{remark}\label{rem:graph_walk_terminology}
  ...
  \begin{thmenum}
    ...
    \begin{itemize}
      \item \Textcite[6]{König1990GraphTheory} (originally \enquote{Kantenfolge}, literally \enquote{edge sequence}) for a finite graph whose arcs or edges can be \hyperref[def:set_enumeration]{enumerated}, with repetition allowed, so that consecutive arcs/edges in the sequence have a common vertex.

      \item \Textcite[21]{Diestel2017GraphTheory}, \textcite[13]{Harary1969GraphTheory}, \textcite[4]{Bollobás1998ModernGraphTheory}, \textcite[571]{Stanley2012EnumerativeCombinatoricsVol1}, \textcite[22]{ЕмеличевИПр1990ТеорияГрафов} (as \enquote{\textru{маршрут}}), \textcite[128]{Зыков2004ТеорияГрафов} (as \enquote{\textru{маршрут}}) and \textcite[\S 7.2.3]{Новиков2013ДискретнаяМатематика} (as \enquote{\textru{маршрут}}) for a finite alternating sequence of vertices and incident edges in simple undirected graphs. We generalize this definition to directed graphs and optionally allow walks to be infinite.

      \item \Textcite[191]{Erickson2019Algorithms} for a sequence of adjacent vertices. This coincides with our usage in the case of simple undirected graphs.

      \item \Textcite[12]{Мирчев2001Графи} (as \enquote{\textbg{маршрут}}) for a sequence of (undirected) edges where consecutive arcs have an endpoint in common. This coincides with our usage in the case of nonempty finite walks in undirected graphs.
    \end{itemize}

    \thmitem{rem:graph_walk_terminology/path} The term \enquote{path} is used by
    \begin{itemize}
      \item \Textcite[13]{Harary1969GraphTheory} and \textcite[571]{Stanley2012EnumerativeCombinatoricsVol1} for walks with distinct vertices. We use this definition.

      \item \Textcite[377]{Knuth1997ArtVol1} and \textcite[191]{Erickson2019Algorithms} also for walks with distinct vertices, however our definition of \enquote{walk} differs from theirs.

      \item \Textcite[def. 10.4.1]{Rosen2019DiscreteMathematics} for sequences of (undirected) edges for which there exist sequences of vertices that jointly constitute what we call a walk.

      \item \Textcite[6]{Diestel2017GraphTheory} and \textcite[4]{Bollobás1998ModernGraphTheory} for a subgraph with an implicit ordering of the vertices and one arc for every pair of consecutive vertices. This also roughly coincides with our usage in the case of undirected graphs.

      \item \Textcite[def. 1.1.3]{Knauer2019AlgebraicGraphTheory}, \textcite[13]{GondranMinoux1984GraphsAndAlgorithms} and \textcite[11]{Мирчев2001Графи} for a sequence of arcs where the tail of each arc is the head of the next. This corresponds to what we call finite nonempty directed walks.

      \item \Textcite[278]{БелоусовТкачёв2004ДискретнаяМатематика} for a sequence of pairwise adjacent vertices in simple directed graphs. This corresponds to what we call finite paths.
    \end{itemize}

    \thmitem{rem:graph_walk_terminology/simple_path} The term \enquote{simple path} is used by
    \begin{itemize}
      \item \Textcite[def. 1.1.3]{Knauer2019AlgebraicGraphTheory}, \textcite[279]{БелоусовТкачёв2004ДискретнаяМатематика} (as \enquote{\textru{простой путь}}) and \textcite[11]{Мирчев2001Графи} for walks with no repeated vertices --- what we call \enquote{paths}. \Textcite[32]{GondranMinoux1984GraphsAndAlgorithms} use \enquote{elementary path} instead. \Textcite[279]{БелоусовТкачёв2004ДискретнаяМатематика} allow the endpoints to coincide.
    \end{itemize}

    \thmitem{rem:graph_walk_terminology/semipath} The term \enquote{semipath} is used by
    \begin{itemize}
      \item \Textcite[def. 1.1.3]{Knauer2019AlgebraicGraphTheory} for what we call nonempty finite generalized walks.
    \end{itemize}

    \thmitem{rem:graph_walk_terminology/trail} The term \enquote{trail} is used by
    \begin{itemize}
      \item \Textcite[13]{Harary1969GraphTheory}, \textcite[571]{Stanley2012EnumerativeCombinatoricsVol1} and \textcite[5]{Bollobás1998ModernGraphTheory} for walks with no repeated arcs.
    \end{itemize}

    \thmitem{rem:graph_walk_terminology/chain} The term \enquote{chain} is used by
    \begin{itemize}
      \item \Textcite[22]{ЕмеличевИПр1990ТеорияГрафов} (as \enquote{\textru{цепь}}), \textcite[128]{Зыков2004ТеорияГрафов} (as \enquote{\textru{цепь}}) and \textcite[\S 7.2.3]{Новиков2013ДискретнаяМатематика} (as \enquote{\textru{цепь}}) for walks with no repeated arcs. In case of no repeated vertices, both use \enquote{simple chain} (\enquote{простая цепь}).

      \item \Textcite[12]{GondranMinoux1984GraphsAndAlgorithms} and \textcite[10]{Мирчев2001Графи} as (as \enquote{\textbg{верига}}) for a sequence of arcs where consecutive arcs have an endpoint in common. This corresponds to what we call finite nonempty generalized walks.

      \item \Textcite[278]{БелоусовТкачёв2004ДискретнаяМатематика} as (as \enquote{\textru{цепь}}) for possibly infinite sequences of pairwise adjacent vertices in simple undirected graphs. This corresponds to what we call paths.
    \end{itemize}
  \end{thmenum}
\end{remark}
...
\begin{definition}\label{def:graph_cycle}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:graph_cycle/simplified}\mimprovised The walk is nonempty, the vertices \( v_1, \ldots, v_n \) are distinct and, in case the graph is undirected and, if \( n = 2 \), additionally \( e_1 \neq e_2 \).
...
\begin{remark}\label{rem:graph_cycle_terminology}
  ...
  \begin{thmenum}
    ...
    \begin{itemize}
      \item \Textcite[5]{Bollobás1998ModernGraphTheory} for closed walks with no repeated arcs.

      \item \Textcite[13]{GondranMinoux1984GraphsAndAlgorithms} for what we call closed nonempty walks.

      \item \Textcite[def. 10.4.1]{Rosen2019DiscreteMathematics} for closed walks in undirected (multi)graphs.

      \item \Textcite[def. 1.1.3]{Knauer2019AlgebraicGraphTheory} and \textcite[279]{БелоусовТкачёв2004ДискретнаяМатематика} (as \enquote{\textru{контур}}) for nonempty closed walks where only the endpoints may repeat, but only for simple directed graphs.

      \item \Textcite[11]{Мирчев2001Графи} (as \enquote{\textbg{контур}}) for closed nonempty generalized walks.
    \end{itemize}

    \thmitem{rem:graph_cycle_terminology/cycle} The term \enquote{cycle} is used by
    \begin{itemize}
      \item \Textcite[279]{БелоусовТкачёв2004ДискретнаяМатематика} (as \enquote{\textru{цикл}}) for \cref{def:graph_cycle/direct}.

      \item \Textcite[22]{ЕмеличевИПр1990ТеорияГрафов} (as \enquote{\textru{цикл}}) and \textcite[\S 7.2.3]{Новиков2013ДискретнаяМатематика} (as \enquote{\textru{цикл}}) for closed walks with no repeated arcs, which generalizes our usage.

      \item \Textcite[11]{Мирчев2001Графи} (as \enquote{\textbg{цикъл}}) for closed directed walks, which generalizes our usage in the case of directed graphs.

      \item \Textcite[example 10.2.6]{Rosen2019DiscreteMathematics} for (undirected) cycle graphs as defined in \cref{def:cycle_graph}, and in \cite[def. 10.4.2]{Rosen2019DiscreteMathematics} for closed directed walks.

      \item \Textcite[191]{Erickson2019Algorithms} for closed walks that \enquote{enter and leave each vertex at most once}. This coincides with our usage.

      \item \Textcite[13]{Harary1969GraphTheory}, \textcite[8]{Diestel2017GraphTheory}, \textcite[5]{Bollobás1998ModernGraphTheory} and \textcite[377]{Knuth1997ArtVol1} for graphs constructed from a path with its head connecting to its tail, with at least three vertices. This coincides with our usage for simple directed graphs.

      \item \Textcite[13]{GondranMinoux1984GraphsAndAlgorithms} for closed nonempty generalized walks where consecutive arcs are distinct. The condition for consecutive arcs prevents the pathological case \cref{ex:def:graph_cycle/pathological}, thus their definition generalizes ours.

      \item \Textcite[def. 1.1.3]{Knauer2019AlgebraicGraphTheory} for closed nonempty directed walks.
    \end{itemize}
  \end{thmenum}
\end{remark}
...
\begin{example}\label{ex:def:graph_cycle}
  ...
  \begin{thmenum}
    ...
    \thmitem{ex:def:graph_cycle/pathological} The additional requirement in \cref{def:graph_cycle/simplified} rules out the pathological cycle \( a \reloset e \to b \reloset e \to a \), which would prevent undirected graphs from possibly being \hyperref[def:acyclic_graph]{acyclic}.
...
\end{definition}
\begin{comments}
  \item Every acyclic graph is a union of \hyperref[def:tree]{trees}, for which reason acyclic graphs are also called \enquote{forests}. This usage is mentioned by \textcite[13]{Diestel2017GraphTheory}, \textcite[32]{Harary1969GraphTheory}, \textcite[21]{Bollobás1998ModernGraphTheory}, \textcite[def. 1.3.5.]{Knauer2019AlgebraicGraphTheory}, \textcite[782]{Rosen2019DiscreteMathematics}, \textcite[191]{Erickson2019Algorithms}, \textcite[53]{ЕмеличевИПр1990ТеорияГрафов} (as \enquote{\textru{лес}}), \textcite[\S 9.1.1]{Новиков2013ДискретнаяМатематика} (as \enquote{\textru{лес}}) and \textcite[21]{Мирчев2001Графи} (as \enquote{\textbg{гора}}).
...
\begin{proof}
  ...
  Then \( \deg'(v) \) is necessarily even unless \( v \) is \( s \), \( v_k \), or \( v_{k+1} = f \). \Cref{thm:odd_degree_vertices} implies that all three cannot simultaneously have odd degrees in \( G' \) --- either all vertices in \( G' \) can have even degrees, or all except for two.
