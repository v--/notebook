...
\begin{definition}\label{def:graph_walk}\mimprovised
  We will define several related notions, all of which are commonly called \term[bg=маршрут (\cite[11]{Мирчев2001}), ru=маршрут (\cite[23]{Емеличев1990}), en=walk (\cite[10]{Diestel2005})]{walks}. The relationship between them is discussed in \fullref{rem:undirected_walks} and \fullref{thm:walk_in_undirected_counterpart}.
  ...
  \begin{thmenum}[series=def:graph_walk]
    ...
    Since walks in undirected graphs are directed in nature, we use the notations for directed paths and, when speaking about arbitrary-kind graphs, refer to them as \enquote{directed walks} (e.g. in \fullref{def:graph_cycle}).
  \end{thmenum}
...
\begin{remark}\label{rem:graph_walk_terminology}
  ...
  \begin{thmenum}
    ...
    \begin{itemize}
      \item \incite[21]{Diestel2005}, \incite[13]{Harary1969}, \incite[4]{Bollobas1998}, \incite[572]{Rosen1999}, \incite[22]{Емеличев1990} (as \enquote{маршрут}) and \incite[\textnumero 7.2.3]{Новиков2013} (as \enquote{маршрут}) for a finite alternating sequence of vertices and incident edges in simple undirected graphs. We generalize this definition to directed graphs and optionally allow walks to be infinite.
    ...
    \begin{itemize}
      \item \incite[13]{Harary1969} for walks with distinct vertices. We use this definition.

      \item \incite[377]{Knuth1997Vol1}, \incite[10]{Savage1998} and \incite[191]{Erickson2019} also for walks with distinct vertices, however our definition of \enquote{walk} differs from theirs.
    ...
    \begin{itemize}
      \item \incite[13]{Harary1969}, \incite[5]{Bollobas1998} and \incite[572]{Rosen1999} for walks with no repeated arcs.
    \end{itemize}

    \thmitem{rem:graph_walk_terminology/chain} The term \enquote{chain} is used by
    \begin{itemize}
      \item \incite[22]{Емеличев1990} (as \enquote{цепь}) and \incite[\textnumero 7.2.3]{Новиков2013} (as \enquote{цепь}) for walks with no repeated arcs. In case of no repeated vertices, both use \enquote{simple chain} (\enquote{простая цепь}).
...
\begin{definition}\label{def:graph_cycle}
  ...
  \begin{equation}\label{eq:def:graph_cycle}
    v_0 \reloset {e_1} \to v_1 \reloset {e_2} \to \cdots \reloset {e_{n-1}} \to v_{n-1} \reloset {e_n} \to v_n
  \end{equation}
  is a \term[bg=цикъл (\cite[11]{Мирчев2001}), ru=цикл (\cite[279]{БелоусовТкачёв2004}), en=cycle (\incite[13]{Harary1969})]{cycle} if any of the following equivalent conditions hold:
  \begin{thmenum}
...
\begin{remark}\label{rem:graph_cycle_terminology}
  ...
  \begin{thmenum}
    ...
    \begin{itemize}
      ...
      \item \incite[532]{Rosen1999}, \incite[22]{Емеличев1990} (as \enquote{цикл}) and \incite[\textnumero 7.2.3]{Новиков2013} (as \enquote{цикл}) for closed walks with no repeated arcs, which generalizes our usage.
      ...
      \item \incite[13]{Harary1969}, \incite[8]{Diestel2005}, \incite[5]{Bollobas1998} and \incite[377]{Knuth1997Vol1} for graphs constructed from a path with its head connecting to its tail, with at least three vertices. This coincides with our usage for simple directed graphs.
...
\begin{definition}\label{def:acyclic_graph}
  We say that a \hyperref[rem:arbitrary_kind_graph]{graph} is \term[bg=ацикличен (ориентиран граф) (\cite[14]{Мирчев2001}), ru=ацикличный (неориентированный граф); безконтурный (ориентированный граф) (\cite[280]{БелоусовТкачёв2004})]{acyclic} if it satisfies any of the following equivalent properties:
  \begin{thmenum}
    \thmitem{def:acyclic_graph/cycles}\mcite[32]{Harary1969} It has no \hyperref[def:graph_cycle]{cycles}.
    \thmitem{def:acyclic_graph/walks} It has no nontrivial closed walks.
  \end{thmenum}
\end{definition}
\begin{comments}
  \item Every acyclic graph is a union of \hyperref[def:tree]{trees}, for which reason acyclic graphs are also called \enquote{forests}. This usage is mentioned by \incite[13]{Diestel2005}, \incite[32]{Harary1969}, \incite[21]{Bollobas1998}, \incite[6]{Knauer2011}, \incite[191]{Erickson2019}, \incite[550]{Rosen1999}, \incite[53]{Емеличев1990} (as \enquote{лес}), \incite[\textnumero 9.1.1]{Новиков2013} (as \enquote{лес}) and \incite[21]{Мирчев2001} (as \enquote{гора}).
\end{comments}
\begin{defproof}
...
\begin{definition}\label{def:graph_bridge}
  We say that an edge in an \hyperref[def:undirected_multigraph]{undirected multigraph} is a \term[bg=мост (\cite[134]{Мирчев2001}, ru=мост \cite[\textnumero 8.1.2]{Новиков2013}]{bridge} if it satisfies any of the following equivalent definitions:
  \begin{thmenum}
    \thmitem{def:graph_bridge/components}\mcite[26]{Harary1969} After removing it from its \hyperref[def:graph_connected_component]{connected component}, it is no longer connected.
...
\end{definition}
\begin{comments}
  \item Another term for condensations from Russian origin is \enquote{Herz's graph} (\enquote{граф Герца}). It is used by \incite[\textnumero 8.5.2]{Новиков2013}.
\end{comments}
\begin{defproof}
...
\begin{definition}\label{def:eulerian_walk}\mcite[65]{Harary1969}
  We say that a \hyperref[def:graph_walk]{graph walk} is \term[bg=Ойлеров (цикыл) (\cite[110]{Мирчев2001}), ru=Эйлеровый (маршрут) (\cite[192]{Емеличев1990})]{Eulerian} if each arc/edge in the graph occurs exactly once.
\end{definition}
