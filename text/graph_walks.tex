...
\begin{remark}\label{rem:graph_walk_terminology}
  ...
  \begin{thmenum}
    ...
    \begin{itemize}
      \item \incite[6]{König1990GraphTheory} (originally \enquote{Kantenfolge}, literally \enquote{edge sequence}) for a finite graph whose arcs or edges can be \hyperref[def:set_enumeration]{enumerated}, with repetition allowed, so that consecutive arcs/edges in the sequence have a common vertex.

      \item \incite[21]{Diestel2017GraphTheory}, \incite[13]{Harary1969GraphTheory}, \incite[4]{Bollobás1998ModernGraphTheory}, \incite[571]{Stanley2012EnumerativeCombinatoricsVol1}, \incite[22]{ЕмеличевИПр1990ТеорияГрафов} (as \enquote{\textru{маршрут}}), \incite[128]{Зыков2004ТеорияГрафов} (as \enquote{\textru{маршрут}}) and \incite[\S 7.2.3]{Новиков2013ДискретнаяМатематика} (as \enquote{\textru{маршрут}}) for a finite alternating sequence of vertices and incident edges in simple undirected graphs. We generalize this definition to directed graphs and optionally allow walks to be infinite.
      ...
      \item \incite[12]{Мирчев2001Графи} (as \enquote{\textbg{маршрут}}) for a sequence of (undirected) edges where consecutive arcs have an endpoint in common. This coincides with our usage in the case of nonempty finite walks in undirected graphs.
    \end{itemize}

    \thmitem{rem:graph_walk_terminology/path} The term \enquote{path} is used by
    \begin{itemize}
      \item \incite[13]{Harary1969GraphTheory} and \incite[571]{Stanley2012EnumerativeCombinatoricsVol1} for walks with distinct vertices. We use this definition.
      ...
      \item \incite[6]{Diestel2017GraphTheory} and \incite[4]{Bollobás1998ModernGraphTheory} for a subgraph with an implicit ordering of the vertices and one arc for every pair of consecutive vertices. This also roughly coincides with our usage in the case of undirected graphs.

      \item \incite[def. 1.1.3]{Knauer2019AlgebraicGraphTheory}, \incite[13]{GondranMinoux1984GraphsAndAlgorithms} and \incite[11]{Мирчев2001Графи} for a sequence of arcs where the tail of each arc is the head of the next. This corresponds to what we call finite nonempty directed walks.
    ...
    \begin{itemize}
      \item \incite[def. 1.1.3]{Knauer2019AlgebraicGraphTheory}, \incite[279]{БелоусовТкачёв2004ДискретнаяМатематика} (as \enquote{\textru{простой путь}}) and \incite[11]{Мирчев2001Графи} for walks with no repeated vertices --- what we call \enquote{paths}. \incite[32]{GondranMinoux1984GraphsAndAlgorithms} use \enquote{elementary path} instead. \incite[279]{БелоусовТкачёв2004ДискретнаяМатематика} allow the endpoints to coincide.
    \end{itemize}

    \thmitem{rem:graph_walk_terminology/semipath} The term \enquote{semipath} is used by
    \begin{itemize}
      \item \incite[def. 1.1.3]{Knauer2019AlgebraicGraphTheory} for what we call nonempty finite generalized walks.
    \end{itemize}

    \thmitem{rem:graph_walk_terminology/trail} The term \enquote{trail} is used by
    \begin{itemize}
      \item \incite[13]{Harary1969GraphTheory}, \incite[571]{Stanley2012EnumerativeCombinatoricsVol1} and \incite[5]{Bollobás1998ModernGraphTheory} for walks with no repeated arcs.
    \end{itemize}

    \thmitem{rem:graph_walk_terminology/chain} The term \enquote{chain} is used by
    \begin{itemize}
      \item \incite[22]{ЕмеличевИПр1990ТеорияГрафов} (as \enquote{\textru{цепь}}), \incite[128]{Зыков2004ТеорияГрафов} (as \enquote{\textru{цепь}}) and \incite[\S 7.2.3]{Новиков2013ДискретнаяМатематика} (as \enquote{\textru{цепь}}) for walks with no repeated arcs. In case of no repeated vertices, both use \enquote{simple chain} (\enquote{простая цепь}).

      \item \incite[12]{GondranMinoux1984GraphsAndAlgorithms} and \incite[10]{Мирчев2001Графи} as (as \enquote{\textbg{верига}}) for a sequence of arcs where consecutive arcs have an endpoint in common. This corresponds to what we call finite nonempty generalized walks.

      \item \incite[278]{БелоусовТкачёв2004ДискретнаяМатематика} as (as \enquote{\textru{цепь}}) for possibly infinite sequences of pairwise adjacent vertices in simple undirected graphs. This corresponds to what we call paths.
    \end{itemize}
  \end{thmenum}
\end{remark}
...
\begin{remark}\label{rem:graph_cycle_terminology}
  ...
  \begin{thmenum}
    ...
    \begin{itemize}
      \item \incite[5]{Bollobás1998ModernGraphTheory} for closed walks with no repeated arcs.

      \item \incite[13]{GondranMinoux1984GraphsAndAlgorithms} for what we call closed nonempty walks.

      \item \incite[def. 10.4.1]{Rosen2019DiscreteMathematics} for closed walks in undirected (multi)graphs.

      \item \incite[def. 1.1.3]{Knauer2019AlgebraicGraphTheory} and \incite[279]{БелоусовТкачёв2004ДискретнаяМатематика} (as \enquote{\textru{контур}}) for nonempty closed walks where only the endpoints may repeat, but only for simple directed graphs.

      \item \incite[11]{Мирчев2001Графи} (as \enquote{\textbg{контур}}) for closed nonempty generalized walks.
    \end{itemize}

    \thmitem{rem:graph_cycle_terminology/cycle} The term \enquote{cycle} is used by
    \begin{itemize}
      \item \incite[279]{БелоусовТкачёв2004ДискретнаяМатематика} (as \enquote{\textru{цикл}}) for \cref{def:graph_cycle/direct}.

      \item \incite[22]{ЕмеличевИПр1990ТеорияГрафов} (as \enquote{\textru{цикл}}) and \incite[\S 7.2.3]{Новиков2013ДискретнаяМатематика} (as \enquote{\textru{цикл}}) for closed walks with no repeated arcs, which generalizes our usage.

      \item \incite[11]{Мирчев2001Графи} (as \enquote{\textbg{цикъл}}) for closed directed walks, which generalizes our usage in the case of directed graphs.

      \item \incite[example 10.2.6]{Rosen2019DiscreteMathematics} for (undirected) cycle graphs as defined in \cref{def:cycle_graph}, and in \cite[def. 10.4.2]{Rosen2019DiscreteMathematics} for closed directed walks.
      ...
      \item \incite[13]{Harary1969GraphTheory}, \incite[8]{Diestel2017GraphTheory}, \incite[5]{Bollobás1998ModernGraphTheory} and \incite[377]{Knuth1997ArtVol1} for graphs constructed from a path with its head connecting to its tail, with at least three vertices. This coincides with our usage for simple directed graphs.

      \item \incite[13]{GondranMinoux1984GraphsAndAlgorithms} for closed nonempty generalized walks where consecutive arcs are distinct. The condition for consecutive arcs prevents the pathological case \cref{ex:def:graph_cycle/pathological}, thus their definition generalizes ours.

      \item \incite[def. 1.1.3]{Knauer2019AlgebraicGraphTheory} for closed nonempty directed walks.
    \end{itemize}
  \end{thmenum}
\end{remark}
...
\end{definition}
\begin{comments}
  \item Every acyclic graph is a union of \hyperref[def:tree]{trees}, for which reason acyclic graphs are also called \enquote{forests}. This usage is mentioned by \incite[13]{Diestel2017GraphTheory}, \incite[32]{Harary1969GraphTheory}, \incite[21]{Bollobás1998ModernGraphTheory}, \incite[def. 1.3.5.]{Knauer2019AlgebraicGraphTheory}, \incite[782]{Rosen2019DiscreteMathematics}, \incite[191]{Erickson2019Algorithms}, \incite[53]{ЕмеличевИПр1990ТеорияГрафов} (as \enquote{\textru{лес}}), \incite[\S 9.1.1]{Новиков2013ДискретнаяМатематика} (as \enquote{\textru{лес}}) and \incite[21]{Мирчев2001Графи} (as \enquote{\textbg{гора}}).
