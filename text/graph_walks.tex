...
\begin{remark}\label{rem:graph_walk_terminology}
  ...
  \begin{thmenum}
    ...
    \begin{itemize}
      \item \incite[22]{ЕмеличевИПр1990ТеорияГрафов} (as \enquote{цепь}), \incite[128]{Зыков2004ТеорияГрафов} (as \enquote{цепь}) and \incite[\S 7.2.3]{Новиков2013ДискретнаяМатематика} (as \enquote{цепь}) for walks with no repeated arcs. In case of no repeated vertices, both use \enquote{simple chain} (\enquote{простая цепь}).

      \item \incite[12]{GondranMinoux1984GraphsAndAlgorithms} and \incite[10]{Мирчев2001Графи} as (as \enquote{верига}) for a sequence of arcs where consecutive arcs have an endpoint in common. This corresponds to what we call finite nonempty generalized walks.
...
\begin{remark}\label{rem:graph_cycle_terminology}
  ...
  \begin{thmenum}
    ...
    \begin{itemize}
      ...
      \item \incite[def. 1.1.3]{Knauer2019AlgebraicGraphTheory} and \incite[279]{БелоусовТкачёв2004ДискретнаяМатематика} (as \enquote{контур}) for nonempty closed walks where only the endpoints may repeat, but only for simple directed graphs.
    ...
    \begin{itemize}
      \item \incite[279]{БелоусовТкачёв2004ДискретнаяМатематика} (as \enquote{цикл}) for \cref{def:graph_cycle/direct}.

      \item \incite[22]{ЕмеличевИПр1990ТеорияГрафов} (as \enquote{цикл}) and \incite[\S 7.2.3]{Новиков2013ДискретнаяМатематика} (as \enquote{цикл}) for closed walks with no repeated arcs, which generalizes our usage.
...
\begin{comments}
  \item This algorithm is implemented in \cite{notebook:code} as
    \WriteCodeRef{alg:cycle_removal}
\end{comments}
...
\end{definition}
\begin{comments}
  \item Every acyclic graph is a union of \hyperref[def:tree]{trees}, for which reason acyclic graphs are also called \enquote{forests}. This usage is mentioned by \incite[13]{Diestel2017GraphTheory}, \incite[32]{Harary1969GraphTheory}, \incite[21]{Bollobás1998ModernGraphTheory}, \incite[def. 1.3.5.]{Knauer2019AlgebraicGraphTheory}, \incite[782]{Rosen2019DiscreteMathematics}, \incite[191]{Erickson2019Algorithms}, \incite[53]{ЕмеличевИПр1990ТеорияГрафов} (as \enquote{лес}), \incite[\S 9.1.1]{Новиков2013ДискретнаяМатематика} (as \enquote{лес}) and \incite[21]{Мирчев2001Графи} (as \enquote{гора}).
