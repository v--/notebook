...
\begin{definition}\label{def:graph_walk}\mimprovised
  ...
  \begin{figure}[!ht]
    \begin{equation}\label{eq:fig:def:graph_walk}
      ...
      \end{aligned}
    \end{equation}
    \caption{A \hyperref[def:graph_walk/path]{path}'s \hyperref[def:graph_walk/subgraph]{induced graph} in an \hyperref[def:undirected_multigraph]{undirected multigraph}.}\label{fig:def:graph_walk}
  \end{figure}
...
\begin{proposition}\label{thm:acyclic_undirected_graph_orientation}
  Any \hyperref[def:multigraph_orientation]{orientation} of an \hyperref[def:acyclic_graph]{acyclic} \hyperref[def:undirected_multigraph]{undirected multigraph} is also \hyperref[def:acyclic_graph]{acyclic}.
\end{proposition}
\begin{comments}
...
\end{definition}
\begin{comments}
  \item \incite[425]{БелоусовТкачёв2004} use the term \enquote{bicomponent} (\enquote{бикомпонента}) for strongly connected components. This is unrelated to \hyperref[def:multipartite_graph]{bipartite graphs}, which \incite[\S 7.3.3]{Новиков2013} calls \enquote{bigraph} (\enquote{биграф}). \incite[\S 8.5.2]{Новиков2013} prefers the term \enquote{strongly connected component} (\enquote{компоненты сильной связи}) to \enquote{bicomponent}.
