...
\begin{defproof}
  ...
  \ImplicationSubProof{def:acyclic_graph/closed_walks}{def:acyclic_graph/walk} Suppose that \( G \) has no nontrivial closed walks.
...
\begin{proof}
  ...
  \NecessitySubProof Conversely, suppose that the acyclic graph \( G \) has order \( n \). In a walk of length \( m \geq n \), there are \( m + 1 \) vertices, hence, by \fullref{thm:pigeonhole_principle}, at least one vertex must repeat. As per \fullref{def:acyclic_graph/walks}, every walk in \( G \) is a path, hence no walk can have repeated vertices.
