...
\begin{definition}\label{def:graph_walk}\mimprovised
  ...
  In a \hyperref[rem:graph_of_unspecified_type]{(hyper)graph of unspecified type}, consider a (finite or infinite) nonempty alternating sequence
  ...
  \begin{thmenum}[series=def:graph_walk]
    ...
    Since walks in undirected graphs and hypergraphs are directed in nature, we use the notations for directed paths and, when speaking about graph of unspecified types, refer to them as \enquote{directed walks} (e.g. in \cref{def:graph_cycle}).
  ...
  \begin{figure}
    \begin{equation}\label{eq:fig:def:graph_walk}
      ...
      \end{aligned}
    \end{equation}
    \caption{A \hyperref[def:graph_walk/path]{path}'s \hyperref[def:graph_walk/subobject]{induced subgraph} in an \hyperref[def:hypergraph/multigraph]{undirected multigraph}.}\label{fig:def:graph_walk}
  \end{figure}

  We introduce the following terminology:
  \begin{thmenum}[resume=def:graph_walk]
    \thmitem{def:graph_walk/subobject} Each walk induces a \hyperref[def:directed_multigraph/subobject]{subgraph} consisting of only those vertices and edges in the walk.

    \thmitem{def:graph_walk/length} We define the \term{length} of a walk as the number of arcs or edges in case of finite walks and the \hyperref[def:extended_real_numbers]{extended real number} \( \infty \) otherwise.
    ...
    \thmitem{def:graph_walk/path}\mcite[191]{Erickson2019Algorithms} We say that the walk is a \term[bg=път (\cite[11]{Мирчев2001Графи}), ru=путь (\cite[278]{БелоусовТкачёв2004ДискретнаяМатематика})]{path} if it has no repeated vertices\fnote{Having no repeated vertices also implies having no repeated edges, but not vice versa.}.

    Unless the path is closed, it can be fully recovered from its induced subgraph, for which reason we can conflate the two.

    \thmitem{def:graph_walk/closed}\mcite[def. 1.1.3]{Knauer2019AlgebraicGraphTheory} We say that a finite walk is \term{closed} if the two endpoints coincide.
...
\end{definition}
\begin{comments}
  \item It may seem tempting to define walks as sequences of arcs (without explicitly mentioning any vertices). This does, however, introduce an additional difficulty --- we can no longer distinguish different empty paths. An alternative is to define walks as a starting vertex followed by a sequence of arcs or edges, but the definition given above has the advantage of being more widely accepted --- see \cref{rem:graph_walk_terminology/walk}.

  Furthermore, hypergraph walks may be ambiguous if the vertices are not explicitly specified.

  \item An alternative definition of the length of infinite walks is discussed in \cref{ex:def:extended_real_numbers/walk_length}.
\end{comments}
...
\begin{proposition}\label{thm:def:graph_walk}
  ...
  \begin{thmenum}
    \thmitem{thm:def:graph_walk/finite_graph_infinite_walk} In a \hyperref[def:graph_cardinality/order]{finite-order graph}, for every infinite \hyperref[def:graph_walk]{walk}, there exists a vertices through which the walk passes infinitely many times.
...
\begin{proof}
  ...
  \SubProofOf{thm:def:graph_walk/finite_graph_finite_path} Follows from \cref{thm:def:graph_walk/finite_graph_finite_path}.
\end{proof}
...
\begin{remark}\label{rem:graph_walk_terminology}
  ...
  \begin{thmenum}
    \thmitem{rem:graph_walk_terminology/walk} \enquote{Walk} (\enquote{\textru{маршрут}} in both Russian and Bulgarian) is used as follows:
    \begin{itemize}
      \item For undirected graphs, a definition of \enquote{walk} similar to ours is given in
      \cite[21]{Diestel2017GraphTheory},
      \cite[13]{Harary1969GraphTheory},
      \cite[4]{Bollobás1998ModernGraphTheory}
      \cite[571]{Stanley2012EnumerativeCombinatoricsVol1},
      \cite[22]{ЕмеличевИПр1990ТеорияГрафов},
      \cite[128]{Зыков2004ТеорияГрафов} and
      \cite[\S 7.2.3]{Новиков2013ДискретнаяМатематика}

      We generalize this definition to directed graphs and optionally allow walks to be infinite.

      \item \Textcite[60]{König1990GraphTheory} defines a \enquote{walk} (originally \enquote{\textde{Kantenfolge}}, literally \enquote{edge sequence}) as a finite undirected graph whose arcs or edges can be \hyperref[def:set_enumeration]{enumerated}, with repetition allowed, so that consecutive arcs/edges in the sequence have a common vertex.

      This definition resembles our notion of induced graph of a walk.

      \item \Textcite[191]{Erickson2019Algorithms} defines a walk as a sequence of adjacent vertices. This coincides with our usage in the case of simple undirected graphs.

      \item \Textcite[12]{Мирчев2001Графи} defines a walk as a sequence of (undirected) edges where consecutive arcs have an endpoint in common. This coincides with our usage in the case of nonempty finite walks in undirected graphs.
    \end{itemize}

    \thmitem{rem:graph_walk_terminology/path} \enquote{Path} (\enquote{\textru{путь}} in Russian and \enquote{\textru{път}} in Bulgarian) is used as follows:
    \begin{itemize}
      \item \Textcite[13]{Harary1969GraphTheory} and \textcite[571]{Stanley2012EnumerativeCombinatoricsVol1} define a \enquote{path} as a walk with distinct vertices. We use this definition (although our notions of walks generalizes theirs).

      \Textcite[191]{Erickson2019Algorithms} also uses this definition, however his notion of walk is different --- see above.

      \item \Textcite[363]{Knuth1997ArtVol1} defines a \enquote{path} as a sequence of adjacent vertices in an undirected graph. This corresponds to our notion of a walk.

      Later, in \cite[373]{Knuth1997ArtVol1}, he defines an \enquote{oriented path} in a similar way, but adjusted for directed graphs.

      \item \Textcite[def. 10.4.1]{Rosen2019DiscreteMathematics} defines a \enquote{path} as a sequences of (undirected) edges for which there exist sequences of vertices that jointly constitute what we call a walk.

      \item \Textcite[6]{Diestel2017GraphTheory} and \textcite[4]{Bollobás1998ModernGraphTheory} define a \enquote{path} as a subgraph with an implicit ordering of the vertices and one arc for every pair of consecutive vertices. This also roughly coincides with our notion of induced graphs of directed walks.

      \item \Textcite[def. 1.1.3]{Knauer2019AlgebraicGraphTheory}, \textcite[13]{GondranMinoux1984GraphsAndAlgorithms} and \textcite[11]{Мирчев2001Графи} define a \enquote{path} as a sequence of arcs where the tail of each arc is the head of the next. This corresponds to what we call finite nonempty directed walks.

      \item \Textcite[278]{БелоусовТкачёв2004ДискретнаяМатематика} define a \enquote{path} as a sequence of pairwise adjacent vertices in a simple directed graph. This corresponds to what we call finite paths.

      \item As a specialization, a \enquote{simple path} is used for walks with no repeated vertices in
      \cite[def. 1.1.3]{Knauer2019AlgebraicGraphTheory},
      \cite[279]{БелоусовТкачёв2004ДискретнаяМатематика} (as \enquote{\textru{простой путь}})
      \cite[11]{Мирчев2001Графи} (as \enquote{\textbg{прост път}}).

      This corresponds to our notion of paths.

      \Textcite[32]{GondranMinoux1984GraphsAndAlgorithms} use \enquote{elementary path} instead. \Textcite[279]{БелоусовТкачёв2004ДискретнаяМатематика} allow the endpoints to coincide.

      \item As a generalization, \textcite[def. 1.1.3]{Knauer2019AlgebraicGraphTheory} define \enquote{semipaths} as what we would call nonempty finite generalized walks.
    \end{itemize}

    \thmitem{rem:graph_walk_terminology/trail} \enquote{Trail} is used as follows:
    \begin{itemize}
      \item A \enquote{trail} is defined as a walk with no repeated edges in
      \cite[13]{Harary1969GraphTheory},
      \cite[571]{Stanley2012EnumerativeCombinatoricsVol1} and
      \cite[5]{Bollobás1998ModernGraphTheory}.
    \end{itemize}

    \thmitem{rem:graph_walk_terminology/chain} \enquote{Chain} is used as follows:
    \begin{itemize}
      \item The Russian \enquote{\textru{цепь}} is used for a walk with no repeated edges in
      \cite[128]{Зыков2004ТеорияГрафов},
      \cite[22]{ЕмеличевИПр1990ТеорияГрафов} and
      \cite[\S 7.2.3]{Новиков2013ДискретнаяМатематика}.

      In case of no repeated vertices, they use \enquote{\textru{простая цепь}} (\enquote{simple chain}).

      \item \Textcite[278]{БелоусовТкачёв2004ДискретнаяМатематика} uses the same term for a possibly infinite sequence of pairwise adjacent vertices in a simple undirected graph.

      This corresponds to what we call a path.

      \item A \enquote{chain} is defined as a sequence of adjacent edges in
      \cite[12]{GondranMinoux1984GraphsAndAlgorithms} and
      \cite[10]{Мирчев2001Графи} (as \enquote{\textbg{верига}}).

      This corresponds to what we call a finite nonempty generalized walk.
    \end{itemize}
  \end{thmenum}
\end{remark}
...
\end{definition}
\begin{comments}
  \item We will see in \cref{thm:free_category_universal_property} that \( F \) is actually left adjoint to the corresponding forgetful functor. The new identity loops are an important part of this adjunction.
\end{comments}

\begin{theorem}[Free category universal property]\label{thm:free_category_universal_property}
  Denote by \( U \) the \hyperref[def:concrete_category]{forgetful functor} which sends a \hyperref[def:category]{category} to its underlying \hyperref[def:directed_multigraph]{directed multigraph}.

  The \hyperref[def:directed_multigraph_free_category]{free category} \( F(G) \) is the unique up to an isomorphism category that satisfies the following \hyperref[rem:universal_mapping_property]{universal mapping property}:
  \begin{displayquote}
    ...
    \begin{equation}\label{eq:thm:free_category_universal_property/diagram}
      \begin{aligned}
...
\end{comments}
\begin{proof}
  See \cref{ex:def:category_adjunction/dm_cat}.
\end{proof}

\paragraph{Cycles}

\begin{definition}\label{def:graph_cycle}
  We say that the \hyperref[def:graph_walk/closed]{closed walk}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:graph_cycle/degree}\mcite[13]{GondranMinoux1984GraphsAndAlgorithms} In the \hyperref[def:graph_walk/subobject]{induced subgraph} of the walk, every vertex has \hyperref[def:graph_cardinality/directed_degree]{degree} \( 2 \).
  \end{thmenum}
\end{definition}
\begin{defproof}
...
\begin{remark}\label{rem:graph_cycle_terminology}
  ...
  \begin{thmenum}
    \thmitem{rem:graph_cycle_terminology/circuit} The term \enquote{circuit} (\enquote{\textru{контур}} in both Russian and Bulgarian) is used as follows:
    \begin{itemize}
      \item \Textcite[5]{Bollobás1998ModernGraphTheory} defines a \enquote{circuit} as a closed walk with no repeated arcs.

      \item \Textcite[13]{GondranMinoux1984GraphsAndAlgorithms} define a \enquote{circuit} as a closed nonempty walk.

      For undirected graphs, this definition is used in \cite[def. 1.1.3]{Knauer2019AlgebraicGraphTheory} and \cite[279]{БелоусовТкачёв2004ДискретнаяМатематика}.

      \item \Textcite[def. 10.4.1]{Rosen2019DiscreteMathematics} defines a \enquote{circuit} in undirected (multi)graphs as a closed walk.

      \item \Textcite[11]{Мирчев2001Графи} defines a \enquote{\textbg{контур}} as a closed nonempty generalized walk.
    \end{itemize}

    \thmitem{rem:graph_cycle_terminology/cycle} \enquote{Cycle} (\enquote{\textru{цикл}} in Russian and \enquote{\textbg{цикъл}} in Bulgarian) is used as follows:
    \begin{itemize}
      \item \Textcite[279]{БелоусовТкачёв2004ДискретнаяМатематика} state \cref{def:graph_cycle/direct}.

      \item \Textcite[22]{ЕмеличевИПр1990ТеорияГрафов} and \textcite[\S 7.2.3]{Новиков2013ДискретнаяМатематика} define a \enquote{\textru{цикл}} for closed walks with no repeated arcs, which generalizes our usage.

      \item \Textcite[11]{Мирчев2001Графи} defines a \enquote{\textbg{цикъл}} as a closed directed walk, which generalizes our usage in the case of directed graphs.

      \Textcite[def. 1.1.3]{Knauer2019AlgebraicGraphTheory} give a similar definition, but explicitly requires cycles to be nonempty.

      \item \Textcite[example 10.2.6]{Rosen2019DiscreteMathematics} defines a \enquote{cycle} as an (undirected) \hyperref[def:cycle_graph]{cycle graph}.

      Later, in \cite[def. 10.4.2]{Rosen2019DiscreteMathematics}, he uses the same term for closed directed walks.

      \item \Textcite[191]{Erickson2019Algorithms} defines a \enquote{cycle} as a closed walk that \enquote{enters and leaves each vertex at most once}. This coincides with our usage.

      \item \Textcite[13]{Harary1969GraphTheory}, \textcite[8]{Diestel2017GraphTheory}, \textcite[5]{Bollobás1998ModernGraphTheory} and \textcite[377]{Knuth1997ArtVol1} define a \enquote{cycle} as a graph constructed from a path with its head connecting to its tail, with at least three vertices. This coincides with our usage for simple directed graphs.

      \item \Textcite[13]{GondranMinoux1984GraphsAndAlgorithms} define a \enquote{cycle} as a closed nonempty generalized walk where consecutive arcs are distinct. The condition for consecutive arcs prevents the pathological case \cref{ex:def:graph_cycle/pathological}, thus their definition generalizes ours.
    \end{itemize}
  \end{thmenum}
\end{remark}
...
\begin{example}\label{ex:def:graph_cycle}
  ...
  \begin{thmenum}
    \thmitem{ex:def:graph_cycle/trivial} We require cycles to be nontrivial, hence \hyperref[def:graph_walk/empty]{empty walk} at a vertex is not a cycle. Without this restriction, no graph can be \hyperref[def:acyclic_graph]{acyclic}.

    Trivial graphs are paths, nevertheless. This also turns out useful --- see \fullref{alg:cycle_removal}.

    \thmitem{ex:def:graph_cycle/pathological} The additional requirement in \cref{def:graph_cycle/simplified} rules out the pathological cycle \( a \reloset e \to b \reloset e \to a \), which would prevent an undirected graph from possibly being \hyperref[def:acyclic_graph]{acyclic}.

    As it turns out, this is the only case preventing a simpler definition of cycles.

    \thmitem{ex:def:graph_cycle/loop} By definition, every loop is a cycle.
...
\begin{algorithm}[Closed walk to cycle]\label{alg:closed_walk_to_cycle}
  Fix a \hyperref[def:hypergraph/multigraph]{directed multigraph} and consider the \emph{nontrivial} \hyperref[def:graph_walk/closed]{closed} walk
...
\end{algorithm}
\begin{comments}
  \item This algorithm is a special case of \fullref{alg:cycle_removal}, implemented in \cite{notebook:code} as
  \begin{CodeRefDisplay}
    \GetCodeRef{alg:cycle_removal}
  \end{CodeRefDisplay}
\end{comments}
...
\end{definition}
\begin{comments}
  \item Every acyclic graph is a union of \hyperref[def:tree]{trees}, for which reason acyclic graphs are also called \enquote{forests}. This usage is mentioned in
  \cite[13]{Diestel2017GraphTheory},
  \cite[32]{Harary1969GraphTheory},
  \cite[21]{Bollobás1998ModernGraphTheory},
  \cite[def. 1.3.5.]{Knauer2019AlgebraicGraphTheory},
  \cite[782]{Rosen2019DiscreteMathematics},
  \cite[191]{Erickson2019Algorithms},
  \cite[53]{ЕмеличевИПр1990ТеорияГрафов} (as \enquote{\textru{лес}}),
  \cite[\S 9.1.1]{Новиков2013ДискретнаяМатематика} (as \enquote{\textru{лес}}) and
  \cite[21]{Мирчев2001Графи} (as \enquote{\textbg{гора}}).
...
\begin{proposition}\label{thm:acyclic_graph_walk_length}
  ...
  Conversely, if a graph of \emph{finite} order \( n \) is \hyperref[def:acyclic_graph]{acyclic}, it has no \hyperref[def:graph_walk]{walk} of length \( n \) or more.
\end{proposition}
\begin{proof}
...
\begin{proposition}\label{thm:acyclic_directed_multigraph}
  An \hyperref[def:acyclic_graph]{acyclic} \hyperref[def:directed_multigraph]{directed multigraph} has no \hyperref[def:directed_multigraph/loop]{loops} or \hyperref[def:directed_multigraph/parallel_arcs]{oppositely directed arcs} (but may have \hyperref[def:directed_multigraph/parallel_arcs]{parallel arcs}).
\end{proposition}
\begin{proof}
...
\begin{definition}\label{def:well_founded_graph}\mimprovised
  We say that a \hyperref[def:directed_multigraph]{directed multigraph} is \term{well-founded} if there exists no walk of the form
  \begin{equation}
...
\begin{proposition}\label{thm:def:well_founded_graph}
  ...
  \begin{thmenum}
    \thmitem{thm:def:well_founded_graph/simple} A simple directed graph is well-founded in the sense of \cref{def:well_founded_graph} if and only if it is well-founded as a \hyperref[def:binary_relation]{binary relation} in the sense of \cref{def:well_founded_relation}.
...
\begin{proof}
  ...
  \SubProofOf{thm:def:well_founded_graph/set} The \hyperref[def:zfc/foundation]{axiom of foundation} via \cref{thm:set_membership_is_well_founded} implies that the relation \( {\in} \) is \hyperref[def:well_founded_relation]{well-founded}; hence, for any set \( A \), the graph \( (A, {\in}) \) is well-founded
...
\begin{definition}\label{def:graph_connectedness}\mcite[def. 1.2.1]{Knauer2019AlgebraicGraphTheory}
  \hyperref[rem:graph_of_unspecified_type]{Graphs} have the following notions of connectedness:
  \begin{thmenum}
...
\begin{definition}\label{def:graph_connected_component}\mcite[def. 1.4.11]{Knauer2019AlgebraicGraphTheory}
...
\end{definition}
\begin{comments}
  \item \Textcite[425]{БелоусовТкачёв2004ДискретнаяМатематика} and \textcite[425]{Зыков2004ТеорияГрафов} use the term \enquote{bicomponent} (\enquote{бикомпонента}) for strongly connected components. This is unrelated to \hyperref[def:multipartite_graph]{bipartite graphs}, which \textcite[\S 7.3.3]{Новиков2013ДискретнаяМатематика} calls \enquote{bigraph} (\enquote{биграф}).

  In \cite[\S 8.5.2]{Новиков2013ДискретнаяМатематика}, Novikov prefers the term \enquote{strongly connected component} (\enquote{компоненты сильной связи}) to \enquote{bicomponent}.

  \textcite[27]{Harary1969GraphTheory} also uses the term \enquote{bigraph} for bipartite graphs, but avoids using \enquote{bicomponent}.
\end{comments}

\begin{proposition}\label{thm:connected_components_partition_graph}
  The (weak/strong) \hyperref[def:graph_connected_component]{connected components} of a \hyperref[rem:graph_of_unspecified_type]{graph} \hyperref[def:set_partition]{partition} its vertices.
\end{proposition}
\begin{proof}
  Trivial.
\end{proof}
...
\begin{definition}\label{def:graph_bridge}
  ...
  \begin{thmenum}
    \thmitem{def:graph_bridge/components}\mcite[26]{Harary1969GraphTheory} After removing it from its \hyperref[def:graph_connected_component]{connected component}, it is no longer connected.
...
\begin{defproof}
  ...
  \ImplicationSubProof{def:graph_bridge/components}{def:graph_bridge/cycle} Suppose that \( G' \) is not connected. If \( e \) is contained in some cycle, by \cref{thm:connected_graph_cycle_removal}, \( G' \) should be connected, hence \( e \) is not contained in a cycle.
...
\begin{definition}\label{def:directed_graph_condensation}\mcite[example 1.6.6]{Knauer2019AlgebraicGraphTheory}
...
\end{definition}
\begin{comments}
  \item Another term for condensations from Russian origin is \enquote{Herz's graph} (\enquote{граф Герца}). It is used by \textcite[\S 8.5.2]{Новиков2013ДискретнаяМатематика}.
\end{comments}
\begin{defproof}
...
\begin{definition}\label{def:eulerian_walk}\mcite[65]{Harary1969GraphTheory}
...
\begin{theorem}[Euler's theorem for graphs]\label{thm:eulers_theorem_for_graphs}\mcite[\S 20]{Euler1741Bridges}
  ...
  There exists an \hyperref[def:eulerian_walk]{Eulerian walk} from \( a \) to \( b \) if and only if all other vertices have even degree.

  Furthermore, either \( a = b \) coincide and they have an even common degree, or \( a \neq b \) and they both have odd degrees.
\end{theorem}
\begin{proof}
  ...
  \NecessitySubProof We will prove by induction on the number of edges that if all vertices in a graph except possibly \( a \) and \( b \) have even degree, then there exists an Eulerian walk from \( a \) to \( b \), and that \( a = b \) if and only if \( a \) and \( b \) have even degrees.
  ...
  \SubProof*{Proof of inductive step without bridges} Suppose that the statement holds for all graphs with \( n \) edges, and let \( G \) be a graph with exactly \( n + 1 \) edges. Fix two vertices \( a \) and \( b \) and suppose that all others have even degree.

  For this sub-case, suppose that every edge incident to \( b \) is a \hyperref[def:graph_bridge]{bridge}.

  Let \( w \) be an arbitrary vertex adjacent to \( b \) and let \( e \) be an edge between them. We will first show that this choice is unique, that is, that \( b \) is incident to only one edge.

  After removing \( e \), \( G \) splits into two connected components. Let \( G' \) be the component containing \( w \) and \( G^\dprime \) --- the component containing \( b \). We will show that \( G^\dprime \) has no edges and no vertices except for \( b \).

  Denote by \( \deg'(v) \) the degree function in \( G' \) and by \( \deg^\dprime(v) \) --- in \( G^\dprime \). Since \( \deg(w) \) is even, necessarily \( \deg'(w) \) is odd. Since \( \deg(v) = \deg'(v) \) for all vertices in \( G' \) except \( w \), we conclude that \( \deg'(v) \) even except possibly for \( a \), in case \( a \) is in \( G' \) and is distinct from \( v_k \). \Cref{thm:odd_degree_vertices} implies that \( G' \) must have at least two vertices of odd degree, hence \( a \) must belong to \( G' \), be distinct from \( v_k \) and have odd degree.

  Then \( a \) also has odd degree in \( G \), and so does \( b \).

  Therefore, every vertex in \( G^\dprime \) has even degree, and we can apply the inductive hypothesis to \( G^\dprime \) and obtain a closed Eulerian walk starting and ending at \( b \). This is also a closed walk in \( G \).

  Aiming at a contradiction, suppose that this walk is nontrivial. Then \fullref{alg:closed_walk_to_cycle} allows us to obtain a cycle at \( b \), which contains at least two non-bridge edges incident to \( b \). This contradicts our assumption that all edges incident to \( b \) are bridges.
  ...
  Therefore, \( e \) is the unique edge incident to \( b \), and thus \( G' \) is the graph
  \begin{equation*}
    (V \setminus \set{ b }, E \setminus \set{ e }, \mscrE).
  \end{equation*}

  We have already shown that \( a \) and \( w \) must have odd degrees in \( G' \), while all other vertices must have even degrees. Therefore, we can also apply the inductive hypothesis to \( G' \) to obtain an Eulerian walk from \( a \) to \( w \), and append \( e \) to obtain an Eulerian walk in \( G \).

  Furthermore, we have also shown that \( a \) and \( b \) and distinct and have odd degrees in \( G \).

  \SubProof*{Proof of inductive step with bridges} Again, suppose that the statement holds for all graphs with \( n \) edges, and let \( G \) be a graph with exactly \( n + 1 \) edges. Fix two vertices \( a \) and \( b \) and suppose that all others have even degree.

  It remains to consider the sub-case where there exists a vertex \( w \) and an edge \( e \) between \( w \) and \( b \) that is not a bridge. Define the graph
  \begin{equation*}
  ...
  \begin{equation}\label{eq:thm:eulers_theorem_for_graphs/proof/necessity/deg}
    \deg'(v) = \begin{cases}
      \deg(v) - 2, &(v = w \T{or} v = b) \T{and} e \T{is a loop}, \\
      \deg(v) - 1, &(v = w \T{or} v = b) \T{and} e \T{is not a loop}, \\
      \deg(v),     &\T{otherwise.}
    \end{cases}
  \end{equation}

  Then \( \deg'(v) \) is necessarily even unless \( v \) is \( a \), \( v_k \), or \( v_{k+1} = b \). \Cref{thm:odd_degree_vertices} implies that all three cannot simultaneously have odd degrees in \( G' \) --- either all vertices in \( G' \) can have even degrees, or all except for two.

  We can now apply the inductive hypothesis to \( G' \) to obtain an Eulerian walk in \( G' \) from \( a \) to \( w \) and append \( e \) to obtain an Eulerian walk in \( G \) from \( a \) to \( b \).

  It remains to show that \( a = b \) if and only if \( \deg(a) \) and \( \deg(b) \) are even.

  If \( a = b \), by \cref{thm:odd_degree_vertices}, \( \deg(a) = \deg(b) \) must be even. Conversely, suppose that \( \deg(a) \) and \( \deg(b) \) are even.
  \begin{itemize}
    \item If \( e \) is a loop, then, by \eqref{eq:thm:eulers_theorem_for_graphs/proof/necessity/deg}, \( \deg'(b) = \deg(b) - 2 \) is even. The degree \( \deg'(a) \) is also even --- either because \( a = b \), or otherwise because \( \deg'(a) = \deg(a) \).

    Then, since both \( \deg'(w) \) and \( \deg'(a) \) are even, the inductive hypothesis implies that \( a = w \), hence also \( a = b \).

    \item If \( e \) is not a loop, \( w \) and \( b \) are distinct, and, by \eqref{eq:thm:eulers_theorem_for_graphs/proof/necessity/deg}, both \( \deg'(w) \) and \( \deg'(b) \) are odd.

    If \( a \neq b \), then a vertex other than \( a \) or \( w \) has odd degree in \( G' \), which contradicts the inductive hypothesis. Therefore, \( a = b \).
  \end{itemize}
\end{proof}
