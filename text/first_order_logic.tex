...
\begin{definition}\label{def:fol_reduct_along_morphism}\mcite[example 5]{MossakowskiKrumnackMaibaum2015DerivedSignatureMorphisms}
  ...
  When defining institutions in \cref{def:fol_institution/models/hom}, we will need to extend \( \red_t \) to a \hyperref[def:functor]{functor} by also reducing \hyperref[def:fol_homomorphism]{homomorphisms}. Given structures \( \mscrY = (Y, J) \) and \( \mscrZ = (Z, K) \) over \( \Theta \), for every \( g: Y \to Z \), we define \( \red_t(g) \) as \( g \) itself.
\end{definition}
\begin{comments}
