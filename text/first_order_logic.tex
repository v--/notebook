...
\begin{definition}\label{def:predicate_logic_alphabet}\mimprovised
  ...
  \begin{thmenum}
    ...
    \thmitem{def:predicate_logic_alphabet/description} Optionally, the \hyperref[con:description_operator/iota]{definite description} symbol \enquote{\( \syndesc \)}.
...
\begin{algorithm}[First-order formula denotation]\label{alg:fol_formula_denotation}\mimprovised
  ...
  \begin{itemize}
    ...
    \item The cases \eqref{eq:alg:fol_formula_denotation/forall} and \eqref{eq:alg:fol_formula_denotation/exists} evaluates \( \psi \) using a \hyperref[def:hol_variable_assignment/modified]{modified assignment} parameterized by the elements of \( X_\sigma \). We see here why it is important that the \hyperref[def:truth_value_algebra/classical]{classical truth value algebra} is \hyperref[def:heyting_algebra/complete]{complete}.
  \end{itemize}
...
\begin{definition}\label{def:fol_semantics}\mimprovised
  ...
  \begin{thmenum}
    ...
    Here \( \varphi \) and the formulas of \( \Gamma \) are closed.
