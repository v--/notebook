...
\begin{concept}\label{con:quantifier}
  \hyperref[con:variable_binding]{Variable binders} in first-order logic are called \term{quantifiers}.
\end{concept}
\begin{comments}
  \item There are two quantifiers, defined in \fullref{def:first_order_language/quantifiers} --- a universal quantifier, \( \forall \), encoding the words \enquote{for each}, and an existential quantifier, \( \exists \), encoding \enquote{there exists}.
