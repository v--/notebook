...
\begin{definition}\label{def:predicate_logic_alphabet}\mimprovised
  ...
  \begin{thmenum}
    ...
    \begin{thmenum}
      \thmitem{def:predicate_logic_alphabet/quantifiers/universal} The \term[ru=квантор общости (\cite[61]{Эдельман1975Логика}); квантор всеобщности (\cite[140]{Мальцев1970АлгебраическиеСистемы})]{universal quantifier} \enquote{\( \synforall \)}.
...
\begin{definition}\label{def:fol_signature}\mcite[282]{Meseguer1989GeneralLogics}
  ...
  \begin{thmenum}
    \thmitem{def:fol_signature/fun} A possibly empty \hyperref[def:formal_language/alphabet]{alphabet} \( \op*{Fun}_\Sigma \), whose elements we call \term[ru=функциональные символы (\cite[73]{ШеньВерещагин2017ЯзыкиИИсчисления})]{function symbols}.

    \thmitem{def:fol_signature/pred} A possibly empty alphabet \( \op*{Pred}_\Sigma \), whose elements we call \term[ru=реляционные символы (\cite[73]{ШеньВерещагин2017ЯзыкиИИсчисления})]{relation symbols}.

    \thmitem{def:fol_signature/arity}\mcite[38]{CitkinMuravitsky2022ConsequenceRelations} A function \( {\#}: \op*{Fun}_\Sigma \cup \op*{Pred}_\Sigma \to \BbbZ_{\geq 0} \) determining the corresponding symbol's \term[ru=арность (\cite[141]{Мальцев1970АлгебраическиеСистемы}); валентность (\cite[73]{ШеньВерещагин2017ЯзыкиИИсчисления})]{arity}.
    ...
    \thmitem{def:fol_signature/infix}\mimprovised A subset \( \op*{Infix}_\Sigma \subseteq \op*{Fun}_\Sigma \cup \op*{Pred}_\Sigma \) of those binary operations we want to use \hyperref[def:function_application_syntax]{infix notation} for.
  \end{thmenum}
...
\begin{definition}\label{def:fol_signature_category}\mcite[282]{Meseguer1989GeneralLogics}
  ...
  \begin{thmenum}
    ...
    We allow changing the notation of a symbol, i.e. sending a prefix symbol to an infix symbol and vice versa.
...
\begin{definition}\label{def:fol_term}\mimprovised
  For a fixed \hyperref[def:fol_signature]{first-order signature} \( \Sigma \), we reuse the function application grammar from \cref{def:function_application_syntax} to formally introduce \hyperref[con:predicate_logic/predicate]{terms}:
  \begin{bnf*}
    \bnfprod{function application} {\underbrace{f \bnfsp \bnftsq{(} \bnfsp \overbrace{\bnfpn{term} \bnfsp \bnftsq{,} \bnfsp \bnfsk \bnfsp \bnftsq{,} \bnfsp \bnfpn{term}}^{\# f \T*{terms separated by commas}} \bnfsp \bnftsq{)}}_{\T*{One rule for each} f \in \op*{Fun}_\Sigma \setminus \op*{Infix}_\Sigma}} \\
    \bnfprod{function application} {\underbrace{\bnftsq{(} \bnfsp \bnfpn{term} \anon \bnfpn{term} \bnfsp \bnftsq{)}}_{\mathclap{\T*{One rule for each} \anon \in \op*{Fun}_\Sigma \cap \op*{Infix}_\Sigma}}} \\
  ...
  \begin{itemize}
    ...
    \item The function application rules are discussed in \cref{rem:fol_application_infix_notation}.
  \end{itemize}
...
\begin{remark}\label{rem:fol_application_infix_notation}
  We have defined first-order terms in \cref{def:fol_term} so that function application differs syntactically between prefix and infix function symbols. We will do the same in \cref{def:fol_formula} for predicate application. Supporting different notations greatly increases the readability of some symbolic expressions in \cite{notebook:code}.

  Examples of infix function and predicate symbols abound --- all binary operations from \fullref{ch:group_theory}, \fullref{ch:ring_theory} and \fullref{ch:order_theory}, as well as the order relations from the latter. Other examples include the binary Peano arithmetic operations in \fullref{sec:natural_numbers} and the set membership predicate and most defined operations in \fullref{ch:set_theory}.

  Unlike the metalingual \enquote{abuse-of-notation} conventions like those in \cref{rem:fol_formula_conventions}, support for infix notation does not require sacrificing syntactic unambiguity or useful properties like the subterm characterization in \cref{thm:lambda_subterm_characterization}. Furthermore, since both binary connective formulas and equality formulas use infix notation, adding support for other infix symbols is not difficult.

  One thing should be noted, however. Since we mostly deal with \hyperref[con:abstract_syntax_tree]{abstract syntax trees}, the actual notation is not relevant outside the object language. We want \fullref{alg:fol_term_signature_translation} to produce an isomorphic syntax tree, but we have allowed signature morphisms to exchange prefix and infix symbols.

  Correspondingly, when only the abstract structure of a term (or formula) is concerned, we assume that a metalingual schema for function application like \( f(x, y) \) encompasses both prefix and infix symbols. Otherwise, we would have to add meaningless cases when using \hyperref[con:evaluation]{pattern matching}. For instance, when defining term denotations in \cref{def:fol_denotation/terms}, we prefer writing
  \begin{equation*}
  ...
  \begin{equation*}
    \Bracks{\tau}_\mscrX^v \coloneqq \begin{cases}
      ...
      I(f)\parens[\big]{ \Bracks{\sigma_1}_\mscrX^v, \ldots, \Bracks{\sigma_n}_\mscrX^v }, &\tau = f(\sigma_1, \ldots, \sigma_n) \T{and} f \not\in \op*{Infix}_\Sigma
    \end{cases}
  \end{equation*}

  Furthermore, we have defined signature morphisms in \cref{def:fol_signature_category/morphisms} so that prefix and infix symbols can be exchanged.
\end{remark}
...
\end{algorithm}
\begin{comments}
  \item As discussed in \cref{rem:fol_application_infix_notation}, we allow exchanging prefix and infix symbols.
...
\begin{definition}\label{def:fol_formula}\mimprovised
  ...
  \begin{bnf*}
    \bnfprod{predicate application} {\underbrace{p \bnfsp \bnftsq{(} \bnfsp \overbrace{\bnfpn{term} \bnfsp \bnftsq{,} \bnfsp \bnfsk \bnfsp \bnftsq{,} \bnfsp \bnfpn{term}}^{\# p \T*{terms separated by commas}} \bnfsp \bnftsq{)}}_{\T*{One rule for each} p \in \op*{Pred}_\Sigma \setminus \op*{Infix}_\Sigma}} \\
    \bnfprod{predicate application} {\underbrace{\bnftsq{(} \bnfsp \bnfpn{term} \anon \bnfpn{term} \bnfsp \bnftsq{)}}_{\mathclap{\T*{One rule for each} \anon \in \op*{Pred}_\Sigma \cap \op*{Infix}_\Sigma}}} \\
  ...
  \begin{itemize}
    ...
    \item The \( \bnfpn{predicate application} \) rules are discussed in \cref{rem:fol_application_infix_notation}.
...
\begin{remark}\label{rem:fol_formula_conventions}
  ...
  \begin{thmenum}
    ...
    \thmitem{rem:fol_formula_conventions/infix} As discussed in \cref{rem:fol_application_infix_notation}, we will mostly disregard the difference between prefix and infix function and predicate applications if we are only interested in the \hyperref[con:abstract_syntax_tree]{abstract syntax trees} of the terms and formulas.
...
\begin{definition}\label{def:closed_fol_formula}\mcite[def. 2.3.8]{VanDalen2004LogicAndStructure}
  ...
  In fact, \enquote{sentence} is a more general placeholder word used in many definitions in \cref{sec:abstract_logics}. In those places, we explicitly disallow non-closed formulas for the reasons outlined in \cref{ex:universal_validity_and_lem}.

  In general, wherever we need closed formulas, we will rely on implicit quantification --- see \cref{rem:mathematical_logic_conventions/quantification}.
\end{definition}

\paragraph{First-order structures}

\begin{definition}\label{def:fol_structure}\mimprovised
  Fix a \hyperref[def:truth_value_algebra]{truth value algebra} \( \BbbH \). A \term[ru=структура (\cite[73]{ШеньВерещагин2017ЯзыкиИИсчисления})]{structure} \( \mscrX = (X, I) \) over the \hyperref[def:fol_signature]{first-order signature} \( \Sigma \) consists of the following:
  \begin{thmenum}
    \thmitem{def:fol_structure/universe} A \hi{nonempty} set \( X \), called a \term[en=universe (\cite[def. 2.1.15]{Hinman2005Logic})]{universe}.
    \thmitem{def:fol_structure/interpretation} A map \( I \), called an \term{interpretation}, that sends each \( n \)-ary function symbol to a function \( X^n \to X \) and every \( n \)-ary predicate to a function \( X^n \to \BbbH \).
  \end{thmenum}
\end{definition}
\begin{comments}
...
\begin{definition}\label{def:fol_variable_assignment}\mcite[def. 2.1.17; 92]{Andrews2002Logic}
  A \term[ru=оценка (\cite[def. 2.2.6]{Герасимов2011Вычислимость}), en=variable assignment (\cite[271]{Farmer2008STTVirtues}); assignment (\cite[def. 2.1.17]{Hinman2005Logic})]{variable assignment} in a given \hyperref[def:fol_structure]{structure} \( \mscrX = (X, I) \) is any function \( v: \op*{Var} \to X \).
...
\begin{definition}\label{def:fol_denotation}\mimprovised
  For the reasons outlined in \cref{rem:intuitionistic_first_order_semantics}, we will be working over the \hyperref[def:truth_value_algebra/classical]{classical truth value algebra} \( \BbbH = \set{ T, F } \).
  ...
  \begin{thmenum}
    \thmitem{def:fol_denotation/terms} We define the \term[ru=значение (\cite[def. 2.2.8]{Герасимов2011Вычислимость}), en=denotation (\cite[91]{Hinman2005Logic})]{denotation} \( \Bracks{\tau}_\mscrX^v \) of a \hyperref[def:fol_term]{first-order term} \( \tau \) as follows:
    ...
    If \( \tau \) is a \hyperref[def:closed_fol_term]{closed term}, the denotation \( \Bracks{\tau}_\mscrX^v \) does not depend on \( v \), and we simply write \( \Bracks{\tau}_\mscrX \).
    ...
    \thmitem{def:fol_denotation/formulas} We define the \term[ru=истинностное значение (\cite[def. 2.2.9]{Герасимов2011Вычислимость}), en=denotation (\cite[91]{Hinman2005Logic})]{denotation} \( \Bracks{\varphi}_\mscrX^v \) of a \hyperref[def:fol_formula]{first-order formula} \( \varphi \) as follows:
    ...
    If the denotation \( \Bracks{\varphi}_\mscrX^v \) does not depend on \( v \), we use the simpler notation \( \Bracks{\varphi}_\mscrX \). Unlike for terms, this can happen even if \( \varphi \) is not a \hyperref[def:closed_fol_formula]{closed formula} --- see \cref{ex:universal_validity_and_lem}.
    ...
    \begin{equation*}
      \begin{aligned}
        &\Bracks{\varphi}_\mscrX: X^n \to \BbbH, \\
...
\begin{remark}\label{rem:intuitionistic_first_order_semantics}
  ...
  Fix a structure \( \mscrX = (X, I) \) and a variable assignment \( v: \op*{Var} \to X \). For any two terms \( \tau \) and \( \sigma \), the denotation of equality based on \hyperref[def:kronecker_delta]{Kronecher's \( \delta \) function} gives us
  \begin{equation*}
  ...
  Therefore, for any variable assignment \( v \) in any structure \( \mscrX \), we have
  \begin{equation*}
    \Bracks{(\tau \syneq \sigma) \synvee \synneg (\tau \syneq \sigma)}_\mscrX^v = T
  \end{equation*}

  This is a particular instance of the law \eqref{eq:thm:classical_tautologies/lem} of the excluded middle that holds unconditionally.

  In the intuitionistic natural deduction system from \cref{def:fol_natural_deduction_systems}, the only equality rule introduction is \ref{inf:def:fol_natural_deduction_systems/equality/intro}, so, unless \( \tau \) coincides with \( \sigma \), a proof of the above must rely on discharging hypotheses. A careful inspection shows that none of the rules with dischargeable hypotheses allows us to obtain the desired proof tree. So, as defined, the intuitionistic semantics is \hyperref[def:abstract_logic/completeness]{incomplete} with respect to the more established rules of natural deduction.

  This particular denotation is also discussed in \cite{MathSE:topological_semantics_for_first_order_logic}, where several much more complicated denotations are suggested. \incite*{Scott1968IntuitionisticAnalysis} suggests a particular adaptation of \hyperref[def:truth_value_algebra/topological]{topological semantics} for first-order logic.
\end{remark}

\paragraph{Universal validity}

\begin{definition}\label{def:fol_universal_validity}\mcite[105]{Hinman2005Logic}
  If, for some \hyperref[def:fol_formula]{first-order formula} \( \varphi \) and some \hyperref[def:fol_structure]{first-order structure} \( \mscrX \), the \hyperref[def:fol_denotation/formula]{denotation} \( \Bracks{ \varphi }_\mscrX^v \) equals \( T \) for any variable assignment \( v \) in \( \mscrX \), we say that \( \varphi \) is \term{universally valid} in \( \mscrX \).
\end{definition}
\begin{comments}
  \item \hyperref[def:closed_fol_formula]{Closed formulas} are universally valid, but the notion is more general --- see \cref{ex:universal_validity_and_lem}.
...
\begin{definition}\label{def:fol_universal_closure}\mcite[def. 2.2.28]{Hinman2005Logic}
  We call a \term[ru=универсальное замыкание (\cite[65]{Герасимов2011Вычислимость})]{universal closure} of the \hyperref[def:hol_term/formula]{higher-order logical formula} \( \varphi \) any formula of the form
  \begin{equation*}
...
\begin{proposition}\label{thm:fol_universal_validity_iff_closure_validity}
 A \hyperref[def:fol_formula]{first-order formula} \( \varphi \) is \hyperref[def:fol_universal_validity]{universally valid} if and only if of its \hyperref[def:fol_universal_closure]{universal closures} are.
\end{proposition}
\begin{comments}
...
\begin{proof}
  ...
  \NecessitySubProof Conversely, suppose that \( \Bracks{ \qforall {x_1} \ldots \qforall {x_n} \varphi }_\mscrX^v = T \) for any assignment \( v \).

  Aiming at a contradiction, suppose that, for some assignment \( v_0 \), we have \( \Bracks{ \varphi }_\mscrX^{v_0} < T \). Then
  \begin{equation*}
    \Bracks{ \qforall {x_n} \varphi }_\mscrX^v
    \reloset {\eqref{eq:def:fol_denotation/formulas/forall}} =
    \bigwedge\set[\big]{ \Bracks{ \varphi }_\mscrX^{v_{x \mapsto a}} \given* a \in X }
  ...
  \begin{table}[!ht]
    ...
    \begin{tabular}{*{6}{l}}
      ...
      \cite[ch. 2]{Эдельман1975Логика}                            & Модель             & Основн. мн. &                  &            &                \\
                                                                  & (model)            & (fund. set) &                  &            &                \\
      \cite[\S 2.2]{Герасимов2011Вычислимость}                    & Интерпретация      & Носитель    & ---              & Оценка     & Значение       \\
