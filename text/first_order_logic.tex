...
\begin{definition}\label{def:equisatisfiability}\mcite[237]{Mimram2020ProgramEqualsProof}
  In a given \hyperref[def:institution]{institution}, we say that two sets \( \Gamma \) and \( \Delta \) of sentences are \term{equisatisfiable} if \( \Gamma \) and \( \Delta \) either both have models (possibly different) or if both do not.
\end{definition}
