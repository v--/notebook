...
\begin{definition}\label{def:first_order_language}\mcite[def. 2.1.2]{Hinman2005}
  ...
  \begin{description}
    ...
    \begin{thmenum}[series=def:first_order_language]
      ...
      \thmitem{def:first_order_language/universal_quantifier_quantifier} The \term[ru=квантор общости (\cite[61]{Эдельман1975})]{universal quantifier} \enquote{\( \synforall \)}.

      \thmitem{def:first_order_language/existential_quantifier} The \term[ru=квантор существования (\cite[61]{Эдельман1975})]{existential quantifier} \enquote{\( \synexists \)}.
    ...
    \begin{thmenum}[resume=def:first_order_language]
      ...
      Predicate symbols also have an associated arity. Predicate symbols of zero arity act as \hyperref[def:propositional_grammar_schema/prop]{propositional variables}.
...
\begin{definition}\label{def:first_order_syntax}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:first_order_syntax/var} We denote by \( \boldop{Var} \) the set of strings generated by the above grammar schema with starting nonterminal \( \bnfpn{variable} \).

    \thmitem{def:first_order_syntax/term} Similarly, we denote by \( \boldop{Term} \) the set of strings generated with starting nonterminal \( \bnfpn{term} \).
    ...
    \begin{equation}\label{eq:def:first_order_syntax/term_variables}
      \boldop{Var}(\tau) \coloneqq \begin{cases}
        ...
        \boldop{Var}(\kappa_1) \cup \ldots \cup \boldop{Var}(\kappa_n), &\tau = f(\kappa_1, \ldots, \kappa_n).
      \end{cases}
    \end{equation}
    ...
    \thmitem{def:first_order_syntax/atomic_formula} We denote by \( \boldop{Var} \) the set of strings generated by the above grammar schema with starting nonterminal \( \bnfpn{atomic formula} \).

    \thmitem{def:first_order_syntax/formula} Similarly, we denote by \( \boldop{Formula} \) the set of strings generated with starting nonterminal \( \bnfpn{formula} \).
    ...
    \begin{equation*}
      \boldop{Bound}(\varphi) \coloneqq \begin{cases}
        ...
        \boldop{Bound}(\psi),                               &\varphi = \synneg \psi, \\
        \boldop{Bound}(\psi_1) \cup \boldop{Bound}(\psi_2), &\varphi = \psi_1 \syncirc \psi_2, {\syncirc} \in \Sigma, \\
        \boldop{Bound}(\psi) \cup \set{ \xi },              &\varphi = \quantifier{q}{\xi} \psi, q \in \set{ \synforall, \synexists }.
      \end{cases}
    \end{equation*}
    ...
    \begin{equation*}
      \boldop{Free}(\varphi) \coloneqq \begin{cases}
        ...
        \boldop{Var}(\tau_1) \cup \boldop{Var}(\tau_2),             &\varphi = \tau_1 \syneq \tau_2, \\
        \boldop{Free}(\psi),                                        &\varphi = \synneg \psi, \\
        \boldop{Free}(\psi_1) \cup \boldop{Free}(\psi_2),           &\varphi = \psi_1 \syncirc \psi_2, {\syncirc} \in \Sigma, \\
        \boldop{Free}(\psi) \setminus \set{ \xi },                  &\varphi = \quantifier{q}{\xi} \psi, q \in \set{ \synforall, \synexists }
      \end{cases}
    \end{equation*}

    \thmitem{def:first_order_syntax/closed_formula}\mcite[def. 2.2.7]{Hinman2005} A formula \( \varphi \) is called a \term{closed formula}.

    Peter Hinman in \incite[def. 2.2.7]{Hinman2005} calls them \term{sentences}. This contrasts with propositional logic, where all formulas are called sentences --- see \fullref{def:propositional_grammar_schema/formula}. We prefer \enquote{closed formula} because of similarity with closed terms defined in \fullref{def:first_order_syntax/closed_term}.
...
\begin{remark}\label{rem:first_order_formula_conventions}
  ...
  \begin{thmenum}
    ...
    as a shorthand for
    \begin{equation*}
      \qforall \xi (p(\xi, \cdots) \synimplies \varphi)
    \end{equation*}
    and
    \begin{equation*}
    ...
    as a shorthand for
    \begin{equation*}
      \qexists \xi (p(\xi, \cdots) \synwedge \varphi).
    \end{equation*}
    ...
    as a shorthand for
    \begin{equation*}
      \qexists \xi \parens[\Big]{ \varphi \synwedge \parens[\Big]{ \qforall \eta \varphi[\xi \mapsto \eta] \synimplies (\xi \syneq \eta) } },
    \end{equation*}
  ...
  \begin{thmenum}
    ...
    \begin{equation}\label{eq:def:first_order_valuation/formula_valuation}
      \Bracks{\varphi}_v \coloneqq \begin{cases}
        ...
        \Bracks{\psi_1}_v \relcirc \Bracks{\psi_2}_v,                    &\varphi = \psi_1 \syncirc \psi_2, {\syncirc} \in \Sigma, \\
...
\begin{definition}\label{def:first_order_semantics}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:first_order_semantics/equivalence}\mcite[def. 2.2.1(ii)]{Hinman2005} As in the case with \hyperref[def:propositional_entailment/equivalence]{propositional semantical equivalence}, we say that \( \varphi \) and \( \psi \) are \term{semantically equivalent} and write \( \varphi \gleichstark \psi \) if both \( \varphi \vDash \psi \) and \( \psi \vDash \varphi \).
...
\begin{comments}
  ...
  \item The concept of universal validity is more general than \hyperref[def:propositional_entailment/tautology]{propositional tautologies} in the sense that we define validity for arbitrary formulas that may or may not be closed.
...
\begin{definition}\label{def:first_order_substitution}
  ...
  \begin{thmenum}
    \thmitem{def:first_order_substitution/propositional} Let \( \varphi \) be a \hyperref[def:propositional_grammar_schema/formula]{propositional formula} with variables \( \boldop{Var}(\varphi) = \set{ P_1, \ldots, P_n } \). For brevity, denote \( V \coloneqq \boldop{Var}(\varphi) \). Let \( \Theta = \set{ \theta_1, \ldots, \theta_n } \) be a set of \hyperref[def:first_order_syntax/formula]{first-order formulas}.
    ...
    \begin{equation}\label{eq:def:first_order_substitution/propositional}
      \varphi[V \mapsto \Theta] \coloneqq \begin{cases}
        ...
        \psi_1[V \mapsto \Theta] \syncirc \psi_2[V \mapsto \Theta], &\varphi = \psi_1 \syncirc \psi_2, {\syncirc} \in \Sigma.
      \end{cases}
    \end{equation}
    ...
    \begin{equation}\label{eq:def:first_order_substitution/term_in_formula}
      \varphi[\kappa \mapsto \mu] \coloneqq \begin{cases}
        ...
        \psi_1[\kappa \mapsto \mu] \syncirc \psi_2[\xi \mapsto \mu],       &\varphi = \psi_1 \syncirc \psi_2, {\syncirc} \in \Sigma, \\
        (\dagger),                                                         &\varphi = \quantifier Q {\xi} \psi, Q \in \set{ \synforall, \synexists },
      \end{cases}
    \end{equation}
    where
    \begin{subequations}
...
\begin{example}\label{ex:first_order_substitution}
  ...
  \begin{thmenum}
    ...
    \begin{align*}
      &\phantom{{}={}}
      \parens[\Big]{ \qforall \eta (p(\xi, \eta) \synvee \qforall \xi p(\xi, \eta)) }[\xi \mapsto \eta]
      \reloset {\eqref{eq:def:first_order_substitution/term_in_formula/quantifiers/renaming}} = \\ &=
      \qforall \zeta \parens*{ \parens[\Big]{ p(\xi, \eta) \synvee \qexists \xi p(\xi, \eta) }[\eta \mapsto \zeta][\xi \mapsto \eta] }
      \reloset {\eqref{eq:def:first_order_substitution/term_in_formula}} = \\ &=
      \qforall \zeta \parens*{ p(\eta, \zeta) \synvee \parens[\Big]{ \qexists \xi p(\xi, \eta) }[\eta \mapsto \zeta][\xi \mapsto \eta] }
      \reloset {\eqref{eq:def:first_order_substitution/term_in_formula/quantifiers/direct}} = \\ &=
      \qforall \zeta \parens*{ p(\eta, \zeta) \synvee \parens[\Big]{ \qexists \xi p(\xi, \zeta) }[\xi \mapsto \eta] }
      \reloset {\eqref{eq:def:first_order_substitution/term_in_formula/quantifiers/trivial}} = \\ &=
      \parens[\Big]{ \qforall \zeta p(\eta, \zeta) } \synvee \parens[\Big]{ \qexists \xi p(\xi, \zeta) }.
    \end{align*}
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \begin{balign*}
      \Bracks[\Big]{ \tau[\xi \mapsto \eta] }_v
      &=
      \Bracks[\Big]{ f(\kappa_1[\xi \mapsto \eta], \ldots, \kappa_n[\xi \mapsto \eta]) }_v
      = \\ &=
      I(f) \parens[\Bigg]{ \Bracks[\Big]{ \kappa_1[\xi \mapsto \eta] }_v, \ldots, \Bracks[\Big]{ \kappa_1[\xi \mapsto \eta] }_v }
      \reloset {\T{ind.}} = \\ &=
      I(f) \parens[\Big]{ \Bracks{\kappa_1}_{v_{\xi \mapsto \eta}}, \ldots, \Bracks{\kappa_n}_{v_{\xi \mapsto \eta}} }
      = \\ &=
      \Bracks[\Big]{ f(\kappa_1, \ldots, \kappa_n) }_{\xi \mapsto \eta}
      = \\ &=
...
\begin{proposition}\label{thm:first_order_substitution_equivalence}
  ...
  \begin{thmenum}
    \thmitem{thm:first_order_substitution_equivalence/propositional} As in \fullref{def:first_order_substitution/propositional}, let \( \varphi \) be a \hyperref[def:propositional_grammar_schema/formula]{propositional formula} with variables \( {V = \set{ P_1, \ldots, P_n }} \) and let \( \Theta = \set{ \theta_1, \ldots, \theta_n } \) be a set of \hyperref[def:first_order_syntax/formula]{first-order formulas}.
    ...
    In particular, \( \vDash \varphi \) in the sense of \fullref{def:propositional_entailment/tautology} implies \( \vDash \varphi[V \mapsto \Theta] \) in the sense of \fullref{def:first_order_semantics/validity}.
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \item If \( \varphi = \psi_1 \syncirc \psi_2, {\syncirc} \in \Sigma \) and if the inductive hypothesis holds for both \( \psi_1 \) and \( \psi_2 \), then
    \begin{equation*}
      \Bracks[\Big]{ \psi[V \mapsto \Theta] }_v
      =
  ...
  Furthermore, for formulas with \( n \) quantifiers with \( \synforall \) as the outermost one, the non-renaming cases \eqref{eq:def:first_order_substitution/term_in_formula/quantifiers/trivial} and \eqref{eq:def:first_order_substitution/term_in_formula/quantifiers/direct} hold. Therefore, since \( \eta \not\in \boldop{Free}(\mu) \),
...
\begin{concept}\label{con:predicate_formula}
  ...
  \begin{equation}\taglabel[\op{IsBottom}]{con:predicate_formula/bottom}
    \ref{con:predicate_formula/bottom}[\xi] \coloneqq \qforall \eta (\xi \leq \eta).
  \end{equation}
...
\begin{proposition}\label{thm:first_order_quantifiers_are_dual}
  ...
  \begin{align}
    \synneg \qforall \xi \varphi &\gleichstark \qexists \xi \synneg \varphi \label{eq:thm:first_order_quantifiers_are_dual/negation_of_universal} \\
    \synneg \qexists \xi \varphi &\gleichstark \qforall \xi \synneg \varphi \label{eq:thm:first_order_quantifiers_are_dual/negation_of_existential}
  \end{align}
\end{proposition}
\begin{proof}
  ...
  \begin{align*}
    \Bracks{ \synneg \qforall \xi \varphi }_v
    &=
    \oline{\Bracks{ \qforall \xi \varphi }_v}
    = \\ &=
    \oline{\bigwedge\set{ \Bracks{\varphi}_{v_{\xi \mapsto x}} \given x \in X }}
    \reloset {\eqref{eq:thm:de_morgans_laws/complement_of_meet}} = \\ &=
    \bigvee\set*{ \oline{\Bracks{\varphi}_{v_{\xi \mapsto x}}} \given x \in X }
    = \\ &=
    \bigvee\set{ \Bracks{\synneg \varphi}_{v_{\xi \mapsto x}} \given x \in X }
    = \\ &=
    \Bracks{\qexists \xi \synneg \varphi}_v.
  \end{align*}
\end{proof}
...
\begin{corollary}\label{thm:relativized_first_order_quantifiers_are_dual}
  ...
  \begin{align}
    \synneg \qforall {p(\xi, \cdots)} \varphi &\gleichstark \qexists {p(\xi, \cdots)} \synneg \varphi \label{eq:thm:relativized_first_order_quantifiers_are_dual/negation_of_universal} \\
    \synneg \qexists {p(\xi, \cdots)} \varphi &\gleichstark \qforall {p(\xi, \cdots)} \synneg \varphi \label{eq:thm:relativized_first_order_quantifiers_are_dual/negation_of_existential}
  \end{align}
\end{corollary}
\begin{proof}
  We will only show \eqref{eq:thm:relativized_first_order_quantifiers_are_dual/negation_of_universal}:
  \begin{align*}
    \synneg \qforall {p(\xi, \cdots)} \varphi
    &=
    \synneg \qforall \xi \parens[\Big]{ p(\xi, \cdots) \synimplies \varphi }
    \gleichstark \\ \reloset {\eqref{eq:thm:boolean_equivalences/conditional_as_disjunction}} \gleichstark
    \synneg \qforall \xi \parens[\Big]{ \synneg p(\xi, \cdots) \synvee \varphi }
    \gleichstark \\ \reloset {\eqref{eq:thm:first_order_quantifiers_are_dual/negation_of_universal}} \gleichstark
    \qexists \xi \parens[\Big]{ p(\xi, \cdots) \synwedge \synneg \varphi }
  \end{align*}
\end{proof}

\begin{proposition}\label{thm:pulling_quantifiers_out}
  For \( Q \in \set{ \synforall, \synexists } \) and \( \syncirc \in \set{ \synvee, \synwedge } \), if \( \eta \) is not free in \( \varphi \) nor \( \psi \), then
  \begin{equation}\label{thm:pulling_quantifiers_out/first}
    \parens[\Big]{ \quantifier Q \xi \varphi } \syncirc \psi \gleichstark \quantifier Q \eta \parens[\Big]{ \varphi[\xi \mapsto \eta] \syncirc \psi }\phantom{.}
  \end{equation}
  and similarly
  \begin{equation}\label{thm:pulling_quantifiers_out/second}
    \varphi \syncirc \parens[\Big]{ \quantifier Q \xi \psi } \gleichstark \quantifier Q \eta \parens[\Big]{ \varphi \syncirc \psi[\xi \mapsto \eta] }.
  \end{equation}
\end{proposition}
\begin{proof}
  ...
  \begin{equation}\label{thm:pulling_quantifiers_out/first_proof}
    \parens[\Big]{ \qforall \xi \varphi } \synwedge \psi \gleichstark \qforall \xi \parens[\Big]{ \varphi[\xi \mapsto \eta] \synwedge \psi }
  \end{equation}
  ...
  \begin{balign*}
    \Bracks[\Big]{ \qforall \eta \parens[\Big]{ \varphi[\xi \mapsto \eta] \synwedge \psi } }_v
    &=
    \bigvee\set[\Big]{ \Bracks[\Big]{\varphi[\xi \mapsto \eta]}_{v_{\eta \mapsto x}} \synwedge \Bracks{\psi}_{v_{\eta \mapsto x}} \given* x \in X }
    \reloset {\eqref{eq:thm:renaming_assignment_compatibility/formulas}} = \\ &=
    \bigvee\set[\Big]{ \Bracks{\varphi}_{v_{\xi \mapsto \eta,\eta \mapsto x}} \synwedge \Bracks{\psi}_{v_{\eta \mapsto x}} \given* x \in X }
    = \\ &=
    \bigvee\set[\Big]{ \Bracks{\varphi}_{v_{\xi \mapsto x}} \synwedge \Bracks{\psi}_{v_{\eta \mapsto x}} \given* x \in X }
    = \\ &=
    \bigvee\set[\Big]{ \Bracks{\varphi}_{v_{\xi \mapsto x}} \synwedge \Bracks{\psi}_v \given* x \in X }
    \reloset {\eqref{eq:thm:def:boolean_algebra/distributive/meet_over_join}} = \\ &=
    \bigvee\set[\Big]{ \Bracks{\varphi}_{v_{\xi \mapsto x}} \given* x \in X } \synwedge \Bracks{\psi}_v
    = \\ &=
    \Bracks[\Big]{ \qforall \xi \varphi }_v \synwedge \Bracks{\psi}_v
    = \\ &=
    \Bracks[\Big]{ \parens[\Big]{ \qforall \xi \varphi } \synwedge \psi }_v.
  \end{balign*}
  ...
  \begin{thmenum}
    ...
    \begin{equation*}
      P_2(\varphi) \coloneqq \begin{cases}
        \qexists \xi P_2(\synneg \psi),                                             &\varphi = \synneg \qforall \xi \psi, \\
        \qforall \xi P_2(\synneg \psi),                                             &\varphi = \synneg \qexists \xi \psi, \\
    ...
    \Fullref{thm:de_morgans_laws}, \fullref{thm:boolean_equivalences/double_negation}, \fullref{thm:first_order_substitution_equivalence/propositional} and \fullref{thm:first_order_quantifiers_are_dual} ensure that \( P_2(\varphi) \gleichstark \varphi \) for any formula \( \varphi \)  not containing \( \synimplies \) or \( \syniff \).
    ...
    \begin{equation*}
      P_3(\varphi) \coloneqq \begin{cases}
        ...
        P_3\parens[\Big]{ P_3(\psi) \syncirc P_3(\theta) },                                &\varphi = \psi \syncirc \theta \T{and} \boldop{Bound}(\varphi) \neq \varnothing, \\
        \quantifier Q \eta P_3 \parens[\Big]{ \varphi[\xi \mapsto \eta] \syncirc \theta }, &\varphi = \parens[\Big]{ \quantifier Q \xi \psi } \syncirc \theta, \\
                                                                                           &Q \in \set{ \synforall, \synexists }, \quad \syncirc \in \set{ \synvee, \synwedge }, \\
                                                                                           &\eta \not\in \boldop{Free}(\psi) \cup \boldop{Free}(\theta), \\
        \quantifier Q \eta P_3 \parens[\Big]{ \varphi \syncirc \theta[\xi \mapsto \eta] }, &\varphi = \psi \syncirc \parens[\Big]{ \quantifier Q \xi \theta }, \\
                                                                                           &Q \in \set{ \synforall, \synexists }, \quad \syncirc \in \set{ \synvee, \synwedge }, \\
                                                                                           &\eta \not\in \boldop{Free}(\psi) \cup \boldop{Free}(\theta), \\
        \hyperref[rem:straightforward_traversal]{\T{straightforward traversal}},           &\T{otherwise.}
      \end{cases}
    ...
    \Fullref{thm:pulling_quantifiers_out} ensures that \( P_3(\varphi) \gleichstark \varphi \) for any formula \( \varphi \) not containing \( \synimplies \) or \( \syniff \). It is not necessarily in prenex normal form because it does nothing to \( \varphi = \synneg \qforall \xi \psi \).
  \end{thmenum}
...
\begin{proof}
  ...
  Since \( \Bracks{\qforall \xi \varphi} = T \), by definition of valuation of \( \synforall \) we have
  \begin{equation*}
...
\begin{example}\label{ex:existential_quantifier_removal_monoids}
  ...
  \begin{equation*}
    \qexists \eta \qforall \xi (\eta \cdot \xi \syneq \xi \synwedge \xi \cdot \eta \syneq \xi).
  \end{equation*}
