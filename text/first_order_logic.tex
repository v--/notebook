...
\begin{remark}\label{rem:first_order_formula_conventions}
  ...
  \begin{thmenum}
    ...
    \thmitem{rem:first_order_formula_conventions/infix} \hyperref[def:function_application_syntax]{infix notation}
    ...
    \thmitem{rem:first_order_formula_conventions/unique_existence}\mcite[177]{Hinman2005Logic} We sometimes want to specify not only existence, but also uniqueness. This is the case in \eqref{eq:def:zfc/choice}, for example. It is conventional to write
    \begin{equation*}
      \qExists \synx \varphi
    \end{equation*}
    as a shorthand for
...
\begin{algorithm}[Prenex normal form conversion]\label{alg:prenex_normal_form_conversion}
  ...
  \begin{thmenum}
    ...
    \Cref{thm:classical_equivalences/conditional_as_disjunction}, \cref{thm:propositional_admissible_rules/biconditional_as_conjunction} and \cref{thm:first_order_substitution_equivalence/propositional} together ensure that \( P_1(\varphi) \gleichstark \varphi \) for any formula \( \varphi \).
    ...
    \begin{equation*}
      P_3(\varphi) \coloneqq \begin{cases}
        ...
        \quantifier Q \syny P_3 \parens[\Big]{ \psi[\synx \mapsto \syny] \syncirc \theta }, &\varphi = \parens[\Big]{ \quantifier Q \synx \psi } \syncirc \theta, \\
                                                                                           &Q \in \op*{Quan}, \quad \syncirc \in \set{ \synvee, \synwedge }, \\
                                                                                           &\syny \not\in \op*{Free}(\quantifier Q \synx \psi) \cup \op*{Free}(\theta), \\
        \quantifier Q \syny P_3 \parens[\Big]{ \psi \syncirc \theta[\synx \mapsto \syny] }, &\varphi = \psi \syncirc \parens[\Big]{ \quantifier Q \synx \theta }, \\
                                                                                           &Q \in \op*{Quan}, \quad \syncirc \in \set{ \synvee, \synwedge }, \\
                                                                                           &\syny \not\in \op*{Free}(\psi) \cup \op*{Free}(\quantifier Q \synx \theta), \\
        \hyperref[rem:straightforward_traversal]{\T{straightforward traversal}},           &\T{otherwise.}
      \end{cases}
    \end{equation*}
