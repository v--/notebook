...
\begin{definition}\label{def:fol_term}\mimprovised
  ...
  Similarly to the case of \( \muplambda \)-terms in \cref{def:propositional_subformula}, we will denote the set of all \term[ru=индивидные переменные / предметные переменные (\cite[84]{Герасимов2014Вычислимость}), en=individual variables (\cite[57]{VanDalen2004LogicAndStructure})]{individuals variables} by \( \op*{Var} \) and the set of all \term{first-order terms} over \( \Sigma \) by \( \op*{Term}_\Sigma \).
\end{definition}
\begin{comments}
...
\begin{definition}\label{def:closed_fol_formula}\mcite[def. 2.3.8]{VanDalen2004LogicAndStructure}
  ...
  In fact, \enquote{sentence} is a more general placeholder word used in many definitions in \cref{sec:logical_consequences}. In those places, we explicitly disallow non-closed formulas for the reasons outlined in \cref{ex:universal_validity_and_lem}.
