...
\begin{remark}\label{rem:fol_formula_conventions}
  ...
  \begin{thmenum}
    ...
    \thmitem{rem:fol_formula_conventions/infix} As mentioned in \cref{def:fol_signature}, many binary function and predicate symbols are intended to use \hyperref[def:function_application_syntax]{infix notation}.

    Examples abound --- all binary operations from \fullref{ch:group_theory}, \fullref{ch:ring_theory} and \fullref{ch:order_theory}, as well as the order relations from the latter. Other examples include the binary Peano arithmetic operations in \fullref{sec:natural_numbers} and the set membership predicate in \fullref{sec:set_theory}.

    Thus, when we have declared that a symbol should use infix notation, we use it in all related terms and formulas in the metalanguage.
    ...
    \thmitem{rem:fol_formula_conventions/unique_existence} We use the abbreviation in \cref{rem:exists_unique_abbreviation} to emulate the unique existence quantifier \( \qExists x \varphi \) discussed in \cref{con:description_operator/unique_existence}.
  \end{thmenum}
\end{remark}
...
\begin{definition}\label{def:fol_denotation}\mimprovised
  ...
  \begin{thmenum}
    ...
    \begin{equation*}
      \begin{aligned}
        &\Bracks{\tau}: X^n \to X, \\
        &\Bracks{\tau}(a_1, \ldots, a_n) \coloneqq \Bracks{\tau}_\mscrX^v,
      \end{aligned}
    \end{equation*}
    ...
    \begin{equation*}
      \begin{aligned}
        &\Bracks{\tau}: X^n \to \BbbH, \\
        &\Bracks{\tau}(a_1, \ldots, a_n) \coloneqq \Bracks{\varphi}_\mscrX^v,
      \end{aligned}
    \end{equation*}
    where \( v \) is any variable assignment such that \( v(x_k) = a_k \) for \( k = 1, \ldots, n \).
