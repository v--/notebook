...
\begin{definition}\label{def:fol_signature}\mcite[282]{Meseguer1989GeneralLogics}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:fol_signature/notation}\mimprovised We allow  \hyperref[def:set_partition]{partitioning} the set \( \op*{Fun}_\Sigma \cup \op*{Pred}_\Sigma \) of all symbols into three subsets called \( \op*{Prefix}_\Sigma \), \( \op*{Cond}_\Sigma \) and \( \op*{Infix}_\Sigma \), based on the \hyperref[def:function_application_notation]{application notation} for we want to use.

    Of course, prefix notation only makes sense for binary symbols. The default for all is to use prefix notation.
  \end{thmenum}
...
\begin{definition}\label{def:fol_term}\mimprovised
  ...
  \begin{bnfextra}
    \bnfprod{function application} {f}                                                                                                                                                                                 \bnfe{if \( \# f = 0 \)}                                     \\
    \bnfprod{function application} {f \bnfsp \bnftsq{(} \bnfsp \overbrace{\bnfpn{term} \bnfsp \bnftsq{,} \bnfsp \bnfsk \bnfsp \bnftsq{,} \bnfsp \bnfpn{term}}^{\# f \T*{terms separated by commas}} \bnfsp \bnftsq{)}} \bnfe{if \( \# f > 0 \) and \( f \in \op*{Prefix}_\Sigma \)} \\
    \bnfprod{function application} {\bnftsq{(} \bnfsp \bnfpn{term} \anon \bnfpn{term} \bnfsp \bnftsq{)}}                                                                                                               \bnfe{if \( \# f > 0 \) and \( f \in \op*{Infix}_\Sigma \)}  \\
    \bnfprod{function application} {f \bnfsp \underbrace{\bnfpn{term} \bnfsp \bnfsk \bnfsp \bnfpn{term}}_{\# f \T*{terms}}}                                                                                            \bnfe{if \( \# f > 0 \) and \( f \in \op*{Cond}_\Sigma \)}
  \end{bnfextra}
...
\begin{remark}\label{rem:fol_application_notation}
  ...
  Examples of condensed notation are mostly restricted to unary operators, where writing parentheses adds little benefit. This includes nearly all unary operators from \fullref{ch:set_theory}, as well as various kinds of \hyperref[def:morphism_invertibility/involution]{involution} and \hyperref[def:moore_closure_operator]{closure operators}.
