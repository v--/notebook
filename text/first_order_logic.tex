...
\begin{definition}\label{def:closed_fol_formula}\mcite[def. 2.3.8]{VanDalen2004LogicAndStructure}
  ...
  In fact, \enquote{sentence} is a more general placeholder word used in many definitions in \cref{sec:logical_consequence}. For the reasons outlined in \cref{rem:implicit_quantification_and_deduction}, we will only consider closed formulas for the sentence functor when defining an \hyperref[def:institution]{institution} in \cref{def:fol_institution/sentences}. Furthermore, we will be explicit about when we want to work with \hyperref[def:fol_quantifier_closure]{universal closures} (see e.g. the theory of semirings in \cref{def:semiring/theory}), and will mostly avoid relying on \hyperref[con:implicit_universal_quantification]{implicit universal quantification}.
\end{definition}
...
\begin{definition}\label{def:fol_denotation}\mimprovised
  ...
  \begin{thmenum}
    ...
    \begin{empheq}[left={\Bracks{\varphi}_\mscrX^v} \coloneqq \empheqlbrace]{align}
      ...
      &\delta_\mscrX\parens[\big]{ \Bracks{N}_\mscrX^v, \Bracks{K}_\mscrX^v },          &&\varphi = (N \syneq K),              \label{eq:def:fol_denotation/formulas/eq} \\
    ...
    \begin{itemize}
      \item In \eqref{eq:def:fol_denotation/formulas/eq}, \( \delta_\mscrX \) is the \hyperref[def:kronecker_delta]{Kronecker delta function} (written using prefix notation).
    ...
    \thmitem{def:fol_denotation/formulas_parametrized} Just as we have associated a \hyperref[con:parametrized_denotation]{parametrized denotation} with every term, if \( \op*{Free}(\varphi) \subseteq \set{ x_1, \ldots, x_n } \), we can define the function
    \begin{equation*}
      \begin{aligned}
...
\begin{definition}\label{def:fol_structure_expansion}\mcite[def. 2.6.3]{Hinman2005Logic}
  ...
  We say that the \hyperref[def:fol_structure]{structure} \( \mscrX^+ = (X^+, I^+) \) over \( \Sigma^+ \) is an \term{expansion} of \( \mscrX = (X, I) \) over \( \Sigma^+ \) if \( X = X^+ \) and \( I^+(f) = I(f) \) for every function or predicate symbol \( f \) in \( \Sigma \).
...
\begin{definition}\label{def:fol_reduct_along_morphism}\mcite[example 5]{MossakowskiKrumnackMaibaum2015DerivedSignatureMorphisms}
  For every \hyperref[def:fol_signature_category/morphisms]{signature morphism} \( t: \Sigma \to \Theta \) and every \hyperref[def:fol_structure]{structure} \( \mscrY = (Y, J) \) over \( \Theta \), we define the \term{reduct} of \( \mscrY \) along \( t \) as the structure \( \red_t(\mscrY) \coloneqq (Y, J_t) \), where, for every (function or predicate) symbol \( f \) in \( \Sigma \),
...
\begin{definition}\label{def:fol_semantics}\mimprovised
  ...
  \begin{thmenum}
    ...
    It is possible to extend this relation to arbitrary formulas, not necessarily closed, by requiring \( \Bracks{\varphi}_\mscrX^v = T \) for every variable assignment \( v \) into \( \mscrX \). We call this convention \enquote{\hyperref[def:fol_implicit_universal_quantification]{implicit universal quantification}}, but only use it sparingly because, as explained in \cref{rem:implicit_quantification_and_deduction}, it breaks \fullref{thm:fol_semantic_deduction_theorem}.
...
\begin{remark}\label{rem:intuitionistic_first_order_semantics}
  ...
  Fix a structure \( \mscrX = (X, I) \) and a variable assignment \( v: \op*{Var} \to X \). For any two terms \( \tau \) and \( \sigma \), the denotation of equality based on \hyperref[def:kronecker_delta]{Kronecker's \( \delta_\mscrX \) function} gives us
  \begin{equation*}
    \Bracks{\tau \syneq \sigma}_\mscrX^v
    =
    \delta_\mscrX(\Bracks{\tau}_\mscrX^v, \Bracks{\sigma}_\mscrX^v)
    =
    \begin{cases}
  ...
  This is a particular instance of the classical tautology \eqref{eq:thm:classical_tautologies/lem}. The \hyperref[con:brouwer_heyting_kolmogorov_interpretation]{Brouwer-Heyting-Kolmogorov} interpretation suggests that either \( \tau \syneq \sigma \) or \( \tau \syneq \sigma \) should be universally valid, but neither is.
...
\end{proposition}
\begin{proof}
  \todo{Adapt propositional characterizations}

  \SubProofOf{thm:def:fol_theory/model_theory_consistency} Follows from \fullref{thm:propositional_semantic_lnc} via the equivalent condition \cref{thm:propositional_semantic_inconsistency/unsatisfiable} for consistency.
...
\begin{proof}
  ...
  \SufficiencySubProof* Suppose that \( h: X \to Y \) is a homomorphism. Fix an atomic formula \( \varphi \) and a variable assignment \( v \).
  ...
  \begin{itemize}
    ...
    \begin{equation*}
      ...
      \delta_\mscrX\parens[\big]{ \Bracks{\tau}_\mscrX^v, \Bracks{\sigma}_\mscrX^v }
      \reloset {\T{ind.}} =
      \delta_\mscrY\parens[\big]{ \Bracks{\tau}_\mscrY^{h \bincirc v}, \Bracks{\sigma}_\mscrY^{h \bincirc v} }
      \reloset {\eqref{eq:def:fol_denotation/formulas/eq}} =
  ...
  \SubProofOf*{def:fol_homomorphism/predicates} For a predicate symbol \( p \) in \( \Sigma \) of arity \( n \), since \( h \) respects atomic formulas, we have
  \begin{equation*}
  ...
  \SubProofOf*{def:fol_homomorphism/functions} For a functional symbol \( f \) in \( \Sigma \) of arity \( n \), we consider the following equality formulas:
  ...
  Fix elements \( a_1, \ldots, a_n \) in \( X \) and consider any assignment \( v \) that satisfies \( v(\synx_k) = a_k \) for \( k = 1, \ldots, n \) and
  \begin{equation*}
  ...
  \begin{equation*}
    \underbrace{I(f)(a_1, \ldots, a_n) = v(\syny)}_{\T{true by assumption}} \T{if and only if} J(f)\parens[\big]{ h(a_1), \ldots, h(a_n) } = h(v(\syny)),
  \end{equation*}
  hence
  \begin{equation*}
    J(f)\parens[\big]{ h(a_1), \ldots, h(a_n) } = h(v(\syny)) = h\parens[\big]{ I(f)(a_1, \ldots, a_n) }.
  \end{equation*}
...
\begin{proposition}\label{thm:def:elementary_embedding}
  \hyperref[def:elementary_embedding]{Elementary embeddings} have the following basic properties:
  \begin{thmenum}
...
\begin{proof}
  ...
  \begin{equation*}
    \Bracks{\synx \syneq \syny}_\mscrX^v = \underbrace{\Bracks{\synx \syneq \syny}_\mscrY^{h \bincirc v}}_{\delta_\mscrY(h(a), h(b)) = \semtop},
  \end{equation*}
  hence \( a = b \).
...
\begin{proposition}\label{thm:cardinality_control_formulas}
  ...
  For \( n > 1 \), we define \( \chi^{< n} \) as the opposite of \( \chi^{\geq n} \) obtained via \cref{alg:propositional_formula_dualization}:
  \begin{equation*}
...
\begin{comments}
  ...
  \end{equation*}
  which can be simplified (as justified in \cref{alg:propositional_constant_collapse/conjunction}).
...
\begin{proposition}\label{thm:pure_equality_isomorphism}
  The \hyperref[def:pure_equality]{theory of pure equality} \( \op*{Th}_{\syneq} \) has the following basic properties:
  \begin{thmenum}
