...
\begin{definition}\label{def:intuitionistic_equivalence_relation}\mimprovised
  We will generalize \hyperref[def:equivalence_relation]{equivalence relations} to an arbitrary \hyperref[def:truth_value_algebra]{truth value algebra} \( \BbbT \).

  Consider the function \( {\semeq}: A^2 \to \BbbT \) on some set \( A \). We will call \( {\semeq} \) an \term{intuitionistic equivalence relation} if it satisfies the following conditions:
  \begin{thmenum}
    \thmitem{def:intuitionistic_equivalence_relation/reflexivity} We have \( (a \semeq a) = \semtop \) for every element \( a \) of \( A \).
    \thmitem{def:intuitionistic_equivalence_relation/symmetry} We have \( (a \semeq b) = (b \semeq a) \) for all \( a \) and \( b \) in \( A \).
    \thmitem{def:intuitionistic_equivalence_relation/transitivity} We have \( (a \semeq b) \wedge (b \semeq c) \leq (a \semeq c) \) for all \( a \), \( b \) and \( c \) in \( A \).
  \end{thmenum}
\end{definition}
\begin{comments}
  \item This definition is based on \tcite{#1, where #2}[def. 13.6.2]{TroelstraVanDalen1988ConstructivismVol2} introduce \enquote{global \( \BbbT \)-sets} for dealing with intuitionistic semantics for predicate logic.
\end{comments}
...
\end{definition}
\begin{comments}
  \item We generalize the \enquote{Kronecker symbol} from \cite[213]{Bourbaki1998Algebra1to3} from rings to semirings, so that we are able to encompass truth value algebras. We use the latter as canonical \hyperref[def:intuitionistic_equivalence_relation]{intuitionistic equivalence relations} when defining denotations for equality in \hyperref[def:fol_structure]{first-order structures}.
\end{comments}

\begin{definition}\label{def:fol_structure}\mimprovised
  Fix a complete \hyperref[def:truth_value_algebra]{truth value algebra} \( \BbbT \). A \( \BbbT \)-valued \term[en=structure (\cite[def. 2.1.15]{Hinman2005Logic})]{structure} \( \mscrX = (X, I) \) over the \hyperref[def:fol_signature]{first-order signature} \( \Sigma \) consists of the following:
  \begin{thmenum}
    \thmitem{def:fol_structure/universe} A \hi{nonempty} set \( X \), called a \term[en=universe (\cite[def. 2.1.15]{Hinman2005Logic})]{universe}.
    \thmitem{def:fol_structure/interpretation} A map \( I \), called an \term{interpretation}, that sends each \( n \)-ary function symbol to a function \( X^n \to X \) and every \( n \)-ary predicate to a function \( X^n \to \BbbT \), and \( \syneq \) to an \hyperref[def:intuitionistic_equivalence_relation]{intuitionistic equivalence relation} \( {\semeq} \) on \( X \).

    We require the following coherence conditions for every \( n \)-ary functional symbol \( f \) and every \( n \)-ary predicate symbol \( p \):
    \begin{align}
      &(x_1 \semeq y_1) \wedge \cdots \wedge (x_n \semeq y_n) \leq I(f)(x_1, \ldots, x_n) \semeq I(f)(y_1, \ldots, y_n), \label{eq:def:fol_structure/interpretation/function_equality} \\
      &(x_1 \semeq y_1) \wedge \cdots \wedge (x_n \semeq y_n) \wedge I(p)(x_1, \ldots, x_n) \leq I(p)(y_1, \ldots, y_n). \label{eq:def:fol_structure/interpretation/predicate_equality}
    \end{align}

    By \term{standard equality}, we will mean that \( {\semeq} \) is metatheoretical equality; then \( {\semeq} \) is \hyperref[def:kronecker_delta]{Kroneker's \( \delta \)-function}.
  \end{thmenum}

  We will refer to the structure itself as \term{intuitionistic} or \term{classical} depending on \( \BbbT \). We presuppose that equality is standard in classical structures.

  Unless explicitly noted otherwise, all structures we consider simultaneously will have the same underlying truth value algebra.
\end{definition}
\begin{comments}
  ...
  \item This definition is based on \bycite[711]{TroelstraVanDalen1988ConstructivismVol2}, where the authors introduce \enquote{global \( \BbbT \)-structures} for dealing with intuitionistic semantics for predicate logic. For the classical case, a similar definition is given in \bycite[def. 2.2.3]{Герасимов2014Вычислимость}.

  \item We prefer the infix notation \( x \semeq y \) to the prefix notation \( I(\syneq)(x, y) \). For disambiguation, we may add a subscript to \( \semeq \), .e.g \( \semeq_\mscrX \).
  ...
  \item \hyperref[con:minimal_logic]{Minimal logic} also requires a structure to interpret the falsum \( \synbot \); see \cref{def:minimal_fol_denotation}.
\end{comments}
...
\begin{definition}\label{def:fol_denotation}\mimprovised
  Fix a complete \hyperref[def:truth_value_algebra/classical]{truth value algebra} \( \BbbT \). Let \( \mscrX = (X, I) \) be a \( \BbbT \)-valued \hyperref[def:fol_structure]{first-order structure} over \( \Sigma \), and let \( v: \op*{Var} \to X \) be a \hyperref[def:fol_variable_assignment]{variable assignment}.
  ...
  \begin{thmenum}
    ...
    \begin{empheq}[left={\Bracks{\varphi}_I} \coloneqq \empheqlbrace]{align}
      &v(x),                                                                                &&\tau = x \in \op*{Var},                   \label{eq:def:fol_denotation/terms/var} \\
    ...
    \thmitem{def:fol_denotation/terms_parametrized} Based on how we have defined \hyperref[con:parametrized_denotation]{parametrized denotations} for propositional formulas in \cref{def:parametrized_propositional_denotation}, assuming that \( \op*{Var}(\tau) \subseteq \set{ x_1, \ldots, x_n } \), we can define the function
    \begin{equation*}
      \begin{aligned}
    ...
    \begin{empheq}[left={\Bracks{\varphi}_\mscrX^v} \coloneqq \empheqlbrace]{align}
      &\semtop,                                                                         &&\varphi = \syntop,                   \label{eq:def:fol_denotation/formulas/top} \\
      &\sembot,                                                                         &&\varphi = \synbot,                   \label{eq:def:fol_denotation/formulas/bot} \\
      &\oline{\Bracks{\psi}_\mscrX^v},                                                  &&\varphi = \synneg \psi,              \label{eq:def:fol_denotation/formulas/neg} \\
      &\Bracks{\psi}_\mscrX^v \bincirc \Bracks{\theta}_\mscrX^v,                        &&\varphi = \psi \syncirc \theta,      \label{eq:def:fol_denotation/formulas/conn} \\
      &\Bracks{N}_\mscrX^v \semeq_\mscrX \Bracks{K}_\mscrX^v,                           &&\varphi = (N \syneq K),              \label{eq:def:fol_denotation/formulas/eq} \\
      &I(p)\parens[\big]{ \Bracks{\tau_1}_\mscrX^v, \ldots, \Bracks{\tau_n}_\mscrX^v }, &&\varphi = p(\tau_1, \ldots, \tau_n). \label{eq:def:fol_denotation/formulas/pred} \\
    ...
    \begin{itemize}
      \item In \eqref{eq:def:fol_denotation/formulas/eq}, \( \approx_\mscrX \) is the \( \BbbT \)-valued interpretation of formal equality in \( \mscrX \).

      \item In \eqref{eq:def:fol_denotation/formulas/pred}, we utilize predicate functions from the structure's interpretation.

      \item The cases \eqref{eq:def:fol_denotation/formulas/forall} and \eqref{eq:def:fol_denotation/formulas/exists} evaluates \( \psi \) using a \hyperref[def:hol_variable_assignment/modified]{modified assignment} parameterized by the elements of \( X_\sigma \). We see here why it is important for \( \BbbT \) to be \hyperref[def:complete_lattice]{complete}.
    \end{itemize}
    ...
    \thmitem{def:fol_denotation/formulas_parametrized} In the same way we have associated a \hyperref[con:parametrized_denotation]{parametrized denotation} with every term, if \( \op*{Free}(\varphi) \subseteq \set{ x_1, \ldots, x_n } \), we can define the function
    \begin{equation*}
      \begin{aligned}
...
\begin{comments}
  ...
  \item A modification of the first-order denotation is needed for \hyperref[con:minimal_logic]{minimal logic} --- see \cref{def:minimal_fol_denotation}.
...
\begin{definition}\label{def:minimal_fol_denotation}
  Similarly to how we modified propositional denotations for \hyperref[con:minimal_logic]{minimal logic} in \cref{def:minimal_propositional_denotation}, we modify first-order denotations for formulas from \cref{def:fol_denotation/formulas}.

  Fix a (complete) \hyperref[def:truth_value_algebra]{truth value algebra} \( \BbbT \). We need a \hyperref[def:fol_structure]{first-order structure} \( \mscrX = (X, I) \) to also interpret the falsum \( \synbot \) as a constant from \( \BbbT \) (not necessarily \( \sembot \)).

  Then we modify denotations as follows:
  \begin{empheq}[left={\Bracks{\varphi}_I} \coloneqq \empheqlbrace]{align}
    &I(\synbot),                             &&\varphi = \synbot,      \label{eq:def:minimal_fol_denotation/bot} \\
    &\Bracks{\psi}_I \rightarrow I(\synbot), &&\varphi = \synneg \psi, \label{eq:def:minimal_fol_denotation/neg} \\
    &\vdots,                                 && \notag
  \end{empheq}
\end{definition}

\begin{definition}\label{def:fol_homomorphism}\mimprovised
  Fix a complete \hyperref[def:truth_value_algebra]{truth value algebra} \( \BbbT \) and a \hyperref[def:fol_signature]{first-order signature} \( \Sigma \). We define a \term[en=homomorphism (\cite[def. 3.3.1]{VanDalen2004LogicAndStructure})]{homomorphism} between the \( \BbbT \)-valued \hyperref[def:fol_structure]{structures} \( \mscrX = (X, I) \) and \( \mscrY = (Y, J) \) over \( \Sigma \) as a function \( h: X \to Y \) which satisfies the following conditions:
  \begin{thmenum}
    ...
    \thmitem{def:fol_homomorphism/predicates} For every predicate symbol \( p \) of arity \( n \) over \( \Sigma \), and also for \( {\syneq} \) (regarded as a binary predicate), we must have
    \begin{equation}\label{eq:def:fol_homomorphism/predicates}
      I(p)\parens[\big]{ x_1, \ldots, x_n } \leq J(p)\parens[\big]{ h(x_1), \ldots, h(x_n) }.
    \end{equation}

    If the converse inequality in \eqref{eq:def:fol_homomorphism/predicates} holds for all predicates (including \( {\syneq} \)), we say that the homomorphism is \term{strong}.
  \end{thmenum}
\end{definition}
\begin{comments}
  ...
  \item The definition is based on \bycite[def. 3.3.1]{VanDalen2004LogicAndStructure}, with adjustments intuitionistic equality based on \bycite[711]{TroelstraVanDalen1988ConstructivismVol2}.
  ...
  \item For minimal logic, with the denotations modified as per \cref{def:minimal_fol_denotation}, we require also want \eqref{eq:def:fol_homomorphism/predicates} to hold for the falsum \( \synbot \).
\end{comments}
...
\begin{definition}\label{def:fol_structure_expansion}\mcite[def. 2.6.3]{Hinman2005Logic}
  ...
  We say that the \hyperref[def:fol_structure]{structure} \( \mscrX^+ = (X^+, I^+) \) over \( \Sigma^+ \) is an \term{expansion} of \( \mscrX = (X, I) \) over \( \Sigma^+ \) if \( X = X^+ \) and if the interpretations of all symbols in \( \Sigma \) (and also of \( \syneq \) and possibly \( \synbot \)) coincide.
...
\begin{proposition}\label{thm:fol_structure_reduct_denotation}
  ...
  The \hyperref[def:fol_denotation]{denotations} of \( \mscrY \) and its \hyperref[def:fol_reduct_along_morphism]{reduct} \( \red_t(\mscrY) \) are compatible in the following sense:
  \begin{thmenum}
...
\begin{definition}\label{def:fol_institution}\mimprovised
  Suppose that we are given a \hyperref[def:grothendieck_universe]{Grothendieck universe} \( \mscrU \), which is safe to assume to be the smallest suitable one as explained in \cref{def:large_and_small_sets}. Fix also a complete \hyperref[def:truth_value_algebra]{truth value algebra} \( \BbbT \).
  ...
  \begin{thmenum}
    ...
    \begin{thmenum}
      \thmitem{def:fol_institution/models/obj} Let \( \ucat{Mod}(\Sigma) \) be the category of \( \mscrU \)-small \( \BbbT \)-valued \hyperref[def:fol_structure]{first-order structures} and \hyperref[def:fol_homomorphism]{their homomorphisms}.

      For \hyperref[con:minimal_logic]{minimal logic}, we must also modify structures as per \cref{def:minimal_fol_denotation}.
    ...
    Here \( \Bracks{\varphi}_I \) is the denotation from \cref{def:fol_denotation/formulas}. For minimal logic, we will also need the modifications from \cref{def:minimal_fol_denotation}.
  \end{thmenum}
\end{definition}
\begin{comments}
...
\begin{definition}\label{def:fol_semantics}\mimprovised
  ...
  We will label the semantics \enquote{classical}, \enquote{intuitionistic}, etc., based on \( \BbbT \), with \enquote{minimal semantics} using the modified denotation from \cref{def:minimal_fol_denotation}.
...
\begin{proposition}\label{thm:def:fol_function_respect}
  Let \( \mscrX = (X, I) \) and \( \mscrY = (Y, J) \) be \hyperref[def:fol_structure]{classical structures} over a common \hyperref[def:fol_signature]{signature} \( \Sigma \).
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \begin{equation*}
      ...
      \reloset {\eqref{eq:def:fol_denotation/formulas/eq}} =
...
\begin{proposition}\label{thm:def:elementary_embedding}
  \hyperref[def:elementary_embedding]{Elementary embeddings} for \hyperref[def:fol_structure]{classical structures} have the following basic properties:
  \begin{thmenum}
...
\begin{comments}
  ...
  \end{equation*}
  which can be simplified (as justified in \cref{alg:propositional_constant_collapse}).
...
\begin{lemma}\label{thm:fol_finite_structure_characterizing_formula}\mcite[thm. 2.3.11(ii)]{Hinman2005Logic}
  ...
  For this to hold, we either need to assume \hyperref[def:truth_value_algebra/classical]{classical semantics}, or we need \( \Sigma \) to contain no predicate symbols.
\end{lemma}
\begin{proof}
...
\begin{definition}\label{def:pure_equality}\mimprovised
  ...
  For this to truly be a theory of equality, we must restrict ourselves to classical structures.
\end{definition}

\begin{proposition}\label{thm:pure_equality_isomorphism}
  The \hyperref[def:pure_equality]{theory of pure equality} \( \op*{Th}_{\syneq} \) for \hyperref[def:fol_structure]{classical structures} has the following basic properties:
  \begin{thmenum}
...
\begin{proposition}\label{thm:def:fol_theory}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:def:fol_theory/model_theory_completeness} Assuming \hyperref[def:truth_value_algebra/classical]{classical semantics}, for every structure \( \mscrX \), the theory \( \op*{Th}(\mscrX) \) is \hyperref[def:complete_set_of_sentences]{complete}.
  \end{thmenum}
\end{proposition}
\begin{proof}
  ...
  \SubProofOf{thm:def:fol_theory/model_theory_completeness} Assume classical semantics and fix a structure \( \mscrX \).
