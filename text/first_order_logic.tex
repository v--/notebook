  ...
  \begin{thmenum}
    ...
    \begin{equation*}
      \op*{Bound}(\varphi) \coloneqq \begin{cases}
        ...
        \op*{Bound}(\psi) \cup \set{ \xi },              &\varphi = \quantifier{q}{\xi} \psi, q \in \op*{Quan}.
      \end{cases}
    \end{equation*}
    ...
    \begin{equation*}
      \op*{Free}(\varphi) \coloneqq \begin{cases}
        ...
        \op*{Free}(\psi) \setminus \set{ \xi },                  &\varphi = \quantifier{q}{\xi} \psi, q \in \op*{Quan}
      \end{cases}
    \end{equation*}
...
\begin{definition}\label{def:first_order_valuation}
  ...
  \begin{thmenum}
    \thmitem{def:first_order_valuation/variable_assignment}\mcite[def. 2.1.16]{Hinman2005} A \term{variable assignment} for the variables of \( \mscrL \) is any function \( v: \op*{Var} \to X \).
...
\begin{definition}\label{def:first_order_substitution}
  ...
  \begin{thmenum}
    \thmitem{def:first_order_substitution/propositional} Let \( \varphi \) be a \hyperref[def:propositional_grammar_schema/formula]{propositional formula} with variables \( \op*{Var}(\varphi) = \set{ P_1, \ldots, P_n } \). For brevity, denote \( V \coloneqq \op*{Var}(\varphi) \). Let \( \Theta = \set{ \theta_1, \ldots, \theta_n } \) be a set of \hyperref[def:first_order_syntax/formula]{first-order formulas}.
    ...
    \begin{equation}\label{eq:def:first_order_substitution/term_in_formula}
      \varphi[\kappa \mapsto \mu] \coloneqq \begin{cases}
        ...
        \psi_1[\kappa \mapsto \mu] \syncirc \psi_2[\xi \mapsto \mu],       &\varphi = \psi_1 \syncirc \psi_2, {\syncirc} \in \op*{Conn}, \\
        (\dagger),                                                         &\varphi = \quantifier Q {\xi} \psi, Q \in \op*{Quan},
      \end{cases}
    \end{equation}
    where
    \begin{subequations}
      \begin{empheq}[left=(\dagger) \coloneqq \empheqlbrace]{align}
        &\varphi,                                                                        && \xi \in \op*{Var}(\kappa), \label{eq:def:first_order_substitution/term_in_formula/quantifiers/trivial} \\
        ...
        &\quantifier Q {\eta} \parens[\Big]{\psi[\xi \mapsto \eta][\kappa \mapsto \mu]}, && \xi \not\in \op*{Var}(\kappa) \T{and} \xi \in \op*{Var}(\mu) \T{and} &\label{eq:def:first_order_substitution/term_in_formula/quantifiers/renaming} \\
...
\begin{comments}
  ...
  \item The rule \eqref{eq:def:first_order_substitution/term_in_formula/quantifiers/trivial} may seem redundant, but when doing inductive proofs (e.g. our proof of \fullref{thm:renaming_assignment_compatibility}), we usually need to separately consider the cases where \( \xi \in \op*{Var}(\kappa) \) and \( \xi \not\in \op*{Var}(\kappa) \setminus \op*{Var}(\mu) \) and the rule \eqref{eq:def:first_order_substitution/term_in_formula/quantifiers/direct} being trivial simplifies the proofs.
...
\begin{proposition}\label{thm:renaming_assignment_compatibility}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:renaming_assignment_compatibility/formulas} For any formula \( \varphi \), any variable \( \xi \) and any other variable \( \eta \) not in \( \op*{Var}(\varphi) \) we have
...
\begin{proof}
  ...
  \begin{itemize}
    \item \( \xi \in \op*{Free}(\mu) \).
    \item \( \eta \not\in \op*{Var}(\kappa) \cup \op*{Var}(\mu) \cup \op*{Var}(\psi) \).
    \item The inductive hypothesis holds for \( \psi \).
  \end{itemize}

  We will only show the case \( \varphi = \qforall \xi \psi \) since the existential case is handled similarly. Since \( \xi \in \op*{Free}(\mu) \), we have
  \begin{equation*}
...
\begin{proposition}\label{thm:pulling_quantifiers_out}
  For \( Q \in \op*{Quan} \) and \( \syncirc \in \set{ \synvee, \synwedge } \), if \( \eta \) is not free in \( \varphi \) nor \( \psi \), then
...
\begin{proof}
  ...
  Fix some \hyperref[def:first_order_structure]{structure} \( \mscrX = (X, I) \) and some \hyperref[def:first_order_valuation/variable_assignment]{variable assignment} \( v: \op*{Var} \to X \). Since \( \eta \) is not free in \( \psi \),
  \begin{balign*}
...
\begin{proof}
  ...
  To avoid the case where \( \xi \in \op*{Var}(\varphi) \) for the purposes for \fullref{thm:renaming_assignment_compatibility}, we substitute it with \( \eta \) that is not a variable in neither \( \tau \) nor \( \varphi \). Then
  \begin{align*}
...
\begin{example}\label{ex:def:first_order_natural_deduction_system/eigenvariables}
  ...
  \begin{thmenum}
    ...
    \thmitem{ex:def:first_order_natural_deduction_system/eigenvariables/invalid_existence} Another invalid proof, in case \( \xi \in \op*{Free}(\varphi) \), is
    \begin{equation*}
      \begin{prooftree}
        \hypo{ \qexists \xi \varphi }
