...
\begin{definition}\label{def:fol_denotation}\mimprovised
  Fix a \hyperref[def:truth_value_algebra]{truth value algebra} \( \BbbH \). Let \( \mscrX = (X, I) \) be a \hyperref[def:fol_structure]{first-order structure} over \( \Sigma \), and let \( v: \op*{Var} \to X \) be a \hyperref[def:fol_variable_assignment]{variable assignment}.
  ...
  \begin{thmenum}
    ...
    \begin{itemize}
      \item In \eqref{eq:def:fol_denotation/formulas/eq}, \( \delta \) is the \hyperref[def:kronecker_delta]{Kronecker delta function} (written with prefix notation).
    ...
    If the denotation \( \Bracks{\varphi}_\mscrX^v \) does not depend on \( v \), we use the simpler notation \( \Bracks{\varphi}_\mscrX \). Unlike for terms, this can happen even if \( \varphi \) is not a \hyperref[def:closed_fol_formula]{closed formula} --- see \cref{ex:def:fol_denotation/independent}.
    ...
    Unless otherwise noted, we assume that \( x_1, \ldots, x_n \) are precisely the \hi{free} variables of \( \tau \) ordered lexicographically.
  \end{thmenum}
\end{definition}
\begin{comments}
