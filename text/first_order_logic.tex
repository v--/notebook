...
\end{definition}
\begin{comments}
  \item The soundness requirement in general logics from \cref{def:general_logic} is satisfied due to \fullref{thm:fol_natural_deduction_soundness}.
...
\begin{definition}\label{def:fol_signature}\mcite[282]{Meseguer1989GeneralLogics}
  ...
  \begin{thmenum}
    \thmitem{def:fol_signature/function} A possibly empty \hyperref[def:formal_language/alphabet]{alphabet} \( \op*{Fun}_\Sigma \), whose elements we call \term[ru=функциональные символы (\cite[84]{Герасимов2014Вычислимость})]{function symbols}.

    \thmitem{def:fol_signature/predicate} A possibly empty alphabet \( \op*{Pred}_\Sigma \), whose elements we call \term[ru=реляционные символы (\cite[84]{Герасимов2014Вычислимость})]{relation symbols}.
...
\begin{definition}\label{def:fol_signature_category}\mcite[282]{Meseguer1989GeneralLogics}
  We define the \hyperref[def:category]{category} \( \cat{Sign} \) of \hyperref[def:fol_signature]{first-order signatures} as follows:
  \begin{thmenum}
    \thmitem{def:fol_signature_category/objects} The objects are, unsurprisingly, the signatures.

    \thmitem{def:fol_signature_category/morphisms} A signature morphism \( t: \Sigma \to \Sigma' \) simply maps the function symbols of \( \Sigma \) to function symbols of \( \Sigma' \) with the same arity, and analogously for predicate symbols.
...
\end{definition}
\begin{comments}
  \item Signature morphisms will be used to define an institution in \cref{def:fol_institution}. We first need to construct the corresponding sentences and translations.
\end{comments}
...
\begin{remark}\label{rem:fol_application_notation}
  ...
  Examples of condensed notation are mostly restricted to unary operators, where writing parentheses adds little benefit. This includes nearly all unary operators from \fullref{ch:set_theory}, as well as various kinds of \hyperref[def:categorical_involution]{involutions}, \hyperref[def:abstract_closure_operator]{closure operators}, and the \hyperref[sec:trigonometric_functions]{trigonometric functions}.
...
\begin{proposition}\label{thm:fol_subterm_characterization}
...
\end{proposition}
\begin{proof}
  We can adapt the proof of \cref{thm:propositional_subformula_characterization}. There are no variable binders, so we are safe from the \( \muplambda \)-term nuances in \cref{thm:lambda_subterm_characterization}.
\end{proof}
...
\begin{definition}\label{def:fol_closed_term}\mcite[def. 2.3.8]{VanDalen2004LogicAndStructure}
...
\end{definition}
\begin{comments}
  \item Closed terms are also called \enquote{ground terms}, for example by \textcite[def. 3.1.5]{BaaderNipkow2012TermRewriting}.
\end{comments}

\begin{algorithm}[Signature translation of first-order terms]\label{alg:fol_term_signature_translation}
  Given a \hyperref[def:fol_signature_category/morphisms]{signature morphism} \( t: \Sigma \to \Sigma' \), we can easily translate any \hyperref[def:fol_term]{first-order term} over \( \Sigma \) to \( \Sigma' \):
  \begin{equation*}
...
\begin{remark}\label{rem:fol_formula_conventions}
  ...
  \begin{thmenum}
    ...
    The particular choice of abbreviations is justified in \cref{thm:fol_quantifier_duality_relativized}. It is actively exploited in \fullref{alg:hol_to_fol}.

    \thmitem{rem:fol_formula_conventions/unique_existence} We will use the abbreviation in \cref{rem:fol_exists_unique_abbreviation} to emulate the unique existence quantifier \( \qExists x \varphi \) discussed in \cref{con:description_operator/unique_existence}.

    \thmitem{rem:fol_formula_conventions/necessary_signature} We will build our theories with minimal signatures and then legitimize new symbols via \hyperref[def:fol_definitional_extension]{definitional extensions}. This is formally justified by \cref{thm:fol_definitional_extension_conservative}, which shows that such an extension is necessarily \hyperref[def:fol_theory/conservative]{conservative}.
...
\begin{proposition}\label{thm:fol_subformula_characterization}
...
\begin{algorithm}[Signature translation of first-order formulas]\label{alg:fol_formula_signature_translation}
  Given a \hyperref[def:fol_signature_category/morphisms]{signature morphism} \( t: \Sigma \to \Sigma' \), we can translate any \hyperref[def:fol_formula]{first-order formula} over \( \Sigma \) to \( \Sigma' \) with the aid of \fullref{alg:fol_term_signature_translation}:
  \begin{equation*}
...
\begin{definition}\label{def:fol_variable_occurrence}\mimprovised
  ...
  We analogously call a variable occurrence of \( x \) \term[ru=свободное (вхождение) (\cite[88]{Герасимов2014Вычислимость}), en=free occurrence (\cite[13]{Shoenfield1967MathematicalLogic})]{free} in \( \varphi \) if the root of the AST can be reached without passing through a quantifier. If an occurrence is not free, we say that it is \term[ru=связоное (вхождение) (\cite[88]{Герасимов2014Вычислимость}), en=bound occurrence \cite[13]{Shoenfield1967MathematicalLogic}]{bound}.
\end{definition}

\begin{definition}\label{def:fol_variable_freeness}\mcite[88]{Герасимов2014Вычислимость}
  In analogy with how we handled variable freeness in \( \muplambda \)-terms defined in \cref{def:lambda_variable_freeness}, we say that a variable \( x \) is \term[ru=свободная (переменная), en=free (variable) (\cite[16]{Shoenfield1967MathematicalLogic})]{free} in a first-order formula \( \varphi \) if \( x \) has at least one \hyperref[def:fol_variable_occurrence]{free occurrence} in \( \varphi \) and \term[ru=связанная (переменная), en=bound (variable) (\cite[16]{Shoenfield1967MathematicalLogic})]{bound} if it has a \hyperref[def:lambda_variable_occurrence]{bound occurrence} in \( M \).
\end{definition}
\begin{comments}
...
\begin{definition}\label{def:fol_closed_formula}\mcite[def. 2.3.8]{VanDalen2004LogicAndStructure}
...
\begin{remark}\label{rem:fol_structure_notation}
  ...
  One convention is to denote the universes via compatible letters like we did with \( X \) and \( \mscrX \), or like we would do with \( G \) and \( \mscrG \). Such notation is used, for example, by \textcite[def. II.1.2]{Эдельман1975Логика}.
...
\end{definition}
\begin{comments}
  \item \Textcite{Andrews2002Logic} introduces no term for modified assignments, and neither do \cite[rem. 2.2.17]{Hinman2005Logic} or \cite[def. 2.2.9]{Герасимов2014Вычислимость}.

  \item Modified assignments are needed for defining denotations for quantifier formulas in \cref{alg:fol_formula_denotation}, but are also useful on other places like \cref{thm:fol_formula_semantics_of_compatible_substitutions}.
\end{comments}
...
\begin{algorithm}[First-order term denotation]\label{alg:fol_term_denotation}\mimprovised
  ...
  If \( \tau \) is a \hyperref[def:fol_closed_term]{closed term}, the \hyperref[alg:fol_formula_denotation]{denotation} \( \Bracks{\tau}_\mscrX^v \) does not depend on \( v \), and we simply write \( \Bracks{\tau}_\mscrX \).
\end{algorithm}
\begin{comments}
...
\begin{definition}\label{def:kronecker_delta}\mimprovised
  ...
  The definition naturally fits in \hyperref[def:truth_value_algebra]{truth value algebras}, where it becomes
...
\begin{algorithm}[First-order formula denotation]\label{alg:fol_formula_denotation}\mimprovised
  ...
  \begin{empheq}[left={\Bracks{\varphi}_\mscrX^v} \coloneqq \empheqlbrace]{align}
    &\semtop,                                                                         &&\varphi = \syntop,                   \label{eq:alg:fol_formula_denotation/top} \\
    &\sembot,                                                                         &&\varphi = \synbot,                   \label{eq:alg:fol_formula_denotation/bot} \\
    &\oline{\Bracks{\psi}_\mscrX^v},                                                  &&\varphi = \synneg \psi,              \label{eq:alg:fol_formula_denotation/neg} \\
    &\Bracks{\psi}_\mscrX^v \bincirc \Bracks{\theta}_\mscrX^v,                        &&\varphi = \psi \syncirc \theta,      \label{eq:alg:fol_formula_denotation/conn} \\
    &\delta\parens[\big]{ \Bracks{N}_\mscrX^v, \Bracks{K}_\mscrX^v },                 &&\varphi = (N \syneq K),              \label{eq:alg:fol_formula_denotation/eq} \\
    &I(p)\parens[\big]{ \Bracks{\tau_1}_\mscrX^v, \ldots, \Bracks{\tau_n}_\mscrX^v }, &&\varphi = p(\tau_1, \ldots, \tau_n), \label{eq:alg:fol_formula_denotation/pred} \\
    &\bigwedge\set[\big]{ \Bracks{\psi}_\mscrX^{v_{x \mapsto a}} \given* a \in X },   &&\varphi = \qforall x \psi,           \label{eq:alg:fol_formula_denotation/forall} \\
    &\bigvee\set[\big]{ \Bracks{\psi}_\mscrX^{v_{x \mapsto a}} \given* a \in X },     &&\varphi = \qexists x \psi,           \label{eq:alg:fol_formula_denotation/exists}
  \end{empheq}

  Most of the cases above are simply the familiar propositional denotation rules from \fullref{alg:propositional_denotation}.
  \begin{itemize}
    \item In \eqref{eq:alg:fol_formula_denotation/eq}, \( \delta \) is \hyperref[def:kronecker_delta]{Kronecker's delta function}. Only this case must be generalized for \hyperref[con:intuitionistic_logic]{intuitionistic logic} --- see \cref{rem:intuitionistic_equality}.

    \item In \eqref{eq:alg:fol_formula_denotation/pred}, we utilize predicate functions from the structure's interpretation.

    \item The cases \eqref{eq:alg:fol_formula_denotation/forall} and \eqref{eq:alg:fol_formula_denotation/exists} evaluates \( \psi \) using a \hyperref[def:fol_variable_assignment]{modified assignment} parameterized by the elements of \( X_\sigma \). We see here why it is important that the \hyperref[def:truth_value_algebra/classical]{classical truth value algebra} is \hyperref[def:heyting_algebra/complete]{complete}.
  \end{itemize}

  If the denotation \( \Bracks{\varphi}_\mscrX^v \) does not depend on \( v \), we use the simpler notation \( \Bracks{\varphi}_\mscrX \). Unlike for terms, this can happen even if \( \varphi \) is not a \hyperref[def:fol_closed_formula]{closed formula} --- see \cref{rem:implicit_quantification_and_deduction}.
\end{algorithm}
\begin{comments}
  \item The term \enquote{denotation} is inspired by Russell's theory of denotations discussed in \cref{con:denotation}. It is used inconsistently in the literature. See \cref{rem:model_theory_structure_terminology}.
...
\begin{definition}\label{def:fol_parameterized_formula_denotation}
  We can define \hyperref[con:parametrized_denotation]{parametrized denotations} based on \fullref{alg:fol_formula_denotation}. Assuming that \( \op*{Free}(\varphi) \subseteq \set{ x_1, \ldots, x_n } \), consider the function
  \begin{equation*}
    \begin{aligned}
      &\Bracks{\tau}_\mscrX: X^n \to X, \\
      &\Bracks{\tau}_\mscrX(a_1, \ldots, a_n) \coloneqq \Bracks{\tau}_\mscrX^v,
  ...
  Unless otherwise noted, we assume that \( x_1, \ldots, x_n \) are precisely the \emph{free} variables of \( \varphi \) ordered lexicographically.
\end{definition}
...
\begin{remark}\label{rem:intuitionistic_equality}
  ...
  We will present other approaches. First, we need to generalize \hyperref[def:fol_congruence]{first-order congruences}. Given a complete \hyperref[def:truth_value_algebra]{truth value algebra} \( \BbbT \) and a \hyperref[def:fol_structure]{structure} \( \mscrX = (X, I) \), consider some function \( \delta: X^2 \to \BbbT \) such that the following conditions hold:
...
\begin{definition}\label{def:fol_homomorphism}\mcite[def. 3.3.1]{VanDalen2004LogicAndStructure}
  ...
  \begin{thmenum}
    ...
    \begin{equation}\label{eq:def:fol_homomorphism/predicates}
      I(p)\parens[\big]{ a_1, \ldots, a_n } \leq J(p)\parens[\big]{ h(a_1), \ldots, h(a_n) }.
    \end{equation}
...
\end{definition}
\begin{comments}
  \item We may use the notation \( h: \mscrX \to \mscrY \) in accordance with the general practice for category morphisms described in \cref{rem:category_as_graph}.

  \item Homomorphisms that are not strong arise naturally --- for example \hyperref[def:cartesian_product_projection]{projections} in \hyperref[def:fol_direct_product]{direct products}.

  The inequality for predicates leads precisely to the desired notion of homomorphism for \hyperref[def:partially_ordered_set]{partially ordered sets} and \hyperref[def:directed_graph]{directed graphs}. The adjective \enquote{strong} is based on Knauer's \enquote{strong graph homomorphisms} defined in \cite[def. 1.4.3]{Knauer2019AlgebraicGraphTheory}.

  \item A homomorphism depends crucially on the underlying signature. If we add new symbols to the signature, for example via \hyperref[def:fol_definitional_extension]{definitional extensions}, we automatically obtain more restrictive homomorphisms.

  \item \Cref{thm:fol_homomorphism_as_substructure} provides a condition equivalent to \cref{def:fol_homomorphism/functions}, while for strong homomorphism \cref{thm:def:fol_function_respect/atomic} packs the two conditions into one.

  \textcite[def. 3.3.1]{VanDalen2004LogicAndStructure} and \textcite[49]{Мальцев1970АлгебраическиеСистемы} define homomorphisms like we do, and neither discuss strong homomorphisms. \Textcite[def. 2.3.26(i)]{Hinman2005Logic} considers strong homomorphisms by default (and simply calls them \enquote{homomorphisms}), while later in \cite[rem. 2.3.27]{Hinman2005Logic} he mentions that our weaker homomorphisms are also useful and calls them \enquote{positive homomorphisms}.
\end{comments}
...
\begin{definition}\label{def:fol_structure_expansion}\mcite[def. 2.6.3]{Hinman2005Logic}
  ...
  We say that the \hyperref[def:fol_structure]{structure} \( \mscrX^+ = (X^+, I^+) \) over \( \Sigma^+ \) is an \term{expansion} of \( \mscrX = (X, I) \) over \( \Sigma^+ \) if \( X = X^+ \) and if the interpretations of all symbols in \( \Sigma \) coincide.
...
\begin{comments}
  ...
  \item The definition is based on \cite[def. 2.6.3]{Hinman2005Logic}, with small adjustments made for the intuitionistic treatment of formal equality.
\end{comments}
...
\begin{definition}\label{def:fol_reduct_along_morphism}\mcite[example 5]{MossakowskiKrumnackMaibaum2015DerivedSignatureMorphisms}
  For every \hyperref[def:fol_signature_category/morphisms]{signature morphism} \( t: \Sigma \to \Sigma' \) and every \hyperref[def:fol_structure]{structure} \( \mscrY = (Y, J) \) over \( \Sigma' \), we define the \term{reduct} of \( \mscrY \) along \( t \) as the structure \( \red_t(\mscrY) \coloneqq (Y, J_t) \), where, for every function or predicate symbol \( s \) in \( \Sigma \), and also for \( {\syneq} \),
  \begin{equation}\label{eq:def:fol_reduct_along_morphism/interpretation}
    J_t(s) \coloneqq J(t(s)).
  \end{equation}

  When defining institutions in \cref{def:fol_institution/models/hom}, we will need to extend \( \red_t \) to a \hyperref[def:functor]{functor} by also reducing \hyperref[def:fol_homomorphism]{homomorphisms}. Given structures \( \mscrY = (Y, J) \) and \( \mscrZ = (Z, K) \) over \( \Sigma' \), for every \( g: Y \to Z \), we define \( \red_t(g) \) as \( g \) itself.
\end{definition}
\begin{comments}
  \item These induced reducts generalize \cref{def:fol_structure_expansion} to cases where \( \Sigma' \) is not an extension of \( \Sigma \). They are needed for defining the institutions in \cref{def:fol_institution}.

  \item Reducts are also compatible with \hyperref[def:fol_theory/morphism]{theory morphisms} --- see \cref{thm:def:fol_theory_morphism_reduct}.
...
\begin{proposition}\label{thm:fol_structure_reduct_denotation}
  Fix a \hyperref[def:fol_signature_category/morphisms]{signature morphism} \( t: \Sigma \to \Sigma' \) and a \hyperref[def:fol_structure]{structure} \( \mscrY = (Y, J) \) over \( \Sigma' \). Let \( v: \op*{Var} \to Y \) be a variable assignment.
...
\begin{definition}\label{def:fol_institution}\mimprovised
  ...
  \begin{thmenum}
    \thmitem{def:fol_institution/signatures} We take the category of signatures \( \cat{Sign} \) from \cref{def:fol_signature_category}.

    \thmitem{def:fol_institution/sentences} We define the \hyperref[def:institution/sentences]{sentence functor} as follows:
    \begin{thmenum}
      \thmitem{def:fol_institution/sentences/obj} Let \( \op*{Sen}(\Sigma) \) be the set of \hyperref[def:fol_closed_formula]{closed formulas} over \( \Sigma \).

      \thmitem{def:fol_institution/sentences/hom} Let \( \op*{Sen}(t: \Sigma \to \Sigma') \) be the function \( \varphi \mapsto \varphi[t] \), which translates formulas using \fullref{alg:fol_formula_signature_translation}.
    \end{thmenum}

    \thmitem{def:fol_institution/models} We define the \hyperref[def:institution/models]{model functor} as follows:
    \begin{thmenum}
      \thmitem{def:fol_institution/models/obj} Let \( \cat{Mod}(\Sigma) \) be the category of \hyperref[def:small_set]{small} \hyperref[def:fol_structure]{first-order structures} and \hyperref[def:fol_homomorphism]{their homomorphisms}.

      \thmitem{def:fol_institution/models/hom} For the signature translation \( s: \Sigma \to \Sigma' \), we define the \hyperref[def:fol_institution/models/hom]{model reduct functor} \( \cat{Mod}(s): \cat{Mod}(\Sigma') \to \cat{Mod}(\Sigma) \) via the operator \( \red_t \) from \cref{def:fol_reduct_along_morphism}.
    \end{thmenum}
...
\end{definition}
\begin{comments}
  \item The cardinality restrictions discussed in \cref{rem:language_alphabet_cardinality} ensure that the formulas are \hyperref[def:small_set]{small sets} and thus they do not affect the \hyperref[def:small_category]{size} of the categories above.
\end{comments}
\begin{defproof}
  Fix a signature morphism \( s: \Sigma \to \Sigma' \) and a structure \( \mscrY = (Y, J) \) over \( \Sigma' \). We must verify \eqref{eq:def:institution/satisfaction}, which we restate here:
  \begin{equation*}
    \mscrY \vDash_{\Sigma'} \underbrace{\op*{Sen}(s)(\varphi)}_{\varphi[s]}
    \quad\T{if and only if}\quad
    \underbrace{\cat{Mod}(s)}_{\red_t}(\mscrY) \vDash_\Sigma \varphi
  \end{equation*}
...
\begin{definition}\label{def:fol_semantics}\mimprovised
  ...
  \begin{thmenum}
    ...
    We say that \( \mscrX \) is a \term[ru=модель (\cite[2.6.21]{Герасимов2014Вычислимость}), en=model (\cite[def. def.2.2.13(ii)]{Hinman2005Logic})]{model} \emph{of} the set of closed formulas \( \Gamma \) and write \( \mscrX \vDash \Gamma \) if and only if \( \Bracks{\varphi}_\mscrX = \semtop \) for every (closed) formula \( \varphi \) in \( \Gamma \).
    ...
    \thmitem{def:fol_semantics/tautology} Based on \cref{def:tautology}, We call the closed formula \( \varphi \) a \term[ru=тавтология (\cite[def. 2.4.1]{Герасимов2014Вычислимость}), en=tautology (\cite[97]{Hinman2005Logic})]{tautology} if \( \varphi \) is satisfied in every model, and a \term[ru=противоречивая формула (\cite[def. 2.4.1]{Герасимов2014Вычислимость})]{contradiction} if it is satisfied in no model.
...
\begin{theorem}[First-order semantic deduction theorem]\label{thm:fol_semantic_deduction_theorem}
  In the \hyperref[def:fol_institution]{first-order institution}, we have
  \begin{equation*}
...
\paragraph{Propositional formula translation}
...
\begin{algorithm}[Propositional formula translation]\label{alg:fol_propositional_formula_translation}
  We extend the \hyperref[def:atomic_fol_propositional_formula_translation]{atomic formula translation} \( \Bbbt \) to an operation \( \varphi \mapsto \varphi[\Bbbt] \) on compound propositional formulas via \hyperref[rem:straightforward_traversal]{straightforward traversal} similarly to how it is done in \fullref{alg:propositional_substitution}.
\end{algorithm}
\begin{comments}
  \item This algorithm allows us to reuse much of what we proved for propositional formulas. \Cref{thm:fol_propositional_formula_translation_entailment} shows that \hyperref[def:propositional_semantics/entailment]{propositional semantic entailment} gives rise to \hyperref[def:fol_semantics/entailment]{first-order semantic entailment}, and \Cref{thm:fol_propositional_formula_translation_derivation} shows the same for \hyperref[def:fol_natural_deduction_consequence]{syntactic derivation} based on natural deduction.
...
\begin{proposition}\label{thm:fol_propositional_formula_translation_semantics}
  Fix a \hyperref[def:fol_structure]{first-order structure} \( \mscrX \), a \hyperref[def:fol_variable_assignment]{variable assignment} \( v \) into \( \mscrX \) and an \hyperref[def:atomic_fol_propositional_formula_translation]{atomic translation} \( \Bbbt \). For simplicity of exposition, suppose that \( \Bbbt \) is defined for all propositional variables under consideration.

  Define the classical \hyperref[def:propositional_interpretation]{propositional interpretation}
...
\begin{corollary}\label{thm:fol_propositional_formula_translation_entailment}
  ...
  If the set \( \Gamma \) of \hyperref[def:propositional_formula]{propositional formulas} \hyperref[def:propositional_semantics/entailment]{semantically entails} \( \varphi \), then the \hyperref[def:fol_quantifier_closure]{universal closures} \( \cl_\forall \Gamma[\Bbbt] \) of the formulas translated via \fullref{alg:fol_propositional_formula_translation} \hyperref[def:fol_semantics/entailment]{semantically entails} \( \cl_\forall \varphi[\Bbbt] \).
\end{corollary}
\begin{comments}
  \item This is the semantic counterpart to \cref{thm:fol_propositional_formula_translation_derivation}.
\end{comments}
\begin{proof}
  Suppose that \( \Gamma \vDash \varphi \).

  Let \( \mscrX \) be a structure satisfying \( \cl_\forall \Gamma[\Bbbt] \). \Cref{thm:fol_universal_validity_iff_closure_validity} implies that every formula in \( \Gamma[\Bbbt] \) is \hyperref[def:fol_universal_validity]{universally valid} in \( \mscrX \). Fix any variable assignment \( v \) in \( \mscrX \).

  \Cref{thm:fol_propositional_formula_translation_semantics} implies that the specific interpretation \( I_v(p) \coloneqq \Bracks{p[\Bbbt]}_\mscrX^v \) also satisfies \( \Gamma \). Then \( I_v \) also satisfies \( \varphi \). Using \cref{thm:fol_propositional_formula_translation_semantics} in the other direction, we conclude that \( v \) satisfies \( \varphi \).

  Generalizing on \( v \), we conclude that \( \varphi \) is universally valid in \( \mscrX \). \Cref{thm:fol_universal_validity_iff_closure_validity} implies that the closure \( \cl_\forall \varphi \) is valid in \( \mscrX \).
\end{proof}

\begin{remark}\label{rem:propositional_logic_as_first_order_logic}
  \Fullref{alg:fol_propositional_formula_translation} demonstrates how the theorems of \hyperref[def:propositional_logic]{propositional logic} can be usefully translated into first-order logic.

  We can do the converse --- we can use first-order logic to study propositional logic.

  Consider thea signature consisting of one predicate symbol for every propositional variable. We have formalized propositional variables in \cref{def:propositional_formula} as \hyperref[def:variable_identifier]{small Latin identifiers}, similarly to how we have formalized individual variables in \cref{def:fol_term}. Thus, to avoid clashes, we must work with the \hyperref[con:syntax_fragment]{fragment} of first-order formulas without individual variables.

  With this signature, the universe of a \hyperref[def:fol_structure]{first-order structure} \( \mscrX = (X, I) \) becomes irrelevant, and its interpretation becomes a propositional interpretation in the sense of \cref{def:propositional_interpretation}.
...
\begin{algorithm}[First-order formula dualization]\label{alg:fol_formula_dualization}
  ...
  \begin{equation}\label{eq:alg:fol_formula_dualization/quantifiers}
    \oline{M}(\quantifier Q x \psi) \coloneqq \quantifier {Q^\oppos} x \oline{M}(\psi).
  \end{equation}
...
\begin{defproof}
  ...
  \begin{balign*}
    \Bracks{\oline{M}(\qforall x \psi)}_\mscrX^v
    &=
    \Bracks{\qexists x \oline{M}(\psi)}_\mscrX^v
    \reloset {\eqref{eq:alg:fol_formula_denotation/exists}} = \\ &=
    \bigvee\set[\big]{ \Bracks{\oline{M}(\psi)}_\mscrX^{v_{x \mapsto a}} \given* a \in X }
    \reloset {\T{ind.}} = \\ &=
  ...
  The case where \( \varphi = \qexists x \psi \) is similar.
\end{defproof}
...
\begin{lemma}\label{thm:fol_quantifiers_and_semantic_equivalence}
  Two \hyperref[def:fol_formula]{first-order formulas} \( \varphi \) and \( \psi \) are \hyperref[def:fol_semantics/equivalence]{semantically equivalent} if and only if \( \quantifier Q x \varphi \) and \( \quantifier Q x \psi \) are.
...
\begin{corollary}\label{thm:fol_quantifier_duality_relativized}
  ...
  \begin{equation}\label{eq:thm:fol_quantifier_duality_relativized}
...
\begin{definition}\label{def:fol_quantifier_closure}\mcite[def. 2.2.28]{Hinman2005Logic}
  ...
  Using the \hyperref[def:lexicographic_order]{lexicographic order} on the variables, we may pick a canonical closure, which we will simply call \enquote{the} (lexicographic) universal closure of \( \varphi \) and denote it by \( \cl_\forall \varphi \).
...
\end{definition}
\begin{comments}
  \item \hyperref[def:fol_closed_formula]{Closed formulas} are universally valid, but the notion is more general --- see \cref{rem:implicit_quantification_and_deduction}.
...
\begin{remark}\label{rem:implicit_quantification_and_deduction}
  For first-order logic, we have defined the \hyperref[def:institution]{institution} in \cref{def:fol_institution} only for \hyperref[def:fol_closed_formula]{closed first-order formulas}. It is tempting to allow formulas with free variables via \hyperref[def:fol_implicit_universal_quantification]{implicit universal quantification}.
  ...
  \begin{balign*}
    ...
    \reloset {\eqref{eq:alg:fol_formula_denotation/pred}} = \\ &=
