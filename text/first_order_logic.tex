...
\begin{definition}\label{def:intuitionistic_equivalence_relation}\mimprovised
  ...
  Consider the function \( {\semeq}: A^2 \to \BbbT \) on some set \( A \). We will call \( {\semeq} \) a \( \BbbT \)-valued \term{equivalence relation} if it satisfies the following conditions:
  \begin{thmenum}
...
\begin{definition}\label{def:fol_structure}\mimprovised
  ...
  \begin{thmenum}
    ...
    \thmitem{def:fol_structure/interpretation} A map \( I \), called an \term{interpretation}, that sends each \( n \)-ary function symbol to a function \( X^n \to X \) and every \( n \)-ary predicate to a function \( X^n \to \BbbT \), and \( \syneq \) to a \( \BbbT \)-valued \hyperref[def:intuitionistic_equivalence_relation]{equivalence relation} \( {\semeq} \) on \( X \).
  ...
  We will label the structure \enquote{classical}, \enquote{intuitionistic}, etc., based on \( \BbbT \). We presuppose that equality is standard in classical structures.
...
\begin{comments}
  ...
  \item Modified assignments are needed for defining denotations for quantifier formulas in \cref{def:fol_denotation}, but are also useful on other places like \cref{thm:renaming_assignment_compatibility}.
\end{comments}
