...
\begin{definition}\label{def:first_order_language}\mcite[def. 2.1.2]{Hinman2005}
  ...
  \begin{description}
    ...
    \begin{thmenum}[series=def:first_order_language]
      ...
      \thmitem{def:first_order_language/quantifiers} The set \( \boldop{Quan} \) consisting of the \term[ru=квантор общости (\cite[61]{Эдельман1975})]{universal quantifier} \enquote{\( \synforall \)} and the \term[ru=квантор существования (\cite[61]{Эдельман1975})]{existential quantifier} \enquote{\( \synexists \)}.
  ...
  \begin{thmenum}
    ...
    \begin{equation*}
      \boldop{Bound}(\varphi) \coloneqq \begin{cases}
        ...
        \boldop{Bound}(\psi_1) \cup \boldop{Bound}(\psi_2), &\varphi = \psi_1 \syncirc \psi_2, {\syncirc} \in \boldop{Conn}, \\
        \boldop{Bound}(\psi) \cup \set{ \xi },              &\varphi = \quantifier{q}{\xi} \psi, q \in \boldop{Quan}.
      \end{cases}
    \end{equation*}
    ...
    \begin{equation*}
      \boldop{Free}(\varphi) \coloneqq \begin{cases}
        ...
        \boldop{Free}(\psi_1) \cup \boldop{Free}(\psi_2),           &\varphi = \psi_1 \syncirc \psi_2, {\syncirc} \in \boldop{Conn}, \\
        \boldop{Free}(\psi) \setminus \set{ \xi },                  &\varphi = \quantifier{q}{\xi} \psi, q \in \boldop{Quan}
      \end{cases}
    \end{equation*}
  ...
  \begin{thmenum}
    ...
    \begin{equation}\label{eq:def:first_order_valuation/formula_valuation}
      \Bracks{\varphi}_v \coloneqq \begin{cases}
        ...
        \Bracks{\psi_1}_v \relcirc \Bracks{\psi_2}_v,                    &\varphi = \psi_1 \syncirc \psi_2, {\syncirc} \in \boldop{Conn}, \\
  ...
  \begin{thmenum}
    ...
    \begin{equation}\label{eq:def:first_order_substitution/propositional}
      \varphi[V \mapsto \Theta] \coloneqq \begin{cases}
        ...
        \psi_1[V \mapsto \Theta] \syncirc \psi_2[V \mapsto \Theta], &\varphi = \psi_1 \syncirc \psi_2, {\syncirc} \in \boldop{Conn}.
      \end{cases}
    \end{equation}
    ...
    \begin{equation}\label{eq:def:first_order_substitution/term_in_formula}
      \varphi[\kappa \mapsto \mu] \coloneqq \begin{cases}
        ...
        \psi_1[\kappa \mapsto \mu] \syncirc \psi_2[\xi \mapsto \mu],       &\varphi = \psi_1 \syncirc \psi_2, {\syncirc} \in \boldop{Conn}, \\
        (\dagger),                                                         &\varphi = \quantifier Q {\xi} \psi, Q \in \boldop{Quan},
      \end{cases}
    \end{equation}
    where
    \begin{subequations}
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \item If \( \varphi = \psi_1 \syncirc \psi_2, {\syncirc} \in \boldop{Conn} \) and if the inductive hypothesis holds for both \( \psi_1 \) and \( \psi_2 \), then
    \begin{equation*}
      \Bracks[\Big]{ \psi[V \mapsto \Theta] }_v
      =
...
\begin{proposition}\label{thm:pulling_quantifiers_out}
  For \( Q \in \boldop{Quan} \) and \( \syncirc \in \set{ \synvee, \synwedge } \), if \( \eta \) is not free in \( \varphi \) nor \( \psi \), then
    ...
    \begin{equation*}
      P_3(\varphi) \coloneqq \begin{cases}
        ...
        \quantifier Q \eta P_3 \parens[\Big]{ \varphi[\xi \mapsto \eta] \syncirc \theta }, &\varphi = \parens[\Big]{ \quantifier Q \xi \psi } \syncirc \theta, \\
                                                                                           &Q \in \boldop{Quan}, \quad \syncirc \in \set{ \synvee, \synwedge }, \\
