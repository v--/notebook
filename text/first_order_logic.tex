...
\begin{definition}\label{def:first_order_syntax}
  ...
  \begin{thmenum}
    ...
    \begin{bnf*}
      ...
      \bnfmore                  {f \thickspace \} \thickspace \scriptsize{\T*{Standalone rule for each} f \in \boldop{Fun} \T*{where} \# f = 0}} \\
...
\begin{definition}\label{def:first_order_semantics}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:first_order_semantics/structure_entailment} If \( \Gamma \vDash_v \psi \) for every variable assignment \( v \) in \( \mscrX \), we say that \enquote{\( \Gamma \) entails \( \psi \) in \( \mscrX \)} and write \( \Gamma \vDash_\mscrX \psi \).

    \thmitem{def:first_order_semantics/entailment}\mcite[def. 2.2.1(iv)]{Hinman2005} Finally, if \( \Gamma \vDash_\mscrX \psi \) for every structure \( \mscrX \), we simply say that \enquote{\( \Gamma \) entails \( \psi \)} and write \( \Gamma \vDash \psi \).
