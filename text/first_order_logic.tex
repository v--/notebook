...
\begin{remark}\label{rem:fol_application_notation}
  ...
  An exception to this rule is the term length, defined in \cref{def:fol_term_length}, which accounts for the string length.
\end{remark}
...
\begin{definition}\label{def:fol_structure}\mcite[def. 2.2.3]{Герасимов2014Вычислимость}
  ...
  \begin{thmenum}
    \thmitem{def:fol_structure/universe} A \hi{nonempty} set \( X \), called a \term[ru=носитель (\cite[def. 2.2.4]{Герасимов2014Вычислимость}), en=universe (\cite[def. 2.1.15]{Hinman2005Logic})]{universe}.
