t\subsection{Topological spaces}\label{subsec:topological_spaces}
...
\begin{definition}\label{def:topological_space}\mcite[11]{Engelking1989}
  A \term{topology} on a set \( X \) is a \hyperref[def:semilattice/lattice]{join-complete} \hyperref[def:semilattice/submodel]{bounded sublattice} of the \hyperref[thm:boolean_algebra_of_subsets]{Boolean algebra of subsets} of \( X \).
...
\begin{definition}\label{def:sierpinski_space}\mcite[example 2.1.7(e)]{Perrone2019}
...
\begin{definition}\label{def:topological_neighborhood}\mcite[def. 4.1]{Александров1977Введение}
...
\end{definition}
\begin{comments}
  \item Many authors --- for example Ryszard Engelking in \cite[12]{Engelking1989}, Walter Rudin in \cite[7]{Rudin1991Functional} and Pavel Alexandrov in \cite[def. 4.1]{Александров1977Введение} --- stick to this definition, while others define neighborhoods to be an arbitrary superset of what we call a neighborhood --- for example John Kelley in \cite[8]{Kelley1975}.
\end{comments}

\begin{definition}\label{def:neighborhood_filter}\mcite[25]{SteenSeebach1995}
...
\begin{example}\label{ex:def:topological_neighborhood}
  ...
  \begin{thmenum}
    \thmitem{ex:def:topological_neighborhood/discrete} With the \hyperref[def:discrete_topology]{discrete topology} on \( X \), the \hyperref[def:topological_neighborhood]{neighborhood filter} of the point \( x \) is the \hyperref[ex:principal_ultrafilter]{principal ultrafilter} of \( x \).
...
\begin{definition}\label{def:topological_base}
  ...
  \begin{thmenum}
    \thmitem{def:topological_base/union}\mcite[12]{Engelking1989} Every open set \( U \in \mscrT \) is the union \( U = \bigcup \mscrB' \) of some subset \( \mscrB' \subseteq \mscrB \).

    \medskip

    \thmitem{def:topological_base/subset}\mcite[12]{Engelking1989} For any point \( x \in X \) and for any neighborhood \( U \) of \( x \), there exists a set \( V \in \mscrB \) in the base such that \( x \in V \subseteq U \).
  \end{thmenum}
\end{definition}
\begin{proof}
...
\end{theorem}
\begin{comments}
  \item Pavel Alexandrov in \cite{Александров1977Введение} calls this theory \enquote{the} Alexandrov-Urysohn theorem. The infinite case of the theorem can be found as \enquote{theorem 2} in his earlier paper \cite{АлександровУрысон1950} with Pavel Urysohn, although this theorem is not the focus of their paper.
\end{comments}
\begin{proof}
...
\end{definition}
\begin{comments}
  \item This is a twist around the usual definition requiring that the family of all finite intersections of members of \( \mscrS \) is a \hyperref[def:topological_base]{basis}. This is the definition given by Ryszard Engelking in \cite[4]{Engelking1989}, by John Kelley in \cite[48]{Kelley1975} and Pavel Alexandrov in \cite[def. 4.7]{Александров1977Введение}).
\end{comments}
...
\begin{definition}\label{def:topological_local_base}\mcite[13]{Engelking1989}
...
\begin{defproof}
  ...
  \SubProofOf[def:binary_operation/idempotent]{idempotence} Since \( \cl(A) \) is the smallest closed set containing \( \cl(A) \), we conclude that \( \cl(\cl(A)) = \cl(A) \).
...
\begin{definition}\label{def:topological_interior_operator}\mcite[thm. 1.1.6]{Engelking1989}
  ...
  The interior is not a \hyperref[def:moore_closure_operator]{closure operator} --- it is \hyperref[def:order_homomorphism/increasing]{monotone} and \hyperref[def:binary_operation/idempotent]{idempotent}, but instead of being \hyperref[def:extensive_function]{extensive}, it satisfies \ref{thm:topology_from_interior_operator/IO2}. It does have a deep relationship with the \hyperref[def:topological_closure_operator]{topological closure operator} -- see \fullref{thm:interior_closure_complement}.
\end{definition}
\begin{comments}
...
\begin{defproof}
  ...
  \SubProofOf[def:binary_operation/idempotent]{idempotence} Since \( \Int(A) \) is the largest an open set contained in \( \Int(A) \), we conclude that \( \Int(\Int(A)) = \Int(A) \)
...
\begin{definition}\label{def:set_cluster_point}\mcite[24]{Engelking1989}
