...
\begin{definition}\label{def:topology_ordering}\mcite[16]{Engelking1989}
  The family of all topologies on a set \( X \) is \hyperref[def:partially_ordered_set]{partially ordered} by set inclusion. If \( \mscrO \subseteq \mscrT \) are topologies on \( X \), we say that \( \mscrO \) is \term[bg=груба, ru=грубая]{coarser} than \( \mscrT \) and that \( \mscrT \) is \term[bg=фина, ru=финная]{finer} than \( \mscrO \).
\end{definition}
\begin{comments}
