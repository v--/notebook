...
\end{definition}
\begin{comments}
  \item As we shall see in \cref{thm:lattice_of_topologies}, this is a \hyperref[def:partially_ordered_set]{partial ordering} in the \hyperref[thm:lattice_of_topologies]{lattice of topologies} on \( X \).
\end{comments}
...
\end{definition}
\begin{comments}
  \item It is the in the \hyperref[def:topological_space_ordering]{finest} topology in the \hyperref[thm:lattice_of_topologies]{lattice of topologies} on \( X \), and is \hyperref[def:category_adjunction]{left adjoint} to the \hyperref[con:forgetful_functor]{forgetful functor} \( U: \cat{Top} \to \cat{Set} \).
  ...
  \item Discrete spaces are the basis of Aristotelian discrete variables; see \cref{con:discrete_and_continuous_variables}.
\end{comments}
...
\end{definition}
\begin{comments}
  \item It is the in the \hyperref[def:topological_space_ordering]{coarsest} topology in the \hyperref[thm:lattice_of_topologies]{lattice of topologies} on \( X \), and is \hyperref[def:category_adjunction]{right adjoint} to the \hyperref[con:forgetful_functor]{forgetful functor} \( U: \cat{Top} \to \cat{Set} \).
\end{comments}
...
\begin{definition}\label{def:sierpinski_space}\mcite[example VI.4.6(c)]{Johnstone1982StoneSpaces}
  The two-element \hyperref[def:bounded_lattice]{bounded lattice} \( \set{ \top, \bot } \) endowed with the topology
  \begin{equation*}
...
\begin{remark}\label{rem:constructing_topologies}
  ...
  \begin{thmenum}
    \thmitem{rem:constructing_topologies/open} By specifying all open sets --- see \cref{def:topological_space}.

    \thmitem{rem:constructing_topologies/closed} By specifying all closed sets --- see \cref{thm:topology_from_closed_sets}.

    \thmitem{rem:constructing_topologies/base} By specifying only a \hyperref[def:topological_base]{base} of open sets --- see \cref{thm:topology_from_base}.

    \thmitem{rem:constructing_topologies/subbase} By specifying only some family of open sets --- see \cref{thm:topology_from_subbase}.

    This is the approach we use for order topologies in \cref{def:order_topology}.

    \thmitem{rem:constructing_topologies/local_base} By specifying a \hyperref[def:topological_local_base]{local base} of open sets at every point --- see \cref{thm:topology_from_local_base}.

    This is the approach we use for metric spaces in \cref{def:metric_topology} and uniform spaces in \cref{def:uniform_topology}.

    \thmitem{rem:constructing_topologies/local_subbase} By specifying a \hyperref[def:topological_local_subbase]{local subbase} of open sets at every point --- see \cref{thm:topology_from_local_subbase}.

    \thmitem{rem:constructing_topologies/closure} By specifying a \hyperref[def:topological_closure_operator]{closure operator} --- see \cref{thm:topology_from_closure_operator}.

    \thmitem{rem:constructing_topologies/interior} By specifying an \hyperref[def:topological_interior_operator]{interior operator} --- see \cref{thm:topology_from_interior_operator}.
  \end{thmenum}
\end{remark}

\begin{proposition}\label{thm:topology_from_closed_sets}
  If \( \mscrF \) is a \hyperref[def:lattice/subobject]{bounded sublattice} of the \hyperref[thm:boolean_algebra_of_subsets]{power set Boolean algebra} of \( X \) that is closed under arbitrary intersections, then \( \mscrT \coloneqq \set{ X \setminus F \given F \in \mscrF } \) is a \hyperref[def:topological_space]{topology} on \( X \). This allows us to construct a topology by specifying the closed sets in advance.
...
\end{definition}
\begin{comments}
  \item Some authors --- for example \textcite[12]{Engelking1989GeneralTopology}, \textcite[7]{Rudin1991FunctionalAnalysis} and \textcite[def. 4.1]{Александров1977ОбщаяТопология} --- stick to this definition, while others define neighborhoods to be an arbitrary superset of what we call a neighborhood --- for example \textcite[8]{Kelley1975GeneralTopology}.
\end{comments}

\begin{definition}\label{def:neighborhood_filter}\mcite[25]{SteenSeebach1995CounterexamplesInTopology}
  For a fixed set \( A \) in a topological space \( (X, \mscrT) \), we define the \term{neighborhood filter} of \( A \) as the family
  \begin{equation*}
...
\begin{proposition}\label{thm:topology_from_base}\mcite[12]{Engelking1989GeneralTopology}
  ...
  Furthermore, \( \mscrB \) satisfies \cref{def:topological_base/union} is thus a \hyperref[def:topological_base]{base} of \( \mscrT \). We say that the base \( \mscrB \) \term{generates} \( \mscrT \).
\end{proposition}
\begin{proof}
  ...
  \begin{align*}
    ...
    \reloset {\ref{alg:fol_formula_to_prenex_normal_form/connectives}} = \\ &=
...
\begin{proof}
  ...
  \SubProofOf{thm:topology_from_base/B1} Since \( X \) is always a member of the topology, \cref{def:topological_base/union} implies that \( X \) is a union.
...
\begin{definition}\label{def:topological_space_weight}
  ...
  See \cref{thm:topological_space_countability} for how weight relates to other measurements of the space's \enquote{size}.
\end{definition}
\begin{defproof}
...
\begin{example}\label{ex:def:topological_space_weight}
  ...
  \begin{thmenum}
    \thmitem{ex:def:topological_space_weight/indiscrete} We have shown in \cref{ex:def:topological_base/indiscrete} that \( \set{ X } \) is the unique base of the indiscrete topology on a nonempty set \( X \) that is not the topology itself. Hence, the weight of any indiscrete topology is \( 1 \).

    The weight of the indiscrete topology on the empty set is \( 0 \).

    \thmitem{ex:def:topological_space_weight/sierpinski} Similarly to \cref{ex:def:topological_space_weight/indiscrete}, \cref{ex:def:topological_base/sierpinski} allows us to conclude that the weight of the Sierpi\'nski space is \( 2 \).
    ...
    \Cref{thm:cardinal_exponentiation_comparison} then implies that \( \card X \leq \card \mscrB \). The base discussed in \cref{ex:def:topological_base/discrete} has the cardinality of \( X \) itself, hence it is possible to attain the equality.
  \end{thmenum}
\end{example}
...
\end{theorem}
\begin{comments}
  \item \Textcite{Александров1977ОбщаяТопология} calls this theory \enquote{the} Alexandrov-Urysohn theorem. The infinite case of the theorem can be found as \enquote{theorem 2} in his earlier paper \cite{АлександровУрысон1950КомпактныеПространства} with Pavel Urysohn, although this theorem is not the focus of their paper.
\end{comments}
\begin{proof}
  ...
  Therefore, \( \mscrB_0 \) satisfies \cref{def:topological_base/subset} is thus a base of \( (X, \mscrT) \).
\end{proof}
...
\begin{example}\label{ex:def:topological_subbase}
  ...
  \begin{thmenum}
    ...
    Indeed, \cref{ex:def:topological_base/discrete} implies that
    \begin{equation*}
      \mscrB = \set[\big]{ \set{ x } \given x \in X }
    \end{equation*}
...
\begin{proposition}\label{thm:topology_from_subbase}
  ...
  \end{equation}
  of finite intersections of \( \mscrS \) generates via \cref{thm:topology_from_base} the coarsest topology \( \mscrT \) containing \( \mscrS \).
...
\end{proposition}
\begin{comments}
  \item We make no claim that \( \mscrB \) is the smallest base containing \( \mscrS \) --- \cref{ex:def:topological_base/not_closed_under_intersections} provides a counterexample.
\end{comments}
\begin{proof}
  \Cref{thm:topology_from_base} already shows well-definedness and minimality of the topology with respect to the base. Minimality with respect to the subbase can be shown similarly.
\end{proof}

\begin{remark}\label{rem:subbase_and_empty_intersection}
  In the context of an ambient topological space, the intersection of empty sets discussed in \cref{def:basic_set_operations/intersection} is safe to use. It can be used, for example, in \eqref{eq:thm:topology_from_subbase/base} to drop the requirement \ref{thm:topology_from_base/B1}, but it would also complicate \cref{thm:global_subbase_to_local_subbase}.

  We prefer not to use empty intersections because of possible ambiguity.
\end{remark}

\begin{proposition}\label{thm:lattice_of_topologies}
  The family \( \mfrakT \) of all \hyperref[def:topological_space]{topologies} on a set \( X \) is a \hyperref[def:lattice]{complete bounded lattice} with respect to the ordering from \cref{def:topological_space_ordering}. Explicitly:

  \begin{thmenum}
    \thmitem{thm:lattice_of_topologies/join} The \hyperref[def:lattice]{join} of an arbitrary family \( \mfrakT' \) of topologies on \( X \) is the topology generated by the family \( \bigcup \mfrakT' \) via \cref{thm:topology_from_subbase}.
    ...
    \thmitem{thm:lattice_of_topologies/meet} The \hyperref[def:lattice]{meet} of an arbitrary family \( \mfrakT' \) of topologies on \( X \) is simply their intersection \( \bigcap \mfrakT' \).
...
\end{proposition}
\begin{proof}
  \SubProofOf{thm:lattice_of_topologies/join} Minimality follows from \cref{thm:topology_from_subbase}.

  \SubProofOf{thm:lattice_of_topologies/top} Every topology is by definition a sublattice of the discrete topology.

  \SubProofOf{thm:lattice_of_topologies/meet} We have shown in \cref{thm:boolean_algebra_of_subsets/meet} that \( \bigcap \mfrakT' \) the meet of \( \mfrakT' \) in the Boolean algebra of sets. It remains only to show that \( \bigcap \mfrakT' \) is a topology.
...
\end{definition}
\begin{comments}
  \item This definition nearly replicates \cref{def:topological_base/subset}.
\end{comments}
...
\begin{example}\label{ex:def:topological_local_base}
  ...
  \begin{thmenum}
    ...
    \thmitem{ex:def:topological_local_base/discrete} \cref{ex:def:topological_base/discrete} implies that, for the \hyperref[def:discrete_topology]{discrete topology} \( \pow(X) \) on a set \( X \), a simple local base is \( \mscrB(x) = \set{ \set{ x } } \) for every point \( x \).
...
\begin{proposition}\label{thm:topology_from_local_base}\mcite[13]{Engelking1989GeneralTopology}
  ...
  \end{equation*}
  generates via \cref{thm:topology_from_base} the coarsest topology \( \mscrT \) containing \( \mscrB(x) \) for every \( x \in X \).

  Furthermore, \( \mscrB(x) \) is a \hyperref[def:topological_local_base]{local base} for \( \mscrT \) at \( x \).
\end{proposition}
\begin{proof}
  \SubProof{Proof that \( \mscrB \) satisfies \cref{thm:topology_from_base}}
  ...
  \SubProof{Proof of minimality of \( \mscrT \)} The proof is similar to that in \cref{thm:topology_from_base}.
...
\end{definition}
\begin{comments}
  \item The supremum of cardinals is well-defined as a consequence of \cref{thm:union_of_set_of_cardinals}.
  \item See \cref{thm:topological_space_countability} for how weight relates to other measurements of the space's \enquote{size}.
\end{comments}
...
\begin{example}\label{ex:def:topological_space_character}
  ...
  \begin{thmenum}
    \thmitem{ex:def:topological_space_character/indiscrete} We have shown in \cref{ex:def:topological_base/indiscrete} that \( \set{ X } \) is the unique base of the indiscrete topology on a nonempty set \( X \) that is not the topology itself. Hence, the character of each point is \( 1 \), and the character of the space itself is also \( 1 \).

    \thmitem{ex:def:topological_space_character/sierpinski} \Cref{ex:def:topological_local_base/sierpinski} implies that the character of both points in the Sierpi\'nski space is \( 1 \), and thus character of the entire space is \( 1 \).

    \thmitem{ex:def:topological_space_character/discrete} Similarly, \cref{ex:def:topological_local_base/discrete} implies that the character of each point with respect to the discrete topology is \( 1 \), and hence the character of the space itself is \( 1 \).

    \thmitem{ex:def:topological_space_character/metric} \Cref{thm:def:metric_topology/first_countable} shows that the character of a \hyperref[def:metric_space]{metric space} is \( \aleph_0 \).
  \end{thmenum}
\end{example}
...
\end{proposition}
\begin{proof}
  Follows from \cref{thm:global_base_to_local_base} once we take finite intersections of \( \mscrS \) and \( \mscrS(x) \).
\end{proof}
...
\begin{proposition}\label{thm:topology_from_local_subbase}
  ...
  Then the family \( \seq{ \mscrB(x) }_{x \in X} \) generates via \cref{thm:topology_from_local_base} the coarsest topology \( \mscrT \) containing \( \mscrS(x) \) for every \( x \in X \) (but the local bases may not be minimal).

  Furthermore, \( \mscrB(x) \) is a \hyperref[def:topological_local_base]{local base} for \( \mscrT \) at \( x \).
\end{proposition}
\begin{proof}
  \SubProof{Proof that \( \mscrB \) satisfies \cref{thm:topology_from_local_base}} We require \ref{thm:topology_from_local_base/BP1} and \ref{thm:topology_from_local_base/BP2} to hold, hence it remains only to show \ref{thm:topology_from_local_base/BP3}.
  ...
  \SubProof{Proof of minimality of \( \mscrT \)} The proof is similar to that in \cref{thm:topology_from_subbase}.

  \SubProof{Proof that \( \mscrB(x) \) is a local subbase} Follows from \cref{thm:topology_from_local_base} by noting that every member of \( \mscrB(x) \) is a finite intersection of members of \( \mscrS(x) \).
\end{proof}
...
\end{proposition}
\begin{proof}
  Both properties are proven in \cref{thm:local_base_can_generate_topology} and the proof works here.
\end{proof}
...
\begin{definition}\label{def:topological_closure_operator}\mcite[thm. 1.1.6]{Engelking1989GeneralTopology}
  ...
  It is a \hyperref[def:abstract_closure_operator]{closure operator} in the sense of \cref{def:abstract_closure_operator}.
\end{definition}
\begin{comments}
...
\begin{proposition}\label{thm:topology_from_closure_operator}
  ...
  \end{equation*}
  generates via \cref{thm:topology_from_closed_sets} the topology
  \begin{equation*}
...
\begin{proof}
  ...
  \SubProof{Proof that \( \mscrF \) satisfies \cref{thm:topology_from_closed_sets}}
...
\begin{proof}
  ...
  \SubProofOf{thm:topology_from_closure_operator/CO2} Already proven in \cref{def:topological_closure_operator}.
  ...
  \SubProofOf{thm:topology_from_closure_operator/CO4} Already proven in \cref{def:topological_closure_operator}.
\end{proof}
...
\begin{definition}\label{def:topological_interior_operator}\mcite[thm. 1.1.6]{Engelking1989GeneralTopology}
  ...
  Points that belong to \( \Int(A) \) are called \term{interior points} of \( A \). \Cref{thm:properties_via_bases/interior} provides a characterization of interior points.

  The interior is not a \hyperref[def:abstract_closure_operator]{closure operator} --- it is \hyperref[def:order_function/preserving]{monotone} and \hyperref[def:binary_operation/idempotent]{idempotent}, but instead of being \hyperref[def:extensive_function]{extensive}, it satisfies \ref{thm:topology_from_interior_operator/IO2}. It does have a deep relationship with the \hyperref[def:topological_closure_operator]{topological closure operator} -- see \cref{thm:interior_closure_complement}.
\end{definition}
\begin{comments}
...
\end{proposition}
\begin{proof}
  Note that \( \cl(A) \coloneqq X \setminus \Int(A) \) satisfies \ref{thm:topology_from_interior_operator/IO1} -- \ref{thm:topology_from_interior_operator/IO4}. Then \( \mscrT \) is a topology, \( \cl(A) \) is the closure operator for \( \mscrT \), and \cref{thm:interior_closure_complement} implies that \( \Int(A) \) is the interior operator for \( \mscrT \).
\end{proof}
...
\end{proposition}
\begin{proof}
  Follows from \cref{thm:interior_closure_complement} and \cref{thm:topological_closure_operator_can_generate_topology}.
\end{proof}
...
\begin{proposition}\label{thm:topology_is_heyting_algebra}
  ...
  Explicitly:
  \begin{thmenum}
    \thmitem{thm:topology_is_heyting_algebra/join} \hyperref[def:lattice]{Arbitrary joins} are given by \hyperref[def:basic_set_operations/union]{unions}.

    \thmitem{thm:topology_is_heyting_algebra/meet} \hyperref[def:lattice]{Finite meets} are given by \hyperref[def:basic_set_operations/intersection]{intersections}.
    ...
    This is similar to \cref{thm:classical_equivalences/conditional_as_disjunction} despite the fact that arbitrary topologies are not Boolean algebras.
...
\end{proposition}
\begin{comments}
  \item This can be used for topological propositional semantics defined in \cref{def:truth_value_algebra/topological}.
\end{comments}
\begin{proof}
  Trivial.
\end{proof}
...
\begin{definition}\label{def:topological_boundary_operator}
  ...
  Points that belong to \( \Fr(A) \) are called \term{boundary points} of \( A \). \Cref{thm:properties_via_bases/boundary} provides a characterization of boundary points.
\end{definition}
...
\begin{proposition}\label{thm:def:topological_boundary_operator}
  ...
  \begin{thmenum}
    \thmitem{thm:def:topological_boundary_operator/pointwise} Every neighborhood of every point in \( \Fr(A) \) intersects both \( A \) and \( X \setminus A \).
...
\end{proposition}
\begin{proof}
  \SubProofOf{thm:def:topological_boundary_operator/pointwise} \todo{Prove}
  ...
  \SubProofOf{thm:def:topological_boundary_operator/complement} Follows from \cref{thm:def:topological_boundary_operator/intersection}.

  \SubProofOf{thm:def:topological_boundary_operator/closed} \Cref{thm:def:topological_boundary_operator/intersection} represents \( \Fr(A) \) as the intersection of two closed sets. Hence, \( \Fr(A) \) is itself closed.
\end{proof}
...
\begin{comments}
  ...
  \item This is related to a similar notion for \hyperref[def:topological_net]{nets}, \cref{def:net_cluster_point}, via \cref{thm:closed_iff_contains_all_net_cluster_points}.
\end{comments}
...
\begin{proposition}\label{thm:def:set_cluster_point}
  ...
  \begin{thmenum}
    ...
    This motivates \cref{def:discrete_set}.
  \end{thmenum}
\end{proposition}
\begin{proof}
  \SubProofOf{thm:def:set_cluster_point/isolated} Closed sets are fixed points of the closure operator.
  \SubProofOf{thm:def:set_cluster_point/discrete} \Cref{thm:def:set_cluster_point/isolated} implies that \( x \) is isolated in \( X \) if and only if \( X \setminus \set{ x } \) is closed, i.e. if \( \set{ x } \) is open. \Cref{ex:def:topological_base/discrete} shows that \( \set[\big]{ \set{ x } } \) is a basis of a discrete space.
\end{proof}
...
\begin{proof}
  ...
  Now let \( x \in \cl(A) \). If \( x \notin A \), then \( A = A \setminus \set{ x } \), hence \( x \in \cl(A) = \cl(A \setminus \set{ x }) \). That is, if \( x \in \cl(A) \), it is a point in \( A \) or a point of \( \drv(A) \) (or both). Therefore,
  \begin{equation*}
...
\end{corollary}
\begin{comments}
  \item Compare this result to \cref{thm:cluster_point_iff_in_closure}.
\end{comments}
\begin{proof}
  \Cref{thm:union_with_derived_set} implies that \( \cl(A) = A \cup \drv(A) \).
...
\begin{definition}\label{def:discrete_set}\mcite[300]{Александров1977ОбщаяТопология})
  We say that a set \( A \) in a topological space \( (X, \mscrT) \) is \term[ru=дискретное (семейство), en=discrete (set) (\cite[exerc. 4G]{Willard2004GeneralTopology})]{discrete} if every point of \( A \) is \hyperref[def:set_cluster_point]{isolated}.
\end{definition}
...
\begin{proof}
  ...
  \SubProofOf{thm:def:topologically_dense_set/complement} \Cref{thm:interior_closure_complement} implies that
  \begin{equation*}
...
\begin{example}\label{ex:def:topologically_dense_set}
  ...
  \begin{thmenum}
    ...
    \thmitem{ex:def:topologically_dense_set/metric} Every \hyperref[def:metric_space]{metric space} is dense in its \hyperref[thm:metric_space_completion]{completion} as shown in \cref{thm:metric_space_is_dense_in_completion}.
    ...
    \Cref{thm:properties_via_bases/nowhere_dense} generalizes similar reasoning to arbitrary topological spaces.
  \end{thmenum}
\end{example}
...
\begin{example}\label{ex:def:topological_space_density}
  ...
  \begin{thmenum}
    \thmitem{ex:def:topological_space_density/discrete} Under the \hyperref[def:discrete_topology]{discrete topology}, the density of a space equals its cardinality. Indeed, \cref{ex:def:topologically_dense_set/discrete} implies that only the space itself is dense.

    Then the space is separable if and only if it is countable.

    \thmitem{ex:def:topological_space_density/indiscrete} On the other end, under the \hyperref[def:indiscrete_topology]{indiscrete topology}, \cref{ex:def:topologically_dense_set/indiscrete} shows that every nonempty set is dense, hence the density of the space is \( 1 \).
  \end{thmenum}
\end{example}
...
\begin{proof}
  ...
  \Cref{thm:global_base_to_local_base} implies that \( \seq{ \mscrB(x) }_{x \in X} \) is a family of local bases. Then
  \begin{equation*}
  ...
  We will show that \( D \) is \hyperref[def:topologically_dense_set]{dense} in \( X \). Indeed, if \( F \) is a proper closed set, then \( X \setminus F \) is open, hence it contains a set \( U \) from \( \mscrB \). The point \( c(U) \) then belongs to \( D \), implying that \( F \) does not belong to \( D \). The only closed set containing \( D \) is \( X \) itself, and \cref{thm:def:topologically_dense_set/no_proper_closed_set} implies that \( D \) is dense.
...
\begin{proof}
  ...
  \NecessitySubProof* Suppose that every neighborhood in \( \mscrB(x) \) intersects both \( A \) and \( X \setminus A \). \Cref{thm:properties_via_bases/open} then implies that \( x \) belongs to neither \( \Int(A) \) nor \( \Int(X \setminus A) \). Therefore,
  \begin{equation*}
    x
    \in
  ...
  \Cref{thm:def:topological_boundary_operator/intersection} implies that \( \cl(A) = A \cup \Fr(A) \).
  \begin{itemize}
    \item If \( x \in A \), then \( x \in U \cap A \), and hence \( U \) intersects \( A \).
    \item If \( x \in \Fr(A) \), then \cref{thm:properties_via_bases/boundary} implies that \( U \) intersects \( A \).
  \end{itemize}
  ...
  \begin{itemize}
    ...
    \item If \( x \in X \setminus A \), every neighborhood in \( \mscrB(x) \) intersects both \( A \) and \( X \setminus A \), and \cref{thm:properties_via_bases/boundary} implies that \( x \in \Fr(A) \).
  \end{itemize}

  In both cases, \( x \in \cl(A) \).

  \SubProofOf{thm:properties_via_bases/interior} \Cref{thm:interior_closure_complement} implies that \( x \in \Int(A) \) if and only if \( x \in X \setminus \cl(X \setminus A) \). \Cref{thm:properties_via_bases/closure} implies that \( x \in X \setminus \cl(X \setminus A) \) if and only if some neighborhood in \( \mscrB(x) \) is disjoint from \( X \setminus A \), i.e. entirely in \( A \).

  \SubProofOf{thm:properties_via_bases/cluster} Follows from \cref{thm:properties_via_bases/closure}.

  \SubProofOf{thm:properties_via_bases/isolated} \hyperref[def:conditional_formula/inverse]{Inverse} of \cref{thm:properties_via_bases/cluster}.

  \SubProofOf{thm:properties_via_bases/dense} Follows from \cref{thm:properties_via_bases/closure}.
