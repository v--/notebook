...
\begin{remark}\label{rem:topologies_and_sigma_algebras_as_lattices}
  Some structures like \hyperref[def:topological_space]{topologies} and \hyperref[def:sigma_algebra]{sigma algebras} are \hyperref[def:lattice/submodel]{bounded sublattices} of the \hyperref[thm:boolean_algebra_of_subsets]{Boolean algebra of subsets} of the ambient space. This allows us to study certain aspects of \hyperref[ch:general_topology]{general topology} and \hyperref[ch:measure_theory]{measure theory} via \hyperref[sec:lattices]{lattice theory}.
\end{remark}

\begin{definition}\label{def:topological_space}\mcite[11]{Engelking1989Topology}
  A \term{topology} on a set \( X \) is a \hyperref[def:lattice]{join-complete} \hyperref[def:lattice/submodel]{bounded sublattice} of the \hyperref[thm:boolean_algebra_of_subsets]{Boolean algebra of subsets} of \( X \).
...
\begin{definition}\label{def:topological_space_ordering}\mcite[16]{Engelking1989Topology}
...
\begin{definition}\label{def:discrete_topology}\mcite[37]{Kelley1975Topology}
...
\begin{definition}\label{def:indiscrete_topology}\mcite[37]{Kelley1975Topology}
...
\begin{definition}\label{def:sierpinski_space}\mcite[example 2.1.7(e)]{Perrone2021Categories}
...
\begin{definition}\label{def:topological_neighborhood}\mcite[def. 4.1]{Александров1977Топология}
...
\end{definition}
\begin{comments}
  \item Some authors --- for example \incite[12]{Engelking1989Topology}, \incite[7]{Rudin1991FuncAnalysis} and \incite[def. 4.1]{Александров1977Топология} --- stick to this definition, while others define neighborhoods to be an arbitrary superset of what we call a neighborhood --- for example \incite[8]{Kelley1975Topology}.
\end{comments}

\begin{definition}\label{def:neighborhood_filter}\mcite[25]{SteenSeebach1995TopCounterexamples}
...
\begin{definition}\label{def:topological_base}
  ...
  \begin{thmenum}
    \thmitem{def:topological_base/union}\mcite[12]{Engelking1989Topology} Every open set \( U \in \mscrT \) is the union \( U = \bigcup \mscrB' \) of some subset \( \mscrB' \subseteq \mscrB \).

    \medskip

    \thmitem{def:topological_base/subset}\mcite[12]{Engelking1989Topology} For any point \( x \in X \) and for any neighborhood \( U \) of \( x \), there exists a set \( V \in \mscrB \) in the base such that \( x \in V \subseteq U \).
  \end{thmenum}
\end{definition}
\begin{proof}
...
\begin{proposition}\label{thm:topology_from_base}\mcite[12]{Engelking1989Topology}
...
\end{theorem}
\begin{comments}
  \item \incite{Александров1977Топология} calls this theory \enquote{the} Alexandrov-Urysohn theorem. The infinite case of the theorem can be found as \enquote{theorem 2} in his earlier paper \cite{АлександровУрысон1950КомпактныеПространства} with Pavel Urysohn, although this theorem is not the focus of their paper.
\end{comments}
\begin{proof}
...
\begin{definition}\label{def:topological_local_base}\mcite[13]{Engelking1989Topology}
...
\begin{proposition}\label{thm:topology_from_local_base}\mcite[13]{Engelking1989Topology}
...
\begin{definition}\label{def:topological_space_character}\mcite[12]{Engelking1989Topology}
...
\begin{definition}\label{def:topological_closure_operator}\mcite[thm. 1.1.6]{Engelking1989Topology}
...
\begin{definition}\label{def:topological_interior_operator}\mcite[thm. 1.1.6]{Engelking1989Topology}
...
\end{definition}
\begin{comments}
  \item As a union of open sets, \( \Int(A) \) is open. It is actually the largest open set containing \( A \) because it is a superset of every closed set contained in \( A \). In particular, the \hyperref[def:fixed_point]{fixed points} of the operator are the open sets of \( \mscrT \).
\end{comments}
\begin{defproof}
...
\begin{definition}\label{def:set_cluster_point}\mcite[24]{Engelking1989Topology}
...
\begin{example}\label{ex:def:set_cluster_point}
  ...
  \begin{thmenum}
    \thmitem{ex:def:set_cluster_point/sierpinski} In the \hyperref[def:sierpinski_space]{Sierpinski space}, \( \bot \) is an isolated point because
    \begin{equation*}
...
\begin{definition}\label{def:derived_set}\mcite[25]{Engelking1989Topology}
...
\begin{definition}\label{def:topologically_dense_set}\mcite[25]{Engelking1989Topology}
...
\begin{definition}\label{def:topological_space_density}\mcite[12]{Engelking1989Topology}
