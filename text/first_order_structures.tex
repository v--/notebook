...
\begin{definition}\label{def:fol_substructure}\mcite[def. 2.3.12]{Hinman2005Logic}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:fol_substructure/model}\mimprovised In case both \( \mscrX \) and \( \mscrY \) are both \hyperref[def:fol_semantics/model]{models} of the same \hyperref[def:fol_theory]{theory}, we say that \( \mscrY \) is a \term[ru=подмодель (\cite[53]{Мальцев1970АлгебраическиеСистемы})]{submodel} of \( \mscrX \).
  \end{thmenum}
\end{definition}
\begin{comments}
...
\begin{comments}
  ...
  \item Since \( \cat{Mod}(\Gamma) \) generally has more objects than \( \cat{Mod}(\Delta) \) when \( \Gamma \subseteq \Delta \), it is possible for \( h \) to be a monomorphism in \( \cat{Mod}(\Delta) \) but not in \( \cat{Mod}(\Gamma) \) (and conversely for split monomorphisms).
\end{comments}
...
\begin{proposition}\label{thm:fol_categorical_subobjects}
  ...
  If \( \mscrY = (Y, J) \) and \( \mscrZ = (Z, K) \) are \hyperref[def:fol_substructure/model]{submodels} of \( \mscrX = (X, I) \), the inclusions \( \iota_Y: Y \to X \) and \( \iota_Z: Z \to X \) induce the same \hyperref[def:categorical_subobject]{categorical subobject} of \( \mscrX \) in \( \cat{Mod}(\Gamma) \) if and only if \( Y = Z \).
\end{proposition}
\begin{comments}
...
\begin{proof}
  ...
  Both \( \iota_Y \) and \( \iota_Z \) are embeddings by \cref{thm:def:fol_embedding/inclusion}, and hence monomorphisms by \cref{thm:def:fol_embedding/monomorphism}, so they induce subobjects.
...
\begin{definition}\label{def:fol_generated_substructure}\mcite[prop. 2.3.16]{Hinman2005Logic}
  ...
  We denote the domain of this generated substructure by \( \braket{ A } \).
\end{definition}
\begin{comments}
  ...
  In particular, as a consequence of \cref{thm:closure_operator_minimality}, \( \braket{ A } \) is the least among all substructure domains containing \( A \).
...
\begin{proposition}\label{thm:homomorphism_induces_congruence}
  Every \hyperref[def:fol_homomorphism]{strong first-order homomorphism} \( h: \mscrX \to \mscrY \) induces a \hyperref[def:fol_congruence]{congruence} \( {\cong} \) on its domain \( \mscrX \) defined by putting \( a \cong b \) if \( h(a) = h(b) \).
\end{proposition}
\begin{proof}
...
\begin{definition}\label{def:fol_quotient_structure}\mcite[62]{Мальцев1970АлгебраическиеСистемы}
  ...
  \begin{thmenum}[resume=def:fol_quotient_structure]
    \thmitem{def:fol_quotient_structure/projection}\mimprovised The projection \( \iota: X \to Y \) is a \hyperref[def:fol_homomorphism]{homomorphism}. Furthermore, we show in \cref{thm:def:fol_quotient_structure/projection} that it is an \hyperref[def:fol_epimorphism]{epimorphism}.

    \thmitem{def:fol_quotient_structure/model}\mimprovised In case both \( \mscrX \) and \( \mscrX / {\cong} \) are both \hyperref[def:fol_semantics/model]{models} of the same \hyperref[def:fol_theory]{theory}, we say that \( \mscrX / {\cong} \) is a \term{quotient model} of \( \mscrX \).
  \end{thmenum}
\end{definition}
\begin{comments}
...
\end{theorem}
\begin{comments}
  \item This is mostly useful when \( \mscrX \) and \( \mscrX / {\cong} \) are \hyperref[def:fol_semantics/model]{models} of the same \hyperref[def:fol_theory]{theory}.
...
\begin{definition}\label{def:fol_epimorphism}\mimprovised
  Consider some \hyperref[def:fol_theory_model_functor/obj]{category of models} \( \cat{Mod}(\Gamma) \). Based on the categorical terminology from \cref{def:morphism_invertibility/right_cancellative}, we say that the homomorphism \( h: \mscrX \to \mscrY \) is an \term{epimorphism} if it is right-cancellative, i.e. if \( g_1 \bincirc h = g_2 \bincirc h \) implies that \( g_1 = g_2 \).
...
\begin{comments}
  ...
  \item Since \( \cat{Mod}(\Gamma) \) generally has more objects than \( \cat{Mod}(\Delta) \) if \( \Gamma \subseteq \Delta \), it is possible for \( h \) to be an epimorphism in \( \cat{Mod}(\Delta) \) but not in \( \cat{Mod}(\Gamma) \) (and conversely for split epimorphisms).
\end{comments}

\begin{proposition}\label{thm:def:fol_quotient_structure}
  \hyperref[def:fol_quotient_structure]{First-order quotient structures} have the following basic properties:
  \begin{thmenum}
    \thmitem{thm:def:fol_quotient_structure/projection} The projection map of a \hyperref[def:fol_quotient_structure/model]{quotient model} is a \hyperref[def:function_invertibility/surjective]{surjective} \hyperref[def:fol_epimorphism]{epimorphism}.
  \end{thmenum}
\end{proposition}
\begin{proof}
  \SubProofOf{thm:def:fol_quotient_structure/projection} Follows from \cref{thm:concrete_category_function_invertibility/surjective}.
\end{proof}
...
\begin{proposition}\label{thm:fol_categorical_quotient_objects}
  ...
  If \( \mscrY = (Y, J) \) and \( \mscrZ = (Z, K) \) are \hyperref[def:fol_quotient_structure/model]{quotient models} of \( \mscrX = (X, I) \), the projections \( \pi_Y: X \to Y \) and \( \pi_Z: X \to Z \) induce the same \hyperref[def:categorical_quotient_object]{categorical quotient object} of \( \mscrX \) in \( \cat{Mod}(\Gamma) \) if and only if \( Y = Z \).
\end{proposition}
\begin{comments}
...
\begin{proof}
  ...
  Both \( \pi_Y \) and \( \pi_Z \) are epimorphisms by \cref{thm:def:fol_quotient_structure/projection}, so they induce quotient objects.
