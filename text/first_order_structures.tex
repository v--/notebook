...
\begin{definition}\label{def:fol_substructure}\mcite[def. 2.3.12]{Hinman2005Logic}
  ...
  We say that \( \mscrY \) is a \term[ru=подструктура (\cite[114]{ШеньВерещагин2017ЯзыкиИИсчисления})]{substructure} of \( \mscrX \) if \( X \) is a subset of \( Y \) and, for every \( n \)-ary symbol \( s \) in \( \Sigma \), the interpretation \( J(s) \) is a \hyperref[def:set_valued_map/restriction]{restriction} of \( I(s) \) to \( Y^n \).

  By abuse of notation, we may write \( J = I\restr_Y \) for the interpretation and \( \mscrY = \mscrX\restr_Y \) for the substructure itself.

  \begin{thmenum}
    \thmitem{def:fol_substructure/determines}\mimprovised If \( Y \) is the universe of a substructure of \( \mscrX \), its interpretation is uniquely defined. In this case we say that \( Y \) \term{determines} a substructure of \( \mscrX \).
    ...
    \thmitem{def:fol_substructure/submodel}\mimprovised In case both \( \mscrX \) and \( \mscrY \) are both \hyperref[def:fol_semantics/model]{models} of the same set of sentences, we say that \( \mscrY \) is a \term[ru=подмодель (\cite[53]{Мальцев1970АлгебраическиеСистемы})]{submodel} of \( \mscrX \).
...
\begin{proposition}\label{thm:fol_substructure_characterization}
  Given a structure \( \mscrX = (X, I) \), the subset \( A \subseteq X \) \hyperref[def:fol_substructure/determines]{determines} a substructure of \( \mscrX \) if and only if it is closed under \( I(f) \) for every function symbol \( f \).
\end{proposition}
\begin{proof}
...
\begin{proposition}\label{thm:def:fol_substructure}
  ...
  \begin{thmenum}
    ...
    For every \hyperref[def:fol_homomorphism]{homomorphism} \( h: \mscrX \to \mscrY \) and every substructure \( \mscrX' = (X', I') \) of \( \mscrX \), the set-theoretic image \( h[X'] \) determines a substructure of \( \mscrY \).
    ...
    \thmitem{thm:def:fol_substructure/homomorphism_preimage} Similarly, for every homomorphism \( h: \mscrX \to \mscrY \) and every substructure \( \mscrY' = (Y, J') \) \emph{of \( \mscrY \)}, the set-theoretic preimage \( h^{-1}(Y') \) determines a substructure of \( \mscrX \).
  \end{thmenum}
\end{proposition}
\begin{proof}
  ...
  Therefore, \( h[X'] \) is closed under function application. \Cref{thm:fol_substructure_characterization} implies that \( h[X'] \) determines a substructure of \( \mscrY \).
...
\begin{definition}\label{def:fol_isomorphism}
  We say that a \hyperref[def:fol_homomorphism]{first-order homomorphism} \( h: \mscrX \to \mscrY \) over \( \Sigma \) is an \term[ru=изоморфизм (\cite[170]{Герасимов2014Вычислимость}), en=isomorphism (\cite[def. 2.3.1]{Hinman2005Logic})]{isomorphism} if any of the following equivalent conditions hold:
  \begin{thmenum}
...
\begin{comments}
  ...
  \item For the etymology of the word \enquote{isomorphism}, see \cref{rem:morphism_etymology}.
\end{comments}
\begin{defproof}
...
\begin{defproof}
  ...
  Thus, \( \fix(h) \) it closed under applications of \( f \), and \cref{thm:fol_substructure_characterization} implies that it determines a substructure of \( \mscrX \).
\end{defproof}
...
\end{definition}
\begin{comments}
  \item \Cref{thm:moore_family_closure_operator} implies that the generated substructure is a \hyperref[def:abstract_closure_operator]{closure operator} on \( \pow(X) \).
...
\begin{corollary}\label{thm:generated_substructure_coincides}
  Given a structure \( \mscrX = (X, I) \), the subset \( A \subseteq X \) determines a substructure of \( \mscrX \) if and only if \( \braket{ A } = A \).
\end{corollary}
\begin{proof}
...
\begin{theorem}[Induction on generated substructures]\label{thm:induction_on_generated_substructures}
  Fix a \hyperref[def:fol_structure]{first-order structure} \( \mscrX = (X, I) \) over \( \Sigma \). Consider a subset \( A \) of \( X \), a formula \( \varphi \) over \( \Sigma \), a variable \( x \) and a variable assignment \( v \) into \( X \).
...
\begin{proposition}\label{thm:fol_substructure_upward_directed_union}
  The union of an \hyperref[def:directed_set]{upward-directed family} of \hyperref[def:fol_substructure]{substructures} of some \hyperref[def:fol_structure]{first-order structure} \( \mscrX \) is also a substructure of \( \mscrX \).
\end{proposition}
\begin{comments}
  \item This is one case where there is no need to consider more general \hyperref[def:fol_generated_substructure]{generated substructures}.
\end{comments}
\begin{proof}
  Let \( \mscrX = (X, I) \) be some structure over \( \Sigma \) and let \( \seq{ X_k }_{k \in \mscrK} \) be an upward-directed family of universes of substructures of \( \mscrX \).

  We will show that their union \( U \coloneqq \bigcup_{k \in \mscrK} X_k \) is also a substructure of \( \mscrX \).

  If \( f \) is an \( n \)-ary function symbol in \( \Sigma \), fix arguments \( a_1, \ldots, a_n \) from \( U \). Let \( X_i \) be a universe containing \( x_i \). Since the family is upward-directed, there exists some universe \( X_{k_0} \) that contains \( a_1, \ldots, a_n \), and hence
  \begin{equation*}
    b \coloneqq I(f)(a_1, \ldots, a_n).
  \end{equation*}

  This demonstrates that \( U \) is closed under applications of function symbols from \( \Sigma \). Therefore, \( U \) determines a substructure of \( \mscrX \).
\end{proof}
...
\begin{theorem}[Direct product universal property]\label{thm:direct_product_universal_property}
  ...
  For a family \( \seq{ \mscrX_k }_{k \in \mscrK} \), the \hyperref[def:fol_direct_product]{direct product} \( \prod_{k \in \mscrK} \mscrX_k \) is a \hyperref[def:categorical_product]{categorical product} in \( \cat{Mod}(\Sigma) \) --- the unique up to a unique \hyperref[def:fol_isomorphism]{isomorphism} structure that satisfies the following \hyperref[con:universal_property]{universal property}:
  \begin{displayquote}
    Suppose we are given a structure \( \mscrY \) and, for every index \( m \) from \( \mscrK \), a homomorphism \( \beta_k: \mscrY \to \mscrX_k \), then there exists a homomorphism \( l_\mscrY: \mscrY \to \prod_{k \in \mscrK} \mscrX_k \) such that, for every index \( m \), the following diagram commutes:
    \begin{equation*}
      \begin{aligned}
...
\begin{proof}
  ...
  \begin{align*}
    ...
    &=
    \seq[\big]{ \beta_m(J(f)(b_1, \ldots, b_n)) }_{k \in \mscrK}
    = \\ &=
    \seq[\big]{ I_m(f)(\beta_m(b_1), \ldots, \beta_m(b_n)) }_{k \in \mscrK}
    = \\ &=
    I(f)\parens[\big]{ \seq{ \beta_m(b_1) }_{k \in \mscrK}, \ldots, \seq{ \beta_m(b_n) }_{k \in \mscrK} }
    = \\ &=
  ...
  \begin{equation*}
    ...
    =
    \bigwedge_{k \in \mscrK} I_m(p)\parens[\big]{ \beta_m(b_1), \ldots, \beta_m(b_n) }
    \geq
    J(p)(b_1, \ldots, b_n)
    =
    \semtop.
...
\begin{proposition}\label{thm:fol_homomorphism_as_substructure}
  Let \( \mscrX = (X, I) \) and \( \mscrY = (Y, J) \) be \hyperref[def:fol_structure]{structures} over a common \hyperref[def:fol_signature]{signature} \emph{without predicate symbols}. The \hyperref[def:function]{set-theoretic function} \( h: X \to Y \) is a \hyperref[def:fol_homomorphism]{homomorphism} if and only if it determines a \hyperref[def:fol_substructure]{substructure} of the \hyperref[def:fol_direct_product]{direct product} \( \mscrX \times \mscrY \).
\end{proposition}
\begin{proof}
...
\end{definition}
\begin{comments}
  \item \Cref{thm:moore_family_closure_operator} implies that this is a \hyperref[def:abstract_closure_operator]{closure operator} on \( X^2 \).
...
\begin{proposition}\label{thm:fol_generated_congruence_characterization}
  ...
  \begin{equation*}\taglabel[\ensuremath{ \logic{Cong}_p }]{inf:def:fol_generated_congruence/pred}
    \begin{prooftree}
...
\begin{theorem}[Quotient structure universal property]\label{thm:quotient_structure_universal_property}
  For every first-order structure \( \mscrX \) and every \hyperref[def:fol_congruence]{congruence} \( {\cong} \) on \( \mscrX \), the \hyperref[def:fol_quotient_structure]{quotient structure} \( \mscrX / {\cong} \) has the following \hyperref[con:universal_property]{universal property}:
  \begin{displayquote}
...
\paragraph{Homomorphism factorization}

\begin{proposition}\label{thm:fol_homomorphism_induces_congruence}
  Fix two \hyperref[def:fol_structure]{first-order structures} \( \mscrX = (X, I) \) and \( \mscrY = (Y, J) \) and consider some \hyperref[def:fol_homomorphism]{strong homomorphism} \( h: X \to Y \).

  The relation
  \begin{equation}\label{eq:thm:fol_homomorphism_induces_congruence}
    a \cong b \T{if and only if} h(a) = h(b)
  \end{equation}
  is a \hyperref[def:fol_congruence]{congruence} \( {\cong} \) on \( \mscrX \).
\end{proposition}
\begin{proof}
  \Cref{thm:function_induces_equivalence} implies that \( {\cong} \) is an equivalence relation on \( X \).
...
\begin{theorem}[First-order homomorphism factorization]\label{thm:fol_homomorphism_factorization}
  As in \cref{thm:function_induces_equivalence}, fix two \hyperref[def:fol_structure]{first-order structures} \( \mscrX = (X, I) \) and \( \mscrY = (Y, J) \) and consider some \hyperref[def:fol_homomorphism]{strong homomorphism} \( h: X \to Y \). Let \( {\cong} \) be the \hyperref[def:fol_congruence]{congruence} on \( \mscrX \) induced by \( h \).

  Let \( \pi: X \to X / {\cong} \) be the \hyperref[def:fol_quotient_structure]{quotient map} and let \( \iota: f[X] \to Y \) be the \hyperref[def:fol_substructure/inclusion]{embedding} of the image of \( f \) into \( Y \).

  Then there exists a unique \hyperref[def:fol_isomorphism]{isomorphism} \( \widetilde{h}: X / {\cong} \to h[X] \) such that the following diagram commutes:
  \begin{equation*}
    \begin{aligned}
      \includegraphics[page=1]{output/thm__fol_homomorphism_factorization}
    \end{aligned}
  \end{equation*}
\end{theorem}
\begin{comments}
  \item If \( \mscrX \) and \( \mscrY \) are \hyperref[def:fol_semantics/model]{models} of the same \hyperref[def:fol_equational_theory]{equational theory} \( T \), \cref{thm:fol_equational_theory_models} implies that \( X / {\cong} \) and \( f[X] \) determine submodels of \( T \).

  \item We state some special cases of this theorem:
  \begin{itemize}
    \item \Cref{thm:homomorphism_theorem_for_groups}.
    \item \Cref{thm:homomorphism_theorem_for_rings}.
    \item \Cref{thm:homomorphism_theorem_for_modules}.
  \end{itemize}
\end{comments}
\begin{proof}
  \Fullref{thm:function_factorization} implies that the function
  \begin{equation*}
    \widetilde{h}(\pi(a)) \coloneqq f(a)
  \end{equation*}
  is bijective.

  It is a homomorphism because \( {\cong} \) is a congruence. Hence, \( \widetilde{h} \) is an isomorphism.
\end{proof}
...
\end{definition}
\begin{comments}
  \item As shown in \cref{thm:fol_equational_theory_models/functions}, if \( \mscrX \) is a model of an \hyperref[def:fol_equational_theory]{equational theory} \( T \), then \( \fun(S, \mscrX) \) is also a model of \( T \).
\end{comments}
...
\begin{definition}\label{def:fol_lattice_of_substructures}
  ...
  We will show in \cref{thm:fol_lattice_of_substructures} that \( \op*{Sub}(\mscrX) \) a \hyperref[def:lattice]{lattice} with respect to the substructure relation. For this reason, we will call \( \op*{Sub}(\mscrX) \) the \term{lattice of substructures} of \( \mscrX \).
...
\begin{proposition}\label{thm:fol_lattice_of_substructures}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:fol_lattice_of_substructures/bottom} The \hyperref[def:extremal_points/top_and_bottom]{bottom element} is the \hyperref[def:fol_intersection_substructure]{intersection} of all substructures of \( \mscrX \).

    \thmitem{thm:fol_lattice_of_substructures/join} The \hyperref[def:lattice]{join} of the family of substructures \( \seq{ (Y_k, I\restr_{Y_k}) }_{k \in \mscrK} \) is the \hyperref[def:fol_generated_substructure]{generated substructure}
    \begin{equation*}
      \mscrY \coloneqq \braket*{ \bigcup_{k \in \mscrK} Y_k }
    \end{equation*}

    \thmitem{thm:fol_lattice_of_substructures/meet} The \hyperref[def:lattice]{meet} of a family of substructures is their \hyperref[def:fol_intersection_substructure]{intersection}.
  \end{thmenum}
\end{proposition}
\begin{proof}
  \SubProofOf{thm:fol_lattice_of_substructures/top} Trivial.

  \SubProofOf{thm:fol_lattice_of_substructures/bottom} Any substructure of \( \mscrX \) is a superstructure of the intersection of \emph{all} substructures. Thus, this intersection is a substructure of all others.
  ...
  \begin{itemize}
    ...
    \item Otherwise, \( a = I(f)(a_1, \ldots, a_n) \), where \( a_1, \ldots, a_n \) belong to \( Z \). But \( Z \) is closed under \( I(f) \) since it determines a structure, so \( a \) also belongs to \( Z \).
  \end{itemize}
...
\begin{theorem}[Lattice theorem for substructures]\label{thm:lattice_theorem_for_substructures}
  ...
  \begin{thmenum}
    ...
    It is however generally not a \hyperref[def:lattice/subobject]{sublattice} of the \hyperref[def:fol_lattice_of_substructures]{lattice of all substructures} \( \op*{Sub}(\mscrX) \) --- as an example, take any nontrivial equivalence relation in the \hyperref[def:pure_equality]{theory of pure equality}.
    ...
    \thmitem{thm:lattice_theorem_for_substructures/reverse} For every substructure \( \mscrQ = (Q, I_\cong\restr_Q) \) of \( \mscrX / {\cong} \), the union \( \bigcup Q \) determines a substructure of \( \mscrX \) compatible with \( {\cong} \).

    \thmitem{thm:lattice_theorem_for_substructures/isomorphism} The map \( \Phi(Y) \coloneqq Y / {\cong} \) is an \hyperref[def:complete_lattice/morphism]{isomorphism} between the lattice of substructures of \( \mscrX \) compatible with \( {\cong} \) and the lattice of all substructures of \( \mscrX / {\cong} \).
  \end{thmenum}
\end{theorem}
\begin{comments}
  \item Simpler forms of this theorem hold in some special cases --- see \fullref{thm:lattice_theorem_for_subgroups}, \fullref{thm:lattice_theorem_for_ring_ideals}, \fullref{thm:lattice_theorem_for_algebra_ideals} and especially \fullref{thm:lattice_theorem_for_submodules}.
\end{comments}
\begin{proof}
  ...
  The set \( Y / {\cong} \) is the universe of the quotient \( \mscrY / {\cong} \). Due to the compatibility condition for \( {\cong} \), it is a subset of \( X / {\cong} \), and it thus determines a substructure of \( \mscrX / {\cong} \). We must show that the two interpretations coincide.
  ...
  Generalizing on \( f \), we conclude that \( Y \) determines a substructure of \( \mscrX \). It is compatible with \( {\cong} \) by construction.

  \SubProofOf{thm:lattice_theorem_for_substructures/isomorphism} Clearly \( \Phi \) is injective, and it preserves and reflects order. \Cref{thm:def:order_function/embedding_iff_reflecting} implies that it is an order embedding. \Cref{thm:def:complete_lattice/embedding} implies that it is an embedding of complete lattices.
