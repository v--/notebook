...
\begin{lemma}\label{thm:dedekind_macnielle_closure}
...
\end{lemma}
\begin{proof}
  \SubProof{Proof that the operator is \hyperref[def:extensive_function]{extensive}} Fix a subset \( A \) of \( P \) and a member \( x \) of \( A \).
...
\begin{definition}\label{def:dedekind_macnielle_completion}\mcite[thm. 12]{Birkhoff1948}
  Let \( (P, \leq) \) be a \hyperref[def:partially_ordered_set]{partially ordered set}. Using the closure operator from \fullref{thm:dedekind_macnielle_closure}, we define the \term{Dedekind-MacNeille completion} of \( P \) as the family
  \begin{equation*}
...
\begin{proof}
  ...
  \SubProof{Proof that \( D(P) \) is join-complete} Let \( \mscrA \) be a family of members of \( D(P) \). Then, since we have shown idempotence in \fullref{thm:dedekind_macnielle_closure}, we have
  \begin{equation*}
...
\begin{proposition}\label{thm:def:dedekind_macneille_completion}
  The \hyperref[def:dedekind_macnielle_completion]{Dedekind-MacNeille completion} has the following basic properties:
  \begin{thmenum}
    \thmitem{thm:def:dedekind_macneille_completion/closed_elements} If \( A = A^{UL} \), then \( A \) is \hyperref[def:closed_ordered_subset]{downward closed} and, if it has a \hyperref[def:extremal_points/supremum_and_infimum]{supremum}, the supremum is in \( A \).

    The converse holds for \hyperref[def:totally_ordered_set]{totally ordered sets}.

    \thmitem{thm:def:dedekind_macneille_completion/totally_ordered} The Dedekind-MacNeille completion of a totally ordered set is also totally ordered.
  \end{thmenum}
\end{proposition}
\begin{proof}
  \SubProofOf{thm:def:dedekind_macneille_completion/closed_elements}

  \SufficiencySubProof* Suppose that \( A = A^{UL} \).
  ...
  Now suppose that \( A \) has a supremum. Since \( \sup A \) is an upper bound for \( A \), it belongs to \( A^U \). But \( \sup A \) is a lower bound of \( A^U \), hence it belongs to \( A^{UL} \). We have assumed that \( A = A^{UL} \), hence \( \sup A \) belongs to \( A \).

  \NecessitySubProof* Suppose that the order is total. Suppose also that \( A \) is downward closed and that, if \( A \) has a supremum, the latter belongs to \( A \).

  Since \( A^U \) is the set of upper bounds of \( A \), every member of \( A \) is a lower bound of \( A^U \). Totality of the ordering implies that, since \( A \) is downward closed, every member not in \( A \) is in \( A^U \). Similarly, every member not in \( A^U \) is in \( A^{UL} \).

  \begin{itemize}
    \item If \( A \) has a supremum, then it is the minimum of \( A^U \) and the maximum of \( A^{UL} \). Furthermore, by assumption \( \sup A \) belongs to \( A \), thus \( A = A^{UL} \).

    \item If \( A \) has no supremum, then \( A \) and \( A^U \) are disjoint, and so are \( A^U \) and \( A^{UL} \). Thus, again \( A = A^{UL} \).
  \end{itemize}

  \SubProofOf{thm:def:dedekind_macneille_completion/totally_ordered} Fix a totally ordered set \( (P, \leq) \) and denote by \( D(P) \) its Dedekind-MacNeille completion.
...
\begin{comments}
  ...
  \item We use the original definition given by \incite[sec. I.IV]{Beman1901Dedekind} (although in greater generality). His definition has been refined, for example by \incite[113]{Enderton1977Sets}, which defines a cut as a \hyperref[def:closed_ordered_subset]{downward closed} nonempty set \( A \) with no maximum (which uniquely defines \( B \) as its complement). We will not have use for such refinements.
\end{comments}
...
\end{definition}
\begin{comments}
  \item \Fullref{thm:def:dedekind_macneille_completion/totally_ordered} implies that the Dedekind completion itself is totally ordered.
