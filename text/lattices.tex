...
\begin{example}\label{ex:lattice_polynomials}
  ...
  \begin{thmenum}
    ...
    \thmitem{ex:lattice_polynomials/boolean} A lattice polynomial for the two-element Boolean algebra \( \set{ T, F } \) corresponds to a \hyperref[def:boolean_operator]{Boolean operator}. We cannot express negation without introducing auxiliary polynomials as a consequence of \fullref{ex:thm:posts_completeness_theorem/and_or}. \hyperref[def:positive_formula]{Positive formulas} in \hyperref[def:cnf_and_dnf]{conjunctive/disjunctive normal form}, however, correspond exactly to lattice polynomials with square-free monomials.
