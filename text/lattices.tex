...
\begin{definition}\label{def:lattice}\mcite[3]{Birkhoff1967LatticeTheory}
  ...
  \begin{thmenum}[resume=def:lattice]
    ...
    \thmitem{def:lattice/homomorphism}\mcite[def. 4.3]{Гуров2013ТеорияРешёток} \hyperref[def:first_order_homomorphism]{First-order homomorphisms} between lattices are functions that preserve joins and meets. It follows from \cref{thm:def:lattice/homomorphism_preserves_order} that such a function is order-preserving.
...
\begin{theorem}[Principle of duality for lattices]\label{thm:lattice_duality}\mcite[14]{Grätzer2011LatticeTheory}
  Consider the \hyperref[def:lattice/theory]{first-order theory of lattices}. Within it, consider the opposite formula \( \varphi^\oppos \) of the \hyperref[def:closed_fol_formula]{closed formula} \( \varphi \), in which we use \fullref{alg:fol_formula_signature_translation} to swap all instances \( \vee \) and \( \wedge \), as well as \( \leq \) and \( \geq \).
...
\begin{proposition}\label{thm:def:complete_lattice}
  ...
  \begin{thmenum}
    ...
    \begin{align}
      x \vee \parens*{ \bigwedge_{k \in \mscrK} y_k } &\geq \bigwedge_{k \in \mscrK} (x \vee y_k), \label{eq:thm:def:complete_lattice/distributive_inequality/join_over_meet} \\
      x \wedge \parens*{ \bigvee_{k \in \mscrK} y_k } &\geq \bigvee_{k \in \mscrK} (x \wedge y_k). \label{eq:thm:def:complete_lattice/distributive_inequality/meet_over_join}
    \end{align}
...
\begin{remark}\label{rem:lattice_polynomials}
  ...
  \end{equation*}
  corresponds to the product (meet) of the polynomials \( X \) and \( Y \vee Z \). Polynomials over two-element lattice \( \set{ \top, \bot } \) correspond to \hyperref[def:propositional_formula]{propositional formulas} in \hyperref[def:cnf_and_dnf]{disjunctive normal forms}.
