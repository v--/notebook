...
\begin{definition}\label{def:semilattice}
  ...
  \begin{thmenum}[series=def:semilattice]
    \thmitem{def:semilattice/join}\mcite[51]{Roman2008} We say that a partially ordered set is a \term{join-semilattice} if the supremum of every nonempty finite set always exists. The operation itself is denoted by \( \vee \) and referred to as \term{join} and rather than supremum. In contrast to suprema, joins are usually written in \hyperref[rem:first_order_formula_conventions/infix]{infix} notation, e.g. \( x \vee y \vee z \) rather than \( \sup\set{ x, y, z } \).

    \thmitem{def:semilattice/meet}\mcite[51]{Roman2008} Analogously, we say that a partially ordered set is aa \term{meet-semilattice} if the infimum of every nonempty finite set exists. The infimum is denoted by \( \wedge \) and called \term{meet}.

    \thmitem{def:semilattice/complete}\mcite[53]{Roman2008} A semilattice is said to be \term{complete} if the corresponding operation is defined for arbitrary sets rather than only finite ones.

    \thmitem{def:semilattice/lattice}\mcite[53]{Roman2008} We say that a partially ordered set is a \term{lattice} if it is both a join-semilattice and a meet-semilattice.
    ...
    \thmitem{def:semilattice/distributive_lattice}\mcite[139]{Roman2008} A lattice is said to be \term{distributive} if the following two conditions hold:
    \begin{align}
  ...
  \begin{thmenum}[resume=def:semilattice]
    ...
    \thmitem{def:semilattice/trivial} The \hyperref[def:trivial_structure]{trivial} join-semilattice and the trivial meet-semilattice are the empty set.
...
\begin{proposition}\label{thm:binary_lattice_operations}\mcite[thm. 3.3]{Roman2008}
...
\begin{remark}\label{rem:lattice_polynomials}
  ...
  There is a nuance, however. In \cite[69]{Roman2008}, a multivariate lattice polynomial is defined to consist only of indeterminates; for example
  \begin{equation*}
...
\begin{example}\label{ex:lattice_polynomials}
  ...
  \begin{thmenum}
    ...
    \thmitem{ex:lattice_polynomials/boolean} A lattice polynomial for the two-element Boolean algebra \( \set{ T, F } \) corresponds to a \hyperref[def:boolean_function]{Boolean function}. We cannot express negation without introducing auxiliary polynomials as a consequence of \fullref{ex:thm:posts_completeness_theorem/and_or}. \hyperref[def:positive_formula]{Positive formulas} in \hyperref[def:cnf_and_dnf]{conjunctive/disjunctive normal form}, however, correspond exactly to lattice polynomials with square-free monomials.
...
\begin{definition}\label{def:lattice_ideal}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:lattice_ideal/principal}\mcite[exmpl. 3.34]{Roman2008}
    \begin{minipage}[t]{0.45\textwidth}
    ...
    \thmitem{def:lattice_ideal/prime}\mcite[77]{Roman2008}
    \begin{minipage}[t]{0.45\textwidth}
    ...
    \thmitem{def:lattice_ideal/maximal}\mcite[77]{Roman2008}
    \begin{minipage}[t]{0.45\textwidth}
    ...
    \thmitem{def:lattice_ideal/submodel}\mcite[77]{Roman2008}
    \begin{minipage}[t]{0.45\textwidth}
...
\begin{definition}\label{def:closed_ordered_set}\mcite[13]{Roman2008}
...
\begin{proposition}\label{thm:def:lattice_ideal}
  ...
  \begin{thmenum}
    \thmitem{thm:def:lattice_ideal/directed_and_closed}\mcite[74]{Roman2008} A set is an ideal if and only if it is \hyperref[def:directed_set]{upward directed} and \hyperref[def:closed_ordered_set]{downward closed}.
    ...
    \thmitem{thm:def:lattice_ideal/prime}\mcite[thm. 3.37]{Roman2008} The complement of a \hyperref[def:lattice_ideal/prime]{prime ideal} (resp. \hyperref[def:lattice_ideal/prime]{prime filter}) is a filter (resp. ideal).
