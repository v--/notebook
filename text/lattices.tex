...
\subsubsection{Semilattices and lattices}
...
\subsubsection{Lattice fixed points}
...
\subsubsection{Lattice polynomials}
...
\subsubsection{Ideals and filters in lattices}
...
\begin{proof}
  ...
  \SubProofOf[def:magma/idempotent]{idempotence} Fix again a subset \( A \) of \( P \).
...
\begin{definition}[Dedekind-MacNeille completion]\label{def:dedekind_macnielle_completion}\mcite[thm. 12]{Birkhoff1948}
...
\begin{example}\label{ex:dedekind_macnielle_join_closure}
  ...
  Consider the sequence of \hyperref[def:rational_number]{rational numbers}
  \begin{equation*}
...
\begin{theorem}[Dedekind-MacNeille completion universal property]\label{thm:dedekind_macneille_completion_universal_property}\mcite[thm. 12]{Birkhoff1948}
  ...
  Via \fullref{rem:universal_mapping_property}, the abelianization functor becomes \hyperref[def:category_adjunction]{left adjoint} to the \hyperref[def:concrete_category]{forgetful functor} from complete lattices to partially ordered sets (resp. lattices).
\end{theorem}
\begin{proof}
  ...
  The last equality holds because \( \varphi \) is order-preserving, hence \( \varphi(x) \geq \varphi(a) \) for every \( a \leq x \).
\end{proof}
