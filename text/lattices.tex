...
\begin{definition}\label{def:lattice}\mcite[3]{Birkhoff1967Lattices}
  ...
  \begin{thmenum}[series=def:lattice]
    \thmitem{def:lattice/join} We denote the supremum operation via \( \vee \) and call it a \term[ru=объединение (\cite[def. 4.2]{Гуров2013Решётки})]{join} and rather than supremum.
    ...
    \thmitem{def:lattice/meet} We denote the infimum operation via \( \wedge \) and call it a \term[ru=сечение (\cite[def. 4.2]{Гуров2013Решётки})]{meet}.
  \end{thmenum}
  ...
  \begin{thmenum}[resume=def:lattice]
    ...
    \thmitem{def:lattice/submodel}\mcite[7]{Birkhoff1967Lattices} Unlike more general ordered sets, not every subset of a lattice is a \hyperref[def:first_order_submodel]{first-order submodel} because \( \vee \) and \( \wedge \) are now regarded as functional symbols. A \term{sublattice} must be closed under joins and meets.

    \thmitem{def:lattice/homomorphism}\mcite[10]{КравченкоШвидефски2019ОбщаяАлгебра} \hyperref[def:first_order_homomorphism]{First-order homomorphisms} between lattices are functions that preserve joins and meets. It follows from \fullref{thm:def:lattice/homomorphism_preserves_order} that such a function is order-preserving.
    ...
    \thmitem{def:lattice/opposite}\mcite[7]{Birkhoff1967Lattices} The \hyperref[def:preordered_set/opposite]{dual} \( X^\oppos \) of a lattice \( X \) is again a lattice, which we will call the \term[ru=двойственная (решётка) (\cite[exerc. 1.3]{КравченкоШвидефски2019ОбщаяАлгебра})]{dual lattice}.
...
\end{definition}
\begin{comments}
  \item There is an unrelated concept also called a \enquote{lattice}, which we define in \fullref{def:point_lattice} as \enquote{point lattice}. See \fullref{rem:lattice_terminology} for a discussion of terminology.
\end{comments}

\begin{theorem}[Principle of duality for lattices]\label{thm:lattice_duality}\mcite[14]{Grätzer2011Lattices}
...
\begin{proposition}\label{thm:lattice_operation_characterization}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:lattice_operation_characterization/absorption} The following \term[ru=поглощение (\cite[def. 1.1]{КравченкоШвидефски2019ОбщаяАлгебра}), en=absorption (\cite[8]{Birkhoff1967Lattices})]{absorption} laws hold:
    \begin{align}
...
\begin{proposition}\label{thm:lattice_from_binary_operations}\mcite[thm. I.8]{Birkhoff1967Lattices}
...
\begin{definition}\label{def:semilattice}\mcite[22]{Birkhoff1967Lattices}
...
\begin{definition}\label{def:bounded_lattice}\mcite[def. 2.12]{DaveyPriestley2002Lattices}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:bounded_lattice/homomorphism}\mcite[\S 2.17]{DaveyPriestley2002Lattices} A \hyperref[def:first_order_homomorphism]{first-order homomorphisms} between bounded lattices is a \hyperref[def:lattice/homomorphism]{lattice homomorphism} that dditionally preserves \( \top \) and \( \bot \).

    \thmitem{def:bounded_lattice/opposite}\mimprovised The \hyperref[def:preordered_set/opposite]{dual} \( X^\oppos \) of a bounded lattice \( X \) is again a bounded lattice with its bounds exchanged.
...
\end{definition}
\begin{comments}
  \item The axioms for bounded lattices correspond exactly to our familiar definition for partially ordered sets. Indeed, using commutativity of joins and meets and the compatibility equivalences from \fullref{thm:lattice_operation_characterization/compatibility}, we can translate the axioms from \fullref{def:bounded_lattice/theory} into
  \begin{align*}
...
\begin{proposition}\label{thm:def:bounded_lattice}
  ...
  \begin{thmenum}
    \thmitem{def:bounded_lattice/finite} Every finite lattice is bounded.

    \thmitem{def:bounded_lattice/neutral} \( \top \) (resp. \( \bot \)) acts as a \hyperref[def:monoid]{neutral element} for \( \wedge \) (resp. \( \vee \)).
  \end{thmenum}
\end{proposition}
\begin{proof}
  \SubProofOf{def:bounded_lattice/finite} Trivial.
  \SubProofOf{def:bounded_lattice/neutral} We have
  \begin{equation*}
    x \wedge \top
...
\begin{definition}\label{def:complete_lattice}\mcite[6]{Birkhoff1967Lattices}
  We say that a \hyperref[def:partially_ordered_set]{partially ordered set} is a \term[ru=полная (решётка) (\cite[133]{Гуров2013Решётки})]{complete lattice} if an \hi{arbitrary} subset has a supremum and an infimum.
  ...
  \begin{thmenum}
    \thmitem{def:complete_lattice/submodel}\mcite[250]{Grätzer2011Lattices} We call a subset of a complete lattice a \term{complete sublattice} if it is closed under arbitrary suprema and infima.
    ...
    Every homomorphism of complete lattices is obviously a lattice homomorphism and, by \fullref{thm:def:lattice/homomorphism_preserves_order}, also an order homomorphism.

    \thmitem{def:complete_lattice/opposite}\mcite[7]{Birkhoff1967Lattices} The \hyperref[def:preordered_set/opposite]{dual} \( X^\oppos \) of a complete lattice \( X \) is again a complete lattice.
...
\begin{proposition}\label{thm:complete_semilattice}\mcite[lemma 34]{Grätzer2011Lattices}
...
\begin{definition}\label{def:modular_lattice}\mcite[13]{Birkhoff1967Lattices}
...
\begin{example}\label{ex:def:modular_lattice}
  ...
  \begin{thmenum}
    ...
    \thmitem{ex:def:modular_lattice/two_element} The two-element lattice \( \set{ \bot, \top } \) is clearly modular. Indeed, due to \fullref{ex:def:modular_lattice/absorption}, it remains to consider the case where \( \synx \mapsto \bot \), \( \synz \mapsto \top \) and \( \syny \mapsto y \), where \( y \) ranges over the two possible values. Then
    \begin{align*}
      \bot \vee (y \wedge \top)
      =
      \bot \vee y
    ...
    \thmitem{ex:def:modular_lattice/pentagon}\mcite[109]{Grätzer2011Lattices} An example of a non-modular lattice is the \term{pentagon}
...
\begin{proposition}\label{thm:modular_lattice_characterization}\mcite[thm. 102(i)]{Grätzer2011Lattices}
...
\begin{definition}\label{def:distributive_lattice}\mcite[12]{Birkhoff1967Lattices}
...
\begin{remark}\label{rem:lattice_polynomials}
  ...
  \incite[sec. II.5]{Birkhoff1967Lattices} discusses only lattice polynomials \enquote{with no coefficients}, that is, whose coefficients are \( \top \). \incite[sec. I.4]{Grätzer2011Lattices} allows polynomials to have non-identity coefficients, for example
  \begin{equation*}
    q(X, Y, Z) = (a \wedge X \wedge Y) \vee (b \wedge X \wedge Z).
  \end{equation*}

  \incite{Marichal2009LatticePolynomials} calls the latter \enquote{weighted polynomials}.
\end{remark}
...
\begin{example}\label{ex:def:distributive_lattice}
  ...
  \begin{thmenum}
    \thmitem{ex:def:distributive_lattice/two_element} The two element lattice \( \set{ \top, \bot } \) is distributive because of the bounded absorption laws from \fullref{def:bounded_lattice/theory}.
    ...
    \thmitem{ex:def:distributive_lattice/diamond}\mcite[109]{Grätzer2011Lattices} An example of a \hyperref[def:modular_lattice]{modular} but non-distributive lattice is the \term{diamond}\fnote{\incite{Birkhoff1967Lattices} uses the notation \enquote{\( M_5 \)} and \incite[327]{PicadoPultr2012Frames} use \enquote{\( D_3 \)}, but \incite{Grätzer2011Lattices} and \incite{DaveyPriestley2002Lattices} prefer \enquote{\( M_3 \)}}
...
\begin{proposition}\label{thm:distributive_modular_lattice_characterization}\mcite[thm. 102(ii)]{Grätzer2011Lattices}
...
\begin{corollary}\label{thm:distributive_lattice_characterization}\mcite[thm. 101]{Grätzer2011Lattices}
...
\begin{definition}\label{def:lattice_ideal}\mcite[25]{Birkhoff1967Lattices}
  Fix some \hyperref[def:lattice]{lattice} \( X \). We will define concepts analogous to those from \fullref{sec:semiring_ideals}. In some cases the notions coincide --- see \fullref{thm:lattice_ideal_as_semiring_ideal}.
  ...
  \begin{thmenum}
    ...
    \begin{paracol}{2}
      \begin{leftcolumn}\mcite[32]{Grätzer2011Lattices}
    ...
    \begin{paracol}{2}
      \begin{leftcolumn}\mcite[28]{Birkhoff1967Lattices}
    ...
    \begin{paracol}{2}
      \begin{leftcolumn}\mcite[3]{PicadoPultr2012Frames}
...
\begin{definition}\label{def:directed_set}\mcite[54]{Grätzer2011Lattices}
  We say that a subset \( A \) of a \hyperref[def:preordered_set]{preordered set} is \term[bg=насочено надясно (множество) (\cite[10]{Проданов1982ФункАнализЧаст1})]{directed from above} (resp. \term{directed from below}) if every two elements of \( A \) have an \hyperref[def:extremal_points/bounds]{upper bound} (resp. lower bound).
...
\begin{definition}\label{def:lattice_prime_element}
  ...
  \begin{thmenum}
    \thmitem{def:lattice_prime_element/direct}\mcite[13]{PicadoPultr2012Frames} If \( x \wedge y \leq p \), then \( x \leq p \) or \( y \leq p \) (or both).
    \thmitem{def:lattice_prime_element/ideals}\mcite[13]{PicadoPultr2012Frames} The \hyperref[def:lattice_ideal/principal]{principal ideal} of \( p \) is \hyperref[def:lattice_ideal/prime]{prime}.
  \end{thmenum}
\end{definition}
