...
\begin{definition}\label{def:directed_set}\mimprovised
  We say that a nonempty subset \( A \) of a \hyperref[def:preordered_set]{preordered set} is \term[bg=насочено надясно (множество) (\cite[10]{Проданов1982ФункционаленАнализЧаст1}), en=upward directed (\cite[25]{Johnstone1982StoneSpaces})]{upward directed} (resp. \term[en=downward directed (\cite[167]{Perrone2024StartingCategoryTheory})]{downward directed}) if every two elements of \( A \) have an \hyperref[def:extremal_points/bounds]{upper bound} (resp. lower bound).

  By simply \enquote{directed}, we will mean \enquote{upward directed}.
\end{definition}
\begin{comments}
