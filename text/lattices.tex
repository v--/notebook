...
\begin{definition}\label{def:bounded_lattice}\mcite[def. 2.12]{DaveyPriestley2002Lattices}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:bounded_lattice/submodel}\mimprovised We call a \hyperref[def:lattice/order]{sublattice} of a bounded lattice a \term{bounded sublattice} if it contains both \( \top \) and \( \bot \).
...
\begin{proof}
  ...
  \begin{equation*}
    x \wedge \top
    \reloset {\eqref{eq:def:bounded_lattice/finite/top}} =
    x \wedge (\top \vee x)
    =
    x \wedge (x \vee \top)
  ...
  \begin{equation*}
    x \vee \top
    \reloset {\eqref{eq:def:bounded_lattice/finite/bot}} =
    x \vee (\top \wedge x)
    =
    x \vee (\top \wedge x)
