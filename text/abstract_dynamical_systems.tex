...
\begin{concept}\label{con:discrete_and_continuous_variables}
  ...
  \incite[17]{Розенфельд1963ОсновныеПонятиеГеометрии} attributes the concept of continuity to Aristotle. In \cite[36]{Aristotle1984Categories}, an English translation of Aristotle's works, we can find the following:
  \begin{displayquote}
...
\begin{comments}
  ...
  \item \incite[11]{Wirsching1998CollatzDynamicalSystem} claims that Collatz himself called \eqref{eq:def:collatz_map} the \enquote{\( 3n + 1 \)}-Funktion. Meanwhile, \incite[11]{Wirsching1998CollatzDynamicalSystem} himself instead refers to the reduced map \eqref{eq:def:collatz_conjecture/reduced} as the \enquote{\( 3n + 1 \) function}, while \incite[4]{Lagarias2021CollatzOverview}, calls it the \enquote{\( 3x + 1 \) function}.
