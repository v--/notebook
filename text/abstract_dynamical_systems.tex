...
\begin{proof}
  ...
  \SubProofOf{thm:def:dynamical_system/iterated} We will first suppose that \( n \) is nonnegative and use induction:
  \begin{itemize}
  ...
  Then, if the system is reversible,
  \begin{equation*}
    \Phi_{(-n)t}
    \reloset {\eqref{eq:thm:def:dynamical_system/negation}} =
    (\Phi_nt)^{-1}
    =
    [(\Phi_t)^n]^{-1}
...
\begin{example}\label{ex:def:dynamical_system}
  ...
  \begin{thmenum}
    ...
    \Fullref{thm:def:dynamical_system/multi_dimensional} implies that, for any symbol \( a \) in \( A^* \), the map \( \Phi_a \) determines all the map \( \Phi_{a^n} \), where \( n \) is any integer. In this example we use multiplicative notation for time.
