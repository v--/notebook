...
\begin{definition}\label{def:zero_of_function}\mimprovised
  Fix a function \( f: A \to G \), where \( G \) is an \hyperref[def:additive_semigroup]{additive (semi)group}. We say that \( a \in A \) is a \term{zero} of \( f \) if \( f(a) = 0_G \). We denote by \( Z(f) \) the set of all zeros.
\end{definition}
\begin{comments}
  \item See \cref{rem:zero_of_function_terminology} for a discussion of the terminology and how it relates to \hyperref[def:polynomial_root]{polynomial roots}.
\end{comments}

\begin{remark}\label{rem:zero_of_function_terminology}
  In the context of the \hyperref[def:zero_of_function]{zeros of a function}, the term \enquote{zero} is also mostly used for \hyperref[con:free_construction/evaluation]{polynomial functions}. For example, explicit definitions are only given for polynomials by
  \textcite[29]{Ahlfors1979ComplexAnalysis}
  \textcite[594]{Rotman2017AdvancedModernAlgebraPart2}
  \textcite[exerc. 1.68]{LidlNiederreiter1997FiniteFields}

  This makes the usage similar to \enquote{root}, where in \cref{def:polynomial_root} we give a more abstract definition limited to polynomials. See \cref{rem:root_terminology} for a broader discussion of the latter term.

  We follow \textcite[209]{Rudin1987RealAndComplexAnalysis} and \textcite[\S 1.10]{GillmanJerison1960RingsOfContinuousFunctions}, who define a \enquote{zero set} of a real-valued function as its \hyperref[def:level_set]{level set} for \( 0 \). We use their notation \( Z(f) \), but prefer to call it a \enquote{set of zeros} to make the distinction with \hyperref[con:measure_zero_set]{measure-zero sets} clear.
...
\begin{remark}\label{rem:locus_terminology}
  The term \enquote{locus} is used as a synonym for \enquote{set of points} in geometry. In particular, \enquote{zero locus} is used for what we call a \enquote{\hyperref[def:zero_of_function]{set of zeros}}, i.e. the preimage of the zero element of some \hyperref[def:additive_semigroup]{additive (semi)group}. A point in the set of zeros is also called a \enquote{root} for unrelated reasons --- see \cref{rem:root_terminology}.

  \begin{itemize}
    \item \Textcite[17]{Розенфельд1963ОсновныеПонятиеГеометрии} explains this with Aristotle's idea that a line is not a set of points, but merely a position where points can be placed. This view was shared by Euclid, and through his \enquote{Elements} book series later the Latin translation of \enquote{place}, \enquote{locus}, became standard terminology. In \cite{Euclid2008Elements}, Fitzpatrick translates several distinct Ancient Greek words as \enquote{place} or \enquote{position}, both as nouns and as verbs.
    ...
    \item Less formally, \textcite[48]{Киселёв2004Геометрия}, the original of which was published in 1892, defines a \enquote{геометрическое место точек} (\enquote{geometric position of points}) as a collection of points satisfying some condition. In modern terminology this corresponds precisely a subset of an abstract space defined via an \hyperref[def:zfc/specification]{axiom of specification}.

    \item \Textcite[1]{Hadamard1898LeçonsdeGéométrieTome1} gives a less clear definition for \enquote{lieux géométriques} (\enquote{geometric loci}), which Mark Saul in \cite[1]{Hadamard2008LessonsInGeometryVol1} translates as
    \begin{displayquote}
...
\begin{proof}
  ...
  \SubProofOf{thm:def:krull_dimension/quotient} Follows from \fullref{thm:lattice_theorem_for_ring_ideals}.
