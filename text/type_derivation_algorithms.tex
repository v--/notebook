...
\end{algorithm}
\begin{comments}
  \item Note that, unlike in \fullref{thm:typed_term_habitation_uniqueness}, we have not restricted which rules can be used, so it is possible that the obtained type is not unique. The restriction to only the base rules is discussed in \fullref{rem:base_typing_rules}.
