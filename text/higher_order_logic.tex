...
\begin{definition}\label{def:hol_signature}\mimprovised
  A \hyperref[con:logical_system_signature]{signature} for higher-order logic consists of several nonempty finite sets:
  \begin{thmenum}
...
\begin{remark}\label{rem:hol_notational_shorthands}
  ...
  \begin{thmenum}
    ...
    We want to attach annotations to free variables so that we do not have to mix type contexts and logical contexts. This leads us to define, in \cref{def:hol_term}, a logical term as a triple \( (\Gamma, M, \tau) \), where \( \Gamma \vdash M: \tau \) and all variables of \( \Gamma \) are free in \( M \).

    This allows us to place the type of every free variable in \( M \) in a superscript. For example, we denote the logical term
    \begin{equation*}
      \parens[\big]{ (f: \tau \synimplies \sigma, x: \tau), f x, \sigma }
    \end{equation*}
    by
    \begin{equation*}
...
\begin{definition}\label{def:hol_term}\mimprovised
  Based on our discussion in \cref{rem:hol_notational_shorthands/variable_annotations}, given the \hyperref[def:simple_type_entailment]{entailment} \( \Gamma \vdash M: \tau \) over a fixed \hyperref[def:hol_signature]{signature of higher-order logic}, we call the triple \( (\Gamma, M, \tau) \) a \term{logical term} if all variables in \( \Gamma \) are free in \( M \).
  ...
  \begin{thmenum}[resume=def:hol_term]
    \thmitem{def:hol_term/over} If \( \Gamma \) is a subcontext of \( \Delta \), we say that \( (\Gamma, M, \tau) \) is a logical term \term{over} \( \Delta \).
...
\begin{comments}
  ...
  \item Even though the rules stated here as simple and can be formalized via the familiar schemas used through \fullref{sec:curry_howard_correspondence}, we only state them in the metalanguage, as per \cref{rem:hol_rule_formalization}.
\end{comments}

\begin{proposition}\label{thm:logical_term_type_uniqueness}
  The type of a \hyperref[def:hol_term]{higher-order logical term} is unique: for every logical term \( (\Gamma, M, \tau) \), if \( \Gamma \vdash M: \tau' \), then \( \tau = \tau' \).
\end{proposition}
\begin{proof}
  We must extend the recursion in the proof of \cref{thm:typed_term_habitation_uniqueness} to handle the new typing rules from \cref{def:hol_typing_rules}, as well as \ref{inf:def:hol_signature/nl_type}.

  The rules are adjusted so that uniqueness is straightforward.
\end{proof}
    ...
    \begin{paracol}{2}
      \begin{leftcolumn}
        \begin{thmenum}
          ...
          \thmitem{def:hol_quantifier_rules/main/assumptions} The eigenvariable \( y \) must not be free in any open assumption of the proof tree.
        \end{thmenum}
      \end{leftcolumn}
      ...
      \begin{rightcolumn}
        \begin{thmenum}
          ...
          \thmitem{def:hol_quantifier_rules/discharge/assumptions} The eigenvariable \( y \) must not be free in any open assumption of the right subtree, except possibly in \( \varphi[x^\tau \mapsto y_*^\tau] \) itself.
  ...
  \begin{paracol}{2}
    \begin{leftcolumn}
      \begin{equation*}\taglabel[\ensuremath{ =_+ }]{inf:def:hol_equality_rules/intro}
        \begin{prooftree}
          \infer0[\ref{inf:def:hol_equality_rules/intro}]{ M^\tau \syneq M^\tau }
        \end{prooftree}
      \end{equation*}
    \end{leftcolumn}
...
\end{definition}
\begin{comments}
  \item Both rules are needed for \cref{thm:hol_equality_is_equivalence}. The latter generalizes rule \logic{R'} from Andrews' system \( \logic{Q}_0 \) --- see \cref{ex:hol_equality_rules_and_rule_r}
...
\begin{proposition}\label{thm:hol_equality_is_equivalence}
  Equality in \hyperref[def:higher_order_logic]{simply typed higher-order logic} acts like an \hyperref[def:equivalence_relation]{equivalence relation} with respect to \hyperref[def:higher_order_logic_proof_tree]{derivability}. More precisely, the following rules are \hyperref[con:inference_rule_admissibility]{admissible} (\ref{inf:def:hol_equality_rules/intro} acts as reflexivity):

  \begin{paracol}{2}
    \begin{leftcolumn}
      \ParacolAlignmentHack
      \begin{equation*}\taglabel[\ensuremath{ =_S }]{inf:thm:hol_equality_is_equivalence/symmetry}
        \begin{prooftree}
          \hypo{ M^\tau \syneq N^\tau }
          \infer1[\ref{inf:thm:hol_equality_is_equivalence/symmetry}]{ N^\tau \syneq M^\tau }.
        \end{prooftree}
      \end{equation*}
    \end{leftcolumn}
    ...
    \begin{rightcolumn}
      \ParacolAlignmentHack
      \begin{equation*}\taglabel[\ensuremath{ =_T }]{inf:thm:hol_equality_is_equivalence/transitivity}
        \begin{prooftree}
          \hypo{ M^\tau \syneq N^\tau }
          \hypo{ N^\tau \syneq K^\tau }
          \infer2[\ref{inf:thm:hol_equality_is_equivalence/transitivity}]{ M^\tau \syneq K^\tau }
        \end{prooftree}
      \end{equation*}
    \end{rightcolumn}
  \end{paracol}
\end{proposition}
\begin{proof}
  \SubProofOf{inf:thm:hol_equality_is_equivalence/symmetry} Let \( \varphi \coloneqq (x^\tau \syneq M^\tau) \). Then
  \begin{equation*}
    \begin{prooftree}
      \infer0[\ref{inf:def:hol_equality_rules/intro}]{ \varphi[x^\tau \mapsto M^\tau] }
      \hypo{ M^\tau \syneq N^\tau }
      \infer2[\ref{inf:def:hol_equality_rules/elim}]{ \underbrace{\varphi[x^\tau \mapsto N^\tau]}_{N^\tau \syneq M^\tau} }
    \end{prooftree}
  \end{equation*}

  \SubProofOf{inf:thm:hol_equality_is_equivalence/transitivity}
  \begin{equation*}
    \begin{prooftree}
      \hypo{ M^\tau \syneq N^\tau }
      \hypo{ N^\tau \syneq K^\tau }
...
\begin{remark}\label{rem:hol_axiomatic_derivations}
  ...
  With the abbreviations in \ref{rem:hol_formula_abbreviations}, the rule \ref{inf:def:propositional_natural_deduction_systems/top/intro} follows from a special case where \( M^\tau = \qabs {\synp^{\syn\omicron}} \synp \).
...
\begin{example}\label{ex:hol_equality_rules_and_rule_r}
  ...
  \end{equation*}
  so that \( \varphi \coloneqq \theta[x^\tau \coloneqq A^\tau] \) and \( \psi \coloneqq \theta[x^\tau \coloneqq B^\tau] \) satisfy rule \( \logic{R'} \) as long as \( f \) is not free in \( A^\tau \) nor \( B^\tau \).

  Note that \( \theta \) already contains an occurrence of \( A^\tau \) --- the variable \( x^\tau \) indicates the instance we want to replace in \( \varphi \) to obtain \( \psi \). We can use the same technique to \enquote{replace} a single instance of \( A^\tau \) for \( B^\tau \) in \( \varphi \) more generally.

  If out example, if \( f \) is not free in \( A^\tau \) nor \( B^\tau \), the substitution in both \( \varphi = \theta[x^\tau \mapsto A^\tau] \) and \( \psi = \theta[x^\tau \mapsto B^\tau] \) must use the non-renaming rule \eqref{eq:def:lambda_term_substitution/abstraction/direct}, and thus
  \begin{equation*}
    \begin{prooftree}
      \hypo{ \varphi }
  ...
  On the other hand, if \( f \) is free in \( A^\tau \) but not \( B^\tau \), then the substitution in \( \varphi = \theta[x^\tau \mapsto A^\tau] \) must use the renaming rule \eqref{eq:def:lambda_term_substitution/abstraction/renaming}, and thus
  \begin{equation*}
    \varphi = \qabs {g^{\tau \synimplies \tau \synimplies \syn\omicron}} g A^\tau (A^\tau[f \mapsto g])
  \end{equation*}
  for some variable \( g \) not free in \( A^\tau \). Then \( \psi \) is not \enquote{\( \varphi \) with a single instance of \( A^\tau \) substituted \( B^\tau \)}, and rule \logic{R'} is inapplicable. Even though \ref{inf:def:hol_equality_rules/elim} still allows us to derive \( \psi \) from \( \varphi \) and \( A^\tau \syneq B^\tau \), the relationship between \( \varphi \) and \( \psi \) is not what rule \logic{R'} expects.

  Admissibility of rule \logic{R'} for arbitrary \( \varphi \) and \( \psi \) can be proven by induction on the abstract syntax tree of \( \varphi \). The above demonstrates how to handle the only non-trivial case.
\end{example}

\begin{definition}\label{def:hol_entailment}\mimprovised
...
\begin{comments}
  ...
  \item This system is based on natural deduction for first-order logic. Church and his students instead consider a system resembling \hyperref[def:axiomatic_derivation_system]{axiomatic derivations} --- see \cref{rem:hol_axiomatic_derivations}.
\end{comments}
