...
\begin{definition}\label{def:hol_definition}\mimprovised
  ...
  \begin{equation}\label{eq:def:hol_definitional_extension/sequent}
  ...
  A \term[en=\( \Gamma \)-definition (\cite[def. 2.6.11]{Hinman2005Logic})]{definition} for \( f \) in \( \Gamma \) is the following formula in \( \Gamma' \):
  \begin{equation}\label{eq:def:hol_definitional_extension/definition}
...
\begin{comments}
  ...
  \item There is an important special case that can be ignored here but which hints at how to introduce new predicate symbols in first-order logic.

  When \( \sigma = \syn\omicron \) and \( \varphi = (\psi \syneq y^{\syn\omicron}) \) the condition \eqref{eq:def:hol_definitional_extension/sequent} becomes meaningless, \( T \) becomes irrelevant and \eqref{eq:def:hol_definitional_extension/definition} simplifies to
  \begin{equation}\label{eq:def:hol_definitional_extension/definition_simplified}
