...
\begin{remark}\label{con:predicate_logic}
...
\begin{concept}\label{con:predicate_logic}
  In this monograph we present particular formulations of \hyperref[def:higher_order_logic]{higher-order logic} and \hyperref[def:first_order_logic]{first-order logic}. They share a lot of features since they are both based on the subject-predicate sentence structure discussed in \cref{con:predicate_logic}. We will occasionally refer to their commonalities as \term[ru=предикатная логика (\cite[49]{КолмогоровДрагалин2006Логика}), en=predicate logic (\cite[57]{VanDalen2004LogicAndStructure})]{predicate logic}.
\end{concept}
