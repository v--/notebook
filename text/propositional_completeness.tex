...
\begin{proof}
  ...
  \SubProofOf{thm:lindenbaum_tarski_algebras/verum} The rule \ref{eq:def:propositional_natural_deduction_systems/top/intro} allows us to construct a proof of \( \syntop \) from \( \Gamma \cup \set{ \varphi } \) from any formula \( \varphi \), hence \( \varphi \leq_\Gamma \top \). Then \( \pi(\syntop) \) is the unique top element.

  \SubProofOf{thm:lindenbaum_tarski_algebras/falsum} The rule \ref{eq:def:propositional_natural_deduction_systems/bot/efq} allows us to construct a proof of any formula \( \varphi \) from \( \synbot \). Then \( \pi(\synbot) \) is the unique bottom element.

  \SubProofOf{thm:lindenbaum_tarski_algebras/negation} The rule \ref{eq:def:propositional_natural_deduction_systems/neg/intro} allows us to construct a proof of \( \synneg \varphi \) from \( \varphi \) for any formula \( \varphi \).
  ...
  \SubProofOf{thm:lindenbaum_tarski_algebras/classical} Finally, now that we have shown that \( L \) is a Heyting algebra, we must show that it is also a Boolean algebra when we consider the additional rule \ref{eq:def:propositional_natural_deduction_systems/bot/dne}.

  According to \fullref{thm:boolean_algebra_heyting_characterization}, we only need to derive \( \varphi \) from \( \synneg \synneg \varphi \); but this is precisely what \ref{eq:def:propositional_natural_deduction_systems/bot/dne} allows us to do.
\end{proof}
...
\begin{definition}\label{def:complete_set_of_sentences}
  ...
  \begin{thmenum}
    \thmitem{def:complete_set_of_sentences/operator}\mcite[def. III.4]{Woodger1983Tarski} Every \hyperref[def:consistent_set_of_sentences]{consistent} superset of \( \Gamma \) is \hyperref[def:logical_theory/equivalent]{equivalent} to \( \Gamma \).
...
\begin{proposition}\label{thm:consistent_implies_satisfiable}\mcite[thm. 1.3.19]{Герасимов2011}
...
\begin{proof}
  ...
  We proceed via \fullref{thm:induction_on_syntax_trees} on \( \varphi \).
...
\begin{proof}
  ...
  \Fullref{thm:consistent_implies_satisfiable} implies that there exists an interpretation \( I \) such that \( \Delta \vdash \varphi \) if and only if \( I \vDash \varphi \).

  Then \( I \) is a model of \( \Delta \), and hence also of \( \Gamma \).
\end{proof}

\begin{theorem}[Classical propositional completeness]\label{thm:classical_propositional_completeness}\mcite[thm. 1.3.24]{Герасимов2011}
