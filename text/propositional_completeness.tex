...
\begin{definition}\label{def:propositional_theory}\mimprovised
  ...
  First and foremost, a (syntactic or semantic) \term[en=theory (\cite[def. 1.4.5]{Hinman2005Logic})]{theory} is, as in \cref{def:general_logic_theory}, a set of sentences closed under logical consequence. For any set of sentences \( \Gamma \), we denote its consequence closure by \( \op*{Th}(\Gamma) \), and say that \( \Gamma \) \term{axiomatizes} \( \op*{Th}(\Gamma) \).
...
\begin{proposition}\label{thm:def:propositional_theory}
  ...
  \begin{thmenum}
    \thmitem{thm:def:propositional_theory/model_theory_consistent_complete} For every interpretation \( I \), the theory \( \op*{Th}(I) \) is consistent and complete.
  \end{thmenum}
\end{proposition}
\begin{proof}
  \SubProofOf{thm:def:propositional_theory/model_theory_consistent_complete} The interpretation \( I \) either satisfies a sentence or does not, so clearly \( \op*{Th}(I) \) is complete.

  Furthermore, \( \Bracks{\synbot}_I = \semtop \) leads to a contradiction since \( \Bracks{\synbot}_I = \sembot \) by definition, so \( I \) cannot be a model of \( \synbot \) and hence \( \synbot \) does not belong to \( \op*{Th}(I) \). This makes the theory consistent.
\end{proof}
...
\end{corollary}
\begin{proof}
  Let \( \Gamma \) be a consistent set of formulas. Denote by \( \Delta \) the \hyperref[def:propositional_theory/complete]{complete} consistent extension of the \hyperref[def:propositional_theory]{theory} \( \op*{Th}(\Gamma) \) given by \cref{thm:extension_to_complete_consistent_set}.
