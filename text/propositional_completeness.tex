...
\begin{proposition}\label{thm:lindenbaum_tarski_theories}
  ...
  \begin{thmenum}
    \thmitem{thm:lindenbaum_tarski_theories/filter} The set \( T \) of propositional sentences is a \hyperref[def:propositional_theory]{theory} (i.e. is closed under logical consequence) if and only if the projection \( \pi[T] \) is a \hyperref[def:lattice_ideal]{lattice filter} in \( \BbbL \).
...
\begin{proof}
  ...
  \NecessitySubProof* Suppose that \( \pi[T] \) is a lattice filter. Fix some sentence \( \varphi \) such that \( T \vdash \varphi \).

  By the compactness shown in \cref{thm:propositional_natural_deduction_derivation_compact}, there exists a finite subset \( T_0 \) such that \( T_0 \vdash \varphi \). Let \( \delta \) be a conjunction of all formulas in \( T_0 \) (in some order). \Cref{thm:syntactic_propositional_conjunction_of_premises} implies that \( \delta \vdash \varphi \).

  By \cref{thm:lindenbaum_tarski_algebras/minimal/conjunction}, \( \delta \) is a lower bound for the formulas from \( T_0 \); the latter belong to \( T \), hence, by \eqref{eq:def:lattice_ideal/filter_meet}, \( \pi(\delta) \) belongs to \( \pi[T] \).

  Now, since \( \pi[T] \) is upward closed, it follows that \( \pi(\varphi) \) also belongs to \( \pi[T] \).

  Generalizing on \( \varphi \), we conclude that \( T \) is closed under logical consequence.
...
\begin{lemma}[Lindenbaum's lemma for propositional logic]\label{thm:propositional_lindenbaums_lemma}
  For the \hyperref[def:propositional_natural_deduction_system]{classical propositional natural deduction system}, every \hyperref[def:propositional_theory/consistent]{consistent} set of sentences can be extended to a \hyperref[def:propositional_theory/complete]{complete} consistent set.
\end{lemma}
\begin{comments}
...
\begin{theorem}[Propositional model existence theorem]\label{thm:propositional_model_existence_theorem}\mcite[thm. 1.3.19]{Герасимов2014Вычислимость}
  With respect to the \hyperref[def:propositional_natural_deduction_system]{classical propositional natural deduction system}, for every (syntactically) \hyperref[def:propositional_theory/complete]{complete} \hyperref[def:propositional_theory/consistent]{consistent} set of formulas \( \Gamma \), there exists a \hyperref[def:truth_value_algebra/classical]{classical} \hyperref[def:propositional_interpretation]{interpretation} \( I \) such that \( \Gamma \vdash \varphi \) if and only if \( I \vDash \varphi \).
\end{theorem}
\begin{proof}
...
\begin{corollary}\label{thm:consistent_implies_satisfiable}
  If a set of sentences is \hyperref[def:propositional_theory/consistent]{consistent} with respect to the \hyperref[def:propositional_natural_deduction_system]{classical propositional natural deduction system}, it is \hyperref[def:propositional_semantics/satisfaction]{satisfiable} with respect to \hyperref[def:truth_value_algebra/classical]{classical semantics}.
\end{corollary}
\begin{proof}
  Let \( \Gamma \) be a consistent set of formulas. Denote by \( \Delta \) the \hyperref[def:propositional_theory/complete]{complete} consistent extension of the \hyperref[def:propositional_theory]{theory} \( \cat{Th}(\Gamma) \) given by \cref{thm:propositional_lindenbaums_lemma}.

  \Cref{thm:propositional_model_existence_theorem} implies that there exists an interpretation \( I \) such that \( \Delta \vdash \varphi \) if and only if \( I \vDash \varphi \).
...
\begin{proof}
  ...
  The set \( \Gamma \cup \set{ \neg \varphi } \) must be \hyperref[def:propositional_theory/consistent]{inconsistent} since otherwise \cref{thm:consistent_implies_satisfiable} would imply that it is satisfiable.
