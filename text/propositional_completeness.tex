...
\begin{definition}\label{def:propositional_theory}\mimprovised
  ...
  \begin{thmenum}
    ...
    \thmitem{def:propositional_theory/consistent}\mcite[def. 1.5.2]{VanDalen2004LogicAndStructure} We call a propositional theory \term[ru=противоречивое (множество формул) (\cite[def. 1.3.15]{Герасимов2014Вычислимость})]{consistent} if no contradiction can be derived, i.e. if \( \synbot \) is not derivable.
...
\end{corollary}
\begin{proof}
  Let \( \Gamma \) be a consistent set of formulas. Denote by \( \Delta \) the \hyperref[def:propositional_theory/complete]{complete} consistent extension of the \hyperref[def:propositional_theory]{theory} \( \cat{Th}(\Gamma) \) given by \cref{thm:extension_to_complete_consistent_set}.
