...
\begin{comments}
  ...
  \item Within a given \hyperref[def:abstract_logic]{abstract logic}, we will by default refer to syntactic consistency, as a counterpart to \hyperref[def:institutional_satisfaction/satisfiable]{satisfiable theories}.
\end{comments}
...
\begin{theorem}[Classical propositional completeness]\label{thm:classical_propositional_completeness}\mcite[thm. 1.3.24]{Герасимов2011Вычислимость}
  The \hyperref[def:propositional_natural_deduction_systems]{classical propositional natural deduction system} is \hyperref[def:abstract_logic/completeness]{complete} with respect to \hyperref[def:truth_value_algebra/classical]{classical semantics}.
\end{theorem}
\begin{proof}
...
\begin{definition}\label{def:finitely_consistent_set_of_sentences}\mimprovised
  We say that the set of sentences \( \Gamma \) is \term{finitely consistent} with respect to some \hyperref[def:consequence_operator]{consequence operator} or \hyperref[def:consequence_relation]{relation} if every finite subset of \( \Gamma \) is \hyperref[def:consistent_set_of_sentences]{consistent}.
\end{definition}
\begin{comments}
  \item This definition generalizes finitely consistent sets of first-order sentences from \bycite[def. 1.4.21]{Hinman2005Logic}.
\end{comments}

\begin{definition}\label{def:finitary_consequence_relation}\mimprovised
  We associate with every \hyperref[def:consequence_relation]{consequence relation} \( {\vdash} \) on a set of sentences \( S \) a \term{finitary consequence} relation \( {\vdash^*} \) such that \( \Gamma \vdash^* \varphi \) when there exists a \hi{finite} subset \( \Gamma' \) of \( \Gamma \) such that \( \Gamma' \vdash \varphi \).
\end{definition}
\begin{comments}
  \item This definition generalizes the finite semantic consequence relation for first-order logic from \bycite[47]{Hinman2005Logic}.
\end{comments}

\begin{proposition}\label{thm:def:finitary_consequence_relation}
  The \hyperref[def:finitary_consequence_relation]{finitary consequence relation} \( \vdash^* \) corresponding to \( \vdash \) has the following basic properties:
  \begin{thmenum}
    \thmitem{thm:def:finitary_consequence_relation/compactness} The relation \( \vdash \) is \hyperref[def:consequence_relation/compactness]{compact} if and only if it coincides with \( \vdash^* \).

    \thmitem{thm:def:finitary_consequence_relation/consistent} The set \( \Gamma \) of sentences is \hyperref[def:finitely_consistent_set_of_sentences]{finitely consistent} with respect to \( \vdash \) if and only if \( \Gamma \) is \hyperref[def:consistent_set_of_sentences]{consistent} with respect to \( \vdash^* \).
  \end{thmenum}
\end{proposition}
\begin{proof}
  \SubProofOf{thm:def:finitary_consequence_relation/compactness} This is simply a restatement of the definition of compactness.

  \SubProofOf{thm:def:finitary_consequence_relation/consistent}

  \SufficiencySubProof Suppose that \( \Gamma \) is finitely consistent with respect to \( \vdash \).

  Suppose that it is inconsistent with respect to \( \vdash^* \). \Cref{thm:propositional_consistent_set/bot} implies that \( \Gamma \vdash^* \syn\bot \), i.e. there exists a finite subset \( \Gamma' \) such that \( \Gamma' \vdash \syn\bot \). Thus, \( \Gamma' \) is inconsistent with respect to \( \vdash \), and, by, \cref{thm:def:consistent_set_of_sentences/superset}, so is \( \Gamma \).

  The obtained contradiction shows that \( \Gamma \) is consistent with respect to \( \vdash \).

  \NecessitySubProof Suppose that \( \Gamma \) is consistent with respect to \( \vdash^* \).

  Suppose that it is not finitely consistent with respect to \( \vdash \), i.e. that there exists an inconsistent finite subset \( \Gamma' \). Then \cref{thm:propositional_consistent_set/bot} implies that \( \Gamma' \vdash \syn\bot \), and hence \( \Gamma \vdash^* \syn\bot \).

  The obtained contradiction shows that \( \Gamma \) is finitely consistent with respect to \( \vdash \).
\end{proof}
