...
\begin{proposition}\label{thm:lindenbaum_tarski_algebras}
  We will now study the \hyperref[def:lindenbaum_tarski_algebra]{Lindenbaum-Tarski algebra} \( \BbbL_\Gamma \) of the set \( \Gamma \) of \hyperref[def:propositional_formula]{propositional sentences} in the different natural deduction systems from \cref{def:propositional_natural_deduction_systems}.
  ...
  \begin{thmenum}
    ...
    \begin{thmenum}
      ...
      \thmitem{thm:lindenbaum_tarski_algebras/intuitionistic/negation} The \hyperref[eq:def:heyting_algebra/pseudocomplement]{pseudocomplement} \( \oline{\pi_\Gamma(\psi)} \) is \( \pi_\Gamma(\synneg \psi) \).
...
\begin{proof}
  ...
  \SubProofOf{thm:lindenbaum_tarski_algebras/minimal/verum} The rule \ref{inf:def:propositional_natural_deduction_systems/top/intro} requires no premises, hence \( \syntop \) can be derived from no hypotheses, and hence from any sentence \( \varphi \) as a vacuous hypothesis. Thus, \( \varphi \leq \syntop \) for every sentence \( \varphi \), i.e. \( \BbbL \) is bounded from above by \( \pi(\syntop) \).
  ...
  The rule \ref{inf:def:propositional_natural_deduction_systems/or/intro_left} can derive \( \varphi \synvee \psi \) from \( \varphi \) and \ref{inf:def:propositional_natural_deduction_systems/or/intro_right} can derive \( \varphi \synvee \psi \) from \( \psi \), hence \( \varphi \synwedge \psi \) is an upper bound of \( \varphi \) and \( \psi \).
  ...
  \begin{equation*}
    \begin{prooftree}
      ...
      \infer3[\ref{inf:def:propositional_natural_deduction_systems/or/elim}]{ \theta }
    \end{prooftree}
  \end{equation*}
  ...
  The rule \ref{inf:def:propositional_natural_deduction_systems/and/elim_left} can derive \( \varphi \) from \( \varphi \synwedge \psi \) and \ref{inf:def:propositional_natural_deduction_systems/and/elim_right} can derive \( \psi \) from \( \varphi \synvee \psi \), hence \( \varphi \synwedge \psi \) is a lower bound of \( \varphi \) and \( \psi \).
  ...
  \begin{equation*}
    \begin{prooftree}
      ...
      \infer2[\ref{inf:def:propositional_natural_deduction_systems/and/intro}]{ \varphi \synvee \psi }
    \end{prooftree}
  \end{equation*}
  ...
  \SubProofOf{thm:lindenbaum_tarski_algebras/intuitionistic} Suppose we are working in intuitionistic logic, where the rule \ref{inf:def:propositional_natural_deduction_systems/bot/efq} is available.

  \SubProofOf{thm:lindenbaum_tarski_algebras/intuitionistic/falsum} The rule \ref{inf:def:propositional_natural_deduction_systems/bot/efq} allows deriving any formula \( \varphi \) from \( \synbot \), hence \( \synbot \leq \varphi \), i.e. \( \BbbL \) is bounded from below by \( \pi(\synbot) \).
  ...
  \begin{equation*}
    \begin{prooftree}
      \hypo{ [\varphi]^u }
      \hypo{ [\theta]^v }
      \infer2[\ref{inf:def:propositional_natural_deduction_systems/and/intro}]{ \varphi \synwedge \theta }
      \ellipsis {\( P \)} { \psi }
  ...
  \begin{equation*}
    \begin{prooftree}
      ...
      \infer[left label=\( v \)]1[\ref{inf:def:propositional_natural_deduction_systems/neg/intro}]{ \synneg \varphi }
    \end{prooftree}
  \end{equation*}
  ...
  \begin{equation*}
    \begin{prooftree}
      ...
      \hypo{ [\varphi]^v }
      \infer2[\ref{inf:def:propositional_natural_deduction_systems/neg/elim}]{ \synbot }
      \infer[left label=\( v \)]1[\ref{inf:def:propositional_natural_deduction_systems/imp/intro}]{ \varphi \synimplies \synbot }
    \end{prooftree}
  \end{equation*}
  ...
  \SubProofOf{thm:lindenbaum_tarski_algebras/classical} Finally, now that we have shown that \( \BbbL \) is a Heyting algebra, we must show that it is also a Boolean algebra when we consider the additional rule \ref{inf:def:propositional_natural_deduction_systems/bot/raa}.

  \Cref{thm:boolean_algebra_heyting_characterization} implies that \( \pi(\varphi) \leq \pi(\synneg \synneg \varphi) \). We must show the converse inequality; but this is precisely what \ref{inf:def:propositional_natural_deduction_systems/bot/raa} allows us to do:
  \begin{equation*}
    \begin{prooftree}
      \hypo{ [\synneg \synneg \varphi]^u }
      \hypo{ [\synneg \varphi]^v }
      \infer2[\ref{inf:def:propositional_natural_deduction_systems/neg/elim}]{ \synbot }
      \infer[left label=\( v \)]1[\ref{inf:def:propositional_natural_deduction_systems/bot/raa}]{ \varphi }
    \end{prooftree}
  \end{equation*}
\end{proof}
...
\end{comments}
\begin{proof}
  Consider the interpretation \( I: \op*{Prop} \to \BbbL_\Gamma \) obtained as a restriction of \( \pi \) to propositional variables. As a consequence of \cref{thm:lindenbaum_tarski_algebras}, the corresponding \hyperref[def:propositional_denotation]{denotation} is then simply the Lindenbaum-Tarski projection \( \pi_\Gamma: \op*{Form} \to \BbbL_\Gamma \).
...
\begin{proposition}\label{thm:intuitionistic_natural_deduction_is_explosive}
  The \hyperref[def:propositional_natural_deduction_systems]{intuitionistic propositional natural deduction system} is \hyperref[def:paraconsistent_consequence_operator]{explosive}.
\end{proposition}
\begin{proof}
  The rule \ref{inf:def:propositional_natural_deduction_systems/bot/efq} can derive an arbitrary formula from \( \synbot \).
\end{proof}

\begin{proposition}\label{thm:minimal_natural_deduction_is_paraconsistent}
  The \hyperref[def:propositional_natural_deduction_systems]{minimal propositional natural deduction system} is \hyperref[def:paraconsistent_consequence_operator]{paraconsistent}.
\end{proposition}
\begin{proof}
  Without \ref{inf:def:propositional_natural_deduction_systems/bot/efq} or \ref{inf:def:propositional_natural_deduction_systems/bot/raa}, no rule allows introducing an arbitrary formula (e.g. a propositional variable) given only \( \synbot \).
\end{proof}
...
\begin{remark}\label{rem:minimal_propositional_semantics}
  ...
  We have presented a natural deduction system for minimal logic in \cref{def:propositional_natural_deduction_systems} based on \cite[35]{TroelstraSchwichtenberg2000BasicProofTheory}. In \cref{thm:intuitionistic_natural_deduction_is_explosive}, we have shown that this system is \hyperref[def:paraconsistent_consequence_operator]{paraconsistent}. We can use the latter as a guide if we want to define semantics corresponding to this deduction system.

  We have defined propositional denotations in \cref{def:propositional_denotation} so that, for any \hyperref[def:propositional_interpretation]{propositional interpretation} in any \hyperref[def:truth_value_algebra]{truth value algebra} \( \BbbT \), the denotation \( \Bracks{\synbot}_I \) of \( \synbot \) is the \hyperref[def:extremal_points/top_and_bottom]{bottom element} \( \sembot \) of \( \BbbT \).
...
\begin{proof}
  ...
  \SubProof{Proof when \( \varphi = \syntop \)} We have \( \Bracks{\syntop}_I = \semtop \) by definition, and the rule \ref{inf:def:propositional_natural_deduction_systems/top/intro} allows us to derive \( \syntop \) from any set of premises.
  ...
  \SufficiencySubProof* Suppose that \( \Gamma \vdash (\psi \synwedge \theta) \). The rule \ref{inf:def:propositional_natural_deduction_systems/and/elim_left} allows us to extend a proof of \( \psi \synwedge \theta \) from \( \Gamma \) to a proof of \( \psi \) from \( \Gamma \). We can analogously build a proof of \( \theta \).
  ...
  \NecessitySubProof* Conversely, if \( \Bracks{\psi \synwedge \theta}_I = \semtop \), then \( \Bracks{\psi}_I = \Bracks{\theta}_I = \semtop \) and, by the inductive hypothesis, there exist proof trees deriving \( \psi \) and \( \theta \) from \( \Gamma \). We can combine them via \ref{inf:def:propositional_natural_deduction_systems/and/intro}.
  ...
  \begin{equation*}
    \begin{prooftree}
      ...
      \infer[left label=\( u \)]1[\ref{inf:def:propositional_natural_deduction_systems/neg/intro}]{ \synneg (\psi \synvee \theta) }
    \end{prooftree}
  \end{equation*}
  ...
  If \( \Bracks{\psi}_I = \semtop \), the inductive hypothesis implies that \( \Gamma \vdash \psi \), and we can use \ref{inf:def:propositional_natural_deduction_systems/or/intro_left} to conclude that \( \Gamma \vdash (\psi \synvee \theta) \).
  ...
  \begin{itemize}
    ...
    The rule \ref{inf:def:propositional_natural_deduction_systems/imp/intro} then allows us to construct a proof tree deriving \( \psi \synimplies \theta \) from \( \Gamma \).
    ...
    \begin{equation*}
      \begin{prooftree}
        \hypo{ \Gamma }
        \ellipsis {\( P \)} { \synneg \psi }
        \infer1[\ref{inf:def:propositional_natural_deduction_systems/imp/intro}]{ \synneg \theta \synimplies \synneg \psi }
  ...
  If \( \Bracks{\psi} = \semtop \), the inductive hypothesis on \( \psi \) implies that \( \Gamma \vdash \psi \). Then \ref{inf:def:propositional_natural_deduction_systems/iff/elim_right} allows deriving \( \theta \) from \( \Gamma \), which by the inductive hypothesis on \( \theta \) implies that \( \Bracks{\theta} = \semtop \).

  In the other direction, if \( \Bracks{\theta} = \semtop \), we simply use \ref{inf:def:propositional_natural_deduction_systems/iff/elim_left} instead.
  ...
  \begin{itemize}
    \item If \( \Bracks{\psi}_I = \Bracks{\theta}_I = \semtop \), by the inductive hypothesis, we conclude that \( \Gamma \vdash \psi \) and \( \Gamma \vdash \theta \). The rule \ref{inf:def:propositional_natural_deduction_systems/iff/intro} allows constructing a proof tree deriving \( \psi \syniff \theta \) from \( \Gamma \).
    ...
    \begin{equation*}
      \begin{prooftree}
        ...
        \hypo{ [\psi]^v }
        \infer2[\ref{inf:def:propositional_natural_deduction_systems/neg/elim}]{ \synbot }
        \infer1[\ref{inf:def:propositional_natural_deduction_systems/bot/efq}]{ \theta }

        \infer[left label={\( u, v \)}]2[\ref{inf:def:propositional_natural_deduction_systems/iff/intro}]{ \psi \syniff \theta }
      \end{prooftree}
    \end{equation*}
  \end{itemize}
\end{proof}
...
\begin{proof}
  ...
  Since it is inconsistent, there exists a proof tree deriving \( \bot \) from \( \Gamma \cup \set{ \neg \varphi } \). Then we can apply \ref{inf:def:propositional_natural_deduction_systems/bot/raa} and obtain a proof of \( \varphi \) from \( \Gamma \).
\end{proof}
