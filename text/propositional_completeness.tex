...
\paragraph{Propositional theories}
...
\begin{definition}\label{def:propositional_theory}\mimprovised
  ...
  First and foremost, a (syntactic or semantic) \term[en=theory (\cite[def. 1.4.5]{Hinman2005Logic})]{theory} is, as in \cref{def:general_logic_theory}, a set of sentences closed under logical consequence. For any set of sentences \( \Gamma \), we denote its consequence closure by \( \cat{Th}(\Gamma) \), and say that \( \Gamma \) \term{axiomatizes} \( \cat{Th}(\Gamma) \).
  ...
  \begin{thmenum}
    ...
    \thmitem{def:propositional_theory/extension} We say that \( T^+ \) is an \term{extension} of \( T \) if it is a superset of \( T \).

    \thmitem{def:propositional_theory/category} Based on propositional theories and their extensions, as per \cref{def:category_of_theories}, we have a (syntactic or semantic) category of small theories \( \cat{Th} \).

    \thmitem{def:propositional_theory/model}\mcite[def. 1.4.10]{Hinman2005Logic} As in \cref{def:theory_of_institutional_model}, we define the \term{theory} \( \cat{Th}(I) \) \emph{of} a propositional interpretation \( I \) as the set of all sentences valid in \( I \).

    \thmitem{def:propositional_theory/consistent}\mcite[def. 1.5.2]{VanDalen2004LogicAndStructure} We call a propositional theory \term[ru=противоречивое (множество формул) (\cite[def. 1.3.15]{Герасимов2014Вычислимость})]{consistent} if \( T \) contains no \hyperref[def:propositional_semantics/tautology]{contradictions}, i.e. if \( T \) does not contain \( \synbot \).

    More generally, a set of sentences \( T \) is consistent if \( \synbot \) cannot be derived from it.

    We will discuss consistent theories in a general setting in \cref{def:consistent_set_of_sentences}, and give equivalent conditions for syntactic theories in \cref{thm:propositional_semantic_inconsistency} and for semantic theories in \cref{thm:propositional_semantic_inconsistency}. The latter also shows that, for \hyperref[def:truth_value_algebra/degenerate]{non-degenerate semantics}, semantic consistency is equivalent to \hyperref[def:propositional_semantics/satisfaction]{satisfiability}.

    Equivalence of semantic and syntactic consistency follows from \fullref{thm:propositional_natural_deduction_soundness} and \fullref{thm:classical_propositional_completeness} (or \fullref{thm:intuitionistic_propositional_completeness}).

    \thmitem{def:propositional_theory/complete}\mcite[def. 1.4.8(ii)]{Hinman2005Logic} In \hyperref[con:classical_logic]{classical logic}, we call a propositional theory \( T \) \term[ru=полное (множество формул) (\cite[def. 1.3.16]{Герасимов2014Вычислимость})]{complete} if, for every sentence \( \varphi \), \( T \) contains either \( \varphi \) or \( \synneg \varphi \) (or possibly both, if the theory is inconsistent).
    ...
    We will discuss complete theories in a general setting in \cref{def:complete_set_of_sentences}, and prove in \cref{thm:propositional_syntactically_complete_set} that it coincides with our notion when restricted to classical propositional logic.
  \end{thmenum}
\end{definition}

\begin{proposition}\label{thm:def:propositional_theory}
  \hyperref[def:propositional_theory]{Propositional theories} have the following basic properties:
  \begin{thmenum}
    \thmitem{thm:def:propositional_theory/model_theory_consistent_complete} For every interpretation \( I \), the theory \( \cat{Th}(I) \) is consistent and complete.
  \end{thmenum}
\end{proposition}
\begin{proof}
  \SubProofOf{thm:def:propositional_theory/model_theory_consistent_complete} The interpretation \( I \) either satisfies a sentence or does not, so clearly \( \cat{Th}(I) \) is complete.

  Furthermore, \( \Bracks{\synbot}_I = \semtop \) leads to a contradiction since \( \Bracks{\synbot}_I = \sembot \) by definition, so \( I \) cannot be a model of \( \synbot \) and hence \( \synbot \) does not belong to \( \cat{Th}(I) \). This makes the theory consistent.
\end{proof}
...
\end{definition}
\begin{comments}
  \item For \hyperref[def:propositional_logic]{propositional logic}, inconsistent sets are characterized in \cref{thm:propositional_syntactic_inconsistency} and \cref{thm:propositional_semantic_inconsistency}.
...
\begin{proposition}\label{thm:propositional_syntactic_inconsistency}
  With respect to \hyperref[def:propositional_natural_deduction]{intuitionistic natural deduction}, the following conditions are equivalent for a set \( \Gamma \) of \hyperref[def:propositional_formula]{propositional sentences}:
  \begin{thmenum}
    \thmitem{thm:propositional_syntactic_inconsistency/abstract} The set \( \Gamma \) is \hyperref[def:consistent_set_of_sentences]{inconsistent}, i.e. every formula is derivable from \( \Gamma \).

    \thmitem{thm:propositional_syntactic_inconsistency/contradiction} For every formula \( \varphi \), both \( \varphi \) and \( \synneg \varphi \) are derivable from \( \Gamma \).

    \thmitem{thm:propositional_syntactic_inconsistency/bot} The falsum \( \synbot \) is derivable from \( \Gamma \).
  \end{thmenum}
\end{proposition}
\begin{comments}
  \item Via \fullref{alg:fol_propositional_formula_translation}, these characterizations also extend to \hyperref[def:first_order_logic]{first-order logic}.

  \item This is the syntactic counterpart to \cref{thm:propositional_semantic_inconsistency}.
\end{comments}
\begin{proof}
  \ImplicationSubProof{thm:propositional_syntactic_inconsistency/abstract}{thm:propositional_syntactic_inconsistency/contradiction} Trivial.

  \ImplicationSubProof{thm:propositional_syntactic_inconsistency/contradiction}{thm:propositional_syntactic_inconsistency/bot} Let \( P \) and \( N \) be \hyperref[def:propositional_natural_deduction_proof_tree]{proof trees} of \( \varphi \) and \( \synneg \varphi \) with assumptions in \( \Gamma \). Then the following is a proof of \( \synbot \) with the same assumptions:
  \begin{equation*}
    \begin{prooftree}
      \hypo{ \Gamma }
  ...
  \ImplicationSubProof{thm:propositional_syntactic_inconsistency/bot}{thm:propositional_syntactic_inconsistency/abstract} Let \( P \) be a proof tree of \( \synbot \) with assumptions from \( \Gamma \). Then the following is a proof of an arbitrary formula \( \varphi \) with the same assumptions:
  \begin{equation*}
    \begin{prooftree}
      \hypo{}
      \ellipsis { \( P \) } { \synbot }
...
\begin{proposition}\label{thm:propositional_semantic_inconsistency}
  With respect to \hyperref[def:truth_value_algebra/intuitionistic]{intuitionistic} \hyperref[def:propositional_semantics/entailment]{propositional entailment relation}, the following conditions are equivalent for a set \( \Gamma \) of \hyperref[def:propositional_formula]{propositional sentences}:
  \begin{thmenum}
    ...
    \thmitem{thm:propositional_semantic_inconsistency/abstract} The set \( \Gamma \) is inconsistent in the sense of \cref{def:consistent_set_of_sentences}, i.e. \( \Gamma \) entails every formula.
...
\begin{corollary}\label{thm:classical_propositional_satisfiable_implies_consistent}
  If a set of propositional sentences is \hyperref[def:propositional_semantics/satisfaction]{satisfiable} with respect to \hyperref[def:truth_value_algebra/classical]{classical semantics}, it is \hyperref[def:propositional_theory/consistent]{consistent} with respect to \hyperref[def:propositional_natural_deduction_system]{classical natural deduction}.
\end{corollary}
\begin{itemize}
  \item Via \fullref{alg:fol_propositional_formula_translation}, these characterizations also extend to \hyperref[def:first_order_logic]{first-order logic}.
\end{itemize}
\begin{proof}
  Suppose that \( \Gamma \) is a satisfiable set of sentences. By \cref{thm:propositional_semantic_inconsistency}, \( \Gamma \) does not semantically entail \( \synbot \).

  Suppose that \( \Gamma \) is syntactically inconsistent, which due to \cref{thm:propositional_syntactic_inconsistency} is equivalent to \( \Gamma \vdash \synbot \). \Fullref{thm:propositional_natural_deduction_soundness} implies that \( \Gamma \vDash \synbot \).

  The obtained contradiction demonstrates that \( \Gamma \) must be syntactically consistent.
\end{proof}
...
\begin{proposition}\label{thm:inconsistent_lindenbaum_tarski_algebra}
  A \emph{nonempty} set of sentences \( \Gamma \) is \hyperref[def:consistent_set_of_sentences]{inconsistent} if and only if its \hyperref[def:lindenbaum_tarski_algebra]{Lindenbaum-Tarski algebra} \( \BbbL_\Gamma \) is trivial, i.e. contains only one equivalence class.
\end{proposition}
\begin{proof}
...
\begin{comments}
  ...
  \item See \cref{ex:con:curry_howard_correspondence/algebraic_types} for how this statement relates to \hyperref[def:simple_algebraic_types]{simple algebraic types} via the \hyperref[con:curry_howard_correspondence]{Curry-Howard correspondence}.
\end{comments}
\begin{proof}
  ...
  \SubProofOf{thm:lindenbaum_tarski_algebras/intuitionistic/negation} The pseudocomplement of \( \pi(\varphi) \) is by definition \( \pi(\varphi) \rightarrow \pi(\synbot) \). \Cref{thm:lindenbaum_tarski_algebras/intuitionistic/conditional} implies that
  \begin{equation*}
...
\begin{corollary}\label{thm:intuitionistic_propositional_soundness_and_completeness}
  For \hyperref[def:propositional_formula]{propositional sentences}, we have \( \Gamma \vdash \varphi \) in the \hyperref[def:propositional_natural_deduction]{intuitionistic natural deduction system} if and only if \( \Gamma \vDash \varphi \) with respect to \emph{every} \hyperref[def:truth_value_algebra]{truth value algebra}.
\end{corollary}
\begin{proof}
...
\begin{remark}\label{rem:minimal_propositional_semantics}
  Minimal logic was introduced by Ingebrigt Johansson as an attempt to further refine intuitionistic logic. An analysis of his works and his interactions with Gerhard Gentzen is given by \textcite{VanDerMolen2016MinimalLogic}.
...
\begin{definition}\label{def:complete_set_of_sentences}
  ...
  \begin{thmenum}
    \thmitem{def:complete_set_of_sentences/operator}\mcite[def. 4]{Tarski1983FundamentalConceptsOfMetamathematics} Every \hyperref[def:consistent_set_of_sentences]{consistent} superset of \( \Gamma \) is \hyperref[def:propositional_semantics/equivalence]{equivalent} to \( \Gamma \).
...
\end{definition}
\begin{comments}
  \item For \hyperref[con:classical_logic]{classical} \hyperref[def:propositional_logic]{propositional logic}, complete sets are characterized in \cref{thm:propositional_syntactically_complete_set}.
...
\begin{proposition}\label{thm:propositional_syntactically_complete_set}
  With respect to the \hyperref[def:propositional_natural_deduction]{classical natural deduction system}, the set \( \Gamma \) of \hyperref[def:propositional_formula]{propositional sentences} is \hyperref[def:complete_set_of_sentences]{complete} if and only if, for any formula \( \varphi \), either \( \Gamma \vdash \varphi \) or \( \Gamma \vdash \synneg \varphi \) (or possibly both, if \( \Gamma \) is \hyperref[def:propositional_theory/consistent]{inconsistent}).
\end{proposition}
\begin{comments}
...
\begin{proof}
  ...
  \begin{itemize}
    \item Suppose that \( \Gamma \cup \set{ \varphi } \) is inconsistent. \Cref{thm:propositional_syntactic_inconsistency/bot} implies that there exists a proof \( P \) deriving \( \synbot \) from \( \Gamma \cup \set{ \varphi } \).
    ...
    \begin{itemize}
      ...
      \item Otherwise, \( P \) is a proof of \( \synbot \) from \( \Gamma \), and \cref{thm:propositional_syntactic_inconsistency/bot} implies that \( \Gamma \) is inconsistent. \Cref{thm:propositional_syntactic_inconsistency/contradiction} then implies that both \( \varphi \) and \( \synneg \varphi \) are derivable from \( \Gamma \).
    \end{itemize}
  ...
  Clearly \( \Gamma, \varphi \vdash \varphi \). Aiming at a contradiction, suppose that \( \Gamma \nvdash \varphi \). Then, by assumption, we have \( \Gamma \vdash \synneg \varphi \), and, after adding \( \varphi \) as a hypothesis, we have \( \Gamma, \varphi \vdash \synneg \varphi \). Then \( \Gamma \cup \set{ \varphi } \) satisfies \cref{thm:propositional_syntactic_inconsistency/bot} and is thus inconsistent, contradicting our assumption.
...
\begin{proposition}\label{thm:lindenbaum_tarski_theories}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:lindenbaum_tarski_theories/consistent} The theory \( T \) is \hyperref[def:propositional_theory/consistent]{inconsistent} if and only if \( \pi[T] = \BbbL \).

    \thmitem{thm:lindenbaum_tarski_theories/complete} In classical logic, the consistent theory \( T \) is \hyperref[def:complete_set_of_sentences]{complete} if and only if \( \pi[T] \) is an \hyperref[def:ultrafilter]{ultrafilter}.
  \end{thmenum}
\end{proposition}
\begin{proof}
  ...
  \SufficiencySubProof* Suppose that \( T \) is a propositional theory.

  The family \( \pi[T] \) is clearly \hyperref[def:closed_ordered_subset]{upward closed}. Indeed, if \( \varphi \) is in \( T \) and if \( \varphi \vdash \psi \), since \( T \) is a theory, \( \psi \) also belongs to \( T \). Thus, if \( \pi(\varphi) \leq \pi(\psi) \) and \( \pi(\varphi) \) belongs to \( \pi[T] \), so does \( \pi(\psi) \).

  We must also show that \( \pi[T] \) is \hyperref[def:directed_set]{downward directed}. Fix two formulas \( \varphi \) and \( \psi \) in \( T \). Then \( \varphi \synwedge \psi \) is also in \( T \). \Cref{thm:lindenbaum_tarski_algebras/minimal/conjunction} implies that the coset \( \pi(\varphi \synwedge \psi) \) is the infimum of \( \pi(\varphi) \) and \( \pi(\psi) \). Then every two elements of \( \pi[T] \) have a lower bound, i.e. \( \pi[T] \) is downward directed.

  Therefore, \( \pi[T] \) is a lattice filter.
  ...
  \SufficiencySubProof* Suppose that \( T \) is inconsistent. Then it can derive any propositional sentence, and, since it is a theory, it also contains every propositional sentence. Naturally, \( \pi[T] = \BbbL \).

  \NecessitySubProof* Suppose that \( \pi[T] = \BbbL \). Fix any formula \( \varphi \). The projection \( \pi_\Gamma(\varphi) \) is in \( \pi[T] \), implying that \( T \vdash \varphi \).

  Generalizing on \( \varphi \), we conclude that \( T \) is inconsistent.

  \SubProofOf{thm:lindenbaum_tarski_theories/complete} \Cref{thm:propositional_syntactically_complete_set} corresponds precisely to \cref{def:ultrafilter/direct}.
\end{proof}

\paragraph{Propositional completeness}

\begin{lemma}[Lindenbaum's lemma for propositional logic]\label{thm:propositional_lindenbaums_lemma}
  For the \hyperref[def:propositional_natural_deduction_system]{classical propositional natural deduction system}, every \hyperref[def:propositional_theory/consistent]{consistent} set of sentences can be extended to a \hyperref[def:propositional_theory/complete]{complete} consistent \emph{theory}.
\end{lemma}
\begin{comments}
  \item The name of the lemma is taken from \cite[lemma 1.3.21]{Герасимов2014Вычислимость}, but the proof is simplified.
  \item This is an analogue of \fullref{thm:fol_lindenbaums_lemma}.
\end{comments}
\begin{proof}
  Denote by \( \BbbL \) the \hyperref[thm:lindenbaum_tarski_algebras]{classical Lindenbaum-Tarski algebra} with no premises.

  Let \( \Gamma \) be a consistent set of formulas. Consider the \hyperref[def:propositional_theory]{theory} \( \cat{Th}(\Gamma) \) axiomatized by \( \Gamma \). \Cref{thm:lindenbaum_tarski_theories} implies that \( \pi[\cat{Th}(\Gamma)] \) is a proper filter of \( \BbbL \). \Fullref{thm:ultrafilter_lemma} implies that there exists an ultrafilter \( M \) containing \( \pi[\cat{Th}(\Gamma)] \).
...
\begin{theorem}[Propositional model existence theorem]\label{thm:propositional_model_existence_theorem}\mcite[thm. 1.3.19]{Герасимов2014Вычислимость}
  With respect to the \hyperref[def:propositional_natural_deduction_system]{classical propositional natural deduction system}, for every \hyperref[def:propositional_theory/complete]{complete} \hyperref[def:propositional_theory/consistent]{consistent} set of formulas \( \Gamma \), there exists a \hyperref[def:truth_value_algebra/classical]{classical} \hyperref[def:propositional_interpretation]{interpretation} \( I \) such that \( \Gamma \vdash \varphi \) if and only if \( I \vDash \varphi \).
\end{theorem}
\begin{proof}
  ...
  \SufficiencySubProof* The assumption \( \Gamma \vdash \synbot \) leads to a contradiction because by \cref{thm:propositional_syntactic_inconsistency/bot}, \( \Gamma \) must be inconsistent. But we have assumed that \( \Gamma \) is consistent.
  ...
  By the inductive hypothesis on \( \psi \), if \( \Bracks{\psi}_I = \semtop \), we have \( \Gamma \vdash \psi \).

  \Cref{thm:propositional_syntactic_deduction_theorem} implies that \( \Gamma, \psi \vdash \theta \). From the \hyperref[def:consequence_relation/transitivity]{transitivity} of the derivability relation, we conclude that \( \Gamma \vdash \theta \). Thus, by the inductive hypothesis on \( \theta \), we have \( \Bracks{\theta}_I = \semtop \).
  ...
  \begin{itemize}
    ...
    \item If \( \Gamma \nvdash \psi \), since \( \Gamma \) is complete, \cref{thm:propositional_syntactically_complete_set} implies that \( \Gamma \vdash \synneg \psi \).
  ...
  \begin{itemize}
    ...
    Since \( \Gamma \) is complete, \cref{thm:propositional_syntactically_complete_set} implies that \( \Gamma \vdash \synneg \psi \) and \( \Gamma \vdash \synneg \theta \). Let \( P \) be a proof tree deriving \( \psi \) from \( \Gamma \), and let \( R \) be a tree deriving \( \theta \).
...
\begin{corollary}\label{thm:propositional_consistent_implies_satisfiable}
  If a set of propositional sentences is (syntactically) \hyperref[def:propositional_theory/consistent]{consistent} with respect to \hyperref[def:propositional_natural_deduction_system]{classical natural deduction}, it is \hyperref[def:propositional_semantics/satisfaction]{satisfiable} with respect to \hyperref[def:truth_value_algebra/classical]{classical semantics}.
\end{corollary}
\begin{comments}
  \item This statement has an analog for both \hyperref[def:first_order_logic]{first-order logic} and \hyperref[def:higher_order_logic]{higher-order logic} --- see \cref{thm:fol_consistent_implies_satisfiable} and \cref{thm:hol_consistent_implies_satisfiable}, respectively.
\end{comments}
\begin{proof}
  Let \( \Gamma \) be a syntactically consistent set of sentences. Denote by \( T \) the \hyperref[def:propositional_theory/complete]{complete} consistent theory extending \( \Gamma \), which we obtain from \cref{thm:propositional_lindenbaums_lemma}.

  \Cref{thm:propositional_model_existence_theorem} implies that there exists an interpretation \( I \) such that \( T \vdash \varphi \) if and only if \( I \vDash \varphi \).

  Then \( I \) is a model of \( T \), and hence also of \( \Gamma \).
\end{proof}
...
\begin{theorem}[Classical propositional completeness]\label{thm:classical_propositional_completeness}\mcite[thm. 1.3.24]{Герасимов2014Вычислимость}
  The \hyperref[def:propositional_natural_deduction]{classical propositional natural deduction system} is \hyperref[def:general_logic/completeness]{complete} with respect to \hyperref[def:propositional_semantics]{classical propositional semantics}.
\end{theorem}
\begin{comments}
  ...
  \item This is the classical counterpart of \fullref{thm:intuitionistic_propositional_completeness} and the propositional counterpart of \fullref{thm:fol_completeness} and \fullref{thm:hol_weak_completeness}.
\end{comments}
\begin{proof}
  ...
  The set \( \Gamma \cup \set{ \neg \varphi } \) must be \hyperref[def:propositional_theory/consistent]{inconsistent} since otherwise \cref{thm:propositional_consistent_implies_satisfiable} would imply that it has a model.
...
\begin{theorem}[Classical propositional semantic compactness]\label{thm:classical_propositional_semantic_compactness}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:classical_propositional_semantic_compactness/consistent} A set of sentences is \hyperref[def:propositional_theory/consistent]{consistent} (does not entail \( \synbot \)) if and only if it is \hyperref[def:finitely_consistent_set_of_sentences]{finitely consistent} (no finite subset entails \( \synbot \)).
  \end{thmenum}
\end{theorem}
\begin{comments}
  ...
  \item \Textcite*[47]{Hinman2005Logic} calls \cref{thm:classical_propositional_semantic_compactness/relation} the \enquote{propositional compactness theorem} and \cref{thm:classical_propositional_semantic_compactness/satisfiable} the \enquote{alternative propositional compactness theorem}.
...
\begin{proof}
  ...
  Aiming at a contradiction, suppose that \( \Gamma \) is inconsistent. \Cref{thm:propositional_syntactic_inconsistency/bot} implies that \( \Gamma \vDash \synbot \). Then, by compactness, there exists a finite subset \( \Gamma_0 \) such that \( \Gamma_0 \vDash \synbot \). Again by \cref{thm:propositional_syntactic_inconsistency/bot}, \( \Gamma_0 \) is inconsistent, contradicting the assumptions that \( \Gamma \) is finitely consistent.
