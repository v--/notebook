...
\begin{comments}
  ...
  \incite[10]{MacLane1998} uses the term \enquote{graphs} for what we call directed multigraphs, and uses them to define categories. For the purposes of category theory, he makes no finiteness assumptions.
\end{comments}
...
\begin{comments}
  ...
  \incite[2]{GondranMinoux1984Graphs} use the term \enquote{graph} to refer to what we call (simple) directed graphs, but allow loops.
\end{comments}
...
\begin{comments}
  ...
  \item Reinhard Diestel defines simple undirected graphs in \cite[2]{Diestel2005} as \enquote{graphs}, and so do \incite[1]{Bollobas1998} and \incite[2]{GodsilRoyle2001}. Michel Gondran and Michel Minoux define (simple) undirected graphs in \cite[3]{GondranMinoux1984Graphs}, after already introducing (simple) directed graphs.
...
\begin{definition}\label{def:graph_walk}\mcite[10]{Diestel2005}
  ...
  \begin{thmenum}[resume=def:graph_walk]
    ...
    \thmitem{def:graph_walk/path} We say that the walk is a \term[ru=путь (\cite[def. 1.1.15]{Карпов2017})]{path} if it has no repeated vertices\fnote{Having no repeated vertices also implies having no repeated edges, but not vice versa.}. Unless the path is closed, it can be fully recovered from its induced subgraph, for which reason we often conflate the two.
...
\end{definition}
\begin{comments}
  \item \incite[6]{Diestel2005} and \incite[4]{Bollobas1998} define paths as \hi{undirected} graphs with an implicit ordering of the vertices, and later treat them as special cases of walks. \incite[165]{GodsilRoyle2001} and Jeff Erickson in \cite[191]{Erickson2019} give a simpler definition that involves only vertices (i.e. without arcs and edges), but is ambiguous in the case of multigraphs.

  \item \incite[29]{GodsilRoyle2001} use the temporary term \term{weak path} for what we call a generalized path.

  \item \incite[13]{GondranMinoux1984Graphs} instead define paths as sequences of adjacent arcs, and call our definition of paths \enquote{elementary paths}.
...
\end{definition}
\begin{comments}
  \item \incite[4]{GodsilRoyle2001} actually define a cycle as a \hyperref[def:graph_connectedness]{connected graph} in which every vertex has degree \( 2 \). This differs from our definition in that walks have endpoints, while their subgraphs do not.
  ...
  \incite[7]{Diestel2005} and \incite[5]{Bollobas1998} both define them as a graph constructed from a path with its head connecting to its tail, and require them to have at least three vertices. This latter condition filters out degenerate cases like \( a \to b \to a \).
\end{comments}
