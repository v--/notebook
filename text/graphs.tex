...
\begin{definition}\label{def:directed_graph}\mcite[2]{GodsilRoyle2001}
  ...
  \begin{thmenum}[series=def:directed_graph]
    ...
    \thmitem{def:directed_graph/arcs} A set \( A_G \) of \hyperref[def:cartesian_product/kuratowskh_pair]{ordered pairs} of \hi{distinct} vertices. As in the case of directed multigraphs, we call the elements of \( A_G \) \term{arcs}.
  \end{thmenum}
...
\begin{definition}\label{def:graph_functors}\mimprovised
  ...
  \begin{thmenum}
    ...
    \thmitem{def:graph_functors/multi_forgetful} A functor that forgets the \hyperref[def:graph_orientation]{orientation} of a multigraph:
    \begin{flalign*}
...
\begin{remark}\label{rem:graphs_linear_algebra_and_topology}
  As we shall see, the \hyperref[def:graph_adjacency]{adjacency} and \hyperref[def:graph_incidence]{incidence} of a directed multigraph can be studied using linear algebra via \hyperref[def:graph_adjacency_matrix]{adjacency} and \hyperref[def:directed_incidence_matrix]{incidence matrices}, while \hyperref[def:graph_connectedness]{connectedness} can be studied using \hyperref[def:graph_connectedness]{topology} via \hyperref[def:directed_multigraph_geometric_realization/embedding]{graph embeddings}.
\end{remark}
...
\begin{definition}\label{def:graph_walk}\mcite[10]{Diestel2005}
  ...
  \begin{thmenum}[series=def:graph_walk]
    ...
    \thmitem{def:graph_walk/generalized}\mimprovised Again in a directed multigraph, we say that \eqref{eq:def:graph_walk} is a \term{generalized walk} if \( \set{ i(e_k), t(e_k) } = \set{ v_{k-1}, v_k } \) for every \( k = 1, \ldots, n \).

    It thus makes sense to reverse some arrows, e.g.
    \begin{equation*}
...
\begin{remark}\label{rem:undirected_walks}
  ...
  The doubling functor \( D_S \) from \fullref{def:graph_functors/simple_doubling} suggests defining walks in a \hyperref[def:undirected_graph]{simple undirected graph} \( G \) as walks in \( D_S(G) \). The following is the same path in the graph's undirected counterpart \hyperref[def:graph_functors/single_forgetful]{\( U_S(D) \)}:
  \begin{equation*}
    \begin{aligned}
...
\begin{proposition}\label{thm:connected_components_partition_graph}
  The (weak/strong) \hyperref[def:graph_connected_component]{connected components} of a \hyperref[def:arbitrary_graph]{graph} \hyperref[def:set_partition]{partition} its vertices.
\end{proposition}
\begin{proof}
  Trivial.
\end{proof}
...
\begin{example}\label{ex:def:directed_multigraph_condensation}
  The \hyperref[def:directed_multigraph_condensation]{condensation} of the directed multigraph \eqref{eq:fig:def:directed_multigraph} is (isomorphic to) the graph itself. If we add the arc \( f \to a \) to \eqref{eq:fig:def:directed_multigraph}, the condensation would be an edgeless directed multigraph with a single vertex.

  We can instead add new arcs \( e_8 \) and \( e_9 \) to \eqref{eq:fig:def:directed_multigraph} to make the example more interesting:
  \begin{equation}\label{eq:ex:def:directed_multigraph_condensation/uncondensed}
    \begin{aligned}
  ...
  Its condensation is
  \begin{equation}\label{eq:ex:def:directed_multigraph_condensation/condensed}
    \begin{aligned}
      \includegraphics[page=2]{output/ex__def__directed_graph_condensation.pdf}
    \end{aligned}
  \end{equation}

  These are the \hyperref[def:graph_connectedness/strong]{strongly connected components} of \eqref{eq:ex:def:directed_multigraph_condensation/uncondensed}.
\end{example}

\begin{definition}\label{def:graph_cycle}\mcite[4]{GodsilRoyle2001}
  We say that a \hyperref[def:graph_walk]{walk} is a \term{cycle} if every vertex has \hyperref[def:graph_cardinalities/degree]{degree} \( 2 \) (in its \hyperref[def:graph_walk/subgraph]{induced subgraph}).
\end{definition}
\begin{comments}
...
\begin{definition}\label{def:acyclic_graph}\mcite[4]{GodsilRoyle2001}
  If a \hyperref[def:arbitrary_graph]{graph} does not have a directed \hyperref[def:graph_cycle]{cycle}, we say that it is \term{acyclic}.
\end{definition}

\begin{proposition}\label{thm:graph_condensation_is_acyclic_dag}
  The \hyperref[def:directed_multigraph_condensation]{condensation} of a directed multigraph is a \hyperref[def:undirected_multigraph_path]{directed acyclic graph}.
\end{proposition}
\begin{proof}
...
\begin{example}\label{ex:well_founded_graphs}
  We can regard any set \( A \) in the sense of \hyperref[def:zfc]{\logic{ZFC}} as the \hyperref[def:simple_directed_graph]{simple directed graph} \( (A, \in) \).
