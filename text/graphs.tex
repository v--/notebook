...
\begin{definition}\label{def:directed_multigraph}\mcite[def. 1.1.1; def. 1.1.2]{Knauer2011AlgGraphTheory}
  A \term[bg=ориентиран (\cite[6]{Мирчев2001Графи}) мултиграф (\cite[7]{Мирчев2001Графи}), ru=ориентированый мультиграф (\cite[16]{ЕмеличевИПр1990Графы})]{directed multigraph} is a quadruple \( G = (V, A, h, t) \) consisting of:
  \begin{thmenum}[series=def:directed_multigraph]
    \thmitem{def:directed_multigraph/vertices} A set \( V \), whose elements we call \term[bg=върхове (\cite[6]{Мирчев2001Графи}), ru=вершины (\cite[279]{ЕмеличевИПр1990Графы})]{vertices}.

    \thmitem{def:directed_multigraph/arcs} A disjoint from \( V \) set \( A \), whose elements we call \term[bg=дъги (\cite[6]{Мирчев2001Графи}), ru=дуги (\cite[279]{ЕмеличевИПр1990Графы})]{arcs}.

    \thmitem{def:directed_multigraph/head} A function \( h: A \to V \), giving the \term[en=head (\cite[544]{Rosen1999DiscreteHandbook})]{head} or \term[bg=начален връх (\cite[7]{Мирчев2001Графи}), ru=начало (\cite[279]{ЕмеличевИПр1990Графы}), en=initial vertex (\cite[28]{Diestel2005Graphs})]{initial endpoint} of an arc.

    \thmitem{def:directed_multigraph/tail} A function \( t: A \to V \), giving the \term[en=tail (\cite[544]{Rosen1999DiscreteHandbook})]{tail} or \term[bg=краен връх (\cite[7]{Мирчев2001Графи}), ru=конец (\cite[279]{ЕмеличевИПр1990Графы}), en=terminal vertex (\cite[28]{Diestel2005Graphs})]{terminal endpoint} of an arc.
  \end{thmenum}
  ...
  \begin{thmenum}[resume=def:directed_multigraph]
    \thmitem{def:directed_multigraph/loop} We call the arc \( e \) a \term[bg=примка (\cite[7]{Мирчев2001Графи}), ru=петля (\cite[279]{ЕмеличевИПр1990Графы})]{loop} if \( h(e) = t(e) \).

    \medskip

    \thmitem{def:directed_multigraph/parallel}\mcite[28]{Diestel2005Graphs} We call the arcs \( e \) and \( f \) \term[bg=паралелни (ребра) (\cite[7]{Мирчев2001Графи}), ru=параллельные (рёбра) (\cite[279]{ЕмеличевИПр1990Графы})]{parallel} if \( h(e) = h(f) \) and \( t(e) = t(f) \) and \term{opposite} if it is not a loop and \( h(e) = t(f) \) and \( t(e) = h(f) \).
...
\begin{remark}\label{rem:digraph}
  ...
  Similarly, in Russian, \enquote{орграф} is used as a shorthand for \enquote{ориентированный граф}, for example by \incite[\S 7.1.5]{Новиков2013ДискретнаяМатематика} or \incite[279]{ЕмеличевИПр1990Графы}.
...
\begin{definition}\label{def:hypergraph}\mcite[def. 1.1.2]{Knauer2011AlgGraphTheory}
  ...
  \begin{thmenum}[series=def:hypergraph/multigraph]
    \thmitem{def:hypergraph/vertices} A set \( V \), whose elements we call \term[ru=вершины (\cite[298]{ЕмеличевИПр1990Графы})]{vertices}.

    \thmitem{def:hypergraph/edges} A disjoint from \( V \) set \( E \), whose elements we call \term[ru=рёбра (\cite[298]{ЕмеличевИПр1990Графы})]{edges}.
  ...
  \begin{thmenum}[resume=def:hypergraph/multigraph]
    ...
    \thmitem{def:hypergraph/uniform}\mcite[299]{ЕмеличевИПр1990Графы} If all edges have \( \kappa \) vertices, we say that the hypergraph is \term[ru=\( \kappa \)-однородный, en=\( \kappa \)-uniform (\cite[629]{Rosen1999DiscreteHandbook})]{\( \kappa \)-uniform}.
...
\end{definition}
\begin{comments}
  \item Our definition of hypergraphs is mostly based on \incite*{DorflerWaller1980Hypergraphs}, with some additional notions from \incite*[ch. XI]{ЕмеличевИПр1990Графы} and \cite[sec. 8.12]{Rosen1999DiscreteHandbook} by Andreas Gyarfas.
...
\begin{remark}\label{rem:simple_graphs}
  ...
  \begin{itemize}
    ...
    \item \incite[9]{Harary1969Graphs}, \incite[2]{Diestel2005Graphs}, \incite[1]{Bollobas1998Graphs}, \incite[9]{ЕмеличевИПр1990Графы} and \incite[\S 7.1.2]{Новиков2013ДискретнаяМатематика} define directed graphs as pairs \( (V, E) \), where \( E \) consists of two-element subsets of \( V \). This explicitly forbid loops.
...
\begin{comments}
  ...
  \incite[3]{Diestel2005Graphs}, \incite[3]{Bollobas1998Graphs}, \incite[def. 4.1.3]{Knauer2011AlgGraphTheory}, \incite[13]{ЕмеличевИПр1990Графы} and \incite[def. 5.14]{БелоусовТкачёв2004ДискретнаяМатематика}, \incite[\S 7.1.6]{Новиков2013ДискретнаяМатематика} define simple undirected graph isomorphisms via \eqref{eq:thm:graph_isomorphisms/simple_undirected}, however avoid discussing isomorphisms for other kinds of graphs.
\end{comments}
\begin{proof}
...
\begin{definition}\label{def:graph_cardinality}
  ...
  \begin{thmenum}
    \thmitem{def:graph_cardinality/order}\mcite[2]{Diestel2005Graphs} We define the \term[ru=порядок (\cite[9]{ЕмеличевИПр1990Графы})]{order} \( \ord(G) \) of an \hyperref[rem:arbitrary_kind_graph]{arbitrary-kind graph or hypergraph} \( G \) as the (cardinal) number of vertices.
    ...
    \thmitem{def:graph_cardinality/directed_degree}\mcite[def. 1.1.7]{Knauer2011AlgGraphTheory} We define the \term[bg=полустепен на изхода (\incite[8]{Мирчев2001Графи}), ru=полустепень исхода (\cite[283]{ЕмеличевИПр1990Графы})]{out-degree} \( \deg_{\op{out}}(v) \) (resp. \term[bg=полустепен на входа (\incite[8]{Мирчев2001Графи}), ru=полустепень захода (\cite[283]{ЕмеличевИПр1990Графы})]{in-degree} \( \deg_{\op{in}}(v) \)) of a vertex \( v \) in a \hyperref[def:directed_multigraph]{directed (multi)graph} as the (cardinal) number of arcs starting (resp. ending) at \( v \).
...
\begin{remark}\label{rem:counting_loops_twice}
  ...
  \begin{itemize}
    \item \incite[27]{ЕмеличевИПр1990Графы}, who diligently distinguish between directed and undirected graphs, defines degrees in (simple) directed graphs as sums of out-degrees and in-degrees, like we do, and in undirected graphs they, like us, count loops twice.
...
\begin{comments}
  ...
  \item As a consequence, the sum of degrees if even. \incite[4]{Bollobas1998Graphs} and \incite[prop. 5.1]{ЕмеличевИПр1990Графы} call this consequence the \enquote{handshaking lemma} because of the observation that the total number of hands shaken at a party is even.
\end{comments}
\begin{proof}
