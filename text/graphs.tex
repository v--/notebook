...
\begin{definition}\label{def:directed_multigraph}\mcite[def. 1.1.1; def. 1.1.2]{Knauer2019AlgebraicGraphTheory}
  ...
  \begin{thmenum}[resume=def:directed_multigraph]
    ...
    \thmitem{def:directed_multigraph/opposite}\mimprovised The category of directed multigraphs has a natural \hyperref[con:opposite_object]{opposite object functor}. Given \( G = (V, A, t, h) \), we define \( G^\oppos \coloneqq (V, A, h, t) \).
  \end{thmenum}
\end{definition}
\begin{comments}
  \item \Textcite[675]{Rosen2019DiscreteMathematics} defines \enquote{directed multigraphs}, but prefers to treat the edge set as a multiset rather than having a head and tail function.
  ...
  \textcite[1]{König1986Graphentheorie}, which, as explained in \cref{rem:graph_definition}, is considered the first book on graph theory, gives a vague definition of a graph that roughly coincides with our notion of directed multigraph, with infinite graphs explicitly considered.
\end{comments}

\begin{remark}\label{rem:digraph}
  The term \enquote{digraph} is used as a shorthand for \enquote{directed graph}, for example by \textcite[10]{Harary1969GraphTheory}, \textcite[def. 1.1.1]{Knauer2019AlgebraicGraphTheory}, \textcite[def. 10.1.2]{Rosen2019DiscreteMathematics}, \textcite[571]{Stanley2012EnumerativeCombinatoricsVol1} and \textcite[372]{Knuth1997ArtVol1}. \Textcite[28]{Diestel2017GraphTheory} also uses this terminology, and in his usage \enquote{graph} exclusively refers to \enquote{undirected graph}.

  Similarly, in Russian, \enquote{орграф} is used as a shorthand for \enquote{ориентированный граф}, for example by \textcite[\S 7.1.5]{Новиков2013ДискретнаяМатематика} or \textcite[279]{ЕмеличевИПр1990ТеорияГрафов}.
...
\begin{definition}\label{def:directed_graph}\mcite[def. 1.1.2]{Knauer2019AlgebraicGraphTheory}
  ...
  \begin{thmenum}[resume=def:directed_graph]
    \thmitem{def:directed_graph/theory}\mimprovised We can regard directed graphs as models of a \hyperref[def:fol_theory]{first-order theory} over a \hyperref[def:fol_signature]{first-order signature} \( \Sigma \) with a single \hyperref[def:fol_signature/notation]{infix} predicate symbol, possibly with the irreflexivity axiom \eqref{eq:def:binary_relation/irreflexive}.

    \thmitem{def:directed_graph/homomorphism}\mimprovised A \hyperref[def:fol_homomorphism]{first-order homomorphism} between the \( G \) and \( H \) is a function \( f: V_G \to V_H \) such that \( (u, v) \in A_G \) implies \( (f(u), f(v)) \in A_H \).
    ...
    \thmitem{def:directed_graph/opposite}\mimprovised Similarly to \hyperref[def:directed_multigraph/opposite]{opposite directed multigraphs}, given \( G = (V, A) \), we define its opposite \( G^\oppos \) as \( (V, A^\oppos) \), where
    \begin{equation*}
...
\begin{comments}
  ...
  \begin{itemize}
    ...
    \item \Textcite[10]{Harary1969GraphTheory}, \textcite[28]{Diestel2017GraphTheory}, \textcite[6]{Bollobás1998ModernGraphTheory} and \textcite[674]{Rosen2019DiscreteMathematics} define undirected multigraphs, although all omit the prefix \enquote{undirected}. Harary disallows loops, instead referring to multigraphs with loops as \enquote{pseudographs}. Bollobás explicitly defines directed multigraphs, while Diestel uses the term \enquote{directed graph} for what we call a \hyperref[def:directed_multigraph]{directed multigraph} and \enquote{oriented graph} for what we call \enquote{simple directed graph}.
...
\begin{definition}\label{def:undirected_graph}\mcite[2; 3]{Diestel2017GraphTheory}
  ...
  \begin{thmenum}[series=def:undirected_graph]
    ...
    \thmitem{def:undirected_graph/edges} A disjoint from \( V \) set \( E \) of unordered pairs of \emph{distinct} vertices, i.e. sets of the form \( \set{ u, v } \), whose elements we call \term{edges}. If we allow vertices whose endpoints coincide, we will say that \( G \) is a \enquote{simple undirected graph, possibly with loops}.
  \end{thmenum}
...
\begin{comments}
  ...
  \item \Textcite[9]{Harary1969GraphTheory}, \textcite[2]{Diestel2017GraphTheory} and \textcite[1]{Bollobás1998ModernGraphTheory} define simple undirected graphs similarly to how we have done it. \Textcite[def. 10.1.1]{Rosen2019DiscreteMathematics} and \textcite[def. 1.1.2]{Knauer2019AlgebraicGraphTheory} also do, however they allow loops.
...
\begin{remark}\label{rem:theory_of_simple_undirected_graphs}
  ...
  This leads to an incompatible notion of homomorphisms, however --- the \hyperref[def:fol_homomorphism]{first-order homomorphisms} of this theory are the directed graph homomorphisms defined in \cref{def:directed_graph/homomorphism} and not the more general undirected graph homomorphisms defined in \cref{def:undirected_graph/homomorphism}.

  Furthermore, \hyperref[def:fol_substructure]{submodels} correspond to full subgraphs, while we find the more general notion of subgraph helpful.
...
\begin{remark}\label{rem:simple_graphs}
  ...
  Defining \enquote{directed graphs} as pairs \( (V, A) \), where \( A \subseteq V \times V \), without further restrictions, is common. It is done by \textcite[10]{Harary1969GraphTheory}, \textcite[def. 1.1.2]{Knauer2019AlgebraicGraphTheory}, \textcite[21]{GondranMinoux1984GraphsAndAlgorithms}, \textcite[571]{Stanley2012EnumerativeCombinatoricsVol1}, \textcite[def. 10.1.2]{Rosen2019DiscreteMathematics}, \textcite[190]{Erickson2019Algorithms}, \textcite[279]{ЕмеличевИПр1990ТеорияГрафов}, \textcite[277]{БелоусовТкачёв2004ДискретнаяМатематика} and \textcite[6]{Мирчев2001Графи}. \Textcite[\S 7.1.5]{Новиков2013ДискретнаяМатематика} also provides the same definition, however he forbids loops unless explicitly mentioned. \Textcite[28]{Diestel2017GraphTheory} uses the term \enquote{oriented graph} for what we call \enquote{simple directed graph}.
