...
\begin{remark}\label{rem:trivial_graph}
  ...
  Another unambiguous concept is that of an \term{edgeless} (resp. \term{arcless}) graph, in which the set of edges or arcs is empty, but the set of vertices may or may not be empty. Every \hyperref[ex:complete_graph]{complete graph} has \( 2^n \) edgeless subgraphs (one for each subset of \( V \)).
\end{remark}
