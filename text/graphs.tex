...
\begin{definition}\label{def:directed_multigraph}\mcite[def. 1.1.1; def. 1.1.2]{Knauer2019AlgGraphTheory}
  ...
  \begin{thmenum}[series=def:directed_multigraph]
    ...
    \thmitem{def:directed_multigraph/head} A function \( h: A \to V \), giving the \term[en=head (\cite[191]{Erickson2019Algorithms})]{head} or \term[bg=начален връх (\cite[7]{Мирчев2001Графи}), ru=начало (\cite[279]{ЕмеличевИПр1990Графы}), en=initial vertex (\cite[28]{Diestel2017Graphs})]{initial endpoint} of an arc.

    \thmitem{def:directed_multigraph/tail} A function \( t: A \to V \), giving the \term[en=tail (\cite[191]{Erickson2019Algorithms})]{tail} or \term[bg=краен връх (\cite[7]{Мирчев2001Графи}), ru=конец (\cite[279]{ЕмеличевИПр1990Графы}), en=terminal vertex (\cite[28]{Diestel2017Graphs})]{terminal endpoint} of an arc.
  \end{thmenum}
  ...
  \begin{thmenum}[resume=def:directed_multigraph]
    ...
    \thmitem{def:directed_multigraph/parallel}\mcite[28]{Diestel2017Graphs} We call the arcs \( e \) and \( f \) \term[bg=паралелни (ребра) (\cite[7]{Мирчев2001Графи}), ru=параллельные (рёбра) (\cite[279]{ЕмеличевИПр1990Графы})]{parallel} if \( h(e) = h(f) \) and \( t(e) = t(f) \) and \term{opposite} if it is not a loop and \( h(e) = t(f) \) and \( t(e) = h(f) \).

    \thmitem{def:directed_multigraph/homomorphism}\mcite[def. 1.4.1]{Knauer2019AlgGraphTheory} A \term{homomorphism} between directed multigraphs \( G \) and \( H \) is a pair of functions
    \begin{align*}
      &f_V: V_G \to V_H, \\
      &f_A: A_G \to A_H,
    \end{align*}
...
\begin{remark}\label{rem:digraph}
  The term \enquote{digraph} is used as a shorthand for \enquote{directed graph}, for example by \incite[10]{Harary1969Graphs}, \incite[def. 1.1.1]{Knauer2019AlgGraphTheory} and \incite[372]{Knuth1997ArtVol1}. \incite[28]{Diestel2017Graphs} also uses this terminology, and in his usage \enquote{graph} exclusively refers to \enquote{undirected graph}.
...
\begin{definition}\label{def:directed_graph}\mcite[def. 1.1.2]{Knauer2019AlgGraphTheory}
...
\begin{definition}\label{def:hypergraph}\mcite[185]{DorflerWaller1980Hypergraphs}
  ...
  \begin{thmenum}[resume=def:hypergraph]
    ...
    \thmitem{def:hypergraph/hyperedge} We say that \( e \) is a \term[ru=гипердуги (\cite[\S 7.1.5]{Новиков2013ДискретнаяМатематика}), en=hyperedge (\cite[684]{Rosen2018DiscreteHandbook})]{hyperedge} if it has more than two endpoints.
    ...
    \thmitem{def:hypergraph/uniform}\mcite[299]{ЕмеличевИПр1990Графы} If all edges have \( \kappa \) vertices, we say that the hypergraph is \term[ru=\( \kappa \)-однородный, en=\( \kappa \)-uniform (\cite[685]{Rosen2018DiscreteHandbook})]{\( \kappa \)-uniform}.

    \thmitem{def:hypergraph/homomorphism}\mcite[186]{DorflerWaller1980Hypergraphs} A \term{homomorphism} between the hypergraphs \( G \) and \( H \) is a pair of functions
    \begin{align*}
      &f_V: V_G \to V_H, \\
      &f_E: E_G \to E_H,
    \end{align*}
...
\begin{comments}
  ...
  \begin{itemize}
    \item \incite[def. 1.1.2]{Knauer2019AlgGraphTheory} does not explicitly mention undirected multigraphs, but their existence is implied.

    \item \incite[10]{Harary1969Graphs}, \incite[28]{Diestel2017Graphs} and \incite[6]{Bollobas1998Graphs} define undirected multigraphs, although all omit the prefix \enquote{undirected}. Harary disallows loops, instead referring to multigraphs with loops as \enquote{pseudographs}. Bollobas explicitly defines directed multigraphs, while Diestel uses the term \enquote{directed graph} for what we call a \hyperref[def:directed_multigraph]{directed multigraph} and \enquote{oriented graph} for what we call \enquote{simple directed graph}.
...
\begin{definition}\label{def:undirected_graph}\mcite[2; 3]{Diestel2017Graphs}
  ...
  \begin{thmenum}[resume=def:undirected_graph]
    ...
    \thmitem{def:undirected_graph/category}\mcite[example 3.1.12]{Knauer2019AlgGraphTheory} Given a Grothendieck universe \( \mscrU \), we can define the \hyperref[def:category]{category} of \( \mscrU \)-small simple undirected graphs, possibly with loops, and their homomorphisms. Disallowing loops leads to a full subcategory.
    ...
    \thmitem{def:undirected_graph/subgraph}\mcite[4]{Diestel2017Graphs} We say that \( H \) is a \term{subgraph} of \( G \) if \( V_G \subseteq V_H \) and \( E_G \subseteq E_H \).
  \end{thmenum}
\end{definition}
\begin{comments}
  ...
  \item \incite[9]{Harary1969Graphs}, \incite[2]{Diestel2017Graphs} and \incite[1]{Bollobas1998Graphs} define simple undirected graphs similarly to how we have done it. \incite[def. 1.1.2]{Knauer2019AlgGraphTheory} also do, however they allow loops.

  \item \incite[def. 1.4.8]{Knauer2019AlgGraphTheory} define subgraphs more generally as graphs which can be embedded via injective graph homomorphisms. We prefer the vertices and edges of subgraphs to be subsets.
...
\begin{remark}\label{rem:simple_graphs}
  ...
  Defining \enquote{directed graphs} as pairs \( (V, A) \), where \( A \subseteq V \times V \), without further restrictions, is common. It is done by \incite[10]{Harary1969Graphs}, \incite[def. 1.1.2]{Knauer2019AlgGraphTheory}, \incite[21]{GondranMinoux1984Graphs}, \incite[10]{Savage1998Computability}, \incite[190]{Erickson2019Algorithms}, \incite[279]{ЕмеличевИПр1990Графы}, \incite[277]{БелоусовТкачёв2004ДискретнаяМатематика} and \incite[6]{Мирчев2001Графи}. \incite[\S 7.1.5]{Новиков2013ДискретнаяМатематика} also provides the same definition, however he forbids loops unless explicitly mentioned. \incite[28]{Diestel2017Graphs} uses the term \enquote{oriented graph} for what we call \enquote{simple directed graph}.
  ...
  \begin{itemize}
    ...
    \item Other aforementioned authors, namely \incite[def. 1.1.2]{Knauer2019AlgGraphTheory}, \incite[190]{Erickson2019Algorithms}, and \incite[7]{Мирчев2001Графи} define undirected graphs as pairs \( (V, E) \), where \( E \) consists of one-element or two-element subsets of \( V \). This definition again allows loops.

    \item \incite[9]{Harary1969Graphs}, \incite[2]{Diestel2017Graphs}, \incite[1]{Bollobas1998Graphs}, \incite[9]{ЕмеличевИПр1990Графы} and \incite[\S 7.1.2]{Новиков2013ДискретнаяМатематика} define directed graphs as pairs \( (V, E) \), where \( E \) consists of two-element subsets of \( V \). This explicitly forbid loops.
  ...
  \incite[def. 1.1.2]{Knauer2019AlgGraphTheory} uses the adjective \enquote{simple} to refer to (multi)graphs without multiple edges, without referring to loops. So do \incite[2]{Diestel2017Graphs} and \incite[22]{Bollobas1998Graphs}, however their definitions of graphs forbid loops by default. \incite[22]{GondranMinoux1984Graphs}, \incite[191]{Erickson2019Algorithms}, \incite[17]{ЕмеличевИПр1990Графы} and \incite[12]{Мирчев2001Графи} use \enquote{simple} to additionally forbid loops.
...
\end{proposition}
\begin{comments}
  \item Our characterizations are used as definitions in \cite[578]{Rosen2018DiscreteHandbook}. The category of hypergraphs is discussed in detail by \incite{DorflerWaller1980Hypergraphs}.

  \incite[3]{Diestel2017Graphs}, \incite[3]{Bollobas1998Graphs}, \incite[def. 1.4.3]{Knauer2019AlgGraphTheory}, \incite[13]{ЕмеличевИПр1990Графы}, \incite[def. 5.14]{БелоусовТкачёв2004ДискретнаяМатематика} and \incite[\S 7.1.6]{Новиков2013ДискретнаяМатематика} define simple undirected graph isomorphisms via \eqref{eq:thm:graph_isomorphisms/simple_undirected}, however avoid discussing isomorphisms for other kinds of graphs.
\end{comments}
\begin{proof}
...
\begin{definition}\label{def:multigraph_orientation}\mcite[def. 6.2.1]{Knauer2019AlgGraphTheory}
...
\begin{definition}\label{def:induced_subgraph}\mcite[def. 1.4.8]{Knauer2019AlgGraphTheory}
...
\begin{definition}\label{def:graph_incidence}\mcite[def. 1.1.1]{Knauer2019AlgGraphTheory}
...
\begin{definition}\label{def:graph_adjacency}\mcite[584; 585]{Rosen2018DiscreteHandbook}
...
\begin{definition}\label{def:graph_independent_set}\mcite[577]{Rosen2018DiscreteHandbook}
...
\begin{definition}\label{def:graph_cardinality}
  ...
  \begin{thmenum}
    \thmitem{def:graph_cardinality/order}\mcite[2]{Diestel2017Graphs} We define the \term[ru=порядок (\cite[9]{ЕмеличевИПр1990Графы})]{order} \( \ord(G) \) of an \hyperref[rem:arbitrary_kind_graph]{arbitrary-kind graph or hypergraph} \( G \) as the (cardinal) number of vertices.
    ...
    \thmitem{def:graph_cardinality/local}\mcite[210]{Diestel2017Graphs} We say that a graph is \term{locally finite} (resp. \term{locally countable}) if the degree of any vertex is finite (resp. countable).
  \end{thmenum}
\end{definition}
...
\begin{remark}\label{rem:counting_loops_twice}
  ...
  \begin{itemize}
    ...
    \item \incite[def. 1.1.7]{Knauer2019AlgGraphTheory} avoids defining \enquote{total} degrees for directed graphs, leaving only out-degrees and in-degrees. For undirected graphs, however, in \cite[def. 1.1.8]{Knauer2019AlgGraphTheory} he defines the degree of a vertex \( v \) as the number of edges incident to \( v \), which without our multiset trick only counts loops once.
...
\begin{definition}\label{def:isolated_vertex}\mcite[5]{Diestel2017Graphs}
...
\begin{corollary}\label{thm:odd_degree_vertices}\mcite[prop. 1.2.1]{Diestel2017Graphs}
