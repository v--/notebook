\section{Transfinite arithmetic}\label{sec:transfinite_arithmetic}
...
\begin{definition}\label{def:ordinal_arithmetic}
  ...
  \begin{thmenum}
    \thmitem{def:ordinal_arithmetic/addition}\mcite[def. 6.3.16]{Hinman2005Logic} The \term{sum} of \( \alpha \) and \( \beta \) extends \eqref{eq:def:peano_arithmetic/theory/PA4} and \eqref{eq:def:peano_arithmetic/theory/PA5} with a case for limit ordinals:
    ...
    From \cref{thm:union_of_set_of_ordinals} it follows that the in limit case \( \alpha + \beta \) is the smallest ordinal strictly larger than \( \alpha + \gamma \) for any \( \gamma < \beta \).

    \thmitem{def:ordinal_arithmetic/multiplication}\mcite[def. 6.3.16]{Hinman2005Logic} Analogously, the \term{product} of \( \alpha \) and \( \beta \) extends \eqref{eq:def:peano_arithmetic/theory/PA6} and \eqref{eq:def:peano_arithmetic/theory/PA7}:
    ...
    \thmitem{def:ordinal_arithmetic/exponentiation}\mcite[exerc. 6.3.34]{Hinman2005Logic} Exponentiation extends \cref{def:monoid/exponentiation}:
...
\begin{remark}\label{rem:ordinal_successor_via_addition}
  ...
  Note that for infinite ordinals \( \op{sc}(\alpha) = 1 + \alpha \) as discussed in \cref{ex:ordinal_addition}.

  This is an extension of \cref{thm:peano_arithmetic_successor_and_addition}.
\end{remark}
...
\begin{proposition}\label{thm:ordinal_addition_is_monotone}
  ...
  \begin{thmenum}
    ...
    Note that the inequality here is not strict --- see \cref{ex:ordinal_addition} for counterexamples.
  \end{thmenum}
\end{proposition}
\begin{proof}
  ...
  \begin{itemize}
    ...
    We cannot make a stronger conclusion here --- see \cref{ex:ordinal_addition} for a counterexample.
  \end{itemize}
\end{proof}
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    By \cref{thm:union_of_set_of_ordinals} we have that \( \gamma \) is an ordinal and that \( \gamma_\beta \leq \gamma \) for every \( \beta < \lambda \). Thus,
    \begin{align*}
      \lambda
    ...
    Aiming at a contradiction, suppose that the strict inequality holds. That is, suppose that \( \lambda < \alpha + \gamma \). Then there exists some \( \delta_0 < \gamma \) such that \( \alpha + \delta_0 > \alpha + \gamma_\beta \) for any \( \beta < \lambda \). It follows from \cref{thm:def:partially_ordered_set} that \( \gamma_\beta < \delta_0 \) for any \( \beta < \lambda \) and thus
    \begin{equation*}
...
\begin{proposition}\label{thm:ordinal_addition_algebraic_properties}
  ...
  As in \cref{thm:ordinals_are_well_ordered}, we adapt the corresponding axioms due to \fullref{thm:burali_forti_paradox}. The more concrete result is:
  \begin{thmenum}
  ...
  \end{thmenum}
   ...
   See \cref{ex:ordinal_addition} for counterexamples to \hyperref[def:binary_operation/commutative]{commutativity}.

   Compare this with \cref{thm:natural_number_addition_properties} and \cref{thm:cardinal_addition_algebraic_properties}.
\end{proposition}
\begin{proof}
  \SubProofOf{thm:ordinal_addition_algebraic_properties/zerosumfree} Suppose that \( \alpha + \beta = 0 \).

  If \( \beta > 0 \), by \cref{thm:ordinal_addition_is_monotone/left} we have \( \alpha + 0 < \alpha + \beta \), which contradicts the minimality of \( \alpha + \beta = 0 \).

  Then \( \beta = 0 \), hence \( 0 = \alpha + \beta = \alpha \), forcing \( \alpha \) to be \( 0 \).

  Therefore, if \( \alpha + \beta = 0 \), both \( \alpha \) and \( \beta \) must be \( 0 \).

  \SubProofOf{thm:ordinal_addition_algebraic_properties/associative} We will use induction on \( \gamma \). \Cref{thm:natural_number_addition_properties} already proves the base and successor cases.
  ...
  \SubProofOf{thm:ordinal_addition_algebraic_properties/left_cancellative} Follows from \cref{thm:def:partially_ordered_set} and \eqref{eq:thm:ordinal_addition_is_monotone/left}.
\end{proof}

\begin{proposition}\label{thm:ordinal_addition_disjoin_union}
  For the \hyperref[def:ordinal_arithmetic/addition]{sum} of two ordinals \( \alpha \) and \( \beta \), the following is an \hyperref[def:partially_ordered_set/morphism]{order isomorphism}:
  \begin{equation*}
    \begin{aligned}
...
\begin{example}\label{ex:ordinal_addition}
  The distinction between \( \alpha + \beta \) and \( \beta + \alpha \) is important. A simple example is provided by any limit ordinal \( \lambda \), in particular by \( \omega \). The examples are inconvenient to demonstrate with the recursive definition, however \cref{thm:ordinal_addition_disjoin_union} eases us.

  In particular, \cref{thm:ordinal_addition_disjoin_union} highlights that adding one ordinal to another, in an informal sense, \enquote{appending} a copy of the second to a copy the first.
  ...
  What \cref{thm:def:partially_ordered_set} gives us is that
  \begin{equation*}
  ...
  What is more interesting is that, as a consequence of \cref{thm:def:partially_ordered_set}, we have \( \lambda < \lambda + 1 \). This can be explained as follows. Instead of \enquote{appending} an infinite set to a finite one, we append a finite set to an infinite one. This way \( \lambda \) cannot \enquote{absorb} \( 1 \) like it does in \( 1 + \lambda \).

  As a consequence of this example, addition of ordinals is not commutative and also not right cancellable.

  As discussed in our proof of \cref{thm:cardinal_addition_algebraic_properties}, this is only a restriction of well-orders and not of the resulting sets themselves.
\end{example}
...
\end{proposition}
\begin{proof}
  \SubProofOf{thm:ordinal_multiplication_is_monotone/left} Similarly to \cref{thm:ordinal_addition_is_monotone/left}, we proceed by induction on \( \beta \):
  \begin{itemize}
    \item The condition \( \alpha < \beta \) is vacuously false for the base case \( \beta = 0 \), hence by \ref{inf:def:propositional_natural_deduction/bot/efq} the statement vacuously holds.
  ...
  \SubProofOf{thm:ordinal_multiplication_is_monotone/right} Similarly to \cref{thm:ordinal_addition_is_monotone/right}, we proceed by induction on \( \gamma \):
  \begin{itemize}
    \item The base case \( \gamma = 0 \) is vacuous.
    \item Suppose that \( \gamma = \op{sc}(\delta) \) and that the inductive hypothesis holds for \( \delta \). Then
    \begin{equation*}
...
\begin{proposition}\label{thm:ordinal_multiplication_algebraic_properties}
  Similarly to \cref{thm:ordinal_addition_algebraic_properties} for ordinal number addition, multiplication is \hyperref[def:entire_semiring]{entire}, \hyperref[def:semigroup_absorbing_element]{absorbing}, \hyperref[def:binary_operation/associative]{associative} and \hyperref[def:binary_operation/cancellative]{left cancellative}:
  \begin{thmenum}
  ...
  Compare this with \cref{thm:natural_number_multiplication_properties} and \cref{thm:cardinal_multiplication_algebraic_properties}.
\end{proposition}
\begin{proof}
  \SubProofOf{thm:ordinal_multiplication_algebraic_properties/entire} Suppose that \( \alpha \cdot \beta = 0 \).

  If \( \beta > 0 \), by \cref{thm:ordinal_multiplication_is_monotone/left} we have
  \begin{equation*}
...
\begin{proposition}\label{thm:ordinal_multiplication_cartesian_product}
  For the \hyperref[def:ordinal_arithmetic/multiplication]{product} of two ordinals \( \alpha \) and \( \beta \), the following is an \hyperref[def:partially_ordered_set/morphism]{order isomorphism}:
  \begin{equation*}
    \begin{aligned}
...
\end{comments}
\begin{proof}
  Similarly to \cref{thm:ordinal_addition_disjoin_union}, we will use induction on \( \beta \) to show that
  \begin{equation*}
    \begin{aligned}
...
\end{corollary}
\begin{proof}
  \SubProof{Proof if \( \alpha \) is zero} \Cref{thm:ordinal_addition_algebraic_properties/zerosumfree} implies that in order for \( \beta \cdot \delta + \rho \) to be zero, both \( \delta \cdot \beta \) and \( \rho \) must be zero.

  \Cref{thm:ordinal_multiplication_algebraic_properties/entire} implies that in order for \( \delta \cdot \beta \) to be zero, \( \beta \) or \( \delta \) must be zero. But \( \beta \) is nonzero by assumption, so \( \delta \) must be zero.

  Therefore, if \( \alpha \) is zero, both \( \delta \) and \( \rho \) must be zero.

  \SubProof{Proof if \( \alpha \) is nonzero} From \cref{thm:ordinal_multiplication_cartesian_product} it follows that the following is an isomorphism:
  \begin{equation*}
    \begin{aligned}
  ...
  \Cref{thm:ordinal_multiplication_is_monotone/right} implies that \( \alpha \leq \beta \cdot \alpha \), thus we can take \( \delta \) and \( \rho \) to be the corresponding elements of the preimage \( f^{-1}(\alpha) \).
\end{proof}
...
\end{corollary}
\begin{proof}
  Follows from \cref{thm:ordinal_division} with \( \beta = \omega \).
\end{proof}
...
\end{proposition}
\begin{proof}
  \SufficiencySubProof Suppose that \( \alpha \) is a limit ordinal. \Cref{thm:ordinal_division_omega} gives us an ordinal \( \delta \) and a natural number \( n \) such that \( \alpha = \omega \cdot \delta + n \).

  Suppose that \( n > 0 \) and let \( m \) be its \hyperref[thm:peano_arithmetic_predecessor_existence]{predecessor} so that \( n = \op{sc}(m) \). Then, by definition of ordinal addition,
  \begin{equation*}
    \alpha
    =
    \omega \cdot \delta + n
  ...
  \NecessitySubProof If \( \alpha = \omega \cdot \delta \), consider any strictly smaller ordinal \( \beta \). \Cref{thm:ordinal_division_omega} gives us an ordinal \( \varepsilon \) and a natural number \( m \) such that \( \beta = \omega \cdot \varepsilon + m \).

  \Cref{thm:ordinal_addition_is_monotone/left} implies that
  \begin{equation*}
  ...
  \end{equation*}
  and \cref{thm:def:totally_ordered_set/embedding_iff_strict} applied to \cref{thm:ordinal_multiplication_is_monotone/left} implies that \( \varepsilon < \delta \).
...
\begin{example}\label{ex:countable_limit_ordinals}
  We already know that \( \omega \) is a limit ordinal. It is clear from \cref{ex:ordinal_addition} that \( \omega + n \) is a successor ordinal for every natural number \( n \).
  ...
  Both ordinals \( \omega \) and \( \omega + \omega \) are countable by \cref{thm:omega_equinumerous_with_omega_squared}.

  Another limit ordinal is \( \omega \cdot \omega = \omega^2 \). It is also countable by \cref{thm:countable_product_of_countable_sets}. Actually \( \omega^n \) for any natural number \( n \) is countable by the same theorem.
...
\begin{proof}
  ...
  Aiming at a contradiction, suppose that \( \alpha \) is the smallest ordinal for which this does not hold. \Cref{thm:omega_equinumerous_with_omega_squared} implies that \( \alpha \geq \omega \).

  Since \( \alpha < \ord(\alpha \times \alpha) \), there \cref{thm:initial_segment_of_ordinal} implies the existence of some segment \( S_{<(\beta_1, \beta_2)} \) such that
  \begin{equation*}
...
\begin{definition}\label{def:cardinal_arithmetic}
  We will define arithmetic operations for them. Unlike in \cref{def:ordinal_arithmetic}, we will directly define the operations as \hyperref[thm:cardinality_existence]{cardinal numbers} of some sets rather than via some form of recursion.

  Fix two ordinals \( \kappa \) and \( \mu \).
  \begin{thmenum}
    \thmitem{def:cardinal_arithmetic/addition}\mcite[def. 6.4.29]{Hinman2005Logic} Based on \cref{thm:ordinal_addition_disjoin_union}, we define their \term{sum} as
    \begin{equation*}
      \kappa + \mu \coloneqq \card(\kappa \amalg \mu),
    \end{equation*}
    where \( \kappa \amalg \mu \) is their \hyperref[def:disjoint_union]{disjoint union}.

    \thmitem{def:cardinal_arithmetic/multiplication}\mcite[def. 6.4.29]{Hinman2005Logic} Based on \cref{thm:ordinal_multiplication_cartesian_product}, we define their \term{product} as
    \begin{equation*}
      \kappa \cdot \mu \coloneqq \card(\kappa \times \mu).
    \end{equation*}

    \thmitem{def:cardinal_arithmetic/exponentiation}\mcite[exerc. 6.4.46]{Hinman2005Logic} We define \term{exponentiation} as
    \begin{equation*}
...
\begin{proposition}\label{thm:cardinal_multiplication_algebraic_properties}
  ...
  Compare this with \cref{thm:natural_number_multiplication_properties} and \cref{thm:ordinal_addition_algebraic_properties}.
\end{proposition}
\begin{proof}
  The result follows from the same considerations as in \cref{thm:cardinal_addition_algebraic_properties}.
\end{proof}
...
\begin{proposition}\label{thm:simplified_cardinal_arithmetic}
  Unlike \hyperref[def:ordinal_arithmetic]{ordinal arithmetic} with its intricacies like \cref{ex:ordinal_addition}, \hyperref[def:cardinal_arithmetic]{cardinal arithmetic} has a simpler behavior:
  \begin{thmenum}
    ...
    \thmitem{thm:simplified_cardinal_arithmetic/infinite}\mcite[6.4.35]{Hinman2005Logic} If either \( \kappa \) or \( \mu \) is infinite, then
    \begin{equation*}
...
\end{proposition}
\begin{proof}
  \SubProofOf{thm:simplified_cardinal_arithmetic/finite} The addition of ordinals defined in \cref{def:ordinal_arithmetic/addition} is an extension of addition of natural numbers, hence the two are equivalent for finite ordinals. The equivalence with cardinal addition defined in \cref{def:cardinal_arithmetic/addition} comes from \cref{thm:ordinal_addition_disjoin_union} and the fact that every finite ordinal is a cardinal as demonstrated in \cref{thm:natural_numbers_are_cardinals}.

  Analogously, equivalence of cardinal and ordinal multiplication follows from \cref{thm:ordinal_multiplication_cartesian_product}.
  ...
  The rest follows from \cref{thm:square_of_infinite_ordinal}.
\end{proof}

\begin{corollary}\label{thm:finite_cardinal_exponent}
  For any infinite cardinal \( \kappa \), we have \( \kappa^n = \kappa \) for every positive natural number \( n \).
\end{corollary}
\begin{proof}
  Induction on \( n \), using \cref{thm:simplified_cardinal_arithmetic/infinite} at each step.
\end{proof}
...
\end{corollary}
\begin{comments}
  \item See also \cref{thm:aleph_zero_is_regular}
\end{comments}
\begin{proof}
  \Cref{thm:simplified_cardinal_arithmetic/finite} states that cardinal exponentiation extends natural number exponentiation. Hence, we can conclude that \( 2^n < \aleph_0 \) for any \( n < \aleph_0 \) since the former are finite and the latter is not.
\end{proof}

\begin{proposition}\label{thm:cardinality_of_disjoint_union}
  If \( \mscrK \) has cardinality \( \kappa \) an if \( \mu \) bounds the cardinality of \( A_k \) for all indices \( k \), then the \hyperref[def:disjoint_union]{disjoint union} \( \coprod_{k \in \mscrK} A_k \) has cardinality at most \( \kappa \cdot \mu \).

  If the cardinality of \( A_k \) is exactly \( \mu \) for every index \( k \), then \( \coprod_{k \in \mscrK} A_k \) has cardinality \( \kappa \cdot \mu \).
\end{proposition}
\begin{comments}
  \item This generalizes the familiar characterization of multiplication as repeated addition.
\end{comments}
\begin{proof}
  By the definition of cardinality, there exist bijective functions \( \iota_\mscrK: \mscrK \to \kappa \) and \( \iota_{A_k}: A_k \to \mu \) for every \( k \) in \( \mscrK \). In case where the cardinality of \( A_k \) is only bounded by \( \mu \), the function \( \iota_{A_k} \) is injective but not necessarily surjective.

  Consider the function
  \begin{equation*}
    \begin{aligned}
      &\Phi:\coprod_{k \in \mscrK} A_k \to \aleph_0 \times \kappa, \\
      &\Phi(k, a_k) \coloneqq (\iota_\mscrK(k), \iota_{A_k}(a_k)).
    \end{aligned}
  \end{equation*}

  It is injective. Indeed, if \( \Phi(k, a_k) = \Phi(m, a_m) \), then \( k = m \) since \( \iota_\mscrK \) is injective, and \( a_k = a_m \) since \( \iota_{A_k} \) is injective. This shows that \( \Phi \) is injective.

  Therefore, \( \coprod_{k \in \mscrK} A_k \) is bounded by \( \aleph_0 \times \kappa \), which is in turn equinumerous with
  \begin{equation*}
    \kappa \cdot \mu = \card(\aleph_0 \times \kappa).
  \end{equation*}

  In the case where \( A_k \) has cardinality exactly \( \mu \) for every \( k \), \( \Phi \) is also surjective. Indeed, fix any natural number \( n \) and any ordinal \( \alpha \) in \( \kappa \). Let \( k \) be the preimage \( \iota_\mscrK^{-1}(n) \) and let \( a_k \) be the preimage \( \iota_{A_k}^{-1}(\alpha) \). Then \( \Phi(k, a_k) = (n, \alpha) \).

  In this case \( \coprod_{k \in \mscrK} A_k \) is equinumerous with \( \kappa \cdot \mu \).
\end{proof}
...
\end{proposition}
\begin{proof}
  We can prove the proposition similarly to \cref{thm:power_set_via_subsets}, but much more convoluted.
...
\begin{proof}
  ...
  Clearly \( \pow(f(A)) \subsetneq \pow(B) \). \Cref{thm:cardinal_exponentiation_power_set} then implies that \( 2^{\card A} < 2^{\card B} \).
...
\end{proposition}
\begin{proof}
  It is clear from \fullref{thm:cantor_power_set_theorem} and \cref{thm:cardinal_exponentiation_power_set} that \( \kappa < 2^\kappa \). By definition, \( \mu \) is the smallest cardinal such that \( \kappa < \mu \). Therefore, \( \mu \leq 2^\kappa \).
\end{proof}

\begin{conjecture}[Generalized continuum hypothesis]\label{hyp:generalized_continuum_hypothesis}\mcite[166]{Enderton1977SetTheory}
  For every ordinal \( \alpha \) we have
  \begin{equation*}
    \aleph_{\alpha + 1} = 2^{\aleph_\alpha},
  \end{equation*}

  For the definition and the properties of the \( \aleph \) hierarchy, see \cref{def:aleph_hierarchy}. For the related \( \beth \) hierarchy, see \cref{def:beth_hierarchy}.
...
\begin{corollary}\label{thm:limit_cardinals_and_gch}
  ...
  The converse holds in \logic{ZFC} as shown in \cref{thm:strong_limit_cardinal_is_weak_limit}.
\end{corollary}
\begin{proof}
  Follows from \cref{thm:infinite_cardinal_is_aleph} and \fullref{hyp:generalized_continuum_hypothesis}.
\end{proof}

\begin{definition}\label{def:beth_hierarchy}\mcite[214]{Enderton1977SetTheory}
  Similarly to \cref{def:aleph_hierarchy}, we use transfinite recursion to define, for each ordinal \( \alpha \), the cardinal
  ...
  Unlike \cref{def:aleph_hierarchy}, it is able to explicitly build the successor of any member of the hierarchy. \Fullref{hyp:generalized_continuum_hypothesis} states that \( \aleph_\alpha = \beta_\alpha \) for every ordinal \( \alpha \), however in general it is only provable that \( \aleph_\alpha \leq \beta_\alpha \) --- see \cref{thm:strong_limit_cardinal_is_weak_limit}.
\end{definition}
