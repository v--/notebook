...
\begin{definition}\label{def:ordinal_arithmetic}
  ...
  \begin{thmenum}
    \thmitem{def:ordinal_arithmetic/addition}\mcite[def. 6.3.16]{Hinman2005Logic} The \term{sum} of \( \alpha \) and \( \beta \) extends \eqref{eq:def:peano_arithmetic/PA4} and \eqref{eq:def:peano_arithmetic/PA5} with a case for limit ordinals:
    ...
    \thmitem{def:ordinal_arithmetic/multiplication}\mcite[def. 6.3.16]{Hinman2005Logic} Analogously, the \term{product} of \( \alpha \) and \( \beta \) extends \eqref{eq:def:peano_arithmetic/PA6} and \eqref{eq:def:peano_arithmetic/PA7}:
...
\begin{remark}\label{rem:ordinal_successor_via_addition}
  ...
  This is an extension of \cref{rem:natural_number_successor_via_addition}.
\end{remark}
...
\end{proposition}
\begin{proof}
  The proof is similar to \cref{thm:power_set_via_subsets}, but much more convoluted.
...
\begin{proposition}\label{thm:strong_limit_cardinal_is_weak_limit}
  ...
  Furthermore, if \( \kappa \) is infinite, then \cref{hyp:generalized_continuum_hypothesis} implies that \( \mu = 2^\kappa \).
\end{proposition}
\begin{proof}
...
\begin{conjecture}[Generalized continuum hypothesis]\label{hyp:generalized_continuum_hypothesis}\mcite[166]{Enderton1977SetTheory}
  ...
  This is a vast generalization of \cref{hyp:continuum_hypothesis} from the case \( \alpha = 0 \) to arbitrary ordinals.
\end{conjecture}

\begin{corollary}\label{thm:limit_cardinals_and_gch}
  \Cref{hyp:generalized_continuum_hypothesis} implies that every \hyperref[def:successor_and_limit_cardinal/weak_limit]{weak limit cardinal} is a \hyperref[def:successor_and_limit_cardinal/strong_limit]{strong limit cardinal}.

  The converse holds in \logic{ZFC} as shown in \cref{thm:strong_limit_cardinal_is_weak_limit}.
\end{corollary}
\begin{proof}
  Follows from \cref{thm:infinite_cardinal_is_aleph} and \cref{hyp:generalized_continuum_hypothesis}.
\end{proof}
