...
\end{definition}
\begin{comments}
  \item The name is inspired by \hyperref[def:cyclotomic_polynomial]{cyclotomic polynomial}. \Fullref{thm:def:cyclotomic_polynomial/splitting} implies that, when the characteristic of \( \Bbbk \) does not divide \( n \), the \( n \)-th cyclotomic extension is the splitting field for the \( n \)-th cyclotomic polynomial.
\end{comments}
...
\begin{definition}\label{def:root_of_unity}\mimprovised
  We say that an element \( x \) is an \hyperref[def:integral_domain]{integral domain} is an \( n \)-th \term[ru=корень из единицы (\cite[43]{Фаддеев1984ЛекцииПоАлгебре}), en=root of unity (\cite[def. 2.41]{LidlNiederreiter1997FiniteFields})]{root of unity} if \( x^n = 1 \), i.e. if it is a \hyperref[def:root_of_polynomial]{root} of the polynomial \( X^n - 1 \).
\end{definition}
\begin{comments}
...
\begin{proof}
  ...
  Then \( \alpha \) is a sixth root of unity. \Fullref{thm:def:group_element_order/neutral} implies that the order of \( \alpha \) divides \( 6 \). We will show that the order cannot be \( 1 \), \( 2 \) or \( 3 \), and it will follow that the order is \( 6 \).
  \begin{itemize}
