\section{Primitive roots}\label{sec:primitive_roots}

\paragraph{Cyclotomic extensions}

\begin{definition}\label{def:cyclotomic_extension}\mcite[A.V.81]{Bourbaki2003Algebra4to7}
  We call the \hyperref[def:splitting_field]{splitting field} of \( X^n - 1 \) over \( \Bbbk \) the \( n \)-th \term[ru=круговое / циклотомическое (поле разложения) (\cite[204]{Кострикин2020АлгебраЧасть3})]{cyclotomic extension} of \( \Bbbk \).
\end{definition}
\begin{comments}
  \item The name is inspired by cyclotomic polynomial, defined in \cref{def:cyclotomic_polynomial}. \Cref{thm:def:cyclotomic_polynomial/splitting} implies that, when the characteristic of \( \Bbbk \) does not divide \( n \), the \( n \)-th cyclotomic extension is the splitting field for the \( n \)-th cyclotomic polynomial.
\end{comments}
...
\begin{comments}
  ...
  \item Roots of unity are defined only for fields by \textcite[def. 2.41]{LidlNiederreiter1997FiniteFields} and \textcite[276]{Lang2002Algebra}, where \( n \) roots of unity exist in the \hyperref[def:algebraic_closure]{algebraic closure}. We see no harm in generalizing their definitions.
\end{comments}
...
\end{definition}
\begin{comments}
  \item \Textcite[def. 2.43]{LidlNiederreiter1997FiniteFields} instead define primitive roots of unity as generators of the group of all \( n \)-th roots of unity in the \( n \)-th \hyperref[def:cyclotomic_extension]{cyclotomic extension}, however they explicitly require \( n \) not to be a divisor of the characteristic. Without the latter restriction, their definition would encompass more cases.
\end{comments}

\begin{definition}\label{def:finite_field_primitive_element}
  We say that an element of the \hyperref[def:finite_field]{finite field} \( \BbbF_q \) is \term[en=primitive element (\cite[def. 2.9]{LidlNiederreiter1997FiniteFields})]{primitive} if it satisfies any of the following equivalent conditions:
  \begin{thmenum}
    \thmitem{def:finite_field_primitive_element/unity}\mimprovised It is a \( (q - 1) \)-th \hyperref[def:primitive_root_of_unity]{primitive root of unity}.
  ...
  Following \textcite[20]{Knuth1997ArtVol2}, if \( q \) is prime, we will call its primitive elements \enquote{primitive elements modulo \( q \)}.
\end{definition}
\begin{comments}
...
\begin{remark}\label{rem:primitive_element_terminology}
  ...
  \begin{itemize}
    \item \Textcite[66]{Rotman2015AdvancedModernAlgebraPart1}, \textcite[124]{Albert1956HigherAlgebra} and \textcite[def. 2.9]{LidlNiederreiter1997FiniteFields} use \enquote{primitive element} for what we call a generator of the multiplicative group of a \hyperref[def:finite_field]{finite field}. \Textcite[89]{Berlekamp2015AlgebraicCodingTheory} prefers the less ambiguous term \enquote{primitive field element}.

    In the context of fields of prime cardinality \( p \), \enquote{primitive root modulo \( p \)} is used by \textcite[12]{Gentle2006RandomNumberGeneration} and \textcite[def. 4.4.3]{Rosen2019DiscreteMathematics}. \Textcite[125]{Albert1956HigherAlgebra} and \textcite[thm. 11.2.2]{Hall1986CombinatorialTheory} uses \enquote{primitive roots} for arbitrary finite fields, however we find that terminology confusing considering we also discuss primitive \( n \)-th root of unity.

    \item \Textcite[A.V.40]{Bourbaki2003Algebra4to7}, \textcite[214]{Jacobson1985BasicAlgebraI} and \textcite[244]{Lang2002Algebra} use \enquote{primitive element} for what we call a \enquote{generator of a \hyperref[def:simple_field_extension]{simple field extension}}.
  \end{itemize}
\end{remark}
...
\begin{remark}\label{rem:primitive_polynomial_terminology}
  ...
  \textcite[106]{Berlekamp2015AlgebraicCodingTheory} remarks that
  \begin{displayquote}
  ...
  \textcite[38]{Gentle2006RandomNumberGeneration} instead refers to irreducible polynomials as \enquote{primitive}, even though, as shown in \cref{ex:def:finite_field_primitive_polynomial/f2_quartic}, their roots may not be primitive elements.
\end{remark}
...
\begin{proof}
  ...
  Then all roots of \( f(X) \) are primitive elements of \( \BbbF_{q^n} \), and \cref{thm:irreducible_monic_polynomial_is_minimal} implies that \( f(X) \) is a minimal polynomial for each of its roots.
