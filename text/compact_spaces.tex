\section{Compact spaces}\label{sec:compact_spaces}

\begin{definition}\label{def:centered_family}\mcite[123]{Engelking1989GeneralTopology}
...
\begin{definition}\label{def:compact_space}\mcite[123]{Engelking1989GeneralTopology}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:compact_space/convergent_nets} Every \hyperref[def:topological_net]{net} has a cluster point or, \hyperref[thm:def:net_limit_point/cluster_point_subnet_limit]{equivalently}, a \hyperref[def:net_limit_point]{convergent} subnet. This property is also called \enquote{generalized sequential compactness} or, when restricted to sequences instead of general nets, simply \enquote{sequential compactness}.
  \end{thmenum}
\end{definition}
\begin{proof}
  ...
  \ImplicationSubProof{def:compact_space/convergent_nets}{def:compact_space/centered_family}\mcite[thm. 3.1.23]{Engelking1989GeneralTopology} Assume that every net has a cluster point. Let \( \{ F_k \}_{k \in \mscrK} \) be a central family of closed sets.
...
\begin{remark}\label{rem:precompact_set}
  If the closure of \( A \) is compact, we call \( A \) \term{relatively compact} or \term{precompact} (although the term \enquote{precompact} is also used for totally bounded sets, see \cref{def:totally_bounded_set}).
\end{remark}
...
\end{proposition}
\begin{proof}
  \SubProofOf{thm:def:compact_space/closed} If \( A \) is compact, by \cref{def:compact_space/convergent_nets} every net in \( A \) has a cluster point in \( A \). By \cref{thm:cluster_point_characterization}, \( A \) is closed in \( X \).
...
\begin{theorem}[Tychonoff's product theorem]\label{thm:tychonoffs_product_theorem}\mcite[thm. 3.2.4]{Engelking1989GeneralTopology}
...
\begin{theorem}[Weierstrass' extreme value theorem]\label{thm:weierstrass_extreme_value_theorem}\mcite[cor. 3.2.9]{Engelking1989GeneralTopology}
...
\begin{definition}\label{def:locally_compact_space}\mcite[148]{Engelking1989GeneralTopology}
