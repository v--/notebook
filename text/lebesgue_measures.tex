...
\begin{comments}
  ...
  \item All characteristic functions of \hyperref[def:small_set]{small sets} can be grouped into a \hyperref[def:functor_representation]{representation} of the \hyperref[ex:def:functor/power_set]{power set functor} --- see \cref{ex:def:functor_representation/power_set}.
\end{comments}

\begin{definition}\label{def:dirichlet_function}\mcite[88]{Müller2022HandbookOfDynamicsAndProbability}
  We call the \hyperref[def:subset_characteristic_function]{characteristic function} of the \hyperref[def:irrational_number]{irrational numbers} in \( \BbbR \) \term{Dirichlet's function}.
\end{definition}
