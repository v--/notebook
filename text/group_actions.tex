...
\begin{definition}\label{def:endomorphism_monoid}\mimprovised
  ...
  Outside of \hyperref[ch:category_theory]{category theory}, whenever the category \( \cat{C} \) is clear from the context, we call \( \cat{C}(X) \) the \term[en=endomorphism monoid (\cite[60]{Jacobson1985BasicAlgebraI})]{endomorphism monoid} over \( X \) and denote it by \( \End(X) \).
\end{definition}
\begin{comments}
  \item \incite[60]{Jacobson1985BasicAlgebraI} defines endomorphism monoids for \hyperref[def:monoid/homomorphism]{monoid homomorphisms}, while \incite[515]{Grätzer2011LatticeTheory} defines them for \hyperref[def:lattice/homomorphism]{lattice homomorphisms}. We generalize both.
\end{comments}
