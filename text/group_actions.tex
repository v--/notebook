...
\begin{definition}\label{def:endomorphism_monoid}\mimprovised
  ...
  Outside of \hyperref[ch:category_theory]{category theory}, whenever the category \( \cat{C} \) is clear from the context, we call \( \cat{C}(X) \) the \term{endomorphism monoid} over \( X \) and denote it by \( \End(X) \).
\end{definition}
\begin{comments}
  \item \incite[515]{Grätzer2011Lattices} defines endomorphism monoids for \hyperref[def:lattice]{lattices}. We simply generalize his definition.
\end{comments}
...
\begin{definition}\label{def:group_action}
  ...
  A \term{left group action} or simply \term[bg=действие (на група) (\cite[def. IV.18]{ГеновМиховскиМоллов1991Алгебра}), ru=действие (группы) (\cite[def. 10.3.1]{Винберг2014Алгебра})]{group action} of \( G \) on \( X \) can be defined equivalently as:
  \begin{thmenum}
...
\begin{definition}\label{def:symmetric_group}\mcite[def. II.2.1]{Aluffi2009Algebra}
  We call the \hyperref[def:automorphism_group]{automorphism group} of a set \( A \) the \term[bg=симетрична група (\cite[376]{Обрешков1962ВисшаАлгебра}), ru=симметрическая группа (\cite[154]{Винберг2014Алгебра})]{symmetric group}\fnote{The term is closely related to symmetric functions defined in \fullref{def:symmetric_function}} on \( A \) and denote it by \( S_A \). The group \( S_A \) consists of bijective functions, which we call \term[bg=субституция (\cite[80]{ГеновМиховскиМоллов1991Алгебра}), ru=подстановка (\cite[154]{Винберг2014Алгебра})); перестановка (\cite[sec. 4.2]{Тыртышников2007ЛинейнаяАлгебра})]{permutations}.
...
\end{definition}
\begin{comments}
  \item Some authors, for example \incite[253]{MacLane1998Categories}, \incite[31]{Jacobson1985AlgebraI} and \incite[121]{Knapp2016BasicAlgebra}, call \( S_n \) the \enquote{symmetric group on \( n \) letters}.
...
\begin{definition}\label{def:cayley_table}\mcite[150]{Rotman2015AlgebraPart1}
...
\end{proposition}
\begin{proof}
  \Fullref{thm:permutation_parity_homomorphism} implies that the quotient \( S_n / A_n \) is isomorphic to the cyclic group \( C_2 \) of two elements.
...
\begin{proposition}\label{thm:group_epimorphisms_are_surjective}\mcite[exerc. I.5.5]{MacLane1998Categories}
