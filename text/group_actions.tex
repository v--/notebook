...
\begin{definition}\label{def:symmetric_group}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:symmetric_group/cycle} If there exists a finite sequence \( k_1, \ldots, k_m \) of distinct numbers such that \( \sigma(k_m) = k_1 \) and \( \sigma(k_{i+1}) = k_i \) for each \( i < m \), we say that the permutation is \term{cyclic} or a \term{cycle} of \term{length} \( m \). For brevity, we denote this cycle by \( \cycle{k_1, \cdots, k_m} \).

    We call cycles of length \( 1 \) \term{trivial cycles} or \term{loops} and cycles of length \( 2 \) --- \term{transpositions}. Every \hyperref[def:symmetric_group/cycle]{transposition} is an \hyperref[def:involution]{involution}, and thus a transposition is equal to its inverse permutation.
...
\end{proposition}
\begin{proof}
  We will use induction by \( m \). The proposition is trivial for transpositions (\( m = 2 \)), hence suppose that it holds for \( m - 1 \) and consider the transposition \( \cycle{ k_1, \cdots, k_m } \).

  We have, by the inductive hypothesis,
  \begin{equation*}
    \cycle{ k_1, \cdots, k_{m - 1} }
    =
    \cycle{ k_1, k_{m - 1} } \bincirc \cycle{ k_1, k_{m-1} } \bincirc \cdots \bincirc \cycle{ k_1, k_2 }.
  \end{equation*}

  By definition of cycle, \( k_{m-1} \) and \( k_1 \) get swapped, therefore
  \begin{equation*}
    \cycle{ k_1, \cdots, k_m } = \cycle{ k_1, k_m } \bincirc \cycle{ k_1, \cdots, k_{m - 1} }.
  \end{equation*}

  This completes the induction.
\end{proof}

\begin{proposition}\label{thm:cycle_cyclic_shift}
  The cycles \( \cycle{ k_1, \cdots, k_m } \) and \( \cycle{ k_{\rem(1 + i, m)}, \cdots, k_{\rem(m + i, m)} } \) are equivalent as permutation functions for any positive integer \( i \).

  We call this \term{cyclic shift} by \( i \).
\end{proposition}
\begin{proof}
...
\begin{example}\label{ex:s3}
  ...
  We will now construct a multiplication table to show that \( S_3 \) is not commutative. This will help us build examples based on \( S_3 \) later on.
  ...
  \begin{itemize}
    ...
    \item The cycles \( \cycle{ 1, 2, 3 } \) and \( \cycle{ 1, 3, 2 } \) are inverses of each other, and all others are \hyperref[def:involution]{involutions}.
    ...
    \item We can also utilize cyclic shifts as justified by \fullref{thm:cycle_cyclic_shift}:
    \begin{equation*}
...
\begin{lemma}\label{thm:group_operation_induces_bijections}
  For each element \( x \) of a group \( G \), consider the function \( \varphi_x \coloneqq x \cdot \id_G \), that is,
  \begin{equation*}
    \begin{aligned}
      &\varphi_x: G \to G \\
...
\begin{example}\label{ex:s3_and_a3}
  In the \hyperref[def:symmetric_group]{symmetric group} \( S_3 \) discussed in \fullref{ex:s3}, the \hyperref[def:alternating_group]{alternating group} \( A_3 \) consists of the identity and the odd-length cycles \( \cycle{ 1, 2, 3 } \) and \( \cycle{ 1, 3, 2 } \).
\end{example}
