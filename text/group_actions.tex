...
\end{definition}
\begin{comments}
  \item \incite[515]{Grätzer2011LatticeTheory} defines endomorphism monoids for \hyperref[def:lattice]{lattices}. We simply generalize his definition.
\end{comments}
...
\begin{example}\label{ex:def:group_action}
  ...
  \begin{thmenum}
    ...
    \thmitem{ex:def:group_action/formula_duality} \Fullref{thm:preorder_duality} shows how the two-element \hyperref[def:cyclic_group]{cyclic group} with identity \( e \) and symbol \( a \) acts on formulas in the \hyperref[def:preordered_set/language]{first-order language of preordered sets} --- the identity \( e \) maps every formula to itself, while \( a \) maps reverses the direction of inequalities.

    For the \hyperref[def:lattice/language]{language of lattices}, as justified by \fullref{thm:lattice_duality}, we instead consider the group with identity \( e \) and symbol \( b \), where \( b \) exchanges joins and meets.
...
\begin{proposition}\label{thm:def:group_action}
  ...
  \begin{thmenum}
    \thmitem{thm:def:group_action/negation} For any group element \( g \), the action \( \Phi: G \times X \to X \) satisfies
...
\begin{proof}
  ...
  \begin{equation*}
    ...
    =
    \Phi_e
    \reloset {\eqref{eq:def:group_action/family/identity}} =
    \id,
  \end{equation*}
...
\begin{definition}\label{def:group_representation}\mimprovised
  Let \( G \) be a \hyperref[def:group]{group} and let \( X \) be a \hyperref[def:vector_space]{vector space} over \( \BbbK \) or, more generally, an object in some \hyperref[def:category]{category} \( \cat{C} \) \hyperref[def:concrete_category]{concrete} over the \hyperref[def:vector_space/category]{category of vector spaces} \( \cat{Vect}_\BbbK \).
...
\begin{definition}\label{def:continuous_group_representation}\mcite[def. 6.8.25]{Sally2013Analysis}
  We say that the \hyperref[def:group_representation]{group representation} \( \Phi: G \times X \to X \) is \term{continuous} if it is a \hyperref[def:continuous_monoid_action]{continuous action}.
\end{definition}
