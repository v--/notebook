...
\begin{definition}\label{def:normal_series}\mcite[95]{Курош1967ТеорияГрупп}
  ...
  \begin{equation}\label{eq:def:normal_series}
    \set{ e } = G_n \subsetneq G_{n-1} \subsetneq \cdots \subsetneq G_1 \subsetneq G_0 = G,
  \end{equation}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:normal_series/equivalence}\mcite[96]{Курош1967ТеорияГрупп} We say that the normal series \eqref{eq:def:normal_series} is \term[ru=изоморфные (ряды), en=equivalent (series) (\cite[197]{Rotman2015AdvancedModernAlgebraPart1})]{equivalent} to
    \begin{equation*}
      \set{ e } = H_m \subsetneq H_{m-1} \subsetneq \cdots \subsetneq H_1 \subsetneq H_0 = G
    \end{equation*}
    if \( n = m \) and if there exists a permutation \( \sigma \) from \( S_n \) such that, for \( k = 0, \ldots, n - 1 \), the quotient groups \( G_k / G_{k+1} \) and \( H_{\sigma(k)} / G_{\sigma(k)+1} \) are isomorphic.

    Permuting the quotients is necessary in some cases like \cref{ex:def:normal_series/cyclic}, where we nevertheless want the two series to be equivalent.
...
\begin{example}\label{ex:def:normal_series}
  ...
  \begin{thmenum}
    ...
    More generally, every \hyperref[def:partial_order_connectedness]{chain} in the natural number divisibility lattice from \cref{thm:natural_number_divisibility_lattice} induces an \enquote{infinite normal series}.

    \thmitem{ex:def:normal_series/cyclic} In the \hyperref[def:cyclic_group]{cyclic group} \( C_6 \), we have the following composition series:
    \begin{align*}
      \set{ e } \subsetneq \braket{ a^2 } \subsetneq C_6, \\
      \set{ e } \subsetneq \braket{ a^3 } \subsetneq C_6.
    \end{align*}

    Since \( \set{ e } \cong C_1 \) and \( \braket{ a^2 } \cong C_3 \), the first series has quotients \( C_6 / C_3 \cong C_2 \) and \( C_3 / C_1 \cong C_3 \).

    Similarly, the second series has quotients \( C_6 / C_2 \cong C_3 \) and \( C_2 / C_1 \cong C_2 \).

    The quotients in the two composition series match up to permutation, so the series themselves are equivalent in the sense of \cref{def:normal_series/equivalence}.
...
\begin{definition}\label{def:group_commutator}\mcite[245]{Jacobson1985BasicAlgebraI}
  Let \( G \) be an arbitrary group. We define the \term[ru=коммутатор (\cite[37]{Кострикин2020АлгебраЧасть3})]{commutator} of the elements \( x \) and \( y \) as
  \begin{equation*}
    [x, y] \coloneqq \underbrace{xy(yx)^{-1}}_{xyx^{-1}y^{-1}}.
  \end{equation*}

  We call the subgroup \hyperref[def:group/subobject]{generated} by all the commutators in \( G \) the \term[ru=коммутант (группы) (\cite[37]{Кострикин2020АлгебраЧасть3})]{commutator subgroup} and denote it by \( [G, G] \).

  In analogy with \hyperref[def:differentiability]{function derivatives} and \hyperref[def:derived_set]{derived sets} in topology, we call \( [G, G] \) the \term[ru=производная подгруппа (\cite[37]{Кострикин2020АлгебраЧасть3})]{derived subgroup} and denote \( [G, G] \) by \( G' \). Via iteration, we extend this notation to \( G^{(k)} \).
\end{definition}
\begin{comments}
...
\begin{definition}\label{def:group_abelianization}\mimprovised
...
\begin{theorem}[Group abelianization universal property]\label{thm:group_abelianization_universal_property}
  The \hyperref[def:group_abelianization]{abelianization} \( G / [G, G] \) of a group \( G \) satisfies the following \hyperref[con:universal_property]{universal property}:
  \begin{displayquote}
...
\begin{proof}
  ...
  In order to use \eqref{eq:thm:group_abelianization_universal_property/homomorphism} as a definition, we must prove that \( x \equiv y \pmod {[G, G]} \) implies that \( \varphi(x) = \varphi(y) \), that is, that \( xy^{-1} \in [G, G] \) implies \( \varphi(xy^{-1}) = e_H \). We will show via \fullref{thm:induction_on_generated_substructures} on \( n \in [G, G] \) that \( \varphi(n) = e_H \).
  \begin{itemize}
...
\begin{definition}\label{def:derived_group_series}\mcite[def. IV.3.9]{Aluffi2009Algebra}
  We define the \term[ru=производный ряд (\cite[48]{Кострикин2020АлгебраЧасть3})]{derived series} of the \hyperref[def:group]{group} \( G \) as the descending sequence of \hyperref[def:group_commutator]{commutator subgroups}
  \begin{equation*}
    \cdots \subseteq G^{(k)} \subseteq \cdots \subseteq G' \subseteq G.
  \end{equation*}
\end{definition}
\begin{comments}
  \item This is nor necessarily a normal series in the sense of \cref{def:normal_series} because it may not be strictly descending (in which case it cannot stabilize; see \cref{thm:simple_solvable_group_is_abelian}).
\end{comments}

\begin{definition}\label{def:solvable_group}\mcite[def. IV.3.9]{Aluffi2009Algebra}
  We say that a group is \term[ru=разрешимая (группа) (\cite[48]{Кострикин2020АлгебраЧасть3})]{solvable} if its \hyperref[def:derived_group_series]{derived series} \hyperref[def:stabilizing_sequence]{stabilizes} at the trivial subgroup.
\end{definition}
