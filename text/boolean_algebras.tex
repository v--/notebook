...
\begin{definition}\label{def:bounded_lattice_complement}\mcite[16]{Birkhoff1967Lattices}
...
\begin{definition}\label{def:boolean_algebra}\mcite[18]{Birkhoff1967Lattices}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:boolean_algebra/submodel}\mcite[18]{Birkhoff1967Lattices} In addition to containing the joins and meets of all its members, a \hyperref[def:first_order_submodel]{first-order submodel} of a Boolean algebra must also contain the complement of each of its members. We call such submodels \term{Boolean subalgebras}.
...
\begin{comments}
  ...
  \item This result is richer than the aforementioned ones --- \incite{Gottschalk1953Quaternality} refers to it as \enquote{quaterniality} rather than \enquote{duality}.
\end{comments}
\begin{proof}
...
\begin{proposition}\label{thm:def:boolean_algebra}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:def:boolean_algebra/distributive}\mcite[lemma 162]{Grätzer2011Lattices} In a \hyperref[def:complete_lattice]{complete} Boolean algebra, for any element \( x \) and any family \( \seq{ y_k }_{k \in \mscrK} \), we have
    \begin{align}
...
\begin{definition}\label{def:ultrafilter}
  We say that a proper \hyperref[def:lattice_ideal]{filter} \( F \) in a \hyperref[def:boolean_algebra]{Boolean algebra} is an \term[bg=ултрафилтер (\cite[18]{Проданов1982ФункАнализЧаст1}), ru=ультрафильтр (\cite[182]{Гуров2013Решётки})]{ultrafilter} if any of the following equivalent conditions hold:
  \begin{thmenum}
    \thmitem{def:ultrafilter/direct}\mcite[182]{Гуров2013Решётки} For every algebra element \( x \), either\fnote{Both \( x \in F \) and \( \oline x \in F \) cannot hold because of \fullref{thm:improper_boolean_ideal}.} \( x \in F \) or \( \oline x \in F \).

    \thmitem{def:ultrafilter/prime} \( F \) is a \hyperref[def:lattice_ideal/prime]{prime filter}.

    \thmitem{def:ultrafilter/maximal}\mcite[233]{DaveyPriestley2002Lattices} \( F \) is a \hyperref[def:lattice_ideal/maximal]{maximal filter}.
  \end{thmenum}
\end{definition}
\begin{proof}
...
\end{definition}
\begin{comments}
  \item \incite[234]{DaveyPriestley2002Lattices} define \enquote{principal ultrafilters} to be maximal \hyperref[def:lattice_ideal/principal]{principal filters}, which notion differs from our definition.
\end{comments}
