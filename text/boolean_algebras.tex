...
\begin{example}\label{ex:def:boolean_algebra}
  ...
  \begin{thmenum}
    \thmitem{ex:def:boolean_algebra/lindenbaum_tarski} As shown in \fullref{eq:def:propositional_natural_deduction_systems/bot/dne}, every \hyperref[def:lindenbaum_tarski_algebra]{Lindenbaum-Tarski algebra} for the \hyperref[def:propositional_natural_deduction_systems]{classical natural deduction system} is a Boolean algebra.
...
\begin{proposition}\label{thm:boolean_algebra_heyting_characterization}
  A \hyperref[def:heyting_algebra]{Heyting algebra} is a \hyperref[def:boolean_algebra]{Boolean algebra} if and only if \( \widetilde{\widetilde{x}} = x \) for any element.
\end{proposition}
\begin{comments}
  \item \Fullref{thm:def:heyting_algebra/dni} implies that \( x \leq \widetilde{\widetilde{x}}\) in any Heyting algebra, so, when using this characterization, there is only one inequality to show.
\end{comments}
\begin{proof}
  \SufficiencySubProof Follows from the definition of a Boolean algebra, considering \fullref{thm:def:boolean_algebra/heyting}.
  \NecessitySubProof Suppose that \( \widetilde{\widetilde{x}} = x \) for any element in a Heyting algebra. It is a bounded lattice by definition, and \fullref{thm:def:heyting_algebra/distributive} implies that it is distributive. We must only show that \( \widetilde{x} \) is a \hyperref[def:bounded_lattice_complement]{complement} of \( x \).

  \Fullref{thm:def:heyting_algebra/semicomplement} implies that \( x \wedge \widetilde{x} = \bot \) for any Heyting algebra. On the other hand,
  \begin{equation*}
    \widetilde{x \vee \widetilde{x}}
    \reloset {\eqref{eq:thm:heyting_algebra_de_morgans_laws/complete_of_meet}} \geq
    \widetilde{x} \wedge \widetilde{\widetilde{x}}
    =
    \widetilde{x} \wedge x
    =
    \bot
  ...
  \begin{equation*}
    x \vee \widetilde{x}
    =
    \widetilde{\widetilde{x \vee \widetilde{x}}}
    =
    \widetilde{\bot}
  ...
  Then \( \widetilde{x} \) is the complement of \( x \).
\end{proof}
...
\end{comments}
\begin{proof}
  The equality \eqref{eq:thm:de_morgans_laws/complement_of_join} is a restatement of \eqref{eq:thm:heyting_algebra_de_morgans_laws/complement_of_join}, while \eqref{eq:thm:de_morgans_laws/complement_of_meet} follows via \fullref{thm:boolean_algebra_duality}.
\end{proof}
