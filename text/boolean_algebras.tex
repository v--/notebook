...
\begin{proposition}\label{thm:def:boolean_algebra}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:def:boolean_algebra/distributive}\mcite[lemma 162]{Gratzer2011Lattices} In a \hyperref[def:complete_lattice]{complete} Boolean algebra, for any element \( x \) and any family \( \seq{ y_k }_{k \in \mscrK} \), we have
    \begin{align}
...
\begin{definition}\label{def:ultrafilter}
  We say that a proper \hyperref[def:lattice_ideal]{filter} \( F \) in a \hyperref[def:boolean_algebra]{Boolean algebra} is an \term[bg=ултрафилтер (\cite[18]{Проданов1982ФункАнализТом1}), ru=ультрафильтр (\cite[182]{Гуров2013Решётки})]{ultrafilter} if any of the following equivalent conditions hold:
  \begin{thmenum}
