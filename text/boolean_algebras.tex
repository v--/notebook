...
\begin{definition}\label{def:boolean_algebra}\mcite[18]{Birkhoff1967LatticeTheory}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:boolean_algebra/homomorphism} A \hyperref[def:fol_homomorphism]{first-order homomorphism} between Boolean algebras is a lattice homomorphism that preserve top and bottom elements. Complements are automatically preserved, as we shall see in \cref{thm:distributive_bounded_lattice_unique_complement}.
...
\end{theorem}
\begin{comments}
  \item This is one of several variants of De Morgan's laws presented here --- see \cref{ex:de_morgans_laws/boolean}.
\end{comments}
\begin{proof}
...
\begin{theorem}[Principle of duality for Boolean algebras]\label{thm:boolean_algebra_duality}\mcite{Gottschalk1953Quaternality}
  Consider the \hyperref[def:lattice/theory]{first-order theory of Boolean algebras}. Within it, consider the \hyperref[def:closed_fol_formula]{closed formula} \( \varphi \). Denote by \( \varphi^{\oppos_C} \) the dual formula in the sense of \fullref{thm:lattice_duality}, in which we swap the connectives \( \vee \) and \( \wedge \), the relations \( \leq \) and \( \geq \) and then replace every occurrence of \( \tau \rightarrow \sigma \) with \( \sigma \rightarrow \tau \).

  , as well as . Denote by \( \varphi^{\oppos_L} \) the formula obtained from \( \varphi \) by swapping each variable with its complement.

  If every Boolean algebra \hyperref[def:first_order_model]{satisfies} \( \varphi \), then every Boolean algebra also satisfies \( \varphi^{\oppos_C} \), \( \varphi^{\oppos_L} \) and \( \varphi^{\oppos_{LC}} \).
  ...
  \begin{paracol}{2}
    \begin{leftcolumn}
      \begin{itemize}
        ...
        \item \( X \) satisfies \( \varphi^{\oppos_{LC}} \).
      \end{itemize}
    \end{leftcolumn}

    \begin{rightcolumn}
      \begin{itemize}
        \item \( X^\oppos \) satisfies \( \varphi^{\oppos_C} \).
        \item \( X^\oppos \) satisfies \( \varphi^{\oppos_L} \).
      \end{itemize}
    \end{rightcolumn}
  \end{paracol}
\end{theorem}
\begin{comments}
  \item Similar statements hold more generally --- see \fullref{thm:preorder_duality} and \fullref{thm:lattice_duality}. This result is richer than the aforementioned ones --- \incite{Gottschalk1953Quaternality} refers to it as \enquote{quaterniality} rather than \enquote{duality}. See \cref{ex:def:group_action/formula_duality} for a discussion of how it relates to the \hyperref[def:klein_four_group]{Klein four-group}.

  Also, unlike in those cases, neither of the opposite formulas can be produced solely via \fullref{alg:fol_formula_signature_translation}.

  \item With Boolean algebra operations regarded as logical operations, these correspond exactly to the transformations of \hyperref[def:propositional_formula]{propositional formulas} from \fullref{alg:opposite_propositional_formula}. In particular, the letter \enquote{L} used in \( \varphi^{\oppos_L} \) comes from \enquote{literal}.
\end{comments}
\begin{proof}
  \Cref{thm:def:boolean_algebra/opposite_complement} implies that complementation coincides in \( X \) and \( X^\oppos \), thus, if \( X \) satisfies \( \varphi \), then, as in \fullref{thm:lattice_duality}, \( X^\oppos \) satisfies \( \varphi^{\oppos_C} \).

  On the other hand, a \hyperref[def:fol_variable_assignment]{variable assignment} in \( X \) corresponds to the valuation of the complemented variables in \( X^\oppos \), thus \( X \) satisfies \( \varphi \) if and only if \( X^\oppos \) satisfies \( \varphi^{\oppos_L} \).

  Finally, \( X \) satisfies \( \varphi \) if and only if \( X^\oppos \) satisfies \( \varphi^{\oppos_C} \) if and only if \( X = (X^\oppos)^\oppos \) satisfies \( \varphi^{\oppos_{LC}} \).
\end{proof}
