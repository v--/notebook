...
\begin{definition}\label{def:boolean_algebra}\mcite[18]{Birkhoff1967LatticeTheory}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:boolean_algebra/submodel}\mcite[18]{Birkhoff1967LatticeTheory} In addition to containing the joins and meets of all its members, a \hyperref[def:fol_substructure/submodel]{first-order submodel} of a Boolean algebra must also contain the complement of each of its members. We call such submodels \term{Boolean subalgebras}.
    ...
    We show in \cref{thm:def:boolean_algebra/opposite_order} that the induced order in the opposite Boolean algebra is the \hyperref[def:partially_ordered_set/opposite]{opposite order}.
...
\begin{definition}\label{def:opposite_boolean_polynomial}\mcite[ch. 4]{HalmosGivant2009BooleanAlgebras}
  ...
  \begin{thmenum}
    ...
    The dual extends the \hyperref[def:dual_lattice_polynomial]{opposite lattice polynomial} by preserving complements.
...
\begin{proof}
  ...
  Let \( x \vee y \in F \). If \( x \notin F \), then \( \oline x \in F \) and hence the following is also a member of \( F \):
  \begin{equation*}
    \oline x \vee (x \vee y)
    =
    (\oline x \vee x) \vee y
...
\end{definition}
\begin{comments}
  \item \Textcite[234]{DaveyPriestley2002LatticeTheory} define \enquote{principal ultrafilters} to be maximal \hyperref[def:lattice_ideal/principal]{principal filters}, which notion differs from our definition.
\end{comments}
