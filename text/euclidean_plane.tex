...
\begin{proposition}\label{thm:plane_rotation_matrix_angle}
  ...
  \begin{equation}\label{eq:thm:plane_rotation_matrix_angle}
    ...
    \end{pmatrix}
  \end{equation}
  is an \hyperref[def:morphism_cancellation/right]{epimorphism} from the real numbers under addition to the group of \hyperref[thm:plane_rotation_matrix]{plane rotation matrices} under composition. The kernel of this map is the set of multiples of \( 2\pi \).
...
\begin{proposition}\label{thm:sum_of_angles_measure}
  The \hyperref[def:sum_of_angles]{angle sum} of \( \angle(r, p) \) and \( \angle(p, s) \) satisfies the following \hyperref[def:equivalence_modulo/real_number]{congruence} of \hyperref[def:angle/measure]{angle measures}:
  \begin{equation*}
...
\begin{proof}
  ...
  \begin{equation*}
    ...
    o + GF(v - o),
  \end{equation*}
  hence \cref{thm:plane_ray_rotation} implies that \( g \bincirc f \) is the \emph{unique} affine map with a rotation sending \( r \) to \( s \).
...
\begin{definition}\label{def:angles_of_transversal}\mcite[\S 37]{Hadamard2008LessonsInGeometryVol1}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:angles_of_transversal/alternate} We say that two \emph{non-\hyperref[def:adjacent_angles]{adjacent}} angles are \term[bg={кръстни, кръстосани (\cite[\S 28]{Гюзелев1873Геометрія})}, ru=накрестлежащие (\cite[\S 72]{Киселёв2004Геометрия})]{alternate} if they lie in different half-planes with respect to \( l \) and are both either interior or exterior.
    ...
    \thmitem{def:angles_of_transversal/corresponding} We say that two \emph{non-\hyperref[def:adjacent_angles]{adjacent}} angles are \term[bg=съответни (\cite[\S 28]{Гюзелев1873Геометрія}), ru=соответственные (\cite[\S 72]{Киселёв2004Геометрия})]{corresponding} if they lie in different in the same half-plane with respect to \( l \) and one of them is internal, while the other is external.
