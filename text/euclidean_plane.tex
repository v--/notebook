...
\begin{proof}
  ...
  \begin{equation*}
    ...
    \end{pmatrix},
  \end{equation*}
  implying via \fullref{thm:matrix_invertibility_via_determinants} that \( (A_h, B_h) \) and \( (A_g, B_g) \) are linearly dependent.
  ...
  \begin{equation*}
    ...
    \end{pmatrix}
  \end{equation*}
  implying via \fullref{thm:matrix_invertibility_via_determinants} that \( (x_g, y_g) \) and \( (x_h, y_h) \) are linearly dependent.
...
\begin{proof}
  ...
  \NecessitySubProof Suppose that \( g \) and \( h \) are not parallel. \Fullref{thm:parallel_lines_in_plane} implies that the vectors \( (A_g, A_h) \) and \( (B_g, B_h) \) are linearly independent, and \fullref{thm:matrix_invertibility_via_determinants} implies that
  \begin{equation*}
    \begin{pmatrix}
      x \\ y
    \end{pmatrix}
...
\begin{definition}\label{def:angle}\mimprovised
  ...
  \begin{thmenum}
    ...
    \begin{thmenum}
      ...
      \thmitem{def:angle/measure/acute}\mcite[\S 16]{Hadamard2008GeometryVol1} \term[bg=остър (\cite[9]{Гюзелев1873Геометрія}), ru=острый (\cite[\S 22]{Киселёв2004Геометрия})]{acute} if \( 0 < \varphi < \pi / 2 \),
      ...
      \thmitem{def:angle/measure/obtuse}\mcite[\S 16]{Hadamard2008GeometryVol1} \term[bg=тъп (\cite[9]{Гюзелев1873Геометрія}), ru=тупой (\cite[\S 22]{Киселёв2004Геометрия})]{obtuse} if \( \pi / 2 < \varphi < \pi \),
...
\end{corollary}
\begin{proof}
  \Fullref{thm:sum_of_angles_measure} then implies that
  \begin{equation*}
    \measuredangle(-r, s) = \measuredangle(-r, r) + \measuredangle(r, s).
  \end{equation*}

  \Fullref{thm:straight_iff_opposite_rays} implies that
  \begin{equation*}
  ...
  Then \eqref{eq:thm:angle_of_opposite_ray/sin} and \eqref{eq:thm:angle_of_opposite_ray/cos} follow from \fullref{thm:trigonometric_function_period_identities}.
\end{proof}
