...
\begin{example}\label{ex:def:topological_net}
  ...
  \begin{thmenum}
    \thmitem{def:topological_net/empty} The most basic example of a net is the empty set. Indeed, an empty set is vacuously directed.
...
\begin{definition}\label{def:subnet}\mcite[70]{Kelley1975}
  ...
  \begin{thmenum}
    \thmitem{def:topological_net/subnet/composition} For every \( m \in \mscrM \), we have \( x_{i_m} = y_m \).

    This condition ensures that the subnet consists of members of the supernet.

    \thmitem{def:topological_net/subnet/cofinality} For every \( k_0 \in \mscrK \), there exists some index \( m_0 \in \mscrM \) such that, for every \( m \geq m_0 \), we have \( i_m \geq k_0 \).
