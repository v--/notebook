...
\begin{definition}\label{def:topological_net}\mcite[65]{Kelley1975GeneralTopology}
  ...
  We reuse the notation for indexed families from \fullref{def:indexed_family}:
  \begin{equation*}
...
\begin{example}\label{ex:def:net_limit_point}
  ...
  \begin{thmenum}
    ...
    \thmitem{ex:def:net_limit_point/sierpinski_constant} As in \fullref{ex:def:net_cluster_point/sierpinski}, consider the \hyperref[def:sierpinski_space]{Sierpi\'nski space} \( \set{ \top, \bot } \) and the constant sequence
    \begin{equation*}
      \top, \top, \top, \top, \top, \top.
    \end{equation*}
