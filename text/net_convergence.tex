...
\begin{definition}\label{def:topological_net}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:topological_net/subnet}\mcite[50]{Engelking1989} We say that the net \( \seq{ y_m }_{m \in \mscrM} \subseteq S \) is a \term{subnet} of \( \seq{ x_k }_{k \in \mscrK} \subseteq S \) if there exists a \hyperref[def:partially_ordered_set/homomorphism]{monotone function} \( \varphi: \mscrM \to \mscrK \).

    if there exists an embedding function \( \varphi: M \to \mscrK \) such that
    \begin{thmenum}
      \thmitem{def:topological_net/subnet/directed} To every \( k \in \mscrK \) there corresponds \( m \in \mscrM \) such that \( \varphi(m) \geq k \).
      \thmitem{def:topological_net/subnet/identity} For every \( m \in \mscrM \) we have \( x_{\varphi(m)} = y_m \).
    \end{thmenum}
  \end{thmenum}
\end{definition}
