...
\begin{proof}
  ...
  \NecessitySubProof Let \( x_0 \) be a cluster point of \( \seq{ x_k }_{k \in \mscrK} \subseteq A \). We will show that \( x_0 \) belongs every closed set that contains \( A \).

  Let \( F \supseteq A \) be a closed set. Denote \( U \coloneqq X \setminus F \). Suppose that \( x_0 \in U \). Then \( U \) is a neighborhood \( x_0 \) and, by \fullref{def:net_cluster_point}, the net \( \seq{ x_k }_{k \in \mscrK} \) is frequently in \( U \).

  But \( U \) does not contain \( A \).
...
\begin{proposition}\label{thm:net_limit_point_subbase}
  Let \( \mscrS \) be a \hyperref[def:topological_subbase]{subbase} in some topological space.

  The point \( x_0 \) is a \hyperref[def:net_limit_point]{limit point} of the net \( \seq{ x_k }_{k \in \mscrK} \) if and only if it is \hyperref[def:net_eventually_in]{eventually in} every neighborhood of \( x_0 \) in \( \mscrS \).
\end{proposition}
\begin{proof}
...
\begin{remark}\label{rem:sequential_spaces}
  By \fullref{thm:cluster_point_iff_in_closure}, in a \hyperref[def:sequential_space]{sequential space}, a set is closed if and only if it is sequentially closed.
...
\begin{proof}
  ...
  Fix a point \( x_0 \in \cl(A) \). We will show that \( x_0 \in A \). By \fullref{thm:cluster_point_iff_in_closure}, there is a net \( \seq{ x_k }_{k \in \mscrK} \subseteq A \) for which \( x_0 \) is a limit point.
