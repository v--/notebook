...
\begin{concept}\label{con:function_arguments}
  ...
  We may use prefixes from \fullref{def:operation_arity_terminology} like \enquote{unary}, \enquote{binary} and so forth on arbitrary functions if there is no possibility of confusion about the number of parameters.
\end{concept}
...
\begin{concept}\label{con:undefinedness}\mimprovised
  ...
  Having such a notion available in the \hyperref[def:metalogic]{metalanguage} can simplify case-wise definitions. For example, consider the characterization of the \hyperref[def:polynomial_degree]{degree} of a univariate polynomials over the real numbers from \fullref{rem:zero_polynomial_degree/minus_one}:
  \begin{equation*}
    \deg(f) = \begin{cases}
...
\begin{remark}\label{rem:undefined_and_primitive_terms}
  Both \incite[57]{Church1956LogicVol1} and \incite[28]{Kleene1971Metamathematics} use the adjective \enquote{undefined} for the symbols that are considered primitive rather than defined via others. Such symbols are the ones in the signatures \hyperref[def:first_order_language/fun]{in first-order logic} or \hyperref[def:simple_type_signature]{in simple type theory}.

  Their notion of undefinedness is distinct from the one we present in \fullref{con:undefinedness} --- they delegate the semantics of an undefined expression to the \hyperref[def:metalogic]{metatheory}, while the notion presented here signifies that the expression has no intended semantics.
\end{remark}
...
\end{proposition}
\begin{comments}
  \item A similar result holds for function types and dependent product types; see \fullref{con:dependent_type_theory}.
\end{comments}
\begin{proof}
