...
\begin{remark}\label{rem:function_definition}
  ...
  Then \( A \) is related under \( \mscrR \) with both \( \set{ A } \) and \( \pow(A) \). This is technically not a graph of a function because we lack uniqueness. It is a generalization, which Jean-Pierre Aubin and Helene Frankowska in \cite[def. 1.3.1]{AubinFrankowska1990}, Asen Dontchev and Ralph Rockafellar in \cite[1]{DontchevRockafellar2014} and Robert Phelps in \cite[VII]{Phelps1993} call \term{set-valued maps}\footnote{Every mapping is technically set-valued, but the values of single-valued functions are treated as atoms, while the values of set-valued maps are treated as sets of atoms}.
...
\begin{definition}\label{def:set_valued_map}\mcite[def. 1.3.1]{AubinFrankowska1990}
  ...
  \begin{thmenum}[series=def:set_valued_map]
    ...
    \thmitem{def:set_valued_map/partial}\mcite[exer. 2.3.12]{Leinster2016Basic} We say that \( F \) is \term{partial} if \( F(x) = \varnothing \) for some \( x \in A \) and \term{total} otherwise.
