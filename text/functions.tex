...
\end{comments}
\begin{proof}
  Consider the union \( \bigcup_{k \in \mscrK} A_k \) of disjoint sets, and the corresponding disjoint union \( \coprod_{k \in \mscrK} A_k \). Denote by \( \iota_k \) the \( k \)-th inclusion map into \( \coprod_{k \in \mscrK} A_k \) and define the map
  \begin{equation*}
    \begin{aligned}
      &\varphi: \coprod_{k \in \mscrK} A_k \to \bigcup_{k \in \mscrK} A_k, \\
      &\varphi(\iota_k(a_k)) \coloneqq a_k.
    \end{aligned}
  \end{equation*}

  It is well-defined because, by \fullref{thm:disjoint_product_inclusions_injective}, \( \iota_k \) is injective, and thus \( a_k \) is entirely determined by \( \iota_k(a_k) \).

  The map \( \varphi \) is injective because, as already mentioned, the inclusion map \( \iota_k \) is injective for every index \( k \) in \( \mscrK \), and their domains are disjoint.

  The map \( \varphi \) is also surjective because, for any element \( a \) of \( \bigcup_{k \in \mscrK} A_k \), there exists an index \( m \) in \( \mscrK \) such that \( a \) belongs to \( A_m \), and thus \( \varphi(\iota_m(a)) = a \).

  Therefore, \( \varphi \) is bijective. \Fullref{thm:categorical_limit_uniqueness} then implies that \( \bigcup_{k \in \mscrK} A_k \) is a coproduct.
\end{proof}
