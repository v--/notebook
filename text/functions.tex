...
\begin{remark}\label{rem:function_definition}
  ...
  \cref{thm:grothendieck_universe_closure/function} may break --- see \cref{rem:small_functions}.
...
\begin{definition}\label{def:set_valued_map}\mcite[def. 1.3.1]{AubinFrankowska1990SetValuedAnalysis}
  ...
  \begin{thmenum}[series=def:set_valued_map]
    ...
    \thmitem{def:set_valued_map/partial}\mcite[exerc. 2.3.12]{Leinster2014BasicCategoryTheory} We say that \( F \) is \term{partial} if \( F(x) = \varnothing \) for some \( x \in A \) and \term{total} otherwise.
