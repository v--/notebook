...
\begin{remark}\label{rem:function_definition}
  ...
  \incite[def. I.3.1]{Bourbaki2004TheoryOfSets} propose using a \hyperref[def:binary_relation]{binary relation} \( R \), called the \term{graph} of the mapping, such that the sets \( A \) and \( B \) are related under \( R \) if \( B \) is the unique value corresponding to \( A \).
  ...
  \incite[def. I.6.3]{Kunen2013SetTheory},
  \incite[43]{Enderton1977SetTheory},
  \incite[30]{Halmos1960NaiveSetTheory},
  \incite[7]{Kelley1975GeneralTopology},
  \incite[1]{Engelking1989GeneralTopology},
  \incite[143]{КолмогоровДрагалин2006Логика},
  \incite[127]{Герасимов2014Вычислимость} and
  \incite[32]{ШеньВерещагин2020НачалаТеорииМножеств}. This renders the discussion of function images and related concepts like surjectivity meaningless.

  \incite[\S 1.2.5]{КусраевКутателадзе2005БулевозначныйАнализ} define functions as sets of ordered pairs and then define \enquote{соответствия} (\enquote{correspondences}) as the triples discussed above.
  ...
  Then \( A \) is related under \( \mscrR \) with both \( \set{ A } \) and \( \pow(A) \). This is technically not a graph of a function because we lack uniqueness. It is a generalization, which \incite[def. 1.3.1]{AubinFrankowska1990SetValuedAnalysis}, \incite[1]{DontchevRockafellar2014SolutionMappings} and \incite[VII]{Phelps1993ConvexDifferentiability} call \term{set-valued maps}\fnote{Every mapping is technically set-valued, but the values of single-valued functions are treated as atoms, while the values of set-valued maps are treated as sets of atoms}.
...
\end{definition}
\begin{comments}
  \item Rather than introducing these concepts for set-valued functions, \incite[8]{Kelley1975GeneralTopology} does it for relations.

  \item Some authors like \incite[1]{Engelking1989GeneralTopology} and \incite[596]{Knapp2016BasicAlgebra} use \enquote{range} for what we call the codomain of a function, while others like \incite[11]{Kelley1975GeneralTopology} and \incite[43]{Enderton1977SetTheory} use \enquote{range} for what we call the image.

  To avoid ambiguity, we avoid the term \enquote{range} altogether.
\end{comments}

\begin{remark}\label{rem:notation_for_function_image}
  The notation \( f[X] \) for the image of a set \( A \) is mostly used within set theory, for example by \incite[44]{Enderton1977SetTheory}. The notation \( f(X) \) is used in other context like topology, for example by \incite[1]{Engelking1989GeneralTopology}, and analysis, for example by \incite[def. 1.3.1]{AubinFrankowska1990SetValuedAnalysis}. We generally prefer \( f(X) \) because it reflects the established convention in fields more relevant to us, but occasionally use \( F[X] \) in order to avoid ambiguities. For example, if \( A \) is a \hyperref[def:transitive_set]{transitive set}, then \( x \in A \) implies that \( x \subseteq A \), yet
  \begin{equation*}
...
\end{concept}
\begin{comments}
  \item Some authors use the \hyperref[def:propositional_alphabet/constants/falsum]{falsum symbol} \( \bot \) for denoting undefined values. For example, \incite[99]{Savage2008ModelsOfComputation} when discussing \hyperref[def:turing_machine]{Turing machines}, or \incite[62]{ШеньВерещагин2020НачалаТеорииМножеств} when discussing set theory.

  \incite[\S 3.2]{Farmer2008STTVirtues} suggests a dedicated constant \( \bot_\tau \) as an undefined value of type \( \tau \) in his presentation of \hyperref[def:higher_order_logic]{higher-order logic}. This is done specifically to provide well-behaved semantics for the \hyperref[con:description_operator/iota]{definite description operator}.

  \item \incite[475]{Farmer2004UndefinednessInCalculus} describes undefinedness as follows:
  \begin{displayquote}
...
\begin{remark}\label{rem:undefined_and_primitive_terms}
  \incite[v]{WhiteheadRussell1927PrincipiaMathematicaVol1} and \incite[28]{Kleene1971Metamathematics} use the adjective \enquote{undefined} for the symbols that are considered primitive rather than defined via others. As discussed in \cref{con:primitive_notion}, we call such symbols \enquote{primitive} to prevent confusion with the notion of undefinedness which we present in \cref{con:undefinedness}.
...
\begin{remark}\label{rem:set_valued_map_as_single_valued}
  Rather than defining a \hyperref[def:function]{single-valued function} to be a special case of \hyperref[def:set_valued_map]{set-valued maps}, \incite[def. 2.3]{Phelps1993ConvexDifferentiability} defines set-valued maps from \( A \) to \( B \) as single-valued functions from \( A \) to \( \pow(B) \).
...
\begin{concept}\label{con:function_superposition}
  Although the terms \enquote{composition} and \enquote{superposition} are used interchangeably, for example by \incite[\S 25]{Фихтенгольц1968ОсновыАнализаТом1}, the term \enquote{superposition} often refers to a certain generalization of \hyperref[def:set_valued_map/composition]{function composition}. The latter is used by \incite[16]{Яблонский2003ДискретнаяМатематика}.
...
\begin{comments}
  ...
  \incite[def. 6.4.1]{Hinman2005Logic},
  \incite[59; 60]{Halmos1960NaiveSetTheory},
  \incite[thm. 6M(3)]{Enderton1977SetTheory},
  \incite[def. 3.4]{Harzheim2005OrderedSets} and
  \incite[136]{Герасимов2014Вычислимость},
  among others.
...
\begin{remark}\label{rem:set_of_all_functions_notation}
  ...
  \begin{thmenum}
    ...
    \incite[30]{Halmos1960NaiveSetTheory},
    \incite[11]{Jech2006SetTheory},
    \incite[9]{Willard2004GeneralTopology},
    \incite[9]{Aluffi2009Algebra},
    \incite[xi]{Golan1999Semirings},
    \incite[exerc. 1.3.28]{Leinster2014BasicCategoryTheory},
    \incite[18]{MacLane1998CategoryTheory},
    \incite[39]{Stanley2012EnumerativeCombinatoricsVol1},
    \incite[11]{CitkinMuravitsky2022ConsequenceRelations},
    \incite[exerc. 3.4.4]{Tao2022AnalysisII},
    \incite[13]{КановейЛюбецкий2010СовременнаяТеорияМножеств} and
    \incite[37]{ШеньВерещагин2020НачалаТеорииМножеств}.

    The similar notation \( {}^A B \) is used by
    \incite[52]{Enderton1977SetTheory} and
    \incite[def. 6.2.8(v)]{Hinman2005Logic}.

    \thmitem{rem:set_of_functions_notation/fun} \( \op{Fun}(A, B) \) is used by
    \incite[251]{Rotman2017AdvancedModernAlgebraPart2} and
    \incite[63]{Гуров2013ТеорияРешёток}.

    Other similar notations include, \( \mscrF(A, B) \) used by \incite[9]{FriedbergInselSpence2018LinearAlgebra}, and \( F(A, B) \), used by \incite[example 1.6.2]{Винберг2014КурсАлгебры}.
  \end{thmenum}
\end{remark}
...
\begin{remark}\label{rem:categorical_image_and_coimage}
  ...
  Most generally, based on categorical \hyperref[def:categorical_kernels]{kernels and cokernels}, \incite[def. IX.1.15]{Aluffi2009Algebra} defines the \term{image} of a morphism \( f: A \to B \) as \( \img f \coloneqq \ker \co\ker f \) and the \term{coimage} \( \co\img f \coloneqq \co\ker \ker f \).
