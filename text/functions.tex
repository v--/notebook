...
\begin{remark}\label{rem:function_definition}
  ...
  \begin{itemize}
    ...
    In terms of \hyperref[def:first_order_structure]{first-order structures}, not every function is a \hyperref[def:first_order_homomorphism]{first-order homomorphism}.
    ...
    For simplicity of exposition, we take multi-valued functions as primitive notions and define single-valued functions as special cases. Kelley does this in \cite[8]{Kelley1975} except that he conflates multi-valued functions and relations.
...
\begin{definition}\label{def:multi_valued_function}
  ...
  \begin{thmenum}[series=def:multi_valued_function]
    ...
    The notation \( f[X] \) is mostly used withing set theory, for example in \cite[44]{Enderton1977Sets}, however the following notation is usually used preferred outside set theory:
    \begin{equation*}
  ...
  \begin{thmenum}[resume=def:multi_valued_function]
    ...
    \thmitem{def:multi_valued_function/restriction} The \term{domain restriction} or simply \term{restriction} of \( F: A \multto B \) to \( X \subseteq A \) is the multi-valued function \( F\restr_{X}: X \rightarrow B \). We say that \( F \) is an \term{extension} of \( F\restr_{X} \).
    ...
    \thmitem{def:multi_valued_function/empty} A function \( F: A \multto B \) is \term{empty} if \( \gph(F) = \varnothing \). This is consistent with \fullref{def:relation/empty}.
  ...
  \begin{thmenum}[resume=def:multi_valued_function]
    ...
    \thmitem{def:multi_valued_function/superposition} Although the terms \enquote{composition} and \enquote{superposition} are used interchangeably, for example by Grigoriy Fichtenholz in \cite[\textnumero 25]{ФихтенгольцОсновыТом1}, the term \enquote{superposition} often refers to a certain generalization of \hyperref[def:multi_valued_function/composition]{function composition}. The latter is used by Sergey Yablonsky in \cite[16]{Яблонский1986}.
...
\begin{definition}\label{def:function}
  ...
  Strictly speaking, the value \( f(x) \) of a single-valued function \( f: A \to B \) is a singleton set. It is prevalent to define the value of a single-valued function to be an element of \( B \) rather than a subset of \( B \). Unless this would be confusing, we identify \( f(x) \) with its only element due to the convention established in \fullref{rem:singleton_sets}.
...
\end{theorem}
\begin{comments}
  \item Note that we do not yet use the notation \( n + 1 \) because we will use this theorem to define addition in the first place.
...
\begin{proof}
  ...
  The conditions imposed on the functions in \( G \) ensure that every function is defined in some \hyperref[def:order_interval/ray]{initial segment} of the natural numbers. A more obvious approach is to require \( g \) to be defined at \( \op{succ}(n) \) if it is defined at \( n \), however we are trying to prove that such a function exists in the first place.
