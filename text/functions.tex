...
\begin{definition}\label{def:set_valued_map}\mcite[def. 1.3.1]{AubinFrankowska1990SetValuedAnalysis}
  ...
  \begin{thmenum}[series=def:set_valued_map]
    ...
    The notation \( F(x) \) refers to a member of \( B \) if \( x \) is a \hyperref[def:first_order_syntax/formula_bound_variables]{bound variable} and to the map \( F \) itself if \( x \) is a \hyperref[def:fol_variable_freeness]{free variable}.
