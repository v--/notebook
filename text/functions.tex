...
\begin{remark}\label{rem:function_definition}
  ...
  What if we wanted to study properties of a mapping and relations between different mappings? In order to do this via set theory, we must represent a mapping as a set. Many authors like Herbert Enderton in \cite[40]{Enderton1977Sets}, John Kelley in \cite[7]{Kelley1975} and Ryszard Engelking in \cite[1]{Engelking1989} propose using a \hyperref[def:binary_relation]{binary relation} \( R \), called the \term{graph} of the mapping, such that the sets \( A \) and \( B \) are related under \( R \) if \( B \) is the unique value corresponding to \( A \).
  ...
  Then \( A \) is related under \( \mscrR \) with both \( \set{ A } \) and \( \pow(A) \). This is technically not a graph of a function because we lack uniqueness. It is a generalization, which Jean-Pierre Aubin and Helene Frankowska in \cite[def. 1.3.1]{AubinFrankowska1990}, Asen Dontchev and Ralph Tyrrell Rockafellar in \cite[1]{DontchevRockafellar2014} and Robert Phelps in \cite[VII]{Phelps1993} call \term{set-valued maps}\footnote{Every mapping is technically set-valued, but the values of single-valued functions are treated as atoms, while the values of set-valued maps are treated as sets of atoms}.
...
\begin{definition}\label{def:set_valued_map}\mcite[def. 1.3.1]{AubinFrankowska1990}
  ...
  \begin{thmenum}[series=def:set_valued_map]
    ...
    Paolo Aluffi in \cite[9]{Aluffi2009} also uses the terminology \enquote{\term{action} of \( F \) on \( x \)} or \enquote{\term{image} of \( F \) on \( x \)}.
...
\end{definition}
\begin{comments}
  \item Rather than introducing these concepts for set-valued functions, John Kelley in \cite[8]{Kelley1975} does it for relations.

  \item Some authors like Ryszard Engeling in \cite[1]{Engelking1989} and Anthony Knapp in \cite[596]{Knapp2016BasicAlgebra} use \enquote{range} for what we call the codomain of a function, while others like John Kelley in \cite[11]{Kelley1975} and Herbert Enderton in \cite[43]{Enderton1977Sets} use \enquote{range} for what we call the image.

  To avoid ambiguity, we avoid the term \enquote{range} altogether.
\end{comments}

\begin{remark}\label{rem:notation_for_function_image}
  The notation \( f[X] \) for the image of a set \( A \) is mostly used within set theory, for example by Herbert Enderton in \cite[44]{Enderton1977Sets}. The notation \( f(X) \) is used in other context like topology, for example by Ryszard Engelking in \cite[1]{Engelking1989}, and analysis, for example by Jean-Pierre Aubin and Helene Frankowska in \cite[def. 1.3.1]{AubinFrankowska1990}. We generally prefer \( f(X) \) because it reflects the established convention in fields more relevant to us, but occasionally use \( F[X] \) in order to avoid ambiguities. For example, if \( A \) is a \hyperref[def:transitive_set]{transitive set}, then \( x \in A \) implies that \( x \subseteq A \), yet
  \begin{equation*}
...
\begin{remark}\label{rem:function_arguments}
  ...
  We define the \term{arity} of \( F \) as its perceived number of arguments. This is not to be confused with relation arity as defined in \fullref{def:relation/arity} --- functions are always binary relations.
  ...
  For example, in \hyperref[def:first_order_semantics]{classical first-order semantics}, to each \( n \)-ary functional symbol there corresponds an \( n \)-ary \hyperref[def:operation_on_set]{operation} on \( X \), i.e. function with signature \( X^n \to X \).

  On the other hand, when working over a vector space like \( \BbbR^2 \), depending on the context we regard \hyperref[rem:functional]{functionals} as either unary or binary functions.
\end{remark}
...
\begin{definition}\label{def:function}
  ...
  \begin{thmenum}
    \thmitem{def:function/endofunction}\mimprovised We call functions from a set to itself \term{endofunctions}.
...
\begin{comments}
  ...
  \begin{itemize}
    ...
    \item \( B^A \), which is consistent with \hyperref[def:cardinal_arithmetic/exponentiation]{cardinal exponentiation} (used by Paolo Aluffi in \cite[example 3.2]{Aluffi2009} and George Bergman in \cite[13]{Bergman2015}).
    \item \( {}^A B \) (used by Herbert Enderton in \cite[52]{Enderton1977Sets}).
  \end{itemize}
...
\begin{remark}\label{rem:set_valued_map_as_single_valued}
  Rather than defining a \hyperref[def:function]{single-valued function} to be a special case of \hyperref[def:set_valued_map]{set-valued maps}, Robert Phelps in \cite[def. 2.3]{Phelps1993} defines set-valued maps from \( A \) to \( B \) as single-valued functions from \( A \) to \( \pow(B) \).
...
\begin{remark}\label{rem:implicit_function_notation}
  ...
  \begin{itemize}
    \item Quotient maps such as \( H \mapsto H / N \) from \fullref{thm:quotient_subgroup_lattice_theorem}.
...
\begin{remark}\label{rem:function_superposition}
  Although the terms \enquote{composition} and \enquote{superposition} are used interchangeably, for example by Grigoriy Fichtenholz in \cite[\textnumero 25]{ФихтенгольцОсновыТом1}, the term \enquote{superposition} often refers to a certain generalization of \hyperref[def:set_valued_map/composition]{function composition}. The latter is used by Sergey Yablonsky in \cite[16]{Яблонский1986}.
...
\begin{proof}
  ...
  The conditions imposed on the functions in \( G \) ensure that every function is defined in some \hyperref[def:order_interval/ray]{initial segment} of the natural numbers. A more obvious approach is to require \( g \) to be defined at \( \op{succ}(n) \) if it is defined at \( n \), however that would hinder our induction schema.
