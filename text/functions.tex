...
\begin{definition}\label{def:set_valued_map}\mcite[def. 1.3.1]{AubinFrankowska1990SetValuedAnalysis}
  ...
  \begin{thmenum}[resume=def:set_valued_map]
    ...
    \thmitem{def:set_valued_map/identity}\mcite[2]{Engelking1989GeneralTopology} We define the \term{identity function} \( \id_A \) on the set \( A \) as the function corresponding to the identity relation \( \increment_A \) from \fullref{def:binary_relation/diagonal}.

    \thmitem{def:set_valued_map/inverse} We define the \term{inverse} \( F^{-1}: B \multto A \) of a set-valued map \( F: A \multto B \) as the function whose graph is the inverse of \( \gph(F) \) in the sense of \fullref{def:binary_relation/converse}.
...
\begin{concept}\label{con:function_arguments}
  ...
  If the number of arguments is well-defined, we call it the \term{arity} of the function and use prefixes from \fullref{def:operation_arity_terminology} like \enquote{unary}, \enquote{binary} and so forth.
\end{concept}
...
\begin{comments}
  ...
  As mentioned in \fullref{def:set_valued_map/image}, we usually prefer the notation \( f[A] \) outside \fullref{ch:set_theory} where we are less prone to ambiguity.
...
\begin{concept}\label{con:undefinedness}\mimprovised
  ...
  Having such a notion available in the \hyperref[con:metalogic]{metalanguage} can simplify case-wise definitions. For example, consider the characterization of the \hyperref[def:polynomial_degree]{degree} of a univariate polynomials over the real numbers from \fullref{rem:zero_polynomial_degree/minus_one}:
  \begin{equation*}
    \deg(f) = \begin{cases}
...
\begin{remark}\label{rem:undefined_and_primitive_terms}
  \incite[v]{WhiteheadRussell1927PrincipiaMathematicaVol1} and \incite[28]{Kleene1971Metamathematics} use the adjective \enquote{undefined} for the symbols that are considered primitive rather than defined via others. As discussed in \fullref{con:primitive_notion}, we call such symbols \enquote{primitive} to prevent confusion with the notion of undefinedness which we present in \fullref{con:undefinedness}.
...
\end{theorem}
\begin{comments}
  \item \Fullref{thm:cartesian_product_via_choice_functions} shows that the Cartesian product can also be expressed via \hyperref[def:choice_function]{choice functions}.
\end{comments}
\begin{proof}
  ...
  \Fullref{thm:indexed_family_extensionality} implies that there is a unique indexed family \( \seq{ a_k }_{k \in \mscrK} \) such that \( a_k = \beta_k(b) \) for every \( k \) in \( \mscrK \). This motivates the definition
  \begin{equation*}
    \begin{aligned}
      &l_B: B \to A \\
...
\begin{comments}
  ...
  See \fullref{rem:family_to_indexed_family} for a general discussion of indexed and non-indexed families.
\end{comments}
...
\end{proposition}
\begin{comments}
  \item The only formal distinction between \( \bigsqcap_{k \in \mscrK} A_k \) as defined here and \( \prod_{k \in \mscrK} A_k \) as defined in \fullref{def:cartesian_product} is that choice functions have attached information about their domain and codomain. Thus, taking the \hyperref[def:set_valued_map/graph]{graph} of the choice functions instead, we obtain \( \prod_{k \in \mscrK} A_k \).

  Following the practice from \fullref{rem:categorical_limit_uniqueness} of not distinguishing between different instances of the same categorical limit (as long as it is used consistently), we will regard elements of Cartesian products as choice functions in the cases where we will find that convenient.
...
\begin{definition}\label{def:set_of_all_functions}\mimprovised
  ...
  Generally, in analogy with \hyperref[def:category/morphisms]{categorical morphism sets}, we will denote the set of functions by \( \fun(A, B) \). This is convenient for more complicated sets like \( \fun(\BbbF_q^n. \BbbF_q) \) in \fullref{thm:functions_over_finite_fields}.
...
\end{definition}
\begin{comments}
  \item Usage of notation in the literature is discussed in \fullref{rem:set_of_all_functions_notation}.

  \item As shown in \fullref{thm:set_of_all_functions_via_cartesian_product}, \( B^A \) can be viewed as a degenerate case of the \hyperref[def:cartesian_product]{Cartesian product}.
\end{comments}
...
\end{proposition}
\begin{comments}
  \item A similar result holds for function types and dependent product types; see \fullref{con:dependent_type}.
\end{comments}
\begin{proof}
...
\begin{remark}\label{rem:disjoint_unions}
  ...
  We do not usually care about the particular construction. In accordance with \fullref{rem:definition_via_characterization}, we will provide a \enquote{canonical} disjoint union definition in \fullref{def:disjoint_union}, and, after proving \fullref{thm:disjoint_union_universal_property}, only rely on the canonical inclusion maps.
\end{remark}
...
\end{definition}
\begin{comments}
  \item As discussed in \fullref{rem:disjoint_unions}, outside this definition and \fullref{thm:disjoint_union_universal_property}, we prefer working with the inclusion \( \iota_k(x) \) rather than with the concrete representation \( (a, k) \).
\end{comments}
...
\begin{proposition}\label{thm:disjoint_union_of_disjoint_sets}
  The union, in the sense of \fullref{def:basic_set_operations/union}, of a family of \hyperref[def:subset_ordering]{\hi{disjoint}} sets, along with the subset inclusion maps, satisfies \fullref{thm:disjoint_union_universal_property} --- it is a \hyperref[def:discrete_category_limits]{categorical coproduct} in \( \hyperref[def:category_of_small_sets]{\ucat{Set}} \) for any appropriate \hyperref[def:grothendieck_universe]{Grothendieck universe} \( \mscrU \) .
\end{proposition}
\begin{comments}
