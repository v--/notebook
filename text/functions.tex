...
\begin{definition}\label{rem:set_of_all_functions}\mimprovised
  ...
  Occasionally, like when dealing with \hyperref[def:coordinate_space]{coordinate spaces} like \( R^n \) or state assignment in \hyperref[def:cellular_automata]{cellular automata} like \( E^{D_0} \), we will find it more convenient to use the succinct notation \( B^A \). This mostly coincides with the cases where we treat functions like \hyperref[def:indexed_family]{indexed families}.
...
\end{proposition}
\begin{comments}
  \item A similar result holds for function types and dependent product types; see \fullref{con:dependent_types}.
\end{comments}
\begin{proof}
