...
\begin{concept}\label{con:variable_dependence}
  ...
  Following this discussion, another kind of variables we should mention here are \hyperref[def:random_variable]{random variables}, whose value is nondeterministic. They are neither dependent nor independent; in fact, they have their own notion of dependence that we define formally in \fullref{def:random_variable_independence}.
\end{concept}
...
\begin{comments}
  ...
  \begin{itemize}
    ...
    \item \( {}^A B \) (used by \incite[52]{Enderton1977SetTheory}).
  \end{itemize}
...
\begin{proof}
  ...
  The cone \( (B, \beta) \) was chosen arbitrarily, and we showed that there exists a unique morphism \( l_B: B \to A \) such that \eqref{eq:thm:discrete_category_limits_in_set/limit} commutes. Therefore, \( (A, \pi) \) is a categorical product of the family \( \mscrA \).
\end{proof}
