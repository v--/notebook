...
\begin{comments}
  ...
  \item \Textcite[\S 2.1.11]{Barendregt1984LambdaCalculus} and \textcite[def. 1A8]{Hindley1997BasicSTT} call the successive renaming of bound variables \enquote{\( \alpha \)-conversion} and consider \( \muplambda \)-terms \enquote{up to \( \alpha \)-conversion} (see \cref{rem:barendregt_convention}). Barendregt calls \enquote{\( \alpha \)-congruence} what we call \( \alpha \)-equivalence, while Hindley says that \enquote{\( M \) \( \alpha \)-converts to \( N \)}.

  \textcite[5]{Pollack2005AlphaConversion} use \enquote{\( \alpha \)-conversion} and \enquote{\( \alpha \)-equivalence} interchangeably, while \textcite[114]{Mimram2020ProgramEqualsProof} uses \enquote{\( \alpha \)-conversion} for what we call \( \alpha \)-equivalence.
...
\begin{remark}\label{rem:barendregt_convention}\mcite[\S 2.1.12]{Barendregt1984LambdaCalculus}
  \Textcite[\S 2.1.12]{Barendregt1984LambdaCalculus} assumes the following convention:
  \begin{displayquote}
...
\end{lemma}
\begin{comments}
  \item If a \( \muplambda \)-term has no variables, this becomes a \hyperref[def:labeled_tree/morphism]{labeled tree isomorphism}.
\end{comments}
\begin{proof}
...
\begin{lemma}\label{thm:substitution_composition_free_variables}
  ...
  \begin{equation}\label{eq:thm:substitution_composition_free_variables}
...
\begin{lemma}\label{thm:composition_of_lambda_substitutions_lemma}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:composition_of_lambda_substitutions_lemma/midpoints} For \( 1 \leq k \leq n - 1 \), we suppose that \( v_k \) is \emph{not} in \( \op*{Free}_{\Bbbs_k \cdots \Bbbs_1}(M) \).
  \end{thmenum}

  We claim that the \hyperref[def:lambda_substitution_composition]{composite substitution} \( (\Bbbs_k)_{v_{n-1} \mapsto v_n} \cdots (\Bbbs_1)_{v_0 \mapsto v_1} \) agrees with the modified composite \( (\Bbbs_n \cdots \Bbbs_1)_{v_0 \mapsto v_n} \) on the free variables of \( M \).
\end{lemma}
\begin{comments}
...
\end{proposition}
\begin{comments}
  \item This check is implemented in \cite{notebook:code} as
  \begin{CodeRefDisplay}
    \GetCodeRef{thm:alpha_equivalence_simplified}
  \end{CodeRefDisplay}
\end{comments}
\begin{proof}
...
\end{corollary}
\begin{comments}
  \item \Textcite[\S 2.1.11]{Barendregt1984LambdaCalculus} calls this property \enquote{\( \alpha \)-conversion} and uses it as a definition, while \textcite[def. 1A8]{Hindley1997BasicSTT} calls it \enquote{change of bound variables} and calls \enquote{\( \alpha \)-conversion} the transitive closure of the relation obtained.
\end{comments}
\begin{proof}
...
\begin{comments}
  ...
  \begin{CodeRefDisplay}
    \GetCodeRef{alg:separation_of_free_and_bound_variables}
  \end{CodeRefDisplay}
\end{comments}
\begin{defproof}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      \item If \( x \notin \Gamma \), then \( M' = \qabs x N'_{\Gamma,x} \).
