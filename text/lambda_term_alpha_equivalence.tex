...
\begin{definition}\label{def:lambda_renaming}
  We say that the \hyperref[def:lambda_substitution]{substitution} \( \Bbbs \) is a \term{renaming substitution} if \( \Bbbs(x) \) is a variable for every \( x \).
\end{definition}
\begin{comments}
...
\begin{proposition}\label{thm:substitution_on_alpha_equivalent_terms}
  If \( M \aequiv N \), for an \hyperref[def:lambda_substitution]{\hi{arbitrary substitution}} \( \Bbbs \), we have \( M[\Bbbs] \aequiv N[\Bbbs] \).
\end{proposition}
\begin{comments}
