...
\begin{comments}
  ...
  \item The rules themselves can be formalized using \( \synlambda \)-term schemas and instantiations akin to how we have formalized the rules in \fullref{sec:natural_deduction}. We will introduce schemas for \( \synlambda \)-terms in \fullref{sec:simple_type_theory}, however they will be more limited than what we need here and in \fullref{sec:lambda_term_reductions}.
\end{comments}
...
\end{proposition}
\begin{comments}
  \item This check can be found as \identifier{lambda_.alpha.are_terms_alpha_equivalent} in \cite{notebook:code}.
\end{comments}
\begin{proof}
...
\end{algorithm}
\begin{comments}
  \item This algorithm can be found as \identifier{lambda_.alpha.separate_free_and_bound_variables} in \cite{notebook:code}.
\end{comments}
\begin{defproof}
