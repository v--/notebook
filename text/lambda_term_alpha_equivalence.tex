...
\begin{definition}\label{def:lambda_renaming}
  We say that an \hyperref[def:atomic_lambda_term_substitution]{atomic substitution} is a \term{renaming substitution} if it sends variables to variables.
\end{definition}
\begin{comments}
...
\end{definition}
\begin{comments}
  \item If the variable names in \( M \) do not have numeric indices, then \( \len(M) \) is exactly the \hyperref[def:formal_language/string_length]{string length} of \( M \). The string length is the definition of term length given by \incite[\S 2.1.3]{Barendregt1984LambdaCalculus}.
...
\begin{proposition}\label{thm:def:lambda_term_length}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:def:lambda_term_length/substitution} For any atomic substitution \( \Bbbs \) and \( \muplambda \)-term \( M \), we have
...
\begin{proof}
  ...
  \SubProofOf{thm:def:lambda_term_length/substitution} We proceed via \fullref{thm:induction_on_abstract_syntax} on of \( M \), simultaneously on all substitutions.
  \begin{itemize}
    ...
    \item If \( M \) is a variable, say \( M = x \), then
    \begin{equation*}
      \len(M[\Bbbs]) = \len(\Bbbs(x)) = \len(x) + (\len(\Bbbs(x)) - \underbrace{\len(x)}_1).
    \end{equation*}
...
\begin{proof}
  ...
  If \( M \) has length \( 1 \), it is necessarily an atom, and the only rule that allow us to conclude that \( M \aequiv N \) due to \ref{inf:def:lambda_term_alpha_equivalence/atom}. It is satisfied only if \( M = N \). Thus, \( \op*{Free}(M) = \op*{Free}(N) \).
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      \item Suppose first that \( b \) is free in \( B \). Then \( a \) is free in \( B[b \mapsto a] \) and, by the inductive hypothesis, it is also free in \( A \).
      ...
      \begin{balign*}
        ...
        = \\ &=
        \parens[\big]{ \parens[\big]{ \op*{Free}(B) \setminus \set{ b } } \cup \set{ a } } \setminus \set{ a }
        = \\ &=
        \op*{Free}(B) \setminus \set{ a, b }
        = \\ &=
        \op*{Free}(\qabs b B) \setminus \set{ a }
        = \\ &=
        \op*{Free}(N) \setminus \set{ a }.
      \end{balign*}

      It remains to show that \( a \) is free in \( N \).

      Since \( A[a \mapsto b] \aequiv B \), we have \( \op*{Free}(B) = \op*{Free}(A[a \mapsto b]) \) by the inductive hypothesis. Furthermore, we have already concluded that \( a \) is free in \( A \), so \cref{thm:lambda_substitution_free_variables_single} implies that
      \begin{equation*}
        \op*{Free}(B) =\parens[\big]{ \op*{Free}(A) \setminus \set{ a } } \cup \set{ b }.
      \end{equation*}

      Thus, \( a \) is not free in \( B \), and hence also not in \( N \). It follows that \( \op*{Free}(M) = \op*{Free}(N) \).

      \item Otherwise, \( b \) is not free in \( B \). Aiming at a contradiction, suppose that \( a \) is free in \( A \).

      We have \( A[a \mapsto B] \aequiv B \), thus
      \begin{equation*}
        \op*{Free}(B)
        \reloset{\T{ind.}} =
      ...
      But we have assumed that \( b \) is not free in \( B \). The obtained contradiction shows that \( a \) is also not free in \( A \). Then \cref{thm:lambda_substitution_noop} implies that \( A[a \mapsto B] = A \), hence \( A \aequiv B \).

      By the inductive hypothesis, \( A \) and \( B \) have the same free variables. Furthermore, \( a \) in not free in \( A \) and \( b \) is not free in \( B \), so
      \begin{equation*}
        \op*{Free}(M)
        =
        \op*{Free}(\qabs a A)
  ...
  \begin{equation*}
    ...
    =
    \op*{Free}(\qabs a A)
    \reloset {\ref{thm:def:lambda_term_alpha_equivalence/abstraction_body_free}} =
    \op*{Free}(\qabs b B)
    =
  ...
  \SubProofOf*[def:binary_relation/reflexive]{reflexivity} We proceed via \fullref{thm:induction_on_abstract_syntax} on the \( \muplambda \)-term \( M \) to show that \( M \aequiv N \) whenever \( M = N \).
  \begin{itemize}
...
\begin{definition}\label{def:lambda_substitution_composition}\mimprovised
  We define the \term{composition} \( \Bbbt\Bbbs \) of the \hyperref[def:lambda_term]{\( \muplambda \)-term} \hyperref[def:atomic_lambda_term_substitution]{substitutions} \( \Bbbs \) and \( \Bbbt \) as
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    For the right side, we can use \cref{thm:def:lambda_term_alpha_equivalence/abstraction_body_free} to conclude that \( a \) is not free in \( B \), and \cref{thm:substitution_chain_contraction/precomposition} to conclude that
    \begin{equation*}
