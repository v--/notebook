...
\begin{proposition}\label{thm:def:lambda_term_length}
  ...
  \begin{thmenum}
    ...
    In particular, if \( \Bbbs \) is a \hyperref[def:lambda_renaming_substitution]{renaming}, then \( \len(M[\Bbbs]) = \len(M) \).
...
\end{proposition}
\begin{comments}
  \item See the corresponding statements for first-order terms in \cref{thm:def:fol_term_length} and for first-order formulas in \cref{thm:def:fol_formula_length}.
\end{comments}
\begin{proof}
  ...
  \begin{itemize}
    ...
    \item If \( M \) is a variable, say \( x \), then
    \begin{equation*}
...
\begin{proposition}\label{thm:def:lambda_term_alpha_equivalence}
  ...
  \begin{thmenum}
    \thmitem{thm:def:lambda_term_alpha_equivalence/same_kind} If two \( \muplambda \)-terms are \( \alpha \)-equivalent, they have the same kind: both are either variables, applications or abstractions.
