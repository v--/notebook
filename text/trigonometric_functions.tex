...
\begin{definition}\label{def:trigonometric_functions}
  ...
  \Fullref{thm:right_triangle_trigonometric_functions} justifies the term \enquote{angle} for the \hyperref[con:function_arguments]{parameter} of the trigonometric functions.
\end{definition}
...
\begin{proof}
  ...
  \SubProofOf{thm:trigonometric_identities/angle_doubling} Follows from \fullref{thm:trigonometric_identities/sum_of_angles}.
\end{proof}
...
\end{proposition}
\begin{comments}
  \item See also \fullref{thm:trigonometric_function_secondary_roots}.
\end{comments}
\begin{proof}
...
\end{definition}
\begin{comments}
  \item The least period of a periodic sequence always exists, as shown in \fullref{thm:def:ultimately_periodic_sequence/least}.
  \item We have synthesized this definition from the following special cases:
  \begin{itemize}
    \item For natural numbers (sequences) by \incite[def. 8.3]{LidlNiederreiter1997FiniteFields}.
    \item For real numbers by \incite[def. 5.1.1]{Tao2022AnalysisII}, \incite[371]{Фихтенгольц1968ОсновыАнализаТом2} and \incite[318]{ИлинСадовничиСендов1989АнализТом2}.
    \item For complex numbers by \incite[45]{Ahlfors1979ComplexAnalysis} and \incite[102]{Ahlfors1979ComplexAnalysis}.
  \end{itemize}
\end{comments}
...
\begin{proposition}\label{thm:trigonometric_function_secondary_roots}
  ...
  \Fullref{thm:trigonometric_function_period_identities} allow us to also derive
  \begin{align}
...
\begin{proof}
  ...
  \Fullref{thm:trigonometric_function_period} implies that both \( \cos(\alpha) \) and \( \sin(\alpha) \) are nonzero, thus
