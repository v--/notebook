...
\begin{remark}\label{rem:set_labeling_terminology}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      \item \Textcite[3]{Bollobás1998ModernGraphTheory}, \textcite{Salomaa1973FormalLanguages}, \textcite{Mimram2020ProgramEqualsProof}, \textcite{CitkinMuravitsky2022ConsequenceRelations} and \textcite{Erickson2019Algorithms} without explicit definitions for labels of graph vertices and edges.

      \item \Textcite[20]{Harary1969GraphTheory} for distinct strings, \enquote{names}, assigned to the vertices of a graph.

      \item \Textcite{Rosen2019DiscreteMathematics}, \textcite{Stanley2012EnumerativeCombinatoricsVol1} and \textcite[315]{Knuth1997ArtVol1} as an informal notion for attaching information.

      \item \Textcite[242]{Diestel2017GraphTheory} for ordinal number labels of graph vertices.
    \end{itemize}

    \item \enquote{Marking} is used by
    \begin{itemize}
      \item \Textcite[4]{Harary1969GraphTheory} for general labels of graph vertices.

      \item \Textcite[def. 1.3.9]{Knauer2019AlgebraicGraphTheory} for general labels of tree nodes.

      \item \Textcite{Rosen2019DiscreteMathematics} for general labels.

      \item \Textcite[def. 5.13]{БелоусовТкачёв2004ДискретнаяМатематика} (as \enquote{\textru{функция разметки}}) for a function labeling edges with nonzero elements of an idempotent semiring.
    \end{itemize}

    \item \enquote{Valuation} is used by
    \begin{itemize}
      \item \Textcite[def. 6.8.1]{Knauer2019AlgebraicGraphTheory} for labeling with natural numbers or positive infinity.
    \end{itemize}

    \item \enquote{Coloring} is used by
    \begin{itemize}
      \item \Textcite[7]{Birkhoff1967LatticeTheory}, \textcite[145]{Bollobás1998ModernGraphTheory}, \textcite[139]{Knauer2019AlgebraicGraphTheory}, \textcite[500]{Jacobson1985BasicAlgebraI}, \textcite{Stanley2012EnumerativeCombinatoricsVol1}, \textcite{Rosen2019DiscreteMathematics}, \textcite{БарановСтечкин2004ЭкстремальнаяКомбинаторика} (as \enquote{\textru{раскраска}}) and \textcite[141]{Мирчев2001Графи} (as \enquote{\textbg{оцветяване}}) without an explicit definition as way of classifying objects.

      \item \Textcite[395]{Erickson2019Algorithms}, \textcite[\S 5.4]{Harzheim2005OrderedSets}, \textcite[124]{Aigner1997CombinatorialTheory}, \textcite[28]{Зыков2004ТеорияГрафов} (as \enquote{\textru{раскраска}}), \textcite[235]{ЕмеличевИПр1990ТеорияГрафов} (as \enquote{\textru{раскраска}}) and \textcite[\S 10.7.1]{Новиков2013ДискретнаяМатематика} (as \enquote{\textru{раскраска}}) as an explicit function labeling graph vertices with positive integers.

      \item \Textcite[595]{ButlerEtAl2016RainbowArithmeticProgressions} as an explicit function labeling the elements of initial segments of an \hyperref[def:arithmetic_progression]{arithmetic progression}.

      \item \Textcite[21]{Rotman2017AdvancedModernAlgebraPart2} as an \hyperref[def:group_action_orbit]{orbit} of a \hyperref[def:group_action]{group action} on a finite set.

      \item \Textcite[174]{Cayley1878Graphs} and \textcite[156]{König1990GraphTheory} for the labels of \hyperref[def:cayley_graph]{Cayley graphs}.
    \end{itemize}

    \item \enquote{Weight} is used by
    \begin{itemize}
      \item \Textcite[def. 5.13]{БелоусовТкачёв2004ДискретнаяМатематика} (as \enquote{\textru{весовая функция}}) for a function labeling edges with nonzero elements of an idempotent semiring.

      \item \Textcite[exerc. 8.4]{Новиков2013ДискретнаяМатематика} (as \enquote{\textru{нагрузка}}) and \textcite[59]{ЕмеличевИПр1990ТеорияГрафов} (as \enquote{\textru{вес}}) for labeling graph vertices with nonnegative real values.

      \item \Textcite[500]{Stanley2012EnumerativeCombinatoricsVol1} for a function taking values in a commutative ring.


      \item \Textcite[def. 1.1.2]{Knauer2019AlgebraicGraphTheory} and \textcite[62]{Зыков2004ТеорияГрафов} for general labels of either vertices or edges of graphs.

      \item \Textcite{Rosen2019DiscreteMathematics} without an explicit definition for numeric labels.

      \item \Textcite{Bollobás1998ModernGraphTheory} and \textcite{Erickson2019Algorithms}, also without an explicit definition, for numeric labels for vertices.

      \item \Textcite[9]{GondranMinoux1984GraphsAndAlgorithms} and \textcite[12]{Мирчев2001Графи} for number-valued graph edge labels (without specifying which kind of numbers) of edges of graphs, although Mirchev also allows vertices to be labeled.

      \item \Textcite[298]{Diestel2017GraphTheory} (in \enquote{weighted with a probability}) for numbers form the unit interval.

      \item \Textcite[387]{Knuth1997ArtVol1} and \textcite[35]{Harary1969GraphTheory} for the number of nodes in a subtree, but later in \cite[417]{Knuth1997ArtVol1} as number-valued labels of nodes.

      \item \Textcite[180]{Harary1969GraphTheory} for explicit labeling with real numbers, with the intention to label \hyperref[def:group_action_orbit]{group action orbits}.

      \item \Textcite[37]{Blizard1988MultisetTheory} for specifying that \enquote{weighted set} is an obsolete synonym for \enquote{multiset}.
    \end{itemize}

    \item \enquote{Multiset} is used by
    \begin{itemize}
      \item \Textcite[473]{Knuth1997ArtVol2}, \textcite[143]{Rosen2019DiscreteMathematics}, \textcite[2]{Aluffi2009Algebra} and \textcite[28]{Новиков2013ДискретнаяМатематика} as an informal notion of \enquote{generalized sets} allowing each element to be repeated finitely many times.

      \item \Textcite[6]{TroelstraSchwichtenberg2000BasicProofTheory} as similar generalized sets with an additional restriction of finiteness.

      \item \Textcite[37]{Blizard1988MultisetTheory}, \textcite[18]{Stanley2012EnumerativeCombinatoricsVol1}, \textcite[309]{Barendregt1984LambdaCalculus}, \textcite[20]{Golan1999Semirings} and \textcite[\S A.3.5]{Mimram2020ProgramEqualsProof} for explicit labeling with positive integers.

      \item \Textcite[def. 1]{Hickman1980Multisets} for explicit labeling with nonzero cardinals.
    \end{itemize}
  \end{itemize}
\end{remark}
...
\end{definition}
\begin{comments}
  \item \Textcite{Blizard1988MultisetTheory} describes a \hyperref[def:fol_theory]{first-order theory} of multisets based on generalizing \hyperref[def:zfc]{\logic{ZFC}}. For our occasional use of multisets, our simple definition is sufficient. We do, however, unlike Blizard allow membership of transfinite multiplicity.

  \item Based on the exact same abstraction, \textcite{Hickman1980Multisets} extends basic set-theoretic relations and operations to multisets. We will not find use for these, however, so we avoid introducing additional formalisms.

  \item We discuss in \cref{rem:set_labeling_terminology} how related terminology is used in the literature.
\end{comments}

\begin{remark}\label{rem:syntax_of_multisets}
  Following \textcite[309]{Barendregt1984LambdaCalculus}, we can reuse the set list notation from \cref{def:set_notation} for \hyperref[def:multiset]{multisets} by interpreting the set-theoretic expression
  \begin{equation*}
    \set{ a, b, c, a, b, a }
  \end{equation*}
...
\end{definition}
\begin{comments}
  \item This definition is based on the following. \Textcite[def. 1]{Hickman1980Multisets} avoids providing a concept of cardinality, but for our usage we will find it necessary. We are mostly concerned with the cardinality of multisets with finitely many elements with finite multiplicities, which, following the introductory discussion in \cite[1]{Blizard1988MultisetTheory}, should simply be the sum of all multiplicities.
