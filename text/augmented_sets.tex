...
\begin{definition}\label{def:fuzzy_set}\mcite[339]{Zadeh1965FuzzySets}
  A \term[ru=нечёткое множество (\cite[27]{БогомоловСалий1997ОбщаяАлгебра})]{fuzzy set} in a universum \( U \) is simply a \hyperref[def:labeled_set]{labeling} of \( U \) with numbers from the unit interval designating degrees of membership.
...
\begin{definition}\label{def:fuzzy_subset}\mcite[339]{Zadeh1965FuzzySets}
  Fix two \hyperref[def:fuzzy_set]{fuzzy sets} \( A \) and \( B \) over the same universum \( U \). We say that \( A \) is a \term{fuzzy subset} of \( B \) if \( f_A(x) \leq f_B(x) \) for every \( x \in U \).
\end{definition}
...
\begin{definition}\label{def:basic_fuzzy_set_operations}\mcite[339]{Zadeh1965FuzzySets}
  Fix two \hyperref[def:fuzzy_set]{fuzzy sets} \( A \) and \( B \) over the same universum \( U \).
