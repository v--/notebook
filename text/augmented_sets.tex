...
\begin{definition}\label{def:fuzzy_set}\mcite[339]{Zadeh1965FuzzySets}
  ...
  A \( \mscrU \)-\term{fuzzy set} is simply a \hyperref[def:labeled_set]{labeling} of \( \mscrU \) with numbers from the unit interval, which we call their \term{degree of membership}.

  We denote a fuzzy set by \( A \) and its membership function by \( f_A \).
\end{definition}
\begin{comments}
  \item We follow the exposition of Zadeh closely, however we use Grothendieck universes as our universe of discourse rather than his informal \enquote{space of points}.
\end{comments}
...
\begin{definition}\label{def:basic_fuzzy_set_operations}\mcite[339]{Zadeh1965FuzzySets}
  ...
  \begin{thmenum}
    \thmitem{def:basic_fuzzy_set_operations/union} The \term{union} of \( A \) and \( B \) is given by the membership function
    \begin{equation}\label{eq:def:basic_fuzzy_set_operations/union}
      f_{A \cup B} (x) \coloneqq \max\set{ f_A(x), f_B(x) }.
    \end{equation}

    \thmitem{def:basic_fuzzy_set_operations/intersection} The \term{intersection} of \( A \) and \( B \) is given by the membership function
...
\begin{definition}\label{def:fuzzy_level_set}\mcite[347]{Zadeh1965FuzzySets}
  For a \hyperref[def:fuzzy_set]{fuzzy set} \( A \) and a number \( \alpha \in [0, 1] \), we introduce the (plain) set
  \begin{equation*}
    \Gamma_\alpha \coloneqq \set{ x \in A \given f_A(x) \geq \alpha }
  \end{equation*}
  of elements with a sufficiently large membership degree in \( A \).
\end{definition}
...
\begin{definition}\label{def:fuzzy_relation}\mcite[345]{Zadeh1965FuzzySets}
  ...
  Explicitly, a fuzzy relation on the plain sets \( S_1, \cdots, S_n \) is a function with signature
  \begin{equation*}
...
\begin{example}\label{ex:def:fuzzy_relation}
  ...
  \begin{thmenum}
    ...
    \begin{equation*}
      \frac {\floor(\log_{10}(n))} {\floor(\log_{10}(m))}.
    \end{equation*}

    This is more simply expressed without rounding down, in which case we can simplify to \( \log_{10}(n - m) \).

    In addition to the relation \( m < n \), indicating whether \( n \) is greater than \( m \), we can thus also define the fuzzy relation \( m \ll m \), indicating that \( n \) is \enquote{much greater} than \( m \):
    \begin{equation*}
