...
\begin{remark}\label{rem:set_labeling_terminology}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \item \incite[def. 5.13]{БелоусовТкачёв2004ДискретнаяМатематика} (as \enquote{\textru{функция разметки}}) for a function labeling edges with nonzero elements of an idempotent semiring.
    \end{itemize}
    ...
    \begin{itemize}
      \item \incite[def. 5.13]{БелоусовТкачёв2004ДискретнаяМатематика} (as \enquote{\textru{весовая функция}}) for a function labeling edges with nonzero elements of an idempotent semiring.

      \item \incite[exerc. 8.4]{Новиков2013ДискретнаяМатематика} (as \enquote{\textru{нагрузка}}) and \incite[59]{ЕмеличевИПр1990ТеорияГрафов} (as \enquote{\textru{вес}}) for labeling graph vertices with nonnegative real values.
...
\end{definition}
\begin{comments}
  \item \incite{Blizard1988MultisetTheory} describes a \hyperref[def:fol_theory]{first-order theory} of multisets based on generalizing \hyperref[def:zfc]{\logic{ZFC}}. For our occasional use of multisets, our simple definition is sufficient. We do, however, unlike Blizard allow membership of transfinite multiplicity.
