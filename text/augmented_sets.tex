...
\begin{remark}\label{rem:set_labeling_terminology}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \item \incite[28]{Зыков2004ТеорияГрафов} (as \enquote{раскраска}), \incite[395]{Erickson2019Algorithms}, \incite[\S 5.4]{Harzheim2005OrderedSets}, \incite[235]{ЕмеличевИПр1990ТеорияГрафов} (as \enquote{раскраска}) and \incite[\S 10.7.1]{Новиков2013ДискретнаяМатематика} (as \enquote{раскраска}) as an explicit function labeling graph vertices with positive integers.
      ...
      \item \incite[21]{Rotman2015AdvancedModernAlgebraPart1} as an \hyperref[def:group_action_orbit]{orbit} of a \hyperref[def:group_action]{group action} on a finite set.
    \end{itemize}
    ...
    \begin{itemize}
      ...
      \item \incite[37]{Blizard1988MultisetTheory}, \incite[18]{Stanley2012EnumerativeCombinatoricsVol1}, \incite[309]{Barendregt1984LambdaCalculus}, \incite[20]{Golan1999Semirings} and \incite[A.3.5]{Mimram2020ProgramEqualsProof} for explicit labeling with positive integers.
...
\begin{definition}\label{def:set_coloring}\mcite[127]{RosenEtAl2018DiscreteMathematicsHandbook}.
  ...
  \begin{thmenum}
    \thmitem{def:set_coloring/monochromatic} We say that a subset of \( S \) is \term[ru=одноцветный (класс) (\cite[\S 10.7.1]{Новиков2013ДискретнаяМатематика}), en=monochromatic (\cite[201]{Bollobás1998ModernGraphTheory})]{monochromatic} if all its elements have the same color.
...
\end{definition}
\begin{comments}
  \item We must provide a justification for \fullref{def:multiset/cardinality}. \incite[def. 1]{Hickman1980Multisets} avoids providing a concept of cardinality, but for our usage we will find it necessary. We are mostly concerned with the cardinality of multisets with finitely many elements with finite multiplicities, which, following the introductory discussion in \incite[1]{Blizard1988MultisetTheory}, should simply be the sum of all multiplicities.
