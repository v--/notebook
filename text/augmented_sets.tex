...
\begin{remark}\label{rem:set_labeling_terminology}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \item \incite[315]{Knuth1997ArtVol1} as an informal notion for attaching information.
    ...
    \begin{itemize}
      \item \incite[7]{Birkhoff1967Lattices}, \incite[145]{Bollobas1998Graphs}, \incite[139]{Knauer2019AlgGraphTheory}, \incite[500]{Jacobson1985AlgebraPart1} and \incite[141]{Мирчев2001Графи} (as \enquote{оцветяване}) without an explicit definition as way of classifying objects.
      ...
      \item \incite[1]{ButlerEtAl2016Progressions} as an explicit function labeling the elements of an \hyperref[def:arithmetic_progression]{finite arithmetic progression}.
    ...
    \begin{itemize}
      ...
      \item \incite[exer. 8.4]{Новиков2013ДискретнаяМатематика} (as \enquote{нагрузка}) and \incite[59]{ЕмеличевИПр1990Графы} (as \enquote{вес}) for labeling vertices with nonnegative real values.

      \item \incite[def. 1.1.2]{Knauer2019AlgGraphTheory} for general labels of either vertices or edges of graphs.

      \item \incite{Bollobas1998Graphs} and \incite{Erickson2019Algorithms} without an explicit definition for numeric labels for vertices.

      \item \incite[9]{GondranMinoux1984Graphs} and \incite[12]{Мирчев2001Графи} for number-valued labels (without specifying which kind of numbers) of edges of graphs, although Mirchev also allows vertices to be labeled.
    ...
    \begin{itemize}
      ...
      \item \incite[37]{Blizard1988MultisetTheory}, \incite[309]{Barendregt1984LambdaCalculus}, \incite[20]{Golan1999Semirings} \incite[A.3.5]{Mimram2020Types} for explicit labeling with positive integers.
...
\begin{definition}\label{def:fuzzy_set}\mcite[339]{Zadeh1965FuzzySets}
  ...
  A \( \mscrU \)-\term[ru=нечёткое множество (\cite[27]{БогомоловСалий1997ОбщаяАлгебра})]{fuzzy set} is simply a \hyperref[def:labeled_set]{labeling} of \( \mscrU \) with numbers from the unit interval.

  We denote a fuzzy set similarly to a regular set, e.g. \( A \), and then denote its membership function by \( f_A \). We call \( f_A(x) \) the \term{membership degree} of \( x \) in \( A \).
\end{definition}
\begin{comments}
  \item We follow the exposition of Zadeh closely, however we use Grothendieck universes as our universe of discourse rather than his informal \enquote{space of points} that may or may not be well-behaved. We will further discuss this in \fullref{def:fuzzy_set_support}.
\end{comments}

\begin{remark}\label{rem:crisp_set}
  In the context of \hyperref[def:fuzzy_set]{fuzzy sets}, in order to distinguish the plain sets defined in \fullref{def:set}, we will refer to the latter as \enquote{crisp sets}. We borrow this terminology from \incite*[11]{Zimmermann2001FuzzySets}.
\end{remark}

\begin{definition}\label{def:fuzzy_set_support}\mcite[def. 2-2]{Zimmermann2001FuzzySets}
  The \term{support} \( \supp A \) of a \( \mscrU \)-\hyperref[def:fuzzy_set]{fuzzy set} \( A \) is the crisp set of all members of \( \mscrU \) with nonzero membership degree in \( A \).
\end{definition}

\begin{definition}\label{def:fuzzy_subset}\mcite[339]{Zadeh1965FuzzySets}
  Fix two \( \mscrU \)-\hyperref[def:fuzzy_set]{fuzzy sets} \( A \) and \( B \). We say that \( A \) is a \term{fuzzy subset} of \( B \) if \( f_A(x) \leq f_B(x) \) for every \( x \in \mscrU \).
\end{definition}
...
\begin{definition}\label{def:fuzzy_relation}\mcite[345]{Zadeh1965FuzzySets}
  ...
  Explicitly, a fuzzy relation on the \( \mscrU \)-small crisp sets \( S_1, \ldots, S_n \) is (an extension to \( \mscrU \) of) a function with signature
  \begin{equation*}
...
\begin{example}\label{ex:def:fuzzy_relation}
  ...
  \begin{thmenum}
    \thmitem{ex:def:fuzzy_relation/relative_magnitude} Given two integers \( n \) and \( m \), we can define their relative magnitude as the ratio of the number of digits in their \hyperref[def:positional_number_system/decimal]{decimal expansions}:
    \begin{equation*}
      \frac {\ceil(\log_{10}(n))} {\ceil(\log_{10}(m))}.
    \end{equation*}

    In case we only want to use this value as a guide, it can be more simply expressed without rounding, in which case we can simplify to \( \log_{10}(n - m) \).

    Then, in addition to the relation \( m < n \), indicating whether \( n \) is greater than \( m \), we can thus also define the fuzzy relation \( m \ll n \), indicating how much \( n \) is greater than \( m \):
    \begin{equation*}
      f_\ll(n, m) \coloneqq \begin{cases}
        \frac {\log_{10}(n - m) - 1} {\log_{10}(n - m)}, &n \geq 10 + m, \\
        0,                                               &\T{otherwise.}
      \end{cases}
    \end{equation*}

    Thus, for example, \( f_\ll(0, 10^k) = (k - 1) / k \), meaning that \( 0 \ll_0 10 \), but \( 0 \ll_{0.9} 10^{10} \).
  \end{thmenum}
\end{example}

\begin{definition}\label{def:basic_fuzzy_set_operations}\mcite[339]{Zadeh1965FuzzySets}
  Fix two \( \mscrU \)-\hyperref[def:fuzzy_set]{fuzzy sets} \( A \) and \( B \).
...
\begin{comments}
  ...
  \item What we call a \enquote{sublevel set} is also referred to as a \enquote{level set}, for example by \incite[28]{Rockafellar1997Convex}, \incite[def. 2-3]{Zimmermann2001FuzzySets} and \incite[281]{AubinFrankowska1990SetValuedAnalysis}.
\end{comments}

\begin{definition}\label{def:fuzzy_convex_set}\mcite[348]{Zadeh1965FuzzySets}
  We say that a \hyperref[def:fuzzy_subset]{fuzzy subset} \( A \) of \( \BbbR^n \) is \term{convex} if its \hyperref[def:fuzzy_level_set]{superlevel sets} for nonzero levels are \hyperref[def:convex_set]{convex}.
...
\begin{example}\label{ex:def:fuzzy_convex_set}
  ...
  \begin{thmenum}
    \thmitem{ex:def:fuzzy_convex_set/convex_set_not_convex_function} Consider the fuzzy set \( A \) whose nonzero values are \( f_A(t) = 1 - \abs{t} \) when \( \abs{t} < 1 \) for a real value \( t \).

    \begin{figure}[!ht]
      \centering
      \includegraphics[page=1]{output/ex__def__fuzzy_convex_set}
      \caption{A plot of \( f_A \) from \fullref{ex:def:fuzzy_convex_set/convex_set_not_convex_function}.}\label{ex:def:fuzzy_convex_set/convex_set_not_convex_function}
    \end{figure}
    ...
    \thmitem{ex:def:fuzzy_convex_set/convex_function_convex_set_not_convex_function_set} Dually, consider the function \( f_A(t) = \min\set{1, \abs{t}} \) defined on \( \BbbR \).
...
\begin{proposition}\label{thm:def:fuzzy_convex_set}
  ...
  \begin{thmenum}
    \thmitem{thm:def:fuzzy_convex_set/concave} If \( f_A \) is a \hyperref[def:convex_function]{concave function}, then \( A \) is a fuzzy convex set.

    \thmitem{thm:def:fuzzy_convex_set/intersection} The \hyperref[def:basic_fuzzy_set_operations/intersection]{fuzzy intersection} of convex fuzzy sets is convex.
  \end{thmenum}
\end{proposition}
\begin{proof}
  \SubProofOf{thm:def:fuzzy_convex_set/concave} Suppose that \( f_A: \BbbR^n \to [0, 1] \) is a concave function. Fix \( x \) and \( y \) from \( \BbbR^n \). Then
  \begin{equation*}
