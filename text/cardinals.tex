...
\begin{remark}\label{rem:countability_terminology_review}
  We mentioned in \cref{def:set_countability} that the terms based on the word \enquote{countable} and \enquote{enumerable} are inconsistently used across the literature. We list in \cref{tab:rem:countability_terminology_review} how this is handled in some of the books we cite.
  ...
  \begin{table}[!ht]
    ...
    \begin{tabular}{l l l}
     ...
     \cite[def. 1.3]{АлександровКолмогоров1948ТеорияМножеств}        & ---                        & Счётное (Countable)             \\
      \bottomrule
    \end{tabular}
    \caption{The table for \cref{rem:sigma_algebra_terminology} discussing the terminology of \( \sigma \)-algebras.}\label{tab:rem:countability_terminology_review}
  \end{table}


  Some authors, for example \incite[159]{Enderton1977SetTheory}, Peter Hinman in \incite[def. 6.2.32(v)]{Hinman2005Logic} and \incite[4]{Engelking1989GeneralTopology}, use the shortened term \enquote{countable} to mean \enquote{at most countable}, while other authors like \incite[def. 2.4(c)]{Rudin1976AnalysisPrinciples} use \enquote{countable} to mean \enquote{countably infinite}. The term \enquote{denumerable} is also used for \enquote{countably infinite}, for example by Peter Hinman in \incite[def. 6.2.32(iv)]{Hinman2005Logic}, \incite[123]{MacLane1998CategoryTheory} and \incite[def. 2.4]{Rudin1976AnalysisPrinciples}.
\end{remark}
...
\begin{remark}\label{rem:cardinality_auxiliary_terminology}
  ...
  On the other hand, the graph itself can have finitely many arcs/edges but uncountably many vertices. So we call a graph \enquote{finite} if both the vertices and edges are finite. So even though we use the terminology for cardinals, we are not dealing with the cardinality of a single set.
