...
\begin{definition}\label{def:equinumerosity}\mcite[129 \\ 145]{Enderton1977Sets}
  ...
  Equinumerosity arises naturally outside the theory of cardinal numbers, unlike set dominance. We are usually instead interested only in injective functions that preserve some structure, i.e. \hyperref[def:first_order_isomorphism]{embeddings}.
\end{definition}

\begin{lemma}\label{thm:three_equinumerous_sets_lemma}\mcite[thm. 3.2]{Jech2003}
...
\begin{theorem}[Cantor-Schr\"oder-Bernstein theorem]\label{thm:cantor_schroder_bernstein_theorem}\mcite[thm. 3.2]{Jech2003}
...
\begin{theorem}[Cantor's power set theorem]\label{thm:cantor_power_set_theorem}\mcite[thm. 3.1]{Jech2003}
...
\begin{definition}\label{def:set_finiteness}
  ...
  \begin{thmenum}
    \thmitem{def:set_finiteness/cardinality}\mcite[27]{Jech2003} The \hyperref[thm:cardinality_existence]{cardinality} of \( A \) is a natural number. That is, we have \( \card(A) < \aleph_0 \).

    \thmitem{def:set_finiteness/dedekind}\mcite[34]{Jech2003} The set \( A \) is not \hyperref[def:equinumerosity]{equinumerous} all of its proper subsets. That is, if \( B \) is a proper subset of \( A \), then no function from \( A \) to \( B \) is injective.
  \end{thmenum}
...
\begin{definition}\label{def:aleph_hierarchy}\mcite[30]{Jech2003}
...
\begin{definition}\label{def:set_countability}\mcite[30]{Jech2003}
...
\begin{remark}\label{rem:countability_etymology}
  Some authors, for example Herbert Enderton in \cite[159]{Enderton1977Sets} and Ryszard Engelking in \cite[4]{Engelking1989}, use the shortened term \enquote{countable} to mean \enquote{at most countable}, while other authors like Thomas Jech in \cite[30]{Jech2003} use \enquote{countable} to mean \enquote{countably infinite}. The terms \enquote{denumerable} and \enquote{enumerable} are also used for \enquote{countably infinite} and \enquote{at most countable} respectively. This is done by Dirk van Dalen in \cite[4.2.7]{VanDalen2004} and by Adam Bobrowski in \cite[4.2.7]{Bobrowski2005}, for example.
\end{remark}

\begin{conjecture}[Continuum hypothesis]\label{hyp:continuum_hypothesis}\mcite[37]{Jech2003}
