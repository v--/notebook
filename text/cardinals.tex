...
\begin{definition}\label{def:successor_and_limit_cardinal}
  \begin{thmenum}
    ...
    Every strong limit cardinal is a weak limit cardinal as shown in \cref{thm:strong_limit_cardinal_is_weak_limit}, however the converse is only true assuming \cref{hyp:generalized_continuum_hypothesis} --- see \cref{thm:limit_cardinals_and_gch}.
...
\begin{comments}
  ...
  \item This hierarchy is important because it describes all infinite cardinals as shown in \cref{thm:infinite_cardinal_is_aleph}. It is intimately connected to the simpler \hyperref[def:beth_hierarchy]{\( \beth \) hierarchy} via \cref{hyp:generalized_continuum_hypothesis}.
\end{comments}
...
\begin{definition}\label{def:set_countability}\mcite[def. 6.2.32]{Hinman2005Logic}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:set_countability/continuum} The cardinality of \( \pow(\aleph_0) \) has a special name --- the \term{cardinality of the continuum}. It is sometimes denoted by \( \mfrakc \). See \cref{hyp:continuum_hypothesis} for its relation to \( \aleph_1 \).
  \end{thmenum}

  See \cref{rem:countability_etymology} for additional terminology that is potentially more ambiguous.
\end{definition}
...
\end{conjecture}
\begin{comments}
  \item Compare this to \cref{hyp:generalized_continuum_hypothesis}.
