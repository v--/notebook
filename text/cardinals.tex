...
\begin{definition}\label{def:equinumerosity}\mcite[129, 145]{Enderton1977SetTheory}
  We say that two sets are \term[bg=равномощни (\cite[79]{ГеновМиховскиМоллов1991Алгебра}), ru=равномощные (\cite[144]{КолмогоровДрагалин2006Логика})]{equinumerous} if there exists a \hyperref[def:function_invertibility/bijective]{bijective function} between them.
  ...
  Equinumerosity arises naturally outside the theory of cardinal numbers, unlike set dominance. We are usually instead interested only in injective functions that preserve some structure, i.e. \hyperref[def:fol_embedding]{embeddings}.
\end{definition}
...
\begin{proof}
  ...
  Use \fullref{thm:omega_recursion} to build the function
  \begin{equation*}
    \begin{aligned}
      &g: C \to B \\
  ...
  To see that \( g \) is also surjective, suppose that there exists some \( y \in B \setminus g[B] \). If \( y \in I \), then by the invariance of \( f \) we have \( g(y) = f(y) \in I \) and thus \( g(y) \notin B \), which contradicts our definition of \( g \). If instead \( y \in C \setminus I \), then \( g(y) = y \) and thus \( y \in g[B] \), which contradicts our choice of \( y \). The obtained contradictions show that \( g \) is surjective.
\end{proof}

\begin{theorem}[Cantor-Schr\"oder-Bernstein theorem]\label{thm:cantor_schroder_bernstein_theorem}\mcite[thm. 6.4.4]{Hinman2005Logic}
  If two sets \hyperref[def:equinumerosity]{dominate} each other, they are \hyperref[def:equinumerosity]{equinumerous}.
\end{theorem}
\begin{proof}
  Let \( f: A \to B \) and \( g: B \to A \) be injective functions. From \cref{thm:function_composition_invertibility} it follows that \( g \bincirc f: A \to A \) is also an injective function. If we restrict its codomain to its image \( g[f[A]] \), it becomes bijective. Hence, \( A \) is equinumerous with \( g[f[A]] \). Since \( g[f[A]] \subseteq g[B] \subseteq A \), from \cref{thm:three_equinumerous_sets_lemma} it follows that \( A \) is equinumerous with \( g[B] \), which is the desired result.
\end{proof}
...
\begin{remark}\label{rem:cardinal_definition}
  ...
  \begin{itemize}
    ...
    \item Transitivity is a consequence of \cref{thm:function_composition_invertibility}.
  ...
  If we partition \( \mscrA \) using the \hyperref[def:equinumerosity]{equinumerosity relation} if follows from \cref{def:antisymmetric_quotient} that the result will be a partial ordered set. The equivalence classes of this partition are then subfamilies of \( \mscrA \) such that every two sets in a single subfamily are equinumerous. For example, if \( \mscrA = \set{ \set{ A, B }, \set{ C, I }, \set{ A }, \set{ C } } \), then the corresponding equivalence classes are \( \set{ \set{ A, B }, \set{ C, I } } \) and \( \set{ \set{ A }, \set{ C } } \).

  Each of these equivalence classes consists of sets that are identical in \enquote{size} (not to be confused with \enquote{large} and \enquote{small} sets as defined in \cref{def:small_set}). In the above example, the corresponding equivalence classes correspond to sets of sizes \( 1 \) and \( 2 \). If we want to extend this notion of \enquote{size} to infinite sets, we must introduce a hierarchy of \enquote{sizes}. A natural candidate for such a hierarchy are the equivalence classes themselves. Unfortunately, this would mean that every family of sets has a different hierarchy. Since the entire universe is only available within the metatheory, we cannot partition the universe itself and must instead resort to finding a concrete representative of each possible equivalence class. We will call these representatives \term{cardinal numbers}.

  As explained in our proof of \cref{thm:cardinality_existence}, it will be convenient for us to define cardinal numbers as certain \hyperref[def:ordinal]{ordinal numbers} --- see \cref{def:cardinal}.
\end{remark}
...
\begin{definition}\label{def:cardinal}
  ...
  A cardinal is by definition an ordinal and this is useful. For example, the cardinals are well-ordered in the sense of \cref{thm:ordinals_are_well_ordered}.
...
\begin{proof}
  ...
  Now suppose that \( n \) is not equinumerous to a proper subset of itself and, aiming at a contradiction, suppose that there exists a subset \( E \subseteq \op{sc}(n) \) and a bijective function \( {f: \op{sc}(n) \to E} \).
  \begin{itemize}
    ...
    \item If \( n \notin E \), then \( E \) is a subset of \( n \) and thus \( f\restr_n: n \to E \) is a bijective function.
  \end{itemize}

  In both cases we obtain that \( n \) is equinumerous with a proper subset of itself, which is a contradiction. Hence, this also holds for \( \op{sc}(n) \).
...
\end{proposition}
\begin{proof}
  Fix a natural number \( n \in \omega \). Note that for every \( m < n \), \( m \) is a proper subset of \( n \) by \cref{thm:ordinal_ordering_via_subsets}. From \cref{thm:natural_number_is_not_equinumerous_to_proper_subset} it follows that no ordinal strictly smaller than \( n \) is equinumerous with \( n \) and hence \( n \) is an initial ordinal.
\end{proof}
...
\begin{proposition}\label{thm:omega_is_a_cardinal}
  ...
  When regarded as a cardinal, we denote it by \( \aleph_0 \). This is consistent with \cref{def:aleph_hierarchy}.
\end{proposition}
\begin{proof}
  We will use induction on \( n < \omega \) to show that no function \( f: \omega \to n \) is surjective. This is trivial for \( 0 \). Suppose that it holds for some fixed \( n \). Let \( f: \omega \to \op{sc}(n) \) be any function. Define
  \begin{equation*}
    \begin{aligned}
      &g: \omega \to n \\
...
\end{proposition}
\begin{proof}
  \SufficiencySubProof First suppose that \( \card(A) \leq \card(B) \). By \cref{thm:ordinal_ordering_via_subsets}, we have \( \card(A) \subseteq \card(B) \) and thus the identity function \( \id_{\card(A)} \) is an injective function from \( \card(A) \) to \( \card(B) \). Since \( A \) is equinumerous with \( \card(A) \) and \( B \) is equinumerous with \( \card(B) \), by \cref{thm:function_composition_invertibility} we obtain that there is an injective function from \( A \) to \( B \) and hence \( B \) dominates \( A \).

  \NecessitySubProof Conversely, let \( f: A \to B \) be an injective function. We again use \cref{thm:function_composition_invertibility} to conclude that \( \card(B) \) dominates \( \card(A) \).
...
\end{corollary}
\begin{comments}
  \item See also \fullref{thm:pigeonhole_principle}.
\end{comments}
\begin{proof}
  Follows from cardinal trichotomy and \cref{thm:cardinality_order_compatibility}.
\end{proof}

\begin{theorem}[Cantor's power set theorem]\label{thm:cantor_power_set_theorem}\mcite[thm. 6.4.10]{Hinman2005Logic}
...
\begin{definition}\label{def:set_finiteness}
  ...
  \begin{thmenum}
    \thmitem{def:set_finiteness/cardinality}\mcite[def. 6.2.32]{Hinman2005Logic} The \hyperref[thm:cardinality_existence]{cardinality} of \( A \) is a natural number. That is, we have \( \card(A) < \aleph_0 \).

    \medskip

    \thmitem{def:set_finiteness/dedekind}\mcite[def. 64]{Dedekind2007NatureAndMeaningOfNumbers} The set \( A \) is not \hyperref[def:equinumerosity]{equinumerous} all of its proper subsets. That is, if \( B \) is a proper subset of \( A \), then no function from \( A \) to \( B \) is injective.
  \end{thmenum}

  If a set is not finite, we say that it is \term{infinite}. If a set does not satisfy \cref{def:set_finiteness/dedekind}, we say that it is \term{Dedekind infinite}.
\end{definition}
\begin{proof}
  \ImplicationSubProof{def:set_finiteness/cardinality}{def:set_finiteness/dedekind} We will use \fullref{thm:omega_induction} on \( n < \aleph_0 \) to prove that all sets of cardinality \( n \) are not Dedekind infinite. The case \( n = 0 \) is vacuous. Suppose that all sets of cardinality \( n \) are not Dedekind infinite and suppose that \( A \) of cardinality \( \op{sc}(n) \) is Dedekind infinite.

  Then there exists a proper subset \( B \) of \( A \) that is equinumerous with \( \op{sc}(n) \). Since \( \op{sc}(n) \) is the cardinality of \( A \), there exists a bijective function \( f: A \to \op{sc}(n) \). Then \( f[B] \subseteq \op{sc}(n) \). Furthermore, the inequality is strict because otherwise any member of \( A \setminus B \) would contradict \fullref{thm:pigeonhole_principle}. Therefore, \( f[B] \) is a proper subset of \( \op{sc}(n) \) that is equinumerous with it. But this contradicts \cref{thm:natural_number_is_not_equinumerous_to_proper_subset}.
  ...
  We now use induction on \( m \) to prove that \( g(n) = g(m) \) implies \( n = m \). Since \( g(0) \notin B \) and \( g(n) \in B \) for any \( n > 0 \), the base case holds. Suppose that the inductive hypothesis holds for \( m \) and that for some \( n \) we have \( g(n) = g(m + 1) \). It is clear that \( g(0) \neq g(m + 1) \), so necessarily \( n > 0 \). We have
  \begin{equation*}
    f(g(n - 1))
    =
    g(n)
  ...
  Therefore, \( g \) is injective and thus \( A \) dominates \( \omega \). From \cref{thm:cardinality_order_compatibility} it follows that \( \card(A) \geq \aleph_0 \) and thus \( \card(A) \) is not a natural number.
\end{proof}
...
\begin{proof}
  ...
  Conversely, suppose that \( \kappa = \op{sc}(\alpha) \) is a successor ordinal that is a cardinal. That is, \( \kappa \) is not equinumerous with any smaller ordinal and in particular with \( \alpha \). From \cref{thm:cardinality_order_compatibility} it follows that \( \card(\alpha) < \kappa \).

  Let \( A \subseteq \kappa \) be a proper subset of \( \kappa \). We want to show that \( \card(A) < \kappa \).

  If \( \alpha \notin A \), define \( B \coloneqq A \). Otherwise, pick some member \( x_0 \in \kappa \setminus A \) and define
  \begin{equation*}
  ...
  Hence, \( \kappa \) dominates every proper subset, which by \cref{def:set_finiteness/dedekind} means that \( \kappa \) is a finite cardinal.
\end{proof}
...
\end{corollary}
\begin{proof}
  This is the contraposition to \cref{thm:cardinal_is_finite_iff_successor_ordinal} excluding the zero cardinal.
\end{proof}
...
\begin{definition}\label{def:successor_and_limit_cardinal}
  \begin{thmenum}
    ...
    The existence of \( \kappa \) is guaranteed by \cref{thm:successor_cardinal_existence}, but it is natural to ask whether \( \kappa \) can be constructed from \( \mu \) similarly to how the \hyperref[def:ordinal_successor]{ordinal successor operator} gives us a successor ordinal. This turns out to be a deep question --- see \fullref{hyp:generalized_continuum_hypothesis}.
    ...
    See \cref{thm:weak_limit_cardinal_equivalences} for some equivalent conditions.
    ...
    We can benefit from using forward references to \fullref{sec:transfinite_arithmetic}, more precisely \cref{thm:cardinal_exponentiation_power_set}, which justifies using \hyperref[def:cardinal_arithmetic/exponentiation]{cardinal exponentiation} to rewrite the condition for \( \kappa \) being a strong limit cardinal as
    \begin{equation*}
      \mu < \kappa \T{implies} 2^\mu < \kappa.
    \end{equation*}

    Every strong limit cardinal is a weak limit cardinal as shown in \cref{thm:strong_limit_cardinal_is_weak_limit}, however the converse is only true assuming \fullref{hyp:generalized_continuum_hypothesis} --- see \cref{thm:limit_cardinals_and_gch}.

    Strong limit cardinals are further motivated by the usage of \hyperref[rem:strongly_inaccessible_cardinal]{regular strong limit cardinals} in \cref{thm:strong_regular_cardinal_stages}.
  \end{thmenum}
...
\begin{proof}
  ...
  \Cref{thm:set_domination_relation_trichotomy} implies that \( \kappa < \mu \).
...
\begin{proposition}\label{thm:union_of_set_of_cardinals}\mcite[lemma 8B]{Enderton1977SetTheory}
  ...
  See a more thorough discussion of a similar issue in \cref{thm:union_of_set_of_ordinals}.
\end{proposition}
\begin{proof}
  From \cref{thm:union_of_set_of_ordinals} it follows that \( \bigcup A \) is an ordinal. Then there exists some cardinal \( \kappa \in A \) such that \( \alpha \in \kappa \).
  ...
  Therefore, \( \bigcup A \) is a cardinal. It follows from \cref{thm:ordinal_ordering_via_subsets} that it is also the supremum of \( A \).
\end{proof}

\begin{definition}\label{def:aleph_hierarchy}\mcite[def. 6.4.27]{Hinman2005Logic}
  ...
  \begin{equation}\label{eq:def:aleph_hierarchy}
    \aleph_\alpha \coloneqq \begin{cases}
      \omega,                                        &\alpha = 0 \\
      \T{successor cardinal of} \beta,               &\alpha = \op{sc}(\beta) \\
...
\end{definition}
\begin{comments}
  \item Note that \( \aleph_\lambda \) exists and is a cardinal for every limit ordinal \( \lambda \) as a consequence of \cref{thm:union_of_set_of_cardinals}.

  \item See \cref{rem:unbounded_transfinite_recursion} for some technical details.

  \item This hierarchy is important because it describes all infinite cardinals as shown in \cref{thm:infinite_cardinal_is_aleph}. It is intimately connected to the simpler \hyperref[def:beth_hierarchy]{\( \beth \) hierarchy} via \fullref{hyp:generalized_continuum_hypothesis}.
\end{comments}

\begin{remark}[Unbounded transfinite recursion]\label{rem:unbounded_transfinite_recursion}
  Although we cannot formally perform unbounded transfinite recursion, there is an easy way to circumvent this.

  Formally, in \cref{def:aleph_hierarchy}, for every ordinal \( \alpha \) we use \fullref{thm:bounded_transfinite_recursion} define a \( \alpha \)-indexed transfinite sequence \( \aleph_0, \aleph_1, \ldots, \aleph_\omega, \ldots \) and then use the sequence to define \( \aleph_\alpha \). The definition does not depend on any particular ordinal \( \alpha \), however, and thus all ways to obtain \( \aleph_\alpha \) are equivalent.
\end{remark}
...
\end{proposition}
\begin{proof}
  We will use \cref{rem:transfinite_induction} on \( \beta \).
  \begin{itemize}
    \item The condition \( \alpha < \beta \) is vacuously false for the base case \( \beta = 0 \), hence by \ref{inf:def:propositional_natural_deduction/bot/efq} the statement vacuously holds.

    \item Suppose that \( \alpha < \beta \) and \( \aleph_\alpha < \aleph_\beta \). We then have \( \alpha < \op{sc}(\beta) \) and, since, \( \aleph_{\op{sc}(\beta)} > \aleph_\beta \), also \( \aleph_\alpha < \aleph_{\op{sc}(\beta)} \).

    \item Let \( \lambda \) be a limit ordinal and suppose that the proposition holds for all \( \beta < \lambda \) and for arbitrary \( \alpha \). Then \( \aleph_\beta \subseteq \aleph_\lambda \) for every \( \beta < \lambda \), hence \( \aleph_\beta \leq \aleph_\lambda \) by \cref{thm:ordinal_ordering_via_subsets}.
...
\begin{remark}[Cardinal recursion and induction]\label{rem:cardinal_transfinite_recursion_and_induction}
  ...
  \begin{itemize}
    ...
    \item In its structured form presented in \cref{rem:transfinite_induction}, rather than considering successor ordinals and limit ordinals, we consider successor cardinals and weak limit cardinals.
  \end{itemize}
  ...
  See \cref{thm:infinite_cardinal_is_aleph} for how this principles is used.
\end{remark}

\begin{proposition}\label{thm:infinite_cardinal_is_aleph}\mcite[thm. 8A]{Enderton1977SetTheory}
  For every \hyperref[def:set_finiteness]{infinite cardinal} \( \kappa \) there exists an ordinal \( \alpha \) such that \( \kappa = \aleph_\alpha \).
\end{proposition}
\begin{proof}
  We will use \cref{rem:cardinal_transfinite_recursion_and_induction} on \( \kappa \).
  ...
  \begin{itemize}
    ...
    \item If \( \kappa = \aleph_\alpha \) and \( \mu \) is the successor cardinal of \( \kappa \), then by definition \( \kappa = \aleph_{\op{sc}(\alpha)} \).
    ...
    If we suppose that \( \kappa < \aleph_\alpha \), then similarly to \cref{thm:ordinal_ordering_via_addition} there exists some ordinal \( \beta_0 < \alpha \) such that \( \aleph_{\beta_0} > \aleph_{\alpha_\mu} \) for every \( \mu < \kappa \). In particular, \cref{thm:aleph_hierarchy_is_strictly_monotone} implies that \( \beta_0 > \alpha_\mu \) for every \( \mu < \kappa \). Thus,
    \begin{equation*}
...
\end{corollary}
\begin{proof}
  Clear from \cref{def:aleph_hierarchy}.
\end{proof}

\begin{definition}\label{def:set_countability}\mcite[def. 6.2.32]{Hinman2005Logic}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:set_countability/continuum} The cardinality of \( \pow(\aleph_0) \) has a special name --- the \term{cardinality of the continuum}. It is sometimes denoted by \( \mfrakc \). See \fullref{hyp:continuum_hypothesis} for its relation to \( \aleph_1 \).
  \end{thmenum}

  See \cref{rem:countability_terminology} for additional terminology that is potentially more ambiguous.
\end{definition}

\begin{remark}\label{rem:countability_terminology}
  We mentioned in \cref{def:set_countability} that the terms based on the word \enquote{countable} and \enquote{enumerable} are inconsistently used across the literature. We list in \cref{tab:rem:countability_terminology} how this is handled in some of the books we cite.

  \begin{table}
    \centering
    \begin{tabular}{l l l}
      \toprule
      Source                                                          & \( \leq \aleph_0 \)        & \( = \aleph_0 \)                \\
      \midrule
      \cite[30]{Jech2006SetTheory}                                    & At most countable          & Countable                       \\
      \cite[def. 2.3]{Rudin1976AnalysisPrinciples}                    & At most countable          & Countable                       \\
      \cite[def. I.10.5]{Kunen2013SetTheory}                          & Countable                  & Countably infinite              \\
      \cite[3]{Engelking1989GeneralTopology}                          & Countable                  & Countably infinite              \\
      \cite[25]{Kelley1975GeneralTopology}                            & Countable                  & Countably infinite              \\
      \cite[def. 6.2.32]{Hinman2005Logic}                             & Countable                  & Denumerable                     \\
      \cite[3]{Harzheim2005OrderedSets}                               & Countable                  & Denumerable                     \\
      \cite[91]{Halmos1960NaiveSetTheory}                             & Countable / denumerable    & Countably infinite              \\
      \cite[176]{Kleene2002Logic}                                     & Countable / enumerable     & Countably / enumerably infinite \\
      \cite[159]{Enderton1977SetTheory}                               & Countable                  & ---                             \\
      \cite[def. 1.1.14]{Архангельский1988КанторовскаяТеорияМножеств} & Счётное (Countable)        & ---                             \\
      \cite[13]{ШеньВерещагин2020НачалаТеорииМножеств}                & ---                        & Счётное (Countable)             \\
      \cite[def. 1.3]{Александров1977ОбщаяТопология}                  & ---                        & Счётное (Countable)             \\
      \cite[def. 1.3]{АлександровКолмогоров1948ТеорияМножеств}        & ---                        & Счётное (Countable)             \\
      \bottomrule
    \end{tabular}
    \caption{The table for \cref{rem:countability_terminology} discussing terminology related to \hyperref[def:set_countability]{countability}.}\label{tab:rem:countability_terminology}
  \end{table}
\end{remark}

\begin{remark}\label{rem:cardinality_auxiliary_terminology}
  There are multiple occasions where we have to use adjectives based on cardinality. The simplest example is when we want to call a set \enquote{finite} (in accordance with \cref{def:set_finiteness}), \enquote{countably infinite} (in accordance with \cref{def:set_countability}), \enquote{uncountable}, etc.

  As another example, a \hyperref[rem:graph_of_unspecified_type]{graph or hypergraph} can be \enquote{locally finite}, \enquote{locally countable}, etc., in accordance with \cref{def:graph_cardinality/local}.

  On the other hand, the graph itself can have finitely many arcs/edges but uncountably many vertices. So we call a graph \enquote{finite} if both the vertices and edges are finite. So even though we use the terminology for cardinals, we are not dealing with the cardinality of a single set. As a general rule, if some contraption is defined as an \hyperref[def:ordered_tuple]{ordered tuple} \( \mscrA = (A_1, \ldots, A_n) \) of sets, when using cardinality-related terminology for \( \mscrA \), we refer to the largest among \( \card(A_k) \).

  In order not to repeat the same list of adjectives in every relevant definition, we aggregate here the ones we will use:
  \begin{center}
    \begin{tabular}{ll}
      \toprule
      Cardinal            & Adjective \\
      \midrule
      \( < \aleph_0 \)    & Finite \\
      \( \leq \aleph_0 \) & At most countable \\
      \( = \aleph_0 \)    & Countably infinite \\
      \( > \aleph_0 \)    & Uncountable \\
      \bottomrule
    \end{tabular}
  \end{center}
\end{remark}
\begin{comments}
  \item Note that even if a set is finite, it may not be hereditarily finite, i.e. it may consist of infinite sets. This somewhat contrasts with the concepts of \enquote{small} and \enquote{large} sets from \cref{def:small_set}, where, due to \cref{thm:small_set_closure}, all elements of a small set are small, and all components of a small ordered tuple are small.
\end{comments}

\begin{conjecture}[Continuum hypothesis]\label{hyp:continuum_hypothesis}\mcite[165]{Enderton1977SetTheory}
...
\begin{remark}\label{rem:hyp:continuum_hypothesis}
  \Fullref{hyp:continuum_hypothesis} has been shown by G\"odel not to be disprovable in \hyperref[def:zfc]{\( \logic{ZFC} \)} and by Cohen not to be provable in \logic{ZFC}.
\end{remark}

\begin{theorem}[Incompleteness of ZFC]\label{thm:zfc_incompleteness}
  \hyperref[def:zfc]{Zermelo-Fraenkel set theory} is \hyperref[def:fol_theory/complete]{incomplete}.
\end{theorem}
\begin{proof}
  As discussed in \cref{rem:hyp:continuum_hypothesis}, \fullref{hyp:continuum_hypothesis} can neither be proved nor disproved in \logic{ZFC}.
\end{proof}
...
\end{proposition}
\begin{proof}
  We can give a short proof using \fullref{thm:cantor_schroder_bernstein_theorem} using the injective functions \( (n, m) \mapsto 2^n 3^m \) in one direction and \( n \mapsto (n, 0) \) in the other direction. Proving the injectivity of \( f \), however, requires \fullref{thm:fundamental_theorem_of_arithmetic}, and we prove the latter using machinery from \fullref{ch:ring_theory}. We will instead give a direct proof with an explicit construction. We will construct a bijective function from \( \omega \times \omega \) to \( \omega \) --- the function visualized in \cref{fig:thm:omega_equinumerous_with_omega_squared}.
  ...
  Lastly, although it is not necessary for the proof, we can expand the definition of \( d \) to see that \( f \) is actually a \hyperref[def:polynomial_algebra/polynomials]{polynomial}:
  \begin{equation*}
    f(n, m)
    =
    ...
    =
    \sum_{k=1}^{n + m} k + n
    \reloset {\eqref{eq:thm:numeric_arithmetic_progression_partial_sums}} =
    \frac {(n + m) (n + m + 1)} 2 + n.
  \end{equation*}
  ...
  \begin{figure}
    ...
    \hfill\hfill
    \caption{Visualization on an integer coordinate grid of the diagonal sets \( A_k \) and of the bijective function defined in \cref{thm:omega_equinumerous_with_omega_squared}.}\label{fig:thm:omega_equinumerous_with_omega_squared}
  \end{figure}
\end{proof}
...
\begin{proof}
  ...
  Suppose that the inductive hypothesis holds for \( n \). Countability ensures that there exist injective functions \( g: A_1 \times \cdots \times A_n \times \omega \) and \( h: A_{n+1} \to \omega \). Denote by \( f \) the bijective function from \( \omega \) to \( \omega \times \omega \) obtained in \cref{thm:omega_equinumerous_with_omega_squared} and define
  \begin{equation*}
    \begin{aligned}
  ...
  By \cref{thm:function_superposition_invertibility}, the function \( F \) is injective as a \hyperref[con:function_superposition]{superposition} of injective functions.

  Therefore, \( \omega \) dominates the product \( A_1 \times \cdots \times A_n \), i.e. the product is countable.
\end{proof}

\begin{proposition}\label{thm:countably_infinite_disjoin_union_of_nonempty_sets}
  A \hyperref[def:set_countability/countably_infinite]{countably infinite} \hyperref[def:disjoint_union]{disjoint union} of nonempty sets is infinite.
\end{proposition}
\begin{comments}
  \item \todo{Consider stating as a special case of \cref{thm:cardinality_of_disjoint_union}}.
\end{comments}
\begin{proof}
  Let \( \seq{ A_k }_{k \in \omega} \) be a countably infinite family of nonempty sets and consider the family \( \seq{ B_k }_{k \in \omega} \), where
  \begin{equation*}
    B_k \coloneqq \set{ \iota_k(a) \given a \in A_k }.
  \end{equation*}

   We can use the \hyperref[def:zfc/choice]{axiom of choice} to construct a sequence
   \begin{equation*}
     \iota_0(a_0), \iota_1(a_1), \ldots
   \end{equation*}
   of elements of the product \( \prod_{k \in \omega} B_k \).

   This is in fact an injective function from \( \omega \) to the disjoint union
   \begin{equation*}
     \bigcup_{k \in \omega} B_k = \coprod_{k \in \omega} A_k.
   \end{equation*}

   Therefore, the union is at least countably infinite.
\end{proof}

\begin{proposition}\label{thm:countably_infinite_union_of_countably_infinite_sets}\mcite[thm. 6Q]{Enderton1977SetTheory}
...
\begin{proof}
  ...
  Denote the union of the former family by \( A \) and of the latter family by \( B \). Since each \( A_k \) is countably infinite, so is \( A \). Furthermore, there exists an obvious injective function from \( A \) to \( B \), thus
  ...
  Denote by \( f \) the bijective function from \( \omega \) to \( \omega \times \omega \) obtained in \cref{thm:omega_equinumerous_with_omega_squared}. Then the function \( f \bincirc g: \omega \to B \) is bijective by \cref{thm:function_composition_invertibility}. Therefore, the union \( B \) is countably infinite. From \eqref{eq:thm:countably_infinite_union_of_countably_infinite_sets/union_card_inequality} it follows that \( A \) is also countably infinite.
\end{proof}
...
\begin{proof}
  ...
  \end{equation*}
  and the latter is countably infinite by \cref{thm:countably_infinite_union_of_countably_infinite_sets}. Therefore, \( A \) is at most countable.
\end{proof}
