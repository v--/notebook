...
\begin{definition}\label{def:hol_natural_deduction_proof_tree}\mimprovised
  Natural deduction \hyperref[def:proof_tree]{proof trees} for \hyperref[def:hol_expression/formula]{formulas of higher-order logic} require some adaptation compared to their propositional equivalents defined in \cref{def:propositional_natural_deduction_proof_tree}.
  ...
  The difference compared to propositional logic is that we must handle \hyperref[alg:lambda_term_substitution]{substitution} and \hyperref[def:fol_natural_deduction_proof_tree/eigenvariables]{eigenvariables}. Substitution is straightforward, but handling variable requires special care in general since each \hyperref[def:hol_expression]{logical term} has an implicit \hyperref[def:type_context]{type context}.
...
\begin{comments}
  ...
  The higher-order variants follow naturally from the rules of \hyperref[def:martin_lof_type_theory]{Martin-L\"of type theory} via \hyperref[con:curry_howard_correspondence]{Curry-Howard correspondence}, as shown in \cref{rem:dependent_products_and_forall_quantifier_rules}.
...
\begin{definition}\label{def:hol_consequence}\mimprovised
  We can now define a \hyperref[def:consequence_relation]{consequence relation} on \hyperref[def:hol_expression/formula]{formulas} of \hyperref[
def:higher_order_logic]{higher-order logic} as follows: let \( \Gamma \vdash \varphi \) if there exists a \hyperref[def:hol_natural_deduction_proof_tree]{higher-order logic proof tree} of \( \varphi \) whose open assumptions are in \( \Gamma \) and whose allowed rules are the \hyperref[def:natural_deduction_rule]{natural deduction rules} for propositional logic from \cref{def:propositional_natural_deduction}, the quantifier rules from \cref{def:hol_quantifier_rules} and the equality rules from \cref{def:hol_equality_rules}.

  In accordance with \cref{def:general_logic}, we say that \( \varphi \) is \term{derivable} from \( \Gamma \) if \( \Gamma \vdash \varphi \).
\end{definition}
\begin{comments}
  \item We obtain, based on the propositional logic rules considered, a proof system for either \hyperref[rem:minimal_propositional_semantics]{minimal}, \hyperref[def:truth_value_algebra/intuitionistic]{intuitionistic} or \hyperref[def:truth_value_algebra/classical]{classical logic}.
...
\begin{remark}\label{rem:hol_axiomatic_derivations}
  ...
  Andrews spends considerable effort demonstrating that part of the familiar natural deduction rules from \cref{def:propositional_natural_deduction} are \hyperref[con:inference_rule_admissibility]{admissible}.
  ...
  With the abbreviations in \ref{rem:hol_formula_abbreviations}, the rule \ref{inf:def:propositional_natural_deduction/top/intro} is a special case of \ref{inf:thm:hol_equality_is_equivalence_relation/reflexivity} with \( M^\tau = \qabs {\synp^{\syn\omicron}} \synp \).
...
\begin{definition}\label{def:hol_entailment_system}\mimprovised
  ...
  \begin{thmenum}
    ...
    \begin{itemize}
      \item For each signature \( \Sigma \), let \( \op*{Sen}(\Sigma) \) be the set of all sentences (\hyperref[def:hol_closed_expression]{\hi{closed}} \hyperref[def:hol_expression/formula]{logical formulas}) over \( \Sigma \).
...
\begin{definition}\label{def:hol_theory}\mimprovised
  The \hyperref[def:entailment_system]{entailment system} for \hyperref[def:higher_order_logic]{higher-order logic} from \cref{def:hol_entailment_system} allows us consider \hyperref[def:logical_theory]{theories}. For the reasons outlined in \cref{rem:implicit_quantification_and_deduction}, we suppose that a theory necessarily consists of \hyperref[def:hol_closed_expression]{sentences} (closed formulas).
\end{definition}
...
\begin{definition}\label{def:hol_definition}\mimprovised
  ...
  Let \( \varphi \) be a \hyperref[def:hol_expression/formula]{formula} whose free variables are among \( {x_1}^{\tau_1}, \ldots, {x_n}^{\tau_n}, y^\sigma \), and such that
...
\begin{example}\label{ex:simplified_hol_predicate_definition}
  ...
  \begin{equation*}
    \begin{prooftree}
      ...
      \infer[left label=\( v \)]1[\ref{inf:def:propositional_natural_deduction/imp/intro}]{ (\psi \syneq z^{\syn\omicron}) \synimplies (z^{\syn\omicron} \syneq \psi) }
      ...
      \infer2[\ref{inf:def:propositional_natural_deduction/and/intro}]{ \parens[\Big]{ (\psi \syniff y^{\syn\omicron}) \synwedge \qforall {z^{\syn\omicron}} \parens[\big]{ (\psi \syniff z) \synimplies (z \syneq y^{\syn\omicron}) } }[y^{\syn\omicron} \mapsto \psi] }
  ...
  \begin{equation*}
    \begin{prooftree}
      ...
      \hypo{ [\psi]^v }
      \infer2[\ref{inf:def:propositional_natural_deduction/iff/elim_right}]{ y^{\syn\omicron} }

      \hypo{ [\psi \syniff y^{\syn\omicron}]^w }
      \infer[left label=\( w \)]2[\ref{inf:def:propositional_natural_deduction/iff/intro}]{ y^{\syn\omicron} \syniff (\psi \syniff y^{\syn\omicron}) }

      \infer[left label=\( v \)]2[\ref{inf:def:propositional_natural_deduction/iff/intro}]{ \psi \syniff (y^{\syn\omicron} \syniff (\psi \syniff y^{\syn\omicron})) }
