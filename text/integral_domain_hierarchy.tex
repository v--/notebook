...
\begin{proof}
  ...
  Then \( X - \alpha_n \) divides \( f(X) \). Let \( g(X) \) be the quotient of \( f(X) \) by \( a_n (X - \alpha_n) \). \Cref{thm:univariate_polynomial_product/degree} implies that \( g(X) \) has degree \( n - 1 \) and \cref{thm:univariate_polynomial_product/leading} implies that \( g(X) \) is monic.
