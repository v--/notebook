...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \item If both are nonzero, \cref{thm:univariate_polynomial_product/degree} implies that \( g(X) \) has degree one.
  \end{itemize}
...
\end{definition}
\begin{comments}
  \item \Textcite[235]{Lang2002Algebra} and \textcite[\S 17.6]{Тыртышников2007ЛинейнаяАлгебра}) discusses splitting into linear factors for fields, however the concept seems to generalize well enough to GCD domains. Going beyond that can break \fullref{thm:polynomial_factorization_via_roots}.
\end{comments}
...
\begin{proof}
  ...
  Then \( X - \alpha_n \) divides \( f(X) \). Let \( g(X) \) be the quotient of \( f(X) \) by \( a_n (X - \alpha_n) \). \Cref{thm:univariate_polynomial_product/degree} implies that \( g(X) \) has degree \( n - 1 \) and \cref{thm:univariate_polynomial_product/nonzero_leading} implies that \( g(X) \) is monic.
...
\begin{defproof}
  ...
  Since \( D \) is an integral domain, by \cref{thm:def:ring_localization/injective_inclusion}, \( S^{-1} D \) is a superring of \( D \). It is therefore a nonzero commutative simple ring, and thus it satisfies \cref{def:field/simple}.
\end{defproof}

\begin{theorem}[Field of fractions universal property]\label{thm:field_of_fractions_universal_property}
  The \hyperref[def:field_of_fractions]{field of fractions} \( \BbbK \) of the integral domain \( D \) satisfies the following \hyperref[con:universal_property]{universal property}:
  \begin{displayquote}
...
\begin{comments}
  ...
  \begin{itemize}
    ...
    \item There is no other established name for \( D[\mscrX] \). The term \enquote{rational expression} can be found in \cite[232]{Jacobson1985BasicAlgebraI} and \cite[384]{Knapp2016BasicAlgebra}.
  \end{itemize}
\end{comments}
...
\begin{definition}\label{def:bezout_domain}
  ...
  \begin{thmenum}
    \thmitem{def:bezout_domain/identity}\mimprovised The conclusion of \fullref{thm:bezouts_identity} holds: for any two elements \( x \) and \( y \), there exists a \hyperref[def:gcd]{greatest common divisor} \( g \) and elements \( a \) and \( b \) such that
    \begin{equation}\label{eq:def:bezout_domain/identity}
      g = ax + by.
    \end{equation}

    Such coefficients in fact exist for every GCD --- see \cref{thm:def:bezout_domain/gcd}.
...
\end{definition}
\begin{comments}
  \item Bezout domains are, by \eqref{eq:def:bezout_domain/identity}, \hyperref[def:gcd_domain]{GCD domains}.
...
\begin{proposition}\label{thm:def:bezout_domain}
  ...
  \begin{thmenum}
    \thmitem{thm:def:bezout_domain/gcd} For \emph{every} GCD \( g \) of \( x \) and \( y \), there exist elements \( a \) and \( b \) such that \eqref{eq:def:bezout_domain/identity} holds.

    \thmitem{thm:def:bezout_domain/gcd_ideal} Equality holds in \cref{def:gcd/ideals}: if \( g \) is a \hyperref[def:gcd]{greatest common divisor} of \( x \) and \( y \), then
    \begin{equation*}
...
\end{proposition}
\begin{proof}
  \SubProofOf{thm:def:bezout_domain/gcd} By definition, there exist a GCD \( g' \) and coefficients \( a' \) and \( b' \) so that
  \begin{equation*}
    g' = a'x + b'y.
  \end{equation*}

  \Cref{thm:def:gcd/associates} implies that \( g \) and \( g' \) are \hyperref[def:domain_divisibility/associates]{associated}, thus there exists some invertible \( u \) such that \( g = ug' \). THen
  \begin{equation*}
    g = (ua')x + (ub')y.
  \end{equation*}

  \SubProofOf{thm:def:bezout_domain/gcd_ideal} Follows from \eqref{eq:def:bezout_domain/identity}.
  \SubProofOf{thm:def:bezout_domain/ideal_gcd_closed} Follows from \cref{thm:def:bezout_domain/gcd_ideal}.
\end{proof}

\begin{example}\label{ex:def:bezout_domain}
  We list examples of \hyperref[def:bezout_domain]{Bezout domains}:
  \begin{thmenum}
    \thmitem{ex:def:bezout_domain/integers} \Fullref{thm:bezouts_identity} provides a simple demonstration that an \hyperref[def:euclidean_domain]{Euclidean domain} is a Bezout domain.
...
\end{definition}
\begin{comments}
  \item This definition is based on one given by \textcite[69]{Rotman2015AdvancedModernAlgebraPart1} for rational algebraic functions.
\end{comments}
...
\begin{definition}\label{def:irreducible_factorization}\mcite[def. V.1.8]{Aluffi2009Algebra}
  An \term{irreducible factorization} or simply \term{factorization} of a nonzero element \( x \) in an arbitrary \hyperref[def:integral_domain]{integral domain} is a list \( p_1, \ldots, p_r \) of \hyperref[def:domain_divisibility/irreducible]{irreducible elements} such that, for some \hyperref[def:semiring_divisibility/invertible]{invertible element} \( u \),
  \begin{equation*}
    x = u p_1 \cdots p_r.
  \end{equation*}
...
\begin{proposition}\label{thm:def:irreducible_factorization}
  ...
  \begin{thmenum}
    \thmitem{thm:def:irreducible_factorization/existence} If every ascending sequence of \emph{principal} ideals \hyperref[def:stabilizing_sequence]{stabilizes}, then every element has at least one irreducible \emph{factorization}.
...
\begin{proof}
  ...
  \SubProof*{Every non-invertible element has at least one irreducible \emph{factor}} Fix some element \( x \). Let \( x_1 \coloneqq x \). Via recursion on \( k = 1, 2, \ldots \), define a sequence as follows:
  \begin{itemize}
  ...
  \SubProof*{Every element has at least one irreducible \emph{factorization}} Fix again some element \( x \) and define \( x_1 \coloneqq x \). Via recursion on \( k = 1, 2, \ldots \), define the following sequence:
  \begin{itemize}
  ...
  \end{equation}
  be an irreducible factorization of \( x \in D \) in \( D[X] \). By \cref{thm:univariate_polynomial_product/degree}, all polynomials in this factorization are constants. By \cref{thm:def:domain_divisibility/irreducible_in_polynomial_ring}, since they are irreducible in \( D[X] \), they are also irreducible in \( D \).
  ...
  \NecessitySubProof* Due to \cref{thm:def:domain_divisibility/irreducible_in_polynomial_ring} and \cref{thm:def:domain_divisibility/irreducible_in_polynomial_ring}, irreducible and invertible elements in \( D \) are also irreducible and invertible in \( D[X] \). Hence, every irreducible factorization in \( D \) is also an irreducible factorization in \( D[X] \).
\end{proof}
...
\begin{proof}
  ...
  \Fullref{alg:euclidean_division_of_polynomials} allows us to divide any polynomial \( f(X) \) by any nonzero polynomial \( g(X) \) so that
  \begin{equation*}
    f(X) = \quot(f, g) \cdot g(X) + \rem(f, g),
  \end{equation*}
  where \( \deg \rem(f, g) < \deg g \). Note that \( \rem(f, g) \) is possibly zero, in which case we have defined the degree to be \( -\infty \).

  Now let \( g(X) \) be a nonzero polynomial from \( I \) of minimal degree. Since \( \deg \rem(f, g) < \deg g \), minimality requires \( \rem(f, g) \) to be zero. Then \( g(X) \) divides \( f(X) \).
...
\end{proposition}
\begin{proof}
  Note that \( x < n \) is invertible modulo \( n \) if and only if there exists an integer \( a \) such that \( ax \equiv 1 \pmod n \). That is, if there exist integers \( a \) and \( b \) such that \( ax + bn = 1 \).
...
\begin{comments}
  ...
  \begin{itemize}
    \item \Textcite[98]{Rotman2015AdvancedModernAlgebraPart1} and \textcite[def. 3.5.2]{Винберг2014КурсАлгебры} both use the term \enquote{Euclidean ring} for our definition.

    \item \Textcite[36]{Шафаревич2019ОсновныеПонятияАлгебры} and \textcite[def. VI.2]{ГеновМиховскиМоллов1991Алгебра} also use \enquote{Euclidean ring}, however neither state \cref{def:euclidean_domain/multiplication}, and the latter even require degrees to be defined for zero elements.

    \item \Textcite[392]{Knapp2016BasicAlgebra} and \textcite[def 2.5]{Jacobson1985BasicAlgebraI} use the term \enquote{Euclidean domain}, however they both require \( \delta \) to be defined for zero elements and neither states \cref{def:euclidean_domain/multiplication}.
  \end{itemize}
\end{comments}
...
\begin{example}\label{ex:def:euclidean_domain}
  ...
  \begin{thmenum}
    \thmitem{ex:def:euclidean_domain/integers} \Cref{thm:integers_are_euclidean_domain} states that the ring of integers is a Euclidean domain.

    \thmitem{ex:def:euclidean_domain/polynomials} \Cref{thm:def:principal_ideal_domain/field_polynomials} characterizes Euclidean polynomial algebras as univariate polynomial algebras over fields.
...
\begin{algorithm}[Extended Euclidean algorithm]\label{alg:extended_euclidean_algorithm}
  In an \hyperref[def:euclidean_domain]{Euclidean domain}, given a pair \( x \) and \( y \), \fullref{alg:euclidean_algorithm} allows us to construct a greatest common divisor \( g \). We can also explicitly construct coefficients \( a \) and \( b \) for \fullref{thm:bezouts_identity}, so that
  \begin{equation*}
    g = ax + by.
  \end{equation*}
...
\begin{comments}
  \item This algorithm is implemented in \cite{notebook:code} as
  \begin{CodeRefDisplay}
    \GetCodeRef{alg:extended_euclidean_algorithm}
  \end{CodeRefDisplay}
\end{comments}
\begin{defproof}
...
\begin{theorem}[Bezout's identity]\label{thm:bezouts_identity}
  In an \hyperref[def:euclidean_domain]{Euclidean domain} \( D \), for every pair \( x \) and \( y \) of elements and every \hyperref[def:gcd]{greatest common divisor} \( g \) of \( x \) and \( y \), there exist \( a \) and \( b \) in \( D \) such that
  \begin{equation}\label{eq:thm:bezouts_identity}
    g = ax + by.
  \end{equation}

  In particular, if \( x \) and \( y \) are \hyperref[def:coprime_elements]{coprime}, then \( ax + by = 1_D \).
\end{theorem}
\begin{comments}
  \item Since at least one GCD exists, this theorem is equivalent to the condition \cref{def:bezout_domain/identity}, which defines a Bezout domain.

  More generally, all \hyperref[def:principal_ideal_domain]{principal ideal domains} are Bezout domains, however the explicit computation of coefficients is specific to Euclidean domains.
\end{comments}
\begin{proof}
  \Fullref{alg:extended_euclidean_algorithm} gives us a particular GCD \( g \) of \( x \) and \( y \) and elements \( a \) and \( b \) for which \eqref{eq:thm:bezouts_identity} holds.

  If \( g' \) is any GCD of \( x \) and \( y \), as in \cref{thm:def:bezout_domain/gcd}, \( g' = ug \) for some invertible element \( u \), and so
  \begin{equation*}
    g' = (ua)x + (ub)y.
  \end{equation*}
\end{proof}
