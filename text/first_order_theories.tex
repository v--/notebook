...
\begin{definition}\label{def:fol_theory}\mimprovised
  ...
  \begin{thmenum}
    ...
    Equivalence of semantic and syntactic consistency is given in \cref{thm:propositional_semantic_and_syntactic_consistency}.
...
\begin{proposition}\label{thm:def:fol_categorical_theory}
  \hyperref[def:fol_theory/theory]{Categorical theories} have the following basic properties:
  \begin{thmenum}
    \thmitem{thm:def:fol_categorical_theory/unsatisfiable} An \hyperref[def:fol_semantics/satisfaction]{unsatisfiable} theory is vacuously categorical, and \( \kappa \)-categorical for any \( \kappa \).

    \thmitem{thm:def:fol_categorical_theory/complete} Every categorical (semantic) theory is \hyperref[def:fol_theory/complete]{complete}.

    \thmitem{thm:def:fol_categorical_theory/finite}\mcite[thm. 2.5.3(i)]{Hinman2005Logic} If a complete theory has a finite model, it is categorical, and \( \kappa \)-categorical for any \( \kappa \).
  \end{thmenum}
\end{proposition}
\begin{proof}
  \SubProofOf{thm:def:fol_categorical_theory/unsatisfiable} Trivial.

  \SubProofOf{thm:def:fol_categorical_theory/complete} Suppose that the theory \( T \) is categorical.
  ...
  \SubProofOf{thm:def:fol_categorical_theory/finite} Follows from \cref{thm:fol_finite_structure_characterizing_formula}.
\end{proof}
