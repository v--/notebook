...
\begin{definition}\label{def:fol_theory}\mimprovised
  ...
  \begin{thmenum}
    ...
    \thmitem{def:fol_theory/category} Based on theories and their morphisms, as per \cref{def:category_of_theories}, we have a category of small (syntactic or semantic) theories \( \cat{Th} \).
    ...
    \Cref{ex:thm:fol_completeness/skolemization} shows that it may be important whether we consider syntactic or semantic conservative extension.

    \thmitem{def:fol_theory/model}\mcite[def. 2.4.4]{Hinman2005Logic} As in \cref{def:theory_of_institutional_model}, we define the \term{theory} \( \cat{Th}(\mscrX) \) \emph{of} a structure \( \mscrX \) as the set of all sentences valid in \( \mscrX \).
    ...
    Equivalence of semantic and syntactic consistency follows from \fullref{thm:fol_natural_deduction_soundness} and \fullref{thm:fol_completeness}.
    ...
    \thmitem{def:fol_theory/categorical} In accordance with \cref{def:categorical_logical_theory}, we say that a semantic theory \( T \) is \term[ru=категоричная (система аксиом) (\cite[248]{Мальцев1970АлгебраическиеСистемы}), en=categorical (theory) (\cite[def. 2.5.2]{Hinman2005Logic})]{categorical} if all models of \( T \) are \hyperref[def:fol_isomorphism]{isomorphic}.

    This notion is unrelated to category theory --- see \cref{rem:categorical_theory_terminology} for a broader discussion.
  \end{thmenum}
\end{definition}
...
\begin{proposition}\label{thm:def:fol_theory}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:def:fol_theory_morphism_reduct} For every \hyperref[def:fol_theory/morphism]{theory morphism} \( t: (\Sigma, T) \to (\Sigma', T') \), if \( \mscrY \) is a model of \( T' \), its \hyperref[def:fol_reduct_along_morphism]{reduct} \( \red_t(\mscrY) \) is a model of \( T \).
...
\begin{proof}
  ...
  \SubProofOf{thm:def:fol_theory_morphism_reduct} Suppose that \( \mscrY \) is a model of \( T' \).
  ...
  The reduct \( \red_t(\mscrY) \) along the \emph{signature morphism} \( t \). Suppose that \( \mscrY \) is a model of \( T' \), i.e. \( \Bracks{\psi}_\mscrY = \semtop \) for every formula \( \psi \) in \( T' \). In particular, \( \Bracks{\varphi[t]}_\mscrY = \semtop \) for every \( \varphi \) in \( T \).
...
\begin{definition}\label{def:fol_theory_model_functor}\mimprovised
  The \hyperref[def:fol_institution]{first-order institution} has a \hyperref[def:fol_institution/models]{model functor} \( \cat{Mod}: \cat{Sign} \to \cat{Cat}^\oppos \) which sends each \hyperref[def:fol_signature]{signature} \( \Sigma \) to the category \( \cat{Mod}(\Sigma) \) of all \hyperref[def:small_set]{small} \hyperref[def:fol_structure]{structures} and \hyperref[def:fol_homomorphism]{homomorphisms} over \( \Sigma \), and each \hyperref[def:fol_signature_category/morphisms]{signature morphism} \( t: \Sigma \to \Sigma' \) to the corresponding \hyperref[def:fol_institution/models/hom]{reduct functor} \( \cat{Mod}(t): \cat{Mod}(\Sigma') \to \cat{Mod}(\Sigma) \).

  We can extend \( \cat{Mod} \) to a functor \( \cat{Mod}: \cat{Th} \to \cat{Cat}^\oppos \) on the corresponding \hyperref[def:fol_theory/category]{category of small theories}:
  \begin{thmenum}
    \thmitem{def:fol_theory_model_functor/obj} We send a theory \( T \) to the \hyperref[def:subcategory/full]{\emph{full} subcategory} of \( \cat{Mod}(\Sigma) \) of all structures in which \( T \) is \hyperref[def:fol_semantics/model]{valid}.

    We denote it by \( \cat{Mod}(T) \) and refer to it as the \term{category of small models of \( T \)}.
    ...
    The reduct functor only depends on the underlying signature morphism, but, as shown in \cref{thm:def:fol_theory_morphism_reduct}, whether the reduct is a model of \( T \) generally depends on \( T' \).
  \end{thmenum}
\end{definition}
\begin{comments}
  \item For established theories, instead of \( \cat{Mod}(T) \), we introduce specific names. For example, we use \( \cat{Grp} \) for the \hyperref[def:group/category]{category of small groups} or \( \cat{Lat} \) for \hyperref[def:lattice/theory]{category of small lattices}.

  \item This definition is based on the discussion in \cite[105]{GoguenBurstall1992Institutions}.
\end{comments}
...
\end{proposition}
\begin{proof}
  \SubProofOf{thm:def:fol_theory_model_functor/reduct_faithful} Fix two models \( \mscrX = (X, I) \) and \( \mscrY = (Y, J) \) of \( T' \), and consider their reducts \( \red_t(\mscrX) = (X, I_t) \) and \( \red_t(\mscrY) = (Y, J_t) \). \Cref{thm:def:fol_theory_morphism_reduct} implies that they are models of \( T \).
...
\begin{definition}\label{def:fol_theory_forgetful_functor}\mimprovised
  The \hyperref[def:fol_theory_model_functor/obj]{theory model functor} \( \cat{Mod}: \cat{Th} \to \cat{Cat}^\oppos \) induces many \hyperref[con:forgetful_functor]{forgetful functors}.
...
\begin{example}\label{ex:def:fol_theory_model_functor}
  ...
  \begin{thmenum}
    \thmitem{ex:def:fol_theory_model_functor/order} The \hyperref[def:partially_ordered_set/theory]{partially ordered sets} extend the \hyperref[def:partially_ordered_set/theory]{theory of preordered sets} with the antisymmetry condition \eqref{eq:def:binary_relation/antisymmetric}.
    ...
    On the other hand, the \hyperref[def:lattice/theory]{theory of lattices} introduces new function symbols and only reestablishes the familiar inequalities via \hyperref[def:fol_definitional_extension]{definitional extensions}.
...
\begin{definition}\label{def:fol_monomorphism}\mimprovised
  Consider some \hyperref[def:fol_theory_model_functor/obj]{category of models} \( \cat{Mod}(T) \). Based on the categorical terminology from \cref{def:morphism_cancellation/left}, we say that the homomorphism \( h: \mscrX \to \mscrY \) is a \term{monomorphism} if it is left cancellable, i.e. if \( h \bincirc g_1 = h \bincirc g_2 \) implies that \( g_1 = g_2 \).

  We say that \( h \) \term{splits} if it has a \hyperref[def:morphism_invertibility/left]{left inverse}, i.e. a homomorphism \( g: \mscrY \to \mscrX \) such that \( g \bincirc h = \id_\mscrX \).
\end{definition}
\begin{comments}
  \item For the etymology of the term \enquote{monomorphism}, see \cref{rem:morphism_etymology}.
\end{comments}
\begin{comments}
  \item If \( h \) has a left inverse, \cref{thm:def:morphism_invertibility/split} implies that it automatically follows that it is a monomorphism.
...
\begin{remark}\label{rem:def:fol_monomorphism}
  ...
  \begin{itemize}
    \item \( U(h) \) is left cancellable (i.e. a categorical monomorphism) in \( \cat{Set} \).
...
\begin{proposition}\label{thm:def:fol_monomorphisms}
  ...
  \begin{thmenum}
    \thmitem{thm:def:fol_monomorphisms/monomorphism} Every \hyperref[def:fol_embedding]{embedding} is a monomorphism (left cancellable homomorphism).
...
\begin{proof}
  ...
  \begin{equation*}
    ...
    I(p)(a_1, \ldots, a_n),
  \end{equation*}
  so the homomorphism is strong. Thus, it is an embedding because it satisfies \cref{def:fol_embedding/direct}.
\end{proof}
...
\begin{definition}\label{def:categorical_subobject}\mcite[18]{Jacobson1989BasicAlgebraII}
  In some \hyperref[def:category]{category}, define an equivalence relation on \hyperref[def:morphism_cancellation/left]{monomorphisms} \( f: B \to A \) and \( g: C \to A \) with a common codomain \( A \) to hold if there exists an \hyperref[def:morphism_invertibility/isomorphism]{isomorphism} \( h: B \to C \) such that \( f = g \bincirc h \).

  We call the equivalence classes of this relation \term[ru=подобъект (\cite[24]{ЦаленкоШульгейфер1974ОсновыТеорииКатегорий})]{subobjects} of \( A \).
\end{definition}
\begin{comments}
  \item The definition is also discussed in \cite{MathSE:categorical_subobject_definition}.

  \item Due to \cref{thm:fol_categorical_subobjects} and analogous results, we will mostly regard subobjects as concrete canonical objects with an inclusion map.
...
\begin{proposition}\label{thm:categorical_subobjects_in_set}
  Consider the \hyperref[def:category_of_small_sets]{category of small sets} \( \cat{Set} \).

  If \( B \) and \( C \) are \hyperref[def:subset]{subsets} of \( A \), the inclusions \( \iota_B: B \to A \) and \( \iota_C: C \to A \) induce the same \hyperref[def:categorical_subobject]{categorical subobject} of \( A \) in \( \cat{Set} \) if and only if \( B = C \).
\end{proposition}
\begin{comments}
  \item An analogous statement holds for \hyperref[def:categorical_quotient_object]{quotient objects} --- see \cref{thm:categorical_quotient_objects_in_set}.
\end{comments}
\begin{proof}
  \SufficiencySubProof Suppose that \( \iota_B: B \to A \) and \( \iota_C: C \to A \) are equivalent, i.e. there exists some bijection \( f: B \to C \) such that \( \iota_B = \iota_C \bincirc f \).

  Both \( \iota_B \) and \( \iota_C \) are monomorphisms by \cref{thm:function_invertibility_categorical/left_cancellable}, so they induce subobjects.

  For every \( b \) in \( B \), we have \( b = \iota_B(b) = \iota_C(f(b)) = f(b) \), which in particular implies that \( B \) is a subset of \( C \).

  Using \( f^{-1} \), we similarly conclude that \( C \) is a subset of \( B \), and hence that they are equal.

  \NecessitySubProof Conversely, suppose that \( B = C \). Then \( \iota_B = \iota_C \), so obviously they correspond to the same subobject.
\end{proof}
...
\begin{proposition}\label{thm:fol_categorical_subobjects}
  ...
  If \( \mscrY = (Y, J) \) and \( \mscrZ = (Z, K) \) are \hyperref[def:fol_substructure/submodel]{submodels} of \( \mscrX = (X, I) \), the inclusions \( \iota_Y: Y \to X \) and \( \iota_Z: Z \to X \) induce the same \hyperref[def:categorical_subobject]{categorical subobject} of \( \mscrX \) in \( \cat{Mod}(T) \) if and only if \( Y = Z \).
\end{proposition}
\begin{comments}
...
\end{comments}
\begin{proof}
  We can proceed as in \cref{thm:categorical_subobjects_in_set}, but justifying that \( \iota_Y \) and \( \iota_Z \) are monomorphisms is done by \cref{thm:def:fol_monomorphisms/monomorphism}.
\end{proof}

\paragraph{Quotient objects}

\begin{definition}\label{def:fol_epimorphism}\mimprovised
  Consider some \hyperref[def:fol_theory_model_functor/obj]{category of models} \( \cat{Mod}(T) \). Based on the categorical terminology from \cref{def:morphism_cancellation/right}, we say that the homomorphism \( h: \mscrX \to \mscrY \) is an \term{epimorphism} if it is right cancellable, i.e. if \( g_1 \bincirc h = g_2 \bincirc h \) implies that \( g_1 = g_2 \).

  We say that \( h \) \term{splits} if it has a \hyperref[def:morphism_invertibility/right]{right inverse}, i.e. a homomorphism \( g: \mscrY \to \mscrX \) such that \( h \bincirc g = \id_\mscrY \).
\end{definition}
\begin{comments}
  \item For the etymology of the term \enquote{epimorphism}, see \cref{rem:morphism_etymology}.
\end{comments}
\begin{comments}
  \item If \( h \) has a right inverse, \cref{thm:def:morphism_invertibility/split} implies that it automatically follows that it is an epimorphism.
...
\begin{definition}\label{def:categorical_quotient_object}\mcite[18]{Jacobson1989BasicAlgebraII}
  In some \hyperref[def:category]{category}, define an equivalence relation on \hyperref[def:morphism_cancellation/right]{epimorphisms} \( f: A \to B \) and \( g: A \to C \) with a common domain \( A \) to hold if there exists an \hyperref[def:morphism_invertibility/isomorphism]{isomorphism} \( h: B \to C \) such that \( f = h \bincirc g \).

  We call the equivalence classes of this relation \term[ru=факторобъект (\cite[26]{ЦаленкоШульгейфер1974ОсновыТеорииКатегорий})]{quotient objects} of \( A \).
\end{definition}
\begin{comments}
...
\begin{proposition}\label{thm:categorical_quotient_objects_in_set}
  Consider the \hyperref[def:category_of_small_sets]{category of small sets} \( \cat{Set} \).

  If \( B \) and \( C \) are \hyperref[def:set_partition]{partitions} of \( A \), the projections \( \pi_B: A \to B \) and \( \pi_C: A \to C \) induce the same \hyperref[def:categorical_quotient_object]{categorical quotient objects} of \( A \) in \( \cat{Set} \) if and only if \( B = C \).
\end{proposition}
\begin{comments}
  \item An analogous statement holds for \hyperref[def:categorical_subobject]{subobjects} --- see \cref{thm:categorical_subobjects_in_set}.
\end{comments}
\begin{proof}
  \SufficiencySubProof Suppose that \( \pi_B: A \to B \) and \( \pi_C: A \to C \) are equivalent, i.e. there exists some bijection \( f: B \to C \) such that \( \pi_B = \pi_C \bincirc f \).

  Both \( \pi_B \) and \( \pi_C \) are epimorphisms by \cref{thm:categorical_quotient_objects_in_set}, so they induce quotient objects.

  If \( \pi_B(a) = \pi_B(a') \), then
  \begin{equation*}
    \pi_C(a) = f^{-1}(\pi_B(a)) = f^{-1}(\pi_B(a')) = \pi_C(a').
  \end{equation*}

  The converse can be shown analogously. Therefore, \( \pi_B(a) = \pi_B(a') \) if and only if \( \pi_C(a) = \pi_C(a') \).

  Since both \( B \) and \( C \) are partitions of \( A \), it follows that they are equal.

  \NecessitySubProof Conversely, suppose that \( B = C \). Then \( \pi_B = \pi_C \), so obviously they correspond to the same quotient object.
\end{proof}
...
\end{comments}
\begin{proof}
  We can proceed as in \cref{thm:categorical_quotient_objects_in_set}, but justifying that \( \pi_Y \) and \( \pi_Z \) are epimorphism is done by \cref{thm:fol_quotient_map_is_epimorphism}.
\end{proof}

\begin{definition}\label{def:simple_object}\mimprovised
  In a \hyperref[def:category]{category} with a \hyperref[def:universal_objects/zero]{zero object} \( Z \), we say that an object \( A \) is \term{simple} if it is not itself a zero object and if every \hyperref[def:categorical_quotient_object]{quotient object} of \( A \) is equivalent to \( 0_{Z,A}: Z \to A \) or \( \id: A \to A \).
\end{definition}
\begin{comments}
  \item We adapt this definition to groups in \cref{def:simple_group}, to rings in \cref{def:ring/simple}, to \( R \)-modules in \cref{def:module/simple} and to \( R \)-algebras in \cref{def:algebra_over_ring/simple}.

  In fact, this definition arises as a generalization of the above cases.
\end{comments}
...
\begin{definition}\label{def:fol_function_respect}\mcite[def. 2.3.4]{Hinman2005Logic}
  ...
  \begin{thmenum}
    ...
    This is a strengthening of what we require for \emph{strong} homomorphisms.
  \end{thmenum}
\end{definition}
...
\begin{proposition}\label{thm:def:fol_function_respect}
  ...
  \begin{thmenum}
    \thmitem{thm:def:fol_function_respect/atomic} A function \( h: X \to Y \) is a \hyperref[def:fol_homomorphism]{strong homomorphism} if and only if it respects \emph{atomic} formulas.
  \end{thmenum}
\end{proposition}
\begin{proof}
...
\begin{comments}
  ...
  \item A useful characterization of elementary substructures is \fullref{thm:tarski_vaugh_criterion}.
\end{comments}
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \begin{equation*}
      ...
      \bigwedge_{a \in X} \Bracks{\psi}_\mscrX^{v_{x \mapsto a}}
      \reloset {\T{ind.}} =
      \bigwedge_{a \in X} \Bracks{\psi}_\mscrY^{h \bincirc v_{x \mapsto a}}
      =
      \bigwedge_{b \in Y} \Bracks{\psi}_\mscrY^{(h \bincirc v)_{x \mapsto b}},
    \end{equation*}
...
\begin{example}\label{ex:rem:fol_homomorphism_refinements}
  ...
  \begin{thmenum}
    ...
    Consider the constant function \( f(x) = 0 \) in \( \BbbR \). It is trivially an \hyperref[def:order_function/preserving]{order-preserving map} and hence an \hyperref[def:partially_ordered_set/morphism]{order homomorphism}, i.e. a first-order homomorphism in the \hyperref[def:partially_ordered_set/theory]{theory of preordered sets}. Yet, it is not a strong homomorphism because \( f(1) \leq f(0) \).
    ...
    \begin{figure}
      ...
      \caption{A non-injective \hyperref[def:directed_graph/morphism]{strong graph homomorphism}.}\label{fig:ex:rem:fol_homomorphism_refinements/not_embedding}
    \end{figure}
    ...
    \thmitem{ex:rem:fol_homomorphism_refinements/not_isomorphism} \Fullref{thm:downward_lowenheim_skolem_theorem} and \fullref{thm:upward_lowenheim_skolem_theorem} allow constructing examples of elementary substructures and extensions, which are not isomorphic due to cardinality considerations.

    Concrete examples of nonstandard models of \logic{PA} and \logic{ZFC} are discussed in \cref{ex:lowenheim_skolem}.
  \end{thmenum}
\end{example}
...
\begin{definition}\label{def:elementary_equivalence}
  ...
  \begin{thmenum}
    \thmitem{def:elementary_equivalence/direct}\mcite[def. 2.3.3]{Hinman2005Logic} \( \mscrX \vDash \varphi \) if and only if \( \mscrY \vDash \varphi \) for every \emph{closed} formula \( \varphi \) over \( \Sigma \).
...
\begin{comments}
  ...
  \item These formulas are implemented in \cite{notebook:code}, in the module
  \begin{CodeRefDisplay}
    \ManualCodeRef{logic.theories.equality}
  \end{CodeRefDisplay}
\end{comments}
\begin{proof}
  Trivial.
...
\begin{lemma}\label{thm:fol_finite_structure_characterizing_formula}\mcite[thm. 2.3.11(ii)]{Hinman2005Logic}
  Fix a finite structure \( \mscrX = (X, I) \) over \( \Sigma \).
...
\paragraph{Categoricity}

\begin{proposition}\label{thm:fol_categorical_theories}
  \hyperref[def:fol_theory]{Categorical theories} have the following basic properties:
  \begin{thmenum}
    \thmitem{thm:fol_categorical_theories/unsatisfiable} An \hyperref[def:fol_semantics/satisfaction]{unsatisfiable} theory is vacuously categorical, and \( \kappa \)-categorical for any \( \kappa \).

    \thmitem{thm:fol_categorical_theories/complete} Every categorical (semantic) theory is \hyperref[def:fol_theory/complete]{complete}.

    \thmitem{thm:fol_categorical_theories/finite}\mcite[thm. 2.5.3(i)]{Hinman2005Logic} If a complete theory has a finite model, it is categorical, and \( \kappa \)-categorical for any \( \kappa \).
  \end{thmenum}
\end{proposition}
\begin{proof}
  \SubProofOf{thm:fol_categorical_theories/unsatisfiable} Trivial.

  \SubProofOf{thm:fol_categorical_theories/complete} Suppose that the theory \( T \) is categorical.

  Suppose that it is not complete, and let \( \varphi \) be a sentence \hyperref[def:logical_independence]{independent} from \( T \).

  Then there exist a model \( \mscrX \) satisfying \( \varphi \) and a model \( \mscrY \) not satisfying \( \varphi \).

  Since \( T \) is categorical, it follows that \( \mscrX \) and \( \mscrY \) are isomorphic. \Cref{thm:def:elementary_equivalence/isomorphic} implies that they are \hyperref[def:elementary_equivalence]{elementarily equivalent}. But this contradicts the assumption that only one of the models validates \( \varphi \).

  It remains for \( T \) to be complete.

  \SubProofOf{thm:fol_categorical_theories/finite} Follows from \cref{thm:fol_finite_structure_characterizing_formula}.
\end{proof}

\paragraph{Definitional extensions}

\begin{concept}\label{con:primitive_notion}
  In a sufficiently complex \hyperref[con:metalogic]{object theory}, different notions can be defined via each other. If we want to use \hyperref[def:well_founded_relation]{well-founded} notions that are not defined circularly via each other, we must choose a subset of them that should to be characterized via axioms, and then use them to define the rest. Following \textcite[28]{Kleene1971Metamathematics}, we will refer call these notions as \term{primitive}.

  For instance, when formalizing \hyperref[def:lattice]{lattices}, we may use an object theory based on the operations \( {\latvee} \) and \( {\latwedge} \), as we have done in \cref{def:lattice/theory}, or we may use an object theory based on one \hyperref[def:partially_ordered_set]{partial order relation} \( {\synleq} \), as we have done in \cref{thm:lattice_via_order_relation}.

  The object language will feature all three symbols anyway. If we use the formalization from \cref{def:lattice/theory}, however, we will specify the behavior of \( {\latvee} \) and \( {\latwedge} \) via axioms, and use their properties to determine the behavior of \( {\synleq} \). Conversely, \cref{thm:lattice_via_order_relation} shows how we can take \( {\synleq} \) as primitive and use it to specify how \( {\latvee} \) and \( {\latwedge} \) behave.
...
\end{concept}
\begin{comments}
  \item In addition to the adjective \enquote{primitive}, \textcite[28]{Kleene1971Metamathematics} suggests \enquote{technical} and \enquote{undefined}. We will avoid the latter term because it would conflict with undefinedness as described in \cref{con:undefinedness}. See \cref{rem:undefined_and_primitive_terms} for disambiguation of the different notions of undefinedness.

  \item We avoid introducing a distinct term for symbols whose behavior is defined via others. \Textcite[28]{Kleene1971Metamathematics} suggests \enquote{ordinary}, \enquote{logical} and \enquote{defined}.
\end{comments}
...
\begin{definition}\label{def:fol_definition}\mcite[def. 2.6.11]{Hinman2005Logic}
  ...
  \begin{thmenum}
    \thmitem{def:fol_definition/predicate} Consider some symbol \( p \) not in \( \Sigma \) and some formula \( \delta_p \) whose free variables are among \( x_1, \ldots, x_n \).
    ...
    \begin{equation}\label{eq:def:fol_definition/predicate}
      \qforall {x_1} \cdots \qforall {x_n} (p(x_1, \ldots, x_n) \syniff \delta_p).
    \end{equation}

    We call \eqref{eq:def:fol_definition/predicate} a \term{definition} of \( p \) in \( T \).

    \thmitem{def:fol_definition/function} Consider some symbol \( f \) not in \( \Sigma \) and some formula \( \delta_f \) whose free variables are among \( x_1, \ldots, x_n \), and such that \( T \) contains
    \begin{equation}\label{eq:def:fol_definition/function/condition}
      \qforall {x_1} \cdots \qforall {x_n} \qExists y \delta_f,
    \end{equation}
    ...
    \begin{equation}\label{eq:def:fol_definition/function}
      \qforall {x_1} \cdots \qforall {x_n} \qforall y (f(x_1, \ldots, x_n) \syneq y \syniff \delta_f).
    \end{equation}
...
\begin{comments}
  ...
  \item In \hyperref[def:fol_equational_theory]{equational theories}, function definitions can be simplified --- see \cref{thm:fol_definitional_extension_equation}.
\end{comments}

\begin{proposition}\label{thm:fol_definitional_extension_equation}
  If \( f \) is an \( n \)-ary \hyperref[def:fol_signature/function]{function symbol} not in the signature \( \Sigma \), then for any \hyperref[def:fol_term]{term} \( \tau \) over \( \Sigma \) whose free variables are among \( x_1, \ldots, x_n \), the formula
  \begin{equation}\label{eq:thm:fol_definitional_extension_equation}
    f(x_1, \ldots, x_n) \syneq \tau
  \end{equation}
  gives rise to the \hyperref[def:fol_definition/function]{function definition formula} \( \delta_f = (y \syneq \tau) \).
\end{proposition}
\begin{comments}
  \item This proposition precludes the need to verify \eqref{eq:def:fol_definition/function/condition}, thus enabling us to introduce new function symbols as \hyperref[con:syntactic_abbreviation]{notational shorthands} for more complicated terms.
\end{comments}
\begin{proof}
  The verification of \eqref{eq:def:fol_definition/function/condition} is straightforward. Indeed,
  \begin{equation*}
    \qforall {x_1} \cdots \qforall {x_n} \qExists y (y \syneq \tau)
  \end{equation*}
  expands as
  \begin{equation*}
    \qforall {x_1} \cdots \qforall {x_n} \qexists y (y \syneq \tau \synwedge \qforall z (z \syneq \tau \synimplies z \syneq y)).
  \end{equation*}

  Semantically, this follows from \cref{thm:fol_equality_characterization}, while syntactically this results from applying \ref{inf:def:fol_natural_deduction/equality/elim}.
\end{proof}
...
\begin{definition}\label{def:fol_definitional_structure_expansion}\mimprovised
  ...
  \begin{thmenum}
    ...
    \begin{equation*}
      I^+(p)(a_1, \ldots, a_n) \coloneqq \Bracks{\delta_p}_\mscrX(a_1, \ldots, a_n).
    \end{equation*}

    \thmitem{def:fol_definitional_structure_expansion/function} For each new \( n \)-ary function symbol \( f \), we define \( I^+(p)(a_1, \ldots, a_n) \) as the unique value \( y \) for which \( \Bracks{\delta_p}_\mscrX(a_1, \ldots, a_n, y) = \semtop \).
...
\begin{remark}\label{rem:definitional_extensions_and_skolemization}
  Consider the first-order \hyperref[def:zfc]{Zermelo-Fraenkel set theory with choice}.

  A common operation for two sets is their union. We define intersection in \cref{def:basic_set_operations/union}, and show in \cref{thm:zfc_existence_theorems/binary_union} that the definition is correct.

  To ensure correctness, we must prove
  \begin{equation*}
    \qforall \syna \qforall \synb \qExists \sync \underbrace{\qforall \synx (\synx \synin \sync \syniff (\synx \synin \syna \synvee \synx \synin \syna))}_{\delta_U},
  \end{equation*}
  where we have used the unique existence shorthand from \cref{rem:fol_exists_unique_abbreviation}. Due to the \hyperref[def:naive_set_theory/extensionality]{axiom of extensionality}, this is equivalent to the same formula with ordinary existence:
  \begin{equation*}
    \qforall \syna \qforall \synb \qexists \sync \delta_U.
  \end{equation*}

  Anyhow, this allows us to introduce a binary function symbol \( \synU \) such that
  \begin{equation}\label{eq:ex:definitional_extensions_and_skolemization/definition}
    \qforall \syna \qforall \synb \qexists \sync (\sync \syneq \synU(\syna, \synb) \syniff \delta_U),
  \end{equation}

  This symbol \( \synU \) fits the conditions from \cref{def:fol_definition/function}, so it is a first-order definition. This allows us to construct a \hyperref[def:fol_definitional_extension]{definitional extension} by adding \eqref{eq:ex:definitional_extensions_and_skolemization/definition} as an axiom. In this extension, we can freely use \( \synU \) as an operation and not worry about unwanted consequences because, as shown in \cref{thm:fol_definitional_extension_conservative}, this extension is \hyperref[def:fol_theory/conservative]{conservative}.

  To show that \( A \cup A = A \), what we would write as
  \begin{equation*}
    \qforall \syna \qforall \sync (\delta_U[\syna \mapsto \syna, \synb \mapsto \syna, \sync \mapsto \sync] \synimplies \sync \syneq \syna)
  \end{equation*}
  becomes
  \begin{equation*}
    \qforall \syna (\synU(\syna, \syna) \syneq \syna).
  \end{equation*}

  Without uniqueness concerns, a similar effect can be achieved via \fullref{alg:fol_skolemization}, where
  \begin{equation*}
    \qforall \syna \qforall \synb \qexists \sync \qforall \synx (\synx \synin \sync \syniff (\synx \synin \syna \synvee \synx \synin \syna)),
  \end{equation*}
  becomes
  \begin{equation*}
    \qforall \syna \qforall \synb \qforall \synx (\synx \synin \synU(\syna, \synb) \syniff (\synx \synin \syna \synvee \synx \synin \syna)).
  \end{equation*}
\end{remark}
...
\begin{example}\label{ex:def:fol_definitional_extension}
  ...
  \begin{thmenum}
    ...
    This is discussed in more detail in \cref{rem:definitional_extensions_and_skolemization}.

    \thmitem{ex:def:fol_definitional_extension/peano} In \fullref{sec:peano_arithmetic}, we define the inequality \( {\synleq} \) by an equation involving only \( {\synplus} \).

    \thmitem{ex:def:fol_definitional_extension/orders} In the \hyperref[def:partially_ordered_set/theory]{theory of preordered sets}, we start with the relation \( {\synleq} \) and add \( {\synless} \), \( {\syngeq} \) and \( {\syngreater} \) in definitional extensions.

    In the \hyperref[def:lattice/theory]{theory of lattices}, we start with \( {\synvee} \) and \( {\synwedge} \) and then introduce \( {\synleq} \) in a definitional extension. The other order relations can then be stacked on top.
  \end{thmenum}
\end{example}
...
\begin{definition}\label{def:fol_definability}\mcite[def. 2.3.37]{Hinman2005Logic}
  In a \hyperref[def:fol_structure]{first-order structure} \( \mscrX = (X, I) \), we say that the subset \( A \) of \( X^n \) is \term{definable} with \term{parameters} \( b_1, \ldots, b_m \) if there exists a formula \( \delta_A \) with at most \( n + m \) free variables such that
  \begin{equation*}
    A = \set[\big]{ (a_1, \ldots, a_n) \in X^n \given* \Bracks{\delta_A}_\mscrX^v(a_1, \ldots, a_n, b_1, \ldots, b_m) = \semtop }.
  \end{equation*}
...
\begin{proposition}\label{thm:fol_definability_and_definitions}
  ...
  \begin{thmenum}
    \thmitem{thm:fol_definability_and_definitions/function} If the formula \( \delta_f \) is used for a \hyperref[def:fol_definition/function]{function definition} of the symbol \( f \) in a \hyperref[def:fol_definitional_extension]{definitional extension} \( T^+ \) of \( T \), and if \( \mscrX \) is a model of \( T \), then the interpretation \( I^+(f) \) in the \hyperref[def:fol_definitional_structure_expansion]{definitional expansion} \( \mscrX^+ \) is \hyperref[def:fol_definability/function]{definable} by \( \delta_f \) in \( \mscrX \).

    \thmitem{thm:fol_definability_and_definitions/predicates_extension} Similarly, if \( \delta_p \) is used for a \hyperref[def:fol_definition/predicate]{predicate definition} of the symbol \( p \) in a \hyperref[def:fol_definitional_extension]{definitional extension} \( T^+ \) of \( T \), and if \( \mscrX \) is a model of \( T \), then the interpretation \( I^+(p) \) in the definitional expansion of \( \mscrX^+ \) is the \hyperref[def:subset_characteristic_function]{\emph{characteristic function}} of the set \hyperref[def:fol_definability/function]{definable} by \( \delta_p \) in \( \mscrX \).
  \end{thmenum}
\end{proposition}
\begin{proof}
  Straightforward.
...
\begin{proposition}\label{thm:fol_definable_set_automorphism}\mcite[corr. 2.3.40]{Hinman2005Logic}
  \hyperref[def:fol_definability]{Definable sets} are \hyperref[def:invariant_subset]{invariant} under \hyperref[def:categorical_endomorphism]{automorphisms}.
\end{proposition}
\begin{proof}
  In the structure \( \mscrX = (X, I) \), suppose that the set \( A \) is definable by \( \delta_A \).

  Fix an automorphism \( h: \mscrX \to \mscrX \). \Cref{thm:def:elementary_embedding/isomorphism} implies that \( h \) is an elementary embedding, so \( h \) \hyperref[def:fol_function_respect/formula]{respects} \( \delta_A \).

  Therefore, \( h[A] \) is definable by \( \delta_A \).
\end{proof}
...
\begin{definition}\label{def:fol_prenex_normal_form}\mcite[def. 2.2.33]{Hinman2005Logic}
  ...
  We call the \emph{formula} \( \psi \) the \term[ru=матрица (\cite[def. 2.5.1]{Герасимов2014Вычислимость})]{matrix} of \( \varphi \), and the \emph{string} \( \quantifier {Q_1} {x_1} \ldots {Q_n} {x_n} \) --- the \term[en=prefix (\cite[def. 2.2.33]{Hinman2005Logic})]{quantifier prefix} of \( \varphi \).

  \begin{thmenum}
    \thmitem{def:fol_prenex_normal_form/universal}\mcite[def. 2.3.22]{Hinman2005Logic} If the prefix of \( \varphi \) contains only universal quantifiers, we call \( \varphi \) a \term{universal formula}.

    \thmitem{def:fol_prenex_normal_form/existential}\mcite[def. 2.3.22]{Hinman2005Logic} Dually, if the prefix of \( \varphi \) contains only existential quantifiers, we call \( \varphi \) an \term{existential formula}.
  \end{thmenum}
\end{definition}

\begin{algorithm}[First-order formula to prenex normal form]\label{alg:fol_formula_to_prenex_normal_form}
  We introduce an operator \( M_{\logic{PNF}} \) on \hyperref[def:fol_formula]{first-order formulas} that \enquote{pulls out} quantifiers in order to produce a \hyperref[def:fol_semantics/equivalence]{semantically equivalent} formula in \hyperref[def:fol_prenex_normal_form]{prenex normal form}.

  We proceed by recursion on \( \varphi \):
  \begin{thmenum}
    \thmitem{alg:fol_formula_to_prenex_normal_form/atomic} If \( \varphi \) is an atomic formula, let \( M_{\logic{PNF}}(\varphi) \coloneqq \varphi \).

    \thmitem{alg:fol_formula_to_prenex_normal_form/negation} If \( \varphi = \synneg \psi \), let \( \psi' \coloneqq M_{\logic{PNF}}(\psi) \) and let
    \begin{equation*}
      M_{\logic{PNF}}(\varphi) \coloneqq \begin{cases}
        \quantifier {Q^\oppos} x M_{\logic{DNE}}(M_{\logic{PNF}}(\synneg \theta)), &\psi' = \quantifier Q x \theta, Q \in \op*{Quant}, \\
        \synneg \psi',                                                               &\T{otherwise.}
      \end{cases}
    \end{equation*}

    \thmitem{alg:fol_formula_to_prenex_normal_form/connectives} If \( \varphi = \psi \bincirc \theta \), let \( \psi' \coloneqq M_{\logic{PNF}}(\psi) \) and \( \theta' \coloneqq M_{\logic{PNF}}(\theta) \), and let
    \small
    \begin{equation*}
      M_{\logic{PNF}}(\varphi) \coloneqq \begin{cases}
        M_{\logic{PNF}}\parens[\big]{ (\psi' \synimplies \theta') \wedge (\theta' \synimplies \psi') }, &{\syncirc} \T{is} {\syniff} \T{and} \psi' \T{or} \theta' \T{have quantifiers}, \\
        \quantifier {Q^\oppos} n M_{\logic{PNF}}(\omega[x \mapsto n] \synimplies \theta'),            &{\syncirc} \T{is} {\synimplies} \T{and} \psi' = \quantifier Q x \omega \T{and} n = \op*{FVar}(\theta', x), \\
        \quantifier Q n M_{\logic{PNF}}(\omega[x \mapsto n] \synimplies \theta'),                       &\T{otherwise if} \psi' = \quantifier Q x \omega \T{and} n = \op*{FVar}(\theta', x), \\
        \quantifier Q n M_{\logic{PNF}}(\psi' \synimplies \omega[x \mapsto n]),                         &\T{otherwise if} \theta' = \quantifier Q x \omega \T{and} n = \op*{FVar}(\psi', x), \\
    ...
    \begin{equation*}
      M_{\logic{PNF}}(\varphi) \coloneqq \quantifier Q x M_{\logic{PNF}}(\psi).
    \end{equation*}
  \end{thmenum}
\end{algorithm}
\begin{comments}
  ...
  \begin{CodeRefDisplay}
    \GetCodeRef{alg:fol_formula_to_prenex_normal_form}
  \end{CodeRefDisplay}

  The bulk of the implementation is in the module
  \begin{CodeRefDisplay}
    \ManualCodeRef{logic.transformation.pull_quantifiers}
  \end{CodeRefDisplay}
\end{comments}
\begin{defproof}
  ...
  Semantic equivalence of \( \varphi \) and \( M_{\logic{PNF}}(\varphi) \) is also simple to show by induction.
  \begin{itemize}
    \item The case \ref{alg:fol_formula_to_prenex_normal_form/atomic} of atomic formulas is trivial.

    \item If \( \varphi \) is a negation formula, the construction in \ref{alg:fol_formula_to_prenex_normal_form/negation} uses \fullref{alg:propositional_repeated_negation_collapse} to remove repeated negation, and overall semantic equivalence of \( \varphi \) and \( M_{\logic{PNF}}(\varphi) \) follows from the correctness of \fullref{alg:fol_formula_dualization}.

    \item If \( \varphi \) is a connective formula, come additional commentary is needed.

    Semantic equivalence of \( \varphi \) and \( M_{\logic{PNF}}(\varphi) \) follows \cref{thm:intuitionistic_equivalences/biconditional_via_conditionals} in the first case. In the other cases, we take care to rename the variables that may cause collisions; otherwise, equivalence follows from distributivity of joins and meets in the classical truth value algebra. In the second case, we must additionally note that \( \psi' \synimplies \theta' \) is equivalent to \( \synneg \psi' \synvee \theta' \), as shown in \cref{thm:classical_equivalences/conditional_as_disjunction}.

    \item The case \ref{alg:fol_formula_to_prenex_normal_form/quantifiers} of quantifier formulas is also trivial.
  \end{itemize}
\end{defproof}

\begin{proposition}\label{thm:fol_theory_pnf}
  Every \hyperref[def:fol_theory]{first-order theory} has an axiomatization consisting of formulas in \hyperref[def:fol_prenex_normal_form]{prenex normal form}.
\end{proposition}
\begin{proof}
  Take any axiomatization and apply \fullref{alg:fol_formula_to_prenex_normal_form} to the formulas.
\end{proof}

\begin{algorithm}[Skolemization]\label{alg:fol_skolemization}
  We introduce an operator \( M_{\logic{SNF}} \) on \hyperref[def:fol_formula]{first-order formulas} in \hyperref[def:fol_prenex_normal_form]{prenex normal form} that eliminates existential quantifiers, producing a \hyperref[def:fol_prenex_normal_form/universal]{universal formula} over an \hyperref[def:fol_signature_extension]{extended signature}.

  Consider the formula \( \varphi = \quantifier {Q_1} {x_1} \ldots \quantifier {Q_n} {x_n} \psi \) over \( \Sigma \), where \( \psi \) is quantifier-free.
  ...
  \begin{itemize}
    ...
    \item \( \psi_k \) is a formula over \( \Sigma_k \).
    \item \( U_k \) is a \hyperref[def:ordered_tuple]{list} of universally quantified variables in their order of appearance in \( \varphi \).

    If \( x_{i_1}, \ldots, x_{i_m} \) are the variables in \( U_k \), let
    \begin{equation*}
      \varphi_k \coloneqq \qforall {x_{i_1}} \ldots \qforall {x_{i_m}} \quantifier {Q_{k+1}} {x_{k+1}} \ldots \quantifier{Q_n} {x_n} \psi_k.
    \end{equation*}

    \item \( \mscrX_k = (X, I_k) \) is an \hyperref[def:fol_structure_expansion]{expansion} of \( \mscrX \) over \( \Sigma_k \) that satisfies \( \varphi_k \).
  \end{itemize}

  At the end, let
  \begin{align*}
    M_{\logic{SNF}}(\varphi, \Sigma) \coloneqq \Sigma_n,
    &&
    M_{\logic{SNF}}(\varphi, \mscrX) \coloneqq \mscrX_n,
    &&
    M_{\logic{SNF}}(\varphi) \coloneqq \varphi_n.
  \end{align*}

  Then \( M_{\logic{SNF}}(\varphi) \) is a universal formula over \( M_{\logic{SNF}}(\varphi, \Sigma) \) that is valid in \( M_{\logic{SNF}}(\varphi, \mscrX) \).

  We proceed as follows:
  \begin{thmenum}
    \thmitem{alg:fol_skolemization/init} Start at \( n = 0 \) with and empty list \( U_0 \) and with
    \begin{align*}
      \Sigma_0 \coloneqq \Sigma,
      &&
      \psi_0 \coloneqq \psi,
      &&
      \mscrX_0 \coloneqq \mscrX.
    \end{align*}

    \thmitem{alg:fol_skolemization/universal} At step \( k > 0 \), if \( Q_k = \synforall \), let
    \begin{align*}
      \Sigma_k \coloneqq \Sigma_{k-1},
      &&
      \psi_k \coloneqq \psi_{k-1},
      &&
      \mscrX_k \coloneqq \mscrX_{k-1}
      &&
      U_k \coloneqq U_{k-1}, x_k.
    \end{align*}
    ...
    The operator \( \sharp_m \) discussed above gives us a new function symbol \( f_k \) of arity \( m \).
    ...
    We call \( f_k \) a \term[en=Skolem function (\cite[def. 3.4.2]{VanDalen2004LogicAndStructure})]{Skolem function symbol}.

    Since \( \mscrX_{k-1} \) satisfies
    \begin{equation*}
      \varphi_{k-1} \coloneqq \qforall {x_{i_1}} \ldots \qforall {x_{i_m}} \qexists {x_k} \underbrace{\quantifier {Q_{k+1}} {x_{k+1}} \ldots \quantifier{Q_n} {x_n} \psi_k}_\theta,
    \end{equation*}
    for every variable assignment \( v \), there exists a nonempty set \( A_v \) of elements of \( X \) such that, for every \( a \) in \( A_v \), the modified assignment \( v_{x_k \mapsto a} \) satisfies \( \theta \).
...
\begin{proposition}\label{thm:skolemization_equisatisfiability}
  A \hyperref[def:fol_closed_formula]{closed formula} \( \varphi \) is \hyperref[def:fol_semantics/model]{satisfiable} if and only if its skolemization \( M_{\logic{SNF}}(\varphi) \), obtained via \fullref{alg:fol_skolemization}, is satisfiable.
\end{proposition}
\begin{proof}
  The skolemization algorithm allows \hyperref[def:fol_structure_expansion]{expanding} a model of \( \varphi \) to a model of \( M_{\logic{SNF}}(\varphi) \).

  Furthermore, this expansion is constructed so that a model of \( M_{\logic{PNF}}(\varphi) \) is also a model of \( \varphi \).
\end{proof}

\begin{theorem}[Skolem's theorem]\label{thm:skolems_theorem}\mcite[corr. 2.6.29]{Hinman2005Logic}
  For every \hyperref[def:fol_theory]{semantic first-order theory}, there exists a \hyperref[def:fol_theory/conservative]{conservative extension} with an axiomatization consisting of \hyperref[def:fol_prenex_normal_form/universal]{universal formulas}.
\end{theorem}
\begin{comments}
  \item The name of the theorem is taken from \cite[56]{Shoenfield1967MathematicalLogic}.
  \item Note that the construction of this theory is done for semantic theories. Syntactic theories require an adaptation --- see \cref{ex:thm:fol_completeness/skolemization}.
\end{comments}
\begin{proof}
  Fix a theory \( T \) over \( \Sigma \). \Cref{thm:fol_theory_pnf} implies that \( T \) has an axiomatization \( \Gamma \) consisting of formulas in \hyperref[def:fol_prenex_normal_form]{prenex normal form}. \Fullref{alg:fol_skolemization} allows us to form the set
  \begin{equation*}
    \Gamma^+ \coloneqq \set{ M_{\logic{SNF}}(\varphi) \given \varphi \in \Gamma }.
  \end{equation*}

  Since \( \Gamma^+ \) is elementwise equisatisfiable with \( \Gamma \), the entire set \( \Gamma^+ \) is equisatisfiable with \( \Gamma \). It remains to show that the consequence closure \( T^+ \) of \( \Gamma^+ \) is conservative over \( T \).

  Let \( \Sigma^+ \) be the extension of \( \Sigma \) containing all new function symbols in \( \Gamma^+ \).

  Fix a formula \( \theta \) from \( T^+ \) over \( \Sigma \). Suppose that \( \Gamma \) has a model. Since \( \Gamma \) is equisatisfiable with \( \Gamma^+ \), it follows that \( \Gamma^+ \) has a model, say \( \mscrX^+ \). Since \( \theta \) is a semantic consequence of \( \Gamma^+ \), it follows that \( \mscrX^+ \) is a model of \( \theta \). But \( \theta \) only contains symbols from \( \Sigma \), so the reduct of \( \mscrX^+ \) to \( \Sigma \) is a model of \( \theta \). It follows that \( \theta \) belongs to \( T \).

  Generalizing on \( \theta \), it follows that \( T^+ \) is conservative over \( T \). Furthermore, \( \Gamma^+ \) is an axiomatization of \( T^+ \) consisting of universal formulas.
\end{proof}
