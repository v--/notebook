...
\end{algorithm}
\begin{comments}
  \item The overall premise of Skolemization is described in \cref{rem:definitional_extensions_and_skolemization} and \fullref{thm:skolems_theorem}.
...
\begin{proposition}\label{thm:skolems_theorem}
  A \hyperref[def:closed_fol_formula]{closed formula} \( \varphi \) is \hyperref[def:fol_semantics/model]{satisfiable} if and only if its skolemization \( M_{\logic{SNF}}(\varphi) \), obtained via \fullref{alg:fol_skolemization}, is satisfiable.
\end{proposition}
\begin{proof}
...
\begin{theorem}[Skolem's theorem]\label{thm:skolems_theorem}\mcite[corr. 2.6.29]{Hinman2005Logic}
  For every \hyperref[def:fol_theory]{first-order theory}, there exists a \hyperref[def:fol_theory/conservative]{conservative extension} with an axiomatization consisting of \hyperref[def:fol_prenex_normal_form/universal]{universal formulas}.
\end{theorem}
\begin{comments}
...
\end{comments}
\begin{proof}
  Fix a theory \( T \) over \( \Sigma \). \Cref{thm:fol_theory_pnf} implies that \( T \) has an axiomatization \( \Gamma \) consisting of formulas in \hyperref[def:fol_prenex_normal_form]{prenex normal form}. Let \( \Phi = \seq{ \varphi_\alpha }_{\alpha < \kappa} \) be a \hyperref[def:set_enumeration]{transfinite enumeration} of \( \Gamma \).

  Via \fullref{thm:bounded_transfinite_recursion}, for each \( \alpha \leq \kappa \) we will build a \hyperref[def:transfinite_sequence]{transfinite sequence} \( \Psi_\alpha = \seq{ \psi_{\alpha,\beta} }_{\beta < \alpha} \) over the signature \( \Sigma_\alpha \) such that, for every \( \beta < \alpha \), \( \psi_{\alpha,\beta} \) is a universal formula \hyperref[def:equisatisfiability]{equisatisfiable} with \( \varphi_\beta \).

  \begin{itemize}
    \item Let \( \Psi_0 \) be the empty sequence, and let \( \Sigma_0 \) be \( \Sigma \).

    \item If \( \alpha = \op{sc}(\gamma) \), let \( \Psi_\alpha \) be
    \begin{equation*}
      \psi_{\alpha,\beta} \coloneqq \begin{cases}
        M_{\logic{SNF}}(\varphi_\gamma), \beta = \gamma \\
        \psi_{\gamma,\beta},             \T{otherwise.}
      \end{cases}
    \end{equation*}

    Let the signature \( \Sigma_\alpha \) be \( M_{\logic{SNF}}(\psi_{\gamma,\gamma}, \Sigma_\gamma) \).

    \Cref{thm:skolems_theorem} implies that \( \psi_{\gamma,\gamma} \) is equisatisfiable with \( \varphi_\gamma \).

    \item If \( \alpha \) is a limit ordinal, let \( \Psi_\alpha \) be the set-theoretic union of \( \Psi_\gamma \) for \( \gamma < \alpha \).

    Let the signature \( \Sigma_\alpha \) be the signature whose symbols are the union of those of \( \Sigma_\gamma \) across \( \gamma < \alpha \).
  \end{itemize}

  Denote by \( \Sigma^+ \) the signature \( \Sigma_\kappa \) and by \( \Gamma^+ \) the set of formulas of \( \Psi_\kappa \). Since \( \Psi_\kappa \) is componentwise equisatisfiable with \( \Phi \), then \( \Gamma \) is equisatisfiable with \( \Gamma^+ \).

  It remains to show that the consequence closure \( T^+ \) of \( \Gamma^+ \) is conservative over \( T \).

  Indeed, fix a formula \( \theta \) from \( T^+ \) over \( \Sigma \). Suppose that \( \Gamma \) has a model. Since \( \Gamma \) is equisatisfiable with \( \Gamma^+ \), it follows that \( \Gamma^+ \) has a model, say \( \mscrX^+ \). Since \( \theta \) is a semantic consequence of \( \Gamma^+ \), it follows that \( \mscrX^+ \) is a model of \( \theta \). But \( \theta \) only contains symbols from \( \Sigma \), so the reduct of \( \mscrX^+ \) to \( \Sigma \) is a model of \( \theta \). It follows that \( \theta \) belongs to \( T \).
