...
\begin{definition}\label{def:fol_theory}\mimprovised
  ...
  \begin{thmenum}
    ...
    \thmitem{def:fol_theory/category} Based on theories and their morphisms, as per \cref{def:category_of_theories}, we have a category of (syntactic or semantic) theories \( \ucat{Th} \) for every \hyperref[def:grothendieci_universe]{Grothendieck universe} \( \mscrU \).
...
\begin{definition}\label{def:fol_theory_model_functor}\mimprovised
  Consider the \hyperref[def:fol_institution]{first-order institution} over the \hyperref[def:grothendieci_universe]{Grothendieck universe} \( \mscrU \).
...
\end{algorithm}
\begin{comments}
  \item This algorithm can be found as \identifier{math.logic.normal_forms.formula_to_pnf} in \cite{notebook:code}. The bulk of the implementation is in the module \identifier{math.logic.transformation.pull_quantifiers}.
\end{comments}
\begin{defproof}
...
\begin{algorithm}[Skolemization]\label{alg:fol_skolemization}
  ...
  At the end, let
  \begin{align*}
    M_{\logic{SNF}}(\Sigma) \coloneqq \Sigma_n,
    &&
    M_{\logic{SNF}}(\mscrX) \coloneqq \mscrX_n,
    &&
    M_{\logic{SNF}}(\varphi) \coloneqq \varphi_n.
  \end{align*}

  Then \( M_{\logic{SNF}}(\varphi) \) is a universal formula over \( M_{\logic{SNF}}(\Sigma) \) that is valid in \( M_{\logic{SNF}}(\mscrX) \).
...
\begin{comments}
  ...
  \item This algorithm can be found as \identifier{math.logic.formula_to_snf} in \cite{notebook:code}.
\end{comments}

\begin{proposition}\label{thm:skolemization_semantic_equivalence}
  A \hyperref[def:closed_fol_formula]{closed formula} \( \varphi \) is \hyperref[def:fol_semantics/model]{satisfiable} if and only if its skolemization \( M_{\logic{PNF}}(\varphi) \), obtained via \fullref{alg:fol_skolemization}, is satisfiable.
\end{proposition}
\begin{proof}
  The skolemization algorithm allows \hyperref[def:fol_structure_expansion]{expanding} a model of \( \varphi \) to a model of \( M_{\logic{PNF}}(\varphi) \).

  We can reverse the construction of this expansion to show that every model of \( M_{\logic{PNF}}(\varphi) \) is also a model of \( \varphi \).
\end{proof}
