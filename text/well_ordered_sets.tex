...
\begin{definition}\label{def:well_founded_relation}\mcite[def. 6.2.14(vii)]{Hinman2005}
...
\begin{definition}\label{def:well_ordered_set}
  ...
  \begin{thmenum}
    \thmitem{def:well_ordered_set/direct}\mcite[32]{Birkhoff1948} Every nonempty subset of \( P \) has a \hyperref[def:extremal_points/maximum_and_minimum]{minimum}.

    \medspace

    \thmitem{def:well_ordered_set/well_founded}\mcite[def. 6.2.14(viii)]{Hinman2005} The strict order \( < \) is \hyperref[def:well_founded_relation]{well-founded}.
  \end{thmenum}
...
\begin{theorem}[Well-founded induction]\label{thm:well_founded_induction}\mcite[33]{Birkhoff1948}
...
\begin{proof}
  ...
  Suppose that there exists a value \( x \in A \) such that \( \Bracks{\varphi}_{v_{\xi \mapsto x}} = F \). That is,
  \begin{equation*}
