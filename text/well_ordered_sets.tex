...
\begin{definition}\label{def:well_ordered_set}
  ...
  \begin{thmenum}
    \thmitem{def:well_ordered_set/direct}\mcite[def. 2.3]{Jech2003} Every nonempty subset of \( P \) has a \hyperref[def:extremal_points/maximum_and_minimum]{minimum}.
...
\begin{lemma}\label{thm:well_ordered_embedding_inflationary}\mcite[lemma 2.4]{Jech2003}
...
\end{proposition}
\begin{comments}
  \item An analogous result holds for partial orders (\fullref{thm:partial_lexicographic_order_is_partial_order}) and total orders (\fullref{thm:total_ordered_lexicographic_order_is_well_ordered}).
\end{comments}
\begin{proof}
