...
\begin{proof}
  ...
  \SubProofOf{thm:def:triangular_matrix/invertible} Follows from \cref{thm:def:triangular_matrix/determinant} and \cref{thm:matrix_invertibility_via_determinants} by noting that \( 0 \) is the only non-invertible element in a field.
\end{proof}
...
\begin{definition}\label{def:elementary_matrix}
  ...
  \begin{thmenum}
    ...
    \end{equation*}
    acts on the \( n \times m \) matrix \( B = \seq{ b_{i,j} }_{i,j=1}^{m,n} \) by permuting the \emph{rows} of \( B \), i.e.
    \begin{equation*}
      P_\sigma B
      =
      \parens*
...
\begin{proof}
  ...
  \SubProofOf{thm:def:elementary_matrix/permutation_determinant} As discussed in our proof of \cref{thm:determinant_on_columns}, the determinant of any permutation of the vectors of the standard basis is the sign of the permutation.
  ...
  \SubProofOf{thm:def:elementary_matrix/transvection_product_same} This proof only requires a slight modification to our proof of \cref{thm:def:elementary_matrix/transvection_product}.
\end{proof}
...
\begin{algorithm}[PLU decomposition]\label{alg:plu_decomposition}
  Fix an \( n \times n \) matrix \( A \). We will build a \hyperref[def:triangular_matrix]{lower \emph{unitriangular} matrix} \( L \), \hyperref[def:upper_row_echelon_form]{upper row echelon form} \( U \) and a \hyperref[def:elementary_matrix/permutation]{permutation matrix} \( P \) such that \( A = PLU \).
...
\begin{proof}
  ...
  \SubProofOf{thm:alg:plu_decomposition/nonsingular} By \cref{thm:def:matrix_determinant/homomorphism},
  \begin{equation*}
...
\begin{algorithm}[Elementary matrix decomposition]\label{alg:elementary_matrix_decomposition}
  Fix a \hyperref[def:inverse_matrix]{nonsingular} \( n \times n \) matrix \( A \). Let \( A = PLU \) be the decomposition obtained via \fullref{alg:plu_decomposition}. By \cref{thm:alg:plu_decomposition/nonsingular}, \( U \) is a nonsingular matrix. Both \( P \) and \( L \) are products of elementary matrices, hence it suffices to show that \( U \) is a product of elementary matrices in order to show that \( A \) is a product of elementary matrices.
...
\begin{corollary}\label{thm:elementary_matrices_generate_gln}
  Over any \hyperref[def:field]{field} \( \BbbK \), the \hyperref[def:elementary_matrix]{elementary matrices} generate the \hyperref[def:linear_groups]{general linear group} \( \grp{GL}_n(\BbbK) \).
\end{corollary}
\begin{proof}
  Follows from \cref{thm:product_of_elementary_matrices_iff_invertible}.
\end{proof}

\begin{corollary}\label{thm:transversion_matrices_generate_sln}
  Over any \hyperref[def:field]{field} \( \BbbK \), the \hyperref[def:elementary_matrix/transvection]{transvection matrices} generate the \hyperref[def:linear_groups]{special linear group} \( \grp{SL}_n(\BbbK) \).
\end{corollary}
\begin{proof}
  \Cref{thm:elementary_matrices_generate_gln} implies that the elementary matrices generate \( \grp{GL}_n(\BbbK) \). \Cref{thm:def:triangular_matrix/determinant} implies that the transvection matrices have determinant \( 1 \), while the nonunit scaling matrices have a different determinant.

  \Cref{thm:def:elementary_matrix/permutation_determinant} implies that the permutation matrices may determinant \( 1 \), however they are redundant as generators because \( P_{i \to j} T_{j \reloset a \to i} = T_{i \reloset a \to j} \).
\end{proof}

\begin{example}\label{ex:gl2_generators}
  Consider the \hyperref[def:linear_groups]{general linear group} \( \grp{GL}_2(\BbbR) \) over the \hyperref[def:real_numbers]{real numbers}.
\end{example}

\paragraph{Row and column spaces}

\begin{definition}\label{def:column_and_row_spaces}\mcite[39; 41]{Knapp2016BasicAlgebra}
  The \term{column space} of the \( m \times n \) matrix \( A \) is the \hyperref[def:module/subobject]{linear span} of the columns of \( A \), regarded as a subspace of \( R^n \).
...
\begin{comments}
  ...
  \item In accordance with \cref{def:rank_and_nullity}, we should refer to the column rank as \enquote{the rank}. We make a distinction between column and row ranks because the two are not in general equal. \Cref{thm:matrix_ranks_over_field_coincide} implies that over field they coincide, however, and in that case it makes sense to conflate them.
\end{comments}
...
\begin{definition}\label{def:system_of_linear_equations}\mimprovised
  \hyperref[def:system_of_equations]{Systems} of \hyperref[tab:def:polynomial_degree]{linear} \hyperref[def:polynomial_equation]{algebraic equation} have an intimate connection to a kind of \hyperref[def:matrix_equation]{matrix equations}.
  ...
  \begin{thmenum}
    ...
    \Cref{thm:homogeneous_linear_equation} implies that, equivalently, a system is homogeneous if \( b \) is the zero vector.
    ...
    \thmitem{def:system_of_linear_equations/solution_space} We call the \hyperref[def:fol_equation/semantic_solution]{solution set} of a linear system a \term{solution space} --- as we shall see in \cref{thm:linear_system_solution_space}, it is in fact an \hyperref[def:affine_subspace]{affine subspace} of \( R^m \).
  \end{thmenum}
\end{definition}

\begin{proposition}\label{thm:linear_system_solutions}
  Let \( x_0 \) be a \hyperref[def:fol_equation/semantic_solution]{solution} of the \hyperref[def:system_of_linear_equations]{system of linear equations} \( Ax = b \) over a ring \( R \).

  Then the entire set of solutions is the \hyperref[def:power_semigroup]{Minkowski sum} of \( x_0 \) with the \hyperref[def:module/kernel]{null space} of \( A \).
\end{proposition}
\begin{comments}
...
\begin{corollary}\label{thm:homogeneous_linear_equations_solutions}
  ...
  \begin{equation*}
    Ax = \vect 0
  \end{equation*}
  has a nontrivial solution if and only if \( A \) is \emph{not} \hyperref[def:inverse_matrix]{invertible}.
\end{corollary}
\begin{proof}
...
\begin{corollary}\label{thm:linear_system_solution_space}
  ...
  If the system is homogeneous, the solution set is a \hyperref[def:module/subobject]{linear subspace}.
\end{corollary}
\begin{proof}
...
\end{theorem}
\begin{comments}
  \item The names of Kroneker and Capelli are associated with this theorem in Russophone literature --- for example \textcite[\S 7.8]{Тыртышников2007ЛинейнаяАлгебра}, \textcite[thm. 2.3.1]{Винберг2014КурсАлгебры} and \textcite[78]{Курош1968КурсВысшейАлгебры}.
\end{comments}
\begin{proof}
