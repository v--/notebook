...
\begin{remark}\label{rem:system_of_equations}
  ...
  In the case where \( T \) is a \hyperref[def:linear_function]{linear function}, we say that the system itself is linear. We can, in that case, rewrite the system via the matrix of \( T \) with respect to the canonical bases. A linear system is often written in \term{scalar form} as
...
\end{corollary}
\begin{proof}
  Follows from \fullref{thm:degrees_of_freedom} and \fullref{thm:matrix_invertibility_via_determinants}.
\end{proof}
