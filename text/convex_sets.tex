...
\begin{remark}\label{rem:real_field_extensions}
  ...
  Important concepts like \hyperref[def:geometric_cone]{cones} or \hyperref[def:convex_hull]{convexity} require the field to be an extension of \( \BbbR \), and it just so happens that, by \fullref{thm:fundamental_theorem_of_algebra} and \cref{thm:no_finite_extensions_of_closed_fields}, the only nontrivial finite \hyperref[def:field/subobject]{field extension} of \( \BbbR \) is \( \BbbC \).
  ...
  Considering only \( \BbbR \) and \( \BbbC \) leads to certain concepts being defined for complex vector spaces and then real vector spaces become a special case. This is formalized via \hyperref[def:complexification]{complexification}. For example, inner products are defined in \cref{def:inner_product_space} differently for real and complex vector spaces, however we can transition between them due to \fullref{thm:complexification_universal_property} and \cref{thm:complexification_of_symmetric_bilinear_form}.

  Furthermore, complex linear functionals are entirely defined by their real parts as discussed in \cref{rem:complex_linear_functional}.
\end{remark}
...
\begin{definition}\label{def:convex_hull}\mimprovised
  ...
  The convex hull is a \hyperref[def:abstract_closure_operator]{closure operator}. A set that coincides with its convex hull is called a \term{convex set}. The geometric interpretation of convex sets is given in \cref{thm:def:convex_hull/line_segments}. The relationship with affine and conic hulls is discussed in \cref{thm:affine_and_conic_is_convex}.
\end{definition}
\begin{proof}
  The proof that the convex hull is a closure operator is similar to that for affine hulls in \cref{def:affine_hull}.
\end{proof}
...
\begin{definition}\label{def:conic_hull}\mimprovised
  A \term{conic combination} with origin \( O \), points \( x_1, \ldots, x_n \) and \emph{nonnegative} scalars \( t_1, \ldots, t_n \) is the \hyperref[def:affine_combinations]{affine combination}
  ...
  The \term{conic hull} of the set \( S \) is the set of all conic combinations of members of \( S \). This is also a \hyperref[def:abstract_closure_operator]{closure operator}.

  The geometric interpretation of convex sets is given in \cref{thm:def:convex_hull/line_segments}. and drawn in \cref{fig:thm:affine_and_conic_is_convex}.
\end{definition}
\begin{proof}
  The proof that the conic hull is a closure operator is similar to that for affine hulls in \cref{def:affine_hull}.
\end{proof}
...
\begin{definition}\label{def:geometric_ray}\mimprovised
  A \term{ray} with \term{vertex} point \( O \) and nonzero \term{directional} vector \( d \) is the \hyperref[def:conic_hull]{conic hull} with origin \( O \) of the singleton set \( \set{ \tau_d(O) } \). It is often described, in complete analogy with \cref{def:affine_line/parametric}, as the \hyperref[def:parametric_curve]{parametric curve}
  \begin{equation*}
    \begin{aligned}
       &r: [0, \infty) \to A \\
  ...
  \begin{thmenum}
    ...
    \thmitem{def:geometric_ray/unidirectional} We say that the rays \( r(t) = \tau_{t d}(O) \) and \( s(t) = \tau_{t e}(P) \) are \term{unidirectional} if there exist some \emph{positive} scalar \( \lambda \) such that \( d = \lambda e \).
  \end{thmenum}
...
\begin{proof}
  ...
  \NecessitySubProof Let \( r(t) = O + td \) be some ray. \Fullref{thm:rank_nullity_theorem} implies that \hyperref[def:orthogonal_complement]{orthogonal complement} of \( \lin\set{ d } \) has dimension \( n - 1 \). Then
  \begin{equation*}
    O + \lin\set{ d }^\perp
  \end{equation*}
  is a hyperplane.
...
\begin{definition}\label{def:geometric_cone}\mcite[20]{Clarke2013OptimalControl}
  ...
  \Cref{thm:def:convex_hull/conic_cone} gives a necessary and sufficient condition for a cone to coincide with its \hyperref[def:conic_hull]{conic hull}. Despite the name, this is not true in general --- a counterexample is presented in \cref{fig:def:geometric_cone}.
...
\begin{definition}\label{def:line_segment_length}
\end{definition}
...
\begin{example}\label{ex:def:extremal_point}
  ...
  \begin{thmenum}
    \thmitem{ex:def:extremal_point/one} Every point \( x \) is extremal for the \hyperref[def:singleton_set]{singleton set} \( \set{ x } \) because the empty set is vacuously convex.

    \thmitem{ex:def:extremal_point/segment} The extremal points of a \hyperref[def:line_segment]{line segment} \( [x, y] \) are \( x \) and \( y \). This is a restatement of \cref{def:extremal_point/segment}.
...
\begin{proof}
  ...
  Then, by \cref{def:extremal_point/combination}, \( x \) coincides with one of \( x_1, \ldots, x_n \). Hence, it belongs to \( A \).
\end{proof}
...
\end{proposition}
\begin{proof}
  \SubProofOf{thm:def:convex_polytope/convex} Follows from \cref{thm:half_spaces_are_convex} and \cref{thm:def:convex_hull/closed_under_intersections}.
...
\begin{example}\label{ex:def:simplex}
  ...
  \begin{thmenum}
    ...
    Therefore, given two distinct points, their convex combination is a line segment --- see \cref{thm:def:convex_hull/line_segments}.

    \thmitem{ex:def:simplex/triangle} A \( 2 \)-simplex is a \hyperref[def:triangle]{triangle} --- this is actually the definition we will use in \cref{def:triangle}.
  \end{thmenum}
\end{example}
