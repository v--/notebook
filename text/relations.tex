...
\begin{definition}\label{def:sequence}\mimprovised
  We will call families indexed by \( \omega \) \term{infinite sequences} or simply \term{sequences}. We will use several notations, depending on the context:
  \begin{itemize}
...
\begin{definition}\label{def:relation}\mimprovised
  ...
  We call the sequence \( (R, A_1, \ldots, A_n) \) an \( n \)-ary \term{relation}. We say that the tuple
  \begin{equation*}
    (x_1, \ldots, x_n) \in A_1 \times \cdots \times A_n
  \end{equation*}
  is \term{related} with respect to \( R \) if \( (x_1, \ldots, x_n) \in R \).
  ...
  Relations are sometimes implicitly assumed to be binary (i.e. \( n = 2 \)). This is how Herbert Enderton defines them in \mcite[40]{Enderton1977Sets}. We discussed binary relations specifically in \fullref{def:binary_relation}, and give here definitions that hold more generally:
  \begin{thmenum}[series=def:relation]
    \thmitem{def:relation/graph} We call the set \( R \) of tuples the \term{graph} of the relation. In case the sequence \( A_1, \ldots, A_n \) is clear from the context, we can conflate the relation \( (R, A_1, \ldots, A_n) \) with its graph \( R \). If we need to explicitly refer to the graph of a relation, we will use the notation \( \gph(R) \).

    \thmitem{def:relation/signature} We define the \term{signature} of the relation as the sequence \( (A_1, \ldots, A_n) \). Obviously this definition only makes sense if we know what the signature is, either from the context or from the definition of the relation as the sequence \( (R, A_1, \ldots, A_n) \) rather than only via its graph \( R \).

    As a matter of fact, it is common to ignore the signature when defining relations. This is done by John Kelley in \cite[7]{Kelley1975} and Herbert Enderton in \cite[40]{Enderton1977Sets}. Anthony Knapp in \cite[595]{Knapp2016BasicAlgebra} uses signatures for functions but not relations. We use signatures for both relations and functions.

    If we do identify a relation only with its graph, some notions like \hyperref[def:multi_valued_function/range]{ranges} and \hyperref[def:multi_valued_function/image]{images} coincide despite being different and other notions like \hyperref[def:function_invertibility/surjective]{function surjectivity} make no sense.
    ...
    \thmitem{def:relation/empty} The relation is \term{empty} if its graph is the empty set, i.e. if no two elements are related.
...
\begin{definition}\label{def:binary_relation}\mimprovised
  An important special case of \hyperref[def:relation]{relations} are \term{binary relations}. Given two sets \( A \) and \( B \), a binary relation between them is a triple \( (R, A, B) \), where \( R \subseteq A \times B \).
  ...
  \begin{thmenum}[series=def:binary_relation]
    ...
    \thmitem{def:binary_relation/diagonal}\mcite[80]{Engelking1989} For every set \( A \), we define a special relation,
    \begin{equation*}
      \increment_A \coloneqq \set{ (x, x) \given x \in A },
    \end{equation*}
    which we call the \term{diagonal relation}.
  ...
  \begin{thmenum}[resume=def:binary_relation]
    ...
    \thmitem{def:binary_relation/connected}\mcite[example 2.2.1(ii)(d)]{Hinman2005} We call \( R \) \term{total} if any two member of \( A \) are related.
...
\begin{definition}\label{def:equivalence_relation}\mimprovised
  ...
  \begin{thmenum}
    ...
    We sometimes have an obvious choice function, for example in \fullref{thm:representatives_in_univariate_polynomial_quotient_set}. In this case, the canonical projection may be regarded as a function from \( A \) to the subset \( c(A / {\sim}) \) of \( A \). Otherwise, function \( \pi \) can be regarded as a \hyperref[def:multi_valued_function]{multi-valued function} from \( A \) to \( A \).
  \end{thmenum}
\end{definition}
...
\begin{definition}\label{def:relation_closures}\mimprovised
  Let \( R \subseteq A^2 \) be a binary relation on the set \( A \). We introduce several \hyperref[def:closure_operator]{closure operators}:
  \begin{thmenum}
