b\section{Relations}\label{sec:relations}
...
\begin{definition}\label{def:sequence}\mcite[45]{Halmos1960NaiveSetTheory}
  An \term{infinite sequence} or simply \term[ru=последовательность (\cite[10]{Архангельский1988КанторовскаяТеорияМножеств})]{sequence} is a \hyperref[def:indexed_family]{families} indexed by the \hyperref[thm:smallest_inductive_set_existence]{smallest inductive set} \( \omega \).
...
\begin{definition}\label{def:binary_relation}\mcite[40]{Enderton1977SetTheory}
  ...
  \begin{thmenum}[series=def:binary_relation]
    \thmitem{def:binary_relation/inverse}\mcite[44]{Enderton1977SetTheory} We defined the \term{converse relation} of \( R \) as
    \begin{equation*}
