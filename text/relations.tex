...
\begin{definition}\label{def:subsequence}\mcite[36]{Roman2008}
...
\begin{definition}\label{def:relation}
  ...
  \begin{thmenum}[series=def:relation]
    ...
    As a matter of fact, it is common to ignore the signature when defining relations. This is done by Thomas Jech in \cite[10]{Jech2003}, John Kelley in \cite[7]{Kelley1975} and Herbert Enderton in \cite[40]{Enderton1977Sets}. Anthony Knapp in \cite[595]{Knapp2016BasicAlgebra} uses signatures for functions but not relations. We use signatures for both relations and functions.
...
\begin{definition}\label{def:binary_relation}
  ...
  \begin{thmenum}[series=def:binary_relation]
    ...
    \thmitem{def:binary_relation/converse}\mcite[126]{MacLane1998} The \term{converse relation} of \( R \) is
    \begin{equation*}
  ...
  \begin{thmenum}[resume=def:binary_relation]
    \thmitem{def:binary_relation/reflexive}\mcite[2]{Roman2008} \( R \) is \term{reflexive} if \( \Delta_A \subseteq R \), i.e. if every element of \( A \) is related with itself.

    The following formula is used as an axiom for \hyperref[thm:nonstrict_partial_order]{nonstrict partial orders} and \hyperref[def:entourage]{entourages}:
    ...
    \thmitem{def:binary_relation/irreflexive}\mcite[2]{Roman2008} \( R \) is \term{irreflexive} if \( \Delta_A \cap R = \varnothing \), i.e. if no element of \( A \) is related with itself.
    ...
    \thmitem{def:binary_relation/symmetric}\mcite[2]{Roman2008} \( R \) is \term{symmetric} if \( R = R^{-1} \).
    ...
    \thmitem{def:binary_relation/antisymmetric}\mcite[2]{Roman2008} \( R \) is \term{antisymmetric} if \( R \cap R^{-1} = \Delta_X \).
    ...
    \thmitem{def:binary_relation/transitive}\mcite[2]{Roman2008} \( R \) is \term{transitive} if \( R = R \bincirc R \).
...
\begin{definition}\label{def:equivalence_relation}
  ...
  \begin{thmenum}
    ...
    \end{equation*}
     ...
     If we have an easy way to choose a representative from each coset, then \( A / {\sim} \) may be regarded as a subset of \( A \). In general, this is not a subset relation by only an \hyperref[def:first_order_isomorphism]{embedding}.
