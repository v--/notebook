...
\begin{definition}\label{def:cartesian_product}\mimprovised
  ...
  \begin{thmenum}
    \thmitem{def:cartesian_product/kuratowski_pair}\mcite[36]{Enderton1977Sets} The \term{Kuratowski pair} or simply \term{ordered pair} \( \braket{ x, y } \) of the sets \( x \) and \( y \) is defined as
    \begin{equation*}
    ...
    \thmitem{def:cartesian_product/product}\mcite[54]{Enderton1977Sets} The \term{Cartesian product} of an indexed family \( \seq{ A_k }_{k \in \mscrK} \) is the set of all tuples from this family. We denote the Cartesian product by
    \begin{equation*}
...
\begin{definition}\label{def:binary_relation}
  ...
  \begin{thmenum}[series=def:binary_relation]
    ...
    \thmitem{def:binary_relation/converse}\mcite[3]{Birkhoff1948} The \term{converse relation} of \( R \) is
    \begin{equation*}
  ...
  \begin{thmenum}[resume=def:binary_relation]
    \thmitem{def:binary_relation/reflexive}\mcite[def. 6.2.14(i)]{Hinman2005} \( R \) is \term{reflexive} if \( \Delta_A \subseteq R \), i.e. if every element of \( A \) is related with itself.
    ...
    \thmitem{def:binary_relation/irreflexive}\mcite[def. 6.2.14(ii)]{Hinman2005} \( R \) is \term{irreflexive} if \( \Delta_A \cap R = \varnothing \), i.e. if no element of \( A \) is related with itself.
    ...
    \thmitem{def:binary_relation/symmetric}\mcite[def. 6.2.14(ix)]{Hinman2005} \( R \) is \term{symmetric} if \( R = R^{-1} \).
    ...
    \thmitem{def:binary_relation/antisymmetric}\mcite[1]{Birkhoff1948} \( R \) is \term{antisymmetric} if \( R \cap R^{-1} = \Delta_X \).
    ...
    \thmitem{def:binary_relation/transitive}\mcite[def. 6.2.14(iii)]{Hinman2005} \( R \) is \term{transitive} if \( R = R \bincirc R \).
    ...
    \thmitem{def:binary_relation/trichotomic}\mcite[def. 6.2.14(v)]{Hinman2005} \( R \) is \term{trichotomic} if every two elements of \( A \) are either related or equal.
...
\begin{example}\label{ex:def:binary_relation}
  ...
  \begin{itemize}
    ...
    \item \hyperref[def:directed_multigraph/simple]{Directed graphs} can be defined as binary relations.
...
\begin{definition}\label{def:equivalence_relation}
  ...
  \begin{thmenum}
    ...
    \end{equation*}
     ...
     If we have an easy way to choose a representative from each coset, then \( A / {\sim} \) may be regarded as a subset of \( A \). In general, this is not a subset relation by only an \hyperref[def:first_order_embedding]{embedding}.
...
\begin{definition}\label{def:relation_closures}
  ...
  \begin{thmenum}
    ...
    A \term{transitive reduction} of \( R \) is a \hyperref[def:extremal_points/maximal_and_minimal_element]{minimal} relation \( Q \subseteq R \) such that \( \cl^T(Q) = \cl^T(R) \). If there exists a smallest such relation, it is the unique transitive reduction, and we denote it by \( \red^T(R) \).
  \end{thmenum}
\end{definition}
...
\begin{example}\label{ex:thm:def:relation_closures/symmetric_and_transitive}
  We will show that symmetric and transitive closures of relations do not commute. This is also a consequence of the difference between \hyperref[def:directed_multigraph_connectedness/weak]{weak} and \hyperref[def:directed_multigraph_connectedness/strong]{strong} connectedness of directed (multi)graphs.
