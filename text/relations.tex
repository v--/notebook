...
\begin{remark}\label{rem:cartesian_products_and_functions}
  ...
  \begin{itemize}
    ...
    \textcite[def. 6.2.1]{Hinman2005Logic},
    \textcite[def. I.4.11]{Kunen2013SetTheory},
    \textcite[36]{Enderton1977SetTheory},
    \textcite[23]{Halmos1960NaiveSetTheory},
    \textcite[1]{Engelking1989GeneralTopology},
    \textcite[139]{КолмогоровДрагалин2006Логика},
    \textcite[182]{Герасимов2014Вычислимость},
    \textcite[thm. 9]{ШеньВерещагин2020НачалаТеорииМножеств},
    \textcite[\S 1.2.3]{КусраевКутателадзе2005БулевозначныйАнализ}.

    \textcite[72]{Bourbaki2004TheoryOfSets} instead introduce an axiom that guarantees the existence of an ordered pair satisfying \cref{thm:kuratowski_pair_extensionality}.

    \textcite{Архангельский1988КанторовскаяТеорияМножеств} defines the other concepts leading to general Cartesian products without a formal definition of ordered pairs.
    ...
    \textcite[def. 6.2.3]{Hinman2005Logic},
    \textcite[def. I.6.5]{Kunen2013SetTheory},
    \textcite[37]{Enderton1977SetTheory},
    \textcite[24]{Halmos1960NaiveSetTheory},
    \textcite[1]{Engelking1989GeneralTopology},
    \textcite[def. I.5.2.1]{Bourbaki2004TheoryOfSets},
    \textcite[141]{КолмогоровДрагалин2006Логика},
    \textcite[188]{Герасимов2014Вычислимость},
    \textcite[9]{Архангельский1988КанторовскаяТеорияМножеств} and
    \textcite[\S 1.2.3]{КусраевКутателадзе2005БулевозначныйАнализ}.

    \textcite[32]{ШеньВерещагин2020НачалаТеорииМножеств}, when defining Cartesian products, defer the definition of Kuratowski pairs for a few pages, instead relying on an intuitive definition of ordered pairs.
    ...
    Out of the aforementioned authors, \textcite[def. I.3.1]{Bourbaki2004TheoryOfSets} and \textcite[9]{Архангельский1988КанторовскаяТеорияМножеств} define function graphs as subsets of the Cartesian product after discussing functions. Arhangelskii does not define functions formally.

    Most other authors define functions as subsets of Cartesian products ---
    \textcite[def. 6.2.8]{Hinman2005Logic},
    \textcite[42]{Enderton1977SetTheory},
    \textcite[30]{Halmos1960NaiveSetTheory},
    \textcite[1]{Engelking1989GeneralTopology},
    \textcite[143]{КолмогоровДрагалин2006Логика},
    \textcite[190]{Герасимов2014Вычислимость} and
    \textcite[32]{ШеньВерещагин2020НачалаТеорииМножеств}.

    \textcite[def. I.6.3]{Kunen2013SetTheory} and \textcite[\S 1.2.4]{КусраевКутателадзе2005БулевозначныйАнализ} define functions as sets of tuples, without referencing Cartesian products.
    ...
    Both choice functions and general products are explicitly discussed by \textcite[60]{Halmos1960NaiveSetTheory}. \Textcite[thm. 6M(3)]{Enderton1977SetTheory} only briefly mentions choice functions, but discusses general products.

    \textcite[def. 6.4.1]{Hinman2005Logic}, \textcite[def. I.12.2]{Kunen2013SetTheory} and \textcite[194]{Герасимов2014Вычислимость} discuss choice functions, but not general Cartesian products.
    ...
    \textcite[54]{Enderton1977SetTheory} defines general Cartesian products and later mentions choice functions without even stating a connection.

    \textcite[def. I.5.3.1]{Bourbaki2004TheoryOfSets},
    \textcite[2]{Engelking1989GeneralTopology},
    \textcite[146]{КолмогоровДрагалин2006Логика} and
    \textcite[41]{ШеньВерещагин2020НачалаТеорииМножеств}
...
\end{proposition}
\begin{comments}
  \item \Textcite[72]{Bourbaki2004TheoryOfSets} introduce an axiom for the existence of ordered pairs satisfying \eqref{eq:thm:kuratowski_pair_extensionality}. See \cref{rem:cartesian_products_and_functions} for a broader discussion.
\end{comments}
\begin{proof}
...
\begin{definition}\label{def:ordered_tuple}
  ...
  \begin{equation*}
    (a_1, \ldots, a_n),
  \end{equation*}
  where, in accordance with \cref{rem:peano_arithmetic/zero} and \cref{rem:cardinal_colorings}, for convenience we start indexing at \( 1 \).
...
\begin{definition}\label{def:sequence}\mcite[45]{Halmos1960NaiveSetTheory}
  ...
  \begin{equation*}
    \seq{ a_k }_{k=1}^\infty,
  \end{equation*}
  where, in accordance with \cref{rem:peano_arithmetic/zero}, for convenience we start indexing at \( 1 \).
\end{definition}
\begin{comments}
...
\begin{definition}\label{def:relation}\mcite[41]{Enderton1977SetTheory}
  ...
  Relations are sometimes implicitly assumed to be binary (i.e. \( n = 2 \)), for example by \textcite[7]{Kelley1975GeneralTopology} or by \textcite[1]{Engelking1989GeneralTopology}. Herbert Enderton initially defines them as binary in \mcite[40]{Enderton1977SetTheory}, but gives a more general definition one page later. We discuss binary relations specifically in \cref{def:binary_relation}, and give here definitions that hold more generally.
  ...
  \begin{thmenum}[series=def:relation]
    ...
    \thmitem{def:relation/single_set} If all \( A_k \) for \( k = 1, \ldots, n \) are equal to the set \( A \), we say that \( R \subseteq A^n \) is a relation \emph{on} \( A \).
...
\begin{definition}\label{def:binary_relation}\mcite[40]{Enderton1977SetTheory}
  ...
  \begin{thmenum}[series=def:binary_relation]
    ...
    \thmitem{def:binary_relation/diagonal}\mcite[11]{Harzheim2005OrderedSets} For every set \( A \), we define a special relation,
    \begin{equation*}
  ...
  \begin{thmenum}[resume=def:binary_relation]
    ...
    The following formula is used as an axiom for \hyperref[def:partially_ordered_set/strict]{nonstrict partial orders} and \hyperref[def:entourage]{entourages}:
...
\end{definition}
\begin{comments}
  \item Relations are the semantic counterpart to \hyperref[def:fol_signature/predicate]{first-order predicates} and are equivalent to Boolean-valued functions, as discussed in \cref{rem:boolean_valued_functions_and_relations}.
\end{comments}
...
\begin{definition}\label{def:relation_closures}
  Let \( R \subseteq A^2 \) be a binary relation on the set \( A \). We introduce several \hyperref[def:abstract_closure_operator]{closure operators}:
  \begin{thmenum}
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \item Finally, if \( (x, y) \notin \cl^T(R) \), then \( (y, x) \in \cl^T(R) \). As in the previous step, we can show that \( (y, x) \in \cl^T(\cl^S(R))) \). The latter set is symmetric, hence \( (x, y) \in \cl^T(\cl^S(R)) \).
  \end{itemize}
...
\paragraph{Equivalence closures}

\begin{definition}\label{def:equivalence_closure}\mcite[89]{GirardEtAl1989ProofsAndTypes}
  Consider some \hyperref[def:binary_relation]{binary relation} \( R \) on \( A \). The intersection of all \hyperref[def:equivalence_relation]{equivalence relations} on \( A \) containing \( R \) is again an equivalence relation, which we call the \term{equivalence closure} of \( R \).
\end{definition}
\begin{comments}
  \item \Cref{thm:moore_family_closure_operator} implies that the equivalence closure is a \hyperref[def:abstract_closure_operator]{closure operator} on \( A^2 \).

  In particular, as a consequence of \cref{thm:closure_operator_minimality}, it is the least among all equivalence relations containing \( R \).

  \item See \cref{thm:equivalence_closure_characteriation} for a useful characterization. A more flexible construction utilizing \fullref{thm:recursively_defined_relations} is given in \cref{thm:generated_equivalence_relation}.
\end{comments}
\begin{defproof}
  Clearly \( A^2 \) is an equivalence relation, so the intersection is nonempty. It is then straightforward to show that this intersection is itself an equivalence relation.
\end{defproof}

\begin{proposition}\label{thm:equivalence_closure_characteriation}
  Consider some \hyperref[def:binary_relation]{binary relation} \( R \) on a set \( A \). The \hyperref[def:equivalence_closure]{equivalence closure} of \( R \) is the \hyperref[def:relation_closures/reflexive]{reflexive}, \hyperref[def:relation_closures/symmetric]{symmetric} and \hyperref[def:relation_closures/transitive]{transitive} closure \( \cl^T \cl^S \cl^R (R) \) of \( R \).
\end{proposition}
\begin{comments}
  \item This holds for any permutation of the closures as long as \( \cl^T \) is applied \emph{after} \( \cl^S \). This latter restriction is due to \cref{thm:def:relation_closures/symmetric_and_transitive}.
\end{comments}
\begin{proof}
  By \cref{thm:def:relation_closures/reflexive_relation}, \( \cl^S \cl^R (R) \) is reflexive. It is also symmetric as the symmetric closure of \( \cl^R(R) \).

  Then the transitive closure \( E \coloneqq \cl^T \cl^S \cl^R (R) \) is also symmetric and reflexive by \cref{thm:def:relation_closures/reflexive_relation} and \cref{thm:def:relation_closures/symmetric_relation}.

  Therefore, \( E \) is an equivalence relation containing \( R \). It remains to show that any equivalence relation \( E' \) containing \( R \) is a superset of \( E \).

  Fix a pair \( (x, y) \) from \( E \). Then there exists some positive integer \( k \) such that \( (x, y) \) belongs to
  \begin{equation*}
    \parens[\big]{ \cl^S \cl^R (R) }^k.
  \end{equation*}

  We can use induction on \( k \) to show that \( (x, y) \) belongs to \( E' \).
  \begin{itemize}
    \item When \( k = 1 \), either \( (x, y) \) or \( (y, x) \) belong to \( \cl^R (R) \), and hence to \( E' \).
    \item When \( k > 1 \), there exists some element \( z \) of \( A \) such that \( (x, z) \) belongs to \( \parens[\big]{ \cl^S \cl^R (R) }^{k-1} \) and \( (z, y) \) belong to \( \cl^S \cl^R (R) \).

    By the inductive hypothesis \( (x, z) \) belongs to \( E' \). By the same reasoning as in the base case, \( (z, y) \) belongs to \( E' \). Then, since \( E' \) is transitive, \( (x, y) \) also belongs to \( E' \).
  \end{itemize}

  Therefore, \( E \subseteq E' \), making \( E \) the smallest equivalence relation containing \( E \).
\end{proof}
