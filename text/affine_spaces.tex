...
\begin{concept}\label{con:coordinate_systems}
  \todo{Describe coordinate systems in general}
\end{concept}

\begin{definition}\label{def:radius_vector}
\end{definition}

\begin{definition}\label{def:directed_segment}
\end{definition}
...
\begin{definition}\label{def:affine_combinations}
  ...
  The affine combination of \( n \) points can then be defined via \fullref{thm:omega_recursion}:
  \begin{equation*}
...
\begin{proof}
  ...
  \SubProofOf{thm:def:affine_combinations/inverse} Follows from \cref{thm:def:affine_combinations/inverse_of_action}.
...
\begin{definition}\label{def:affine_hull}\mimprovised
  The \term{affine hull} of the set \( S \) of points is the set of all arbitrary (i.e. not necessarily binary) \hyperref[def:affine_combinations]{affine combinations} of members of \( S \). It is a \hyperref[def:abstract_closure_operator]{closure operator}.
\end{definition}
\begin{defproof}
...
\begin{definition}\label{def:affine_subspace}\mcite[25]{Gallier2011Geometry}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:affine_subspace/linear} If \( \vect B \) is a \hyperref[def:module/subobject]{vector subspace} of \( \vect A \).
  \end{thmenum}
\end{definition}
\begin{defproof}
...
\begin{proposition}\label{thm:parallel_subspace_through_point}
  ...
  The case of an \hyperref[def:affine_line]{affine line} is a restatement of Euclid's \enquote{Proposition \( \beta. \)} (see \cite[8]{Euclid2008Elements}):
  \begin{quote}
...
\begin{proof}
  ...
  \Cref{thm:def:affine_combinations/chasles} implies
  \begin{equation*}
  ...
  \ImplicationSubProof{def:affine_dependence/combinations}{def:affine_dependence/linear} Suppose that \cref{def:affine_dependence/combinations} holds for \( E \). Fix some point \( O \) from \( E \). We will show that the set
  \begin{equation*}
    \set{ \vect{O e} \given e \in E \T{and} e \neq O }
  \end{equation*}
  is linearly independent.
...
\begin{proposition}\label{thm:linear_and_affine_bases}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:linear_and_affine_bases/hulls} The \hyperref[def:semimodule/subobject]{linear span} of \( \vect E \) is \( \vect A \) if and only if the \hyperref[def:affine_hull]{affine hull} of \( E \) is \( A \).
  \end{thmenum}
\end{proposition}
\begin{proof}
  \SubProofOf{thm:linear_and_affine_bases/dependence} This is the statement of \cref{def:affine_dependence/linear}.
...
\begin{definition}\label{def:barycentric_coordinate_system}
  ...
  \Cref{thm:linear_and_affine_bases} implies that the set
...
\begin{defproof}
  ...
  \SubProofOf[eq:def:additive_function]{additivity} \Cref{thm:def:affine_combinations/vectors_to_points} implies
  \begin{equation*}
    \tau_{u + v}(O)
    =
    \tau_{u + v - 0}(O)
  ...
  \SubProofOf[eq:def:homogeneous_function]{homogeneity} Similarly,
  \begin{equation*}
    T(\lambda v)
    =
...
\begin{definition}\label{def:category_of_small_affine_spaces}
  We describe the \term{category of small affine spaces} as the following \hyperref[rem:concrete_categories]{concrete category}:

  \begin{itemize}
    \item The \hyperref[def:category/objects]{set of objects} is the set of all \hyperref[def:affine_space]{affine spaces} \( (A, \vect A, \tau) \), whose set of points \( A \) is \hyperref[def:small_set]{small}.
...
\end{definition}
\begin{defproof}
  \SubProofOf{def:affine_line/parametric} \Cref{thm:def:affine_combinations/vectors_to_points} implies that \( l \) satisfies \cref{def:affine_operator/combination} and is therefore an affine function.
  ...
  \ImplicationSubProof{def:affine_line/parametric}{def:affine_line/subspace} Suppose that \( l(t) = \tau_{td}(o) \) is an affine function that \( L = l(\BbbK) \). \Cref{thm:image_of_affine_operator} implies that \( L \) is an affine subspace of \( A \).
...
\begin{proposition}\label{thm:pair_of_points_is_collinear}
  ...
  The case of exactly two points is a restatement of Euclid's first postulate (see \cite[7]{Euclid2008Elements}):
  \begin{quote}
...
\begin{definition}\label{def:crossing_lines}
  We say that two lines \term{cross} if they intersect and are not \hyperref[def:affine_parallelism]{parallel}. \Cref{thm:crossing_lines} implies that they should intersect in exactly one point (because otherwise they would coincide). We call this point the \term{crossing point}.
\end{definition}
...
\end{definition}
\begin{defproof}
  \SubProofOf{def:affine_plane/parametric} \Cref{thm:def:affine_combinations/vectors_to_points} implies that \( \pi \) satisfies \cref{def:affine_operator/combination} and is therefore an affine function.
