...
\begin{algorithm}[Single-digit multiplication with carrying]\label{alg:single_digit_multiplication_with_carrying}
  ...
  \begin{thmenum}
    ...
    \thmitem{alg:single_digit_multiplication_with_carrying/single_neg_guard} If \( n \) and \( m \) have different \emph{nonzero} signs, we can apply the algorithm to \( \abs{n} \) and \( \abs{m} \) since
    \begin{equation*}
      nm = -\abs{n} \cdot \abs{m}.
    \end{equation*}
...
\begin{defproof}
  ...
  The guard steps \ref{alg:single_digit_multiplication_with_carrying/double_neg_guard} and \ref{alg:single_digit_multiplication_with_carrying/single_neg_guard} allow us to assume that \( n \geq 0 \) and \( m \geq 0 \).
...
\begin{algorithm}[Multi-digit multiplication with carrying]\label{alg:multi_digit_mult_with_carrying}
  ...
  \end{equation*}
  where the polynomial degree of \( f_s \) is at most \( p + s + 2 \).
...
\begin{algorithm}[Long division]\label{alg:long_integer_division}
  ...
  \begin{thmenum}
    \thmitem{alg:long_integer_division/n_guard} If \( n < 0 \), let \( q \) and \( r \) be the quotient and remainder of the division of \( \abs{n} \) by \( \abs{m} \). We have
    \begin{equation*}
    ...
    \thmitem{alg:long_integer_division/m_guard} If \( n \geq 0 \) and \( m < 0 \), let \( q \) and \( r \) be the quotient and remainder of the division of \( n \) by \( -m \). We have
    \begin{equation*}
    ...
    \thmitem{alg:long_integer_division/init} Finally, if \( n \geq 0 \) and \( m \geq 0 \), let \( r_{-1} \coloneqq n \) and \( q_{-1} \coloneqq 0 \).

    \thmitem{alg:long_integer_division/step} At step \( s = 0, 1, \ldots \), let \( k \coloneqq p - r - s \) and define
    \begin{equation*}
    ...
    \thmitem{alg:long_integer_division/finish} Finally, at step \( p - l \), the remainder is \( r = r_{p - l} \), as we will show, and we have used \fullref{alg:addition_with_carrying} and \fullref{alg:single_digit_multiplication_with_carrying} to explicitly compute its expansion at every step.
...
\end{comments}
\begin{defproof}
  The guard steps \ref{alg:long_integer_division/n_guard} and \ref{alg:long_integer_division/m_guard} allow us to assume that \( n \geq 0 \) and \( m \geq 0 \).
  ...
  We will use induction on \( s \) to show that
  \begin{equation}\label{eq:alg:long_integer_division/proof/hypothesis}
  ...
  \begin{itemize}
    ...
    \end{equation*}
    hence \eqref{eq:alg:long_integer_division/proof/hypothesis} holds.
