...
\begin{algorithm}[Simply typed substitution]\label{alg:simply_typed_substitution}
  Consider the \hyperref[def:abstract_type_system]{type system} of \hyperref[def:simple_algebraic_types]{simple algebraic types}. Fix a \hyperref[def:atomic_lambda_term_substitution]{atomic substitution} \( \Bbbs \) and a \hyperref[def:type_derivation_tree]{type derivation tree} \( T \) for \( M: \tau \). For every assumption \( v: \sigma \) of \( T \), suppose we are given a derivation tree \( T_v \) for \( \Bbbs(v): \sigma \).

  We will build a derivation tree \( T' \) for \( M[\Bbbs]: \tau \) whose assumptions are those replacement trees \( T_v \) which are assumption trees (i.e. not rule application trees). Here \( M[\Bbbs] \) is \( \muplambda \)-term substitution as defined in \fullref{alg:lambda_term_substitution}, with obvious adjustments for typed terms.

  Most rules follow the same pattern, but there are two exceptions with dischargeable assumptions --- \ref{inf:def:arrow_type/elim} and \ref{inf:def:simple_sum_type/elim} --- that bind variables which we may possibly rename as per \eqref{eq:alg:lambda_term_substitution/abstraction/renaming}.
  ...
  \begin{thmenum}
    ...
    \end{equation*}
    where \( u \not\in \op*{Free}_\Bbbs(M) \). Here \( u \) may be either \( a \) or a fresh variable depending on whether we use \eqref{eq:alg:lambda_term_substitution/abstraction/direct} or \eqref{eq:alg:lambda_term_substitution/abstraction/renaming}.
...
\begin{comments}
  ...
  \begin{CodeRefDisplay}
    \GetCodeRef{alg:simply_typed_substitution}
  \end{CodeRefDisplay}
\end{comments}
...
\begin{algorithm}[Simply typed \( \alpha \)-conversion]\label{alg:simply_typed_alpha_conversion}
  ...
  \begin{thmenum}
    ...
    By \cref{thm:def:lambda_term_alpha_equivalence/ast}, \( N = \qabs {b^\rho} B \) for some variable \( b \) and \( \muplambda \)-term \( B \). Note that, as per \cref{def:typed_term_alpha_equivalence}, we require the abstractor variables of \( \rho \)-equivalent abstractions to be equal, so the variable annotation in \( N \) must be equal to \( \rho \).
    ...
    \begin{thmenum}
      ...
      \Cref{thm:def:lambda_term_alpha_equivalence/abstraction_body_free} implies that \( b \) is not free in \( A \) nor \( a \) is in \( B \). Thus, if \( b \) is free in \( A \), the algorithm errors out.
      ...
      Again, if the recursive application fails, then \( A[a \mapsto b] \nequivrel{\alpha} B \), so the current application must also fail.
    \end{thmenum}
...
\begin{comments}
  \item This algorithm is implemented in \cite{notebook:code} as
  \begin{CodeRefDisplay}
    \GetCodeRef{alg:simply_typed_alpha_conversion}
  \end{CodeRefDisplay}
\end{comments}
...
\begin{comments}
  \item This algorithm is implemented in \cite{notebook:code} as
  \begin{CodeRefDisplay}
    \GetCodeRef{alg:simply_typed_reduction}
  \end{CodeRefDisplay}
\end{comments}
\begin{defproof}
...
\begin{remark}\label{rem:beta_reduction_and_cut_elimination}
  The rule \ref{inf:thm:reduction_typing_rules/beta} can be written in \hyperref[rem:natural_deduction_in_sequent_style]{sequent style} as follows:
  \begin{equation*}
    \begin{prooftree}
  ...
  This latter rule is a now clearly special case of \ref{inf:def:abstract_sequent_calculus_system/cut} in which \( \Delta_1 \) is empty and \( \Delta_2 = \tau \).

  Therefore, \( \beta \)-reduction can be seen as a simply typed counterpart to eliminating usage of the cut rule, and \cref{fig:alg:simply_typed_reduction/arrow_elim/beta} --- as a proof tree transformation that aids with \fullref{thm:cut_elimination}.
\end{remark}
