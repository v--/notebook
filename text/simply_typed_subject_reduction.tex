...
\begin{lemma}\label{thm:single_step_reduction_deconstruction}
  ...
  \begin{thmenum}
    ...
    \begin{thmenum}
      ...
      This can also be reformulated as in \fullref{rem:single_step_reduction_deconstruction_abs_lift}.
...
\begin{algorithm}[Simply typed reduction]\label{alg:simply_typed_reduction}
  ...
  \begin{thmenum}
    ...
    \thmitem{alg:simply_typed_reduction/arrow_intro} If \( T \) is an application tree for \ref{inf:def:arrow_type/intro/explicit}, then \( \tau = \rho \rightarrow \pi \) for some types \( \rho \) and \( \pi \) and \( M = \qabs {a^\rho} A \) for some variable \( A \) and \( \muplambda \)-term \( A \).
    ...
    \begin{thmenum}
      ...
      Let \( T_E \) be the left premise subtree of \( T_A \). We can attempt to use \fullref{alg:simply_typed_alpha_conversion} on \( E \aequiv N \), which should transform \( T_E \) into a derivation tree \( T_N \) of \( N: \tau \).
