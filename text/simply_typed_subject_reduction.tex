...
\begin{algorithm}[Simply typed substitution]\label{alg:simply_typed_substitution}
  ...
  \begin{thmenum}
    ...
    \end{equation*}
    where \( u \notin \op*{Free}_\Bbbs(M) \). Here \( u \) may be either \( a \) or a fresh variable depending on whether we use \eqref{eq:alg:lambda_term_substitution/abstraction/direct} or \eqref{eq:alg:lambda_term_substitution/abstraction/renaming}.
