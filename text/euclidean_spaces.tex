...
\begin{definition}\label{def:euclidean_space}\mimprovised
  ...
  As described in \cref{rem:structure_hierarchy}, this introduces a standard \hyperref[def:norm]{norm}, \hyperref[def:metric_space]{metric}, \hyperref[def:uniform_space]{uniformity} and \hyperref[def:topological_space]{topology}, which we call the \( n \)-dimensional \term{Euclidean norm} (resp. metric, uniformity or topology).
...
\begin{remark}\label{rem:euclidean_space_etymology}
  ...
  \begin{itemize}
    ...
    \textcite[\S 2.19]{Rudin1987RealAndComplexAnalysis},
    \textcite[example A-7.1]{Rotman2015AdvancedModernAlgebraPart1},
    \textcite[xv]{HugWeil2020ConvexGeometry},
    \textcite[1]{WheedenZygmund2015MeasureAndIntegral},
    \textcite[example 1.1.6]{Tao2022AnalysisII},
    \textcite[13]{HillePhillips1996FunctionalAnalysis},
    \textcite[11]{Tao2011MeasureTheory},
    \textcite[444]{ИльинСадовничийСендов1985АнализТом1},
    \textcite[39]{Вулих1973ВещественныйАнализ} and
    \textcite[105]{Обрешков1962ВисшаАлгебра}

    \item It is defined as a finite-dimensional \hyperref[def:inner_product_space]{real inner product space} by
    \textcite[\S 8.1.1]{Berger1987GeometryI} and
    \textcite[121]{Halmos1974VectorSpaces}.

    \item It is defined as a real inner product space, possibly infinite-dimensional, by
    \textcite[\S 24.1]{Тыртышников2007ЛинейнаяАлгебра},
    \textcite[def. 5.5.1]{Винберг2014КурсАлгебры} and
    \textcite[216]{Станилов1974АналитичнаГеометрия}.
  \end{itemize}
...
\begin{proposition}\label{thm:isometry_iff_affine_orthogonal_operator}
...
\begin{definition}\label{def:argmin_argmax}
  ...
  \hyperref[thm:order_duality]{Duality} the allows us to define
  \begin{equation*}
    \argmax_{a \in A} f(a).
  \end{equation*}
\end{definition}
...
\begin{definition}\label{def:orthogonal_projection}
  ...
  The definition does not depend on the choice of coordinate system. \Cref{thm:def:orthogonal_projection} lists important properties of orthogonal projections.
...
\begin{proof}
  ...
  \SubProofOf{thm:def:orthogonal_projection/distance} From \cref{thm:def:orthogonal_projection/normal} it follows that, for \( y \in L \),
  \begin{equation*}
    \norm{ x - y }^2
    =
    \norm{ x - \pi(x) + \pi(x) - y }^2
