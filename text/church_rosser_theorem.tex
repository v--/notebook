...
\end{proposition}
\begin{proof}
  We will use \fullref{thm:induction_on_recursively_defined_relations} on \( M \ppred N \) simultaneously on all compatible substitutions to show that \( M[\Bbbs] \pred N[\Bbbt] \).
  \begin{itemize}
    ...
    Furthermore, \fullref{thm:lambda_substitution_restriction} implies that \( A[\Bbbs_{x \mapsto u}] = A[\Bbbs] \), thus
    \begin{equation*}
    ...
    \Fullref{def:delta_reduction/substitution} implies that
    \begin{equation*}
...
\begin{lemma}\label{thm:parallel_reduction_deconstruction}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:parallel_reduction_deconstruction/app} If \( M = AB \), we have three possibilities:
    \begin{thmenum}
...
\begin{comments}
  ...
  We mostly follow \incite[14]{TroelstraSchwichtenberg2000Proofs}, who use the phrase \enquote{(weak) Church-Rosser property} synonymously with \enquote{(weak) confluence}.
  \begin{itemize}
