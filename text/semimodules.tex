...
\begin{definition}\label{def:semimodule}\mcite[149]{Golan1999Semirings}
  ...
  \begin{thmenum}[series=def:semimodule]
    \thmitem{def:semimodule/action}\mimprovised A left semimodule is a \hyperref[def:semiring/morphism]{homomorphism} from \( R \) to the \hyperref[def:endomorphism_semiring]{endomorphism semiring} \( \End(M) \). A right semimodule is a homomorphism from the \hyperref[def:semiring/opposite]{opposite semiring} \( R^{\oppos} \) to \( \End(M) \).
    ...
    \begin{thmenum}
      ...
      \thmitem{def:semimodule/operation/scalar_addition_distributivity} Scalar multiplication distributes over scalar addition:
      \begin{equation}\label{eq:def:semimodule/operation/scalar_addition_distributivity}
        (r +_R s) \cdot x = r \cdot x + s \cdot x.
      \end{equation}

      \thmitem{def:semimodule/operation/vector_addition_distributivity} Scalar multiplication distributes over vector addition:
  ...
  \begin{thmenum}[resume=def:semimodule]
    \thmitem{def:semimodule/theory}\mimprovised In order to fit the heterogeneous operation \( \cdot \) into the framework of \hyperref[def:fol_semantics/model]{first-order logic models}, we can extend the \hyperref[def:monoid/theory]{theory of monoids} by adding, for every semiring element \( r \), a unary \hyperref[def:fol_signature/function]{functional symbol} \( m_r \).
    ...
    \thmitem{def:semimodule/homomorphism}\mcite[156]{Golan1999Semirings} A \hyperref[def:fol_homomorphism]{first-order homomorphism} between two \( R \)-semimodules \( M \) and \( N \) is a function \( \varphi: M \to N \) that is a \hyperref[def:monoid/morphism]{monoid homomorphism} and satisfies
    ...
    \thmitem{def:semimodule/submodel}\mcite[150]{Golan1999Semirings} The set \( A \subseteq M \) is a \hyperref[def:fol_substructure/submodel]{submodel} of \( M \) if it is a \hyperref[def:monoid/subobject]{submonoid} of \( M \) that is closed under scalar multiplication, i.e. \( rM = m_r[M] \subseteq M \) for every \( r \in R \). We say that \( A \) is an \( R \)-\term{sub-semimodule} of \( M \).
    ...
    As a consequence of \cref{thm:fol_equational_theory_models/image}, the \hyperref[def:set_valued_map/image]{image} of an \( R \)-semimodule homomorphism \( \varphi: M \to N \) is an \( R \)-sub-semimodule of \( M \).

    \thmitem{def:semimodule/generated}\mimprovised For an arbitrary set \( A \), we denote the \hyperref[def:fol_generated_substructure]{generated submodel} by \( \lin{ A } \) and call it the \term[ru=линейная оболочка (\cite[sec. 3.2]{Тыртышников2007ЛинейнаяАлгебра})]{linear span} of \( A \).

    \Cref{rem:span_over_different_semirings} shows how it is important to be unambiguous about over which semiring we take the span of \( A \). In case of possible ambiguity, we will use subscripts like \( \lin_R A \).

    The linear span can be characterized via \hyperref[def:linear_combination]{linear combinations} --- see \cref{thm:span_via_linear_combinations}.

    \thmitem{def:semimodule/category}\mimprovised For a fixed semiring \( R \), the \hyperref[def:fol_theory_model_functor/obj]{category of \( \mscrU \)-small models} \( \ucat{SMod}_R \) of left semimodules is \hyperref[def:concrete_category]{concrete} over \hyperref[def:monoid]{\( \ucat{Mon} \)}.

    Other notations are in use, for example \( R-\cat{Mod} \) for \( R \)-modules by \textcite[158]{Aluffi2009Algebra}, that better highlight whether we are considering left or right (semi)modules. We will prefer \( \cat{SMod}_R^\oppos \) for the category of right semimodules.
...
\begin{defproof}
  ...
  \begin{itemize}
    ...
    \item Distributivity of scalar addition holds because \( \varphi \) is a \hyperref[def:semigroup/morphism]{semigroup homomorphism} from \( (R, +) \) to \( (M, +) \).
  ...
  \ImplicationSubProof{def:semimodule/operation}{def:semimodule/action} Let \( \cdot: R \times M \to M \) be an operation satisfying all conditions from \cref{def:semimodule/operation}. Define the function \( \varphi(r) \coloneqq (x \mapsto r \cdot x) \). We will show that this is a semiring homomorphism.
...
\begin{proposition}\label{thm:def:semimodule}
  ...
  \begin{thmenum}
    \thmitem{thm:def:semimodule/union} The union of a \hyperref[def:order_function/preserving]{ascending sequence} \( N_1 \subseteq N_2 \subseteq \cdots \) of \( R \)-\hyperref[def:semimodule/submodel]{sub-semimodules} of \( M \) is also an \( R \)-sub-semimodule of \( M \).
  \end{thmenum}
\end{proposition}
\begin{proof}
...
\begin{comments}
  ...
  \item \Textcite*[def. 2.3.2]{HillePhillips1996FunctionalAnalysis} state
  \begin{displayquote}
...
\begin{definition}\label{def:linear_combination}\mimprovised
  ...
  A \term[ru=линейная комбинация (\cite[\S 3.2]{Тыртышников2007ЛинейнаяАлгебра}), en=linear combination (\cite[39]{FriedbergInselSpence2018LinearAlgebra})]{linear combination} in these indeterminates is simply an element of the \hyperref[def:free_semimodule]{free semimodule} over them.
