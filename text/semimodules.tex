...
\begin{definition}\label{def:linear_combination}\mimprovised
  ...
  The \hyperref[def:free_semimodule]{free semimodule} \( R^{\oplus M} \) consists of indexed families \( \seq{ t_x }_{x \in M} \), where only finitely many nonzero entries. We call such an entry a \term[ru=линейная комбинация (\cite[\S 3.2]{Тыртышников2007ЛинейнаяАлгебра}), en=linear combination (\cite[6]{Treil2017LinearAlgebra})]{linear combination}.

  We refer to the entries of a linear combination as \term[ru=коэффициенты (линейной комбинации) (\cite[\S 3.2]{Тыртышников2007ЛинейнаяАлгебра})]{coefficients} and to the pairs \( (t_x, x) \) as \term{terms}.
  ...
  \begin{thmenum}
    ...
    We call \eqref{eq:def:linear_combination/sum} the \term{sum} of the linear combination.
