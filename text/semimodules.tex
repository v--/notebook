...
\begin{definition}\label{def:semimodule}
  ...
  \begin{thmenum}[resume=def:semimodule]
    ...
    \thmitem{def:semimodule/homomorphism} A \hyperref[def:first_order_homomorphism]{first-order homomorphism} between two \( R \)-semimodules \( M \) and \( N \) is a function \( \varphi: M \to N \) that is a \hyperref[def:monoid/homomorphism]{monoid homomorphism} and satisfies
    ...
    \thmitem{def:semimodule/generated} For an arbitrary set \( A \), we denote the \hyperref[def:first_order_generated_substructure]{generated submodel} by \( \linspan{ A } \) and call it the \term[ru=линейная оболочка (\cite[sec. 3.2]{Тыртышников2007ЛинейнаяАлгебра})]{linear span} of \( A \).
...
\begin{definition}\label{def:free_semimodule}\mimprovised
  ...
  We can regard the function \( t \) as an indexed family \( \seq{ t_x }_{x \in A} \).
...
\begin{definition}\label{def:linear_combination}\mimprovised
  Fix a \hyperref[def:semiring]{(semi)ring} \( R \) and an \hyperref[def:semimodule]{\( R \)-semimodule} \( M \).

  The \hyperref[def:free_semimodule]{free semimodule} \( R^{\oplus M} \) consists of indexed families \( \seq{ t_x }_{x \in M} \), where only finitely many nonzero entries. We call such an entry a \term[bg=линейна комбинация (\cite[46]{ПетровЗяпков2000ЛААГТом1}), ru=линейная комбинация (\cite[\S 3.2]{Тыртышников2007ЛинейнаяАлгебра}), en=linear combination (\cite[6]{Treil2017LinearAlgebra})]{linear combination}.

  We refer to the entries of a linear combination as \term[bg=коефициенти (на линейната комбинация) (\cite[46]{ПетровЗяпков2000ЛААГТом1}), ru=коэффициенты (линейной комбинации) (\cite[\S 3.2]{Тыртышников2007ЛинейнаяАлгебра})]{coefficients} and to the pairs \( (t_x, x) \) as \term{terms}.
  ...
  \begin{thmenum}
    ...
    \thmitem{def:linear_combination/sum} \Fullref{thm:free_semimodule_universal_property} suggests evaluating \( \seq{ t_x }_{x \in M} \) as
    \begin{equation}\label{eq:def:linear_combination/sum}
      \sum_{x \in M} t_x \cdot x.
    \end{equation}

    We call \eqref{eq:def:linear_combination} the \term{sum} of the linear combination.

    \thmitem{def:linear_combination/of} A linear combination \hi{of the vectors} \( x_1, \ldots, x_n \) is simply a linear combination \( \seq{ t_x }_{x \in M} \) such that \( t_x = 0 \) for every \( x \) outside \( x_1, \ldots, x_n \).

    To be precise, we allow the possibility that \( t_{x_k} = 0 \) for \( k = 1, \ldots, n \).

    We then use the notation
    \begin{equation}\label{eq:def:linear_combination/of}
      \sum_{k=1}^n t_k \cdot x_k
    \end{equation}
    ambiguously both for the combination itself and for its sum.
  \end{thmenum}
\end{definition}
\begin{comments}
  \item Linear combinations are assumed knowledge and even informal definitions can rarely be found. Even those authors like \incite[\S 3.2]{Тыртышников2007ЛинейнаяАлгебра} and \incite[6]{Treil2017LinearAlgebra} that give definitions do it informally, without distinguishing between the syntax-semantics distinction.

  \item Linear combinations and their sums are thus an instance of the \hyperref[con:syntax_semantics_duality]{syntax-semantics duality}.

  \item This issue of finiteness is discussed in more generality in \fullref{rem:binary_operation_syntax_trees/infinite/direct_sum}.
\end{comments}
