...
\begin{definition}\label{def:semimodule}\mcite[149]{Golan1999Semirings}
  ...
  \begin{thmenum}[resume=def:semimodule]
    ...
    \thmitem{def:semimodule/homomorphism}\mcite[156]{Golan1999Semirings} A \hyperref[def:fol_homomorphism]{first-order homomorphism} between two \( R \)-semimodules \( M \) and \( N \) is a function \( \varphi: M \to N \) that is a \hyperref[def:monoid/homomorphism]{monoid homomorphism} and satisfies
    ...
    \thmitem{def:semimodule/submodel}\mcite[150]{Golan1999Semirings} The set \( A \subseteq M \) is a \hyperref[def:fol_substructure/submodel]{submodel} of \( M \) if it is a \hyperref[def:monoid/submodel]{submonoid} of \( M \) that is closed under scalar multiplication, i.e. \( rM = m_r[M] \subseteq M \) for every \( r \in R \). We say that \( A \) is an \( R \)-\term{sub-semimodule} of \( M \).
