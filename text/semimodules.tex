...
\begin{definition}\label{def:linear_combination}\mimprovised
  Fix some \hyperref[def:indexed_family]{indexed family} \( \seq{ X_k }_{k \in \mscrK} \) of \hyperref[def:formal_language/symbol]{symbols}. These will act as indeterminates in the sense of \cref{con:free_construction/indeterminate}.
  ...
  \begin{thmenum}
    ...
    Only finitely many terms are nonzero, so this sum is well-defined even when \( \mscrK \) is infinite. We discuss this more generally in \cref{rem:binary_operation_syntax_trees/infinite}.
