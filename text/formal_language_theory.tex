...
\begin{remark}\label{rem:object_language_dots}
  ...
  It is also not clear whether the metalingual variables in the \hyperref[def:fol_formula]{equality formula} \( (x \doteq y) \) are allowed to refer to the same symbol or not. Regarding this issue, \incite[78]{Kleene2002Logic} remarks
  \begin{displayquote}
...
\begin{remark}\label{rem:object_language_disambiguation_conventions}
  ...
  \begin{itemize}
    \item \incite[rem. 2.1.3]{Hinman2005Logic} follows a similar convention to ours, but he places dots much more sparsely --- mostly over the formal equality symbol \( {\syneq} \) and the symbols in \hyperref[def:fol_signature]{first-order signatures}.

    \item \incite[60]{Church1956LogicVol1}, \incite*[\S 51]{Andrews2002Logic} and \incite[8]{Shoenfield1967MathematicalLogic} rely on another convention --- to use bold letters for metalingual variables and normal-weight letters for variables in the object language.
