...
\begin{concept}\label{con:figurate_number}
  ...
  The philosophical development leading to figurate numbers is described briefly by \textcite{Розенфельд1963ОсновныеПонятиеГеометрии}. Rosenfeld, Zhmud and the Deza spouses, as well as \textcite[1; 2]{Apostol1976AnalyticNumberTheory}, attribute the concept to the \hyperref[rem:pythagoreanism]{Pythagorean school}.
...
\begin{remark}\label{rem:triangular_point_configurations}
  ...
  The number of points in these configurations is studied in \cref{thm:triangular_point_configuration_cardinality}. The recurrence obtained there is readily inferred from an informal geometric description in the spirit of \cref{fig:ex:def:triangular_point_configuration/basic/21} by \textcite[34]{ConwayGuy1998BookOfNumbers} and \textcite[4]{Deza2012FigurateNumbers}, who discuss figurate numbers in depth, as well as by \textcite[2]{Apostol1976AnalyticNumberTheory} and \textcite[14]{Розенфельд1963ОсновныеПонятиеГеометрии}.
...
\begin{example}\label{ex:def:triangular_point_configuration}
  ...
  \begin{thmenum}
    ...
    It is attributed to the \hyperref[rem:pythagoreanism]{Pythagorean school} as a geometric representation of concordant ratios in music theory. Other scientific and mystic properties ascribed to the tetractys are discussed  by \textcite[1]{Cornford1923PythagoreanMysticism} and \textcite[99]{Жмудь1990ПифагорИстория}.
  \end{thmenum}
\end{example}
...
\begin{proof}
  ...
  Let \( v' \) be a rotation of \( u \) by \( \alpha \) radians. \Cref{thm:hexagonal_point_lattice_vector_rotation} implies that \( v' \) is a point of \( L \). It is unidirectional with \( v \), thus \( v' = a v \) for some positive real number \( a \). Since \( u \) and \( v' \) are congruent, they have the same norm,
  \begin{equation*}
    \norm{u} = \norm{v'} = a \norm{v}.
  \end{equation*}
...
\begin{proof}
  ...
  \Cref{thm:v_minus_u_as_rotation} implies that \( v - u \) is a rotation of \( v \) by \( \pi / 3 \) radians. Then they have the same norm. \Cref{thm:primitive_hexagonal_lattice_vector_norm} then that, since \( v \) is primitive, so is \( v - u \).
...
\end{comments}
\begin{defproof}
  \Cref{thm:hexagonal_point_lattice_equilateral_triangle} implies that all sides have the same amount of points, so the notion is well-defined.
\end{defproof}
...
\begin{proof}
  ...
  Let \( n > 1 \). \Cref{thm:hexagonal_point_lattice_minimal_basis} implies that \( u \) and \( v \) form a basis of \( L \).
  ...
  \Cref{thm:v_minus_u_as_rotation} implies that \( v - u \) is also minimal, thus the triangle \( ABC \) is equilateral.
...
\end{definition}
\begin{comments}
  \item \Cref{thm:numeric_arithmetic_progression_partial_sums} implies that
  \begin{equation*}
  ...
  \item \Textcite[48]{Knuth1997ArtVol1} calls triangular numbers \enquote{termial numbers} and introduces the notation \( n? \) for the \( n \)-th \enquote{termial}.
\end{comments}
\begin{proof}
  ...
  \Cref{thm:triangular_point_configuration_cardinality} then implies that \eqref{eq:def:triangular_number/arithmetic} holds.
  ...
  \Cref{thm:triangular_point_configuration_existence} constructs a triangular point configuration of order \( n \). \Cref{thm:triangular_point_configuration_cardinality} then implies that this configuration has exactly \( m \) elements.
\end{proof}
...
\begin{definition}\label{def:hypercubic_point_configuration}\mimprovised
  ...
  Based on the dimension \( d \), we will replace the term \enquote{hypercubic} with \term{square} or \term{cubic} in accordance with \cref{def:hypercube_terminology}.
\end{definition}
...
\begin{example}\label{ex:def:hypercubic_point_configuration}
  ...
  \begin{thmenum}
    ...
    ; they are based on the illustration by \textcite[2]{Apostol1976AnalyticNumberTheory} used to motivate square numbers.
...
\begin{proof}
  ...
  Fix any \( 1 < k \leq d \). Since \( H \) is a parallelotope, the vectors \( v_1, \ldots, v_d \) are orthogonal. Thus, in the plane generated by \( v_1 \) and \( v_k \), we can rotate \( v_1 \) by either \( \pi / 2 \) or \( 3\pi / 2 \) radians to obtain a vector \( v_k' \) unidirectional with \( v_k \). \Cref{thm:hypercubic_point_lattice_vector_rotation} implies that \( v_k' \) is again a lattice vector, hence \( v_k' = a v_k \) for some real number \( a \geq 1 \). Similarly, we can rotate \( v_k \) to obtain a vector \( v_1' \) unidirectional with \( v_1 \), hence \( v_1' = b v_1 \) for some real number \( b \geq 1 \). Since \( v_k' \) and \( v_1 \) have the same norm, as do \( v_1' \) and \( v_k \), we have
  \begin{equation*}
    \norm{v_k} = \norm{v_1'} = a \norm{v_1} = a \norm{v_k'} = ab \norm{v_k},
  \end{equation*}
  hence \( ab = 1 \).
...
\begin{proof}
  ...
  \Cref{thm:hypercubic_point_configuration_cardinality} then implies that \( m = n^d \)
  ...
  \Cref{thm:hypercubic_point_configuration_existence} constructs a \( d \)-dimensional hypercubic point configuration of order \( n \). \Cref{thm:hypercubic_point_configuration_cardinality} then implies that this configuration has exactly \( m \) elements.
\end{proof}

\paragraph{Pythagorean triples}

\begin{definition}\label{def:fermats_equation}\mcite[48]{GowersEtAl2008PrincetonCompanion}
  \term[ru=уравнение Ферма (\cite[314]{Бухштаб1966ТеорияЧисел})]{Fermat's equation} of order \( n \) over a \hyperref[def:ring]{ring} is the \hyperref[def:polynomial_equation]{algebraic equation}
...
\begin{example}\label{ex:def:fermats_equation}
  ...
  \begin{thmenum}
    ...
    \thmitem{ex:def:fermats_equation/finite} Over the \hyperref[def:finite_field]{finite field} \( \BbbF_q \), \cref{thm:binomial_theorem_positive_characteristic} implies that
    \begin{equation*}
      a^q + b^q = (a + b)^q.
    \end{equation*}
...
\begin{theorem}[Fermat's last theorem]\label{thm:fermats_last_theorem}\mcite[8]{Хинчин1934ВеликаяТеоремаФерма}
  \hyperref[def:fermats_equation]{Fermat's equation} has no \hyperref[def:polynomial_equation]{nontrivial solutions} over the \hyperref[def:integers]{ring of integers} if \( n > 2 \).
\end{theorem}
\begin{comments}
...
\begin{remark}\label{rem:thm:fermats_last_theorem}
  ...
  Several books were written on the history of the theorem and the attempts to prove it. One of them was already mentioned. According to another, \textcite[8]{Хинчин1934ВеликаяТеоремаФерма}, in 1670 the son of Pierre de Fermat published a book about \hyperref[def:polynomial_equation/diophantine]{Diophantine equations}, containing some notes from Pierre the Fermat in its margins.
...
\end{definition}
\begin{comments}
  \item Both this definition and \cref{def:pythagorean_triple} are inspired by \fullref{thm:pythagorean_theorem}.
\end{comments}
...
\begin{definition}\label{def:rational_point}\mcite[67]{GowersEtAl2008PrincetonCompanion}
  We say that a point in a \hyperref[def:euclidean_space]{Euclidean space} is \term[ru=рациональная (точка) (\cite[22]{Александров1977ОбщаяТопология})]{rational} if its coordinates are \hyperref[def:rational_numbers]{rational numbers}.
\end{definition}
