...
\begin{remark}\label{rem:pythagoreanism}\mcite{Жмудь1990Пифагор}
...
\begin{example}\label{ex:def:triangular_point_configuration}
  ...
  \begin{thmenum}
    ...
    It is attributed to the \hyperref[rem:pythagoreanism]{Pythagorean school} as a geometric representation of concordant ratios in music theory. Other scientific and mystic properties ascribed to the tetractys are discussed  by \incite[1]{Cornford1923PythagoreanMysticism} and \incite[99]{Жмудь1990Пифагор}.
  \end{thmenum}
\end{example}
...
\begin{definition}\label{def:diophantine_equation}\mcite[5]{Apostol1976AnalyticNumberTheory}
  We call \hyperref[def:algebraic_equation]{algebraic equations} over the \hyperref[def:ring_of_integers]{ring of integers} \term[ru=Диофантово уравнение (\cite[\S 1.9]{ШеньВерещагин2017Вычислимость})]{Diophantine equations}.
\end{definition}

\begin{definition}\label{def:fermats_equation}\mcite[291]{Rosen2018DiscreteHandbook}
...
\begin{remark}\label{rem:thm:fermats_last_theorem}
  ...
  Next to the \hyperref[def:fermats_equation]{Pythagorean equation}, Fermat wrote a note, which can be found in the original Latin in the very beginning of Wiles' paper. In \cite[291]{Rosen2018DiscreteHandbook} it has been translated as follows:
  \begin{displayquote}
