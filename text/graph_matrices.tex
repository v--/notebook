...
\begin{remark}\label{rem:graph_network_terminology}
  ...
  \begin{itemize}
    \item \Textcite[655]{Knuth1997ArtVol1} defines a \enquote{network} as a simple graph, directed or undirected, where either the vertices or arcs/edges are weighted.

    \item \Textcite[13]{Мирчев2001Графи} defines a \enquote{network} (\enquote{мрежа}) similarly, but with weights assigned only to vertices.

    \item \Textcite[62]{Harary1969GraphTheory} defines a \enquote{network} similarly, but with weights assigned only to arcs/edges, and with weights restricted to positive real numbers.

    \item \Textcite[54]{Bollobás1998ModernGraphTheory} defines an \enquote{electric network} as a simple undirected graph with weighted edges.

    \item \Textcite[\S 7.3.3]{Новиков2013ДискретнаяМатематика} defines a \enquote{network} (\enquote{сеть}) as a simple directed graph with two distinguished vertices --- an initial and terminal vertex.

    \item \Textcite[151]{Diestel2017GraphTheory} defines a \enquote{network} as a directed multigraph with weighted arcs and two distinguished vertices, with the intention of studying \enquote{flows} between these vertices.

    \item \Textcite[def. 6.8.1]{Knauer2019AlgebraicGraphTheory} defines a \enquote{transportation network} as a simple directed weakly connected graph with two distinguished vertices and weighted arcs.

    \item \Textcite[655]{Knuth1997ArtVol1} defines a \enquote{network} as a graph of arbitrary kind where either the vertices or arcs/edges are weighted.

    \item \Textcite[def. 5.17]{БелоусовТкачёв2004ДискретнаяМатематика} define a \enquote{network} (\enquote{сеть}) as directed acyclic graphs and study distances from vertices with in-degree \( 0 \).
  \end{itemize}
...
\begin{comments}
  ...
  \item Only unweighted adjacency matrices are usually considered, for example in
  \textcite[28]{Diestel2017GraphTheory},
  \cite[151]{Harary1969GraphTheory},
  \cite[54]{Bollobás1998ModernGraphTheory},
  \cite[195]{Erickson2019Algorithms},
  \cite[32]{Зыков2004ТеорияГрафов},
  \cite[27]{ЕмеличевИПр1990ТеорияГрафов},
  \cite[\S 7.4.2]{Новиков2013ДискретнаяМатематика} and
  \cite[52]{Мирчев2001Графи}.

  \item Rather than considering explicit weights, \textcite[def. 2.1.1]{Knauer2019AlgebraicGraphTheory} define adjacency matrices for directed multigraphs as matrices of the underlying directed graph \hyperref[def:graph_functors/directed_forgetful]{\( U_D(G) \)}, weighted by the number of parallel arcs. We provide an example in \cref{ex:def:graph_adjacency_matrix/multi}.
\end{comments}
...
\end{proposition}
\begin{proof}
  \SufficiencySubProof* Suppose that \( G \) is acyclic. Denote by \( n \) the order of \( G \). By \cref{thm:acyclic_graph_walk_length}, it has no paths of length \( n \). \Cref{thm:adjacency_matrix_power} then implies that \( M^n = 0 \).

  \NecessitySubProof* Suppose that \( M^k = 0 \) for some positive integer \( k \). By \cref{thm:acyclic_graph_walk_length}, \( G \) has no paths of length \( k \). \Cref{thm:acyclic_graph_walk_length} then implies that \( G \) is acyclic.
\end{proof}
...
\begin{example}\label{ex:def:graph_adjacency_matrix}
  ...
  \begin{thmenum}
    ...
    \Cref{thm:adjacency_matrix_degree} implies that
    \begin{itemize}
...
\begin{proposition}\label{thm:adjacency_matrix_components}\mcite[thm. 2.1.8; thm. 2.1.9]{Knauer2019AlgebraicGraphTheory}
  Let \( G = (V, A) \) be a finite \hyperref[def:directed_graph]{simple directed graph}, with loops allowed. Let \( G_1, \ldots, G_s \) be an enumeration of its \hyperref[def:graph_connectedness/strong]{strongly connected components} such that there are no arcs from \( G_{k+1} \) to \( G_k \).
