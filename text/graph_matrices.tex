...
\begin{remark}\label{rem:networks}
  ...
  \begin{itemize}
    ...
    \item \incite[\textnumero 7.3.3]{Новиков2013} defines a \enquote{network} (\enquote{сеть}) as a simple directed graph with two distinguished vertices --- an initial and terminal vertex.
    ...
    \item \incite[655]{Knuth1997Vol1} defines a \enquote{network} as a graph of arbitrary kind where either the vertices or arcs/edges are weighted. \incite[13]{Мирчев2001} follows the same practice, again with the term \enquote{network} (\enquote{мрежа}).
...
\begin{definition}\label{def:graph_adjacency_matrix}\mimprovised
  Let \( G = (V, A) \) be a finite \hyperref[def:directed_graph]{simple directed graph} with loops and let \( v_1, \ldots, v_n \) be an enumeration of its vertices. We define the \term{unweighted} \term[bg=матрица на съседство (\cite[52]{Мирчев2001}), ru=матрица смежностей (\cite[32]{Зыков2004}), en=adjacency matrix (\incite[28]{Diestel2005})]{adjacency matrix} (resp. \term{weighted} with respect to the \hyperref[def:weighted_set]{weight function} \( w: A \to R \)) as the matrix
  \begin{equation*}
...
\begin{comments}
  ...
  \item Only unweighted adjacency matrices are usually considered, for example by \incite[28]{Diestel2005}, \incite[54]{Bollobas1998}, \incite[195]{Erickson2019}, \incite[32]{Зыков2004}, \incite[\textnumero 7.4.2]{Новиков2013} and \incite[52]{Мирчев2001}. All of the aforementioned authors agree on the definition of unweighted adjacency matrix, and we simply extend that with weights.

  \item Rather than considering explicit weights, \incite[552]{Rosen1999} and \incite[def. 2.1.1]{Knauer2011} define adjacency matrices for directed multigraphs as matrices of the underlying directed graph with loops \hyperref[def:graph_functors/directed_forgetful]{\( U_D(G) \)} weighted by the number of parallel arcs. We provide an example in \fullref{ex:def:graph_adjacency_matrix/multi}.
\end{comments}
...
\begin{proposition}\label{thm:adjacency_matrix_components}\mcite[thm. 2.1.8; thm. 2.1.9]{Knauer2011}
  Let \( G = (V, A) \) be a finite \hyperref[def:directed_graph]{simple directed graph} with loops. Let \( G_1, \ldots, G_s \) be an enumeration of its \hyperref[def:graph_connectedness/strong]{strongly connected components} such that there are no arcs from \( G_{k+1} \) to \( G_k \).
