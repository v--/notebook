...
We shall not attempt to give a precise definition for an \term{algebraic structure}. There are very general frameworks for doing so --- for example \term{algebras over monads} discussed in \cite[ch. VI]{MacLane1998} or \term{universal algebras} discussed in \cite[sec. 1.2.]{ЦаленкоШульеейфер1974}. Their complexity, however, is unjustified for us. We will instead build standard algebraic structures from \enquote{base building blocks}, although we will utilize very general definitions like categorical kernels and images defined in \fullref{def:zero_morphisms}.
