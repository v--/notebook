...
\begin{proof}
  ...
  \SubProofOf{thm:ordinal_cofinality/successor} For any successor ordinal \( \beta = \op{succ}(\alpha) \), since \( \alpha \) is a maximum of \( \beta \), \fullref{thm:def:cofinal_set/top} implies that a subset of \( \beta \) is cofinal if and only if it contains \( \alpha \). Hence, \( \set{ \alpha } \) is cofinal (while \( \varnothing \) is not). Therefore, \( \beta \) has cofinality \( 1 \).
