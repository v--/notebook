...
\begin{remark}\label{rem:type_theory_rule_classification}
  ...
  \begin{thmenum}
    ...
    \thmitem{rem:type_theory_rule_classification/introduction} An \term{introduction rule} allows us to assert the type to a term. In the words of Martin-L\"of, \enquote{The introduction rules say what are the canonical elements}.
    ...
    Furthermore, to ensure that \( {\syncoloneqq} \) is an \hyperref[def:equivalence_relation]{equivalence relation}, we need inferences rules resembling those from \fullref{ex:recursively_defined_relation}:
    \begingroup
    ...
    Unlike in Martin-L\"of's presentation, in \cite[27]{UnivalentProject2024OctoberHoTT}, the other equality rules are split into two kinds, additionally \incite[\S 8.1.9]{Mimram2020ProgramEqualsProof} also distinguishes congruence rules, which the former assume implicit:
    \begin{thmenum}
      ...
      \thmitem{rem:type_theory_rule_classification/equality/congruence} A \term{congruence rule} propagates \hyperref[con:equality]{definitional equality}, allowing substituting definitionally equal terms and types and making type derivation \hyperref[con:extensionality]{extensional}.

      If, in accordance with \fullref{con:equality}, we denote the equality judgment
      \begin{center}
        \( \synx \) and \( \syny \) are definitionally equal terms of type \( \tau \)
      \end{center}
      by
