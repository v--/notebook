...
\begin{concept}\label{con:curry_howard_correspondence}
  \textcite[479]{Howard1980FormulasAsTypes} writes
  \begin{displayquote}
...
\end{concept}
\begin{comments}
  \item From the perspective of first-order logic, the correspondence is described in \cite[def. 5.1.11]{Mimram2020ProgramEqualsProof}.

  \item Different authors refer to this concept slightly differently:
  \begin{itemize}
    \item This phrase \enquote{Curry-Howard correspondence} is used by \textcite[45]{AwodeyWarren2009HoTT} and \textcite[def. 4.1.7]{Mimram2020ProgramEqualsProof}.

    \item A variation, the \enquote{Curry-Howard isomorphism}, is used by \textcite[74]{Hindley1997BasicSTT} and \textcite[14]{GirardEtAl1989ProofsAndTypes}.

    \item An alternative suggested by Howard himself is \enquote{formulas-as-types correspondence}, also used by \textcite[74]{Hindley1997BasicSTT}, \textcite[\S 1.3.4]{TroelstraSchwichtenberg2000BasicProofTheory} and \textcite[572]{Barendregt1984LambdaCalculus}.

    \item Another variation, the \enquote{propositions-as-types correspondence}, is used by \textcite[def. 5.4.14]{Barendregt1992LambdaCalculiWithTypes}, as well as by the aforementioned \textcite[8]{AwodeyWarren2009HoTT} and \textcite[def. 4.1.7]{Mimram2020ProgramEqualsProof}.
  \end{itemize}
\end{comments}

\begin{remark}\label{rem:type_theory_rule_classification}
  \textcite[24]{MartinLöf1984IntuitionisticTypeTheory} uses (metatheoretic) \hyperref[def:inference_rule]{inference rules} to build his entire type theory, without an underlying \hyperref[con:logical_system]{logical system} (or, rather, with only a minimalist one), and hence without \( \muplambda \)-calculus available.
  ...
  \begin{thmenum}
    ...
    \end{bnf*}
    which we can recast as the inference rule using a fixed \hyperref[con:type_universe]{type universe} \( \BbbU \):
    \begin{equation*}
      \begin{prooftree}
        \hypo{ \tau: \BbbU }
        \hypo{ \sigma: \BbbU }
        \infer2[\ensuremath{ \rightarrow_{\logic{form}} }]{ \tau \synimplies \sigma: \BbbU }
      \end{prooftree}
    \end{equation*}

    Note that, since the rules are part of the metalanguage, we do not try to formalize them and hence do not use formalized schemas. Here \( \tau \) and \( \sigma \) are metalingual variables.
      ...
      \begin{rightcolumn}
        ...
        \begin{equation*}\taglabel[\ensuremath{ \coloneqq_{\logic{term}} }]{rem:type_theory_rule_classification/equality/term}
          \begin{prooftree}
            \hypo{ M \judgeq M': \tau }
            \hypo{ \tau \judgeq \tau': \BbbU }
            \infer2[\ref{rem:type_theory_rule_classification/equality/term}]{ M \judgeq M': \tau' }
          \end{prooftree}
        \end{equation*}
      \end{rightcolumn}
    \end{paracol}

    Furthermore, to ensure that \( {\judgeq} \) is an \hyperref[def:equivalence_relation]{equivalence relation}, we need inference rules resembling those from \cref{thm:generated_equivalence_relation}:
    ...
    \begin{paracol}{3}
      \begin{nthcolumn}{0}
        ...
        \begin{equation*}\taglabel[\ensuremath{ \coloneqq_{\logic{refl}} }]{rem:type_theory_rule_classification/equality/refl}
          \begin{prooftree}
            \hypo{ M: \tau }
            \infer1[\ref{rem:type_theory_rule_classification/equality/refl}]{ M \judgeq M: \tau }
          \end{prooftree}
        \end{equation*}
      \end{nthcolumn}
      ...
      \begin{nthcolumn}{1}
        ...
        \begin{equation*}\taglabel[\ensuremath{ \coloneqq_{\logic{symm}} }]{rem:type_theory_rule_classification/equality/symm}
          \begin{prooftree}
            \hypo{ M \judgeq N: \tau }
            \infer1[\ref{rem:type_theory_rule_classification/equality/symm}]{ N \judgeq M: \tau }
          \end{prooftree}
        \end{equation*}
      \end{nthcolumn}
      ...
      \begin{nthcolumn}{2}
        ...
        \begin{equation*}\taglabel[\ensuremath{ \coloneqq_{\logic{trans}} }]{rem:type_theory_rule_classification/equality/trans}
          \begin{prooftree}
            \hypo{ M \judgeq N: \tau }
            \hypo{ N \judgeq K: \tau }
            \infer2[\ref{rem:type_theory_rule_classification/equality/trans}]{ M \judgeq K: \tau }
          \end{prooftree}
        \end{equation*}
      \end{nthcolumn}
    \end{paracol}
    ...
    These five \enquote{common} equality rules are given in \cite[433]{UnivalentFoundationsProgram2013HoTT} and, in a less refined form, they are also discussed by \textcite[14,15]{MartinLöf1984IntuitionisticTypeTheory}.

    Finally, since we use \hyperref[alg:lambda_term_substitution]{substitution}, and since we are explicit about \hyperref[def:lambda_term_alpha_equivalence]{\( \alpha \)-equivalence}, we must also add the following rules mimicking \fullref{alg:simply_typed_substitution} and \fullref{alg:simply_typed_alpha_conversion}:
    \begin{paracol}{2}
      \begin{leftcolumn}
        \ParacolAlignmentHack
        \begin{equation*}\taglabel[\ensuremath{ \coloneqq_\mapsto }]{rem:type_theory_rule_classification/equality/subst}
          \begin{prooftree}
            ...
            \hypo{ N: \tau }
            \infer2[\ref{rem:type_theory_rule_classification/equality/subst}]{ M[x \judgeq N]: \sigma }
          \end{prooftree}
        \end{equation*}
      \end{leftcolumn}
      ...
      \begin{rightcolumn}
        ...
        \begin{equation*}\taglabel[\ensuremath{ \coloneqq_\alpha }]{rem:type_theory_rule_classification/equality/alpha}
          \begin{prooftree}
            ...
            \hypo{ N: \tau }
            \hypo{ M \aequiv N }
            \infer3[\ref{rem:type_theory_rule_classification/equality/alpha}]{ M \judgeq N: \tau }
          \end{prooftree}
        \end{equation*}
      \end{rightcolumn}
    \end{paracol}

    Unlike in Martin-L\"of's presentation, in \cite[27]{UnivalentFoundationsProgram2013HoTT}, the other equality rules are split into two kinds. Additionally, they implicitly assume the presence of congruence rules, which \textcite[\S 8.1.9]{Mimram2020ProgramEqualsProof} makes explicit.
    ...
    \begin{thmenum}
      ...
      \begin{equation*}
        \begin{prooftree}
          ...
          \hypo{ N: \tau }
          \infer2[\ensuremath{ \rightarrow_\beta }]{ (\qabs {x^\tau} M) N \judgeq M[x \mapsto N]: \sigma }.
        \end{prooftree}
      \end{equation*}
      ...
      \begin{equation*}
        \begin{prooftree}
          \hypo{ M: \tau \synimplies \sigma }
          \infer1[\ensuremath{ \rightarrow_\eta }]{ M \judgeq \qabs {x^\tau} M x: \tau \synimplies \sigma },
        \end{prooftree}
      \end{equation*}
      ...
      This side condition resembles the conditions for eigenvariables from \cref{def:fol_natural_deduction_proof_tree}.
      ...
      \begin{equation*}
        \begin{prooftree}
          \hypo{ \tau \judgeq \tau': \BbbU }
          \hypo{ \sigma \judgeq \sigma': \BbbU }
          \infer2{ \tau \synimplies \sigma \judgeq \tau' \synimplies \sigma': \BbbU }
        \end{prooftree}
      \end{equation*}
      ...
      \begin{paracol}{2}
        \begin{leftcolumn}
          ...
          \begin{equation*}
            \begin{prooftree}
              \hypo{ M \judgeq M': \sigma }
              \infer1{ \qabs {x^{\tau}} M \judgeq \qabs {x^{\tau}} M': \tau \synimplies \sigma }
            \end{prooftree}
          \end{equation*}
        \end{leftcolumn}
        ...
        \begin{rightcolumn}
          ...
          \begin{equation*}
            \begin{prooftree}
              \hypo{ M \judgeq M': \tau \synimplies \sigma }
              \hypo{ N \judgeq N': \tau }
              \infer2{ M N \judgeq M' N': \sigma }
            \end{prooftree}
          \end{equation*}
        \end{rightcolumn}
      \end{paracol}
...
\end{remark}
\begin{comments}
  \item Our list of rules is based on the corresponding rules for dependent products in \cite{MartinLöf1984IntuitionisticTypeTheory}, \cite[\S A.2.4]{UnivalentFoundationsProgram2013HoTT} and \cite[\S 8.1.10]{Mimram2020ProgramEqualsProof}.
...
\begin{comments}
  ...
  \item We have formulated the rule so that it resembles \ref{inf:def:propositional_natural_deduction/bot/efq}. We do not forbid a variable to have type \( \syn\Bbbzero \), but provide a term of arbitrary type given such a variable.
...
\begin{definition}\label{def:simple_unit_type}\mimprovised
  ...
  There is an \hyperref[rem:type_theory_rule_classification/intro]{introduction rule} resembling the verum rule \ref{inf:def:propositional_natural_deduction/top/intro}:
...
\begin{definition}\label{def:simple_product_type}\mimprovised
  ...
  Based on the natural deduction rules for conjunction formulas from \cref{def:propositional_natural_deduction/and}, we state one \hyperref[rem:type_theory_rule_classification/intro]{introduction rule} and two \hyperref[rem:type_theory_rule_classification/elim]{elimination rules}:
  \begin{paracol}{3}
    \begin{nthcolumn}{0}
      \ParacolAlignmentHack
...
\begin{remark}\label{rem:simple_product_type_equality_rules}
  ...
  \Textcite[corr. 1.23]{Barendregt1974SurjectivePairing} demonstrates that there is no triple of untyped \( \muplambda \)-terms \( P_+ \), \( P_{-L} \) and \( P_{+R} \) satisfying \eqref{eq:rem:simple_product_type_equality_rules/eta}.
...
\begin{definition}\label{def:simple_sum_type}\mimprovised
  Similarly to \hyperref[def:simple_product_type]{product types}, we will introduce rules for the sum type \( \tau \synplus \sigma \) based on the discussion in \cref{ex:def:beta_eta_reduction/disjoint} and \cref{ex:def:type_derivation_tree/disjoint}. We will need the constants \( \synS_{+L} \), \( \synS_{+R} \) and \( \synS_- \) and three rules resembling the disjunction rules from \cref{def:propositional_natural_deduction/or}:
  \columnratio{0.25,0.25,0.5}
  \begin{paracol}{3}
    \begin{nthcolumn}{0}
      \ParacolAlignmentHack
...
\begin{remark}\label{rem:curry_howard_variables}
  ...
  We have defined propositional variables in \cref{def:propositional_formula} as small Latin identifiers, and type variables in \cref{def:simple_type} as small Greek identifiers. We must find a way to unify the two when translating between types and formulas.
...
\begin{algorithm}[Type derivation to proof tree]\label{alg:type_derivation_to_proof_tree}
  ...
  We give a straightforward algorithm for converting a \hyperref[def:type_derivation_tree]{type derivation tree} in this system into a \hyperref[def:propositional_natural_deduction_proof_tree]{proof tree} for the \hyperref[def:propositional_natural_deduction]{propositional intuitionistic natural deduction system}.

  \begin{thmenum}
    \thmitem{alg:type_derivation_to_proof_tree/form} First, via \hyperref[con:evaluation]{pattern matching} on the type \( \tau \), we define an operator for converting types into \hyperref[def:propositional_formula]{propositional formulas}:
    \begin{equation*}
    ...
    \thmitem{alg:type_derivation_to_proof_tree/instantiation} As discussed in \cref{ex:def:propositional_natural_deduction/instantiation_dependence}, the application of some rules of natural deduction like \ref{inf:def:propositional_natural_deduction/bot/efq}, \ref{inf:def:propositional_natural_deduction/or/intro_left} and \ref{inf:def:propositional_natural_deduction/or/intro_right} requires an explicit \hyperref[alg:propositional_schema_instantiation]{propositional schema instantiation}.

    Each derivation tree has an associated \hyperref[def:atomic_lambda_instantiation]{\( \muplambda \)-schema instantiation} from which we can infer the required propositional instantiation.

    It will be sufficient to specify how the instantiation acts on formula placeholders --- the rest can be inferred from the premises. Given an atomic \( \muplambda \)-schema instantiation \( \BbbI \), we define the propositional instantiation \( \op*{Inst}(\BbbI) \) by matching, in order, the formula placeholders of the \( k \)-th premise of \( R \) to the type placeholder of the \( k \)-th premise of \( \op*{Rule}(R) \).
    ...
    Here \( \BbbI \) maps the type placeholder \( \syn\tau \) to \( \tau \) and \( \syn\sigma \) to \( \sigma \). For the corresponding rule \ref{inf:def:propositional_natural_deduction/or/intro_left}, \( \op*{Inst}(\BbbI) \) maps \( \syn\varphi \) to \( \tau \) and \( \syn\psi \) to \( \sigma \).

    This allows us to unambiguously apply the rule \ref{inf:def:propositional_natural_deduction/or/intro_left} with \( \op*{Inst}(\BbbI) \) and obtain
    \begin{equation*}
      \begin{prooftree}
        \hypo{ \tau }
        \infer1[\ref{inf:def:propositional_natural_deduction/or/intro_left}]{ \tau \synvee \sigma }
      \end{prooftree}
    \end{equation*}
...
\begin{example}\label{ex:con:curry_howard_correspondence}
  ...
  \begin{thmenum}
    ...
    \begin{equation*}
      \begin{prooftree}
        \hypo{ [\tau]^\synx }
        \infer[left label=\( \synx \)]1[\ref{inf:def:propositional_natural_deduction/imp/intro}]{ \tau \synimplies \tau }
      \end{prooftree}
    \end{equation*}
    ...
    \begin{equation*}
      \begin{prooftree}
        \hypo{ [\tau]^\synx }
        \infer1[\ref{inf:def:propositional_natural_deduction/imp/intro}]{ \sigma \synimplies \tau }
        \infer[left label=\( \synx \)]1[\ref{inf:def:propositional_natural_deduction/imp/intro}]{ \tau \synimplies (\sigma \synimplies \tau) }
      \end{prooftree}
    \end{equation*}
    ...
    \thmitem{ex:con:curry_howard_correspondence/beta_reduction} We will show in \cref{rem:beta_reduction_and_cut_elimination} how \hyperref[def:beta_eta_reduction]{\( \beta \)-reduction} corresponds to a special case of \fullref{thm:cut_elimination}.
  \end{thmenum}
\end{example}
