...
\begin{remark}\label{rem:type_theory_rule_classification}
  ...
  \begin{thmenum}
    ...
    Furthermore, to ensure that \( {\judgeq} \) is an \hyperref[def:equivalence_relation]{equivalence relation}, we need inference rules resembling those from \cref{ex:recursively_defined_relation}:
...
\begin{example}\label{ex:con:curry_howard_correspondence}
  ...
  \begin{thmenum}
    ...
    \thmitem{ex:con:curry_howard_correspondence/beta_reduction} We will show in \cref{rem:beta_reduction_and_cut_elimination} how \hyperref[def:beta_eta_reduction]{\( \beta \)-reduction} corresponds to a special case of \hyperref[def:abstract_sequent_calculus_system]{cut elimination}.
  \end{thmenum}
\end{example}
