...
\begin{remark}\label{rem:universal_graph_cardinals}
  In the special case where \( S \) is a \hyperref[def:cardinal_number]{cardinal number} \( \rho \), the vertices of \( \oline{K_\rho} \) are simply the smaller ordinals. This is discussed in \fullref{rem:cardinal_colorings} regarding \( \rho \)-colorings.
...
\end{definition}
\begin{comments}
  \item The term \enquote{clique} is used as a synonym for complete subgraph by \incite[def. 1.4.11]{Knauer2011}, \incite[5]{GondranMinoux1984Graphs}, \incite[394]{Erickson2019}, \incite[111]{Емеличев1990} (as \enquote{клика}) and \incite[\S 7.3.1]{Новиков2013} (as \enquote{клика}). Other authors use it to mean a maximal complete subgraph, for example \incite[20]{Harary1969}, \incite[112]{Bollobas1998}, \incite[596]{Rosen1999} and \incite[103]{Мирчев2001} (as \enquote{клика}).
...
\begin{theorem}[Mantel's theorem]\label{thm:mantel_theorem}\mcite[thm. I.2]{Bollobas1998}
  If a \hyperref[def:undirected_graph]{simple undirected graph} with \( n \) vertices has more than \( n^2 / 4 \) edges, it contains a \hyperref[def:triangle_subgraph]{triangle subgraph}.
\end{theorem}
\begin{proof}
