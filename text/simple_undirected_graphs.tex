...
\begin{proposition}\label{thm:simple_undirected_graph_isomorphisms}
  Fix two simple undirected graphs \( G = (V_G, E_G) \) and \( H = (V_H, E_H) \). A function \( f: V_G \to V_H \) is a \hyperref[def:morphism_invertibility/isomorphism]{categorical isomorphism} from \( G \) to \( H \) if and only if both \( f \) and \( f^{-1} \) are \hyperref[def:undirected_graph/homomorphism]{undirected graph homomorphisms} or, more succinctly, if
  \begin{equation}\label{eq:thm:simple_undirected_graph_isomorphisms}
