...
\begin{comments}
  ...
  \item For \( \oline{K_n} \), \incite[18]{Bollobas1998Graphs} prefers the term \enquote{empty graph}, while \incite[def. 1.3.1]{Knauer2019AlgGraphTheory} uses \enquote{totally disconnected} and \enquote{discrete graph}. \incite[16]{Harary1969Graphs} also uses \enquote{totally disconnected}.
\end{comments}
...
\begin{comments}
  ...
  \item An \hyperref[def:multigraph_orientation]{orientation} of a complete graph is sometimes called a \enquote{tournament} --- for example by \incite[319]{Diestel2017Graphs}, \incite[30]{Bollobas1998Graphs} and \incite[\S 7.3.3]{Новиков2013ДискретнаяМатематика} (as \enquote{турнир}).
\end{comments}
...
\end{definition}
\begin{comments}
  \item The term \enquote{clique} is used as a synonym for complete subgraph by \incite[def. 1.4.11]{Knauer2019AlgGraphTheory}, \incite[5]{GondranMinoux1984Graphs}, \incite[394]{Erickson2019Algorithms}, \incite[111]{ЕмеличевИПр1990Графы} (as \enquote{клика}) and \incite[\S 7.3.1]{Новиков2013ДискретнаяМатематика} (as \enquote{клика}). Other authors use it to mean a maximal complete subgraph, for example \incite[20]{Harary1969Graphs}, \incite[112]{Bollobas1998Graphs} and \incite[103]{Мирчев2001Графи} (as \enquote{клика}).
...
\begin{definition}\label{def:path_graph}\mcite[def. 1.3.1]{Knauer2019AlgGraphTheory}
  Given every positive integer \( n \), we regard it as the set of smaller cardinals and define the \term[en=path (\cite[4]{Bollobas1998Graphs})]{path graph} \( P_n \) as the \hyperref[def:undirected_graph]{simple undirected graph}
  \begin{equation*}
...
\begin{definition}\label{def:graph_box_product}\mcite[def. 4.3.1]{Knauer2019AlgGraphTheory}
  We define the \term{box product} \( G \boxprod H \) of two \hyperref[def:simple_undirected_graph]{simple undirected graphs} \( G \) and \( H \) as the graph whose vertex set is the \hyperref[def:cartesian_product/product]{Cartesian product} \( V_G \times V_H \) and whose edge set is
...
\begin{comments}
  ...
  \item We can characterize \( Q_n \) via bit strings --- see \fullref{thm:hypercube_graphs_as_strings}.
\end{comments}

\begin{proposition}\label{thm:hypercube_graphs_as_strings}
  The \hyperref[def:hypercube_graph]{hypercube graph} \( Q_n \) is \hyperref[def:undirected_graph/homomorphism]{isomorphic} to the graph whose vertices are \hyperref[def:positional_number_system/binary]{bit} strings of length \( n \), with two strings adjacent if their \hyperref[def:hamming_distance]{Hamming distance} is \( 1 \).
\end{proposition}
\begin{proof}
...
\begin{definition}\label{def:petersen_graph}\mcite[2]{Watkins1969PetersenGraphs}
  ...
  \begin{figure}[!ht]
    \begin{subcaptionblock}{\textwidth/2}
      \centering
      \includegraphics{output/def__petersen_graph__p52}
      \caption{\( P_{5,2} \)}\label{fig:def:petersen_graph/p52}
    \end{subcaptionblock}
    \hfill
    \begin{subcaptionblock}{\textwidth/2}
      \centering
      \includegraphics{output/def__petersen_graph__p73}
      \caption{\( P_{7,3} \)}\label{fig:def:petersen_graph/p73}
    \end{subcaptionblock}
    \par
    \begin{subcaptionblock}{\textwidth/2}
      \centering
      \includegraphics{output/def__petersen_graph__p31}
      \caption{\( P_{3,1} \)}\label{fig:def:petersen_graph/p31}
    \end{subcaptionblock}
    \hfill
    \begin{subcaptionblock}{\textwidth/2}
      \centering
      \includegraphics{output/def__petersen_graph__p21}
...
\begin{definition}\label{def:multipartite_graph}\mcite[6]{Bollobas1998Graphs}
...
\begin{proposition}\label{thm:square_graphs}
  ...
  \begin{figure}[!ht]
    ...
    \end{subcaptionblock}
    \hfill
    \begin{subcaptionblock}{\textwidth/3}
      \centering
...
\begin{theorem}[Mantel's theorem]\label{thm:mantel_theorem}\mcite[thm. I.2]{Bollobas1998Graphs}
