...
\begin{definition}\label{def:cycle_graph}\mcite[def. 1.3.1]{Knauer2019AlgGraphTheory}
  For every integer \( n > 2 \), we define \term[en=cycle graph (\cite[573]{Rosen2018DiscreteHandbook})]{cycle graph} \( C_n \) as \hyperref[def:path_graph]{\( P_n \)} with an additional edge connecting \( n - 1 \) to \( 0 \).
...
\begin{comments}
  ...
  \item \incite[21]{Harary1969Graphs} and \incite[def. 4.1.1]{Knauer2019AlgGraphTheory} use regular unions with the convention that the vertex sets and edge sets are disjoint. \incite[19]{ЕмеличевИПр1990Графы} also use regular unions, however they explicitly refer to unions as \enquote{disjoint} (\enquote{дизъюнктное объединение}) if the vertex sets and edge sets are disjoint.
\end{comments}
