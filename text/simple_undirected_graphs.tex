...
\begin{comments}
  ...
  \item Following \textcite[16]{Harary1969GraphTheory}, we use the notation \( \oline{K_S} \). This is justified by edgeless graphs being \hyperref[def:graph_complement]{complements} of \hyperref[def:complete_graph]{complete graphs}, which are in turn denoted by \( K_n \). We find it simpler to define them the other way around.

  \item For \( \oline{K_n} \), \textcite[18]{Bollobás1998ModernGraphTheory} prefers the term \enquote{empty graph}, while \textcite[def. 1.3.1]{Knauer2019AlgebraicGraphTheory} uses \enquote{totally disconnected} and \enquote{discrete graph}. \Textcite[16]{Harary1969GraphTheory} also uses \enquote{totally disconnected}.
\end{comments}
...
\begin{theorem}[Edgeless graph universal property]\label{thm:edgeless_graph_universal_property}
  Given a set \( S \), the \hyperref[def:edgeless_graph]{edgeless graph} \( \oline{K_S} \) is the unique up to a unique isomorphism simple undirected graph that satisfies the following \hyperref[con:universal_property]{universal property}:
  \begin{displayquote}
    For every simple undirected graph \( G = (V, E) \) and every function \( f: S \to V \), there exists a unique \hyperref[def:undirected_graph/morphism]{homomorphism} \( \widetilde{f}: \oline{K_S} \to G \) such that the following diagram commutes:
    \begin{equation*}
      \begin{aligned}
...
\end{theorem}
\begin{comments}
  \item \Cref{thm:adjunction_from_universal_morphism/unit} implies that \( V \mapsto \oline{K_V} \) is \hyperref[def:adjoint_functor]{left adjoint} to the \hyperref[con:forgetful_functor]{forgetful functor} from simple undirected graphs to sets.

  \Cref{thm:concrete_category_function_invertibility/injective} then implies that the \hyperref[def:morphism_cancellation/left]{categorical monomorphisms} are precisely the injective graph homomorphisms.
\end{comments}
\begin{proof}
...
\begin{comments}
  ...
  \item An \hyperref[def:multigraph_orientation]{orientation} of a complete graph is sometimes called a \enquote{tournament} --- for example in
  \cite[319]{Diestel2017GraphTheory},
  \cite[30]{Bollobás1998ModernGraphTheory} and
  \cite[\S 7.3.3]{Новиков2013ДискретнаяМатематика} (as \enquote{\textru{турнир}}).
\end{comments}
...
\begin{theorem}[Complete graph universal property]\label{thm:complete_graph_universal_property}
  Given a set \( S \), the \hyperref[def:complete_graph]{complete graph} \( K_S \) is the unique up to a unique isomorphism simple undirected graph that satisfies the following \hyperref[con:universal_property]{universal property}:
  \begin{displayquote}
    For every simple undirected graph \( G = (V, E) \) and every function \( f: V \to S \), there exists a unique \hyperref[def:undirected_graph/morphism]{homomorphism} \( \widetilde{f}: G \to K_S \) such that the following diagram commutes:
...
\end{theorem}
\begin{comments}
  \item \Cref{thm:adjunction_from_universal_morphism/counit} implies that \( V \mapsto K_V \) is \hyperref[def:adjoint_functor]{right adjoint} to the \hyperref[con:forgetful_functor]{forgetful functor} from simple undirected graphs to sets.

  \Cref{thm:concrete_category_function_invertibility/surjective} then implies that the \hyperref[def:morphism_cancellation/right]{categorical epimorphism} are precisely the surjective graph homomorphisms.
\end{comments}
\begin{proof}
...
\begin{definition}\label{def:complete_subgraph}\mimprovised
  We say that the \hyperref[def:undirected_graph/subobject]{subgraph} \( H \) of the \hyperref[def:undirected_graph]{simple undirected graph} \( G \) is \term{complete} if any of the following equivalent conditions hold:
  \begin{thmenum}
    \thmitem{def:complete_subgraph/direct} There is an edge in \( H \) between any pair of vertices in \( H \).

    \thmitem{def:complete_subgraph/isomorphic} \( H \) is \hyperref[def:undirected_graph/morphism]{isomorphic} to the \hyperref[def:complete_graph]{complete graph} \( K_\rho \), where \( \rho \) is the (cardinal) \hyperref[def:graph_cardinality/order]{order} of \( H \).
...
\end{definition}
\begin{comments}
  \item The term \enquote{clique} is used as a synonym for complete subgraph in
  \cite[def. 1.4.11]{Knauer2019AlgebraicGraphTheory},
  \cite[5]{GondranMinoux1984GraphsAndAlgorithms},
  \cite[394]{Erickson2019Algorithms},
  \cite[111]{ЕмеличевИПр1990ТеорияГрафов} (as \enquote{\textru{клика}}) and
  \cite[\S 7.3.1]{Новиков2013ДискретнаяМатематика} (as \enquote{\textru{клика}}).

  Other authors use it to mean a maximal complete subgraph, for example
  \cite[20]{Harary1969GraphTheory},
  \cite[112]{Bollobás1998ModernGraphTheory} and
  \cite[103]{Мирчев2001Графи} (as \enquote{\textbg{клика}}).
...
\begin{comments}
  ...
  \item \Textcite[21]{Harary1969GraphTheory}, \textcite[def. 4.1.1]{Knauer2019AlgebraicGraphTheory}, \textcite[def. 10.2.9]{Rosen2019DiscreteMathematics} and \textcite[23]{Зыков2004ТеорияГрафов} use ordinary unions with the convention that the vertex sets and edge sets are disjoint. \Textcite[19]{ЕмеличевИПр1990ТеорияГрафов} also use regular unions, however they explicitly refer to unions as \enquote{disjoint} (\enquote{дизъюнктное объединение}) if the vertex sets and edge sets are disjoint.
\end{comments}

\begin{proposition}\label{thm:undirected_graph_coproduct}
  The \hyperref[def:categorical_coproduct]{categorical coproduct} of a family of graphs in the category of small \hyperref[def:undirected_graph]{simple undirected graphs} is their \hyperref[def:graph_disjoint_union]{disjoint union}.
\end{proposition}
\begin{proof}
  Let \( (H, \alpha) \) be a \hyperref[def:category_of_cocones/objects]{cocone} for the discrete diagram \( \seq{ G_k }_{k \in \mscrK} \). We want to define a graph homomorphism \( l: \coprod_{k \in \mscrK} G_k \to H \) such that, for every \( m \in \mscrK \),
  \begin{equation*}
...
\end{definition}
\begin{comments}
  \item \Textcite[21]{Harary1969GraphTheory} attributes the introduction of graph joins to \cite[164]{Зыков1949СоединенияГрафов}. Zykov calls them \enquote{произведения} (\enquote{products}) and denotes them via juxtaposition, however his paper generally uses arcane terminology like \enquote{complex} instead of \enquote{graph}. In a later book, \cite[23]{Зыков2004ТеорияГрафов}, Zykov uses the same term, but translates it as \enquote{join}.

  \textcite[def. 4.1.4]{Knauer2019AlgebraicGraphTheory} uses both our terminology and our notation. \Textcite[4]{Diestel2017GraphTheory} uses \enquote{\( \Ast \)}, however avoids introducing a special term. The term \enquote{term} is used by \textcite[21]{Harary1969GraphTheory} and \textcite[\S 7.3.1]{Новиков2013ДискретнаяМатематика} (as \enquote{\textru{соединение}}), however the latter two authors denote this operation by \enquote{\( + \)}.
\end{comments}
...
\begin{definition}\label{def:graph_box_product}\mcite[def. 4.3.1]{Knauer2019AlgebraicGraphTheory}
  We define the \term{box product} \( G \square H \) of two \hyperref[def:undirected_graph]{simple undirected graphs} \( G \) and \( H \) as the graph whose vertex set is the \hyperref[def:cartesian_product]{Cartesian product} \( V_G \times V_H \) and whose edge set is
  ...
  \begin{equation}\label{eq:def:graph_box_product/recursive}
    G^{\square n} \coloneqq \begin{cases}
      K_1                           &n = 0 \\
      G^{\square n - 1} \square G &n > 1.
    \end{cases}
  \end{equation}
\end{definition}
\begin{comments}
...
\begin{proposition}\label{thm:box_product_of_complete_graphs}
  In the case of \hyperref[def:complete_graph]{complete graphs} \( K_A \) and \( K_B \), two vertices in their \hyperref[def:graph_box_product]{box product} \( K_A \square K_B \) are adjacent if and only if either their first or second components match (but not both).
\end{proposition}
\begin{proof}
...
\begin{proposition}\label{thm:hypercube_graphs_as_strings}
  The \hyperref[def:hypercube_graph]{hypercube graph} \( Q_n \) is \hyperref[def:undirected_graph/morphism]{isomorphic} to the graph whose vertices are \hyperref[def:bit_string]{bit strings} of length \( n \), with two strings adjacent if their \hyperref[def:hamming_distance]{Hamming distance} is \( 1 \).
\end{proposition}
\begin{proof}
  ...
  Suppose that the proposition holds for \( n - 1 \) and consider \( Q_n \), which is, by definition, \( Q_{n-1} \square K_2 \).
...
\end{proposition}
\begin{proof}
  We have \( 2n \) vertices and \( n \) edges by \cref{def:petersen_graph/union}. \Cref{def:petersen_graph/inter} and \cref{def:petersen_graph/inter} each add \( n \) edges.
\end{proof}
...
\end{definition}
\begin{itemize}
  \item It is tempting to require the subsets of the partition to be nonempty, however many authors choose not to do so. We follow the lead of
  \cite[17]{Diestel2017GraphTheory},
  \cite[27]{Harary1969GraphTheory},
  \cite[6]{Bollobás1998ModernGraphTheory},
  \cite[def. 1.3.1]{Knauer2019AlgebraicGraphTheory},
  \cite[26]{GondranMinoux1984GraphsAndAlgorithms} and
  \cite[11]{ЕмеличевИПр1990ТеорияГрафов}.
\end{itemize}
...
\begin{proof}
  ...
  Let \( V_1, \ldots, V_r \) be \emph{any} partition of the vertices of \( G \). By \cref{thm:pigeonhole_principle/simple}, there exists an index \( k \) such that \( V_k \) contains at least two vertices of \( H \). But then \( V_k \) is not an independent set.
...
\begin{example}\label{ex:thm:multipartite_graph_complete_subgraph}
  ...
  \begin{thmenum}
    ...
    \thmitem{ex:thm:multipartite_graph_complete_subgraph/converse} The converse of \cref{thm:multipartite_graph_complete_subgraph} does not hold.
...
\end{proposition}
\begin{proof}
  An explicit \hyperref[def:undirected_graph/morphism]{isomorphism} is
  \begin{equation*}
    \begin{aligned}
...
\begin{proposition}\label{thm:square_graphs}
  The \hyperref[def:cycle_graph]{cycle graph} \( C_4 \) and the \hyperref[def:complete_multipartite_graph]{complete bipartite graph} \( K_{2,2} \) are \hyperref[def:undirected_graph/morphism]{isomorphic} to the \hyperref[def:hypercube_graph]{square graph} \( Q_2 \)
...
\begin{proposition}\label{thm:small_turan_graph}
  If \( 0 \leq n \leq r \), the \hyperref[def:turan_graph]{Turan graph} \( T_{n,r} \) is \hyperref[def:undirected_graph/morphism]{isomorphic} to the \hyperref[def:complete_graph]{complete graph} \( K_n \).
\end{proposition}
\begin{proof}
...
\begin{proof}
  ...
  \Cref{thm:sum_of_graph_degrees} implies that
  \begin{equation*}
  ...
  We want to maximize \( m \). \Cref{thm:multipartite_graph_degree} implies that equality is attained in \eqref{eq:thm:turan_graph_edge_count/proof/degree_ineq} for complete multipartite graphs, thus \( G \) must be complete \( s \)-partite.
  ...
  \begin{balign*}
    ...
    = \\ &=
    \frac {\underline{sq(n-q)} - sq + s(n-q-1) + \underline{(r-s)q(n-q)}} 2
    = \\ &=
  ...
  \begin{balign*}
    ...
    = \\ &=
    \frac {(r-1)n^2 + \underline{sn} - \underline{sn} + s^2 - sr} {2r}
    = \\ &=
...
\end{corollary}
\begin{proof}
  \Cref{thm:turan_graph_edge_count} implies that the maximum number of edges is
  \begin{equation*}
    \frac {r - 1} r \cdot \frac {n^2 - s^2} 2 + \binom s 2.
  \end{equation*}
...
\begin{proof}
  ...
  \SubProof{Proof of base case} If \( n \leq r \), \cref{thm:small_turan_graph} implies that \( T_{n,r} \) is isomorphic to \( K_n \).

  \Cref{thm:complete_graph_edge_count} implies that any graph \( G \) of order \( n \) has its edge count is bounded by \( \binom n 2 \), which is precisely the edge count of \( T_{n,r} \).
...
\begin{theorem}[Mantel's theorem]\label{thm:mantels_theorem}\mcite[thm. I.2]{Bollobás1998ModernGraphTheory}
