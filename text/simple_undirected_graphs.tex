...
\begin{comments}
  ...
  \item For \( \oline{K_n} \), \incite[18]{Bollobas1998Graphs} prefers the term \enquote{empty graph}, while \incite[def. 1.3.1]{Knauer2011AlgGraphTheory} uses \enquote{totally disconnected} and \enquote{discrete graph}. \incite[16]{Harary1969Graphs} also uses \enquote{totally disconnected}.
\end{comments}
...
\begin{definition}\label{def:graph_complement}\mcite[def. 5.1.1]{Knauer2011AlgGraphTheory}
  We define the \term[ru=дополнение (\cite[15]{Емеличев1990Графы})]{complement} of the \hyperref[def:undirected_graph]{simple undirected graphs} \( G = (V, E) \) as
  \begin{equation*}
...
\begin{definition}\label{def:complete_graph}\mimprovised
  For every set \( V \), we define the \term[bg=пълен граф (\cite[12]{Мирчев2001Графи}), ru=полный граф (\cite[10]{Емеличев1990Графы}), en=complete graph (\cite[def. 1.3.1]{Knauer2011AlgGraphTheory})]{complete graph} \( K_V \) as the \hyperref[def:graph_complement]{complement} of the \hyperref[def:edgeless_graph]{edgeless graph} \( \oline{K_V} \) on \( V \).
...
\begin{comments}
  ...
  \item An \hyperref[def:multigraph_orientation]{orientation} of a complete graph is sometimes called a \enquote{tournament} --- for example by \incite[289]{Diestel2005Graphs}, \incite[30]{Bollobas1998Graphs}, Stephen Maurer in \cite[570]{Rosen1999DiscreteHandbook} and \incite[\S 7.3.3]{Новиков2013ДискретнаяМатематика} (as \enquote{турнир}).
\end{comments}
...
\begin{definition}\label{def:path_graph}\mcite[def. 1.3.1]{Knauer2011AlgGraphTheory}
...
\begin{definition}\label{def:cycle_graph}\mcite[def. 1.3.1]{Knauer2011AlgGraphTheory}
  For every integer \( n > 2 \), we define \term[en=cycle graph (\cite[532]{Rosen1999DiscreteHandbook})]{cycle graph} \( C_n \) as \hyperref[def:path_graph]{\( P_n \)} with an additional edge connecting \( n - 1 \) to \( 0 \).
...
\begin{definition}\label{def:graph_disjoint_union}\mimprovised
  We define the \term[ru=дизъюнктное объедиение (графов) (\cite[19]{Емеличев1990Графы})]{disjoint union} of the family of \hyperref[def:undirected_graph]{simple undirected graphs} \( \seq{ G_k }_{k \in \mscrK} \), where \( G_k = (V_k, E_k) \), as the graph
  \begin{equation*}
...
\begin{comments}
  ...
  \item \incite[21]{Harary1969Graphs} and \incite[def. 4.1.1]{Knauer2011AlgGraphTheory} use regular unions with the convention that the vertex sets and edge sets are disjoint. \incite[19]{Емеличев1990Графы} also use regular unions, however they explicitly refer to unions as \enquote{disjoint} (\enquote{дизъюнктное объединение}) if the vertex sets and edge sets are disjoint.
\end{comments}
...
\begin{definition}\label{def:graph_join}\mcite[def. 4.2.1]{Knauer2011AlgGraphTheory}
...
\end{definition}
\begin{comments}
  \item \incite[21]{Harary1969Graphs} attributes the introduction of graph joins to \cite[164]{Зыков1949СоединенияГрафов}. Zykov calls them \enquote{products} and denotes them via juxtaposition, however his paper generally uses arcane terminology like \enquote{complex} instead of \enquote{graph}.

  The term \enquote{join} is used by \incite[21]{Harary1969Graphs}, \incite[def. 4.2.1]{Knauer2011AlgGraphTheory} and \incite[\S 265]{Новиков2013ДискретнаяМатематика} (as \enquote{соединение}). The aforementioned authors denote this operation by \enquote{\( + \)}. Lowell Beineke in \cite[547]{Rosen1999DiscreteHandbook} also calls this operation \enquote{join}, however he denotes it by \enquote{\( \Ast \)}. \incite[15]{Diestel2005Graphs} also uses \enquote{\( \Ast \)}, however avoids introducing a special term.
\end{comments}
...
\begin{definition}\label{def:multipartite_graph}\mcite[def. 1.3.1]{Knauer2011AlgGraphTheory}
  Given a positive integer \( r \), we say that the \hyperref[def:undirected_graph]{simple undirected graph} \( G = (V, E) \) is \term[ru=\( r \)-дольный (граф) (\cite[11]{Емеличев1990Графы})]{\( r \)-partite} if \( V \) can be \hyperref[def:set_partition]{partitioned} into \( r \) (possibly empty) \hyperref[def:graph_independent_set]{independent} subsets \( V_1, \ldots, V_r \).

  We use the term \term[ru=двудольный (граф) (\cite[11]{Емеличев1990Графы})]{bipartite} when \( r = 2 \) and \term[ru=трёхдольный (граф) (\cite[11]{Емеличев1990Графы})]{tripartite} when \( r = 3 \). When no concrete value of \( r \) is meant, we say that the graph is \term[en=multipartite (graph) (\cite[ex. 8.5.5]{Knauer2011AlgGraphTheory})]{multipartite}.
\end{definition}
\begin{itemize}
  \item It is tempting to require the subsets of the partition to be nonempty, however many authors choose not to do so. This includes \incite[17]{Diestel2005Graphs}, \incite[27]{Harary1969Graphs}, \incite[21]{Bollobas1998Graphs}, \incite[def. 1.3.1]{Knauer2011AlgGraphTheory}, \incite[26]{GondranMinoux1984Graphs}, \incite[530]{Rosen1999DiscreteHandbook} and \incite[11]{Емеличев1990Графы}. We follow their lead.
\end{itemize}
...
\begin{proposition}\label{thm:bipartite_iff_no_odd_cycles}\mcite[thm. 1.3.5]{Knauer2011AlgGraphTheory}
...
\end{definition}
\begin{comments}
  \item The term \enquote{clique} is used as a synonym for complete subgraph by \incite[def. 1.4.11]{Knauer2011AlgGraphTheory}, \incite[5]{GondranMinoux1984Graphs}, \incite[394]{Erickson2019Algorithms}, \incite[111]{Емеличев1990Графы} (as \enquote{клика}) and \incite[\S 7.3.1]{Новиков2013ДискретнаяМатематика} (as \enquote{клика}). Other authors use it to mean a maximal complete subgraph, for example \incite[20]{Harary1969Graphs}, \incite[112]{Bollobas1998Graphs}, Arthur White in \cite[596]{Rosen1999DiscreteHandbook} and \incite[103]{Мирчев2001Графи} (as \enquote{клика}).
...
\begin{proof}
  ...
  Let \( V_1, \ldots, V_r \) be \hi{any} partition of the vertices of \( G \). By \fullref{thm:pigeonhole_principle}, there exists an index \( k \) such that \( V_k \) contains at least two vertices of \( H \). But then \( V_k \) is not an independent set.

  Therefore, \( G \) cannot possibly be \( r \)-partite.
\end{proof}

\begin{definition}\label{def:triangle_graph}\mcite[3]{Diestel2005Graphs}
...
\begin{definition}\label{def:complete_multipartite_graph}\mcite[33]{Harary1969Graphs}
  For a positive integer \( r \), we define the \term[ru=полный \( r \)-дольный (граф) (\cite[\S 1]{Емеличев1990Графы})]{complete \( r \)-partite} graph \( K_{V_1,\ldots,V_r} \) as the \hyperref[def:graph_join]{graph join}
  \begin{equation*}
...
\begin{theorem}[Turan's theorem]\label{thm:turans_theorem}\mcite[thm. 7.1.1]{Diestel2005Graphs}
