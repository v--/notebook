...
\begin{concept}\label{con:arithmetic}
  In a broad sense, the study of numbers is named \term[ru=арифметика (\cite[48]{БашмаковаЮшкевич1951ПроисхождениеСистемСчисления}), en=arithmetic (\cite[3]{Stillwell2010MathHistory})]{arithmetic}, based on the Greek word \enquote{\textel{ἀριθμός}} (transliterated as \enquote{arithmos} and translated in \cite[217]{DiggleEtAl2021CambridgeGreekLexicon} as \enquote{number}).
  ...
  \begin{itemize}
    ...
    A historical overview of this \enquote{elementary} arithmetic can be found in \cite{Karpinski1925HistoryOfArithmetic}. A notable book of historical interest on the topic is \cite{Fibonacci2002LiberAbaci}.
    ...
    \thmitem{con:arithmetic/algebraic} Ever since antiquity, the study of integers goes way beyond the arithmetic operations. \hyperref[def:prime_number]{Prime numbers} and \hyperref[def:polynomial_equation/diophantine]{Diophantine equations} have notably led to some of the most abstract mathematics in existence.

    The abstract study of numbers is colloquially called \term[ru=теория чисел (\cite[7]{Бухштаб1966ТеорияЧисел}), en=theory of numbers (\cite[1]{Apostol1976AnalyticNumberTheory})]{number theory}, although some influential books like \cite{Gauss1801DisquisitionesArithmeticae} and \cite{Serre1973Arithmetic} call it \enquote{arithmetic}.
...
\begin{remark}\label{rem:peano_arithmetic}
  ...
  \begin{thmenum}
    \thmitem{rem:peano_arithmetic/sets} \Textcite[1]{Peano1889ArithmeticesPrincipia} not only presents a formal theory of arithmetic, but, more importantly, he formulates an early variant of \hyperref[con:predicate_logic]{predicate logic} based on \hyperref[def:set_theory_signature]{set theory}.

    Peano's axioms are most faithfully represented by what \textcite[70]{Enderton1977SetTheory} and \textcite[53]{Mendelson2008NumberSystems} call \enquote{Peano systems} --- triples \( (N, e, S) \) such that
    \begin{itemize}
      \item \( S \) is an \emph{injective} endofunction on \( N \) (injectivity subsumes \eqref{eq:def:peano_arithmetic/theory/PA1}).
    ...
    \begin{itemize}
      \item Our list of axioms coincides with \cite[def. 2.7.7]{VanDalen2004LogicAndStructure} and \cite[\S 5.2.5]{Mimram2020ProgramEqualsProof}.

      \item \Textcite[105]{КолмогоровДрагалин2006Логика} and \textcite[167]{Герасимов2014Вычислимость} extend our list with axioms for equality. We will not need them because we handle equality as a primordial logical primitive.

      \item \Textcite*[def. 4.1.1(ii)]{Hinman2005Logic} extends our signature with the strict inequality \( \synless \) and adds corresponding axioms. We instead present \( \synleq \) and related predicates via \hyperref[def:fol_definitional_extension]{definitional extensions} in \cref{def:peano_arithmetic_ordering}.
    \end{itemize}
  \end{thmenum}
\end{remark}
...
\end{proposition}
\begin{comments}
  \item In other words, every first-order model \( \mscrX \) has a submodel isomorphic to \( \mscrN \). \Textcite[164]{Hinman2005Logic} calls this submodel the \enquote{standard part} of \( \mscrX \).
\end{comments}
\begin{proof}
...
\begin{proposition}\label{thm:peano_arithmetic_standard_models}
  ...
  \begin{itemize}
    ...
    \thmitem{thm:peano_arithmetic_standard_models/initial} Any \hyperref[def:universal_objects/initial]{initial object} in the \hyperref[def:semiring/category]{category of small semirings}.
  \end{itemize}
\end{proposition}
\begin{comments}
...
\begin{definition}\label{def:peano_arithmetic_ordering}
  ...
  As we shall show in \cref{thm:natural_numbers_are_well_ordered}, this predicate acts as a \hyperref[def:well_ordered_set]{well-order}. We will show in \cref{thm:peano_arithmetic_order_compatibility} that the corresponding \hyperref[def:partially_ordered_set/strict]{strict relation} can be defined via
...
\begin{proposition}\label{thm:peano_arithmetic_order_compatibility}
  In \hyperref[def:peano_arithmetic]{Peano arithmetic}, the inequality predicates from \cref{def:peano_arithmetic_ordering} are indeed related via \eqref{eq:thm:partially_ordered_set_theories/strict/compatibility}.
\end{proposition}
\begin{proof}
  ...
  \SubProofOf{eq:thm:partially_ordered_set_theories/strict/compatibility}
  \begin{equation*}
    \begin{prooftree}
      ...
      \infer1[\ref{inf:def:fol_natural_deduction/forall/intro}*]{ \eqref{eq:thm:partially_ordered_set_theories/strict/compatibility} }
    \end{prooftree}
  \end{equation*}
\end{proof}
...
\begin{theorem}[Strong induction on natural numbers]\label{thm:strong_induction_on_natural_numbers}
  ...
  \begin{displayquote}
    In order for \( p(n) \) to hold \emph{for every} \( n \) in \( N \), it is sufficient for \( p(n) \) to hold \emph{whenever} \( p(m) \) holds for every \( m < n \).
  \end{displayquote}
\end{theorem}
\begin{comments}
