...
\begin{example}\label{ex:def:beta_eta_reduction}
  ...
  \begin{thmenum}
    ...
    \begin{equation*}
      \qabs \synx \qabs \syny \syny \label{eq:ex:def:beta_eta_reduction/numerals/c0} \tag{\ensuremath{ c_0 }}
    \end{equation*}
    ...
    \thmitem{ex:def:beta_eta_reduction/pairs} We can encode the \hyperref[def:ordered_tuple]{ordered pairs} via the combinators
    \begin{align}
      C_{\T{ons}} &\coloneqq \qabs \synx \qabs \syny \qabs \synp \synp \synx \syny \label{ex:def:beta_eta_reduction/pairs/cons}\tag{\ensuremath{ C_{\T{ons}} }}, \\
      C_{\T{ar}}  &\coloneqq \qabs \synp \synp (\qabs \synx \qabs \syny \synx) \label{ex:def:beta_eta_reduction/pairs/car}\tag{\ensuremath{ C_{\T{ar}} }}, \\
      C_{\T{dr}}  &\coloneqq \qabs \synp \synp (\qabs \synx \qabs \syny \syny) \label{ex:def:beta_eta_reduction/pairs/cdr}\tag{\ensuremath{ C_{\T{dr}} }}.
    \end{align}
    ...
    The combinator \( C_{\T{ons}} \) allows combining \( A \) and \( B \) into a single term:
    \begin{equation*}
      C_{\T{cons}} A B \bred* \qabs v v A B,
    \end{equation*}
    where \( v \) is not free in \( A \) or \( B \).

    Applying \( C_{\T{ar}} \), we obtain
    \begin{balign*}
      C_{\T{ar}} (C_{\T{cons}} A B)
      &\bred
      C_{\T{ar}} (\qabs v v A B)
      = \\ &=
    ...
    Similarly,
    \begin{equation*}
      C_{\T{dr}} (C_{\T{cons}} A B) \bred* B.
    \end{equation*}

    Therefore, the combinators act as expected.
    ...
    We will define truth values differently in \fullref{sec:simply_typed_lambda_calculus}, for which reason we avoid using these combinators as boolean values more generally, but for the purposes of this example define \( T \) and \( F \) as
    \begin{align}
      &\overbrace{\qabs \synx \qabs \syny \synx}^K \label{ex:def:beta_eta_reduction/boolean/t}\tag{T}, \\
      &\underbrace{\qabs \synx \qabs \syny \syny}_{c_0} \label{ex:def:beta_eta_reduction/boolean/f}\tag{F}.
    \end{align}
    ...
    This allows us to implement \hyperref[def:boolean_function]{Boolean functions}. For example, negation can be encoded as follows:
    \begin{equation}\label{ex:def:beta_eta_reduction/boolean/n}\tag{N}
      \qabs \synx \synx F T.
    \end{equation}
