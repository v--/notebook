...
\begin{comments}
  ...
  Other authors like \incite[8.4.7]{Barendregt1984LambdaCalculus} only consider horizontal ordering.
\end{comments}
...
\end{definition}
\begin{comments}
  \item This definition is not established. It is based on reflexive closures discussed in \fullref{def:relation_closures/reflexive}. It allows us to make explicit the dependency on \( \alpha \)-equivalence, which is implicit in our primary sources --- \cite[ch. 3]{Barendregt1984LambdaCalculus}, \cite[1B2]{Hindley1997STT} and \cite[191]{Герасимов2011Вычислимость}.
\end{comments}
...
\begin{definition}\label{def:beta_eta_reduction}\mcite[1B1; 1C1]{Hindley1997STT}
...
\begin{example}\label{ex:def:beta_eta_reduction}
  ...
  \begin{thmenum}
    ...
    \thmitem{ex:def:beta_eta_reduction/boolean}\mcite[6.2.2]{Barendregt1984LambdaCalculus} We can encode the \hyperref[con:boolean_value]{Boolean values} via the combinators
    \begin{subequations}
      \begin{align}
    ...
    \thmitem{ex:def:beta_eta_reduction/numerals}\mcite[6.4.4]{Barendregt1984LambdaCalculus} For every natural number \( n \), we can define the \term{Church numeral}
...
\begin{definition}\label{def:reduction_strategy}\mcite[13.1]{Barendregt1984LambdaCalculus}
...
\end{definition}
\begin{comments}
  \item The definition is based on \cite[6.1.2]{Barendregt1984LambdaCalculus}, but is adapted to handle equivalences explicitly.
\end{comments}
...
\end{definition}
\begin{comments}
  \item This definition is based on \cite[3.2.3]{Barendregt1984LambdaCalculus}, where only parallel \( \beta \)-reduction is considered.
  \item Of course, we also consider equivalences and multi-step reductions based on the above single-step reductions.
\end{comments}
