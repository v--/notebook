...
\begin{definition}\label{def:beta_eta_reduction}\mcite[1B1; 1C1]{Hindley1997}
  ...
  We define \term{\( \beta \)-reduction} \( {\bred} \) as the relation generated by the rules of \fullref{def:lambda_reduction/single} extended with \ref{inf:def:lambda_reduction/beta}. We similarly define \term{\( \eta \)-reduction} \( {\ered} \) based on \ref{inf:def:lambda_reduction/eta}. Combining both, we obtain \term{\( \beta\eta \)-reduction} \( {\bered} \).
\end{definition}
\begin{comments}
...
\begin{definition}\label{def:parallel_reduction}
  ...
  \begin{thmenum}
    \thmitem{def:parallel_reduction/base} We reuse \ref{inf:def:lambda_reduction/abs} and \ref{inf:def:lambda_reduction/alpha} and combine \ref{inf:def:lambda_reduction/app_left} and \ref{inf:def:lambda_reduction/app_right} into
  ...
  \thmitem{def:parallel_reduction/beta_eta} Additionally, we replace \ref{inf:def:lambda_reduction/beta} and \ref{inf:def:lambda_reduction/eta} with
  \begin{TwoColumns}
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    By \fullref{thm:lambda_reduction_free_variables}, every free variable in \( N \) is also free in \( A \), thus \( x \) cannot be free in \( N \). Then we can apply \ref{inf:def:lambda_reduction/eta} to obtain \( M \pred* N \).
  \end{itemize}
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \item If \( (\qabs x A) \ppred N \) due to \ref{inf:def:beta_eta_reduction/eta}, then \fullref{thm:parallel_reduction_deconstruction/abs/red} holds.
