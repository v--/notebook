...
\begin{algorithm}[Prenex normal form conversion]\label{alg:prenex_normal_form_conversion}
  ...
  \begin{thmenum}
    ...
    Denote by \( \widehat{P_3} \) the idempotent closure of \( P_3 \). That is, the function that applies \( P_3 \) repeatedly until it converges to fixed point. A fixed point must always exist because we move quantifiers \enquote{outwards} at each step.

    \Fullref{thm:pulling_quantifiers_out} ensures that \( \widehat{P_3}(\varphi) \gleichstark \varphi \) for any formula \( \varphi \) not containing \( \rightarrow \) or \( \leftrightarrow \).
  \end{thmenum}

  The formula \( \widehat{P_3}(P_2(P_1(\varphi))) \) is then in prenex normal form and is semantically equivalent to \( \varphi \).
\end{algorithm}
