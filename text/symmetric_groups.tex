...
\begin{example}\label{ex:def:symmetric_group}
  ...
  \begin{thmenum}
    \thmitem{ex:def:symmetric_group/automorphism} Every \hyperref[def:categorical_endomorphism]{automorphism} is a permutation, but not vice versa.
...
\begin{proposition}\label{thm:transpositions_are_involutions}
  Every \hyperref[def:transposition]{transposition} is an \hyperref[def:categorical_involution]{involution}.
\end{proposition}
\begin{proof}
  Trivial.
\end{proof}
...
\begin{definition}\label{def:cayley_table}\mcite[150]{Rotman2015AdvancedModernAlgebraPart1}
  The \term[ru=таблица умножения (\cite[135]{Шафаревич2019ОсновныеПонятияАлгебры})]{multiplication table} or \term[ru=таблица Кэли (\cite[135]{Шафаревич2019ОсновныеПонятияАлгебры})]{Cayley table} for a finite \hyperref[def:semigroup]{(semi)group} \( G = \set{ a_1, \ldots, a_n } \), is an \( n \times n \) matrix whose \( (i, j) \)-th entry is \( a_i a_j \).
...
\begin{example}\label{ex:s3}
  ...
  \begin{itemize}
    ...
    \item The cycles \( \cycle{ 1, 2, 3 } \) and \( \cycle{ 1, 3, 2 } \) are inverses of each other, and \cref{thm:transpositions_are_involutions} implies that all others are \hyperref[def:categorical_involution]{involutions}.
...
\begin{corollary}\label{thm:category_of_groups_is_balanced}
  The category \( \hyperref[def:group/category]{\cat{Grp}} \) of small groups is \hyperref[def:balanced_category]{balanced}.
\end{corollary}
\begin{proof}
  Follows from \cref{thm:fol_equational_theory_balanced_category} in light of \cref{thm:group_epimorphisms_are_surjective}.
\end{proof}
