...
\begin{definition}\label{def:fixed_point}\mcite[286]{Golan1999Semirings}
...
\begin{theorem}[Fixed point existence]\label{thm:fixed_point_existence}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:fixed_point_existence/banach} \fullref{thm:banach_fixed_point_theorem}: Every \hyperref[def:lipschitz_continuity/contraction]{contraction mapping} in a \hyperref[def:complete_metric_space]{complete metric space} has a unique fixed \hyperref[def:fixed_point]{point}.
  \end{thmenum}
\end{theorem}

\paragraph{Knaster-Tarski theorem}

\begin{theorem}[Knaster-Tarski theorem]\label{thm:knaster_tarski_theorem}\mcite[thm. 1]{Tarski1955FixedPoint}
...
\begin{comments}
  ...
  \item This particular name for the theorem is used by \incite[50]{DaveyPriestley2002Lattices} and \incite[321]{PicadoPultr2012Frames}, although only special cases are stated.
\end{comments}
\begin{proof}
...
\begin{theorem}[Knaster-Tarski iteration theorem]\label{thm:knaster_tarski_iteration}\mcite[lemma 1]{Echenique2005FixedPoint}
...
\end{theorem}
\begin{comments}
  \item Although we call this \enquote{the} Knaster-Tarski theorem, it has a weaker conclusion than \fullref{thm:knaster_tarski_theorem}, concerned with the existence of only one particular fixed point. Only the existence of certain fixed points is actually how the general (i.e. non-sequential) theorem is stated by some authors such as \incite[50]{DaveyPriestley2002Lattices} and \incite[321]{PicadoPultr2012Frames}.
  ...
  \item This theorem is called \enquote{constructive} by \incite{Echenique2005FixedPoint}, but it utilizes the law of double negation elimination \eqref{eq:thm:classical_tautologies/dne} so it can be argued that it is not, in fact, constructive, at least not in the sense of the \hyperref[con:brouwer_heyting_kolmogorov_interpretation]{Brouwer-Heyting-Komogorov interpretation}.
...
\begin{theorem}[Recursively defined relations]\label{thm:recursively_defined_relations}
  ...
  \begin{thmenum}
    \thmitem{thm:recursively_defined_relations/closed} \( \Rho \) is closed with respect to the rules --- for every rule \( R_k \), we have \( (a_k, b_k) \in \Rho \) if, for every \( s = 1, \ldots, m_k \),
    \begin{equation*}
...
\end{comments}
\begin{proof}
  \Fullref{thm:boolean_algebra_of_subsets} implies that \( \pow(D^2) \) is a complete lattice, so we will use \fullref{thm:knaster_tarski_iteration/continuous}.
  ...
  \end{equation*}
  is the least fixed point of \( E \). It satisfies \fullref{thm:recursively_defined_relations/closed} as a fixed point of \( T \) and \fullref{thm:recursively_defined_relations/minimal} because of continuity.
\end{proof}
...
\begin{theorem}[Induction on recursively defined relations]\label{thm:induction_on_recursively_defined_relations}
  In the setting of \fullref{thm:recursively_defined_relations}, in order to prove a statement for a specific pair \( (b, c) \in \Rho \), it is sufficient to do the following:
  \begin{displayquote}
...
\end{comments}
\begin{proof}
  By \fullref{thm:recursively_defined_relations/minimal}, every pair of elements of \( D \) related by \( \Rho \) is the conclusion of some rule.
\end{proof}
