...
\begin{theorem}[Recursively defined relations]\label{thm:recursively_defined_relations}
  ...
  \begin{thmenum}
    ...
    More precisely, fix a rule \( R_k \) and a \hyperref[def:fol_variable_assignment]{variable assignment} \( v \). For \( s = 1, \ldots, m_k \), suppose that \( \Bracks{\varphi_s}_\mscrD^v = \semtop \) and that \( (\Bracks{\tau'}_\mscrD^v, \Bracks{\sigma'}_\mscrD^v) \) is in \( \Rho \) whenever \( \tau' \mathrel{\rho} \sigma' \) is a subformula of \( \varphi_s \). Then \( (\Bracks{\tau_k}_\mscrD^v, \Bracks{\sigma_k}_\mscrD^v \) is also in \( \Rho \).
...
\begin{theorem}[Induction on recursively defined relations]\label{thm:induction_on_recursively_defined_relations}
  ...
  \begin{displayquote}
    If \( (a, b) \in \Rho \) due to an application of the rule \( R_k \), suppose that the statement holds for all premises of \( R_k \) and prove the statement for the conclusion \( (a, b) \).
  \end{displayquote}
