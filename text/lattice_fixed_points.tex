...
\begin{comments}
  ...
  \item This particular name for the theorem is used by \textcite[50]{DaveyPriestley2002LatticeTheory}, although only a special case is stated.
\end{comments}
\begin{proof}
...
\begin{example}\label{ex:thm:knaster_tarski}
  ...
  \begin{thmenum}
    \thmitem{ex:thm:knaster_tarski/closure} Every \hyperref[def:abstract_closure_operator]{closure operator} introduces a concept of \enquote{closed} elements, which are precisely its fixed points. The join of two closed elements may differ from the ambient join:
    \begin{itemize}
      \item Unions of subgroups are not subgroups in general, and similarly for subrings, ideals and for arbitrary first-order structures; as shown in \cref{thm:fol_lattice_of_substructures}, their join in instead the substructure generated by their union.
    ...
    \thmitem{ex:thm:knaster_tarski/recursive_relation} \hyperref[thm:recursively_defined_relations]{Recursively defined relations} in logic are fixed points of order-preserving operators.
  \end{thmenum}
\end{example}
...
\end{definition}
\begin{comments}
  \item Instead of complete lattices, we can work over \hyperref[def:partially_ordered_set]{partially ordered sets} closed under upward-directed suprema. \Textcite[def. 8.1]{DaveyPriestley2002LatticeTheory} call such a partially ordered sets \enquote{complete}, which unfortunately conflicts with our notion of complete lattice. \Textcite[192]{Birkhoff1967LatticeTheory} instead requires all \hyperref[def:partial_order_connectedness]{chains} to have a supremum and calls such partially ordered sets \enquote{inductive}. \Textcite[83]{БелоусовТкачёв2004ДискретнаяМатематика} calls a partially ordered set \enquote{индуктивным} (\enquote{inductive}) if it satisfies Davey and Priestley's condition.
\end{comments}
...
\begin{proposition}\label{thm:def:scott_continuity}
  ...
  \begin{thmenum}
    ...
    \begin{displayquote}
      If \( \mscrA \) is an upward-directed family in \( X \), for every \( x \) in \( f(\bigcup \mscrA) \) there exists a set \( A \in \mscrA \) such that \( x \) is in \( f(A) \).
    \end{displayquote}
  \end{thmenum}
\end{proposition}
\begin{proof}
  ...
  The set \( \set{ x, y } \) is directed, so \( f(x \vee y) = f(x) \vee f(y) \). Furthermore, \eqref{eq:def:lattice/theory/leq/join} implies that \( x \vee y = y \), so \( f(x) \vee f(y) = f(y) \), and using \eqref{eq:def:lattice/theory/leq/join} again we obtain \( f(x) \leq f(y) \).
...
\end{theorem}
\begin{comments}
  \item Although we call this \enquote{the} Knaster-Tarski theorem, it has a weaker conclusion than \fullref{thm:knaster_tarski_theorem}, concerned with the existence of only one particular fixed point. Only the existence of certain fixed points is actually how the general (i.e. non-sequential) theorem is stated by some authors such as \textcite[50]{DaveyPriestley2002LatticeTheory}.
  ...
  \item This theorem is called \enquote{constructive} by \textcite{Echenique2005TarskiFixedPointTheorem}, but it utilizes \fullref{thm:propositional_semantic_dne}, so it can be argued that it is not, in fact, constructive, at least not in the sense of the \hyperref[con:brouwer_heyting_kolmogorov_interpretation]{Brouwer-Heyting-Komogorov interpretation}.
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \begin{equation*}
      ...
      =
      \parens*{ \bigvee_{\gamma < \beta} f(x'_\gamma) } \vee f(x'_\beta)
    \end{equation*}
  ...
  \begin{equation*}
    f(x_\omega)
    =
    f\parens*{ \bigvee_{k=0}^\infty x_k }
    =
...
\begin{example}\label{ex:thm:knaster_tarski_iteration}
  ...
  \begin{thmenum}
    ...
    The other fixed points contain \hyperref[def:peano_arithmetic_standard_model]{nonstandard natural numbers}, which cannot be obtained by finitely many iterative application of \( \op{sc} \) to \( \varnothing \).
...
\begin{theorem}[Recursively defined relations]\label{thm:recursively_defined_relations}
  Consider some \hyperref[def:fol_signature]{first-order signature} \( \Sigma \) with a binary relation symbol \( \rho \). Suppose that we have \( n \) we have \hyperref[def:inference_rule]{inference rules} \( R_1, \ldots, R_n \), where, for \( k = 1, \ldots, n \),
  \begin{equation*}
    \begin{prooftree}
      \hypo{ \varphi_1 }
      \hypo{ \cdots }
  ...
  Here \( \tau_k \) and \( \sigma_k \) are \hyperref[def:fol_term]{terms} over \( \Sigma \), while \( \varphi_1, \ldots, \varphi_{m_k} \) are \hyperref[def:fol_formula]{formulas} over \( \Sigma \) (usually applications of \( \rho \)).

  Then, for any \hyperref[def:fol_structure]{structure} \( \mscrD = (D, I) \) over \( \Sigma \), there exists a \hyperref[def:binary_relation]{binary relation} \( \Rho \) on \( D \) such that the following hold:
  \begin{thmenum}
    \thmitem{thm:recursively_defined_relations/closed} \( \Rho \) is closed with respect to the rules.

    More precisely, fix a rule \( R_k \) and a \hyperref[def:fol_variable_assignment]{variable assignment} \( v \). Suppose that \( v \) satisfies \( \varphi_s \) for every \( s = 1, \ldots, m_k \) and that \( (\Bracks{\tau'}_\mscrD^v, \Bracks{\sigma'}_\mscrD^v) \) is in \( \Rho \) whenever \( \tau' \mathrel{\rho} \sigma' \) is a subformula of \( \varphi_s \). Then \( (\Bracks{\tau_k}_\mscrD^v, \Bracks{\sigma_k}_\mscrD^v) \) is also in \( \Rho \).
...
\begin{proof}
  ...
  Consider the operator \( E: D^2 \to D^2 \) that extends a relation \( r \) with \( (\Bracks{\tau_k}_\mscrD^v, \Bracks{\sigma_k}_\mscrD^v) \in \Rho \) if there exists a rule
  \begin{equation*}
    \begin{prooftree}
      \hypo{ \varphi_1 }
      \hypo{ \cdots }
    ...
    \end{prooftree}
  \end{equation*}
  such that, for every \( s = 1, \ldots, m_k \), we have \( \Bracks{\varphi_s}_\mscrD^v = \semtop \) and \( (\Bracks{\tau'}_\mscrD^v, \Bracks{\sigma'}_\mscrD^v) \in r \) whenever \( \tau' \mathrel{\rho} \sigma' \) is a subformula of \( \varphi_s \).
  ...
  \begin{itemize}
    ...
    \item Otherwise, \( c = \Bracks{\tau_k}_\mscrD^v \) and \( d = \Bracks{\sigma_k}_\mscrD^v \) for some rule \( R_k \), in which case, for every \( s = 1, \ldots, m_k \), we have \( \Bracks{\varphi_s}_\mscrD^v = \semtop \) and, whenever \( \tau' \mathrel{\rho} \sigma' \) is a subformula of \( \varphi_s \), there exists a set \( A_s \) from \( \mscrA \) such that \( (\Bracks{\tau'}_\mscrD^v, \Bracks{\sigma'}_\mscrD^v) \in A_s \).

    There are finitely many subformulas in \( \varphi_s \), so, since the family \( \mscrA \) is directed, there exists an upper limit \( A_0 \) that contains \( (\Bracks{\tau'}_\mscrD^v, \Bracks{\sigma'}_\mscrD^v) \) whenever \( \tau' \mathrel{\rho} \sigma' \) is a subformula of \( \varphi_s \) for some \( s = 1, \ldots, m_k \).
...
\begin{theorem}[Induction on recursively defined relations]\label{thm:induction_on_recursively_defined_relations}
  In the setting of \fullref{thm:recursively_defined_relations}, in order to prove a statement for a specific pair \( (a, b) \) belongs to \( \Rho \), it is sufficient to do the following:
  \begin{displayquote}
    If \( (a, b) \) belongs to \( \Rho \) due to an application of the rule \( R_k \), suppose that the statement holds for all premises of \( R_k \) and prove the statement for the conclusion \( (a, b) \).
  \end{displayquote}
...
\begin{proposition}\label{thm:generated_equivalence_relation}\mimprovised
  We can use \fullref{thm:recursively_defined_relations} to succinctly characterize the \hyperref[def:equivalence_closure]{equivalence closure} \( {\congdot} \) of a binary relation \( {\dotsim} \) on a set \( A \) using the following (metatheoretic) inference rules:
  \begin{paracol}{2}
    \begin{nthcolumn}{0}
      \ParacolAlignmentHack
      \begin{equation*}\taglabel[\ensuremath{ \logic{Equiv}_I }]{inf:thm:generated_equivalence_relation/intro}
        \begin{prooftree}
          \hypo{ a \sim b }
          \infer1[\ref{inf:thm:generated_equivalence_relation/intro}]{ a \cong b }
        \end{prooftree}
      \end{equation*}
    \end{nthcolumn}
    ...
    \begin{nthcolumn}{1}
      \ParacolAlignmentHack
      \begin{equation*}\taglabel[\ensuremath{ \logic{Equiv}_R }]{inf:thm:generated_equivalence_relation/refl}
        \begin{prooftree}
          \infer0[\ref{inf:thm:generated_equivalence_relation/refl}]{ a \cong a }
        \end{prooftree}
      \end{equation*}
    \end{nthcolumn}
  \end{paracol}
  ...
  \begin{paracol}{2}
    \begin{nthcolumn}{0}
      \ParacolAlignmentHack
      \begin{equation*}\taglabel[\ensuremath{ \logic{Equiv}_S }]{inf:thm:generated_equivalence_relation/symm}
        \begin{prooftree}
          \hypo{ a \cong b }
          \infer1[\ref{inf:thm:generated_equivalence_relation/symm}]{ b \cong a }
        \end{prooftree}
      \end{equation*}
    \end{nthcolumn}
    ...
    \begin{nthcolumn}{1}
      \ParacolAlignmentHack
      \begin{equation*}\taglabel[\ensuremath{ \logic{Equiv}_T }]{inf:thm:generated_equivalence_relation/trans}
        \begin{prooftree}
          \hypo{ a \cong b }
          \hypo{ b \cong c }
          \infer2[\ref{inf:thm:generated_equivalence_relation/trans}]{ a \cong c }
        \end{prooftree}
      \end{equation*}
    \end{nthcolumn}
  \end{paracol}
\end{proposition}
\begin{comments}
  \item It is possible to add new rules to this list. We will do so when defining generated first-order congruence relations in \cref{def:fol_generated_congruence} and free term models in \cref{def:fol_free_term_model}.
\end{comments}
\begin{proof}
  Let \( {\cong} \) be the relation obtained using these rules. Let \( {\approx} \) be any equivalence relation on \( A \).

  Let \( a \cong b \). We will use \fullref{thm:induction_on_recursively_defined_relations} to show that \( a \approx b \). It will follow that \( {\cong} \) is a subset of every equivalence relation that contains \( {\sim} \), and hence coincides with their intersection, the equivalence closure.
  \begin{itemize}
    \item If \( a \cong b \) due to \ref{inf:thm:generated_equivalence_relation/intro}, then \( a \sim b \), and hence \( a \approx b \) because \( {\approx} \) contains \( {\sim} \).

    \item If \( a \cong b \) due to \ref{inf:thm:generated_equivalence_relation/refl}, then \( a = b \), and hence \( a \approx b \) because \( {\approx} \) is reflexive.

    \item If \( a \cong b \) due to \ref{inf:thm:generated_equivalence_relation/symm}, where the inductive hypothesis holds for \( b \cong a \), then \( b \approx a \) and, because \( {\approx} \) is symmetric, \( a \approx b \).

    \item Finally, if \( a \cong b \) due to \ref{inf:thm:generated_equivalence_relation/symm}, where the inductive hypothesis holds for \( a \cong c \) and \( c \cong b \), then \( a \approx c \) and \( c \approx a \) and, because \( {\approx} \) is transitive, \( a \approx b \).
  \end{itemize}
\end{proof}

\begin{example}\label{ex:thm:generated_equivalence_relation}
  We will inspect the \hyperref[def:equivalence_relation]{equivalence relation} \( {\cong} \) \hyperref[thm:generated_equivalence_relation]{generated} by the relation \( {\sim} \) over \( A \) more closely.

  The corresponding operator from the proof of \fullref{thm:recursively_defined_relations} becomes
  \begin{equation*}
    \begin{aligned}
      E(R) \coloneqq \thickspace & R \thickspace {\cup} \\
                                 & \set{ (a, b) \given a \sim b } \thickspace {\cup} \\
                                 & \set{ (a, a) \given a \in A } \thickspace {\cup} \\
                                 & \set{ (b, a) \given (a, b) \in A } \thickspace {\cup} \\
                                 & \set{ (a, c) \given (a, b) \in A \T{and} (b, c) \in A }.
    \end{aligned}
  \end{equation*}

  To obtain \( {\cong} \), we must recursively apply \( E \), starting at \( {\sim} \):
  \begin{itemize}
    \item At the zeroth step, no elements of \( A \) are related.
    \item At the first step \( E(\varnothing) \), we only have \( a \cong b \) if \( a \sim b \) or \( a = b \).
    \item At the second step \( E(E(\varnothing)) \), we have \( a \cong b \) if \( a \sim b \), if \( b \sim a \), if \( a = b \) or if, for some \( c \), both \( a \sim c \) and \( c \sim b \).
    \item After the second step, we have a lot more cases to consider.
  \end{itemize}
\end{example}

\paragraph{Recursively defined abstract syntax}

\begin{theorem}[Recursively defined abstract syntax]\label{thm:recursively_defined_abstract_syntax}
  Fix an \hyperref[def:formal_language/alphabet]{alphabet} \( \mscrA \) and an arity function \( \#: \mscrA \to \BbbZ_{\geq 0} \).

  Then there exists a set \( \mscrT \) of \hyperref[def:labeled_tree]{labeled trees} such that the tree \( T \) with label \( f \) and children \( T_1, \ldots, T_n \) belongs to \( \mscrT \) if and only if \( f \) is an \( n \)-ary symbol in \( \mscrA \), \( T_1, \ldots, T_n \) belong to \( \mscrT \) and \( T = B_+^f(T_1, \ldots, T_n) \) is obtained by \hyperref[def:ordered_tree_grafting]{grafting} the subtrees to a new node labeled by \( f \).
\end{theorem}
\begin{comments}
  \item The trees in \( \mscrT \) are by definition \hyperref[def:well_founded_graph]{well-founded} and by construction locally-finite, thus they are all finite.
\end{comments}
\begin{proof}
  Let \( \mscrU \) be a \hyperref[def:grothendieck_universe]{Grothendieck universe} containing \( \mscrA \). Because of the cardinality considerations we discuss in \cref{rem:language_alphabet_cardinality}, \( \mscrU \) depends only on \( \mscrA \) itself and not on its symbols.

  Let \( L_\mscrA \) be the family of trees in \( \mscrU \) that are labeled by \( \mscrA \). We will use \fullref{thm:knaster_tarski_iteration} to justify defining a subset of \( L_\mscrA \) that enforces the recursive condition.

  Consider the operator
  \begin{equation*}
    \begin{aligned}
      &E: \pow(L_\mscrA) \to \pow(L_\mscrA), \\
      &E(U) \coloneqq \set{ B_+^f(T_1, \ldots, T_{\# f}) \given f \in \mscrA \T{and} T_1, \ldots, T_n \in U },
    \end{aligned}
  \end{equation*}
  where \( B_+^f(T_1, \ldots, T_{\# f}) \) is the tree obtained by \hyperref[def:ordered_tree_grafting]{grafting} \( T_1, \ldots, T_{\# f} \) to a new root labeled by \( a \).

  We will show that \( U \) is \hyperref[def:scott_continuity]{Scott-continuous}; then \fullref{thm:knaster_tarski_iteration/continuous} will imply that our desired set can be defined as
  \begin{equation*}
    \mscrT \coloneqq \bigcup_{k=0}^\infty E^k(\varnothing).
  \end{equation*}

  Fix an upward-directed family \( \set{ U_k }_{k \in \mscrK} \) of subsets of \( \mscrU \). We will use the characterization of Scott-continuity from \cref{thm:def:scott_continuity/lattice_of_subsets}, which requires us to prove that, for every tree \( T = B_+^f(T_1, \ldots, T_{\# f}) \) in \( E(\bigcup_{k \in \mscrK} U_k) \), there exists an index \( k_0 \) such that \( T \) belongs to \( E(U_{k_0}) \).

  By construction, the trees \( T_1, \ldots, T_{\# f} \) belong to \( \bigcup_{k \in \mscrK} U_k \); thus, for \( i = 1, \ldots \# f \), the subtree \( T_i \) belongs to some particular set \( U_{k_i} \). Since the family of subsets is upward directed, there exists some set \( U_{k_0} \) that is an upper bound of \( U_{k_1}, \ldots, U_{k_n} \). Thus, for \( i = 1, \ldots \# f \), \( T_i \) belongs to \( U_{k_0} \).

  Then \( T \) is in \( E(U_{k_0}) \), as desired. This shows that \( E \) is a Scott-continuous operator; hence, as already discussed, \( \mscrT \) can be defined by iterating \( E \).
\end{proof}

\begin{theorem}[Induction on abstract syntax]\label{thm:induction_on_abstract_syntax}
  In the setting of \fullref{thm:recursively_defined_abstract_syntax}, in order to prove a statement for all trees in \( \mscrT \), it is sufficient to do the following:
  \begin{displayquote}
    Given a syntax tree \( T = B_+^f(T_1, \ldots, T_{\# f}) \), suppose that the statement holds for \( T_1, \ldots, T_{\# f} \) and prove it for \( T \).
  \end{displayquote}
\end{theorem}
\begin{comments}
  \item We will usually rely on \hyperref[con:evaluation]{pattern matching} to avoid working with trees directly.
\end{comments}
\begin{proof}
  Restatement of \fullref{thm:induction_on_rooted_trees}.
\end{proof}
