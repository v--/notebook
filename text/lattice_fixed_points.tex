...
\end{comments}
\begin{proof}
  By the cardinality assumptions in \cref{rem:language_alphabet_cardinality}, the alphabet \( \mscrA \) is a \hyperref[def:small_set]{small set}, i.e. it belongs to the \hyperref[def:smallest_grothendieck_universe]{smallest Grothendieck universe} \( \mscrU_0 \).

  Let \( L_\mscrA \) be the family of trees in \( \mscrU_0 \) that are labeled by \( \mscrA \). We will use \fullref{thm:knaster_tarski_iteration} to justify defining a subset of \( L_\mscrA \) that enforces the recursive condition.
