...
\begin{example}\label{ex:def:untyped_lambda_term}
  ...
  \begin{thmenum}
    ...
    \thmitem{ex:def:untyped_lambda_term/naive_subterm} Suppose that, as in \fullref{def:propositional_subformula} for propositional subformulas, we say that \( N \) is a subterm of \( M \) if it is itself a \hyperref[def:formal_language/subword]{subword} of \( M \).
