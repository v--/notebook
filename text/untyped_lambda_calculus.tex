...
\begin{definition}\label{def:lambda_term_alpha_equivalence}\mimprovised
  ...
  \begin{TwoColumns}
    \begin{equation*}\taglabel[\( \logic{Var}_\alpha \)]{rul:def:lambda_term_alpha_equivalence/var}
      \begin{prooftree}
        \infer0[\ref{rul:def:lambda_term_alpha_equivalence/var}]{ x \aequiv x }
      \end{prooftree}
    \end{equation*}
  \BeginSecondColumn
    \begin{equation*}\taglabel[\( \logic{App}_\alpha \)]{rul:def:lambda_term_alpha_equivalence/app}
      \begin{prooftree}
        \hypo{ A \aequiv C }
        \hypo{ B \aequiv D }
        \infer2[\ref{rul:def:lambda_term_alpha_equivalence/app}]{ AB \aequiv CD }
      \end{prooftree}
    \end{equation*}
  \end{TwoColumns}

  \begin{equation*}\taglabel[\( \logic{Abs}_\alpha \)]{rul:def:lambda_term_alpha_equivalence/abs}
    \begin{prooftree}
      \hypo{ A[a \mapsto n] \aequiv B[b \mapsto n] \T{for every} n \not\in \op*{Free}(\qabs a A) }
      \infer1[\ref{rul:def:lambda_term_alpha_equivalence/abs}]{ \qabs a A \aequiv \qabs b B }
    \end{prooftree}
  \end{equation*}
\end{definition}
\begin{comments}
  \item The gist of \ref{rul:def:lambda_term_alpha_equivalence/abs} is that we want \( A[a \mapsto n] \aequiv B[a \mapsto n] \) to hold for every variable \( n \) that is not free in either \( A \) or \( B \), but we still want to allow \( n \) to be \( a \) or \( b \). We do not need a separate condition for \( n \) not to be free in \( \qabs b B \) because, as we will show in \fullref{thm:def:lambda_term_alpha_equivalence/free}, if \( \qabs a A \) and \( \qabs b B \) are equivalence per our definition, they have the same free variables.
  ...
  \begin{itemize}
    \item We have restricted \ref{rul:def:lambda_term_alpha_equivalence/var} to variables only, while Pollack states the rule for general terms.
    \item We have generalized the premise in \ref{rul:def:lambda_term_alpha_equivalence/abs} to hold for all suitable variables simultaneously, and have allowed this variable to be \( a \) or \( b \).
  \end{itemize}
...
\begin{proof}
  ...
  No term has length \( 0 \). If \( M \) has length \( 1 \), it is necessarily a variable, and, the only rule that allows us to conclude that \( M \aequiv N \) is \ref{rul:def:lambda_term_alpha_equivalence/var}, and it is satisfied only if \( M = N \). Then \( \op*{Free}(M) = \set{ M } = \set{ N } = \op*{Free}(N) \).

  If the length of \( M \) is strictly greater than \( 1 \), we have the following possibilities:
  \begin{itemize}
    \item If \( M = AB \), the rule \ref{rul:def:lambda_term_alpha_equivalence/app} is applicable if \( N \) is also an application, say \( N = CD \), and if both \( A \aequiv C \) and \( B \aequiv D \). Then
    \begin{equation*}
      \op*{Free}(M)
      =
    ...
    \item Otherwise, we have \( M = \qabs a A \). The rule \ref{rul:def:lambda_term_alpha_equivalence/abs} implies that \( N = \qabs b B \), where in particular \( A = A[a \mapsto a] \aequiv B[b \mapsto a] \). Then
    \begin{align*}
      \op*{Free}(\qabs a A)
      &=
  ...
  \begin{itemize}
    \item If \( M \) is a variable, the rule \ref{rul:def:lambda_term_alpha_equivalence/var} directly implies that \( M \aequiv M \).
    \item If \( M = AB \) and the inductive hypothesis holds for \( A \) and \( B \), then \( A \aequiv A \) and \( B \aequiv B \), and the rule \ref{rul:def:lambda_term_alpha_equivalence/app} implies that \( M \aequiv M \).
    \item If \( M = \qabs x N \) and if the inductive hypothesis holds for \( N \), then \( N \aequiv N \). The rule \ref{rul:def:lambda_term_alpha_equivalence/abs} implies that \( M \aequiv M \).
  \end{itemize}
  ...
  There are no terms of length \( 0 \), so symmetry holds vacuously. A term of length \( 1 \) is necessarily a variable, and the rule \ref{rul:def:lambda_term_alpha_equivalence/var} implies that \( \alpha \)-equivalent variables are equal. Hence, symmetry holds for terms of length \( 1 \).

  For terms strictly longer than \( 1 \) symbol, we have two possibilities:
  \begin{itemize}
    \item Suppose first that \( M = AB \). Then, if \( M \aequiv N \), \ref{rul:def:lambda_term_alpha_equivalence/app} implies that \( N = CD \), where \( A \aequiv C \) and \( B \aequiv D \).

    By the inductive hypothesis, \( C \aequiv A \) and \( D \aequiv B \). Applying \ref{rul:def:lambda_term_alpha_equivalence/app}, we conclude that \( N \aequiv M \).

    \item Otherwise, we have \( M = \qabs a A \). The rule \ref{rul:def:lambda_term_alpha_equivalence/abs} implies that \( N = \qabs b B \) and, for every variable not in \( M \), \( A[a \mapsto n] \aequiv B[b \mapsto n] \).
    ...
    Furthermore, \fullref{thm:def:lambda_term_alpha_equivalence/free} implies that \( n \) is not free in \( N \), so we can apply \ref{rul:def:lambda_term_alpha_equivalence/abs} to obtain \( N \aequiv M \).
  \end{itemize}
  ...
  We conclude via \ref{rul:def:lambda_term_alpha_equivalence/abs} that \( M \aequiv K \).
\end{proof}
...
\begin{example}\label{ex:def:lambda_term_alpha_equivalence}
  ...
  \begin{thmenum}
    \thmitem{ex:rul:def:lambda_term_alpha_equivalence/combinator} The combinator \( \ref{eq:ex:def:lambda_term/combinator/i} \) has the same essential structure regardless of how we name its variables:
    \begin{equation*}
      I = \qabs \synx \synx \aequiv \qabs \syny \syny \aequiv \qabs \syna \syna \aequiv \cdots
    \end{equation*}

    \thmitem{ex:rul:def:lambda_term_alpha_equivalence/nested} Consider a more complicated case:
    \begin{equation*}
    ...
    In the notation of \ref{rul:def:lambda_term_alpha_equivalence/abs}, we have
    \begin{equation*}
    ...
    We further reduce our check to \( C[\syna \mapsto m] = nm \) and \( D[\syna \mapsto m] = nm \). The rule \ref{rul:def:lambda_term_alpha_equivalence/app} allows us to reduce this problem to checking whether \( n \aequiv n \) and \( m \aequiv m \), while \ref{rul:def:lambda_term_alpha_equivalence/var} confirms that the latter equivalences hold. Therefore, our entire verification is correct and
    \begin{equation*}
      \qabs \synx \qabs \syny \synx \syny \aequiv \qabs \syny \qabs \synx \syny \synx.
    \end{equation*}

    \thmitem{ex:rul:def:lambda_term_alpha_equivalence/freeing} In \fullref{ex:def:lambda_variable_freeness/both} we discussed how \( \synx \) is both bound and free in \( M = I \synx = (\qabs \synx \synx) \synx \).
...
\begin{proof}
  ...
  \end{equation*}
  and \ref{rul:def:lambda_term_alpha_equivalence/abs} allows us to conclude that \( M[\sigma] \aequiv N[\sigma] \).
\end{proof}
...
\begin{proof}
  ...
  Finally, we can combine \eqref{eq:thm:substitution_composition_is_alpha_equivalent/proof/composite} and \eqref{eq:thm:substitution_composition_is_alpha_equivalent/proof/repeated} to conclude that \( M[\rho\sigma] \aequiv M[\sigma][\rho] \) via \ref{rul:def:lambda_term_alpha_equivalence/abs}.
\end{proof}
...
\begin{proof}
  ...
  Therefore, via \ref{rul:def:lambda_term_alpha_equivalence/abs} we can conclude that \( M[\sigma] \aequiv N[\sigma] \).
\end{proof}

\paragraph{Mechanizing \( \alpha \)-equivalence}

\begin{proposition}\label{thm:alpha_equivalence_simplified}
  The rule \ref{rul:def:lambda_term_alpha_equivalence/abs}, specifying when two abstractions are \( \alpha \)-equivalent, is equivalent to the following pair of rules:
  \begin{equation*}\taglabel[\( \logic{Lift}_\alpha \)]{rul:thm:alpha_equivalence_simplified/lift}
    \begin{prooftree}
      \hypo{ A \aequiv B }
      \infer1[\ref{rul:thm:alpha_equivalence_simplified/lift}]{ \qabs x A \aequiv \qabs x B }
    \end{prooftree}
  \end{equation*}

  \begin{equation*}\taglabel[\( \logic{Ren}_\alpha \)]{rul:thm:alpha_equivalence_simplified/ren}
    \begin{prooftree}
      \hypo{ a \neq b }
      \hypo{ a \not\in \op*{Free}(B) }
      \hypo{ A \aequiv B[b \mapsto a] }
      \infer3[\ref{rul:thm:alpha_equivalence_simplified/ren}]{ \qabs a A \aequiv \qabs b B }
    \end{prooftree}
  \end{equation*}
\end{proposition}
\begin{proof}
  \SufficiencySubProof We will show that \ref{rul:thm:alpha_equivalence_simplified/lift} and \ref{rul:thm:alpha_equivalence_simplified/ren} are \hyperref[con:inference_rule_admissibility]{admissible} with respect to \ref{rul:def:lambda_term_alpha_equivalence/abs}.

  \SubProof*{Proof that \ref{rul:thm:alpha_equivalence_simplified/lift} is admissible} Fix a variable \( x \) and terms \( A \aequiv B \). Let \( n \) be any variable not free in \( \qabs x A \). \Fullref{thm:substitution_on_alpha_equivalent_terms} then implies that \( A[x \mapsto n] \aequiv B[x \mapsto n] \).

  The rule \ref{rul:def:lambda_term_alpha_equivalence/abs} then implies that \( \qabs x A \aequiv \qabs x B \).

  Thus, \ref{rul:thm:alpha_equivalence_simplified/lift} is admissible.

  \SubProof*{Proof that \ref{rul:thm:alpha_equivalence_simplified/ren} is admissible} Fix variables \( a \equiv b \) and terms \( A \) and \( B \) such that \( a \) is not free in \( B \) and \( A \aequiv B[b \mapsto a] \).
  ...
  The rule \ref{rul:def:lambda_term_alpha_equivalence/abs} then implies that \( \qabs a A \aequiv \qabs b B \).

  \NecessitySubProof We will show that \ref{rul:def:lambda_term_alpha_equivalence/abs} is admissible with respect to \ref{rul:thm:alpha_equivalence_simplified/lift} and \ref{rul:thm:alpha_equivalence_simplified/ren}.
  ...
  \begin{itemize}
    \item If \( a = b \), then \fullref{thm:lambda_substitution_noop} implies that \( A[a \mapsto a] = A \) and \( B[b \mapsto a] = B \), thus \ref{rul:thm:alpha_equivalence_simplified/lift} implies that \( \qabs a A \aequiv \qabs b B \).

    \item Otherwise, \( A \aequiv B[b \mapsto a] \), and to use \ref{rul:thm:alpha_equivalence_simplified/ren} we must only verify that \( a \) is not free in \( B \).

    If we suppose that \( a \) is free in \( B \), this will contradict \fullref{thm:def:lambda_term_alpha_equivalence/free} since \( A[a \mapsto b] \aequiv B \). Thus, \( a \) is indeed not free in \( B \), and we can use \ref{rul:thm:alpha_equivalence_simplified/ren} to conclude that \( \qabs A \aequiv \qabs b B \).
  \end{itemize}
\end{proof}
...
\end{corollary}
\begin{proof}
  This is a restatement of \ref{rul:thm:alpha_equivalence_simplified/ren}.
\end{proof}
...
\begin{proof}
  ...
  \end{equation*}
  and \ref{rul:thm:alpha_equivalence_simplified/ren} implies that
  \begin{equation*}
...
\begin{defproof}
  ...
  \begin{itemize}
    ...
    \item If \( M = NK \) and if the inductive hypothesis holds for both \( N \) and \( K \), then the rule \ref{rul:def:lambda_term_alpha_equivalence/app} allows us to conclude that \( M \aequiv M'_\Gamma \) and the inductive hypothesis allows us to conclude that the bound variables of \( M'_\Gamma \) are disjoint from \( \Gamma \).
    ...
    \begin{itemize}
      ...
      By the inductive hypothesis, \( N \aequiv N'_{\Gamma,x} \) and the rule \ref{rul:thm:alpha_equivalence_simplified/lift} allows us to conclude that
      \begin{equation*}
      ...
      \end{align*}
      so that, in the notation of \ref{rul:thm:alpha_equivalence_simplified/ren},
      \begin{equation*}
        A \aequiv B[b \mapsto a].
      \end{equation*}

      By definition, \( a = n \) is not free in \( B = N \). Thus, we can apply \ref{rul:thm:alpha_equivalence_simplified/ren} and obtain \( M \ M' \).
