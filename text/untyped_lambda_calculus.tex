...
\begin{definition}\label{def:lambda_term_ast}
  ...
  \begin{thmenum}
    \thmitem{def:lambda_term_ast/variable} If \( M \) is a variable, we define \( T(M) \) as the singleton \hyperref[def:labeled_tree]{labeled tree} with label \( M \). As per \fullref{rem:tree_grafting_nodes}, we use \( 0 \) as the value of the underlying node.
