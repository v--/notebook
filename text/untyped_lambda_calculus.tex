...
\begin{definition}\label{def:lambda_term_ast}
  ...
  \begin{thmenum}
    \thmitem{def:lambda_term_ast/variable} If \( M \) is a variable, let \( T(M) \) be a tree with a single node labeled by it.

    \thmitem{def:lambda_term_ast/application} If \( M = NK \), assuming we have already built \( T(N) \) and \( T(K) \), we obtain \( T(M) \) by joining their roots, and labeling the new node via the symbol \( \cdot \):
    \begin{equation*}
      \includegraphics[page=1]{output/def__lambda_term_ast}
    \end{equation*}

    \thmitem{def:lambda_term_ast/abstraction} If \( \varphi = \abs x N \), assuming we have already built \( T(N) \), we obtain \( T(M) \) by adding one node, and labeling it via \( \abs* x \):
    \begin{equation*}
...
\begin{example}\label{ex:def:lambda_term}
  ...
  \begin{thmenum}
    ...
    \thmitem{ex:def:lambda_term/naive_subterm} Suppose that, as in \fullref{def:propositional_subformula} for propositional subformulas, we say that \( N \) is a subterm of \( M \) if it is itself a \hyperref[def:formal_language/subword]{subword} of \( M \).
...
\begin{example}\label{ex:def:lambda_variable_freeness}
  ...
  \begin{thmenum}
    \thmitem{ex:def:lambda_variable_freeness/abstractor} The term \( \ref{eq:ex:def:lambda_term/combinator/i} = \qabs x x \) has exactly one occurrence of \( x \):
    \begin{equation*}
...
\begin{definition}\label{def:lambda_term_alpha_equivalence}\mimprovised
  ...
  \begin{thmenum}
    \thmitem{def:lambda_term_alpha_equivalence/variable} If both \( M = x = N \) for some variable \( x \).
    \thmitem{def:lambda_term_alpha_equivalence/application} If \( M = A B \) and \( N = C D \) and both \( A \aequiv C \) and \( B \aequiv D \).
    \thmitem{def:lambda_term_alpha_equivalence/abstraction} If \( M = \qabs a A \) and \( N = \qabs b B \) and \( A[a \mapsto c] \aequiv B[b \mapsto c] \) for every variable \( c \) free in neither \( A \) nor \( B \).
  \end{thmenum}
\end{definition}
\begin{comments}
  ...
  \item \Fullref{thm:def:lambda_term_alpha_equivalence/matching_abstraction} gives us an additional rule that can be used for simplifying induction proofs. We choose not to include it, however.
\end{comments}
...
\begin{example}\label{ex:def:lambda_term_alpha_equivalence}
  ...
  \begin{thmenum}
    ...
    \thmitem{ex:def:lambda_term_alpha_equivalence/freeing} In \fullref{ex:def:lambda_variable_freeness/both} we discussed how \( x \) is both bound and free in \( M = Ix = (\qabs x x) x \).
