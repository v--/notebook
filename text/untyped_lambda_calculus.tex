...
\begin{definition}\label{def:lambda_term_ast}
  ...
  \begin{thmenum}
    \thmitem{def:lambda_term_ast/variable} If \( M \) is a variable, we define \( T(M) \) as the singleton \hyperref[def:labeled_tree]{labeled tree} with node \( 0 \) and label \( M \).

    \thmitem{def:lambda_term_ast/application} If \( M = NK \), assuming we have already built \( T(N) \) and \( T(K) \), we define \( T(M) \) by \hyperref[def:ordered_tree_grafting]{grafting} \( T(N) \) and \( T(K) \) to a root labeled with \( \cdot \):
    \begin{equation*}
      \includegraphics[page=1]{output/def__lambda_term_ast}
    \end{equation*}

    \thmitem{def:lambda_term_ast/abstraction} If \( \varphi = \qabs x N \), assuming we have already built \( T(N) \), we define \( T(M) \) by \hyperref[def:ordered_tree_grafting]{grafting} \( T(N) \) to a root labeled with \( \qabs* x \):
    \begin{equation*}
