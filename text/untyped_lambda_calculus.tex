...
\begin{definition}\label{def:untyped_lambda_term_ast}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:untyped_lambda_term_ast/abstraction} If \( \varphi = \qabs u N \), assuming we have already built \( T(N) \), we obtain \( T(M) \) by adding a new root with value \( \qabs* u \):
    \begin{equation*}
...
\begin{definition}\label{def:lambda_abstractor}\mcite[def. 1A5]{Hindley1997}
  Given an abstraction \( M = \qabs u N \), we call \( \qabs* u \) the \term{abstractor} and \( N \) --- the \term{body} of \( M \).
...
\begin{definition}\label{def:uniform_lambda_substitution}\mimprovised
  A \term{uniform substitution} of \( \synlambda \)-terms is a pair \( (\sigma, \sharp) \), where
  \begin{thmenum}[series=def:uniform_lambda_substitution]
    \thmitem{def:uniform_lambda_substitution/sigma} \( \sigma: \op*{Term} \to \op*{Term} \) substitutes terms with other terms.
    \thmitem{def:uniform_lambda_substitution/sharp} \( \sharp: \set{ V \subseteq \op*{Var} \given V \T{is finite} } \to \op*{Var} \) provides us with a new variable identifier not present in some given finite set.

    Considering \fullref{rem:grammar_rules_for_variables}, unless otherwise noted, will suppose that this function simply provides the smallest variable identifier with respect to the \hyperref[def:lexicographic_order]{lexicographic order}.
  \end{thmenum}

  We will use the notation conventions from \fullref{rem:uniform_substitution_notation}. In particular, this notation makes it convenient to specify \( \sigma \) by only how it acts on finitely many variables.
...
\begin{comments}
  ...
  \item Checking whether \( u \) belongs to \( \op*{Var}_\sigma \) in \fullref{def:uniform_lambda_substitution/abstraction} is not strictly necessary; see \fullref{ex:def:lambda_substitution/self} for a discussion.

  \item The substitution rules for abstractions are adjusted so that \fullref{thm:lambda_substitution_free_variables} holds and the pitfalls of \fullref{ex:def:uniform_lambda_substitution} are avoided.
\end{comments}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \begin{balign*}
        ...
        \op*{Free}\parens[\Big]{ N[x \mapsto L] } \cup \op*{Free}\parens[\Big]{ K[x \mapsto L] }
        \reloset {\T{ind.}} = \\ &=
        \parens[\bigg]{ \parens[\Big]{ \op*{Free}(N) \setminus \set{ w } } \cup \op*{Free}(L) }
        \cup
        \parens[\bigg]{ \parens[\Big]{ \op*{Free}(K) \setminus \set{ w } } \cup \op*{Free}(L) }
        = \\ &=
        \parens[\bigg]{ \parens[\Big]{ \op*{Free}(N) \cup \op*{Free}(K) } \setminus \set{ w } } \cup \op*{Free}(L)
        = \\ &=
        \parens[\Big]{ \op*{Free}(NK) \setminus \set{ w } } \cup \op*{Free}(L).
      \end{balign*}
      ...
      \begin{balign*}
        ...
        \op*{Free}\parens[\Big]{ N[x \mapsto L] } \cup \op*{Free}\parens[\Big]{ K[x \mapsto L] }
        \reloset {\T{ind.}} = \\ &=
        \parens[\bigg]{ \parens[\Big]{ \op*{Free}(N) \setminus \set{ w } } \cup \op*{Free}(L) } \cup \op*{Free}(K)
        = \\ &=
        \parens[\Big]{ \op*{Free}(N) \setminus \set{ w } } \cup \op*{Free}(L) \cup \op*{Free}(K)
        = \\ &=
        \parens[\Big]{ \op*{Free}(NK) \setminus \set{ w } } \cup \op*{Free}(L),
      \end{balign*}
    ...
    \begin{itemize}
      ...
      \begin{balign*}
        \op*{Free}\parens[\Big]{ M[x \mapsto L] }
        &=
        \op*{Free}\parens[\Big]{ \qabs u N[x \mapsto L] }
        = \\ &=
        \op*{Free}\parens[\Big]{ N[x \mapsto L] } \setminus \set{ u }
        \reloset {\T{ind.}} = \\ &=
        \parens[\bigg]{ \parens[\Big]{ \op*{Free}(N) \setminus \set{ w } } \cup \op*{Free}(L) } \setminus \set{ u }
        = \\ &=
        \parens[\Big]{ \op*{Free}(N) \setminus \set{ x, u } } \cup \op*{Free}(L)
        = \\ &=
        \parens[\Big]{ \op*{Free}(M) \setminus \set{ w } } \cup \op*{Free}(L),
      \end{balign*}
      ...
      \begin{balign*}
        ...
        \reloset {\T{ind.}} = \\ &=
        \parens[\bigg]{ \parens[\Big]{ \op*{Free}(N[y \mapsto z]) \setminus \set{ w } } \cup \op*{Free}(L) } \setminus \set{ z }
        = \\ &=
        \parens[\Big]{ \op*{Free}(N[y \mapsto z]) \setminus \set{ x, z } } \cup \op*{Free}(L)
        \reloset {\T{ind.}} = \\ &=
        \parens[\bigg]{ \parens[\bigg]{ \parens[\Big]{ \op*{Free}(N) \setminus \set{ u } } \cup \op*{Free}(z) } \setminus \set{ x, z } } \cup \op*{Free}(L)
        = \\ &=
        \parens[\Big]{ \op*{Free}(N) \setminus \set{ x, u } } \cup \op*{Free}(L)
        = \\ &=
        \parens[\Big]{ \op*{Free}(M) \setminus \set{ w } } \cup \op*{Free}(L)
...
\begin{example}\label{ex:def:uniform_lambda_substitution}
  We list examples of \hyperref[def:uniform_lambda_substitution]{substitution} of \( \synlambda \)-terms:
  \begin{thmenum}
    \thmitem{ex:def:uniform_lambda_substitution/capture} The gist of \fullref{thm:lambda_substitution_free_variables} is that substitution avoids \enquote{capturing} free variables under the scope of some abstraction. Instead of \( (\qabs x y)[y \mapsto x] \) giving
    \begin{equation*}
    ...
    \thmitem{ex:def:uniform_lambda_substitution/self} The rule \eqref{eq:def:uniform_lambda_substitution/abstraction/noop} is not strictly necessary. Without it, in the substitution \( (\qabs x x)[x \mapsto y] \), we would instead use \eqref{eq:def:uniform_lambda_substitution/abstraction/renaming} and rename \( x \) to \( z \) first:
    \begin{equation*}
      (\qabs x x)[x \mapsto y]
      \reloset {\eqref{eq:def:uniform_lambda_substitution/abstraction/renaming}} =
