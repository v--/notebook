...
\end{proposition}
\begin{proof}
  \Fullref{thm:real_numbers_archimedean} implies that there exists an integer \( n \) such that \( n \ln x > \ln y \). Then
  \begin{equation*}
    x^n = e^{n \ln x} > e^{\ln y} = y.
  \end{equation*}
\end{proof}

\begin{definition}\label{def:complex_nth_root}
  \todo{Define complex nth roots}.
\end{definition}

\begin{proposition}\label{thm:complex_quadratic_polynomial_roots}
  The \hyperref[def:polynomial_root]{roots} of the \hyperref[def:complex_numbers]{complex} \hyperref[def:polynomial_degree_terminology]{quadratic} \hyperref[def:polynomial_algebra]{polynomial} \( p(X) = a X^2 + b X + c \) are
  \begin{equation}\label{eq:thm:complex_quadratic_polynomial_roots}
