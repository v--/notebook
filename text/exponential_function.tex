\section{Exponential function}\label{sec:exponential_function}
...
\begin{definition}\label{def:exponential_function}
  ...
  \Cref{thm:def:exponential_function/interpolates_power} justifies the notation \( e^z = \exp(z) \).
\end{definition}
\begin{proof}
...
\begin{proposition}\label{thm:def:exponential_function}
  ...
  \begin{thmenum}
    \thmitem{thm:def:exponential_function/monotone}
    ...
    \thmitem{thm:def:exponential_function/interpolates_power} The notation \( \exp(x) \) is consistent with iterated multiplication as defined in \eqref{eq:def:semiring/identity}, that is, \( \exp(n) = \underbrace{e \cdot \ldots \cdot e}_{n \text{times}} \) and for positive integers \( n \), \( \exp(n) =  \) and \( \exp(-n) =\tfrac 1 {\exp(n)} \).
    ...
    \thmitem{thm:def:exponential_function/conjugate} \( \oline{\exp(z)} = \exp(\oline{z}) \).
...
\end{proposition}
\begin{proof}
  \SubProofOf{thm:def:exponential_function/eulers_identity} By \cref{eq:thm:trigonometric_function_basic_roots/whole} and \cref{thm:exponential_trigonometric_identities/eulers_formula}, we have
  \begin{equation*}
    \exp(i\pi) = \cos(\pi) + i\sin(\pi) = -1.
  \end{equation*}

  \SubProofOf{thm:def:exponential_function/derivative} Follows from \cref{thm:power_series_are_locally_uniform_convergent} and \cref{thm:derivative_limit_exchange}.
  ...
  \SubProofOf{thm:def:exponential_function/interpolates_power} We use induction on \( n \) to prove \( \exp(n) = e^n \). The case \( \exp(0) = 1 \) is obvious. If we assume that \( \exp(n) = e^n \), by \cref{thm:def:exponential_function/homomorphism}, we have
  \begin{equation*}
    \exp(n + 1)
    =
    \exp(n) \exp(1)
  ...
  \end{equation*}
  is a series of positive real numbers. To see its convergence, we apply \cref{thm:dalamberts_ratio_test}:
  \begin{equation*}
  ...
  \SubProofOf{thm:def:exponential_function/conjugate} By \cref{thm:exponential_trigonometric_identities/eulers_formula},
  \begin{balign*}
    \oline{\exp(a + bi)}
     & \reloset {\ref{thm:def:exponential_function/homomorphism}} =
    \oline{\exp(a) \exp(bi)}
    \reloset {\ref{thm:def:exponential_function/real_positive}} =   \\ &=
    \exp(a) \oline{(\cos(b) + i\sin(b))}
    =                                                                      \\ &=
    \exp(a) (\cos(b) - i\sin(b))
  ...
  \SubProofOf{thm:def:exponential_function/periodic} By \cref{thm:def:exponential_function/eulers_identity},
  \begin{equation*}
    \exp(x + 2i\pi) = \exp(x) \exp(2i\pi) = \exp(x).
  \end{equation*}

  Furthermore, this is also the minimal period. If we assume that \( \sin(x) \) has another period, say \( p \in (0, 2\pi) \), we would have \( \sin(p) = \sin(0) = 0 \) and \cref{thm:trigonometric_identities/pythagorean_identity} would imply that \( \cos(p) \in \{ -1, 1 \} \). But then \( \cos(p) \) would be an extreme point for \( \cos \), which is not possible because \( \cos \) is convex in \( [0, 2\pi] \) and only has three extremal points --- \( 0, \pi, 2\pi \).
  ...
  \SubProofOf{thm:def:exponential_function/real_bijective} First, assume that \( e^t \) is not injective on \( \BbbR \). Then there exist \( t, r \in \BbbR \), \( t \neq r \), such that \( e^t = e^r \). By \cref{thm:def:exponential_function/real_positive}, both are positive real numbers. In particular, we can divide by \( e^t \) to obtain
  \begin{equation*}
    1
    =
    \frac {e^r} {e^t}
  ...
  We know that \( e^0 = 1 \) from \cref{thm:def:exponential_function/interpolates_power}. Thus, it is enough to show that \( e^t = 1 \) if and only if \( t = 0 \).
  ...
  \SubProofOf{thm:def:exponential_function/bijective} Fix \( a + bi \in S_c \), that is, \( b \in [c, c + 2\pi) \). By \cref{thm:def:exponential_function/homomorphism},
  \begin{equation*}
    e^{a + bi} = e^a e^{bi}.
  \end{equation*}

  By \cref{thm:def:exponential_function/unit_circle}, \( b \mapsto e^{bi} \) is injective for \( b \in [c, c + 2\pi) \) and by \cref{thm:def:exponential_function/real_bijective}, \( a \mapsto e^a \) is injective on \( \BbbR \). It follows that their product is also injective.

  \SubProofOf{thm:def:exponential_function/compound_interest}\mcite[3.31]{Rudin1976AnalysisPrinciples}By \fullref{thm:binomial_theorem},
  \begin{balign*}
...
\begin{definition}\label{def:logarithm}
  ...
  We define the \term{natural logarithm} \( \log(x) \) as \hyperref[def:set_valued_map/inverse]{inverse function} of \( e^x \) from \( \BbbC \setminus \{ 0 \} \) to the strip \( S_c \coloneqq \{ a + bi \colon c \leq b < c + 2\pi \} \).
...
\end{definition}
\begin{proof}
  The well-definedness follows from \cref{thm:def:exponential_function/bijective}.
\end{proof}
...
\begin{proposition}\label{thm:def:logarithm}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:def:logarithm/concave} \( \log \) is concave.
    \thmitem{thm:def:logarithm/power} \( \log(x^p) = p \log(x^p) \).
  \end{thmenum}
\end{proposition}
\begin{proof}
  \SubProofOf{thm:def:logarithm/homomorphism} Follows from \cref{thm:def:exponential_function/homomorphism}.
\end{proof}
...
\begin{proposition}\label{thm:def:power_function}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:def:power_function/monotone}
  \end{thmenum}
\end{proposition}
\begin{proof}
...
\end{proposition}
\begin{proof}
  \SubProofOf{thm:exponential_trigonometric_identities/eulers_formula} Simply note that \cref{def:exponential_function} is a termwise sum of \cref{def:trigonometric_functions/sine} and \cref{def:trigonometric_functions/cosine}, therefore \cref{thm:exponential_trigonometric_identities/eulers_formula/identity} holds.

  \SubProofOf{thm:exponential_trigonometric_identities/inverse_eulers_formula} Follows from \cref{thm:exponential_trigonometric_identities/eulers_formula}.

  \SubProofOf{thm:exponential_trigonometric_identities/de_moivre} From \cref{thm:exponential_trigonometric_identities/eulers_formula},
  \begin{equation*}
    (\cos(z) + i \sin(z))^n
    =
    {e^{iz}}^n
...
\begin{definition}\label{def:hyperbolic_trigonometric_functions}
  In analogy with \cref{thm:exponential_trigonometric_identities/inverse_eulers_formula}, we define \term{hyperbolic trigonometric functions}.
...
\end{proposition}
\begin{proof}
  \Cref{thm:real_numbers_archimedean} implies that there exists an integer \( n \) such that \( n \ln x > \ln y \). Then
  \begin{equation*}
