...
\begin{proposition}\label{thm:def:exponential_function}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:def:exponential_function/interpolates_power} The notation \( \exp(x) \) is consistent with iterated multiplication as defined in \cref{def:group/exponentiation}, that is, \( \exp(n) = \underbrace{e \cdot \ldots \cdot e}_{n \text{times}} \) and for positive integers \( n \), \( \exp(n) =  \) and \( \exp(-n) =\tfrac 1 {\exp(n)} \).
