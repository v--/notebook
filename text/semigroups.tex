...
\begin{definition}\label{def:pointed_set}\mcite[26]{MacLane1998CategoryTheory}
  ...
  \begin{thmenum}
    ...
    Furthermore, as a consequence of \cref{thm:fol_equational_theory_models/image}, the image of a pointed set homomorphism is a submodel of its codomain.
...
\begin{definition}\label{def:set_with_involution}\mimprovised
  A \term{set with an involution} is, unsurprisingly, a \hyperref[def:set]{set} \( X \) equipped with a unary \hyperref[def:morphism_invertibility/involution]{involution} \( \oline{\anon} \).

  Sets with involutions have the following metamathematical properties:
  \begin{thmenum}
    \thmitem{def:set_with_involution/theory} To logically describe involutions, over a \hyperref[def:fol_signature]{signature} with one \hyperref[def:fol_signature/notation]{condensed} unary \hyperref[def:fol_signature/function]{function symbol}, \( \syninv \), we will use the \hyperref[def:fol_theory]{first-order theory} with only the following axiom:
    ...
    \thmitem{def:set_with_involution/homomorphism} A \hyperref[con:logical_model]{structure-preserving map} between two sets with involution \( X \) and \( Y \) is a function \( \varphi: X \to Y \) that satisfies
    \begin{equation}\label{eq:def:set_with_involution/homomorphism}
      \varphi(\oline x) = \oline{\varphi(x)}.
    \end{equation}

    For nonempty sets, these are precisely the \hyperref[def:fol_homomorphism]{first-order homomorphisms}.
    ...
    \thmitem{def:set_with_involution/submodel} A \term{subset with involution} of \( X \) is any subset of \( X \) endowed with the same operation.

    As long as we allow empty universes of first-order models, \cref{thm:fol_equational_theory_models/substructure} implies that the subsets are precisely the \hyperref[def:fol_substructure/submodel]{first-order submodels} of \( X \), and from \cref{thm:fol_categorical_subobjects} that they are the \hyperref[def:categorical_subobject]{categorical subobjects} of \( X \).

    Furthermore, as a consequence of \cref{thm:fol_equational_theory_models/image}, the \hyperref[def:set_valued_map/image]{image} of a homomorphism \( \varphi: X \to Y \) is a submodel of \( Y \).
  \end{thmenum}
\end{definition}
