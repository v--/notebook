...
\begin{definition}\label{def:pointed_set}\mcite[26]{MacLane1998CategoryTheory}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:pointed_set/morphism} We say that the function \( \varphi: X \to Y \) between pointed set is a \term[en=map (between pointed sets) (\cite[26]{MacLane1998CategoryTheory})]{pointed set homomorphism} if
    ...
    \thmitem{def:pointed_set/category} The first-order theory of pointed sets naturally gives rise to a \hyperref[def:fol_theory_model_functor/obj]{category of small models}.

    We denote this category by \( \cat{Set}_* \).
    ...
    \thmitem{def:pointed_set/trivial}\mimprovised In accordance with \cref{def:trivial_object}, we will call a one-element pointed set \term{trivial} because, as shown in \cref{thm:equational_theory_zero_morphisms/trivial}, it is a zero object in \( \cat{Set}_* \).
  \end{thmenum}
\end{definition}

\paragraph{Sets with involutions}

\begin{definition}\label{def:set_involution}\mcite[36]{Schechter1997AnalysisHandbook}
  We say that an \hyperref[def:function/endofunction]{endofunction} \( f: A \to A \) is an \term[ru=инволюция (множеств) (\cite[65]{Гуров2013ТеорияРешёток})]{involution} if \( f \) is its own \hyperref[def:function_invertibility/bijective]{inverse}.
\end{definition}
\begin{comments}
  \item Involutions generalize to arbitrary categories --- see \cref{def:categorical_involution} --- which makes the term slightly ambiguous.
...
\begin{definition}\label{def:set_with_involution}\mimprovised
  A \term{set with an involution} is, unsurprisingly, a \hyperref[def:set]{set} \( X \) equipped with a unary \hyperref[def:set_involution]{involution} \( \oline{\anon} \).
  ...
  \begin{thmenum}
    ...
    \thmitem{def:set_with_involution/morphism} We say that the function \( \varphi: X \to Y \) between sets with involutions is a \term{homomorphism} if
    \begin{equation}\label{eq:def:set_with_involution/morphism}
      \varphi(\oline x) = \oline{\varphi(x)}.
    \end{equation}

    As long as we allow empty universes for first-order models, these are precisely the \hyperref[def:fol_homomorphism]{first-order homomorphisms}, which makes the concepts from \fullref{sec:first_order_structures} applicable.

    \thmitem{def:set_with_involution/category} \hyperref[def:small_set]{Small} sets with involution and their homomorphisms naturally form a \hyperref[def:category]{category}.

    This generalizes the \hyperref[def:fol_theory_model_functor/obj]{category of small models} by also allowing empty sets.
...
\begin{definition}\label{def:binary_operation}
  ...
  \begin{thmenum}
    \thmitem{def:binary_operation/associative}\mcite[\S 1.18(\( \alpha \))]{Ляпин1960Полугруппы} \term[bg=асоциативен закон (\cite[11]{ГеновМиховскиМоллов1991Алгебра}), ru=ассоциативное действие, en=associative law (\cite[17]{Jacobson1985BasicAlgebraI})]{Associativity}:
    \begin{equation}\label{eq:def:binary_operation/associative}
      (\synx \cdot \syny) \cdot \synz \syneq \synx \cdot (\syny \cdot \synz).
    \end{equation}

    \thmitem{def:binary_operation/commutative}\mcite[\S 1.18(\( \beta \))]{Ляпин1960Полугруппы} \term[bg=комутативен (закон) (\cite[11]{ГеновМиховскиМоллов1991Алгебра}), ru=коммутативное (действие), en=commutative law (\cite[17]{Jacobson1985BasicAlgebraI})]{Commutativity}:
    ...
    \begin{equation}\label{eq:def:binary_operation/idempotent}
      \synx \syneq \synx \cdot \synx.
    \end{equation}

    \thmitem{def:binary_operation/cancellative}\mcite[\( 1.18(\varepsilon) \)]{Ляпин1960Полугруппы} Left and right \term[ru=сокращение, bg=съкращение (\cite[77]{ГеновМиховскиМоллов1991Алгебра}), en=cancellation law (\cite[17]{Jacobson1985BasicAlgebraI})]{cancellation}:
    \begin{align}
...
\begin{remark}\label{rem:binary_operations_to_infinitary}
  ...
  \begin{thmenum}
    \thmitem{rem:binary_operations_to_infinitary/lattice} The operations of a \hyperref[def:lattice]{lattice} arise by specializing \hyperref[def:extremal_points/supremum_and_infimum]{suprema and infima} to binary sets. Conversely, as discussed in \cref{def:lattice/theory/leq}, the binary lattice operations induce a \hyperref[def:partially_ordered_set]{partial order}, which can be used to define arbitrary suprema and infima. If the lattice happens to be \hyperref[def:complete_lattice]{complete}, instead of the binary operations, we can use
    \begin{equation*}
...
\begin{remark}\label{rem:bourbaki}
  ...
  \begin{itemize}
    \item The ubiquitous \hyperref[def:empty_set]{empty set} symbol \( \varnothing \) is attributed to the 1939 edition of \cite[E II.1]{Bourbaki1970ThéorieDesEnsembles} by \cite{MacTutor:EarliestUsesOfLogicSymbols}. Another early notation is \( 0 \), used by \textcite[12]{Hausdorff1935Mengenlehre} and as late as \cite[10]{Натансон1974ВещественныйАнализ}.

    \item The ubiquitous \hyperref[def:naive_set_theory]{set membership} symbol \( {\in} \) is also attributed to Bourbaki's 1939 set theory book. Another early notation is \( \varepsilon \), used again by \textcite[11]{Hausdorff1935Mengenlehre}, as early as \cite[X]{Peano1889ArithmeticesPrincipia} and as late as \cite[1]{Kelley1975GeneralTopology}.
  ...
  \textcite[121]{Kuyk1977ComplementarityInMathematics} comments the work of Bourbaki as follows:
  \begin{displayquote}
...
\begin{remark}\label{rem:magma_terminology}
  ...
  \begin{itemize}
    ...
    When describing the design of the computer algebra system Magma, \textcite[1]{BosmaCannonMatthews1994MagmaLanguage} explain that the name is based on Bourbaki's definition. Some authors use \enquote{magma} to refer to this system rather than the algebraic structure, for example \textcite{Rotman2015AdvancedModernAlgebraPart1} and \textcite{ConwaySloane1998SpherePackings}.
    ...
    \cite[def. 1.7.4]{Schubert1972Categories},
    \cite[168]{Awodey2010CategoryTheory},
    ...
    \item There are also other terms for a set with a binary operation, for example \enquote{\textru{мультипликативное множество}} (\enquote{multiplicative set}), used in \cite[20]{Ляпин1960Полугруппы}, or \enquote{monad}, used in \cite[29]{Jacobson1985BasicAlgebraI}.
  \end{itemize}
  ...
  \cite[135]{Кострикин2020АлгебраЧасть1},
  \cite[121]{БелоусовТкачёв2004ДискретнаяМатематика},
...
\begin{concept}\label{con:opposite_object}\mimprovised
  We sometimes have a natural notion of \term{opposite object}. We list the relevant for us cases in \cref{rem:con:opposite_object}. In all of them, we have a \hyperref[def:category]{category} \( \cat{C} \) with an \hyperref[def:categorical_involution]{involutive} functor \( (\anon)^\oppos \).
\end{concept}
\begin{comments}
  \item For most of the examples in \cref{rem:con:opposite_object}, it will be sufficient to define \( (\anon)^\oppos \) on objects and extend it by sending every morphism to itself. An exception are \hyperref[def:opposite_category]{opposite categories}, which require \hyperref[def:opposite_functor]{opposite functors} --- see \cref{thm:opposite_category_functor}.
\end{comments}
...
\begin{remark}\label{rem:con:opposite_object}
  ...
  \begin{itemize}
    ...
    \item We have \hyperref[def:partially_ordered_set/opposite]{opposite (pre)ordered sets}, in which the order is reversed, with \hyperref[def:lattice/opposite]{opposite lattices} and \hyperref[def:boolean_algebra/opposite]{opposite Boolean algebras} as special cases.
    ...
    \item We have \hyperref[def:directed_multigraph/opposite_arcs]{opposite directed multigraphs}, in which the arrows are reversed, with \hyperref[def:directed_graph]{directed graphs} as special cases.
    ...
    Categories also have a duality principle --- \fullref{thm:category_duality}.
  \end{itemize}
\end{remark}

\begin{definition}\label{def:semigroup}\mcite[28]{Ляпин1960Полугруппы}
  A \term[ru=полугруппа (\cite[28]{Ляпин1960Полугруппы}), en=semigroup (\cite[1]{Golan1999Semirings})]{semigroup} is a nonempty set \( G \) equipped with an \hyperref[def:binary_operation/associative]{associative} \hyperref[def:operation_arity_terminology]{binary} \hyperref[def:operation_on_set]{operation} \( \cdot: G \times G \to G \).

  We often denote this operation by juxtaposition as \( xy \) instead of \( x \cdot y \). Thus written, the operation generalizes \hyperref[def:semiring]{multiplication in semirings}, even though we allow the operation to not be commutative in order to also encompass \hyperref[def:set_valued_map/composition]{function composition}. If the operation is meant to generalize addition, we modify the notation and terminology according to \cref{def:additive_semigroup}.
  ...
  \begin{thmenum}[series=def:semigroup]
    ...
    A notational variation that turns out to be useful in practice is the theory of additive semigroups, which we present in \cref{def:additive_semigroup}.

    \thmitem{def:semigroup/morphism}\mcite[348]{Ляпин1960Полугруппы} We say that the function \( \varphi: G \to H \) between semigroups is a \term[ru=гомоморфизм (полугрупп)]{semigroup homomorphism} if, whenever \( x \) and \( y \) are in \( G \), we have
    ...
    \thmitem{def:semigroup/category} \hyperref[def:small_set]{Small} semigroups and their homomorphisms naturally form a \hyperref[def:category]{category}.

    This generalizes the \hyperref[def:fol_theory_model_functor/obj]{category of small models} by also allowing empty semigroups.
    ...
    \thmitem{def:semigroup/opposite}\mimprovised We define the \term{opposite semigroup} \( (G, \cdot)^\oppos \) as \( (G, \star) \), where \( {\star} \) reverses the order of multiplicands:
    \begin{equation*}
      x \star y \coloneqq y \cdot x.
    \end{equation*}

    By sending every homomorphism to itself, this becomes an \hyperref[con:opposite_object]{opposite object functor} in the category of small semigroups.
    ...
    \thmitem{def:semigroup/commutative}\mcite[29]{Ляпин1960Полугруппы} If the semigroup operation is \hyperref[def:binary_operation/commutative]{commutative}, we say that the semigroup itself is \term[ru=коммутативная полугруппа]{commutative}.
  \end{thmenum}
\end{definition}

\begin{definition}\label{def:power_semigroup}\mcite[1]{McCarthyHayes1973PowerSemigroup}
  The \hyperref[def:basic_set_operations/power_set]{power set} \( \pow(G) \) of a \hyperref[def:semigroup]{semigroup} \( (G, \cdot) \) is also a semigroup with the pointwise operation
  \begin{equation*}
    A \odot B \coloneqq \set{ a \cdot b \given a \in A \T{and} b \in B }.
  \end{equation*}

  We will call \( (\pow(G), \odot) \) the \term{power semigroup} of \( G \).

  The pointwise operation \( \odot \) generalizes \( \cdot \) via the canonical embedding sending \( g \) to its singleton set \( \set{ g } \). We usually use the same symbol for both.

  We may, following \textcite[3]{Stewart1998MatrixAlgorithmsVol1}, call this extension the \term{Minkowski operation} corresponding to \( \cdot \). In particular, in \hyperref[def:topological_vector_space]{topological vector spaces}, we will call the sum \( A + B \) of subsets their \term[en=Minkowski sum (\cite[\S 11.1.3]{Berger1987GeometryI})]{Minkowski sum}.
\end{definition}
\begin{comments}
  \item General Minkowski operations are not well-established, but Minkowski sums are ubiquitous. For example, Minkowski sums in topological vector spaces are discussed without a name in
  \cite[6]{Rudin1991FunctionalAnalysis},
  \cite[4]{Rockafellar1997ConvexAnalysis},
  \cite[4]{Clarke2013OptimalControl} and
  \cite[25]{МагарилИльяевТихомиров2002ВыпуклыйАнализ}.

  \item The pointwise product of \hyperref[def:semiring_ideal]{semiring ideals} is distinct from their \hyperref[def:semiring_ideal/product]{product ideal}. We use multiplicative notation for the latter.

  \item Subset inclusion induces a natural \hyperref[def:ordered_semigroup]{ordering} on \( \pow(G) \) --- see \cref{ex:def:ordered_semigroup/power}
\end{comments}
...
\begin{remark}\label{rem:square_and_quadratic}
  ...
  According to \textcite[8]{ConwayGuy1998BookOfNumbers}, both \enquote{square} and \enquote{quadratic} derive from the Latin \enquote{\textla{ex quadrem}}. In Russian, \enquote{square root} and \enquote{quadratic equation} both use the same adjective, \enquote{\textru{квадратный}} (\enquote{quadratic}), however the established English terms are for some reason different.
\end{remark}
...
\begin{proposition}\label{thm:semigroup_exponentiation_properties}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:semigroup_exponentiation_properties/multiplication} If \( G \) is \hyperref[def:semigroup/commutative]{commutative}, then, for any two members \( x \) and \( y \) of \( G \) and any positive integer \( n \),
...
\begin{definition}\label{def:semigroup_neutral_element}\mcite[115]{БелоусовТкачёв2004ДискретнаяМатематика}
  In the \hyperref[def:semigroup/theory]{theory of semigroups}, the following axioms state that \( \syne \) is a (left or right) \term[ru=нейтральный елемент, en=neutral element (\cite[7]{Perrone2024StartingCategoryTheory})]{neutral element} for \( {\synmult} \):
  \begin{align}
    \qforall \synx (\syne \syneq \synx \synmult \syne), \label{eq:def:semigroup_neutral_element/left} \\
    \qforall \synx (\synx \syneq \syne \synmult \synx). \label{eq:def:semigroup_neutral_element/right}
  \end{align}

  These axioms do not depend of associativity, so we may in fact consider them more generally.

  Because of the uniqueness shown in \cref{thm:semigroup_neutral_element_uniqueness}, we will not need to distinguish between left and right neutral elements.
\end{definition}

\begin{remark}\label{rem:neutral_element_terminology}
  The \hyperref[def:semigroup_neutral_element]{neutral element} of a \hyperref[def:monoid]{monoid} (or \hyperref[def:group]{group}) is also called an \enquote{identity} and \enquote{unit}. Since the latter two terms are overloaded, we prefer \enquote{neutral element} in the general case of abstract monoids.

  Here is a terminological survey among our sources:
  \begin{itemize}
    \item The phrase \enquote{neutral element} (resp. \enquote{\textru{нейтральный елемент}} in Russian) is preferred in
    \cite[7]{Perrone2024StartingCategoryTheory},
    \cite[80]{Тыртышников2017ОсновыАлгебры} and
    \cite[116]{БелоусовТкачёв2004ДискретнаяМатематика}.

    \item The word \enquote{identity}, hinting at identity morphisms, is preferred in
    \cite[1]{Golan1999Semirings},
    \cite[def. II.1.2(ii)]{Aluffi2009Algebra} and
    \cite[def. 1.2.1]{Bourbaki1998Algebra1to3}.

    \item The word \enquote{unit} (resp. \enquote{\textru{единица}} in both Russian in Bulgarian), hinting at (semi)rings, is preferred in
    \cite[29]{Jacobson1985BasicAlgebraI},
    \cite[3]{Lang2002Algebra},
    \cite[18]{Курош1967ТеорияГрупп},
    \cite[135]{Кострикин2020АлгебраЧасть1},
    \cite[76]{ГеновМиховскиМоллов1991Алгебра} and
    \cite[372]{Обрешков1962ВисшаАлгебра}.

    For additional ambiguity regarding this term, see \cref{rem:semiring_divisibility_terminology/invertible}.
  \end{itemize}
\end{remark}

\begin{proposition}\label{thm:semigroup_neutral_element_uniqueness}
  In a \hyperref[def:semigroup]{semigroup}, if \( l \) and \( r \) are left and right \hyperref[def:semigroup_neutral_element]{neutral elements}, they are equal. In particular, neutral elements are unique.
\end{proposition}
\begin{proof}
  \begin{equation*}
    l
...
\begin{definition}\label{def:monoid}\mcite[def. 1.1]{Jacobson1985BasicAlgebraI}
  A \term[ru=моноид (\cite[94]{Мальцев1970АлгебраическиеСистемы})]{monoid} is a \hyperref[def:semigroup]{semigroup} with a \hyperref[def:semigroup_neutral_element]{neutral element}\fnote{This makes monoids \hyperref[def:pointed_set]{pointed sets}.}.
  ...
  \begin{thmenum}
    ...
    To obtain an \hyperref[def:fol_equational_theory]{equational theory} of monoids over this signature, we add the axioms \eqref{eq:def:semigroup_neutral_element/left} and \eqref{eq:def:semigroup_neutral_element/right} for left and right neutral element (with \( \syn1 \) substituted for \( \syne \)).

    A notational variation that turns out to be useful in practice is the theory of monoids, which we present in \cref{def:additive_semigroup}.

    \thmitem{def:monoid/morphism}\mcite[def. 1.6]{Jacobson1985BasicAlgebraI} W say that the function \( \varphi: M \to N \) between monoids is a \term[en=homomorphism (of monoids) (\cite[def. 1.6]{Jacobson1985BasicAlgebraI})]{monoid homomorphism} if it is both a \hyperref[def:semigroup/morphism]{semigroup homomorphism} and a \hyperref[def:pointed_set/morphism]{pointed set homomorphism}, i.e. it respects both the monoid operation and the neutral element.

    These are precisely the \hyperref[def:fol_homomorphism]{first-order homomorphisms}, which makes the concepts from \fullref{sec:first_order_structures} applicable.

    \thmitem{def:monoid/category}\mcite[12]{MacLane1998CategoryTheory} The first-order theory of monoids naturally gives rise to a \hyperref[def:fol_theory_model_functor/obj]{category of small models}. We denote this category by \( \cat{Mon} \).

    We list several properties:
    \begin{itemize}
      \item \( \cat{Mon} \) is \hyperref[def:concrete_category]{concrete} over both the \hyperref[def:pointed_set/category]{category of small pointed sets} and the \hyperref[def:pointed_set/category]{category of small semigroups}.

      \item The theory of monoids is equational and thus has \hyperref[def:fol_free_term_model]{free term models}; \Fullref{thm:fol_free_term_model_universal_property} provides a \hyperref[def:adjoint_functor]{left adjoint} to the \hyperref[def:fol_theory_forgetful_functor/set]{forgetful functor} \( U: \cat{Mon} \to \cat{Set} \).
      ...
      \item Because of this left adjoint, \cref{thm:concrete_category_function_invertibility/injective} implies that a monoid homomorphism is a \hyperref[def:morphism_cancellation/left]{monomorphism} if and only if it is \hyperref[def:function_invertibility/injective]{injective}.

      \item For epimorphisms a similar statement does not hold, unfortunately. \Cref{ex:def:balanced_category/equational} provides an example --- the embedding \( \iota: \BbbN \to \BbbZ \) is an epimorphism due to \fullref{thm:group_of_differences_universal_property}, but it is clearly not surjective.

      \item Another consequence of the theory being equational is that, by \cref{thm:fol_equational_category_of_models_is_complete}, \( \cat{Mon} \) is \hyperref[def:complete_category]{complete}.

      Furthermore, we will shown in \cref{thm:category_of_small_monoids_is_cocomplete} that it is also \hyperref[def:cocomplete_category]{cocomplete}.
    \end{itemize}
    ...
    \thmitem{def:monoid/trivial}\mcite[exerc. II.1.3]{Aluffi2009Algebra} In accordance with \cref{def:trivial_object}, we will call a one-element monoid \term{trivial} because, as shown in \cref{thm:equational_theory_zero_morphisms/trivial}, it is a zero object in \( \cat{Mon} \).
    ...
    \thmitem{def:monoid/commutative}\mimprovised If the monoid operation is \hyperref[def:binary_operation/commutative]{commutative}, we say that the monoid itself is \term[en=commutative monoid (\cite[41]{Jacobson1985BasicAlgebraI})]{commutative}.

    They form their own \hyperref[def:subcategory/full]{full subcategory} of \( \cat{Mon} \), which we denote by \( \cat{CMon} \).
  \end{thmenum}
\end{definition}
...
\begin{example}\label{ex:def:monoid}
  ...
  \begin{thmenum}
    ...
    \thmitem{ex:def:monoid/limit_ordinal} For a \hyperref[def:successor_and_limit_ordinal]{limit ordinal}, however, the set of smaller ordinals is \emph{non-commutative} under \hyperref[def:ordinal_arithmetic/addition]{ordinal addition} as a consequence of \cref{thm:ordinal_addition_algebraic_properties}.
    ...
    \thmitem{ex:def:monoid/semilattice} If a \hyperref[def:semilattice]{join-semilattice} is bounded from below, it is a monoid. A similar statement holds for \hyperref[def:lattice]{meet-semilattices} that are bounded from above.
...
\begin{definition}\label{def:monoid_idempotent}\mcite[1]{Golan1999Semirings}
  We say that an element \( x \) of a \hyperref[def:monoid]{monoid} (or, more generally, a \hyperref[def:semigroup]{semigroup}) is \term[ru=идемпотент (\cite[72]{Ляпин1960Полугруппы})]{idempotent} if \( x^2 = x \). Since the neutral element is always idempotent, we call idempotent elements distinct from it \term[en=nontrivial (idempotent) (\cite[287]{Rudin1991FunctionalAnalysis})]{nontrivial}.
\end{definition}
\begin{comments}
