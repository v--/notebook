...
\begin{definition}\label{def:monoid}\mcite[def. 1.1]{Jacobson1985BasicAlgebraI}
  A \term[ru=моноид (\cite[94]{Мальцев1970АлгебраическиеСистемы})]{monoid} is a \hyperref[def:semigroup]{semigroup} with a distinguished element \( e \) such that \( xe = x = ex \) for every \( x \). Such an element is obviously unique, and we call it the \term[ru=нейтральный елемент (\cite[80]{Тыртышников2017ОсновыАлгебры}), en=neutral element (\cite[165]{Knauer2019AlgebraicGraphTheory})]{neutral element}\fnote{There are several terminological conventions for neutral elements --- see \cref{rem:neutral_element_terminology}.} of the monoid. This makes monoids \hyperref[def:pointed_set]{pointed sets}.
  ...
  \begin{thmenum}
    ...
    \begin{align}
      \qforall \synx (\syn1 \synmult \synx \syneq \synx), \label{eq:def:monoid/theory/neutral/left} \\
      \qforall \synx (\synx \synmult \syn1 \syneq \synx). \label{eq:def:monoid/theory/neutral/right}
    \end{align}
...
\begin{remark}\label{rem:neutral_element_terminology}
  The neutral element of a \hyperref[def:monoid]{monoid} (or \hyperref[def:group]{group}) is also called an \enquote{identity} and \enquote{unit}. Since the latter two terms are overloaded, we prefer \enquote{neutral element} in the general case of abstract monoids.
