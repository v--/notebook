...
\begin{definition}\label{def:pointed_set}\mcite[26]{MacLane1998CategoryTheory}
  ...
  \begin{thmenum}
    \thmitem{def:pointed_set/theory}\mimprovised Pointed sets can also be viewed as \hyperref[def:fol_semantics/model]{models} of an empty \hyperref[def:first_order_theory]{theory} for a \hyperref[def:first_order_language]{first-order logic language} with a constant symbol, i.e. a nullary \hyperref[def:fol_signature/functions]{functional symbol}.
    ...
    \thmitem{def:pointed_set/category} We denote the \hyperref[def:category_of_first_order_models]{category of \( \mscrU \)-small models} for this theory by \( \ucat{Set}_* \).
  \end{thmenum}
\end{definition}
...
\begin{definition}\label{def:set_with_involution}\mimprovised
  ...
  \begin{thmenum}
    ...
    \thmitem{def:set_with_involution/category} We introduce no special notation for the corresponding \hyperref[def:category_of_first_order_models]{category of \( \mscrU \)-small models}.
  \end{thmenum}
\end{definition}
...
\begin{definition}\label{def:binary_operation}
  ...
  The following \hyperref[def:first_order_syntax/formula]{formulas}, or rather their \hyperref[def:fol_quantifier_closure]{universal closures}, are common \hyperref[def:first_order_theory/axiomatized]{axioms}:
  \begin{thmenum}
...
\begin{definition}\label{def:semigroup}\mcite[28]{Ляпин1960Полугруппы}
  ...
  \begin{thmenum}[series=def:semigroup]
    ...
    \thmitem{def:semigroup/category}\mimprovised We introduce no special notation for the \hyperref[def:category_of_first_order_models]{category of \( \mscrU \)-small models} for the theory of semigroups.
