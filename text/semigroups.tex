...
\begin{definition}\label{def:pointed_set}\mcite[26]{MacLane1998CategoryTheory}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:pointed_set/morphism} Given pointed sets \( (X, e_X) \) and \( (Y, e_Y) \), we say that the function \( \varphi: X \to Y \) is a \term[en=map (between pointed sets) (\cite[26]{MacLane1998CategoryTheory})]{homomorphism} if
...
\begin{definition}\label{def:semigroup}\mcite[28]{Ляпин1960Полугруппы}
  ...
  \begin{thmenum}[series=def:semigroup]
    ...
    \thmitem{def:semigroup/morphism}\mcite[348]{Ляпин1960Полугруппы} Given semigroups \( G \) and \( H \), we say that the function \( \varphi: G \to H \) is a \term[ru=гомоморфизм (полугрупп)]{homomorphism} if, whenever \( x \) and \( y \) are in \( G \), we have
    ...
    As long as we allow empty universes for first-order models, \cref{thm:fol_equational_theory_models/substructure} implies that the sub-semigroups are precisely the \hyperref[def:fol_substructure/submodel]{first-order submodels} of \( X \), and from \cref{thm:fol_categorical_subobjects} that they are the \hyperref[def:categorical_subobject]{categorical subobjects} of \( X \).
...
\begin{definition}\label{def:monoid}\mcite[def. 1.1]{Jacobson1985BasicAlgebraI}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:monoid/morphism}\mcite[def. 1.6]{Jacobson1985BasicAlgebraI} Given monoids \( M \) and \( N \), we say that the function \( \varphi: M \to N \) is a \term{homomorphism} if it is both a \hyperref[def:semigroup/morphism]{semigroup homomorphism} and a \hyperref[def:pointed_set/morphism]{pointed set homomorphism}, i.e. it respects both the monoid operation and the neutral element.
    ...
    \begin{itemize}
      \item \( \ucat{Mon} \) is \hyperref[def:concrete_category]{concrete} with respect to both the \hyperref[def:pointed_set/category]{category of \( \mscrU \)-small pointed sets} and the \hyperref[def:pointed_set/category]{category of \( \mscrU \)-small semigroups}.
    ...
    It follows from \cref{thm:fol_equational_theory_models/substructure} that submonoids are precisely the \hyperref[def:fol_substructure/submodel]{first-order submodels} of \( M \), and from \cref{thm:fol_categorical_subobjects} that they are precisely the \hyperref[def:categorical_subobject]{categorical subobjects} of \( M \).

    Furthermore, as a consequence of \cref{thm:fol_equational_theory_models/image}, the image of a monoids homomorphism is a submonoid of its codomain.

    \thmitem{def:monoid/trivial}\mimprovised In accordance with \cref{def:trivial_object}, we will call a one-element monoid \term{trivial} because, as shown in \cref{thm:equational_theory_zero_morphisms/trivial}, it is a zero object in \( \ucat{Mon} \).

    \thmitem{def:monoid/opposite}\mimprovised The \hyperref[def:semigroup/opposite]{opposite semigroup} \( M^{\oppos} \) of a monoid \( M \) is again a monoid with the same neutral element.
