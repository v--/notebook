...
\begin{definition}\label{def:semigroup}\mcite[28]{Ляпин1960Полугруппы}
  ...
  We often denote this operation by juxtaposition as \( xy \) instead of \( x \cdot y \). Thus written, the operation generalizes \hyperref[def:semiring]{multiplication}, even though we allow the operation to not be commutative in order to also encompass \hyperref[def:set_valued_map/composition]{function composition}. If the operation is meant to generalize addition, we modify the notation and terminology according to \cref{def:additive_semigroup}.
...
\begin{definition}\label{def:semiring_neutral_element}\mcite[115]{БелоусовТкачёв2004ДискретнаяМатематика}
  In the \hyperref[def:semiring/theory]{theory of semirings}, the following axioms state that \( \syne \) is a (left or right) \term[ru=нейтральный елемент, en=neutral element (\cite[7]{Perrone2024StartingCategoryTheory})]{neutral element} for \( {\synmult} \):
  \begin{align}
    \qforall \synx (\syne \synmult \synx \syneq \synx), \label{eq:def:semiring_neutral_element/left} \\
    \qforall \synx (\synx \synmult \syne \syneq \synx). \label{eq:def:semiring_neutral_element/right}
  \end{align}

  Because of the uniqueness shown in \cref{thm:semiring_neutral_element_uniqueness}, we will not need to distinguish between left and right neutral elements.
\end{definition}

\begin{remark}\label{rem:neutral_element_terminology}
  The \hyperref[def:semiring_neutral_element]{neutral element} of a \hyperref[def:monoid]{monoid} (or \hyperref[def:group]{group}) is also called an \enquote{identity} and \enquote{unit}. Since the latter two terms are overloaded, we prefer \enquote{neutral element} in the general case of abstract monoids.
...
\begin{proposition}\label{thm:semiring_neutral_element_uniqueness}
  In a \hyperref[def:semiring]{semiring}, if \( l \) and \( r \) are left and right \hyperref[def:semiring_neutral_element]{neutral elements}, they are equal. In particular, neutral elements are unique.
\end{proposition}
\begin{proof}
  \begin{equation*}
    l
    \reloset {\eqref{eq:def:semiring_neutral_element/right}} =
    lr
...
\begin{definition}\label{def:monoid}\mcite[def. 1.1]{Jacobson1985BasicAlgebraI}
  A \term[ru=моноид (\cite[94]{Мальцев1970АлгебраическиеСистемы})]{monoid} is a \hyperref[def:semigroup]{semigroup} with a \hyperref[def:semiring_neutral_element]{neutral element}\fnote{This makes monoids \hyperref[def:pointed_set]{pointed sets}.}.
  ...
  \begin{thmenum}
    ...
    To obtain an \hyperref[def:fol_equational_theory]{equational theory} of monoids over this signature, we add the axioms \eqref{eq:def:semiring_neutral_element/left} and \eqref{eq:def:semiring_neutral_element/right} for left and right neutral element (with \( \syn1 \) substituted for \( \syne \)).
