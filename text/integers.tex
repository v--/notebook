...
\begin{definition}\label{def:integers}\mimprovised
  We define the \hyperref[def:ring]{ring} \( \BbbZ \) of \term[bg=цели числа (\cite[374]{ГеновМиховскиМоллов1991Алгебра}), ru=целые числа (\cite[def. 20.1]{Проскуряков1951ТеоретическиеОсновыАрифметики})]{integers} as the \hyperref[def:ring_of_differences]{ring of differences} of the \hyperref[def:semiring/commutative]{commutative semiring} \( \BbbN \) of zero-based \hyperref[def:natural_numbers]{natural numbers}.
\end{definition}
\begin{comments}
  \item We consider the following an indispensable part of \( \BbbZ \):
  \begin{itemize}
    \item The ordering defined in \cref{def:integer_ordering}, which makes \( \BbbZ \) an \hyperref[def:ordered_semiring]{ordered (semi)ring}.
    \item The Euclidean domain structure discussed in \cref{rem:integer_domain_chain}.
    \item The order topology discussed in \cref{thm:order_topology_on_integers_is_discrete}.
  \end{itemize}
\end{comments}
...
Rather than defining an order from first principles and then using the signum function from \cref{def:signum} with its established properties, it will be more convenient for us to define the signum function, prove its properties, and then define the order itself.
...
\end{lemma}
\begin{proof}
  \ExistenceSubProof Due to the trichotomy on natural numbers shown in \cref{def:peano_arithmetic_ordering}, we have the following mutually exclusive possibilities:
  \begin{itemize}
    \item If \( a = b \), then \( x = [(a, a)] = [(0_\BbbN, 0_\BbbN)] \), hence \( x \) is zero.
    \item If \( a < b \), by \cref{def:peano_arithmetic_ordering}, there exists some natural number \( c > 0 \) such that \( a + c = 0_\BbbN + b \). Then
    \begin{equation*}
  ...
  By \cref{thm:natural_number_addition_properties}, \( \BbbN \) is cancellative, so \( a = b \).

  \SubProof{Proof of exclusive conditions} Finally, suppose that \( n = \iota(a) = -\iota(b) \). Then \( a + b = 0_\BbbN \). Since \( \BbbN \) is \hyperref[def:zerosumfree]{zerosumfree} by \cref{thm:natural_number_addition_properties}, it follows that \( a = b = 0_\BbbN \), and hence \( n \) is zero.
...
\begin{definition}\label{def:integer_signum}\mimprovised
  ...
  This is a well-defined \hyperref[def:set_valued_map/partial]{total function} due to \cref{thm:integer_signum_lemma}.
\end{definition}
\begin{comments}
  \item We will later show that this matches the signum function on \( \BbbZ \) defined in \cref{def:signum}.
\end{comments}
...
\begin{proposition}\label{thm:def:integer_signum}
  ...
  \begin{thmenum}
    ...
    This is an analog of \cref{thm:def:ordered_semiring_positivity/sum}.

    \thmitem{thm:def:integer_signum/product} \( \sgn(nm) = \sgn(n) \cdot \sgn(m) \).

    This is an analog of \cref{thm:def:signum}.

    \thmitem{thm:def:integer_signum/inverse} \( \sgn(-n) = -\sgn(n) \).

    This is an analog of \cref{thm:def:ordered_semiring_positivity/additive_inverse}.
  \end{thmenum}
\end{proposition}
\begin{proof}
  ...
  \SubProofOf{thm:def:integer_signum/inverse} From \cref{thm:def:integer_signum/product} it follows that \( \sgn(-n) = \sgn(-1) \cdot \sgn(n) = -\sgn(n) \).
\end{proof}

\begin{definition}\label{def:integer_ordering}\mimprovised
  We extend the \hyperref[def:peano_arithmetic_ordering]{natural number ordering} \( \leq_\BbbN \) to the \hyperref[def:integers]{integers} \( \BbbZ \) via the following truth table:
  \begin{equation*}
...
\end{definition}
\begin{comments}
  \item We will now use terms like \enquote{positive} and \enquote{negative} from \cref{def:ordered_semiring}, although we are yet to prove that this ordering is compatible with the algebraic structure.
\end{comments}
...
\begin{proposition}\label{thm:def:integer_ordering}
  ...
  \begin{thmenum}
    ...
    This partially ensures compatibility with the signum function from \cref{def:signum}.

    \thmitem{thm:def:integer_ordering/negative} \( 0 < n \) if and only if \( \sgn(n) = -1 \).

    This also ensures compatibility with the signum function from \cref{def:signum}.

    \thmitem{thm:def:integer_ordering/inverse} \( n \leq m \) if and only if \( -m \leq -n \).

    This is an analog of \cref{thm:ordered_ring/additive_inversion}
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \item If \( n \) is negative and \( m \) is not, then by \cref{thm:def:integer_signum/inverse} \( -m \) is negative and \( -n \) is not. Hence, \( -m \leq -n \).
  \end{itemize}
  ...
  \begin{itemize}
    ...
    \item If \( n \), \( m \) and \( k \) are negative, then \( -k \leq -n \) and, by \cref{thm:def:integer_ordering/inverse}, \( n \leq k \).
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      \item If \( n + s + k \) is nonnegative, then \( n + s + k \leq n + (s + 1) + k \) by \cref{thm:natural_numbers_are_well_ordered}, even without the inductive hypothesis.
      \item If \( n + s + k \) is negative, then \( -n - s - k \) is positive, and, again by \cref{thm:natural_numbers_are_well_ordered},
      \begin{equation*}
        -n - (s + 1) - k \leq -n - (s + 1) - k + 1 = -n - s - k.
      \end{equation*}

      \Cref{thm:def:integer_ordering/inverse} then implies that
      \begin{equation*}
        n + s + k \leq n + (s + 1) + k.
      \end{equation*}
    \end{itemize}
  ...
  \begin{itemize}
    \item If both \( n \) and \( m \) are nonnegative, the statement follows from \cref{thm:natural_numbers_are_well_ordered}.
    \item If both \( n \) and \( m \) are negative, \cref{thm:def:integer_ordering/inverse} implies that \( -m \leq -n \), then the previous case implies that \( (-m)k \leq (-n)k \), and finally \( nk \leq mk \).
    \item If \( n \) is negative and \( m \) is nonnegative, we have two cases:
    \begin{itemize}
      \item The case \( k = 0 \) is trivial due to the absorption property \eqref{def:semigroup_absorbing_element}.

      \item If \( k > 0 \), \cref{thm:def:integer_signum/product} implies that
      \begin{equation*}
...
\begin{corollary}\label{thm:integer_multiplicative_invertibility}
  Only \( 1 \) and \( -1 \) are \hyperref[def:semiring_divisibility/invertible]{invertible} in the ring \( \BbbZ \) of \hyperref[def:integers]{integers}.
\end{corollary}
\begin{proof}
  We proceed similarly to \cref{thm:natural_number_multiplicative_invertibility}.

  Let \( 1 = nm \). We will show that either \( n = m = 1 \) or \( n = m = -1 \).

  We can assume that \( n \) and \( m \) are both nonzero since, by \cref{thm:integers_are_integral_domain}, \( \BbbZ \) is entire and hence \( nm \) would be zero.

  Furthermore, \cref{thm:def:integer_signum/product} implies that \( 1 = \sgn(nm) = \sgn(n) \cdot \sgn(m) \), hence \( \sgn(n) = \sgn(m) \). By \cref{thm:def:integer_ordering}, either \( n > 0 \) and \( m > 0 \) or \( n < 0 \) and \( m < 0 \).

  \begin{itemize}
    \item If both \( n > 0 \) and \( m > 0 \), they are both embeddings of natural numbers, and, by \cref{thm:natural_number_multiplicative_invertibility}, \( n = m = 1 \).

    \item Otherwise, if \( n < 0 \) and \( m < 0 \), from the previous case applied to \( -n \) and \( -m \) we conclude that \( n = m = -1 \).
  \end{itemize}
\end{proof}
...
\end{proposition}
\begin{proof}
  Since positive integers are embeddings of natural numbers, the property follows from \cref{thm:natural_numbers_archimedean}.
\end{proof}
...
\end{corollary}
\begin{proof}
  Follows from the same argument as our proof in \cref{thm:order_topology_on_integers_is_discrete} by noting that the subbase \eqref{eq:def:order_topology/subbase} contains the interval \( \set{ 0 } = (-\infty, 1) \).
\end{proof}

\paragraph{Integer division}

\begin{definition}\label{def:integer_absolute_value}\mimprovised
  We will define absolute values for general complex numbers in \cref{def:complex_absolute_value}, but we will find absolute values useful for proving some structural properties of the ring of integers. So, we will introduce the auxiliary definition
  \begin{equation*}
    \begin{aligned}
...
\begin{proof}
  ...
  Otherwise, we have \( n > 0 \), and \cref{thm:integers_archimedean} implies that there exists some positive integer \( l \) such that \( n < lm \). Since the positive integers are well-founded, we can take \( l \) to be minimal, and then \( n \geq (l - 1)m \).
  ...
  Indeed, suppose that \( km \leq n \). Since \( km = (-k)(-m) \), we conclude that \( l \geq -k \). \Cref{thm:ordered_ring/additive_inversion} implies that \( -l \leq k \).
...
\begin{remark}\label{rem:integer_division_uniqueness}
  ...
  \begin{thmenum}
    \thmitem{rem:integer_division_uniqueness/max} \textcite[23]{Jacobson1985BasicAlgebraI}, \textcite[thm. A-2.2]{Rotman2015AdvancedModernAlgebraPart1}, \textcite[prop. 1.1]{Knapp2016BasicAlgebra}, \textcite[83]{Aluffi2009Algebra} and \textcite[sec. 2.12]{Тыртышников2007ЛинейнаяАлгебра} assume that
    \begin{equation*}
    ...
    Note that \eqref{eq:rem:integer_division_uniqueness/max/q_cases} is well-defined due to \cref{thm:integer_division_boundedness_lemma}.
...
\end{remark}
\begin{comments}
  \item All these quotient algorithms are implemented in \cite{notebook:code}, in the module
  \begin{CodeRefDisplay}
    \ManualCodeRef{arithmetic.divisibility}
  \end{CodeRefDisplay}
\end{comments}
...
\end{algorithm}
\begin{comments}
  \item We can safely use this algorithm when restricted to nonnegative integers since, by \cref{thm:alg:integer_division/quot_positivity}, the quotient of nonnegative integers is nonnegative.

  \item We use the notation \( \quot(n, m) \) and \( \rem(n, m) \) from \cref{def:euclidean_domain}.

  \item The quotient and remainder are \( q_{\max} \) and \( r_{\max} \) from \cref{rem:integer_division_uniqueness/max}, where Euclidean division of integers is extensively discussed.
\end{comments}
\begin{defproof}
  \ExistenceSubProof We must show that the maximum and minimum in \eqref{eq:alg:integer_division/quot} are attained. Both follow from \cref{thm:integer_division_boundedness_lemma}.
...
\end{proposition}
\begin{proof}
  By \cref{thm:integers_are_integral_domain}, \( \BbbZ \) is entire. The Euclidean domain structure is described by \fullref{alg:integer_division}.
\end{proof}

\begin{proposition}\label{thm:subgroups_of_integers}
  If \( H \) is a \emph{nontrivial} \hyperref[def:group/subobject]{subgroup} of \( \BbbZ \), then \( H = n\BbbZ \), where \( n \) is the smallest positive number in \( H \).
\end{proposition}
\begin{comments}
...
\begin{proof}
  ...
  \Cref{thm:integer_division_boundedness_lemma} implies that there exists a largest integer \( k \) such that
  \begin{equation*}
    km \leq n.
  \end{equation*}
  ...
  If \( k < 0 \), then \cref{thm:ordered_ring/strict_prod} implies that \( km < 0 \cdot m = 0 \) and \( 0 < n \). Thus, if \( k \) is negative, then \( 0 \) is a larger integer satisfying \( 0 \cdot m \leq n \), which contradicts the maximality of \( k \). Then \( k \) must be nonnegative.
  ...
  \begin{itemize}
    ...
    \item If \( m < 0 \), \cref{thm:alg:integer_division/minus_m} implies that
    \begin{equation*}
      \quot(n, m) = -\quot(n, -m),
    \end{equation*}
...
\begin{remark}\label{rem:integer_domain_chain}
  ...
  \begin{itemize}
    \item \( \BbbZ \) is a \hyperref[def:euclidean_domain]{Euclidean domain} as shown in \cref{thm:integers_are_euclidean_domain}.
    \item As an Euclidean domain, \( \BbbZ \) is a \hyperref[def:principal_ideal_domain]{principal ideal domain} as a consequence of \cref{thm:def:euclidean_domain/pid}.
...
\begin{proposition}\label{thm:natural_number_divisibility_lattice}
  The semiring of \hyperref[def:natural_numbers]{natural numbers} is a \hyperref[def:extremal_points/bounds]{bounded lattice} with respect to \hyperref[def:semiring_divisibility]{semiring divisibility}. Explicitly:
  \begin{thmenum}
    \thmitem{thm:natural_number_divisibility_lattice/join} The \hyperref[def:lattice]{join} of \( n \) and \( m \) is their unique \hyperref[def:lcm]{least common multiple}, denoted by \( \lcm(n, m) \).
    ...
    \thmitem{thm:natural_number_divisibility_lattice/meet} Dually, the \hyperref[def:lattice]{meet} of \( n \) and \( m \) is their unique \hyperref[def:gcd]{greatest common divisor}, denoted by \( \gcd(n, m) \) and constructed via \fullref{alg:euclidean_algorithm}.
...
\begin{proof}
  ...
  \SubProofOf[def:binary_relation/antisymmetric]{antisymmetry} If \( n \mid m \) and \( m \mid n \), there exist numbers \( a \) and \( b \) such that \( n = am \) and \( m = bn \). Then \( n = abn \), and we can cancel \( n \) to obtain that \( ab = 1 \). But, by \cref{thm:natural_number_multiplicative_invertibility}, \( 1 \) is the only invertible element in \( \BbbN \), hence \( a = b = 1 \), and thus \( n = m \).
  ...
  By \cref{thm:def:semiring_ideal/division}, the lattice of principal ideals in \( \BbbN \) must be dual to it. Indeed, by \fullref{thm:bezouts_identity}, we have that
  \begin{equation*}
  ...
  Thus, by \eqref{eq:def:peano_arithmetic_ordering/nonstrict}, \( n \leq m \).
\end{proof}

\begin{remark}\label{rem:integer_gcd}
  We discuss in \cref{rem:choice_of_associates}, in general \hyperref[def:gcd_domain]{GCD domain}, the \hyperref[def:gcd]{greatest common divisors} are not unique.

  The \hyperref[def:gcd]{greatest common divisor} of integers of is, by convention, \emph{positive}. This leaves a canonical choice for both the greatest common divisor and the least common multiple. By \cref{thm:natural_number_divisibility_lattice}, the \hyperref[thm:semiring_divisibility_order]{divisibility order} of positive integers is compatible with the usual \hyperref[def:integer_ordering]{integer ordering}. In particular, this shows that the GCD is associative, which allows us to write \( \gcd(a_1, \ldots, a_n) \) rather than nesting applications of \( \gcd \).
...
\begin{definition}\label{def:integer_parity}\mimprovised
  We associate with each \hyperref[def:integers]{integer} its \term{parity} --- we say that the integer is \term{even} if it is \hyperref[def:semiring_divisibility]{divisible} by \( 2 \) and \term{odd} otherwise.
\end{definition}
\begin{comments}
  \item Even and odd integers are \hyperref[con:assumed_knowledge]{assumed knowledge} from primary school, so it is difficult to find a definition in the literature.
\end{comments}
...
\begin{proof}
  ...
  Suppose that \( n + 1 \) is also even; i.e. \( n + 1 = 2k \) for some integer \( k \).

  Then \( 2m + 1 = 2k \), hence \( 2(m - k) = 1 \), which contradicts \cref{thm:integer_multiplicative_invertibility}. It follows that \( n + 1 \) is odd.

  \SubProof*{Proof if \( n \) is odd} Suppose that \( 2 \) does not divide \( n \).

  \Cref{thm:integers_archimedean} implies that there exists a positive integer multiple of \( 2 \) exceeding \( n \); let \( m \) be the smallest such multiple. Then \( 2m > n \) and \( 2(m - 1) < n \) (we ignore the possibility \( 2(m - 1) = n \) since \( n \) is odd).
...
\end{proposition}
\begin{comments}
  \item This proposition is a direct corollary of \fullref{thm:euclids_lemma}, but we will give a direct proof since we will delay discussions of prime numbers until \fullref{sec:prime_numbers}.
\end{comments}
\begin{proof}
  We will show that \( nm \) is odd if and only if \( n \) and \( m \) both are.

  \Cref{alg:integer_division} gives us integer \( q \) and \( r \) with \( r = 0 \) or \( r = 1 \) such that \( n = 2q + r \). Clearly \( 2q \) is even, so, by \cref{thm:integer_sum_parity}, both \( n \) and \( r \) have the same parity.

  Similarly, \( m = 2s + t \), where \( t = 0 \) or \( t = 1 \). Then
  \begin{equation*}
    nm = (2q + r) (2s + t) = 4qs + 2qt + 2rs + rt.
  \end{equation*}

  Clearly \( nm \) and \( rt \) have the same parity. But \( rt \) is odd if and only if \( t = r = 1 \), which happens if and only if both \( n \) and \( m \) are odd.
\end{proof}

\begin{corollary}\label{thm:integer_parity_grading}
  \hyperref[def:integer_parity]{Integer parity} induces a \( \BbbF_2 \)-\hyperref[def:graded_ring]{ring grading} on \( \BbbZ \).
\end{corollary}
\begin{proof}
