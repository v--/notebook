...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      \item The case \( k = 0 \) is trivial due to the absorption property \eqref{def:semiring/theory/axioms/absorption}.
...
\begin{corollary}\label{thm:integer_multiplicative_invertibility}
  Only \( 1 \) and \( -1 \) are \hyperref[def:divisibility/invertible]{invertible} in the ring \( \BbbZ \) of \hyperref[def:integers]{integers}.
\end{corollary}
\begin{proof}
...
\begin{proposition}\label{thm:natural_number_divisibility_lattice}
  The semiring of \hyperref[def:natural_numbers]{natural numbers} is a \hyperref[def:extremal_points/bounds]{bounded lattice} with respect to \hyperref[def:divisibility]{semiring divisibility}. Explicitly:
  \begin{thmenum}
