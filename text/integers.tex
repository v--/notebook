...
\begin{comments}
  ...
  \item We are also free to use the general absolute value function defined in \fullref{def:totally_ordered_ring_absolute_value}.
\end{comments}
...
\end{lemma}
\begin{proof}
  Since \( \BbbZ \) is a PID, it is also a Bezout domain as a consequence of \fullref{thm:def:principal_ideal_domain/bezout}.
\end{proof}
...
\begin{remark}\label{rem:integer_domain_chain}\hfill
  ...
  \begin{itemize}
    ...
    \item By \fullref{thm:def:principal_ideal_domain/ufd}, \( \BbbZ \) is a \hyperref[def:unique_factorization_domain]{unique factorization domain} and by \fullref{thm:def:principal_ideal_domain/bezout}, it is a \hyperref[def:bezout_domain]{Bezout domain}.
    \item By \fullref{thm:def:unique_factorization_domain/gcd} (or by definition of Bezout domain), \( \BbbZ \) is a \hyperref[def:gcd_domain]{GCD domain}.
  \end{itemize}
\end{remark}
...
\begin{definition}\label{def:prime_number}\mcite[2]{Knapp2016BasicAlgebra}
  A \term[bg=просто число (\cite[10]{ДочевДимитровЧуканов1976}), ru=простое число (\cite[62]{Кострикин2004Том1})]{prime number} is an integer greater than \( 1 \) whose only proper \hyperref[def:divisibility]{divisor} is \( 1 \). Non-prime integers greater than \( 1 \) are called \term{composite numbers}.
\end{definition}
...
\end{theorem}
\begin{proof}
  As discussed in \fullref{rem:integer_domain_chain}, \( \BbbZ \) is a unique factorization domain.
\end{proof}
