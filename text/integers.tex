...
\end{corollary}
\begin{proof}
  We proceed similarly to \fullref{thm:natural_number_multiplicative_invertibility}.

  Let \( 1 = nm \). We will show that either \( n = m = 1 \) or \( n = m = -1 \).

  We can assume that \( n \) and \( m \) are both nonzero since, by \fullref{thm:integers_are_integral_domain}, \( \BbbZ \) is entire and hence \( nm \) would be zero.

  Furthermore, \fullref{thm:def:integer_signum/product} implies that \( 1 = \sgn(nm) = \sgn(n) \cdot \sgn(m) \), hence \( \sgn(n) = \sgn(m) \). By \fullref{thm:def:integer_ordering}, either \( n > 0 \) and \( m > 0 \) or \( n < 0 \) and \( m < 0 \).

  \begin{itemize}
    \item If both \( n > 0 \) and \( m > 0 \), they are both embeddings of natural numbers, and, by \fullref{thm:natural_number_multiplicative_invertibility}, \( n = m = 1 \).
...
\end{proposition}
\begin{proof}
  Since positive integers are embeddings of natural numbers, the property follows from \fullref{thm:natural_numbers_archimedean}.
\end{proof}
