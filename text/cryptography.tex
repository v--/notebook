\chapter{Cryptography}\label{sec:cryptography}
...
\begin{algorithm}[Diffie-Hellman key exchange]\label{alg:diffie_hellman_key_exchange}\mcite[99]{Koblitz1994NumberTheoryAndCryptography}
  ...
  The keys are assumed to be members of some finite \hyperref[def:cyclic_group]{cyclic group} \( G \). Fix a generator \( g \) of \( G \), assumed to be public.
  ...
  The shared key is then
  \begin{equation*}
    k_{a,b} \coloneqq c(a, b, \bnfves) = p_b^{s_a}.
  \end{equation*}

  Both users now have access to the same secret because \cref{thm:semigroup_exponentiation_properties/repeated} implies that
  \begin{equation*}
