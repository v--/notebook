...
\begin{algorithm}[RSA cryptosystem]\label{alg:rsa}\mcite{RivestShamirAdleman1978RSA}
  ...
  \begin{thmenum}
    \thmitem{alg:rsa/keys} Three choices are needed to generate a key pair --- two prime numbers \( p \) and \( q \), whose product we denote by \( n \), and a positive integer \( e \) \hyperref[def:coprime_numbers]{coprime} with \( \varphi(n) = (p - 1) (q - 1) \), where \( \varphi \) is \hyperref[def:eulers_totient_function]{Euler's totient function}.

    By \cref{thm:bezout_lemma}, there exist integer \( c \) and \( d \) such that
    \begin{equation*}
