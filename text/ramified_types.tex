...
\begin{example}\label{ex:self_reference}
  ...
  \begin{thmenum}
    \thmitem{ex:self_reference/impredicativity} \hyperref[con:impredicative_definition]{Impredicativity}, defined formally for \hyperref[def:hol_expression/formula]{higher-order formulas} in \cref{def:hol_formula_predicativity}, allows a \hyperref[con:proposition]{proposition} to refer to itself.
    ...
    \Fullref{thm:burali_forti_paradox} chronologically precedes Russell's paradox, but is of similar nature.

    \thmitem{ex:self_reference/godels_theorem} As discussed in \cref{rem:thm:peano_arithmetic_incompleteness}, G\"odel proved \fullref{thm:peano_arithmetic_incompleteness} by encoding first-order formulas and proofs as natural numbers in order to craft a self-referential formula.
    ...
    Confining ourselves to \hyperref[def:strongly_normalizing_lambda_term]{strongly normalizing \( \muplambda \)-terms} prevents self-application. Such restrictions arise naturally by typing restrictions --- see, for example, \fullref{thm:simply_typable_terms_are_strongly_normalizing}.
  \end{thmenum}
\end{example}

\begin{remark}\label{rem:vicious_circle_principle}
  When explaining their resolution of certain paradoxes such as \fullref{thm:russels_paradox}, \textcite[83]{WhiteheadRussell1927PrincipiaMathematicaVol1} write the following:
  \begin{displayquote}
  ...
  \begin{displayquote}
    \enquote{no totality can contain members defined in terms of itself}.
  \end{displayquote}

  \Textcite[347]{Church1956LogicVol1} discusses the vicious circle principle and relates it to Poincar\'{e}'s notion of \hyperref[con:impredicative_definition]{impredicative definitions}.
  ...
  We can formalize this principle via the formula
  \begin{equation*}
    \parens[\big]{ \qforall {\synp^{\syn\iota \synimplies \syn\omicron}} \synp(\synx^{\syn\iota}) \syniff \synp(\syny^{\syn\iota}) } \synimplies (\synx^{\syn\iota} \syneq \syny^{\syn\iota}).
  \end{equation*}
  ...
  \begin{equation*}
    P \coloneqq \qabs {\synx^{\syn\iota}} \parens[\big]{ \qforall {\synp^{\syn\iota \synimplies \syn\omicron}} \synp(\synx^{\syn\iota}) \syniff \synp(\syny^{\syn\iota}) } \synimplies (\synx^{\syn\iota} \syneq \syny^{\syn\iota}).
  \end{equation*}
...
\begin{remark}\label{rem:hol_parametrized_denotation}
  We described in \cref{rem:simply_typed_lambda_term_dependency} how (typed) \( \muplambda \)-abstraction can be used to create ad-hoc functions entirely within the \hyperref[con:metalanguage]{object language}.

  This approach, usable in \hyperref[def:higher_order_logic]{higher-order logic}, reduces the utility of free variables, which can emulate functions in more restrictive systems such as \hyperref[def:propositional_logic]{propositional logic} (see \cref{def:parametrized_propositional_denotation}) or \hyperref[def:first_order_logic]{first-order logic} (see \cref{def:fol_parameterized_formula_denotation}).
...
\begin{definition}\label{def:ramified_theory_of_types}
  ...
  He presented this system in \cite{Russell1908TypeTheory}. Along with Alfred Whitehead, he used it to derive a foundational portion of set theory. These efforts were published under the name \enquote{Principia Mathematica} in three volumes --- \cite{WhiteheadRussell1927PrincipiaMathematicaVol1}, \cite{WhiteheadRussell1927PrincipiaMathematicaVol2} and \cite{WhiteheadRussell1927PrincipiaMathematicaVol3}.

  Russell tried to circumvent self-reference by introducing types and orders. His theory predates most of what is shown in this chapter, and, as originally presented, it is archaic. Still, both types and orders are relevant concepts. Type theory has developed immensely, unlike orders. We will attempt to describe the latter on modified \hyperref[def:hol_expression/formula]{formulas of higher-order logic}. Our efforts are based on Russell's own writings, which we quote in \cref{rem:hol_formula_order_origin}, and on their modernized reinterpretation in \cite[\S 2B]{KamareddineLaanNederpelt2005TypeTheory}.

  Russell's approach is purely syntactic\fnote{Russell's books precede the modern semantic notions.}, and relies fundamentally on \hyperref[con:syntactic_substitution]{substitution}. Variables in formulas are regarded as placeholders for either individuals or other formulas. A formula with free variables is regarded as a function\fnote{Russell's books also precede \( \muplambda \)-calculus, and hence also precedes the practice of creating ad-hoc functions via \( \muplambda \)-abstraction.}. \enquote{Applying} such a function must be done via substitution rather than via the more elaborate constructs of \( \muplambda \)-calculus (the two approaches are discussed in \cref{rem:hol_parametrized_denotation}). The ramified types allow finer control over which formulas are allowed to be substituted for which variables.
  ...
  \begin{thmenum}
    ...
    The role of orders comes into play when handling propositional functions. As already mentioned, we are only interested in formulas. A propositional function with ramified type \( (\rho_1^{k_1}, \cdots, \rho_m^{k_m})^n \) is \emph{not} a logical predicate of higher-order logic with unramified type \( \rho_1 \synimplies \cdots \synimplies \rho_m \synimplies \syn\omicron \), but rather a formula \( \varphi \) with lexicographically-ordered\fnote{An alphabetical ordering of free variables is also suggested by \textcite[250]{Russell1908TypeTheory} and \textcite[22]{KamareddineLaanNederpelt2005TypeTheory}. Lexicographic ordering allows also treating suffixes.} free variables of the corresponding types and orders. Arguments are to be applied via substitution, as per \cref{rem:hol_parametrized_denotation}.
    ...
    We give Russell's justification in \cref{rem:hol_formula_order_origin/class}.
    ...
    Thus, \( \synp \) in \( \zeta_n \) can quantify over \( \zeta_n' \) since \( \zeta_n' \) is of the lowest possible order. But we still avoid circularity because \( \zeta_n' \) and \( \zeta_n \) are distinct.
  \end{thmenum}
\end{definition}

\begin{remark}\label{rem:unramified_theory_of_types}
  \Textcite[xix]{WhiteheadRussell1927PrincipiaMathematicaVol1} write
  \begin{displayquote}
  ...
  Russell's ramified types were later simplified to what was called \enquote{simple types}. For example, when adapting the \enquote{simple theory of types} to his \( \muplambda \)-calculus\fnote{We discuss Church's theory in \fullref{sec:higher_order_logic}.} \textcite{Church1940STT} writes
  \begin{displayquote}
  ...
  To explain what lead to the abandonment of ramified types, \textcite[45]{Kleene1971Metamathematics} writes
  \begin{displayquote}
...
\begin{remark}\label{rem:unpropositional_functions_in_ramified_theory_of_types}
  ...
  Unfortunately, this leaves it unclear how to ramify an individual-valued function type like \( (\syn\iota \synimplies \syn\omicron) \synimplies \syn\iota \), which \textcite[57]{Church1940STT} uses for the \hyperref[con:description_operator]{definite description operator}.
...
\begin{remark}\label{rem:hol_formula_order_origin}
  We discussed Russell's ramified type theory in \cref{def:ramified_theory_of_types}. We will present several excerpts from Russell's \cite{Russell1908TypeTheory} which demonstrate his original ideas.
  ...
  \begin{thmenum}
    ...
    \thmitem{rem:hol_formula_order_origin/functions} The following except hints at how to treat functions whose values are not propositions (see also Russell's theory of descriptions discussed in \cref{con:denotation}):
    \begin{displayquote}
    ...
    Russell proceeds to introduce the definite description operator \( \turnediota \) (which we discuss in \cref{con:description_operator} and \cref{rem:hol_definite_description}) and to use it to build classes that can be used as individuals. For example, he defines the singleton class \( \iota\text{\lq} x \) as an abbreviation for \( \hat y (y = x) \), and zero \( 0 \) as \( \iota\text{\lq} \Lambda \) (he admits that this produces a distinct zero for every type).
...
\end{definition}
\begin{comments}
  \item A quantifiable type has infinite order if and only if is unramifiable. We discuss the use of \( \infty \) in this case in \cref{ex:def:extended_real_numbers/formula_order}.
\end{comments}
\begin{proof}
  ...
  Otherwise, let \( \rho^n \) be a predicative ramification. We will use \fullref{thm:induction_on_abstract_syntax} on \( \tau \) to show that \( n = \ord(\tau) \). The cases \( \tau = \syn\iota \) and \( \tau = \syn\omicron \) are obvious.
...
\begin{definition}\label{def:hol_formula_ramification_order}\mimprovised
  ...
  We say that \( n \) is the \term{ramification order} of the \hyperref[def:hol_expression/formula]{higher-order formula} \( \varphi \) if any of the following equivalent conditions hold:
  \begin{thmenum}
...
\end{definition}
\begin{comments}
  \item A formula has infinite ramification order if and only if is unramifiable (this will be shown). We discuss the use of \( \infty \) in this case in \cref{ex:def:extended_real_numbers/formula_order}.
\end{comments}
\begin{defproof}
...
\begin{definition}\label{def:hol_formula_free_type}\mimprovised
  We define the \term{free type} of a \hyperref[def:hol_expression/formula]{formula} \( \varphi \) of \hyperref[def:higher_order_logic]{higher-order logic} with free variables \( x_1^{\tau_1}, \ldots, x_n^{\tau_n} \) (ordered \hyperref[def:lexicographic_order]{lexicographically}) as \( \tau_1 \synimplies \cdots \synimplies \tau_n \synimplies \syn\omicron \) (if \( n = 0 \), the free type is simply \( \syn\omicron \)).
\end{definition}

\begin{definition}\label{def:hol_formula_predicativity}\mimprovised
  We say that a \hyperref[def:hol_expression/formula]{formula} \( \varphi \) of \hyperref[def:higher_order_logic]{higher-order logic} is \term{predicative} if any of the following equivalent conditions hold:
  \begin{thmenum}
...
\begin{example}\label{ex:def:hol_formula_ramification_order}
  We list examples of \hyperref[def:hol_expression/formula]{higher-order formulas} and their \hyperref[def:hol_formula_ramification_order]{ramification orders}:
  \begin{thmenum}
    ...
    \thmitem{ex:def:hol_formula_ramification_order/group} In the notation of higher-order logic, the group axiom \eqref{eq:def:group/theory/inverse/left} becomes
    \begin{equation}\label{eq:ex:def:hol_formula_ramification_order/group/inverse_left}
      \qforall {\synx^{\syn\iota}} (\synx \synmult \syninv \synx \syneq \synx).
    \end{equation}

    The formula has no free variables and only one bound variable of type \( \syn\iota \). The ramification order of \eqref{eq:ex:def:hol_formula_ramification_order/group/inverse_left} is thus \( 1 + \max\set{ 0 } \). It is rightfully referred to as a \enquote{first-order axiom}.
    ...
    \thmitem{ex:def:hol_formula_ramification_order/axiom_of_specification} Consider the \hyperref[def:function_application_notation/infix]{infix} constant \( {\synin}: \syn\iota \synimplies \syn\iota \synimplies \syn\omicron \) from \fullref{ch:set_theory}.
    ...
    Now consider the full axiom of specification \eqref{eq:def:zfc/specification/sol}, which we will restate here:
    \begin{equation*}
      \qforall {\synp^{\syn\iota \synimplies \syn\omicron}} \qforall {\syna^{\syn\iota}} \qexists {\synb^{\syn\iota}} \qforall {\synx^{\syn\iota}} \parens[\big]{ \synx \synin \synb \syniff (\synx \synin \syna \synwedge \synp(\synx)) }.
    \end{equation*}
    ...
    \thmitem{ex:def:hol_formula_ramification_order/peano_arithmetic} As another example, consider the full induction axiom \eqref{eq:def:peano_arithmetic/theory/PA3/sol}, which we restate here:
    \begin{equation*}
    ...
    Hence, \eqref{eq:def:peano_arithmetic/theory/PA3/sol} is indeed a second-order axiom.
...
\paragraph{Bounded higher-order logic}

\begin{definition}\label{def:bounded_hol}\mimprovised
  We call \term{bounded higher-order logic} of order \( n \) the \hyperref[con:syntax_fragment]{fragment} of \hyperref[def:higher_order_logic]{higher-order logic} featuring only formulas of \hyperref[def:hol_formula_ramification_order]{ramification order} \( n \) and below.
\end{definition}
\begin{comments}
...
\begin{example}\label{ex:def:bounded_hol}
  ...
  \begin{thmenum}
    \thmitem{ex:def:bounded_hol/zeroth} \Textcite[201]{Andrews2002Logic} writes that \hyperref[def:propositional_logic]{propositional logic} can be regarded as \enquote{zeroth-order logic}.
    ...
    Many-sorted first-order logic may be closer to higher-order logic in its formalisms than to standard one-sorted first-order logic. This leads to \cref{def:many_sorted_fol}.
    ...
    We introduce some specific terminology in \cref{def:second_order_logic}. What mostly matters to us is unisorted \emph{monadic} second-order logic, in which only \( \syn\iota \) and \( \syn\iota \synimplies \syn\omicron \) are allowed as variable types. The latter can be regarded as characteristic functions for sets of individuals, so \textcite[def. 3.6.1(i)]{Hinman2005Logic} calls them \enquote{set variables}. He suggests denoting set variables by capital Latin letters.

    As an example, consider again the full induction axiom \eqref{eq:def:peano_arithmetic/theory/PA3/sol}, which we will restate here again (with \( \synS^+ \) as a successor symbol):
    \begin{equation*}
      \qforall {\synp^{\syn\iota \synimplies \syn\omicron}}
      \parens[\big]
        {
    ...
    Using the dedicated set variable syntax, this becomes
    \begin{equation*}
...
\begin{definition}\label{def:many_sorted_fol}\mimprovised
  We have studied ordinary univariate \hyperref[def:first_order_logic]{first-order logic} extensively, however many-sorted variants require typed variables. This is easily captured as \hyperref[def:bounded_hol]{bounded higher-order logic} of order \( 1 \), in which variables range over individuals of different sorts.
\end{definition}
\begin{comments}
  \item Many-sorted first-order logic, with semantics similar to ours, is discussed in \cite[def. 6.2]{TroelstraVanDalen1988ConstructivismVol2}.
\end{comments}

\begin{definition}\label{def:second_order_logic}\mimprovised
  Of special interest is \hyperref[def:bounded_hol]{bounded higher-order logic} of order \( 2 \). We will call it \term[en=full second-order logic (\cite[277]{Hinman2005Logic})]{full second-order logic}.

  A more restricted fragment is \term[en=monadic second-order logic (\cite[def. 3.6.1]{Hinman2005Logic})]{monadic second-order logic}, in which we allow only individuals and univariate (monadic) predicates as variables. We regard the predicates as \hyperref[def:subset_characteristic_function]{characteristic functions} for sets of individuals.
\end{definition}
\begin{comments}
  \item Monadic second-order logic covers most of what we would possibly need beyond first-order logic, for example the \enquote{full} variants of \hyperref[def:peano_arithmetic]{Peano arithmetic} and \hyperref[def:zfc]{Zermelo-Fraenkel set theory}. It can also be used for describing some non-categorical theories like \hyperref[def:complete_lattice]{complete lattices} --- see \cref{rem:logical_theory_of_complete_lattices} --- or \hyperref[def:hypergraph]{hypergraphs} --- see \cref{rem:logical_theory_of_graphs/hypergraphs}.

  \item We can introduce a simplified syntax for monadic second-order logic --- see \cref{ex:def:bounded_hol/second}.
\end{comments}
