...
\begin{example}\label{ex:def:hol_formula_ramification_order}
  ...
  \begin{thmenum}
    ...
    \begin{equation}\label{eq:ex:def:hol_formula_ramification_order/group/axiom}
      \qforall {\synx^{\syn\iota}} (\synx \syntimes \syninv\synx \syneq \synneutral \synwedge \syninv\synx \syntimes \synx \syneq \synneutral).
    \end{equation}
...
\begin{example}\label{ex:def:bounded_hol}
  ...
  \begin{thmenum}
    ...
    If we denote these set variables by capital Latin letters, we will obtain a slightly simpler notation. Consider the reinterpretation from \eqref{eq:ex:def:hol_formula_ramification_order/peano_arithmetic/pa3} of the axiom \eqref{eq:def:peano_arithmetic/PA3}, which we will restate here again:
    \begin{equation*}
      \qforall {\synp^{\syn\iota \synimplies \syn\omicron}} \parens[\big]{ \synp(\syn0) \synwedge \qforall {\synn^{\syn\iota}} \parens[\big]{ \synp(\synn) \synimplies \synp(\synS\synn) } \synimplies \qforall {\synn^{\syn\iota}} \synp(\synn) }.
    \end{equation*}
