...
\begin{example}\label{ex:self_reference}
  ...
  \begin{thmenum}
    ...
    \thmitem{ex:self_reference/godels_theorem} As discussed in \cref{rem:thm:peano_arithmetic_incompleteness}, G\"odel proved \fullref{peano_arithmetic_incompleteness} by encoding first-order formulas and proofs as natural numbers in order to craft a self-referential formula.
...
\begin{example}\label{ex:def:hol_formula_ramification_order}
  ...
  \begin{thmenum}
    ...
    Hence, \eqref{eq:def:peano_arithmetic/theory/PA3_SOL} is indeed a second-order axiom.
...
\begin{example}\label{ex:def:bounded_hol}
  ...
  \begin{thmenum}
    ...
    Many-sorted first-order logic may be closer to higher-order logic in its formalisms than to standard one-sorted first-order logic. This leads to \cref{def:many_sorted_hol}.
