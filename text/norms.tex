...
\begin{definition}\label{def:triangle_inequality}\mimprovised
  For a binary real-valued function \( d: X^2 \to G \) from an arbitrary set \( X \) to an \hyperref[def:ordered_semigroup]{ordered} \hyperref[def:additive_semigroup]{additive (semi)groups}, we will refer to the \hyperref[def:inequality]{inequality}
...
\end{definition}
\begin{comments}
  \item Due to the relation to \hyperref[def:additive_function/sub]{subadditive functions} shown in \cref{thm:subadditivity_to_triangle_inequality}, the subadditivity inequality is often called \enquote{the triangle inequality} in relation to norms. This is discussed in \cref{rem:triangle_inequality_terminology}.
\end{comments}

\begin{remark}\label{rem:triangle_inequality_terminology}
  \enquote{The triangle inequality} is generally used to refer to \eqref{eq:def:triangle_inequality}, however, due the relation to \hyperref[def:additive_function/sub]{subadditive functions} shown in \cref{thm:subadditivity_to_triangle_inequality} and \cref{thm:triangle_inequality_to_subadditivity}, it is sometimes used to refer to \eqref{eq:def:additive_function/sub}. As can be seen from our breakdown, this refers exclusively for subadditivity of norms, where the distinction is blurred.

  \begin{itemize}
    \item Authors using the term for both norms and metrics include
    \textcite[4]{Yoshida1980FunctionalAnalysis} and \cite[30]{Yoshida1980FunctionalAnalysis},
    \textcite[3]{WheedenZygmund2015MeasureAndIntegral} and \cite[191]{WheedenZygmund2015MeasureAndIntegral},
    \textcite[4.3.3]{Tao2022AnalysisI} and
    \textcite[94]{Rotman2015AdvancedModernAlgebraPart1} and \cite[673]{Rotman2015AdvancedModernAlgebraPart1}.

    \item Authors using the term when defining metrics include
    \textcite[217]{Birkhoff1967LatticeTheory},
    \textcite[48]{Carothers2000RealAnalysis},
    \textcite[118]{Kelley1975GeneralTopology},
    \textcite[248]{Engelking1989GeneralTopology},
    \textcite[40]{Schechter1997AnalysisHandbook},
    \textcite[55]{HalmosGivant2009BooleanAlgebras},
    \textcite[436]{Натансон1974ВещественныйАнализ},
    \textcite[15]{ЛюстерникСоболев1965ФункциональныйАнализ} as \enquote{\textru{аксиома треугольника}} (triangle axiom).

    \item Authors using the term for what we call the triangle inequality for metrics in other contexts include
    \textcite[161]{Арнольд2014ОДУ},
    \textcite[53]{Боровков1999ТеорияВероятностей},
    \textcite[13]{Зорич2019АнализЧасть2},
    \textcite[\S 1.4]{Rudin1987RealAndComplexAnalysis},
    \textcite[362]{Grätzer2011LatticeTheory} and
    \textcite[\S 9.1.1.1]{Berger1987GeometryI}.

    \item Authors using the term when defining norms include
    \textcite[16]{Clarke2013OptimalControl},
    \textcite[12]{FabianEtAl2001FunctionalAnalysis} and
    \textcite[151]{Folland1999RealAnalysis}.

    \item Authors using the term for norms in other contexts include
    \textcite[7]{BerghLöfström1976InterpolationSpaces},
    \textcite[9]{Ahlfors1979ComplexAnalysis},
    \textcite[21]{Malliavin1995IntegrationAndProbability},
    \textcite[578]{Lang2002Algebra},
    \textcite[13]{Strang2023LinearAlgebra},
    \textcite[def. 1.1.1]{Хелемский2014ФункциональныйАнализ},
    \textcite[112]{Deimling1985NonlinearFA} and
    \textcite[605]{Knapp2016BasicAlgebra}.
  \end{itemize}
\end{remark}

\begin{definition}\label{def:translation_invariant_binary_function}\mimprovised
   We say that a binary function between \hyperref[def:ordered_semigroup]{ordered} \hyperref[def:additive_semigroup]{additive (semi)groups} is \term{translation-invariant} if
  \begin{equation*}
    f(x, y) = f(x + d, y + d).
  \end{equation*}
  for any \( d \).
...
\begin{proposition}\label{thm:subadditivity_to_triangle_inequality}
  Fix two \hyperref[def:additive_semigroup]{additive (semi)groups} \( G \) and \( H \), and suppose that \( H \) is \hyperref[def:ordered_semigroup]{ordered}.
...
\begin{proposition}\label{thm:triangle_inequality_to_subadditivity}
  As in \cref{thm:subadditivity_to_triangle_inequality}, fix two \hyperref[def:additive_semigroup]{additive (semi)groups} \( G \) and \( H \), and suppose that \( H \) is \hyperref[def:ordered_semigroup]{ordered}.
...
\begin{definition}\label{def:norm}
  ...
  If we remove \cref{def:norm/N1}, then \( \norm{\cdot} \) is called a \term{seminorm}.
...
\begin{proof}
  ...
  \SubProofOf{def:metric_space/M1} Follows from \cref{def:norm/N1}.

  \SubProofOf{def:metric_space/M2} By \cref{def:norm/N2},
  \begin{equation*}
    \rho(x, y) = \norm{ x - y } = \norm{ (-1) (y - x) } = \abs{-1} \norm{y - x} = \rho(y, x).
  \end{equation*}
...
\begin{proof}
  ...
  \ImplicationSubProof{def:rotund_norm/linearly_dependent}{def:rotund_norm/no_sphere_segments} Let \cref{def:rotund_norm/linearly_dependent/equation} hold and fix \( x, y \in S_E, t \in (0, 1) \). Define \( z \coloneqq tx + (1-t)y \).
  ...
  Now assume that the vectors \( tx \) and \( (1-t)y \) do not satisfy the left part of \cref{def:rotund_norm/linearly_dependent/equation}. This implies \( \norm z < 1 \). Thus, \( x \) and \( y \) are necessarily distinct, but \( z \) is not contained in the unit sphere and the segment \( [x, y] \) is not contained in \( S_E \).
...
\begin{proof}
  \begin{enumerate}
    \item First, let the dual norm \( \norm{\cdot}^* \) be rotund and assume that \( \norm{\cdot} \) is not smooth.
          ...
          Fix \( x \in S_X \). Since \( D(x) \) is nonempty (by \cref{thm:hahn_banach_implies_duality_mapping_nonempty}) and since \( \norm{\cdot} \) is not smooth, then there exist two different functionals \( x^*, y^* \in D(x) \), such that
          \begin{balign*}
            \inprod {x^*} x
            =
            \inprod {y^*} x
    ...
    \item Now let the dual norm \( \norm{\cdot}^* \) be smooth and assume that \( \norm{\cdot} \) is not rotund. Then there exist points \( x, y \in S_X \) such that the while segment \( [x, y] \) is contained in \( S_X \).
          ...
          Fix \( t \in (0, 1) \) and define \( z \coloneqq tx + (1-t)y \in S_X \). Denote by \( J: X \to X^{**} \) the canonical embedding into the double-dual. By \cref{thm:hahn_banach_implies_duality_mapping_nonempty}, there exists a functional \( z^* \in X^* \), such that
          \begin{balign*}
            \inprod {J(z)} {z^*}
            =
            \inprod{z^*} z
...
\begin{proof}
  ...
  \SubProofOf{thm:def:conjugate_exponent/holder_lemma} \Cref{thm:def:logarithm/concave} implies that the \hyperref[def:logarithm]{logarithm function} is \hyperref[def:convex_function]{concave}, thus
  \begin{equation*}
    \ln(ab)
  ...
  \Cref{thm:def:exponential_function/monotone} implies that the exponential function is monotonic, hence \eqref{eq:thm:def:conjugate_exponent/holder_lemma} follows.
\end{proof}
...
\begin{proof}
  ...
  For a fixed \( k \), we can apply \cref{thm:def:conjugate_exponent/holder_lemma} to the quotients \( \abs{ x_k } / a \) and \( \abs{ y_k } / b \):
  \begin{equation*}
...
\begin{definition}\label{def:p_norm}\mcite[\S 22.4]{Тыртышников2007ЛинейнаяАлгебра}
  ...
  \begin{figure}
    ...
    \caption{The \hyperref[def:metric_space/ball]{unit circles} of the \( 1 \)-norm (lightest), \( 2 \)-norm, \( 5 \)-norm and \( \infty \)-norm (darkest) in the \hyperref[def:euclidean_plane]{Euclidean plane}.}\label{fig:def:p_norm}
  \end{figure}
\end{definition}
\begin{comments}
  ...
  \item We generalize the definition by \textcite[\S 22.4]{Тыртышников2007ЛинейнаяАлгебра} from \( \BbbC \) and by \textcite[example 1.2.3(b)]{AtkinsonHan2001TheoreticalNumericalAnalysis} from \( \BbbR \) to a more general field \( \BbbK \) with absolute value.
\end{comments}
\begin{defproof}
