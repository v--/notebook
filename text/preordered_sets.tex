...
\begin{remark}\label{rem:order_homomorphism_terminology}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      \item \incite[3]{Kelley1975}, \incite[example 1.5.9]{Perrone2019} and \incite[92]{MacLane1998} for what we call a nonstrictly order-preserving map.
      \item \incite[\textnumero 47]{ФихтенгольцОсновыТом1} and \incite[def. 4.28]{Rudin1976Principles} for what we call either a nonincreasing or nondecreasing real-valued function.
    \end{itemize}

    \item \term{Weakly monotone} is used by \incite[175]{MacLane1998} for what we call a nondecreasing function.

    \item \term{Strictly monotone} is used \incite[def. 4.28]{Rudin1976Principles} for what we call an increasing real-valued function.

    \item \term{Isotone} is used by \incite[3]{Birkhoff1948} and \incite[3]{Kelley1975} for what we call a nondecreasing function.

    \item \term{Order-preserving} is used by
    \begin{itemize}
       \item \incite[3]{Birkhoff1948}, \incite[3]{Kelley1975} and \incite[95]{MacLane1998} for what we call a nondecreasing function.

       \item \incite[4]{Engelking1989} for what we call a strictly order-preserving function, however only for totally ordered sets.
    \end{itemize}

    \item \term{Order-reversing} is used by \incite[95]{MacLane1998} for what we call analogously.

    \item \term{Order-reflecting} is used by \incite[example 1.5.9]{Perrone2019} for what we call analogously.

    \item \term{Nondecreasing} is used by
    \begin{itemize}
      \item \incite[208]{Birkhoff1948} for what we call analogously, in the context of real-valued functions.
      \item \incite[8]{Engelking1989} for what we call analogously, however only for totally ordered sets.
      \item \incite[\textnumero 47]{ФихтенгольцОсновыТом1} for what we call analogously, however only for the real numbers.
    \end{itemize}

    Both use \term{nonincreasing} similarly.

    \item \term{Increasing} is used by \incite[\textnumero 47]{ФихтенгольцОсновыТом1} and \incite[def. 4.28]{Rudin1976Principles} for what we call analogously, however only for the real numbers (both use \term{decreasing} in the same context).
  \end{itemize}
\end{remark}
