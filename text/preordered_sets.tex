...
\begin{proposition}\label{thm:def:extremal_points}
  ...
  \begin{thmenum}
    ...
    The converse holds in \hyperref[def:totally_ordered_set]{totally ordered sets} --- see \fullref{thm:def:totally_ordered_set/maximal_iff_maximum}.
...
\begin{comments}
  ...
  \item Embeddings and isomorphisms are simpler for totally ordered sets, where \fullref{thm:def:totally_ordered_set/embedding_iff_strict} holds.
\end{comments}
...
\begin{remark}\label{rem:order_homomorphism_terminology}
  ...
  \begin{itemize}
    ...
    \item \incite[132]{ИльинСадовничийСендов1985Том1}, \incite[\textnumero 47]{ФихтенгольцОсновыТом1} and \incite[def. 17]{Зорич2019Том1} use \enquote{возрастающая функция} (resp. \enquote{убывающая функция}) for what we call a strictly order-preserving (resp. order-reversing) map. The terms can be translated as \enquote{growing} and \enquote{diminishing}, respectively. For non-strict maps, the authors suggest adding a \enquote{не} prefix, similarly to how \enquote{non} is added in English.
  \end{itemize}
\end{remark}
