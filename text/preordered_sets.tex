...
\begin{definition}\label{def:preordered_set}\mcite[13]{Harzheim2005OrderedSets}
  ...
  \begin{thmenum}[resume=def:preordered_set]
    ...
    \thmitem{def:preordered_set/opposite}\mcite[316]{PicadoPultr2012FramesAndLocales} We define the \term{dual} or \term{opposite} preordered set of \( (P, \leq) \) as \( (P, \geq) \).
...
\end{definition}
\begin{comments}
  \item Following \incite[3]{Grätzer2011LatticeTheory} and \incite[def 3.1]{Гуров2013ТеорияРешёток}, we prefer the term \enquote{preorder} to \enquote{quasi-order}. Other authors like \incite[20]{Birkhoff1967LatticeTheory} and \incite[13]{Harzheim2005OrderedSets} instead prefer \enquote{quasi-order}.
\end{comments}

\begin{theorem}[Principle of duality for preorders]\label{thm:preorder_duality}\mcite[5]{Grätzer2011LatticeTheory}
  Consider the \hyperref[def:preordered_set/theory]{first-order theory of preordered sets}. Within it, consider the opposite formula \( \varphi^\oppos \) of the \hyperref[def:closed_fol_formula]{closed formula} \( \varphi \), in which we swap all instances of \( \leq \) and \( \geq \) via \fullref{alg:fol_formula_signature_translation}.
...
\end{theorem}
\begin{comments}
  \item The corresponding syntactic statement is that the formula \( \varphi \) is \hyperref[def:general_logic]{derivable} in the \hyperref[def:preordered_set/theory]{theory of preordered sets} if and only if the opposite formula \( \varphi^\oppos \) is also derivable.
...
\begin{definition}\label{def:inequality}\mimprovised
  When regarding some \hyperref[def:preordered_set]{preorder relation} \( \leq \) as a \hyperref[def:fol_signature/predicates]{first-order predicate}, we call the corresponding \hyperref[def:first_order_syntax/atomic_formula]{atomic formulas} \term{inequalities}. More simply put, an inequality is a formula of the form \( \tau \leq \sigma \).
...
\begin{definition}\label{def:order_function}
  Fix \hyperref[def:preordered_set]{preordered sets} \( (P, \leq_P) \) and \( (Q, \leq_Q) \) and consider an arbitrary function \( f: P \to Q \). \Cref{rem:order_homomorphism_terminology} demonstrates how widely the terminology for such functions varies. We try to choose the most unambiguous terminology.
...
\begin{remark}\label{rem:order_homomorphism_terminology}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      \item \incite[30]{Grätzer2011LatticeTheory}, \incite[23]{DaveyPriestley2002LatticeTheory}, \incite[317]{PicadoPultr2012FramesAndLocales}, \incite[3]{Kelley1975GeneralTopology} and \incite[92]{MacLane1998CategoryTheory} for what we call a nonstrictly order-preserving map.

      \item \incite[132]{ИльинСадовничийСендов1985АнализТом1}, \incite[\S 47]{Фихтенгольц1968ОсновыАнализаТом1} and \incite[def. 4.28]{Rudin1976AnalysisPrinciples} for what we call either a nonincreasing or nondecreasing real-valued function.
    \end{itemize}

    \item \enquote{Weakly monotone} is used \incite[175]{MacLane1998CategoryTheory} for what we call a nondecreasing function.
    ...
    \item \enquote{Isotone} is used \incite[2]{Birkhoff1967LatticeTheory}, \incite[30]{Grätzer2011LatticeTheory}, \incite[35]{Harzheim2005OrderedSets}, \incite[23]{DaveyPriestley2002LatticeTheory}, \incite[3]{Kelley1975GeneralTopology} and \incite[def. 3.9]{Гуров2013ТеорияРешёток} for what we call a nonstrictly order-preserving function.

    \item \enquote{Reverse isotone} is used \incite[def. 3.9]{Гуров2013ТеорияРешёток} for what we call a nonstrictly order-reflecting map.

    \item \enquote{Antitone} is used by
    \begin{itemize}
      \item \incite[30]{Grätzer2011LatticeTheory} for what we call an order-reversing map.
      \item \incite[3]{Birkhoff1967LatticeTheory} for when the order-reversing condition \eqref{eq:def:order_function/reversing} is an equivalence rather than an implication
    \end{itemize}

    \item \enquote{Anti-isotone} is used \incite[35]{Harzheim2005OrderedSets} and \incite[def. 3.9]{Гуров2013ТеорияРешёток} for what we call an order-reversing map.

    \item \enquote{Order-preserving} is used by
    \begin{itemize}
       \item \incite[2]{Birkhoff1967LatticeTheory}, \incite[30]{Grätzer2011LatticeTheory}, \incite{Harzheim2005OrderedSets}, \incite[3]{Kelley1975GeneralTopology}, \incite[243]{Stanley2012EnumerativeCombinatoricsVol1} and \incite[95]{MacLane1998CategoryTheory} for what we call an order-preserving function.

       \item \incite[4]{Engelking1989GeneralTopology} for what we call a strictly order-preserving function, however only for totally ordered sets.
    \end{itemize}

    \item \enquote{Order-reversing} is used by \incite[35]{Harzheim2005OrderedSets} and \incite[95]{MacLane1998CategoryTheory} for what we call analogously.
    ...
    \begin{itemize}
      ...
      \item \incite[8]{Engelking1989GeneralTopology} for what we call analogously, however only for totally ordered sets.
    ...
    \item \incite[132]{ИльинСадовничийСендов1985АнализТом1}, \incite[\S 47]{Фихтенгольц1968ОсновыАнализаТом1} and \incite[def. 17]{Зорич2019АнализЧасть1} use \enquote{возрастающая функция} (resp. \enquote{убывающая функция}) for what we call a strictly order-preserving (resp. order-reversing) map. The terms can be translated as \enquote{growing} and \enquote{diminishing}, respectively. For non-strict maps, the authors suggest adding a \enquote{не} prefix, similarly to how \enquote{non} is added in English.
  \end{itemize}
\end{remark}
