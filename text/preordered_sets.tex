...
\begin{definition}\label{def:preordered_set}\mcite[13]{Harzheim2005OrderedSets}
  ...
  \begin{thmenum}[resume=def:preordered_set]
    ...
    The theory of preordered sets is a \hyperref[def:fol_theory]{first-order theory} in \( \mscrL \) consisting of the axioms \eqref{eq:def:binary_relation/reflexive} and \eqref{eq:def:binary_relation/transitive} for \( \leq \) and the compatibility axiom
    ...
    \thmitem{def:preordered_set/category}  We denote the \hyperref[def:fol_theory_model_functor/obj]{category of \( \mscrU \)-small models} for the theory of preordered sets by \( \ucat{PreOrd} \).
...
\begin{comments}
  ...
  \cite[10]{ЦаленкоШульгейфер1974ОсновыТеорииКатегорий} (as \enquote{\textru{предпорядок}}),
  \cite[def. 3.1]{Гуров2013ТеорияРешёток} (as \enquote{\textru{предпорядок}}) and
  \cite[66]{БелоусовТкачёв2004ДискретнаяМатематика} (as \enquote{\textru{предпорядок}}). Another possibility is \enquote{quasi-order}, used in
...
\begin{concept}\label{con:duality}
  A quintessential notion in mathematics is that of \term[en=duality (\cite[188]{GowersEtAl2008PrincetonCompanion})]{duality}. \Textcite[5]{ConwayGuy1998BookOfNumbers} describe it as follows:
  \begin{displayquote}
...
\begin{definition}\label{def:inequality}\mimprovised
  ...
  As in the case of \hyperref[def:fol_equation]{equations}, we call the set \hyperref[def:fol_definability]{defined} by an inequality a \term{solution set}.
\end{definition}
...
\begin{proposition}\label{thm:def:extremal_points}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:def:extremal_points/finite_set_has_maximal_element} If \( A \) is \emph{finite}, it has a maximal element and a minimal element.
...
\begin{example}\label{ex:thm:def:extremal_points}
  ...
  \begin{thmenum}
    ...
    \thmitem{ex:thm:def:extremal_points/two_connected} Consider again the two-element set \( \set{ a, b } \), but this time with \( a \leq b \). All such preordered sets are necessarily isomorphic.
...
\begin{proposition}\label{thm:def:order_function}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:def:preordered_set/homomorphism_is_reflecting} An injective function between preordered sets is a \hyperref[def:fol_embedding]{first-order embedding} if and only if it is both \hyperref[def:order_function/preserving]{order-preserving} and \hyperref[def:order_function/preserving]{order-reflecting}.
  \end{thmenum}
\end{proposition}
\begin{defproof}
