...
\begin{definition}\label{def:extremal_points}
  ...
  \begin{thmenum}
    \thmitem{def:extremal_points/upper_and_lower_bounds}\mcite[21]{Harzheim2005}
    \begin{TwoColumns}
    ...
    \thmitem{def:extremal_points/maximum_and_minimum}\mcite[22]{Harzheim2005}
    \begin{TwoColumns}
      If \( m \) is an upper bound of \( A \) that belongs to \( A \), we call it a \term{maximum} or \term[ru=наибольший (елемент) (\cite[def. 3.6]{Гуров2013})]{greatest element}.

      If antisymmetry holds, maxima are unique, and we use the notation \( \max A \).
    \BeginSecondColumn
      Dually, if \( m \) is a lower bound of \( A \) that belongs to \( A \), we call it a \term{minimum} or \term[ru=наименьший (елемент) (\cite[def. 3.6]{Гуров2013})]{least element}.

      If antisymmetry holds, maxima are unique, and we use the notation \( \min A \).
    \end{TwoColumns}

    \thmitem{def:extremal_points/supremum_and_infimum}\mcite[22]{Harzheim2005}
    \begin{TwoColumns}
      If \( m \) is a minimum among all upper bounds of \( A \), we call it a \term[bg=точна горна граница (\cite[19]{Тагамлицки1971Диф}), ru=точная верхняя граница (\cite[\textnumero 5.12]{Ляпин1960})]{least upper bound} or \term[bg=супремум (\cite[10]{Проданов1982}), ru=супремум (\cite[\textnumero 5.12]{Ляпин1960})]{supremum} of \( A \).

      If antisymmetry holds, suprema are unique, and we use the notation \( \sup A \).
    \BeginSecondColumn
      Dually, if \( m \) is a maximum among all lower bounds of \( A \), we call it a \term[bg=точна долна граница (\cite[19]{Тагамлицки1971Диф}), ru=точная нижняя граница (\cite[\textnumero 5.12]{Ляпин1960})]{greatest lower bound} or \term[bg=инфимум (\cite[10]{Проданов1982}), ru=супремум (\cite[\textnumero 5.12]{Ляпин1960})]{infimum} of \( A \).
...
\begin{proposition}\label{thm:def:extremal_points}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:def:extremal_points/maximum_is_maximal} Every maximum of \( A \) is also a maximal element of \( A \).

    Dually, every minimum of \( A \) is also a minimal element of \( A \).

    \thmitem{thm:def:extremal_points/maximum_is_supremum} Every maximum of \( A \) is also a supremum of \( A \).

    Dually, every minimum of \( A \) is also an infimum of \( A \).
    ...
    \thmitem{thm:def:extremal_points/unique_maximal_element} If \( m \) is a unique maximal element of \( A \) and if every nonempty subset of \( A \) has at least one maximal element, then \( m \) is a maximum of \( A \).
...
\begin{proof}
  ...
  \SubProofOf{thm:def:extremal_points/maximum_is_maximal} Let \( m \) be a maximum of \( A \).
  ...
  \SubProofOf{thm:def:extremal_points/maximum_is_supremum} Let \( m \) be a maximum of \( A \).
...
\begin{example}\label{ex:thm:def:extremal_points}
  ...
  \begin{thmenum}
    ...
    Since \( \varnothing \) is empty, it cannot have a maximum, nor a maximal element.
    ...
    Then \( a \) is the only upper bound of \( \set{ a } \), hence also a maximal element, maximum and supremum. It is similarly a lower bound, minimal element, minimum and infimum.
    ...
    \thmitem{ex:thm:def:extremal_points/unique_maximal_element_not_maximum}\mcite{MathSE:unique_maximal_element_that_is_not_maximum} Consider the set \( \BbbZ \) of integers with the standard ordering from \fullref{def:integer_ordering}.

    Adjoin some symbol \( u \) and define a relation to \( \BbbZ \cup \set{ u } \) extending the ordering on \( \BbbZ \) with \( u \leq u \).

    Then \( u \) is incomparable with any integer. It is the unique maximal element of \( \BbbZ \cup \set{ u } \) because no integer is greater than \( u \). But, since it is incomparable with integers, \( u \) is not a maximum.
...
\begin{definition}\label{def:order_function}
  ...
  \begin{thmenum}
    ...
    In the case of a \hyperref[def:totally_ordered_set]{totally ordered set}, we usually prefer the term (strictly) \term[bg=растяща (функция) (\cite[198]{Тагамлицки1971Диф}), ru=возрастающая (функция) (\cite[132]{ИльинСадовничийСендов1985Том1})]{increasing}.
    ...
    In the case of a \hyperref[def:totally_ordered_set]{totally ordered set}, we prefer the term (strictly) \term[bg=намаляваща (функция) (\cite[198]{Тагамлицки1971Диф}), ru=убывающая (функция) (\cite[132]{ИльинСадовничийСендов1985Том1})]{decreasing}.

    \thmitem{def:order_function/monotone}\mcite[\textnumero 47]{ФихтенгольцОсновыТом1} In the context of real-valued functions, we use the \term[bg=монотонна (функция) (\cite[199]{Тагамлицки1971Диф}), ru=монотонная (функция) (\cite[\textnumero 47]{ФихтенгольцОсновыТом1})]{monotone} as a collective term for nondecreasing and nonincreasing functions. Similarly, we use \term{strictly monotone} for increasing and decreasing functions, in which context we refer to usual monotone functions as \term{nonstrictly monotone}.

    Outside of analysis, we generally avoid the term \enquote{monotone} due to ambiguity.

    \thmitem{def:order_function/reflecting}\mcite[35]{Harzheim2005} We call \( f \) \term{order-reflecting} if it satisfies \hyperref[def:material_implication/converse]{converse} of \eqref{eq:def:order_function/preserving}:
    \begin{equation}\label{eq:def:order_function/reflecting}
      f(x) \leq_Q f(y) \T{implies} x \leq_Q y.
    \end{equation}

    \thmitem{def:order_function/embedding}\mcite[23]{DaveyPriestley2002} If \( f \) is an \hyperref[def:function_invertibility/injective]{injective function} that is both order-preserving and order-reflecting, we say that it is an \term[ru=порядковое вложение (\cite[def. 3.9]{Гуров2013})]{order embedding}.

    \thmitem{def:order_function/isomorphism}\mcite[23]{DaveyPriestley2002} If \( f \) is a \hyperref[def:function_invertibility/bijective]{bijective} embedding, we say that it is an \term[ru=порядковый изоморфизм (\cite[def. 3.9]{Гуров2013})]{order isomorphism}.
  \end{thmenum}
\end{definition}
\begin{comments}
...
\begin{remark}\label{rem:order_homomorphism_terminology}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      \item \incite[30]{Gratzer2011} \incite[23]{DaveyPriestley2002}, \incite[3]{Kelley1975} and \incite[92]{MacLane1998} for what we call a nonstrictly order-preserving map.
      \item \incite[132]{ИльинСадовничийСендов1985Том1}, \incite[\textnumero 47]{ФихтенгольцОсновыТом1} and \incite[def. 4.28]{Rudin1976Principles} for what we call either a nonincreasing or nondecreasing real-valued function.
    \end{itemize}
    ...
    \begin{itemize}
      ...
      \item \incite[132]{ИльинСадовничийСендов1985Том1}, \incite[\textnumero 47]{ФихтенгольцОсновыТом1} and \incite[def. 17]{Зорич2019Том1} for what we call analogously, however only for the real numbers.
    \end{itemize}

    The term \enquote{nonincreasing} is used similarly.

    \item \enquote{Increasing} and \enquote{decreasing} are used by \incite[132]{ИльинСадовничийСендов1985Том1}, \incite[\textnumero 47]{ФихтенгольцОсновыТом1}, \incite[def. 17]{Зорич2019Том1}, \incite[198]{Тагамлицки1971Диф} and \incite[def. 4.28]{Rudin1976Principles} for what we call analogously, however only for the real numbers.
  \end{itemize}
\end{remark}
...
\begin{proposition}\label{thm:def:order_function}
  ...
  \begin{thmenum}
    ...
    The converse holds for totally ordered sets --- see \fullref{thm:total_order_embedding_iff_strict}.
  \end{thmenum}
\end{proposition}
\begin{defproof}
  \SubProofOf{thm:def:order_function/injective_implies_strict} Let \( f: P \to Q \) be an injective homomorphism.

  Let \( x <_P y \) for some members \( x \) and \( y \) of \( P \). Since \( f \) is an order homomorphism, we have \( f(x) \leq_Q f(y) \). Since it is also injective, \( f(x) = f(y) \) implies \( x = y \), hence it remains for \( f(x) <_Q f(y) \) to hold.
