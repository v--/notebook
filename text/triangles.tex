...
\begin{definition}\label{def:triangle}
  ...
  Given a triangle with vertices \( A \), \( B \) and \( C \), we usually consider its \hyperref[def:angle]{\emph{undirected} angles}
  \begin{align*}
...
\begin{proof}
  ...
  If \( P \notin \set{ P_a, P_b, P_c } \), then \( T \) entirely contains the ball with center \( P \) and radius
  \begin{equation*}
    \varepsilon < \min\set{ \norm{\vect{P P_a}}, \norm{\vect{P P_b}}, \norm{\vect{P P_c}} }.
  \end{equation*}
...
\begin{theorem}[Triangle inequality]\label{thm:triangle_inequality}
  \todo{}
\end{theorem}
