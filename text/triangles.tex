...
\begin{proof}
  ...
  \Fullref{thm:angles_of_transversal_parallel_lines} implies that the angles \( \angle(\vect{AB}, \vect{AC}) \) and \( \angle(\vect{CA'}, \vect{CA}) \) both have measure \( \alpha \). Similarly, \( \angle(\vect{BA}, \vect{BC}) \) and \( \angle(\vect{CB'}, \vect{CB}) \) both have measure \( \beta \).

  From \fullref{thm:sum_of_angles_measure} it follows that
  \begin{equation*}
    \pi
    =
...
\end{corollary}
\begin{proof}
  There can be only one right angle because, by \fullref{thm:sum_of_triangle_angles}, the other two angles must sum to \( \pi / 2 \), and one of the angles being zero would violate affine independence of the vertices.
\end{proof}
...
\end{proposition}
\begin{proof}
  \todo{Use \fullref{thm:triangle_rotation_around_midpoint}}.
  ...
  Furthermore, \Fullref{thm:angles_of_transversal_parallel_lines} implies that
  \begin{equation*}
...
\begin{proof}
  ...
  The triangles \( A M_C C \) and \( B M_C C \) have sides of equal length, hence, by \fullref{thm:triangle_congruence/three_sides}, they are congruent. Then the angles \( \measuredangle(\vect{M_C A}, \vect{M_C C}) \) and \( \measuredangle(\vect{M_C B}, \vect{M_C C}) \) have equal measure, and since they \hyperref[def:adjacent_angles]{adjacent}, from \fullref{thm:sum_of_angles_measure} it follows that both are \( \pi / 2 \).
...
\begin{proof}
  ...
  \Fullref{thm:isosceles_median_and_altitude} implies that \( CM_C \) is a height.

  Then the triangles \( A M_C C \) and \( B M_C C \) have a common side, \( C M_C \), two equal sides, \( A M_C \) and \( B M_C \), and equal angles between them. \Fullref{thm:triangle_congruence/two_sides_and_angle} implies that the triangles are congruent, thus the sides \( A M \) and \( B M \) have equal lengths.
\end{proof}
...
\begin{proof}
  ...
  \Fullref{thm:angles_of_transversal_parallel_lines} then implies that the angles of the corners are right. Hence, this is indeed a rectangle.

  The Lebesgue measure is invariant under rotations and translations as a consequence of \fullref{thm:lebesgue_measure_invariant_under_rigid_motions}. Hence, we can translate our rectangle by \( -A \) and rotate by \( -\angle(\vect{AB}, (1, 0)) \) to obtain the rectangle as the Cartesian product of intervals
  \begin{equation*}
  ...
  \SubProof{Proof when \( \alpha \) is obtuse} Consider the angle \( \angle(\vect{A O_C}, \vect{A C}) \). \Fullref{thm:sum_of_angles_measure} implies that its measure is \( \pi - \alpha \). Then
  \begin{equation*}
    \sin \alpha
...
\end{theorem}
\begin{comments}
  \item We only state this theorem due to its cultural significance, as it gave name to related concepts like Pythagorean triangles and triples discussed in \fullref{def:pythagorean_triangle} and \fullref{def:pythagorean_triple}, the Pythagorean identity \eqref{eq:thm:trigonometric_identities/pythagorean_identity} and \fullref{thm:generalized_pythagorean_theorem}.
\end{comments}
\begin{proof}
  Special case of \fullref{thm:law_of_cosines_for_triangles}.
\end{proof}
...
\begin{lemma}\label{thm:sierpinski_triangle_lemma}
  ...
  \begin{figure}[!ht]
    ...
    \caption{The tiling of a triangle from \fullref{thm:sierpinski_triangle_lemma}}\label{fig:thm:sierpinski_triangle_lemma}
  \end{figure}
...
\begin{proof}
  ...
  \SubProofOf{thm:sierpinski_triangle_lemma/translation} \Fullref{thm:sierpinski_triangle_lemma/vectors} implies that the line \( M_A M_B \) is parallel to \( A B \).

  Hence, we can apply \fullref{thm:angles_of_transversal_parallel_lines} to the transversal \( A M_B \) to conclude that the \hyperref[def:angles_of_transversal/corresponding]{corresponding angles} \( \angle(\vect{AB}, \vect{AC}) \) and \( \angle(\vect{M_B M_A}, \vect{M_B C}) \) have equal measures.
  ...
  \SubProofOf{thm:sierpinski_triangle_lemma/congruence} Follows from \fullref{thm:sierpinski_triangle_lemma/scaling} and \fullref{thm:sierpinski_triangle_lemma/translation}.
\end{proof}
...
\begin{remark}\label{rem:sierpinski_triangle_resemblance}
  ...
  \begin{thmenum}
    ...
    \thmitem{rem:sierpinski_triangle_resemblance/rule18} The \hyperref[def:elementary_cellular_automaton]{elementary cell automaton} with rule \( 18 \), when visualized as per \fullref{rem:elementary_cellular_automaton_visualization}, has visual resemblance as can be seen in \fullref{fig:ex:def:elementary_cellular_automaton/18}.
  \end{thmenum}
\end{remark}
