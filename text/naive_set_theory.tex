...
\begin{remark}\label{rem:epsilon_and_set_membership}
  The symbol \( \in \) is derived from \( \varepsilon \). Some older books like \cite{Halmos1976Measures} and \cite{Kelley1975Topology} even use \( \varepsilon \) for set membership. \Fullref{thm:epsilon_induction} is named after set membership.
\end{remark}
...
\begin{remark}\label{rem:self_reference}
  \hyperref[con:impredicativity]{Impredicativity} was a driving philosophical concern of Bertrand Russell, who was concerned that self-reference leads to contradictions such as \fullref{thm:russels_paradox}. For this reason Russell introduced a theory of \enquote{types} with which he grades objects of discourse. The following is a quote from \cite[237]{Russell1908TypeTheory}:
  \begin{displayquote}
    A type is defined as the range of significance of a propositional function, i.e., as the collection of arguments for which the said function has values. Whenever an apparent variable occurs in a proposition, the range of values of the apparent variable is a type, the type being fixed by the function of which \enquote{all values} are concerned. The division of objects into types is necessitated by the reflexive fallacies which otherwise arise. These fallacies, as we saw, are to be avoided by what may be called the \enquote{vicious-circle principle;} i.e., \enquote{no
    totality can contain members defined in terms of itself.}
  \end{displayquote}

  This lead to concepts such as first-order propositions, second-order propositions, etc, where a proposition of order \( n \) could quantify over propositions of order \( n - 1 \).

  These ideas later lead to the developments discussed in \fullref{sec:higher_order_logic}.
\end{remark}
...
\begin{definition}\label{def:singleton_set}\mcite[10]{Halmos1960Sets}
  A \term{singleton set} is a set having a unique element.
\end{definition}
\begin{comments}
...
\begin{definition}\label{def:unordered_pair}\mcite[9]{Halmos1960Sets}
...
\begin{remark}\label{rem:unordered_tuples}
  ...
  \begin{equation*}
    \qforall \syn s (\synx \in \syn s \syniff \synx \syneq \syna.
  \end{equation*}
...
\begin{definition}\label{def:basic_set_operations}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:basic_set_operations/difference}\mcite[27]{Enderton1977Sets} We define the \term{difference} of \( A \) and \( B \) as
    \begin{equation*}
    ...
    \thmitem{def:basic_set_operations/symmetric_difference}\mcite[9]{Jech2006Sets} We define the \term{symmetric difference} of two sets \( A \) and \( B \) as
    \begin{equation}
