...
\begin{remark}\label{rem:set_theory_formula_variables}
  In addition to using the symbols \( \synx \), \( \syny \), \( \synz \) for variables in logical formulas, as discussed in \cref{rem:mathematical_logic_conventions/greek_alphabet}, out of necessity we additionally use \( \sigma \) (from \enquote{\hi{s}et}), \( \tau \), \( \rho \) (from \enquote{\hi{r}esult}; in formulas like \( \ref{eq:def:basic_set_operations/union/predicate} \)).
\end{remark}

\begin{definition}\label{def:set_theory_signature}
\end{definition}

\begin{definition}\label{def:naive_set_theory}\mcite[subsec. 6.1]{Hinman2005Logic}
  The \term{language of na\"ive set theory} is a \hyperref[def:first_order_language]{first-order language} \( \mscrL \) with only a single \hyperref[def:fol_signature/notation]{infix} binary relation \( \in \) called \term{set membership}. If \( \Bracks{\synx \in \syny}_v = T \), we say that (under the \hyperref[def:fol_variable_assignment]{variable assignment} \( v \)) \( \synx \) is a \term{member} or \term{element} of \( \syny \) and that \( \syny \) \term{contains}\fnote{The term \enquote{contains} is used for both the membership relation and the subset relation. This tends not to cause confusion in practice.} \( \synx \).
  ...
  \term{Na\"ive set theory} is a \hyperref[def:fol_theory]{first-order theory} axiomatized by the following:
  \begin{thmenum}
    ...
    As a consequence, a set is only distinguished by what it contains and thus the ordering and repetition of members of a set play no role. This axiom is also important in \logic{ZFC} --- see \cref{def:zfc/extensionality}.

    It is very common when dealing with sets, as in \eqref{eq:def:naive_set_theory/extensionality}, to use \hyperref[rem:fol_formula_conventions/relativization]{relativization of quantifiers} with \( \in \).

    As explained in \cref{rem:mathematical_logic_conventions/quantification}, we avoid excessive universal quantification. We actually add as an axiom of the theory the \hyperref[def:fol_quantifier_closure]{universal closure} of \eqref{eq:def:naive_set_theory/extensionality}:
...
\begin{remark}\label{rem:epsilon_and_set_membership}
  The symbol \( \in \) is derived from \( \varepsilon \). Some older books like \cite{Halmos1976MeasureTheory} and \cite{Kelley1975GeneralTopology} even use \( \varepsilon \) for set membership. \Fullref{thm:epsilon_induction} is named after set membership.
\end{remark}
...
\begin{definition}\label{def:set}\mimprovised
  ...
  We call any member of \( V \) a \term{set}, even though in a non-standard model it may not be a set within the metatheory (see \cref{rem:standard_model_of_set_theory}).
...
\begin{definition}\label{def:set_builder_notation}\mimprovised
  As mentioned in \cref{rem:set_definition_recursion}, set theory somewhat blurs the line between logic and metalogic. In particular, some \hyperref[def:fol_definability]{definable} subsets of the universe \( U \) of the fixed model \( \mscrU \) are themselves sets within the object logic.
  ...
  Within the metatheory, fix an \( n \)-tuple \( u_1, \ldots, u_n \) of members of the universe \( U \), which we will call \term{parameters}. Denote by \( A \) the set of members \( x \) of \( U \) such that \( \Bracks{\varphi}(x, u_1, \ldots, u_n) = T \)\fnote{We have used the notation for formula valuation from \fullref{alg:propositional_denotation_function}}.
  ...
  Examples such as \fullref{thm:russels_paradox} show that unrestricted comprehension can easily lead to an inconsistent object logic. In more elaborate set theories like \hyperref[def:zfc]{\logic{ZFC}}, we only allow restricted comprehension via the \hyperref[def:zfc/specification]{axiom schema of specification}. Instead of defining \( A \) as a set of all members of \( U \) satisfying a certain property, restricted comprehension allows us to define \( A \) as a subset not of the universe \( U \), but of some well-behaved subset \( B \) of \( U \). The corresponding notation is based on \fullref{alg:propositional_denotation_function}:
  \begin{equation*}
  ...
  Within \logic{ZFC}, subsets of \( U \) which are not sets in the object logic are called \term{proper classes}. Sets and proper classes are collectively called \term{classes}. We avoid referencing proper classes because that can easily lead us to an inconsistent theory. See \cref{def:large_and_small_sets} for a clever workaround.
...
\begin{theorem}[Russell's paradox]\label{thm:russels_paradox}
  \hyperref[def:naive_set_theory]{Na\"ive set theory} is \hyperref[def:fol_theory/consistent]{inconsistent}. More precisely, the instance of the \hyperref[def:naive_set_theory/unrestricted_comprehension]{schema of unrestricted comprehension} with
...
\begin{proof}
  ...
  Thus, \( \psi \) and \( \synneg\psi \) are both derivable in the same theory, and we can use \ref{inf:def:propositional_natural_deduction/neg/elim} to also derive \( \synbot \), which shows that na\"ive set theory is inconsistent.
\end{proof}

\begin{concept}\label{con:universal_set}
\end{concept}

\paragraph{Subsets}

\begin{definition}\label{def:subset}\mcite[3]{Enderton1977SetTheory}
  ...
  The relation \( \subseteq \) is called the \term{inclusion relation} or \term{subset relation}, and it gives a partial ordering between sets as shown in \cref{thm:boolean_algebra_of_subsets}. If an entire family of sets are not pairwise comparable, we say that they are \term{disjoint}.
...
\begin{definition}\label{def:subset_ordering}
  \todo{Subset ordering}.
\end{definition}
...
\begin{definition}\label{def:subsingleton_set}\mcite[112]{UnivalentFoundationsProgram2013HoTT}
  If all elements of a set are equal, we call it a \term{subsingleton set}. If a subsingleton is nonempty, we call it a \term[ru=синглет (\cite[131]{КановейЛюбецкий2010СовременнаяТеорияМножеств}); синглетон (\cite[129]{КолмогоровДрагалин2006Логика}), en=singleton (\cite[10]{Halmos1960NaiveSetTheory})]{singleton}.
\end{definition}
\begin{comments}
...
\begin{definition}\label{def:unordered_pair}\mcite[9]{Halmos1960NaiveSetTheory}
  An \term[ru=неупорядоченная пара (\cite[181]{Герасимов2014Вычислимость})]{unordered pair} consisting of the sets \( a \) and \( b \) is the set \( \braket{ a, b } \).
...
\end{definition}
\begin{comments}
  \item \hyperref[def:zfc]{\logic{ZFC}} has a dedicated axiom for the existence of unordered pairs --- \hyperref[def:zfc/pairing]{axiom of pairing}.
\end{comments}

\begin{remark}\label{rem:unordered_tuples}
...
\begin{remark}\label{rem:family_of_sets}
  ...
  In order to reduce repetitiveness, sets whose elements we consider to be other sets, are will called \term{families of sets}. In particular, if all (different) sets are \hyperref[def:subset_ordering]{disjoint}, we say that the family is a \term{disjoint family}. It is usually assumed that the sets are nonempty.
...
\begin{definition}\label{def:basic_set_operations}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:basic_set_operations/power_set}\mcite[4]{Enderton1977SetTheory} The \term{power set} \( \pow(A) \) of \( A \) is the family of all subsets of \( A \). Symbolically,
    \begin{equation*}
      \pow(A) \coloneqq \set{ B \given B \subseteq A }.
    \end{equation*}

    The operation \( \pow \) is not technically a function since its domain is supposed to be the set of all sets, whose existence contradicts \fullref{thm:russels_paradox}. Nevertheless, this notation makes sense and is justified by \cref{rem:unbounded_transfinite_recursion} and \cref{ex:unary_functors_in_set}.
...
\begin{proof}
  ...
  \SubProofOf{eq:def:peano_arithmetic/theory/PA3} It follows from \fullref{thm:omega_induction} that \eqref{eq:thm:omega_induction} is a theorem of \logic{ZF}. Let \( \mscrV = (V, I) \) be our ambient \hyperref[rem:standard_model_of_set_theory]{standard} \hyperref[rem:transitive_model_of_set_theory]{transitive} model of \logic{ZFC}.
  ...
  To show that \eqref{eq:thm:omega_induction} really corresponds to \eqref{eq:def:peano_arithmetic/theory/PA3} (and hence that \( \omega \) satisfies \eqref{eq:def:peano_arithmetic/theory/PA3}), fix some formula \( \varphi \) of \hyperref[def:peano_arithmetic]{Peano arithmetic} (\hi{not \logic{ZFC}!}) and suppose that \( \synx, \synz_1, \ldots, \synz_n \) are all of its free variables. Fix also some parameter values \( u_1, \ldots, u_n \in \omega \) and, as in \cref{def:set_builder_notation}, define the set
  \begin{equation*}
  ...
  Since our choice of parameters \( u_1, \ldots, u_n \) was arbitrary, we can conclude that the universal closure \eqref{eq:def:peano_arithmetic/theory/PA3_quantified} of \eqref{eq:def:peano_arithmetic/theory/PA3} is satisfied by \( \omega \) for every formula \( \varphi \) of \logic{PA}.
\end{proof}
...
\begin{remark}\label{rem:set_theory_natural_numbers_without_operations}
  ...
  Since the ordering in \cref{def:natural_numbers_ordering} is defined via addition, we must define some other ordering. Luckily, as we shall see in \fullref{sec:ordinals}, \( n < m \) corresponds to \( n \in m \). In particular, the members of \( m \) are ordered.
