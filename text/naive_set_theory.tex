...
\begin{remark}\label{rem:pure_set_theory}\mcite[8]{Enderton1977SetTheory}
  What we lose in this formalization are objects which are not sets, usually called \term{atoms} or \term{urelements} (because of the German prefix \enquote{ur}, meaning primordial). It is not necessary for us to add such elements since we can encode everything via sets. Theories without atoms, like our versions of nai\"ve set theory and \hyperref[def:zfcu]{\logic{ZFC+U}}, are called \term{pure set theories}.
\end{remark}

\begin{remark}\label{rem:set_theory_formula_variables}
  In addition to using the symbols \( \synx \), \( \syny \), \( \synz \) for variables in logical formulas, as discussed in \cref{rem:mathematical_logic_conventions/greek_alphabet}, out of necessity we additionally use \( \sigma \) (from \enquote{\emph{s}et}), \( \tau \), \( \rho \) (from \enquote{\emph{r}esult}; in formulas like \( \ref{eq:def:basic_set_operations/union/predicate} \)).
\end{remark}

\begin{remark}\label{rem:set_theory_proper_classes}
\end{remark}
...
\begin{remark}\label{rem:standard_model_of_set_theory}
  ...
  \begin{itemize}
    ...
    \thmitem{rem:standard_model_of_set_theory/skolems_paradox} It is possible that \hyperref[thm:cardinality_existence]{cardinality} is incompatible between the object theory and metatheory --- see \cref{thm:skolems_paradox}. We want to avoid sets that are countable in the metatheory yet uncountable in the object theory, for example.
  \end{itemize}
...
\begin{concept}\label{con:proper_class}
\end{concept}
...
\begin{definition}\label{def:set_builder_notation}\mimprovised
  ...
  Within \logic{ZFC}, subsets of \( U \) which are not sets in the object logic are called \term{proper classes}. Sets and proper classes are collectively called \term{classes}. We avoid referencing proper classes because that can easily lead us to an inconsistent theory. See \cref{def:small_set} for a clever workaround.
...
\begin{proof}
  ...
  We will show that the negation \( \synneg\psi \) of \( \psi \) is also derivable in this theory. An explicit form of the negation can be obtained by utilizing the equivalences from \cref{thm:fol_quantifier_duality} and \cref{thm:classical_equivalences/biconditional_negation}:
  \begin{equation*}
    \synneg\psi = \qforall \sigma \qexists \synx (\synx \in \sigma \syniff \synx \in \synx).
  \end{equation*}

  This holds when \( \synx \) and \( \sigma \) take on the same value, hence it is satisfiable in na\"ive set theory. \Fullref{thm:fol_completeness} implies that \( \synneg\psi \) is derivable in the theory.
...
\begin{definition}\label{def:singleton_set}\mcite[10]{Halmos1960NaiveSetTheory}
  We call a \hyperref[def:empty_set]{nonempty} \hyperref[def:singleton_set]{subsingleton set} a \term[ru=синглет (\cite[131]{КановейЛюбецкий2010СовременнаяТеорияМножеств}); синглетон (\cite[129]{КолмогоровДрагалин2006Логика})]{singleton}.
\end{definition}
\begin{comments}
...
\begin{definition}\label{def:basic_set_operations}
  ...
  \begin{thmenum}
    ...
    The operation \( \pow \) is not technically a function since its domain is supposed to be the set of all sets, whose existence contradicts \fullref{thm:russels_paradox}. Nevertheless, this notation makes sense and is justified by \cref{rem:unbounded_transfinite_recursion} and \cref{ex:def:functor}.
...
\begin{proof}
  ...
  To show that \eqref{eq:thm:omega_induction} really corresponds to \eqref{eq:def:peano_arithmetic/theory/PA3} (and hence that \( \omega \) satisfies \eqref{eq:def:peano_arithmetic/theory/PA3}), fix some formula \( \varphi \) of \hyperref[def:peano_arithmetic]{Peano arithmetic} (\emph{not \logic{ZFC}!}) and suppose that \( \synx, \synz_1, \ldots, \synz_n \) are all of its free variables. Fix also some parameter values \( u_1, \ldots, u_n \in \omega \) and, as in \cref{def:set_builder_notation}, define the set
  \begin{equation*}
