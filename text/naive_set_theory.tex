...
\begin{definition}\label{def:set_builder_notation}
  ...
  Also fix an \( n \)-tuple \( u_1, \ldots, u_n \) of members of \( U \), which we will call \term{parameters}. Denote by \( A \) the subset of \( U \) consisting of members \( x \) of \( U \) such that \( \Bracks{\varphi}(x, u_1, \ldots, u_n) = T \).
...
\begin{definition}\label{def:transitive_set}\mcite[subsec. 6.1.3(i)]{Hinman2005}
...
\begin{proof}
  ...
  Fix any variable assignment \( v: \boldop{Var} \to V \). As in our proof of \fullref{thm:omega_induction}, we consider the modified assignment \( v_{\sigma \mapsto \omega} \) that \enquote{eliminates} the outer existential quantifier in \eqref{eq:thm:omega_induction}.
