...
\begin{definition}\label{def:naive_set_theory}\mcite[subsec. 6.1]{Hinman2005}
  ...
  \begin{thmenum}
    ...
    \begin{equation}\label{eq:def:naive_set_theory/extensionality_quantified}
      \qforall \xi \qforall \eta \parens[\Bigg]{ \parens[\Big]{ \qforall \zeta (\zeta \in \xi \leftrightarrow \zeta \in \eta) } \rightarrow \parens[\Big]{ \xi \doteq \eta } }.
    \end{equation}

    The \hyperref[def:material_implication/converse]{converse} of \eqref{eq:def:naive_set_theory/extensionality} obvious.

    \thmitem{def:naive_set_theory/unrestricted_comprehension} The \term{axiom schema of unrestricted comprehension} states that any formula defines a set. For each formula \( \varphi \) not containing \( \sigma \) as a free variable, the following is an axiom:
    \begin{equation}\label{eq:def:naive_set_theory/unrestricted_comprehension}
      \qexists \sigma \qforall \xi (\xi \in \sigma \leftrightarrow \varphi).
    \end{equation}

    It is important to highlight that \( \varphi \) may have any number of free variables as long as \( \sigma \) is not among them. Of course, this axiom is only interesting if \( \xi \in \boldop{Free}(\varphi) \). If \( \eta_1, \ldots, \eta_n \) are all the other free variables of \( \varphi \), then the \hyperref[def:universal_closure]{universal closure} of the corresponding axiom is
    \begin{equation}\label{eq:def:naive_set_theory/unrestricted_comprehension_quantified}
      \qforall {\eta_1} \cdots \qforall {\eta_n} \qexists \sigma \qforall \xi (\xi \in \sigma \leftrightarrow \varphi).
    \end{equation}

    In other words, the set \( \sigma \) is not unique in general, but depends on the free variables \( \eta_1, \ldots, \eta_n \). For this reason, they are called \term{parameters} of the axiom.
...
\begin{definition}\label{def:empty_set}
  ...
  \begin{equation*}\taglabel[\op{IsEmpty}]{eq:def:empty_set/predicate}
    \ref{eq:def:empty_set/predicate}[\sigma] \coloneqq \qforall \eta \neg \eta \in \sigma.
  \end{equation*}
...
\begin{proof}
  ...
  \begin{equation}\label{eq:thm:russels_paradox_comprehension_axiom}
    \psi \coloneqq \qexists \sigma \qforall \xi (\xi \in \sigma \leftrightarrow \neg (\xi \in \xi)).
  \end{equation}

  We will show that the negation \( \neg\psi \) of \( \psi \) is also derivable in this theory. An explicit form of the negation can be obtained by utilizing the equivalences from \fullref{thm:first_order_quantifiers_are_dual} and \fullref{thm:boolean_equivalences/biconditional_negation}:
  \begin{equation*}
    \neg\psi = \qforall \sigma \qexists \xi (\xi \in \sigma \leftrightarrow \xi \in \xi).
  \end{equation*}

  This holds when \( \xi \) and \( \sigma \) take on the same value, hence it is satisfiable in na\"ive set theory. \Fullref{thm:classical_first_order_logic_is_sound_and_complete} implies that \( \neg\psi \) is derivable in the theory.
...
\begin{definition}\label{def:subset}\mcite[3]{Enderton1977Sets}
  ...
  \begin{equation*}\taglabel[\op{IsSubset}]{eq:def:subset/predicate}
    \ref{eq:def:subset/predicate}[\rho, \sigma] \coloneqq \qforall \xi (\xi \in \rho \rightarrow \xi \in \sigma),
  \end{equation*}
...
\begin{definition}\label{def:basic_set_operations}
  ...
  \begin{thmenum}
    ...
    \begin{equation*}\taglabel[\op{IsUnion}]{eq:def:basic_set_operations/union/predicate}
      \ref{eq:def:basic_set_operations/union/predicate}[\rho, \sigma] \coloneqq \qforall \xi \parens[\Big]{ \xi \in \rho \leftrightarrow \qexists {\eta \in \sigma} \xi \in \eta }.
    \end{equation*}
    ...
    \begin{equation*}\taglabel[\op{IsIntersection}]{eq:def:basic_set_operations/intersection/predicate}
      \ref{eq:def:basic_set_operations/intersection/predicate}[\rho, \sigma] \coloneqq \qforall \xi \parens[\Big]{ \xi \in \rho \leftrightarrow \qforall {\eta \in \sigma} \xi \in \eta }.
    \end{equation*}
    ...
    \begin{equation*}\taglabel[\op{IsDifference}]{eq:def:basic_set_operations/difference/predicate}
      \ref{eq:def:basic_set_operations/difference/predicate}[\rho, \sigma, \tau] \coloneqq \qforall \xi \parens[\Big]{ \xi \in \rho \leftrightarrow (\xi \in \sigma \wedge \neg(\xi \in \tau)) }.
    \end{equation*}
    ...
    \begin{equation*}\taglabel[\op{IsPowerSet}]{eq:def:basic_set_operations/power_set/predicate}
      \ref{eq:def:basic_set_operations/power_set/predicate}[\rho, \sigma] \coloneqq \qforall \xi \parens[\Big]{ \xi \in \rho \leftrightarrow \ref{eq:def:subset/predicate}[\xi, \sigma] }.
    \end{equation*}
  \end{thmenum}
\end{definition}
...
\begin{proposition}\label{thm:boolean_algebra_of_subsets}
  Let \( X \) be an arbitrary set. Then the \hyperref[def:basic_set_operations/power_set]{power set} \( \pow(X) \) endowed with the \hyperref[def:subset]{inclusion} partial order \( \subseteq \) is a \hyperref[def:semilattice/complete]{complete} \hyperref[def:boolean_algebra]{Boolean algebra}\footnote{We will not make use here of the full apparatus of Boolean algebras. Instead, we regard this statement as a shorthand that would otherwise require enumerating a lot of properties.}.

  Furthermore, the algebra is \hyperref[def:boolean_algebra/trivial]{trivial} if and only if \( X \) is the empty set.
...
\begin{definition}\label{def:ordinal_successor}\mcite[68]{Enderton1977Sets}
  We define the \term{successor} \( \op{succ}(A) \) of a set \( A \) as the set
  \begin{equation*}
  ...
  \begin{equation*}\taglabel[\op{IsSucc}]{eq:def:ordinal_successor/predicate}
    \ref{eq:def:ordinal_successor/predicate}[\rho, \sigma] \coloneqq \qforall \xi \parens[\Big]{ \xi \in \rho \leftrightarrow (\xi \in \sigma \vee \xi = \sigma) },
  \end{equation*}
