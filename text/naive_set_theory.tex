...
\begin{remark}\label{rem:self_reference}
  \hyperref[con:impredicativity]{Impredicativity} was a driving philosophical concern of Bertrand Russell, who was concerned that self-reference leads to contradictions such as \fullref{thm:russels_paradox}. For this reason Russell introduced a \enquote{hierarchy of types} with which he grades objects of discourse so that propositions of one type cannot reference propositions of higher types. This and later developments are briefly discussed in \fullref{rem:type_theory}.

  This lead Russell to introduce first-order propositions, second-order propositions, etc, where a proposition of order \( n \) could quantify over propositions of lower orders.
