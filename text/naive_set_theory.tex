...
\begin{remark}\label{rem:set_theory_formula_variables}
  In addition to using the symbols \( \xi \), \( \eta \), \( \zeta \) for variables in logical formulas, as discussed in \fullref{rem:mathematical_logic_conventions/variable_symbols}, out of necessity we additionally use \( \sigma \) (from \enquote{\hi{s}et}), \( \tau \), \( \rho \) (from \enquote{\hi{r}esult}; in formulas like \( \ref{eq:def:basic_set_operations/union/predicate} \)).
\end{remark}

\begin{definition}\label{def:naive_set_theory}\mcite[subsec. 6.1]{Hinman2005}
  The \term{language of na\"ive set theory} is a \hyperref[def:first_order_language]{first-order language} \( \mscrL \) with only a single \hyperref[rem:first_order_formula_conventions/infix]{infix} binary relation \( \in \) called \term{set membership}. If \( \Bracks{\xi \in \eta}_v = T \), we say that (under the \hyperref[def:first_order_valuation/variable_assignment]{variable assignment} \( v \)) \( \xi \) is a \term{member} or \term{element} of \( \eta \) and that \( \eta \) \term{contains}\footnote{The term \enquote{contains} is used for both the membership relation and the subset relation. This tends not to cause confusion in practice.} \( \xi \).

  For the sake of simplicity, we will not introduce into the language any other functional or predicate symbols, but will use \hyperref[rem:predicate_formula]{predicate formulas} when needed mostly for formulating axioms. See the \( \ref{eq:def:grothendieck_universe/predicate}[\upsilon] \) predicate for an extreme example.
  ...
  \begin{thmenum}
    ...
    \begin{equation}\label{eq:def:naive_set_theory/extensionality}
      \parens[\Big]{ \qforall \zeta (\zeta \in \xi \syniff \zeta \in \eta) } \synimplies \parens[\Big]{ \xi \syneq \eta }.
    \end{equation}
    ...
    \begin{equation}\label{eq:def:naive_set_theory/extensionality_quantified}
      \qforall \xi \qforall \eta \parens[\Bigg]{ \parens[\Big]{ \qforall \zeta (\zeta \in \xi \syniff \zeta \in \eta) } \synimplies \parens[\Big]{ \xi \syneq \eta } }.
    \end{equation}

    The \hyperref[def:material_implication/converse]{converse} of \eqref{eq:def:naive_set_theory/extensionality} obvious.

    \thmitem{def:naive_set_theory/unrestricted_comprehension} The \term{axiom schema of unrestricted comprehension} states that any formula defines a set. For each formula \( \varphi \) not containing \( \rho \) as a free variable, but possibly containing \( \xi \), the following is an axiom:
    \begin{equation}\label{eq:def:naive_set_theory/unrestricted_comprehension}
      \qexists \rho \qforall \xi (\xi \in \rho \syniff \varphi).
    \end{equation}

    It is important to highlight that \( \varphi \) may have any number of free variables as long as \( \rho \) is not among them. Of course, this axiom is only interesting if \( \xi \) is free in \( \varphi \). If \( \eta_1, \ldots, \eta_n \) are all the other free variables of \( \varphi \), then the \hyperref[def:universal_closure]{universal closure} of the corresponding axiom is
    \begin{equation}\label{eq:def:naive_set_theory/unrestricted_comprehension_quantified}
      \qforall {\eta_1} \cdots \qforall {\eta_n} \qexists \rho \qforall \xi (\xi \in \rho \syniff \varphi).
    \end{equation}
...
\begin{definition}\label{def:set}\mimprovised
  ...
  If \( x \) is related to \( A \) in the object theory, that is if \( \Bracks{\xi \in \eta}_{\xi \to x, \eta \to A} = T \), we say that \( x \) is a \term{member} or \term{element} of \( A \) or, in a geometric context, a \term{point} in \( A \).
\end{definition}
...
\begin{remark}\label{rem:standard_model_of_set_theory}
  ...
  \begin{itemize}
    \thmitem{rem:standard_model_of_set_theory/set_builder_notation} \hyperref[def:set_builder_notation]{Set-builder notation} relies on constructing sets in the metatheory and then using them in the object theory. If the model is not standard, then, for any \hyperref[def:first_order_valuation/variable_assignment]{variable assignment} \( v \) in the model, it does not hold that \( \Bracks{\xi \in \eta}_{v_{\xi \mapsto x, \eta \mapsto y}} = T \) if and only if \( x \in y \).
...
\begin{definition}\label{def:set_builder_notation}\mimprovised
  ...
  Within the metatheory, fix an \( n \)-tuple \( u_1, \ldots, u_n \) of members of the universe \( U \), which we will call \term{parameters}. Denote by \( A \) the set of members \( x \) of \( U \) such that \( \Bracks{\varphi}(x, u_1, \ldots, u_n) = T \)\footnote{We have used the notation for formula valuation from \fullref{def:propositional_valuation/formula_valuation_function}}.
  ...
  For example, the \hyperref[def:basic_set_operations/intersection]{intersection} of the sets \( B \) and \( C \) is given by the formula \( \xi \in \eta \synwedge \xi \in \zeta \), where \( B \) is a value for the parameter \( \eta \) and \( C \) is a parameter for \( \zeta \). The intersection can thus be written as
  \begin{equation*}
  ...
  \begin{equation*}
    \Bracks{ \xi \in \sigma }_{ v_{\xi \mapsto x} } = T
    \T{if and only if}
    x \isinE A,
  \end{equation*}
...
\begin{remark}\label{rem:singleton_sets}
  Sets with a single elements are usually called \term{singletons}. It is sometimes convenient, especially with connection to \hyperref[sec:geometry]{geometry} or \hyperref[def:function]{set-valued maps} (e.g. when dealing with \hyperref[def:net_limit_point]{limits of nets} or \hyperref[def:subdifferentials]{subdifferentials}), to not distinguish between singleton sets and their corresponding element.
\end{remark}
...
\begin{definition}\label{def:empty_set}
  ...
  \begin{equation*}\taglabel[\op{IsEmpty}]{eq:def:empty_set/predicate}
    \ref{eq:def:empty_set/predicate}[\sigma] \coloneqq \qforall \eta \synneg \eta \in \sigma.
  \end{equation*}
...
\begin{theorem}[Russell's paradox]\label{thm:russels_paradox}
  ...
  \begin{equation}\label{eq:thm:russels_paradox_comprehension_formula}
    \varphi = (\xi \not\in \xi)
  \end{equation}
  allows us to derive \( \synbot \) in \hyperref[rem:classical_logic]{classical logic}.
\end{theorem}
\begin{comments}
  \item Thus, the set
...
\begin{proof}
  ...
  \begin{equation}\label{eq:thm:russels_paradox_comprehension_axiom}
    \psi \coloneqq \qexists \sigma \qforall \xi (\xi \in \sigma \syniff \synneg (\xi \in \xi)).
  \end{equation}

  We will show that the negation \( \synneg\psi \) of \( \psi \) is also derivable in this theory. An explicit form of the negation can be obtained by utilizing the equivalences from \fullref{thm:first_order_quantifiers_are_dual} and \fullref{thm:boolean_equivalences/biconditional_negation}:
  \begin{equation*}
    \synneg\psi = \qforall \sigma \qexists \xi (\xi \in \sigma \syniff \xi \in \xi).
  \end{equation*}

  This holds when \( \xi \) and \( \sigma \) take on the same value, hence it is satisfiable in na\"ive set theory. \Fullref{thm:classical_first_order_logic_is_sound_and_complete} implies that \( \synneg\psi \) is derivable in the theory.

  Thus, \( \psi \) and \( \synneg\psi \) are both derivable in the same theory, and we can use \eqref{eq:def:minimal_propositional_natural_deduction_system/neg/elim} to also derive \( \synbot \), which shows that na\"ive set theory is inconsistent.
\end{proof}
...
\begin{definition}\label{def:subset}\mcite[3]{Enderton1977Sets}
  ...
  \begin{equation*}\taglabel[\op{IsSubset}]{eq:def:subset/predicate}
    \ref{eq:def:subset/predicate}[\rho, \sigma] \coloneqq \qforall \xi (\xi \in \rho \synimplies \xi \in \sigma),
  \end{equation*}
  which is valid when \( \rho \) is a subset of \( \sigma \), will occasionally be useful for us.
\end{definition}
\begin{comments}
  \item Some authors, such as \cite{Kelley1975}, use the notation \( A \subset B \) to mean \enquote{all elements of \( A \) belong to \( B \)}, even in the case when \( A = B \). To avoid ambiguity, we use the more explicit notations \( A \subseteq B \) and \( A \subsetneq B \).
\end{comments}
...
\begin{definition}\label{def:basic_set_operations}
  ...
  \begin{thmenum}
    ...
    The corresponding \hyperref[rem:predicate_formula]{predicate formula} is
    \begin{equation*}\taglabel[\op{IsUnion}]{eq:def:basic_set_operations/union/predicate}
      \ref{eq:def:basic_set_operations/union/predicate}[\rho, \sigma] \coloneqq \qforall \xi \parens[\Big]{ \xi \in \rho \syniff \qexists {\eta \in \sigma} \xi \in \eta }.
    \end{equation*}
    ...
    \begin{equation*}\taglabel[\op{IsIntersection}]{eq:def:basic_set_operations/intersection/predicate}
      \ref{eq:def:basic_set_operations/intersection/predicate}[\rho, \sigma] \coloneqq \qforall \xi \parens[\Big]{ \xi \in \rho \syniff \qforall {\eta \in \sigma} \xi \in \eta }.
    \end{equation*}
    ...
    \begin{equation*}\taglabel[\op{IsDifference}]{eq:def:basic_set_operations/difference/predicate}
      \ref{eq:def:basic_set_operations/difference/predicate}[\rho, \sigma, \tau] \coloneqq \qforall \xi \parens[\Big]{ \xi \in \rho \syniff (\xi \in \sigma \synwedge \synneg(\xi \in \tau)) }.
    \end{equation*}
    ...
    \begin{equation*}\taglabel[\op{IsPowerSet}]{eq:def:basic_set_operations/power_set/predicate}
      \ref{eq:def:basic_set_operations/power_set/predicate}[\rho, \sigma] \coloneqq \qforall \xi \parens[\Big]{ \xi \in \rho \syniff \ref{eq:def:subset/predicate}[\xi, \sigma] }.
    \end{equation*}
  \end{thmenum}
\end{definition}
...
\begin{proposition}\label{thm:boolean_algebra_of_subsets}
  Let \( X \) be an arbitrary set. Then the \hyperref[def:basic_set_operations/power_set]{power set} \( \pow(X) \) endowed with the \hyperref[def:subset]{inclusion} partial order \( \subseteq \) is a \hyperref[def:complete_lattice]{complete} \hyperref[def:boolean_algebra]{Boolean algebra}\footnote{We will not make use here of the full apparatus of Boolean algebras. Instead, we regard this statement as a shorthand that would otherwise require enumerating a lot of properties.}.
...
\begin{definition}\label{def:ordinal_successor}\mcite[68]{Enderton1977Sets}
  We define the \term[ru=операция следования (\cite[1.1.11]{КолмогоровДрагалин2006})]{successor} \( \op{succ}(A) \) of a set \( A \) as the set
  \begin{equation*}
  ...
  \begin{equation*}\taglabel[\op{IsSucc}]{eq:def:ordinal_successor/predicate}
    \ref{eq:def:ordinal_successor/predicate}[\rho, \sigma] \coloneqq \qforall \xi \parens[\Big]{ \xi \in \rho \syniff (\xi \in \sigma \synvee \xi = \sigma) },
  \end{equation*}
...
\begin{definition}\label{def:inductive_set}\mcite[68]{Enderton1977Sets}
  ...
  \begin{equation*}\taglabel[\op{IsInductive}]{eq:def:inductive_set/predicate}
    \ref{eq:def:inductive_set/predicate}[\sigma] \coloneqq
      \parens[\Big]{ \qexists {\xi \in \sigma} \ref{eq:def:empty_set/predicate}[\xi] }
      \synwedge
      \parens[\Big]{ \qforall {\xi \in \sigma} \qexists {\eta \in \sigma} \ref{eq:def:ordinal_successor/predicate}[\eta, \xi] }.
  \end{equation*}
\end{definition}
...
\begin{theorem}[Induction via inductive sets]\label{thm:omega_induction}\mcite[thm. 6.2.17]{Hinman2005}
  ...
  \begin{itemize}
    ...
    \item If \( A \) contains \( n \), it also contains \( \op{succ}(n) \).
  \end{itemize}

  More precisely, the following is a theorem of both na\"ive set theory and \hyperref[def:zfc]{\logic{ZF}} (\( \omega \) satisfies \( \sigma \) and \( \xi \) plays the role of \( A \) above):
  \footnotesize
        ...
        \parens[\Bigg]
          {
            \parens[\Big]
            {
              \underbrace{ \qexists {\eta \in \xi} \ref{eq:def:empty_set/predicate}[\eta] }_{\mathclap{\T{base case}}}
            }
            \synwedge
            \qforall \eta \parens[\Big]
              {
                \overbrace
                  {
                    \underbrace{ \eta \in \xi }_{\mathclap{\substack{\T{inductive} \\ \T{hypothesis}}}}
                    \synimplies
                    \underbrace
                      {
                        \qexists {\zeta \in \xi} \ref{eq:def:ordinal_successor/predicate}[\zeta, \eta]
                      }_{\mathclap{\substack{\T{inductive step} \\ \T{conclusion}}}}
                  }^{\T{inductive step}}
              }
          }
        \synimplies
        \underbrace{ \ref{eq:def:subset/predicate}[\sigma, \xi] }_{\T{conclusion}}
      }
  \end{equation}
  \normalsize
\end{theorem}
...
\end{comments}
\begin{proof}
  The antecedent of (the inner formula in) \eqref{eq:thm:omega_induction} is a restatement of the predicate formula \( \ref{eq:def:inductive_set/predicate}[\xi] \). The situation resembles the \hyperref[eq:def:zfc/infinity]{axiom of infinity}, but, instead of existence of an inductive set \( \xi \), it states the existence of a set \( \sigma \) such that if \( \xi \) is an inductive set, then \( \sigma \) is a subset of \( \xi \) (if we restrict \( \eta \) to range only over members of \( \sigma \), then we would obtain equality of \( \xi \) and \( \sigma \) instead). In other words, we have reduced the verification of \eqref{eq:thm:omega_induction} to showing that there exists a minimal inductive set in both na\"ive set theory and \logic{ZF}.
...
\begin{definition}\label{def:transitive_set}\mcite[def. 6.1.3(i)]{Hinman2005}
  ...
  \begin{equation*}\taglabel[\op{IsSetTransitive}]{eq:def:transitive_set/predicate}
    \ref{eq:def:transitive_set/predicate}[\sigma] \coloneqq \qforall {\xi \in \sigma} \qforall {\eta \in \xi} {\eta \in \sigma}
  \end{equation*}
\end{definition}
\begin{comments}
