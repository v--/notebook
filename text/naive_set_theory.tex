...
\begin{concept}\label{con:impredicativity}
  The following informal definition is given by \incite[42]{Kleene1971Metamathematics}:
  \begin{displayquote}
...
\end{theorem}
\begin{comments}
  \item This is a contradiction presented by \incite[222]{Russell1908TypeTheory} as a motivation for his theory of types.
\end{comments}
\begin{comments}
  \item Thus, the set
...
\begin{remark}\label{rem:self_reference}
  \hyperref[con:impredicativity]{Impredicativity} was a driving philosophical concern of Bertrand Russell, who was concerned that self-reference leads to contradictions such as \fullref{thm:russels_paradox}. For this reason Russell introduced a \enquote{hierarchy of types} with which he grades objects of discourse so that propositions of one type cannot reference propositions of higher types. This and later developments related to type theory are briefly discussed in \cref{rem:type_theory}. \Cref{ex:def:type_derivation_tree/self_application} demonstrates how self-referential \hyperref[def:lambda_term]{\( \muplambda \)-terms} are not \hyperref[def:typability]{typable}.
\end{remark}
