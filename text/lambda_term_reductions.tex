...
\paragraph{Simulating functions}

\begin{algorithm}[\texorpdfstring{\( \muplambda \)}{λ}-term to function]\label{alg:untyped_lambda_term_to_function}
  \hyperref[def:lambda_term]{\( \muplambda \)-abstractions} were introduced by \textcite[352]{Church1932LambdaCalculus} as a syntax for \hyperref[def:function]{functions}.

  Fix a \( \muplambda \)-term \( M \). First, we must determine the arity of \( M \) --- the number of directly nested abstractions:
  ...
  For every \( n \leq \op*{Arity}(M) \), we can define an \( n \)-ary operator corresponding to \( M \):
  ...
  In case \( n < \op*{Arity}(M) \), we call \( \op*{Apply}_M(A_1, \ldots, A_n) \) a \term[en=partial application (\cite[21]{Mimram2020ProgramEqualsProof})]{partial application} of \( M \).
\end{algorithm}
\begin{comments}
  \item This algorithm is implemented in \cite{notebook:code} as
  \begin{CodeRefDisplay}
    \GetCodeRef{alg:untyped_lambda_term_to_function}
  \end{CodeRefDisplay}

  \item For \hyperref[def:typed_lambda_term]{typed \( \muplambda \)-terms}, we need to restrict each argument to the corresponding type. See \cref{rem:simply_typed_lambda_term_dependency}.

  \item As described in \cref{rem:quantifiable_type_uncurrying}, knowing the arity of a \( \muplambda \)-term allows us to use the dedicated function syntax from \cref{def:function_application_notation}.
\end{comments}

\begin{concept}\label{con:currying}
  By definition, a \( \muplambda \)-abstraction encodes a unary function. As seen in \fullref{alg:untyped_lambda_term_to_function}, however, nested \( \muplambda \)-abstractions can encode a function of several arguments.

  This process is sometimes called \term[en=currying (\cite[23]{UnivalentFoundationsProgram2013HoTT})]{currying} in honor of Haskell Curry's work on combinatory logic (which we briefly discuss in \cref{rem:typing_style}).

  \term[en=uncurrying (\cite[394]{Perrone2024StartingCategoryTheory})]{uncurrying}.
\end{concept}

\paragraph{Subterm occurrences}

\begin{definition}\label{def:lambda_subterm_occurrence}\mimprovised
  An \term{occurrence} of a subterm \( N \) of \( M \) is a \hyperref[def:tree/subobject]{subtree} of the \hyperref[con:abstract_syntax_tree]{abstract syntax tree} \( T(M) \) of that is \hyperref[def:labeled_tree/morphism]{isomorphic} to \( T(N) \).
\end{definition}
\begin{comments}
...
\begin{definition}\label{def:subterm_occurrence_ordering}\mimprovised
  ...
  \begin{thmenum}
    \thmitem{def:subterm_occurrence_ordering/vertical} Under the \term{vertical ordering}, \( R \leq S \) if \( R \) is a \hyperref[def:tree/subobject]{subtree} of \( S \).
...
\end{definition}
\begin{comments}
  \item This definition is loosely based on Mimram's \cite[138]{Mimram2020ProgramEqualsProof}, but differs in several aspects:
  \begin{itemize}
  ...
  Other authors like \textcite[\S 8.4.7]{Barendregt1984LambdaCalculus} only consider horizontal ordering.
\end{comments}
...
\end{definition}
\begin{comments}
  \item This definition is not established. It is based on reflexive closures as defined in \cref{def:relation_closures/reflexive}. It allows us to make explicit the dependency on \( \alpha \)-equivalence, which is implicit in our primary sources --- \cite[ch. 3]{Barendregt1984LambdaCalculus} and \cite[def. 1B2]{Hindley1997BasicSTT}.
\end{comments}
...
\begin{definition}\label{def:lambda_term_reduction}\mcite[315]{Barendregt1984LambdaCalculus}
  ...
  \begin{thmenum}
    ...
    Since \hyperref[def:lambda_term]{atomic terms} are \( \alpha \)-equivalent to themselves, there is no need to separately consider them.
...
\begin{example}\label{ex:def:beta_eta_reduction}
  ...
  \begin{thmenum}
    ...
    \begin{equation*}
      KAB
      =
      (\qabs {\underline{\synx}} \qabs \syny \underline{\synx}) \underline{A} B
      \bred
      (\qabs {\underline{v}} A) \underline{B}
      \bred
      A.
    \end{equation*}
    ...
    \begin{balign*}
      SABC
      &=
      \parens[\big]{ \qabs {\underline{\synx}} \qabs \syny \qabs \synz \underline{\synx} \synz (\syny \synz) } \underline{A}BC
      \bred \\ &\bred
      \parens[\big]{ \qabs {\underline{u}} \qabs v A v (\underline{u} v) } \underline{B}C
      \bred \\ &\bred
      \parens[\big]{ \qabs {\underline{w}} A \underline{w} (B \underline{w}) } \underline{C}
      \bred \\ &\bred
      A C (B C).
    \end{balign*}
    ...
    For example, we have
    \begin{equation*}
      (\qabs {\underline{\synx}} \qabs \syny \underline{\synx} \syny) \underline{A} B \bred (\qabs v A v) B,
    \end{equation*}
    where \( v \) is not free in \( A \).

    The latter term is a \( \beta \)-redex, hence we can use \( \beta \)-reduction again:
    \begin{equation*}
      (\qabs {\underline{v}} A \underline{v}) \underline{B} \bred AB.
    \end{equation*}
    ...
    \begin{equation*}
      \omega_n \omega_n
      =
      (\qabs {\underline{\synx}} \underline{\synx^n}) \underline{\omega_n}
      \bred
      \omega_n^n.
    \end{equation*}
    ...
    \begin{balign*}
      S_+ C_n
      &=
      \parens[\big]{ \qabs {\underline{\synf}} \qabs \synx \qabs \syny \synx (\underline{\synf} \synx \syny) } \underline{C_n}
      \bred \\ &\bred
      \qabs \synx \qabs \syny \synx \parens[\big]{ C_n \underline{\synx} \syny }
      = \\ &=
      \qabs \synx \qabs \syny \synx \parens[\big]{ \parens[\big]{ \qabs {\underline{\synx}} \qabs \syny \underline{\synx}^n \syny } \underline{\synx} \syny }
      \bred \\ &\bred
      \qabs \synx \qabs \syny \synx \parens[\big]{ \parens[\big]{ \qabs {\underline{\syny}} \synx^n \underline{\syny} } \underline{\syny} }
      \bred \\ &\bred
    ...
    \begin{balign*}
      ...
      P_{-L} (\qabs v v A B)
      = \\ &=
      (\qabs {\underline{\synp}} {\underline{\synp}} (\qabs \synx \qabs \syny \synx)) (\qabs v v A B)
      \bred \\ &\bred
      (\qabs v v A B) (\qabs \synx \qabs \syny \synx)
      = \\ &=
      (\qabs {\underline{v}} \underline{v} A B) (\underline{\qabs \synx \qabs \syny \synx})
      \bred \\ &\bred
      (\qabs {\underline{\synx}} \qabs \syny \underline{\synx}) \underline{A} B
      \bred \\ &\bred
      (\qabs {\underline{w}} A) \underline{B}
      \bred \\ &\bred
      A,
    \end{balign*}
    ...
    \begin{itemize}
      \item \Textcite[def. 6.2.4]{Barendregt1984LambdaCalculus} introduces new metasyntax:
      \begin{itemize}
        \item \( [A, B] \) for \( P_+ A B \)
        \item \( C_0 \) for \( P^{-L} C \)
        \item \( C_1 \) for \( P^{-R} C \)
      ...
      \item \Textcite[\S 3.3.3]{Mimram2020ProgramEqualsProof} uses the identifiers \ManualCodeRef{pair}, \ManualCodeRef{fst} and \ManualCodeRef{snd} to denote the combinators.

      \item \Textcite[exerc. 2.5]{AbelsonSussman2012eSICP} ask to implement these combinators in the programming language Scheme, imitating the built-in procedures. The language uses its own naming convention --- according to \cite[115]{AbelsonSussman2012eSICP}:
      \begin{itemize}
        \item \enquote{cons} stands for \enquote{construct}.
        \item \enquote{car} is an abbreviation for \enquote{\underline{c}ontents of \underline{a}ddress part of \underline{r}egister}.
        \item \enquote{cdr} stands for \enquote{\underline{c}ontents of \underline{d}escriptor part of \underline{r}egister}.
      \end{itemize}
    ...
    The programming language Haskell has \ManualCodeRef{Left} and \ManualCodeRef{Right} value constructors that roughly correspond to the functions \( \iota_\mscrA \) and \( \iota_\mscrB \), and an \ManualCodeRef{Either} type constructor which roughly corresponds to the function \( (\mscrA, \mscrB) \mapsto \mscrA \amalg \mscrB \). Documentation for the corresponding Haskell constructs can be found in \cite{HackageDocs:base-4.21:either}.
    ...
    The first line binds the identifier \ManualCodeRef{either} to a specific type. We will discuss this particular example in \cref{ex:def:type_derivation_tree/disjoint}, after discussing types in general. What we need to deduce here from the entire definition is the intended interpretation of \ManualCodeRef{either} --- it should either apply a function \( f: \mscrA \to \mscrC \) to a value from \( \mscrA \) or \( g: \mscrB \to \mscrC \) to a value from \( \mscrB \).
    ...
    Here \ManualCodeRef{interval (Left a)} produces the (infinite) Haskell list \ManualCodeRef{[a, a + 1, a + 2..]}, while \ManualCodeRef{interval (Right b)} produces \ManualCodeRef{[b, b - 1, b - 2..]}.

    The following \( \muplambda \)-combinators, based on \cite{MathOF:product_type_in_simply_typed_lambda_terms}, correspond to Haskell's \ManualCodeRef{Left} and \ManualCodeRef{Right} constructors and to the \ManualCodeRef{either} function, respectively:
    \begin{align*}
    ...
    \thmitem{ex:def:beta_eta_reduction/boolean}\mcite[\S 6.2.2]{Barendregt1984LambdaCalculus} We can use the terms \ref{eq:ex:def:lambda_term/combinator/k} and \ref{eq:ex:def:beta_eta_reduction/numerals/c0} as \hyperref[def:truth_value_algebra]{Boolean values}.
...
\begin{comments}
  ...
  \begin{CodeRefDisplay}
    \ManualCodeRef{lambda_.untyped.reduction.test_beta}
  \end{CodeRefDisplay}
\end{comments}
...
\begin{example}\label{ex:reduction_strategies}
  ...
  \begin{balign*}
    (C_0 \Omega) I
    &=
    \parens[\big]{ \parens[\big]{ \qabs {\underline{\synx}} \qabs \syny \syny } \underline{\Omega} } I
    \bred \\ &\bred
    (\qabs {\underline{\syny}} \underline{\syny}) \underline{I}
    \bred \\ &\bred
    I.
  \end{balign*}
...
\begin{definition}\label{def:delta_reduction}\mcite[def. 15.3.1]{Barendregt1984LambdaCalculus}
  ...
  To avoid complex coherence conditions, we will simply suppose that \( \delta \)-reduction is governed by a \hyperref[def:set_valued_map/partial]{single-valued partial function} \( \op*{\delta} \) sending \hyperref[def:lambda_term]{constants} to \hyperref[def:lambda_combinator]{combinators}. Thus, for every constant \( C \), there are either no reduction rules or exactly one rule of the form
...
\end{definition}
\begin{comments}
  \item Unlike \( \beta \)- and \( \eta \)-reduction, \( \delta \)-reduction is not a widely established concept and has no widely accepted definition. Our definition is loosely based on \cite[def. 15.3.1]{Barendregt1984LambdaCalculus}, but with vast simplifications. The usefulness of the obtained reduction is discussed in \cref{rem:delta_reduction}.
...
\begin{example}\label{ex:thm:lambda_term_reduction_free_variables}
  The following table illustrates the different possibilities from \cref{thm:beta_contractum_free_variables}:
  \begin{equation*}
    \begin{array}{*{4}{l}}
      \toprule
...
\begin{example}\label{ex:def:function_fixed_point_combinator}
  ...
  \begin{thmenum}
    ...
    If, as per \fullref{alg:untyped_lambda_term_to_function}, we regard \( N \) as a function on \( \muplambda \)-terms, \( MN \) corresponds to a fixed point of \( N \). Since neither Boolean value is a fixed point of negation, we conclude that \( N \) actually corresponds to generalization of negation in \hyperref[def:truth_value_algebra/classical]{classical semantics}.
...
\end{proposition}
\begin{comments}
  \item \Textcite*[177]{CurryFeysCraig1958CombinatoryLogicVol1} call \( Y \) \enquote{the paradoxical combinator} because it can be used to construct terms \enquote{of a more or less paradoxical nature}.
\end{comments}
\begin{proof}
  ...
  \begin{equation*}
    YF
    =
    \parens[\big]{ \qabs {\underline{\synx}} (\qabs \syny \underline{\synx} \syny \syny) (\qabs \syny \underline{\synx} \syny \syny) } \underline{F}
    \bred
  ...
  \begin{equation*}
    P =
    (\qabs {\underline{v}} F \underline{v} \underline{v}) \underline{(\qabs v F v v)}
    \bred
...
\begin{proposition}\label{thm:substitution_on_single_step_reduction}
  ...
  If \( M \pred N \), for any atomic substitution \( \Bbbs \), we have \( M[\Bbbs] \pred N[\Bbbs] \).
\end{proposition}
\begin{proof}
  We will use \fullref{thm:induction_on_recursively_defined_relations} on \( M \pred N \):
  \begin{itemize}
    ...
    By assumption, \( u \) is not in \( \op*{Free}_\Bbbs(M) \), hence it belongs to \( \op*{Free}_\Bbbs(A) \) only if it is free in \( \Bbbs(x) \). The generalized condition in \cref{thm:lambda_substitution_chain_contraction/composition} is satisfied, so we conclude that
    \begin{equation*}
    ...
    \Cref{thm:lambda_substitution_chain_contraction/composition} implies that
    \begin{equation*}
    ...
    \Cref{thm:nary_lambda_substitution_composition_is_alpha_equivalent} then implies that
    \begin{equation*}
      N[\Bbbs]
      =
      A[x \mapsto B][\Bbbs]
    ...
    Since \( x \) is not free in \( N \), the substitutions \( \Bbbs_{x \mapsto u} \) and \( \Bbbs \) agree on the free variables of \( N \). \Cref{thm:lambda_substitutions_agree} then implies that
    \begin{equation*}
