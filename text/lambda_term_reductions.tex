...
\begin{example}\label{ex:def:subterm_occurrence_ordering}
  ...
  \begin{thmenum}
    ...
    \begin{figure}[!ht]
      ...
      \hfill
      \hfill
      \caption{The abstract syntax tree of the \( \muplambda \)-term \( C_0 \Omega I \) from \fullref{ex:def:subterm_occurrence_ordering/c0_omega_i}.}\label{fig:ex:def:subterm_occurrence_ordering/c0_omega_i}
    \end{figure}
...
\begin{definition}\label{def:lambda_term_reduction}\mcite[315]{Barendregt1984LambdaCalculus}
  ...
  In accordance with \fullref{con:lambda_conversion}, we will call the \hyperref[def:binary_relation/converse]{converse relation} \enquote{\( {\Anon} \)-expansion}.
...
\end{definition}
\begin{comments}
  \item We insist on explicitly handling \( \alpha \)-equivalence because otherwise reduction would not be compatible with substitution --- see the proof of \fullref{thm:substitution_on_single_step_reduction}
\end{comments}
...
\begin{definition}\label{def:beta_eta_reduction}\mcite[def. 1B1; 1C1]{Hindley1997BasicSTT}
  ...
  We define \term{\( \beta \)-reduction} \( {\bred} \) as the relation generated by the rules of \fullref{def:lambda_term_reduction/single} extended with \ref{inf:def:beta_eta_reduction/beta}. We similarly define \term{\( \eta \)-reduction} \( {\ered} \) based on \ref{inf:def:beta_eta_reduction/eta}. Combining both, we obtain \term{\( \beta\eta \)-reduction} \( {\redrel{\beta\eta}} \).
\end{definition}
\begin{comments}
  \item Of course, we also consider equivalences and multi-step reductions based on the above single-step reductions.
\end{comments}

\begin{algorithm}[Lambda term to function]\label{alg:untyped_lambda_term_to_function}
...
\begin{comments}
  ...
  \item For \hyperref[def:typed_lambda_term]{typed \( \muplambda \)-terms}, \( \op*{Apply}_M \) becomes a \hyperref[def:set_valued_map/partial]{partial function} because we need to restrict each argument to the corresponding type. See \fullref{rem:simply_typed_lambda_term_dependency}.

  \item As described in \fullref{rem:simple_type_function_syntax}, knowing the arity of a \( \muplambda \)-term allows us to use the dedicated function syntax from \fullref{def:function_syntax}.
\end{comments}
...
\begin{example}\label{ex:def:beta_eta_reduction}
  ...
  either _ g (Right y) = g y
    ...
    The first line binds the identifier \verb|either| to a specific type. We will discuss this particular example in \fullref{ex:def:type_derivation_tree/disjoint}, after discussing types in general. What we need to deduce here from the entire definition is the intended interpretation of \verb|either| --- it should either apply a function \( f: \mscrA \to \mscrC \) to a value from \( \mscrA \) or \( g: \mscrB \to \mscrC \) to a value from \( \mscrB \).

    Our proof of \fullref{thm:simple_algebraic_type_arithmetic} lists multiple examples of nontrivial \hyperref[def:type_derivation_tree]{type derivation trees} using conditional logic via \hyperref[def:simple_sum_type]{sum types}. As a simpler usage example, consider \hyperref[def:order_interval/unbounded]{unbounded segments} of integers that can be either
    \begin{align*}
  ...
  interval = either (\a -> [a..]) (\b -> [b, b - 1..])
    ...
    We will define truth values differently in \fullref{sec:simply_typed_lambda_terms} and also in \fullref{def:type_of_booleans}, for which reason we avoid using these combinators as boolean values more generally, but for the purposes of this example define \( T \) and \( F \) as
    \begin{align*}
...
\begin{definition}\label{def:delta_reduction}\mcite[def. 15.3.1]{Barendregt1984LambdaCalculus}
  Allowing constants in our \( \muplambda \)-terms provides a multitude of possibilities with respect to reduction. Any rule that allows rewriting constants can be extended to arbitrary \( \muplambda \)-terms via the rules from \fullref{def:lambda_term_reduction/single}, similarly to how \( \beta \)- and \( \eta \)-reductions were defined in \fullref{def:beta_eta_reduction}.
...
\begin{comments}
  ...
  \item We require the \( \delta \)-contractums to be combinators so that \fullref{thm:lambda_term_reduction_free_variables} holds.
\end{comments}
...
\begin{remark}\label{rem:delta_reduction}
  ...
  As will be shown in \fullref{ex:def:type_derivation_tree/pairs}, there are nuances when typing the corresponding combinators. For product types, which we will define in \fullref{def:simple_product_type}, we will use constants with specific typing rules, and use \( \beta\delta \)-reduction to emulate \hyperref[rem:type_theory_rule_classification/equality/comp]{computation rules}.
\end{remark}
...
\begin{proof}
  ...
  Otherwise, \fullref{thm:lambda_substitution_noop} implies that \( M[x \mapsto N] = M \).
\end{proof}

\begin{example}\label{ex:thm:lambda_term_reduction_free_variables}
  The following table illustrates the different possible cases from \fullref{thm:beta_contractum_free_variables}:
  \begin{equation*}
    \begin{array}{*{4}{l}}
      \toprule
...
\begin{proof}
  ...
  \begin{itemize}
    \item If \( M \pred N \) due to \ref{inf:def:beta_eta_reduction/beta}, then \( M = (\qabs x K) L \) and \( N = K[x \mapsto L] \). \Fullref{thm:beta_contractum_free_variables} implies that \( \op*{Free}(N) \subseteq \op*{Free}(M) \).
  ...
  \begin{itemize}
    ...
    \Fullref{thm:def:lambda_term_alpha_equivalence/free} implies that the free variables of \( M \) and \( M' \) coincide, as do the free variables of \( N \) and \( N' \).
...
\begin{remark}\label{rem:beta_equivalence_and_free_variables}
  If we consider \hyperref[def:lambda_term]{\( \muplambda \)-terms} up to \hyperref[def:lambda_term_alpha_equivalence]{\( \alpha \)-equivalence}, working with the resulting equivalence classes would not differ much compared to raw terms. In particular, \fullref{thm:beta_contractum_free_variables} implies that the set of free variables does not depend on a representative of the class, and \fullref{thm:substitution_on_alpha_equivalent_terms} implies the same for \hyperref[def:lambda_term_substitution]{substitution}.
...
\begin{example}\label{ex:def:function_fixed_point_combinator}
  ...
  \begin{thmenum}
    \thmitem{ex:def:function_fixed_point_combinator/neg} Let \( M \) be a fixed point combinator. Consider the negation term \ref{ex:def:beta_eta_reduction/boolean/n} from \fullref{ex:def:beta_eta_reduction/boolean}.
    ...
    For the successor combinator \ref{eq:ex:def:beta_eta_reduction/numerals/succ} from \fullref{ex:def:beta_eta_reduction/numerals}, the fixed point \( MS_+ \) corresponds to a generalized numeral that is its own successor.
  \end{thmenum}
\end{example}
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \item If \( M \pred N \) due to \ref{inf:def:lambda_term_reduction/alpha}, we use \fullref{thm:substitution_on_alpha_equivalent_terms}.
    ...
    By assumption, \( u \) is not in \( \op*{Free}_\Bbbs(M) \), hence it belongs to \( \op*{Free}_\Bbbs(A) \) only if it is free in \( \Bbbs(x) \). The generalized condition in \fullref{thm:substitution_chain_contraction/composition} is satisfied, so we conclude that
    \begin{equation*}
    ...
    \Fullref{thm:substitution_chain_contraction/composition} implies that
    \begin{equation*}
    ...
    \Fullref{thm:nary_substitution_composition_is_alpha_equivalent} then implies that
    \begin{equation*}
      N[\Bbbs]
      =
      A[x \mapsto B][\Bbbs]
    ...
    Since \( x \) is not free in \( N \), the substitutions \( \Bbbs_{x \mapsto u} \) and \( \Bbbs \) agree on the free variables of \( N \). \Fullref{thm:lambda_substitution_restriction} then implies that
    \begin{equation*}
