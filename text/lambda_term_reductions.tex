...
\begin{example}\label{ex:def:beta_eta_reduction}
  ...
  interval = either (\a -> [a..]) (\b -> [b, b - 1..])
    ...
    We will define truth values differently in \fullref{sec:simply_typed_lambda_terms} and also in \cref{def:type_of_booleans}, for which reason we avoid using these combinators as boolean values more generally, but for the purposes of this example define \( T \) and \( F \) as
    \begin{align*}
