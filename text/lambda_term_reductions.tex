...
\begin{lemma}\label{thm:beta_eta_redex_alpha_equivalence}
  Let \enquote{\( {\Anon} \)} be a combination of \enquote{\( \beta \)} and \enquote{\( \eta \)}. If \( M \aequiv N \) and \( M \) contains a redex of the corresponding kinds, then \( N \) also does.

  Furthermore, if \( M \) is itself a redex, so is \( N \).
\end{lemma}
\begin{proof}
  We use \fullref{thm:induction_on_recursively_defined_relations} on \( M \aequiv N \) simultaneously for all \( N \):
  \begin{itemize}
    ...
    \item If \( M \aequiv N \) due to \ref{inf:def:lambda_term_alpha_equivalence/abs}, then \( M = \qabs a A \) and \( N = \qabs b B \) and \( A[a \mapsto n] \aequiv B[b \mapsto n] \) for every \( n \) not free in \( M \), where the inductive hypothesis holds for the latter.

    We have the following possibilities:
    \begin{itemize}
      \item If \( A = Ea \), then \( A[a \mapsto n] = E[a \mapsto n] n \), hence \( B[b \mapsto n] \) is also an application term whose second subterm is \( n \). It follows that \( B = Fb \), and hence both \( M \) and \( N \) are \( \eta \)-redexes.

      \item Otherwise, \( A \) must contain a redex. Since \( b \) is not free in \( A \), we can show by case analysis that \( A[a \mapsto b] \) also contains a redex. But \( A[a \mapsto b] \aequiv B \), and, by the inductive hypothesis, \( B \) also contains a redex.

      It follows that \( N \) contains a redex.
    \end{itemize}
  \end{itemize}
\end{proof}

\begin{proposition}\label{thm:beta_eta_reducible_iff_redex}
  Let \enquote{\( {\Anon} \)} be a combination of \enquote{\( \beta \)} and \enquote{\( \eta \)}. Fix a term \( M \). Then there exists a term \( N \) such that \( M \pred N \) if and only if \( M \) contains a redex of the corresponding kinds.
\end{proposition}
\begin{proof}
  ...
  \begin{itemize}
    ...
    \item If \( M \pred N \) due to \ref{inf:def:lambda_reduction/abs}, then \( M = \qabs x A \) and \( \qabs x B  \) with \( A \pred B \). Via the inductive hypothesis, we conclude that \( A \), and hence also \( M \), contain a redex.
    ...
    \item If \( M \pred N \) due to \ref{inf:def:beta_eta_reduction/beta} or \ref{inf:def:beta_eta_reduction/eta} \( M \) itself is a redex.
  \end{itemize}
