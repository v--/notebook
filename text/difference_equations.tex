...
\begin{comments}
  ...
  \item \incite[22]{Müller2022HandbookOfDynamicsAndProbability} restricts the function to \( 0 \leq a \leq 4 \) and \( 0 \leq x \leq 1 \), so that, as shown in \fullref{thm:def:logistic_map/unit}, the unit interval is invariant.
...
\begin{proposition}\label{thm:def:logistic_map}
  ...
  \begin{thmenum}
    \thmitem{thm:def:logistic_map/unit} If \( 0 \leq a \leq 4 \) and \( 0 \leq x \leq 1 \), then \( 0 \leq f(x) \leq 1 \).
...
\end{proposition}
\begin{proof}
  \SubProofOf{thm:def:logistic_map/unit} The function \( f(x) = -ax^2 + ax \) has derivative
  \begin{equation*}
    f'(x) = -2ax + a,
  \end{equation*}
...
\end{definition}
\begin{comments}
  \item It follows from \fullref{thm:def:logistic_map/unit} that the entire trajectory is bounded between \( 0 \) and \( (1 + \alpha) / \beta \).
