...
\begin{proposition}\label{thm:recurrence_relations_and_difference_equations}
  ...
  Conversely, using \fullref{thm:difference_operator_factorization}, we can express every finite difference in an difference equation via consecutive elements of the sequence, thus turning it into a recurrence relation.
\end{proposition}
\begin{comments}
  ...
  \item \Fullref{ex:def:difference_equation/fibonacci} demonstrates such a transformation.
\end{comments}

\begin{proposition}\label{ex:def:difference_equation}
  ...
  \begin{thmenum}
    ...
    \thmitem{ex:def:difference_equation/fibonacci} A slightly more substantial example is the \hyperref[def:fibonacci_numbers]{Fibonacci sequence}
