...
\begin{definition}\label{def:propositional_alphabet}\mcite[def. 1.1.1]{VanDalen2004}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:propositional_alphabet/connectives} The set \( \boldop{Conn} \) of \term{binary propositional connectives}, namely
    \begin{thmenum}
...
\begin{definition}\label{def:propositional_subformula}
  ...
  \begin{equation*}
    \boldop{Subform}(\varphi) \coloneqq \begin{cases}
      ...
      \set{ \varphi } \bigcup \boldop{Subform}(\psi) \cup \boldop{Subform}(\psi), &\varphi = \psi \syncirc \theta, {\syncirc} \in \boldop{Conn}.
    \end{cases}
  \end{equation*}
\end{definition}
...
\begin{definition}\label{def:institution}\mcite[def. 1]{GoguenBurstall1992}
  An \term{institution} \( \mscrI \) consists of the following:
  \begin{thmenum}
    \thmitem{def:institution/signatures} A \hyperref[def:category]{category} \( \cat{Sign}_\mscrI \), whose objects we will call \term{signatures}.

    \thmitem{def:institution/sentences} A \hyperref[def:functor]{functor} \( \boldop{Sen}_\mscrI: \cat{Sign}_\mscrI \to \cat{Set} \). We call the elements of the set \( \boldop{Sen}_\mscrI(\Sigma) \) \term{sentences}.

    \thmitem{def:institution/models} A \hyperref[def:functor]{functor} \( \cat{Mod}_\mscrI: \cat{Sign}_\mscrI \to \cat{Cat}^{\oppos} \). We call the objects of the category \( \boldop{Mod}_\mscrI(\Sigma) \) \term{models} and the morphisms --- \term{homomorphisms}.

    \thmitem{def:institution/satisfaction} A family of \hyperref[def:relation]{relations} indexed by \( \obj(\cat{Sign}_\mscrI) \), where, for a given signature \( \Sigma \), \( \vDash_\Sigma \) is a relation between \( \obj(\boldop{Mod}_\mscrI(\Sigma)) \) and \( \cat{Sen}_\Sigma \) such that, for each signature morphism \( s: \Sigma \to \Tau \), each sentence \( \varphi \) in \( \boldop{Sen}_\mscrI(\Sigma) \) and each model \( \mscrT \) in \( \boldop{Mod}\mscrI_\Tau \), the following condition holds:
    \begin{equation}\label{eq:def:institution/satisfaction}
      \mscrT \vDash_\Tau \aunderbrace{\boldop{Sen}_\mscrI(s)}_{\boldop{Sen}_\mscrI(\Sigma) \to \boldop{Sen}_\mscrI(\Tau)}(\varphi)
      \quad\T{if and only if}\quad
      \aunderbrace{\boldop{Mod}(s)}_{\boldop{Mod}_\mscrI(\Tau) \to \boldop{Mod}_\mscrI(\Sigma)}(\mscrT) \vDash_\Sigma \varphi
    \end{equation}
...
\begin{definition}\label{def:institution_satisfaction}\mcite[def. 2(1)]{GoguenBurstall1992}
  Fix an \hyperref[def:institution]{institution} \( \mscrI \) and a signature \( \Sigma \) in \( \cat{Sign}_\mscrI \).
...
\end{definition}
\begin{comments}
  \item We must distinguish between the phrases \enquote{\( \mscrX \) is a model} and \enquote{\( \mscrX \) is a model of \( \Gamma \)}. In the concrete institutions we will consider, the category \( \boldop{Mod}_\mscrI(\Sigma) \) consists of objects that are not called models --- for example \hyperref[def:propositional_interpretations]{propositional interpretation} or \hyperref[def:first_order_structure]{first-order structures}.
\end{comments}

\begin{definition}\label{def:institution_entailment}\mcite[def. 2(9)]{GoguenBurstall1992}
  Again, fix an \hyperref[def:institution]{institution} \( \mscrI \) and a signature \( \Sigma \) in \( \cat{Sign}_\mscrI \).

  We say that the set of sentences \( \Gamma \) \term{entails} the sentence \( \psi \) and write \( \Gamma \vDash_\Sigma \psi \) if, whenever for some model \( \mscrX \) we have \( \mscrX \vDash_\Sigma \Gamma \), we also have \( \mscrX \vDash_\Sigma \psi \).

  We prefer the notation \( \gamma_1, \ldots, \gamma_n \vDash_\Sigma \psi \) to \( \set{ \gamma_1, \ldots, \gamma_n } \vDash_\Sigma \psi \). In particular, we write \( \varphi \vDash_\Sigma \psi \) in case of one sentence \( \vDash_\Sigma \psi \) in case of zero sentences.
\end{definition}

\begin{proposition}\label{thm:institution_entailment_properties}
  The \hyperref[def:institution_entailment]{entailment relation} \( {\vDash_\Sigma} \) for a signature \( \Sigma \) in the \hyperref[def:institution]{institution} \( \mscrI \) has the following basic properties:
  \begin{thmenum}
    \thmitem{thm:institution_entailment_properties/weakening} If \( \Gamma \subseteq \Delta \) are sets of sentences and if \( \Gamma \vDash \varphi \), then \( \Delta \vDash \varphi \).

    This corresponds to \enquote{weakening} rules in sequent calculus --- see \cite[29]{TroelstraSchwichtenberg2000}.

    \thmitem{thm:institution_entailment_properties/preorder} Regarded as a binary relation between sentences, it is a \hyperref[def:preordered_set]{preorder}, that is, it is \hyperref[def:binary_relation/reflexive]{reflexive} and \hyperref[def:binary_relation/transitive]{transitive}.

    This motivates Lindenbaum-Tarski algebras --- see \fullref{def:lindenbaum_tarski_algebra}.

    \thmitem{thm:institution_entailment_properties/consequence} Denote the set of sentences \( \boldop{Sen}_\mscrI(\Sigma) \) by \( S \). Then the following is a \hyperref[def:consequence_operator]{Tarski consequence operator}:
    \begin{equation*}
      \begin{aligned}
        &\op{Cn}_\Sigma: \pow(S) \to \pow(S), \\
        &\op{Cn}_\Sigma(\Gamma) \coloneqq \set{ \psi \in S \given \Gamma \vDash_\Sigma \psi }
      \end{aligned}
    \end{equation*}
  \end{thmenum}
\end{proposition}
\begin{proof}
  \SubProofOf{thm:institution_entailment_properties/weakening} Trivial.

  \SubProofOf{thm:institution_entailment_properties/preorder}

  \SubProofOf*[def:binary_relation/reflexive]{reflexivity} For any model \( \mscrX \) and sentence \( \varphi \) for the given signature, if \( \mscrX \vDash_\Sigma \varphi \), then, vacuously, \( \mscrX \vDash_\Sigma \varphi \). The first judgment implies the second one, that is, itself. Then \( \varphi \vDash_\Sigma \varphi \).

  \SubProofOf*[def:binary_relation/transitive]{transitivity} Suppose that \( \varphi \vDash_\Sigma \psi \) and \( \psi \vDash_\Sigma \theta \).

  If, for some model \( \mscrX \), we have \( \mscrX \vDash_\Sigma \varphi \), this implies \( \mscrX \vDash_\Sigma \psi \), which in turn implies \( \mscrX \vDash_\Sigma \theta \).

  Therefore, \( \varphi \vDash_\Sigma \theta \).

  \SubProofOf{thm:institution_entailment_properties/consequence}

  \SubProofOf*[def:extensive_function]{extensiveness} Because of reflexivity shown in \fullref{thm:institution_entailment_properties/preorder}, the set \( \op{Cn}_\Sigma(\Gamma) \) should contain every sentence in \( \Gamma \).

  \SubProofOf*[def:idempotent_function]{idempotence} Idempotence follows from transitivity.

  \SubProofOf*[def:order_function/preserving]{monotonicity} Finally, suppose that \( \Gamma \subseteq \Delta \). If \( \Gamma \vDash_\Sigma \psi \), \fullref{thm:institution_entailment_properties/weakening} implies that \( \Delta \vDash_\Sigma \psi \). This shows that \( \op{Cn}_\Sigma(\Gamma) \subseteq \op{Cn}_\Sigma(\Delta) \).
\end{proof}
...
\end{definition}
\begin{comments}
  \item In \hyperref[con:classical_logic]{classical logic}, the contrapositive is \hyperref[def:propositional_entailment/equivalence]{equivalent} to the original formula due to \fullref{thm:boolean_equivalences/contrapositive}.
\end{comments}
...
\begin{remark}\label{rem:material_implication}
  ...
  Material implication fits naturally in \hyperref[con:classical_logic]{classical} \hyperref[con:metalogic]{metalogic} due to \fullref{thm:semantic_deduction_theorem}, although \fullref{rem:first_order_entailment_without_closed_formulas} highlights possible complications with arbitrary \hyperref[def:first_order_syntax/formula]{first-order formulas}.
\end{remark}
...
\begin{definition}\label{def:propositional_formula_variables}\mimprovised
  ...
  \begin{equation*}
    \boldop{Var}(\varphi) \coloneqq \begin{cases}
      ...
      \boldop{Var}(\psi) \cup \boldop{Var}(\theta), &\varphi = \psi \syncirc \theta, {\syncirc} \in \boldop{Conn}.
    \end{cases}
  \end{equation*}
\end{definition}

\begin{definition}\label{def:propositional_valuation}\mimprovised
  We define (\hyperref[con:classical_logic]{classical}) \term[ru=значение истинности (формулы) (\cite[8]{Эдельман1975}), en=valuation \cite[def. 1.2.1]{VanDalen2004}]{valuation} for propositional formulas.
  ...
  \begin{thmenum}
    ...
    \begin{equation}\label{eq:def:propositional_valuation/formula_valuation}
      \Bracks{\varphi}_I \coloneqq \begin{cases}
        ...
        \Bracks{\psi}_I \relcirc \Bracks{\theta}_I &\varphi = \psi \syncirc \theta, {\syncirc} \in \boldop{Conn},
      \end{cases}
    \end{equation}
...
\begin{proposition}\label{thm:def:propositional_entailment}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:def:propositional_entailment/weakening} If \( \Gamma \vDash \varphi \), then \( \Gamma, \Delta \vDash \varphi \) for any set of formulas \( \Delta \).
...
\begin{proof}
  ...
  \SubProofOf{thm:def:propositional_entailment/weakening} Trivial.
...
\end{proposition}
\begin{comments}
  \item Most of these equivalences fail in \hyperref[con:intuitionistic_logic]{intuitionistic logic}.
\end{comments}
\begin{proof}
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \item If \( \varphi \neq \chi \) and \( \varphi = \psi \syncirc \theta, {\syncirc} \in \boldop{Conn} \) and if the inductive hypothesis holds for both \( \psi \) and \( \theta \), then for any interpretation \( I \),
    \begin{equation*}
      \Bracks[\Big]{ \varphi[\chi \mapsto \omega] }_I
      =
  ...
  \begin{equation*}
    \begin{aligned}
      \id(\varphi) \coloneqq \begin{cases}
        ...
        \id(\psi) \syncirc \id(\theta), &\varphi = \psi \syncirc \theta, {\syncirc} \in \boldop{Conn}, \\
      \end{cases}
    \end{aligned}
  \end{equation*}
  ...
  \begin{equation*}
    \begin{aligned}
      R(\varphi) \coloneqq \begin{cases}
        ...
        R(\psi) \syncirc R(\theta),  &\varphi = \psi \syncirc \theta, {\syncirc} \in \boldop{Conn}, \\
      \end{cases}
    \end{aligned}
  \end{equation*}
