...
\end{proposition}
\begin{proof}
  \SubProofOf{thm:intuitionistic_tautologies/self} For any interpretation \( I \) we have
  \begin{equation*}
    \Bracks{\varphi \synimplies \varphi}_I
    =
    \Bracks{\varphi}_I \implies \Bracks{\varphi}_I
...
\begin{proposition}\label{thm:intuitionistic_equivalences}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:intuitionistic_deduction_consequences/top_elim} We can eliminate verum from the antecedent of conditional formulas:
    \begin{equation}\label{eq:thm:intuitionistic_deduction_consequences/top_elim}
...
\begin{proof}
  ...
  \SubProofOf{thm:intuitionistic_deduction_consequences/top_elim} Follows from \fullref{thm:def:heyting_algebra/top_left}.
\end{proof}
...
\begin{corollary}\label{thm:intuitionistic_equivalences}
  We list some consequences of \Fullref{thm:propositional_semantic_deduction_theorem}:
  \begin{thmenum}
    \thmitem{thm:intuitionistic_deduction_consequences/top} \( \varphi \vDash \top \) for every formula \( \varphi \).

    \thmitem{thm:intuitionistic_deduction_consequences/bot} \( \synbot \vDash \varphi \) for every formula \( \varphi \).
  \end{thmenum}
\end{corollary}
\begin{proof}
  \SubProofOf{thm:intuitionistic_deduction_consequences/top} Follows from \fullref{thm:def:heyting_algebra/top_left}.
...
\begin{definition}\label{def:minimal_propositional_semantics}\mimprovised
  In addition to classical and intuitionistic semantics discussed in \fullref{def:propositional_semantics}, we will also briefly consider \term{minimal semantics}, obtained by interpreting \( \synbot \) as a propositional variable. Thus, a propositional interpretation must now provide a value for \( \synbot \).
...
\begin{definition}\label{def:paraconsistent_consequence_operator}\mcite{StanfordPlato:paraconsistent_logic}
  We say that the \hyperref[def:consequence_operator]{consequence operator} \( \vdash \) is \term{explosive} if, for any two formulas \( \varphi \) and \( \psi \), the following holds:
  \begin{equation*}
    \varphi \synwedge \neg \varphi \synimplies \psi.
  \end{equation*}
...
\begin{proof}
  ...
  Then \( I \vDash \syntop \) and \( I \vDash \neg \syntop \). Yet, if \( I(P) = F \) for some variable \( P \), we cannot conclude that \( I \vDash P \).
\end{proof}
...
\end{proposition}
\begin{proof}
  Fix two formulas, \( \varphi \) and \( \psi \), and an interpretation \( I \) such that \( I \vDash \varphi \) and \( I \vDash \psi \). Then \( I \vDash \varphi \synwedge \neg \varphi \) because
  \begin{equation*}
    \Bracks{\varphi \synwedge \neg \varphi}_I
    =
    \Bracks{\varphi}_I \synwedge \Bracks{\neg \varphi}_I
    =
    T \wedge T
    =
    T.
