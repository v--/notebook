...
\begin{definition}\label{def:propositional_syntax}\mimprovised
  ...
  \begin{thmenum}
    \thmitem{def:propositional_syntax/grammar_schema} Consider the following \hyperref[def:formal_grammar/schema]{grammar schema}:
    \begin{bnf*}
    ...
    \thmitem{def:propositional_syntax/language} By \enquote{language of propositional logic}\fnote{Various authors refer to \enquote{the language of propositional logic} --- for example \incite[def. 1.1.1]{VanDalen2004} and \incite[13]{Hinman2005} --- in relation to various syntactic constructions. These authors do not formally define this language, however. Since we use the mechanism of \hyperref[def:formal_language]{formal languages}, we prefer being more concrete about which syntactic notions we refer to.} we will mean the set \( \boldop{Form} \) of formulas (and hence also the variables).
    ...
    More complicated syntax like that of \( \lambda \)-calculus or first-order logic requires a more elaborate definition --- see \fullref{ex:def:lambda_term/naive_subterm}.
