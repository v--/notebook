...
\begin{definition}\label{def:propositional_formula}\mimprovised
  ...
  We will also call the formulas \term{sentences} in relation to \hyperref[def:abstract_logic]{abstract logics}. This contrasts with predicate logic, where only specific formulas are called sentences --- see \cref{ex:universal_validity_and_lem}.
\end{definition}
\begin{comments}
...
\begin{concept}\label{con:improper_symbol}
  In \cref{def:propositional_valuation/formula_valuation}, we have defined valuations for \hyperref[def:propositional_formula]{propositional formulas}. Based on this definition, the symbols used to define the formal grammar of formulas can be divided into two collections:
  \begin{thmenum}
...
\begin{definition}\label{def:propositional_semantics}\mimprovised
  ...
  \begin{thmenum}
    ...
    Instead, we say that \( I \) is a \term[en=model (\cite[def. 1.4.1(i)]{Hinman2005Logic})]{propositional model} \hi{of} the set of \hyperref[def:propositional_formula]{propositional formulas} \( \Gamma \) if \( \Bracks{\varphi}_I = T \) for every formula \( \varphi \in \Gamma \).

    \thmitem{def:propositional_semantics/entailment} We use the terminology and notation from \cref{def:institutional_entailment} regarding entailment: \( \Gamma \vDash \varphi \) if, whenever \( I \) satisfies \( \Gamma \), it also satisfies \( \varphi \).
  \end{thmenum}
\end{definition}
