...
\begin{definition}\label{def:propositional_semantics}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:propositional_semantics/entailment}\mcite[def. 1.4.1(ii)]{Hinman2005} If \( \Gamma \vDash_I \psi \) for all possible interpretations, we say that \( \Gamma \) entails \( \psi \) and write \( \Gamma \vDash \psi \).

    If \( \Gamma \not\vDash_I \varphi \) for at least one interpretation, we write \( \Gamma \not\vDash \varphi \).
