...
\begin{remark}\label{rem:propositional_formula_notation_conventions}
  ...
  \begin{thmenum}
    ...
    \thmitem{rem:propositional_formula_notation_conventions/associative} Because of the associativity of \( \synwedge \) and \( \synvee \), which is implied by \cref{def:parametrized_propositional_denotation} and \cref{def:standard_boolean_functions}, we may skip the parentheses in chains like
    \begin{equation*}
...
\begin{definition}\label{def:truth_value_algebra}\mimprovised
  In order to determine whether a sentence holds under given circumstances, we must have a set of \term[ru=истинностное значение (\cite[def. 1.1.12]{Герасимов2014Вычислимость}), en=truth value (\cite[\S 6.5.9]{Mimram2020ProgramEqualsProof})]{truth values}, for example the Boolean values \( T \) and \( F \) from \cref{def:truth_value_algebra}. The aforementioned set is naturally a \hyperref[def:boolean_algebra]{Boolean algebra}. The most general setting we will consider are \hyperref[def:heyting_algebra]{Heyting algebras}.

  When working with \hyperref[def:institution/models]{models} and notions related to them, we will presume that an underlying Heyting algebra \( \BbbT \) is fixed. In accordance with \cref{rem:mathematical_logic_conventions/propositional_constants}, we will denote the top and bottom element of \( \BbbT \) by \( T \) and \( F \).
...
\begin{concept}\label{con:parametrized_denotation}
  \hyperref[con:variable_binding]{Free variables} can introduce ambiguity in the \hyperref[con:denotation]{denotation} of a \hyperref[con:predicate_logic/predicate]{logical term} or \hyperref[con:proposition]{formula}. The resolution of this ambiguity requires us to explicitly assign values to the variables.
...
\end{definition}
\begin{comments}
  \item \Fullref{thm:propositional_semantic_inconsistency} lists some conditions equivalent to (un)satisfiability.
\end{comments}
...
\begin{example}\label{ex:def:propositional_semantics}
  ...
  \begin{thmenum}
    ...
    We will give a simple counterexample when \( \BbbT \) is the more general three-valued Heyting algebra from \cref{ex:def:heyting_algebra/three_valued}, where \( F < N < T \).
