...
\begin{definition}\label{def:propositional_language}\mcite[14]{Hinman2005}
  ...
  \begin{thmenum}
    ...
    \begin{thmenum}
      ...
      \thmitem{def:propositional_language/connectives/conditional} \term{Conditional}\footnote{Note that \enquote{conditional} and \enquote{biconditional} are nouns in this context.} \enquote{\( \synimplies \)}, also known as \term{if\ldots then} and \hyperref[def:material_implication]{\term{material implication}}.
      \thmitem{def:propositional_language/connectives/biconditional} \term{Biconditional} \enquote{\( \syniff \)}, also known as \term{iff}\footnote{\enquote{iff} is a common abbreviation for \enquote{if and only if}.} and \term{material equivalence}.
    \end{thmenum}
