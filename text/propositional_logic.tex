...
\end{definition}
\begin{comments}
  \item Propositional formulas are also called \term{sentences}, for example by Peter Hinman in \cite[ch. 1]{Hinman2005}. This contrasts with first-order logic, where only specific formulas are called sentences --- see \fullref{def:first_order_syntax/closed_formula}.
\end{comments}
...
\begin{definition}\label{def:propositional_semantics}
  ...
  \begin{thmenum}
    \thmitem{def:propositional_semantics/satisfies}\mcite[def. 1.4.1(i)]{Hinman2005} Given an interpretation \( I \) and a set \( \Gamma \) of formulas, we say that \( I \) \term{satisfies} \( \Gamma \) if we have \( \Bracks{\varphi}_I = T \) for every formula \( \varphi \in \Gamma \). We also say that \( I \) is a \term{model} of \( \Gamma \) and write \( I \vDash \Gamma \).
    ...
    \thmitem{def:propositional_semantics/satisfiable}\mcite[def. 1.4.8]{Hinman2005} We say that \( \Gamma \) is \term{satisfiable} if there exists at least one model for \( \Gamma \).

    \thmitem{def:propositional_semantics/entailment}\mcite[def. 1.4.1(ii)]{Hinman2005} We say that the set of formulas \( \Gamma \) \term{entails} the formula \( \psi \) and write \( \Gamma \vDash \psi \) if every model of \( \Gamma \) is also a model of \( \psi \).

    \thmitem{def:propositional_semantics/tautology}\mcite[def. 1.3.2(i)]{Hinman2005} A (semantic) \term{tautology} is a formula \( \varphi \)\footnote{We also say that \( \varphi \) is \term{valid}.} such that either:
    \begin{thmenum}
    ...
    \thmitem{def:propositional_semantics/contradiction}\mcite[def. 1.3.2(i)]{Hinman2005} Dually, a (semantic) \term{contradiction} is a formula \( \varphi \) such that either:
    \begin{thmenum}
    ...
    \thmitem{def:propositional_semantics/equisatisfiability}\mcite[116]{NerodeShore1993} A weaker notion than that of semantic equivalence is that of \term{equisatisfiability}. We say that the families \( \Gamma \) and \( \Delta \) are equisatisfiable if the following holds:
    \begin{displayquote}
...
\begin{definition}\label{def:propositional_substitution}
  ...
  \begin{thmenum}
    \thmitem{def:propositional_substitution/single}\cite[1.3.13]{Hinman2005} We define the \term{substitution} of the propositional formula \( \theta \) with \( \chi \) in \( \varphi \) as
