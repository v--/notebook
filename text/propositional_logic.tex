...
\begin{remark}\label{rem:grammar_rules_for_variables}
  ...
  \begin{bnf*}
    \bnfprod{Capital Latin letter}           {\bnftsq{A} \bnfor \bnftsq{B} \bnfor \cdots \bnfor \bnftsq{Z}} \\
    \bnfprod{Small Latin letter}             {\bnftsq{a} \bnfor \bnftsq{b} \bnfor \cdots \bnfor \bnftsq{z}} \\
    \bnfprod{Latin letter}                   {\bnfpn{Capital Latin letter} \bnfor \bnfpn{Small Latin letter}} \\
    \bnfprod{Short Latin identifier}         {\bnfpn{Latin letter} \bnfor \bnfpn{Latin letter} \bnfsp \bnfpn{natural number}}, \\
    \bnfprod{Short capital Latin identifier} {\bnfpn{Capital Latin letter} \bnfor \bnfpn{Capital Latin letter} \bnfsp \bnfpn{natural number}}, \\
    \bnfprod{Short small Latin identifier}   {\bnfpn{Small Latin letter} \bnfor \bnfpn{Small Latin letter} \bnfsp \bnfpn{natural number}}, \\
  ...
  In \fullref{def:first_order_syntax}, we will use the nonterminal \( \bnfpn{Greek identifier} \) with analogous rules for denoting first-order variables.
...
\end{definition}
\begin{comments}
  \item In \hyperref[def:classical_logic]{classical logic}, the contrapositive is \hyperref[def:propositional_entailment/equivalence]{equivalent} to the original formula due to \fullref{thm:boolean_equivalences/contrapositive}.
\end{comments}
...
\begin{remark}\label{rem:material_implication}
  ...
  Material implication fits naturally in \hyperref[def:classical_logic]{classical} \hyperref[con:metalogic]{metalogic} due to \fullref{thm:semantic_deduction_theorem}.
\end{remark}
...
\begin{definition}\label{def:propositional_valuation}\mimprovised
  We define (\hyperref[def:classical_logic]{classical}) valuation for propositional formulas.

  \begin{thmenum}
    \thmitem{def:propositional_valuation/interpretation} A \term{propositional interpretation} is a function with signature \( I: \op*{Prop} \to \set{ T, F } \).
...
\begin{definition}\label{def:propositional_institution}
  \hyperref[def:propositional_valuation/interpretation]{Propositional interpretations} naturally give rise to an \hyperref[def:institution]{institution} as follows:
  \begin{thmenum}
    ...
    \thmitem{thm:propositional_institution/sentences} The \hyperref[def:institution/sentences]{sentence functor} can then simply be described as \( \anon \mapsto \op*{Form} \).

    \thmitem{thm:propositional_institution/models} Let \( \BbbI \) be the discrete category on the function set \( \fun(\op*{Prop}, \set{ T, F }) \). The \hyperref[def:institution/models]{model functor} can then be described as \( \anon \mapsto \BbbI \).
...
\begin{proposition}\label{thm:propositional_formulas_and_boolean_functions}
  ...
  \begin{thmenum}
    \thmitem{thm:propositional_formulas_and_boolean_functions/equivalence_classes} \hyperref[def:propositional_entailment/equivalence]{Semantic equivalence} \( \gleichstark \) is an \hyperref[def:equivalence_relation]{equivalence relation} on the set \( \op*{Form} \) of all propositional formulas.
