...
\end{proposition}
\begin{proof}
  \SubProofOf{thm:propositional_denotation_characterization/negation} Assume non-degenerate semantics.
...
\begin{remark}\label{rem:classical_propositional_interpretations}
  Note that a propositional interpretation may be used to define denotations different from the one in \cref{def:propositional_denotation}. For example, the semantics for \hyperref[con:minimal_logic]{minimal logic} defined in \cref{def:minimal_propositional_semantics} require a slight variation.
...
\begin{definition}\label{def:minimal_propositional_semantics}\mimprovised
  ...
  Formula denotations are thus defined as follows:
  \begin{empheq}[left={\Bracks{\varphi}_I} \coloneqq \empheqlbrace]{align}
