...
\begin{definition}\label{def:propositional_semantics}
  ...
  \begin{thmenum}
    \thmitem{def:propositional_semantics/entailment}\mcite[def. 1.4.1(ii)]{Hinman2005} We say that the set of \hyperref[def:propositional_syntax/formula]{propositional formulas} \( \Gamma \) \term{entails} \( \psi \) under the \hyperref[def:propositional_valuation/interpretation]{interpretation} \( I \) if, whenever \( \Bracks{\varphi}_I = T \) for all \( \varphi \in \Gamma \), it follows that \( \Bracks{\psi}_I = T \). We denote this via \( \Gamma \vDash_I \psi \).

    If \( \Gamma \vDash_I \psi \) for all possible interpretations, we say that \( \Gamma \) entails \( \psi \) and write \( \Gamma \vDash \psi \).
    ...
    \begin{thmenum}
      \thmitem{def:propositional_semantics/tautology/interpretations} We have \( \Bracks{\varphi}_I = F \) for every interpretation \( I \).
      \thmitem{def:propositional_semantics/tautology/entailment} We have the entailment \( \varphi \vDash \bot \).
      \thmitem{def:propositional_semantics/tautology/equivalence} We have the equivalence \( \varphi \gleichstark \bot \).
    \end{thmenum}
  \end{thmenum}
\end{definition}
...
\begin{definition}\label{def:propositional_model}
  We say that the \hyperref[def:propositional_valuation/interpretation]{interpretation} \( I \) is a \term{propositional model} of the set of \hyperref[def:propositional_syntax/formula]{propositional formulas} \( \Gamma \) if \( \Bracks{\varphi}_I = T \) for every formula \( \varphi \in \Gamma \).
