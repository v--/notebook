...
\begin{comments}
  ...
  \item If desired, we can utilize a smaller propositional language without losing its semantical properties. Important example are the \hyperref[def:cnf_and_dnf]{conjunctive normal forms} in \fullref{alg:full_cnf_and_dnf}, although similar constructions can be made based on other \hyperref[def:boolean_closure/complete]{complete sets of Boolean functions} like those described in \cref{thm:complete_sets_of_boolean_functions}.
...
\begin{example}\label{ex:def:propositional_formula_ast}
  ...
  \begin{thmenum}
    ...
    \thmitem{ex:def:propositional_formula_ast/associative_conjunction} In \cref{def:parametrized_propositional_denotation} we will define semantics for conjunction and disjunction so that they become associative as Boolean operators. Following our general discussion of such binary operations in \cref{rem:binary_operation_syntax_trees}, we will generally conflate the formula
    \begin{equation*}
...
\begin{comments}
  ...
  \item Propositional denotations are implemented as \identifier{math.logic.propositional.evaluate_propositional_formula} in \cite{notebook:code}.
\end{comments}
...
\begin{definition}\label{def:propositional_semantics}\mimprovised
  ...
  \begin{thmenum}
    ...
    We say that \( I \) is a \term[en=model (\cite[def. 1.4.1(i)]{Hinman2005Logic})]{propositional model} \hi{of} the set of \hyperref[def:propositional_formula]{propositional formulas} \( \Gamma \) and write \( I \vDash \Gamma \) if and only if \( \Bracks{\varphi}_I = \semtop \) for every formula \( \varphi \) in \( \Gamma \).

    If \( \Gamma \) has a model, we say that it is \term[ru=выполнимая (формула) (\cite[def. 1.1.14]{Hinman2005Logic}), en=satisfiable (set) (\cite[def. 1.4.5]{Hinman2005Logic})]{satisfiable}.
...
\begin{example}\label{ex:def:propositional_semantics}
  ...
  \begin{thmenum}
    \thmitem{ex:def:propositional_semantics/trivial} Within the one-element Heyting algebra, where \( \semtop = \sembot \), every formula is satisfied because, due to antisymmetry, there is simply no truth value other than \( \semtop \). \Cref{thm:inconsistent_lindenbaum_tarski_algebra} implies that this precisely is the \hyperref[def:lindenbaum_tarski_algebra]{Lindenbaum-Tarski algebra} of any \hyperref[def:consistent_set_of_sentences]{inconsistent} set of formulas.
...
\begin{definition}\label{def:propositional_tautology}
  ...
  \begin{thmenum}
    \thmitem{def:propositional_tautology/interpretations}\mcite[def. 1.2.4(i)]{VanDalen2004LogicAndStructure} We have \( \Bracks{\varphi}_I = \semtop \) for every interpretation \( I \).
...
\begin{definition}\label{def:propositional_contradiction}
  ...
  \begin{thmenum}
    \thmitem{def:propositional_contradiction/interpretations}\mcite[def. 1.4.1(i)]{Hinman2005Logic} We have \( \Bracks{\varphi}_I = \sembot \) for every interpretation \( I \).
...
\begin{proposition}\label{thm:intuitionistic_tautologies}
  ...
  \begin{thmenum}
    ...
    \enquote{EFQ} stands for \enquote{ex contradictione quodlibet} --- a variation of the last principle without an explicit falsum. The phrase is used by \incite[2]{DienerMcKubreJordens2020MaterialImplication}.
...
\begin{definition}\label{def:paraconsistent_consequence_operator}\mcite{StanfordPlato:paraconsistent_logic}
  We say that the \hyperref[def:consequence_operator]{consequence operator} \( \vdash \) for propositional logic is \term{explosive} if the following metalingual variant of \eqref{eq:thm:intuitionistic_tautologies/ecq} holds:
  \begin{equation*}
...
\end{proposition}
\begin{proof}
  Fix an interpretation \( I \) such that \( I(\synbot) = \semtop \) and \( I(p) = \sembot \) for some propositional variable \( p \). Then
  \begin{equation*}
    \Bracks{\synneg \syntop}_I
    \reloset {\eqref{eq:def:minimal_propositional_semantics/neg}} =
    \Bracks{\syntop}_I \rightarrow I(\synbot)
    =
    \semtop \rightarrow \semtop
