...
\end{definition}
\begin{comments}
  \item If we call \( I \) a \enquote{model}, we refer to this definition rather than to an arbitrary interpretation, which is an model in the terminology of \hyperref[def:institution]{institutions}. See \cref{rem:institutional_model_terminology}.
\end{comments}
...
\begin{definition}\label{def:propositional_semantics}\mimprovised
  ...
  \begin{thmenum}
    ...
    \thmitem{def:propositional_semantics/classical} In the special case where the \hyperref[def:truth_value_algebra]{truth value algebra} is the two-element Boolean algebra \( \set{ T, F } \), we will call them \term{\hyperref[con:classical_logic]{classical}} or \term{Boolean}.
