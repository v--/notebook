...
\begin{definition}\label{def:propositional_semantics}\mimprovised
  ...
  \begin{thmenum}
    ...
    We also say that \( \varphi \) is \term[en=valid (\cite[70]{VanDalen2004LogicAndStructure})]{valid} in \( \mscrX \).


    \thmitem{def:propositional_semantics/satisfiable} If \( \Gamma \) has a model, we say that it is \term[ru=выполнимая (формула) (\cite[def. 1.1.14]{Герасимов2014Вычислимость}), en=satisfiable (set) (\cite[def. 1.4.5]{Hinman2005Logic})]{satisfiable}.
...
\begin{comments}
  ...
  \item \enquote{EFQ} stands for \enquote{ex falso quodlibet}. It is a shortening of \enquote{ex falso sequitur quodlibet}, which \incite[35]{TroelstraSchwichtenberg2000BasicProofTheory} translate from Latin as \enquote{from a falsehood follows whatever you like}. It is also known as the \enquote{the principle of explosion} because it allows us to show that intuitionistic semantics is \hyperref[def:paraconsistent_consequence_operator]{explosive} --- see \cref{thm:intuitionistic_semantics_are_explosive}.
...
\begin{theorem}[Ex contradictione quodlibet]\label{thm:propositional_semantic_ecq}
  If a \hyperref[def:propositional_interpretation]{propositional interpretation} \hyperref[def:propositional_semantics/model]{satisfies} both \( \varphi \) and \( \synneg \varphi \) for some formula \( \varphi \), it also satisfies every other formula.
\end{theorem}
\begin{comments}
  \item Via \fullref{alg:fol_propositional_formula_translation}, this theorem also extends to \hyperref[def:first_order_logic]{first-order logic}.
...
\begin{algorithm}[Brute-force propositional satisfaction]\label{thm:brute_force_propositional_satisfaction}
  There is a very straightforward algorithm for finding a \hyperref[def:propositional_interpretation]{propositional interpretation} \( I_\varphi \) that \hyperref[def:propositional_semantics/model]{satisfies} \( \varphi \), if one exists.
