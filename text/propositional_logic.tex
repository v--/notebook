...
\begin{remark}\label{rem:classical_propositional_interpretations}
  Note that a propositional interpretation may be used to define denotations different from the one in \cref{def:propositional_denotation}. An example is provided in \cref{rem:minimal_propositional_semantics}, where we discuss possible semantics for \hyperref[con:minimal_logic]{minimal logic}.
...
\begin{proposition}\label{thm:semantic_propositional_conjunction_of_premises}
  For \hyperref[def:propositional_formula]{propositional formulas} we have \( \varphi_1, \ldots, \varphi_n \vDash \psi \) if and only if \( (\varphi_1 \synwedge \cdots \synwedge \varphi_n) \vDash \psi \).
\end{proposition}
\begin{proof}
