...
\begin{definition}\label{def:propositional_syntax}\mimprovised
  ...
  \begin{thmenum}
    \thmitem{def:propositional_syntax/prop} We will denote by \( \op*{Prop} \) the set of all \term[ru=пропозициональные переменные (\cite[43]{КолмогоровДрагалин2006Логика}), en=propositional variable (\cite[27]{Church1956LogicVol1})]{propositional variables}, that is, all strings generated by the above grammar schema with starting nonterminal \( \bnfpn{variable} \).

    \thmitem{def:propositional_syntax/formula} Similarly, we will denote by \( \op*{Form} \) the set of \term[ru=формула (\cite[43]{КолмогоровДрагалин2006Логика}), en=propositional formula (\cite[41]{Mimram2020ProgramEqualsProof})]{formulas}, that is, strings generated with starting nonterminal \( \bnfpn{formula} \).
    ...
    We will also call \term{sentences} in relation to \hyperref[def:abstract_logic]{abstract logics}. This contrasts with predicate logic, where only specific formulas are called sentences --- see \cref{ex:universal_validity_and_lem}.
  \end{thmenum}
\end{definition}
\begin{comments}
  ...
  \item We implicitly associate with each propositional formula an \hyperref[con:abstract_syntax_tree]{abstract syntax tree} --- see \cref{def:propositional_formula_ast}. The grammar of propositional formulas is unambiguous as shown via \cref{thm:propositional_formula_grammar/unambiguous}, which makes it possible to perform proofs on formulas via \fullref{thm:induction_on_rooted_trees}.
...
\begin{example}\label{ex:def:propositional_formula_ast}
  ...
  \begin{thmenum}
    ...
    \thmitem{ex:def:propositional_formula_ast/associative_conjunction} In \cref{def:propositional_valuation/valuation_function} we will define semantics for conjunction and disjunction so that they become associative as Boolean operators. Following our general discussion of such binary operations in \cref{rem:binary_operation_syntax_trees}, we will generally conflate the formula
    \begin{equation*}
...
\begin{definition}\label{def:propositional_valuation}\mimprovised
  ...
  \begin{thmenum}
    ...
    \thmitem{def:propositional_valuation/formula_valuation} Given an interpretation \( I \), we define the corresponding \term[ru=значение истинности (формулы) (\cite[8]{Эдельман1975Логика})]{valuation} of a formula \( \varphi \) recursively as follows:
    \begin{empheq}[left={\Bracks{\varphi}_I} \coloneqq \empheqlbrace]{align}
      &T,                                         &&\varphi = \syntop,                                         \label{eq:def:propositional_valuation/formula_valuation/top} \\
      &F,                                         &&\varphi = \synbot,                                         \label{eq:def:propositional_valuation/formula_valuation/bot} \\
      &I(\varphi),                                &&\varphi \in \op*{Prop},                                    \label{eq:def:propositional_valuation/formula_valuation/prop} \\
      &\oline{\Bracks{\psi}_I},                   &&\varphi = \synneg \psi,                                    \label{eq:def:propositional_valuation/formula_valuation/neg} \\
      &\Bracks{\psi}_I \relcirc \Bracks{\theta}_I &&\varphi = \psi \syncirc \theta, {\syncirc} \in \op*{Conn}, \label{eq:def:propositional_valuation/formula_valuation/conn}
    \end{empheq}
    ...
    Unless otherwise noted, we assume that \( p_1, \ldots, p_n \) are precisely the variables of \( \varphi \) ordered lexicographically. We will call \( \Bracks{\varphi} \) the \term{valuation function} of \( \varphi \).
  \end{thmenum}
\end{definition}
\begin{comments}
  ...
  \begin{itemize}
    ...
    \item \incite[\S 2.5.6]{Mimram2020ProgramEqualsProof} uses analogous terminology, but only in the case where \( \BbbH = \set{ T, F } \), which in accordance to \cref{def:truth_value_algebra/classical}, we will call \enquote{classical semantics}.
...
\begin{definition}\label{def:propositional_institution}\mimprovised
  ...
  \begin{thmenum}
    ...
    \thmitem{def:propositional_institution/satisfaction} Finally, let the \hyperref[def:institution/satisfaction]{satisfaction} relation \( I \vDash_{\anon} \varphi \) hold if \( \Bracks{\varphi}_I = T \).
  \end{thmenum}
\end{definition}
\begin{defproof}
...
\begin{definition}\label{def:propositional_semantics}\mimprovised
  ...
  \begin{thmenum}
    ...
    However, as per \cref{rem:institutional_model_terminology}, unlike in general institutions, we do not refer to an arbitrary \hyperref[def:propositional_valuation/interpretation]{propositional interpretation} \( I \) as a \enquote{model} in isolation.

    Instead, we say that \( I \) is a \term[en=model (\cite[def. 1.4.1(i)]{Hinman2005Logic})]{propositional model} \hi{of} the set of \hyperref[def:propositional_syntax/formula]{propositional formulas} \( \Gamma \) if \( \Bracks{\varphi}_I = T \) for every formula \( \varphi \in \Gamma \).

    \thmitem{def:propositional_semantics/entailment} We use the terminology and notation from \cref{def:institutional_entailment} regarding entailment: \( \Gamma \vDash \varphi \) if, whenever \( I \) satisfies \( \Gamma \), it satisfies \( \varphi \).
  \end{thmenum}
\end{definition}

\begin{remark}\label{rem:classical_propositional_interpretations}
  Note that a propositional interpretation may be used to define valuations different from the one in \cref{def:propositional_valuation/formula_valuation}. For example, \hyperref[con:minimal_logic]{minimal logic} as defined in \cref{def:minimal_propositional_semantics} requires a slight variation.
...
\begin{example}\label{ex:def:propositional_semantics}
  ...
  \begin{thmenum}
    ...
    Indeed, let \( U \) be an open set. For any \hyperref[def:propositional_valuation]{propositional interpretation} \( I \) such that \( I(\synp) = U \), we have
    \begin{equation*}
      \Bracks{\synp \synvee \synneg \synp}_I
      =
...
\begin{proof}
  ...
  \begin{equation*}
    ...
    \reloset {\eqref{eq:def:propositional_valuation/formula_valuation/conn}} =
  ...
  \begin{equation*}
    ...
    \reloset {\eqref{eq:def:propositional_valuation/formula_valuation/neg}} =
  ...
  \begin{equation*}
    ...
    \reloset {\eqref{eq:def:propositional_valuation/formula_valuation/bot}} =
    \oline{F}
...
\begin{definition}\label{def:minimal_propositional_semantics}\mimprovised
  ...
  \begin{empheq}[left={\Bracks{\varphi}_I} \coloneqq \empheqlbrace]{align}
    ...
    &\vdots,                                 && \notag
  \end{empheq}
  the other cases being the same as in \cref{def:propositional_valuation/formula_valuation}.
\end{definition}
\begin{comments}
...
\end{theorem}
\begin{comments}
  \item This is one of several deduction theorems presented here --- see \cref{ex:deduction_theorem_list/propositional_semantical}.
\end{comments}
\begin{proof}
...
\begin{example}\label{ex:deduction_theorem_list}
  ...
  \begin{thmenum}
    \thmitem{ex:deduction_theorem_list/propositional_semantic} \Fullref{thm:propositional_semantic_deduction_theorem}.
    \thmitem{ex:deduction_theorem_list/implicational_syntactic} \Fullref{thm:implicational_syntactic_deduction_theorem}.
    \thmitem{ex:deduction_theorem_list/propositional_syntactic} \Fullref{thm:propositional_syntactic_deduction_theorem}.
  \end{thmenum}
\end{example}
