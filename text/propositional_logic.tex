...
\begin{definition}\label{def:propositional_syntax}\mimprovised
  ...
  \begin{thmenum}
    ...
    We will, by default, prefer the small Latin letters \( p \), \( q \) and \( r \) for concrete variable identifiers, and \( \upsilon \)whenever we need to refer to abstract propositional variables in the metalanguage.
...
\begin{definition}\label{def:propositional_valuation}\mimprovised
  ...
  \begin{thmenum}
    ...
    \thmitem{def:propositional_valuation/valuation_function} If \( \op*{Var}(\varphi) \subseteq \set{ \upsilon_1, \ldots, \upsilon_n } \), the valuation \( \Bracks{\varphi}_I \) only depends on the particular values \( I(\upsilon_1), \ldots, I(\upsilon_n) \) of \( I \). Hence, if the variables are clear from the context, we obtain a Boolean function
    \begin{equation*}
      \begin{aligned}
      ...
      \end{aligned}
    \end{equation*}
    where \( I \) is any interpretation such that \( I(\upsilon_k) = x_k \) for \( k = 1, \ldots, n \).

    Unless otherwise noted, we assume that \( \upsilon_1, \ldots, \upsilon_n \) are precisely the variables of \( \varphi \), ordered lexicographically as discussed in \fullref{rem:grammar_rules_for_variables}. We will call this the \term{valuation function} of \( \varphi \).
  \end{thmenum}
\end{definition}
\begin{comments}
...
\begin{example}\label{ex:trivial_heyting_semantics}
  Within the one-valued Heyting algebra, where \( T = F \), every formula is satisfied because there is simply no \enquote{non-true} truth value.
\end{example}
...
\begin{example}\label{ex:heyting_semantics_lem_counterexample}
  ...
  Let \( I \) be a \hyperref[def:propositional_valuation]{propositional interpretation} such that \( I(P) = N \). Then the valuation of \eqref{eq:thm:classical_tautologies/lem} is
  \begin{equation*}
    \Bracks{P \synvee \synneg P}_I
    =
    \Bracks{P}_I \vee \widetilde{\Bracks{P}_I}
    =
    N \vee F
    =
    N.
...
\begin{example}\label{ex:topological_semantics_lem_counterexample}
  ...
  Let \( U \) be an open set. We will examine \eqref{eq:thm:classical_tautologies/lem}. Given any \hyperref[def:propositional_valuation]{propositional interpretation} \( I \) such that \( I(P) = U \), we have
  \begin{equation*}
    \Bracks{P \synvee \synneg P}_I
    =
  ...
  If \( U \) is empty, then \( \Bracks{P \synvee \synneg P}_I = \BbbR \) and \eqref{eq:thm:classical_tautologies/lem} holds in this case. If \( U \) is the open unit interval \( (0, 1) \), then \( \Bracks{P \synvee \synneg P}_I = \BbbR \setminus \set{ 0, 1 } \) and \eqref{eq:thm:classical_tautologies/lem} does not hold.
\end{example}
...
\begin{theorem}[Propositional semantic deduction theorem]\label{thm:propositional_semantic_deduction_theorem}
  For an arbitrary \hyperref[def:truth_value_algebra]{truth value Heyting algebra} and arbitrary propositional formulas, we have
  \begin{equation*}
...
\begin{corollary}\label{thm:intuitionistic_deduction_consequences}
  ...
  \begin{thmenum}
    \thmitem{thm:intuitionistic_deduction_consequences/top} \( \varphi \vDash \syntop \) for every formula \( \varphi \).

    \thmitem{thm:intuitionistic_deduction_consequences/bot} \( \synbot \vDash \varphi \) for every formula \( \varphi \).
  \end{thmenum}
\end{corollary}
\begin{proof}
...
\begin{comments}
  ...
  \item Our definition for minimal semantics is based on the general principles outlined by \incite[3]{VanDerMolen2016}, \incite[35]{TroelstraSchwichtenberg2000}, \incite[116]{Galatos2007} and \incite[1]{DienerMcKubreJordens2016}. None of the aforementioned discuss semantics.
  \item Both \eqref{eq:thm:intuitionistic_tautologies/dni} and \eqref{eq:thm:intuitionistic_tautologies/lnc} hold in minimal logic, but our semantic proofs for them are not valid in minimal logic. We will demonstrate simple syntactic proofs in \fullref{thm:syntactic_minimal_tautologies}.
\end{comments}
