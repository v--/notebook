...
\begin{definition}\label{def:propositional_formula_ast}\mimprovised
  ...
  \begin{thmenum}
    \thmitem{def:propositional_formula_ast/atomic} If \( \varphi \) is a propositional constant or variable, we define \( T(\varphi) \) as the singleton \hyperref[def:labeled_tree]{labeled tree} with label \( \varphi \). As per \fullref{rem:tree_grafting_nodes}, we use \( 0 \) as the value of the underlying node.
