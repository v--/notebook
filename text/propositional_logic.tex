...
\begin{definition}\label{def:propositional_language}\mcite[14]{Hinman2005}
  The \term{language of propositional logic}\fnote{This is strictly speaking, an \hyperref[def:formal_language]{alphabet} rather than a \hyperref[def:formal_language/language]{formal language}. Nonetheless, this is the established terminology.} consists of:

  \begin{thmenum}
    \thmitem{def:propositional_language/constants} Two \term{propositional constants} (also known as \term{truth values}):
    \begin{thmenum}
      \thmitem{def:propositional_language/constants/verum} The \term{verum} \enquote{\( \syntop \)}.
      \thmitem{def:propositional_language/constants/falsum} The \term{falsum} \enquote{\( \synbot \)}.
    \end{thmenum}

    \thmitem{def:propositional_language/negation} \term{Negation} \enquote{\( \synneg \)}.
    \thmitem{def:propositional_language/connectives} The set \( \Sigma \) of \term{propositional binary connectives}, namely
    \begin{thmenum}
      \thmitem{def:propositional_language/connectives/disjunction} \term{Disjunction} \enquote{\( \synvee \)}, also known as \hyperref[def:standard_boolean_operators]{\term{or}} and \hyperref[def:lattice/join]{\term{join}}.
      \thmitem{def:propositional_language/connectives/conjunction} \term{Conjunction} \enquote{\( \synwedge \)}, also known as \hyperref[def:standard_boolean_operators]{\term{and}} and \hyperref[def:lattice/meet]{\term{meet}}.
      \thmitem{def:propositional_language/connectives/conditional} \term{Conditional}\fnote{Note that \enquote{conditional} and \enquote{biconditional} are nouns in this context.} \enquote{\( \synimplies \)}, also known as \term{if\ldots then} and \hyperref[def:material_implication]{\term{material implication}}.
      \thmitem{def:propositional_language/connectives/biconditional} \term{Biconditional} \enquote{\( \syniff \)}, also known as \term{iff}\fnote{\enquote{iff} is a common abbreviation for \enquote{if and only if}.} and \term{material equivalence}.
    \end{thmenum}
...
\begin{definition}\label{def:propositional_syntax}\mimprovised
  ...
  \begin{thmenum}
    ...
    \thmitem{def:propositional_syntax/prop} We denote by \( \boldop{Prop} \) the set of strings generated by the above grammar schema with starting nonterminal \( \bnfpn{variable} \).

    \thmitem{def:propositional_syntax/formula} Similarly, we denote by \( \boldop{Form} \) the set of strings generated with starting nonterminal \( \bnfpn{formula} \).
...
\begin{remark}\label{rem:propositional_formula_parentheses}
  ...
  \begin{thmenum}
    \thmitem{rem:propositional_formula_parentheses/outermost} We may skip the outermost parentheses in formulas with top-level \hyperref[def:propositional_language/connectives]{connectives}, e.g. we may write \( P \synwedge Q \) rather than \( (P \synwedge Q) \).

    \thmitem{rem:propositional_formula_parentheses/associative} Because of the associativity of \( \synwedge \) and \( \synvee \), which is implied by \fullref{def:propositional_valuation/valuation_function} and \fullref{def:standard_boolean_operators}, we may skip the parentheses in chains like
    \begin{equation*}
...
\begin{definition}\label{def:material_implication}\mcite[1]{DienerMcKubreJordens2016}
  Theorems in mathematics usually have the form \( P \synimplies Q \). Formulas of this form are called \term{material implications} in order to distinguish them from logical implication, which relates to the metatheoretic concept of \hyperref[def:propositional_semantics/entailment]{entailment}. Note that the term \enquote{material implication} sometimes also refers to the conditional connective \hyperref[def:propositional_language/connectives/conditional]{\( \synimplies \)} itself.
  ...
  \begin{thmenum}
    ...
    \thmitem{def:material_implication/inverse} The formula \( \synneg P \synimplies \synneg Q \) is the \term[bg=противоположна,ru=противоположная]{inverse} of \( \varphi \).

    \thmitem{def:material_implication/converse} The formula \( Q \synimplies P \) is the \term[bg=обратна,ru=обратная]{converse} of \( \varphi \).

    \thmitem{def:material_implication/contrapositive} The formula \( \synneg Q \synimplies \synneg P \) is the \term{contrapositive} of \( \varphi \).
  \end{thmenum}
\end{definition}
\begin{comments}
...
\begin{definition}\label{def:propositional_semantics}\mimprovised
  ...
  \begin{thmenum}
    ...
    \begin{thmenum}
      ...
      \thmitem{def:propositional_semantics/tautology/equivalence} We have the equivalence \( \varphi \gleichstark \syntop \).
    \end{thmenum}
    ...
    \begin{thmenum}
      ...
      \thmitem{def:propositional_semantics/contradiction/entailment} We have the entailment \( \varphi \vDash \synbot \).
      \thmitem{def:propositional_semantics/contradiction/equivalence} We have the equivalence \( \varphi \gleichstark \synbot \).
    \end{thmenum}
  \end{thmenum}
\end{definition}
...
\begin{definition}\label{def:cnf_and_dnf}\mcite[I.1.\S4]{Яблонский2003}
  ...
  \begin{thmenum}
    ...
    \begin{bnf*}
      ...
      \bnfprod{negative literal}     {\synneg \bnfpn{positive literal}} \\
      \bnfprod{literal}              {\bnfpn{positive literal} \bnfor \bnfpn{negative literal}} \\
      \bnfprod{disjunct}             {\bnfpn{literal} \bnfor \bnftsq{(} \bnfsp \bnfpn{literal}  \bnfsp \bnftsq{\( \synvee \)}   \bnfsp \bnfpn{disjunct} \bnfsp \bnftsq{)}} \\
      \bnfprod{CNF}                  {\bnfpn{CNF}     \bnfor \bnftsq{(} \bnfsp \bnfpn{disjunct} \bnfsp \bnftsq{\( \synwedge \)} \bnfsp \bnfpn{CNF}      \bnfsp \bnftsq{)}} \\
      \bnfprod{conjunct}             {\bnfpn{literal} \bnfor \bnftsq{(} \bnfsp \bnfpn{literal}  \bnfsp \bnftsq{\( \synwedge \)} \bnfsp \bnfpn{conjunct} \bnfsp \bnftsq{)}} \\
      \bnfprod{DNF}                  {\bnfpn{DNF}     \bnfor \bnftsq{(} \bnfsp \bnfpn{conjunct} \bnfsp \bnftsq{\( \synvee \)}   \bnfsp \bnfpn{DNF}      \bnfsp \bnftsq{)}}
    \end{bnf*}
    ...
    \begin{thmenum}
      \thmitem{def:cnf_and_dnf/perfect/fullness} Every disjunct contains \( n \) literals and the \( k \)-th literal is either \( P_k \) or \( \synneg P_k \).
...
\begin{example}\label{ex:def:cnf_and_dnf}
  ...
  \begin{thmenum}
    \thmitem{def:cnf_and_dnf/perfect_cnf} The \hyperref[thm:boolean_equivalences]{Boolean equivalence} \eqref{eq:thm:boolean_equivalences/conditional_as_disjunction} allows us to convert the \hyperref[def:propositional_language/connectives/conditional]{conditional} \( P \to Q \) to \( \synneg P \synvee Q \), which is both in CNF and in DNF.
...
\begin{algorithm}[Perfect CNFs and DNFs]\label{alg:perfect_cnf_and_dnf}\mcite[thm. I.1.3]{Яблонский2003}
  ...
  \begin{thmenum}
    ...
    \begin{equation}\label{alg:perfect_cnf_and_dnf/dnf}
      \dot\bigvee_{f(x_1, \ldots, x_n) = T} P_1^{\oline{x_1}} \synwedge \cdots \synwedge P_n^{\oline{x_n}},
    \end{equation}
  \end{thmenum}
\end{algorithm}
\begin{defproof}
  ...
  \begin{center}
    \begin{tabular}{c c | c}
      \( x_k \) & \( a_k \) & \( P_k^{\oline{x_k}}\Bracks{a_k} \) \\
      \hline
      \( F \)   & \( F \)   & \( T \) \\
      \( F \)   & \( T \)   & \( F \) \\
      \( T \)   & \( F \)   & \( F \) \\
  ...
  Therefore, \( P_k^{\oline{x_1}}\Bracks{a_k} = T \) if and only if \( x_k = a_k \). Then
  \begin{equation*}
    \parens*{ P_1^{\oline{x_1}} \synvee \cdots \synvee P_n^{\oline{x_1}} }\bracks{ a_1, \ldots, a_n }
    =
    \begin{cases}
  ...
  \begin{equation*}
    \parens*{ \dot\bigvee_{(x_1, \ldots, x_n) \in B} P_1^{\oline{x_1}} \synvee \cdots \synvee P_n^{\oline{x_n}} }\bracks{ a_1, \ldots, a_n } = T
  \end{equation*}
