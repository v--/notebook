...
\begin{concept}\label{con:reserved_symbol}
  \hyperref[con:logical_system]{Logical systems} are generally designed to encode multiple \hyperref[con:metalogic]{object languages}\fnote{An exception is the syntax of propositional logic, which we define in \fullref{def:propositional_syntax}. It provides only a way to study logical relations between \hyperref[con:proposition]{propositions}, irrespective of how these propositions are structured.}.

  A particular object language may require additional symbols. We call a \term[en=signature (\cite[98]{GoguenBurstall1992Institutions})]{signature} the collection of symbols specific to an object language but not the \hyperref[con:logical_system]{logical system} used to describe it.

  For example, \hyperref[def:peano_arithmetic]{Peano arithmetic} as expressed in the \hyperref[def:first_order_syntax]{syntax of first-order logic}, requires the symbols \( \syn0 \), \( \syns \), \( \synplus \) and \( \syncirc \) in order to express the axioms \ref{eq:def:peano_arithmetic/PA6} and \ref{eq:def:peano_arithmetic/PA7} for multiplication:
  \begin{align*}
    \synx \syncirc \syn0        &\syneq \syn0,                    \\
    \synx \syncirc \syns(\syny) &\syneq \synx \syncirc \syny + \synx
  \end{align*}

  The equality symbol \( {\syneq} \) is not part of the signature of Peano arithmetic because it is common for all languages expressible in first-order logic. We will call such symbols \term{reserved}.

  To avoid \hyperref[def:grammar_ambiguity]{syntactic ambiguity}, we generally disallow symbols in the signature to clash with reserved symbols. There is an exception for \hyperref[con:variable]{variables}, however. We generally have an unbounded supply of variables because they follow the identifier rules from \fullref{def:variable_identifier}. We allow non-reserved symbols to take precedence over variables. For example, in \fullref{def:simply_typed_hol_syntax}, we allow \( \syn\iota \) and \( \syn\omicron \) as base types, even though they would otherwise be considered variables.
\end{concept}
\begin{comments}
  \item The term \enquote{reserved} comes from programming languages, where certain words are marked as reserved and cannot be used for naming. For example, \cite[\S 6.4.1]{ISO:9899:2018} lists several dozens of words that are called \enquote{reserved} and cannot be used as identifiers in the programming language C.

  \item It is common to call reserved symbols \enquote{logical}, but this is unfortunately ambiguous --- see \fullref{rem:logical_symbol_terminology}. Still, with some disambiguation, we find useful the adjectives \enquote{logical} and \enquote{non-logical} when discussing simply typed higher-order logic in \fullref{sec:predicate_logic}.
...
\begin{remark}\label{rem:logical_symbol_terminology}
  It is common to call \hyperref[con:reserved_symbol]{reserved symbols} \enquote{logical}, distinguishing them from \enquote{nonlogical} symbols that appear in a signature. This terminology is unfortunately ambiguous, as discussed in \cite{MathSE:are_variables_logical_symbols}. For example, for the \hyperref[def:first_order_syntax]{syntax of first-order logic}, the terms are used as follows:
  \begin{itemize}
    \item \incite[178]{Gentzen1935LogischeSchließen}, when presenting first-order logic, calls \enquote{Logische Zeichen} (\enquote{logical symbols}) the \hyperref[def:propositional_alphabet/negation]{negation}, \hyperref[def:propositional_alphabet/connectives]{propositional connectives} and \hyperref[con:quantifier]{quantifiers}.
    ...
    \begin{itemize}
      ...
      \item \hyperref[con:variable]{Variables}, separated into \hyperref[def:variable_binding]{free} object variables, bound object variables and propositional variables.
    \end{itemize}
    ...
    \item \incite*[102]{BoolosEtAl2002ComputabilityAndLogic} further extends the list of logical symbols with variables.
...
\begin{definition}\label{def:propositional_alphabet}\mcite[4; 5]{Smullyan1995FOL}
  The \hyperref[def:formal_language/alphabet]{alphabet} of \hyperref[con:reserved_symbol]{reserved symbols} of propositional logic consists of the following:
...
\begin{definition}\label{def:propositional_syntax}\mimprovised
  ...
  \begin{thmenum}
    ...
    \end{bnf*}
    where we have used the connectives from \fullref{def:propositional_alphabet/connectives} and the variable identifier rules from \fullref{def:variable_identifier}.
    ...
    We will also call \term{sentences} in relation to \hyperref[def:abstract_logic]{abstract logics}. This contrasts with first-order logic, where only specific formulas are called sentences --- see \fullref{def:first_order_syntax/closed_formula}.
  \end{thmenum}
\end{definition}
\begin{comments}
...
\begin{definition}\label{def:propositional_semantics}\mimprovised
  ...
  \begin{thmenum}
    ...
    \thmitem{def:propositional_semantics/fuzzy} If the Heyting algebra is the unit interval \( [0, 1] \), as in \fullref{ex:def:heyting_algebra/totally_ordered}, we obtain \term{fuzzy semantics}, more often called \term[en=fuzzy logic (\cite[17]{Rosen2019DiscreteMathematics})]{fuzzy logic}.
  \end{thmenum}
\end{definition}
\begin{comments}
...
\end{definition}
\begin{comments}
  \item In the English translation of Tarski's \cite[\enquote{On the Concept of Logical Consequence}, page 419]{Woodger1983TarskiPapers}, \enquote{tautology} is transiently described as \enquote{a statement which \enquote*{says nothing about reality}}.
\end{comments}
...
\begin{comments}
  ...
  Unless we are specifically interested in \( \synbot \) and \( \synneg \), a more reasonable approach may be to only consider the \hyperref[con:syntax_fragment]{fragment} of propositional logic without them. This is approach to minimal logic mentioned in \mcite[49]{Mimram2020ProgramEqualsProof}, and this is the approach used in \fullref{def:minimal_implication_logic}.
