...
\begin{comments}
  ...
  \item A modification of the propositional denotation is needed for \hyperref[con:minimal_logic]{minimal logic} --- see \cref{def:minimal_propositional_denotation}.
...
\begin{concept}\label{con:improper_symbol}
  In \cref{def:propositional_denotation}, we have defined denotations for \hyperref[def:propositional_formula]{propositional formulas}. Based on this definition, the symbols used to define the formal grammar of formulas can be divided into two collections:
  \begin{thmenum}
...
\begin{remark}\label{rem:minimal_semantics_and_falsum}
  ...
  What matters to us is that, in minimal logic, the falsum \( \synbot \) plays no special role that otherwise allows principles like \fullref{thm:propositional_semantic_efq}.

  To achieve this semantically, in \cref{def:minimal_propositional_semantics}, we will simply interpret \( \synbot \) as a \hyperref[def:propositional_variable]{propositional variable}. This also requires us to modify denotations as defined in \cref{def:propositional_denotation}.

  Therefore, for minimal semantics, we will modify the denotation, not the underlying \hyperref[def:truth_value_algebra]{truth value algebra}.
...
\begin{definition}\label{def:minimal_propositional_denotation}\mimprovised
  In accordance with \cref{rem:minimal_semantics_and_falsum}, we will adapt the propositional denotation from \cref{def:propositional_denotation} for \hyperref[con:minimal_logic]{minimal logic}. We treat \( \synbot \) as a propositional variable and presume that \( I \) is defined for \( \synbot \).

  We define the \term{minimal denotation} as follows:
  \begin{empheq}[left={\Bracks{\varphi}_I} \coloneqq \empheqlbrace]{align}
    &I(\synbot),                             &&\varphi = \synbot,      \label{eq:def:minimal_propositional_denotation/bot} \\
    &\Bracks{\psi}_I \rightarrow I(\synbot), &&\varphi = \synneg \psi, \label{eq:def:minimal_propositional_denotation/neg} \\
    &\vdots,                                 && \notag
  \end{empheq}
  the other cases being the same as in \cref{def:propositional_denotation}.

  We will refer to semantics using this denotation as \term{minimal semantics}. See \cref{def:propositional_semantics} for a broader discussion.
\end{definition}
...
\begin{definition}\label{def:propositional_institution}\mimprovised
  ...
  \begin{thmenum}
    ...
    \thmitem{def:propositional_institution/models} Let \( \BbbI \) be the discrete category on the \hyperref[def:set_of_all_functions]{function set} \( \fun(\op*{Prop}, \BbbT) \), i.e. the set of all propositional interpretations. The \hyperref[def:institution/models]{model functor} can then be described as \( \anon \mapsto \BbbI \).

    As discussed in \cref{rem:minimal_semantics_and_falsum}, for minimal logic, we will regard \( \synbot \) as a propositional variable.
    ...
    Here \( \Bracks{\varphi}_I \) is the propositional denotation from \cref{def:propositional_denotation}. For minimal logic, we will also need the modifications from \cref{def:minimal_propositional_denotation}.
  \end{thmenum}
\end{definition}
\begin{comments}
...
\begin{definition}\label{def:propositional_semantics}\mimprovised
  ...
  We will label the semantics \enquote{classical}, \enquote{intuitionistic}, etc., based on \( \BbbT \), with \enquote{minimal semantics} using the modified denotation from \cref{def:minimal_propositional_denotation}.
...
\begin{remark}\label{rem:classical_propositional_interpretations}
  Note that a propositional interpretation may be used to define denotations different from the one in \cref{def:propositional_denotation}. For example, the semantics for \hyperref[con:minimal_logic]{minimal logic} defined in \cref{def:minimal_propositional_denotation} require a slight variation.
...
\begin{proposition}\label{thm:propositional_denotation_characterization}
  \hyperref[def:propositional_satisfaction]{Propositional satisfaction} of compounds formulas can be characterized as follows:
  \begin{thmenum}
    \thmitem{thm:propositional_denotation_characterization/negation} Assuming \hyperref[def:truth_value_algebra/degenerate]{non-degenerate intuitionistic semantics}, from \( I \vDash \varphi \) it follows that \( I \nvDash \synneg \varphi \).

    The converse holds in \hyperref[def:truth_value_algebra/classical]{classical semantics}.

    \thmitem{thm:propositional_denotation_characterization/conjunction} We have \( I \vDash \varphi \synwedge \psi \) if and only if \( I \vDash \varphi \) and \( I \vDash \psi \).

    \thmitem{thm:propositional_denotation_characterization/disjunction} If \( I \vDash \varphi \) or \( I \vDash \psi \) (or both), it follows that \( I \vDash \varphi \synvee \psi \).

    The converse holds in classical semantics.

    \thmitem{thm:propositional_denotation_characterization/conditional} From \( I \vDash \varphi \synimplies \psi \) it follows that \( I \vDash \varphi \) implies \( I \vDash \psi \).

    The converse holds in classical semantics.

    \thmitem{thm:propositional_denotation_characterization/biconditional} We have \( I \vDash \varphi \syniff \psi \) if and only if \( I \vDash \varphi \) is equivalent to \( I \vDash \psi \).
  \end{thmenum}
\end{proposition}
\begin{proof}
  \SubProofOf{thm:propositional_denotation_characterization/negation} Assume non-degenerate intuitionistic semantics.
  ...
  \SubProofOf{thm:propositional_denotation_characterization/conjunction} We have
  \begin{equation*}
  ...
  \SubProofOf{thm:propositional_denotation_characterization/disjunction} We have
  \begin{equation*}
  ...
  \SubProofOf{thm:propositional_denotation_characterization/conditional} \Cref{thm:def:heyting_algebra/leq} implies that \( \Bracks{\varphi \synimplies \psi}_I = \semtop \) if and only if \( \Bracks{\varphi}_I \leq \Bracks{\psi}_I \).
  ...
  \SubProofOf{thm:propositional_denotation_characterization/biconditional} \Cref{thm:def:heyting_algebra/eq} implies that \( \Bracks{\varphi \syniff \psi}_I = \semtop \) if and only if \( \Bracks{\varphi}_I = \Bracks{\psi}_I \).
\end{proof}
...
\begin{proposition}\label{thm:minimal_semantics_is_paraconsistent}
  \hyperref[def:minimal_propositional_denotation]{Minimal propositional semantics} is \hyperref[def:paraconsistent_consequence_operator]{paraconsistent}.
\end{proposition}
\begin{proof}
...
\begin{proposition}\label{thm:semantic_propositional_conjunction_of_premises}
  With respect to \hyperref[def:minimal_propositional_denotation]{minimal semantics}, we have \( \varphi_1, \ldots, \varphi_n \vDash \psi \) if and only if \( (\varphi_1 \synwedge \cdots \synwedge \varphi_n) \vDash \psi \).
\end{proposition}
\begin{proof}
...
\begin{theorem}[Propositional semantic deduction theorem]\label{thm:propositional_semantic_deduction_theorem}
  With respect to \hyperref[def:minimal_propositional_denotation]{minimal semantics}, for an arbitrary \hyperref[def:truth_value_algebra]{truth value algebra} and for arbitrary propositional formulas, we have
  \begin{equation*}
...
\begin{corollary}\label{thm:intuitionistic_deduction_consequences}
  ...
  \begin{thmenum}
    \thmitem{thm:intuitionistic_deduction_consequences/top} Under \hyperref[def:minimal_propositional_denotation]{minimal semantics}, \( \varphi \vDash \syntop \) for every formula \( \varphi \).
...
\begin{example}\label{ex:def:propositional_semantics}
  ...
  \begin{thmenum}
    ...
    Let \( I \) be a \hyperref[def:propositional_denotation]{propositional interpretation} such that \( I(\synp) = \mathbfsf{N} \). Then the denotation of \eqref{eq:thm:classical_tautologies/lem} is
    \begin{equation*}
...
\begin{proposition}\label{thm:intuitionistic_equivalences}
  ...
  \begin{thmenum}
    ...
    This equivalence is used when defining minimal semantics in \cref{def:minimal_propositional_denotation}.
...
\begin{comments}
  ...
  \item This principle is useful as a \hyperref[def:inference_rule]{inference rule} --- see \cref{inf:def:propositional_natural_deduction_systems/bot/efq}.
...
\begin{comments}
  ...
  \item This principle is useful as a \hyperref[def:inference_rule]{inference rule} --- see \cref{inf:def:propositional_natural_deduction_systems/bot/raa}.
  ...
  He introduces this principle in order to give a name for the inference rule \cref{inf:def:propositional_natural_deduction_systems/bot/raa}.
...
\end{comments}
\begin{proof}
  If \( I \nvDash \varphi \), \cref{thm:propositional_denotation_characterization/negation} implies that \( I \vDash \synneg \varphi \).
...
\begin{proof}
  ...
  Otherwise, if \( I \nvDash \synneg \varphi \), \cref{thm:propositional_denotation_characterization/negation} implies that \( I \vDash \synneg \synneg \varphi \), and \fullref{thm:propositional_semantic_dne} implies that \( I \vDash \varphi \).
\end{proof}
