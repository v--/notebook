...
\begin{definition}\label{def:tree}\mcite[thm. 1.5.1]{Diestel2017GraphTheory}
  ...
  \begin{thmenum}[resume=def:tree]
    \thmitem{def:tree/morphism}\mimprovised We say that a function between tree nodes is a \term{tree homomorphism} if it preserves adjacency, i.e. if it is an \hyperref[def:undirected_graph/morphism]{undirected graph homomorphisms}.

    \thmitem{def:tree/category} \hyperref[def:small_set]{Small} trees and their homomorphisms naturally form a \hyperref[def:category]{category}.

    \thmitem{def:tree/subobject}\mcite[def. 5.9]{БелоусовТкачёв2004ДискретнаяМатематика} We say that the tree \( S \) is a \term{subtree} of \( T \) if it is a \hyperref[def:undirected_graph/subobject]{subgraph} of \( T \).

    It follows from \cref{thm:subgraphs} that subtrees correspond to \hyperref[def:categorical_subobject]{categorical subobjects}.
  \end{thmenum}
\end{definition}
\begin{defproof}
...
\begin{definition}\label{def:rooted_tree}\mcite[783]{Rosen2019DiscreteMathematics}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:rooted_tree/node_height}\mcite[15]{Diestel2017GraphTheory} The \term{height} of a node \( v \) is the length of the unique path from \( r \).

    \thmitem{def:rooted_tree/height}\mcite[790]{Rosen2019DiscreteMathematics} The \term{height} of \( T \) is the maximal height of all vertices.
  ...
  \begin{thmenum}[resume=def:rooted_tree]
    \thmitem{def:rooted_tree/morphism}\mimprovised We say that a function between rooted tree nodes is a \term{rooted tree homomorphism} if it is a \hyperref[def:tree/morphism]{tree homomorphism} that additionally preserves the root.

    \thmitem{def:rooted_tree/category} \hyperref[def:small_set]{Small} rooted trees and their homomorphisms naturally form a \hyperref[def:category]{category}.

    \thmitem{def:rooted_tree/subtree}\mcite[784]{Rosen2019DiscreteMathematics} For a fixed node \( v \) of the rooted tree \( T \), consider the \hyperref[def:tree/subobject]{subtree} \( T_{\geq v} \) of descendants of \( v \), with \( v \) as the root. We call it the subtree \term{rooted at} \( v \).

    Note that this is a separate notion from \hyperref[def:categorical_subobject]{categorical subobjects}, which would instead be subtrees containing the root \( r \) of \( T \). In fact, we will have no interest in such subobjects.

    If \( v \) is a child of \( r \), we say that its subtree is \term[en=subtree of the root (\cite[308]{Stanley2012EnumerativeCombinatoricsVol1})]{immediate}.
  \end{thmenum}
\end{definition}
\begin{comments}
  \item \Textcite[\S 9.2.1]{Новиков2013ДискретнаяМатематика} calls a \enquote{rooted tree} (\enquote{корневое дерево}) what we call an \hyperref[def:oriented_tree]{oriented tree}. The difference is inessential due to \cref{thm:oriented_and_rooted_trees}.
\end{comments}
...
\begin{comments}
  ...
  \item \Textcite[def. 5.6]{БелоусовТкачёв2004ДискретнаяМатематика} require oriented trees to be acyclic rather than well-founded, however that definition leads to pathological infinite oriented trees --- see \cref{ex:infinite_oriented_tree}.

  \item Another possible convention for oriented trees is to place the same restrictions on out-degrees rather than in-degrees, which essentially reverses the directions of the arcs. This is the approach taken by \textcite[373]{Knuth1997ArtVol1}.
\end{comments}
...
\begin{example}\label{ex:infinite_oriented_tree}
  ...
  If we were to follow \cite[def. 5.6]{БелоусовТкачёв2004ДискретнаяМатематика} and require oriented trees to be merely acyclic, then \eqref{eq:ex:infinite_oriented_tree} would satisfy the definition. For finite graphs acyclicity is sufficient due to \cref{thm:def:well_founded_graph/finite_acyclic}.
...
\begin{theorem}[Induction on rooted trees]\label{thm:induction_on_rooted_trees}
...
\end{theorem}
\begin{comments}
  \item We will mostly use this induction principle by way of \fullref{thm:induction_on_abstract_syntax}, but also occasionally use it directly, e.g. for \hyperref[def:propositional_natural_deduction_proof_tree]{natural deduction proof trees} in \cref{thm:propositional_natural_deduction_soundness}.

  \item We will usually deal with only certain kinds of rooted trees, for example \hyperref[con:abstract_syntax_tree]{abstract syntax trees} or \hyperref[def:proof_tree]{proof trees}. In this case the statement should be conditional on the tree \( T \) being an AST or proof tree.
\end{comments}
\begin{proof}
...
\begin{definition}\label{def:ordered_tree}\mimprovised
  ...
  \begin{thmenum}
    \thmitem{def:ordered_tree/morphism} We say that a function between ordered tree nodes is an \term{ordered tree homomorphism} if it is a \hyperref[def:order_function/preserving]{strictly order-preserving} \hyperref[def:rooted_tree/morphism]{rooted tree homomorphism}.

    \thmitem{def:ordered_tree/category} \hyperref[def:small_set]{Small} ordered trees and their homomorphisms naturally form a \hyperref[def:category]{category}.
  \end{thmenum}
\end{definition}
\begin{comments}
  ...
  \item \Textcite[\S 2.3]{Holtkamp2011RootedTrees} uses \enquote{planar rooted tree} as a synonym for \enquote{ordered tree}, while \cite[573]{Stanley2012EnumerativeCombinatoricsVol1} uses \enquote{plane tree} as a synonym.
\end{comments}
...
\begin{definition}\label{def:ordered_tree_grafting}\mimprovised
  ...
  \begin{thmenum}
    \thmitem{def:ordered_tree_grafting/union} Consider the \hyperref[def:graph_disjoint_union]{graph disjoint union} \( \parens[\big]{ \coprod_{k \in \mscrK} T_k } \amalg T \), with the \hyperref[thm:order_category_isomorphism_properties/discrete_limits]{disjoint union partial order} on its nodes.
...
\begin{definition}\label{def:nary_tree}\mcite[def. 11.1.3]{Rosen2019DiscreteMathematics}
  We say that the \hyperref[def:ordered_tree]{ordered tree} is \term{\( n \)-ary} if each node has \emph{at most} \( n \) children and \term{full \( n \)-ary} if each internal node has \emph{exactly} \( n \) children.
...
\begin{definition}\label{def:ordered_tree_enumeration}\mimprovised
  Fix a finite \hyperref[def:ordered_tree]{ordered tree} \( T = (V, E, r, \leq) \). Let \( R_1, \ldots, R_n \) be the immediate subtrees of \( T \). We will define two \hyperref[def:set_enumeration]{enumeration} of \( V \) --- the \term{pre-order enumeration}\fnote{This concept is not related to \hyperref[def:preordered_set]{preordered sets}.}
  ...
  Additionally, if \( T \) is a \hyperref[def:nary_tree]{full binary tree}, we can also define the \term{in-order enumeration}
...
\begin{algorithm}[Finite ordered tree to binary tree]\label{alg:finite_ordered_tree_to_binary_tree}
  For every finite \hyperref[def:ordered_tree]{ordered tree} \( T \), we will build a \hyperref[def:nary_tree]{binary tree} \( B \) that has the same \hyperref[eq:def:ordered_tree_enumeration/pre]{pre-order enumeration}.
...
\begin{definition}\label{def:labeled_tree}\mimprovised
  ...
  \begin{thmenum}
    \thmitem{def:labeled_tree/morphism} We say that a function between \( L \)-labeled tree nodes is an \term{labeled tree homomorphism} if it is a \hyperref[def:ordered_tree/morphism]{ordered tree homomorphism} that additionally preserves labels.

    \thmitem{def:labeled_tree/category} \hyperref[def:small_set]{Small} labeled trees and their homomorphisms naturally form a \hyperref[def:category]{category}.
  \end{thmenum}
\end{definition}
\begin{comments}
  ...
  \item For \hyperref[rem:graph_of_unspecified_type]{general graphs} and even \hyperref[def:tree]{general trees} we allow both vertices and arcs/edges to be labeled, however in the case of ordered trees we are mostly interested in the vertices.

  We have chosen the term \enquote{labeled tree} for brevity; \textcite[471]{Mimram2020ProgramEqualsProof} uses the same term for generic trees with labeled nodes, while \textcite[\S 2.8]{Holtkamp2011RootedTrees} uses it for rooted trees with labeled nodes.
\end{comments}

\begin{definition}\label{def:canonical_singleton_tree}\mimprovised
  For every possible \hyperref[def:labeled_set]{label} \( l \), there corresponds the tree obtained by \hyperref[def:ordered_tree_grafting]{grafting} zero subtrees to a new node labeled by \( l \). We call this the \term{canonical singleton tree} labeled by \( l \).
\end{definition}
\begin{comments}
  \item The node itself (without the label) is fixed to be \( 0 \) as discussed in \cref{rem:tree_grafting_nodes}.
\end{comments}
