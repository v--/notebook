...
\begin{definition}\label{def:tree}\mcite[thm. 1.5.1]{Diestel2005Graphs}
  We call the (possibly infinite) \hyperref[def:undirected_graph]{simple undirected graph} \( T = (E, V) \) a \term[bg=дърво (\cite[21]{Мирчев2001Графи}), ru=дерево (\cite[53]{Емеличев1990Графы})]{tree} if any of the following equivalent conditions hold:
  ...
  \begin{thmenum}[resume=def:tree]
    ...
    \thmitem{def:tree/subtree}\mcite[649]{Rosen1999DiscreteHandbook} We say that the tree \( R \) is a \term{subtree} of \( T \) if it is a \hyperref[def:undirected_graph/subgraph]{subgraph} of \( T \).
  \end{thmenum}
\end{definition}
\begin{comments}
...
\begin{definition}\label{def:rooted_tree}\mcite[15]{Diestel2005Graphs}
  A \term[ru=ориентированное дерево (\cite[323]{Емеличев1990Графы}); корневое дерево (\cite[\S 9.2.1]{Новиков2013ДискретнаяМатематика})]{rooted tree} is a triple \( T = (V, E, r) \), where \( (V, E) \) is a \hyperref[def:tree]{tree} and \( r \) is a distinguished node, called the \term{root}.
  ...
  \begin{thmenum}
    ...
    \thmitem{def:rooted_tree/ancestor_descendant}\mcite[637]{Rosen1999DiscreteHandbook} If \( u \leq v \), we say that \( u \) is an \term[ru=предок (\cite[298]{БелоусовТкачёв2004ДискретнаяМатематика})]{ancestor} of \( v \) and that \( v \) is a \term[ru=потомок (\cite[298]{БелоусовТкачёв2004ДискретнаяМатематика})]{descendant} of \( u \).

    \thmitem{def:rooted_tree/parent_child}\mcite[637; 639]{Rosen1999DiscreteHandbook} If \( u \) is the penultimate element in the path from \( r \) to \( v \), we say that \( u \) is a \term[ru=отец (\cite[298]{БелоусовТкачёв2004ДискретнаяМатематика})]{parent} of \( v \) and that \( v \) is a \term[ru=сын (\cite[298]{БелоусовТкачёв2004ДискретнаяМатематика})]{child} of \( u \).

    \thmitem{def:rooted_tree/leaf}\mcite[638]{Rosen1999DiscreteHandbook} If a node has no children, we call it a \term[ru=лист (\cite[298]{БелоусовТкачёв2004ДискретнаяМатематика})]{leaf node}.

    \thmitem{def:rooted_tree/siblings}\mcite[639]{Rosen1999DiscreteHandbook} If \( u \) and \( v \) has the same parent, we call them \term{siblings}.
...
\begin{definition}\label{def:ordered_tree}\mimprovised
  An \term[ru=упорядоченное дерево (\cite[\S 9.3.5]{Новиков2013ДискретнаяМатематика}), en=ordered tree (\cite[639]{Rosen1999DiscreteHandbook})]{ordered tree} is a quadruple \( T = (V, E, r, \leq) \), where \( (V, E, r) \) is a \hyperref[def:rooted_tree]{rooted tree} and \( {\leq} \) is a \hyperref[def:partially_ordered_set]{partial order}, called the \term{sibling order} or \term{horizontal order}, such that the children of every node are \hyperref[def:totally_order_chain]{totally ordered}, while non-siblings are pairwise-incomparable.
...
\end{definition}
\begin{comments}
  \item \incite[639]{Rosen1999DiscreteHandbook} provides a similar definition, however he doesn't explicitly deal with non-sibling elements. We want to be explicit about this or order to strictly differentiate between vertical and horizontal ordering.

  \item \incite[\S 2.3]{Holtkamp2011RootedTrees} uses \enquote{planar rooted tree} as a synonym for \enquote{ordered tree}.
\end{comments}
...
\begin{remark}\label{rem:tree_grafting_nodes}
  When building \hyperref[def:ordered_tree]{ordered trees} recursively, for example in \fullref{def:propositional_syntax/formula_ast}, \fullref{def:lambda_term_ast} or \fullref{def:natural_deduction_proof_tree}, we start with singleton trees and then graft existing trees onto new roots.
...
\begin{definition}\label{def:n_ary_tree}\mcite[642]{Rosen1999DiscreteHandbook}
...
\begin{comments}
  ...
  We have chosen the term \enquote{labeled tree} for brevity; \incite[471]{Mimram2020Types} and Paul Stockmeyer in \cite[655]{Rosen1999DiscreteHandbook} use the same term for generic trees with labeled nodes, while \incite[\S 2.8]{Holtkamp2011RootedTrees} uses it for rooted trees with labeled nodes.
\end{comments}
