...
\begin{definition}\label{def:tree}\mcite[thm. 1.5.1]{Diestel2017GraphTheory}
  ...
  \begin{thmenum}[resume=def:tree]
    ...
    \thmitem{def:tree/subtree}\mcite[def. 5.9]{БелоусовТкачёв2004ДискретнаяМатематика} We say that the tree \( S \) is a \term{subtree} of \( T \) if it is a \hyperref[def:undirected_graph/subobject]{subgraph} of \( T \).
  \end{thmenum}
\end{definition}
\begin{defproof}
...
\begin{definition}\label{def:rooted_tree}\mcite[783]{Rosen2019DiscreteMathematics}
  ...
  \begin{thmenum}[resume=def:rooted_tree]
    \thmitem{def:rooted_tree/subtree}\mcite[784]{Rosen2019DiscreteMathematics} A \term{rooted subtree} with root \( s \) is the \hyperref[def:tree/subtree]{subtree} consisting of \( s \) and all its descendants. If \( s \) is a child of \( r \), we say that its subtree is \term[en=subtree of the root (\cite[308]{Stanley2012EnumerativeCombinatoricsVol1})]{immediate}.
