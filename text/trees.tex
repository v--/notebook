...
\begin{definition}\label{def:ordered_tree_grafting}\mimprovised
  ...
  \begin{thmenum}
    \thmitem{def:ordered_tree_grafting/union} Consider the \hyperref[def:graph_disjoint_union]{graph disjoint union} \( \parens[\Big]{ \coprod_{k \in \mscrK} T_k } \coprod T \), with the \hyperref[thm:order_category_isomorphism_properties/coproduct]{disjoint union partial order} on its nodes.
...
\begin{remark}\label{rem:tree_grafting_nodes}
  ...
  We choose \( 0 \) as a \enquote{canonical} node for singleton trees. The actual nodes do not matter when the nodes are \hyperref[def:labeled_set]{labeled}.
