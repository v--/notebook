...
\begin{definition}\label{def:tree}\mcite[thm. 1.5.1]{Diestel2017Graphs}
  ...
  \begin{thmenum}[resume=def:tree]
    ...
    \thmitem{def:tree/subtree}\mcite[696]{Rosen2018DiscreteHandbook} We say that the tree \( R \) is a \term{subtree} of \( T \) if it is a \hyperref[def:undirected_graph/subgraph]{subgraph} of \( T \).
  \end{thmenum}
\end{definition}
\begin{comments}
...
\begin{definition}\label{def:rooted_tree}\mcite[15]{Diestel2017Graphs}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:rooted_tree/ancestor_descendant}\mcite[698]{Rosen2018DiscreteHandbook} If \( u \leq v \), we say that \( u \) is an \term[ru=предок (\cite[298]{БелоусовТкачёв2004ДискретнаяМатематика})]{ancestor} of \( v \) and that \( v \) is a \term[ru=потомок (\cite[298]{БелоусовТкачёв2004ДискретнаяМатематика})]{descendant} of \( u \).

    \thmitem{def:rooted_tree/parent_child}\mcite[698]{Rosen2018DiscreteHandbook} If \( u \) is the penultimate element in the path from \( r \) to \( v \), we say that \( u \) is a \term[ru=отец (\cite[298]{БелоусовТкачёв2004ДискретнаяМатематика})]{parent} of \( v \) and that \( v \) is a \term[ru=сын (\cite[298]{БелоусовТкачёв2004ДискретнаяМатематика})]{child} of \( u \).

    \thmitem{def:rooted_tree/leaf}\mcite[698]{Rosen2018DiscreteHandbook} If a node has no children, we call it a \term[ru=лист (\cite[298]{БелоусовТкачёв2004ДискретнаяМатематика})]{leaf node}.

    \thmitem{def:rooted_tree/siblings}\mcite[698]{Rosen2018DiscreteHandbook} If \( u \) and \( v \) has the same parent, we call them \term{siblings}.
...
\begin{definition}\label{def:ordered_tree}\mimprovised
  An \term[ru=упорядоченное дерево (\cite[\S 9.3.5]{Новиков2013ДискретнаяМатематика}), en=ordered tree (\cite[698]{Rosen2018DiscreteHandbook})]{ordered tree} is a quadruple \( T = (V, E, r, \leq) \), where \( (V, E, r) \) is a \hyperref[def:rooted_tree]{rooted tree} and \( {\leq} \) is a \hyperref[def:partially_ordered_set]{partial order}, called the \term{sibling order} or \term{horizontal order}, such that the children of every node are \hyperref[def:totally_order_chain]{totally ordered}, while non-siblings are pairwise-incomparable.
...
\begin{definition}\label{def:n_ary_tree}\mcite[699]{Rosen2018DiscreteHandbook}
...
\begin{comments}
  ...
  We have chosen the term \enquote{labeled tree} for brevity; \incite[471]{Mimram2020Types} uses the same term for generic trees with labeled nodes, while \incite[\S 2.8]{Holtkamp2011RootedTrees} uses it for rooted trees with labeled nodes.
\end{comments}
