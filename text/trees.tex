...
\begin{definition}\label{def:tree}\mcite[thm. 1.5.1]{Diestel2005}
  We call the (possibly infinite) \hyperref[def:undirected_graph]{simple undirected graph} \( T = (E, V) \) a \term[bg=дърво (\cite[21]{Мирчев2001}), ru=дерево (\cite[159]{Зыков2004})]{tree} if any of the following equivalent conditions hold:
...
\begin{definition}\label{def:rooted_tree}\mcite[15]{Diestel2005}
  A \term[ru=корневое дерево (\cite[\textnumero 9.2.1]{Новиков2013})]{rooted tree} is a triple \( T = (V, E, r) \), where \( (V, E) \) is a \hyperref[def:tree]{tree} and \( r \) is a distinguished node, called the \term{root}.
...
\end{definition}
\begin{comments}
  \item \incite[\textnumero 9.2.1]{Новиков2013} calls a \enquote{rooted tree} (\enquote{корневое дерево}) what we call an \hyperref[def:oriented_tree]{oriented tree}. The difference is inessential due to \fullref{thm:oriented_and_rooted_trees}.
\end{comments}
...
\begin{definition}\label{def:ordered_tree}\mimprovised
  An \term[ru=упорядоченное дерево (\cite[\textnumero 9.3.5]{Новиков2013}), en=ordered tree (\cite[639]{Rosen1999})]{ordered tree} is a quadruple \( T = (V, E, r, \leq) \), where \( (V, E, r) \) is a \hyperref[def:rooted_tree]{rooted tree} and \( {\leq} \) is a \hyperref[def:partially_ordered_set]{partial order}, called the \term{sibling order} or \term{horizontal order}, such that the children of every node are \hyperref[def:totally_order_chain]{totally ordered}, while non-siblings are pairwise-incomparable.
...
\begin{comments}
  ...
  \item \incite[\textnumero 2.3]{Holtkamp2011} uses \enquote{planar rooted tree} as a synonym for \enquote{ordered tree}.
\end{comments}

\begin{definition}\label{def:ordered_tree_grafting}\mimprovised
  For every \hyperref[def:totally_ordered_set]{totally ordered set} \( \mscrK \) and every \( \mscrK \)-indexed family \( \seq{ T_k }_{k \in \mscrK} \) of \hyperref[def:ordered_tree]{ordered trees} with roots \( \seq{ r_k }_{k \in \mscrK} \), we define the \term{grafting product} \( B_+(\seq{ T_k }_{k \in \mscrK}) \) as follows:
  \begin{thmenum}
    \thmitem{def:ordered_tree_grafting/union} Consider the \hyperref[def:graph_disjoint_union]{graph disjoint union} \( \coprod_{k \in \mscrK} T_k \), with the \hyperref[thm:order_category_isomorphism_properties/coproduct]{disjoint union partial order} on its nodes.

    \thmitem{def:ordered_tree_grafting/root} Add a new node \( 0 \) and, for every \( k \in \mscrK \), an edge from \( 0 \) to \( \iota(r_k) \).

    \thmitem{def:ordered_tree_grafting/order} Extend the sibling order so that \( \iota_k(r_k) < \iota_m(r_m) \) whenever \( k < m \).
  \end{thmenum}
\end{definition}
\begin{comments}
  \item We base our terminology on \cite[\textnumero 3.6]{Holtkamp2011} and our notation on the earlier \cite[9]{Hoffman2002}.
...
\begin{definition}\label{def:n_ary_tree}\mcite[642]{Rosen1999}
  We say that the \hyperref[def:ordered_tree]{ordered tree} is \( n \)-\term{ary} if each node has \hi{at most} \( n \) children and \term{full \( n \)-ary} if each non-leaf node has \hi{exactly} \( n \) children.
...
\begin{algorithm}[Finite ordered tree to binary tree]\label{alg:finite_ordered_tree_to_binary_tree}
  ...
  \begin{thmenum}
    ...
    \begin{thmenum}
      \thmitem{alg:finite_ordered_tree_to_binary_tree/even} If \( n = 2k \), define \( B_k \) by grafting \( v_n \) onto \( v_{2k-1} \) and yield \( B_k \) as the result of the algorithm.
...
\begin{comments}
  ...
  We have chosen the term \enquote{labeled tree} for brevity; \incite[471]{Mimram2020} and \incite[655]{Rosen1999} use the same term for generic trees with labeled nodes, while \incite[\textnumero 2.8]{Holtkamp2011} uses it for rooted trees with labeled nodes.
\end{comments}
