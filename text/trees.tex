...
\begin{definition}\label{def:rooted_tree}\mcite[def. 11.1.2; 783; 784]{Rosen2019DiscreteMathematics}
  ...
  \begin{thmenum}
    \thmitem{def:rooted_tree/order}\mcite[15]{Diestel2017Graphs} We define the (nonstrict) \hyperref[def:partially_ordered_set]{partial order} \( u \leq v \) to hold if \( u \) lies on the path from \( r \) to \( v \). We call \( \leq \) the \term{ancestor order} or \term{vertical order} of \( T \) in order to distinguish it from the horizontal order in \hyperref[def:ordered_tree]{ordered trees}.

    \thmitem{def:rooted_tree/height}\mcite[15]{Diestel2017Graphs} The \term{height} of a node \( v \) is the length of the unique path from \( r \). This is precisely the height of \( v \) in the sense of \fullref{def:partial_order_element_height}.
  ...
  \begin{thmenum}[resume=def:rooted_tree]
    \thmitem{def:rooted_tree/subtree} A \term{rooted subtree} with root \( s \) is the \hyperref[def:tree/subtree]{subtree} consisting of \( s \) and all its descendants. If \( s \) is a child of \( r \), we say that its subtree is \term[en=subtree of the root (\cite[308]{Stanley2012EnumCombinatoricsVol1})]{immediate}.
...
\begin{definition}\label{def:ordered_tree}\mimprovised
  An \term[ru=упорядоченное дерево (\cite[\S 9.3.5]{Новиков2013ДискретнаяМатематика}), en=ordered tree (\cite[573]{Stanley2012EnumCombinatoricsVol1})]{ordered tree} is a quadruple \( T = (V, E, r, \leq) \), where \( (V, E, r) \) is a \hyperref[def:rooted_tree]{rooted tree} and \( {\leq} \) is a \hyperref[def:partially_ordered_set]{partial order}, called the \term{sibling order} or \term{horizontal order}, such that the children of every node are \hyperref[def:totally_order_chain]{totally ordered}, while non-siblings are pairwise-incomparable.
...
\end{definition}
\begin{comments}
  \item The definition in \cite[573]{Stanley2012EnumCombinatoricsVol1} is similar, however it relies on recursively built trees rather than graph-theoretic trees.

  \item \incite[\S 2.3]{Holtkamp2011RootedTrees} uses \enquote{planar rooted tree} as a synonym for \enquote{ordered tree}, while \cite[573]{Stanley2012EnumCombinatoricsVol1} uses \enquote{plane tree} as a synonym.
\end{comments}
