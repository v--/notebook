...
\begin{definition}\label{def:rooted_tree}\mcite[def. 11.1.2; 783; 784]{Rosen2019DiscreteMathematics}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:rooted_tree/height}\mcite[15]{Diestel2017GraphTheory} The \term{height} of a node \( v \) is the length of the unique path from \( r \).
...
\begin{comments}
  ...
  \item We will usually deal with only certain kinds of rooted trees, for example \hyperref[con:abstract_syntax_tree]{abstract syntax trees} or \hyperref[con:proof_tree]{proof trees}. In this case the statement should be conditional on the tree \( T \) being an AST or proof tree.
\end{comments}
\begin{proof}
