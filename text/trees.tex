...
\begin{definition}\label{def:rooted_tree}\mcite[15]{Diestel2005}
  A \term{rooted tree} is a triple \( T = (V, E, r) \), where \( (V, E) \) is a \hyperref[def:tree]{tree} and \( r \) is a distinguished vertex, called the \term{root}.

  \begin{thmenum}
    \thmitem{def:rooted_tree/depth} The \term{depth} of a node \( v \) is the length of the path from \( r \) to \( v \). The term \term{level} is also sometimes used but with a slightly different connotation --- the greater the depth, the lower the level.

    \thmitem{def:rooted_tree/ancestor_descendant} If \( v \) has a strictly greater depth than \( u \), we say that \( u \) is an \term{ancestor} of \( v \) and that \( v \) is a \term{descendant} of \( u \). Every node is a descendant of the root.

    \thmitem{def:rooted_tree/parent_child} If \( u \) is an ancestor of \( v \) and there is no vertex between \( u \) and \( v \), we call \( u \) the \term{immediate ancestor} or \term{parent} of \( v \) (it is unique), and similarly call \( v \) an \term{immediate descendant} or \term{child} or \( u \).

    \thmitem{def:rooted_tree/leaf} If a non-root node has no children, we say that it is a \term{leaf node}.

    \thmitem{def:rooted_tree/siblings} Finally, \( u \) is neither an ancestor nor descendant of \( v \), we call them \term{siblings}.
  \end{thmenum}
\end{definition}

\begin{definition}\label{def:ordered_tree}\mcite[exer. 3.52(b)]{Erickson2019}
  An \term{ordered tree} is a \hyperref[def:rooted_tree]{rooted tree} with a \hyperref[def:partially_ordered_set]{partial order} such that every set of \hyperref[def:arborescence/siblings]{siblings} is a \hyperref[def:partial_order_chain]{chain}.
\end{definition}
