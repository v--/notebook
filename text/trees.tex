...
\begin{definition}\label{def:set_theoretic_tree}\mcite[def. 9.10]{Jech2003}
  A set-theoretic \term{tree} is a \hyperref[def:partially_ordered_set]{partially ordered set} \( (T, \leq) \) such that, for each element \( x \in T \), the \hyperref[def:order_interval/ray]{initial segment} \( T_{<x} \) of all predecessors of \( x \) is \hyperref[def:well_ordered_set]{well-ordered} by \( < \).

  \begin{thmenum}
    \thmitem{def:set_theoretic_tree/level} The \term{level} of an element \( x \) is \( \ord(T_{<x}) \).
    \thmitem{def:set_theoretic_tree/root} We call the element \( x \) a \term{root} if it has level \( 0 \), that is, if it is not contained in any initial segment.
  \end{thmenum}
\end{definition}

\begin{proposition}\label{thm:set_theoretic_tree_as_arborescence}
  Every finite \hyperref[def:arborescence]{arborescence} \( (V, A, r) \) is a \hyperref[def:set_theoretic_tree]{set-theoretic tree} if we define \( x \leq y \) to hold if \( x \) is an ancestor of \( y \).

  The converse holds only partially: given a \hyperref[def:set_theoretic_tree/root]{root} \( x \) in a finite set-theoretic tree \( (T, \leq) \), the family \( T_{\geq x} \) is an arborescence with arcs induced by the order relation.
\end{proposition}
\begin{proof}
  \SufficiencySubProof Define a partial order \( x \leq y \) on the arborescence \( (V, A, r) \) to hold if there is a path from \( x \) to \( y \).

  Fix a vertex \( x \). We will show that \( \leq \) is a total order when restricted to \( T_{< x} \). For any two distinct predecessors \( y \) and \( z \) of \( x \), both \( y \) and \( z \) lie on the unique path from \( r \) to \( x \). Therefore, either \( y \leq z \) or \( z \leq y \). Hence, the order is total.

  \Fullref{thm:finite_totally_ordered_set_is_well_ordered} then implies that \( T_{< x} \) is well-ordered. Therefore, \( (V, \leq) \) is a set-theoretic tree.

  \NecessitySubProof Let \( r \) be a root of the set-theoretic tree \( (T, \leq) \). Given \( x > r \), there exists a unique path from \( r \) to \( x \). Indeed, the set \( T_{<x} \) is well-ordered, and hence it is a finite sequence, that is, a function from its \hyperref[def:set_theoretic_tree/level]{level} \( n \) to \( T \). Each member is strictly less than the next, hence this is the unique path.
\end{proof}
