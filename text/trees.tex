...
\begin{definition}\label{def:rooted_tree}\mcite[15]{Diestel2005}
  ...
  \begin{thmenum}
    \thmitem{def:rooted_tree/order} We define the (nonstrict) \hyperref[def:partial_ordered_set]{partial order} \( u \leq v \) to hold if \( u \) lies on the path from \( r \) to \( v \). We call \( \leq \) the \term{tree order} of \( T \).
