...
\begin{definition}\label{def:rooted_tree}\mcite[15]{Diestel2005}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:rooted_tree/parent_child} If \( u \) is an ancestor of \( v \) and there is no vertex between \( u \) and \( v \), we call \( u \) the \term{immediate ancestor}, \term{predecessor} or \term{parent} of \( v \) (it is unique), and similarly call \( v \) an \term{immediate descendant}, \term{successor} or \term{child} or \( u \).
...
\begin{defproof}
  ...
  \SubProofOf[rem:well_founded_relation]{well-foundedness} Follows from \fullref{thm:finite_totally_ordered_set_is_well_ordered}.
\end{defproof}
