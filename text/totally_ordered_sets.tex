...
\begin{definition}\label{def:totally_ordered_set}
  We say that a partially ordered set \( (P, \leq) \) is \term[ru=линейно упорядоченое множество (\cite[def. 3.4]{Гуров2013})]{linearly} or \term{totally ordered} if any of the following equivalent conditions hold:
  \begin{thmenum}
    \thmitem{def:totally_ordered_set/connected}\mcite[2]{Birkhoff1967} The nonstrict relation \( \leq \) is \hyperref[def:binary_relation/connected]{connected}.

    \thmitem{def:totally_ordered_set/trichotomic}\mcite[4]{Engelking1989} The strict relation \( < \) satisfies \hyperref[def:binary_relation/trichotomy]{trichotomy}.

    \thmitem{def:totally_ordered_set/chain}\mcite[2]{Birkhoff1967} The set \( P \) is itself a \hyperref[def:partial_order_chain/chain]{chain}.
...
\begin{proposition}\label{thm:def:totally_ordered_set}
  ...
  \begin{thmenum}
    \thmitem{thm:def:totally_ordered_set/embedding_iff_strict} An \hyperref[def:order_function/preserving]{order-preserving map} between totally ordered sets is an \hyperref[def:order_function/embedding]{order embedding} if and only if it is \hyperref[def:order_function/preserving]{strict}.

    \thmitem{thm:def:totally_ordered_set/maximal_iff_maximum} In a totally ordered set, \( x \) is \hyperref[def:extremal_points/maximal_and_minimal_element]{maximal} (resp. maximal) in the set \( A \) if and only if \( x \) is a \hyperref[def:extremal_points/maximum_and_minimum]{maximum} (resp. minimum) of \( A \).
    ...
    \thmitem{thm:def:totally_ordered_set/cofinal_iff_unbounded} If a totally ordered set is \hyperref[def:extremal_points/upper_and_lower_bounds]{unbounded from above}, then a subset is \hyperref[def:cofinal_set]{cofinal} if and only if it is itself unbounded from above.
  \end{thmenum}
\end{proposition}
\begin{proof}
  ...
  \SubProofOf{thm:def:totally_ordered_set/maximal_iff_maximum}
  ...
  \NecessitySubProof* If \( x \) is a maximum of \( A \), \fullref{thm:def:extremal_points/maximum_is_maximal} implies that is is maximal in \( A \).
...
\begin{definition}\label{def:well_ordered_set}\mcite[180]{Birkhoff1967}
  We say that a \hyperref[def:totally_ordered_set]{totally ordered set} \( (P, \leq) \) is \term[bg=добра (верига) (\cite[14]{Проданов1982}), ru=вполне упорядоченое (множество) (\cite[def. 3.15]{Гуров2013})]{well-ordered} if it satisfies the \hyperref[def:chain_condition]{descending chain condition}.
\end{definition}
...
\begin{proposition}\label{thm:def:well_ordered_set}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:def:well_ordered_set/embedding_extensive} Every \hyperref[def:order_function/embedding]{order embedding} of a well-ordered set to itself is \hyperref[def:extensive_function]{extensive}, that is, if \( f: P \to P \) is an embedding, we have \( x \leq f(x) \) for every \( x \).
...
\begin{proof}
  ...
  \SubProofOf{thm:def:well_ordered_set/embedding_extensive} We proceed by induction on \( P \). Fix \( x_0 \) and suppose that \( y \leq f(y) \) for all \( y < x_0 \).
...
\begin{definition}\label{def:order_topology}\mcite[exer. 1.7.4(a)]{Engelking1989}
  Let \( P \) be a \hyperref[def:partially_ordered_set]{totally ordered set} with more than one element. The \term{order topology} induced by \( \leq \) is the topology generated by the \hyperref[def:topological_subbase]{subbase} of open \hyperref[def:order_interval/unbounded]{open segments}
...
\begin{proposition}\label{thm:order_topology_intervals}
  ...
  \begin{thmenum}
    \thmitem{thm:order_topology_intervals/open} \hyperref[def:order_interval/unbounded]{Open rays} and \hyperref[def:order_interval/open]{open intervals} are open sets.

    \thmitem{thm:order_topology_intervals/closed} \hyperref[def:order_interval/unbounded]{Closed rays} and \hyperref[def:order_interval/closed]{closed intervals} are closed sets.
  \end{thmenum}
\end{proposition}
\begin{proof}
