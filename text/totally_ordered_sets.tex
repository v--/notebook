...
\begin{definition}\label{def:totally_ordered_set}
  We say that a partially ordered set \( (P, \leq) \) is \term[ru=линейно упорядоченое множество (\cite[def. 3.4]{Гуров2013ТеорияРешёток}), en=linearly ordered set (\cite[def. 1.3.1]{Harzheim2005OrderedSets})]{linearly} or \term[en=totally ordered set (\cite[def. 1.3]{DaveyPriestley2002LatticeTheory})]{totally ordered} if any of the following equivalent conditions hold:
  \begin{thmenum}
    \thmitem{def:totally_ordered_set/connected}\mcite[def. 1.3.1]{Harzheim2005OrderedSets} The nonstrict relation \( \leq \) is \hyperref[def:binary_relation/connected]{connected}.
    ...
    \thmitem{def:totally_ordered_set/chain}\mcite[2]{Birkhoff1967LatticeTheory} The set \( P \) is itself a \hyperref[def:partial_order_connectedness/chain]{chain}.

    \thmitem{def:totally_ordered_set/width}\mimprovised The set \( P \) has \hyperref[def:partial_order_connectedness/width]{width} \( 1 \).
  \end{thmenum}
\end{definition}
\begin{defproof}
  \ImplicationSubProof{def:totally_ordered_set/connected}{def:totally_ordered_set/trichotomic} If \( {\leq} \) is connected, then for any pair \( x \) and \( y \) of elements of \( P \), either \( x \leq y \) or \( x \geq y \).

  In the first case, either \( x < y \) or \( x = y \), and in the second case either \( x > y \) or \( x = y \).

  Generalizing on \( x \) and \( y \), we conclude that \( {<} \) satisfies trichotomy.

  \ImplicationSubProof{def:totally_ordered_set/trichotomic}{def:totally_ordered_set/chain} If \( {<} \) is trichotomic, then for any pair \( x \) and \( y \) of elements of \( P \), either \( x < y \), \( x > y \) or \( x = y \).

  In the first case we have \( x \leq y \), in the second case we have \( x \geq y \), and in the third case we have both.

  Generalizing on \( x \) and \( y \), we conclude that \( {\leq} \) is connected.
...
\begin{proposition}\label{thm:def:totally_ordered_set}
  ...
  \begin{thmenum}
    \thmitem{thm:def:totally_ordered_set/embedding_iff_strict} An \hyperref[def:order_function/preserving]{order-preserving map} between totally ordered sets is an \hyperref[def:partially_ordered_set/morphism]{order embedding} if and only if it is \hyperref[def:order_function/preserving]{strict}.

    \thmitem{thm:def:totally_ordered_set/maximal_iff_greatest} In a totally ordered set, \( x \) is \hyperref[def:extremal_points/maximal_and_minimal_element]{maximal} (resp. maximal) in the set \( A \) if and only if \( x \) is the \hyperref[def:extremal_points/maximum_and_minimum]{greatest} (resp. least) element of \( A \).
    ...
    Then \( (P, \leq) \) is \hyperref[def:partially_ordered_set/morphism]{order-isomorphic} to \( (Q, \subseteq) \).
...
\begin{proof}
  ...
  \SubProofOf{thm:def:totally_ordered_set/maximal_iff_greatest} Fix an element \( x \) in a totally ordered set \( (P, \leq) \).
  ...
  \SubProofOf{thm:def:totally_ordered_set/cofinal_iff_unbounded} We will prove the converse --- that a set is bounded if and only if it is not cofinal.

  \SufficiencySubProof* Let \( A \) be bounded from above by \( x \). \Cref{thm:def:partial_order_connectedness/unbounded} then implies that there exists some \( y \) such that \( x < y \). But no element of \( A \) is greater than \( y \), implying that \( A \) is not cofinal.

  \NecessitySubProof* Suppose that \( A \) is not cofinal. Fix some \( x \) from \( P \). Then, for every \( y \) in \( A \), \( x < y \) does not hold, that is, \( y \leq x \) for every \( y \) in \( A \). Therefore, \( x \) is an upper bound of \( A \).
\end{proof}

\paragraph{Dense total orders}

\begin{definition}\label{def:dense_total_order}\mcite[200]{Birkhoff1967LatticeTheory}
  We say that a subset \( A \) of a \hyperref[def:totally_ordered_set]{totally ordered set} \( (P, \leq) \) is \term[ru=плотное (множество) (\cite[51]{ШеньВерещагин2020НачалаТеорииМножеств}), en=dense subset (in partially ordered set) (\cite[def. 1.10.5]{Harzheim2005OrderedSets}); order-dense subset (\cite[200]{Birkhoff1967LatticeTheory})]{dense} in \( P \) if, whenever \( a < c \) for some members \( a \) and \( c \) of \( P \), there exists some \( b \) in \( A \) such that \( a < b < c \).
...
\begin{definition}\label{def:well_ordered_set}\mimprovised
  We say that a \hyperref[def:totally_ordered_set]{totally ordered set} \( (P, \leq) \) is \term[bg=добра (верига) (\cite[14]{Проданов1982ФункционаленАнализЧаст1}), ru=вполне упорядоченое (множество) (\cite[def. 3.15]{Гуров2013ТеорияРешёток}), en=well-ordered (set) (\cite[180]{Birkhoff1967LatticeTheory})]{well-ordered} if it satisfies the \hyperref[def:chain_condition]{descending chain condition}.
\end{definition}
\begin{comments}
  \item We use the definition via descending chains from \textcite[180]{Birkhoff1967LatticeTheory}, but allow \( P \) to be empty, as in
  \cite[def. 9.8]{Harzheim2005OrderedSets},
  \cite[def. 3.15]{Гуров2013ТеорияРешёток} or
  \cite[16]{Проданов1982ФункционаленАнализЧаст1}.
\end{comments}
...
\begin{proposition}\label{thm:def:well_ordered_set}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:def:well_ordered_set/embedding_extensional} Every \hyperref[def:partially_ordered_set/morphism]{order embedding} of a well-ordered set to itself is \hyperref[def:extensive_function]{extensive}, that is, if \( f: P \to P \) is an embedding, we have \( x \leq f(x) \) for every \( x \).
...
\end{proposition}
\begin{proof}
  \SubProofOf{thm:def:well_ordered_set/finite_chain} A finite totally ordered set cannot have a strictly descending chain by definition, so it must be well-ordered.

  \SubProofOf{thm:def:well_ordered_set/embedding_extensional} Fix some embedding \( f: P \to P \). We proceed by \fullref{thm:epsilon_induction} on \( P \). Fix \( x_0 \) and suppose that \( y \leq f(y) \) for all \( y < x_0 \).

  Aiming at a contradiction, suppose that \( x_0 > f(x_0) \). Thus, there exists some \( y_0 < x_0 \) such that \( f(x_0) = y_0 \). By the inductive hypothesis we have \( f(x_0) = y_0 \leq f(y_0) \). Thus, either \( f(x_0) < f(y_0) \), which contradicts that \( f \) is an order homomorphism, or \( f(x_0) = f(y_0) \), which contradicts the injectivity of \( f \).

  The obtained contradiction demonstrates that \( x \leq f(x) \) for all \( x \) in \( P \).

  \SubProofOf{thm:def:well_ordered_set/unique_isomorphism} Let \( f: P \to Q \) and \( g: P \to Q \) be two order isomorphisms between well-ordered sets. We will show that \( f = g \).
...
\begin{definition}\label{def:order_topology}\mcite[57]{Kelley1975GeneralTopology}
  ...
  \begin{equation}\label{eq:def:order_topology/subbase}
    \mscrS \coloneqq \set{ P_{>a} \given a \in P } \cup \set{ P_{<b} \given b \in P }.
  \end{equation}
  ...
  \begin{equation}\label{eq:def:order_topology/base}
    \mscrB = \mscrS \cup \set{ \varnothing } \cup \set{ (a, b) \given a, b \in P \T{and} a < b }.
  \end{equation}
\end{definition}
\begin{defproof}
  ...
  \SubProof*{Proof that \( \mscrB \subseteq \mscrC \)} For \( B \) in \( \mscrB \). The definition of \( \mscrB \) suggests the following possibilities:
  \begin{itemize}
  ...
  In all cases, \( B \) is an intersection of either one or two members of \( \mscrS \), hence \( B \) belongs to \( \mscrC \).

  Generalizing on \( B \), we conclude that \( \mscrB \subseteq \mscrC \).

  \SubProof*{Proof that \( \mscrC \subseteq \mscrB \)} Now let \( C = S_1 \cap \cdots \cap S_n \), where \( S_1, \ldots, S_n \) are members of \( \mscrS \). We will show by induction on \( n > 0 \) that \( C \) is in \( \mscrB \).
