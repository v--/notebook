...
\end{proposition}
\begin{proof}
  \SufficiencySubProof Follows from \fullref{thm:def:order_function/injective_implies_strict}.
  ...
  \SubProof*{Proof that \( f^{-1} \) is order-preserving} Since \( f \) is injective, its inverse \( f^{-1} \) is a single-valued partial function, and is total and injective on \( f(P) \). It then follows from \fullref{thm:def:order_function/injective_implies_strict} that the restriction of \( f^{-1} \) to \( f(P) \) is order-preserving.
\end{proof}
