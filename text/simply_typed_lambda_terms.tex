...
\begin{definition}\label{def:atomic_lambda_schema_instantiation}\mimprovised
  An \hyperref[con:schemas_and_instances]{atomic instantiation} for \hyperref[def:type_assertion]{type assertions} featuring \hyperref[def:lambda_term_schema]{\( \muplambda \)-term schemas} and \hyperref[def:simple_type_schema]{simple type schemas} is a \hyperref[def:set_valued_map/partial]{partial map} \( \BbbI \) sending
  \begin{itemize}
...
\begin{algorithm}[Typed \( \muplambda \)-term schema instantiation]\label{alg:lambda_schema_instantiation}
  We extend the \hyperref[def:atomic_lambda_schema_instantiation]{atomic \( \muplambda \)-term instantiation} \( \BbbI \) to all \hyperref[def:typed_lambda_term]{typed \( \muplambda \)-terms} as follows:
  \begin{thmenum}
    \thmitem{alg:lambda_schema_instantiation/types} We extend the instantiation \( \BbbI \) to compound simple types straightforwardly:
    \begin{equation}\label{eq:alg:lambda_schema_instantiation/types}
    ...
    \begin{equation}\label{eq:alg:lambda_schema_instantiation/terms}
      \Phi[\BbbI] \coloneqq \begin{cases}
    ...
    \thmitem{alg:lambda_schema_instantiation/assertion} Finally, given an assertion \( \Psi: \Alpha \), we let
    \begin{equation}\label{eq:alg:lambda_schema_instantiation/assertion}
...
\begin{comments}
  ...
  \item The parts of this algorithm can be found in the module \identifier{math.lambda_.instantiation} in \cite{notebook:code}.
\end{comments}
...
\begin{algorithm}[Simple type schema inference]\label{alg:simple_type_schema_inference}
  ...
  We will present a simple algorithm for inferring an \hyperref[def:atomic_lambda_schema_instantiation]{atomic instantiation} \( \BbbI \) such that \( \Alpha[\BbbI] = \tau \), if such an instantiation exists.
...
\begin{algorithm}[\( \muplambda \)-term schema inference]\label{alg:lambda_term_schema_inference}
  ...
  We will present an algorithm analogous to \fullref{alg:simple_type_schema_inference} for inferring an \hyperref[def:atomic_lambda_schema_instantiation]{atomic instantiation} \( \BbbI \) such that \( \Phi[\BbbI] = M \), if such an instantiation exists.
  ...
  \begin{thmenum}
    ...
    Otherwise, \( M = \qabs {x^\tau} N \). We use \fullref{alg:simple_type_schema_inference} to obtain an atomic instantiation \( \BbbI_\Alpha \) such that \( \Alpha[\BbbI_\Alpha] = \tau \). If the type instantiation inference errors out, so does the current algorithm.
