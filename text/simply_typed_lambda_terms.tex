...
\begin{concept}\label{con:ramified_type_theory}
  ...
  \begin{thmenum}
    ...
    If we substitute one of the variables, say \( y \), with a constant, we will obtain a characterization of the other. For example\fnote{We use here the function syntax from \cref{def:function_syntax}, adapted for \hyperref[def:simple_type]{simple types} based on \cref{rem:simple_type_function_syntax}.}:
    \begin{displayquote}
...
\begin{concept}\label{con:simple_type_theory}
  ...
  \begin{thmenum}
    \thmitem{con:simple_type_theory/hol} The original usage, due to \bycite[56]{Church1940STT}, allows two base types, arrow types and no type variables, with the specific typing rules discussed in \cref{def:simply_typed_hol}. The intended usage of this system is as \hyperref[rem:predicate_logic]{higher-order logic}.
...
\begin{remark}\label{rem:simple_type_parentheses}
  ...
  \begin{thmenum}
    \thmitem{rem:simple_type_parentheses/outermost} As in \cref{rem:propositional_formula_parentheses/outermost} and \cref{rem:lambda_term_parentheses/outermost}, we avoid writing the outermost parentheses in types.
