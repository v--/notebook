...
\begin{algorithm}[Type assertion schema inference]\label{alg:type_assertion_schema_inference}
  With \fullref{alg:lambda_term_schema_inference} and \fullref{alg:lambda_type_schema_inference}, we can infer an \hyperref[def:atomic_lambda_instantiation]{atomic \( \muplambda \)-term instantiation} from a \hyperref[def:simple_type_assertion_schema]{type assertion schema} and a \hyperref[def:type_assertion]{type assertion}. It is sufficient to take the union of the componentwise instantiations (as long as there are no conflicting type variables).
\end{algorithm}
\begin{comments}
  \item This algorithm is implemented in \cite{notebook:code} as
  \begin{CodeRefDisplay}
