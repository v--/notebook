...
\begin{definition}\label{def:simple_type}\mimprovised
  Fix a nonempty finite set \( \op*{Base} \), whose elements we will call \term{base types}. To ensure \hyperref[def:grammar_ambiguity]{syntactic unambiguity}, we disallow the \hyperref[con:improper_symbol]{improper symbols} from \cref{def:simple_type_alphabet} as base types (but do not consider base types to be reserved).
...
\begin{concept}\label{con:simple_type_theory}
  ...
  \begin{thmenum}
    \thmitem{con:simple_type_theory/hol} The original usage, due to \bycite[56]{Church1940STT}, allows two base types, arrow types and no type variables, with the specific typing rules discussed in \cref{def:hol_typing}. The intended usage of this system is as \hyperref[rem:predicate_logic]{higher-order logic}.
...
\begin{comments}
  ...
  For example, the most restrictive example we will encounter is the higher-order logic system from in \fullref{sec:predicate_logic}, in which we omit type variables, product and sum types.
\end{comments}
