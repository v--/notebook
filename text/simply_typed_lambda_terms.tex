...
\begin{remark}\label{rem:type_theory}
  The concept of types in mathematics is attributes to Russell and his ramified theory of types, which he created for the purposes of logic. We will discuss his theory in \cref{def:ramified_theory_of_types}. What matters to us here is the following passage from \cite[237]{Russell1908TypeTheory}:
  \begin{displayquote}
  ...
  These conventions were already used by \textcite{CoquandHuet1986CoC}, who introduce the \hyperref[def:lambda_cube/combinations]{calculus of constructions}. Barendregt later shows this to be the most powerful system of his \hyperref[def:lambda_cube]{\( \muplambda \)-cube}. The systems of the \( \muplambda \)-cube are instances of the more general \hyperref[def:pure_type_system]{pure type systems}.
...
\begin{comments}
  ...
  \item This terminology is unfortunately not well-established. For example, \textcite[def. 2A4]{Hindley1997BasicSTT} prefers the phrase \enquote{type assignment}, \textcite[def. 5.1.10]{Barendregt1992LambdaCalculiWithTypes} uses \enquote{statement}, while \textcite[ch. 4]{Mimram2020ProgramEqualsProof} uses the type assertion syntax without introducing terminology for such expressions.
...
\begin{concept}\label{con:type_universe}
  ...
  Without this restriction, as described in \cite[\S 8.2.3]{Mimram2020ProgramEqualsProof}, it is possible to obtain a variant of \fullref{thm:russels_paradox} attributed to Jean-Yves Girard.

  A workaround suggested in \cite[\S 1.3]{UnivalentFoundationsProgram2013HoTT} is the introduction of a sequence
  \begin{equation*}
    \syn\BbbU_0: \syn\BbbU_1: \syn\BbbU_2: \cdots
  \end{equation*}
  such that, for every nonnegative integer \( k \), \( \tau: \syn\BbbU_k \) implies \( \tau: \syn\BbbU_{k+1} \).
\end{concept}
\begin{comments}
...
\begin{comments}
  ...
  \item \Textcite[def. 5.1.1]{Barendregt1992LambdaCalculiWithTypes} calls unordered type contexts \enquote{bases}, and later in \cite[def. 5.1.5]{Barendregt1992LambdaCalculiWithTypes} calls a base a \enquote{context} if it is totally ordered. We will avoid unordered type context in light of the possible complications shown in \cref{ex:def:mltt_well_formed_context}.

  \textcite[note 2A5.2]{Hindley1997BasicSTT} considers contexts to be unordered, but does not go beyond simple types.
\end{comments}
...
\begin{definition}\label{def:simple_type_alphabet}\mimprovised
  ...
  \begin{thmenum}
    \thmitem{def:simple_type_alphabet/connectives} The set \( \op*{Conn} \) of type connectives:
    \begin{thmenum}
    ...
    \begin{thmenum}
      \thmitem{def:simple_type_alphabet/aux/parentheses} Parentheses \enquote{\( ( \)} and \enquote{\( ) \)} for grouping types unambiguously.

      \thmitem{def:simple_type_alphabet/aux/colon} The colon \enquote{\( : \)} for \hyperref[def:type_assertion]{type assertions} in \hyperref[def:typed_lambda_term]{typed \( \muplambda \)-terms}.
    \end{thmenum}
  \end{thmenum}
\end{definition}
\begin{comments}
...
\begin{definition}\label{def:simple_type}\mimprovised
  ...
  \begin{bnf*}
    ...
    \bnfprod{connective type} {\bnftsq{(} \bnfsp \bnfpn{type} \bnfsp \bnfpn{connective} \bnfsp \bnfpn{type} \bnfsp \bnftsq{)}} \\
    \bnfprod{type}            {\bnfpn{atomic type} \bnfor \bnfpn{connective type}}
  \end{bnf*}
  ...
  As per \cref{rem:improper_symbols_and_parsing}, we disallow the \hyperref[con:improper_symbol]{improper symbols} from the \hyperref[def:simple_type_alphabet]{alphabet of simple types} and the \hyperref[def:inference_rule_alphabet]{alphabet of inference rules} as universes and constants.
...
\begin{remark}\label{rem:arrow_type_name}
  ...
  \begin{itemize}
    \item \Textcite[56]{Church1940STT} and \textcite[\S 51]{Andrews2002Logic} use the notation \enquote{\( (\tau\sigma) \)} for what we would write as \( \sigma \synimplies \tau \). This is discussed in \cref{rem:type_theory}.

    Neither introduce a dedicated term for arrow types.

    \item \Textcite[\S 6.2.3]{GirardEtAl1989ProofsAndTypes} uses arrow notation and call them \enquote{arrow types}.

    \item \Textcite[def. 1.2.1]{TroelstraSchwichtenberg2000BasicProofTheory} and \textcite[269]{Farmer2008STTVirtues} use arrow notation and call them \enquote{function types}.

    \item \Textcite[\S 4.1.1]{Mimram2020ProgramEqualsProof} uses the arrow notation and calls them \enquote{arrow types}.

    \item \Textcite[def. 2A1]{Hindley1997BasicSTT} uses the arrow notation and calls them \enquote{composite types}.

    \item \Textcite[def. 3.2.2]{Barendregt1992LambdaCalculiWithTypes} also uses the arrow notation but introduces no special term.
  \end{itemize}
\end{remark}
...
\begin{concept}\label{con:simple_type_theory}
  ...
  \begin{thmenum}
    \thmitem{con:simple_type_theory/hol} The original usage, due to \cite[56]{Church1940STT}, allows two base types, arrow types and no type variables, with the specific typing rules listed in \cref{def:hol_typing_rules}. This system is intended to serve as a foundation for \hyperref[def:higher_order_logic]{higher-order logic}.

    Other authors who follow this convention are \textcite[\S 51]{Andrews2002Logic} and \textcite[269]{Farmer2008STTVirtues}.
    ...
    This system is studied in \cite[def. 3.2.2]{Barendregt1992LambdaCalculiWithTypes}, \textcite{Hindley1997BasicSTT} and \textcite[def. 1.2.1]{TroelstraSchwichtenberg2000BasicProofTheory}.
    ...
    \textcite{GirardEtAl1989ProofsAndTypes} extends this theory with product types (whose logical counterpart he calls the \enquote{the noble part of natural deduction} in \cite[\S 5.3]{GirardEtAl1989ProofsAndTypes}).
    ...
    \textcite[ch. 4]{Mimram2020ProgramEqualsProof} initially only considers type variables and arrow types, without base types, but later allows the full spectrum of simple algebraic types.
...
\begin{remark}\label{rem:simple_type_parentheses}
  We use some \enquote{abuse-of-notation} \hyperref[con:metalanguage]{metalingual} syntactic conventions:
  \begin{thmenum}
    ...
    \thmitem{rem:simple_type_parentheses/arrow} We suppose that the arrow in arrow types is \hyperref[con:one_sided_associativity/right]{right-associative}, which allows us to vastly simplify notation in some cases. This helps highlight the types of terms than can be \enquote{applied} to abstractions. See \cref{ex:def:type_derivation_tree/pairs} and \cref{ex:def:type_derivation_tree/disjoint} for examples.

    This convention is used by our main sources --- \cite[\S 2A1.1]{Hindley1997BasicSTT}, \cite[notation 3.1.2]{Barendregt1992LambdaCalculiWithTypes} and \cite[\S 4.1.1]{Mimram2020ProgramEqualsProof}.
  \end{thmenum}
\end{remark}
...
\begin{definition}\label{def:simple_type_ast}\mimprovised
  ...
  \begin{thmenum}
    ...
    \thmitem{def:simple_type_ast/application} If \( \tau = \sigma \syncirc \rho \), where \( {\syncirc} \in \op*{Conn} \), assuming we have already built \( T(\sigma) \) and \( T(\rho) \), we define \( T(\tau) \) by \hyperref[def:ordered_tree_grafting_product]{grafting} \( T(\sigma) \) and \( T(\rho) \) to a new root labeled with \( \syncirc \):
    \begin{equation*}
...
\end{concept}
\begin{comments}
  \item Superscripts are also used in \cite{Mimram2020ProgramEqualsProof} and \cite[ch. 5]{Hindley1997BasicSTT}. On the other hand, \textcite{Barendregt1992LambdaCalculiWithTypes} uses colons for both annotations and assertions.
  ...
  \textcite[186]{Mimram2020ProgramEqualsProof} also uses \enquote{type annotation} when discussing \hyperref[rem:typing_style]{Church style typing}.
\end{comments}
...
\begin{comments}
  ...
  \item For \hyperref[def:higher_order_logic]{higher-order logic}, we will need \( \muplambda \)-terms for which all variables have type annotations. We will achieve this by attaching a \hyperref[def:type_context]{type context} to the \( \muplambda \)-term --- see \cref{def:hol_expression}.

  \item Our definition is based on Barendregt's \cite[def. 3.2.1]{Barendregt1992LambdaCalculiWithTypes}. He prefers calling such terms \enquote{pseudoterms} because the grammar may produce \enquote{ill-typed} terms that are not \hyperref[def:typability]{typable} according to the typing rules. He does this in accordance with his more general terminology discussed in \cref{con:pseudoterm_expression}.
  ...
  We borrow the phrase \enquote{typed term} from \cite[ch. 5]{Hindley1997BasicSTT}, who however follows Church in annotating every term. Our variant, also used by \textcite[\S 4.1.3]{Mimram2020ProgramEqualsProof}, only guarantees uniqueness (but not existence) of a type for each term; this is shown in \cref{thm:typed_term_habitation_uniqueness}.

  \item Both typed and untyped \( \muplambda \)-terms are implemented in \cite{notebook:code}, in the module
  \begin{CodeRefDisplay}
    \ManualCodeRef{lambda_.terms}
  \end{CodeRefDisplay}

  A parser is implemented in
  \begin{CodeRefDisplay}
    \ManualCodeRef{lambda_.parsing}
  \end{CodeRefDisplay}
\end{comments}
...
\end{algorithm}
\begin{comments}
  \item The phrase \enquote{type erasure} is used for \( \muplambda \)-terms by \textcite[def. 5A2]{Hindley1997BasicSTT}.
...
\begin{definition}\label{def:simple_type_schema}
  ...
  \begin{bnf*}
    ...
    \bnfprod{connective type schema} {\bnftsq{(} \bnfsp \bnfpn{type schema} \bnfsp \bnftsq{\( \synimplies \)} \bnfsp \bnfpn{type schema} \bnfsp \bnftsq{)}} \\
    \bnfprod{type schema}            {\bnfpn{base type} \bnfor \bnfpn{type placeholder} \bnfor }, \\
    \bnfmore                         {\bnfpn{connective type schema}}
  \end{bnf*}

  We have used here the variable identifier rules from \cref{def:variable_identifier}.
\end{definition}
\begin{comments}
  \item To distinguish type schemas from term schemas, which we also denote via capital Greek identifiers, for types we will prefer letters from the start of the alphabet. Thus, \( \Phi: \Tau \) is a type assertion for term and type schemas.
...
\begin{comments}
  ...
  \item Similarly to propositional formula schemas, in the metalanguage we will denote arbitrary schemas via capital Greek identifiers like \( \Tau \).
\end{comments}

\begin{definition}\label{def:atomic_lambda_instantiation}\mimprovised
  We define \hyperref[con:schemas_and_instances]{atomic instantiations} for \hyperref[def:simple_type_assertion_schema]{type assertion schemas} as \hyperref[def:set_valued_map/partial]{partial maps} \( \BbbI \) sending \hyperref[def:lambda_term_schema]{variable placeholders} to \hyperref[def:lambda_term]{variables}, \hyperref[def:lambda_term_schema]{\( \muplambda \)-term placeholders} to \hyperref[def:lambda_term]{\( \muplambda \)-terms} and \hyperref[def:simple_type_schema]{type placeholders} to \hyperref[def:simple_type]{simple types}.
\end{definition}
\begin{comments}
...
\begin{algorithm}[Simple type instantiation]\label{alg:simple_type_schema_instantiation}
  We extend the \hyperref[def:atomic_lambda_instantiation]{atomic \( \muplambda \)-term instantiation} \( \BbbI \) to all \hyperref[def:simple_type_schema]{simple type schemas} straightforwardly
  \begin{equation}\label{eq:alg:simple_type_schema_instantiation}
    \Tau[\BbbI] \coloneqq \begin{cases}
      \Tau,                                  &\Tau \T{is a base type},        \\
      \BbbI(\Tau),                           &\Tau \T{is a type placeholder}, \\
      \Sigma[\BbbI] \synimplies \Rho[\BbbI], &\Tau = \Sigma \synimplies \Rho, \\
    \end{cases}
  \end{equation}
\end{algorithm}
\begin{comments}
  ...
  \item This algorithm is implemented in \cite{notebook:code} as
  \begin{CodeRefDisplay}
    \GetCodeRef{alg:simple_type_schema_instantiation}
  \end{CodeRefDisplay}
\end{comments}

\begin{algorithm}[Simple type schema inference]\label{alg:simple_type_schema_inference}
  Fix a \hyperref[def:simple_type]{simple type} \( \tau \) and a \hyperref[def:simple_type_schema]{simple type schema} \( \Tau \) over the same base types.

  We will present a simple algorithm for inferring an \hyperref[def:atomic_lambda_instantiation]{atomic instantiation} \( \BbbI \) such that \( \Tau[\BbbI] = \tau \), if such an instantiation exists.

  We will do case analysis on the abstract syntax tree of \( \Tau \) as defined in \cref{def:simple_type_ast}.
  \begin{thmenum}
    \thmitem{alg:simple_type_schema_inference/base_type} If \( \Tau \) is a base type, either \( \tau \) is the same base type and \( \BbbI \) is the empty instantiation, or otherwise no match is possible and the algorithm errors out.

    \thmitem{alg:simple_type_schema_inference/variable} If \( \Tau \) is a type variable placeholder, either \( \tau \) is a variable and we define \( \BbbI(\Tau) \coloneqq \tau \), or no match is possible and the algorithm errors out.

    \thmitem{alg:simple_type_schema_inference/connective} Suppose that \( \Tau = \Sigma \bincirc \Rho \), where \( {\bincirc} \) is \( {\rightarrow} \), \( {\times} \) or \( {+} \).

    If \( \tau = \sigma \bincirc \rho \), we proceed further; otherwise, no match is possible and the algorithm errors out.

    If \( \tau \) has the required form, we use the algorithm recursively to obtain instantiations \( \BbbI_\Sigma \) and \( \BbbI_\Rho \) such that \( \Sigma[\BbbI_\Sigma] = \sigma \) and \( \Rho[\BbbI_\Rho] = \rho \). If any of the recursive applications error out, so does the current application.

    Both \( \BbbI_\Sigma \) and \( \BbbI_\Rho \) are partial functions. If they disagree on their common domain, the algorithm errors out.

    Otherwise, we define \( \BbbI \) as the (set-theoretic) union of \( \BbbI_\Tau \) and \( \BbbI_\Sigma \), so that
    \begin{equation*}
      \Tau[\BbbI] = \Sigma[\BbbI_\Sigma] \bincirc \Rho[\BbbI_\Rho] = \sigma \bincirc \rho = \tau.
    \end{equation*}
  \end{thmenum}
\end{algorithm}
\begin{defproof}
  The algorithm makes it clear that \( \Tau[\BbbI] = \tau \) in case it finishes successfully. We can prove by contradiction and case analysis that no such instantiation is possible if the algorithm error out.
\end{defproof}
\begin{comments}
  \item This algorithm is implemented in \cite{notebook:code} as
  \begin{CodeRefDisplay}
    \GetCodeRef{alg:simple_type_schema_inference}
  \end{CodeRefDisplay}
\end{comments}

\begin{algorithm}[Typed \( \muplambda \)-term schema instantiation]\label{alg:lambda_term_schema_instantiation}
  We will extend the \hyperref[def:atomic_lambda_instantiation]{atomic \( \muplambda \)-term instantiation} \( \BbbI \) to all \hyperref[def:lambda_term_schema]{\( \muplambda \)-term schemas}.

  We will not concern ourselves with \hyperref[def:lambda_variable_occurrence]{free and bound occurrence} of variables as in the case of \hyperref[def:atomic_lambda_term_substitution]{atomic substitution}. As discussed in \cref{rem:pseudoterm_schemas}, lifting this restriction can be quite subtle. In particular, we suppose that all occurrences of the same variable placeholder refer to the same variable.

  Thus, we extend \( \BbbI \) to compound \( \muplambda \)-term schemas as follows:
  \begin{equation}\label{eq:alg:lambda_term_schema_instantiation}
    \Phi[\BbbI] \coloneqq \begin{cases}
      ...
      \qabs {\BbbI(x)^{\Tau[\BbbI]}} \Psi[\BbbI], &\Phi = \qabs {x^\Tau} \Psi.
    \end{cases}
  \end{equation}
\end{algorithm}
\begin{comments}
  \item This algorithm is implemented in \cite{notebook:code} as
  \begin{CodeRefDisplay}
    \GetCodeRef{alg:lambda_term_schema_instantiation}
  \end{CodeRefDisplay}
\end{comments}

\begin{algorithm}[\( \muplambda \)-term schema inference]\label{alg:lambda_term_schema_inference}
  ...
  We will present an algorithm analogous to \fullref{alg:simple_type_schema_inference} for inferring an \hyperref[def:atomic_lambda_instantiation]{atomic instantiation} \( \BbbI \) such that \( \Phi[\BbbI] = M \), if such an instantiation exists.
  ...
  \begin{thmenum}
    ...
    \thmitem{alg:lambda_term_schema_inference/abstraction} Finally, suppose that \( \Phi = \qabs {X^\Phi} \Psi \). If \( M \) is not an abstraction, the algorithm errors out.

    Otherwise, \( M = \qabs {x^\tau} N \). We use \fullref{alg:simple_type_schema_inference} to obtain an atomic instantiation \( \BbbI_\Phi \) such that \( \Phi[\BbbI_\Phi] = \tau \). If the type instantiation inference errors out, so does the current algorithm.
    ...
    Since \( \BbbI_\Phi \) matches only type placeholders and \( \BbbI_\Psi \) matches only term placeholders, no collisions are possible. We simply define \( \BbbI \) as their union, with the addition of \( \BbbI(X) = x \) in case \( x \) is not free in \( N \).

    Then
    \begin{equation*}
      \Phi[\BbbI] = \qabs {{X[\BbbI]}^{\Phi[\BbbI_\Phi]}} \Psi[\BbbI_\Psi] = \qabs {x^\tau} N = M.
    \end{equation*}
  \end{thmenum}
\end{algorithm}
\begin{defproof}
...
\begin{comments}
  \item This algorithm is implemented in \cite{notebook:code} as
  \begin{CodeRefDisplay}
    \GetCodeRef{alg:lambda_term_schema_inference}
  \end{CodeRefDisplay}
\end{comments}

\begin{definition}\label{def:simple_type_assertion_schema}\mimprovised
  A \term{simple type assertion schema} is simply a \hyperref[def:type_assertion]{type assertion} featuring \hyperref[def:lambda_term_schema]{\( \muplambda \)-term schemas} and \hyperref[def:simple_type_schema]{simple type schemas}.
\end{definition}

\begin{algorithm}[Type assertion schema instantiation]\label{alg:type_assertion_schema_instantiation}
  With \fullref{alg:lambda_term_schema_instantiation} and \fullref{alg:simple_type_schema_instantiation}, we can extend \hyperref[def:atomic_lambda_instantiation]{atomic \( \muplambda \)-term instantiation} \( \BbbI \) to all \hyperref[def:simple_type_assertion_schema]{type assertion schemas} straightforwardly:
  \begin{equation}\label{eq:alg:type_assertion_schema_instantiation}
    (\Phi: \Tau)[\BbbI] \coloneqq \Phi[\BbbI]: \Tau[\BbbI]
  \end{equation}
\end{algorithm}
\begin{comments}
  \item This algorithm is implemented in \cite{notebook:code} as
  \begin{CodeRefDisplay}
    \GetCodeRef{alg:type_assertion_schema_instantiation}
  \end{CodeRefDisplay}
\end{comments}

\begin{algorithm}[Type assertion schema inference]\label{alg:type_assertion_schema_inference}
  With \fullref{alg:lambda_term_schema_inference} and \fullref{alg:simple_type_schema_inference}, we can infer an \hyperref[def:atomic_lambda_instantiation]{atomic \( \muplambda \)-term instantiation} from a \hyperref[def:simple_type_assertion_schema]{type assertion schema} and a \hyperref[def:type_assertion]{type assertion}. It is sufficient to take the union of the componentwise instantiations (as long as there are no conflicting type variables).
\end{algorithm}
\begin{comments}
  \item This algorithm is implemented in \cite{notebook:code} as
  \begin{CodeRefDisplay}
    \GetCodeRef{alg:type_assertion_schema_inference}
  \end{CodeRefDisplay}
\end{comments}
...
\begin{comments}
  ...
  \item Typing rules are often presented in via \hyperref[rem:natural_deduction_in_sequent_style]{sequent style} rather than our preferred natural deduction style. See \cref{rem:dependent_type_rule_sequents} for a discussion.
...
\begin{example}\label{ex:type_universe_rules}
  ...
  \begin{paracol}{2}
    \begin{leftcolumn}
      \begin{equation*}\taglabel[\ensuremath{ \BbbU_+ }]{inf:ex:type_universe_rules/intro}
        \begin{prooftree}
          \infer0[\ref{inf:ex:type_universe_rules/intro}]{ \syn\BbbU_k: \syn\BbbU_{k+1} }
        \end{prooftree}
      \end{equation*}
    \end{leftcolumn}

    \begin{rightcolumn}
      \begin{equation*}\taglabel[\ensuremath{ \BbbU_{\logic{cum}} }]{inf:ex:type_universe_rules/cum}
        \begin{prooftree}
          \hypo{ \tau: \syn\BbbU_k }
          \infer1[\ref{inf:ex:type_universe_rules/cum}]{ \tau: \syn\BbbU_{k+1} }
        \end{prooftree}
      \end{equation*}
    \end{rightcolumn}
  \end{paracol}
...
\begin{comments}
  ...
  \item The phrases \enquote{explicit typing} and \enquote{implicit typing} are used by \textcite[119]{Barendregt1992LambdaCalculiWithTypes} as alternatives for what we described in \cref{rem:typing_style} as Church and Curry style typing, correspondingly.
\end{comments}
...
\begin{definition}\label{def:type_derivation_tree}\mimprovised
  ...
  \begin{thmenum}[series=def:type_derivation_tree]
    ...
    \thmitem{def:type_derivation_tree/open_assumptions} A set of type assertions whose entries we will call \term{open assumptions}.
  \end{thmenum}
  ...
  \begin{thmenum}[resume=def:type_derivation_tree]
    ...
    \thmitem{def:type_derivation_tree/application} We define \term{rule application} trees based on a straightforward adaptation of propositional natural deduction rule applications from \cref{def:propositional_natural_deduction_proof_tree/application}. Schema instantiation is based on \fullref{alg:lambda_term_schema_instantiation}.

    Again, some assumptions of the subtrees can be \term{discharged} if the rule allows it. The important difference is that variables play the role of markers.

    We provide several comments on discharging:
    \begin{itemize}
      \item We require all type assertions to be compatible, i.e. the same variable cannot have multiple types.

      \item We also disallow discharging an assumption \( x: \tau \) if other open assumptions depend on \( x \). As we shall see in \cref{ex:def:mltt_well_formed_context/discharging}, such a derivation tree becomes ill-formed in the sense of \cref{def:mltt_well_formed_context/derivation}.
    \end{itemize}
  \end{thmenum}
\end{definition}
\begin{comments}
  \item Rule application is implemented programmatically in \cite{notebook:code}, in the function
  \begin{CodeRefDisplay}
    \GetCodeRef{def:type_derivation_tree}
  \end{CodeRefDisplay}
\end{comments}
...
\begin{remark}\label{rem:mixed_terms_and_typing_style}
  We mentioned in \cref{def:simple_type_system_style} that we consider pathological the rules featuring both typed and untyped abstractors. Yet, we allow systems to feature \emph{distinct} rules that handle either typed or untyped terms, and call such a system \enquote{gradually typed}.

  Our reasoning is that a gradually typed system should allow typed, untyped, and even mixed \( \muplambda \)-terms. A rule with mixed schemas is instead able to \emph{only} handle mixed terms, making it suitable for very specific circumstances. We find it counterproductive to devote our attention to such rules.
...
\begin{proposition}\label{thm:assumptions_and_free_variables}
  ...
  Then the variables having \hyperref[def:type_derivation_tree/open_assumptions]{open assumptions} in \( T \) are precisely the free variable of \( M \).
\end{proposition}
\begin{proof}
...
\begin{definition}\label{def:simple_type_derivation}\mimprovised
  In a fixed \hyperref[def:simple_type_system]{simple type system} (specifying terms, types and rules), we define a \hyperref[def:type_derivation_relation]{type derivation relation} as follows: for a \hyperref[rem:well_formed_context]{well-formed} \hyperref[def:type_context]{type context} \( \Gamma \) and a \hyperref[def:type_assertion]{type assertion} \( M: \tau \), we let \( \Gamma \vdash M: \tau \) if there exists a \hyperref[def:type_derivation_tree]{type derivation tree} with conclusion \( M: \tau \) whose \hyperref[def:type_derivation_tree/open_assumptions]{open assumptions} are a \hyperref[def:subsequence]{sublist} of \( \Gamma \).

  As per \cref{def:general_logic}, we say that the type assertion \( M: \tau \) is \term[en=derivable (\cite[def. 3.1.4]{Barendregt1992LambdaCalculiWithTypes})]{derivable} from \( \Gamma \) if this relation holds.
\end{definition}
\begin{defproof}
...
\begin{remark}\label{rem:type_context_only_variables}
  ...
  \begin{itemize}
    ...
    \item While logic studies \hyperref[def:consequence_relation]{logical consequence} and \hyperref[con:hypothetical_judgment]{hypothetical judgments}, for type theory terms should ideally be typed unconditionally. Thus, while in logic \hyperref[def:tautology]{tautologies} with empty contexts are a curiosity, in type theory contexts are an instrument for dealing with \hyperref[def:lambda_variable_freeness]{free variables}.
  \end{itemize}
...
\begin{example}\label{ex:def:type_derivation_tree}
  ...
  \begin{thmenum}
    ...
    Furthermore, due to our reluctance to identify \( \alpha \)-equivalent \( \muplambda \)-terms, the combinator \( (\qabs \synf \synf) (\qabs \synx \synx) \) is also typable:
    \begin{equation*}
      \begin{prooftree}
        \hypo{ \synf: \tau \synimplies \tau }
        \infer[left label={\( \synf \)}]1[\ref{inf:def:arrow_type/intro/implicit}]{ \qabs \synf \synf: (\tau \synimplies \tau) \synimplies (\tau \synimplies \tau) }

        \hypo{ \synx: \tau }
        \infer[left label={\( \synx \)}]1[\ref{inf:def:arrow_type/intro/implicit}]{ \qabs \synx \synx: \tau \synimplies \tau }

        \infer2[\ref{inf:def:arrow_type/elim}]{ (\qabs \synf \synf) (\qabs \synx \synx): \tau \synimplies \tau }
      \end{prooftree}
    \end{equation*}
    ...
    The latter issue is discussed in \cite{MathOF:product_type_in_simply_typed_lambda_terms}, where it is also proven that no such explicitly typed triple of combinators\fnote{As noted in \cref{rem:simple_product_type_equality_rules}, such a triple is called a \enquote{surjective pairing}} exist.
    ...
    \end{Verbatim}
    as long as we identify \ManualCodeRef{Either a b} with
    \begin{equation*}
      (\tau \synimplies \sigma) \synimplies (\sigma \synimplies \rho) \synimplies \rho.
    \end{equation*}

    This works even with explicit typing, but the identification is not particularly apt because \ManualCodeRef{Either a b} must depend on \( \rho \). One solution to this is to introduce sum types, which we will do in \cref{def:simple_sum_type}.
...
\end{example}
\begin{comments}
  \item Many of these examples are verified programmatically in \cite{notebook:code}, in the modules
  \begin{CodeRefDisplay}
    \ManualCodeRef{lambda_.type_derivation.test_tree}
  \end{CodeRefDisplay}
  and
  \begin{CodeRefDisplay}
    \ManualCodeRef{lambda_.type_derivation.test_inference}
  \end{CodeRefDisplay}
\end{comments}
...
\begin{remark}\label{rem:typing_style}
  ...
  \Cref{thm:typed_term_habitation_uniqueness} proves that this tackles the non-uniqueness of type derivations for \hyperref[def:lambda_combinator]{combinators} shown in \cref{ex:def:type_derivation_tree/i} --- the combinator \( \ref{eq:ex:def:lambda_term/combinator/i} = \qabs \synx \synx \) inhabits \( \tau \synimplies \tau \) for any type \( \tau \), however, when \( \tau \) is fixed, \ref{inf:def:arrow_type/intro/explicit} uniquely determines \emph{the} type of \( I^\tau = \qabs {\synx^\tau} \synx \) as \( \tau \synimplies \tau \) at the cost of introducing a distinct abstraction term for every possible annotation of the abstractor variable.
...
\begin{proof}
  ...
  We will use \fullref{thm:induction_on_abstract_syntax} on \( T \) to show that if \( T' \) derives \( M: \tau' \) from \( \Gamma \), the trees coincide and \( \tau = \tau' \):
  \begin{itemize}
...
\begin{comments}
  ...
  \item This algorithm is implemented in \cite{notebook:code} as
  \begin{CodeRefDisplay}
    \GetCodeRef{alg:simply_typed_combinator_type_derivation}
  \end{CodeRefDisplay}
\end{comments}
