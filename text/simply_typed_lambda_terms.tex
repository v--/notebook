...
\begin{algorithm}[Type assertion schema instantiation]\label{alg:type_assertion_schema_instantiation}
  With \fullref{alg:lambda_term_schema_instantiation} and \fullref{alg:lambda_type_instantiation}, we can extend \hyperref[def:atomic_lambda_instantiation]{atomic \( \muplambda \)-term instantiation} \( \BbbI \) to all \hyperref[def:simple_type_assertion_schema]{type assertion schemas} straightforwardly:
