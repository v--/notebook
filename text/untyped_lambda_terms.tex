...
\begin{comments}
  ...
  \item We will extend this alphabet with new \hyperref[con:variable_binding]{variable binders} in \fullref{sec:dependent_types}, and also adapt it for higher-order logic in \cref{def:hol_signature}.
...
\begin{remark}\label{rem:variable_binding_properties}
  We will use several variable binders --- the \( \muplambda \)-abstraction terms in \cref{def:lambda_abstractor}, dependent and polymorphic types in \fullref{sec:dependent_types}, higher-order formulas in \fullref{sec:predicate_logic} and first-order formulas in \fullref{sec:first_order_logic}. In all those cases, we will find useful the same patterns of definitions:
  \begin{itemize}
...
\end{definition}
\begin{comments}
  \item This is implemented as an algorithm as \identifier{math.lambda_.untyped.substitution.apply_term_substitution} in \cite{notebook:code}.
