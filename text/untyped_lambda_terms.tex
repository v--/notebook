...
\begin{proposition}\label{thm:lambda_substitution_noop}
  We have \( M[\sigma] = M \) if and only if the free variables of \( M \) are fixed by the substitution \( \sigma \).
\end{proposition}
\begin{proof}
  \SufficiencySubProof Suppose that \( M[\sigma] = M \). We will use \fullref{thm:induction_on_rooted_trees} on \( M \) to show that \( \sigma(x) = x \) whenever \( x \) is free in \( M \):
  \begin{itemize}
    \item If \( M \) is a constant, it has no free variables, so the statement vacuously holds.
    \item If \( M \) is a variable, then \( \sigma(M) = M[\sigma] = M \), proving the statement.
    ...
    Thus, \( N[\sigma] = N \) and \( K[\sigma] = K \). Applying the inductive hypothesis, we conclude that \( \sigma \) fixes the free variables of both \( N \) and \( K \), and hence of \( M \).

    \item Suppose that \( M = \qabs x N \) and the inductive hypothesis holds for \( N \).

    \Fullref{thm:lambda_substitution_single_rule} implies that, for some variable \( v \) not free in \( M \) or \( M[\sigma] \), we have
    \begin{equation*}
      M[\sigma] = \qabs v N[\sigma_{x \mapsto v}].
    \end{equation*}

    Since \( M = M[\sigma] \), however, \( v \) must be equal to \( x \) and \( N[\sigma_{x \mapsto x}] \) to \( N \).

    The inductive hypothesis on \( N \) implies that \( \sigma_{x \mapsto x} \) fixes all free variables of \( N \). Then \( \sigma \) fixes all free variables of \( M \).
  \end{itemize}

  \NecessitySubProof We will use \fullref{thm:induction_on_rooted_trees} on \( M \) to show that \( M[\sigma] = M \) for all \( \sigma \) that fix the free variables of \( M \):
  \begin{itemize}
