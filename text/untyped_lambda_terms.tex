...
\begin{algorithm}[\( \muplambda \)-term substitution]\label{alg:lambda_term_substitution}\mimprovised
  ...
  \begin{empheq}[left={M[\Bbbs]} \coloneqq \empheqlbrace]{align}
    ...
    &\Bbbs(M),                       &&M \in \op*{Var},                                     \label{eq:alg:lambda_term_substitution/var}                 \\
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    Therefore, \( x \) cannot be in \( \op*{Free}_\Bbbs(M) \), and we must use the rule \eqref{eq:alg:lambda_term_substitution/abstraction/direct}. The free variables of \( N \) are those of \( N \) with the eventual addition of \( x \), all of which are fixed by \( \Bbbs_{x \mapsto x} \). Hence,
    \begin{equation*}
      M[\Bbbs]
      =
