...
\begin{comments}
  ...
  \item We will extend this alphabet with new \hyperref[con:variable_binding]{variable binders} in \fullref{sec:dependent_types}, and also adapt it for simply typed higher-order logic in \cref{def:simply_typed_hol_signature}.
...
\begin{comments}
  ...
  \item We need all the parentheses so that we can prove unambiguity in \cref{thm:lambda_term_grammar/unambiguous}. In the metalanguage, we will use the conventions from \cref{rem:lambda_term_parentheses} regarding parentheses.
\end{comments}
...
\begin{remark}\label{rem:lambda_term_parentheses}
  We will use some \enquote{abuse-of-notation} \hyperref[con:metalogic]{metalingual} syntactic conventions somewhat resembling those from \cref{rem:propositional_formula_parentheses}:
  \begin{thmenum}
    \thmitem{rem:lambda_term_parentheses/outermost} As in \cref{rem:propositional_formula_parentheses/outermost}, we avoid writing the outermost parentheses in terms like \( (\synx \syny) \) or \( ((\synx \syny) \synz) \).
...
\begin{remark}\label{rem:binders_and_syntactic_ambiguity}
  \Cref{thm:lambda_term_grammar/unambiguous} implies that \( \muplambda \)-terms are \hyperref[def:grammar_ambiguity]{syntactically unambiguous}. This is so because we have taken precautions to restrict which symbols are allowed as constants.
\end{remark}
...
\begin{remark}\label{rem:variable_binding_properties}
  We will use several variable binders --- the \( \muplambda \)-abstraction terms in \cref{def:lambda_abstractor}, dependent and polymorphic types in \fullref{sec:dependent_types}, higher-order logic formulas in \fullref{sec:predicate_logic} and first-order formulas in \fullref{sec:first_order_logic}. In all those cases, we will find useful the same patterns of definitions:
  \begin{itemize}
...
\begin{definition}\label{def:lambda_term_substitution}\mimprovised
  ...
  \begin{thmenum}[resume=def:lambda_term_substitution]
    ...
    \thmitem{def:lambda_term_substitution/operation} Finally, for an arbitrary \( \muplambda \)-term \( M \), we define the following operation (for which will use the notational convention from \cref{rem:substitution_notation}):
...
\begin{remark}\label{rem:renaming_substitution_rules}
  ...
  In practice, however, we would prefer to not rename bound variables, especially in \hyperref[def:lambda_combinator]{combinators}. Furthermore, the rule \eqref{eq:def:lambda_term_substitution/abstraction/direct} allows us to prove \cref{thm:lambda_substitution_noop}.

  See \cref{ex:def:lambda_term_substitution/ignoring} for concrete examples of ignoring the latter rule.
\end{remark}
...
\end{proposition}
\begin{proof}
  We will use \cref{thm:induction_on_rooted_trees} on the \hyperref[def:lambda_term_ast]{abstract syntax tree} of \( M \) simultaneously for all possible substitutions:
  \begin{itemize}
...
\end{proposition}
\begin{proof}
  We will use \cref{thm:induction_on_rooted_trees} on the \hyperref[def:lambda_term_ast]{abstract syntax tree} of \( M \) to show \eqref{eq:thm:lambda_substitution_restriction} simultaneously for all suitable substitutions.
...
\end{proposition}
\begin{proof}
  \SufficiencySubProof Suppose that \( M[\Bbbs] = M \). We will use \cref{thm:induction_on_rooted_trees} on the \hyperref[def:lambda_term_ast]{abstract syntax tree} of \( M \) to show that \( \Bbbs(x) = x \) whenever \( x \) is free in \( M \):
  \begin{itemize}
  ...
  \NecessitySubProof We will use \cref{thm:induction_on_rooted_trees} on the \hyperref[def:lambda_term_ast]{abstract syntax tree} of \( M \) to show that \( M[\Bbbs] = M \) for all \( \Bbbs \) that fix the free variables of \( M \):
  \begin{itemize}
...
\begin{example}\label{ex:def:lambda_term_substitution}
  ...
  \begin{thmenum}
    ...
    \thmitem{ex:def:lambda_term_substitution/nested_noop} Consider the substitution \( \ref{eq:ex:def:lambda_term/combinator/k}[\syny \mapsto \synx] \). Since \( K \) is a combinator, \cref{thm:lambda_substitution_combinators} holds and \( K[\syny \mapsto \synx] = K \).
    ...
    \thmitem{ex:def:lambda_term_substitution/capture} The gist of \cref{thm:lambda_substitution_free_variables} is that substitution avoids \enquote{capturing} free variables under the scope of some abstraction.
    ...
    \thmitem{ex:def:lambda_term_substitution/ignoring} We discussed in \cref{rem:renaming_substitution_rules} that we can ignore \eqref{eq:def:lambda_term_substitution/abstraction/direct} and always use the renaming rule \eqref{eq:def:lambda_term_substitution/abstraction/renaming}.
