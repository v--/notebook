...
\begin{concept}\label{con:lambda_conversion}
  ...
  This usage originates from Alonzo Church himself. When introducing \( \lambda \)-calculus in \cite[357]{Church1932LambdaCalculus}, he defines \hyperref[def:lambda_substitution]{single-variable substitution} as the first three of five postulates, and later states
  \begin{displayquote}
...
\begin{comments}
  ...
  \item This definition is loosely based on \cite[def. 1A7]{Hindley1997BasicSTT}, but is modified to allow substituting multiple variables simultaneously, as well as to handle constants. This results in less rules since \eqref{eq:def:lambda_substitution/abstraction/direct} subsumes three of the four cases.

  \item Due to how fresh variables are chosen, the simultaneous substitution \( M[x \mapsto N, y \mapsto K] \) cannot be properly emulated by iterated substitution as in \fullref{rem:simulating_simultaneous_substitution} --- this can only be done \enquote{up to \( \alpha \)-equivalence}, as shown in \fullref{thm:substitution_composition_is_alpha_equivalent}. \Fullref{ex:def:lambda_substitution/composed_vs_iterated} demonstrates subtleties that arise.
...
\begin{example}\label{ex:def:lambda_substitution}
  ...
  \begin{thmenum}
    ...
    \thmitem{ex:def:lambda_substitution/nested_noop} Consider the substitution \( \ref{eq:ex:def:lambda_term/combinator/k}[\syny \mapsto \synx] \). Since \( K \) is a combinator, \fullref{thm:lambda_substitution_combinators} holds and \( K[\syny \mapsto \synx] = K \).
    ...
    We will show in \fullref{ex:def:lambda_substitution/composed_vs_iterated} how this becomes more subtle when two variables are substituted simultaneously.
    ...
    \thmitem{ex:def:lambda_substitution/composed_vs_iterated} We will show some subtleties of renaming during simultaneous substitution. Consider
    \begin{equation*}
    ...
    The abstractor variable \( \syna \) is in \( \op*{Free}_\Bbbs(M) \), thus we must use the renaming rule \eqref{eq:def:lambda_substitution/abstraction/renaming}. This new variable must be \( \sharp(\set{ \syna, \synx }) \), which per our remarks in \fullref{def:lambda_substitution/sharp} must be \( \synb \). Then
    \begin{equation*}
