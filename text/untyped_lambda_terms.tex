...
\begin{comments}
  ...
  \item The dot is not strictly necessary, and was in fact not used in \incite{Church1932LambdaCalculus} where \( \synlambda \)-calculus was introduced. It later became standard to include it. Our main reference books --- \incite[def. 1A1]{Hindley1997BasicSTT}, \incite[22]{Barendregt1984LambdaCalculus}, \incite[6]{BarendregtDekkersStatman2013LambdaCalculusWithTypes}, \incite[\S 4.1.3]{Mimram2020ProgramEqualsProof} and \incite[188]{Герасимов2011Вычислимость} --- all use it.
\end{comments}
...
\begin{concept}\label{con:variable_binding}
  ...
  Variables that are not bound will be called \term{free}, and for determining their interpretation we need additional machinery.
\end{concept}
\begin{comments}
...
\begin{remark}\label{rem:barendregt_convention}\mcite[148]{Mimram2020ProgramEqualsProof}
...
\begin{definition}\label{def:lambda_term_substitution}\mimprovised
  ...
  \begin{thmenum}[series=def:lambda_term_substitution]
    \thmitem{def:lambda_term_substitution/sigma} \( \Bbbs: \op*{Var} \to \op*{Term} \) is a function specifying how variables need to be replaced with terms. We allow only finitely many variables to not be fixed by \( \Bbbs \).

    \thmitem{def:lambda_term_substitution/sharp} \( \sharp: F \to \op*{Var} \), where \( F \) is the family of all variable contexts (i.e. finite subsets of \( \op*{Var} \)), is a function providing us with a fresh variable identifier not present in some given context.
  ...
  \begin{thmenum}[resume=def:lambda_term_substitution]
    ...
    \thmitem{def:lambda_term_substitution/operation} Finally, for an arbitrary \( \synlambda \)-term \( M \), we define the following operation (for which will use the notational convention from \fullref{rem:substitution_notation}):
...
\begin{comments}
  ...
  \item Due to how fresh variables are chosen, the simultaneous substitution \( M[x \mapsto N, y \mapsto K] \) cannot be properly emulated by iterated substitution as in \fullref{rem:simulating_simultaneous_substitution} --- this can only be done \enquote{up to \( \alpha \)-equivalence}, as shown in \fullref{thm:substitution_composition_is_alpha_equivalent}. \Fullref{ex:def:lambda_term_substitution/composed_vs_iterated} demonstrates subtleties that arise.
...
\begin{remark}\label{rem:renaming_substitution_rules}
  ...
  In practice, however, we would prefer to not rename bound variables, especially in \hyperref[def:lambda_combinator]{combinators}; the rule \eqref{eq:def:lambda_term_substitution/abstraction/direct} allow us to prove \fullref{thm:lambda_substitution_noop}.
...
\begin{example}\label{ex:def:lambda_term_substitution}
  ...
  \begin{thmenum}
    ...
    We will generalize this observation to arbitrary substitutions in \fullref{thm:substitution_composition_is_alpha_equivalent}.
  \end{thmenum}
\end{example}
