...
\begin{comments}
  ...
  \item Variables and constants are \hyperref[con:assumed_knowledge]{assumed knowledge}, so definitions across the literature are scarce. \Textcite[340]{Kline1972MathematicalThoughtVol1} attributes both words \enquote{variable} and \enquote{constant} to Leibniz and later Johann Bernoulli.

  \Textcite[9]{Church1956LogicVol1} writes
  \begin{displayquote}
...
\begin{definition}\label{def:lambda_term_alphabet}\mimprovised
  ...
  \begin{thmenum}
    ...
    \begin{thmenum}
      ...
      \thmitem{def:lambda_term_alphabet/aux/placeholder} The placeholder circle \enquote{\( \anon \)} for defining abstract syntax trees of \( \muplambda \)-applications in \fullref{def:lambda_term_ast/application}.
    \end{thmenum}
  \end{thmenum}
\end{definition}
\begin{comments}
...
\begin{remark}\label{rem:lambda_term_abstractor_dot}
  ...
  This dot is not strictly necessary and was in fact not used in \cite{Church1932LambdaCalculus} where \( \muplambda \)-calculus was introduced. Including it has since become standard, as a leftover from a dated convention in logic --- see \cref{ex:dot_delimiters_in_logic/quantifiers}.
...
\begin{definition}\label{def:lambda_term_ast}\mimprovised
  ...
  \begin{thmenum}
    ...
    \thmitem{def:lambda_term_ast/application} If \( M = NK \), assuming we have already built \( T(N) \) and \( T(K) \), we define \( T(M) \) by \hyperref[def:ordered_tree_grafting_product]{grafting} \( T(N) \) and \( T(K) \) to a new root labeled with the placeholder symbol \enquote{\( \anon \)}:
    \begin{equation*}
...
\begin{remark}\label{rem:lambda_term_notation_conventions}
  We will use some \enquote{abuse-of-notation} \hyperref[con:metalanguage]{metalingual} syntactic conventions somewhat resembling those from \cref{rem:propositional_formula_notation_conventions}:
  \begin{thmenum}
    ...
    This convention is used by Church himself in \cite[58]{Church1940STT}, as well as our main sources --- \cite[1]{Hindley1997BasicSTT}, \cite[notation 2.1.3]{Barendregt1984LambdaCalculus} and \cite[112]{Mimram2020ProgramEqualsProof}.
...
\end{definition}
\begin{comments}
  \item Note that the analog of \cref{thm:propositional_subformula_characterization} no longer holds --- a variable that is a substring of a term is not necessarily a subterm --- see \cref{ex:def:lambda_term/naive_subterm}.
\end{comments}

\begin{proposition}\label{thm:lambda_subterm_characterization}
  Suppose that the \hyperref[def:formal_language/substring]{substring} \( N \) of the \( \muplambda \)-term \( M \) is \emph{not a variable}. Then \( N \) is a \hyperref[def:lambda_subterm]{subterm} of \( M \) if and only if \( N \) is itself a \( \muplambda \)-term.
\end{proposition}
\begin{proof}
  We can give a proof similar to \cref{thm:propositional_subformula_characterization}.
\end{proof}

\begin{proposition}\label{thm:lambda_term_ast_subterm}
  The \( \muplambda \)-term \( N \) is a \hyperref[def:lambda_subterm]{subterm} of \( M \) if and only if the \hyperref[def:lambda_term_ast]{abstract syntax tree} \( T(M) \) has a \hyperref[def:tree/subobject]{subtree} \hyperref[def:labeled_tree/morphism]{isomorphic} to \( T(N) \).
\end{proposition}
\begin{proof}
  Trivial.
\end{proof}
...
\begin{remark}\label{rem:variable_binding_properties}
  ...
  \begin{itemize}
    ...
    \item Capture-avoiding substitution as defined in \fullref{alg:lambda_term_substitution}, with properties like \cref{thm:lambda_substitution_free_variables}, \cref{thm:lambda_substitution_single_rule}, \cref{thm:lambda_substitutions_agree} and \cref{thm:lambda_substitutions_agree}.
    \item \( \alpha \)-equivalence as defined in \cref{def:lambda_term_alpha_equivalence}, with properties like \cref{thm:def:lambda_term_alpha_equivalence}, \cref{thm:nary_lambda_substitution_composition_is_alpha_equivalent}, \cref{thm:substitution_on_alpha_equivalent_terms}, \cref{thm:alpha_conversion} and \fullref{alg:separation_of_free_and_bound_variables}.
  \end{itemize}
...
\end{remark}
\begin{comments}
  \item For \hyperref[def:fol_formula]{first-order formulas}, capture-avoiding substitution is defined in \fullref{alg:fol_formula_substitution} and characterized in \cref{thm:alg:fol_formula_substitution}, while \( \alpha \)-equivalence is defined in \cref{def:fol_formula_alpha_equivalence} and characterized in \cref{thm:def:fol_formula_alpha_equivalence}.
\end{comments}
...
\begin{remark}\label{rem:apparent_variables}
  ...
  \textcite[228]{Russell1908TypeTheory} himself attributes the terminology to Peano, and gives the following characterization:
  \begin{displayquote}
...
\begin{remark}\label{rem:barendregt_variable_convention}
  \Textcite[\S 2.1.13]{Barendregt1984LambdaCalculus} assumes the following variable convention:
  \begin{displayquote}
  ...
  \Textcite[\S 3.6.1]{Mimram2020ProgramEqualsProof} calls this the \enquote{Barendregt convention} and rephrases it as follows:
  \begin{displayquote}
...
\begin{definition}\label{def:atomic_lambda_term_substitution}\mimprovised
  An \hyperref[con:syntactic_substitution]{atomic instantiation} for \hyperref[def:lambda_term]{\( \muplambda \)-terms} is a pair \( (\Bbbs, \sharp) \), where
  ...
  \begin{thmenum}[resume=def:atomic_lambda_term_substitution]
    ...
    \begin{equation}\label{eq:def:atomic_lambda_term_substitution/modified}
    ...
    \thmitem{def:atomic_lambda_term_substitution/free} For defining capture-avoiding substitutions, we will also need the following auxiliary definition:
    \begin{equation}\label{eq:def:atomic_lambda_term_substitution/free}
      \op*{Free}_\Bbbs(M) \coloneqq \bigcup_{\mathclap{v \in \op*{Free}(M)}} \op*{Free}(\Bbbs(v)).
    \end{equation}
  \end{thmenum}
\end{definition}
\begin{comments}
  \item The auxiliary function \( \sharp \) allows determinism when choosing a new variable during renaming in \fullref{alg:lambda_term_substitution}. This enables programmatic implementations. A more theoretical consequence of this determinism is \cref{thm:lambda_substitutions_agree}.
\end{comments}

\begin{algorithm}[\( \muplambda \)-term substitution]\label{alg:lambda_term_substitution}\mimprovised
  We can extend an \hyperref[def:atomic_lambda_term_substitution]{atomic simultaneous substitution} \( (\Bbbs, \sharp) \) to an arbitrary \( \muplambda \)-term \( M \) as follows:
  \begin{empheq}[left={M[\Bbbs]} \coloneqq \empheqlbrace]{align}
    ...
    &\qabs x N[\Bbbs_{x \mapsto x}], &&M = \qabs x N \T{and} x \notin \op*{Free}_\Bbbs(M), \label{eq:alg:lambda_term_substitution/abstraction/direct}  \\
...
\begin{comments}
  ...
  \item Due to how fresh variables are chosen, the substitution \( M[x \mapsto N, y \mapsto K] \) cannot be properly emulated by iterated substitution as in \cref{rem:simulating_simultaneous_substitution} --- this can only be done \enquote{up to \( \alpha \)-equivalence}, as shown in \cref{thm:nary_lambda_substitution_composition_is_alpha_equivalent}. \Cref{ex:alg:lambda_term_substitution/composed_vs_iterated} demonstrates subtleties that arise.
  ...
  \begin{CodeRefDisplay}
    \GetCodeRef{alg:lambda_term_substitution}
  \end{CodeRefDisplay}
\end{comments}
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      \item Suppose first that \( x \notin \op*{Free}_\Bbbs(M) \). We must thus use \eqref{eq:alg:lambda_term_substitution/abstraction/direct}. We have
      \begin{align*}
        \op*{Free}(M[\Bbbs])
...
\begin{corollary}\label{thm:lambda_substitution_free_variables_single}
  ...
  \begin{equation}\label{eq:thm:lambda_substitution_free_variables_single}
    \op*{Free}( M[x \mapsto N] ) = \begin{cases}
      (\op*{Free}(M) \setminus \set{ x }) \cup \op*{Free}(N), &x \in \op*{Free}(M) \\
  ...
  \begin{equation}\label{eq:thm:lambda_substitution_free_variables_single/subset}
    \op*{Free}( M[x \mapsto N] ) \subseteq (\op*{Free}(M) \setminus \set{ x }) \cup \op*{Free}(N).
  \end{equation}
\end{corollary}
\begin{proof}
...
\begin{proposition}\label{thm:lambda_substitution_single_rule}
  For any abstraction \( M = \qabs x N \) and any substitution \( \Bbbs \), there exists a variable \( v \notin \op*{Free}_\Bbbs(M) \) such that
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \item Otherwise, \( x \notin \op*{Free}_\Bbbs(M) \) and we must use \eqref{eq:alg:lambda_term_substitution/abstraction/direct}:
    \begin{equation*}
...
\begin{proposition}\label{thm:lambda_substitutions_agree}
  If the substitutions \( \Bbbs \) and \( \Bbbt \) agree on the free variables of \( M \), then \( M[\Bbbs] = M[\Bbbt] \).
\end{proposition}
\begin{proof}
  We will use \fullref{thm:induction_on_abstract_syntax} on of \( M \) to show that \( M[\Bbbs] = M[\Bbbt] \) simultaneously for all suitable substitutions.
...
\end{corollary}
\begin{proof}
  Vacuously follows from \cref{thm:lambda_substitution_identity} since combinators simply have no free variables.
\end{proof}
...
\begin{example}\label{ex:alg:lambda_term_substitution}
  ...
  \begin{thmenum}
    ...
    We will generalize this observation to arbitrary substitutions in \cref{thm:nary_lambda_substitution_composition_is_alpha_equivalent}.
  \end{thmenum}
\end{example}
