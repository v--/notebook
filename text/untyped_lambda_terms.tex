...
\begin{comments}
  ...
  \item A related notions is that of a \hyperref[def:random_variable]{random variables}, whose value is nondeterministic. They are neither dependent nor independent; in fact, they have their own notion of dependence that we define formally in \fullref{def:random_variable_independence}.
...
\begin{definition}\label{def:lambda_term_alphabet}\mimprovised
  ...
  \begin{thmenum}
    \thmitem{def:lambda_term_alphabet/binder} The \hyperref[con:variable_binder]{variable binder} \( \synlambda \), after which \( \muplambda \)-calculus is named.
    ...
    \begin{thmenum}
      ...
      \thmitem{def:lambda_term_alphabet/aux/auxiliary} The colon \enquote{\( : \)} for \hyperref[def:type_assertion]{type assertions} in typed \( \muplambda \)-terms.
...
\begin{comments}
  ...
  \item We will extend this alphabet with new \hyperref[con:variable_binder]{variable binders} in \fullref{sec:dependent_types}, and also extend it for simply typed higher-order logic in \fullref{def:simply_typed_hol_alphabet}.

  \item The auxiliary dot after the variable binders is not strictly necessary, and was in fact not used in \incite{Church1932LambdaCalculus} where \( \muplambda \)-calculus was introduced. It later became standard to include it. Our main reference books --- \incite[def. 1A1]{Hindley1997BasicSTT}, \incite[22]{Barendregt1984LambdaCalculus}, \incite[\S 4.1.3]{Mimram2020ProgramEqualsProof} and \incite[188]{Герасимов2011Вычислимость} --- all use it.
\end{comments}

\begin{definition}\label{def:lambda_term}\mimprovised
  Fix a (finite) \hyperref[def:formal_language/alphabet]{alphabet} \( \op*{Const} \), whose elements we will call \term[en=constant (\cite[202]{Andrews2002Logic})]{constant terms}. To ensure \hyperref[def:grammar_ambiguity]{syntactic unambiguity}, we disallow the \hyperref[con:reserved_symbol]{reserved symbols} from \fullref{def:lambda_term_alphabet} as constants (but do not consider constants to be reserved).
