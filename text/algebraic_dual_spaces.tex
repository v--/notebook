...
\begin{definition}\label{def:dual_vector_space}\mcite[50]{Knapp2016BasicAlgebra}
  Let \( V \) be a \hyperref[def:vector_space]{vector space} over the \hyperref[def:field]{field} \( \BbbK \). By \cref{\cref{thm:fol_equational_theory_models/functions}}, the set \( \hom(V, \BbbK) \) of all \hyperref[def:semimodule/morphism]{linear maps} from \( V \) to the underlying field \( \BbbK \) also form a vector space over \( \BbbK \).
...
\begin{concept}\label{con:functional}
  ...
  \begin{itemize}
    \item In the context of linear algebra, and in particular \cref{def:dual_vector_space}, the term \enquote{functional} refers to \enquote{linear functional}, i.e. a \hyperref[def:semimodule/morphism]{linear map} from a \hyperref[def:vector_space]{vector space} to its base field.
...
\begin{remark}\label{rem:finite_dimensional_dual_space_isomorphism}
  As discussed in \cref{rem:vector_space_and_dual_space}, the vector space \( \BbbK^n \) is isomorphic to its dual.

  We discussed in \cref{rem:matrices_as_functions} that vectors in \( \BbbK^n \) can be regarded as \hyperref[def:array/column_vector]{column vectors}. Depending on the situation, we regard linear functionals as either:
  \begin{itemize}
...
\begin{remark}\label{rem:double_dual}
  We discussed in \cref{rem:vector_space_and_dual_space} that any vector space \( V \) can be embedded into its dual \( V^* \). The dual can, in turn, be embedded into the double dual \( V^{**} \).
...
\begin{remark}\label{rem:linear_functionals_over_c}
  \Cref{thm:linear_functionals_over_c} allows us to identify the dual space \( X* \) of a complex vector space \( X \) with \( \hom(X, \BbbR) \) in the case of an algebraic \hyperref[def:dual_vector_space]{dual} or with the corresponding subspace in the case of a \hyperref[def:continuous_dual_space]{continuous dual space}.
