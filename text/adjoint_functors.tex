...
\begin{definition}\label{def:universal_morphism}\mcite[55]{MacLane1998CategoryTheory}
  ...
  \begin{thmenum}
    ...
    As shown in \cref{thm:hom_adjunction_from_universal_morphism/counit}, this property can be used to characterize a \hyperref[def:adjoiunt_functor]{right adjoint functor}.
  \end{thmenum}
...
\end{theorem}
\begin{comments}
  \item \Cref{thm:hom_adjunction_from_universal_morphism/unit} implies that \( D \) is \hyperref[def:adjoint_functor]{left adjoint} to \( U \).
\end{comments}
\begin{proof}
...
\begin{theorem}[Free category universal property]\label{thm:free_category_universal_property}
  ...
  The \hyperref[def:directed_multigraph_free_category]{free category} \( F(G) \) is the unique up to an isomorphism category that satisfies the following \hyperref[con:universal_property]{universal property}:
  \begin{displayquote}
...
\end{theorem}
\begin{comments}
  \item \Cref{thm:hom_adjunction_from_universal_morphism/unit} implies that \( F \) is \hyperref[def:adjoint_functor]{left adjoint} to \( U \).
\end{comments}
\begin{proof}
...
\begin{definition}\label{def:adjoint_functor}\mimprovised
  ...
  We will usually use \cref{thm:hom_adjunction_from_universal_morphism} to characterize an adjunction via \hyperref[def:universal_morphism]{universal morphisms}.
\end{definition}
\begin{comments}
...
\begin{proof}
  ...
  For the latter, \cref{thm:hom_adjunction_from_universal_morphism/unit} implies that \( \varepsilon^\oppos \) is the unit of an adjunction if and only if, for every object \( X \) in \( \cat{D}^\oppos \), the pair \( (X, \varepsilon_X^*) \) satisfies the following universal property:
  \begin{displayquote}
...
\begin{example}\label{ex:def:adjoint_functor}
  ...
  \begin{thmenum}
    \thmitem{ex:def:adjoint_functor/free_construction} The free constructions, discussed in \cref{con:free_construction}, follow the pattern from \cref{thm:hom_adjunction_from_universal_morphism/unit}.

    A simple example is \fullref{thm:discrete_category_universal_property}, whose universal property we restate here:
    \begin{displayquote}
      For every category \( \cat{C} \) and every function \( f: A \to U(\cat{C}) \), there exists a unique \hyperref[def:functor]{functor} \( \widetilde{f}: F(A) \to \cat{C} \) such that the following diagram commutes:
      \begin{equation*}
        \begin{aligned}
