...
\begin{definition}\label{def:universal_morphism}\mcite[55]{MacLane1998CategoryTheory}
  ...
  \begin{thmenum}
    ...
    \begin{displayquote}
      For every object \( X \) in \( \cat{D} \) and every morphism \( {f: A \to U(X)} \) in \( \cat{C} \), there exists a unique morphism \( \widetilde{f}: \widetilde{A} \to X \) such that the following diagram commutes:
      \begin{equation*}
        \begin{aligned}
    ...
    \begin{displayquote}
      For every object \( X \) in \( \cat{D} \) and every morphism \( f: U(X) \to A \) in \( \cat{C} \), there exists a unique morphism \( \widetilde{f}: X \to \widetilde{A} \) such that the following diagram commutes:
      \begin{equation*}
        \begin{aligned}
  ...
  We refer to either \( (A, \iota) \) a \term[en=universal morphism (\cite[233]{MacLane1971CategoricalAlgebraFoundations}) / universal arrow (\cite[55]{MacLane1998CategoryTheory})]{universal morphism}.
\end{definition}
...
\begin{definition}\label{def:hom_adjunction}\mcite[def. 4.1.1]{Perrone2024StartingCategoryTheory}
  Suppose that \( \cat{C} \) and \( \cat{D} \) are \hyperref[def:small_category]{locally small} categories. A \term[en=adjunction (\cite[def. 4.1.1]{Perrone2024StartingCategoryTheory})]{\( \hom \)-adjunction} between the oppositely-directed \hyperref[def:functor]{functors} \( F: \cat{C} \to \cat{D} \) and \( G: \cat{D} \to \cat{C} \) is a \hyperref[thm:natural_isomorphism]{natural isomorphism} \( \alpha \) with components
...
\begin{definition}\label{def:unit_counit_adjunction}\mcite[thm. 4.2.17]{Perrone2024StartingCategoryTheory}
  A \term[en=adjunction (\cite[def. 4.1.1]{Perrone2024StartingCategoryTheory})]{unit-counit adjunction} between the oppositely-directed \hyperref[def:functor]{functors} \( F: \cat{C} \to \cat{D} \) and \( G: \cat{D} \to \cat{C} \) is a pair of \hyperref[thm:natural_isomorphism]{natural transformations}
  \begin{align*}
...
\begin{proposition}\label{thm:hom_adjunction_induces_unit_counit_adjunction}
  For every \hyperref[def:hom_adjunction]{\( \hom \)-adjunction} \( \alpha \) between \( F: \cat{C} \to \cat{D} \) and \( G: \cat{D} \to \cat{C} \), the indexed families
  \begin{align*}
...
\begin{definition}\label{def:adjoint_functor}\mimprovised
  ...
  Usually, we will use \cref{thm:hom_adjunction_from_universal_morphism} to construct an adjunction via \hyperref[def:universal_morphism]{universal morphisms}.
\end{definition}
\begin{comments}
...
\begin{proof}
  ...
  This finishes the proof for left adjoints. The other direction is dual. If \( G' \) and \( G^\dprime \) are two right adjoints to \( F \), then by \cref{thm:category_adjunction_duality}, \( G'^\oppos \) and \( {G^\dprime}^\oppos \) are left adjoints and are thus isomorphic. Then by \cref{thm:morphism_invertibility_duality}, \( G' \) and \( G^\dprime \) are also isomorphic.
\end{proof}
