...
\begin{definition}\label{def:functor}\mcite[def. 1.2.1 \\ def. 1.2.10]{Leinster2016Basic}
  Fix some \hyperref[def:category]{categories} \( \cat{C} \) and \( \cat{D} \). A \term{functor} \( F: \cat{C} \to \cat{D} \) is a \hyperref[directed_multigraph/homomorphism]{directed multigraph homomorphism} between the underlying multigraphs that is compatible with composition and identities.
...
\end{definition}
\begin{defproof}
  The definition \eqref{eq:def:functor_as_family_of_function} ensures that the directed multigraph homomorphism conditions \eqref{eq:def:directed_multigraphs/homomorphism/head} and \eqref{eq:def:directed_multigraphs/homomorphism/terminal} hold.

  Indeed, for any morphism \( f: A \to B \) in \( \cat{C} \) we have
  \begin{equation*}
    F(\dom(f)) = F(A) = \dom(F(f)),
  \end{equation*}
  which implies \eqref{eq:def:directed_multigraphs/homomorphism/head}. We also have
  \begin{equation*}
    F(\co\dom(f)) = F(B) = \co\dom(F(f)),
  \end{equation*}
  which implies \eqref{eq:def:directed_multigraphs/homomorphism/terminal}.
\end{defproof}
...
\begin{definition}\label{def:subcategory}\mcite[def. 1.2.18]{Leinster2016Basic}
  ...
  \begin{itemize}
    \item The underlying multigraph \( U(\cat{D}) \) is a \hyperref[def:directed_multigraph/submodel]{subgraph} of \( U(\cat{C}) \). That is, every object in \( \cat{D} \) is an object in \( \cat{C} \) and every morphism in \( \cat{D} \) is a morphism in \( \cat{C} \).
  ...
  \begin{thmenum}
    ...
    \thmitem{def:subcategory/full} We say that \( \cat{D} \) is a \term{full subcategory} if the underlying multigraph \( U(\cat{C}) \) is a \hyperref[def:directed_multigraph/submodel]{full subgraph}. That is, in case \( \cat{D}(A, B) = \cat{C}(A, B) \) for every pair of objects \( A \) and \( B \) of \( \cat{D} \).
...
\begin{definition}\label{def:opposite_functor}\mcite[33]{MacLane1998}
...
\begin{proposition}\label{thm:commutative_diagrams_preserved_and_reflected}
  ...
  More precisely, let \( \cat{C} \) be an arbitrary category, let \( D \) be a diagram in \( \cat{C} \), and let \( p = (A, f_1, \ldots, f_n) \) and \( q = (A, g_1, \ldots, g_m) \) be two \hyperref[def:directed_multigraph_path/directed]{directed paths} with the same endpoints in \( D \).
...
\begin{example}\label{ex:directed_multigraphs_as_functors}\mcite[exmpl. 1.3.46]{Perrone2019}
  ...
  \begin{equation}\label{eq:ex:directed_multigraphs_as_functors/index/dots}
    \begin{aligned}
      \includegraphics[page=1]{output/ex__directed_multigraphs_as_functors.pdf}
    \end{aligned}
  \end{equation}
  ...
  \begin{equation}\label{eq:ex:directed_multigraphs_as_functors/index/annotated}
    \begin{aligned}
      \includegraphics[page=2]{output/ex__directed_multigraphs_as_functors.pdf}
    \end{aligned}
  \end{equation}
  ...
  \begin{equation}\label{eq:ex:directed_multigraphs_as_functors/index/diagram}
    \begin{aligned}
      \includegraphics[page=3]{output/ex__directed_multigraphs_as_functors.pdf}
      \quad\quad\quad\quad
      \includegraphics[page=4]{output/ex__directed_multigraphs_as_functors.pdf}
    \end{aligned}
  \end{equation}
