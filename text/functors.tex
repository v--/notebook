...
\begin{definition}\label{def:functor}\mcite[def. 1.2.1; def. 1.2.10]{Leinster2014BasicCategoryTheory}
  Fix some \hyperref[def:category]{categories} \( \cat{C} \) and \( \cat{D} \). A \term{functor} \( F: \cat{C} \to \cat{D} \) is a \hyperref[def:directed_multigraph/morphism]{directed multigraph homomorphism} between the underlying multigraphs that is compatible with composition and identities.
  ...
  \begin{thmenum}[series=def:functor]
    \thmitem{def:functor/obj}
    \thmitem{def:functor/hom}
  \end{thmenum}

  \begin{thmenum}[resume=def:functor]
    \thmitem{def:functor/domain_and_codomain} We say that the category \( \cat{C} \) is the \term{domain} and \( \cat{D} \) --- the \term{codomain} of the functor \( F \). These are technically not the domain and codomain of \( F \) when regarded as a function, however it is consistent with \cref{def:category_of_small_categories}.

    \thmitem{def:functor/endofunctor} Similarly to \cref{def:function/endofunction} for functions, if the domain \( \cat{C} \) and codomain \( \cat{D} \) of a functor coincide, we say that it is an \term{endofunctor}.
  \end{thmenum}
...
\end{definition}
\begin{defproof}
  The definition \eqref{eq:def:functor_as_family_of_function} ensures that the directed multigraph homomorphism conditions \eqref{eq:def:directed_multigraph/morphism/head} and \eqref{eq:def:directed_multigraph/morphism/tail} hold.
...
\begin{remark}\label{rem:functor_size}
  It is possible that \( \cat{C} \) is \hyperref[def:small_set]{small} in the sense of \cref{def:small_category}, but the \hyperref[def:functor]{functor} \( F \), as the set \eqref{eq:def:functor_as_single_function}, is not \hyperref[def:small_set]{small} in the sense of \cref{def:small_set}. Without using universes, we cannot prove the existence of any functor from the category of smalls sets to itself, for example.
\end{remark}
...
\begin{definition}\label{def:subcategory}\mcite[def. 1.2.18]{Leinster2014BasicCategoryTheory}
  We call the category \( \cat{D} \) a \term{subcategory} of \( \cat{C} \) if the following hold:
  \begin{itemize}
    \item The underlying multigraph \( U(\cat{D}) \) is a \hyperref[def:directed_multigraph/subobject]{subgraph} of \( U(\cat{C}) \). That is, every object in \( \cat{D} \) is an object in \( \cat{C} \) and every morphism in \( \cat{D} \) is a morphism in \( \cat{C} \).
...
\begin{remark}\label{rem:contravariant_functor}\mcite[def. 1.2.10]{Leinster2014BasicCategoryTheory}
...
\begin{example}\label{ex:dual_space_contravariant_functor}
  ...
  Unfortunately, \( F(f) \) is supposed to be a morphism from \( V^* \) to \( W^* \), but is actually a morphism from \( W^* \) to \( V^* \). This makes \( F \) a \hyperref[rem:contravariant_functor]{contravariant functor} or, equivalently, a functor from \( \cat[\BbbK]{Vect}^\oppos \) to \( \cat[\BbbK]{Vect} \).
\end{example}
...
\begin{proof}
  ...
  \SubProofOf{thm:def:functor/inverses} If \( f^{-1} \) is a left inverse of \( f \), by \cref{thm:def:functor/half_inverses} we have that \( F(f^{-1}) \) is a left inverse of \( F(f) \). But \( F(f^{-1}) \) is also a right inverse, and again by \cref{thm:def:functor/half_inverses} \( F(g) \) is a right inverse of \( F(f^{-1}) \).
  ...
  \SubProofOf{thm:def:functor/isomorphisms} Follows from \cref{thm:def:functor/inverses}.
\end{proof}

\begin{definition}\label{def:category_of_small_categories}
  We denote the \hyperref[def:category]{category} of \hyperref[def:small_set]{small} \hyperref[def:category]{categories} by \( \cat{Cat} \) or, if the universe is clear from the context, simply by \( \cat{Cat} \). See \cref{def:small_category} for a further discussion of universes and categories.

  \begin{itemize}
    \item The \hyperref[def:category/objects]{set of objects} \( \obj(\cat{Cat}) \) is the set of all \hyperref[def:small_set]{small} categories.
...
\begin{defproof}
  ...
  \SubProofOf{def:category/C1} For every two \hyperref[def:small_set]{small} categories \( \cat{C} \) and \( \cat{D} \) and every functor \( F: \cat{C} \to \cat{D} \), for every object \( A \in \cat{C} \) we have
  \begin{equation*}
...
\begin{definition}\label{def:universal_categories}
  The \hyperref[def:category_of_small_categories]{category of small categories} \( \cat{Cat} \) has an initial and a terminal object.

  Similarly to how we use the \hyperref[def:ordinal]{ordinals} \( 0 \) and \( 1 \) to denote the initial and terminal object in the category of small sets, we denote the initial category by \( \cat{0} \) and the final category by \( \cat{1} \). Note that the final category is only unique up to an isomorphism.
...
\begin{definition}\label{def:opposite_functor}\mcite[33]{MacLane1998CategoryTheory}
  ...
  This is somewhat in contrast to the general practice of inverting morphisms when taking opposites. Thus, we have the \hyperref[rem:contravariant_functor]{contravariant} \term{oppositization functor}
  \begin{equation*}
    (\anon*)^\oppos: \cat{Cat}^\oppos \to \cat{Cat}.
  \end{equation*}

  As an \hyperref[def:function/endofunction]{endofunction} on \( \obj(\cat{Cat}) \), the oppositization functor is clearly an \hyperref[def:morphism_invertibility/involution]{involution}.
...
\begin{example}\label{ex:def:functor_invertibility}
  \hfill
  \begin{thmenum}
    \thmitem{ex:def:functor_invertibility/power} The power set functor described in \cref{ex:unary_functors_in_set} is clearly \hyperref[def:functor_invertibility/injective_on_morphisms]{injective on morphisms}, hence by \cref{thm:def:functor_invertibility/injective}, it is also \hyperref[def:functor_invertibility/injective_on_objects]{injective on objects} and \hyperref[def:functor_invertibility/faithful]{faithful}.
...
\begin{remark}\label{rem:natural_transformations_into_set}
  ...
  For example, consider the power set functor \( \pow: \cat{Set} \to \cat{Set} \) discussed in \cref{ex:unary_functors_in_set}. The \hyperref[def:set_valued_map/identity]{identity function} \( \id_{\mscrU_0} \) is then a natural transformation from the identity functor \( \id_{\cat{Set}} \) to \( \pow \).
