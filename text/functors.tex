...
\begin{definition}\label{def:subcategory}\mcite[def. 1.2.18]{Leinster2016Basic}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:subcategory/full} We say that \( \cat{D} \) is a \term{full subcategory} if the underlying multigraph \( U(\cat{C}) \) is a \hyperref[def:directed_multigraph/subgraph]{full subgraph}. That is, in case \( \cat{D}(A, B) = \cat{C}(A, B) \) for every pair of objects \( A \) and \( B \) of \( \cat{D} \).
...
\begin{definition}\label{def:opposite_functor}\mcite[33]{MacLane1998}
  ...
  As an \hyperref[def:function/endofunction]{endofunction} on \( \obj(\ucat{Cat}) \), the oppositization functor is clearly an \hyperref[def:set_with_involution]{involution}.
...
\begin{example}\label{ex:directed_multigraphs_as_functors}\mcite[example 1.3.46]{Perrone2019}
...
\begin{definition}\label{def:function_currying}\cite[12]{Farmer2008}
  Given the \hyperref[rem:function_arguments]{binary} function \( f: A \times B \to C \), we can define another function \( g: A \to \fun(B, C) \) as
  \begin{equation*}
