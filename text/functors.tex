...
\begin{definition}\label{def:categorical_diagram}
  ...
  We say that the diagram \( D \) over \( \cat{C} \) \term{commutes} if, whenever \( p = (f_1, \ldots, f_n) \) and \( q = (g_1, \ldots, g_m) \) are two \hyperref[def:graph_walk/path/directed]{directed paths} in \( D(\cat{I}) \) with identical endpoints and either \( n > 1 \) or \( m > 1 \), then
  \begin{equation*}
...
\begin{proposition}\label{thm:commutative_diagrams_preserved_and_reflected}
  ...
  More precisely, let \( \cat{C} \) be an arbitrary category, let \( D \) be a diagram in \( \cat{C} \), and let \( p = (A, f_1, \ldots, f_n) \) and \( q = (A, g_1, \ldots, g_m) \) be two \hyperref[def:graph_walk/path/directed]{directed paths} with the same endpoints in \( D \).
