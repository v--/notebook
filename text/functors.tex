...
\begin{proposition}\label{thm:def:functor_invertibility}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:def:functor_invertibility/faithful_reflects_isomorphisms} A \hyperref[def:functor_invertibility/faithful]{faithful} functor reflects isomorphisms. That is, for every functor \( F: \cat{C} \to \cat{D} \) and morphisms \( f: A \to B \) and \( g: B \to A \) in \( \cat{C} \), if \( F(g) \) is a full inverse of \( F(f) \), then \( g \) is an inverse of \( f \).
...
\begin{proof}
  ...
  \SubProofOf{thm:def:functor_invertibility/faithful_reflects_isomorphisms} Suppose that \( F(g) \) is a two-sided inverse of \( F(f) \). We have
  \begin{equation*}
    F(g \bincirc f) = F(g) \bincirc F(f) = \id_{F(A)} = F(\id_A).
  \end{equation*}

  Since \( F \) is faithful, it follows that \( g \bincirc f = \id_A \). We similarly conclude that \( f \bincirc g = \id_B \).
...
\begin{definition}\label{def:diagonal_functor}\mcite[exercise 3.1.1]{Leinster2014BasicCategoryTheory}
