...
\begin{example}\label{ex:def:category_adjunction}
  ...
  \begin{thmenum}
    ...
    For every functor \( S: F(Q) \to \cat{C} \), define \( \varphi_{Q, \cat{C}} \) so that it sends \( S \) to a \hyperref[def:directed_multigraph_path]{directed path} containing only one arc.

    Qefine the canonical embedding \( \iota \):
    \begin{equation*}
      \begin{aligned}
        &\iota: Q \to [U \bincirc F](Q) \\
        &\iota_V(v) \coloneqq v, \\
        &\iota_A(e) \coloneqq i(e) \overset e \to t(e).
      \end{aligned}
    \end{equation*}
