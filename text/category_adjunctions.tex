...
\begin{proposition}\label{thm:universal_objects_as_adjunctions}
  Fix a category \( \cat{C} \). We can characterize the universal objects in \( \cat{C} \) from \cref{def:universal_objects} via adjunctions with the \hyperref[def:universal_categories]{terminal category} \( \cat{1} \).
  ...
  \begin{thmenum}
    ...
    In particular, the uniqueness proved in \cref{thm:def:universal_objects/initial} follows from \cref{thm:functor_adjoint_uniqueness}.
...
\begin{proposition}\label{thm:concrete_category_function_invertibility}
  ...
  \begin{thmenum}
    \thmitem{thm:concrete_category_function_invertibility/injective} Every \hyperref[def:function_invertibility/injective]{injective} morphism in \( \cat{C} \) is a \hyperref[def:morphism_invertibility/left_cancellative]{monomorphism}.

    The converse holds if \( U \) has a \hyperref[def:category_adjunction]{left adjoint}, or if the monomorphism splits.

    \thmitem{thm:concrete_category_function_invertibility/surjective} Dually, every \hyperref[def:function_invertibility/surjective]{surjective} morphism in \( \cat{C} \) is an \hyperref[def:morphism_invertibility/right_cancellative]{epimorphism}.
...
\begin{proof}
  ...
  \Cref{thm:function_invertibility_categorical/left_cancellative} implies that \( U(f) \) is left-cancellative on set-theoretic functions. \Cref{thm:def:functor_invertibility/faithful_reflects_cancellative} implies that \( f \) is also left-cancellative.
  ...
  Since \( f \) is left-cancellative, we get \( \varphi_{S,A}(g_1) = \varphi_{S,A}(g_2) \). The function \( \varphi_{S,A} \) is injective, so \( g_1 = g_2 \).

  We conclude that \( U(f) \) is also left-cancellative. \Cref{thm:function_invertibility_categorical/left_cancellative} implies that \( U(f) \) is injective.
