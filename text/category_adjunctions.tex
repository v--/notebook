...
\begin{concept}\label{con:univalence_principle}
  Isomorphic objects are frequently identified, in a tradition somewhat resembling the \enquote{identity of indiscernibles} principle discussed in \fullref{rem:identity_of_indiscernibles}. We will refer to this principle as \term{univalence}, and will now proceed to justify the name.

  \incite[266]{Bourbaki2004TheoryOfSets}, when trying to formalize a general notion of mathematical structure, classify structures into \enquote{species} (\enquote{esp\`ece}; what we now call \enquote{signatures}) and call a species \enquote{univalent} if any two of its structures are isomorphic (what we now call \enquote{categorical}).

  Whether it is Bourbaki's influence or not, Vladimir Voevodski is credited to the \enquote{univalence axiom}, which, in his own words,
  \begin{displayquote}
    \textellipsis provides a natural way to assert that \enquote{two homotopy equivalent types are equal}.
  \end{displayquote}

  Voevodski himself calls it the \enquote{equivalence axiom} in his second talk on the subject, transcribed in \cite{Voevodsky2014EquivalenceAxiom}. Even so, his proposed axiom lead to the name \enquote{univalent foundations} for the usage of \hyperref[con:homotopy_type_theory]{homotopy type theory} as a \hyperref[con:logical_system]{logical system}. In \cite[4]{UnivalentProject2024OctoberHoTT} this principle is described as follows:
  \begin{displayquote}
    The univalence axiom implies, in particular, that isomorphic structures can be identified, a principle that mathematicians have been happily using on workdays, despite its incompatibility with the \enquote{official} doctrines of conventional foundations.
  \end{displayquote}
\end{concept}

\begin{remark}\label{rem:free_construction_univalence}
  If \( X \) is isomorphic to the \hyperref[con:free_construction]{free object} \( F(A) \), we also refer to \( X \) endowed with this isomorphism as \enquote{free}. This is in line with \hyperref[con:univalence_principle]{univalence} --- the practice of regarding isomorphic objects as equal. In this case \( F(A) \) is \enquote{the free object on \( A \)}, while \( X \) is simply a \enquote{free object}.

  Such identification is common for \hyperref[def:module]{module over rings}, where such an isomorphism is equivalent to the existence of a \hyperref[def:hamel_basis]{Hamel basis}. See \fullref{rem:free_module} for a broader discussion of the latter.
\end{remark}
