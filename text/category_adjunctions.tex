...
\begin{definition}\label{def:concrete_category}\mcite[26]{MacLane1998CategoryTheory}
  ...
  We list several examples in \cref{ex:def:category_adjunction}. The forgetful functor is usually clear from the context, and we identify a concrete category \( (\cat{C}, U) \) with its underlying set \( \cat{C} \). The corresponding free functor, however, often requires a nontrivial but straightforward construction.
\end{definition}
...
\begin{theorem}[Discrete category universal property]\label{thm:discrete_category_universal_property}
  Denote by \( U \) the \hyperref[def:concrete_category]{forgetful functor} which sends a \hyperref[def:category]{category} to its set of objects.
...
\begin{proof}
  ...
  \Cref{thm:function_invertibility_categorical/left_cancellable} implies that \( U(f) \) is left cancellable on set-theoretic functions. \Cref{thm:def:functor_invertibility/faithful_reflects_cancellative} implies that \( f \) is also left cancellable.
  ...
  If \( U(f) \) is empty, it is injective as a consequence of \cref{thm:def:functor_invertibility/preserves_inverses}.
  ...
  \Cref{thm:def:functor_invertibility/preserves_inverses} implies that \( U(f^{-1}) \) is the two-sided inverse of \( U(f) \), i.e. \( U(f) \) is bijective.
\end{proof}
