...
\begin{remark}\label{rem:free_construction_univalence}
  ...
  Such identification is common for \hyperref[def:module]{module over rings}, where such an isomorphism is equivalent to the existence of a \hyperref[def:hamel_basis]{Hamel basis}. See \cref{rem:free_module} for a broader discussion of the latter.
\end{remark}
...
\begin{remark}\label{rem:category_adjunctions_category_order}
  \hyperref[def:category_adjunction]{Adjunctions} (and also \hyperref[def:equivalence_of_categories]{equivalences}) of categories involve a pair of opposite functors.

  Namely, \( F: \cat{C} \to \cat{D} \) is left adjoint to \( G: \cat{D} \to \cat{C} \) if it there exists a \hyperref[def:natural_isomorphism]{natural isomorphism}
  \begin{equation*}
    \varphi: \cat{D}(F(\anon*), \anon*) \Rightarrow \cat{C}(\anon*, G(\anon*)).
  \end{equation*}

  The role of \( G \) is often that of a \hyperref[con:forgetful_functor]{forgetful functor}, making \( F \) the focus of the adjunction and \( \cat{D} \) the target of our free constructions. In fact, instead of the symbol \enquote{\( G \)}, \textcite[def. 9.1]{
  Awodey2010CategoryTheory} prefers the symbol \( U \) that is often used for forgetful functors.

  Different authors prefer different letters for denoting the categories \( \cat{C} \) and \( \cat{D} \) (e.g. \( \mscrA \) and \( \mscrB \)), but the obvious alphabetic order is followed in
  \cite[def. 9.1]{Awodey2010CategoryTheory},
  \cite[def. 4.1.1]{Perrone2024StartingCategoryTheory},
  \cite[def. 2.1.1]{Leinster2014BasicCategoryTheory},
  \cite[116]{Riehl2016CategoryTheory},
  \cite[493]{Aluffi2009Algebra} and
  \cite[178]{ЦаленкоШульгейфер1974ОсновыТеорииКатегорий}.

  Although this difference is only notational, there is an opposite convention, where \( \cat{C} \) and \( \cat{D} \) are swapped and the free constructions belong to \( \cat{C} \). This can lead to some confusion since the roles of \( \cat{C} \) and \( \cat{D} \) are asymmetric. This latter convention is used in
  \cite[80]{MacLane1998CategoryTheory} and
  \cite[def. 16.4.1]{Schubert1972Categories}.

  We use the alphabetic order, which seems to dominate.
\end{remark}
...
\begin{definition}\label{def:concrete_category}\mcite[def. 5.1]{AdámekHerrlichStrecker1990JoyOfCats}
  A \term[en=concrete category (\cite[def. 5.1]{AdámekHerrlichStrecker1990JoyOfCats})]{relatively concrete category} over the \term{base category} \( \cat{X} \) is a pair \( (\cat{C}, U) \), where \( \cat{C} \) is a category and \( U: \cat{C} \to \cat{X} \) is a \hyperref[def:functor_invertibility/faithful]{faithful functor}.

  If \( \cat{X} \) is the category \hyperref[def:category_of_small_sets]{\( \cat{Set} \)} of \hyperref[def:small_set]{small sets}, we will simply call \( (\cat{C}, U) \) a \term[ru=конкретная категория (\cite[228]{ЦаленкоШульгейфер1974ОсновыТеорииКатегорий}), en=concrete category (\cite[26]{MacLane1998CategoryTheory})]{concrete category}. For every morphism \( f: A \to B \) in \( \cat{C} \), \( U(f) \) is a function from the (small) set \( U(A) \) to \( U(B) \).
\end{definition}
\begin{comments}
  \item This definition of relatively concrete (with respect to \( \cat{X} \)) categories is taken from \cite[def. 5.1]{AdámekHerrlichStrecker1990JoyOfCats}, although the authors use \enquote{concrete} rather than \enquote{relatively concrete}, and call a category concrete over \( \cat{Set} \) a \enquote{construct}. The book is based around the more general relatively concrete categories.

  Usually, only concrete categories over \( \cat{Set} \) are considered, for example in \cite[26]{MacLane1998CategoryTheory} and \cite[def. 1.6.17]{Riehl2016CategoryTheory}. This is pushed even further in \cite[228]{ЦаленкоШульгейфер1974ОсновыТеорииКатегорий}, where \( U \) is assumed to be a full embedding and not merely a faithful functor.
...
\begin{concept}\label{con:forgetful_functor}
  In a \hyperref[def:concrete_category]{concrete category} \( (\cat{C}, U) \), we call \( U \) a \term[en=forgetful]{forgetful functor} and any \hyperref[def:category_adjunction]{left adjoint} to \( U \) a \term{free functor}.
\end{concept}
