...
\begin{remark}\label{rem:matrices_as_bilinear_forms}
  Similarly to what we discussed in \cref{rem:matrices_as_functions}, matrices correspond to linear functions and vice versa. Square matrices correspond to \hyperref[def:bilinear_form]{bilinear forms}.

  Let \( e_1, \ldots, e_n \) be the \hyperref[def:coordinate_space]{standard basis} of \( \BbbK^n \). This basis allows us to identify vectors of \( \BbbK^n \) with column vectors. To every \( n \times n \) matrix \( A \), there corresponds a bilinear form
  \begin{equation*}
...
\begin{proposition}\label{thm:quadratic_forms}
  ...
  \emph{Real} quadratic forms are simply quadratic forms over \( \BbbK = \BbbR \). \emph{Complex} quadratic forms, by contrast, differ from the general theory --- see \cref{rem:complex_quadratic_form}.
\end{proposition}
\begin{proof}
...
\begin{definition}\label{def:complexification}\mcite[def. 2.1]{ConradBlurb:Complexification}
  Let \( V \) be a real vector space. The \term{complexification} \( V^\BbbC \) of \( V \) is the \hyperref[def:external_semimodule_direct_sum]{direct sum} \( V \oplus V \) equipped with the canonical inclusion
  \begin{equation*}
    \begin{aligned}
...
\begin{theorem}[Complexification universal property]\label{thm:complexification_universal_property}
  The \hyperref[def:complexification]{complexification} \( V^\BbbC \) of a real vector space \( V \) satisfies the following \hyperref[con:universal_property]{universal property}:
  \begin{displayquote}
  ...
  \Cref{thm:hom_adjunction_from_universal_morphism/unit} implies that \( (\anon*)^\BbbC \) is \hyperref[def:adjoint_functor]{left adjoint} to the \hyperref[def:complexification]{decomplexification} \hyperref[con:forgetful_functor]{forgetful functor}
  \begin{equation*}
...
\begin{definition}\label{def:antilinear_function}\mimprovised
  We say that the function \( L: V \to W \) between complex vector spaces is \term{antilinear} if it satisfies the additivity condition \eqref{eq:def:additive_function} and if
  \begin{equation}\label{eq:def:antilinear_function}
    L(tx) = \oline t L(x).
  \end{equation}

  That is, we enhance the homogeneity condition \eqref{eq:def:real_homogeneous_function} from \cref{def:semimodule/morphism} with \hyperref[def:complex_numbers]{complex conjugation}.
\end{definition}

\begin{definition}\label{def:sesquilinear_form}\mcite[258]{Knapp2016BasicAlgebra}
  A \term{sesquilinear\fnote{\enquote{sesqui} is a Latin prefix meaning \enquote{one and a half}} form} over the \hyperref[def:complex_numbers]{complex} vector space \( V \) is a function \( L: V \times V \to \BbbC \) that is \hyperref[def:linear_function]{linear} in the first argument and \hyperref[def:antilinear_function]{antilinear} in the second.
...
\begin{remark}\label{rem:complex_quadratic_form}
  ...
  \begin{equation*}
    L(x, x) = \oline {L(x, x)},
  \end{equation*}
  which ensures that \( Q(x) \coloneqq L(x, x) \) is always a real number. Thus, we regard \( Q \) as a function from \( V \) to \( \BbbR \). This is not a quadratic form in the sense of \cref{thm:quadratic_forms}, but we will call it a \term{complex quadratic form}.
...
\begin{definition}\label{def:inner_product_space}\mimprovised
  A \term{real inner product space} is a vector space \( V \) over \( \BbbR \) equipped with a \hyperref[def:real_function_definiteness]{positive definite} \hyperref[def:symmetric_function]{symmetric} \hyperref[def:bilinear_form]{bilinear form}
  \begin{equation*}
    \inprod \anon \anon: V \times V \to \BbbR.
  \end{equation*}

  A \term{complex inner product space} is a vector space \( V \) over \( \BbbC \) equipped with a \hyperref[def:real_function_definiteness]{positive definite} \hyperref[def:hermitian_form]{Hermitian} \hyperref[def:sesquilinear_form]{sesquilinear form}
  \begin{equation*}
    \inprod \anon \anon: V \times V \to \BbbC.
  \end{equation*}

  This notation is generalized for application of linear functionals --- see \cref{rem:dual_space_bilinear_form}.
...
\begin{remark}\label{rem:inner_product_basis_decomposition}
  Let \( V \) be an \hyperref[def:inner_product_space]{inner product space} over \( \BbbC \) and let \( e_1, \ldots, e_n \) be an orthonormal basis of \( V \). By \cref{thm:basis_projection_orthonormal},
  \begin{equation*}
...
\begin{theorem}[Generalized Pythagorean theorem]\label{thm:generalized_pythagorean_theorem}
...
\begin{proof}
  ...
  \begin{equation*}
    \lin\set{ f_1, \ldots, f_n } = \lin\set{ e_1, \ldots, e_n }.
  \end{equation*}
...
\end{proposition}
\begin{proof}
  \Cref{thm:finite_dimensional_orthonormal_basis_existence} ensures that an orthonormal basis \( e_1, \ldots, e_n \) of \( U \) exists. \Cref{thm:def:vector_space/expansion} allows us to expand this to a basis of \( V \) and \fullref{thm:gramm_schmidt_orthogonalization} allows us to orthogonalize the entire basis of \( V \). Let \( e_{n+1}, \ldots, e_m \) be the remaining vectors of the orthogonalized basis. Then
  \begin{equation*}
    U = \lin{ e_1, \ldots, e_n }.
  \end{equation*}
  ...
  \begin{equation*}
    \lin{ e_{n+1}, \ldots, e_m } \subsetneq U^\perp.
  \end{equation*}

  If \( e_{m+1} \) is an additional basis vector of \( U^\perp \), then it is a basis vector of \( V \). But that would mean that both \( e_1, \ldots, e_m \) and \( e_1, \ldots, e_{m+1} \) are bases of \( V \), which contradicts \cref{thm:vector_space_dimension}.

  Hence,
  \begin{equation*}
    U^\perp = \lin{ e_{n+1}, \ldots, e_m }.
  \end{equation*}
...
\begin{proof}
  ...
  \begin{balign*}
    ...
    \inprod {x + y} {x + y}
    =    \\ &=
    Q(x) + \oline t \inprod x y +  \inprod y x + \abs{t}^2 Q(y)
    =    \\ &=
    Q(x) + 2\real \parens*{ t \oline{ \inprod x y} } + \abs{t}^2 Q(y).
  \end{balign*}
