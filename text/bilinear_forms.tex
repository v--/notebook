...
\begin{definition}\label{def:antilinear_function}\mimprovised
  ...
  That is, we enhance the homogeneity condition \eqref{eq:def:linear_function/homogeneity} from \fullref{def:linear_function} with \hyperref[def:complex_numbers]{complex conjugation}.
\end{definition}
...
\begin{proposition}\label{thm:quadratic_forms_are_nondegenerate}
  \hyperref[thm:real_valued_definiteness]{Definite} \hyperref[thm:quadratic_forms]{quadratic forms} are \hyperref[def:degenerate_bilinear_form]{nondegenerate}.
\end{proposition}
\begin{proof}
...
\begin{definition}\label{def:inner_product_space}\mimprovised
  A \term{real inner product space} is a vector space \( V \) over \( \BbbR \) equipped with a \hyperref[thm:real_valued_definiteness/positive_definite]{positive definite} \hyperref[def:symmetric_function]{symmetric} \hyperref[def:bilinear_form]{bilinear form}
  \begin{equation*}
    \inprod \anon \anon: V \times V \to \BbbR.
  \end{equation*}

  A \term{complex inner product space} is a vector space \( V \) over \( \BbbC \) equipped with a \hyperref[thm:real_valued_definiteness/positive_definite]{positive definite} \hyperref[def:hermitian_form]{Hermitian} \hyperref[def:sesquilinear_form]{sesquilinear form}
  \begin{equation*}
