...
\begin{definition}\label{def:zero_locus}\mimprovised
  ...
  We also call the members of \( f^{-1}(\vect 0) \) \term{roots} of \( f \), in analogy with \hyperref[def:multiple_root]{polynomial roots}.
...
\begin{example}\label{ex:real_algebraic_curves}
  ...
  The polynomial \( X^2 + Y^2 \) is irreducible \( \BbbR[X, Y] \) as shown in \fullref{thm:axx_byy_irreducible}. \Fullref{thm:quadratic_polynomial_irreducibility} implies that the set
  \begin{equation*}
...
\begin{definition}\label{def:ellipse}
  ...
  It is an \hyperref[def:affine_algebraic_set/curve]{algebraic curve} because, by \fullref{thm:axx_byy_czz_irreducible} and \fullref{thm:homogeneous_polynomial_constant}, the polynomial is irreducible over \( \BbbC \).
