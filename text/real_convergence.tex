...
\begin{proof}
  ...
  \Fullref{def:net_limit_point} is satisfied, hence \( \{ x_k \} \) converges to \( \overline x \).
...
\begin{proposition}\label{thm:real_monotone_sequence_converges_iff_bounded}
  A \hyperref[def:order_homomorphism]{monotone} sequence of real numbers \hyperref[def:net_limit_point]{converges} if and only if it is \hyperref[def:metric_space/bounded_sequence]{bounded}.
\end{proposition}
\begin{proof}
