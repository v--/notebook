...
\end{theorem}
\begin{proof}
  Let \( \seq{ x_k }_{k=1}^\infty \) be a fundamental sequence of real numbers. By \cref{thm:fundamental_sequence_is_bounded}, the sequence is bounded. By \fullref{def:bolzano_weierstrass}, it has a convergent subsequence
  \begin{equation*}
    \{ x_{k_m} \}_{m=1}^\infty \to x.
  \end{equation*}

  By \cref{thm:fundamental_subsequence_convergence}, the sequence itself has the same limit \( \lim_{k \to \infty} x_k = x \).
\end{proof}
...
\begin{proof}
  ...
  Fix \( \varepsilon > 0 \). Then by \cref{def:net_limit_point}, there exist indices \( k_0 \) and \( m_0 \) such that
  \begin{balign*}
...
\end{lemma}
\begin{proof}
  \SubProofOf{thm:squeeze_lemma/sequences} Fix \( \varepsilon > 0 \). Then by \cref{def:net_limit_point}, there exist indices \( k^- \) and \( k^+ \) such that
  \begin{balign*}
  ...
  \Cref{def:net_limit_point} is satisfied, hence \( \{ x_k \} \) converges to \( \oline x \).

  \SubProofOf{thm:squeeze_lemma/functions} The proof is analogous to that of \cref{thm:squeeze_lemma/sequences}, but the machinery is different. Fix \( \varepsilon > 0 \). Then by \cref{def:local_convergence/neighborhoods}, there exist radii \( \delta^- \) and \( \delta^+ \) such that
  \begin{balign*}
     & f_-(I \cap B(\oline x, \delta^-)) \subseteq B(\oline y, \tfrac \varepsilon 3) \\
     & f_+(I \cap B(\oline x, \delta^+)) \subseteq B(\oline y, \tfrac \varepsilon 3)
  \end{balign*}

  Take \( \delta < \min \{ \delta^-, \delta^+ \} \) and \( x \in I \cap B(\oline x, \delta) \). Analogously to our proof of \cref{thm:squeeze_lemma/sequences}, we obtain the inequality
  \begin{equation*}
  ...
  \end{equation*}
  holds and thus by \cref{def:local_convergence/neighborhoods}, the function \( f \) converges to \( \oline y \) at \( \oline x \).
\end{proof}
...
\end{proposition}
\begin{proof}
  \SufficiencySubProof Let \( \seq{ x_k }_{k=1}^\infty \) be a convergent monotone sequence. Denote its limit by \( x \). Fix \( \varepsilon > 0 \). By \cref{def:net_limit_point}, there exists \( k_0 \) such that
  \begin{equation*}
  ...
  Thus, \cref{def:net_limit_point} is satisfied and the sequence \( \seq{ x_k }_{k=1}^\infty \) converges.
\end{proof}
