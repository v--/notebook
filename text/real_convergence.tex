\subsection{Real convergence}\label{subsec:real_convergence}
...
\begin{lemma}[Squeeze lemma]\label{thm:squeeze_lemma}
  ...
  \begin{thmenum}
    \thmitem{thm:squeeze_lemma/sequences} Let \( \seq{ x_k }_{k=1}^\infty, \{ x_k^- \}_{k=1}^\infty, \{ x_k^+ \}_{k=1}^\infty \) be three sequences in \( I \). If both \( \{ x_k^- \}_{k=1}^\infty \) and \( \{ x_k^+ \}_{k=1}^\infty \) converge to the same value \( \overline x \in I \) and if the following inequalities
    \begin{equation*}
      x_k^- \leq x_k \leq x_k^+
    \end{equation*}
...
\begin{proof}
  ...
  Take \( \delta < \min \{ \delta^-, \delta^+ \} \) and \( x \in I \cap B(\overline x, \delta) \). Analogously to our proof of \fullref{thm:squeeze_lemma/sequences}, we obtain the inequality
  \begin{equation*}
...
\begin{proposition}\label{thm:real_monotone_sequence_converges_iff_bounded}
  A \hyperref[def:order_homomorphism]{monotone} sequence of real numbers \hyperref[def:net_convergence/limit]{converges} if and only if it is \hyperref[def:metric_space/bounded_sequence]{bounded}.
\end{proposition}
\begin{proof}
  \SufficiencySubProof Let \( \seq{ x_k }_{k=1}^\infty \) be a convergent monotone sequence. Denote its limit by \( x \). Fix \( \varepsilon > 0 \). By \fullref{def:net_convergence/limit}, there exists \( k_0 \) such that
  \begin{equation*}
  ...
  Thus, \fullref{def:net_convergence/limit} is satisfied and the sequence \( \seq{ x_k }_{k=1}^\infty \) converges.
\end{proof}
