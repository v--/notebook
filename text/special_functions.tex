...
\begin{theorem}[Stirling's gamma approximation]\label{thm:stirlings_gamma_approximation}\mcite[24]{Артин2009ГаммаФункция}
  ...
  \begin{equation}\label{eq:thm:stirlings_gamma_approximation/mu}
    \mu(x) \coloneqq \sum_{k=0}^\infty \parens*{ \parens*{ x + k + \frac 1 2 } \ln\parens*{ 1 + \frac 1 {x + k} } - 1 }.
  \end{equation}
...
\end{theorem}
\begin{comments}
  \item This approximation is implemented in \cite{notebook:code} as
    \WriteCodeRef{thm:stirlings_gamma_approximation}
\end{comments}

\paragraph{Logistic function}

\begin{definition}\label{def:logistic_function}
  \todo{Logistic function}
\end{definition}
