...
\begin{definition}\label{def:semigroup}\mcite[28]{Ляпин1960Полугруппы}
  ...
  \begin{thmenum}[series=def:semigroup]
    ...
    \thmitem{def:semigroup/exponentiation}\mcite[20]{Ляпин1960Полугруппы} We define an additional \term{exponentiation} operation for positive integers \( n \) \hyperref[rem:natural_number_recursion]{recursively} as
...
\begin{definition}\label{def:monoid}\mcite[def. 1.1]{Jacobson1985AlgebraPart1}
  ...
  \begin{thmenum}
    ...
    \thmitem{def:monoid/exponentiation}\mimprovised We extend \hyperref[def:semigroup/exponentiation]{semigroup exponentiation} to all nonnegative integers by additionally defining
    \begin{equation*}
      x^0 \coloneqq e.
    \end{equation*}
...
\begin{example}\label{ex:def:monoid}
  ...
  \begin{thmenum}
    ...
    \thmitem{ex:def:monoid/semilattice} If a \hyperref[def:semilattice]{join-semilattice} is bounded from below, it is a monoid as a consequence of \eqref{eq:def:bounded_lattice/theory/meet}. A similar statement holds for \hyperref[def:lattice/meet]{meet-semilattice}.
