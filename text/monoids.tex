...
\begin{definition}\label{def:operation_on_set}\mcite[12]{ЦаленкоШульеейфер1974}
  An \( n \)-ary \term[bg=операция (\cite[page 1 in \textparagraph 19]{НеновБожилов2007}), ru=операция (\cite[12]{ЦаленкоШульеейфер1974})]{operation} on a \hyperref[def:set]{plain set} \( A \) is a \hyperref[def:function]{function} from \( A^n \) to \( A \).
\end{definition}
\begin{comments}
...
\begin{definition}\label{def:semigroup}\mcite[144]{MacLane1998}
  A \term[bg=полугрупа (\cite[page 2 in \textparagraph 19]{НеновБожилов2007}), ru=полугруппа (\cite[28]{Ляпин1960})]{semigroup} is a nonempty set \( G \) equipped with a \hyperref[rem:function_arguments]{binary function} \( \cdot: G \times G \to G \), called the \term{semigroup operation}. We often denote this operation by juxtaposition as \( xy \) instead of \( x \cdot y \).
