...
\begin{definition}\label{def:operation_on_set}\mcite[\S 2.1]{Мальцев1970}
...
\begin{remark}\label{rem:trivial_object}
  Some algebraic theories have an established \term{trivial structure} --- for example the \term{trivial group} \( \set{ e } \) defined by Anthony Knapp in \cite[146]{Knapp2016BasicAlgebra} and Paolo Aluffi in \cite[42]{Aluffi2009}, also called a \term{zero group} by Saunders Mac Lane in \cite[25]{MacLane1998} or the \term{trivial module} \( \set{ 0 } \), used by Paolo Aluffi in \cite[175]{Aluffi2009}, also called a \term{zero module} by Anthony Knapp in \cite[146]{Knapp2016BasicAlgebra}, Saunders Mac Lane in \cite[25]{MacLane1998} and even Paolo Aluffi himself in \cite[341]{Aluffi2009}. Dimension theory for modules allows characterizing the trivial module as a zero-dimensional module or vector space.
...
\begin{proposition}\label{thm:zero_morphisms_pointed}
  ...
  \begin{thmenum}
    ...
    \thmitem{thm:zero_morphisms_pointed/kernel} The \hyperref[def:zero_morphisms/kernel]{categorical kernel} of a morphism \( f: A \to B \) is the largest \hyperref[def:first_order_submodel]{submodel} of \( A \) contained in the \hyperref[def:set_valued_map/inverse]{preimage}
    \begin{equation*}
      f^{-1}(e_B) = \set{ x \in A \given f(x) = e_B }.
    \end{equation*}

    We denote this submodel via \( \ker f \). The kernel pair is \( (\ker f, \iota) \), where \( \iota: \ker f \to A \) is the canonical embedding.
