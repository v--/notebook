...
\begin{definition}\label{rem:pointed_set}\mcite[26]{MacLane1998}
  ...
  \begin{thmenum}
    \thmitem{rem:pointed_set/theory} Pointed sets can also be viewed as \hyperref[def:first_order_model]{models} of an empty \hyperref[def:first_order_theory]{theory} for a \hyperref[def:first_order_language]{first-order logic language} with a constant symbol, i.e. a nullary \hyperref[def:first_order_language/fun]{functional symbol}.

    \thmitem{rem:pointed_set/homomorphism} A \hyperref[def:first_order_homomorphism]{first-order homomorphism} between the pointed sets \( (X, e_{X}) \) and \( (Y, e_{Y}) \) is, explicitly, a function \( \varphi: X \to Y \) that satisfies
    \begin{equation}\label{eq:rem:pointed_set/homomorphism}
      \varphi(e_{X}) = e_{Y}.
    \end{equation}

    \thmitem{rem:pointed_set/submodel} The set \( S \subseteq X \) is a \hyperref[thm:substructure_is_model]{submodel} of \( X \) if \( e \in S \).
    ...
    \thmitem{rem:pointed_set/trivial} The \hyperref[rem:trivial_structure]{trivial} pointed set is, up to an isomorphism, the set \( \set{ e } \).
    ...
    \thmitem{rem:pointed_set/category} We denote the \hyperref[def:category_of_small_first_order_models]{category of \( \mscrU \)-small models} for this theory by \( \ucat{Set}_* \).
  \end{thmenum}
\end{definition}
...
\begin{definition}\label{def:magma}
  A \term{magma} is a set \( M \) equipped with a \hyperref[rem:function_arguments]{binary function} \( \cdot: M \times M \to M \), called the \term{magma operation}. Unless specified otherwise, we denote this operation by juxtaposition as \( xy \) instead of \( x \cdot y \).
  ...
  \begin{thmenum}[series=def:magma]
    \thmitem{def:magma/theory} In analogy to the \hyperref[rem:pointed_set/theory]{theory of pointed sets}, we can define the theory of magmas as an empty theory over a language with a single \hyperref[rem:first_order_formula_conventions/infix]{infix} binary functional symbol.

    \thmitem{def:magma/homomorphism} A \hyperref[def:first_order_homomorphism]{first-order homomorphism} between the magmas \( (M, \cdot_{M}) \) and \( (N, \cdot_{N}) \) is, explicitly, a function \( \varphi: M \to N \) such that
    ...
    As a consequence of \fullref{thm:positive_formulas_preserved_under_homomorphism}, the image of a magma homomorphism is a submagma of its codomain.
    ...
    \thmitem{def:magma/initial} The \hyperref[thm:substructures_form_complete_lattice/bottom]{initial substructure} of any magma is isomorphic to the trivial magma \( \set{ e } \).
  ...
  \begin{thmenum}[resume=def:magma]
    \thmitem{def:magma/associative} We can add the (\hyperref[def:universal_closure]{universal closure} of) following axiom to the theory:
...
\begin{example}\label{ex:def:magma}
  ...
  \begin{thmenum}
    \thmitem{ex:def:magma/algebraic} \hyperref[eq:def:magma/associative]{Associative} binary operations on a set are abundant and are part of the definition of essential algebraic structures like \hyperref[def:group]{groups}, \hyperref[def:semiring]{semirings}, \hyperref[def:semimodule]{semimodules} and \hyperref[def:semilattice]{(semi)lattices}.

    These operations are \term{homogeneous} in the sense that their signature only contains a single set, unlike \hyperref[def:group_action]{group actions} and scalar products in \hyperref[def:semimodule]{semimodules}.

    \thmitem{ex:def:magma/composition} The quintessential example of a non-\hyperref[def:magma/commutative]{commutative} operation is \hyperref[def:set_valued_map/composition]{composition} in any set of functions or, more generally, \hyperref[def:category/composition]{morphism composition} in any \hyperref[def:category]{category}.
...
\begin{definition}\label{def:ordered_magma}
  ...
  The category of small ordered magmas is a \hyperref[def:concrete_category]{concrete category} over both \hyperref[def:magma/category]{\( \cat{Mag} \)} and \hyperref[def:partially_ordered_set]{\( \cat{Pos} \)}.
\end{definition}
...
\begin{example}\label{ex:def:ordered_magma}
  ...
  \begin{thmenum}
    \thmitem{ex:def:ordered_magma/natural_numbers} The \hyperref[def:natural_numbers]{natural numbers} with addition form an ordered magma as a consequence of \fullref{thm:natural_numbers_are_well_ordered}; and so do \( \BbbZ \), \( \BbbQ \), \( \BbbR \) and \( \BbbC \).
...
\begin{remark}\label{rem:binary_operation_syntax_trees}
  \hyperref[def:magma]{Binary operations} are easily extended to higher arities. Given a binary operation \( +: M \times M \to M \), we can extend it via \hyperref[rem:natural_number_recursion]{natural number recursion} to arbitrary \( n \)-tuples \( x_1, \ldots, x_n \) via parentheses:
  ...
  \begin{thmenum}
    ...
    For an \hyperref[def:magma/associative]{associative} binary operation, abstract syntax trees can instead be represented as finite ordered rooted trees like
    ...
    \thmitem{rem:binary_operation_syntax_trees/commutativity} If, additionally, the operation is \hyperref[def:magma/commutative]{commutative}, we can regard the syntax tree as if the tree was a plain unordered (but still node-labeled) tree.
...
\begin{definition}\label{def:monoid}
  ...
  \begin{thmenum}
    ...
    \end{equation}
    over the combined language of \hyperref[rem:pointed_set/theory]{pointed sets} and \hyperref[def:magma/theory]{magmas}.

    \thmitem{def:monoid/homomorphism} A \hyperref[def:first_order_homomorphism]{first-order homomorphism} between monoids is a function that satisfies both \eqref{eq:rem:pointed_set/homomorphism} and \eqref{eq:def:magma/homomorphism}.
    ...
    \thmitem{def:monoid/trivial} The \hyperref[rem:trivial_structure]{trivial} monoid is the \hyperref[rem:pointed_set/trivial]{trivial pointed set} \( \set{ e } \). It is isomorphic to \hyperref[thm:substructures_form_complete_lattice/bottom]{initial substructure} of any monoid.
    ...
    \thmitem{def:monoid/category} The \hyperref[def:category_of_small_first_order_models]{category of \( \mscrU \)-small models} \( \ucat{Mon} \) of monoids is \hyperref[def:concrete_category]{concrete} with respect to both \hyperref[rem:pointed_set/category]{\( \ucat{Set}_* \)} and \hyperref[def:magma/category]{\( \ucat{Mag} \)}.
...
\begin{remark}\label{rem:internal_direct_sum}
  If \( N_1, \ldots, N_m \) are nontrivial submonoids of \( M \), if \( N_i \cap N_j = \set{ e_M } \) for \( i \neq j \) and if
  \begin{equation*}
    M = \sum_{k=1}^m N_k,
  \end{equation*}
  we say that \( M \) is an \term{internal direct sum}. The sum above is the usual sum of subsets in the \hyperref[def:monoid/power_set]{power set monoid} of \( M \).
  ...
  The internal and external are isomorphic and are usually conflated. In particular, the notation for external direct sum is used for internal direct sums.
\end{remark}
