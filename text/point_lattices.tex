...
\begin{definition}\label{def:point_lattice}\mimprovised
  A \term{point lattice} in an \hyperref[def:euclidean_space]{Euclidean space} is a \hyperref[def:discrete_set]{discrete} \hyperref[def:group/subobject]{subgroup} of its \hyperref[def:semiring]{additive group}.
\end{definition}
\begin{comments}
  ...
  \item This definition is stated as a characterization by \textcite[thm. 21.2(ii)]{Gruber2007ConvexAndDiscreteGeometry}. The aforementioned authors define lattices via explicit bases and proceed to study basis changes. We prefer a basis-free definition.
...
\begin{remark}\label{rem:lattice_terminology}
  ...
  We use the term \enquote{point lattice} to disambiguate. \Textcite[356]{Gruber2007ConvexAndDiscreteGeometry} suggests using \enquote{geometric} for possible disambiguation, while Marcel Berger refers to \enquote{plane lattices}. Both \enquote{geometric lattice} and \enquote{point lattice} are also concepts in lattice theory, and are discussed in \cite[80]{Birkhoff1967LatticeTheory}, however we decided that \enquote{point lattice} is unambiguous enough.
  ...
  \begin{itemize}
    ...
    \textcite[\S I.2]{Cassels1997NumberGeometry},
    \textcite[\S 9.14.29]{Berger1987GeometryI},
    \textcite[28]{GrünbaumShephard1987TilingsAndPatterns},
    \textcite[1]{Rogers1964PackingAndCovering},
    \textcite[356]{Gruber2007ConvexAndDiscreteGeometry},
    \textcite[376]{Винберг2014КурсАлгебры} (as \enquote{\textru{решётка}}) and
    \textcite[155]{Шафаревич2019ОсновныеПонятияАлгебры} (as \enquote{\textru{решётка}}).

    \item Explicit definitions for order-theoretic lattices are given by
    \textcite[6]{Birkhoff1967LatticeTheory},
    \textcite[9]{Grätzer2011LatticeTheory},
    \textcite[def. 1.6.14]{Harzheim2005OrderedSets},
    \textcite[def. 2.4]{DaveyPriestley2002LatticeTheory},
    \textcite[40]{HalmosGivant2009BooleanAlgebras},
    \textcite[3]{Johnstone1982StoneSpaces},
    \textcite[80]{Kelley1975GeneralTopology},
    \textcite[145]{Bourbaki2004TheoryOfSets},
    \textcite[174]{Riehl2016CategoryTheory},
    \textcite[exerc. VI.2.1]{MacLane1998CategoryTheory},
    \textcite[248]{Stanley2012EnumerativeCombinatoricsVol1},
    \textcite[192]{Carothers2000RealAnalysis},
    \textcite[3]{HillePhillips1996FunctionalAnalysis},
    \textcite[2]{Yoshida1980FunctionalAnalysis},
    \textcite[210]{Rotman2015AdvancedModernAlgebraPart1},
    \textcite[def. 8.2]{Jacobson1985BasicAlgebraI},
    \textcite[227]{Cohn2013MeasureTheory} (only for lattices of vectors),
    \textcite[def. 6.5.1]{Bobrowski2005FunctionalAnalysis} (only for lattices of continuous functions),
    \textcite[174]{GelbaumOlmsted2003CounterexamplesInAnalysis} (also only for lattices of continuous functions),
    \textcite[46]{CitkinMuravitsky2022ConsequenceRelations} (only for distributive lattices),
    \textcite[10]{Barendregt1984LambdaCalculus} (only for complete lattices).

    In Russian, \enquote{решётка} (\enquote{lattice}) is mostly used, for example by
    \textcite[127]{Гуров2013ТеорияРешёток},
    \textcite[129]{Мальцев1970АлгебраическиеСистемы},
    \textcite[\S 2.1.2]{КусраевКутателадзе2005БулевозначныйАнализ},
    \textcite[212]{БелоусовТкачёв2004ДискретнаяМатематика} and
    \textcite[360]{КанторовичАкилов1984ФункциональныйАнализ},
    however \enquote{структура} (\enquote{structure}) is sometimes used in older literature, for example by
    \textcite[16]{Владимиров1969БулевыАлгебры}
    \textcite[58]{Ляпин1960Полугруппы} and
    \textcite[178]{Курош1973ОбщаяАлгебра} (who doesn't even mention \enquote{решётка}).
  \end{itemize}
\end{remark}
...
\begin{comments}
  ...
  \item Primitive vectors as standalone objects are discussed in \cite[24]{Cassels1997NumberGeometry} and \textcite[358]{Gruber2007ConvexAndDiscreteGeometry}, however we wanted to highlight the construction of primitive vectors from existing ones.
\end{comments}
...
\begin{proof}
  ...
  The intersection of subgroups is a subgroup, so \( L_m \) is indeed a discrete subgroup of \( L \). Furthermore, since \( L \) spans \( \BbbR^n \), \( L_m \) spans \( V_m \). Thus, the problem is well-posed, and we can begin.
...
\end{definition}
\begin{comments}
  \item \Textcite[5]{ConwaySloane1998SpherePackings} instead define the determinant by squaring instead of taking the absolute value.
\end{comments}
\begin{defproof}
...
\end{definition}
\begin{comments}
  \item \Textcite[69]{Cassels1997NumberGeometry} uses the term \enquote{fundamental parallelepiped} for arbitrary dimensions.
\end{comments}
...
\begin{comments}
  ...
  \item Grids whose origin is the zero vector are simply point lattices, for which reason Cassels also suggests calling grids \enquote{inhomogeneous lattices}. The latter term is also used by \textcite[396]{Gruber2007ConvexAndDiscreteGeometry}.
\end{comments}

\paragraph{Hypercube lattices}

\begin{definition}\label{def:hypercubic_point_lattice}
  We say that a \hyperref[thm:generated_submodule]{spanning} \hyperref[def:point_lattice]{point lattice} in the \hyperref[def:euclidean_plane]{Euclidean space} \( \BbbR^d \) is \term{hypercubic} of dimension \( d \) if it has a \hyperref[def:point_lattice_basis]{basis} of \( d \) pairwise \hyperref[def:orthogonality]{orthogonal} \hyperref[def:minimal_lattice_vector]{minimal vectors}.
...
\begin{comments}
  ...
  \begin{itemize}
    \item \Textcite[296]{Aluffi2009Algebra} in the context of \hyperref[def:polytope]{polytopes}.
    \item \Textcite[92]{ГашковЧубариков2005СложностьВычисления} (as \enquote{\textru{целочисленная решётка}}) in the context of number geometry (which applies Euclidean geometry to number theory).
    \item \Textcite[256]{БолтянскийЯглом1966Равносоставленость} (as \enquote{\textru{целочисленная решётка}}) in the context of \hyperref[def:euclidean_plane]{plane geometry}.
    \item \Textcite[348]{Зыков2004ТеорияГрафов} (as \enquote{\textru{целочисленная решётка}}) in the context of \hyperref[def:graph_geometric_realization/embedding]{graph embeddings}.
  \end{itemize}

  Authors who discuss general point lattices and also explicitly use the term \enquote{integer lattice} include
  \begin{itemize}
    \item \Textcite[106]{ConwaySloane1998SpherePackings} in the context of \hyperref[def:topological_space_packing]{packing}.
    \item \Textcite[322]{Cassels1997NumberGeometry} in the context of number geometry.
    \item \Textcite[310]{Gruber2007ConvexAndDiscreteGeometry} in the context of \hyperref[def:polytope]{polytopes}.
    \item \Textcite[fig. 3.8.5]{GrünbaumShephard1987TilingsAndPatterns} in the context of \hyperref[def:topological_space_tiling]{tilings}.
  \end{itemize}
\end{comments}
...
\end{definition}
\begin{comments}
  \item For this definition we generalize \enquote{the} hexagonal lattice discussed by \textcite[\S 6.2]{ConwaySloane1998SpherePackings}.
...
\begin{example}\label{ex:primitive_lattice_vectors_not_basis}
  ...
  Thus, it is possible for primitive linearly independent vectors to not form a basis. Even minimal vectors may fail to form a basis. \Textcite{ConwaySloane1995MinimalVectorBases} give an example of a particular lattice in \( \BbbR^{11} \) where no \( 11 \) \hyperref[def:minimal_lattice_vector]{minimal vectors} form a basis.
...
\begin{proof}
  ...
  \begin{equation*}
    \measuredangle(u, v - u) \equiv \measuredangle(u, v) + \measuredangle(v, v - u) \pmod {2\pi}.
  \end{equation*}
  ...
  \begin{equation}\label{eq:thm:v_minus_u_as_rotation/proof/cos_v_and_minus_u/equation}
    \underbrace{\frac 3 \pi \cdot \measuredangle(u, v - u)}_a - \underbrace{\frac 3 \pi \cdot \measuredangle(v, v - u)}_b \equiv 1 \pmod 6,
  \end{equation}
